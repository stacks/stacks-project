...
adequate functor $G$ the map $\text{Hom}(G, Q(F)) \to \text{Hom}(G, F)$
is a bijection.
\end{lemma}
...
(2)' or every $i \in I$ the map $\text{Hom}(L_i, Q(F)) \to \text{Hom}(L_i, F)$
...
implies that
$\text{Hom}(L, Q(F)) \to \text{Hom}(L, F)$
and
$\text{Hom}(K, Q(F)) \to \text{Hom}(K, F)$
...
Then
\begin{enumerate}
\item $i$ is fully faithful, exact, and its image is a Serre subcategory,
...
Part (1) is clear from the definitions and
Lemma \ref{lemma-extension-adequate}.
Recall that $\mathcal{P}$ is equivalent to the category
$\textit{PMod}((\textit{Aff}/\text{Spec}(A))_\tau, \mathcal{O})$.
Hence (2) by
...
For every $B \in \text{Ob}(\textit{Alg}_A)$ and $B$-module $N$
...
of module-valued functors. Then for any $B \in \text{Ob}(\textit{Alg}_A)$
...
$h(C) = \text{Mor}_A(B[N], C)$. Similarly for $h_1$ and $h_2$.
...
$$
\text{Hom}_\mathcal{P}(\mathcal{O}_h, F) = F(B[N])
$$
...
$$
in $\mathcal{P}$. Namely, suppose that $C \in \text{Ob}(\textit{Alg}_A)$
...
Let $A$ be a ring. Let $F$ be a module-valued functor
such that for any $B \in \text{Ob}(\textit{Alg}_A)$ the
...
say $(*)$ holds if for all $B \in \text{Ob}(\textit{Alg}_A)$ the
...
comes from a morphsm $\varphi : \underline{P} \to I^i$ with
...
Let $S = \text{Spec}(A)$ be an affine scheme.
The category $\textit{Adeq}(\mathcal{O}_S)$ is equivalent to the
category of adequate module-valued functors on $\textit{Alg}_A$.
\end{lemma}

\begin{proof}
The correspondence is given by $\mathcal{F} \mapsto F_{\mathcal{F}, A}$.
Given an adequate functor $F$ we choose an exact sequence
$0 \to F \to \underline{M} \to \underline{N}$ and we associate
to $F$ the adequate module $\text{Ker}(M^a \to N^a)$ where
$M^a$ denotes the quasi-coherent $\mathcal{O}$-module on
$(\textit{Sch}/S)_\tau$ associated to the quasi-coherent sheaf
$\widetilde{M}$ on $S$. We omit the proof that the constructions
...
$D^+(\textit{Adeq}(\mathcal{O}_U))$ is the localization
...
given $\mathcal{F}^\bullet \in \text{Ob}(K^+(\textit{Adeq}(\mathcal{O}_U)))$
there exists a quasi-isomorphism
$\mathcal{F}^\bullet \to u\mathcal{G}^\bullet$
where $\mathcal{G}^\bullet \in \text{Ob}(K^+(\textit{QCoh}(\mathcal{O}_U)))$.
...
\vcenter{
\xymatrix{
\textit{Adeq}(\mathcal{O}_U) \ar[r]_-k \ar@{=}[d] &
...
\label{lemma-RA-zero}
Let $U = \text{Spec}(A)$ be an affine scheme.
For any object $\mathcal{F}$ of $\textit{Adeq}(\mathcal{O}_U)$
...
D(\mathcal{O}) = D(\textit{Mod}((\textit{Sch}/S)_\tau, \mathcal{O}))
$$
...
If $U = \text{Spec}(A)$ is an affine scheme, then the bounded
below version
\begin{equation}
...
\begin{proof}
Let $A : \textit{Mod}(\mathcal{O}) \to \textit{Adeq}(\mathcal{O}_U)$
...
$RA : D^+_{\textit{Adeq}}(\mathcal{O}) \to D^+(\textit{Adeq}(\mathcal{O}_U))$.
...
\begin{enumerate}
\item Given $\mathcal{F}^\bullet \in K^+(\textit{Adeq}(\mathcal{O}_U))$
...
$$
\text{Ext}^i_{\textit{Adeq}(\mathcal{O}_S)}(\mathcal{F}, \mathcal{G})
\longrightarrow
...
$0 \to \text{Hom}_A(P, K) \to \text{Hom}_A(P, M) \to \text{Hom}_A(P, N) \to 0$
is exact.
\item An $A$-module $I$ is said to be {\it pure injective}
if for every universally exact sequence
$0 \to K \to M \to N \to 0$ of $A$-module the sequence
$0 \to \text{Hom}_A(N, I) \to \text{Hom}_A(M, I) \to \text{Hom}_A(K, I) \to 0$
is exact.
\end{enumerate}
\end{definition}
...
Write $M = \text{colim}_{i \in I} P_i$ as a filtered colimit of
...
$\text{Hom}_A(P, \bigoplus P_i) \to \text{Hom}_A(P, M)$
...
$M^\wedge = \text{Hom}_\mathbf{Z}(M, \mathbf{Q}/\mathbf{Z})$.
\begin{enumerate}
...
$\text{Hom}_A(N, M^\wedge) =
\text{Hom}_\mathbf{Z}(N \otimes_A M, \mathbf{Q}/\mathbf{Z})$
...
gives
$$
\text{Hom}_A(N, ((M^\wedge)^\wedge)^\wedge) =
...
$\text{Hom}_A(M, I^\bullet)$ where $I^\bullet$ is a pure injective
resolution of $N$.
\end{definition}
...
of the complex $\text{Hom}_A(P_\bullet, N)$ where $P_\bullet$
...
$$
A^{\bullet, \bullet} = \text{Hom}_A(P_\bullet, I^\bullet)
$$
Each of its rows is exact except in degree $0$ where its cohomology
is $\text{Hom}_A(M, I^q)$. Each of its columns is exact except in degree $0$
where its cohomology is $\text{Hom}_A(P_p, N)$. Hence the two spectral
...
$\text{Ext}^i_{\textit{Adeq}(\mathcal{O}_U)}(M^a, N^a)$, see
...
equivalent to $\textit{Adeq}(\mathcal{O}_U)$, see
...
and $\mathcal{O}$ the structure sheaf of the big site $(\textit{Sch}/S)_\tau$.
In
...
$v : \textit{Adeq}(\mathcal{O}_S) \to \textit{QCoh}(\mathcal{O}_S)$, and
in fact that $f_* = v$ induces an equivalence
$\textit{Adeq}(\mathcal{O}_S)/\mathcal{C} \to \textit{QCoh}(\mathcal{O}_S)$
...
$D_{\textit{Adeq}}(\mathcal{O}))$ and $D_{\textit{QCoh}}(\mathcal{O}_S)$.
...
Let $S$ be a scheme.
Let $\tau, \tau' \in \{Zar, \acute{e}tale, smooth, syntomic, fppf\}$.
Denote $\mathcal{O}_\tau$, resp.\ $\mathcal{O}_{\tau'}$
the structure sheaf $\mathcal{O}$ viewed as a sheaf on
$(\textit{Sch}/S)_\tau$, resp.\ $(\textit{Sch}/S)_{\tau'}$.
Then $D_{\textit{Adeq}}(\mathcal{O}_\tau)$ and
$D_{\textit{Adeq}}(\mathcal{O}_\tau)$ are canonically isomorphic.

\medskip\noindent
We sketch the proof. Assume
that the toplogy $\tau$ is stronger than the topology $\tau'$. Consider the
morphism of ringed sites
$\epsilon : (\textit{Sch}/S)_\tau \to (\textit{Sch}/S)_{\tau'}$, see
Topologies, Section \ref{topologies-section-change-topologies}
and
Descent, Remark \ref{descent-remark-change-topologies-ringed}.
We will use the existence of the unbounded total derived functors
$L\epsilon^*$ and $R\epsilon_*$ on $\mathcal{O}$-modules
and their adjointness, see \cite{KS}.
Given an $\mathcal{O}_{\tau'}$-module $\mathcal{F}$ we see that
$\epsilon^*\mathcal{F}$ is just the $\tau$-sheafification of $\mathcal{F}$.
Hence $\epsilon^*$ is exact and $L\epsilon^* = \epsilon^*$.
Lemma \ref{lemma-adequate-fpqc}
shows that $\epsilon^*\mathcal{F} = \mathcal{F}$
for adequate modules. Thus we obtain
$$
\epsilon^* :
D_{\textit{Adeq}}(\mathcal{O}_{\tau'})
\to
D_{\textit{Adeq}}(\mathcal{O}_\tau)
$$
We claim that $R\epsilon_*$ is a quasi-inverse. Namely, let
$\mathcal{G}^\bullet$ be an object of $D_{\textit{Adeq}}(\mathcal{O}_\tau)$.
Choose a
quasi-isomorphism $\mathcal{G}^\bullet \to \mathcal{I}^\bullet$ as in
Cohomology on Sites, Section \ref{sites-cohomology-section-K-injective}.
In particular $\mathcal{I}^\bullet = \lim \mathcal{I}_n^\bullet$,
...
are termwise split surjections, and each
$\mathcal{I}_n^\bullet$ is quasi-isomorphic to
$\tau_{\geq -n}\mathcal{G}^\bullet$. Then
$$
R\epsilon_*\mathcal{G}^\bullet = \epsilon_*\mathcal{I}^\bullet =
\lim \epsilon_*\mathcal{I}_n^\bullet
$$
Observe that $\epsilon_*$ preserves injectives, see
Homology, Lemma \ref{homology-lemma-adjoint-preserve-injectives}.
The cohomology sheaves of the complex $\epsilon_*\mathcal{I}_n^\bullet$
are $\epsilon_*H^i(\mathcal{G}^\bullet)$ by the vanishing of cohomology of
Lemma \ref{lemma-same-cohomology-adequate}.
Hence we may apply the reasoning in the proof of
Cohomology on Sites, Lemma \ref{sites-cohomology-section-K-injective}
to see that the cohomology sheaves of
$R\epsilon_*\mathcal{G} = \epsilon_*\mathcal{I}^\bullet$
are equal to $\epsilon_*$ applied to the cohomology sheaves of
$\mathcal{G}^\bullet$ hence adequate. Hence we obtain a functor
$$
R\epsilon_* :
D_{\textit{Adeq}}(\mathcal{O}_\tau)
\to
D_{\textit{Adeq}}(\mathcal{O}_{\tau'})
$$
and the remarks above also show that $\epsilon^*$ and $R\epsilon_*$ are
...
adjoint functors
$Rf_* : D_{\textit{Adeq}}(\mathcal{O}) \to D_{\textit{QCoh}}(\mathcal{O}_S)$
and
$Lf^* : D_{\textit{QCoh}}(\mathcal{O}) \to D_{\textit{Adeq}}(\mathcal{O}_S)$.
Moreover $Rf_* Lf^* \cong \text{id}_{D_{\textit{QCoh}}(\mathcal{O})}$.
...
adjointness, see \cite{KS}. 
In this case $f_*$ is just the restriction to the subcategory
$S_{Zar}$ of $(\textit{Sch}/S)_{Zar}$. Hence it is clear that
$Rf_* = f_*$ induces
$Rf_* : D_{\textit{Adeq}}(\mathcal{O}) \to D_{\textit{QCoh}}(\mathcal{O}_S)$.
Suppose that $\mathcal{G}^\bullet$ is an object of
$D_{\textit{QCoh}}(\mathcal{O})$. We may choose a system
...
$\text{colim}\ f^*\mathcal{K}_n^\bullet$, see
...
\text{colim}\ H^i(f^*\mathcal{K}_n^\bullet)$ is adequate for each $i$. By
Lemma \ref{lemma-abelian-adequate}
...
we may assume that $S = \text{Spec}(A)$ is affine. Because $S$ is affine
$D_{\textit{QCoh}}(S) = D(\textit{QCoh}(\mathcal{O}_S))$, see
the discussion in
Coherent, Section \ref{coherent-section-derived-quasi-coherent}.
...
\medskip\noindent
The final assertion
$Rf_* Lf^* \cong \text{id}_{D_{\textit{QCoh}}(\mathcal{O})}$
...
to a quasi-isomorphism in $D_{\textit{QCoh}}(\mathcal{O}_S)$, hence
...
$$
Rf_* :
\text{Hom}_{D_{\textit{Adeq}}(\mathcal{O})/D_\mathcal{C}(\mathcal{O})}(X, Y)
\to
\text{Hom}_{D_{\textit{QCoh}}(\mathcal{O}_S)}(Rf_*X, Rf_*Y)
$$
