...
\item A (pre)sheaf of $\mathcal{O}$-modules $\mathcal{F}$
...
$\mathcal{O}$-modules on $(\Sch/S)_\tau$. If $\mathcal{G}$
is parasitic for the $\tau$-topology, then $H^p_\tau(U, \mathcal{G}) = 0$
for every $U$ open in $S$, resp.\ \'etale over $S$, resp.\ smooth over $S$,
resp.\ syntomic over $S$, resp.\ flat and locally of finite presentation
over $S$.
\end{lemma}
