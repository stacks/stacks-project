...
Lemma \ref{lemma-kill-colimit-weak} by Lemma \ref{lemma-cd-bound-dim-local}
...
\begin{enumerate}
\item $A$ is $I$-adically complete and has a dualizing complex,
\item if $x \in \text{Ass}(\mathcal{F})$ and
...
Lemma \ref{lemma-finiteness-pushforwards-and-H1-local}.
Set $d = 1$. Let $\mathfrak p$ be a prime of $A$ not contained in $V(I)$
...
or we have $\dim((A/\mathfrak p)_\mathfrak q) > d + 1$.
...
$$
is an isomorphism. Setting $V = \Spec(A) \setminus V(J)$ we obtain
an exact sequence
$$
...
$$
In particular we see that $Y \setminus Z$ is connected.
\end{example}
