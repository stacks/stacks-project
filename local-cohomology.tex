...
local cohomology of a $A_\mathfrak p$-module with respect to
...
Assumption (3) says that property (2) of
...
If $V(J) \subset V(f)$ with $f \in A$, $f \not \in \mathfrak p$,
then some power of $f$ is the looked for element of $A$.
\end{proof}
...
$$
where the colimit is over the closed subsets $Z = V(J')$
with $J' \subset J$ and $V(J') \cap V(I) \subset V(J) \cap V(I)$.
...
\end{equation}
This subset is stable under specialization.
The result above becomes the statement that
$H^0_T(M) \longrightarrow H^0(R\Gamma_J(M)^\wedge)$
is an isomorphism, see Section \ref{section-supports}.
...
In Lemma \ref{lemma-kill-colimit-weak-general} if in stead of the empty
...
Theorem \ref{theorem-finiteness} (insert better reference here).
...
for all $\mathfrak q \in V(\mathfrak p) \cap V(J) \cap V(I)$,
\item for $\mathfrak p \not \in V(I)$, $\mathfrak p \not \in T$
with $\text{depth}(M_\mathfrak p) < s$
one of the following holds:
\begin{enumerate}
\item we have $\dim(\text{Supp}(M_\mathfrak p)) < s + 2$\footnote{For example
if $M$ is $(S_s)$ on the complement of $V(I) \cup T$.}, or
\item we have
$\delta(\mathfrak p) > d + \delta_{max} - 1$
where $\delta$ is a dimension function and $\delta_{max}$
is the maximum value of $\delta$ on $V(J) \cap V(I)$, or
\item we have
$$
\text{depth}_{A_\mathfrak p}(M_\mathfrak p) +
...
that $V(\mathfrak p) \cap V(J) = \emptyset$. Thus setting
$N = H^0_\mathfrak p(M)$ we see that
$R\Gamma_J(N) = R\Gamma_{J'}(N) = 0$ for all
$J' \subset J$ with $V(J') \cap V(I) = V(J) \cap V(I)$.
Thus we may replace $M$ by $M/N$.
Repeating we may assume that
...
Proof of claim II. Choose $\mathfrak p \in V(J)$ not in $V(J_0)$.
It suffices to show that
$H^i_{\mathfrak pA_\mathfrak p}(M_\mathfrak p) = 0$ for $i \leq s$, see
Lemma \ref{lemma-isomorphism}. Observe that
$V(\mathfrak p) \cap V(I) = \{\mathfrak m\}$.
Since $\mathfrak p$ is not in $B$ we see that
$\text{depth}(M_\mathfrak p) > s$ and hence the groups
vanish by Dualizing Complexes, Lemma \ref{dualizing-lemma-depth}.
...
and an integer $t > 0$ such that $(J_0I)^t$ annihilates $H^s_J(M)$.
The map $\colim H^0_J(M) \to H^0(R\Gamma_\mathfrak m(M)^\wedge)$
is an isomorphism by
Dualizing Complexes, Lemma \ref{dualizing-lemma-completion-local-H0}.
In particular, we may assume $s > 0$. Hence the
assumptions of Lemma \ref{lemma-kill-completion}
are satisfied for every $\mathfrak p \in \text{Ass}(M)$
(see previous paragraph).
Thus the annihilator $\mathfrak a \subset A$ of
