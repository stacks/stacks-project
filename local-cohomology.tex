...
\section{Algebraization of coherent formal modules; generalities}
\label{section-algebraization-modules-general}

\noindent
We develop a little bit of theory that will be useful later.
...
\noindent
First we prove a stronger version of
...
$$
\end{enumerate}
Here for $x \in X$ we set $\delta(x) = \dim \overline{\{x\}}$.
...
(3) and (4) guarantee that Lemma \ref{lemma-algebraize-local-cohomology-bis}
applies to
...
In Situation \ref{situation-algebraize} let $(\mathcal{F}_n)$ be an object
of $\textit{Coh}(U, I\mathcal{O}_U)$. For $x \in X$ set
$\delta(x) = \dim(\overline{\{x\}}$. Assume
\begin{enumerate}
\item $A$ is local and $\mathfrak a = \mathfrak m$ is the maximal ideal,
\item $A$ has a dualizing complex and is complete with respect to $I$,
...
with $V(\mathfrak p) \cap V(f) \not = \{\mathfrak m_y^\wedge\}$ we have
$$
\text{depth}((\mathcal{F}^\wedge_y)_\mathfrak p) + \delta(y) \geq 2
\quad\text{or}\quad
\text{depth}((\mathcal{F}^\wedge_y)_\mathfrak p) +
\dim(\mathcal{O}_{X, y}^\wedge/\mathfrak p) + \delta(y) > 3
$$
\item for $y \in U \cap Y$ given a nonmaximal prime
$\mathfrak p \subset \mathcal{O}_{X, y}^\wedge$ with
$V(\mathfrak p) \cap V(f) = \{\mathfrak m_y^\wedge\}$
we have
$$
\text{depth}((\mathcal{F}^\wedge_y)_\mathfrak p) + \delta(y) > 2
$$
\end{enumerate}
Then there exists a coherent $\mathcal{O}_U$-module
$\mathcal{F}$ such that $(\mathcal{F}_n)$ is the completion of $\mathcal{F}$.
...
Choose $N$ large and consider the map
$f^N : (\mathcal{F}_n) \to (\mathcal{F}_n)$.
By Lemma \ref{lemma-map-kernel-cokernel-on-closed}
we may replace $(\mathcal{F}_n)$ by the image of this map.
Thus we may assume $f$ is injective on $(\mathcal{F}_n)$
since over affine opens of $U$ the object $(\mathcal{F}_n)$
corresponds to a finite module over a Noetherian ring
(Cohomology of Schemes, Lemma \ref{coherent-lemma-inverse-systems-affine}).
After this replacement the equivalent conditions of
Lemma \ref{lemma-equivalent-f-good} hold $(\mathcal{F}_n)$ on $U$
and we see that hypothesis (c) holds by Lemma \ref{lemma-topology-I-adic-f}.
...
By Lemma \ref{lemma-helper-algebraize} and assumptions (4) and (5)
of the lemma we can choose finite $A$-modules
$N_1$ and $N_2$ annihilated by $f^c$ for some $c \geq 0$
such that for $n \gg 0$ there are maps
$$
H^i_\mathfrak m(M_n) \to \Hom_A(N_i, E), \quad i = 1, 2
$$
with the properties stated in the lemma (in particular the kernels
of these maps are finite length $A$-modules).
...
$$
The module $\Hom_A(N_2, E)$ has the descending chain condition, see 
Dualizing Complexes, Proposition \ref{dualizing-proposition-matlis}
and its addendum Remark \ref{dualizing-remark-matlis}.
Thus we find that the images of the maps
$H^2_\mathfrak m(M_n) \to \Hom_A(N_2, E)$
stabilize to some $Q \subset \Hom_A(N_2, E)$. For later use, we choose
$m \geq 0$ such that $Q$ is equal to the image of
$H^2_\mathfrak m(M_m) \to \Hom_A(N_2, E)$. On the other hand, since
the kernels $K_n$ of $H^2_\mathfrak m(M_n) \to \Hom_A(N_2, E)$
have finite length, the system $\{K_n\}$ satisfies ML.
Therefore we obtain a short exact sequence
$$
0 \to \lim K_n \to Ob \to Q \to 0
$$
By the above the limit topology on $\lim H^2_\mathfrak m(M_n)$
is the $f$-adic topology. Since $Q$ is annihilated by $f^c$
we conclude the same is true for the limit topology on $K = \lim K_n$.
Thus $K/fK$ is a subquotient of $K_n$ for some $n \gg 1$.
It follows that $K$ is a finite $A$-module, see
Algebra, Lemma \ref{algebra-lemma-finite-over-complete-ring}.

\medskip\noindent
Choose $n$ very large. Consider the boundary map
$$
\delta_n :
H^0(U, \mathcal{F}_n)
\longrightarrow
...
If $p \in P_n$, then $\delta_n(p)$ maps to zero in $Q$
because $\delta_n(p)$ maps to zero in
$H^1(U, f^n\mathcal{F}_{n + m}) = H^2_\mathfrak m(M_m)$
(this is where the choice of $m$ comes in).
Hence
$$
...
If we ever need this exact result
we will carefully state and prove a precise lemma here.
\end{remark}
