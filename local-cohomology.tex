...
of the lemma we can choose, an integer $m \geq 0$, finite $A$-modules
...
We can unwind conditions (4) and (5) of
...
\begin{enumerate}
\item If $\delta(y) \geq 3$, then (4) and (5) are void.
\item If $\delta(y) = 2$, then (4) is void and (5) is
$\mathfrak p \in \text{Ass}(\mathcal{F}_y^\wedge[1/f])
\Rightarrow
\dim(\mathcal{O}_{X, y}^\wedge/\mathfrak p) \geq 2$.
\item If $\delta(y) = 1$, then (4) is
$\mathfrak p \in \text{Ass}(\mathcal{F}_y^\wedge[1/f])
\Rightarrow
\dim(\mathcal{O}_{X, y}^\wedge/\mathfrak p) \geq 3$
and (5) is
$\mathfrak p \subset \mathcal{O}_{X, y}^\wedge,
f \not \in \mathfrak p, \dim(\mathcal{O}_{X, y}^\wedge/\mathfrak p) = 1
...
at closed points of the scheme $U \cap Y$, when verifying conditions
(4) and (5) of Lemma \ref{lemma-algebraization-principal-bis}.
\end{remark}
...
\section{Algebraization of coherent formal modules; general}
...
$\text{cd}(A, I) = 1$. Let $(\mathcal{F}_n)$
be an object of $\textit{Coh}(U, I\mathcal{O}_U)$. For $y \in Y$
assume
\begin{enumerate}
\item[(a)] if $\delta_Z(y) = 2$ given a nonmaximal prime
$\mathfrak p \subset \mathcal{O}_{X, y}^\wedge$ with
