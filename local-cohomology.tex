...
\section{Formal sections; principal ideal}
...
\section{Formal sections; cohomological dimension one}
...
Lemma \ref{proposition-application-H0} applies and we find an open
...
$$
is essentially surjective. This functor is fully faithful on a
suitable subcategory, see Lemma \ref{lemma-fully-faithful}.

\medskip\noindent
Let $(\mathcal{F}_n)$ be an object of $\textit{Coh}(U, I\mathcal{O}_U)$.
When is $(\mathcal{F}_n)$ in the essenital image of the completion
...
In Situation \ref{situation-algebraize} we can consider the completion functor
$$
\textit{Coh}(\mathcal{O}_U)
\longrightarrow
\textit{Coh}(U, I\mathcal{O}_U),\quad
...
However, we can refine this a little bit.
Namely, suppose that $V \subset U$ is an open subscheme containing
$U \cap Y$. Then we have
$$
...
supported on $U \cap Y$. This we can consider the functor
\begin{equation}
\label{equation-completion}
\colim \textit{Coh}(\mathcal{O}_V)
\longrightarrow
\textit{Coh}(U, I\mathcal{O}_U),\quad
\mathcal{F} \longmapsto \mathcal{F}^\wedge
\end{equation}
Here the colimit on the right is over the open subschemes
$V \subset U$ with $V \cap Y = U \cap Y$.

\medskip\noindent
This refinement does not change the essential image as the
following lemma demonstrates.

\begin{lemma}
\label{lemma-essential-image-completion}
The essential image of the functor (\ref{equation-completion})
is the same as the essential image of
$\textit{Coh}(\mathcal{O}_U) \to \textit{Coh}(U, I\mathcal{O}_U)$
and the same as the essential image of the composition
$\textit{Coh}(\mathcal{O}_X) \to \textit{Coh}(\mathcal{O}_U)
\to \textit{Coh}(U, I\mathcal{O}_U)$.
\end{lemma}

\begin{proof}
This is true because the functors
...
In Situation \ref{situation-algebraize}
any object of $\textit{Coh}(U, I\mathcal{O}_U)$
which is annihilated
by a power of $I$ is in the essential image of (\ref{equation-completion}).
Moreover, if $\mathcal{F}$, $\mathcal{G}$ are in $\textit{Coh}(\mathcal{O}_U)$
and either $\mathcal{F}$ or $\mathcal{G}$ is annihilated by a power of $I$,
then the maps
$$
\xymatrix{
\Hom_U(\mathcal{F}, \mathcal{G}) \ar[d] &
\Ext_U(\mathcal{F}, \mathcal{G}) \ar[d] \\
\Hom_{\textit{Coh}(U, I\mathcal{O}_U)}(\mathcal{F}^\wedge, \mathcal{G}^\wedge) &
\Ext_{\textit{Coh}(U, I\mathcal{O}_U)}(\mathcal{F}^\wedge, \mathcal{G}^\wedge)
}
$$
are isomorphisms.
\end{lemma}
...
\begin{proof}
Suppose $(\mathcal{F}_n)$ is an object of $\textit{Coh}(U, I\mathcal{O}_U)$
which is annihilated by $I^c$ for some $c \geq 1$. Then
...
\medskip\noindent
Let $\mathcal{F}$, $\mathcal{G}$ be objects of $\textit{Coh}(\mathcal{O}_U)$
such that either $\mathcal{F}$ or $\mathcal{G}$ is annihilated by
$I^c$ for some $c \geq 1$. Then
$\mathcal{H} = \SheafHom_{\mathcal{O}_U}(\mathcal{G}, \mathcal{F})$
is a coherent $\mathcal{O}_U$-module annihilated by $I^c$.
Hence we see that
$$
...
$$
is a short exact sequence defining an element of
$\Ext_U(\mathcal{F}, \mathcal{G})$. If this maps to a split
short exact sequence after completion, then we either have
splittings $\pi' : \mathcal{E}^\wedge \to \mathcal{G}^\wedge$ and
$s' : \mathcal{F}^\wedge \to \mathcal{E}^\wedge$.
If $\mathcal{G}$ is annihilated by a power of $I$, then by
the statement on $\Hom$ we see that
$\pi' = \pi^\wedge$ for a splitting
$\pi : \mathcal{E} \to \mathcal{G}$.
Similarly, if $\mathcal{F}$ is annihilated by a power of $I$, then
$s' = s^\wedge$ for a splitting
$s : \mathcal{F} \to \mathcal{E}$.
In this way we see that the map of $\Ext$'s is injective.
To prove surjectivity we can use
Cohomology of Schemes, Lemma \ref{coherent-lemma-existence-easy}.
Namely, for example in the case that $\mathcal{F}$ is annihilated
by a power of $I$ part (1) lemma shows that given an extension
$$
...
In Situation \ref{situation-algebraize}. Let
$(\mathcal{F}_n) \to (\mathcal{G}_n)$ be a morphism of
$\textit{Coh}(U, I\mathcal{O}_U)$
whose kernel and cokernel are annihilated by a power of $I$.
Then $(\mathcal{F}_n)$ is in the essential image of
the completion functor
$\textit{Coh}(\mathcal{O}_U) \to
\textit{Coh}(U, I\mathcal{O}_U)$
if and only if $(\mathcal{G}_n)$ is.
\end{lemma}
...
an immediate specialization. Observe that the sets
$\{y \in Y \mid \delta_Z(y) \leq k\}$ are stable under specialization.
Moreover, for $y \in Y$ and $z \in \overline{\{y\}} \cap Z$ we have
$$
...
is the maximum value of $\delta$ on $Z$. In particular, if $A$
is a local ring with maximal $\mathfrak m = \mathfrak a$, i.e.,
$Z = \{\mathfrak m\}$, then $\delta_Z(y) = \dim(\overline{\{y\}})$. 

\medskip\noindent
Given a point $y \in U \cap Y$ we can consider the $I$-adic completion
$$
...
In Situation \ref{situation-algebraize} let $(\mathcal{F}_n)$ be an object
of $\textit{Coh}(U, I\mathcal{O}_U)$. Let $s, d$ be integers. We say
{\it $(\mathcal{F}_n)$ satisfies the $(s, d)$-inequalities} if
we have
\begin{enumerate}
\item $\text{cd}(A, I) \leq d$, 
...
$$
\text{depth}((\mathcal{F}^\wedge_y)_\mathfrak p) + \delta_Z(y) \geq s
\quad\text{or}\quad
\text{depth}((\mathcal{F}^\wedge_y)_\mathfrak p) +
\dim(\mathcal{O}_{X, y}^\wedge/\mathfrak p) + \delta_Z(y) > s + d
$$
\item if $V(\mathfrak p) \cap V(I\mathcal{O}_{X, y}^\wedge) =
\{\mathfrak m_y^\wedge\}$, then
$$
\text{depth}((\mathcal{F}^\wedge_y)_\mathfrak p) + \delta_Z(y) > s
$$
\end{enumerate}
\end{enumerate}
We say {\it $(\mathcal{F}_n)$ satisfies the strict $(s, d)$-inequalities}
if we have
\begin{enumerate}
\item $\text{cd}(A, I) \leq d$, 
\item for $y \in U \cap Y$ and a prime
...
\begin{enumerate}
\item If $\text{cd}(A, I) \leq d$ and if $(\mathcal{F}_n)$ is annihilated
by a power of $I$, then $(\mathcal{F}_n)$ satisfies the $(s, d)$-inequalities
for any $s$.
\item If $\text{cd}(A, I) \leq d$ and $A$ has a dualizing complex, then
$(\mathcal{F}_n)$ satisfies the $(s, d)$-inequalities
if and only if for all $y \in U \cap Y$ the tuple
$\mathcal{O}_{X, y}^\wedge, I\mathcal{O}_{X, y}^\wedge,
\{\mathfrak m_y^\wedge\}, s - \delta_Z(y), d$
is as in Situation \ref{situation-bootstrap}.
\item If $(\mathcal{F}_n)$ satisfies the strict $(s, d)$-inequalities, then
$(\mathcal{F}_n)$ satisfies the $(s, d)$-inequalities, see
Lemma \ref{lemma-bootstrap-bis-bis}.
\item If $(\mathcal{F}_n)$ satisfies the $(s + 1, d)$-inequalities, then
$(\mathcal{F}_n)$ satisfies the strict $(s, d)$-inequalities.
\end{enumerate}
In the rest of this section we will show that the category of
{\it algebraizable} objects $(\mathcal{F}_n)$ of
$\textit{Coh}(U, I\mathcal{O}_U)$
which satisfy the strict $(1, d)$-inequalities is equivalent to
a full subcategory of the category of coherent $\mathcal{O}_U$-modules.

\begin{lemma}
\label{lemma-sanity}
In Situation \ref{situation-algebraize}. Let $\mathcal{F}$ be a
coherent $\mathcal{O}_U$-module. Assume
\begin{enumerate}
\item $A$ is $I$-adically complete and has a dualizing complex,
\item the completion $\mathcal{F}^\wedge$ of $\mathcal{F}$
satisfies the strict $(1, d)$-inequalities.
\end{enumerate}
...
Observe that $\delta_Z(y) = 1$ and $\dim(\mathcal{O}_{W, y}) \leq d$.
...
\item $A$ is $I$-adically complete and has a dualizing complex,
\item the completion $\mathcal{F}^\wedge$ of $\mathcal{F}$
satisfies the strict $(1, d)$-inequalities, and
...
Properties, Lemma \ref{properties-lemma-complement-closed-point-Jacobson}.
Then $\delta_Z(y) = 1$ and the assumption
...
satisfies the $(2, d)$-inequalities and that for
...
object of $\textit{Coh}(U, I\mathcal{O}_U)$. Let $d \geq \text{cd}(A, I)$.
With notation as above assume
\begin{enumerate}
\item $A$ is $I$-adically complete and has a dualizing complex,
\item $(\mathcal{F}_n)$ is the completion of a coherent $\mathcal{O}_U$-module,
\item $(\mathcal{F}_n)$ satisfies the strict $(1, d)$-inequalities.
\end{enumerate}
...
We will construct $\mathcal{F}$ by applying
Lemma \ref{lemma-make-S2-along-T-simple}
...
$x' \leadsto y \leadsto z$, $y \in Y \setminus Z$, and $\delta_Z(y) = 1$.
By Lemma \ref{lemma-cd-bound-dim-local} and the fact that $z$
...
we will generalize this to more general formal coherent modules;
...
$\delta_Z = \delta$ is the dimension function associated with
...
(3) guarantees that Lemma \ref{lemma-algebraize-local-cohomology-bis}
applies to
...
\item $(\mathcal{F}_n)$ satisfies the $(2, 1)$-inequalities.
\end{enumerate}
Then there exists a coherent $\mathcal{O}_U$-module $\mathcal{F}$
...
Lemma \ref{lemma-equivalent-f-good} hold for the system
$(\mathcal{F}_n)$ on $U$. We will use this without further mention
in the rest of the proof.

\medskip\noindent
We will check hypotheses (a), (b), (c), and (d) of
Lemma \ref{lemma-when-done}. Hypothesis (a) holds by assumption.
Hypothesis (c) holds by Lemma \ref{lemma-topology-I-adic-f}.

\medskip\noindent
Pick a system of modules $\{M_n\}$ as in Lemma \ref{lemma-system-of-modules}.
...
In this way we see that hypothesis (d)\footnote{As modified by the footnote.}
of Lemma \ref{lemma-when-done} holds.
...
However, since $\{K_n\}$ is pro-isomorphic to a system of
...
Thus hypothesis (b) of Lemma \ref{lemma-when-done}
...
$\mathfrak p \in \text{Ass}(\mathcal{F}_y)
\Rightarrow
...
$(\mathcal{F}_n)$ satisfies the $(2, d)$-inequalities, see
Definition \ref{definition-s-d-inequalities}.
Then there exists a canonical map $(\mathcal{F}_n) \to (\mathcal{F}_n'')$
...
of coherent formal modules. First, let us prove the result in case
the ideal has cohomological dimension $1$.
...
\end{reference}
In Situation \ref{situation-algebraize} assume $A$ is $I$-adically complete and
has a dualizing complex. Let $(\mathcal{F}_n)$ be an object of
$\textit{Coh}(U, I\mathcal{O}_U)$. Assume $(\mathcal{F}_n)$ satisfies the
$(2, 1)$-inequalities, see Definition \ref{definition-s-d-inequalities}.
Then there exists a coherent $\mathcal{O}_U$-module
$\mathcal{F}$ such that $(\mathcal{F}_n)$ is the completion of $\mathcal{F}$.
\end{proposition}

\noindent
Observe that our assumptions imply $\text{cd}(A, I) = 1$.

\begin{proof}
By Lemma \ref{lemma-map-kernel-cokernel-on-closed}
we may replace $(\mathcal{F}_n)$ by the object
...
to a system $(\mathcal{F}_n'')$ with the properties
...
Since the system $\{H^0(U, \mathcal{F}_N)\}$ is pro-isomorphic to an
...
Algebra, Lemma \ref{algebra-lemma-finite-over-complete-ring}.
In particular hypothesis (d)\footnote{As modified by the footnote.} holds.
...
has ML and we finally conclude that hypothesis (b) holds.
This concludes the proof.
\end{proof}

\begin{proposition}[Algebraization for ideals with few generators]
\label{proposition-d-generators}
In Situation \ref{situation-algebraize} assume $A$ is $I$-adically complete,
has a dualizing complex, and $V(I) = V(f_1, \ldots, f_d)$ for some $d \geq 1$
and $f_1, \ldots, f_d \in A$.
Let $(\mathcal{F}_n)$ be an object of $\textit{Coh}(U, I\mathcal{O}_U)$.
Assume that $(\mathcal{F}_n)$ satisfies the $(??, ??)$-inequalities.
Then there exists a coherent $\mathcal{O}_U$-module $\mathcal{F}$
such that $(\mathcal{F}_n)$ is the completion of $\mathcal{F}$.
\end{proposition}

\begin{proof}
We may assume $I = (f_1, \ldots, f_d)$. We will prove the
proposition by induction on $d$. The case $d = 1$ follows
immediately from Proposition \ref{proposition-cd-1}.

\medskip\noindent
Choose generators $h_1, \ldots, h_a$ of $\mathfrak a$.
Set $U_j = D(h_j) \subset U$. Recall that the restriction
of $(\mathcal{F}_n)$ to $U_j$ corresponds to finite
$A_{h_j}^\wedge$-modules $N_j$. See
Cohomology of Schemes, Lemma \ref{coherent-lemma-inverse-systems-affine}.

\medskip\noindent
As a first step for $N$ large we consider the morphism
$$
f_1^N, \ldots, f_d^N :
(\mathcal{F}_n)
\longrightarrow
\bigoplus\nolimits_{i = 1, \ldots, d} (\mathcal{F}_n)
$$
of $\textit{Coh}(U, I\mathcal{O}_U)$.
Since $\textit{Coh}(U, I\mathcal{O}_U)$ is an abelian category, see
Cohomology of Schemes, Lemma \ref{coherent-lemma-inverse-systems-abelian},
we can look at the image of this map.
On the affine opens $U_j$ this corresponds to
replacing $N_j$ by the quotient of $N_j$ by its
$f_1^N, \ldots, f_d^N$-torsion submodule.
Since $A_{h_j}^\wedge$ is Noetherian and $N_j$ finite
we see that for $N$ large enough we obtain the quotient
$N_j/H^0_I(N_j)$ by all $I$-power torsion.
By Lemma \ref{} we may replace $(\mathcal{F}_n)$ by this image.
Hence we may assume $H^0_I(N_j) = 0$ for all $j$.

\medskip\noindent
Consider the finite sets $\text{Ass}(N_j) = \{\mathfrak q_{jk}\}$
of associated primes of the modules $N_j$.
Let $\mathfrak p_{jk} \subset A$ be the corresponding primes.
By the above we see that $I \not \subset \mathfrak p_{jk}$
for all $j, k$. Applying Algebra, Lemma \ref{algebra-lemma-silly-silly}
we can find an element of the form
$$
f = f_1 + \sum\nolimits_{i = 2, \ldots, d} a_i f_i
$$
which is not contained in $\mathfrak p_{jk}$ for any $j, k$.
Then we see that multiplication by $f$ is injective on $N_j$.
Thus for every $m \geq 1$ we find a short exact sequence
$$
0 \to (\mathcal{F}_n) \xrightarrow{f^m} (\mathcal{F}_n) \to
(\mathcal{G}_{m, n}) \to 0
$$
in $\textit{Coh}(U, I\mathcal{O}_U)$.

\medskip\noindent
Fix $m$ for the moment. Set $A_m = A/(f^m)$ and let
$\mathfrak a_m$ and $I_m$ be the images in $A_m$.
Observe that $V(I_m)$ can be cut out by $d - 1$ elements
(namely $f_2, \ldots, f_d$). Set $X_m = \Spec(A_m)$,
$U_m = X_m \setminus V(\mathfrak a_m)$, etc.
Since $(\mathcal{G}_{m, n})$ is annihilated by $f^m$ it defines
an object of $\textit{Coh}(U_m, I_m\mathcal{O}_{U_m})$.
We claim the induction hypothesis applies to $(\mathcal{G}_{m, n})$.
FIXME.

\medskip\noindent
By the above we find coherent modules $\mathcal{G}_m$ on
$U_m$ whose completion is $(\mathcal{G}_{m, n})$.
By our construction of $(\mathcal{G}_{m, n})$
we see that $\mathcal{G}_{m + 1}/f^m\mathcal{G}_{m + 1}$
and $\mathcal{G}_m$ have the same completion.
FIXME.
\end{proof}
