...
\label{lemma-1}
\begin{reference}
See \cite[Lemma 1 page 299]{MatCA}. Attributed to Lech.
\end{reference}
...
\label{lemma-2}
\begin{reference}
See \cite[Lemma 2 page 300]{MatCA}. Attributed to Lech.
\end{reference}
Let $A$ be a ring. If $f_1, \ldots, f_{r - 1}, f_rg_r$
...
\begin{proof}
Use the definition to show that there is an exact sequence
$$
0 \to
A/(f_1, \ldots, f_{r - 1}, g_r) \xrightarrow{f_1}
A/(f_1, \ldots, f_{r - 1}, f_rg_r) \to
A/(f_1, \ldots, f_{r - 1}, f_r) \to 0
$$
and use additivity of lengths (Algebra, Lemma \ref{}).
\end{proof}

\begin{lemma}
\label{lemma-3}
\begin{reference}
See \cite[Lemma 3 page 300]{MatCA}. Attributed to Lech.
\end{reference}
...
\begin{proof}
Use Lemmas \ref{} and \ref{} and induction.
\end{proof}
...
\medskip\noindent
Surjective. Let $x_1, \ldots, x_d$ be a regular system of parameters for $A$.
Applying $F$ we find
...
$$
0 \to M[x_1] \to M \to M \to M/x_1M \to 0
$$
and flatness we see that $M[x_1] \otimes_{A, F} A \cong M[x_1]$ and
$M/x_1 M \otimes_{A, F} A \cong M/x_1M$.
To get a contradiction, assume $M/x_1M$ is nonzero.
Arguing as above, we find a nonzero element $z \in (M/x_1M)[\mathfrak m]$.
By Algebra, Lemma
\ref{algebra-lemma-annihilator-flat-base-change}
we have $\text{Ann}_A(z \otimes 1) = (x_1^p, \ldots, x_d^p)$.
This implies $x_1 \not \in \text{Ann}_A(M/x_1M)$ which is absurd.
Thus we see that $M$ is $x_1$-divisible.
If $\varphi$ is not surjective, then we can choose
$e \in \bigoplus_{j \in J} E$ not in $M$.
Arguing as above we may assume $\mathfrak m e \subset M$,
in particular $x_1 e \in M$. There exists an element
...
is surjective. If $d = 1$, this is true by construction of $\varphi$.
If $d > 1$, then we observe that $E[x_1]$ is the injective hull
of the residue field of the regular ring $A/x_1A$, see
Dualizing Complexes, Lemma \ref{dualizing-lemma-quotient}.
Observe that $M[x_1]$ as a module over $A/x_1A$
is $\mathfrak m/(x_1)$-power torsion and satisfies
$M[x_1] \otimes_{A/x_1A, F} A/x_1A \cong M[x_1]$, see above.
Thus by induction on $\dim(A)$ we conclude that $\varphi[x_1]$
is surjective as desired.
\end{proof}
...
\begin{proof}
Immediate from cohomology and base change, see
Derived Categories of Schemes, Lemma
