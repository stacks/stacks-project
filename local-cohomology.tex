...
and $b_j \in B$. By
Cohomology of Schemes, Lemma \ref{coherent-lemma-homs-over-open}
we can represent $b_i$ by maps $\varphi_j : I^n \to A$
...
\quad\text{and}\quad
1 \otimes 1 : I^cN_k \otimes_{B_k} M_k \to I^cM_k
$$
...
with $K_m = \Ker(E(m, -d - s - 1) \to E(m'(m, d), -d - s - 1))$
we have
$$
K_{m, \mathfrak q} =
\Ker(E(m, -d - s - 1)_{\mathfrak q} \to E(k, -d - s - 1)_{\mathfrak q})
...
element
$$
u = 1 + \sum\nolimits_{i \geq 1} \frac{t^i}{(xy)^{2^i}} \in A_{xy}/t^nA_{xy}
$$
...
Namely, if it were, then it would have a nonzero global
section. This would imply the powerseries $u$
is an element of the fraction field of $k[x, y][[t]]$
which is not the case. Alternative argument: $\mathcal{L}$
would be invertible in an open neighbourhood $V \subset U$
of $U \cap Y$. However $\text{Pic}(V) = 0$ and hence $\mathcal{L}_2$
would be trivial which is not the case.
\end{example}
...
\section{Algebraization of coherent formal modules; bis}
\label{section-algebraization-modules-bis}
...
\begin{lemma}
\label{lemma-improvement-formal-coherent-module}
In Situation \ref{situation-algebraize} let
$(\mathcal{F}_n)$ be an object of $\textit{Coh}(U, I\mathcal{O}_U)$.
Assume $A$ has a dualizing complex. There exists a canonical map
$(\mathcal{F}_n) \longrightarrow (\mathcal{F}'_n)$
in $\textit{Coh}(U, I\mathcal{O}_U)$ with the following property:
for all $y \in U \cap Y$ the induced map
$$
\mathcal{F}_y^\wedge \longrightarrow (\mathcal{F}')_y^\wedge
$$
is the canonical map of Lemma \ref{lemma-make-S2-along-Z} for pair
$V(I\mathcal{O}_{X, y}^\wedge) \subset \Spec(\mathcal{O}_{X, y}^\wedge)$.
\end{lemma}

\begin{proof}
For $f \in \mathfrak a$ apply the construction of
Lemma \ref{lemma-make-S2-along-Z} to the coherent module
over $A_f^\wedge$ corresponding to $(\mathcal{F}_n)$, see above.
The constructions glue by the final assertion of the lemma
and the fact that the formal fibres of $A$ are Gorenstein
(and hence Cohen-Macaulay). Details omitted.
\end{proof}
