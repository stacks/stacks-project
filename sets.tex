...
\item For any prime $\mathfrak p \subset R$ the spectrum of the
algebraic closure $\overline{\kappa(\mathfrak p)}$.
\item For any subring $R' \subset R$ the scheme
$\text{Spec}(R')$.
\item Any scheme of finite type over a countable ring.
\item And so on.
\end{enumerate}
\end{enumerate}
\end{lemma}
...
Statement (4) follows from (6).
Statement (5) follows as the size of $S$ is
$\leq \max\{|I|, \sup_i \text{size}(S_i)\} \leq \text{size}(T)^{\aleph_0}$
by Lemma \ref{lemma-bound-size}. Statement (6) follows from (5).
...
of $V$. Thus we see that (5) applies in the situation of (6).

\medskip\noindent
Statement (7) is translated, via Lemma \ref{lemma-bound-affine},
...
The cardinality of this ring is easily seen to be
bounded by $|R|^{|R|}$, but is not bounded by
...
we take $\text{size}(S) = \max\{\aleph_0, |S|\}$ (where $|S|$ is the
cardinality of the underlying set). As above we use the function
