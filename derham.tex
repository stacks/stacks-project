...
(and the finiteness already shown); an alternative is
to use Local Cohomology, Lemma
...
$\dim H^d(X, \Omega_{X/k}) = 1$ by Lemma \ref{lemma-duality-hodge}.
...
Section \ref{section-first-chern-class}.
\end{enumerate}
The rest of this section consists in checking the axioms (A1) -- (A9).
...
of smooth projective schemes over $k$. We observe here that the
chern character $ch^{dR} : K_0(\textit{Vect}(X)) \to H_{dR}^*(X/k)$
used in the definition of $\gamma$ is by its very construction
compatible with the more general chern character constructed
in Lemma \ref{lemma-chern-character}.
...
\medskip\noindent
In this paragraph, we are going to reduce axiom (A6) to the case where $k$
is algebraically closed. We urge the reader to skip it.
Denote $\overline{k}$ the algebraic closure of $k$. Observe that
$$
H_{dR}^{2d}(X_{\overline{k}}/\overline{k}) =
H_{dR}^{2d}(X/k) \otimes_k \overline{k}
$$
by Lemma \ref{lemma-proper-smooth-de-Rham}. Denote
$\overline{\gamma}$ and $\overline{\gamma}_i$ the images
of $\gamma$ and $\gamma_i$ in
$$
H_{dR}^{2d}(X_{\overline{k}} \times_{\Spec(\overline{k})}
X_{\overline{k}} / \overline{k})
\quad\text{and}\quad
H_{dR}^i(X_{\overline{k}}/\overline{k})
\otimes_{\overline{k}} H_{dR}^{2d - i}(X_{\overline{k}}/\overline{k})
$$
Then of course $\overline{\gamma} = \sum \overline{\gamma}_i$, i.e.,
$\overline{\gamma}_0$ is the first K\"unneth component of
$\overline{\gamma}$. By linear algebra\footnote{Given $k$-vector spaces
$V$, $W$, an element $v \in V$, and a tensor $\xi \in V \otimes_k W$
there exists a linear map $\lambda : W \to K$ such that
$(1 \otimes \lambda)\xi = v$ if and only if this holds over $\overline{k}$.}
it suffices to show that
there exists a $\overline{k}$-linear map
$\overline{\lambda} : H_{dR}^{2d}(X_{\overline{k}}/\overline{k}) \to
\overline{k}$ such that
$(1 \otimes \overline{\lambda})\overline{\gamma}_0 = 1$
in $H^0_{dR}(X_{\overline{k}}/\overline{k})$. Thus to finish the
reduction argument, it suffices to show that
$\overline{\gamma}$ is equal to the class $\gamma([\Delta_{\overline{k}}])$
in de Rham cohomology over $\overline{k}$. Namely,
recall that $\gamma([\Delta])$ is defined by choosing an
$\alpha \in K_0(\textit{Vect}(X \times_{\Spec(k)} X))$ with
$ch(\alpha) = [\Delta]$ and then taking $ch^{dR}(\alpha)$
in $H_{dR}^{2d}(X \times_{\Spec(k)} X/k)$. Now of course
the pullback $\overline{\alpha}$ of $\alpha$ to
$X_{\overline{k}} \times_{\Spec(\overline{k})} X_{\overline{k}}$
satisfies $ch(\overline{\alpha}) = [\Delta_{\overline{k}}]$.
Thus it suffices to show that $ch^{dR}(\alpha)$ pulls back to
$ch^{dR}(\overline{\alpha})$. This follows from the fact that
we can define $ch^{dR}$ on the category of all quasi-compact
and quasi-separated schemes over $\mathbf{Q}$ compatible with
pullbacks, see Lemma \ref{lemma-chern-character}.

\medskip\noindent
Proof of Axiom (A6) for $X$ in case $k$ is algebraically closed.
...
\medskip\noindent
Proof of Axiom (A6) for $X$ irreducible in case $k$ is algebraically closed.
In this case $H^0_{dR}(X/k) = k$ because $H^0(X, \mathcal{O}_X) = k$
