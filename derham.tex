...
a section $s : S \to X$ of $f$. We may and do assume $S$ is affine.
...
$$
and we set $\xi = \gamma^{0, 0}(1)$. For later use, we observe
that the construction of $\xi$ commutes with base change by
morphisms $S' \to S$, see
Lemma \ref{lemma-gysin-transverse}.

\medskip\noindent
The section $s$ defines a splitting $s^* : Rf_*\mathcal{O}_X \to \mathcal{O}_S$
of the map $\mathcal{O}_S \to Rf_*\mathcal{O}_X$. By duality
$R^df_*\Omega^d_{X/S}$ is locally free of rank $1$ (and of course even free
of rank $1$), see Lemma \ref{lemma-relative-duality-hodge} and its proof.
Since $S$ is affine, we can write
$H^d(X, \Omega^d_{X/S}) = H^0(S, R^df_*\Omega^d_{X/S})$.
Thus we may interpret the element $\xi$ as a global section of
$R^df_*\Omega^d_{X/S}$. In fact, we claim $\xi$ is a basis element.
Namely, since we have base change in top degree
(see for example Limits, Lemma
\ref{limits-lemma-higher-direct-images-zero-above-dimension-fibre})
we see that $R^df_*\Omega^d_{X/S} \otimes \kappa(s) =
H^d(X_s, \Omega^d_{X_s/\kappa(s)})$. By the compatibility of
the construction of $\xi$ with base change, we see that
the image of $\xi$ in this fibre is nonzero by
Lemma \ref{lemma-class-of-a-point}. Thus $\xi$ is a nowhere vanishing
section of an invertible module and hence a generator.
...
$$
This is also a section of smooth morphism (namely either projection)
...
$$
of Remark \ref{remark-how-to-use}. We set $\eta = \gamma^{0, 0}(1)$.

\medskip\noindent
Consider the morphism $g = (\text{id}, s) : X \to X \times_S X$.
Then $g^{-1}(\Delta) = Z$ scheme theoretically. Hence we see that
$g^*\eta = \xi$ by Lemma \ref{lemma-gysin-transverse}.
Similarly, for the morphism $g' = (s, \text{id}) : X \to X \times_S X$
we see that $(g')^*\eta = \xi$.

\medskip\noindent
Let $\theta \in H^d(X, \Omega^{d - 1}_{X/S})$. We have to show that
$\text{d}(\theta)$ is zero in $H^d(X, \Omega^d_{X/S})$.
We may write $\text{d}(\theta) = u \xi$ for some $u \in H^0(S, \mathcal{O}_S)$.
...
and note that there are no tors as we are looking in top degree)
we see that
$$
H^{2d}(X \times_S X, \Omega^{2d - 1}_{X \times_S X/S}) =
H^d(X, \Omega^{d - 1}_{X/S}) \otimes_A H^d(X, \Omega^d_{X/S}) \oplus
H^d(X, \Omega^d_{X/S}) \otimes_A H^d(X, \Omega^{d - 1}_{X/S})
$$
Combined with the fact that $\xi$ is a generator this means
we can write
$$
...
\gamma^{d - 1, d}(\theta) =
\text{pr}_1^*\theta \cup \eta =
\text{pr}_2^*\theta \cup \eta
$$
in the Hodge cohomology ring of $X \times_S X$ over $S$.
From the first equality we conlude that
$\theta_1 = (\theta \otimes 1) \cup \eta_1$
where $\eta_1$ is the image of $\eta$ in the summand
$$
H^d(X \times_S X, \Omega^0_{X/S} \boxtimes \Omega^d_{X/S}) =
