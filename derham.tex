...
\begin{proof}
This follows from Lemma \ref{lemma-gysin-projection} combined with
the commutativity of the diagram
$$
\xymatrix{
i_*(i^{-1}\Omega^p_{X/S} \otimes_{\mathcal{O}_X|_Z}^\mathbf{L}
R\mathcal{H}_Z(\Omega^c_{X/S})) \ar[r] \ar[d] &
i_*R\mathcal{H}_Z(\Omega^{p + c}_{X/S}) \ar[d] \\
\Omega^p_{X/S} \otimes_{\mathcal{O}_X}^\mathbf{L}
\Omega^c_{X/S} \ar[r] &
\Omega^{p + c}_{X/S}
}
$$
which guarantees that the cup product taken on $Z$ maps to the
corresponding cup product on $X$. The vertical arrow on the left
comes from
$$
i_*(i^{-1}\Omega^p_{X/S} \otimes_{\mathcal{O}_X|_Z}^\mathbf{L}
R\mathcal{H}_Z(\Omega^c_{X/S})) =
\Omega^p_{X/S} \otimes_{\mathcal{O}_X}^\mathbf{L}
i_*R\mathcal{H}_Z(\Omega^c_{X/S}) \to
\Omega^p_{X/S} \otimes_{\mathcal{O}_X}^\mathbf{L} \Omega^c_{X/S}
$$
The diagram commutes by the construction of the top horizontal arrow
in Cohomology, Remark \ref{cohomology-remark-support-cup-product}.
\end{proof}
...
and locally on $S$ the map $f_*\mathcal{O}_X \to Rf_*\mathcal{O}_X$
splits in $D(\mathcal{O}_S)$. The map
...
\medskip\noindent
Uniqueness of $t$. By Lemma \ref{lemma-relative-bottom-part-degenerates}
the map $f_*\mathcal{O}_X \to Rf_*\mathcal{O}_X$ is locally split.
Hence our duality produces in particular
a perfect $\mathcal{O}_S$-bilinear pairing
$$
