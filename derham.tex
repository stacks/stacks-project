...
we have $\omega_X^\bullet = \Omega^d_{X/k}[d]$. We obtain a $k$-linear map
$t : H^d(X, \Omega^d_{X/k}) \to k$ such that the statement is true by
...
\Omega^{d - p}_{X/k}
$$
In particular, we see that the pairing
$H^0(X, \mathcal{O}_X) \times H^d(X, \Omega^d_{X/k}) \to k$
is perfect, which implies that any other linear map
...
we need $g$ to be a unit. Each of the pairings
$\langle -, - \rangle_{p, q}$ satisfies
$$
\langle g \xi, \xi' \rangle_{p, q} =
\langle \xi, g \xi' \rangle_{p, q}
$$
for $g \in H^0(X, \mathcal{O}_X)$, $\xi \in H^q(X, \Omega^p_{X/k})$,
and $\xi' \in H^{d - q}(X, \Omega^{d - p}_{X/k})$ and we see that
using $t'$ instead of $t$ we still get perfect pairings for all $p, q$.
\end{proof}
