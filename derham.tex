...
$$
in $D(f^{-1}\mathcal{O}_S)$ with the following two properties
\begin{enumerate}
...
$s' s = 1$. In other words, $s$ gives an invertible section of
$(L^\star \to X)_*\mathcal{O}_{L^\star}$ over $U$ and $s' = s^{-1}$.
Then we can consider the $1$-form
...
The map $\Omega^\bullet_{X/S} \to \Omega^\bullet_{X/S}[2]$
in $D(X, f^{-1}\mathcal{O}_S)$ deduced
from the short exact sequence in Lemma \ref{lemma-the-complex-for-L-star}
