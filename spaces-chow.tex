...
\medskip\noindent
Let $\alpha \in A_{k + r - 1}(P)$. Write $\alpha = \sum m_j[W_j]$ with
$m_j \not = 0$ and $W_j$ pairwise distinct integral closed subspaces of
$\delta$-dimension $k + r$. Then the family $\{W_j\}$
...
is rationally equivalent to $\pi_j^*\beta_j$ for some
$k$-cycle $\beta_j \in A_k(Z_j)$. Then
$\beta = \sum m_j \beta_j$ will be a $k$-cycle on $X$
and $\pi^*\beta = \sum m_j \pi_j^*\beta_j$ will be rationally
equivalent to $\alpha$ (see
Remark \ref{remark-infinite-sums-rational-equivalences}).
...
Suppose that we can show that $\alpha|_{\pi^{-1}(U)} = f^*\beta$
for some $\beta \in Z_k(U)$. By Lemma \ref{lemma-exact-sequence-open}
we see that
$\beta = \beta'|_U$ for some $\beta' \in Z_k(X)$.
By the exact sequence
$A_k(\pi^{-1}(X \setminus U)) \to A_k(P) \to A_k(\pi^{-1}(U))$
of Lemma \ref{lemma-restrict-to-open}
we see that $\alpha - f^*\beta'$ comes from
a cycle $\alpha' \in A_{k + r}(\pi^{-1}(X \setminus U))$.
