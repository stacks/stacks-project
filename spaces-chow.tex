...
$q^*\text{div}_\mathcal{L}(s)$. By the same token the ring map
$\mathcal{O}_{X, \xi'}^h \to \mathcal{O}_{T, \xi}^h$
is unramified. It is also flat as $q$ is flat.
Thus $a, b$ map to nonzerodivisors in
$\mathcal{O}_{T, \xi}^h$ and $q^*(s)/s_\xi = a/b$ in the total quotient
ring of $\mathcal{O}_{T, \xi}^h$. Thus by definition the coefficient of
...
by Algebra, Lemma \ref{algebra-lemma-pullback-module} and the unramifiedness
shown above.
\end{proof}
