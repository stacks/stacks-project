...
Let $\mathcal{U} \subset \Polarizedstack$ be an open substack with
the following property: for every $\Spec(k) \to \mathcal{U}$
corresponding to $(X \to \Spec(k), \mathcal{L})$ we have
\begin{enumerate}
...
\end{enumerate}
Then $\mathcal{U}$ is quasi-compact.
\end{lemma}
...
it suffices to prove that any point of $\mathcal{U}$ is
in the image of the induced map $|H| \to |\Polarizedstack|$ 
