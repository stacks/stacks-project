...
\begin{proof}
Omitted. Hint: If $(\pi_*\mathcal{F})_T \to \mathcal{Q}$ is an object of
$\Quotfunctor_{\pi_*\mathcal{F}/Y/B}$ over $T$
...
Then the morphism
$\Quotfunctor_{\mathcal{F}/X/B} \to \Quotfunctor_{\pi_*\mathcal{F}/Y/B}$ of
Lemma \ref{lemma-quot-functorial} is a closed immersion.
\end{lemma}

\begin{proof}
Set $\mathcal{G} = \pi_*\mathcal{F}$. Then we have a cartesian diagram
$$
\xymatrix{
\Quotfunctor_{\mathcal{F}/X/B} \ar[r] \ar[d]_{\pi_*} &
\Cohstack_{X/B} \ar[d]^{\pi_*} \\
\Quotfunctor_{\mathcal{G}/Y/B} \ar[r] &
\Cohstack_{Y/B}
}
$$
where the horizontal arrows are the ones constructed in the proof of
Quot, Proposition \ref{quot-proposition-quot}.
The diagram is cartesian because for every $T/B$ the functor
$\pi_{T, *} : \QCoh(\mathcal{O}_{X_T}) \to \QCoh(\mathcal{O}_{Y_T})$
is fully faithful and reflects surjections.
Thus the lemma follows from Lemma \ref{lemma-coherent-closed}.
\end{proof}
