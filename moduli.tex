...
$\mathcal{F}_i$),
\ref{spaces-limits-lemma-descend-invertible-modules} (to find $\mathcal{L}_i$),
and
\ref{spaces-limits-lemma-descend-separated} (to make $X_i$ separated).
Because
$$
...
(to find $\mathcal{L}_i$, $\mathcal{N}_i$),
\ref{limits-lemma-descend-separated-finite-presentation}
(to make $X_i \to B_i$ separated),
\ref{limits-lemma-eventually-proper}
(to make $Y_i \to B_i$ proper), and
...
\section{Properties of Polarized}
...
By Limits on Stacks, Lemma \ref{stacks-limits-lemma-limit-preserving-diagonal}
this implies that
...
for some scheme $T$ over $B$ then for every $t \in T$ we have
$$
...
$$
\mathit{Isom}_U(Y, X) \cap \mathit{Mor}^{P', \mathcal{M}}_B(Y, X)
$$
The intersection is an intersection of open subspaces of
$\mathit{Mor}_B(Y, X)$ (see Lemma \ref{lemma-Isom-in-Mor} and
Remark \ref{remark-Mor-numerical}).
Now $\mathit{Mor}^{P', \mathcal{M}}_B(Y, X)$
is a Noetherian algebraic space as it is of finite
presentation over $B$ by Lemma \ref{lemma-Mor-qc-over-base}.
