...
is a morphism $\psi : G \to G'$ of schemes over $B$ such that for
...
$\sim_{S'}$ generated by $j(S') : R(S') \to U(S') \times U(S')$.
...
Section \ref{groupoids-section-quotient-sheaves}
above and
Groupoids, Section \ref{groupoids-section-quotient-sheaves}.
Namely, now we are going to take quotient stacks instead of
quotient sheaves.
...
\begin{proof}
Consider the union $W$ of the images $g(U') \subset U$ of
morphisms $g : U' \to U$ with the properties:
\begin{enumerate}
\item $g$ is flat of finite presentation, etale, smooth, or syntomic, and
\item the base change $s' : U' \times_{g, U} R \to U'$ has property
$\mathcal{P}$.
\end{enumerate}
Since each such morphism $g$ is open we see that $W$ is an open subspace
of $U$, and since $\mathcal{P}$ is local in the $\tau$ topology the
restriction of $s$ to $s^{-1}(W)$ has property $\mathcal{P}$. This means
the assertion of the lemma makes sense.
Next, consider the diagram in Lemma \ref{lemma-diagram}.
Let $W_1 \subset R$ be the maximal open subspace over which the morphism
