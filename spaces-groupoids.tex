...
if and only if the identity morphism $e : S \to G$ is a closed immersion
...
immersion) if $G \to B$ is separated (resp.\ quasi-separated).
...
$t(s^{-1}(W)) \subset W$.
\item A locally closed subspace $Z \subset U$ is called $R$-invariant
...
$$
are cartesian and that $s : R \to U$ is flat and
...
$R'' = R \times_{f \circ s, U', t'} R'$. By the assumptions above
...
Since we assumed $\{s : R \to U\}$ is an fppf covering the morphism
$$
...
\begin{proof}
We will verify that
Stacks, Lemma \ref{lemma-when-gerbe}
...
Thus (b) $\Rightarrow$ (a) in (1), (2), and (3).
