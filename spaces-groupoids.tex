...
\noindent
This chapter is devoted to generalities concering groupoid spaces.
We recommend reading the beautiful paper \cite{K-M} by Keel and Mori.
...
The discussion of associated stacks is new here.
...
to check the axioms for an algebraic group space $G$ over $B$, it
...
Let $B \to S$ as in Section \ref{section-notation}.
\begin{enumerate}
\item A {\it group space over $B$}, or more precisely a
{\it algebraic group space over $B$} is a pair $(G, m)$, where
...
\item A {\it morphism $\psi : (G, m) \to (G', m')$ of group spaces over $B$}
...
\noindent
Let $(G, m)$ be a group space over the algebraic space $B$.
...
Let $B \to S$ as in Section \ref{section-notation}.
Let $(G, m)$ be a group space over $B$.
Let $B' \to B$ be a morphism of algebraic spaces.
The pullback $(G_{B'}, m_{B'})$ is a group space over $B'$.
\end{lemma}
...
$B$ over $S$ is an algebraic group space over $B$. We will
frequently use this in the examples below.

\begin{example}
\label{example-multiplicative-group}
(Multiplicative group space.)
Let $B \to S$ as in Section \ref{section-notation}.
...
This is representable by the algebraic group space
$$
...
(Roots of unity as a group space.)
Let $B \to S$ as in Section \ref{section-notation}.
...
Let $B \to S$ as in Section \ref{section-notation}.
Let $n \geq 1$.
The determinant defines a morphisms of group spaces
$$
...
\section{Actions of group spaces}
...
Let $B \to S$ as in Section \ref{section-notation}.
Let $(G, m)$ be a group space over $B$, and
...
Let $B \to S$ as in Section \ref{section-notation}.
\begin{enumerate}
\item A {\it groupoid space over $B$}, or more precisely a
{\it groupoid in algebraic spaces over $B$} is a
...
of groupoid spaces over $B$} is given by morphisms
...
\noindent
Let $(U, R, s, t, c)$ be a groupoid space over $B$.
...
is a groupoid space over $B$. Note that $i$ is an isomorphism,
and $e$ is a section of both $s$ and $t$.
Moreover, given a groupoid space over $B$ we denote
$$
...
groupoid space over $B$'' to stress the existence of identity and
inverse.

\begin{lemma}
\label{lemma-groupoid-pre-equivalence}
Let $B \to S$ as in Section \ref{section-notation}.
Given a groupoid space $(U, R, s, t, c)$ over $B$
...
there is a unique way to extend it to a groupoid space
$(U, R, s, t, c)$ over $B$.
\end{lemma}
...
Consider a morphism
$f : (U, R, s, t, c) \to (U', R', s', t', c')$
of groupoid spaces over $B$. Then pullback $f^*$ given by
$$
...
the groupoid space constructed in Lemma \ref{lemma-groupoid-from-action}.
The rule
...
\section{The stabilizer group space}
\label{section-stabilizer}

\noindent
Please compare with
Groupoids, Section \ref{groupoids-section-stabilizer}.
Given a groupoid space we get a group space as follows.
...
$$
is a group space over $U$ with compostion law
...
Let $B \to S$ as in Section \ref{section-notation}.
Let $(U, R, s, t, c)$ be a groupoid space over $B$.
The group space $j^{-1}(\Delta_{U/B}) \to U$
is called the {\it stabilizer of the groupoid space $(U, R, s, t, c)$}.
\end{definition}

\noindent
In the literature the stabilizer group space is often denoted $S$
...
Let $B \to S$ as in Section \ref{section-notation}.
Let $(U, R, s, t, c)$ be a groupoid space over $B$, and let $G/U$ be its
stabilizer. Denote $R_t/U$ the algebraic space $R$ seen as an algebraic space
over $U$ via the morphism $t : R \to U$. There is a canonical left action
$$
...
such that $(U', R', s', t', c')$ is a groupoid space over
$B$ and such that $U' \to U$, $R' \to R$ defines a morphism
$(U', R', s', t', c') \to (U, R, s, t, c)$ of groupoid spaces over $B$.
Moreover, for any scheme $T$ over $B$ the functor of groupoids
$$
...
Let $(U, R, s, t, c)$ be a groupoid space over $B$.
Let $g : U' \to U$ be a morphism of algebraic spaces over $B$.
The morphism of groupoid spaces
...
Let $B \to S$ as in Section \ref{section-notation}.
Let $(U, R, s, t, c)$ be a groupoid space over the base $B$.
\begin{enumerate}
...
If $j : R \to U \times_B U$ comes from the action of a group space
$G/B$ on $U$ as in Lemma \ref{lemma-groupoid-from-action} then we
...
We will say a groupoid space $(U, R, s, t, c)$ over $B$ has a
{\it representable quotient}
...
\begin{proof}
Consider the commutative diagram of sheaves
$$
\xymatrix{
...
$U'/R' \to U/R$ is injective. If $U' \to U$ is a surjective
map of sheaves, then $U'/R' \to U/R$ is surjective also.
Finally, if $\{g : U' \to U\}$ is a fppf covering, then
...
We are going to take associated stacks instead of quotient sheaves.
...
is an open subspace of $|U|$. Let $W \subset U$ denote the corresponding
open subspace, see
...
Let $B \to S$ be as in Section \ref{section-notation}.
Let $(U, R, s, t, c, e, i)$ be a groupoid space over $B$.
The diagram
\begin{equation}
\label{equation-pull}
\xymatrix{
...
when given a groupoid space $(U, R, s, t, c)$ over $B$. As in the previous
...
Let $B \to S$ be as in Section \ref{section-notation}.
Let $(U, R, s, t, c)$ be a groupoid space over $B$.
Let $G \to U$ be the stabilizer group space.
...
\begin{proof}
The group space $G \to U$ is the base change of $R \to U \times_B U$
...
then $G \to U$ is separated (resp.\ quasi-separated). See
...
and Lemma \ref{spaces-morphisms-lemma-base-change-quasi-compact}
