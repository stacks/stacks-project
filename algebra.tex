...
by Thierry Coquand very helpful.
...
Thus the inclusion $\mathfrak q' \supset \mathfrak p R[x]$
...
$K$. Let $S \susbet S' \subset L$ be the subring $S'$ of
...
\begin{lemma}
\label{lemma-not-injective}
Let $R$ be a ring. Let $R[x] \subset S$ be a finite ring
extension. Let $\mathfrak q \subset S$ be a prime.
Then $R \to S$ is not quasi-finite at $\mathfrak q$.
\end{lemma}
...
\begin{lemma}
\label{lemma-is-surjective}
Let $R$ be a ring. Let $\varphi : R[x] \to S$ be a finite ring
map. Let $\mathfrak q \subset S$ be a prime, and
let $\mathfrak p \subset R[x]$ be the inverse image.
Assume that $R$ is integrally closed in $S$, and
that $R \to S$ is quasi-finite at $\mathfrak q$.
Then $R[x]_{\mathfrak p} \to S_{\mathfrak p}$
is surjective.
\end{lemma}

\begin{proof}
Consider the ``conductor'' ideal
$$
J = \{ g \in S \mid gS \subset \text{Im}(\varphi) \}.
$$
...
First, assume $J \not\subset \mathfrak q$.
Pick $g \in J$, $g\not\in \mathfrak q$.
By definition $g = \varphi(f)$ for some
$f \in R[x]$. Of course $f \not\in \mathfrak p$.
Then $R[x]_f \to S_g = S_f$ is surjective, by definition
of the conductor ideal. Hence we win.

\medskip\noindent
The second case is when $J \subset \mathfrak q$.
Let $\mathfrak q' \subset S$ be a prime,
$J \subset \mathfrak q' \subset \mathfrak q$,
minimal over $J$. Let $\mathfrak p' \subset R[x]$
be the corresponding prime of $R[x]$.
Let $\mathfrak m' \subset \mathfrak m \subset R$
be the corresponding prime ideals of $R$.
We claim that the image of
$x$ is transcendental in
$\kappa(\mathfrak q')$ over $\kappa(\mathfrak m')$.
Assume this is not so in order to produce a contradiction.
In this case $\kappa(\mathfrak q')$ is algebraic over
$\kappa(\mathfrak m')$. Thus there exists a
monic polynomial $f = x^d + \sum_{i<d} a_i x^i$
with $a_i \in \kappa(\mathfrak m')$ which maps to
zero in $\kappa(\mathfrak q')$. Picking a common
denominator, and clearing denominators, we find
a polynomial $f = \sum_{i \leq d} a_ix^i$
with $a_d \not \in \mathfrak m'$ such that
$\varphi(f) \in \mathfrak q'$. Because 
$\mathfrak q'$ is minimal over $J$ we have
$\sqrt{JS_{\mathfrak q'}} = \mathfrak q' S_{\mathfrak q'}$.
Hence there exists an $n \geq 1$ such that
$\varphi(f^n) \in JS_{\mathfrak q'}$. In other
words, for some $y \in S \setminus \mathfrak q'$
we have $y \varphi(f^n) \in J$, in other words
$y\varphi(f^n)S \subset \text{Im}(\varphi)$.
By Lemma \ref{}
