...
Let $k \subset K$ be a separable finitely generated field extension.
...
\begin{proposition}
\label{proposition-characterize-separable-field-extension}
Let $k \subset K$ be a field extension.
The following are equivalent:
\begin{enumerate}
...
with Proposition \ref{proposition-characterize-separable-field-extension}.
\end{proof}
...
separable by
Proposition \ref{proposition-characterize-separable-field-extension}.
\end{proof}
...
\noindent
In this section we show that fields are formally smooth
over their prime subfields.

\begin{lemma}
\label{lemma-formally-smooth-extensions}
...
by combining Lemmas \ref{lemma-formally-smooth-extensions}
and \ref{lemma-compose-formally-smooth}.

\medskip\noindent
Assume $k$ has characteristic $p > 0$. (The method of proof here
will also work in characteristic $0$.)
...
Lemmas \ref{lemma-formally-smooth-extensions}
...
\longrightarrow 0
$$
of $K$ as a $k$-algebra. We claim that the sequence
$$
J/J^2 \longrightarrow
\bigoplus\nolimits_{\lambda \in K} K\text{d}x_\lambda \longrightarrow
\Omega_{K/k} \longrightarrow 0
$$
(see Lemma \ref{lemma-differential-seq})
is exact on the left also. Namely, consider any finitely generated
subextension $k \subset K' \subset K$. Then we get a commutative
diagram
$$
\xymatrix{
J'/(J')^2 \ar[r] \ar[d] &
...
by saying that $R$ is complete and separated.}.
\end{definition}

\noindent
Warning: It can happen that the completion $\text{lim}\ R/\mathfrak m^n$
of a local ring is non-complete.
...
\medskip\noindent
Suppose that $I$ is a totally ordered set, and
...
that $(\hat\Lambda', k', \phi')$ is an ``upper bound'' for the
system. Thus an application of Zorn's Lemma to the set of
systems as above shows that $k$ has a Cohen ring. Some details omitted.
\end{proof}
...
Let $(R, \mathfrak m)$ be a complete local ring.
\begin{enumerate}
\item $R$ has a coefficient ring $\Lambda$,
\item in case (3) above the coefficient ring
is of the form $W/p^nW$ where $W$ is a Cohen ring,
...
Let $R$ be a domain. If $R$ is N-1 then so is any localization of $R$.
Same for N-2.
\end{lemma}
...
type ring map $R \to S$ with $S$ a domain we have that $S$ is Japanese.
...
A Nagata ring is universally Japanese.
\end{proposition}
...
holds for $R \to S$ as well. The proof is completely analogous.
\end{proof}
