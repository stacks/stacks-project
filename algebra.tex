...
\begin{lemma}
\label{lemma-global-complete-intersection}
...
Let $\mathfrak q$ be a prime of $S$, let $\mathfrak p \subset R$
be the corresponding prime of $R$, and let $\mathfrak q'$
...
$R[x_1, \ldots, x_n]_{\mathfrak q'}/(f_1)$ is flat over $R_{\mathfrak p}$.
...
$$
\item for $m \geq j \geq 1$ the element $f_j$ maps to
$x_{n + j}$ in $\kappa(\mathfrak p)[x_1, \ldots, x_{n + m}]$, and
\item the element $f_i$ maps to a unit times
...
$$
\Psi' : R_{\mathfrak p}[x_1, \ldots, x_n]
\longrightarrow
S_{\mathfrak q}
$$
and $f_1', \ldots, f_e' \in R_{\mathfrak p}[x_1, \ldots, x_n]$
with all the required properties. Then there exists an element
$f \in R$, $f \not \in \mathfrak p$ such that
$ff_j'$ comes from an element $f_j \in R[x_1, \ldots, x_n]$.
...
We may write $R \to S$ as a colimit of local ring maps
$\text{colim}_{\lambda \in \Lambda}\ (R_\lambda \to S_\lambda)$
as in Lemma \ref{lemma-limit-no-condition-local}.
Denote $\mathfrak p_\lambda$ the maximal ideal of $R_\lambda$
and denote $\mathfrak q_\lambda$ the maximal ideal of $S_\lambda$.
Let $\mathfrak n_\lambda \subset \kappa(\mathfrak p_\lambda)[x_1, \ldots, x_n]$
be the inverse image of $\mathfrak n$.
We may pick $\lambda$ large enough so that
\begin{enumerate}
\item $\psi(x_i)$ corresponds to an element
of $S_\lambda/\mathfrak p_\lambda S_\lambda$,
\item each $\overline{f}_i$ corresponds to an element
of $\kappa(\mathfrak p_\lambda)[x_1, \ldots, x_n]_{\mathfrak n_\lambda}$, and
\item each $\overline{f}_i$ maps to zero under the induced map
$$
\psi_\lambda :
\kappa(\mathfrak p_\lambda)[x_1, \ldots, x_n]_{\mathfrak n_\lambda}
\longrightarrow
S_\lambda/\mathfrak p_\lambda S_\lambda
$$
\end{enumerate}
The induced map $\psi_\lambda$ may not be surjective.
Choose elements $y_1, \ldots, y_m \in S_\lambda$
such that $S_\lambda$ is a localization of the
$R_\lambda$-sub algebra generated by $y_1, \ldots, y_m$.
We may write $y_j = y'_j + \psi_\lambda(h_j)$ with
$y'_j \in \mathfrak p_\lambda S_\lambda$. The induced homomorphism
\begin{eqnarray*}
\left(
\kappa(\mathfrak p_\lambda)[x_1, \ldots, x_n, x_{n + 1}, \ldots, x_{n + m}]
\right)_{\mathfrak n_\lambda, x_{n + 1}, \ldots, x_{n + m}}
& \longrightarrow &
S_\lambda/\mathfrak p_\lambda S_\lambda \\
x_i & \longmapsto & \psi_\lambda(x_i) \\
x_{n + j} & \longmapsto & y_j'
\end{eqnarray*}
is surjective. 
...
\item The ring map $R \to S$ is of finite presentation,
...
\begin{proof}
Let $S = R[x_1, \ldots, x_n]/I$ be an arbitrary presentation.
Let $\mathfrak q' \subset R[x_1, \ldots, x_n]$
be the prime corresponding to $\mathfrak q$.

\medskip\noindent
The implication (1) $\Rightarrow$ (3) is clear.
Let us prove the converse. Assume (3).
By Theorem \ref{theorem-openess-flatness} there exists a
$g \in S$, $g \not \in \mathfrak q$ such that
$S_g$ is flat over $R$. By condition 
