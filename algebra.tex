...
Lemma \ref{lemma-completion-faithfully-flat} often gets used
to show that $\bigcap_{n \in \mathbf{N}} I^n = (0)$
when $I \subset R$ is a non-unit ideal in a Noetherian
local ring $R$. Namely, if $x \in \bigcap_{n \in \mathbf{N}} I^n$,
then $x$ maps to zero in $\hat R$ which implies $x = 0$
because $R \to \hat R$ is faithfully flat.

\medskip\noindent
More generally, let $R$ be a Noetherian ring and
$I \subset R$ an ideal. Suppose that $f \in \bigcap_{n \in \mathbf{N}} I^n$.
Then we see that for every prime ideal $I \subset \mathfrak p$
the image of $f$ in the localization $R_{\mathfrak p}$. In other
words, there exists a $g \in R$, $g \not \in \mathfrak p$
