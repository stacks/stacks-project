...
Hence, by Lemma \ref{}, the sequence $f_1,\ldots,f_d$
...
\begin{proof}
Immediate from Lemma \ref{lemma-integral-no-inclusion} and the definitions.
\end{proof}
...
It has global dimension equal to the number of variables.
\end{proposition}
...
be prime ideals.
Let $r = \text{trdeg}(\kappa(\mathfrak p)/k)$ and
$s = \text{trdeg}(\kappa(\mathfrak q)/k)$. Then
\begin{enumerate}
\item $r \geq s$ with equality if and only if $\mathfrak p =
\mathfrak q$,
\item
$\dim (k[x_1,\ldots,x_n]/\mathfrak p) = r$ and similarly for $\mathfrak q$,
\item
$\dim (k[x_1,\ldots,x_n]_{\mathfrak p}) = n - r$ and similarly
for $\mathfrak q$, and
\item every maximal chain of primes 
$\mathfrak p = \mathfrak p_0 \subset \mathfrak p_1 \subset \ldots
\subset \mathfrak p_e = \mathfrak q$
between $\mathfrak p$ and
$\mathfrak q$ has length $e = r - s$.
\end{enumerate}
\end{lemma}

\begin{proof}
First pick $k[y_1,\ldots,y_{r'}] \subset k[x_1,\ldots,x_n]/\mathfrak p$
