...
on the order of the elements $f_1,\ldots,f_r$.
...
\begin{lemma}
Let $R$ be a Noetherian local ring.
Suppose that $J = (f_1,\ldots,f_c)$ is an ideal generated
by a regular sequence $f_1,\ldots,f_c$. Then
the graded ring $\bigoplus J^n/J^{n+1}$ is 
graded isomorphic to $R/J[X_1,\ldots,X_c]$.
\end{lemma}

\begin{proof}
By induction on $c$. We have to show that given any relation
$\sum_{|I| = n} a_I f^I \in J^{n + 1}$ with $a_I \in R$ we
actually have $a_I \in J$ for all multiindices $I$. Since
...
A finitely presented ring map $R \to S$ is called a local complete
intersection if 
