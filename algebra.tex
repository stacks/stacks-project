...
Set $X = \text{Spec}(S)$.
Let $k \subset K$ be a field extension.
Set $X_K = \text{Spec}(K \otimes_k S)$.
Let $\mathfrak q \subset S$ be a prime corresponding to $x \in X$
and let $\mathfrak q_K \subset K \otimes_k S$ be a prime corresponding
...
\begin{lemma}
\label{lemma-formally-smooth-finite-type-field}
Let $k$ be a field. Let $S$ be a $k$-algebra.
If $S$ is of finite type and formally smooth over $k$
then $S$ is smooth over $k$.
\end{lemma}
...
\begin{enumerate}
\item $\dim_x X = d$,
\item $\Omega_{S/k, \mathfrak q}$ is free of rank $d$.
\end{enumerate}
Then $S$ is smooth at $\mathfrak q$ over $k$.
\end{lemma}

\begin{proof}
We may replace $S$ by $S_g$ for some $g \in S$, $g \not \in \mathfrak q$
such that $\dim(X) = d$, all irreducible components of $X$ pass through
$x$ and $\Omega_{S/k}$ is free of rank $d$.
...
the base change of $\Omega_{S/k}$, hence free of rank $d$ over $S_K$.
By Lemma \ref{lemma-dimension-preserved-field-extension}
the dimension of $X_K = \text{Spec}(S_K)$ is $d$.

Moreover, $S \cap \mathfrak m = \mathfrak q$. 
...
\begin{lemma}
\label{lemma-smooth-differentials}
Let $R \to S$ be a ring map.
Let $\mathfrak q \subset S$ be a prime.
Let $d \geq 0$ be an integer.
Assume that
\begin{enumerate}
\item there exists a $g \in S$, $g \not \in \mathfrak q$
such that $S_g$ is of finite presentation over $R$,
\item $\dim_{\mathfrak q}(S/R) = d$,
\item $\Omega_{S/R, \mathfrak q}$ is free of rank
$d$ over $S_{\mathfrak q}$.
\end{enumerate}
The $S/R$ is smooth at $\mathfrak q$.
\end{lemma}

\begin{proof}
By Lemmas \ref{} and \ref{} we may find a $g \in S$,
$g \not \in \mathfrak q$ such that
after replacing $S$ by $S_g$ we have
$\dim(S/R) = d$, and $\Omega_{S/R}$ free of rank $d$.
