...
as an $R$-module as desired. This proves (4).
...
see that each $\mathfrak p_i$ has height $1$.
...
Note that $\mathfrak p_iR^\wedge = \bigcap_{j = 1, \ldots s_i} \mathfrak q_j$
by (2)(b); see e.g.\ the proof of
Lemma \ref{lemma-codimension-1-analytically-unramified}.
By that lemma we also see that $(R^\wedge)_{\mathfrak q_{ij}}$ is regular.
Set $I_{ij} = \text{Ker}(R^\wedge \to (R^\wedge)_{\mathfrak q_{ij}})$.
Since $(R^\wedge)_{\mathfrak q_{ij}}$ is a domain we see that
$I_{ij}$ is a prime ideal.
We conclude that $R^\wedge$ is reduced if $N := \bigcap I_{ij}$
is zero.
...
Pick $x \in \mathfrak m$ in the maximal ideal.
