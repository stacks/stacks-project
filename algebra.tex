...
of Lemma \ref{lemma-differential-seq} is split exact.
\end{lemma}
...
$\sum c_i \text{d}b_i \mapsto \sum \beta(c_i)\text{d}\beta(b_i)$
as a right inverse to the map $\Omega_{S/R}\otimes_S S' \to \Omega_{S'/R}$.
Suppose $\xi = \sum \text{d}a_i \otimes b_i$ with $a_i \in S$ and
$b_i \in S'$ is an element of $\Omega_{S/R}\otimes_S S'$.
Then we can set
$$
\tau(\xi) = \sum \beta(b_i)(a_i - \beta(\alpha(a_i))) \bmod I^2.
$$
To see that this gives a well defined $S$-module homomorphism
$\Omega_{S/R} \otimes_S S' \to I/I^2$ it suffices to show that
$D(x) = x - \beta(\alpha(x)) \bmod I^2$ is an $R$-derivation
from $S$ into $I/I^2$. We omit the verification. Since it is
clear that $\tau$ is a left inverse to
$\text{d} : I/I^2  \to \Omega_{S/R}\otimes_S S'$ we win.
\end{proof}
...
\section{Smooth ring maps; Noetherian case}
...
then the map becomes a split injection. Denote $S' = R[x_1, \ldots, x_n]/I^2$.
Note that there is a presentation (see Lemma \ref{lemma-differential-seq})
$$
I^2/I^4 \to \bigoplus\nolimits_{j = 1}^m S' \text{d}x_j 
\to \Omega_{S'/R} \to 0
$$
and hence
$S \otimes_{S'} \Omega_{S'/R} = \bigoplus\nolimits_{j = 1}^m S\text{d}x_j$.
Thus the map
$$
\text{d} : I/I^2 \to S \otimes_{S'} \Omega^1_{S'/R}
$$
is the same as the map in the naive cotangent complex above.

\medskip\noindent
Let $\mathfrak q' \subset R[x_1, \ldots, x_n]$ be the prime ideal
corresponding to $\mathfrak q$. Set $I = (f_1, \ldots, f_m)$.
Let $N \in \mathbf{N}$ be a large integer.
Consider the ring
$$
...
It is clear that $B'_N$ is an Artinian local ring.
Consider a filtration of the kernel $J_N$ of $B'_N \to B_N$
by $B'_N$-submodules
$$
0 \subset J_{N, 1} \subset J_{N, 2} \subset \ldots \subset J_{N, n(N)} = J_N
$$
...
$$
B'_N  \to B'_N/J_{N, 1} \to B'_N/J_{N, 2} \to \ldots \to B'_N/J_N
= B_N = S_{\mathfrak q}/\mathfrak q^NS_{\mathfrak q}
$$
Applying condition (4) successively to these small extensions we see there
exists a commutative diagram
$$
\xymatrix{
S \ar[r] \ar[rd] & B_N \ar@{=}[r] &
S_{\mathfrak q}/\mathfrak q^NS_{\mathfrak q} \\
R \ar[r] \ar[u] & B'_N \ar@{=}[r] \ar[u] &
S'_{\mathfrak q'} / (\mathfrak q')^N S'_{\mathfrak q'} \ar@{=}[r] \ar[u] &
\left(R[x_1, \ldots, x_n]/(I^2 + (\mathfrak q')^N)\right)_{\mathfrak q'}
}
$$
...
$$
J_N \to B_N \otimes_{B'_N} \Omega_{B'_N/R} \to \Omega_{B_N/R} \to 0
$$
coming from the surjection $B'_N \to B_N$ with kernel $J_N$ (see
