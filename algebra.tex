...
\begin{lemma}
\label{lemma-standard-etale-base-change}
...
\begin{proof}
The following proof is a little round-about and there may be ways to
shorten it.
...
Thus we may assume that $R$ is Noetherian, $R \to S$ is finite
and etale at $\mathfrak q$ (but no longer necessarily etale at all primes).
...
assume that (a) $R$ is Noetherian, (b) $S$ finite over $R$, and
(c) $S = R[x]/I$.
...
$$
for certain irreducible monic polynomials
$\overline{h}_i \in \kappa(\mathfrak p)[x]$ and certain
$e_2, \ldots, e_n \geq 1$. Here the numbering chosen so that
...
Set $f = m^l + h$ where $l \deg(m) > \deg(h)$, and $l \geq 2$ so
that $f$ is monic as a polynomial over $R$.
Set $g = f' = \text{d}f/\text{d}x$.

\medskip\noindent
Step 10. The ring map $R[x] \to S = R[x]/I$ satisfies
\begin{enumerate}
\item it maps $f$ to zero, and
\item it maps $g$ to an element of $S \setminus \mathfrak q$.
\end{enumerate}
...
$$
\overline{f} = \overline{k}^l
\overline{h}_1^{ld_1} \overline{h}_2^{ld_2} \ldots \overline{h}_n^{ld_n}
+
...
$$
and the fact that $\overline{h}_1$ is separable.
