...
f = f_0 + f_1 + \ldots + f_n
$$
is the decomposition of $f$ into homogenous pieces in $S$ then $f_i \in I$
...
define $S_{(\mathfrak p)} \subset S_{\mathfrak p}$ the subring
of elements of the form $r/f$ where $r, f \in S$ are homogeneous
elements of the same degree such that $f \not\in \mathfrak p$.
Given a graded $S$-module $M$ we let
$M_{(\mathfrak p)}$ be the $S_{(\mathfrak p)}$-module
which is a submodule of $M_{\mathfrak p}$ consisting of elements
of the form $x/f$ with $x \in M$ and $f \in S$ homogeneous
of the same degree such that $f \not \in \mathfrak p$.

\medskip\noindent
Furthermore, for $f \in S$ homogenenous of degree $> 0$ we define
$$
...
\item The ring maps $S \to S_f \leftarrow S_{(f)}$ induce
a homeomorphism $D_{+}(f) \to \text{Spec}(S_{(f)})$.
...
the form $V_{+}(I)$ for some homogenous ideal $I \subset S$.
\end{enumerate}
\end{lemma}
