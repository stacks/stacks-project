...
\begin{proof}
The first assertion follows directly from Lemma \ref{lemma-four-rings} above.
...
have dimension $0$. This follows in the same manner as in 
Example \ref{example-factor-polynomials} above. But we can
also see it because $R \to S$ is finite since each $\alpha_i \in S$
...
\begin{remark}
\label{remark-variant-etale-makes-quasi-finite-finite}
There is a variant of the lemma, where the etale ring
map $R \to R'$ has trivial residue field extension
$\kappa(\mathfrak p) = \kappa(\mathfrak p')$
and you drop property (3) from the list of properties.
\end{remark}
