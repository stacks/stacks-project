...
$R$-\textit{bilinear} if for each $x \in M$
...
An abelian group $N$ is called $(A,B)$-bimodule if it is both an 
...
\end{align}
Then, letting $y=\phi_i(x_i)$,\begin{center}
$\nu_i\circ\phi_i(x_i)-\nu_j\circ\phi_j(\mu_{ij}(x_i))=
...
$\beta\in I$ with $i_k\preceq\beta$ for all $k=1,...,n$. Then \begin{center} 
$\sum_{k=1}^n
\iota_{i_k}(x_{i_k})=\sum_{k=1}^n
[\iota_{i_k}(x_{i_k}) - \iota_{\beta}\circ\mu_{i_{k}\beta}(x_k)
+ \iota_{\beta} \circ \mu_{i_{k}\beta}(x_k)]$
\end{center}
Then by linearity, we have \begin{center} $\pi(\sum_{k=1}^n
\iota_{i_k}(x_{i_k}))=\pi\circ\iota_{\beta}(\sum_{k=1}^n 
\mu_{i_{k}\beta}(x_k))=\mu_{\beta}(\sum_{k=1}^n
\mu_{i_{k}\beta}(x_k))$
\end{center}
...
\begin{proof}
An $R$-linear map $\hat{f}\in \text{Hom}(M\otimes N, P)$ corresponds to an 
...
\begin{center} $\frac{1}{s}\otimes m=\frac{1}{st}\otimes
tm=\frac{1}{st}\otimes 0=0$.
\end{center}
...
\indent We used the fact that $a\neq0,1$ to ensure that
...
\begin{proof}
By Proposition \ref{proposition-main-theorem} (see also
Lemma \ref{lemma-quasi-finite-open-integral-closure})
we may assume that $S$ is integral over $k[t_1,\ldots,t_n]$. 
...
Moreover, $\dim_{\mathfrak q}(S) = n - c$.
\end{lemma}
...
Let $\mathfrak q_K$ be a prime of $K \otimes_k S$
...
they hold for the pair $(K\otimes_k S, \mathfrak q_K)$.
\end{lemma}
...
the corresponding prime. Consider the following
...
$$
Since both vertical arrows are flat and since they have
the same fibre rings, we see from
...
a {\it relative global complete intersection} if
$S = R[x_1, \ldots, x_n]/(f_1, \ldots, f_c)$ such that
...
Then $f_1, \ldots, f_c$ is a regular sequence in the ring
$R[x_1, \ldots, x_n]$.
\end{lemma}

\begin{proof}
Let $1 \leq i \leq c$. Assume, to get a contradiction,
that $a \in R[x_1, \ldots, x_n]$ with $a \not \in (f_1, \ldots, f_{i - 1})$
but $f_ia \in (f_1, \ldots, f_{i - 1})$. Choose a prime ideal
$\mathfrak q \subset S$ such that $a$ maps to a nonzero
element of $S_{\mathfrak q}$. 
...
Let $R \to S$ be a ring map.
Assume that $R \to S$ is a relative global complete intersection.
\begin{enumerate}
\item For any $R \to R'$ the base change
$R' \to R' \otimes_R S$ is a relative
global complete intersection.
\item For any $g \in S$ the map $R \to S_g$ is a relative
global complete intersection.
\item If $R \to S$ factors as $R \to R_f \to S$ for some $f \in R$.
Then $R_f \to S$ is a relative global complete intersection.
\end{enumerate}
\end{lemma}
...
For (2) we use that if $S$ is isomorphic to
$R[x_1, \ldots, x_n]/(f_1, \ldots, f_c)$
and $g$ is the image of $g' \in R[x_1, \ldots, x_n]$
then
$S_g \cong R[x_1, \ldots, x_n, x_{n + 1}]/(f_1, \ldots, f_c, x_{n + 1}g' - 1)$.
...
exists a $g \in S$, $g \not \in \mathfrak q$
such that $S_g$ is a relative global complete
intersection over $R$.
\end{lemma}
...
Let $R$ be a ring.
Let $S$ be a relative global complete intersection.
Let $\mathfrak q \subset S$ be a prime.
There exist an element $g \in S$, $g \not \in \mathfrak q$,
a Noetherian ring $R_0$, a ring map $R_0 \to R$,
and a relative global complete intersection $S_0$ over $R_0$
such that $S_g \cong R\otimes_{R_0} S_0$ as
$R$-algebras.
\end{lemma}

\begin{proof}
Write $S = R[x_1, \ldots, x_n]/(f_1, \ldots, f_c)$ as in
...
intersection. Since $R \otimes_{R_0} (S_0)_g \cong S_g$ we win.
\end{proof}
...
\medskip\noindent
We first reduce to the case that $R$ is Noetherian.
Write $S = R[x_1, \ldots, x_n]/(f_1, \ldots, f_c)$ as in
Definition \ref{definition-relative-global-complete-intersection}.
Let $R_0 \subset R$ be the $\mathbf{Z}$-algebra of $R$
generated by all the coefficients of the polynomials
$f_1, \ldots, f_c$. Let $S_0 = R_0[x_1, \ldots, x_n]/(f_1, \ldots, f_n)$.
Clearly, $S = R \otimes_{R_0} S_0$.
Let $\mathfrak q \subset S$ be a prime of $S$ lying over
the prime  $\mathfrak p \subset R$, and
let $\mathfrak q_0 \subset S_0$ be the corresponding prime
lying over the prime $\mathfrak p_0$ of $R_0$.
Because $\dim (S \otimes_R \kappa(\mathfrak p) ) = n - c$
...
conclude that there exists a $g \in S_0$, $g \not \in \mathfrak q_0$
such that $R_0 \to (S_0)_g$ is a relative global complete
intersection. If we show that $R_0 \to (S_0)_g$ is flat at
$\mathfrak q_0$, then $R \to S$ will be flat at $\mathfrak q$,
see Lemma \ref{lemma-flat-base-change}. Thus we have reduced
to the case where $R$ is Noetherian.

\medskip\noindent
Let $\mathfrak q \subset S$ be a prime of $S$ lying over
$\mathfrak p \subset R$, and let $\mathfrak q'$
be the corresponding prime of $R[x_1, \ldots, x_n]$.
We will show that $S_{\mathfrak q}$ is flat over $R_{\mathfrak p}$.
By Lemma \ref{lemma-lci} for example we see that $f_1, \ldots, f_c$
form a regular sequence in the local ring
$R[x_1, \ldots, x_n]_{\mathfrak q'}\otimes \kappa(\mathfrak p)$.
Moreover, the local ring $R[x_1, \ldots, x_n]_{\mathfrak q'}$
is flat over $R_{\mathfrak p}$. 
By Lemma \ref{lemma-grothendieck} the local ring
$R[x_1, \ldots, x_n]_{\mathfrak q'}/(f_1)$ is flat
over $R_{\mathfrak p}$. (This is where we use $R$ is Noetherian.)
We may apply Lemma \ref{lemma-grothendieck} again because
$f_2$ is not a zero divisor in the ring
$R[x_1, \ldots, x_n]_{\mathfrak q'}/(f_1) \otimes \kappa(\mathfrak p)$.
And so on.
\end{proof}
...
\begin{proof}
Consider a presentation $\alpha : S = R[x_1, \ldots, x_n]/I$.
A presentation of $S_g$ over $R$ is gotten from $\alpha$
by taking $\alpha' : S_g = R[x_1, \ldots, x_n, x_{n + 1}]/J$ with
$J = (I, g' x_{n + 1} - 1)$. Here $g'$ is a polynomial 
that maps to $g$ in $S$. In this case $J/J^2
\cong (I/I^2)_g \oplus S_g \cdot (g' x_{n + 1} - 1)$.
...
$$
which proves that $NL(\alpha')$ is quasi-isomorphic to
$\Omega_{S/R} \otimes_S S_g$. Since $\Omega_{S/R}$ over $S$
is finite projective, so is its base change.
...
\right).
$$
Set $S = R[x_1, \ldots, x_{n + 1}]/(f_1, \ldots, f_c, x_{n + 1}g - 1)$.
...
\begin{proof}
Assume $g = g_I$ does not end up in $\mathfrak q$.
Then the algebra $S_g$ is standard smooth over $R$.
(Verfification omitted, but similar to
Example \ref{example-make-standard-smooth}.)
Conversely, suppose that all $g_I$ end up in $\mathfrak q$.
In this case there is no way that $I/I^2$
\end{proof}
...
relatve global complete intersection. Thus we may assume
...
g_I = \det (\partial f_j/\partial x_i)_{j = 1, \ldots, c, i \in I}.
$$
Because $S \otimes \kappa(\mathfrak p)$ is smooth at the point
corresponding to $\mathfrak q$ 
