...
\begin{definition}
\label{definition-directed-limit}
...
$M=\underrightarrow{\lim} M_i$, $\pi:\bigoplus_{i\in I} M_i \rightarrow M$ the 
projection map and $\phi_{i}=\pi \circ
\iota_{i}:M_i\rightarrow M$.
\end{definition}

\noindent
Note that $\phi_{i}=\phi_{j}\circ \mu_{ij}$ in the above construction. The 
direct limit, realized as the pair
$(M,\phi_{i})$, satisfies the universal property: for any other such pair 
...
Let $M=\underrightarrow{\lim}M_i$
with $\mu_i:M_i\rightarrow M$, then $\mu_i(x_i)=0$ 
for $x_i\in M_i$ if and only if
...
$\phi=\underrightarrow{\lim}\phi_{i}:M\rightarrow N$ such that $\phi \circ 
...
\pi_2:\bigoplus_{i\in I}N_i\rightarrow N$ are projection maps, $\iota_i:M_i\rightarrow 
...
More precisely, let 
$M=\underrightarrow{\lim}{M_i}$,
and $N$ an $R$-module. Then $(M_i\otimes N, \mu_{ij}\otimes 1)$ is a direct 
system. Then
\begin{align}
\underrightarrow{\lim}(M_i\otimes N) \cong
(\underrightarrow{\lim}M_i)\otimes N.
\end{align}
...
\begin{proof}
Let $P=\underrightarrow{\lim}(M_i\otimes N)$, $M=\underrightarrow{\lim}M_i$. 
...
because of the relations $M'/I^nM' \subset M/I^nM' \to M/I^nM$.
\end{proof}
