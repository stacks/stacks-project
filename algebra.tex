...
and $D(g)$ is a finite discrete set. Hence this is equivalent to (1).
...
combine (7) above with Lemma \ref{lemma-finite-presentation-fs-Noetherian}.
...
\noindent
In particular we see that $\Omega_{S/R} = 0$. If $R \to S$ is smooth,
...
\noindent
The following lemma basically says roughly that after an etale extension
a quasi-finite ring map becomes finite.
To interpret the result recall that the locus where a
...
$$
S' = R' \otimes_R S = A_1 \times \ldots \times A_n \times B
$$
\end{enumerate}
...
\begin{proof}
Let $\mathfrak q_1, \ldots, \mathfrak q_n$ be the distinct primes of
$S$ lying over $\mathfrak p$ at which the morphism $R \to S$ is
quasi-finite. There are finitely many because the ring
$S \otimes_R \kappa(\mathfrak p)$ is of finite type over
$\kappa(\mathfrak p)$, hence Noetherian, hence has finitely many
irreducible components of dimension $0$.
Let $S' \subset S$ be the integral closure
of $R$ in $S$. Let $\mathfrak q'_i = S' \cap \mathfrak q_i$,
$i = 1, \ldots, n$.
By Zariski's Main Theorem, for each $i$ there exists an element $g_i \in S'$,
$g_i \not \in \mathfrak q_i'$ such that $S'_{g_i} \cong S_{g_i}$.
Set $I = (g_1, \ldots, g_n) \subset S'$. We claim:
\begin{enumerate}
\item $\mathfrak q'_1, \ldots, \mathfrak q'_n$ are pairwise distinct,
\item $I$ is not contained in $\mathfrak q_i$ for any $i$,
\item for any prime $\mathfrak q$ of $S$ lying over $\mathfrak p$
at which $R \to S$ is not quasi-finite  we have $I \subset \mathfrak q$, and
\item any prime $\mathfrak q' \subset S'$ lying over
$\mathfrak p$ is an element of
$\{\mathfrak q'_1, \ldots, \mathfrak q'_n\} \cup V(I)$.
\end{enumerate}
Let us show $\mathfrak q'_1 \not = \mathfrak q'_2$.
If $g_1 \in \mathfrak q'_2$, then we are done.
Thus from $S'_{g_1} \cong S_{g_1}$ and the fact that $\mathfrak q_1$,
$\mathfrak q_2$ define distinct points of $\text{Spec}(S_{g_1})$
(see Lemma \ref{lemma-standard-open}) we conclude
$\mathfrak q'_1 \not = \mathfrak q'_2$ as desired. Thus part (1)
of the claim holds. Part (2) is clear since $g_i \not \in \mathfrak q_i$.
Suppose $\mathfrak q$ is a prime of $S$ lying
over $\mathfrak p$ at which $R \to S$ is not quasi-finite.
Note that each $S'_{g_i} = S_{g_i}$ is quasi-finite over $R$.
Hence $g_i \in \mathfrak q$ for all $i$. Hence $I \subset \mathfrak q$.
This proves part (3) of the claim. Part (4) holds for the same reason:
if $\mathfrak q' \subset S'$ lies over $\mathfrak p$ and is not
contained in $V(I)$, then $g_i \not \in \mathfrak q'$ for some
$i$, and hence $\mathfrak q'$ corresponds to a prime $\mathfrak q$
of $S$ at which $S/R$ is quasi-finite, i.e., $\mathfrak q = \mathfrak q_{i'}$
for some $i'$ and hence $\mathfrak q' = \mathfrak q'_{i'}$ for some $i'$.

\medskip\noindent
For each $i$ we may choose an element $t_i \in S'$ such that
$t_i \not \in \mathfrak q'_i$, $t_i \in I$, and
$t_i \in \mathfrak q'_j$ for all $j \not = i$ (see Lemma \ref{lemma-silly}).
Let $f_i(x) \in R[x]$ be a monic polynomial such that
$f_i(t_i) = 0$ (exists because $S'$ integral over $R$).
Denote $\overline{f}_i$ the image of $f_i$ in $\kappa(\mathfrak p)[x]$.
We can factor $\overline{f}_i = x^{e_i} \overline{h}_i$
with $\overline{h}_i \in \kappa(\mathfrak p)[x]$ such that
$\overline{h}_i(0) \not = 0$.

\medskip\noindent
Suppose $R \to R'$ is etale, and $\mathfrak p' \subset R'$ is a prime lying
over $\mathfrak p$ such that $\kappa(\mathfrak p) = \kappa(\mathfrak p')$.
We may replace the system
$(R \to S, \mathfrak p, \mathfrak q_i, S', \mathfrak q'_i, g_i, I, t_i, f_i)$
by the system
$(R' \to R' \otimes_R S, \mathfrak p',
R' \otimes \mathfrak q_i + \mathfrak p' \otimes S, R' \otimes_R S',
R' \otimes \mathfrak q'_i + \mathfrak p' \otimes S', 1 \otimes g_i,
I(R' \otimes_R S'), 1 \otimes t_i, f_i)$.
This preserves all the properties of the system
$(R \to S, \mathfrak p, \mathfrak q_i, S', \mathfrak q'_i, g_i, I, t_i, f_i)$
except that $S'$ may no longer be the integral closure of $R$
in $S$ (but it is still integral over $R$ of course).
Thus we may succesively apply Lemma \ref{lemma-factor-mod-lift-etale}
for each of the factorizations $\overline{f}_i = x^{e_i} \overline{h}_i$
and succesively replace $R$ each time.

\medskip\noindent
At the end of this sequence of replacements, we have $f_i = k_i h_i$ and
$1 = a_i k_i + b_i h_i$ in $R[x]$, and the image of $k_i$ in
$\kappa(\mathfrak p)[x]$ is $x^{e_i}$. Since $f_i(t_i) = 0$ this implies that
$\text{Spec}(S') = D(k_i(t_i)) \coprod D(h_i(t_i))$, and similarly
for $\text{Spec}(S)$. Note that the only prime of $S'$ lying
over $\mathfrak p$ contained in $D(h_i(t_i))$ is $\mathfrak q'_i$.
...
\medskip\noindent
By Lemma \ref{} we may replace $(R, \mathfrak p)$ by $(R', \mathfrak p')$
and assume that $f_i = g_i h_i$ with $g_i \bmod \mathfrak p = x^n$
and $h_i(0) \bmod \mathfrak p \not = 0$.
...
By Lemma \ref{lemma-silly} there exists a $g \in I$,
$g \not \in \mathfrak q_i'$, $i = 1, \ldots, n$.
Then the ring map $S'_g \to S_g$ is an isomorphism since each of
the localizations $S'_{gg_i} \to S_{gg_i}$ is an isomorphism
and $IS'_g = S'_g$, see Lemma \ref{lemma-cover}.
...
$g \not \in \mathfrak q_i$, $i = 1, \ldots, n$
such that $R \to S_g$ is quasi-finite. Such an element exists
because the quasi-finite locus is open in $\text{Spec}(S)$
(see Lemma \ref{lemma-quasi-finite-open})
and Lemma \ref{lemma-silly}. By
Lemma \ref{lemma-quasi-finite-open-integral-closure} there exists
a finite ring map $R \to S'$ and an $R$-algebra map
