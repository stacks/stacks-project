...
In this case, if $x \not \in B$, then we can find a $t \in A$
such that $tx \in A$ and we see that it suffices to prove the
lemma for $ty$ (with $Nm(ty) = t^{[L : k]}x \in A$ and $t$
(with norm in $A$ also). Hence we may assume $y \in B$
such that $x \in A$. In this case
