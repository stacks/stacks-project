...
$$g_2Y=\left(\begin{matrix}
0 & 0 \\
y_{21}' & y_{22}'
\end{matrix}\right).$$ With a further $GL(2,k)$-action on the right by $g_3$,
$g_2Y$ can be brought into $$g_2Yg_3^{-1}=\left(\begin{matrix}
0 & 0 \\
0 & 1
\end{matrix}\right),$$ and thus such $Y$'s form an irreducible algebraic set
...
\begin{lemma}
\label{lemma-quasi-finite-isolate-prime}
Let $R \to S$ be a ring map. Let $\mathfrak q \subset S$ be a prime.
Assume that there exists a $g \in S$, $g \not \in \mathfrak q$
such that $R \to S_g$ is of finite type and quasi-finite at $\mathfrak q$.
Then there exists a $g \in S$, $g \not \in \mathfrak q$ such that
$R \to S_g$ is finite type, quasi-finite at $\mathfrak q$ and
$S_g \otimes_R \kappa(\mathfrak p)$ is a local ring (i.e., $\mathfrak q$
is the only prime of $S_g$ lying over $\mathfrak p$).
\end{lemma}

\begin{proof}
Let $g$ be as in the hypothesis of the lemma.
Let $\overline{\mathfrak q}$ be the prime of
$S_g \otimes_R \kappa(\mathfrak p)$ corresponding to $\mathfrak q$.
By assumption the subset $\{\overline{\mathfrak q}\}$ of the spectrum of
$S_g \otimes_R \kappa(\mathfrak p)$ is open and closed. Since the
topology of $\text{Spec}(S_g \otimes_R \kappa(\mathfrak p))$ is
the induced topology from $\text{Spec}(S)$, see
Remark \ref{remark-fundamental-diagram}, there exists a $g' \in S$
such that $D(g')$ restricts to the open $\{\overline{q}\}$
of $\text{Spec}(S_g \otimes_R \kappa(\mathfrak p))$.
Then $gg'$ works.
\end{proof}
...
\noindent
The lemma tells us that it does not really
...
\begin{lemma}
\label{lemma-etale-locally-standard}
Let $R \to S$ be a ring map.
Let $\mathfrak q \subset S$ be a prime.
...
\begin{proof}
By assumption there exists a $g \in S$, $g \not \in \mathfrak q$
such that $R \to S_g$ is etale. We may replace $S$ by $S_g$.
Then $R \to S$ is quasi-finite, see Lemma \ref{lemma-etale-quasi-finite}
and standard smooth, see Lemma \ref{lemma-etale-standard-smooth}.
Write $S = R[x_1, \ldots, x_n]/(f_1, \ldots, f_c)$.
As $R \to S$ is quasi-finite we have $n =  c$.
We will prove the lemma by induction on $n$.

\medskip\noindent
If $n = 1$, then we have $S = R[x]/(f)$ with $\gcd(f, f') = 1$.
FIXME.
...
there exists a $g \in S'$, $g \not \in \mathfrak q'$ such that
$S'_g \cong S_g$.
Denote $\mathfrak p$ the prime of $R$ lying under $\mathfrak q$.
By Lemma \ref{lemma-quasi-finite-isolate-prime}
we may assume that $\mathfrak q'$ is the only prime of $S'_g$
lying over $\mathfrak p$.

\medskip\noindent
Consider $\overline{S}' = S' \otimes_R \kappa(\mathfrak p)$.
This is a ring integral over $\kappa(\mathfrak p)$.
The element $g$ has the property that $\overline{S}'_g$ is local,
in fact it is isomorphic to $\kappa(\mathfrak q)$ which is a finite
separable extension of $\kappa(\mathfrak p)$, see
Lemma \ref{lemma-etale-at-prime}. Hence we see that
$\overline{S}' = \kappa(\mathfrak q) \times \overline{S}''$ for
some $k$-algebra $\overline{S}''$, see
Lemma \ref{lemma-disjoint-implies-product}.
Let $\alpha \in \kappa(\mathfrak q)$ be a nonzero element which generates
$\kappa(\mathfrak q)$ over $\kappa(\mathfrak p)$.
Consider the element
$\overline{t} = \alpha \times 1 \in
\kappa(\mathfrak q) \times \overline{S}''$.
Since $\overline{S}' = S' \otimes_R \kappa(\mathfrak p)$
we see there exists a $t \in S'$ whose image in $\overline{S}'$
is equal to $\overline{t}/\overline{r}$ for some
$\overline{r} \in R/\mathfrak p$.
