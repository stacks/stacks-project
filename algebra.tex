...
$e$ maps to $1$ in $\kappa(\mathfrak p)$ for all
$\mathfrak p \in U$ and maps to $0$ in
$\kappa(\mathfrak p)$ for all $\mathfrak p \not\in U$.
This induces a 1-1 correspondence between
...
over a field or over $\mathbf{Z}$. We will show later that these
are examples of Jacobson rings. Here we just give the definition
and a few simple lemmas.
...
points in $Z$ is dense in $Z$. If not then there exists
an open $U \subset \text{Spec}(R)$ such that $U \cap Z$
is nonempty and does not contain a closed point. We may
...
Hence $\mathfrak m \in D(f) \cap V(I) = U \cap Z$
...
\cdot \mathfrak m_n \not= 0$.
In particular a discrete valuation ring is not a Jacobson
ring.
\end{example}

\noindent
It will be shown later that any finitely generated algebra
over a Jacobson ring is Jacobson.
