...
$$
\text{Spec}(R) = \bigcup_{i = 1}^n D(f_i)
$$
...
0 \longrightarrow
R \longrightarrow
\bigoplus_i R_{f_i} \longrightarrow
\bigoplus_{i,j}R_{f_if_j}
\]
...
\[
1=\sum_{i=1}^n a_i\widetilde{f_i}
\]
Therefore, we finally conclude that for all $i$,
\[
\frac{x_i}{f_i} =
\frac{\widetilde{x_i}}{\widetilde{f_i}} =
\sum_{j=1}^n\frac{a_j\widetilde{f_j}\widetilde{x_i}}{\widetilde{f_i}} =
\sum_{j=1}^n\frac{a_j\widetilde{f_i}\widetilde{x_j}}{\widetilde{f_i}} =
\frac{\sum_{j=1}^na_j\widetilde{x_j}}{1}.
\]
Thus, we have
\[
\alpha\left(\sum_{j=1}^na_j\widetilde{x_j}\right) =
...
\noindent
In the situation of Lemma \ref{lemma-dimension-height-polynomial-ring}
above we have: If $\mathfrak q$ is
maximal then any maximal chain of primes between $\mathfrak p$ and
$\mathfrak q$ has length $n - \text{height}(\mathfrak p)$.
...
components of $\text{Spec}(S)$ have dimension $n - c$.
...
which generate $I/I^2_{\mathfrak q}$
...
By the Hilbert-Nullstellensatz $[\kappa : k] < \infty$.
Hence $K \otimes_k \kappa$ is finite over $K$ hence
any maximal ideal $\overline{\mathfrak n} \subset K \otimes_k \kappa$
has a residue field which is finite over $K$. Let $\mathfrak n \subset
S'$ be the corresponding maximal ideal of $S'$, and let
$\mathfrak n' \subset K[x_1, \ldots, x_n]$ denote the corresponding
maximal ideal of $K[x_1, \ldots, x_n]$. 
...
$NL(\alpha) \otimes_S S' = NL(\alpha')$. Thus
$I'/(I')^2 = I/I^2 \otimes_S S'$. Since $(I'/(I')^2)_{\mathfrak n}$
is free of rank 
