...
elements $m_n \in M/I^nM$ such that $m_n \cong m_{n + 1} \bmod I^nM$
...
The completion $\hat R$ of $R$ with respect to $I$ is
Noetherian.
\end{lemma}
...
Consider the map
$$
R[[x_1, \ldots, x_n]] \longrightarrow \hat R,
\quad
x_i \longmapsto f_i.
$$
...
\noindent
Warning: It can happen that the completion $\text{lim}_n\ R/\mathfrak m^n$
of a local ring is non-complete (this does not happen for Noetherian rings).
