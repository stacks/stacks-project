...
is a Noetherian local ring. For any $R$-module $M$
...
\label{lemma-NAK}
(Nakayama's lemma.)
If $M$ is a finite nonzero module over $R$, then
$\mathfrak m M \not= M$.
\end{lemma}

\begin{proof}
Here is a silly way to prove this: If $\mathfrak m M = M$ for
$M$ finite then by induction ${\mathfrak m}^nM = M$. Hence the
completion of $M$ with respect to the maximal ideal is zero.
Hence $M \otimes_R \hat R = 0$, see Lemma \ref{lemma-completion-tensor}.
But $R \to \hat R$ is faithfully flat by
Lemma \ref{lemma-completion-faithfully-flat} and hence we
conclude $M = 0$ by Lemma \ref{lemma-easy-ff}.
\end{proof}
...
\begin{proof}
By the previous lemma all the steps in the filtration
...
\begin{proof}
We prove this by induction on the number of generators
of $I$. If this number is $0$, then $I = (0)$
and there is nothing to prove. Suppose that the ideal
$I = (x,x_2,\ldots,x_r)$ can be generated by $r$ elements and
that we have proved the result for all cases where there are
at most $r-1$ generators.

\medskip\noindent
First we show the result holds if $x$ is nilpotent on $M$.
This we do by induction on the minimal integer $r$ such that $x^r M  = 0$.
If $r = 1$, then $M$ is a module over $R/xR$ and the result 
follows from the induction hypothesis for $r-1$ above.
...
the result for $M''$ and any submodule of $M'$. Hence
we get the result for $M$ from Lemma \ref{lemma-hilbert-ses}.

\medskip\noindent
If $x$ is not nilpotent on $M$, let $M' \subset M$ be
the largest submodule on which $x$ is nilpotent. Hence
the result holds for any submodule of $M'$ by the above.
Considering the exact sequence $0 \to M' \to M \to M/M' \to 0$
we see by Lemma \ref{lemma-hilbert-ses} again that
it suffices to prove the result for $M/M'$. In other
words we may assume that multiplication by $x$ is injective.

\medskip\noindent
In this case we have a short exact sequence
$$
0 \to M \to M \to M/xM \to 0.
$$
Denote $\overline{M} = M/xM$. For any $n$ we have
$xI^{n} \subset I^{n+1}$. Hence we have an exact sequence
$$
M/I^{n}M \to M/I^{n+1}M \to \overline{M}/\overline{I}^{n+1}\overline{M} \to 0.
$$
Note that the kernel of the first map is the module
$\{m \in M \mid xm \in I^{n+1}M\}/I^nM$.
Hence we conclude from the above that
$$
\varphi_{I,M}(n)
\leq 
\sum\nolimits_{i=0}^n \varphi_{\overline{I}, \overline{M}}(i)
$$

\medskip\noindent
On the other hand, let $c$ be such that
$xM \cap I^nM = I^{n-c}(xM \cap I^c)$. Set $M' = \{m \in M \mid
xm \in I^c\}$.
Consider the filtration of $M/I^{n+1}M$ given by powers of $x$.
In other words: $0 \subset x^n(M/I^{n+1}M) \subset x^{n-1}(M/I^{n+1}M)
\subset \ldots \subset x (M/I^{n+1}M) \subset M/I^{n+1}M$.
Note that $x^j(M/I^{n+1}M) = (x^jM + I^{n+1}M)/I^{n+1}M$. Thus
the subquotients $x^j(M/I^{n+1}M) / x^{j+1}(M/I^{n+1}M)$
are isomorphic to $(x^jM + I^{n+1}M)/(x^{j+1}M+I^{n+1}M)$.
These modules are quotients of the modules $M/(I^{n+1-j}M + xM)$
via multiplication by $x^j$. Hence we get
$$
\sum\nolimits_{i=0}^n \varphi_{I,M}(i)
\leq
\sum\nolimits_{j=0}^n
\sum\nolimits_{i=0}^{n-j}
\varphi_{\overline{I},\overline{M}}(i)
$$
...
Let $R$ be a local Noetherian ring. Let $x$ be an element 
