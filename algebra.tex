...
\begin{proof}
We have to show that the inverse image of any quasi-compact
...
Hence we win by Lemma \ref{lemma-qc-open} again.
\end{proof}
...
\Leftrightarrow
\text{multiplication by }g\text{ is nilpotent on }
...
geometry have lot's of maximal ideals. For example finite algebras
...
\begin{proof}\footnote{This is an awkward proof.
For a better one see Lemma \ref{lemma-noetherian-dim-1-Jacobson}.}
...
points to closed points, and has the property
that it induces finite residue field extensions on
closed points. Then so does
$\text{Spec}(C) \to \text{Spec}(A)$.
...
is clear by Lemma \ref{lemma-jacobson}
and Topology, Lemma \ref{topology-lemma-jacobson-inherited}.
We will use below that also the case $R \to R_f$
of a localization at a single element is clear
(for the same reason).
...
$b_i \in R$ with $b_e \not= 0$.
The map $R \to R_{b_e}$ satisfies the conclusion of
the lemma (see above). Hence we may invert $b_e$ and assume that
$g$ is monic. In this case we see that $R[x]/\mathfrak q$ is a 
...
Let $L \supset K$ be an extension such that
...
we get a reduced ring $\overline{R}$ and
a polynomial $x^n + \overline{a_{n-1}}x^{n-1} + \ldots +
\overline{a_0}$ which divides $x^m$. It is
easy to see that $\overline{a_i} = 0$ for all $i$,
for example by localizing at all the minimal
primes and using Lemma \ref{lemma-minimal-prime-reduced-ring}.
\end{proof}
...
Then the going down property holds for $R \to S$.
\end{proposition}

\begin{proof}
Let $\mathfrak q' \subset S$ be a prime, and
let $\mathfrak p' = R \cap \mathfrak q'$.
Also let $\mathfrak p \subset \mathfrak p'$ be
a prime. We have to show there is a prime
$\mathfrak q \subset \mathfrak q'$ such that
...
$g^n = \sum_{j < n} a_j g^j \in \mathfrak pS \subset \mathfrak q'$
...
we see that $z^ja_{n - j} \in \mathfrak p$, $j = 
n-1,\ldots, 0$. Because not all $a_i$, $i=n-1,\ldots,0$
...
Let $k$ be a field and let $\mathfrak m \subset k[x_1, \ldots, x_n]$
be a maximal ideal. The field extension $k \subset \kappa(\mathfrak m)$
is finite. The same is true for any maximal ideal in
any finite type $k$-algebra.
\end{theorem}

\begin{proof}
It is enough to prove the theorem for the case of a polynomial
algebra $k[x_1, \ldots, x_n]$, because any finitely generated
$k$-algebra is a quotient of such a polynomial algebra.
We prove the theorem by induction on $n$. The case $n = 0$ is clear.
...
extension by induction hypothesis. Thus $\kappa(\mathfrak m)$
is finite over $k$ as well.

\medskip\noindent
If $\mathfrak p = (0)$ we consider the ring
extension $k[x_n] \subset k[x_1, \ldots, x_n]/\mathfrak m$.
This is a finitely generated ring extension, and hence the
image in $\text{Spec}(k[x_n])$ is constructible by
...
Any Noetherian domain $R$ of dimension $1$
with infinitely many primes is Jacobson.
\end{lemma}

\begin{proof}
Namely, in this case if $x \in R$ is not zero, then $R/xR$ is
a zero dimensional Noetherian ring. Hence it has finitely many
...
This proves that $(0) = \bigcap \mathfrak m$ the intersection
...
to show that $I \otimes_R M \to M$ is injective. By the proof of
\ref{lemma-characterize-flat} we see that this kernel is
...
to its $\mathfrak mS$-adic completion is faitfully flat
...
\begin{lemma}
\label{lemma-flatimplies}
Let $R$ be a local ring with maximal ideal $\mathfrak m$.
Let $R \to S$ be a local homomorphism of local rings.
Suppose that $M$ is a finite $S$-module. Choose any
short exact sequence $0 \to K \to S^n \to M \to 0$.
If $M$ is flat over $R$, then $K/ mK \to (S/\mathfrak mS)^n$
is injective.
\end{lemma}

\begin{proof}
This is immediate from Lemma \ref{lemma-flat-tor-zero}.
\end{proof}

\begin{lemma}
\label{lemma-flatNoetherian}
Let $R$ be a Noetherian local ring with maximal ideal $\mathfrak m$.
Let $R \to S$ be a flat local homomorphism of local rings.
Assume $S$ Noetherian. Suppose that $M$ is a finite $S$-module.
Choose any short exact sequence $0 \to K \to S^n \to M \to 0$.
If $K/\mathfrak mK \to (S/\mathfrak mS)^n$ is injective, then $M$ is
flat over $R$.
\end{lemma}

\begin{proof}
This is immediate from Lemma \ref{lemma-mod-injective}.
\end{proof}

\noindent
Suppose that $R \to S$ is a ring map of finite type.
Let $M$ be a finite $S$-module. In this situation
a {\it presentation of $M/S/R$} will be given by a presentation
$\alpha : R[x_1,\ldots, x_n] \to S$ and a surjection
$\alpha' : R[x_1,\ldots,x_n]^r \to M$. To such a presentation
we may associate the complex of $S$-modules
$$
M(\alpha, \alpha') :
\text{Ker}(\alpha') \otimes_{R[x_1,\ldots,x_n]} S \longrightarrow S^r
$$
Note that the cokernel of this map is canonically identified with
$M$ (by right exactness of the tensor product).
A {\it morphism of presentations of $M/S/R$} from the presentation
$(\alpha : R[x_1,\ldots,x_n] \to S, \alpha' : R[x_1,\ldots,x_n]^r \to M)$
to the presentation
$(\beta : R[y_1,\ldots,y_m] \to S, \beta' : R[y_1,\ldots,y_m]^s \to M)$
is given by a map
$\varphi : R[x_1,\ldots,x_n] \to R[y_1,\ldots,y_m]$
such that $\alpha = \beta \circ \varphi$ together with
a map $\varphi' : R[x_1,\ldots,x_n]^r \to R[y_1,\ldots,y_m]^s$
which is a $\varphi$-linear map such that
$\alpha' = \beta' \circ \varphi'$. This induces a map
of complexes
$$
M(\alpha,\alpha') \longrightarrow M(\beta,\beta').
$$

\begin{lemma}
\label{lemma-Mqis}
Compare with Lemma \ref{lemma-NL-homotopy}.
Any two morphisms of presentations $(\varphi_1,\varphi_1')$
and $(\varphi_2, \varphi_2')$ induce homotopic maps of complexes.
...
$K\otimes_{S'}S \to S^n$
$(K\otimes_{S'}S)/\mathfrak m (K\otimes_{S'}S) \to (S/\mathfrak m)^n$
...
\begin{lemma}
\label{lemma-flatthreerings}
Let $R \to S' \to S$ be local homomorphisms of 
local rings. Let $\mathfrak m \subset R$ be the maximal ideal
of $R$. Assume $S' \to S$ surjective. Let $M$ be a finite
$S$-module. Let $0 \to K' \to (S')^n \to M \to 0$ be
a presentation of $M$ as an $S'$-module. Let $0 \to K
\to S^n \to M \to 0$ be the presentation over
$S$ using the same generators of $M$. 
\end{lemma}
