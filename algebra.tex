...
\begin{lemma}
\label{lemma-image-standard-open}
Let $R$ be a ring. Let $f \in R$. Let
$U \subset D(f)$ be open. The following are equivalent
\begin{enumerate}
\item For some $g \in R$ we have $U = D(g)$.
\item For some $g \in R$ we have $U = D(fg)$.
\item For some $g \in R_f$ the open $U$ is the image of
$D(g) \subset \text{Spec}(R_f)$ under the canonical map
$\text{Spec}(R_f)\to \text{Spec}(R)$.
\end{enumerate}
\end{lemma}

\begin{proof}
The only thing we need to prove it that the third condition
implies the others. Write $g \in R_f$ as $h/f^n$ for some
$h \in R$ and $n \geq 0$. Then it is easy to see
that the image of $D(g) \subset \text{Spec}(R_f)$
is $D(hf) \subset \text{Spec}(R)$.
\end{proof}
...
We claim this is surjective. Once we have seen this the result
