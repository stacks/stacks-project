...
we may assume $S$ is finitely presented over $R$.
By the last statement of Lemma \ref{lemma-lci}
we may find a surjection
$\psi : \kappa(\mathfrak p)[x_1, \ldots, x_n]_{\mathfrak a}
...
is surjective. Consider the induced map
$$
\overline{\Psi} :
R[x_1, \ldots, x_{n + m}]/(f_1, \ldots, f_{c + m})
\longrightarrow
S_g.
$$
By construction it induces an surjection between the localizations
at the primes $\mathfrak b/(f_i)$ and $\mathfrak qS_g$.
On the other hand, both sides are flat over $R_{\mathfrak p}$.
FIXME.
This on the one hand implies that
a localization of
$R[x_1, \ldots, x_{n + m}]/(f_1, \ldots, f_{c + m})$ is
a relative global complete intersection, and on the other hand
that $\overline{\Psi}$ becomes an isomorphism after localizing
both sides at suitable elements (because both sides are
