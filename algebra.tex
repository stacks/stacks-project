...
\noindent
We first introduce the notion of a normal domain, and the we
...
elements $y_j \in M$, $j = 1, \ldots, m$, and elements $g_{ij} \in R$,
...
\begin{enumerate}
\item for every $x \in S$ there exists an $n > 0$ such that
...
\begin{proof}
Let $k \subset k^{perf}$ be an algebraic purely inseparable extension
such that $k^{perf}$ is perfect (actually, $k^{perf} = \bigcup k^{1/p^n}$
is called the perfection of $k$; insert future reference here).
...
Suppose we are given a ring $R$ and a sequence of
$R$-modules
$$
M_1 \xleftarrow{\varphi_2} M_2 \xleftarrow{\varphi_3} M_3 \leftarrow \ldots
$$
with maps as indicated. In this case we define
$$
\text{lim}_i\ M_i
=
\{(x_i) \in \prod M_i \mid \varphi_i(x_i) = x_{i - 1},\ i = 2, 3, \ldots\}.
$$
As explained in Homology, Section \ref{homology-section-inverse-systems}
this is actually a limit in the category of $R$-modules, as defined
in Categories, Section \ref{categories-section-limits}.
...
$$
0 \to \text{lim}_i\ K_i \to \text{lim}_i\ L_i \to \text{lim}_i\ M_i \to 0
$$
is exact.
\end{lemma}
...
Homology, Lemma \ref{lemma-Mittag-Leffler} to the inverse
...
By Homology, Lemma \ref{lemma-Mittag-Leffler} we see that
...
\noindent
The following lemma does not rely the other results in this section.
...
\begin{enumerate}
\item If $S$ is flat over $R$ then $S'$ is flat over $R'$.
\item If $R' \to S'$ and $R \to R'$ are flat then $S$ is flat over $R$.
\end{enumerate}
\end{lemma}

\begin{proof}
Proof of (1). If $S$ is flat over $R$, then $R' \otimes_R S$
is flat over $R'$ (Lemma \ref{lemma-flat-base-change})
and hence $S'$ is flat over $R'$
as localizations are flat (Lemma \ref{lemma-flat-localization})
and compositions of flat ring maps are flat
(Lemma \ref{lemma-composition-flat}).
Proof of (2). Suppose that $R \to R'$ and $R' \to S'$ are flat.
By (1) applied to the diagram reflected in the northwest diagonal
we see that $S \to S'$ is flat and hence faithfully flat
(Lemma \ref{lemma-local-flat-ff}).
Moreover, $R \to S'$ is flat as a composition of flat maps.
Hence flatness of $R \to S$ follows from Lemma \ref{lemma-flat-permanence}.
\end{proof}
...
\item If $R \to S$ is unramfied, then there exists a finite type
...
\noindent
Here are some properties of module that descend.
...
\medskip\noindent
Part (3) is Lemma \ref{lemma-flatness-descends}.
\end{proof}
...
Let $k \subset K$ be a finitely generated field extension.
Then the ring $K \otimes_k A$ is Noetherian as well.
Namely, $K = S^{-1}B$ for some finite type $k$-algebra
$B$ and $S \subset B$ is a multiplicative subset.
Then we have $A_{k'} = S^{-1}(B \otimes_k A)$.
Hence $K \otimes_k A$ is a localization of the finite type $A$-algebra
