...
nd hence $\text{Spec}(S_{\mathfrak q})$ is also open
in $\text{Spec}(S)$.
...
$d$, and assume at least one coefficient of $F$ is a 
unit of $R$. Let $S = R[X,Y]/(F)$ as a graded ring.
Then for all $n \geq d$ the $R$-module $S_n$
is finite locally free of rank $d$.
\end{lemma}

\begin{proof}
The module has a presentation
$$
R[X,Y]_{n-d} \to R[X,Y]_n \to S_n \to 0
$$
and hence has finite presentation.
Thus by Lemma \ref{lemma-finite-projective}
it is enough to show that it is flat.
Let $R' \subset R$ be the subring generated by
the coefficients of $F$ and the inverse of the
invertible coefficient. Then $S' = R'[X,Y]/(F)$
is a graded ring over $R'$ such that
$S' \otimes_R R' = S$. Hence it suffices
to show the result for $S'_n$. Since $R'$ is finitely
generated over the Noetherian ring $\mathbf{Z}$ we 
may assume $R$ is Noetherian.
To show flatness we may localize and assume that
$R$ is local Noetherian with maximal ideal $\mathfrak m$.
In this case the result
follows from Lemma \ref{lemma-mod-injective}
because the modules $N = R[X,Y]_n$ is flat, and
multiplication by $F$ is injective modulo $\mathfrak m$
because at least one coefficient is not zero.
The statement on the rank follows by considering
the vector spaces $S_n \otimes_R \kappa(\mathfrak p)$
which visibly have dimension $d$.
\end{proof}
...
$F$ is not zero in $\kappa(\mathfrak p)[X,Y]$.
There exists an $f \in R$ $f \not\in \mathfrak p$,
an integer $e$, and a $G \in R[X,Y]$
...
prime to the image of $F(X,Y)$. Pick
$f \in R$, $f \not \in \mathfrak p$ such that
the localizations $(S_d)_f$ and  $(S_{d+e})_f$
are free of rank $d$. We may and do replace $R$
by $R_f$. Then we may take the determinant
$f \in R$ of the linear map of free modules
$G : S_d \to S_{d + e}$. Note that $f \not \in \mathfrak p$
because of our choice of $G$. Inverting $f$ 
we may assume that multiplication by $G$
from $S_{d} \to S_{d+e}$ is an isomorphism.
From this in turn it follows that
the image of $G$ $\kappa(\mathfrak p')[X,Y]$ is relatively
prime to the image of $F(X,Y)$ for all primes $\mathfrak p'$ 
of $R$. At this point multiplication by $G$ as a map
$S_n \to S_{n+1}$ will be an
isomorphism for all $n \geq d$ since we can compute 
whether it is an isomorphism by computing determinants
and we may do this on fibres over $\kappa(\mathfrak p')$
where it is obvious.
\end{proof}
