...
$$
(x/f)^d + \sum_{i = 1, \ldots, d} a_i/f^i (x/f)^{d - i} = 0
$$
...
$$
(x/f)^d + \sum_{i = 1, \ldots, d} (a_i/f_i) (x/f)^{d - i} = 0
$$
in $S^{-1}B$ for some $a_i \in A$ and $f_i \in S$. This means that
$$
(f'f_1 \ldots f_d x)^d +
\sum_{i = 1, \ldots, d} f^i(f')^if_1^i \ldots f_i^{i - 1} \ldots f_d^i a_i
...
surjective for any $R$-module $M$.
\end{enumerate}
\end{lemma}

\begin{proof}
Let $K = \text{Ker}(M \to N)$ be the kernel of a surjective
map $M \to N$ of $R$-modules.. For every $n$ the sequence
$$
K/I^nK \to M/I^nM \to N/I^nN \to 0
$$
is exact. Denote $A_n = \text{Im}(K/I^nK \to M/I^nM)$.
Note that the maps $A_{n + 1} \to A_n$ are surjective.
Apply Homology, Lemma \ref{homology-lemma-Mittag-Leffler} to
see (1). To see (2) choose generators $x_i \in M$, $i \in I$.
Then the map $\bigoplus_{i \in I} R \to M$ is surjective.
Hence by (1) we see $\bigoplus R^\wedge \to M^\wedge$ is surjective.
\end{proof}
...
$x_1, \ldots, x_t$ of $M$. Let $(m_n) \in M^\wedge$.
...
This shows that $(R^t)^\wedge \to M^\wedge$ is surjective,
...
to prove the the kernel of $(R^t)^\wedge \to M^\wedge$ is
exactly $K^\wedge$.
...
is identified with $I^\wedge \to R^\wedge$ and this is injective.
...
to $I$ is $I$-adically complete. Here is a lemma from a unpublished note
...
complete if and only if $K_n$ is equal to $I^nM^\wedge$.
\end{lemma}
...
\noindent
To use this lemma note that $I^nM^\wedge$ is the completion of
...
we see that the kernel of $M^\wedge \to M/I^nM$ is equal
to $(I^nM)^\wedge = I^n M^\wedge$ (see remark above the lemma).
...
$$
\mathfrak q = \{f \in S \mid f^q \in xR = \mathfrak p\} = yS
$$
...
$R_0$ once more we may assume that $x_0^2 = 0$ in $S_0$.
However, since $R_0 \subset R$ is reduced we see that
$S_0$ is reduced (because $R_0$ is Noetherian and the previous
paragraph applies) and hence $x_0 = 0$ as desired.
\end{proof}
