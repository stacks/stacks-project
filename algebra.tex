...
\medskip\noindent
Next we show that if $0 \to R^\alpha \to R^\beta \to R^\gamma \to 0$
is exact, then $\beta = \alpha+\gamma$. This is obvious in the
vector space case, and dividing out the maximal ideal is an exact functor,
so it is true in the module case as well. From this argument we also
see that the rank of a finite projective module over $R$ is well
defined: if $P \cong R^\alpha \cong R^\beta$, then $\alpha=\beta$. 
