...
\noindent
Any Noetherian ring has property $(S_0)$.
...
reduced. Hence it suffices to show that if $R$ is a local
Noetherian domain of dimension $d$ satisfying $(S_2)$ and $(R_1)$
then $R$ is normal. If $d = 0$, the result is clear.
If $d = 1$, then the result follows from Lemma \ref{lemma-characterize-dvr}.

\medskip\noindent
Let $R$ be a local Noetherian domain with maximal ideal
$\mathfrak m$ and dimension $d$ which satisfies $(R_1)$ and
$(S_2)$. Pick $x = f/g$, $f, g \in R$ in the fraction field of $R$
