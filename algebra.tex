...
The following are equivalent:
\begin{enumerate}
\item There exists a nonzero divisor in $I$.
\item We have $\mathfrak m \not\in \text{Ass}(R)$ and
$I \not \subset \mathfrak q$ for all $\mathfrak q \in \text{Ass}(R)$.
\end{enumerate}
\end{lemma}

\begin{proof}
If there exists a nonzero divisor $x$ in $\mathfrak m$,
then it is clear that no nonzero element of $R$ has annihilator
$\mathfrak m$. Also, $x$ clearly cannot be in any associated
prime of $R$. Conversely, suppose $I \not \subset \mathfrak q$
for all $\mathfrak q \in \text{Ass}(R)$. In this case we can
choose $x \in I$, $x \not \in \mathfrak q$ for all
$\mathfrak q \in \text{Ass}(R)$ by Lemma \ref{lemma-silly}.
...
$\text{rank}(\varphi_{i+1}) + \text{rank}(\varphi_i)
= n_i$ and each $I(\varphi_i) = R$.
\end{lemma}
...
In this case the complex is isomoprhic to a direct sum of trivial
complexes.
\end{lemma}
...
Let $x \in R$ be an element whose annihilator is $\mathfrak q$.
Note that the maps $\varphi_i$
induce maps $xR^{n_i} \to xR^{n_{i-1}}$ which are isomorphic
to the maps $\varphi_{i, \mathfrak q} : (R/\mathfrak q)^{n_i}
\to (R/\mathfrak q)^{n_{i-1}}$ induced by $\varphi_i$.


Since $R/\mathfrak q$ is a domain
we deduce immediately by localizing to its fraction field
that the rank of $\varphi_{\mathfrak q}$ is $m$ and that
$I(\varphi_{\mathfrak q})$ is not the zero ideal. Hence we
conclude by Lemma \ref{lemma-ideal-nonzerodivisor}.
