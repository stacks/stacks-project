...
The following are equivalent:
\begin{enumerate}
\item $S$ is a local complete intersection over $k$.
\item All of its local rings are complete intersection rings over $k$,
\item All localizations at maximal ideals are complete intersection rings
over $k$.
\end{enumerate}
\end{lemma}

\begin{proof}
Assume all the localizations at maximal ideals
are complete intersection rings. We will prove (1).
Let $\mathfrak m$ be a maximal ideal of $S$. According to Definition
\ref{definition-lci-field} we have to show that
$S_g$ is a global complete intersection for some $g \in S$,
$g \not \in \mathfrak m$. This is clear by combining
Lemmas \ref{lemma-lci} and \ref{lemma-lci-local}.
The other implications are proved similarly (omitted).
\end{proof}
