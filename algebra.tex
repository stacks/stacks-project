...
$M/\mathfrak m$ then they are independent over $R$.
...
$F'_2 = \oplus S' e'_i$ for some basis $e'_i$.
We take $F_2 = \oplus S e_i$. Each $e'_i$ is mapped via
$F'_2 \to F'_1$ to an element $\xi_i'$ of the kernel
$\text{Ker}(F'_1 \to F'_0)$.
On the other hand, let $K_1 = \text{Ker}(F_1 \to F_0)$,
and $K_0 = \text{Ker}(F_0 \to M)$.
The problem is to find an element $\xi_i \in K_1$
lifting the element $\xi_i'$.
...
K_{0, \mathfrak q'}$ is the kernel of
...
$F_{1,\mathfrak q'}' \to F_{0, \mathfrak q'}'$.
Thus, after inverting a suitable $g$ we may assume all the
elements $\xi_i'$ lift to elements $\xi_i \in K_1$.
At this point we simply define the map
$F_2 = \oplus S e_i \to F_1$ to be the map that sends
$e_i$ to $\xi_i$.

\medskip\noindent
Furthermore, we may continue like this since, at the next stage for
example we will have an exact sequence
$$
