...
Let $R \to S$ be a ring map.
\begin{enumerate}
\item We say that $R \to S$ is essentially of finite type if
$S$ is the localization of an $R$-algebra of finite type.
\item We say that $R \to S$ is essentially of finite presentation if
...
Set $S_\lambda =
(R_\lambda[x_1,\ldots,x_n]/(f_{1,\lambda}, \ldots, f_{n,\lambda}))_{\mathfrak q_\lambda}$. We leave it to the reader to see that this works.
\end{proof}
...
with $R_n = \mathbf{Z}[z, y_1, y_2, \ldots, y_n]/(y_i^2 - zy_{i+1}, i=1,\ldots,n-1)$ and $S_n = R_n/(z, x_n^2)$.
\end{remark}
...
S_\lambda = 
(R_\lambda[x_1,\ldots,x_n]/(f_{1,\lambda},\ldots,f_{m,\lambda}))_{\mathfrak q_\lambda}
$$
...
\noindent
In this section we use the auxilliary results of Section
\ref{section-limits-flat} to prove the following theorem.
...
such that $M_{\mathfrak q}$ is flat over $R$.
Let $\mathfrak p \subset R$ be the inverse image
of $\mathfrak q$ in $R$.
Note that $M_{\mathfrak q}$ is actually flat over $R_{\mathfrak p}$.
...
$\mathfrak q_{\lambda}$. Since $M = M_\lambda \otimes_{S_\lambda} S
= M_\lambda \otimes_{ \otimes_R} $
