...
\begin{proof}
This is clear from the fact that {\it every} maximal chain
of primes in $k[x_1, \ldots, x_n]$ has length $n$ by Proposition
\ref{proposition-finite-gl-dim-polynomial-ring} above.
\end{proof}
...
ideal $\mathfrak m$ of $S$.
\end{lemma}

\begin{proof}
Immediate from Lemma \ref{lemma-dimension-height-polynomial-ring} above.
\end{proof}
...
Then $\dim_x(X)$ is equal to
$\max \dim(Z)$ where the maximum is over those
irreducible components $Z$ of $X$ passing through $x$.
\end{lemma}
...
\begin{proof}
Let $X = \bigcup Z_i$ be the decomposition of $X$ into
...
Let $Z = \bigcup_{x \not \in Z_i} Z_i$. Then $U = X \setminus Z$
is an open subset of $X$ containing the point $X$.
We claim that for any open $W \subset U$, with $x \in W$
we have $\dim(W) = \dim(S_{\mathfrak m})$.
The irreducible components of $W$ are the irreducible sets
$W_i = Z_i \cap W$ for those $i$ with $x \in Z_i$.
Let $\mathfrak q_i \subset S$ be a minimal prime corresponding
to $Z_i$. Note that
$x \in Z_i \Leftrightarrow \mathfrak m \supset \mathfrak q_i$, and
in this case $\dim(Z_i) = \dim(W_i) = \dim((S/\mathfrak q_i)_{\mathfrak m})$
according to Lemma \ref{lemma-dimension-spell-it-out} above.
Since the $S_{\mathfrak m}/\mathfrak q_iS_{\mathfrak m}
= (S/\mathfrak q_i)_{\mathfrak m}$, and since the $\mathfrak q_i$
which occur correspond exactly
(see Lemma \ref{lemma-irreducible-components-containing-x})
to the minimal primes of
$S_{\mathfrak m}$ we win.
\end{proof}
...
\begin{proof}
As in the proof of Lemma \ref{lemma-dimension-closed-point-finite-type-field}
it suffices to prove that the dim
...
\mid f_1, \ldots, f_i 
\text{ form a regular sequence in }
...
lemma. Then $\dim_{\mathfrak q}(\overline{S}/R)$
(see Definition \ref{definition-relative-dimension}) is
equal to
$\dim(S_{\mathfrak q}/\mathfrak pS_{\mathfrak q}
+ f_1S_{\mathfrak q} + \ldots + f_iS_{\mathfrak q})$
by Lemma \ref{}. Since $f_1, \ldots, f_i$ is a regular sequence
this is equal to FIXME
By Lemma
...
which generate $I/I^2_{\mathfrak q}$. By Lemma \ref{lemma-NAK}
...
\text{height}(\mathfrak q') - e$ and we conclude that $e = c$.
Thus (5) implies (2).
...
Before we can conclude (1) holds we have to get rid
of the irreducible components of $\text{Spec}(S)$ whose
dimension is too large. Let
$V(I) = \bigcup Z_i \subset \text{Spec}(k[x_1, \ldots, x_n])$
be the decomposition into irreducible components, and let
$\mathfrak q_i \in Z_i$ be the generic points.
Note that if $\mathfrak q' \in Z_i$, then $\dim Z_i \leq n - c$
because $\dim (k[x_1, \ldots, x_n]/\mathfrak q_i)_{\mathfrak q}
\leq \dim S_{\mathfrak q} = n - \text{height}(\mathfrak q')
= n - c + \text{height}(\mathfrak q)$ which
using Lemma \ref{lemma-dimension-height-polynomial-ring}
gives the inequality.
Thus we may choose a $g'' \in k[x_1, \ldots, x_n]$ such that
$g'' \not \in \mathfrak q'$, and such that 
$g'' \in \mathfrak q_i$ for those $i$ such that $\dim Z_i > n - c$.
...
$$
and by our choice of $g''$ this ring has dimension $\leq n - c$.
...
Because tensoring with $K$ oer $k$ is an exact functor we have
...
\geq \dim(S'_{\mathfrak n})$, by the characterization of dimension
...
if and only if it is a local complete intersection.
...
$R \to S_g$ is syntomic.
\item The local ring map $R_{\mathfrak p} \to S_{\mathfrak q}$
is flat, and $\kappa(\mathfrak p) \to S \otimes_R \kappa(\mathfrak p)$
