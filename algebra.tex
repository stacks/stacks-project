...
if and only if $x$ is a nonzero divisor on $M$. As an application
...
and hence $S'$ is flat over $R'$. Conversely, suppose that
...
(Crit\'ere de platitude par fibres.)
Let $R$, $S$, $S'$ be rings and let $R \to S \to S'$ be ring
homomorphisms. Let $M$ be an $S'$-module. Let $\mathfrak q'$ be
a prime of $S'$ lying over $\mathfrak q \subset S$ and
$\mathfrak p \subset R$. Assume
\begin{enumerate}
\item The ring maps $R \to S$ and $R \to S'$ are of finite presentation.
\item The module $M$ is of finite presentation over $S'$.
\item The module $M_{\mathfrak q'}/\mathfrak p(M_{\mathfrak q'})$
is a flat $S_{\mathfrak q}/\mathfrak p S_{\mathfrak q}$-module.
\item The module $M_{\mathfrak q'}$ is a flat $R_{\mathfrak p}$-module.
\end{enumerate}
Then $M_{\mathfrak q'}$ is a flat $S_{\mathfrak q}$-module.
\end{lemma}

\begin{proof}
Write $R = \text{colim}\ R_\lambda$.
Similarly write $S = \text{colim}_\lambda S_\lambda$.
Reduce to FIXME.
\end{proof}
...
Note that $\kappa(\mathfrak p) \subset \kappa(\mathfrak q)$
is a finite field extension. Hence
$\text{trdeg}_k(\kappa(\mathfrak p)) = \text{trdeg}_k(\kappa(\mathfrak q))$.
Hence $\dim(k[y_1, \ldots, y_d]_{\mathfrak p}) = \dim(S_{\mathfrak q})$.
Thus $k[y_1, \ldots, y_d]_{\mathfrak p} \to S_{\mathfrak q}$
satisfies all the assumptions of Lemma \ref{lemma-CM-over-regular-flat}
and hence is flat.

\medskip\noindent
By Theorem \ref{theorem-openess-flatness}
above we see that for some $g \in S$,
$g \not \in \mathfrak q$ the ring $S_g$ is flat over
$k[y_1, \ldots, y_d]$. For any prime $\mathfrak q'$ of $S_g$
the local ring $(S_g)_{\mathfrak q'}$ has dimension
$\leq$ to the dimension of $k[y_1, \ldots, y_d]_{\mathfrak p'}$
with $\mathfrak p' = k[y_1, \ldots, y_d] \cap \mathfrak q'$.
This is true because $k[y_1, \ldots, y_d] \to S$ is finite
(and hence $k[y_1, \ldots, y_d] \to S_g$ quasi-finite).
Hence Lemma \ref{lemma-finite-flat-over-regular-CM}
applies to all the local ring maps
$k[y_1, \ldots, y_d]_{\mathfrak p'} \to (S_g)_{\mathfrak q'}$
and we win.
\end{proof}
...
\begin{proof}
Omitted. But: This was proved during the course of the proof of
Lemma \ref{lemma-finite-type-over-field-CM-open}.
\end{proof}
...
Cohen-Macaulay fibres. Then $S = S_0 \times \ldots \times S_d$
is a product of rings $S_i$ such that each $S_i$ satisfies
(a), (b), (c) and has all fibres equidimensional of dimension $i$.
\end{lemma}

\begin{proof}
Set $X = \text{Spec}(S)$ and $Y = \text{Spec}(R)$ and denote
$f : X \to Y$ the resulting map. By
Lemma \ref{lemma-dimension-fibres-bounded-quasi-compact-open-upstairs}
the sets
$V_n = \{\mathfrak q \in \text{Spec}(S) \mid \dim_{\mathfrak q}(S/R) \leq n\}$
are quasi-compact open in $X$. It suffices to show that
$V_n$ is also closed. By
Lemma \ref{lemma-constructible-stable-specialization-closed}
it suffices to show that $V_n$ is closed under specialization.
Hence suppose we have primes $\mathfrak q \subset \mathfrak q' \subset S$
lying over $\mathfrak p \subset \mathfrak p' \subset R$.
We have to show that $\dim_{\mathfrak q}(S/R) \geq \dim_{\mathfrak q'}(S/R)$.
FIXME
