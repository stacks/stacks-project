...
$\mathcal{U}$ over $(\Sch/S)_{fppf}$ which is representably
...
of stacks in groupoids over $(\Sch/S)_{fppf}$. We will say that
...
over $(\Sch/S)_{fppf}$. Assume $p$ is representable by algebraic
...
Quot, Lemma \ref{quot-lemma-where-isomorphism}
...
\mathit{Mor}_{B}(Z, X) : (\Sch/S)^{opp} \longrightarrow \textit{Sets},
\quad
...
canonical bijection
$$
\text{Mor}_{\textit{Sh}((\Sch/S)_{fppf})}(T, \mathit{Mor}_B(Z, X))
=
...
canonical bijection
$$
\text{Mor}_{\textit{Sh}((\Sch/S)_{fppf})}(T, \text{Res}_{Z/B}(X))
=
...
Quot, Lemmas \ref{quot-lemma-where-unramified} and
