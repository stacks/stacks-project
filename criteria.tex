...
\begin{lemma}
\label{lemma-representable-by-spaces-over-stack}
Let $S$ be a scheme.
...
\begin{proof}
By assumption (1) there exists a representable stack in setoids
$\mathcal{V}$ a smooth surjective morphism $\mathcal{V} \to \mathcal{Y}$.
Then $\mathcal{U} = \mathcal{V} \times_{\mathcal{Y}} \mathcal{X}$
is representable by an algebraic space by assumption (2).
The projection
$$
\mathcal{U} = \mathcal{V} \times_{\mathcal{Y}} \mathcal{X}
\longrightarrow \mathcal{X}
$$
is, as the base change of the morphism $\mathcal{V} \to \mathcal{Y}$,
...
action
$$
a : G \times_{\text{Spec}(Z)} X \longrightarrow X
$$
...
The stack $\mathcal{H}_d$ is isomorphic to the quotient stack
...
Lemma \ref{lemma-representable-by-spaces-over-stack}.
\end{proof}
