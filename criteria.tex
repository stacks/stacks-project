...
$$
(\textit{Sch}/\Spec(k))_{fppf} \times_{y, \mathcal{Y}, p} \mathcal{X}.
$$
...
\mathit{Mor}_{B}(Z, X) : (\textit{Sch}/S)^{opp} \longrightarrow \textit{Sets},
\quad
...
\text{Res}_{Z/B}(X) : (\textit{Sch}/S)^{opp} \longrightarrow \textit{Sets},
\quad
T \longmapsto \{(a, b)\text{ as above}\}
\end{equation}
Sometimes we think of this as a functor defined on the category
