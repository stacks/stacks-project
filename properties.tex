...
if $\mathcal{I}$ is as in (4) hold and $W = \Spec(R) \subset X$ is an affine
...
is restrocompact by Lemma \ref{lemma-retrocompact}.
\end{proof}
...
\href{http://amathew.wordpress.com/2011/07/30/%
quasi-coherent-sheaves-presentable-categories-and-a-result-of-gabber/}%
...
Write $U_i = \text{Spec}(A_i)$ and $U_{ijk} = \text{Spec}(A_{ijk})$.
...
Let $X$ be a scheme. The inclusion functor
...
\medskip\noindent
Pick a cardinal $\kappa$ as in
Lemma \ref{lemma-colimit-kappa}.
Pick a collection $(\mathcal{F}_t)_{t \in T}$ of $\kappa$-generated
quasi-coherent sheaves as in
Lemma \ref{lemma-set-of-iso-classes}.
Given an object $\mathcal{G}$ of $\textit{QCoh}(\mathcal{O}_X)$ we set
$$
Q(\mathcal{G}) = \colim_{(t, \alpha)} \mathcal{F}_t
$$
The colimit is over the category of pairs $(t, \alpha)$ where
$t \in T$ and $\alpha : \mathcal{F}_t \to \mathcal{G}$ is a
morphism of $\mathcal{O}_X$-modules. A morphism
$(t, \alpha) \to (t' ,\alpha')$ is given by a morphism
$\beta : \mathcal{F}_t \to \mathcal{F}_{t'}$ such that
$\alpha' \circ \beta = \alpha$. By
Schemes, Section \ref{schemes-section-quasi-coherent}
...
$$
holds for $\kappa$-generated quasi-coherent modules $\mathcal{H}$ by
choice of the system $(\mathcal{F}_t)_{t \in T}$. It follows formally from
Lemma \ref{lemma-colimit-kappa}
that this equality continuous to hold for any quasi-coherent
...
\begin{lemma}
\label{lemma-sections-supported-in-closed-subset}
...
ideal $I \subset \mathcal{O}_X(U)$ such that
$Z \cap U = V(I)$ and for any choice of $I$ we have
...
Hence we may work locally on $X$ to verify (2).
Thus we may assume $X = \Spec(A)$ and $\mathcal{F} = \widetilde{M}$.
The retrocompactness of $X \setminus Z$ just means that $X \setminus Z$
is quasi-compact open. This means we can cover it by finitely many
principal opens, hence $Z = V(I)$ for some finitely generated ideal
$I \subset A$. Let $\mathcal{I} \subset \mathcal{O}_X$ be the quasi-coherent
sheaf of ideals corresponding to $I$. We have
$$
\mathcal{F}'(X) = \{x \in M \mid x = 0\text{ in }M_\mathfrak p
...
finitely generated this is equivalent to $I^n x = 0$ for some $n$. Similarly
sections of $\mathcal{F}'$ over any affine open of $X$ are the sections
of $\mathcal{F}$ which are annihilated by some power of $\mathcal{I}$.
Hence we see that $\mathcal{F}' = \colim \mathcal{F}_n$, where
$\mathcal{F}_n$ is the subsheaf of sections annihilated by
$\mathcal{I}^n$, see
Definition \ref{definition-subsheaf-sections-annihilated-by-ideal}.
Thus the result follows from Lemma \ref{lemma-sections-annihilated-by-ideal}
