...
finite presentation if and only if $M$ is a finite presentation $R$-module.
\end{lemma}
...
Equation (\ref{constructions-equation-multiply}),
...
such that $f^*\mathcal{O}_Y(n)|_U \to \mathcal{L}^{\otimes n}|_U$
is an isomorphism for all $n \in d\mathbf{Z}$.
\end{enumerate}
\end{lemma}
...
\medskip\noindent
Let $s \in S_{+}$ homogeneous. We define a module map
$$
\Gamma(D_{+}(s), \mathcal{O}_Y(n))
\longrightarrow
\Gamma(f^{-1}(D_{+}(s)), \mathcal{L}^{\otimes n}) =
\Gamma(X_s, \mathcal{L}^{\otimes n})
$$
Namely, given an element $s'/s^m$ in the left hand side
(so $s' \in S_{m\deg(s) + n}$), then we can take the corresponding
section $s'|_{X_s} \otimes s^{-m}$ of $\mathcal{L}^{\otimes n}$.
It is easy to see that these maps are compatible with restriction
to smaller standard opens of $\text{Proj}(S)$ and the multiplication
maps. Hence we get the $\mathcal{O}_Y$-module maps
$f^*\mathcal{O}_Y(n) \to \mathcal{L}^{\otimes n}$
by Sheaves, Lemma \ref{sheaves-lemma-f-map-basis-below-modules}.
Then (3) is true by construction. The final statement holds if
$x \in X_s$ and $d = \deg(s)$, see Constructions,
Lemma \ref{constructions-lemma-when-invertible}.
\end{proof}
