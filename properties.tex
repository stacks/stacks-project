...
\noindent
We use the lemma above to obtain the following amusing lemma.

\begin{lemma}
\label{lemma-inject-section-lifts}
Let $X$ be an affine scheme.
Let $U$ be a quasi-compact open subscheme of $X$.
Let $\mathcal{F}$ be a quasi-coherent $\mathcal{O}_X$-module.
Let $s \in \mathcal{F}(U)$.
Then there exists a quasi-coherent $\mathcal{O}_X$-module $\mathcal{F}'$
and an injective map $\alpha : \mathcal{F} \to \mathcal{F}'$ such that
$\alpha(s)$ is the restriction to $U$ of a global section
$s' \in \Gamma(X, \mathcal{F}')$.
\end{lemma}

\begin{proof}
Say $X = \text{Spec}(A)$ and $\mathcal{F} = \widetilde{M}$.
Since $U$ is quasi-compact we have
$U = D(f_1) \cup D(f_2) \cup \ldots \cup D(f_t)$ for some $t \geq 0$ and
$f \in A_i$. Set $I = (f_1, \ldots, f_t) \subset A$.
By Lemma \ref{lemma-sections-over-quasi-compact-open-in-affine}
we see that $s$ corresponds to an $A$-module map
$$
\varphi : I^n \to M/M_n
$$
for some $n$. Pick an injective $A$-module $J$ and an injective
$A$-module map $M_n \to J$ (this is possible, see Injectives,
Lemma \ref{injectives-lemma-injectives-modules}).
Since $J$ is injective we can extend the map $M_n \to J$ to
an $A$-module map $M \to J$. Hence we see that
$$
M \longmapsto M' = M/M_n \oplus J
$$
is an injective $A$-module map such that $\varphi$ becomes a
map $\varphi' : I^n \to M'$ not $M'/(M')_n$! After replacing
$\mathcal{F}$ by $\widetilde{M'}$ we may therefore assume
that $s$ corresponds to a $\varphi : I^n \to M$ for some $n$.

\medskip\noindent
Set $K = \text{Ker}(\varphi) \subset I^n$.
...
$$
with exact rows which defines $M'$ as the push out of the maps
$\varphi : I^n/K \to M$ and $I^n/K \to R/J$. It is clear that
the map $\varphi$ composed with the map $M \to M'$ is the restriction
of a map $A \to M'$ to $I^n$. This exactly means that
the image of $s$ as a section of $\widetilde{M'}$ over $U$ comes
