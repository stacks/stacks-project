...
Since $\pi$ is affine, it is quasi-compact and separated,
and hence $L^*$ is quasi-compact and quasi-separated since $X$ is
assumed quasi-compact and separated.
...
The result of the lemma follows from this equality by looking at
degree zero terms.
\end{proof}
...
Let $X$ be a scheme. Let $\mathcal{L}$ be an invertible $\mathcal{O}_X$-module.
Assume every point of $X$ is contained in an affine open subset
of the form $X_s$, where $s \in \Gamma(X, \mathcal{L}^{\otimes n})$ and
$n \geq 1$. Then $X$ is quasi-separated.
\end{lemma}
...
Let $X$ be a scheme. Let $\mathcal{L}$ be an invertible $\mathcal{O}_X$-module.
Set $S = \bigoplus_{n \geq 0} \Gamma(X, \mathcal{L}^{\otimes n})$
as a graded ring. If every point of $X$ is contained in one of the
open subschemes $X_s$, for some $s \in S_{+}$ homogeneous, then
there is a canonical morphism of schemes
$$
f : X \longrightarrow \text{Proj}(S),
$$
to the homogeneous spectrum of $S$ (see
Constructions, Section \ref{constructions-section-proj}).
This morphism has the property that $f^{-1}(D_{+}(s)) = X_s$.
\end{lemma}
...
\begin{lemma}
\label{lemma-map-into-proj-many-affines}
Let $X$ be a scheme. Let $\mathcal{L}$ be an invertible $\mathcal{O}_X$-module.
Set $S = \bigoplus_{n \geq 0} \Gamma(X, \mathcal{L}^{\otimes n})$.
Assume (a) every point of $X$ is contained in an
open subschemes $X_s$ which is affine, for some $s \in S_{+}$ homogeneous, 
and (b) $X$ is quasi-compact. Then the canonical morphism of schemes
$f : X \longrightarrow \text{Proj}(S)$ of Lemma \ref{lemma-map-into-proj}
above is an open immersion.
\end{lemma}
...
and there exists an open immersion
$$
j : X \longrightarrow Y = \text{Proj}(S)
$$
such that $\mathcal{L}^{\otimes n} \cong j^*\mathcal{O}_Y(n)$ compatible
with the multiplication maps. Then $\mathcal{L}$ is ample.
\end{lemma}
