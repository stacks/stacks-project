...
Let $X$ be a scheme. An affine stratification is a stratification
$X = \coprod_{i \in I} X_i$ whose strata are affine.
\end{definition}
...
If $i \in I'$, then $X_i$ is open because the union of the
closures of the other strata is the complement of $X_i$.
Thus $U = \bigcup_{i \in I'} X_i$ is an affine
(Schemes, Lemma \ref{schemes-lemma-disjoint-union-affines})
open subscheme of $X$.
...
stratification $Z = Z_1 \amalg \ldots \amalg Z_n$
with index set $\{1, \ldots, n\}$ and setting $Z_0 = U$
we obtain the desired stratification of $X$.
\end{proof}

\noindent
If a scheme $X$ has a finite affine stratification, then
of course $X$ is quasi-compact. Conversely, we have the following
existence result.

\begin{lemma}
\label{lemma-qc-affine-stratification}
Let $X$ be a quasi-compact scheme.
Then $X$ has a finite affine stratification.
\end{lemma}

\begin{proof}
Write $X = U_1 \cup \ldots \cup U_n$ as a union of affine opens.
By induction on $n$ the scheme $U = U_1 \cup \ldots \cup U_{n - 1}$
has a finite affine stratification, say $U = \bigcup_{i \in I} U_i$.
On the other hand, the closed subset $X \setminus U$ is equal to
$U_n \setminus (U_n \cap U)$ and hence affine as a closed subscheme
of an affine scheme. Thus we can
let $I' = \{0\} \amalg I$ with partial order given by letting
$0$ be the minimal element of $I'$.
\end{proof}

\begin{definition}
\label{definition-affine-stratification-number}
Let $X$ be a nonempty quasi-compact scheme. The
{\it affine stratification number}
is the smallest integer $n \geq 0$ such that the following equivalent
...
Let $X$ be a scheme which has an open covering by
...
Lemma \ref{lemma-qc-affine-stratification}.
\end{proof}
...
We will see later (insert future reference here) that
the affine stratification number of $\mathbf{P}^n_k$ is
actually equal to $n$.
\end{example}
