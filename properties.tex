...
\begin{proof}
Since $I^n \subset I^{n + 1}$ and $M_n \subset M_{n + 1}$ there
is a canonical map of $A$-modules
$$
\text{Hom}_A(I^n, M/M_n)
\longrightarrow
\text{Hom}_A(I^{n + 1}, M/M_{n + 1})
$$
which we will use as the transition maps in the system.
...
$$
is exact. First we define the arrow of the
lemma. Namely, suppose that $\varphi : I^n \to M$ is an $A$-module map.
...
It is easy to see that this is compatible with the transition maps
in the system $\text{Hom}_A(I^n, M)$. Thus we get the map of the
first arrow of the lemma. To get the second arrow we note that
$\widetilde{M}$ and $\widetilde{M/M_n}$ agree over the open
$\text{Spec}(A) \setminus V(I)$ since the sheaf $\widetilde{M_n}$
is clearly supported on $V(I)$. Also, note that each $M_n = 0$
in case we have $Ix = 0 \Rightarrow x = 0$ for $x \in M$. Thus
the first assertion follows from the second.
...
Let $\mathcal{F}$ be a quasi-coherent $\mathcal{O}_X$-module.
\begin{enumerate}
\item The $\mathcal{O}_X$-submodule $\mathcal{F}_n \subset \mathcal{F}$
whose sections over an open $W \subset X$ are those $s \in \mathcal{F}(W)$
such that $\mathcal{I}^ns = 0$ is a quasi-coherent submodule.
\item Assume that $X$ is quasi-compact and quasi-separated and
that $\mathcal{I}$ is of finite type. The canonical map
$$
...
$X = \bigcup_{j = 1, \ldots, m} W_j$. Moreover for each pair
