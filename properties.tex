...
$\text{Spec}(R) = U \subset X$ of $x$ such that
there exists a trivialization $\mathcal{O}_U \to \mathcal{L}|_U$.
By assumption, there exists
a $n \geq 1$ and a $s \in \Gamma(X, \mathcal{L}^{\otimes n})$
such that $X_s \subset U$. Let $f \in R$ correspond to
$s|_U$ via the trivialization chosen above.
Then clearly $X_s = X_s \cap U = D(f)$ is affine.
Since $U$ can be chosen arbitrarily small we win.
\end{proof}

\begin{lemma}
\label{lemma-s-opens-basis-quasi-separated}
Let $X$ be a scheme. Let $\mathcal{L}$ be an invertible $\mathcal{O}_X$-module.
...
$$
see Constructions, Lemma \ref{constructions-lemma-proj-scheme}.
