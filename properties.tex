...
Choose a finite affine open covering $S = \bigcup V_j$ and choose
finitely many sections $s_{jl} \in \mathcal{F}(U_j)$ generating
...
\begin{lemma}
\label{lemma-push-sections-supported-in-closed-subset}
Let $f : X \to Y$ be a quasi-compact and quasi-separated morphism
...
\begin{proof}
Omitted. (Hint: The assumption that $f$ is quasi-compact and
quasi-separated implies that $X \setminus f^{-1}Z$ is retrocompact
in $X$ so that Lemma \ref{lemma-sections-supported-in-closed-subset}
applies to $f^{-1}Z$ and $\mathcal{F}$.)
\end{proof}
