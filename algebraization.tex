...
that $y$ is the generic point of $V_k$ and that $V_0 \subset Z$.
...
+ e - m_k
\end{align*}
Since $m_k \geq 0$ we conclude that
$\delta(V_0, W_0, \ldots, W_k) \leq e + \delta(V'_0, W'_0, \ldots, W'_k)$.
\end{proof}
...
In Situation \ref{situation-algebraize} let $(\mathcal{F}_n)$ be an object
of $\textit{Coh}(U, I\mathcal{O}_U)$. Let $s, d$ be integers.
...
$$
\text{depth}((\mathcal{F}^\wedge_y)_\mathfrak p) + \delta^Y_Z(y) \geq s
\quad\text{or}\quad
\text{depth}((\mathcal{F}^\wedge_y)_\mathfrak p) +
\dim(\mathcal{O}_{X, y}^\wedge/\mathfrak p) + \delta^Y_Z(y) > s + d
$$
\item if $V(\mathfrak p) \cap V(I\mathcal{O}_{X, y}^\wedge) =
\{\mathfrak m_y^\wedge\}$, then
$$
\text{depth}((\mathcal{F}^\wedge_y)_\mathfrak p) + \delta^Y_Z(y) > s
$$
\end{enumerate}
\end{enumerate}
...
with the following properties
\begin{enumerate}
\item if $\delta^Y_Z(y) = 2$ we have $\text{depth}(\mathcal{F}''_{n, y}) \geq 1$,
\item for $\delta^Y_Z(y) = 1$ we have $\text{depth}(\mathcal{F}''_{n, y}) \geq 2$,
\item $(\mathcal{F}''_n)$ is isomorphic as a pro-system to an object
