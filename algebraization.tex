...
\begin{remark}
\label{remark-interesting-case-final}
In Situation \ref{situation-algebraize} let $(\mathcal{F}_n)$
be an object of $\textit{Coh}(U, I\mathcal{O}_U)$. Assume
\begin{enumerate}
\item $A$ is an $I$-adically complete local ring with a dualizing complex,
\item all irreducible components of $X$ have the same dimension,
\item the scheme $U$ is Cohen-Macaulay,
\item $I$ is generated by $d$ elements,
\item $\dim(X) - \dim(Z) > \max(d + 2, d' + d + 1)$, and
\item $\mathcal{F}_n$ is a finite locally free
$\mathcal{O}_U/f^n\mathcal{O}_U$-module.
\end{enumerate}
where $d' = \max_{f \in \mathfrak a} \text{cd}(A_f, I_f)$.
Then $(\mathcal{F}_n)$ is the completion of a coherent
$\mathcal{O}_U$-module. This follows from
Proposition \ref{proposition-d-generators}
after unwinding the definitions\footnote{The cohomological dimension of
$I$ in $A$ at points $y \in U \cap Y$ comes in when verifying
condition (2) of Definition \ref{definition-s-d-inequalities}.
Namely, with $d'$ as defined we have
$\dim(\mathcal{O}_{X, y}^\wedge/\mathfrak p) \leq d'$
for the primes $\mathfrak p$ occuring in that condition.
The dimension of $Z$ comes in via the inequality
$\delta^Y_Z(y) \geq \delta(y) - \dim(Z)$ of
Lemma \ref{lemma-discussion}.}.
Please compare with Remarks \ref{remark-interesting-case-variant},
\ref{remark-interesting-case}, and
...
Definition \ref{definition-s-d-inequalities}.
\end{enumerate}
We are unable to prove or disprove that $(\mathcal{F}_n)$
extends to $X$ under these assumptions. If you know the answer
to this question, please email
