...
be an object of $\textit{Coh}(U, I\mathcal{O}_U)$. The
...
\end{enumerate}
If $A$ is $I$-adically complete these conditions are also
equivalent to
\begin{enumerate}
\item[(6)] there exists an object $(\mathcal{G}_n)$ of
...
Thus we see that (6) implies (5).
\end{proof}

\noindent
If in Situation \ref{situation-algebraize} the ring $A$ is not
$I$-adically complete, then the correct thing
is to ask whether $(\mathcal{F}_n)$ is in the essential
image of the restriction functor
$$
...
In Situation \ref{situation-algebraize} let $(\mathcal{F}_n)$ be an
object of $\textit{Coh}(U, I\mathcal{O}_U)$. Let $A \to A'$ be a ring
map and denote $I'$, $\mathfrak a'$, $X'$, $Y'$, $Z'$, and
$U'$ the base changes of $A$, $I$, $\mathfrak a$, $X$, $Y$, $Z$, and $U$.
Denote $g : U' \to U$ the induced morphism
and set $(\mathcal{F}'_n) = g^*(\mathcal{F}_n)$, see
...
affine base change, see Cohomology of Schemes, Lemma
...
$\textit{Coh}(U, I\mathcal{O}_U)$ which does not
extend to $X$, in other words, it is not the
