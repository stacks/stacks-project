...
Cohomology of Schemes, Lemma \ref{coherent-lemma-existence-easy}.
To see part (1), denote $A', I', \mathfrak a'$ the $I$-adic completions
of $A, I, \mathfrak a$, set $X' = \Spec(A')$, $Y' = V(I')$, and
$U' = X' \setminus V(\mathfrak a')$. Note that $X' \to X$ is a
...
$$
the vertical functors are equivalences. Some details omitted, but
see More on Algebra, Lemma \ref{more-algebra-lemma-neighbourhood-equivalence}
for the affine case. Hence it suffices to prove (1) in the
$I$-adically complete case. However, in that case (1) reduces to (2) by
...
> 2
$$
Since $(\mathcal{F}_n)$ satisfies the $(d + 1, d + 2)$-inequalities,
...
\delta^Y_Z(y) > d + 2
$$
Looking at the inequalities we proved above this gives what we want
except possibly if $\dim(\mathcal{O}_{X, y}^\wedge/\mathfrak p) = 1$.
However, if $\mathcal{O}_{X, y}^\wedge/\mathfrak p) = 1$, then we have
...
\delta^{Y'}_{Z'}(y') > 3
$$
This follows immediately from the inequalities above and the
fact that $(\mathcal{F}_n)$ satisfies the $(d + 1, d + 2)$-inequalities.
...
$f_1, \ldots, f_d \in A$,
\item $(\mathcal{F}_n)$ satisfies the $(d + 1, d + 2)$-inequalities, see
Definition \ref{definition-s-d-inequalities}.
\end{enumerate}
Then $(\mathcal{F}_n)$ extends to $X$.  In particular, if $A$ is
$I$-adically complete, then $(\mathcal{F}_n)$ is the completion
...
\begin{proof}
We may assume $I = (f_1, \ldots, f_d)$. Then we see that
