...
\noindent
Elucidation:
Since $x$ is a closed point, we can view $x$ as a reduced closed subspace of
$X$ (Properties of Spaces, Definition
\ref{spaces-properties-definition-reduced-induced-space})
and consider the blowing up $X' \to X$ of $X$ along $x$
(Divisors on Spaces, Definition \ref{spaces-divisors-definition-blow-up}).
The morphism $X' \to X$ is proper
...
\medskip\noindent
A Nagata algebraic space is locally Noetherian and hence Noetherian if
and only if it is quasi-compact and quasi-separated. As an aside we mention
that for locally Noetherian algebraic spaces, being quasi-separated is
...
\medskip\noindent
In this paragraph we reduce the problem of getting rid of bad curves
to the case where $X$ is a scheme. This is standard and we encourage
the reader to skip the proof. Choose $(U, u) \to (X, x)$ as in
Decent Spaces, Lemma
\ref{decent-spaces-lemma-decent-space-elementary-etale-neighbourhood}.
With notation as in the commutative diagram displayed above, set
$V = Y \times_X U$, $U' = X' \times_X U$, and $V' = Y' \times_X V$.
As $\kappa(u) = \kappa(x)$ we see
that the fibres of $Y$, $X'$, $Y'$ over $x$ are isomorphic to the
fibres of $V$, $U'$, $V'$ over $u$.
Since blowing up and normalization commute with \'etale localization
by construction, we see that $V$, $U'$, and $V'$ play the same role
as $Y$, $X'$, and $Y'$.
In other words, $U' \to U$ is the blowing up in $u$ and $V'$ is the
normalization of $V \times_U U'$. Looking at bad curves for
$g : V \to U$ and $g' : V' \to U'$ we obtain a commutative diagram
$$
\xymatrix{
BadCurves(g') \ar[r] \ar[d] & BadCurves(g) \ar[d] \\
BadCurves(f') \ar[r] & BadCurves(f)
}
$$
where the vertical arrows are bijective (see remark on fibres above).
Continuing to do normalized blowups of $X'$ at points over $x$,
producing $f'', f''', \ldots$ and $g'', g''', \ldots$ each time
with bijective sets of bad curves. Thus we see that it suffices to
prove we can get rid of bad curves for the proper birational morphism
$V \to U$. This reduces us to the case discussed in the next paragraph.

\medskip\noindent
Assume $X$ is an affine scheme, $X$ and $Y$ are normal, and $x \in T$.
...
the local ring of $Y$ at the generic point of $C$. This makes sense as
the generic point of $C$ is in the schematic locus of $Y$ by
Spaces over Fields, Lemma
\ref{spaces-over-fields-lemma-codim-1-point-in-schematic-locus}
(this applies because $Y$ is separated as a proper algebraic space
over an affine scheme). Choose an element
...
\noindent
If $A$ defines a rational singularity, then for every normal modification
$X \to \Spec(A)$ the local rings $\mathcal{O}_{X, x}$ for $x \in X$ closed
define rational singularities as well. This follows from
Lemma \ref{lemma-exact-sequence}.
The reason for the terminology in (2) is the following lemma.
...
Here $(A, \mathfrak m, \kappa)$ be a local Noetherian normal Nagata domain of
...
be a proper birational morphism of schemes with $X$ normal.
We denote $C_1, \ldots, C_r$ the irreducible
components of the special fibre $X_s$ of $f$.
\end{situation}
...
Let $(A, \mathfrak m, \kappa)$ be a local Noetherian normal Nagata domain of
dimension $2$ which defines a rational singularity. Assume $A$ has
a dualizing complex. Then there exists a morphism $f : X \to \Spec(A)$ as in
Situation \ref{situation-rational} such that the dualizing sheaf $\omega_X$
...
$\kappa$ is identified with the residue field of the image $x$ of the
closed point of $\Spec(R)$. Let us say that the formal arc $a$ is
{\it centered at $x$} in this case. We say the formal arc $T \to X$
is {\it nonsingular} if the induced map
$\mathfrak m_x/\mathfrak m_x^2 \to \mathfrak m_R/\mathfrak m_R^2$
is surjective.

\medskip\noindent
Let $a : T \to X$, $T = \Spec(R)$ be a nonsingular formal arc centered
at a closed point $x$ of $X$. Assume $X$ is locally Noetherian.
Let $b : X_1 \to X$ be the blowing up of $X$ at $x$.
Since $a$ is nonsingular, we see that there is an element
$f \in \mathfrak m_x$ which maps to a uniformizer in $R$.
In particular, we find that the generic point of $T$ maps to
a point of $X$ not equal to $x$. In other words, with $K = f.f.(R)$
the fraction field, the restriction of $a$ defines a morphism
$\Spec(K) \to X \setminus \{x\}$. Since the morphism $b$ is
...
$$
Let $x_1 \in X_1$ be the image of the closed point of $T$. Observe that
$x_1$ is a closed point as it is a $\kappa = \kappa(x)$-rational point
on the fibre of $X_1 \to X$ over $x$. Since we have a factorization
$$
\mathcal{O}_{X, x} \to \mathcal{O}_{X_1, x_1} \to R
$$
...
(X, x) & (X_1, x_1) \ar[l] & (X_2, x_2) \ar[l] &
(X_3, x_3) \ar[l] & \ar[l] \ldots
}
$$
...
and $A$ is regular and the lemma is true.
...
\medskip\noindent
However, it gets worse as there exists a Nagata normal Noetherian domain $A$
...
up singular points resolves $\Spec(A)$ in this situation.
...
Thus all singularities of $X$ are rational singularities
(see discussion following Definition \ref{definition-reduce-to-rational}).
...
Situation \ref{situation-rational-double-point}.
(We will see in the course of the discussion below
...
Dualizing, Section \ref{dualizing-section-Riemann-Roch} we have
$$
...
$$
for some $a_{ijk} \in A$ where $i \leq j \leq k$. Observe that
the quadratic form
$q = \sum \overline{a}_{ij} t_i t_j \in \kappa[t_1, t_2, t_3]$
is well defined up to multiplicatio by an element of $\kappa^*$
by our choices. We are going to use this to study the blow up $X$ a bit more.
...
$$
In other words, $E \subset \mathbf{P}^2_\kappa$ is given
by the quadratic form $q$ introduced above.
...
\medskip\noindent
{\bf Claim.} If $X$ is singular at $p$, then either
$\kappa' = \kappa$ or if not, then after changing our choice of
$x_1, x_2, x_3$ we have
$$
x_3^2 = \sum a_{ijk}x_ix_jx_k
$$
in $A$. Equivalent statement: the quadratic form $q$ is a square
of a linear form times a scalar. Namely, if $A'_{\mathfrak m'}$
...
\medskip\noindent
The case where $q$ is not a square up to a scalar. By the claim any
...
(small detail omitted; use that $q$ is not a square up to a scalar).
...
find that $\overline{a}_{111} = 0$ and that
the quadratic form for $x_1, y_2, y_3 \in \mathfrak m'$ is
$$
q' =
...
two points or one point of degree $2$. We conclude that $q'$
is not a square up to a scalar and that moreover, if the blowup
$X' \to X$ of $X$ at $p$ has a singular point $p'$, then it
is $\kappa$-rational as before and does
not lie on the strict transform of $x_1 = 0$.
Another way of saying this is that $x_1$ maps to an element
of $\mathfrak m'' = \mathfrak m_{p'} \subset \mathcal{O}_{X', p'}$
which is not in $(\mathfrak m'')^2$.

\medskip\noindent
Thus if the process does not stop then we obtain a sequence of
blowings up
$$
\Spec(A) \leftarrow X \leftarrow X' \leftarrow X'' \leftarrow \ldots
$$
which produces a sequence of local rings
$$
A \to \mathcal{O}_{X, p} \to \mathcal{O}_{X', p'} \to \ldots
$$
inducing isomorphisms on residue fields
such that $x_1$ remains in the maximal ideal but not in the maximal
ideal squared. On the other hand, for each $n$ we have
$\mathfrak m_n = (x_1, x_{n, 2}, x_{n, 3})$ such that $x_{n, 2}$
and $x_{n, 3}$ do map into $\mathfrak m_{n + 1}^2$. Consider the map
$$
\Psi_n : A \longrightarrow A_n/\mathfrak m_n^n
$$
Note that $x_1^{n - 1}$ does not map to zero under $\Psi_n$
because if so then the zero locus of $x_1^{n - 1}$ on $X_{n + 1}$ would
contain $n E_{n + 1}$ which it does not by our remarks above.
On the other hand, $\Ker(\Psi_n)$ contains $y_2, y_3$ such that
$\mathfrak m = (x_1, y_2, y_3)$. Proof omitted; FIXME.
Taking the limit over $n$ we obtain
$$
A \longrightarrow R
$$
where $R$ is a complete discrete valuation ring and $x_1$ maps
to a uniformizer. Thus after a finite number of these blowups
we reach a nonsingular point by
Lemma \ref{lemma-sequence-blowups-along-arc-becomes-nonsingular} (FIXME)
...
The element $y_3 \in A'$ maps to a uniformizer.
...
Then $p$ must correspond to a zero of $h$ by the reasoning
used in proving the claim above.
If $h$ does not have a double zero at $p$, then the quadratic
form $q'$ at $p$ is not a square and we conclude (for the point $p$)
by the previous case. Since the degree of $h$ is $3$ we
get at most one problematic singular point $p \in C$
...
the right shape. Computing what happens with a second blow
up we see that $\overline{a}_{122}$ has to be nonzero; FIXME.
Thus $x'$ is as in the next paragraph.

\medskip\noindent
Here we can choose our generators $x_1, x_2, x_3$ of $\mathfrak m$
such that
$$
x_3^2 + x_1(a x_2^2 + b x_2x_3 + c x_3^2) \in \mathfrak m^4
$$
and $\overline{a}$ is nonzero.
Doing the blowing up as before we see that if $X$ has a singular
point $p$ where $q'$ is a square up to scalars, then it is the
point $y_2 = y_3 = 0$ and the new equation looks like
$$
y_3^2 + x_1(a y_2^2 + b y_2 y_3 + c y_3^2) \in (\mathfrak m')^4
$$
which is the same shape as before. Thus we see that
we get an infinite sequence of blow ups as before which
finishes the proof by the same reasoning as before.
