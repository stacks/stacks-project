...
\item if $\varphi \in \text{End}_{D(R)}(M^\bullet)$
is zero on $H^i(M^\bullet)$, then $\varphi^2 = 0$.
\end{enumerate}
\end{lemma}
...
to see that $\varphi^2$ factors through $\tau_{\leq -2}M^\bullet = 0$.
\end{proof}
...
Let $C$ be an object of the category $\mathcal{C}'$ above.
