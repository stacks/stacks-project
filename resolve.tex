...
generated by Varieties, Lemma \ref{varieties-lemma-chi-tensor-finite}.
...
Thus it suffices to see that $R^1(X_i \to X_{i - 1})_*\mathcal{O}_{X_i} = 0$
...
Let $A$ be a Nagata Noetherian local ring of dimension $2$ which has a
dualizing module $\omega_A$. If there exists a nonzero $d \in A$
such that for all normal modifications $X \to \Spec(A)$ the
cokernel of $\Gamma(X, \omega_X) \to \omega_A$ is annihilated
by $d$, then reduction to rational singularities is possible for $A$.
\end{lemma}
...
$$
is transformed into the short exact sequence
$$
0 \to f_*\omega_X \to \omega_A \to
\text{Ext}_A^2(R^1f_*\mathcal{O}_X, \omega_A) \to 0
$$
By Matlis duality, if $R^1f_*\mathcal{O}_X$ is not annihilated by $d$,
...
(\ref{equation-a-torsion}). FIXME: add references to duality chapter.
\end{proof}
...
is the result of sequences of blowings up in closed points, then
there exists a map
$$
...
\begin{proof}
Suppose we have constructed the map $c : \Omega^r_{X/A_0} \to \omega_X$
and suppose that $b : X' \to X$ is a blow up in a closed point.
Set $\Omega^r_X = (\Omega^r_{X/A_0})^{**}$ and
$\Omega^r_{X'} = (\Omega^r_{X'/A_0})^{**}$.
Since $\omega_X$ is invertible we can factor $c$ through a map
$\Omega^r_X \to \omega_X$. By the universal
property of the dualizing module, a map
$\Omega^r_{X'} \to \omega_{X'}$
is the same thing as a map $b_*\Omega^r_{X'} \to \omega_X$.
Thus in turn it suffices to produce a map
$$
b_*\Omega^r_{X'} \longrightarrow \Omega^r_X
$$
By our assumptions the sheaves $\Omega^r_{X'}$ and
$\Omega^r_X$ are invertible. Consider the exact sequence
$$
b^*\Omega_{X/A_0} \to \Omega_{X'/A_0} \to \Omega_{X'/X} \to 0
$$
A local calculation (omitted) shows that $\Omega_{X'/X}$ is isomorphic
to an invertible module on the exception divisor $E$. It follows that
either
$$
...
a fixed nonzer element of $B$. We conclude by
