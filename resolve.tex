...
Namely, recall that an admissible ring is a complete
...
McQuillan works with what might be called {\it weakly admissible} rings.
A weakly admissible ring $A$ is a
...
$S$, then the diagonal transformation $X \to X \times X$ is representable.
\end{lemma}

\begin{proof}
Given $U \to X$ where $U \in \Ob((\Sch/S)_{fppf})$ we have to
show that $U \to X$ is representable, see
Spaces, Lemma \ref{spaces-lemma-representable-diagonal}.
In other words, given $V \to X$ where $V \in \Ob((\Sch/S)_{fppf})$
we have to show $U \times_X V$ is representable.
Write $X = \colim X_\lambda$ as in Definition
\ref{definition-affine-formal-algebraic-space}.
The discussion above shows that Zariski locally on $U$ and $V$
the morphisms factors through some $X_\lambda$.
In this case $U \times_X V = U \times_{X_\lambda} V$
which is a scheme.
\end{proof}
...
\item a set theoretic condition (see Remark \ref{remark-set-theoretic}).
\end{enumerate}
\end{lemma}

\begin{proof}
It follows from Lemma \ref{lemma-covering-by-thickenings}
that an affine formal algebraic space
...
Here we put a few words concerning the size of things. FIXME.
\end{remark}
