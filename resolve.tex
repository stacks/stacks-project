...
We first reformulate the main result of that section in
...
Let $A \to B$ be a local homomorphism of local Noetherian rings
such that $\mathfrak m_B = \sqrt{\mathfrak m_A B}$ and such
that the induced map on completions is an isomorphism.
...
\begin{proof}
It suffices to prove this for the map $A \to A^\wedge$ to the completion.
Thus let $X \to \Spec(A)$ and $Y \to \Spec(A)$ be two objects of
(\ref{equation-modification}) for $A$ and let
$f^\wedge : X^\wedge \to Y^\wedge$ be a morphism between the base
change of $X$ and $Y$ to $A^\wedge$.
TO BE CONTINUED.
\end{proof}
