...
we have seen that this category only depends on the completion of
$A$\footnote{We will not use this somewhat subtle fact in this
chapter. In fact, we will only use
Restricted Power Series, Lemmas
\ref{restricted-lemma-dominate-by-admissible-blowup} and
\ref{restricted-lemma-descend-admissible-blowup}
in this chapter.}
...
\noindent
A few lemmas are needed before we finish the arguments.
...
For a closed point $q \in Y$ with image $p \in X$
lying over the closed point of $\Spec(A^\wedge)$ the
...
Let $\mathfrak m_q \subset B' = B \otimes_A A^\wedge$ be the
maximal ideal corresponding to $q$ and let $\mathfrak m_p \subset B$
...
$$
for all $n$. Since $\mathfrak m B \subset \mathfrak m_p$ and
$\mathfrak m^\wedge B' \subset \mathfrak m_q$ we see that
$B/\mathfrak m_p^n$ and $B'/\mathfrak m_q^n$ are both
quotients of the ring displayed above by the $n$th power
of the same maximal ideal. The lemma follows.
\end{proof}

\begin{lemma}
\label{lemma-blowup-still-good}
Let $(A, \mathfrak m, \kappa)$ be a local Noetherian Nagata domain of
...
We have to show that $H^1(Y, \mathcal{O}_Y) = 0$. Observe that
$Y \to \Spec(A^\wedge)$ is an isomorphism over the puncture
spectrum $U^\wedge = \Spec(A^\wedge) \setminus \{\mathfrak m^\wedge\}$.
Combining Restricted Power Series,
Lemmas \ref{restricted-lemma-dominate-by-admissible-blowup} and
\ref{restricted-lemma-descend-admissible-blowup}
we find there exists a proper birational morphism $X \to \Spec(A)$
whose base change to $\Spec(A^\wedge)$ dominates $Y$.
...
By Lemma \ref{lemma-blow-up-normal-rational} we see that $Y$
...
Let $(A, \mathfrak m, \kappa)$ be a local Noetherian domain of
dimension $2$. Assume there exists an alteration $f : X \to \Spec(A)$
(as in Morphisms, Definition \ref{morphisms-definition-alteration} or
Spaces over Fields, Definition \ref{spaces-over-fields-definition-alteration})
with $X$ regular. Then the integral closure $B$ of $A$ in its
fraction field is finite over $A$ and the $\mathfrak m$-adic
completion of $B$ is a normal ring, i.e., the completions of $B$
at its maximal ideals are normal domains.
\end{lemma}

\begin{proof}
We first prove this when $X$ is a scheme.
Set $C = \Gamma(X, \mathcal{O}_X)$.
By Cohomology of Schemes, Lemma
\ref{coherent-lemma-proper-over-affine-cohomology-finite}
we see that $C$ is a finite $A$-module.
As $X$ is normal (Properties, Lemma \ref{properties-lemma-regular-normal})
...
\medskip\noindent
Assume that $C^\wedge$ is a normal ring (we will show this below).
...
finite $A$-modules is exact and given by tensoring with $A^\wedge$
(Algebra, Lemma \ref{algebra-lemma-completion-flat}).
...
faithfull flaty and the same after localizing at $\mathfrak q$.
...
By Lemma \ref{lemma-iso-completions} the morphism
$Y = X \times_{\Spec(A)} \Spec(A^\wedge) \to X$
induces an isomorphism on complete local rings at closed points
of the special fibres. Hence we see that $Y$ is a regular scheme
(More on Algebra, Lemma \ref{more-algebra-lemma-completion-regular}
and Properties, Lemma \ref{properties-lemma-characterize-regular}).
...
hence a finite union of integral normal schemes
(Properties, Lemma \ref{properties-lemma-normal-Noetherian};
actually you can prove that $Y$ is connected in our case but
we do not need this) and hence $\Gamma(Y, \mathcal{O}_Y)$
is a finite product of normal domains
(Properties, Lemma \ref{properties-lemma-normal-integral-sections}).

\medskip\noindent
The proof in case $X$ is an algebraic space is exactly the same.
First, we replace some of the references to scheme theory by
references to the theory of algebraic spaces:
...
To see that $Y = X \times_{\Spec(A)} \Spec(A^\wedge)$
is a regular algebraic space we apply
Lemma \ref{lemma-iso-completions} (which is formulated only for schemes)
to $V = U \times_{\Spec(A)} \Spec(A^\wedge) \to U$
where $U$ is an affine scheme with a surjective \'etale
morphism $U \to X$. We conclude that $Y$ has regular local
rings at all closed points $y \in |Y|$. Since $Y$ is proper
over $A^\wedge$ this indeed implies that $Y$ is regular:
as $|Y|$ is sober and $|Y| \to |\Spec(A^\wedge)|$ closed,
every point of $|Y|$ does indeed specialize to a closed point
of the closed fibre and because $Y$ is decent these specializations
can be lifted to $V$; some details omitted.
The final arguments of the proof proceed exactly as before.
\end{proof}
