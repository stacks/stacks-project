...
$$
where $E$ is the exceptional divisor. Then $d > 0$, the sheaf
$\mathcal{O}_X(-dE)/I\mathcal{O}_X$ is supported in finitely many
closed points of $X$, and
$$
\text{length}_A(A/I) >
...
$$
the bottom row is exact and the vertical arrow surjective. The final
assertion of the lemma follows once we prove that
$\Gamma(X, \mathcal{O}_X/\mathcal{O}_X(-dE))$ is nonzero.
This is clear because the image of $1 \in \Gamma(X, \mathcal{O}_X)$
is nonzero as $d > 0$.
\end{proof}
