...
$$
provided the sizes of $\mathfrak X$ and $\mathfrak Y$ are not too large.
\end{lemma}
...
the size of $\mathfrak X$ and $\mathfrak Y$ is at most the size of
...
on this notion. The analogue of Lemma \ref{lemma-fully-faithful}
still holds in this setting (with the same proof).
\end{remark}
...
$S$, then the diagonal transformation $\Delta : X \to X \times X$
is representable and a closed immersion.
\end{lemma}

\begin{proof}
Suppose given $U \to X$ and $V \to X$ where $U, V \in \Ob((\Sch/S)_{fppf})$.
...
Given $(a, b) : W \to X \times X$ for $W \in (\Sch/S)_{fppf}$
consider the map $X \times_{\Delta, X \times X, (a, b)} W \to W$.
...
Thus $X \to X \times X$ is a closed immersion.
\end{proof}
...
the following hold
\begin{enumerate}
\item any morphism $U \to X$ with $U \in \Ob((\Sch/S)_{fppf})$
affine factors through $T \to X$ which is representable and a
thickening with $T \in \Ob((\Sch/S)_{fppf})$ affine, and
...
thickenings where $T \in \Ob((\Sch/S)_{fppf})$ is affine. This category
...
\begin{lemma}
\label{lemma-classical-affine-formal-algebraic-space}
...
of Lemma \ref{lemma-classical-affine-formal-algebraic-space}. Let $A$
...
affine formal schemes considered in \cite{EGA}.

\begin{lemma}
\label{lemma-presentation-representable}
Let $S$ be a scheme. Let $f : X \to Y$ be a transformation
...
\begin{proof}
Let $T \in \Ob((\Sch/S)_{fppf})$ and $T \to Y$ a map. We have to show that
the algebraic space $X \times_Y T$ is a scheme.
Write $X = \colim X_\lambda$ as in Definition
...
introducing a species of formal algebraic spaces without first introducing
...
\begin{proof}
Suppose given $U \to X$ and $V \to X$ with $U, V \in \Ob((\Sch/S)_{fppf})$.
Then $U \times_X V$ is a sheaf. For every $i$ the morphism
$$
(U \times_X X_i) \times_{X_i} (V \times_X X_i)
= (U \times_X V) \times_X X_i \to U \times_X V
$$
is representable and \'etale and its source is a scheme
(use Lemmas \ref{} and \ref{}). These maps are jointly surjective
hence $U \times_X V$ is an algebraic space by
Bootstrap, Theorem \ref{}.
The morphism $U \times_X V \to U \times_S V$ is a monomorphism.
It is also locally quasi-finite, because on precomposing with
the morphisms displayed above we obtain the composition
$$
(U \times_X X_i) \times_{X_i} (V \times_X X_i)
\to (U \times_X X_i) \times_S (V \times_X X_i)
\to U \times_S V
$$
which is locally quasi-finite. Hence we conclude that
$U \times_X V$ is a scheme by Morphisms of Spaces, Lemma \ref{}.
FIXME.
\end{proof}
...
in Topologies, Section \ref{topologies-definition-big-small-fppf}.
...
it suffices to assume the diagonal $\Delta : X \to X \times X$ is not
