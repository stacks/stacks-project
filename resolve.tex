...
$$
This is a diagram as in
Pushouts of Spaces, Situation \ref{spaces-pushouts-situation-formal-glueing}.
By Pushouts of Spaces, Lemma
\ref{spaces-pushouts-lemma-fully-faithful-on-separated} the functor which
...
Hence we can apply Pushouts of Spaces, Lemma
\ref{spaces-pushouts-lemma-equivalence-on-affine}
...
X \ar[r] &
\Spec(A^\wedge) \ar[d] \\
R_0 \ar@<1ex>[r] \ar@<-1ex>[r] & X'_0 \ar[rr] & & \Spec(A)
}
$$
...
Pushouts of Spaces, Situation \ref{spaces-pushouts-situation-coequalizer-glue}
over the base $\Spec(A^\wedge)$.
For $n \geq 1$ let $X_n \to X$ be the coequalizer of
$s_n, t_n : R_n \to Y_n = X' \coprod Z_n$ as in Pushouts of Spaces, Equation
(\ref{spaces-pushouts-equation-system-coequalizers}).
Finally, pick $m, n \geq 1$ as in
Pushouts of Spaces, Lemma \ref{spaces-pushouts-lemma-essentially-constant}.
