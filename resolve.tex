...
\noindent
Let $A$ be a Noetherian ring. Let $a \in A$. Set $A_n = A/(a^n)$ so
that the $a$-adic completion of $A$ is $A^\wedge = \lim A_n$.
...
an isomorphism $B_{n + 1}/a^nB_{n + 1} \to B_n$.
\end{enumerate}
Morphisms in $\mathcal{C}$ are given by systems of homomorphisms.
Let $\mathcal{C}'$ be the category of $A^\wedge$-algebras $B$ which are
$(a)$-adically complete and such that $B/aB$ is of finite type over
$A_1$. Then the functor
$$
\mathcal{C} \longrightarrow \mathcal{C}',\quad
(B_n) \longmapsto B = \lim B_n
$$
is an equivalence of categories. Let $A[x_1, \ldots, x_r]^\wedge$ denote
the $a$-adic completion of the polynomial ring. We think of elements
of $A[x_1, \ldots, x_r]^\wedge$ as power series in $x_1, \ldots, x_r$
with coefficients in $A^\wedge$ which tend to zero in the $a$-adic topology.
...
$$
where the map sends the residue class of $g \in J$ to the form
$\text{d}g = \sum (\partial g/\partial x_i) \text{d}x_i$ where
the partial derivative is taken by thinking of $g$ as a power
series. The reader may verify this is well defined. In fact, the
presentation defines presentations
$$
B_n = B/a^nB = A_n[x_1, \ldots, x_r]/J_n
$$
where
$$
J_n = JA_n[x_1, \ldots, x_r] =
J/(J \cap (a^n)) = J + (a^n) / (a^n)
$$
and we have
$$
NL^\wedge_{B/A} =
...
$$
as complexes. It follows that $NL^\wedge_{B/A}$ represents
$R\lim NL_{B_n/A_n}$ in the derived category. In particular, it
is (up to quasi-isomorphism) independent of the chosen
presentation.

\begin{lemma}
\label{lemma-get-morphism-nonzerodivisor}
Let $B$, $C$ be objects of the category $\mathcal{C}'$ above.
Let $c \geq 0$ be an integer such that $a^c$ is homotopic to
zero on $NL^\wedge_{B/A}$. Assume $a$ is a nonzerodivisor on $C$.
...
\begin{proof}
We first prove this in case $a$ is a nonzero divisor on $C$.
...
$$
By assumption there is a $B$-module map
...
$$
A computation with power series (omitted)
...
\begin{lemma}
\label{lemma-get-morphism}
Let $B$, $C$ be objects of the category $\mathcal{C}'$ above.
Let $c \geq 0$ be an integer such that $a^c$ is homotopic to
zero on $NL^\wedge_{B/A}$. Let $d \geq 0$ be an integer such that
...
\medskip\noindent
Let $C \to C'$ be the quotient of $C$ by $C[a^d]$. For $m > d$ the
diagram
$$
\xymatrix{
0 \ar[r] &
