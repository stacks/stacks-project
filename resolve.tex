...
$$
Then $M$ is the total ring of fractions of $C$,
...
$$
As $A^\wedge$ is Nagata, we see that $C'$ is finite over
...
curve into a strict normal crossings divisor.
...
must stabilize which proves that for some $n$ the scheme $Y_n$ is regular
...
By the universal property of blowing up this gives us a factorization
$X \to Y_i$.
...
is a regular curve for $i = 1, \ldots, n$. There are two ways to finish
the proof from this point.

\medskip\noindent
First method. By induction it suffices
to prove that after a finite number of blowups the curves $Y_1$ and
$Y_2$ are disjoint. Suppose that $p \in X$ is contained in both
$Y_1$ and $Y_2$. Letting $Z = Y_1 \cap Y_2$ we find that
$l = \text{length}_{\mathcal{O}_{X, p}} \mathcal{O}_{Z, p}$ is a
numerical invariant of the situation. An easy computation shows
that after blowing up $p$ the invariant decreases by $1$.

\medskip\noindent
Second method. Let $Z = \bigcup Y_i \subset X$ (scheme theoretic union).
Then $f : Z^\nu = \coprod Y_i \to Z$ is the normalization. Let $p \in Y$
be a singular closed point of $Y$; this exactly means that $p$ is contained
in more than one of the $Y_i$. Let $X' \to X$ be the blowup of $p$.
We obtain a factorization
$$
Z^\nu \to Z' \to Z
$$
where (a) $Z'$ is the strict transform of $Z$, (b) $Z' = \bigcup Y'_i$
is the scheme theoretic union of the strict transforms of $Y_i$, and
(c) $Z' \to Z$ is not an isomorphism because
$\mathfrak m_{Z, p} \subset \mathcal{O}_{Z, p}$ is not principal.
Continuing this process we obtain
$$
Z^\nu \to \ldots \to Z''' \to Z'' \to Z' \to Z
$$
which has to stabilize by the exact same argument as used in the proof
of Lemma \ref{lemma-resolve-curve}.
\end{proof}
...
regular scheme having codimension $|J|$ in $X$.
\end{enumerate}
\end{lemma}
...
of $D$ passing through $p$ correspond one to one to the height
one primes $(x_1), \ldots, (x_r)$. Moreover, any sum of these prime
ideals is another one and the quotient of $\mathcal{O}_{X, p}$
by such an ideal is regular too. It follows that (2) holds.
...
for $D_1, \ldots, D_r$. Since
$\mathcal{O}_{D_1, p} = \mathcal{O}_{X, p}/(x_1)$ is regular
we see that $\mathcal{O}_{X, p}$ is regular, etc, etc.
\end{proof}
...
\begin{proof}
Let $X' \to X$ be a blowup in a closed point $p$. Then the inverset image
...
\medskip\noindent
Assume $Z$ is an effective Cartier divisor and whose irreducible components
$Y_i$ are regular. Let $p \in Z$ be a closed point contained in more than
one of the irreducible components, say $p \in Y_i$ for $i \leq I$.
For $i, j \in I$, $i \not = j$, set
$$
e_{i, j} =
\text{length}_{\mathcal{O}_{X, p}} \mathcal{O}_{Y_i \cap Y_j, p}
$$
If $I = \{i, j\}$ has two elements and $e_{i, j}(p) = 1$, then blowing up at
$p$ is not necessary. If $e_{i, j} > 1$ for some $i, j \in I$, then
after blowing up the maximum decreases (see Lemma \ref{}).
If $e_{i, j} = 1$ for all $i, j$, then after blowing up
at $p$ we are done. FIXME.
\end{proof}
