...
blowing up of $X_i$ at a closed point and $f_n$ and $f$ agree.
\end{lemma}
...
exceptional curve of the first kind which is in a
fibre of the morphism $X \to S$. Then there exists a contraction
$X \to X'$ of $E$ and $X'$ is proper over $S$ and has an ample
invertible sheaf.
\end{lemma}

\begin{proof}
Choose an isomorphism $\mathbf{P}^1_k \to E$ as in the definition
of exceptional curves of the first kind.
Since $X \to S$ is proper, so is the morphism $E \to S$
(Morphisms, Lemma \ref{morphisms-lemma-closed-immersion-proper} and
\ref{morphisms-lemma-composition-proper}).
It follows that $k = H^0(E, \mathcal{O}_E)$ is finite as an $R$-module
...
\medskip\noindent
We claim that the Stein factorization $X \to X' \to \mathbf{P}^{n + 1}_S$
of $\varphi$ is the desired contraction. Then $X'$ is proper over $S$
and has an ample invertible sheaf, namely the pull back of
$\mathcal{O}_{\mathbf{P}^{n + 1}_S}(1)$. To check the Stein factorization
is a contraction of $E$ we will verify
the conditions of Lemma \ref{lemma-contraction}.
...
$\mathbf{P}^0_S \cong S$. Since by assumption $E$ is in a fibre
of the morphism $E \to S$ we see that all of $E$ maps to a point
under the morphism $\varphi$. This finishes the proof.
\end{proof}

\begin{lemma}
\label{lemma-contract-when-quasi-projective}
Let $S$ be a Noetherian scheme. Let $X$ be a quasi-projective scheme over $S$.
Let $E \subset X$ be an exceptional curve of the first kind which is in a
fibre of the morphism $X \to S$. Then there exists a contraction of $E$.
\end{lemma}

\begin{proof}
The question is local on $S$. Thus we may assume that $S$ is affine.
Then we can write $X$ as an open subscheme of a scheme projective over $S$
(see More on Morphisms, Lemma \ref{more-morphisms-lemma-quasi-projective}).
Thus we may apply Lemma \ref{lemma-contract-ample}.
\end{proof}
