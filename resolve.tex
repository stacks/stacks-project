...
Let $f : X \to Y$ be a finite morphism of Noetherian normal integral schemes
over $S$. Assume the extension of function fields is
purely inseparable of degree $p$. For $i \geq 1$ there is a canonical map
$$
\text{Tr} : f_*\Omega^i_{X/S} \longrightarrow (\Omega_{Y/S}^i)^{**}
$$
whose stalk in the generic point of $Y$ recovers the trace map of
...
nonzero divisor on $A/(f + g)$ and $A/\mathfrak q_j^{(m_j)}$ and since
...
which is finite over a modification of $\Spec(A)$ (see Lemma \ref{}).
Picture
$$
\xymatrix{
Y \ar[d]_\pi \ar[r]_g & \Spec(B) \ar[d] \\
X \ar[r] & \Spec(A)
}
$$
