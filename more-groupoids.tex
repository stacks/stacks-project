...
The idea behind the results in this section is that,
if the quotient stack $[U/R]$
(Groupoids in Spaces, Section \ref{spaces-groupoids-section-stacks})
is algebraic, then the fibres of
$s: R \to U$ are base changes of the fibres of the morphism $U \to X$.
Similarly, local properties of $R \to U$ should reflect local
properties of the (perhaps nonexistent) morphism $U \to [U/R]$. This doesn't
completely work, but we can make some of it work without knowing
that $[U/R]$ exists.
...
\noindent
Here is a second lemma. The $R$-invariant open $W \subset U$ should be
thought of as corresponding to the largest open of $[U/R]$ over which
...
\begin{proof}
Consider the following part of diagram (\ref{equation-pull})
$$
\xymatrix{
U' \times_{g, U, t} (R \times_{t, U, t} R)
\ar[rr]_-{c \circ (i, 1)}
...
$$
This diagram is commutative and both squares are cartesian.
...
open subscheme on which $c \circ (i, 1)$ is flat. Since the projection
