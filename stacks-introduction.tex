...
\noindent
In this short chapter we give an informal introduction of algebraic stacks.
...
$$
This means you work fibred categories over the categories of schemes,
...
\medskip\noindent
I hope you see how to add more/different objects to this enlarged category.
...
We have the formula
$S \times_{\mathcal{M}_{1, 1}} S' = (S \times S') \times_{\mathcal{M}_{1, 1} \times \mathcal{M}_{1, 1}} \mathcal{M}_{1, 1}$.
...
$\mathcal{M}_{1, 1}$ and $\Sch$ the category of all schemes).
...
\noindent
The first condition is the ``sheaf'' condition. We're going to spell it
out since there is a cocycle condition that you may not be aware of.
...
More precise statement of the ``glueing'' condition: given
...
\medskip\noindent
As you can see even the statement of this condition is somewhat annoying.
The proof is more interesting but it still takes a certain amount of mental
agility to boil it down to a manageable statement that you can try to
prove on a napkin. Perhaps the simplest variant which is already a bit
interesting is the following: Suppose we have a Galois extension
...
\noindent
An important part of proving $\mathcal{M}_{1, 1}$ is an algebraic stack
is finding a smooth cover. In fact, it turns out that the existence of
a smooth cover {\it implies}\footnote{This is a bit of a cheat because
in checking the smoothness you have to prove something very close to
the key fact -- since after all the smoothness is defined in terms of
...
that you are trying to show provides the smooth cover.} the other two
conditions! In the case of elliptic curves we use the Weierstrass equation
to construct one.
...
\begin{proof}
Very rough sketch of one way to prove this. Consider the sub group scheme
$$
H =
\left\{
\left(
...
$$
\begin{matrix}
\mathcal{M}_{1, 1} \to \text{Spec}(\mathbf{Z}) \\
\text{is smooth}
\end{matrix}
\Leftrightarrow
\begin{matrix}
W \to \text{Spec}(\mathbf{Z}) \\
\text{is smooth}
\end{matrix}
$$
