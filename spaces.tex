...
base, and morphisms of type $\mathcal{P}$ satisfy descent for fppf covering,
...
Let $S$ be a scheme in $\textit{Sch}_{fppf}$.
...
Lemma \ref{lemma-fibre-product-spaces}.
\end{proof}
...
\item We say $F$ is {\it weakly locally separated over $S$}\footnote{This is
probably nonstandard notation.} if $\Delta$ is an
immersion.
\item We say $F$ is {\it locally separated over $S$} if $\Delta$ is a
...
Topologies, Lemma \ref{topologies-lemma-change-alpha-Zariski}.
...
Let $S \to S'$ be a morphism of this site.
Let $F$ be an algebraic space over $S$.
Suppose that $T$ is a scheme over $S'$ and $f : T \to F$ is a morphism
from $T$ to the algebraic space $j_!F$ over $S'$.
\begin{enumerate}
\item the composition $p \circ f : T \to j_!F \to S$ turns $T$ into a scheme
over $S$, and $f$ corresponds to a morphism $\tilde f : T \to F$ from
this scheme over $S$ into $F$,
\item for any property $\mathcal{P}$ as in
Definition \ref{definition-relative-representable-property}
we have $\mathcal{P}(f) \Leftrightarrow \mathcal{P}(\tilde f)$.
\end{enumerate}
\end{lemma}

\begin{proof}
The first assertion is immediate from Lemma \ref{lemma-change-base-scheme}
