...
size of $U$.} and
a surjective \'etale morphism $U \to X'$ in $\Sh((\Sch'/S)_{fppf})$
such that $R \cong U \times_{X'} U$ in $\Sh((\Sch'/S)_{fppf})$.
\end{lemma}
...
such that there exists $U, R \in \Ob((\Sch/S)_{fppf})$
as in the statement. Then Lemma \ref{lemma-space-presentation} and
Theorem \ref{theorem-presentation} imply that $X = h_U/h_R$ is an object of
