...
\begin{proof}
Let $U \in \text{Ob}((\textit{Sch}/S)_{fppf})$. Let
...
\item universally closed, see
Morphisms, Definition \ref{morphisms-lemma-composition-proper},
\item (quasi-)separated, see
Schemes, Lemma \ref{schemes-lemma-separated-permanence},
\item monomorphism, see
...
Let $S$ be a scheme contained in $\textit{Sch}_{fppf}$.
Let $H$ be a sheaf on $(\textit{Sch}/S)_{fppf}$ whose diagonal
...
Finally, let $U, V \in \text{Ob}((\textit{Sch}/S)_{fppf})$
...
$(\textit{Sch}/S)_{fppf}$, then $F$ is an algebraic space.
\end{lemma}
...
$$
X = \text{Spec}(\overline{\mathbf{Q}})/G
\longrightarrow
S = \text{Spec}(\mathbf{Q}).
$$
...
$\textit{Spaces}'/S$ the category of algebraic spaces over $S$
defined using $\textit{Sch}'_{fppf}$. The construction of
...
$U \in \text{Ob}((\textit{Sch}/S)_{fppf})$
and a surjective \'etale morphism
$U \to X'$ in the category $\textit{Sh}((\textit{Sch}'/S)_{fppf})$.
\end{lemma}
...
such that there exists an object $U \in \text{Ob}((\textit{Sch}/S)_{fppf})$
and a surjective \'etale morphism $h'_U \to X$ (with notation
as in the proof of
Lemma \ref{lemma-change-big-site}).
Let $R$ be an object of $(\textit{Sch}'/S)_{fppf}$ representing
$h'_U \times_X h'_U$. Note that $R \to U \times_S U$ is a monomorphism
of schemes. Hence by
Sets, Lemma \ref{sets-lemma-what-is-in-it}
the scheme $R$ is isomorphic to an object of $(\textit{Sch}/S)_{fppf}$
and hence we may (after replacing $R$ by an isomorphic scheme) assume
$R \in \text{Ob}((\textit{Sch}/S)_{fppf})$. Now we use
...
of $(\textit{Sch}/S')_{fppf}$ at the object $S/S'$. Hence
...
we have seen that the category of sheaves on $(\textit{Sch}/S)_{fppf}$
is equivalent to the category of pairs $(\mathcal{F}', \mathcal{F}' \to h_S)$
consisting of a sheaf on $(\textit{Sch}/S')_{fppf}$ and a map of sheaves
...
Let $\textit{Sch}_{fppf}$ be a big fppf site.
Let $S \to S'$ be a morphism of this site.
Let $F'$ be a sheaf on $(\textit{Sch}/S')_{fppf}$.
The following are equivalent:
\begin{enumerate}
\item The restriction $F'|_{(\textit{Sch}/S)_{fppf}}$
