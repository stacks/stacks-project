...
\item monomorphism, see
Descent, Lemma \ref{fpqc-descent-lemma-descending-property-monomorphism},
\item surjective, see
...
on the base, but in
Descent, Lemma \ref{fpqc-descent-lemma-descending-fppf-property-immersion}
...
Consider the property $\mathcal{P} = ``\text{surjective}''$.
...
we have already encountered such open coverings as a method of
construction algebraic spaces.
...
an algebraic space $F$ is one of the sited $\mathcal{T}$ described above.
\end{definition}
...
\noindent
In each of these cases the diagonal is actually quasi-finite
and separated, hence quasi-affine (by More on Morphisms,
...
We will use the following lemma as a convenient way to construct them.
...
\begin{enumerate}
\item If $G$ is finite then $[U/G]$ is Zariski locally quasi-separated.
\item If $G$ is finite and $U \to S$ is separated, then $[U/G]$
is separated.
\end{enumerate}
\end{lemma}
