...
\end{enumerate}
Add more as needed. Of these the following are also fpqc local on the base
...
Let $j = (s, t) : R \to U \times_S U$
be an etale equivalence relation on $U$ over $S$.
Let $U' \subset U$ be an open subscheme, and let
$R' = R|_{U'}$ be the restriction of $R$ to $U$.
Then $R'/U' \to R/U$ is representable, and is
an open immersion.
\end{lemma}

\begin{proof}
By Lemma \ref{lemma-pullback-etale-equivalence-relation}
we see that $R'$ is an etale equivalence 
relation on $U'$ over $S$.
Note that $R'$ is simply the inverse image of the
open $U'\times_S U' \subset U \times_S U$ under $j$.
By Groupoids, Lemma \ref{groupoids-lemma-quotient-groupoid-restrict}
the map of sheaves
$F' = U'/R' \to F = U/R$ is injective.

\medskip\noindent
Suppose that
$a : T \to U/R$ is a morphism from a scheme into $U/R$.
This means there exists an fppf covering
$\{\varphi_j : T_j \to T\}_{j \in J}$ of $T$ and
...
U \times_S U
$$
factor through $j : R \to U\times_S U$ by some maps
...
$$
commute. Consider the open subsets
$W_j = a_j^{-1}(U') \subset T_j$. Since $T_j \to T$ is
flat and of finite presentation the image $\varphi_j(W_j) \subset T$
