...
\begin{proof}
Denote $c : U \to R/U$ the morphism in question.
...
Let $j = (s, t) : R \to U \times_S U$
be an etale equivalence relation on $U$ over $S$.
Assume that $U$ is affine and maps into an affine of $S$.
Then the quotient $F = U/R$
is an algebraic space, and $U \to F$ is a etale and surjective.
\end{lemma}

\begin{proof}
Since $j : R \to U \times_S U$ is a monomorphism we see that
$j$ is separated. Since $U$ is affine and maps into an affine
we see that $U \times_S U$ is affine. Hence we see that $R$ is
separated. In particular the morphisms $s, t$ are separated as
well as etale.
...
We have to show that $T \times_{a, F, c} U$ is representable.
Suppose that $T = \bigcup V_i$ is an affine open covering of $T$.
If we can show that each
$$
V_i \times_{a|_{V_i}, F, c} U
$$
is representable, then we can apply
Schemes, Lemma \ref{schemes-lemma-glue-functors} and
conclude that $T \times_{a, F, c} U$ is representable (some
details omitted). Hence we may assume that $T$ is affine.

\medskip\noindent
As seen in the proofs of Lemmas
\ref{lemma-finding-opens} and
\ref{lemma-when-it-works-it-works}
there exists an fppf covering
$\{\varphi_i : T_i \to T\}_{i \in I}$ and
morphisms $a_i : T_i \to U$ such that
that
$a_i \times a_{i'} : T_i \times_T T_{i'} \to U\times_S U$
factors through $R$, and such that $c \circ a_i = \varphi_i \circ a$.
However, since $T$ is affine, we can assume that the covering
is a standard fppf covering, see
Topologies, Lemma \ref{topologies-lemma-fppf-affine} and
Definition \ref{topologies-definition-standard-fppf}.
This means that $I$ is finite and each $T_i$ is affine.
Setting $T' = \coprod_{i \in I} T_i$,
$\varphi = \coprod \varphi_i$ and $a' = \coprod a_i$ we
see that we reduce to a covering
$\{T' \to T\}$ consisting of a single affine scheme $T'$ and
a surjective, flat morphism $\varphi : T' \to T$
of finite presentation and a morphism $a' : T' \to U$ such
that $a' \times a' : T' \times_T T' \to U \times_S U$ factors
through $R$ and such that $c \circ a' = a \circ \varphi$.

\medskip\noindent
In this situation note that
$$
T' \times_{\varphi, T} T \times_{a, F, c} U =
T' \times_{c \circ a', F, c} U =
T' \times_{a', U, 1} U \times_{c, F, c} U =
T' \times_{a', U, t} R
$$
is representable.
...
By Lemma \ref{lemma-finding-opens} each $U_i/R_i \to U/R$
is representable, and an open immersion.
Suppose we can prove the lemma for each $(U_i, R_i)$.
Then by Lemma \ref{lemma-when-it-works-it-works} we see that
$U_i \to F_i$ is etale and surjective. It follows from
this that $\prod F_i$ is an algebraic space and
$U \to \coprod F_i$ is etale and surjective.
Hence by Lemma \ref{lemma-coproduct-representable-sheaf}
it follows that $F = U/R$ is an algebraic space.
\end{proof}
