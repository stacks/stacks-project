...
arrow is closed. As the left horizontal arrows are quasi-compact
...
Goal: Show that $R \to A$ is of finite type.
Say $y_1, \ldots, y_m$ generate $A'$ over $R'$.
Choose $x_1, \ldots, x_n$ in $A$ such that we may write
$y_j = \sum r_{ij} \otimes x_i$ for certain $r_{ij} \in R'$.
In this case the subring $A_1 \subset A$ generated by
the $x_i$ is of finite type over $R$ and has the property that
$R' \otimes_R A_1$ surjects onto $A'$. However, since $R \to R'$
is flat we have $R' \otimes_R A_1 \subset R' \otimes_R A = A'$
and hence equality holds.
And since $R \to R'$ is faithfully flat we conclude that
$A_1 \subset A$ is and equality too.
This implies that $f$ is locally of finite type.
Therefore Lemma \ref{lemma-descending-properties} applies and we win.
\end{proof}
...
Goal: Show that $R \to A$ is of finite presentation.
By Lemma \ref{lemma-descending-property-locally-finite-type}
or its proof we already know that $R \to A$ is of finite type.
Choose a presentation
$$
A = R[x_1, \ldots, x_n]/I
$$
which induces the presentation
$$
A' = R'[x_1, \ldots, x_n]/(R' \otimes_R I)
$$
as $R \to R'$ is flat. By assumption the ideal
$R' \otimes_R I$ is finitely generated over $R'[x_1, \ldots, x_n]$
(see Algebra, Lemma \ref{algebra-lemma-finite-presentation-any-presentation}).
Say $g_1, \ldots, g_m$ generate $R' \otimes_R I$ over $R'[x_1, \ldots, x_n]$.
Choose $f_1, \ldots, f_k$ in $I$ such that we may write
$g_j = \sum P_{ij} \otimes f_i$ for certain $P_{ij} \in R'[x_1, \ldots, x_n]$.
In this case the ideal $I_1 \subset I$ generated by
the $f_i$ is finitely generated over $R[x_1, \ldots, x_n]$
and has the property that $R' \otimes_R I_1$ surjects onto
$R' \otimes_R I$. However, since $R \to R'$
is flat we have $R' \otimes_R I_1 \subset R' \otimes_R I$
and hence equality holds.
And since $R \to R'$ is faithfully flat we conclude that
$I_1 \subset I$ is and equality too.
This implies that $f$ is locally of finite presentation.
Therefore Lemma \ref{lemma-descending-properties} applies and we win.
\end{proof}
...
\begin{lemma}
\label{lemma-descending-property-radicial-iso-local-rings}
For $t \in \{injective, surjective, bijective\}$
consider the property
$\mathcal{P}_t(f : X \to Y) =$``$f$ is radicial and
for every $x \in X$ the map
$\mathcal{O}_{Y, f(x)} \to \mathcal{O}_{X, x}$ is $t$''.
Then $\mathcal{P}_t$ is fpqc local on the base.
\end{lemma}

\begin{proof}
We have seen that all the required properties
of ``radicial'' in Lemma \ref{lemma-descending-property-radicial}
and its proof; we will use these without further mention below.
Let $t$ be one of ``injective'', or ``surjective''.
The property $\mathcal{P}_t$
is preserved under any base change.
Namely, given
...
$$
and a prime $\mathfrak q' \subset A'$ lying over $\mathfrak p' \subset R'$,
$\mathfrak q \subset A$, and $\mathfrak p \subset R$ we get
a commutative diagram
$$
\xymatrix{
A'_{\mathfrak q'} =
(R'_{\mathfrak p'} \otimes_{R_{\mathfrak p}} A_{\mathfrak q})_{\mathfrak q'} &
A_{\mathfrak q} \ar[l] \\
R'_{\mathfrak p'} \ar[u] &
R_{\mathfrak p} \ar[l] \ar[u]
}
$$
which shows that if $R_{\mathfrak p} \to A_{\mathfrak q}$ is an
injective (resp.\ surjective) so is $R'_{\mathfrak p'} \to A'_{\mathfrak q'}$.
It is immediate that property $\mathcal{P}_t$ is Zariski local on the base.

\medskip\noindent
To see that $\mathcal{P}_t$ it is fpqc local on the base
we reduce to the following algebra problem (in exactly the same
manner as in the proofs of
Lemmas \ref{lemma-descending-property-locally-finite-type}
and \ref{lemma-descending-property-locally-finite-presentation}).
Given: a diagram of rings as above with $R \to R'$ faithfully flat,
$R \to A$ and $R' \to A'$ radicial, and
$R'_{\mathfrak p'} \to A'_{\mathfrak q'}$
injective (resp.\ surjective) for {\it any} prime $\mathfrak q'$ of $A'$.
Goal: Prove $R_{\mathfrak p} \to A_{\mathfrak q}$ is injective
(resp.\ surjective) for any prime $\mathfrak q$ of $A$.

\medskip\noindent
The case: ``injective''. Choose a $\mathfrak q'$.
Since $R_{\mathfrak p} \to R'_{\mathfrak p'}$ is
faithfully flat it is injective. This, and injectivity of
$R'_{\mathfrak p'} \to A'_{\mathfrak q'}$ shows that
$R_{\mathfrak p} \to A_{\mathfrak q}$ is injective
for any $\mathfrak q \subset A$ (as $A \to A'$ is faithfully flat
hence surjective on the spectra).

\medskip\noindent
The case: ``surjective''.
Choose a prime $\mathfrak q'$.
Since $\mathfrak q'$ is the unique prime lying over $\mathfrak p'$
we have
$A'_{\mathfrak q'} = A'_{\mathfrak p'}$ by
Algebra, Lemma \ref{algebra-lemma-unique-prime-over-localize-below}.
Hence we see that $A'_{\mathfrak q'}$ is actually just equal
to $R'_{\mathfrak p'} \otimes_{R_{\mathfrak p}} A_{\mathfrak q}$.
Hence the surjective map
$R'_{\mathfrak p'} \to
R'_{\mathfrak p'} \otimes_{R_{\mathfrak p}} A_{\mathfrak q}$
is just the base change of the map $R_{\mathfrak p} \to A_{\mathfrak q}$
by the faithfully flat ring map $R_{\mathfrak p} \to R'_{\mathfrak p'}$.
We conclude that $R_{\mathfrak p} \to A_{\mathfrak q}$ is surjective
as desired.

\medskip\noindent
Hence property $\mathcal{P}_t$ satisfies the three assumptions
of Lemma \ref{lemma-descending-properties} and we win.
\end{proof}

\begin{remark}
\label{remark-be-careful}
In Lemma \ref{lemma-descending-property-radicial-iso-local-rings}
the condition of $f$ being radicial can be dropped for
$t$ equal to ``injective'' (as the proof shows). But we cannot
drop the condition when $t$ equals ``surjective'' or ``bijective''.
An example is the ring map
$R = \mathbf{Z}_{(7)} \to A = (\mathbf{Z}[x]/(x^2 - 2))_{(7, x - 3)}$
and the base change by
$R = \mathbf{Z}_{(7)} \to R' = \mathbf{Z}_7$ ($7$-adic integers).
\end{remark}
...
This implies that each $f_i$ is universally open, radicial
and induces an isomorphism on local rings.
By Lemmas \ref{lemma-descending-property-universally-open}
and \ref{lemma-descending-property-radicial-iso-local-rings}
we deduce that $f$ is universally open, radicial
and induces an isomorphism on local rings.
Hence $f$ is an open immersion (nice exercise in the definitions).
\end{proof}
...
This implies that each $f_i$ is universally closed, radicial
and induces a surjection on local rings.
By Lemmas \ref{lemma-descending-property-universally-closed}
and \ref{lemma-descending-property-radicial-iso-local-rings}
we deduce that $f$ is universally closed, radicial
and induces a surjection on local rings.
By Morphisms, Lemma \ref{morphisms-lemma-closed-immersion}
criterion (4) we conclude $f$ is a closed immersion.
\end{proof}
