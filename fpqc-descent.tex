...
in terms of schemes. Namely descent data for quasi-coherent sheaves
with respect to $\mathcal{U}$ correspond to descent data on modules
with respect to the ring map
$$
\Gamma(S, \mathcal{O})
\longrightarrow
\prod\nolimits_{i = 1, \ldots, n} \Gamma(U_i, \mathcal{O})
$$
...
$(V, \varphi) \mapsto f^*(V, \varphi)$ constructed in that lemma
...
\begin{enumerate}
\item Let $\mathcal{U} = \{U_i \to S'\}_{i \in I}$ and
...
Let $(Y_j, \varphi_{jj'})$ be a descent datum relative to the
...
\begin{proof}
This follows immediately from Lemma \ref{lemma-pullback} via the
...
$\{g_i : U_i \to V_j\}_{\alpha(i) = j}$ is a Zariski
...
(see Definition \ref{definition-property-local})
...
and
\item for any surjective flat morphisms of affines
$X \to S$, any descent datum $(V, \varphi)$ relative
...
\begin{proof}
We only sketch the proof. Let $S$ be a scheme.
...
\medskip\noindent
Let $\mathcal{V} : S = \bigcup_{j \in J} V_j$ be an affine open covering.
Set $W_{i, j} = \varphi_i^{-1}(V_j)$.
Consider the $\tau$-covering
$\mathcal{W} : \{W_{i, j} \to S\}_{(i, j) \in I \times J}$.
By Lemma \ref{lemma-refine-coverings-fully-faithful}
the pullback functor between categories of
descent data $DD(\mathcal{U}) \to DD(\mathcal{W})$ is fully faithful.
Hence we may pullback $(X_i, \varphi_{ii'})$ to a descent
datum
$(X_{i, j}, \varphi_{i,j,i',j'})$
relative to $\mathcal{W}$ and work with that.
This works because the morphisms
$X_{i, j} = W_{i, j} \times_{U_i} X_i \to W_{i, j}$
still have property $\mathcal{P}$, as
$\mathcal{P}$ is stable under base change.

\medskip\noindent
For a fixed $j$ there exist a standard $\tau$-covering
$\mathcal{V}_j : \{V_{j, a} \to V_j\}_{a = 1, \ldots, n}$ and
for each $a$ an index $i(a) \in I$ and a morphism
$V_{j, a} \to W_{i(a), j}$.
See Topologies, Lemmas
\ref{topologies-lemma-fpqc-affine},
\ref{topologies-lemma-fppf-affine},
\ref{topologies-lemma-etale-affine},
\ref{topologies-lemma-smooth-affine}, and
\ref{topologies-lemma-syntomic-affine}.
By pulling back the descent datum to this covering
we get a descent datum relative to $\mathcal{V}_j$.
Note that each morphism $X_{j, a} \to V_{j, a}$ has property
$\mathcal{P}$ as it is the base change of $X_{i(a), j} \to W_{i(a), j}$.
Assume for the moment that these descent data are
all effective. If so then we obtain schemes $Y_j \to V_j$, and
it will turn out that there is a natural descent datum
(construction omitted\footnote{It would be better to
first talk about base change for descent data, stacks, etc
before doing this argument in this ad hoc fashion.})
for these $Y_j$ relative to $\mathcal{V} : S = \bigcup_{j \in J} V_j$.
Hence by Lemma \ref{lemma-Zariski-refinement-coverings-equivalence}
we see that the $Y_j$ glue to a scheme $Y$ over $S$ as desired.

\medskip\noindent
This reduces us to the case where the covering is a
standard covering of affines
$\{U_i \to S\}_{i = 1, \ldots, n}$.
By Lemma \ref{lemma-family-is-one} this reduces to the covering
$\{\coprod U_i \to S\}$ for which we have assumed the
result in property (2) of the lemma.
\end{proof}
