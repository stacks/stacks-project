...
$$
and we set $R = A \times_C B$. If $B \to C$ is injective then
$R$ is a subring of $A$.
\end{situation}
...
and $C$ is its field of fractions. Then for every unit $u$ of $A$ either
$u \in R$ or $u^{-1} \in R$.
\end{lemma}

\begin{proof}
Namely, if the image $c$ of $u$ in $C$ is in $B$, then $u \in R$.
Otherwise, $c^{-1} \in B$
...
\item $C$ is the fraction field of $A$,
\item $B$ is a discrete valuation ring,
\item $C$ is the fraction field of $B$,
\end{enumerate}
Then we have the following two possibilities
\begin{enumerate}
\item[(a)] If $A^*$ is not contained in $R$, then
$\Spec(A) \to \Spec(R)$ and $\Spec(B) \to \Spec(R)$
are open immersions and $C = A \otimes_R B$.
\item[(b)] If $A^*$ is contained in $R$, then $B$ dominates one of
the local rings of $A$ at a maximal ideal, and $A \otimes B \to C$
is not surjective.
\end{enumerate}
\end{lemma}

\begin{proof}
Assumption (a) implies there is a unit of $A$ whose image in $C$ lies in
...
be a nonzero element. Since $\dim(A) = 1$ we see that $C = A_x$.
...
$C = A_x = \bigcup x^{-n}A = \bigcup x^{-1}R$.
...
\item $C$ is the fraction field of $B$,
\item $A \otimes B \to C$ is surjective.
\end{enumerate}
...
\begin{proof}
Special case: $B$ is integrally closed in $C$. This means that
...
\medskip\noindent
General case. Let $B' \subset C$ be the integral closure of $B$.
See Lemma \ref{lemma-semi-local-dimension-one-conductor}.
Then the special case applies to $R' = A \times_C B'$.
...
there exists an integer $n$ such that $x^n \in R = A \times_C B$.
...
leaves no primes except $(0)$. Thus $C = R_{xy} = R_x \otimes_A R_y$.
