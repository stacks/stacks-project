...
\section{Coherent sheaves on $\mathbf{P}^n$}
...
by $\text{Im}(s : \mathcal{O}(-1) \to \mathcal{O})$ which is
...
i_*\mathcal{G} \ar[r] \ar[d] & 0 \\
& 0 & 0 & 0
}
$$
Thus the induction hypothesis applies to the exact sequence
$0 \to \mathcal{H} \to \mathcal{O}_H^{\oplus r} \to \mathcal{G} \to 0$
...
\noindent
Some material leading up to a discussion of algebraic cuvrves.

\begin{lemma}
\label{lemma-affine}
Let $X$ be a scheme all of whose local rings are Noetherian of
dimension at most $1$. Let $j : U \to X$ be an open subscheme.
Then $R^pj_*\mathcal{F} = 0$, $p > 0$ for every quasi-coherent
$\mathcal{O}_U$-module $\mathcal{F}$.
\end{lemma}
...
ring of dimension at most $1$. In this case $X$ has a closed points
...
Let $X$ be an affine Noetherian scheme of dimension $1$.
Then any open subscheme $U \subset X$ is affine.
\end{lemma}
...
\begin{proof}
Let $\mathfrak m_B$ be the radical of $B$.
...
\end{enumerate}
Then then $\Spec(A) \to \Spec(R)$ and $\Spec(B) \to \Spec(R)$
are open immersions and $K = A \otimes_R B$.
\end{lemma}
...
leaves no primes except $(0)$. Thus $K = R_{xy} = R_x \otimes_A R_y$.
...
Let $A$ be a domain with fraction field $K$. Let $B \subset K$
be a Noetherian $1$-dimensional semi-local ring whose fraction
field is $K$. If $A \otimes B \to K$ is surjective, then there
exists an $x \in A$ such that $x^{-1}$ is in the radical of $B$.
\end{lemma}

\begin{proof}
By Lemma \ref{lemma-semi-local-dimension-one-conductor} we may assume that
$B$ is a semi-local Dedekind domain. Let $\mathfrak m_1, \ldots, \mathfrak m_n$
be the maximal ideals of $B$. Let $v_i : K \to \mathbf{Z}$, $i = 1, \ldots, n$
be the corresponding discrete valuations (see
Algebra, Lemma \ref{algebra-lemma-characterize-Dedekind}).
We are looking for a nonzero $x \in A$ with $v_i(x) < 0$ for
$i = 1, \ldots, n$. We will prove this by induction on $n$.

\medskip\noindent
If $n = 1$ and the result is wrong, then $A \subset B$ and the map
$A \otimes B \to K$ is not surjective, contradiction.

\medskip\noindent
If $n > 1$, then by induction we can find a nonzero $x \in A$ such that
$v_i(x) < 0$ for $i = 1, \ldots, n - 1$. If $v_n(x) < 0$ then we are
done, so we may assume $v_n(x) \geq 0$. By the base case we can find
$y \in A$ nonzero such that $v_n(y) < 0$. After replacing $x$ by a
power we may assume that $v_i(x) < v_i(y)$ for $i = 1, \ldots, n - 1$.
...
Let $f : X \to S$ be a morphism of schemes. Let $x \in X$ be a point with
image $s \in S$. If
\begin{enumerate}
...
As $f$ is of finite type, $B$ is of finite type over $A$.
...
is Noetherian of dimension at most $1$. Then there exists an affine
...
affine by Lemma \ref{lemma-affines-glued-in-closed-affine}.
...
of dimension at most $1$. Then there exists an affine open subscheme
