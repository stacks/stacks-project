...
we see that $H^0(X, \mathcal{O}_X)$ is a finite dimensional
...
\medskip\noindent
If $X$ is geometrically reduced, then $K/k$ is separable by
Lemma \ref{lemma-proper-geometrically-reduced-global-sections}.

\medskip\noindent
If $X$ is geometrically irreducible, then $X_{k'}$ is irreducible
for every field extension $k'/k$. We have
$$
K \otimes_k k' = H^0(X_{k'}, \mathcal{O}_{X_{k'}})
$$
by
Cohomology of Schemes, Lemma \ref{coherent-lemma-flat-base-change-cohomology}.
Since $K \otimes_k k'$ is Artinian
(being finite dimensional over $k'$, see
Algebra, Lemma \ref{algebra-lemma-finite-dimensional-algebra})
it is a product of Artinian local rings
(Algebra, Lemma \ref{algebra-lemma-artinian-finite-length}).
Since $X_{k'}$ is irreducible, we conclude that $K \otimes_k k'$
has a unique prime. This implies that $K/k$ is purely inseparable
because if not, then $k' \otimes_k \overline{k}$ would have more
than one prime ideal corresponding to the distinct $k$-embeddings
$k' \to \overline{k}$, see Fields, Lemma \ref{fields-lemma-separable-degree}.
See proof of Algebra, Lemma \ref{algebra-lemma-Galois-orbit} to see why
prime ideals of $K \otimes_k \overline{k}$ correspond
1-to-1 with embeddings $K \to \overline{k}$.

\medskip\noindent
If $X$ is geometrically integral, then $X$ is both geometrically reduced
and geometrically irreducible, see
Lemma \ref{lemma-geometrically-integral}.
Hence $K/k$ is both purely inseparable and separable
which implies $K = k$.
\end{proof}
...
Let $k$ be a field. Let $X \to \Spec(k)$ be of finite type.
Then there exists a finite purely inseparable extension $k \subset k'$
...
Then the stalk $A' = (\mathcal{O}_{X^\nu})_x$ is the
...
Alternatively, you can use Morphisms, Lemma
...
Since $A_{red}$ has finitely many minimal primes we see
that $L$ is the total ring of fractions of $A_{red}$
(Algebra, Lemma \ref{algebra-lemma-total-ring-fractions-no-embedded-points}).
Thus $A'$ is the integral closure of $A$ in the total
...
the maximal ideals of $A'$ (by going up, see
Algebra, Lemma \ref{algebra-lemma-integral-going-up}).
Thus the lemma now follows
from its algebraic counterpart:
More on Morphisms, Lemma \ref{more-algebra-lemma-number-of-branches-1}.
\end{proof}
...
(insert future reference here; this is related to
Lemma \ref{lemma-injective-tangent-spaces-unramified})
