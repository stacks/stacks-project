...
\begin{lemma}
\label{lemma-image-irreducibles}
Let $k \subset K$ be an extension of fields.
Let $X$ be a scheme over $k$.
For every irreducible component
$\overline{T} \subset X_K$ the image of $\overline{T}$ in $X$
is an irreducible component in $X$.
...
Hence we conclude that the image $p(T)$ of an irreducible component
is a closed irreducible subset which contains a generic point of an
irreducible component of $X$, hence $p(T)$ is an irreducible component of $X$.

\medskip\noindent
Finally we show that the map is surjective.
Surjectivity of $p$ implies that any generic point $\xi$
of an irreducible component $T$ of $X$
is the image of some point $\xi' \in X_{\overline{k}}$. Choose any
specialization $\eta \leadsto \xi'$ where $\eta$
is the generic point of an irreducible component of $X_{\overline{k}}$.
Then it is clear that $p(\eta) = \xi$ as there are no
points specializing to $\xi$. Then it is clear that
$p(\overline{\{\eta\}}) = T$ as desired.
\end{proof}
...
which maps to $T$, see
Lemma \ref{lemma-image-irreducibles}.
...
is a $\text{Gal}(\overline{k}/k)$-invarian closed subset of $X_{\overline{k}}$
