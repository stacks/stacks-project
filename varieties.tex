...
\begin{proof}
The local rings $\mathcal{O}_{X, x}$ and $\mathcal{O}_{Y, y}$
are essentially of finite type over $k$ and $K$, whence
Noetherian. Hence the equality of the outer two numbers
by flatness of $\mathcal{O}_{X, x} \to \mathcal{O}_{Y, Y}$
(Lemma \ref{lemma-change-fields-flat}) and
Algebra, Lemma \ref{algebra-lemma-dimension-base-fibre-equals-total}.
By Morphisms, Lemma \ref{morphisms-lemma-dimension-fibre-after-base-change}
we have $\dim_y(Y) = \dim_x(X)$. Since the transcendence degree
and dimension of the local ring add up to the dimension
...
Let $k'/k$ be a field extension. Let $X$ be a quasi-compact and
quasi-separated scheme over $k$ with $H^0(X, \mathcal{O}_X) = k$.
Then the pullback map $\text{Pic}(X) \to \text{Pic}(X_{k'})$ is injective.
\end{lemma}

\begin{proof}
Let $\mathcal{L}$ be an invertible $\mathcal{O}_X$-module
whose pullback to $X_{k'}$ is trivial. For any quasi-coherent
$\mathcal{O}_X$-module $\mathcal{F}$ on $X$ with pullback
$\mathcal{F}'$ on $X_{k'}$ we have
$H^0(X_{k'}, \mathcal{F}') = H^0(X, \mathcal{F}) \otimes_k k'$, see
Cohomology of Schemes, Lemma \ref{coherent-lemma-flat-base-change-cohomology}.
Thus $H^0(X, \mathcal{L})$ and $H^0(X, \mathcal{L}^{\otimes -1})$
are both nonzero. Choose nonzero elements $s$ and $t$ in
$H^0(X, \mathcal{L})$ and $H^0(X, \mathcal{L}^{\otimes -1})$.
Pulling back to $X_{k'}$ we see that $s$ and $t$ generate
$\mathcal{L}|_{X_{k'}}$ and $\mathcal{L}^{\otimes -1}|_{X_{k'}}$.
Thus the product $st \in H^0(X, \mathcal{L})$ is nonzero
as the pullback to $X_{k'}$ is nonzero. Then $st$ differs from
$1$ by an element of $k^*$ and we conclude that $s$
and $t$ define trivializations of $\mathcal{L}$ and
...
$$
\mathbf{Z} = \text{Pic}(\mathbf{P}^1_k) \longrightarrow \text{Pic}(X)
$$
...
will be in the kernel of the map $\mathbf{Z} \to \text{Pic}(X_{k'})$
...
the map $F_A : A \to A$, $a \mapsto a^p$ is a ring endommorphism
...
$F_A : A \to A$ of a ring $A$ of characteristc $p > 0$. See the
...
we see $F_{X/S} = (F_X, \text{id}_S) : X \longrightarrow X^{(p)}$
...
\begin{proof}
By Lemma \ref{lemma-frobenius} the morpisms $F_X : X \to X$ and the base change
...
\begin{proof}
We will use that $\text{Pic}(X) = H^1(X, \mathcal{O}_X^*)$, see
...
\begin{proof}
See Lemma \ref{lemma-regular-functions-proper-variety}
