...
of the faitfully flat map $A \to B$), we conclude
...
after pulling back to $X_B$ and faithfull flatness of $R \to R \otimes_A B$.
Some details omitted.
\end{proof}
...
\end{enumerate}
Then the pullback map $\text{Pic}(X) \to \text{Pic}(X_B)$ is injective.
\end{lemma}

\begin{proof}
Let $\mathcal{L}$ be an invertible $\mathcal{O}_X$-module
whose pullback $\mathcal{L}'$ to $X_{k'}$ is trivial.
