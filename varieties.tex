...
numbers for invertible modules.
...
If this number is zero, then the function is constant with vale
...
$$
0 \to \mathcal{L}_1^{\otimes -1} \xrightarrow{s} \mathcal{O}_X
\to \mathcal{O}_{Z(s)} \to 0
$$
Set
$P(n_1, \ldots, n_d) =
...
and
$Q(n_1, \ldots, n_d) =
\chi(Z(s), \mathcal{L}_1^{\otimes n_1} \otimes \ldots \otimes
\mathcal{L}_d^{\otimes n_d})$.
...
of $n_2 \ldots n_d$ in $Q$. By induction this coefficient is positive
...
\begin{proof}
Since $f$ is finite the sheaf $f_*\mathcal{O}_Y$ is a
coherent $\mathcal{O}_X$-module. By the projection formula
(Cohomology, Section \ref{cohomology-section-projection-formula}) and
Cohomology of Schemes, Lemma
\ref{coherent-lemma-relative-affine-cohomology}
we have
$$
\chi(Y, f^*\mathcal{L}^{\otimes n}) =
\chi(X, f_*\mathcal{O}_Y \otimes_{\mathcal{O}_X} \mathcal{L}^{\otimes n})
$$
Since $\deg(f)$ is equal to the rank of $f_*\mathcal{O}_Y$ in the generic
point of $X$ we can apply
Lemma \ref{lemma-numerical-polynomial-leading-term}
to see that the above is equal to
$$
\deg(f) \chi(X, \mathcal{L}^{\otimes n})
$$
up to adding a polynomial in $n$ of degree $< \dim(X)$. Since
$\deg_\mathcal{L}(X)$ is the leading coefficient of
$\chi(X, \mathcal{L}^{\otimes n})$ scaled by $\dim(X)!$ and
since $\deg_{f^*\mathcal{L}}(Y)$ is the leading coefficient
of $\chi(Y, f^*\mathcal{L}^{\otimes n})$ scaled by the same
coefficient the lemma is proved.
\end{proof}
