...
$\dim(X) = r$ and $\dim(Y) = s$, and $X \cap Y$ is nonempty.
Then $\dim(X \cap Y) \geq r + s - n$.
\end{lemma}
...
Since $X$ is irreducible, we see that $X \to \Spec(k)$ has
relative dimension $r$ and similarly for $Y$. Then the morphism
$X \times Y \to \Spec(k)$ which we can factor as
$X \times Y \to Y \to \Spec(k)$ has relative dimension $r + s$
(Morphisms, Lemmas \ref{morphisms-lemma-base-change-relative-dimension-d} and
...
r + s - n
$$
This implies that result by Lemma \ref{lemma-dimension-locally-algebraic}.
\end{proof}
...
More precisely, it is the morphism associated to
...
\longrightarrow
k\left[T_0, \ldots, T_n, \frac{1}{T_i}\right]
$$
which is smooth of relative dimension $1$. Hence
$\pi^{-1}(X)$ and $\pi^{-1}(Y)$ are nonempty closed subsets of
...
Let $\overline{V}$ and $\overline{W}$ be the closure of $V$ and $W$
in $\mathbf{A}^{n + 1}$. Since $0 \in \mathbf{A}^{n + 1}$
...
Since the fibres of $\pi$ have dimension $1$ this implies
that $\pi(V \cap W) \subset X \cap Y$ has dimension at least
$r + s - n$ as desired (some details omitted).
\end{proof}
