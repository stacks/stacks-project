...
Coherent, Lemma \ref{coherent-lemma-property-higher-rank-cohomological}
are satisfied.
...
$$
Consider the $k$-algebra $R = \Gamma(X, \mathcal{O}_{Z(s)})$.
If $Z(s)$ is disconnected, then either $R$ is zero
in case $Z(s) = \emptyset$ or $R$ contains a nontrivial
idempotent in case $Z(S) = U \amalg V$ with
$U, V \subset Z(s)$ open and nonempty (the reader may wish to consult
Lemma \ref{lemma-proper-geometrically-reduced-global-sections}).
Thus the map $\Gamma(X, \mathcal{O}_X) \to R$ cannot be an isomorphism.
It follows that eiter $H^0(X, \mathcal{L}^{\otimes -n})$ or
