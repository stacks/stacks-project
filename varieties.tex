...
Let $X \to \text{Spec}(k)$ be a smooth morphism where $k$ is a field.
Then $X$ is geometrically normal over $k$.
\end{lemma}

\begin{proof}
Let $k'$ be a finite purely inseparable extension of $k$.
It suffices to prove that $X_{k'}$ is normal, see
...
Hence it suffices to show that a scheme $X$ smooth over a field is normal.
We see that $X$ is regular by
Lemma \ref{lemma-smooth-regular}. Hence
