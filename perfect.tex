...
\begin{proof}
Let $E$ be a perfect object of $D(\mathcal{O}_X)$. Let $a, b \in \mathbf{Z}$
be integers such that $E$ has tor amplitude in $[a, b]$.
The existence of such a pair of integers follows from
Cohomology, Lemma \ref{cohomology-lemma-perfect}
and the fact that $X$ is quasi-compact. We will prove
by induction on $b - a$ that $E$ can be represented by
$0$ if $b < a$ or by a complex $\mathcal{E}^a \to \ldots \to \mathcal{E}^b$
with $\mathcal{E}^i$ finite locally free if $b \geq a$.
The base case is the case $b - a < 0$ and
then indeed $E = 0$ and the induction assumption is true.
...
of quasi-coherent $\mathcal{O}_X$-modules. Observe that
$$
H^b(E) =  H^b(\mathcal{F}^\bullet) =
\Ker(\mathcal{F}^b \to \mathcal{F}^{b + 1})/
\Im(\mathcal{F}^{b - 1} \to \mathcal{H}^b)
$$
...
$H^b(\mathcal{H}^\bullet) = H^b(E)$ is surjective.
...
the resolution property of $X$. This produces a map in the derived category
$$
\mathcal{E}^b[-b] \longrightarrow E
$$
which determines a surjection onto the last nonzero cohomology sheaf
of $E$, namely $H^b(E)$. Thus if we denote $E'$ the cone on this map,
then we have a distinguished triangle
$$
\mathcal{E}^b[-b] \to E \to E' \to \mathcal{E}^b[-b + 1]
$$
It follows from Cohomology, Lemma \ref{cohomology-lemma-cone-tor-amplitude}
that $E'$ has tor amplitude in $[a, b]$ but since $H^b(E') = 0$
by construction, we find that actually $E'$ has tor amplitude in
$[a, b - 1]$. Of course, $E'$ also is perfect
(Cohomology, Lemma \ref{cohomology-lemma-two-out-of-three-perfect})
and by induction hypothesis we find a complex
$\mathcal{E}^a \to \ldots \to \mathcal{E}^{b - 1}$
of finite locally free $\mathcal{O}_X$-modules representing $E'$.
A compuation shows that the map $E' \to \mathcal{E}^b[-b + 1]$
is given by map $\mathcal{E}^{b - 1} \to \mathcal{E}^b$ whose
composition with $\mathcal{E}^{b - 2} \to \mathcal{E}^{b - 1}$
is zero. Rotating the triangle we find that
the complex $\mathcal{E}^a \to \ldots \to \mathcal{E}^b$
represents $E$ and the proof is complete.
\end{proof}
