...
This is true because $f_*\mathcal{I}^n$ is flasque by
...
Let $f : X \to S$ be a morphism of schemes which is flat and locally of finite presentation. Let $E$ be a pseudo-coherent object of $D(\mathcal{O}_X)$. The following are equivalent
\begin{enumerate}
    \item $E$ is $S$-perfect, and
    \item For every point $s \in S$, the object $L(X_s \to X)^* (E)$ of $D(\mathcal{O}_{X_s})$ is locally bounded.
\end{enumerate}
\end{lemma}

\begin{proof}
Since everything is local we immediately reduce to the
case that $X$ and $S$ are affine. This case
follows from More on Algebra, Lemmas
\ref{more-algebra-lemma-lift-from-fibre-relativelu-perfect} 
and \ref{more-algebra-lemma-tor-amplitude-localization}
via the dictionary of Lemma
\ref{lemma-affine-locally-rel-perfect} and (the affine
case of) Lemma \ref{lemma-flat-base-change-tor-independent}.
\end{proof}
...
\label{detecting-boundedness}

\noindent
In this section, we show that compact generators of $D_\QCoh$ of a quasi-compact, quasi-separated scheme, as constructed in Section \ref{section-generating}, have a special property. We recommend reading that section first as it is very similar to this one.
...
immersion and let $K$ be the perfect object of $D(\mathcal{O}_X)$
corresponding to the Koszul complex on $f_1, \ldots, f_r$ over $A$.
For $E \in D_\QCoh(\mathcal{O}_X)$ and $a \in \mathbf{Z}$, the following are equivalent
\begin{enumerate}
\item The canonical map $\tau_{\geq a}E \to \tau_{\geq a} Rj_*(E|_U)$ is an isomorphism, and
\item $\Hom_{D(\mathcal{O}_X)}(K[-n], E) = 0$ for all $n \geq a$.
\end{enumerate}
\end{lemma}

\begin{proof}
Choose a distinguished triangle $N \to E \to Rj_*(E|_U) \to N[1]$. Then (1) is equivalent to $\tau_{\geq a} N = 0$. Observe that
$$
\Hom_{D(\mathcal{O}_X)}(K[n], Rj_*(E|_U)) =
\Hom_{D(\mathcal{O}_U)}(K|_U[n], E) = 0
$$
for all $n$ as $K|_U = 0$. Thus (2) is equivalent to $\mathrm{Hom} _{D(\mathcal{O}_X)} (K[-n], N) = 0$ for all $n \geq a$. 
Observe that there are distinguished triangles
$$
K^\bullet(f_1^{e_1}, \ldots, f_i^{e'_i}, \ldots, f_r^{e_r}) \to
K^\bullet(f_1^{e_1}, \ldots, f_i^{e'_i + e''_i}, \ldots, f_r^{e_r}) \to
...
of Koszul complexes, see
More on Algebra, Lemma \ref{more-algebra-lemma-koszul-mult}.
Hence if $\Hom_{D(\mathcal{O}_X)}(K[-n], N) = 0$ for all $n \geq a$
then the same thing is true for $K$ replaced by
$K_e$ as in Lemma \ref{lemma-represent-cohomology-class-on-closed}.
Since $N|_U = 0$, that lemma implies that $H^n(X, N) = 0$ for $n \geq a$.
Our lemma follows since $N$
is determined by the complex of $A$-modules $R\Gamma(X, N)$, see
Lemma \ref{lemma-affine-compare-bounded}.
\end{proof}
...
For $E \in D_\QCoh(\mathcal{O}_X)$ and $a \in \mathbf{Z}$, the following are equivalent
\begin{enumerate}
\item The canonical map $\tau_{\leq a}E \to \tau_{\leq a} Rj_*(E|_U)$ is an isomorphism, and
\item $\Hom_{D(\mathcal{O}_X)}(K[-n], E) = 0$ for all $n \leq a$.
\end{enumerate}
\end{lemma}

\begin{proof}
Choose a distinguished triangle $E \to Rj_*(E|_U) \to N \to E[1]$. Then (1) is equivalent to $\tau_{\leq a} N = 0$. Observe that
$$
\Hom_{D(\mathcal{O}_X)}(K[n], Rj_*(E|_U)) =
\Hom_{D(\mathcal{O}_U)}(K|_U[n], E) = 0
$$
for all $n$ as $K|_U = 0$. Thus (2) is equivalent to $\mathrm{Hom} _{D(\mathcal{O}_X)} (K[-n], N) = 0$ for all $n \leq a$. 
Observe that there are distinguished triangles
$$
K^\bullet(f_1^{e_1}, \ldots, f_i^{e'_i}, \ldots, f_r^{e_r}) \to
K^\bullet(f_1^{e_1}, \ldots, f_i^{e'_i + e''_i}, \ldots, f_r^{e_r}) \to
...
of Koszul complexes, see
More on Algebra, Lemma \ref{more-algebra-lemma-koszul-mult}.
Hence if $\Hom_{D(\mathcal{O}_X)}(K[-n], N) = 0$ for all $n \leq a$
then the same thing is true for $K$ replaced by
$K_e$ as in Lemma \ref{lemma-represent-cohomology-class-on-closed}.
Since $N|_U = 0$, that lemma implies that $H^n(X, N) = 0$ for $n \leq a$.
Our lemma follows since $N$
is determined by the complex of $A$-modules $R\Gamma(X, N)$, see
Lemma \ref{lemma-affine-compare-bounded}.
\end{proof}
...
\begin{enumerate}
    \item $\mathrm{Hom}_{D(\mathcal{O}_X)} (P[-i], E) = 0$ for $i \gg 0$, and
    \item $\mathrm{Hom}_{D(\mathcal{O}_X)} (P[-i], \tau_{\geq a} E) = 0$ for $i \gg 0$.
\end{enumerate}
\end{lemma}

\begin{proof}
Using the triangle $ \tau _{< a} E \to E \to \tau _{\geq a} E \to$
we see that the equivalence follows if we can show
$$ 
\mathrm{Hom}_{D(\mathcal{O}_X)} (P[-i], \tau_{< a} E) = H^i (X, P^{\vee} \otimes_{\mathcal{O}_X} ^{L} \tau_{<a} E) = 0 
$$
for $i \gg 0$. This is true since $X$ is quasi-compact and
quasi-separated and $P^{\vee} \otimes_{\mathcal{O}_X} ^{L} \tau_{<a} E \in D^-_{\QCoh} (\mathcal{O}_X),$
see Lemma \ref{lemma-quasi-coherence-direct-image}.
\end{proof}

\begin{lemma}
\label{lemma-bounded-below-truncation}
Let $X$ be a quasi-compact and quasi-separated scheme. Let $P \in D_{perf} (\mathcal{O}_X)$ and $E \in D_{\QCoh} (\mathcal{O}_X).$ Let $a \in \mathbf{Z}$. The following are equivalent
\begin{enumerate}
    \item $\mathrm{Hom}_{D(\mathcal{O}_X)} (P[-i], E) = 0$ for $i \ll 0$, and
    \item $\mathrm{Hom}_{D(\mathcal{O}_X)} (P[-i], \tau_{\leq a} E) = 0$ for $i \ll 0$.
\end{enumerate}
\end{lemma}

\begin{proof}
Using the triangle $ \tau _{\leq a} E \to E \to \tau _{> a} E \to$
we see that the equivalence follows if we can show
$$ 
\mathrm{Hom}_{D(\mathcal{O}_X)} (P[-i], \tau_{> a} E) = H^i (X, P^{\vee} \otimes_{\mathcal{O}_X} ^{L} \tau_{> a} E) = 0 
$$
for $i \ll 0$. This is true since $P^\vee \otimes ^{L}_{\mathcal{O}_X} \tau_{> a} E  \in D^+_\QCoh (\mathcal{O}_X)$ 
as $P$ is perfect, and since the cohomology groups of a bounded
below complex of sheaves vanish in degrees $\ll 0$ 
(see Cohomology of Sheaves, Section \ref{cohomology-section-derived-functors}).
\end{proof}
...
be a perfect complex which generates 
$D_\QCoh (\mathcal{O}_X).$ Let
$E \in D_\QCoh (\mathcal{O}_X)$. The following are
equivalent
\begin{enumerate}
    \item $E \in D^-_\QCoh (\mathcal{O}_X)$, and
    \item $\Hom_{D(\mathcal{O}_X)} (G[-i], E) = 0$ for $i \gg 0$.
\end{enumerate}
\end{proposition}

\begin{proof}
(1) $\implies$ (2) follows from Lemma \ref{lemma-quasi-coherence-direct-image}. 
Let's prove (2) $\implies$ (1). First, note that if (2) holds
for one perfect generator $G$ then it holds for any: In fact 
(2) is equivalent to $R\Hom_{\mathcal{O}_X} (P, E) \in D^- (\mathbf{Z})$
for every perfect complex $P$ on $X$. This is because $G$ is a 
classical generator for $D_{perf} (\mathcal{O}_X)$ (see Lemmas \ref{lemma-quasi-coherence-direct-sums} and \ref{theorem-bondal-van-den-Bergh} and Derived Categories, Lemma \ref{derived-proposition-generator-versus-classical-generator}) and $D^-_\QCoh (\mathcal{O}_X)$
is a saturated triangulated subcategory of $D_\QCoh (\mathcal{O}_X).$
We will prove (2) $\implies$ (1) using the induction principle of
Cohomology of Schemes, Lemma \ref{coherent-lemma-induction-principle}.

First, (2) $\implies$ (1) if $X$ is affine: By the comment above, we
are allowed to assume $G = \mathcal{O}_X[0]$ in which case (2) means
that $H^i (X, E) = 0$ for $i \gg 0$. (1) follows since $E$ is
...
$V$ an affine open, and assume the implication (2) $\implies$ (1) is
known for the schemes $U, V, U \cap V$. Suppose $E\in D_\QCoh(\mathcal{O}_X)$
satisfies (2) for some perfect generator $G$ of $D_\QCoh (\mathcal{O}_X)$.
By Lemma \ref{lemma-direct-summand-of-a-restriction} and Theorem \ref{theorem-bondal-van-den-Bergh} there exists a perfect
...
\begin{equation} \label{equation-detecting-bounded-above}
    K' = R (V \to X)_* K = R (V \to X)_! K,
\end{equation}
see Lemmas \ref{cohomology-lemma-pushforward-restriction} and 
\ref{cohomology-lemma-pushforward-perfect}. This is a perfect
complex on $X$ supported on the closed set $X \setminus U \subset V$
and isomorphic to $K$ on $V$. By the first paragraph of the
proof, we know $R\Hom_{\mathcal{O}_X} (Q, E)$ and
$R\Hom _{\mathcal{O}_X}(K', E)$ are bounded above.

By the second description of $K'$ in \ref{equation-detecting-bounded-above}
we have
$$
    \Hom _{D(\mathcal{O}_V)} (K[-i], E|_V) = \Hom _{D(\mathcal{O}_X)} (K'[-i], E) =  0
$$
for $i \gg 0$. Therefore, we may apply Lemma \ref{lemma-orthogonal-koszul-first-variant} to $E|_V$ to
obtain an integer $a$ such that $\tau _{\geq a} (E|_V)
= \tau _{\geq a} R (U \cap V \to V)_* (E|_{U \cap V}).$
Then $\tau _{\geq a} E = \tau _{\geq a} R (U \to X)_* (E |_U)$
(check that the canonical map is an isomorphism after restricting to
$U$ and to $V$). Hence using Lemma \ref{lemma-bounded-truncation} twice we see that
$$
    \mathrm{Hom}_{D(\mathcal{O}_U)} (Q|_U [-i], E|_U) = \mathrm{Hom}_{D(\mathcal{O}_X)} (Q[-i], R (U \to X)_* (E|_U)) = 0
$$
...
$\tau _{\geq a} E \in D^-_\QCoh(\mathcal{O}_X)$. Thus 
$E \in D^-_\QCoh (\mathcal{O}_X)$. 
\end{proof}

\begin{proposition}
\label{proposition-detecting-bounded-below}
Let $X$ be a quasi-compact and quasi-separated scheme. Let $G \in D_{perf} (\mathcal{O}_X)$
...
\begin{proof}
(1) $\implies$ (2) since $G^\vee \otimes ^{L}_{\mathcal{O}_X} E \in D^+_\QCoh (\mathcal{O}_X)$ 
as $G$ is perfect, and since the cohomology groups of a bounded
below complex of sheaves vanish in degrees $\ll 0$ 
(see Cohomology of Sheaves, Section \ref{cohomology-section-derived-functors}).
Let's prove (2) $\implies$ (1). First, note that if (2) holds
for one perfect generator $G$ then it holds for any: In fact 
...
First, (2) $\implies$ (1) if $X$ is affine: By the comment above, we
are allowed to assume $G = \mathcal{O}_X[0]$ in which case (2) means
that $H^i (X, E) = 0$ for $i \ll 0$. (1) follows since $E$ is
determined by $R\Gamma (X, E),$ see Lemma \ref{lemma-affine-compare-bounded}.
...
corresponding to the Koszul complex on $f_1, \dots , f_r$.
Let $K' \in D_{perf} (\mathcal{O}_X)$ be
\begin{equation} \label{equation-detecting-bounded-below}
    K' = R (V \to X)_* K = R (V \to X)_! K,
\end{equation}
see Lemmas \ref{cohomology-lemma-pushforward-restriction} and 
\ref{cohomology-lemma-pushforward-perfect}. This is a perfect
...
and isomorphic to $K$ on $V$. By the first paragraph of the
proof, we know $R\Hom_{\mathcal{O}_X} (Q, E)$ and
$R\Hom _{\mathcal{O}_X}(K', E)$ are bounded below.
...
$$
    \Hom _{D(\mathcal{O}_V)} (K[-i], E|_V) = \Hom _{D(\mathcal{O}_X)} (K'[-i], E) =  0
$$
for $i \ll 0$. Therefore, we may apply Lemma \ref{lemma-orthogonal-koszul-second-variant} to $E|_V$ to
obtain an integer $a$ such that $\tau _{\leq a} (E|_V)
= \tau _{\leq a} R (U \cap V \to V)_* (E|_{U \cap V}).$
Then $\tau _{\leq a} E = \tau _{\leq a} R (U \to X)_* (E |_U)$
(check that the canonical map is an isomorphism after restricting to
...
$Q|_U$, we have $E|_U \in D^+_\QCoh (\mathcal{O}_U).$ But then since
the functor $R (U \to X)_*$ preserves bounded below objects (see Section \ref{cohomology-section-derived-functors}) we get 
$\tau _{\leq a} E \in D^+_\QCoh(\mathcal{O}_X)$. Thus 
$E \in D^+_\QCoh (\mathcal{O}_X)$. 
\end{proof}
