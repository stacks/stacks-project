...
With $X$, $f_1, \ldots, f_c \in \Gamma(X, \mathcal{O}_X)$, and
$\mathcal{F}$ as in Remark \ref{remark-support-c-equations}.
If $\mathcal{F}$ is quasi-coherent, then
...
$$
by Lemma \ref{lemma-extended-alternating-represented}.
Given a local section $s$ of $\mathcal{F}$ we can consider the
local section
$$
\frac{s}{f_1 \ldots f_c}
$$
of $\mathcal{F}_{1 \ldots c}$. The image of this section in
$\mathcal{H}_Z^q(\mathcal{F})$ depends only on $s$ modulo
