...
$R\Hom(K, L)$ is in $D^+_{\textit{Coh}}(A)$.
Since $L$ is bounded below and $K$ is bounded below there is a
convergent spectral sequence
$$
\text{Ext}^p_A(K, H^q(L)) \Rightarrow \text{Ext}^{p + q}_A(K, L)
$$
and there are convergent spectral sequences
$$
\text{Ext}^i_A(H^{-j}(K), H^q(L)) \Rightarrow \text{Ext}^{i + j}_A(K, H^q(L))
$$
This finishes the proof as the modules $\text{Ext}^p_A(M, N)$
...
$\text{Ext}_{\mathcal{O}_X}^n(K, L)$ are finite.
\end{lemma}

\begin{proof}
Recall that
$$
\text{Ext}_{\mathcal{O}_X}^n(K, L) =
...
\item if $L$ is bounded, then $\text{Ext}^n_{D(\mathcal{O}_X)}(K, L)$
is zero for all but finitely many $n$.
\end{enumerate}
\end{lemma}

\begin{proof}
Part (2) follows from (1) as $\text{Ext}^n_{D(\mathcal{O}_X)}(K, L) =
...
Let $N$ be the integer of Lemma \ref{lemma-quasi-coherence-direct-image}
for the morphism $X \to \Spec(\mathbf{Z})$. Then we see that
$H^0(X, K^\vee \otimes_{\mathcal{O}_X}^\mathbf{L} L)$
...
$$
are zero. Namely, by the lemma cited, we have
$$
H^0(X, K^\vee \otimes_{\mathcal{O}_X}^\mathbf{L} L) =
...
\medskip\noindent
We still have to prove that $K = \text{cocolim} K_n$.
...
that $\text{Ext}^i_X(P, P)$ is zero for $i \not = 0$ and equal
to $R$ for $i = 0$, see
Differential Graded Algebra, Lemma
...
we find that
these $\text{Ext}$-groups are zero unless $i = 0$.
...
$$
has a right adjoint. If this right adjoint exists, we will denote it
$$
DQ_X\footnote{This is probably nonstandard notation. However, we have already
used $Q_X$ for the coherator and $RQ_X$ for its derived extension.}:
...
$$
Rg_*Lg^*Rf_*E \to Rg_*Rf'_*L(g')^*E = Rf_*(Rh_* L(g')^* E)
$$
...
and the second arrow comes from the given map $L(g')^*L \to K'$.
...
$$
in other words $X' = S' \times_S X$. Setting $E' = L(g')^*E$
...
$$
\text{Ext}^i_{\mathcal{O}_S}(L, \mathcal{F}) \longrightarrow
\text{Ext}^i_{\mathcal{O}_X}(E,
\mathcal{G}^\bullet \otimes_{\mathcal{O}_X} f^*\mathcal{F})
$$
for $\mathcal{F}$ quasi-coherent on $S$ compatible with boundary maps.
...
sheaves in degrees $\geq -m - 1 + a$ these $\text{Ext}$-groups are zero
for $i \leq m + 1$ by
...
(Cohomology, Lemma \ref{cohomology-lemma-dual-perfect-complex}).
We obtain an inverse system $\ldots E_3^\vee \to E_2^\vee \to E_1^\vee$
...
the maps
$$
\text{Ext}^i_{\mathcal{O}_S}(L_{n + 1}, \mathcal{F})
\longrightarrow
\text{Ext}^i_{\mathcal{O}_S}(L_n, \mathcal{F})
$$
...
$\text{Ext}^i_{\mathcal{O}_S}(L, \mathcal{F}) =
\text{Ext}^i_{\mathcal{O}_S}(L_n, \mathcal{F})$
...
Let $\mathcal{G}$ be a finitely presented $\mathcal{O}_X$-module,
...
Lemma \ref{lemma-quasi-coherence-direct-image}.
Now it suffices to show that $K$ is $m$-pseudo-coherent for
every $m$. Choose an approximation $P \to E$ by a perfect complex $P$
of $(X, E, m - N - 1)$. This is possible by
Theorem \ref{theorem-approximation}.
Choose a distinguished triangle
$$
P \to E \to C \to P[1]
...
in degrees $\geq m - N - 1$ and hence
the same is true for $C \otimes^\mathbf{L} \mathcal{G}$.
...
the sets
$$
H^i(U, K \otimes^\mathbf{L} E),\quad \text{Ext}^i(E|_U, K|_U)
$$
are countable.
\end{lemma}
...
$$
Note that $H^i(A) = \text{Ext}^i(E, E)$ and
$H^i(M) = \text{Ext}^i(E, K)$.
...
it suffices show that $\text{Ext}^i(E, E)$ and
$\text{Ext}^i(E, K)$ are countable for each $i$.
...
$$
R\Gamma(U, Lf^*K) = \text{hocolim} R\Hom(L_n, K)
$$
...
A perfect object of $D(\mathcal{O}_X)$ is $S$-perfect.
\end{lemma}
...
\medskip\noindent
Second proof: follows from Cohomology, Lemma \ref{cohomology-lemma-perfect}
and the claim that a complex in $D(\mathcal{O}_X)$ having
locally bounded tor dimension has locally bounded tor dimension
in $D(f^{-1}\mathcal{O}_S)$. The claim follows from the fact
that the ring homomorphisms
$\mathcal{O}_{S, f(x)} = (f^{-1}\mathcal{O}_S)_x \to \mathcal{O}_{X, x}$
are flat,
More on Algebra, Lemma \ref{more-algebra-lemma-flat-push-tor-amplitude},
and the criterion for tor dimension in terms of stalks from
Cohomology, Lemma \ref{cohomology-lemma-tor-amplitude-stalk}.
\end{proof}
...
and a flat morphism of finnite presentation $X_0 \to S_0$.
...
$$
is an isomorphism where $U = f_0^{-1}(U_0)$ and $U_i = f_{i0}^{-1}(U_i)$.
...
in \cite{lieblich-complexes} when it applies. Next,
suppose that $f : X \to S$ is only assumed to be locally
