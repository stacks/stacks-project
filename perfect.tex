...
(Properties, \ref{properties-proposition-coherator}).
...
is left adjoint to
$\Phi : D(\textit{QCoh}(\mathcal{O}_X)) \to D(\textit{QCoh}(\mathcal{O}_Y))$
because $\Phi$ is computed by evaluating $f_*$ on a K-injective
complex and $f^*$ is adjoint to $f_*$.
...
subset $T \setminus U_n$ contained in $U_1 \cup \ldots \cup U_{n - 1}$.
...
$RQ_X(Rj_*E|_U) = Rj_*RQ_U(E|_U)$. Finally, the maps
$$
RQ_U(A|_U) \to A|_U
\quad\text{and}\quad
E|_U \to RQ_U(E|_U)
...
object of $\textit{QCoh}(\mathcal{O}_X)$. Then $\mathcal{J}$
...
$\textit{QCoh}(\mathcal{O}_X)$ we can extend $\sigma$ to a map
...
(Properties, \ref{properties-proposition-coherator}), we can
...
are isomorphisms. We will prove this by induction on $n$:
the smallest integer $n \geq 0$ such that $E$ and $A$ are supported
on a closed subset of $X$ which is contained in the union of $n$
affine opens of $X$. If $n = 0$ then $E$ and $A$ are zero; this is
...
$U_1 \cup \ldots \cup U_n$ with $U_i \subset X$ affine open.
Set $U = U_n$. The inclusion morphism
$j : U \to X$ is a flat morphism of Noetherian schemes.
Consider the distinguished triangles
$$
...
E \to Rj_*(E|_U) \to E' \to E[1]
$$
where $\Phi$ is as in Lemma \ref{lemma-Noetherian-pushforward}.
Note that $A \to \Phi(A|_U)$ and $E \to Rj_*(E|_U)$ are quasi-isomorphisms
over $U = U_n$. Hence $A'$ and $E'$ are supported on the closed
subset $T \setminus U_n$ contained in $U_1 \cup \ldots \cup U_{n - 1}$.
By induction hypothesis the statement is true for $A'$ and $E'$. By
Derived Categories, Lemma \ref{derived-lemma-third-isomorphism-triangle}
it suffices to prove the maps
$$
RQ_X(\Phi(A|_U)) \to \Phi(A|_U)
\quad\text{and}\quad
Rj_*(E|_U) \to RQ_X(Rj_*E|_U)
$$
are isomorphisms. By
Lemmas \ref{lemma-Noetherian-pushforward} and
\ref{lemma-flat-pushforward-coherator} we have
$RQ_X(\Phi(A|_U)) = Rj_*(RQ_U(A|_U))$ and $RQ_X(Rj_*E|_U) = \Phi(RQ_U(E|_U))$.
Finally, the maps
$$
RQ_U(A|_U) \to A|_U
\quad\text{and}\quad
...
$D_{\textit{QCoh}}(\mathcal{O}_X)$ of a quasi-compact
...
For $E \in D_{\textit{QCoh}}(\mathcal{O}_X)$ the following are equivalent
\begin{enumerate}
\item $E = Rj_*(E|_U)$, and
...
\medskip\noindent
Namely, by Cohomology, Lemma \ref{cohomology-lemma-pushforward-restriction}
...
such that $D_{\textit{QCoh}}(X)$ is $R$-linearly equivalent to
...
of $D_{\textit{QCoh}}(\mathcal{O}_X)$. Set
$$
...
$$
D(A, \text{d}) \to D_{\textit{QCoh}}(\mathcal{O}_X) \to
D(B, \text{d})
$$
...
\begin{enumerate}
\item $E_0$ is perfect and $K_0 \in D_{\textit{QCoh}}(\mathcal{O}_{S_0})$, or
\item $E_0$ is pseudo-coherent and
$K_0 \in D_{\textit{QCoh}}(\mathcal{O}_{S_0})$ has finite tor dimension.
\end{enumerate}
\end{lemma}
