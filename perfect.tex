...
is an isomorphism. There is a canonical map
...
modules into quasi-coherent modules. Thus the functor
...
$$
Let $E^\wedge = R\SheafHom(E, \mathcal{O}_X)$. Since $E$ is pseudo-coherent
we see that
$R\SheafHom(E, \mathcal{G}) = E^\wedge \otimes^\mathbf{L} \mathcal{G}$
and $Lh^*E^\wedge = (E')^\wedge$ (insert future reference to duals of
pseudo-coherent complexes). The proof of
Lemma \ref{lemma-compare-base-change}
provides us with a canonical map
\begin{align*}
Lg^* Rf_*(E^\wedge \otimes^\mathbf{L} \mathcal{G})
& \to
Rf'_*(Lh^*(E^\wedge \otimes^\mathbf{L} \mathcal{G})) =
Rf'_*(Lh^*E^\wedge \otimes^\mathbf{L} Lh^*\mathcal{G})) \\
& \to
Rf'_*(Lh^*E^\wedge \otimes^\mathbf{L} h^*\mathcal{G})
\end{align*}
The second coming from the map $Lh^*\mathcal{G} \to h^*\mathcal{G}$.
In other words a canonical map
$$
...
\medskip\noindent
We will use the induction principle to prove this map is an isomorphism
(Cohomology on Schemes, Lemma \ref{coherent-lemma-induction-principle}).
If $X$ is affine, then we can represent $E$ by a bounded above complex of
free $\mathcal{O}_X$-modules and the result is clear. The induction step
is clear from Mayer-Vietoris. Some details omitted. FIXME.
\end{proof}
...
Choose an approximation $P \to E$ by a perfect complex $P$ of $(X, E, -m)$
...
sheaves in degrees $\geq -m$ these $\text{Ext}$-groups are zero
for $i \leq m$ (by
Derived Categories, Lemma \ref{derived-lemma-negative-exts}
and the spectral sequence for ext's -- insert
future reference here). This reduces us to the case that
