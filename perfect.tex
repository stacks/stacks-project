...
are supported on $Z$ hence it is an object of
$D_Z(\mathcal{O}_X)$. Thus we see that
...
The morphisms $U_{i_0 \ldots i_p} \to X$ are affine
as they are given over affine opens of $X$ by inverting the function
...
$$
functorial in $\mathcal{F}$. Namely, we have
$$
i_*\mathcal{H}^c_Z(\mathcal{F}) =
\Coker(\bigoplus \mathcal{F}_{1 \ldots \hat i \ldots c}
\to \mathcal{F}_{1 \ldots c})
$$
Given a local section $s$ of $\mathcal{F}$ we can consider the corresponding
local section
$$
\frac{s}{f_1 \ldots f_c}
$$
of $\mathcal{F}_{1 \ldots c}$. It is clear that the image of this section in
the cokernel $\mathcal{H}_Z^q(\mathcal{F})$ depends only on $s$ modulo
the image of $(f_1, \ldots, f_c) : \mathcal{F}^{\oplus c} \to \mathcal{F}$.
Since $i_*i^*\mathcal{F}$ is equal to the cokernel of
$(f_1, \ldots, f_c) : \mathcal{F}^{\oplus c} \to \mathcal{F}$
we see that we get our map (keep in mind that $i_*$ is fully faithful).
\end{remark}
