...
\end{enumerate}
Then $Rf_*E$ is a perfect object of $D(\mathcal{O}_X)$.
\end{lemma}
...
is bounded from above. To see it is bounded below we use that
$$
Rf_*(E) \otimes_{\mathcal{O}_S}^\mathbf{L} \mathcal{F} =
...
the description of $\epsilon^* = \text{id}_{small, \acute{e}tale, Zar}^*$
...
By
Descent, Remark \ref{descent-remark-change-topologies-ringed-sites} and
Lemma \ref{descent-lemma-quasi-coherent-gives-quasi-coherent}
given a quasi-coherent $\mathcal{O}_X$-module $\mathcal{F}$ the sheaf
$\mathcal{F}^a = \epsilon^*\mathcal{F}$ is a quasi-coherent
module on $X_{\acute{e}tale}$. Thus $\epsilon^*$ induces
$$
\epsilon^* : D_{QCoh}(\mathcal{O}_X) \to D_{QCoh}(\mathcal{O}_{\acute{e}tale})
$$
We will show below the functor
