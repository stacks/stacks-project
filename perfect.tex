...
\ref{perfect-lemma-application-nice-K-injective}.
...
$$
\Hom_X(M, K \otimes^\mathbf{L}_{\mathcal{O}_X} g'_*\mathcal{O}_{X'}) =
H^0\left(R\Gamma(X, R\SheafHom(M, K)) \otimes^\mathbf{L}_R R'\right)
$$
in the following two cases
\begin{enumerate}
...
$$
H^0(X, K \otimes^\mathbf{L}_{\mathcal{O}_X} g'_*\mathcal{O}_{X'})
= H^0(R\Gamma(X, K) \otimes^\mathbf{L}_A A')
$$
Note that the left hand side is equal to $H^0(X', L(g')^*K)$
