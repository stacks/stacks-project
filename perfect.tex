...
Lemma \ref{lemma-compare-base-change}.
If $K$ is perfect, then $K$ is bounded below and any base change
$K \otimes_R^\mathbf{L} R'$ is bounded below as well, hence
we see that (1) implies (2). Conversely, assume (2).
By More on Algebra, Lemma
\ref{more-algebra-lemma-lift-from-fibre-relativelu-perfect}
we see that $K_\mathfrak q$ is perfect over $A_\mathfrak q$
for all primes $\mathfrak q$ of $A$. Since $K$ is in $D^-(A)$
and pseudo-coherent it then follows from
More on Algebra, Lemma \ref{more-algebra-lemma-check-perfect-stalks}
that $K$ is perfect as desired.
\end{proof}
