...
Let $X$ be a scheme.
\begin{enumerate}
\item If $L$ in $D^+_\QCoh(\mathcal{O}_X)$ and
$K$ in $D(\mathcal{O}_X)$ is pseudo-coherent, then
$R\SheafHom(K, L)$ is in $D_\QCoh(\mathcal{O}_X)$.
\item If $X = \Spec(A)$ is affine then
$$
R\SheafHom(\widetilde{M^\bullet}, \widetilde{K^\bullet})
=
\widetilde{R\Hom(M^\bullet, K^\bullet)}
$$
for any bounded below complex of $A$-modules $M^\bullet$ and any bounded
above complex of finite free $A$-modules $K^\bullet$.
...
Hence to prove (1) we may replace $X$ by an affine open.
By Lemmas \ref{lemma-affine-compare-bounded} and
\ref{lemma-pseudo-coherent-affine}
in order to prove (1) it suffices to prove (2).

\medskip\noindent
Part (2) follows from the computation of the
internal hom of Cohomology, Lemma
\ref{cohomology-lemma-Rhom-complex-of-direct-summands-finite-free}.

\medskip\noindent
To prove (3) recall that on any ringed space the $n$th cohomology sheaf of
...
Hence (3) follows from the equivalence of categories of
Lemma \ref{lemma-affine-compare-bounded}.
\end{proof}
...
\ref{more-morphisms-definition-pseudo-coherent}.}.
