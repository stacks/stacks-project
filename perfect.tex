...
Let $X$ be a scheme.
\begin{enumerate}
\item If $X$ is Noetherian, then we denote $K'_0(X)$ the
...
$$
\item If $X$ is quasi-compact and quasi-separated, then we denote
$K_0(X)$ the {\it Grothendieck group of $X$}. It is the zeroth K-group
of the strictly full, saturated, triangulated subcategory
...
\noindent
Let $X$ be a quasi-compact and quasi-separated scheme.
Let us denote $\textit{Vect}(X)$ the category
...
$$
c : K_0(X) = K_0(D_{perf}(\mathcal{O}_X)) \longrightarrow K_0(\textit{Vect}(X))
$$
...
$$
\sigma_{\geq n}\mathcal{E}^\bullet \to \sigma_{\geq n - 1}\mathcal{E}^\bullet \to
\mathcal{E}^{n - 1}[-n + 1] \to (\sigma_{\geq n}\mathcal{E}^\bullet)[1]
$$
...
\begin{remark}
\label{remark-K-ring}
Let $X$ be a quasi-compact and quasi-separated scheme. The K-group
$K_0(X)$ is canonically a commutative ring. Namely, using the derived
tensor product
$$
...
\medskip\noindent
If $\textit{Vect}(X)$ is as above, then it is clearly the case that
$K_0(\textit{Vect}(X))$ also has a ring structure: if $\mathcal{E}$
and $\mathcal{F}$ are finite locally free $\mathcal{O}_X$-modules,
then we set
$$
