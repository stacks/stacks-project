...
\text{decent}.
$$
The notion of a very reasonable algebraic space is somewhat obsolete.
It was introduced because the assumption was need to prove some results
which are now proven for the class of reasonable spaces.
The class of decent spaces is the largest class of spaces $X$ where one can
hope to have a good relationship between the topology of $|X|$ and
...
\noindent
In this section we prove that a reasonable algebraic space
has a dense open subspace which is a scheme, and that locally
such an algebraic space has a finite stratification by subschemes.
...
\'etale and has degrees of fibres bounded bby $n - 1$. By induction
...
\begin{lemma}
\label{lemma-stratifiy-reasonable}
Let $S$ be a scheme. Let $X$ be a quasi-compact, reasonable algebraic space
over $S$. There exist an integer $n$ and open subspaces
$$
\emptyset = U_{n + 1} \subset
...
reasonable and decent algebraic spaces. To parse some of the statements
please keep in mind that a reasonable space is decent.
...
Let $S$ be a scheme. Let $X$ be a reasonable algebraic space over $S$.
...
Then $X \cong \Spec(k)$ for some field $k$.
\end{lemma}
...
As $X$ is decent there exists a monomorphism $\Spec(k) \to X$
and $V = \Spec(k) \times_X U$ is a scheme finite \'etale
over $\Spec(k)$. Namely, this
...
Morphisms, Lemma \ref{morphisms-lemma-etale-over-field}.
On the other hand $V \to U$ is a monomorphism (as $\Spec(k) \to X$
is a monomorphism) and surjective (as $\Spec(k) \to X$ is surjective by
Properties of Spaces,
...
$U \to X$ factors through $\Spec(k)$ which implies that
$\Spec(k) \to X$ is also a surjection of sheaves, whence
