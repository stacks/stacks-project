...
elementary distinguihsed squares. We first prove a slightly weaker
...
Decent Spaces, Lemma \ref{lemma-bounded-fibres} and
Decent Spaces, Definition \ref{definition-very-reasonable}.
...
\medskip\noindent
Part (2) is an immediate consequence of part (1) and the definitions.
\end{proof}
...
\item the dimension of the local ring of $X$ at $x$ is zero
(Properties of Spaces, Definition
...
\begin{proof}
Observe that any $Y$ as in (2) is decent by
Lemma \ref{lemma-etale-named-properties}.
Thus it suffices to prove the equivalence of (1) and (3)
as then the equivalence with (2) follows since the dimension
...
of $U$. Thus $\dim(\mathcal{O}_{U, u}) = 0$ and we see that (2) holds.

\medskip\noindent
Assume (2). The point $x$ is contained in an irreducible component
...
Then the dimension of the local ring of $Y$ at $y$ is $\leq$ to the
...
mapping to $x \in |X|$ and $v \in V$. Then $U \to U$ is locally
...
Let $S$ be a scheme. Let $f : X \to Y$ be a morphism of algebraic spaces
over $S$. Assume $f$ is quasi-compact and quasi-separated. Then
$f$ is universally closed if and only if the existence part of the
valuative criterion holds (Morphisms of Spaces, Definition
...
Namely, the implication in one direction is given by
Morphisms of Spaces, Lemma
\ref{spaces-morphisms-lemma-quasi-compact-existence-universally-closed}.
For the converse, assume $f$ is quasi-separated and quasi-compact and
...
$$
as in Morphisms of Spaces,
Definition \ref{spaces-morphisms-definition-valuative-criterion}.
A formal argument shows that the existence of the
desired diagram
$$
\xymatrix{
...
$$
can be reduced to the case of the morphism $X_A \to \Spec(A)$.
In this case the algebraic space $X_A$ is quasi-separated, hence
decent (property $(\gamma)$ of Lemma \ref{lemma-bounded-fibres}).
Hence the existence of $A \subset A'$ and the arrow
$\Spec(A') \to X_A$ follows from
Proposition \ref{proposition-characterize-universally-closed}.
\end{proof}
...
\ref{spaces-morphisms-definition-valuative-criterion}, and
...
\item the valuative criterion holds as in Morphisms of Spaces, Definition
\ref{spaces-morphisms-definition-valuative-criterion},
\item given any commutative solid diagram
$$
\xymatrix{
...
is representable, hence the base change $T \times_Y X \to X$ is represenble.
...
of algebraic spaces over $S$. If $X$ and $Y$ are decent
...
\begin{proof}
The morphism $p$ is quasi-finite at every $w \in F_{x, z}$, see
Morphisms of Spaces, Lemma
...
morphism of algebraic spaces is quasi-finite.
\end{proof}
...
Set $X_k = \Spec(k) \times_Y X$. Consider the following conditions
\begin{enumerate}
\item
\label{item-fibre-finite}
$f^{-1}(\{y\})$ is a finite subset of $|X|$,
\item
\label{item-fibre-discrete}
$f^{-1}(\{y\})$ is a discrete subset of $|X|$,
\item
\label{item-fibre-no-specializations}
$f^{-1}(\{y\})$ has only closed points,
\item
...
\medskip\noindent
The map $|X_k| \to f^{-1}(\{y\})$ is surjective by
Properties of Spaces, Lemma \ref{spaces-properties-lemma-points-cartesian}
and we see
