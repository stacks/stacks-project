...
In order to check the assumptions we use
Cohomology on Sites, Remark \ref{sites-cohomology-remark-colim-global}.
Namely, let $\mathcal{B} \subset \Ob(\mathcal{X})$ be the set
of objects lying over affine schemes. By our abuse of language we
may and do think of objects of $\mathcal{B}$ as affine schemes
$U$ over $\mathcal{X}$. We check each of the conditions (1) -- (4)
of the remark in turn:
\begin{enumerate}
\item Since $\mathcal{X}$ is quasi-compact, there exists a
$U \in \mathcal{B}$ such that $U \to \mathcal{X}$ is surjective
and smooth (Properties of Stacks, Lemma
\ref{stacks-properties-lemma-quasi-compact-stack}),
hence $h_U^\# \to *$ is surjective.
\item Since coverings in $\mathcal{X}$ as fppf coverings, we see that
every covering of $U \in \mathcal{B}$ is refined by a finite affine
fppf covering, see
Topologies, Lemma \ref{topologies-lemma-fppf-affine}.
\item Let $U \to \mathcal{X}$ and $U' \to \mathcal{X}$ be
in $\mathcal{B}$. The product $h_U^\# \times h_{U'}^\#$ in
$\Sh(\mathcal{X})$ is equal to the sheaf on $\mathcal{X}$
determined by the algebraic space $W = U \times_\mathcal{X} U'$.
...
a surjective \'etale morphism $U'' \to W$. All in all
we see that $h_{U''}^\# \to h_U^\# \times h_{U'}^\#$
is surjective as desired.
\item Let $a, b : U \to U'$ be a morphism of affine schemes
over $\mathcal{X}$. Then the equalizer of $a$ and $b$ is an
