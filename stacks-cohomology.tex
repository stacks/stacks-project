...
Let $f : \mathcal{X} \to \mathcal{Y}$ be a morphism of algebraic stacks.
The functor $Lf^*$ induces a commutative diagram
$$
\xymatrix{
D_{\mathcal{M}_\mathcal{X}}(\mathcal{O}_\mathcal{X}) \ar[r] &
D(\mathcal{O}_\mathcal{X}) \\
...
\ar[r] \ar[u]^{Lf^*} &
D(\mathcal{O}_\mathcal{Y}) \ar[u]^{Lf^*}
}
$$
The composition
$$
...
$$
\Hom_{D_{\textit{QCoh}}(\mathcal{O}_\mathcal{X})}(Lf^*_{\textit{QCoh}}E, F)
=
\Hom_{D_{\textit{QCoh}}(\mathcal{O}_\mathcal{Y})}(E, Rf_{\textit{QCoh}, *}F)
$$
for $E \in D_{\textit{QCoh}}(\mathcal{O}_\mathcal{Y})$ and
$F \in D^{+}_{\textit{QCoh}}(\mathcal{O}_\mathcal{X})$.
\end{proposition}

\begin{proof}
We have to show that $Lf^*E$ is an object of
