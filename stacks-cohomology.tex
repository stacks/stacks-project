...
of $\mathcal{E}$ to $\mathcal{C}/U = U_{lisse,\etale}$
(resp.\ $=U_{flat,fppf}$) is quasi-coherent. Thus we may assume
...
\begin{enumerate}
\item the cokernel $\Coker(\varphi)$ is a coherent
$\mathcal{O}_\mathcal{X}$-module, and
\item the kernel $\Ker(\varphi)$ may not be coherent, but it is
...
\begin{proof}
The rules given for taking kernels and cokernels in
...
$\Coker(\varphi)$ and $Q(\Ker(\varphi))$ are coherent.
...
is exact. Hence $f^*\Coker(\varphi)$ is the cokernel of
a map between coherent $\mathcal{O}_U$-modules hence coherent
...
$\QCoh(\mathcal{O}_\mathcal{X})$. In fact, let
$\mathcal{F}$ be a coherent $\mathcal{O}_\mathcal{X}$-module.
Any quasi-coherent quotient module of $\mathcal{F}$ is coherent.
Given a quasi-coherent module $\mathcal{G}$ and a map
$\mathcal{G} \to \mathcal{F}$ whose kernel is parasitic,
then $\mathcal{G}$ is a coherent $\mathcal{O}_\mathcal{X}$-module.
\end{lemma}
...
is exact and moreover by definition $\textit{Coh}(\mathcal{O}_\mathcal{X})$
...
We omit the proof of the final two statements (hint: compare
