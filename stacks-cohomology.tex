...
for all algebraic stacks $\mathcal{X}$,
\item $f^*$ maps $\mathcal{M}_\mathcal{X}$ into $\mathcal{M}_\mathcal{Y}$
for every morphism of algebraic stacks $f : \mathcal{Y} \to \mathcal{X}$,
\item if $f_i : \mathcal{X}_i \to \mathcal{X}$ is a family of smooth
morphisms of algebraic stacks with
$|\mathcal{X}| = \bigcup |f_i|(|\mathcal{X}_i|)$, then an object
...
$\mathcal{O}_\mathcal{X}$-module. Then $R^if_*\mathcal{F}$
...
quasi-separated morphism of algebraic stacks. Let 
$\mathcal{F}$ be a locally quasi-coherent
$\mathcal{O}_\mathcal{X}$-module with the flat base change property.
...
Lemma \ref{lemma-general-pushforward}
the lemma will follow. Properties (1) and (2) follow from
Sheaves on Stacks, Lemma \ref{stacks-sheaves-lemma-lqc-colimits}
and
...
consisting of parasitic modules is a weak Serre subcategory.
\end{enumerate}
...
is $(\mathcal{F}^+)^+$. Let $\varphi : \mathcal{F} \to \mathcal{G}$ be
a map of parasitic $\tau$-modules. It is immediate from the definitions
that $\text{Ker}(\varphi)$ is parasitic. On the other hand, it is equally
clear that the presheaf
$x \mapsto \text{Coker}(\mathcal{F}(x) \to \mathcal{G}(x))$
is parasitic. Hence $\text{Coker}(\varphi)$ is parasitic by (2)(a).
Finally, we have to show that given a short exact sequence
$0 \to \mathcal{F}_1 \to \mathcal{F}_2 \to \mathcal{F}_3 \to 0$
...
proof of the key lemma) we reduce to the case where the
algebraic stack is representable by a scheme.
