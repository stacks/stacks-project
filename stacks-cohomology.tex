...
of $\mathcal{M}_\mathcal{X}$ both the kernel and the cokerenl of the
...
that the map $Q(\mathcal{F}) \to \mathcal{F}$ is becomes an isomorphism after
...
\noindent
In the book \cite{LMB} many of the results above are proved using the
...
\item the adjunction map $\text{id} \to g^{-1}g_!^{Sh}$ is an isomorphism, and
