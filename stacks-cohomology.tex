...
in the category of abelian groups, see
Modules on Sites, Definition \ref{sites-modules-definition-g-shriek}.
Suppose that $\varphi : V \to U$, $s \in \mathcal{H}(U)$, and
$f \in \mathcal{O}(V)$.
...
\medskip\noindent
To see that $g_!\mathcal{O}' \to \mathcal{O}$ is surjective it hence
suffice to show that $1 \in \Gamma(\mathcal{C}, \mathcal{O})$ is
...
left adjoint to $g^{-1} = g^*$. Finally, since
$g^{-1}g_!\mathcal{K}^\bullet = \mathcal{K}^\bullet$ by
Lemmas \ref{lemma-lisse-etale-modules} and \ref{lemma-lisse-etale}
