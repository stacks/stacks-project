...
square zero kernel, the functor of fibre categories
$$
\mathcal{X}_{\Spec(A')}
\longrightarrow
...
\begin{enumerate}
\item with $A \to B \leftarrow B'$ as in Definition \ref{definition-RS-star}
if $B$ is finite over $A$, then $A'$ is of finite type over $\Lambda$, see
...
Then $\text{Ext}^i_A(E, k) = \text{Hom}_k(H^{-i}(E \otimes^\mathbf{L} k), k)$.
\end{lemma}
...
$$
\text{Hom}_A(E_x, NL_{A/\Lambda}) = \text{Ext}^0_A(E_x, NL_{A/A'}) =
\text{Ext}^1_A(E_x, I)
$$
