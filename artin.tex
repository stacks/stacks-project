...
\noindent
Let $S$ be a locally Noetherian scheme. If $\mathcal{X}$ is an algebraic stack
over $S$, then the predeformation categories associated to $\mathcal{X}$
are all deformation categories as defined in
...
$S$. For any field $k$ of finite type over $S$ and any object $x_0$
of $\mathcal{X}$ lying over $k$ the predeformation category
$p : \mathcal{F}_{\mathcal{X}, k, x_0} \to \mathcal{C}_\Lambda$
(\ref{equation-predeformation-category}) is a deformation category.
In other words, $\mathcal{F}_{\mathcal{X}, k, x_0}$ satisfies the
Rim-Schlessinger condition (RS).
\end{lemma}
...
$$
the top horizontal arrow is an equivalence by Lemma \ref{lemma-pushout}. 
Since $\mathcal{F}(B)$ is the category of objects of $\mathcal{X}_{\Spec(B)}$
with an identification with $x_0$ over $k$ we win.
\end{proof}

\noindent
The lemma above provides some motivation for Artin's criteria later.
