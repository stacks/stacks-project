...
$\overline{E}$ of $E$. Choose an irreducible component
$A \subset \overline{E}$ containing $u_0$. By
Properties, Lemma \ref{properties-lemma-countable-dense-subset}
we can find a countable subset
$\{u_1, u_2, u_3, \ldots\} \subset E \cap A$
dense in $A$. It follows that $u_1, u_2, u_3, \ldots$
is an infinite sequence of (pairwise distinct) finite type points such
that $u_0$ is a limit point of the sequence and such that $x$ is not versal
at $u_n$ for $n \geq 1$.

\medskip\noindent
After passing to a subsequence we
