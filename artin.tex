...
category $\Sch_\alpha/S$ (and such that the inclusion
functors are continuous). Denote
$$
...
are in $\textit{Noetherian}_\alpha/S$ is a site which
...
\noindent
The following lemma has a variant for algebraic spaces.
...
\begin{enumerate}
\item $a'$ is representable (by schemes), and
...
\noindent
Let $S$ be a locally Noetherian scheme. Let
$(\textit{Noetherian}/S)_\etale \subset (\Sch/S)_\tau$
...
then there exists an open neighbourhood $u_0 \in U' \subset U$
such that $x$ is versal at every finite type point of $U'$.
As before, all of these conditions only involve evaluating
...
Categories, Lemma \ref{categories-definition-representable-morphism}),
\item $F$ satisfies axioms [-1], [0], [1], [2], [3], [4], [5]
(see above), and
\item $\mathcal{O}_{S, s}$ is a G-ring for all finite type points $s$ of $S$.
\end{enumerate}
