...
$u_0 : \Spec(k) \to U$ be a finite type point of $U$, see
...
\xi\text{ is versal}
$$
We use this notion to state the approximation result in terms of
versal objects.
...
Then $\mathcal{X}$ is limit preserving on objects, and there exists an
effective, versal formal object $\xi$ over $R$.
...
\item[{[3]}] every tangent space $T\mathcal{F}_{\mathcal{X}, k, x_0}$ as in
(\ref{equation-tangent-space}) has finite dimension,
\item[{[4]}] the functor (\ref{equation-approximation}) is an equivalence,
\item[{[5]}] $\mathcal{X}$ satisfies openness of versality.
\end{enumerate}
...
\ref{criteria-section-limit-preserving} we have defined what
it means to be limit preserving on objects. However, the example
...
an equivalence
$$
\mathcal{X}_T = \colim \mathcal{X}_{T_i}
$$
of fibre categories.
\end{definition}
...
{\it openness of versality} if given
a scheme $U$ locally of finite type over $S$,
an object $x$ of $\mathcal{X}$ over $U$, and a finite type point $u_0 \in U$
such that $x$ is versal at $u_0$, then there exists
an open neighbourhood $u_0 \in U' \subset U$ such that
$x$ is versal at every finite type point of $U'$.
\end{definition}

\noindent
This axiom is often the hardest to check. The following example shows that
requiring this axiom is necessary however.
...
satisfies [0], [1], [2], [3] but not [4]. Namely, over $U = \Spec(k[s, t]/(s))$
...
\noindent
Here is our first step towards the main theorem of this chapter.
