...
\begin{proof}
This is an absurdly long proof, but most of it is absolutely standard
arguments on limits and \'etale localization. We urge the reader to skip ahead
to the very last part of the proof where something interesting happens.
...
\medskip\noindent
Proof of surjectivity: descending $\hat x|_Z$.
Since $W_{red}$ is a scheme locally of finite type over $S$
we see using Limits, Proposition
\ref{limits-proposition-characterize-locally-finite-presentation}
that after replacing $\lambda$ by a bigger index
we may assume there is a morphism
$x_{\lambda, 0} : Z_\lambda \to W_{red}$ whose
pullback to $Z$ is $\hat x|_Z$.
...
Finally, again after increasing $\lambda$, we may assume the diagram
$$
\xymatrix{
V'_\lambda \times_{V_\lambda} Z_\lambda \ar[d] \ar[r]_-{x'_\lambda} &
X'/_{T'} \times_{g, W} W_{red} \ar[d] \\
Z_\lambda \ar[r] &
W_{red}
}
$$
commutes; some details omitted.

\medskip\noindent
Proof of surjectivity: algebra. Write $W = \text{Spf}(B)$.
