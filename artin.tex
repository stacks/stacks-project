...
Also, $\mathcal{R}$ is representably by an algebraic space $R$
...
\medskip\noindent
Let $S$ be a locally Noetherian scheme. Let
$F : (\Sch/S)_{fppf}^{opp} \to \textit{Sch}$ be a functor.
...
\item $\mathcal{X}$ satisfies axioms [-1], [0], [1], [2], [3], [4], and
openness of versality,
\item $\mathcal{O}_{S, s}$ is a G-ring for all finite type points $s$ of $S$.
\end{enumerate}
...
is representably by algebraic spaces by condition (1) and
...
Stacks, Lemmas \ref{stacks-lemma-2-fibre-product-gives-stack-in-setoids} and
\ref{stacks-lemma-stack-in-setoids-characterize}, i.e.,
...
Lemma \ref{lemma-fibre-product-RS}. Axiom [3] for $Y$ follows
from Lemmas \ref{lemma-finite-dimension} and
\ref{lemma-fibre-product-tangent-spaces}.
Axiom [4] follows from Lemmas \ref{lemma-fibre-product-effective} and
\ref{lemma-effective}. Axiom [5] for $Y$ follows directly from
axiom [5] for $\mathcal{X}$. Thus all the assumptions of
Proposition \ref{proposition-spaces-diagonal-representable}
are satisfied and $Y$ is an algebraic space.

\medskip\noindent
At this it follows from Lemma \ref{lemma-diagonal-representable}
...
groupouids in an obvious manner.
\end{lemma}
...
$$
of finite type $\Lambda$-algebras, where $B' \to B$ is surjective with
square zero kernel and $A \to B$ finite, the functor of fibre categories
$$
\mathcal{X}_{\Spec(A')}
\longrightarrow
...
\noindent
We make some observations: (1) if $B'$, $B$,
and $A$ are of finite type over $\Lambda$, so is $A'$, see
Algebra, Lemma \ref{algebra-lemma-fibre-product-finite-type},
(2) $\Spec(A') = \Spec(A) \amalg_{\Spec(B)} \Spec(B')$, see
More on Morphisms, Lemma \ref{more-morphisms-lemma-pushout-along-thickening},
(3) if $\mathcal{X}$ is an algebraic stack, then $\mathcal{X}$ satisfies
(RS*) by Lemma \ref{lemma-pushout}, and (4) if $\mathcal{X}$ satisfies
...
pairs $(x, A' \to A)$ where
\begin{enumerate}
\item $A' \to A$ is a surjection of finite type $S$-algebras whose kernel
...
$$
where $I = \text{Ker}(A' \to A)$. It is simply transitive if
$\text{Lift}(x, A') \not = \emptyset$.
\end{enumerate}
\end{enumerate}
\end{lemma}
...
$$
\text{Mod}_{fg}(A) \longrightarrow \textit{Sets},\quad
...
Suppose $A \to B$ is a finite type ring map and $y = x|_{\Spec(B)}$.
Let $M \in \text{Mod}_{fg}(A)$, $N \in \text{Mod}_{fg}(B)$
...
$$
commute. Here $I = \text{Ker}(A' \to A)$, $J = \text{Ker}(B' \to B)$,
...
$$
Let $k = \kappa(\mathfrak p)$ be a residue field at a finite type point $u_0$
...
$\mathcal{X}$ over $\Spec(A)$. Further, let $A \to B$ be a finite
type ring map and let $\alpha : x|_{\Spec(B)} \to y|_{\Spec(B)}$
be a morphism of $\mathcal{X}$ over $\Spec(B)$. Consider the ring map
$$
...
Let $S$ be a locally Noetherian scheme. Let $\mathcal{X}$ be a category
fibred in groupoids over $(\Sch/S)_{fppf}$. Let $A$ be a finite type
$S$-algebra such that $\Spec(A) \to S$ maps into an affine open. There
is a notion of a {\it short exact sequence}
$$
(x, A_3' \to A) \to (x, A_2' \to A) \to (x, A_1' \to A)
$$
of deformation situations: we ask the corresponding maps between
the kernels $I_i = \text{Ker}(A_i' \to A)$ give a short exact sequence
$$
0 \to I_1 \to I_2 \to I_3 \to 0
$$
of finite $A$-modules. Note that in this case the map $A_1' \to A_3'$
...
Contrary to the spaces of infinitesimal deformations and and infinitesimal
...
\begin{enumerate}
\item for every finite type $S$-algebra $A$ such that $\Spec(A) \to S$
maps into an affine open and every object $x$ of $\mathcal{X}$ over
$\Spec(A)$ an $A$-linear functor
$$
\mathcal{O}_x : \text{Mod}_{fg}(A) \to \text{Mod}(A)
$$
of {\it obstruction modules},
\item for $(x, A)$ as in (1), a finite type ring map $A \to B$,
$M \in \text{Mod}_{fg}(A)$, $N \in \text{Mod}_{fg}(B)$, and an $A$-linear
...
$o_x(A') \in \mathcal{O}_x(I)$ where $I = \text{Ker}(A' \to A)$.
\end{enumerate}
...
is called the obstruction module. The element $o_x(A')$ is the obstruction.
...
\begin{enumerate}
\item given $A$ of finite type over $\Lambda$ and
$x \in H^0(W_A, \mathcal{F}_A)$ we set
$\mathcal{O}_x(M) = H^1(W_A, \mathcal{F}_A \otimes_A M)$, and
...
of $\mathcal{F}$ over $S$ imples that the sequence above is short exact.
\end{example}
...
$f(a_1', \ldots, a'_n) \in I$. We omit the verfication that
...
Then $\text{Ext}^i_A(E, k) = \text{Hom}_k(H^{-1}(E \otimes^\mathbf{L} k), k)$.
\end{lemma}
...
such that
\begin{enumerate}
\item[(a)] $I = \text{Ker}(A' \to A)$ has square zero and is isomorphic to $k$
as an $A$-module,
...
$\text{Ker}(A' \to A) = \text{Ker}(A'' \to A) = I$. Pick
$f'' \in A''$ lifting $f$. Then the localization of $A'$ at
$(f'', f)$ is isomorphic to $A''$
(for example by Limits, Lemma \ref{limits-lemma-diagram-localize}).
...
$f_{f + 1} \otimes 1, \ldots, f_{m - 1} \otimes 1$.
Set
$$
...
$k = \kappa(u_0)$. Then the following are equivalent
\begin{enumerate}
...
\begin{proof}
Let $\mathfrak p = \text{Ker}(A \to k)$ be the prime corresponding to $u_0$.

\medskip\noindent
Assume that $x$ versal at $u_0$ and that
$H^{-1}(\xi \otimes_A^{\mathbf{L}} k)$ is not surjective.
...
an object $x'$ over $A'$.
Let $\mathfrak p' \subset A'$ be kernel of the map to $k$.
By Artin-Rees (Algebra, Lemma \ref{algebra-lemma-Artin-Rees})
there exists an $n > 1$ such that $(\mathfrak p')^n \cap I = 0$.
Then we see that
$$
B' = A'_{\mathfrak p'} / (\mathfrak p')^n A'_{\mathfrak p'} \to
A_\mathfrak p/\mathfrak p^nA_\mathfrak p = B
$$
is a small, essential extension, see
Formal Deformation Theory, Lemma
...
$m \geq n$ and a map $q : A_\mathfrak p/\mathfrak p^mA_\mathfrak p \to B'$
such that the composition
$A_\mathfrak p/\mathfrak p^mA_\mathfrak p \to B' \to B$ is the quotient map.
...
\medskip\noindent
Assume $H^{-1}(E \otimes_A^\mathbf{L} k) \to
H^{-1}(NL_{A/\Lambda} \otimes_A^\mathbf{L} k)$
is surjective and
$H^0(E \otimes_A^\mathbf{L} k) \to
H^0(NL_{A/\Lambda} \otimes_A^\mathbf{L} k)$
...
Let $I = \text{Ker}(C' \to C)$. Note that $I$ is a $1$-dimensional $k$-vector
...
\end{enumerate}
In this case, if $x$ is versal at a finite type point $u_0 \in U$
then there exists an open neighbourhood $u_0 \in U' \subset U$
such that $x$ is versal at every finite type point of $U'$.
\end{lemma}
...
we see that $x$ is versal at $u_0$ exactly when
...
\item
\label{item-map}
for every finite type $S$-algebra $A$ such that $\Spec(A) \to S$
...
for $(x, A)$ as in (\ref{item-map}) and a finite type ring map $A \to B$
setting $y = x|_{\Spec(A)}$ there is a functoriality map
$E_x \to E_y$ in $D(A)$.
\end{enumerate}
These data are subject to the following conditions
\begin{enumerate}
...
of finite type setting $y = x|_{\Spec(B)}$ and $z = x|_{\Spec(C)}$ the
...
Remark \ref{remark-canonical-element},
\item[(vi)]
given a deformation situation $(x, A' \to A)$ with $I = \text{Ker}(A' \to A)$
the composition $E_x \to NL_{A/\Lambda} \to NL_{A/A'}$ is zero in
$$
\text{Hom}_A(E_x, NL_{A/\Lambda}) =
\text{Ext}^0_A(E_x, NL_{A/\Lambda}) = \text{Ext}^1_A(E_x, I)
$$
...
Let $k$ be a $B$-algebra which is a field and which is a finite
$A$-module. Then the functoriality map $E_x \to E_y$ induces
bijections
$$
...
\begin{proof}
The assumption implies that $k$ is finite as a $B$-module too (see
...
have finitely generated cohomology groups.
\end{lemma}
...
$\mathcal{X}$ has (RS*) so that we can speak of the functors
$\text{Inf}_x(-)$ and $T_x(-)$, see Lemma \ref{lemma-properties-lift-RS-star}.
Let $U = \Spec(A)$ be an affine scheme of finite type over $S$ which maps
into an affine open $\Spec(\Lambda)$. Let $x$ be an object of $\mathcal{X}$
...
where $I' = \text{Ker}(A'' \to A)$, and
\item[(iii)] $x$ lifts to an object over $\Spec(A')$ if and only if
$o_x(A') = 0$.
\end{enumerate}
It is possible to incorporate infinitesimal deformations into this
picture as well, but we refrain from doing so in order to get the sharpest
possible result.
\end{situation}

\noindent
Let $\Lambda \to A$ be a finite type ring map of Noetherian rings.
The naive cotangent complex $NL_{A/\Lambda}$ is a complex of $A$-modules.
An important role will be played by
$K^\bullet \otimes_A^\mathbf{L} NL_{A/\Lambda}$. Note that if we choose a
presentation $\alpha : \Lambda[x_1, \ldots, x_n] \to A$
with kernel $J$ then the complex $NL(\alpha)$ is canonically homotopy
equivalent to $NL_{A/\Lambda}$ hence
$K^\bullet \otimes_A^\mathbf{L} NL_{A/\Lambda}$
and $K^\bullet \otimes_A^\mathbf{L} NL(\alpha)$ are homotopy equivalent
too, see More on Algebra, Lemma \ref{more-algebra-lemma-derived-tor-homotopy}.
Next, suppose we are given a complex $K^\bullet$ of $A$-modules and an
element $\xi \in H^1(K^\bullet \otimes_A^\mathbf{L} NL_{A/\Lambda})$. Then
for any surjection $A' \to A$ of $\Lambda$-algebras with kernel $I$
of square zero we obtain $NL_{A/\Lambda} \to NL_{A/A'} = I[1]$
whence $\xi$ maps to an element
$\xi_{A'} \in H^2(K^\bullet \otimes_A^\mathbf{L} I)$.
Moreover, the map $NL_{A/\Lambda} \to \Omega_{A/\Lambda}$ maps
...
$o_x(A_1') \in H^2(K^\bullet \otimes_A^\mathbf{L} I_1)$
equals $\partial\theta$ where
$\theta \in H^1(K^\bullet \otimes_A^\mathbf{L} I_3)$
is the element corresponding to $x'_2|_{\Spec(A_3')}$ via
$A_3' = A[I_3]$ and the given map
$T_x(-) \to H^1(K^\bullet \otimes_A^\mathbf{L} -)$.
\end{enumerate}
...
$$
which is permissible by the remarks above. Consider the element
...
Observe that
$\text{Ker}(C' \to A) = \text{Im}(J/J^2 \to \bigoplus A\text{d}x_i)$.
...
By conditon (iv) we conclude that $o_x(P/J^2)$ maps to $\partial x_{can}$
in
$$
H^1(K^\bullet \otimes_A^\mathbf{L} \text{Im}(J/J^2 \to \bigoplus A\text{d}x_i))
$$
...
perfect object of $D(A)$. In this case, if $x$ is versal at a finite
type point $u_0 \in U$ then there exists an open neighbourhood
