...
in the category of schemes over $S$ where
\begin{enumerate}
\item $X$, $X'$, $Y$, $Y'$ are affine,
\item $X$, $X'$, $Y$, $Y'$ are of finite type over $S$, and
\item $X \to X'$ (and hence $Y \to Y'$) is a closed immersion
defined by an ideal of square zero\footnote{We may in the future
weaken the condition (RS*) by only allowing diagrams where $X \to Y$
...
\noindent
Observe that if $\mathcal{X}$ is an algebraic stack, then $\mathcal{X}$
satisfies (RS*) by Lemma \ref{lemma-pushout}.
...
\noindent
In this section we try to formulate one possible notion of obstruction theory.
...
in groupoids over $(\Sch/S)_{fppf}$. One important ingredient in describing
an obstruction theory is to describe the category of situations to which it
applies. We choose the category of pairs $(x, A' \to A)$ where
\begin{enumerate}
\item $A' \to A$ is a surjection of finite type $S$-algebras whose kernel
is an ideal of square zero, and
...
Let $S$ be a locally Noetherian base. Let $\mathcal{X}$ be a category fibred
in groupoids over $(\Sch/S)_{fppf}$. An {\it obstruction theory} is a rule
which assigns
\begin{enumerate}
\item to every deformation situation $(x, A' \to A)$ a pair
$(\mathcal{O}_x(A'), o_x(A'))$ where $\mathcal{O}_x(A')$ is an $A$-module
and $o_x(A') \in \mathcal{O}_x(A')$,
\item for every morphism of deformation situations
$(y, B' \to B) \to (x, A' \to A)$ an $A$-linear map
$\mathcal{O}_x(A') \to \mathcal{O}_y(B')$ sending $o_x(A')$ to $o_y(B')$
\end{enumerate}
such that $\mathcal{O}_x$ is a functor on the category of
deformation situations and moreover
$$
\text{Lift}(x, A') \not = \emptyset
\Leftrightarrow
o_x(A') = 0
...
\noindent
This last condition explains the terminology. Then module $\mathcal{O}_x(A')$
...
make them useful additional conditions are needed. For example, with the
definition above, any $\mathcal{X}$ has an obstruction theory, simply
by setting $\mathcal{O}_x(A')$ equal to the free rank 1 module on
$o_x(A')$ if $x$ doesn't lift to $A'$ and equal to zero else.
...
& =
\text{Ext}^1_{D(R)}(NL_{R/\Lambda}, I) \\
& =
\text{Ext}^1_{D(A)}(NL_{R/\Lambda} \otimes_R A, I).
\end{align*}
