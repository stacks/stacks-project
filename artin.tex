...
Thus we obtain an obstruction theory.
\end{example}
...
naive cotangent complex (rather than the full one) in this section.

\medskip\noindent
In the following definition we use the naive cotangent complex as
...
such that
(a) $I = \text{Ker}(A' \to A)$ has square zero and is isomorphic to $k$
as an $A$-module,
(b) $\Omega_{A'/\Lambda} \otimes k = \Omega_{A/\Lambda} \otimes k$, and
(c) $E \to NL_{A'/A}$ is zero.
\end{lemma}
...
\begin{lemma}
\label{lemma-naive-obstruction-theory-characterize-versal}
Let $S$ and $\mathcal{X}$ be as in
Definition \ref{definition-naive-obstruction-theory}
and let $\mathcal{X}$ be endowed with a naive obstruction theory.
Let $U = \Spec(A)$ be an affine scheme of finite type over $S$ which
maps into an affine open $\Spec(\Lambda)$. Let $u_0 \in U$ be a finite
type point with residue field $k = \kappa(u_0)$. Then the following are
equivalent
\begin{enumerate}
\item $x$ is versal at $u_0$, and
\item $\xi_x : E_x \to NL_{A/\Lambda}$ induces a surjection
$H^{-1}(E_x \otimes_A^{\mathbf{L}} k) \to
H^{-1}(NL_{A/\Lambda} \otimes_A^{\mathbf{L}} k)$
and an injection
$H^0(E_x \otimes_A^{\mathbf{L}} k) \to
...
$H^{-1}(\xi_x \otimes_A^{\mathbf{L}} k)$
is surjective.

\medskip\noindent
Assume that $x$ versal at $u_0$. By Lemma \ref{lemma-compute-ext-into-field}
the map $H^0(\xi_x \otimes_A^{\mathbf{L}} k)$ is dual to the map
$\text{Ext}^0_A(NL_{A/\Lambda}, k) \to \text{Ext}^0_A(E_x, k)$. Note that
$$
\text{Ext}^0_A(NL_{A/\Lambda}, k) = \text{Der}_\Lambda(A, k)
\quad\text{and}\quad
T_x(k) = \text{Ext}^0_A(E_x, k)
$$
Condition (iv) of a naive obstruction theory assures us the map
$\text{Ext}^0_A(NL_{A/\Lambda}, k) \to \text{Ext}^0_A(E_x, k)$
...
Hence $H^0(\xi_x \otimes_A^{\mathbf{L}} k)$ is injective.

\medskip\noindent
Assume $H^{-1}(E_x \otimes_A^{\mathbf{L}} k) \to
H^{-1}(NL_{A/\Lambda} \otimes_A^{\mathbf{L}} k)$
is surjective and
$H^0(E_x \otimes_A^{\mathbf{L}} k) \to
H^0(NL_{A/\Lambda} \otimes_A^{\mathbf{L}} k)$
injective. Set $R = A_\mathfrak p^\wedge$ and let $\xi$ be the
formal object over $R$ associated to $x|_{\Spec(R)}$.
The map $d\underline{\xi}$ on tangent spaces is surjective
...
$\xi_x$ induces an injection
$$
\text{Ext}^1_A(NL_{A/\Lambda}, I)
\longrightarrow
\text{Ext}^1_A(E_x, I)
$$
The image of $ob$ in this last group is the obstruction to lifting
$x$ to $A \times_C C'$. By (RS*) the fact that $y/C$ lifts to $y'/C'$
implies that $x$ lifts to $A \times_C C'$. Hence $ob = 0$ and we are done.
\end{proof}
...
open $\Spec(\Lambda)$. Say $U = \Spec(A)$ and $k = \kappa(u_0)$. Let
$\xi_x : E_x \to NL_{A/\Lambda}$ be the obstruction map.
Let $C$ be the cone of $\xi_x$ so that we have a distinguished triangle
$$
E_x \to NL_{A/\Lambda} \to C \to E_x[1]
$$
in $D(A)$. By Lemma \ref{lemma-naive-obstruction-theory-characterize-versal}
...
Lemma \ref{lemma-naive-obstruction-theory-characterize-versal}
