...
\left( \lim_n H_{et}^i(X, \mathbf{Z}/\ell^n\mathbf{Z}) \right)
...
for the big and small sites $(\textit{Sch}/S)_{fppf}$, $S_{\acute{e}tale}$, etc
can be found in
...
$H^p_{\acute{e}tale}(S, \mathcal{F})$\footnote{We will sometimes abbreviate this
...
of $S$ is the category $\textit{Sh}(S_{\acute{e}tale})$ of sheaves of sets on
...
whose objects are those $U/S \in \text{Ob}(S_{\acute{e}tale})$ with
...
$\mathcal{O}_S$
on any of the sites $S_{Zar}$, $S_{\acute{e}tale}$, or $(\textit{Sch}/S)_\tau$
discussed above.
\end{definition}
...
$(\textit{Sch}/S)_{\acute{e}tale}$, see
...
$\textit{Sh}(S_{\acute{e}tale}) \to \textit{Sets}$ given by the stalk functor
...
scheme $T/S \in \text{Ob}((\textit{Sch}/S)_{\acute{e}tale})$.
...
$\overline{s}$ in
Lemma \ref{lemma-stalk-gives-point}.
\item Let $p : \textit{Sh}(pt) \to \textit{Sh}(S_{\acute{e}tale})$ be a point
...
$$
for $U \in \text{Ob}(S_{\acute{e}tale})$ which will conclude the proof. See
Lemma \ref{lemma-stalk-gives-point}
...
$(\textit{Sch}/S)_{Zar}$, $(\textit{Aff}/S)_{Zar}$,
$(\textit{Sch}/S)_{\acute{e}tale}$, $(\textit{Aff}/S)_{\acute{e}tale}$,
$(\textit{Sch}/S)_{smooth}$, $(\textit{Aff}/S)_{smooth}$,
$(\textit{Sch}/S)_{syntomic}$, $(\textit{Aff}/S)_{syntomic}$,
...
Suppose $\overline{s} : \text{Spec}(k) \to S$ is a geometric
point with $k$ algebraically closed. Consider the functor
$$
u : (\textit{Sch}/S)_{fppf} \longrightarrow \textit{Sets},
\quad
u(U) = U(k) = \text{Mor}_S(\text{Spec}(k), U).
$$
...
By
Sites, Remark \ref{sites-remark-improve-proposition-points-limits}
we see that $u$ defines a point $p$ of $(\textit{Sch}/S)_{fppf}$ with
stalks
$$
...
$(\text{Spec}(k), \text{id})$, hence we see that
$$
\mathcal{F}_p = \mathcal{F}(\text{Spec}(k))
$$
...
Let $S = \text{Spec}(A)$ be an affine scheme. Let $(p, u)$ be a point of
...
see that $\text{Spec}(B)$ is an object of $(\textit{Aff}/S)_{fppf}$. Recall
that
$$
B = \colim_{(U, x)} \mathcal{O}(U) =
\colim_{(\text{Spec}(C), x)} C
$$
where $x \in u(\text{Spec}(C))$. Hence there are canonical maps $C \to B$
and we see that the system has an initial object $(\text{Spec}(B), x_B)$
for a suitable $x_B \in u(\text{Spec}(B))$. It follows that
$\mathcal{F}_p = \mathcal{F}(\text{Spec}(B))$ for any sheaf $\mathcal{F}$
...
empty for all $\varphi : U \to S$ in $\text{Ob}(S_{\acute{e}tale})$.
...
Let $S$ be a scheme.
Let $\mathcal{F}$ be an abelian sheaf on $S_{\acute{e}tale}$.
Let $U \in \text{Ob}(S_{\acute{e}tale})$ and $\sigma \in \mathcal{F}(U)$.
\begin{enumerate}
...
of $R$ are reflected in its (strict) henselization.
...
Let $R$ be a local ring. The following are equivalent: $R$ is Noetherian,
the henselization $R^h$ of $R$ is Noetherian, and
the strict henselization $R^{sh}$ of $R$ is Noetherian.
In this case the map of completions $R^\wedge \to (R^h)^\wedge$ is an
isomorphism.
\end{lemma}

\begin{proof}
Since $R \to R^h \to R^{sh}$ are flat local ring maps, we see that
$R^h$ or $R^{sh}$ being Noetherian implies that $R$ is Noetherian, see
Algebra, Lemma \ref{algebra-lemma-descent-Noetherian}.
In the rest of the proof we assume $R$ is Noetherian.

\medskip\noindent
Denote $\mathfrak m \subset R$ the maximal ideal. We have seen that
$\mathfrak mR^h$ is the maximal ideal of $R$,
$R/\mathfrak m = R^h/\mathfrak mR^h$, and $R \to R^h$ is flat, see
Theorem \ref{theorem-henselization}.
Hence we see that $R^h/\mathfrak m^nR^h \cong R/\mathfrak m^n$
because $R^h/\mathfrak m^nR^h$ is flat hence free (see
Algebra, Lemma \ref{algebra-lemma-local-artinian-characterize-flat})
of rank $1$ over $R/\mathfrak m^n$. This implies that
$R^\wedge \to (R^h)^\wedge$ is an isomorphism.

\medskip\noindent
Next we prove that $(R^h)^\wedge$ is flat over $R^h$. Write
$R^h = \colim_i R_i$ as a directed colimit of localizations
of \'etale $R$-algebras. By
Algebra, Lemma \ref{algebra-lemma-colimit-rings-flat}
it suffices to show that $(R^h)^\wedge$ is flat over each $R_i$.
This holds because $R_i^\wedge = (R^h)^\wedge$ (by first paragraph proof)
is flat over $R_i$ (see
Algebra, Lemma \ref{algebra-lemma-completion-faithfully-flat}).
It follows from the flatness of
$R^h \to (R^h)^\wedge$ and the Noetherianness of
$R^\wedge = (R^h)^\wedge$ (see
Algebra, Lemma \ref{algebra-lemma-completion-Noetherian-Noetherian})
that $R^h$ is Noetherian, see
Algebra, Lemma \ref{algebra-lemma-descent-Noetherian}.

\medskip\noindent
By
Algebra, Lemma \ref{algebra-lemma-completion-Noetherian}
the completion $(R^{sh})^\wedge$ is a Noetherian local ring (as we are
completing with respect to the finitely generated maximal
ideal $\mathfrak mR^{sh}$).
Hence if we can show that $(R^{sh})^\wedge$ is flat over $R^{sh}$
then $R^{sh}$ is Noetherian by
Algebra, Lemma \ref{algebra-lemma-descent-Noetherian}
again. Using the limit argument of the second paragraph above
we see that it suffices to show that $(R^{sh})^\wedge$ is flat over $R$.

\medskip\noindent
Pick a separable algebraic extension
$\kappa \subset \kappa^{sep}$ which is separably algebraically closed.
Pick a basis $\{\overline{x}_j\}_{j \in J}$ of $\kappa^{sep}$ over $\kappa$.
Let $R^{sh}$ be the strict henselization of $R$ with respect to
$\kappa \subset \kappa^{sep}$. Choose $x_j \in R^{sh}$ mapping to
$\overline{x}_j$ in in the residue field
$\kappa^{sh} = R^{sh}/\mathfrak mR^{sh}$ of the strict henselization.
Arguing as in the first paragraph of the proof, 
$R^{sh}/\mathfrak m^nR^{sh}$ is free on the elements
$x_j$ as a module over $R/\mathfrak m^n$, see
Algebra, Lemma \ref{algebra-lemma-local-artinian-basis-when-flat}.
It follows that $(R^{sh})^\wedge$ is isomorphic to the $\mathfrak m$-adic
completion of $\bigoplus_{j \in J} R$ as an $R$-module. By
More on Algebra, Lemma \ref{more-algebra-lemma-completed-direct-sum-flat}
we see this is flat over $R$ and we win.
\end{proof}
...
\begin{proof}
To see this note that $R \to R^{sh}$
...
$U/X \in \text{Ob}(X_{\acute{e}tale})$
the sequence of abelian groups
$0 \to \mathcal{F}_1(U) \to \mathcal{F}_2(U) \to \mathcal{F}_3(U)$
is exact. Hence for every $V/Y \in \text{Ob}(Y_{\acute{e}tale})$
the sequence of abelian groups
...
$$
\text{Hom}_{{\textit{Sh}(X_{\acute{e}tale})}} (f^{-1}\mathcal{G}, \mathcal{F})
=
\text{Hom}_{\textit{Sh}(Y_{\acute{e}tale})} (\mathcal{G}, f_*\mathcal{F})
$$
functorially, for any $\mathcal{F} \in \textit{Sh}(X_{\acute{e}tale})$ and
$\mathcal{G} \in \textit{Sh}(Y_{\acute{e}tale})$. We similarly have
$$
\text{Hom}_{{\textit{Ab}(X_{\acute{e}tale})}} (f^{-1}\mathcal{G}, \mathcal{F})
=
\text{Hom}_{\textit{Ab}(Y_{\acute{e}tale})} (\mathcal{G}, f_*\mathcal{F})
$$
...
is exact.
\item The functor
$f^{-1} : \textit{Sh}(Y_{\acute{e}tale}) \to \textit{Sh}(X_{\acute{e}tale})$
...
$$
\text{Mor}_{\textit{Sh}(X_{\acute{e}tale})}(f^{-1}h_V, \mathcal{F}) =
\text{Mor}_{\textit{Sh}(Y_{\acute{e}tale})}(h_V, f_*\mathcal{F}) =
...
$\mathcal{O}_Y$. Say $Y = \text{Spec}(B)$ and $X = \text{Spec}(A)$. Since
...
$V \in \text{Ob}(X_{\acute{e}tale})$ affine we have
\begin{align*}
(g_*\mathcal{F})(V)
& =
\text{Mor}_{\textit{Sh}(Y_{\acute{e}tale})}(h_V, g_*\mathcal{F}) \\
& =
\text{Mor}_{\textit{Sh}(X_{\acute{e}tale})}(g^{-1}h_V, \mathcal{F}) \\
& =
\text{Mor}_{\textit{Sh}(X_{\acute{e}tale})}(h_{X \times_Y V}, \mathcal{F}) \\
& =
...
(\textit{Sh}(U_{i, \acute{e}tale}), \mathcal{O}_{U_i})
\to
(\textit{Sh}(X_{\acute{e}tale}), \mathcal{O}_X)$
by
...
$$
\xymatrix{
(\textit{Sh}(W_{\acute{e}tale}), \mathcal{O}_W) \ar[r] \ar[d]_{g'} &
(\textit{Sh}(X_{\acute{e}tale}), \mathcal{O}_X) \ar[d]^g \\
(\textit{Sh}(V_{\acute{e}tale}), \mathcal{O}_V) \ar[r] &
(\textit{Sh}(Y_{\acute{e}tale}), \mathcal{O}_Y)
}
$$
...
it suffices to show that for any $V \in \text{Ob}(Y_{\acute{e}tale})$
...
which commutes with fibre products and equalizers.
\end{proof}
...
$$
\colim_{i\in I} H_{et}^p(X, \mathcal{F}_i) = H_{et}^p(X,
...
\begin{enumerate}
\item
for any $\mathcal{V}\in\text{Ob}(Y_{\acute{e}tale})$ and any \'etale covering
...
for some $i\in I$ and some $u_i \in U_i$ by
Lemma \ref{lemma-geometric-lift-to-cover}.
Pick an affine open neighborhood $\text{Spec}(A)$ of $u_i$ in $\mathcal{U}_i$.
...
$$
is a short exact sequence in $\textit{Ab}(S_{\acute{e}tale})$, then
the sequence
$$
0 \to \Gamma(S_{\acute{e}tale}, \mathcal{F}_1) \to
\Gamma(S_{\acute{e}tale}, \mathcal{F}_2) \to
\Gamma(S_{\acute{e}tale}, \mathcal{F}_3)\to 0
$$
is also exact. Indeed, exactness is clear except possibly at the last step. But
given a section $s \in \Gamma(S_{\acute{e}tale}, \mathcal{F}_3)$, we know that
there exist
...
$\mathcal{F}\in \textit{Ab}(X_{\acute{e}tale})$, $R^q f_*\mathcal{F}=0$.
\end{proposition}
...
henselian. This implies that
$\text{Spec}(A) = \coprod_{i=1}^r \text{Spec}(A_i)$, and we can apply
...
$\text{Spec}(K)_{\acute{e}tale}$ of $K$ correspond to surjective families of
...
$$
\textit{Sh}(S_{\acute{e}tale}) \longrightarrow
...
$$
\textit{Sh}(S_{\acute{e}tale}) \longrightarrow G\textit{-Sets},
\quad
\mathcal{F} \longmapsto \mathcal{F}_{\overline{s}}.
$$
\end{theorem}
...
$\mathcal{F}$ on $\text{Spec}(K)_{\acute{e}tale}$ defined by
...
is to say that every sheaf on $\text{Spec}(K)_{\acute{e}tale}$ is representable
...
The reason is that in our construction of $\text{Spec}(K)_{\acute{e}tale}$
we chose a sufficiently large set of schemes \'etale over $\text{Spec}(K)$,
whereas sheaves on $\text{Spec}(K)_{\acute{e}tale}$ form a proper class.
\end{remark}
...
\begin{eqnarray*}
\Gamma(S, \mathcal{F}) & = &
\text{Mor}_{\textit{Sh}(S_{\acute{e}tale})}(h_{\text{Spec}(K)},  \mathcal{F})
\\
...
is trivial. Hence by
Theorem \ref{theorem-equivalence-sheaves-point}
the category of sheaves on $\text{Spec}(k)_{\acute{e}tale}$ is equivalent
...
$$
\textit{Sh}(S_{\acute{e}tale}) \longrightarrow \textit{Sets},\quad
\mathcal{F} \longmapsto \mathcal{F}_{\overline{s}}
$$
is isomorphic to the functor
$$
\textit{Sh}(S_{\acute{e}tale})
\longrightarrow
...
Let $\mathcal{F}$ be an abelian sheaf on $\text{Spec}(K)_{\acute{e}tale}$
...
Sheaves on $\text{Spec}(K)_{\acute{e}tale}$.
...
\item the sheaf $\mathbf{G}_m|_{\text{Spec}(K)_{\acute{e}tale}}$ corresponds to
...
\begin{align*}
H_{et}^0(S_{\acute{e}tale}, \mathbf{G}_m) & = \Gamma(S, \mathcal{O}_S^*) \\
H_{et}^1(S_{\acute{e}tale}, \mathbf{G}_m) & =
H_{Zar}^1(S, \mathcal{O}_S^*) = \text{Pic}(S) \\
H_{et}^i(S_{\acute{e}tale}, \mathbf{G}_a) & = H_{Zar}^i(S, \mathcal{O}_S)
\end{align*}
...
H_{et}^1(\text{Spec}(K), \text{GL}_d(\mathcal{O}))
$$
...
$H_{et}^q(\text{Spec}(K), \mathbf{G}_m) = 0$.
\end{lemma}
...
Let $X$ be a scheme and $\mathcal{F}$ an abelian sheaf on $X_{\acute{e}tale}$.
We say that
...
$X_{\acute{e}tale}$ is {\it constructible} if there exists a finite
decomposition of $X$
...
Then
$$
\text{Hom}_{\textit{Ab}(\mathcal{U}_{\acute{e}tale})}
\left(\underline A_\mathcal{U}, \underline B_\mathcal{U}\right) =
\text{Hom}_{\textit{Ab}}(A, B),
$$
...
any constructible sheaf on $X_{\acute{e}tale}$ is a torsion sheaf.
\end{remark}
...
\item it is a full exact subcategory of $\textit{Ab}(X_{\acute{e}tale})$ ;
...
$\textit{Ab}(X_{\acute{e}tale})$ and $\mathcal{F}$ is constructible then
$\Im(\varphi)$ is
...
$j_! :
\textit{Ab}(\mathcal{U}_{\acute{e}tale})
\to
\textit{Ab}(X_{\acute{e}tale})$
...
$$
for all $\mathcal{F} \in \textit{Ab}(\mathcal{U}_{\acute{e}tale})$ and
$\mathcal{G} \in \textit{Ab}(X_{\acute{e}tale})$.
\end{definition}

\noindent
To describe it more explicitly, recall that $j^{-1}$ is just the restriction
functor $\mathcal{U}_{\acute{e}tale}\to X_{\acute{e}tale}$, that is,
$$
j^{-1}\mathcal{G}(\mathcal{U}'\to \mathcal{U}) = \mathcal{G}
\left(\mathcal{U}'\to \mathcal{U} \xrightarrow{j} X\right).
$$
For $\mathcal{F} \in \textit{Ab}(\mathcal{U}_{\acute{e}tale})$
we consider the presheaf
$$
\begin{matrix}
j_!^{\textit{PSh}}\mathcal{F}: &
X_{\acute{e}tale} &
\longrightarrow &
\textit{Ab} \\
& (\mathcal{V}\to X) &
\longmapsto &
\displaystyle \bigoplus_{\mathcal{V}\xrightarrow{\varphi}
\mathcal{U}\text{ over }X}
...
j^{-1}\mathcal{G})$ for any $\mathcal{G} \in \textit{Ab}(X_{\acute{e}tale})$
...
$\mathcal{G}\mapsto \mathcal{G}(\mathcal{U})$ on
$\textit{Ab}(X_{\acute{e}tale})$, and the
...
$\mathcal{F}$ a sheaf on $\mathcal{V}_{\acute{e}tale}$.
...
the last step is replaced by considering a Galois closure of $Y$.
\end{remark}

\begin{lemma}
\label{lemma-torsion-colimit-constructible}
Let $X$ be a noetherian scheme and $\mathcal{F}$ a torsion sheaf on
$X_{\acute{e}tale}$.
Then $\mathcal{F}$ is a directed (filtered) colimit of constructible sheaves.
\end{lemma}

\begin{proof}[Sketch of proof]
Let $j: \mathcal{U} \to X$ in $X_{\acute{e}tale}$ and
$s\in \mathcal{F}(\mathcal{U})$ for
...
$H_{et}^q(X, \mathcal{F}) = 0$ for all $q\geq 2$.
\end{theorem}
...
we have $\colim
H_{et}^q(X, \mathcal{F}) = H_{et}^q(X, \colim \mathcal{F}_i)$ for some
...
$q \geq 2$, $H_{et}^q(X, j_! \mathcal{F}) = 0$.
\end{lemma}
...
$\mathcal{F}$ on $X_{\acute{e}tale}$. This can be shown using
...
field $k$ with $q = p^f$ elements. Let $\alpha : X \to \text{Spec}(k)$ denote
...
$X_{\bar k} = \text{Spec}(\bar k) \times_{\text{Spec}(k)} X$
to get the morphism $\text{id}_{\text{Spec}(\bar k)} \times \pi_X :
...
$$
\xymatrix{
X_{\bar k} \ar[rd] \ar^{\text{Spec}(\sigma) \times \text{id}_X}[rr] &&
X_{\bar k} \ar[ld] \\
& X
}
$$
...
$(R^j\alpha_!\mathcal{F})_{\text{Spec}(\bar k)}$ with
$H^j_c (X_{\bar k}, \mathcal{F}|_{X_{\bar k}})$. We omit the proof.
...
If $x \in X(k)$ is a rational point and $\bar x : \text{Spec}(\bar k) \to X$
...
$X=\text{Spec}(k[\lambda, \frac{1}{\lambda(\lambda-1)}])$, and
...
$S = \text{Spec}(\mathbf{F}_p[x, \frac{1}{x(x-1)}])$ and the affine curve
$f: X \to \mathbb P^1-\{0, 1, \infty\}$ over $S$ given by the equation
...
set
$$
A(q) = \lim\sup\nolimits_{X/k} \frac{\sharp X(k)}{g_X}
$$
