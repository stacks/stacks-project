...
\begin{enumerate}
\item $R^h$, $R^{sh}$ are colimits of etale $R$-algebras,
...
\noindent
At this point we are ready to compare the strict henselization
of the local ring of a scheme with the etale local ring of the scheme
as defined earlier.
...
$$
where the left hand side is the strict henselization of
$\mathcal{O}_{S, s}$ as described in
Theorem \ref{theorem-henselization}
and right hand side is the ring defined in
Definition \ref{definition-etale-local-rings}.
\end{lemma}
...
If $R$ is noetherian then $R^h$ is also noetherian and they have the
same completion: $\widehat R \cong \widehat{R^h}$. In particular, $R\subset
R^h \subset \hat R$. The henselization of $R$ is in general much
smaller than its completion and inherits many of its properties ({\it e.g} if
$R$ is reduced, then so is $R^h$, but not $\hat R$ in general).
\end{remark}
