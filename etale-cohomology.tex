...
\begin{matrix}
\text{sheaves of sets }\mathcal{F}\text{ on the \'etale site }X_{\text{\'et}}
\text{ endowed with an} \\
...
$$
A \supseteq \ldots \supseteq F^n A \supseteq F^{n+1}A \supseteq \ldots
\supseteq 0.
$$
...
\item We define $DF(\mathcal{A})$ (resp. $D F^+(\mathcal{A})$)
...
\text{-modules }M\text{ with continuous}\atop
\pi_1(X, \overline x)\text{-action where we use }
