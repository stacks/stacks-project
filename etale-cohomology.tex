...
\begin{enumerate}
\item for each $T_{/S}$, a usual (i.e., Zariski) sheaf $\mathcal{F}_T$ on
$T_{\mathrm{Zar}}$,
...
$$
\begin{matrix}
\mathcal{F} : & (\textit{Sch}/S)^{opp} & \longrightarrow & \text{Ab} \\
...
$$
is not an equalizer. (We will see that $\mathcal{F}$ is in fact does
...
descent data with respect to $\mathcal{V}$ are effective.
\end{theorem}
...
$$
\begin{matrix}
\mathcal{F}^a : & \textit{Sch}/S & \to & \text{Ab}\\
...
\begin{enumerate}
\item $\mathcal{C}$ is a full subcategory of $\textit{Sch}/S$ for
some scheme $S$,
\item any Zariski covering of $T/S$ can be refined by a covering of
$\mathcal{C}$,
...
\ref{theorem-descent-quasi-coherent}. In
...
$\mathcal{O}$-modules on $S$ and quasi-coherent modules
on sites $(\textit{Sch}/S)_{fppf}$, $S_{etale}$, etc.
...
\end{enumerate}
\item
Assume that $S$ is separated. Choose an affine open covering $S = \bigcup_{i \in
...
(\'etale morphisms are open), we see that $\mathcal{V} = \bigcup_{i\in I}\bigcup_{j
