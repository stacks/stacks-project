...
\medskip\noindent
The point of this remark is to make sure we have the map of stalks
corresponding to $a$ pointing in the correct direction. Note that the
above means that according to
Sites, Definition \ref{sites-definition-morphism-points}
$a$ defines a morphism $a : p \to p'$ between the points $p, p'$ of
...
for any sheaf of sets $\mathcal{F}$ on $S_{etale}$.
\item The functor
...
Let $S$ be a scheme. Let $p$ be a point of the small etale site
...
$u(S) = \{*\}$, $u(U \times_V W) = u(U) \times_{u(V)} u(W)$, and
if $\{U_i \to U\}$ is an etale covering, then
...
we can write $u(U) = p^{-1}(h_U^\#)$. Now, if
$U = V \amalg W$, then $h_U = (h_V \amalg h_W)^\#$ and we get
...
|U_{\overline{v}}| \times u(V)
$$
Note that the index set $\{1, \ldots, n\}$ is indeed the fibre of
$U$ over $\overline{s}$ as indicated. Note that
$u(V) \not = \emptyset$ as $s$ is in the image of $V \to S$.

\medskip\noindent
We will use the discussion above to construct a functorial bijective map
$$
c : |U_{\overline{s}}| \longrightarrow u(U)
$$
for $U \in \text{Ob}(S_{etale})$ which will conclude the proof.
(Todo: Find a clever construction of this map which doesn't involve the
choices we make below.) First we
claim that it suffices to construct the map for $U$ affine. We omit the
proof of this claim.
Assume $U \to S$ in $S_{etale}$ with $U$ affine, and let
$\overline{u} : \overline{s} \to U$ be an element of $|U_{\overline{s}}|$
(see discussion in
Lemma \ref{lemma-stalk-gives-point}
and its proof). Choose a $(V, \overline{v})$ as above.
Then the pair $(\overline{u}, \overline{v})$ gives a geometric point of
$U \times_S V$ lying over $\overline{v}$ and determines one of the
components $U_i$ of $U \times_S V$. Pick any $x \in u(V)$.
Let $x_i \in u(U_i)$ be the element mapping to $x$ via the bijection
$u(U_i) \to u(V)$. Finally, let
$c(\overline{u}) \in u(U)$ be the image of $x$ under the map
$u(U_i) \subset u(U \times_S V) \to u(U)$.
Why is this independent of the choice of $x \in u(V)$?
Well, if $x' \in u(V)$ is a second such choice, then since
$u(V \times_S V) = u(V) \times u(V)$ we can choose a
$x'' \in u(V \times_S V)$ mapping to $x$ under the first projection and to
$x'$ under the second projection. Moreover, we have
$$
U \times_S V \times_S V =
(V \times_S V) \amalg \ldots \amalg (V \times_S V) \amalg R
$$
where $R$ has no point lying over $s$
similarly to the decomposition of $U \times_S V$. And the geometric point
$(\overline{u}, \overline{v}, \overline{v})$ of this scheme picks out one
of the components $V \times_S V$ which maps into the component $U_i$
under both projections $U \times_S V \times_S V \to U \times_S V$.
It follows from some diagram chasing that the corresponding map
$u(V \times_S V) \subset u(U \times_S V \times_S V) \to u(U)$
maps the element $x''$ to the same element as both $x$ and $x'$ are mapped
to via the maps above. This proves independence of $x$.
Next, we have to check independence of the choice of $(V, \overline{v})$.
This is verified using the fact that any two such can be dominated
by a third (details omitted). By the discussion of the preceding
paragraph the map is a bijection. Finally, we have to check that
the map $c$ is functorial in $U$. We omit the verification.
\end{proof}
...
Modules, Lemma \ref{lemma-support-section-closed}.
...
other words what happens when given a morphism of schemes. The formal
aspects of this are discussed in
Topologies, Section \ref{topologies-section-etale}.
In this and the next section we discuss this material briefly in the
setting of small etale sites.

\begin{definition}
\label{definition-direct-image-presheaf}
Let $f: X\to Y$ be a morphism of schemes, $\mathcal{F} $ a presheaf on
$X_{etale}$. The {\it direct image} of $\mathcal{F}$ (under $f$) is
$$
\begin{matrix}
f_*\mathcal{F} : &Y_{etale}^{opp} & \longrightarrow & \textit{Sets} \\
...
morphism is again etale, and defines a functor
$f_* : \textit{PSh}(X_{etale}) \to \textit{PSh}(Y_{etale})$
since base change is a functor.
\end{definition}

\begin{remark}
\label{remark-direct-image-sheaf}
If $\{V_i \to V\}_{i\in I}$ is an etale covering in $Y_{etale}$
then $\{X \times_Y V_i \to X \times_Y V\}$ is an etale covering in
$X_{etale}$. This means that the functor $V \mapsto X \times_Y V$ is
a continuous functor of sites, see
Sites, Definition \ref{sites-definition-continuous}.
The sheaf condition for $\mathcal{F}$ with respect to
$\{X \times_Y V_i \to X \times_Y V\}$
is equivalent to the sheaf condition for $f_*\mathcal{F}$ with respect to
$\{V_i \to V\}_{i\in I}$. Thus if $\mathcal{F}$ is a sheaf, so is
$ f_*\mathcal{F}$.
\end{remark}

\begin{definition}
\label{definition-direct-image-sheaf}
The previous functor therefore induces
$f_* : \textit{Sh}(X_{etale}) \to \textit{Sh}(Y_{etale})$,
$f_* : \textit{Ab}(X_{etale}) \to \textit{Ab}(Y_{etale})$,
called {\it direct image} again.
The functor $f_*$ is left exact.
Tts right derived functors $\{R^pf_*\}_{p \geq 1}$ are called
...
$f^{-1} : \textit{Sh}(Y_{etale})\to \textit{Sh}(X_{etale})$
and
...
\noindent
It is not a complete triviality that such an adjoint exists.
We could construct it directly, but it is better to do this in
the general setting of sites and sheaves as explained in
Remark \ref{remark-functoriality-general}
below. The upshot is that we get a morphism of sites
$f_{small} : X_{etale} \to Y_{etale}$ which in turn induces
a morphism of topoi
$$
f_{small} :
\textit{Sh}(X_{etale})
...
\textit{Sh}(Y_{etale})
$$
In this chapter we will usually (but not always) use $f_*$, $f^{-1}$
for pullback and pushforward along this morphism, and not the more
precise notation $f_{small, *}$, $f_{small}^{-1}$.
...
Moreover, there exists a functor $f^{-1}$ which
is left adjoint to $f_*$ and is exact, see
Sites, Definition \ref{sites-definition-morphism-sites} and
Proposition \ref{sites-proposition-get-morphism}.

\medskip\noindent
To recover our special case, notice that a morphism of schemes
$f : X \to Y$ induces a continous functor $u_f: Y_{etale} \to X_{etale}$
by setting $u_f (V \to Y) = V \times_Y X \to X$.
\end{remark}

\begin{lemma}
\label{lemma-stalk-pullback}
Let $f : X \to Y$ be a morphism of schemes.
Let $\overline{x} \to X$ be a geometric point.
Let $\mathcal{G}$ be a sheaf on $Y_{etale}$.
Then there is a canonical identification
$$
\left(f^{-1}\mathcal{G}\right)_{\overline{x}} = \mathcal{G}_{\overline{y}}.
$$
...
\begin{proof}
The exactness of $f^{-1}$ is a consequence of
Sites, Proposition \ref{sites-proposition-get-morphism}.
In the literature it is often deduced from the first statement via
Theorem \ref{theorem-exactness-stalks}.

\medskip\noindent
To prove the first statement we argue as follows. Note that by
Lemma \ref{lemma-stalk-exact}
...
$$
\begin{matrix}
\text{Spec}(K)_{etale} & \longrightarrow & \mathcal{G}\textit{-Sets} \\
...
$\text{Spec}(K)_{etale}$ correspond to surjective families of maps in
$\mathcal{C}^0(\mathcal{G}\text{-Sets})$.
\end{remark}
...
$$
\begin{matrix}
\textit{Sh}(\text{Spec}(K)_{etale}) & \longrightarrow &
\mathcal{C}^0(\mathcal{G}\text{-Sets}) \\
\mathcal{F} & \longmapsto & \mathcal{F}_{\text{Spec} K^{sep}}
\end{matrix}
$$
is an equivalence of categories. In other words, every sheaf on
$\text{Spec}(K)_{etale}$ is representable.
\end{lemma}

\begin{proof}
The category $\text{Spec}(K)_{etale}$ has some extra structure (maybe pushouts 
or
...
Let $\mathcal{F}$ be an abelian sheaf on $\text{Spec}(K)_{etale}$. Then
\begin{enumerate}
...
have
\begin{eqnarray*}
\Gamma(\text{Spec}(K)_{etale}, \mathcal{F}) & = &
\text{Hom}_{\textit{Sh}(\text{Spec}(K)_{etale})}(h_{\text{Spec}(K)}, 
\mathcal{F})
\\
...
H_{et}^1(S_{etale}, \mathbf{G}_m) & = & H^1(S, \mathcal{O}_S^*) \, = \, H^1(S,
\mathcal{O}_S^*) \, = \, \text{Pic}(S) \; ;\\
H_{et}^i(S_{etale}, \mathbf{G}_a) & = & H_{Zar}^i(S, \mathcal{O}_S).
\end{matrix}
$$
Also, for any quasi-coherent sheaf $\mathcal{F}$ on $S_{etale}$, $H^i(S_{etale},
...
$$
\begin{matrix}
\textit{Sh}(S_{etale}) & \longrightarrow & \{ \text{discrete $\mathcal{G}$-sets
...
\noindent
In particular, the category $\textit{Ab}(S_{etale})$ corresponds to the full
