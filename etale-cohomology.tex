...
$f : X \to Y$ and $f' : X' \to Y'$ are bounded.
This means that
$$
f_* :
...
$$
Thus it suffices to show the lemma when $Y$ is the spectrum of an
algebraically closed field.

\medskip\noindent
Assume $Y$ is the spectrum of an algebraically closed field.
Then $K$ is an object of $D^+(\mathbf{Z})$ and
$E \in D^+(Y)$ has torsion cohomology sheaves.
Say $H^i(E) = 0$ for $i \geq a$ and $H^j(K) = 0$ for $j \geq b$.
We want to show that
$$
H^n(X, E \otimes_\mathbf{Z}^\mathbf{L} f^{-1}K) =
H^n(R\Gamma(X, E) \otimes_\mathbf{Z}^\mathbf{L} K)
$$
...
$\mathcal{E}^\bullet$ of torsion sheaves. (Take a
complex of torsion sheaves $\mathcal{A}^\bullet$
representing $E$ using Lemma \ref{sites-cohomology-lemma-torsion}
and then take $\mathcal{E}^\bullet = \tau_{\geq a}\mathcal{A}^\bullet$.)
Then $E \otimes_\mathbf{Z}^\mathbf{L} f^{-1}K$ is
represented by
$$
\text{Tot}(\mathcal{E}^\bullet \otimes_\mathbf{Z} f^{-1}K^\bullet)
$$
...
f^{-1}\sigma_{\geq -a + n + 1}K^\bullet) = 0
$$
and
$$
H^n(R\Gamma(X, E)
...
$\mathcal{E}^\bullet$ is a single torsion abelian sheaf sitting
...
is a torsion abelian sheaf on $X$. In this case we
...
an algebraically closed field. Then $K \in D(\mathbf{Z})$.
Let $T(K)$ be the property that the statement of the
lemma holds for $K \in D(\mathbf{Z})$.
...
Property (3) says the lemma holds when $K = \mathbf{Z}[k]$
