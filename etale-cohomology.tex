...
$(\textit{Sch}/S)_{fppf}$ and $(\textit{Sch}/S)_{syntomic}$.
\end{lemma}
...
Let $R$ be a strictly henselian ring and $S=\Spec(R)$. Then the global
sections functor
...
Lemma \ref{lemma-geometric-lift-to-cover}.
Pick an affine open neighborhood $\Spec(A)$ of $u_i$ in $\mathcal{U}_i$.
...
$$
\xymatrix{
\Spec(A) \ar[dr] \ar@{^{(}->}[r] & {\mathcal{U}_i} \ar[d]\\
& {S.} \ar@/^1pc/[ul]
}
$$
...
\Spec(\mathcal{O}_{Y, \bar y}^{sh})$. It suffices to show that for
all $q\geq 1$, $H_{et}^q(X_{\bar y}^{sh}, \mathcal{G})=0$. Since $f$ is
finite, $X_{\bar y}^{sh}$ is finite over $\Spec(\mathcal{O}_{Y, \bar
y}^{sh})$, thus $X_{\bar y}^{sh} = \Spec(A)$ for some ring $A$
...
$\Spec(A) = \coprod_{i=1}^r \Spec(A_i)$, and we can apply
...
$X \to \Spec(K)$
such that $\text{Mor}_K(\Spec(K^{sep}), X)$ is
...
$\mathcal{F}_{\overline{s}} = \text{Mor}_K(\Spec(K^{sep}), X) = M$
...
\begin{eqnarray*}
\Gamma(S, \mathcal{F}) & = &
\text{Mor}_{\textit{Sh}(S_{\acute{e}tale})}(h_{\Spec(K)},  \mathcal{F})
\\
...
H_{et}^1(\Spec(K), \text{GL}_d(\mathcal{O}))
$$
...
be the latter. Hence
$$
(R^q j_*\mathbf{G}_{m,\eta})_{(X, \bar x)} = H_{et}^q(\Spec
...
& = &
H_{et}^q(\Spec(\kappa(\eta)^{sep}) \times_X \eta, \mathbf{G}_m) \\
& = & H_{et}^q (\Spec(\kappa(\eta)^{sep}), \mathbf{G}_m) \\
& = & 0 \ \ \text{ for } q \geq 1
\end{eqnarray*}
...
so
$\text{Ker}\left(
\underline A_\mathcal{U} \xrightarrow{\varphi} \underline B_\mathcal{U}
\right) = \underline{\text{Ker}(\varphi)}_\mathcal{U}$ and
...
Since pullback if exact, we thus have
$\text{Ker}(\varphi|_{X_i}) =
\text{Ker}(\mathcal{F}|_{X_i} \xrightarrow{\varphi} \mathcal{G}|_{X_i})$
which is finite
...
and on the other hand
$$
\left(j_* \mathcal{F} \right)_{\bar x} = H_{et}^0(\Spec(\mathcal{O}_{X,
\bar x}^{sh})\times_X \mathcal{V}, \mathcal{F}).
$$
But $j$ is finite and $\mathcal{O}_{X, \bar x}$ is strictly henselian, hence
$\Spec(\mathcal{O}_{X, \bar x}^{sh})\times_X \mathcal{V}$ splits
completely into spectra of strictly henselian local rings
$$
\Spec(\mathcal{O}_{X, \bar x}^{sh})\times_X \mathcal{V} =
\coprod_{\bar v \to \bar x} \Spec(\mathcal{O}_{X, \bar x}^{sh})
$$
...
j_!\underline{\mathbf{Z}/n\mathbf{Z}})$ which maps to $s$. Thus,
$\text{Im}(\varphi) \subset \mathcal{F}$ is a constructible subsheaf and
$s \in \text{Im}(\varphi)(\mathcal{U})$. A similar argument applies for a
finite collection of sections, and the result follows by taking colimits.
\end{proof}
