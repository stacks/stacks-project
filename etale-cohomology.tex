...
Then $p$ defines a point of the site $S_{etale}$
(Sites, Definition \ref{sites-definition-point})
and its associated stalk functor
...
Having said this we see that $p(U \times_V W) = p(U) \times_{p(V)} p(W)$
whenever $U \to V$ and $W \to V$ are morphisms in $S_{etale}$. Moreover, given
...
\begin{proof}
Before discussing this in the setting of etale sheaves
we first deduce the lemma from the general material in the chapter on
sites and sheaves, using that $\mathcal{F} \mapsto \mathcal{F}_{\overline{s}}$
comes from a point of the small etale site of $S$, see
Lemma \ref{lemma-stalk-gives-point}.
By
Sites, Lemmas \ref{sites-lemma-adjoint-point-push-stalk} and
\ref{sites-lemma-point-pushforward-sheaf}
we have (2) and we have that
$\mathcal{F} \mapsto \mathcal{F}_{\overline{s}}$
is a left adjoint hence commutes with arbitrary colimits (see
...
$\mathcal{F} \mapsto \mathcal{F}_{\overline{x}}$
is exact. Assertions (1) and (3) follow formally from (4).
We will also give a direct proof of (1), (2) and (3).
...
a geometric point $\overline{s}$ lying over $s$.
...
for pullback and pushforward along this morpshism, and not the more
...
\end{align*}
in the fourth equality the pair $(U, \overline{u})$ and the map
...
\begin{matrix}
* & \text{if} & \overline{x} \not \in Z \\
\mathcal{F}_x & \text{if} & \overline{x} \in Z
\end{matrix}
\right.
$$
