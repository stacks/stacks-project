...
U/X
\longmapsto
\text{colim}_{U \to X \times_Y V}\ \mathcal{G}(V)
\end{equation}
...
\medskip\noindent
The pair of functors $(f_*, f^{-1})$ define
a morphism of small etale topoi
$$
f_{small} :
\textit{Sh}(X_{etale})
\longrightarrow
...
\begin{proof}
Let $V \subset Y$ be an affine open. Then $h_V$ is a subsheaf of the
final sheaf $*$ on $Y_{etale}$. As $g$ is exact we see that
$g^{-1}h_V$ is a subsheaf of the final sheaf on $X_{etale}$.
Hence by
Lemma \ref{lemma-support-subsheaf-final}
there exists an open subscheme $W \subset X$ such that
$g^{-1}h_V = h_W$. Choose an affine open $U \subset W$.
Let $j_U, j_V, j_W, j_{U/W}$ denote the morphisms
$j_U : U \to X$, $j_W : W \to X$, $j_{U/W} : U \to W$ and $j_V : V \to Y$.
The associated morphisms of etale sites are all localizations and hence
the material of
Sites, Section \ref{sites-section-localization}
applies. Consider the functors
$$
g'_* :
\textit{Sh}(U_{etale})
\to
...
$g'_*\mathcal{F} = j_V^{-1} g_* j_{U, *}\mathcal{F}$ and
$(g')^{-1}\mathcal{G} = j_U^{-1}g^{-1}j_{V!}\mathcal{G}$.
These are adjoint because
\begin{align*}
\text{Mor}_{\textit{Sh}(U_{etale})}((g')^{-1}\mathcal{G}, \mathcal{F})
& =
\text{Mor}_{\textit{Sh}(U_{etale})}
(j_U^{-1}g^{-1}j_{V!}\mathcal{G}, \mathcal{F}) \\
& =
\text{Mor}_{\textit{Sh}(X_{etale})}
(g^{-1}j_{V!}\mathcal{G}, j_{U, *}\mathcal{F}) \\
& =
\text{Mor}_{\textit{Sh}(Y_{etale})}
(j_{V!}\mathcal{G}, g_*j_{U, *}\mathcal{F}) \\
& =
\text{Mor}_{\textit{Sh}(V_{etale})}
(\mathcal{G}, j_V^{-1}g_*j_{U, *}\mathcal{F}) \\
& =
\text{Mor}_{\textit{Sh}(V_{etale})}
(\mathcal{G}, g'_*\mathcal{F})
\end{align*}
In order to show that $(g'_*, (g')^{-1})$ defines a morphism of topoi
we have to show that $(g')^{-1}$ is exact. Of the three functors
$j_U^{-1}, g^{-1}, j_{V!}$ only $j_{V!}$ is not exact. But it defines
an equivalence of categories
$$
j_{V!} :
\textit{Sh}(V_{etale})
\longrightarrow
\textit{Sh}(Y_{etale})/h_V
$$
Moreover, the functor $g^{-1}$ transforms $h_V$ into $h_W$ and hence
defines an exact (!) functor
$\textit{Sh}(Y_{etale})/h_V \to \textit{Sh}(X_{etale})/h_W$.
Also, $j_U^{-1}h_W = *$ and hence $j_U^{-1}$ defines an exact functor
$\textit{Sh}(X_{etale})/h_W \to
\textit{Sh}(U_{etale})/* = \textit{Sh}(U_{etale})$.
Since an equivalence of categories is exact, this implies that
the composition $(g')^{-1} = j_U^{-1}g^{-1}j_{V!}$ is exact.
It follows that $(g'_*, (g')^{-1})$ defines a morphism of topoi.
Finally, we define
$$
(g')^\sharp : \mathcal{O}_V \to g'_*\mathcal{O}_U
$$
to be
$$
\mathcal{O}_V \to j_V^{-1}\mathcal{O}_Y
\xrightarrow{j_V^{-1}g^\sharp}
j_V^{-1}g_*\mathcal{O}_X
\to
j_V^{-1} g_* j_{U, *}\mathcal{O}_U =
(g')_*\mathcal{O}_U
$$
Here the first map is the inverse of the isomorphism
$j_V^\sharp : j_V^{-1}\mathcal{O}_Y \to \mathcal{O}_V$.
To be continued.
\end{proof}
