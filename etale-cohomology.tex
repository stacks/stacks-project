...
field $k$ of characteristic $p > 0$. Then
$H_\etale^q(X, \underline{\mathbf{Z}/p\mathbf{Z}})$
is a finite $\mathbf{Z}/p\mathbf{Z}$-module for all $q$.
\end{lemma}

\begin{proof}
This follows immediately from the long exact cohomology sequence
associated to the Artin-Schreier sequence, the finite dimensionality
of the cohomology groups $H^q(X, \mathcal{O}_X)$
(Cohomology of Schemes, Lemma
\ref{coherent-lemma-proper-over-affine-cohomology-finite}),
and Lemma \ref{lemma-F-1}.
\end{proof}
...
we find
$$
H^q_\etale(Z, f_*\underline{M}|_Z) = H^q(Y \times_X Z, \underline{M}).
$$
...
\section{Higher vanishing for torsion sheaves on curves}
\label{section-vanishing-torsion}

\noindent
The goal of this section is to prove
Theorem \ref{theorem-vanishing-affine-curves}. To do this we will use
The proof of this follows the ``m\'ethode de la trace''
...
then essentially consists in the following observation: if $\mathcal{F}$
...
Let $X$ be a smooth affine curve over an algebraically closed field.
Let $j : U \to X$ an open immersion. Let $\ell$ be a prime number.
Then
$H_\etale^q(X, j_! \underline{\mathbf{Z}/\ell\mathbf{Z}}) = 0$
for all $q \geq 2$.
\end{lemma}

\begin{proof}
The case $\ell$ not equal to the characteristic of the ground field.
Consider the short exact sequence
$$
...
\longrightarrow 0.
$$
We know that the cohomology of the middle sheaf vanishes in degree at
least $2$ by
Lemma \ref{lemma-vanishing-cohomology-mu-smooth-curve}
and that of the skyscraper sheaf on the right vanishes in degree at least $1$.
Thus applying the long exact cohomology sequence, we get the vanishing of
$j_!\underline{\mathbf{Z}/\ell\mathbf{Z}}_U$ in degree at least $2$.

\medskip\noindent
The case $\ell = p$ equal to the characteristic of the ground field
follows from the more general Lemma \ref{lemma-vanishing-affine-char-p-p}.
\end{proof}

\begin{lemma}
\label{lemma-somewhat-easier}
Let $X$ be an affine curve over an algebraically closed field.
Let $j : U \to X$ an open immersion. Let $\ell$ be a prime number.
Then
$H_\etale^q(X, j_! \underline{\mathbf{Z}/\ell\mathbf{Z}}) = 0$
for all $q \geq 2$.
...
assume our curve is smooth. Let $\nu : X' \to X$ be the normalization
morphism which is finite as varieties are Nagata schemes.
Let $j' : U' \to X'$ be the inverse image of $U$. In general it won't be
true that $\nu_*j'_!\underline{\mathbf{Z}/\ell\mathbf{Z}}$ is equal to
$j_!\underline{\mathbf{Z}/\ell\mathbf{Z}}$, but there will be a canonical
injective map
$$
j_!\underline{\mathbf{Z}/\ell\mathbf{Z}} \to
\nu_*j'_!\underline{\mathbf{Z}/\ell\mathbf{Z}}
$$
whose cokernel is supported in finitely many points. Hence the vanishing
of higher cohomology of the sheaf on the right implies the vanishing
of the lemma. Details omitted.
\end{proof}
...
Let $X$ be an affine curve over an algebraically closed field $k$.
Let $j : U \to X$ an open immersion. Let $\ell$ be a prime number.
Let $\mathcal{G}$ a finite locally constant sheaf of $\mathbf{F}_\ell$-vector
spaces on $U$. Then $H_\etale^q(X, j_!\mathcal{G}) = 0$ for all
$q \geq 2$.
\end{lemma}
...
vanishing for $j_!f_*f^{-1}\mathcal{G}$.
By Zariski's Main Theorem we can choose a diagram
$$
\xymatrix{
...
$$
with $\overline{f} : Y \to X$ finite. Then we see that
$j_!f_*f^{-1}\mathcal{F} = \overline{f}_*j'_!f^{-1}\mathcal{F}$
(for example by looking at stalks; details omitted).
Thus it suffices to prove the vanishing for $j'_!f^{-1}\mathcal{F}$.
However, the existence of the filtration given by
Lemma \ref{lemma-pullback-filtered}
reduces us to the vanishing of cohomology of
...
By Lemma \ref{lemma-colimit} it suffices to prove the result for a
constructible sheaf.

\medskip\noindent
Writing $\mathcal{F} = \mathcal{F}_1 \oplus \cdots \oplus \mathcal{F}_r$
...
$$
Looking at the long exact cohomology sequence, we see that it suffices to
assume that $\mathcal{F}$ is $\ell$-torsion. Thus we may assume that
$\mathcal{F}$ is a constructible sheaf of $\mathbf{F}_\ell$-vector spaces
for some prime number $\ell$.

\medskip\noindent
By definition this means there is a nonempty open $U \subset X$
such that $\mathcal{F}|_U$ is finite locally constant sheaf of
$\mathbf{F}_\ell$-vector spaces. Consider the short exact sequence
$$
0 \to j_!j^{-1}\mathcal{F} \to \mathcal{F} \to i_*i^{-1}\mathcal{F} \to 0
$$
where $i : Z \to X$ is the inclusion of the complement, see
Lemma \ref{lemma-ses-associated-to-open}. Since the \'etale cohomology
of $i_*i^{-1}\mathcal{F}$ vanishes in degrees $\geq 1$ (because $Z$
is a finite disjoint union of spectra of algebraically closed fields)
we reduce to the case $\mathcal{F} = j_!\mathcal{G}$ where
$\mathcal{G}$ is a finite locally constant sheaf of $\mathbf{F}_\ell$-vector
spaces on $U$. This case is handled in Lemma \ref{lemma-vanishing-easier}.
\end{proof}
...
Let $X$ be a separated, finite type, dimension $1$ scheme over an
...
\begin{proof}
We may replace $X$ by its reduction by
Proposition \ref{proposition-topological-invariance}.
Let $X = \bigcup X_i$ be the irreducible components of $X$.
We think of each $X_i$ as a reduced closed subscheme of $X$,
hence each $X_i$ is a curve. The morphism $f : \coprod X_i \to X$
is finite and is an isomorphism away from finitely many points.
Hence $\mathcal{F} \to f_*f^{-1}\mathcal{F}$ is an isomorphism
over this open and the kernel and cokernel are torsion sheaves
supported in those points. Hence it suffices to prove the
vanishing for $f^{-1}\mathcal{F}$, i.e., we reduce to the case
where $X$ is a curve. (Observe that if $X$ is affine, then all
$X_i$ are affine too.)

\medskip\noindent
If $X$ is a curve, then $X$ is either affine or projective, see
Varieties, Lemma \ref{varieties-lemma-curve-affine-projective}.
In the affine case we apply Theorem \ref{theorem-vanishing-affine-curves}.
In the projective case we see that $X = U \cup V$ with $U$ and $V$
affine. Hence Mayer-Vietoris guarantees the vanishing in
degrees $\geq 3$.
\end{proof}
...
\section{Invariance under extension of algebraically closed ground field}
\label{section-invariance}
...
\medskip\noindent
However, this phenomenon does not happen for varieties over algebraically
closed fields of characteristic $0$. Moreover, if we only look at
coefficients of order prime to $p$, then it also doesn't happen in
characteristic $p$. In this section we prove some preliminary results
in this direction that we will need in our proof of the proper base change
theorem.
...
\begin{proof}
This proof is a classic example of applying algebraization and
approximation. We prove this in a number of stages.
...
Moreover, in this case the morphism $U_0 \to \Spec(A/\mathfrak m)$
...
\medskip\noindent
General case. Let $(A, \mathfrak m)$ be a henselian local ring. By
More on Algebra, Lemma \ref{more-algebra-lemma-henselian-local-limit-G-rings}
we can write $A = \colim A_i$ as a filtered colimit of
henselian local Noetherian G-rings. By
Limits, Lemma \ref{limits-lemma-descend-finite-presentation}
there exists an $i$ and a finitely presented morphism $X_i \to \Spec(A_i)$
whose base change to $\Spec(A)$ is $X$. After increasing $i$
we may assume that $X_i \to \Spec(A_i)$ is proper
(Limits, Lemma \ref{limits-lemma-eventually-proper}).
The category of schemes finite \'etale over $X$ is the limit of the
category of schemes finite \'etale over $X_j$, $j \geq i$ (by
Limits, Lemmas
\ref{limits-lemma-descend-finite-presentation},
\ref{limits-lemma-descend-finite-finite-presentation}, and
\ref{limits-lemma-descend-etale})
The same thing is true for schemes finite \'etale over $X_0 = \lim (X_j)_0$.
Thus we formally deduce the result for $X / \Spec(A)$
from the result for the $X_j / \Spec(A_j)$ which we dealt with above.
\end{proof}
...
is $\geq n$ is open. Since $f$ is of finite type, every point is
...
a morphism of schemes giving rise to the cartesian diagram
$$
\xymatrix{
X' \ar[r]_{g'} \ar[d]_{f'} & X \ar[d]^f \\
Y' \ar[r]^g & Y
