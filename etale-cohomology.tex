...
Therefore, on the category $\textit{Mod}_G$ the functors
...
$$
for $M \in \textit{Mod}_G$. It is known that these maps are
...
for a topological group $G$ and $M \in \Ob(\textit{Mod}_G)$
please email
\href{mailto:stacks.project@gmail.com}{stacks.project@gmail.com}.
...
$$
The abelian group $\text{Pic}^0(X)$ can be identified with
$\text{Pic}^0(X) = \underline{\text{Pic}}^0_{X/k}(k)$, i.e.,
the $k$-valued points of an abelian variety
$\underline{\text{Pic}}^0_{X/k}$ over $k$ of dimension $g$.
Consequently, if $n \in k^*$ then
$\text{Pic}^0(X)[n] \cong (\mathbf{Z}/n\mathbf{Z})^{2g}$
...
$n : \text{Pic}(X) \to \text{Pic}(X)$. Consider the commutative diagram with
exact rows
$$
\xymatrix{
0 \ar[r] &
...
show the kernel of $n : \text{Pic}^0(X) \to \text{Pic}^0(X)$
...
$$
\mathcal{F}^a : (\Sch/S)_{fppf} \longrightarrow \textit{Sets},\quad
...
suffices to show that $\mathcal{F}^a$ is a sheaf for the fppf
...
we can choose an fppf covering $\{U_i \to U\}$
such that $\xi$ restricts to zero on $U_i$.
By More on Morphisms, Lemma
\ref{more-morphisms-lemma-qf-fp-flat-dominates-fppf}
we may assume that $U_i \to U$ is locally quasi-finite.
Pick an $i \in I$ and a point $u_i \in u_i$ mapping to $s$.
Choose an elementary \'etale neighbourhood
$(V, v) \to (U, u)$ such that there exists an open
$$
U_i \times_U V  \supset W
$$
which contains a unique point $w \in W$ mapping to $v \in V$
and such that $W \to V$ is finite, see
More on Morphisms, Lemma
\ref{more-morphisms-lemma-etale-makes-quasi-finite-finite-at-point}.
Then $W \to V$ is finite locally free, because it is finite
and because $U_i \times_U V \to V$ is flat and locally of finite presentation
as a base change of the morphism $U_i \to U$, see
...
Hence $W \to V$ is open, and after shrinking $V$
we may assume that $W \to V$ is surjective finite locally free.
After replacing $(U, u)$ by $(V, v)$ we may assume there exists
...
Let $\mathcal{F}$ be a sheaf on $S_\etale$. The rule
$$
\mathcal{F}^a : (\Sch/S)_{ph} \longrightarrow \textit{Sets},\quad
(f : X \to S) \longmapsto \Gamma(X, f_{small}^{-1}\mathcal{F})
$$
is a sheaf and a fortiori a sheaf on $(\Sch/S)_\etale$.
In fact, $\mathcal{F}^a$ is equal to
...
The statement about the \'etale topology is the content
of Lemma \ref{lemma-describe-pullback}. To finish the proof it
suffices to show that $\mathcal{F}^a$ is a sheaf for the ph
...
\xymatrix{
\mathcal{F}^a(U) \ar[r] &
\mathcal{F}^a(V) \ar@<1ex>[r] \ar@<-1ex>[r] &
\mathcal{F}^a(V \times_U V)
}
$$
Set $\mathcal{G} = \mathcal{F}^a|_{U_\etale}$.
...
We have
$$
\mathcal{F}^a(V) = \Gamma(V, f^{-1}\mathcal{G}) =
...
$$
Thus it suffices to compute the equalizer of these two maps of sheaves.
