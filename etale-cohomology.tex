...
$q \geq 2$.
\item If $S$ is a Noetherian scheme of dimension $d$, then
...
scheme. The second statement follows from this and the vanishing
of Cohomology, Lemma \ref{cohomology-lemma-vanishing-Noetherian}.
Details omitted.
\end{proof}
...
\begin{proof}
Omitted. Hints: If $X$ is proper over $k$, then this is a consequence of
Cohomology of Schemes, Lemma
\ref{coherent-lemma-proper-over-affine-cohomology-finite}.
In general one reduces to the proper case by Chow's lemma.
\end{proof}
...
\begin{proof}[Sketch of proof]
Let $d = \dim(X)$. It suffices to show that
...
$\mathcal{F}$ on $X$ (even though $X$ is not proper). For any $p$-linear
endomorphism $F$ of a finite dimensional $k$-vector space, the map
$1 - F$ is surjective (as $k$ is assumed algebraically closed),
...
\medskip\noindent
Of course the map (\ref{equation-base-change}) has no chance of being
an isomorphism in the generality given above. The goal is to show
