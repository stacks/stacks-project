...
\medskip\noindent
General case. Let $A$ be a henselian local ring. Write $A = \colim A_i$
as a filtered colimit of finite type $\mathbf{Z}$-algebras.
...
(Limits, Lemmas \ref{limits-lemma-descend-finite-finite-presentation} and
\ref{limits-lemma-descend-etale}).
Fix such an $i$, write $A = A_i$, and $U(A) = U_i$.
Since $k$ is algebraically closed we can find a $k$-valued point
$t$ in $\Spec(A)$. Let $U = U(A)_t$ be the fibre of $U(A)$ over $t$.
We claim that $U' \cong U_{k'}$.

\medskip\noindent
Namely, we now have two finite \'etale morphisms
$$
U(A) \to X_A
\quad\text{and}\quad
U_A \to X_A
$$
where the second is the base change of the finite \'etale
morphism $U = U(A)_t \to X$. By construction these
morphisms have isomorphic fibres at $t$.
