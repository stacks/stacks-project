...
The category of $G$-modules is denoted $\text{Mod}_G$.
\end{definition}
...
as explained in SGA4.
\medskip\noindent
Let $f : Y \to X$ be a \'etale morphism of schemes. There are pairs of
...
$H_\etale^q(Y, f^{-1}\mathcal{F})= 0$. The m\'ethod de la trace will be used in 
the proof of Lemma 0.3. We will need some facts on the composition of adjoint 
functors:
\begin{lemma}
Let $\mathcal{C}_1$, $\mathcal{C}_2$, and $\mathcal{C}_3$ be categories such 
that there are a pair of functors $G: \mathcal{C}_3 \rightarrow \mathcal{C}_2$ 
and $F: \mathcal{C}_2 \rightarrow \mathcal{C}_1$ with left adjoints $G^{-1}$ 
and $F^{-1}$ respectively. The functor $H = F \circ G$ is left adjoint to 
$H^{-1} = G^{-1} \circ F^{-1}$. Furthermore, given objects $X_3 \in 
\mathcal{C}_3$ and $X_2 \in \mathcal{C}_2$ such that $X_2 = G(X_3)$ then we 
have:
\begin{equation}
\epsilon_{X_3}^GG^{-1}(\epsilon^F_{X_2}) = \epsilon^H_{X_3}
\end{equation}
Where $\epsilon$ is the counit of the adjunctions. 
\end{lemma}
\begin{proof}
Given objects $X \in \mathcal{C}_3$, $Y \in \mathcal{C}_1$ we have: \
\[\begin{aligned}
\SheafHom(Y, H(X)) ={} & \SheafHom(Y, F\circ G(X)) \\ & \cong 
\SheafHom(F^{-1}(Y),G(X))) \\& \cong \SheafHom(G^{-1}F^{-1}(Y),X) \\& = 
\SheafHom(H^{-1}(Y),X)\]
\end{aligned}\]
Thus, the first fact is established. Now consider the right hand side of 
equation (1).  By the counit formula, [Reference needed] it gives the 
adjunction of $\epsilon^F_{X_2}$ with respect to the adjoint functor $G$. In 
particular, $\epsilon_{X_2}^F$ is the morphism:
\[F^{-1}F(X_2) \rightarrow X_2\]
Adjoint under $F$ to the identity map. This can be rewritten as:
\[F^{-1}FG(X_3) \rightarrow G(X_3)\]
which under adjunction by $G$ gives rise to the map:
\[G^{-1}F^{-1}FG(X_3) \rightarrow X_3\]
Now we claim this is the adjoint to the identity under the adjunction given by 
$H$, establishing the equivalence of the right and left hand side. As seen by 
the above chain of isomorphisms, adjunction under $H$ is given by applying 
adjunction under $G$ follow by adjunction under $F$. This will just reverse the 
steps just taken above. Showing that this morphism is adjunct with respect to 
$H$ to the identity map: 
\[F(X_2) \rightarrow F(X_2)\]
Or in other words the identity map:
\[H(X_3) \rightarrow H(X_3)\]
\end{proof}
\begin{lemma}
Let $X = \lim_{i\in I}{X_i}$ be a directed limit of $S$-schemes, where S is 
quasi-separated and quasi-compact with affine transition morphisms. Let 
$g_i:X_i \rightarrow S$, $g:X \rightarrow S$, and $f_i:X \rightarrow X_i$ be 
the structure and projection morphisms respectively. Let  $\mathcal{F}$ be an 
abelian sheaf on X, then we have:
\[R^pg_{*}\mathcal{F} = \colim_{i \in I}{R^pg_{i*}(f_{i*}}\mathcal{F})\]
\end{lemma}
\begin{proof}
This will follow from Lemma 52.5, with $\mathcal{F}_i = f_{i*}\mathcal{F}$. We 
must check that this system of sheaves $\mathcal{F}_i$ satisfies the conditions 
of the lemma. First, for $i' \geq i$, let $f_{i'i}: X_{i'} \rightarrow X_i$ be 
the transition map. Then we must specify a morphism:
\[f_{i'i}^{-1}f_{i*}\mathcal{F} \rightarrow  f_{i'*}\mathcal{F}\]
By adjointness, this morphism can be chosen to be adjoint to the identity map:
\[f_{i*}\mathcal{F} = f_{i'i*}f_{i'*}\mathcal{F}\]
Denote the chosen morphism by $\phi_{i'i}:f^{-1}_{i'i}\mathcal{F}_i \rightarrow 
\mathcal{F}_{i'}$. It is the counit of the adjunction given by 
($f_{i'i*}$,$f_{i'i}^{-1}$). We must prove that for all $i'' \geq i' \geq i$  
the maps satisfy $\phi_{i''i} = \phi_{i''i'}\circ f_{i''i'}^{-1}\phi_{i'i}$. 
This is simply equation (1) from Lemma 0.1. Lastly, we must prove that the map 
given by adjunction:
\[\colim_{i \in I}{f_i^{-1}f_{i*}\mathcal{F}}= \colim_{i \in 
I}{f_i^{-1}\mathcal{F}_i} \rightarrow \mathcal{F}\]
is an isomorphism.  First, we make a few observations. Consider some \'etale 
morphism $U \rightarrow X$. Assume that $U$ is quasi-compact and 
quasi-separated, then in the affine case it follows from Algebra Lemma 141.3 
that there exists an index $i_0$ and an \'etale morphism $U_{i_0} \rightarrow 
X_{i_0}$ such that the pullback to $X$ is the original \'etale morphism. The 
fact may be extended to general quasi-compact and quasi-separated schemes. We 
also note in the case U is affine, by Sites and Sheaves Lemma 11.2, that:
\begin{equation}
(\colim_{i \in I}{f_i^{-1}f_{i*}\mathcal{F}})(U) = \colim_{i \in 
I}{f_i^{-1}f_{i*}\mathcal{F}(U)}
\end{equation}
For surjectivity, suppose we have $s \in \mathcal{F}(U)$, then we must show 
that after refining to some cover of $U$ that the restrictions of  $s$ to the 
cover lifts to the colimit. We may first take a covering by quasi-compact and 
quasi-seperated schemes (i.e an affine cover of $U$.) Thus, we may assume $U$ 
is quasi-separated and quasi-compact.  Then as noted above $U$ will come from 
the pullback of some $U_{i_0} \rightarrow X_{i_0}$.  By (1), we may study the 
sections of the individual sheafs comprising the colimit. Note that 
$f_i^{-1}f_{i*}\mathcal{F}$ is the sheaf associated to the presheaf: 
\[
W/X
\longmapsto
\colim_{V \to X_i \times_X W} f_{i*}\mathcal{F}(V/X)
\]
In particular, we note that when $W = U$ there is a contribution to the colimit 
by $f_{i_0*}\mathcal{F}(U_{i_0}/X)$. Since $X_i \times_X U_{i_0} = U$, we also 
have:
\[ f_{i_0*}\mathcal{F}(U_{i_0}) = \mathcal{F}(X_i \times_X U_{i_0}) = 
\mathcal{F}(U)  \]
Now by Lemma 10.8 in Sites and Sheaves, every section in the sheafification of 
a presheaf may be lifted to a section in the presheaf after refining the cover. 
Thus, by refining our cover even further, we may find a section mapping to 
every $s \in \mathcal{F}(U)$. Injectivity follows similarly. 
\end{proof}
\begin{lemma}
Fix a prime $\ell$ and let $X = \lim_{i \in I}{X_i}$ be a limit of a directed 
system of $S$-schemes, where S is a quasi-compact and quasi-separated. 
Furthermore, suppose that each $X_i$ is finite \'etale of constant degree 
relatively prime to $\ell$ over S.  For any abelian $\ell$ torsion sheaf or a 
non-$\ell$ divisible sheaf $\mathcal{G}$ on $S$ such that $H_\etale^n(S, 
\mathcal{G}) \neq 0$ there exists an $\ell$ torsion sheaf or non $\ell$ 
divisible sheaf $\mathcal{F}$ on $X$ such that $H_\etale^n(X, \mathcal{F}) \neq 
0$
\end{lemma}
\begin{proof}
Let $g:X\rightarrow S$ and $g_i:X_i\rightarrow S$ denote the structure 
homomorphisms. Fix a sheaf $\mathcal{G}$ on $S$, as in the statement of the 
Lemma. Since the morphism $f_i$ is of constant degree relatively prime to 
$\ell$ over S, multiplication by the degree of the morphism in $\mathcal{G}$ is 
an isomorphism.(This is also clearly true in the case of a sheaf that is not 
$\ell$ divisible.) Thus, the ``m\'ethode de la trace'' can be applied, allowing 
us to conclude $H^n(X_i, g_i^{-1}\mathcal{G}) \neq 0$. It is trivial that the 
system of sheafs given by $\mathcal{G}_i = g_i^{-1}$ satisify the conditions of 
Theorem 52.1, with the colimit sheaf given by $g^{-1}\mathcal{F}$. This tells 
us that:
\[\colim_{i\in I}H^p_\etale(X_i,g_i^{-1}\mathcal{G}) = 
H^p_\etale(X,\mathcal{G})\]
By virtue of the $g_i$ being finite \'etale morphisms, the transition maps of 
the left hand side are injective. Thus, the colimit is non-zero since each of 
the elements constituiting it is. We may therefore conclude that  
$H^n(X,g^{-1}\mathcal{G}) \neq 0$, giving us the desired result with 
$\mathcal{F} = g^{-1}\mathcal{G}$. Conversely, given a sheaf $\mathcal{F}$ as 
in the statement of the lemma, we note that since the $g_i$ are finite 
morphisms the higher direct images vanish. [Proposition 55.2] Then, by applying 
Lemma 0.2 with the clear correspondence in notation, we may also conclude the 
same for $g$. The vanishing of the higher direct images tells us that $H^n(X, 
\mathcal{G}) = H^n(S, g_*\mathcal{G}) \neq 0$, giving us the desired result for 
$\mathcal{F} = g_*\mathcal{G}$.
\end{proof}
\begin{lemma}
...
\end{proof}
\begin{lemma}
Let K be a field with $G = Gal(K^{sep}/K)$ and let H be a maximal pro-$\ell$ 
subgroup with $L/K$ being the corresponding field extension. Then for any $n 
\geq 0$, $H^n(\Spec{K},\mathcal{F}) = 0$, for all $\ell$-torsion $\mathcal{F}$ 
if and only if $H^n(\Spec{L}, \underline{\mathbf{Z}/\ell\mathbf{Z}}) = 0$.
\end{lemma}
\begin{proof}
Note that since $\Spec{L} = \lim_{i\in I}{\Spec{L_i}}$ where each $L_i/K$ is of 
degree relatively prime to $\ell$, we may apply Lemma 0.3 to reduce the problem 
to showing that for any $n \geq 0$, $H^n(\Spec{L},\mathcal{F}) = 0$ for all 
$\ell$-torsion sheaves if and only if $H^n(\Spec{L}, 
\underline{\mathbf{Z}/\ell\mathbf{Z}}_{\Spec{L}}) = 0$. The forward direction 
is trivial. It remains to show the reverse direction. Note by the 
correspondence specified in Lemma 58.1, we may think of $\mathcal{F}$ as a 
continuous $G' = G/H$-module $M$. For any element $x \in M$, it must be fixed 
by open subgroup $U$ by continuity. Hence, the element has a finite orbit of 
order $G'/U$. Let $M'$ be the module spanned by the orbit of $x$, this is a 
finite dimensional vector space over $\mathbf{F}_\ell$ which, regarded as an 
e\'tale sheaf is finite locally constant. In particular, its trivialization as 
a Galois module is given by the finite seperable extension corresponding to 
$G'/U$ via Galois Theory. In particular, we have shown that $\mathcal{F}$ is 
the filtered colimit of its finite locally constant subsheafs. Applying Theorem 
52.1 to this colimit, we reduce to the case where $\mathcal{F}$ is a finite 
locally constant sheaf. In this case, the result is clear by Lemma 0.4. Since 
all non-trivial seperable field extensions of $L$ have degree which is 
divisible by $p$ by definition of $L$. The Lemma says that every such 
$\mathcal{F}$ has a finite filtration whose successive quotients are 
$\underline{\mathbf{Z}/\ell\mathbf{Z}}_{\Spec{L}}$. By applying the exact 
cohomology sequence to the filtration we may conclude via induction that if 
$H^n(\Spec{L}, \underline{\mathbf{Z}/\ell\mathbf{Z}}_{\Spec{L}}) = 0$ then 
$H^n(\Spec{L},\mathcal{F}) = 0$.
\\\\
(A more abstract way of giving the above d\'evissage argument, that makes clear 
the specification to fields at this point in the argument , but would require 
another movement of section is: every $\ell$-torsion sheaf is the filtered 
colimit of $\ell$-torsion constructible subsheafs but constructable is the same 
as finite locally constant in the case of fields, as fields contain no proper 
subschemes)
\end{proof}
\begin{corollary}
Let K be a field with $G = Gal(K^{sep}/K)$ and let H be a pro-$\ell$ subgroup 
with $L/K$ being the corresponding field extension. Then for any $n \geq 1$, 
$H^q(\Spec{K},\mathcal{F}) = 0$, $q \geq n$ for all $\ell$-torsion sheaves if 
and only if $H^n(\Spec{L}, \underline{\mathbf{Z}/\ell\mathbf{Z}_{\Spec{L}}}) = 
0$.
\end{corollary}
\begin{proof}
The forward direction is trivial, so we need only prove the reverse direction. 
We proceed by induction on $q$. The case of $q = n$ is Lemma 0.5. Now let 
$\mathcal{F}$ be a sheaf on $\Spec{K}$ as in the statement of the theorem. Let 
$f: \Spec{K^{sep}} \rightarrow \Spec{K}$ be the inclusion of a geometric point. 
...
\rightarrow 0\]
Note that since $K^sep$ may be written as the filtered colimit of finite 
separable extensions. f is the limit of a directed system of finite e\'tale 
morphisms. We may, as was seen in the proof of Lemma 0.3, conclude that $f$ has 
...
\begin{proof}
First set $p = \text{char}(K)$. By Lemma 50.58, Theorem 60.6, and Example 58.3, 
the Proposition is equivalent to showing that if 
$H^2(\Spec{K'},\mathbf{G}_m|_{\Spec(K')_\etale}) = 0$ for all finite extensions 
$K'/K$ then:
\begin{itemize}
...
\end{itemize}
We prove part two first. Since $\mathcal{F}$ is a torsion sheaf, we may use the 
$\ell$-primary decomposition as well as the compatibility of cohomology with 
colimits (i.e direct sums.) [Theorem 52.1] to reduce showing 
$H^q(\Spec{K},\mathcal{F}) = 0$, $q \geq 2$ for all torsion sheaves to showing 
the same is true for all $\ell$-torsion sheaves for every prime $\ell$. This 
allows us to analyze each prime individually.

Suppose that $\ell \neq p$. For any extension $K'/K$ we have the Kummer 
sequence [Lemma 28.1]:
\[0 \to
\mu_{\ell, \Spec{K'}} \to
\mathbf{G}_{m, \Spec{K'}} \xrightarrow{(\cdot)^{\ell}}
\mathbf{G}_{m, \Spec{K'}} \to 0\]
$H^q(\Spec{K'},\mathbf{G}_m|_{\Spec(K')_\etale}) = 0$ for $q = 2$ by assumption 
and for $q = 1$ by Theorem 24.1 combined with the fact that Zarisiki line 
bundles on a field are clearly trivial. Thus, by the long-exact cohomology 
sequence we may conclude that $H^2(\Spec{K'},\mu_{\ell, \Spec{K'}}) = 0$, for 
any separable $K'/K$. Now let $H$ be an $\ell$-Sylow Subgroup and $L$ be the 
corresponding extension. We can write $L$ as the colimit of finite extensions, 
applying Theorem 52.1 to this colimit we see that $H^2(\Spec{L},\mu_{\ell, 
\Spec{L}}) = 0$. Now $\mu_{\ell}$ must be the constant sheaf. If it weren't, 
that would imply there exists a Galois extension of degree relatively prime to 
$p$ of $L$ which is not true by definition of $L$. (i.e the extension by the 
$\ell$ roots of unity in $L$) Hence, via Corollary 0.6, we conclude the result 
for $\ell \neq p$. 

Now suppose that $\ell = p$. We consider the Artin-Schrier exact Sequence:
$$
...
Cohomology of $\mathbf{G}_{a, \Spec{K}}$ vanishes, by Remark 23.14 and the 
vanishing of the higher cohomology of the structure sheaf of an affine scheme. 
[Cohomology of Schemes, Lemma 2.2] Note this can be applied to any field of 
characteristic $p$. In particular, we can apply it to the field extension $L$ 
defined by a $p$-Sylow subgroup $H$. This allows us to conclude 
$H^n(\Spec{L},\underline{\mathbf{Z}/p\mathbf{Z}}_{\Spec{L}}) = 0$ for $n \geq 
2$, from which the result follows for $\ell = p$, by Corollary 0.6.
\\\\
Now for the first part of the proposition. We want to study the vanishing of 
the cohomology sheaves $H^q(\Spec{K},\mathbf{G}_m|_{\Spec(K)_\etale}) = 0$. 
Note that as observed above we have this for $q = 2$, as well as for any finite 
field extension of $K$. Then, the statement was observed to be true for $q = 
1$, also for any finite field extension of $K$. (The so called Hilbert 90 
Theorem.) Now we will make some reductions consider: 
$(\mathbf{G}_m|_{\Spec(K)_\etale})_{\text{tor}}$ the filtered colimit of all 
torsion subsheaves of $\mathbf{G}_m|_{\Spec(K)_\etale}$. Then denote the 
quotient sheaf by $\mathcal{G}$, we have the exact sequence:
\[1 \rightarrow (\mathbf{G}_m|_{\Spec(K)_\etale})_{\text{tor}} \rightarrow 
\mathbf{G}_m|_{\Spec(K)_\etale} \rightarrow \mathcal{G} \rightarrow 1\]
We may apply the results of part two of the proposition to 
$(\mathbf{G}_m|_{\Spec(K)_\etale})_{\text{tor}}$. This, in conjunction with the 
long exact cohomology sequence applied to the above, allows us to reduce to 
proving the higher sheaf cohomology of $\mathcal{G}$ vanishes given that the 
cohomology vanishes for $q = 1$ and $q = 2$. Now the Kummer sequence [Lemma 
28.1] tells us that multiplication by a prime $\ell$ is an isomorphism on 
$\mathcal{G}$ where $\ell$ is not the characteristic of $p$. (Since the kernel 
is torsion and the map is a surjection on $\mathbf{G}_m|_{\Spec(K)_\etale}$) 
This is the same as saying that the sheaf $\mathcal{G}$ is $\ell$ divisible for 
$\ell$ not the characteristic. Note that by Commutative Algebra Lemma 44.2, 
this will also be true for $\ell = p$ in the case of $K$ being a perfect field. 
It follows by the proof of Lemma 0.3, in particular the method d\'e la trace, 
that we can reduce to studying the vanishing of the of 
$H^n(\Spec{L},f^{-1}(\mathbf{G}_m|_{\Spec(K)_\etale})$ where $\Spec{L}$ is the 
galois extension corresponding to $G_p$, a maximal pro-$p$ quotient of 
$Gal(K^{sep}/K)$ and $f:\Spec{L}\rightarrow\Spec{K}$ is the morphism given by 
inclusion. Since $f:\Spec{L} \rightarrow \Spec{K}$ can be written as the 
filtered limit of morphisms $f_i: \Spec{K_i} \rightarrow \Spec{K}$ of finite 
separable field extensions of $K$ with degree relatively prime to the 
characteristic of the field. The m\'ethod de la trace tells us that the 
vanishing of the cohomology of the pullback sheaves 
$H^i(\Spec{K_i},f_i^{-1}\mathcal{G})$ implies the vanishing of the sheaf 
$\mathcal{G}$. Then we simply use a limit argument (which is outlined in the 
proof of Lemma 0.3.) to conclude the same for $f$. 


In the case of characteristic 0 or a perfect field, we may finish the 
proposition now, as here we may reduce to studying the cohomology of a sheaf on 
$\Spec{K^{sep}}$ which will clearly have trivial higher cohomology groups. 
Thus, it remains to show for a non-perfect field $K$ of $\text{char}{K} = p$ 
that the cohomology of the sheaf of $H^n(\Spec{L}, f^_{-1}(\mathcal{G})$ 
vanishes. Now it is easily checkable that for a morphism such as $f:\Spec{L} 
\rightarrow \Spec{K}$ that 
$f^{-1}(\mathbf{G}_m|_{\Spec(K)_\etale}) = \mathbf{G}_m|_{\Spec(L)_\etale}$ and 
of course this respects torsion. Hence, as we know the result of vanishing of 
this sheaf for $q = 1, q = 2$ for any separable extension from the first 
section. This will allow us to of course know the vanishing for 
$f^{-1}(\mathcal{G})$, which by abuse of notation, we shall also call 
$\mathcal{G}$ from this point.
\\
Now note that all of the extensions of $\Spec{L}$ to $\Spec{K}^{sep}$ are 
$p$-divisible by definition of $L$. Denote the morphisms $g_i: \Spec{K^{sep}} 
\rightarrow \Spec{L_i}$ where $K^sep$ is a finite 
\end{proof}
...
If you do not want to do the exercise yourself, then you can observe
that this follows by combining
Fundamental Groups, Lemmas
\ref{pione-lemma-finite-etale-connected-galois-category} and
\ref{pione-lemma-galois}.
\end{exercise}
