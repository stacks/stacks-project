...
\medskip\noindent
We first prove this when $Y$ is affine. In this case choose an
...
(j_{i', i, small}, j_{i', i, small}^\sharp)
\end{align*}
Hence by the uniqueness mentioned above we conclude that
...
Modules on Sites,
Lemma \ref{sites-modules-lemma-morphism-locally-ringed-topoi-localization}
...
Exactly as in the first part of the proof one shows that
these morphisms $f'$ (for varying affine opens $V \subset Y$)
glue on overlaps, so we get a morphism $f : X \to Y$.
Moreover, the $2$-isomorphisms $t$ automatically
are compatible on overlaps as before and we obtain
a global $2$-isomorphism
...
\noindent
In this section we describe the category of sheaves on the spectrum of
a field. As a consequence we obtain the equivalence of etale cohomology
with Galois cohomology in this case. Before we state the result we introduce
...
$$
\begin{matrix}
\textit{Sh}(\text{Spec}(K)_{etale}) &
\longrightarrow &
G\textit{-Sets} \\
\mathcal{F} &
\longmapsto &
\mathcal{F}_{\text{Spec}(K^{sep})}
\end{matrix}
$$
is an equivalence of categories.
\end{lemma}
...
\begin{proof}
The category $\text{Spec}(K)_{etale}$ has some extra structure
(maybe pushouts or something -- figure it out) which makes it
automatic that all sheaves are representable. In the language
of
Section \ref{section-G-sets}, we have
...
\ref{lemma-sheaves-equivalence-point}
...
the category of sheaves is definitively a proper class.
\end{remark}
...
\mathcal{F})
\\
& = & \text{Hom}_{\mathcal{C}^0(\mathcal{G}\textit{-Sets})}(\{*\}, M) \\
& = & M^{\mathcal{G}}
\end{eqnarray*}
...
Let $S$ be a scheme. A geometric point $\overline{s}$ of $S$ is called
{\it separable algebraic} if $\kappa(s) \subset \kappa(\overline{s})$ is
a separable algebraic closure of $\kappa(s)$.
\end{definition}
...
Let $S$ be a scheme.
Let $\overline{s}$ be a separable algebraic geometric point of $S$.
Set $G = \text{Gal}(\kappa(\overline{s})/\kappa(s))$.
Define an action of $G$ on $\mathcal{F}_{\overline{s}}$
as follows
$$
\begin{matrix}
\mathcal{G} & \times & \mathcal{F}_{\overline{s}} &
\longrightarrow &
\mathcal{F}_{\overline{s}} \\
...
$\mathcal{F}_{\overline{s}}$. We can thus restate the theorem of last time as follows.

\begin{theorem}
\label{theorem-equivalence-sheaves-point}
The action of $\mathcal{G}$ on the stalks $\mathcal{F}_{\overline{s}}$ is continuous
(for the discrete topology on $\mathcal{F}_{\overline{s}}$. Moreover, if $\overline{s}=
...
$H_{et}^0(S,\mathcal{F}) = (\mathcal{F}_{\overline{s}})^\mathcal{G}$, and more
