...
$\phi(n)$ connected finite \'etale coverings witha given free
...
sections functor: if $X \in \text{Ob}(\mathcal{C})$ and
$\mathcal{F} \in \textit{Ab}(\mathcal{C})$,
$$
H^p(X, \mathcal{F}) :=
R^p\Gamma(X, \mathcal{F}) =
H^p(\Gamma(X, \mathcal{I}^\bullet))
$$
...
For more general discussion of cohomology on sites, see
...
$\mathcal{U} = \{U_i \to U\}_{i \in I}$ a covering of $\mathcal{C}$,
and $\mathcal{F} \in \textit{PAb}(\mathcal{C})$ an abelian presheaf. We define
the {\it \u Cech complex}
...
$\textit{PAb}(\mathcal{C})$ and $\mathcal{U}$ is a family of morphisms with fixed
...
\section{The big and small sites}
...
you can simply assume $\tau = etale$ throughout.
...
and to use the preceding results on descent on modules we single out a special class
of $\tau$-coverings of affine schemes called standard coverings.
...
The details are in the chapter on topologies on schemes; there is a lot of freedom
in the choices made, but in the end the actual choices made will not affect the
etale (or other) cohomology of $S$ (just as in \cite{SGA4} the actual choice of
universe doesn't matter at the end). Moreover, the way the material is written
the reader who is happy using strongly inaccessible cardinals (i.e., universes)
can do so as a substitute.
...
\begin{enumerate}
\item A {\it big $\tau$-site of $S$} is any of the sites $(\textit{Sch}/S)_\tau$
constructed as explained above and in more detail in
...
are schemes $T$ over $S$ whose structure morphism $T \to S$ is an open immersion,
resp.\ etale. A covering in $S_\tau$ is a covering
...
``closure'' properties, i.e., given a scheme $T$ in it any locally closed subscheme
of $T$ is isomorphic to an object of $(\textit{Sch}/S)_\tau$.
...
$\text{Spec}(R)$ one can complete, localize, or take the quotient of $R$ by an ideal
while staying inside the category, etc.
...
because the schemes $T_i$ are not objects of the category $(\textit{Sch}/S)_\tau$.
But our choice of the sites $(\textit{Sch}/S)_\tau$ is such that there always does exist
...
Let $U$ be a scheme and $\mathcal{U} = \{U_i \to U\}_{i \in I}$ an fpqc
covering of $U$. Let $V = \coprod_{i \in I} U_i$. Then
\begin{enumerate}
\item $\mathcal{V} = \{V \to U\}$ is an fpqc covering, and
\item the \u Cech complexes
$\check{\mathcal{C}}^\bullet (\mathcal{U}, \mathcal{F})$ and
$\check{\mathcal{C}}^\bullet (\mathcal{V}, \mathcal{F})$ agree whenever
$\mathcal{F}$ is a functor $(\textit{Sch}/S)^{opp} \to \textit{Ab}$
which satisfies the sheaf condition for the fpqc topology.
\end{enumerate}
\end{lemma}

\begin{proof}
The proof is straightforward by unwinding the definitions.
Namely, first note that $\mathcal{U}$ is a refinement of $\mathcal{V}$
...
Next, we observe that if $\{T_j\}_{j \in J}$ is a family of schemes,
then the family of morphisms with fixed target
$\{T_i \to \coprod_{j \in J} T_j\}_{i \in J}$ is an fpqc covering, and so
\begin{equation}
...
site, since coproducts may not lead to coverings. But it does for all the
usual ones (at least all the ones we will study).
...
Let $\mathcal{C}$ be a site as in Theorem \ref{theorem-quasi-coherent}.
Then
$$
H_{Zar}^p(S, \mathcal{F}) = H^p(S, \mathcal{F}^a)
$$
...
\begin{proof}
The result is true for $p = 0$ by the sheaf property. We first prove the result
...
$S$. Then we want to prove that $H^p(S, \mathcal{F}^a) = 0$ for all $p > 0$.
...
\medskip\noindent
Assume that $S$ is separated. Choose an affine open covering
$S = \bigcup_{i \in I} \mathcal{U}_i$ of $S$. The family
$\mathcal{U} = \{ \mathcal{U}_i \to S\}_{i \in I}$ is then an fpqc covering,
and all the intersections
$\mathcal{U}_{i_0} \times_S \cdots \times_S \mathcal{U}_{i_p}$ are affine
since $S$ is separated. So all rows of the spectral sequence
...
\\
To show that the functor $\textit{Ab}(S_{et}) \to \textit{Ab}$ is exact, consider
any short exact sequence in $\textit{Ab}(S_{et})$:
$
...
functor $\text{Mod}_{\Lambda} \to \textit{Ab}$, $M \mapsto N\otimes_{\Lambda} M$.
