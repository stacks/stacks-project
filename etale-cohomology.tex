...
amounts to some Galois cohomology. We now review the relevant
facts about Brauer groups, most of which are discussed in the chapter
...
We will see that $[A]$ has order $\deg(A)$ for any finite central simple
...
$H_{cont}^1(\text{Gal}(K^{sep}/K), \text{PGL}_d(K^{sep}))$.
\end{lemma}
...
\noindent
If $A$ is a finite central simple algebra of degree $d$ over a field $K$, we
denote $\xi_A$ the corresponding cohomology class in
$H_{cont}^1(\text{Gal}(K^{sep}/K), \text{PGL}_d(K^{sep}))$.
...
H_{cont} ^1(\text{Gal}(K^{sep}/K), \text{PGL}_d(K^{sep}))
\longrightarrow
H^2_{cont}(\text{Gal}(K^{sep}/K), (K^{sep})^*).
$$
...
$\xi' \in H_{cont}^2(\text{Gal}(K'|K), (K')^*)$. Then write
...
$\text{Br}(K') = 0$. Then
\begin{enumerate}
\item $H^q_{cont}(\text{Gal}(K^{sep}/K), (K^{sep})^*) = 0$
for all $q \geq 1$, and
\item $H_{cont}^q(\text{Gal}(K^{sep}/K), M) = 0$
