...
on $Y_{\acute{e}tale}$, the same is true for $f^{-1}\mathcal{F}$
on $X_{\acute{e}tale}$.
\end{lemma}
...
Noetherian induction and the following amusing elementary lemma.
...
abelian sheaves. The functor $j_!$ is characterized by the
...
Let $j : U \to X$ be an \'etale morphism of schemes. If
$\overline{x} : \Spec(k) \to X$ is a geometric point of $X$, then 
$$
...
on sheaves of sets, abelian sheaves, and sheaves of $\Lambda$-modules.
\end{lemma}
