...
decomposition of $X$ into locally closed subschemes
$X=\coprod_i X_i$ such that $\mathcal{F}|_{X_i}$ is
...
Let $j: \mathcal{U} \to X$ be an \'etale morphism of schemes.
The restriction functor $j^{-1}$ has a left adjoint which is denoted
$j_! :
\textit{Ab}(\mathcal{U}_{\acute{e}tale})
\to
\textit{Ab}(X_{\acute{e}tale})$
and called {\it extension by zero}. It is characterized by the
...
& (\mathcal{V}\to X) &
\longmapsto &
\bigoplus\nolimits_{\mathcal{V}\xrightarrow{\varphi}
\mathcal{U}\text{ over }X}
\mathcal{F}(\mathcal{V}\xrightarrow{\varphi}\mathcal{U}),
\end{matrix}
$$
then $j_!\mathcal{F}$ is the sheafification of $j_!^{\textit{PSh}}\mathcal{F}$.
...
$H_{et}^q(S, \underline{\mathbf{Z}/p\mathbf{Z}}) = 0$ for all
$q \geq 2$. It also follows that if $S$ is a Noetherian scheme of
characteristic $p$ and dimension $d$, then
$H_{et}^q(S, \underline{\mathbf{Z}/p\mathbf{Z}}) = 0$ for all $q \geq 2 + d$
...
$H_{et}^q(X, \underline{\mathbf{Z}/p\mathbf{Z}}) = 0$ for $q >= dim(X) + 1$.
Sketch of proof: Let $d = \dim(X)$. It suffices to show that
$H_{et}^{d + 1}(X, \underline{\mathbf{Z}/p\mathbf{Z}}) = 0$.
...
\begin{theorem}[$\mathbf{Q}_\ell$-sheaves]
\label{theorem-B}
...
\begin{theorem}[Grothendieck, see \cite{SGA1}]
