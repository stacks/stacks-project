...
with affine transition morphisms $f_{i'i} : X_{i'} \to X_i$. With
...
Lemma \ref{limits-lemma-descend-etale}
...
$$
where the transition maps are $f_{i'}^{-1}\varphi_{i'i}$ for some
canonical maps
$\varphi_{i'i} : f_{i'i}^{-1}f_{i, *}\mathcal{F} \to f_{i', *}\mathcal{F}$
satisfying a cocycle condition as in Theorem \ref{theorem-colimit}.
\end{lemma}
...
\begin{proof}
Formal combination of Lemmas \ref{lemma-relative-colimit}
...
Let $U \to X$ be an \'etale morphism such that $U \to S$ factors as
$U \to V \to S$ with $V \to S$ \'etale and $U \to V$ flat of finite
presentation with geometrically connected fibres. We claim that
\begin{align*}
f^{-1}g_*\mathcal{F}(U)
& = g_*\mathcal{F}(V) \\
...
quasi-affine. Thus we can use the induction principle of
...
type over $\mathbf{Z}[1/n]$, we have $X = \mathbf{A}^1_S$, and
...
\medskip\noindent
Assume $\dim(T) = n \geq 0$ and we know the result for all situations where $T$
...
has geometrically connected fibres. Of course, we may then replace
$S$ by $V$ and $T$ by $V \times_S T$ (exactly as in the discussion in the first
paragraph of the proof). Observe that $\dim(V \times_S T) \leq \dim(T)$
...
We will use below that $\dim(T') \leq \dim(T) = n$, see
...
(Cohomology of Sites, Lemma \ref{sites-cohomology-lemma-Leray})
we have
$$
H^q(Y', (e')^{-1}\mathcal{I}') =
...
split over $\sigma(s)$
such that $\tilde \xi$ maps to zero in
$H^q(Z_t, e^{-1}\mathcal{I}|_{Z_K})$.
...
\medskip\noindent
At this point we replace $X$ by $U$. After this replacement it may
...
as a scheme is the limit of the nonempty affine open subschemes of $T$.
...
Y \ar[d]_e &
Y \times_T V \ar[l]_{j'} \ar[d]_{e_V} \\
Z \ar[r]^i &
Y &
V \ar[l]_j
}
...
Since $\dim(Z) < n$ by induction the image of $\tilde \xi$
