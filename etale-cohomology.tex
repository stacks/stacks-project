...
\begin{enumerate}
\item For any $\mathcal{F} \in \textit{Ab}((\Sch/T)_\etale)$
we have
$$
(Rf_{big, *}\mathcal{F})|_{S_\etale} = Rf_{small, *}(\mathcal{F}|_{T_\etale})
$$
in $D(S_\etale)$.
\item For any object $\mathcal{F}$ of
$\textit{Mod}((\Sch/T)_\etale, \mathcal{O})$
we have
$$
(Rf_{big, *}\mathcal{F})|_{S_\etale} = Rf_{small, *}(\mathcal{F}|_{T_\etale})
$$
...
$$
Observe that $\epsilon_{S, *} \circ a_S^{-1} = \pi_S^{-1}$
by Lemma \ref{lemma-describe-pullback-pi-fppf}.
...
$$
\xymatrix{
\Sh((\Sch/X)_{fppf}) \ar[r]_{j_{fppf}} \ar[d]_{\epsilon_X} &
\Sh((\Sch/Y)_{fppf}) \ar[d]^{\epsilon_Y} \\
\Sh((\Sch/X)_\etale) \ar[r]^j &
\Sh((\Sch/Y)_\etale)
}
$$
and
$$
\xymatrix{
\Sh((\Sch/X)_{fppf}) \ar[r]_{j_{fppf}} \ar[d]_{a_X} &
