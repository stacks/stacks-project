...
sheaves are. An {\it \'etale sheaf} $\mathcal{F}$ on $X$ of sets (respectively 
...
$\mathcal{F}(\mathcal{U})$ are called {\it sections} of $\mathcal{F}$ on 
...
\varphi^* (s) = s |_\mathcal{V} $ and called {\it restriction mapping}. This 
...
\ref{theorem-descent-quasi-coherent}, hence we can descend line bundles.
\end{proof}
...
\begin{definition}
An object $K\in D^-(\Lambda)$ is said to have {\it finite 
...
$R\Gamma(X_{\bar k}, K|_{X_{\bar k}})$. The {\it global Lefschetz number} of 
$K$ is the trace $
...
\mathbf{Q}_\ell(-1)$ is the {\it Tate twist} of $\mathbf{Q}_\ell$, {\it i.e.} 
