...
is the projection. More generally we have
$$
\left(R^nf_*K\right)_{\overline{s}} =
H_\etale^n( X \times_S \Spec(\mathcal{O}_{S, s}^{sh}), p^{-1}K)
$$
for $K \in D^+(X_\etale)$.
\end{theorem}
...
For any sheaf $\mathcal{F}$ on $S_\etale$ we will construct a canonical map
$$
...
$$
This is less elementary but perhaps easier to grok.
\end{remark}
...
$\mathcal{O}_{X, \overline{x}}$ is strictly henselian
we have
