...
that
$$
B = \text{colim}_{(U, x)} \mathcal{O}(U) =
\text{colim}_{(\text{Spec}(C), x)} C
$$
...
$R^h = \text{colim}_i\ R_i$ as a directed colimit of localizations
...
$\mathcal{F}$ an abelian sheaf on $X_{\acute{e}tale}$. Denote $X_i = 
X\times_{\text{Spec}
...
It now suffices to observe that $\text{Br}(K') = \text{colim} \text{Br}(K'')$.
\end{proof}
...
\lim\nolimits_n H^*(X, \mathbf{Z}/\ell^n\mathbf{Z})
$$
...
$$
M = \lim\nolimits_n M_n = \lim \mathcal{F}_{n, \bar x}
$$
...
$$
H^i(X, \mathcal{F}) := \lim\nolimits_n H^i(X, \mathcal{F}_n)
\qquad\text{ and }\qquad
H_c^i(X, \mathcal{F}) := \lim\nolimits_n H_c^i(X, \mathcal{F}_n).
$$
...
So
$$
A(q)\leq \lim\nolimits_{n\to\infty}(\cdots) = \left(\sum_{j=1}^\infty
