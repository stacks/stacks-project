...
Let $x \in X$ be a point with $\text{trdeg}_K(\kappa(x)) = a$.
Then there is a map
$$
K(t_1, \ldots, t_a)^{sep} \longrightarrow \mathcal{O}_{X, x}^{sh}
$$
such that the residue field of the target is a purely inseparable
extension of the source.
\end{lemma}

\begin{proof}
We may assume $X$ is affine. Then we can choose a morphism
$X \to \mathbf{A}^a_K$ such that $x$ maps to the generic point
$\eta \in \mathbf{A}^a_K$.
See for example Algebra, Lemma
\ref{algebra-lemma-refined-quasi-finite-over-polynomial-algebra}
for a more general and stronger result.
The induced map
$$
K(t_1, \ldots, t_a)^{sep} =
...
$$
on \'etale local rings has all the desired properties.
Some details omitted.
\end{proof}
...
\noindent
Short section discussing the K\"unneth formula.
...
\medskip\noindent
Property (\ref{sites-cohomology-item-base-change-P}) holds
...
\medskip\noindent
Property (\ref{sites-cohomology-item-restriction-A}) follows from the
commutative diagram (3) in
Topologies, Lemma \ref{topologies-lemma-morphism-big-small-etale}.

\medskip\noindent
Property (\ref{sites-cohomology-item-A-sheaf}) is
Lemma \ref{lemma-describe-pullback-pi-fppf}.

\medskip\noindent
Property (\ref{sites-cohomology-item-A-and-P}) holds by
Lemma \ref{lemma-compare-higher-direct-image-proper} part (3).

\medskip\noindent
Property (\ref{sites-cohomology-item-refine-tau-by-P})
is implied by
More on Morphisms, Lemma
\ref{more-morphisms-lemma-dominate-fppf-etale-locally}.
\end{proof}
