...
Hence we can let $D_{\textit{QCoh}}(\mathcal{O}_\etale)$ be the triangulated
...
also $\epsilon^* : D_{\textit{QCoh}}(\mathcal{O}_X) \to
D_{\textit{QCoh}}(\mathcal{O}_\etale)$.

\begin{lemma}
\label{lemma-derived-quasi-coherent-small-etale-site}
Let $X$ be a scheme. The functor
$\epsilon^* : D_{\textit{QCoh}}(\mathcal{O}_X) \to
D_{\textit{QCoh}}(\mathcal{O}_\etale)$
...
shows that the category $D_{\textit{QCoh}}(\mathcal{O}_S)$ can be defined
...
\item denoting
$$
\Phi : D(\textit{QCoh}(\mathcal{O}_X)) \to D(\textit{QCoh}(\mathcal{O}_Y))
$$
...
$$
\xymatrix{
D(\textit{QCoh}(\mathcal{O}_X)) \ar[d]_\Phi \ar[r] &
D_{\textit{QCoh}}(\mathcal{O}_X) \ar[d]^{Rf_*} \\
D(\textit{QCoh}(\mathcal{O}_Y)) \ar[r] &
D_{\textit{QCoh}}(\mathcal{O}_Y)
}
...
Recall that $\textit{QCoh}(\mathcal{O}_X)$ is a Grothendieck abelian category
(Properties of Spaces, Proposition
\ref{spaces-properties-proposition-coherator}).
Hence any $K$ in $D(\textit{QCoh}(\mathcal{O}_X))$
...
\medskip\noindent
Let $A$ be an object of $D(\textit{QCoh}(\mathcal{O}_Y))$ and
...
& =
\Hom_{D(\textit{QCoh}(\mathcal{O}_Y))}(A, \Phi(RQ_X(E)))
\end{align*}
...
\begin{proof}
We first use the induction principle to prove $i_X$ is fully faithfu.
...
$i_U : D(\textit{QCoh}(\mathcal{O}_U)) \to D_{\textit{QCoh}}(\mathcal{O}_U)$
is fully faithful''.
...
\medskip\noindent
Suppose that $A, B$ are objects of $D(\textit{QCoh}(\mathcal{O}_X))$.
...
& =
\Mor_{D(\textit{QCoh}(\mathcal{O}_X))}(A, j_{V, *}(B|_V)) \\
& =
...
Set $B'' = j_{U, *}\mathcal{C}^\bullet$ in $D(\textit{QCoh}(\mathcal{O}_X))$
and choose a distinguished triangle
$$
B \to B'' \to B' \to B^\bullet[1]
$$
in $D(\textit{QCoh}(\mathcal{O}_X))$. Since the first arrow of the triangle
...
morphism in $D(\textit{QCoh}(\mathcal{O}_U))$ by assumption.
...
an element of $\Hom_{D(\textit{QCoh}(\mathcal{O}_X))}(A, B'')$.
...
in $D(\textit{QCoh}(\mathcal{O}_U))$. Thus we can choose $s$
such that
...
$\Hom_{D(\textit{QCoh}(\mathcal{O}_X))}(A, B''[-1])$
...
$$
\Phi : D(\textit{QCoh}(\mathcal{O}_U)) \to D(\textit{QCoh}(\mathcal{O}_X))
$$
of the left exact functor
$j_* : \textit{QCoh}(\mathcal{O}_V) \to \textit{QCoh}(\mathcal{O}_X)$
fits into a commutative diagram
$$
\xymatrix{
D(\textit{QCoh}(\mathcal{O}_V)) \ar[d]_\Phi \ar[r]_{i_V} &
D_{\textit{QCoh}}(\mathcal{O}_V) \ar[d]^{Rj_*} \\
D(\textit{QCoh}(\mathcal{O}_X)) \ar[r]^{i_X} &
D_{\textit{QCoh}}(\mathcal{O}_X)
}
$$
...
$\textit{QCoh}(\mathcal{O}_S)$ is a filtered colimit $\colim_i \mathcal{F}_i$
...
$\textit{QCoh}(\mathcal{O}_S)$. Since $\textit{QCoh}(\mathcal{O}_S)$ is
...
over $S$. Every injective object of $\textit{QCoh}(\mathcal{O}_X)$ is
...
$\mathcal{J}'$ of $\textit{QCoh}(\mathcal{O}_U)$ such that there
exists an injection $\mathcal{J}|_U \to \mathcal{J}'$. Then
$$
\mathcal{J} \to j_*\mathcal{J}'
$$
is an injective morphism in $\textit{QCoh}(\mathcal{O}_X)$,
...
$\mathcal{J}$ is an injective object of $\textit{QCoh}(\mathcal{O}_X)$,
then $f_*\mathcal{J}$ is an injective object of
$\textit{QCoh}(\mathcal{O}_Y)$.
\end{lemma}
...
Morphisms of Spaces, Lemma \ref{spaces-morphisms-lemma-pushforward}.
Next, Let $K$ be an object of $D(\textit{QCoh}(\mathcal{O}_X))$.
Since $\textit{QCoh}(\mathcal{O}_X)$ is a Grothendieck abelian category
(Properties of Spaces, Proposition
...
$D(\textit{QCoh}(\mathcal{O}_Y))$. To finish the proof of the lemma
...
\begin{enumerate}
\item $E_0$ is perfect and $K_0 \in D_{\textit{QCoh}}(\mathcal{O}_{X_0})$, or
\item $E_0$ is pseudo-coherent and
$K_0 \in D_{\textit{QCoh}}(\mathcal{O}_{X_0})$ has finite tor dimension.
\end{enumerate}
\end{lemma}
