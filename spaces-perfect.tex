...
$$
\text{Ext}^i_{\mathcal{O}_B}(L, \mathcal{F}) \longrightarrow
\text{Ext}^i_{\mathcal{O}_X}(E,
\mathcal{G}^\bullet \otimes_{\mathcal{O}_X} f^*\mathcal{F})
$$
...
the maps
$$
\text{Ext}^i_{\mathcal{O}_Y}(L_{n + 1}, \mathcal{F})
\longrightarrow
\text{Ext}^i_{\mathcal{O}_Y}(L_n, \mathcal{F})
$$
...
$\text{Ext}^i_{\mathcal{O}_Y}(L, \mathcal{F}) =
\text{Ext}^i_{\mathcal{O}_Y}(L_n, \mathcal{F})$
...
Then $L\varphi^*E$ is a perfect complex on the scheme $U$ (use
Lemma \ref{lemma-descend-perfect}) and we can apply the result for schemes, see
...
to $U$ are locally constructible.
\end{proof}
