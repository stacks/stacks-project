...
\noindent
In this section we consider the special case of the situation
discussed in Section \ref{section-modules-cohomology} where
$\mathcal{C} = \mathbf{N}$ viewed as a category with a unique morphism
$i \to j$ if and only if $i \leq j$. We will also assume we are
given a ring $R$ and we always take $\mathcal{O}$ to be the constant
(pre)sheaf of rings with value $R$. A sheaf $\mathcal{A}$ of
differential graded $R$-algebras
on $\mathbf{N}$ is the same thing as an inverse system
$(A_n)$ of differential graded $R$-algebras.
A sheaf $\mathcal{M}$ of differential graded $\mathcal{A}$-modules
is the same thing as an inverse system $(M_n)$ of differential
graded modules, where $M_n$ is a module over $A_n$ to be sure.

\begin{lemma}
\label{lemma-pro-isomorphic}
In the situation above, suppose that $\mathcal{A} = (A_n)$ and
$\mathcal{B} = (B_n)$ are pro-isomorphic inverse systems of
differential graded $R$-algebras. Then there is a canonical
$R$-linear equivalence of $\mathit{QC}(\mathcal{A}, \text{d})$ with
$\mathit{QC}(\mathcal{B}, \text{d})$.
\end{lemma}

\begin{proof}
Given a morphism $\varphi : (A_n) \to (B_n)$ we will
construct a functor from $\mathit{QC}(\mathcal{A}, \text{d})$
to $\mathit{QC}(\mathcal{B}, \text{d})$.
The morphism of pro-systems is given by a sequence
...
$$
This is clearly functorial and produces the desired functor.
Etc, etc, etc.
\end{proof}
...
\begin{enumerate}
\item Let $\mathcal{A}$ be the sheaf of rings on $\mathbf{N}$
corresponding to the inverse system of rings $A_n = R/I^n$.
...
Koszul algebras. The category $\mathit{QC}(\mathcal{B})$.
\item the full subcategory $D_{comp}(R, I) \subset D(R)$
of derived complete objects (More on Algebra, Definition
\ref{more-algebra-definition-derived-complete}).
\end{enumerate}
\end{proposition}
