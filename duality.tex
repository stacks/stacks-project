...
Let $(K_n)$ be an inverse system as above and denote
...
\colim \Hom_X(K_n, N) \ar[d] \\
\Hom_U(K, L|_U) \ar[r] &
\Hom_U(K, M|_U) \ar[r] &
\Hom_U(K, N|_U)
}
$$
whose rows (which are long exact; not just 3 terms)
are exact by Derived Categories, Lemma
\ref{derived-lemma-representable-homological} and
Algebra, Lemma \ref{algebra-lemma-directed-colimit-exact}.
Hence if the statement of the lemma holds for $L$ and $N$,
then it holds for $M$. Thus, using the distinguished triangles
...
\medskip\noindent
Choose $\mathcal{F}^\bullet$ and $\mathcal{I} \subset \mathcal{O}_X$
such that $K_n = \mathcal{I}^n\mathcal{F}^\bullet$ as above.
...
\begin{enumerate}
\item Let $(K_n)$ and $(L_n)$ be inverse systems of
$D^b_{\textit{Coh}}(\mathcal{O}_X)$ as above.
...
\item Given $K \in D^b_{\textit{Coh}}(\mathcal{O}_U)$ there exists an
inverse system $(K_n)$ as above such that $(K_n|_U)$ is constant
with value $K$.
...
are inverse systems as above and such that the restriction of these
distinguished triangles to $U$ is isomorphic to the distinguished triangle
we started out with.
\end{lemma}

\begin{proof}
Let $(K_n)$ be as in Lemma \ref{lemma-extension-by-zero-triangle} part (2).
...
Let $j : U \to X$ be an open immersion of Noetherian schemes.
The constructions above determine a functor
$$
Rj_! :
...
for all $n$. It seems likely that the inverse systems $(K_n)$ as above
are well defined up to linear-pro-isomorphisms. If we ever need this
