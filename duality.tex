...
$$
The arrows labeled $\alpha$, resp.\ $\beta$ are the maps from
Derived Categories of Schemes, Lemma
\ref{perfect-lemma-affine-morphism-and-hom-out-of-perfect}
for the diagram with corners $X', X, Y', Y$, resp.\ $Y', Y, Y', Y$.
...
used in the third paragraph of the proof.
The map $\mu$ comes from the multiplication map
$(M, g'_*a'(g^*K))_X \otimes A' \to (M, g'_*a'(g^*K))_X$
and the identification $(M, g'_*a'(g^*K))_X = ((g')^*M, a'(g^*K))_{X'}$
and the triangle is commutative, see
...
$g'_* \circ a' \to a \circ g_*$.
Thus all of these are canonically isomorphic and we have two
maps $(f_*M, K)_Y \otimes A' \to (g^*f_*M, g^*K)_{Y'}$
into the common value. The first is the going from the
upper right hand corner by the sequence of arrows
d, l, d, $\mu$, r, r and the second is $\beta$.
However, both arrows define the same map
$(f_*M, K)_Y \to (g^*f_*M, g^*K)_{Y'}$
in $D(A)$ hence define the same arrow
$(f_*M, K)_Y \otimes A' \to (g^*f_*M, g^*K)_{Y'}$
in $D(A')$.
\end{proof}
