...
Let $\mathcal{C}$ be a site with fibre products.
Let $X$ be an object of $\mathcal{C}$.
Let us return to our sheaf of abelian groups
$\mathcal{F}$ on the site $\mathcal{C}$ of the introduction.
...
Note that if $a, b : K \to L$ are homotopic maps,
...
\textit{Ab}
$$
see Categories, Section \ref{categories-section-limits} for notation.
By Lemma \ref{lemma-product-hypercoverings} and
Lemma \ref{lemma-homotopy}, and the remarks on homotopies above,
...
\begin{proof}
We first prove this for $i = 0$. Let $K$ be a hypercovering of $X$.
Then
$$
...
$$
with obvious map. Thus the sheaf property of $\mathcal{F}$
implies that $\check{H}^0(K, \mathcal{F}) = H^0(X, \mathcal{F})$.
This proves the result for $i = 0$.

\medskip\noindent
Observe that for any object $Z = \{U_i \to X\}$ of
$\text{SR}(\mathcal{C}, X)$ and any abelian sheaf we have
\begin{eqnarray*}
\mathcal{F}(Z)
& = &
\prod \mathcal{F}(U_i) \\
...
\medskip\noindent
The ``correct'' way to finish the proof is to use
spectral sequences. We will add this later. FIXME.

\medskip\noindent
Finally, we prove the theorem by induction on $i$.
We have seen the result for $i = 0$.
