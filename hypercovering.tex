...
\noindent
This definition is essentially eqyuivalent to
...
simplicial object of $\textit{SR}(\mathcal{C})$ as follows.
...
$\textit{SR}(\mathcal{C})$ implies that $(I_n, \alpha(\varphi))$
is a simplicial set
...
of $\textit{SR}(\mathcal{C})$. The following are equivalent
\begin{enumerate}
...
of $\textit{SR}(\mathcal{C})$ is given by $\{U_l\}_{l \in I \amalg J}$
...
object $K' = \text{sk}_{n + 1}(\text{cosk}_n K)$ of $\textit{SR}(\mathcal{C})$.
Write
$$
...
object $K$ of $\textit{SR}(\mathcal{C})$
such that for $i \in I_a$, $0 \leq a \leq n$
we have $U_{a, i} \in \mathcal{B}$ and such that
\begin{enumerate}
...
$$
E = \{(U, V, W, a, b) \mid U, V, W \in \mathcal{B}, a : U \to V, b : U \to W\}
$$
Then we set $I_1 = I_0 \amalg E$. For $i \in I_1$ we set
$U_{1, i} = U_{0, i}$ if $i \in I_0$ and $U_{1, i} = U$
if $i = (U, V, W, a, b) \in E$. The map
...
identity on $I_0 \subset I_1$ and map $(U, V, W, a, b) \in E \subset I_1$
to $V$, resp.\ $W$. The corresponding morphisms
$f_{\delta^1_0, i}, f_{\delta^1_1, i} : U_{1, i} \to U_{0, i}$ are
the identity if $i \in I_0$ and $a, b$ in case $i = (U, V, W, a, b) \in E$.
...
If a simplicial object $K$ of $\textit{SR}(\mathcal{D})$
is a hypercovering, then $u(K)$ is a hypercovering.
\end{lemma}

\begin{proof}
If we write $K_n = \{U_{n, i}\}_{i \in I_n}$ as in the introduction
to this section, then $u(K)$ is the object of $\textit{SR}(\mathcal{C})$
...
$K \in \textit{SR}(\mathcal{D}, Y)$ a hypercovering of $Y$.
...
Let $\mathcal{C}$ be a site. Let
$K$ and $L$ be objects of $\textit{SR}(\mathcal{C})$.
Write $K = \{U_i\}_{i \in I}$ and $L = \{V_j\}_{j \in J}$.
Assume $U = \coprod_{i \in I} U_i$ and $V = \coprod_{j \in J} V_j$
exist. Then we get
$$
\Mor_{\textit{SR}(\mathcal{C})}(K, L) \longrightarrow \Mor_\mathcal{C}(U, V)
$$
...
\end{enumerate}
Then we get another simplicial object $L$ of $\textit{SR}(\mathcal{C})$
with $L_n = \{U_n\}$, see
...
$(\beta, g_k) : \{W_k\}_{k \in K} \to \{V_j\}_{j \in J}$
in $\textit{SR}(\mathcal{C})$ such that
...
all finite limits hence $\textit{SR}(\mathcal{C}/X)$
...
the limit of the diagram in $\textit{SR}(\mathcal{C}/X)$
