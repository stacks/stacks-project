...
is the category $(\Delta/[m])^{opp}_{\leq n}$. This is
a connected category. Hence,
Categories, Lemma \ref{categories-lemma-connected-limit-over-X}
shows that $F'(\text{cosk}_n \text{sk}_n K) =
F(\text{cosk}_n \text{sk}_n K)$ as a presheaf of sets. Whence the claim.
...
the morphism $\mathcal{F} \to \mathcal{G}$. Note that
for every $U \in \text{Ob}(\mathcal{C})$, if we set
