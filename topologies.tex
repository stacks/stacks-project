...
$S_{affine, Zar}$, is the full subcategory of $(\Sch/S)_{Zar}$
whose objects are those $U/S$ such that $U$ is an affine scheme and
such that $U \to S$ is an open immersion. A covering of $S_{affine, Zar}$
is any covering $\{U_i \to U\}$ of $S_{Zar}$ with
$U \in \Ob(S_{affine, Zar})$ which is a standard Zariski covering.
\end{enumerate}
\end{definition}
