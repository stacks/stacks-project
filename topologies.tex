...
A {\it big \'etale site} is any site $\textit{Sch}_{\acute{e}tale}$ as in
Sites, Definition \ref{sites-definition-site} constructed as follows:
\begin{enumerate}
...
$\textit{Sch}_{\acute{e}tale}$ is in some sense induced from the \'etale topology
on the category of all schemes.

\begin{lemma}
\label{lemma-etale-induced}
Let $\textit{Sch}_{\acute{e}tale}$ be a big \'etale site as in
Definition \ref{definition-big-etale-site}.
Let $T \in \text{Ob}(\textit{Sch}_{\acute{e}tale})$.
Let $\{T_i \to T\}_{i \in I}$ be an arbitrary \'etale covering of $T$.
\begin{enumerate}
\item There exists a covering $\{U_j \to T\}_{j \in J}$ of $T$ in the site
$\textit{Sch}_{\acute{e}tale}$ which refines $\{T_i \to T\}_{i \in I}$.
\item If $\{T_i \to T\}_{i \in I}$ is a standard \'etale covering, then
it is tautologically equivalent to a covering in $\textit{Sch}_{\acute{e}tale}$.
...
schemes $T_i$ is isomorphic to an object of $\textit{Sch}_{\acute{e}tale}$ by
Sets, Lemma \ref{sets-lemma-what-is-in-it},
...
Let $S$ be a scheme. Let $\textit{Sch}_{\acute{e}tale}$ be a big \'etale
site containing $S$.
\begin{enumerate}
\item The {\it big \'etale site of $S$}, denoted
$(\textit{Sch}/S)_{\acute{e}tale}$, is the site $\textit{Sch}_{\acute{e}tale}/S$
introduced in Sites, Section \ref{sites-section-localize}.
\item The {\it small \'etale site of $S$}, which we denote
$S_{\acute{e}tale}$, is the full subcategory of $(\textit{Sch}/S)_{\acute{e}tale}$
whose objects are those $U/S$ such that $U \to S$ is \'etale.
A covering of $S_{\acute{e}tale}$ is any covering $\{U_i \to U\}$ of
$(\textit{Sch}/S)_{\acute{e}tale}$ with $U \in \text{Ob}(S_{\acute{e}tale})$.
\item The {\it big affine \'etale site of $S$}, denoted
$(\textit{Aff}/S)_{\acute{e}tale}$, is the full subcategory of
$(\textit{Sch}/S)_{\acute{e}tale}$ whose objects are affine $U/S$.
A covering of $(\textit{Aff}/S)_{\acute{e}tale}$ is any covering
$\{U_i \to U\}$ of $(\textit{Sch}/S)_{\acute{e}tale}$ which is a
...
have to show properties (1), (2) and (3) of
Sites, Definition \ref{sites-definition-site}.
Since $(\textit{Sch}/S)_{\acute{e}tale}$ is a site, it suffices to prove
that given any covering $\{U_i \to U\}$ of $(\textit{Sch}/S)_{Zar}$
with $U \in \text{Ob}(S_{\acute{e}tale})$ we also have $U_i \in \text{Ob}(S_{\acute{e}tale})$.
...
Let $S$ be a scheme. Let $\textit{Sch}_{\acute{e}tale}$ be a big \'etale
site containing $S$. The underlying categories of the sites
$\textit{Sch}_{\acute{e}tale}$, $(\textit{Sch}/S)_{\acute{e}tale}$,
$S_{\acute{e}tale}$, and $(\textit{Aff}/S)_{\acute{e}tale}$ have fibre products.
In each case the obvious functor into the category $\textit{Sch}$ of
all schemes commutes with taking fibre products. The categories
$(\textit{Sch}/S)_{\acute{e}tale}$, and $S_{\acute{e}tale}$ both have a final object,
namely $S/S$.
\end{lemma}

\begin{proof}
For $\textit{Sch}_{\acute{e}tale}$ it is true by construction, see
Sets, Lemma \ref{sets-lemma-what-is-in-it}.
Suppose we have $U \to S$, $V \to U$, $W \to U$ morphisms
of schemes with $U, V, W \in \text{Ob}(\textit{Sch}_{\acute{e}tale})$.
The fibre product $V \times_U W$ in $\textit{Sch}_{\acute{e}tale}$
is a fibre product in $\textit{Sch}$ and
is the fibre product of $V/S$ with $W/S$ over $U/S$ in
the category of all schemes over $S$, and hence also a
fibre product in $(\textit{Sch}/S)_{\acute{e}tale}$.
This proves the result for $(\textit{Sch}/S)_{\acute{e}tale}$.
...
Let $S$ be a scheme. Let $\textit{Sch}_{\acute{e}tale}$ be a big \'etale
site containing $S$.
The functor $(\textit{Aff}/S)_{\acute{e}tale} \to (\textit{Sch}/S)_{\acute{e}tale}$
is special cocontinuous and induces an equivalence of topoi from
$\textit{Sh}((\textit{Aff}/S)_{\acute{e}tale})$ to
$\textit{Sh}((\textit{Sch}/S)_{\acute{e}tale})$.
\end{lemma}
...
Sites, Lemma \ref{sites-lemma-equivalence}.
Denote the inclusion functor
$u : (\textit{Aff}/S)_{\acute{e}tale} \to (\textit{Sch}/S)_{\acute{e}tale}$.
...
Let $\textit{Sch}_{\acute{e}tale}$ be a big \'etale site.
Let $f : T \to S$ be a morphism in $\textit{Sch}_{\acute{e}tale}$.
The functor $T_{\acute{e}tale} \to (\textit{Sch}/S)_{\acute{e}tale}$
is cocontinuous and induces a morphism of topoi
$$
i_f :
\textit{Sh}(T_{\acute{e}tale})
\longrightarrow
\textit{Sh}((\textit{Sch}/S)_{\acute{e}tale})
$$
For a sheaf $\mathcal{G}$ on $(\textit{Sh}/S)_{\acute{e}tale}$
we have the formula $(i_f^{-1}\mathcal{G})(U/T) = \mathcal{G}(U/S)$.
The functor $i_f^{-1}$ also has a left adjoint $i_{f, !}$ which commutes
with fibre products and equalizers.
\end{lemma}
...
\begin{proof}
Denote the functor $u : T_{\acute{e}tale} \to (\textit{Sch}/S)_{\acute{e}tale}$.
...
Let $S$ be a scheme. Let $\textit{Sch}_{\acute{e}tale}$ be a big \'etale
site containing $S$.
The inclusion functor $S_{\acute{e}tale} \to (\textit{Sch}/S)_{\acute{e}tale}$
satisfies the hypotheses of Sites, Lemma \ref{sites-lemma-bigger-site}
and hence induces morphisms of topoi
$$
\xymatrix{
\textit{Sh}((\textit{Sch}/S)_{\acute{e}tale}) \ar[r]^-{\pi_S} &
\textit{Sh}(S_{\acute{e}tale}) &
\textit{Sh}(S_{\acute{e}tale}) \ar[r]^-{i_S} &
\textit{Sh}((\textit{Sch}/S)_{\acute{e}tale})
}
$$
such that $\pi_S \circ i_S = \text{id}$. Moreover, $i_S = i_{\text{id}_S}$
...
\begin{proof}
In this case the functor $u : S_{\acute{e}tale} \to (\textit{Sch}/S)_{\acute{e}tale}$,
in addition to the properties seen in the proof of
...
$\mathcal{F}$ on the big \'etale site we denote $\mathcal{F}|_{S_{\acute{e}tale}}$
this restriction.
\end{definition}
...
big site and a sheaf $\mathcal{G}$ on the big site that
\begin{align*}
\text{Mor}_{\textit{Sh}(S_{\acute{e}tale})}(\mathcal{F}|_{S_{\acute{e}tale}}, \mathcal{G})
& =
\text{Mor}_{\textit{Sh}((\textit{Sch}/S)_{\acute{e}tale})}(\mathcal{F},
i_{S,*}\mathcal{G}) \\
\text{Mor}_{\textit{Sh}(S_{\acute{e}tale})}(\mathcal{G}, \mathcal{F}|_{S_{\acute{e}tale}})
& =
\text{Mor}_{\textit{Sh}((\textit{Sch}/S)_{\acute{e}tale})}(\pi_S^{-1}\mathcal{G},
\mathcal{F})
\end{align*}
...
The functor
$$
u : (\textit{Sch}/T)_{\acute{e}tale} \longrightarrow (\textit{Sch}/S)_{\acute{e}tale},
\quad
V/T \longmapsto V/S
$$
is cocontinuous, and has a continuous right adjoint
$$
v : (\textit{Sch}/S)_{\acute{e}tale} \longrightarrow (\textit{Sch}/T)_{\acute{e}tale},
\quad
(U \to S) \longmapsto (U \times_S T \to T).
$$
They induce the same morphism of topoi
$$
f_{big} :
\textit{Sh}((\textit{Sch}/T)_{\acute{e}tale})
\longrightarrow
\textit{Sh}((\textit{Sch}/S)_{\acute{e}tale})
$$
We have $f_{big}^{-1}(\mathcal{G})(U/T) = \mathcal{G}(U/S)$.
...
\medskip\noindent
The functor $u : S_{\acute{e}tale} \to T_{\acute{e}tale}$, $u(U \to S) = (U \times_S T \to T)$
...
Moreover, both $S_{\acute{e}tale}$, $T_{\acute{e}tale}$ have final objects, namely
...
$T_{\acute{e}tale} \to S_{\acute{e}tale}$. This in turn gives rise to the morphism
of topoi, see
...
$T_{\acute{e}tale}$. Let $U \to S$ be an open
immersion of $\textit{Sch}_{\acute{e}tale}$. According to
(2) above we have $(f_{small, *}(\pi_{T, *}\mathcal{G}))(U/S) =
\pi_{T, *}\mathcal{G}(U \times_S T/T) = \mathcal{G}(U\times_S T/T)$.
On the other hand,
$(\pi_{S, *}(f_{big, *}\mathcal{G}))(U/S) =
...
$$
\xymatrix{
\textit{Sh}(T_{\acute{e}tale}) \ar[d]_{f_{small}} &
\textit{Sh}((\textit{Sch}/T)_{\acute{e}tale}) \ar[d]^{f_{big}} \ar[l]^{\pi_T}\\
\textit{Sh}(S_{\acute{e}tale}) &
\textit{Sh}((\textit{Sch}/S)_{\acute{e}tale}) \ar[l]_{\pi_S}
}
$$
In general $i_S \circ f_{small} \not = f_{big} \circ i_T$.
...
Given schemes $X$, $Y$, $Y$ in $\textit{Sch}_{\acute{e}tale}$
and morphisms $f : X \to Y$, $g : Y \to Z$ we have
$g_{big} \circ f_{big} = (g \circ f)_{big}$ and
$g_{small} \circ f_{small} = (g \circ f)_{small}$.
\end{lemma}
...
Let $S$ be a scheme contained in a big \'etale site $\textit{Sch}_{\acute{e}tale}$.
A sheaf $\mathcal{F}$ on the big \'etale site $(\textit{Sch}/S)_{\acute{e}tale}$
is given by the following data:
\begin{enumerate}
\item for every $T/S \in \text{Ob}((\textit{Sch}/S)_{\acute{e}tale})$ a sheaf
$\mathcal{F}_T$ on $T_{\acute{e}tale}$,
\item for every $f : T' \to T$ in
$(\textit{Sch}/S)_{\acute{e}tale}$ a map
$c_f : f_{small}^{-1}\mathcal{F}_{T} \to \mathcal{F}_{T'}$.
\end{enumerate}
These data are subject to the following conditions:
\begin{enumerate}
\item[(\romannumeral1)] given any $f : T' \to T$ and $g : T'' \to T'$ in
$(\textit{Sch}/S)_{\acute{e}tale}$ the composition $g_{small}^{-1}c_f \circ c_g$
is equal to $c_{f \circ g}$, and
\item[(\romannumeral2)] if $f : T' \to T$ in $(\textit{Sch}/S)_{\acute{e}tale}$ is
\'etale then $c_f$ is an isomorphism.
\end{enumerate}
\end{lemma}

\begin{proof}
Given a sheaf $\mathcal{F}$ on $\textit{Sh}((\textit{Sch}/S)_{\acute{e}tale})$
we set $\mathcal{F}_T = i_p^{-1}\mathcal{F}$ where $p : T \to S$
is the structure morphism. Note that
...
$u : T_{\acute{e}tale} \to T'_{\acute{e}tale}$ is the base change functor associated
...
\item $(\textit{Sch}/X)_{fppf} \longrightarrow (\textit{Sch}/Y)_{\acute{e}tale}$,
...
\item $(\textit{Sch}/X)_{syntomic} \longrightarrow (\textit{Sch}/Y)_{\acute{e}tale}$,
...
\item $(\textit{Sch}/X)_{smooth} \longrightarrow (\textit{Sch}/Y)_{\acute{e}tale}$,
\item $(\textit{Sch}/X)_{smooth} \longrightarrow (\textit{Sch}/Y)_{Zar}$,
\item $(\textit{Sch}/X)_{\acute{e}tale} \longrightarrow (\textit{Sch}/Y)_{\acute{e}tale}$,
\item $(\textit{Sch}/X)_{\acute{e}tale} \longrightarrow (\textit{Sch}/Y)_{Zar}$,
\item $(\textit{Sch}/X)_{Zar} \longrightarrow (\textit{Sch}/Y)_{Zar}$,
\item $(\textit{Sch}/X)_{fppf} \longrightarrow Y_{\acute{e}tale}$,
\item $(\textit{Sch}/X)_{syntomic} \longrightarrow Y_{\acute{e}tale}$,
\item $(\textit{Sch}/X)_{smooth} \longrightarrow Y_{\acute{e}tale}$,
\item $(\textit{Sch}/X)_{\acute{e}tale} \longrightarrow Y_{\acute{e}tale}$,
...
\item $(\textit{Sch}/X)_{\acute{e}tale} \longrightarrow Y_{Zar}$,
