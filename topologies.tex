...
is cocontinuous, commutes with fibre products, is continuous
exactly as in Lemma \ref{lemma-put-in-T} above. Hence (1) follows
from Sites, Lemmas \ref{sites-lemma-when-shriek}
and \ref{sites-lemma-preserve-equalizers}.
...
\medskip\noindent
Statement (3): See Sites, Example \ref{sites-example-continuous-map}.

\medskip\noindent
To see (4) let us compute $f_{small, *}(\mathcal{G})$ for a
sheaf $\mathcal{G}$ on $T_{Zar}$. Let $U \to S$ be an open
immersion of $\textit{Sch}_{Zar}$. According to
(3) above we have $f_{small, *}(\mathcal{G})(U/S) = 
