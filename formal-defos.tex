...
$\Lambda$-algebra $A$ and where $\varphi : A/\mathfrak m_A \to k$ is a
...
\to \mathcal{C}$ such that $\mathcal{F}^{\textnormal{opp}}$ is a category 
fibered in groupoids over $\mathcal{C}^{\textnormal{opp}}$ via 
$p^{\textnormal{opp}}: \mathcal{F}^{\textnormal{opp}} \to \mathcal 
C^{\textnormal{opp}}$.
\end{definition}
...
let $f: U \to V$ be a morphism in $\mathcal{C}$, and let $x \in 
\textnormal{Ob}(\mathcal{F}(U))$.  A pushforward of $x$ along $f$ is a morphism 
$x \to y$ of $\mathcal{F}$ lying over $f$.  We sometimes write $x 
\to f_*x$ for ``the'' pushforward of $x$ along $f$, if it is assumed a 
...
pseudo-functor $\mathcal{C} \to \textnormal{(Groupoids)}$.  We will 
...
Let $F: \mathcal{C} \to \textnormal{(Groupoids)}$ be a functor.  There 
...
morphism $(U,x) \to (V,y)$ is a pair $(f,a)$ where $f \in 
\textnormal{Hom}_{\mathcal{C}}(U,V)$ and $a \in 
\textnormal{Hom}_{F(V)}(F(f)(x),y)$.  The morphism $\mathcal{F} \to 
\mathcal{C}$ sends $(U,x)$ to $U$.
\item Given a category $\mathcal{F}$ cofibered in groupoids over $\mathcal{C}$, 
we denote by $\overline{\mathcal{F}} : \mathcal{C} \to 
\textit{Sets}$ the associated functor of isomorphism classes. Similarly, 
if $\varphi: \mathcal{F} \to \mathcal G$ is a morphism of cofibered 
categories, we denote by $\overline{\varphi}: \overline{\mathcal{F}} \to 
\overline{\mathcal G}$ the associated morphism of functors.
\item 
\label{item-convention-cofibered-sets}
Let $F: \mathcal{C} \to \textit{Sets}$ be a functor.  Then the 
...
the category of functors $\mathcal{C} \to \textit{Sets}$ to the 
...
\mathcal{C} \to \textit{Sets}$ is a functor, we denote the 
associated category cofibered in sets also by $F$; and if $\varphi: F 
\to G$ is a morphism of functors, we denote still by $\varphi$ the 
...
$\textnormal{Hom}_{\mathcal{C}}(U,-): \mathcal{C} \to 
\textit{Sets}$.  This defines a fully faithful embedding of $\mathcal 
C^{\textnormal{opp}}$ into the category of functors $\mathcal{C} \to 
...
\mathcal{F}|_{\mathcal{C}'} \to \mathcal{C}'$.
\end{enumerate}
\end{remarks}
...
Let $R \to S$ be a local homomorphism of local rings.  Let $\kappa(S)$ 
...
$T^*_{R/k} \to T^*_{S/k}$ is surjective, since $R_2 \cong k \oplus 
...
\]
the left vertical map is surjective by surjectivity of $T^*_{R/k} \to 
...
R_n \to \lim S_n \cong S$ is surjective.
\end{proof}
...
$\widehat{~}: \textnormal{Cof}(\mathcal{C}_\Lambda) \to 
\textnormal{Cof}(\widehat{\mathcal{C}}_\Lambda)$ on the $1$-category 
truncations (denoted still by $\textnormal{Cof}(\mathcal{C}_\Lambda)$ and 
...
\textnormal{Cof}(\widehat{\mathcal{C}}_\Lambda) \to 
\textnormal{Cof}(\mathcal{C}_\Lambda)$ on the $1$-truncations.  

\medskip \noindent
The functors $\widehat{~}$ and $|_{\mathcal{C}_\Lambda}$ can be considered 
``2-adjoint'' in the following sense.  Let $\mathcal{F} \in 
\textnormal{Ob}(\textnormal{Cof}(\mathcal{C}_\Lambda))$ and let $\mathcal G 
\in \textnormal{Ob}(\textnormal{Cof}(\widehat{\mathcal{C}}_\Lambda))$.  Then 
there is an equivalence of categories
\[ 
\Phi: \textnormal{Hom}_{\mathcal{C}_\Lambda}(\mathcal G|_{\mathcal 
C_\Lambda}, \mathcal{F}) 
\xrightarrow{\sim} 
...
groupoids.
\end{enumerate}
The equivalence $\Phi: \textnormal{Hom}_{\mathcal{C}_\Lambda}(\mathcal 
...
\textnormal{Hom}_{\mathcal{C}_\Lambda}(\mathcal G|_{\mathcal{C}_\Lambda}, 
...
\textnormal{CofSet}(\mathcal{C}_\Lambda) \to 
\textnormal{CofSet}(\widehat{\mathcal{C}}_\Lambda)$ and $|_{\mathcal 
C_\Lambda}: \textnormal{CofSet}(\widehat{\mathcal{C}}_\Lambda) \to 
\textnormal{CofSet}(\mathcal{C}_\Lambda)$ which we denote by the same symbols. 
...
bijection
\[ 
\textnormal{Hom}_{\mathcal{C}_\Lambda}(G|_{\mathcal{C}_\Lambda}, F) 
\xrightarrow{\sim} 
\textnormal{Hom}_{\widehat{\mathcal{C}}_\Lambda}(G, \widehat{F}) 
\]
for $F \in \textnormal{Ob}(\textnormal{CofSet}(\mathcal{C}_\Lambda))$ and $G 
...
\[
\textnormal{Hom}_{\mathcal{C}_\Lambda}(\underline{R}|_{\mathcal{C}_\Lambda}, 
\mathcal{F}) 
\xrightarrow{\sim} 
\widehat{\mathcal{F}}(R).
\]
...
\[ \widehat{\mathcal{F}}(R) \to \textnormal{Hom}_{\widehat{\mathcal 
...
\underline{R}(S) = \text{Hom}_{\mathcal{C}_\Lambda}(R,S)$ to $f_* \xi$.  Then 
...
exists $x' \in \textnormal{Ob}(\mathcal{F}(B))$, a morphism $x' \to x$ 
lying over $B \to A$, and a morphism $\varphi(x') \to y$ lying 
over $\textnormal{id}: B \to B$, such that the diagram
\[
\xymatrix{
...
$\textnormal{id}: B_1 \to B_1$ such that
\[
\xymatrix{
...
$\textnormal{id}: B_2 \to B_2$ so that the diagram
\[
\xymatrix{
...
\textnormal{(S1)} is assumed to hold only when $A_2 \to A$ is a small 
extension.
\end{lemma}
...
specifically for a functor $L: \textnormal{f.g.-Mod}_{R} \to 
\textnormal{Mod}_{R}$ because this is the case that occurs below.
\end{remark}

\begin{remark}
\label{remark-linear-functor}
If $L: \textnormal{f.g.-Mod}_{R} \to \textnormal{Mod}_{R}$ is a 
...
$\textnormal{f.g.-Mod}_{R} \to \textit{Sets}$ 
...
Let $L: \textnormal{f.g.-Mod}_{R} \to \textit{Sets}$ be a 
...
\textnormal{f.g.-Mod}_{R} \to \textnormal{Mod}_{R}$ such that the 
diagram
\[ 
\xymatrix{
...
Let $L_1,L_2: \textnormal{f.g.-Mod}_R \to \textit{Sets}$ be 
...
\textnormal{f.g.-Mod}_{K} \to \textnormal{Mod}_{K}$ is determined by 
$L(K)$:

\begin{lemma}
\label{lemma-linear-functor-over-field}
Let $K$ be a field. Let $L: \textnormal{f.g.-Mod}_{K} \to 
\textnormal{Mod}_{K}$ be a $K$-linear functor.  Then $L$ is isomorphic to the 
functor $L(K) \otimes_{K} - : \textnormal{f.g.-Mod}_{K} \to 
\textnormal{Mod}_{K}$.
\end{lemma}
...
\mapsto R[M]$ determines a functor $\textnormal{Mod}_{R} \to S 
...
Let $R$ be an $S$-algebra. Then the functor $\textnormal{Mod}_{R} \to S 
...
\begin{proof}
The functor $\textnormal{Mod}_{R} \to S \textnormal{-Alg}/R$ of Lemma 
\ref{lemma-preserves-products} restricts to a functor 
$\textnormal{f.g.-Mod}_{R} \to \mathcal{C}$ by the assumption on 
$\mathcal{C}$. Let $L$ be the composition $\textnormal{f.g.-Mod}_{R} \to 
...
satisfy \textnormal{(S2)}.  Then $d \varphi: T \mathcal{F} \to T 
...
\]
with $f: A' \to A$ a small extension. Let $I = \textnormal{Ker}(f)$ and 
let $g: A' \times_{A} A' \to k[I]$ be the map $(x,y) \mapsto q(x) + y - 
x$ where $q: A' \to A'/\mathfrak{m}_{A'} = k$.  Assume $\mathcal{F}$ 
satisfies \textnormal{(S2)}. If there exists a morphism $x_1 \to 
...
\]
The induced map $y \to x_1 \times_{x} x_2$ lies over $\textnormal{id}: 
...
\[ \textnormal{Aut}_{A'}(x') \to \textnormal{Aut}_{A}(x). \]
...
surjective ring map $A' \to A$, the map $\textnormal{Aut}_{A'}(x') 
\to \textnormal{Aut}_{A}(x)$ is surjective.
\item For every morphism $x' \to x$ in $\mathcal{F}$ lying over a small 
extension $A' \to A$, the map $\textnormal{Aut}_{A'}(x') \to 
...
objects
\[ (x',x', a: x \to x), (x',x', \textnormal{id}: x \to x) \]
...
\otimes_{k} I) \to \textnormal{Lift}(x,A').\]
...
\to \textnormal{Lift}(\varphi(x),A')$, denoted still by $\varphi$, and 
...
\textnormal{Ob}(\mathcal{C}_\Lambda)$ by induction on 
...
$\textnormal{Inf}(x'/x)$ of a morphism $x' \to x$ lying over a small 
...
\to V$ to $\textnormal{Aut}_{V}(f_*x)$ and sending a morphism
\[
\xymatrix{
V' \ar[rr] &                    & V\\
          & U \ar[ul]^{f}  \ar[ur]_{f'} & 
...
\]
to the homomorphism $\textnormal{Aut}_{V'}(f'_*x) \to 
...
$\textnormal{Aut}(x): \mathcal{C}_{A} \to \textit{Sets}$.

\begin{lemma}
\label{lemma-Aut-functor-RS}
Let $\mathcal{F}$ be a category cofibered in groupoids over $\mathcal 
C_\Lambda$ satisfying \textnormal{(RS)}.  Let $x \in \textnormal{Ob}(\mathcal 
F(A))$.  Then $\textnormal{Aut}(x): \mathcal{C}_{A} \to 
\textit{Sets}$ satisfies \textnormal{(RS)}.
\end{lemma}
...
Let $\mathcal{F}$ be a category cofibered in groupoids over $\mathcal 
C_\Lambda$ satisfying \textnormal{(RS)}.  Let $x' \to x$ be a 
...
Let $\mathcal{F}$ be a category cofibered in groupoids over $\mathcal 
C_\Lambda$ satisfying \textnormal{(RS)}. Let $x' \to x$ be a morphism 
...
$\textnormal{Inf}(x/x_0) = 0$ for any object $x \to x_0$ of $\mathcal 
...
\mathcal{C} \to \textnormal{(Groupoids)}$, and a morphism of groupoids 
in functors on $\mathcal{C}$ amounts to a morphism of the corresponding functors 
$\mathcal{C} \to \textnormal{(Groupoids)}$ (where 
...
$\mathcal{C} \to \textnormal{(Groupoids)}$, or equivalently as the 
...
$\mathcal{C} \to \textnormal{(Groupoids)}$. The {\it quotient category 
...
\to T'$ to $(f, \textnormal{id}_{x'}): (T,x) \to (T,x')$.
\end{enumerate} 
\end{definition}
...
$\textnormal{CofSet}(\mathcal{C}_\Lambda)$ is a right adjoint, so it commutes 
...
\[ T[U/R] = \textnormal{Coker}(ds-dt: TR \to TU).
\]
\item Let $x_0$ be the unique object of $[U/R](k)$.  The space of infinitesimal 
automorphisms of $[U/R]$ is 
\[ \textnormal{Inf}_{x_0}([U/R]) = \textnormal{Ker}(ds: TR \to TU) \cap 
\textnormal{Ker}(dt: TR \to TU).  
\]
\end{enumerate}
\end{lemma}
...
$k$-vector spaces
\[ 
0 \to \textnormal{Inf}_{f(u_0)}(\mathcal{F}) \to T(U \times_{f, 
