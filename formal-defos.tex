...
$\underline{R} = \text{Mor}_{\widehat{\mathcal{C}}_\Lambda}(R, -)$
...
\lim_n G(S/\mathfrak{m}_{S}^n) =
G(\lim_n S/\mathfrak{m}_{S}^n) = G(S).
$$
...
$$
L : \Hom_{R}(M,N) \longrightarrow \Hom_{R}(L(M),L(N))
$$
...
F(k[\epsilon] \times_{k} k[\epsilon]) \longrightarrow
F(k[\epsilon])
$$
...
identity and inverse morphisms being the completions of those of $(U,R,s,t,c)$.
...
\section{Smooth groupoids in functors on $\mathcal{C}_\Lambda$}
...
$x_0 \in \text{Ob}(\mathcal{F}(k))$. Then $(U,R,s,t,c)$ is prorepresentable.
\end{lemma}
...
Let $(U,R,s,t,c)$ be a minimal smooth prorepresentable groupoid in functors on
$\mathcal{C}_\Lambda$. If $\varphi: [U/R] \to [U/R]$ is an  equivalence of
categories cofibered in groupoids, then $\varphi$ is an isomorphism.
\end{lemma}
...
Let $(U,R,s,t,c)$ and $(U',R',s',t',c')$ be minimal smooth prorepresentable
groupoids in functors on $\mathcal{C}_\Lambda$. If
