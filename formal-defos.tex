...
{\it minimal} if for any morphism of formal objects
...
\noindent
Here is a very general notion of minimality, in the sense that we do
not need to assume there is a vector space structure on the tangent
spaces.
...
\begin{enumerate}
\item We say $\varphi$ is {\it classically minimal}
if $d\varphi : T\mathcal{F} \to T\mathcal{G}$ is bijective.
\item We say $\varphi$ is {\it minimal} if
if $d\varphi : T\mathcal{F} \to T\mathcal{G}$ is bijective
on $\text{Der}_\Lambda(k, k)$-orbits.
\end{enumerate}
Here we are using the action (\ref{equation-action}) of derivations
on the tangent spaces.
\end{definition}

\noindent
If $k' \subset k$ is separable, then $\text{Der}_\Lambda(k, k) = 0$
and the two conditions are equivalent. We will see later why minimality
is the ``correct'' condition in the case that $k' \subset k$ is not
separable.

\begin{remark}
\label{remark-predeformation-category-minimal-over-iso}
Let $\mathcal{F}$ be a predeformation category.
Then the morphism $\mathcal{F} \to \overline{\mathcal{F}}$ is
classically minimal and smooth by
Remark \ref{remark-smooth-to-iso-classes}.
\end{remark}

\noindent
In this section we study the existence of minimal, smooth morphisms
$U \to \mathcal{F}$ from a prorepresentable functor $U$  to a predeformation
category $\mathcal{F}$. This amounts to constructing a miniversal formal
object for $\mathcal{F}$.

\begin{definition}
\label{definition-miniversal-object}
Let $\mathcal{F}$ be a predeformation category.
Let $\xi$ be a formal object of $\mathcal{F}$ lying over
$R \in \text{Ob}(\widehat{\mathcal{C}}_\Lambda)$.
Let $\underline{\xi}: \underline{R}|_{\mathcal{C}_\Lambda} \to \mathcal{F}$
be the corresponding morphism of
Remark \ref{remark-formal-objects-yoneda}.
\begin{enumerate}
\item We say $\xi$ is {\it classically minimal}
if $\underline{\xi}$ is classically minimal.
\item We say $\xi$ is {\it classically miniversal} if it is
classically minimal and versal.
\item We say $\xi$ is {\it minimal}
if $\underline{\xi}$ is minimal.
\item We say $\xi$ is {\it miniversal} if it is
minimal and versal.
\end{enumerate}
\end{definition}

\noindent
Again we note that classicially miniversal and miniversal agree
if $k' \subset k$ is separable, for example in the classical case.

\begin{remark}
\label{remark-choice-pushforwards-iso-class}
The property of being (classically) minimal does not depend on the choices
of pushforwards in
Remark \ref{remark-formal-objects-yoneda}
...
\noindent
Classically minimal objects satisfy the following uniqueness property.

\begin{lemma}
\label{lemma-miniversal-object-unique}
Let $\mathcal{F}$ be a predeformation category.
A classically miniversal formal object of $\mathcal{F}$, if it exists, is
unique up to a (non-canonical) isomorphism in $\widehat{\mathcal{F}}$.
\end{lemma}

\begin{proof}
Let $\xi$ and $\xi'$ be classically miniversal formal objects of $\mathcal{F}$
lying over $R$ and $R'$ in $\widehat{\mathcal{C}}_\Lambda$. By
Lemma \ref{lemma-versal-object-quasi-initial}
we have morphisms $a : \xi \to \xi'$ and $a' : \xi' \to \xi$.
Let $f : R \to R'$ and $f' : R' \to R$ be the projections of $a$ and $a'$
to $\widehat{\mathcal{C}}_\Lambda$. By
Remark \ref{remark-formal-objects-yoneda-map}
we obtain a $2$-commutative diagram
$$
\xymatrix{
\underline{R}|_{\mathcal{C}_\Lambda} \ar[rd]_{\underline{\xi}} & &
\underline{R'}|_{\mathcal{C}_\Lambda} \ar[ll]^f \ar[ld]^{\underline{\xi'}} \\
& \mathcal{F}
}
$$
hence a commutative diagram of tangent spaces
$$
\xymatrix{
T\underline{R}|_{\mathcal{C}_\Lambda} \ar[rd]_{T\underline{\xi}} & &
T\underline{R'}|_{\mathcal{C}_\Lambda} \ar[ll]^{Tf}
\ar[ld]^{T\underline{\xi'}} \\
& T\mathcal{F}
}
$$
By minimality of $\xi$ and $\xi'$ these maps are isomorpisms. By
Example \ref{example-tangent-space-map-prorepresentable-functor}
this means that
$$
\text{Der}_\Lambda(R', k) \longrightarrow \text{Der}_\Lambda(R, k)
$$
is an isomorphism. By
Lemma \ref{lemma-derivations-surjective}
this means that $f : R \to R'$ is surjective. Similarly $f' : R' \to R$
is surjective. In particular $f' \circ f: R \to R$
is a surjective endomorphism of the Noetherian ring $R$.
We conclude $f' \circ f$ is an isomorphism, see
Algebra, Lemma \ref{algebra-lemma-surjective-endo-noetherian-ring-is-iso}.
Similarly $f \circ f'$ is an isomorphism.
Thus $f$ and $f'$ are isomorphisms. Since $\widehat{\mathcal{F}}$
is cofibred in groupoids we conclude that also $a$ and $a'$
are isomorphisms.
\end{proof}
...
Let $\mathcal{F}$ be a predeformation category.
Assume $\mathcal{F}$ has a classically miniversal formal object.
...
Lemma \ref{lemma-versal-object-S1}.
Let $\xi$ be a classically miniversal formal object of $\mathcal{F}$.
...
Since $\xi$ is minimal, we see that $p(c) \circ p(b) = p(c') \circ p(b')$
...
Finally, by
Example \ref{example-tangent-space-prorepresentable-functor}
we see $\dim_{k} T\mathcal{F} = \dim_{k} TU$ is finite.
\end{proof}

\begin{theorem}
\label{theorem-miniversal-object-existence}
Assume the field extension $k \subset k'$ is separable.
Let $\mathcal{F}$ be a predeformation category. Then $\mathcal{F}$
has a classically minimal versal formal object if and only if
