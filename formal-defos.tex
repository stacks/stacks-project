...
is smooth, then $R$ is isomorphic to a completion as above, but we won't
use this.
\end{example}
...
$$
commutative in $\mathcal{C}_\Lambda$ where
...
natural $k$-vector space structure.
\end{lemma}
...
\noindent
Here is a very general notion of minimality.
...
$U \to \mathcal{F}$ from a prorepresentable functor $U$ 
to a predeformation category $\mathcal{F}$. This amounts to 
constructing a miniversal formal object for $\mathcal{F}$, defined
as follows.
...
$R \in \text{Ob}(\widehat{\mathcal{C}}_\Lambda)$.
We say $\xi$ is {\it minimal} if the corresponding morphism 
$\underline{\xi}: \underline{R}|_{\mathcal{C}_\Lambda} \to \mathcal{F}$ 
...
in Remark \ref{remark-formal-objects-yoneda} which were used to construct
$\underline{\xi} : \underline{R}|_{\mathcal{C}_\Lambda} \to \mathcal{F}$
from $\xi$.
\end{remark}
...
\begin{proof}
Let $\xi$ and $\xi'$ be miniversal formal objects of $\mathcal{F}$
...
\begin{proof}
The assumption means there exists a miniversal formal object $\xi$ of
$\mathcal{F}$. Proof of (S1). Let 
$$
\xymatrix{
           & x_2 \ar[d] \\
x_1 \ar[r] & x   
...
Remark \ref{remark-completion-restriction-adjoint}), we can consider this 
...
\medskip\noindent
Proof of (S2). The first part of (S2) holds since (S1) holds.
Let
$$
\vcenter{
\xymatrix{
...
Let $\mathcal{F}$ be a predeformation category.  Assume the following
conditions hold:
\begin{enumerate}
\item $\mathcal{F}$ satisfies (S1).
\item $\mathcal{F}$ satisfies (S2).
...
\begin{proof}
Assume $\mathcal{F}$ is a predeformation category such that conditions
(1), (2), and (3) hold. Choose an object
$R \in \text{Ob}(\widehat{\mathcal{C}}_\Lambda)$
such that $\underline{R}|_{\mathcal{C}_\Lambda}$ is smooth, see
Example \ref{example-smooth}.
Let $r = \dim_{k} T\mathcal{F}$ and put
$S = R[[X_1, \dots, X_r]]$.
Let $J_{1} = \mathfrak m_S$.  We are going to 
