...
Let $p : \mathcal{F} \to \mathcal{C}_\Lambda$ be a category cofibered in
groupoids. We say $\mathcal{F}$ is {\it smooth}
...
The phrase ``{\it $\mathcal{F}$ is unobstructed}'' is sometimes
used to indicate this property; but be careful: it may happen that
...
\item for $y_0 \in \Ob(\mathcal{G}(k))$ with image $z_0$ in $\mathcal{F}$
the map $\text{Inf}_{y_0}(\mathcal{G}) \to \text{Inf}_{z_0}(\mathcal{F})$
is surjective.
\end{enumerate}
...
$\dim_k \text{Inf}_{x_0}(\mathcal{F}) < \infty$ for some
$x_0 \in \Ob(\mathcal{F}(k))$. Let $\xi$ be a versal formal
