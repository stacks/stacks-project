...
condition (RS) on $\mathcal{F}$ generalizing Schlessinger's (H4).
A deformation
...
definitions and using Lemma \ref{lemma-RS-fiber-square} shows that
$\mathcal{F}$ 
...
is injective.  Here
$ \overline{\mathcal{F}(A_1) \times_{\mathcal{F}(A)} \mathcal F(A_2)}$
denotes the set of isomorphism classes.
...
of $\mathcal{F}(A') \times_{\mathcal{F}(A)} \mathcal{F}(A')$
have the same image 
