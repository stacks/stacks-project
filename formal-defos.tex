...
\title{Formal Deformation Theory with Groupoids}
...
\noindent Following Rim \cite{Rim} and Schlessinger \cite{Schlessinger}, we 
...
one element set, Schlessinger \cite{Schlessinger} introduced conditions 
(H1)-(H4) such that: 
\begin{enumerate}
...
general we do not make this assumption.  Rim's paper \cite{Rim} is the original 
...
\subsection{Completions of categories cofibered in groupoids over $\mathcal 
C_{\Lambda}$}
...
with the definition in Schlessinger's paper \cite{Schlessinger}.
\end{remark}
...
a smooth morphism of functors, cf.\ \cite[Definition 2.2.]{Schlessinger}. In 
...
\begin{remark}
Let $\mathcal F$ be a category cofibered in groupoids over $\mathcal 
C_{\Lambda}$.  Then the morphism $\mathcal F \rightarrow \overline{\mathcal F}$ 
is smooth.
\end{remark}
...
guarantee the existence of a miniversal formal object (as defined in Section 
\ref{subsection-miniversal-objects-existence}).
...
conditions (H1) and (H2) from Schlessinger's paper \cite{Schlessinger}.  
...
(H4) from Schlessinger's paper \cite[Theorem 2.11]{Schlessinger}.  Namely, for 
...
\subsubsection{Prorepresentable groupoids in functors on $\mathcal C_{\Lambda}$}
\begin{definition}
...
\subsubsection{Presentations of deformation categories}
