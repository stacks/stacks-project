...
\begin{lemma}
\label{lemma-moduli-space-finite-properties}
Let $(U, R, s, t, c)$ be a groupoid scheme. Assume $U$ and
$R$ are affine and $s, t : R \to U$ finite locally free.
Let $C \subset \Gamma(U, \mathcal{O}_U)$ be the ring of $R$-invariant
functions on $U$. The morphism
$$
f : [U/R] \longrightarrow M = \Spec(C)
$$
is separated, quasi-compact, and a universal homeomorphism.
\end{lemma}

\begin{proof}
Observe that $[U/R]$ is an algebraic stack by
Criteria for Representability, Theorem
\ref{criteria-theorem-flat-groupoid-gives-algebraic-stack}.
Moreover, $[U/R]$ is separated by
Morphisms of Stacks, Lemma
\ref{stacks-morphisms-lemma-properties-diagonal-from-presentation}.
Hence $f$ is separated (Morphisms of Stacks, Lemma
\ref{stacks-morphisms-lemma-compose-after-separated}).

\medskip\noindent
Since $U \to [U/R]$ is surjective and since $U \to M$ is quasi-compact,
...
Since $U \to [U/R]$ is surjective, it follows that $[U/R] \to M$
is universally closed (Morphisms of Stacks, Lemma
\ref{stacks-morphisms-lemma-image-proper-is-proper}).
To conclude that $[U/R] \to M$ is a universal homeomorphism,
it is enough to show that it is universally bijective, i.e.,
surjective and universally injective. Surjectivity follows from
...
the map $C' \to C^1$ induces a homeomorphism (bijection) on spectra.
On the other hand, the first arrow in
$$
|[U'/R']| \to |\Spec(C^1)| \to |\Spec(C')|
$$
is bijective by
Groupoids, Lemma \ref{groupoids-lemma-points} and
Morphisms of Stacks, Lemma \ref{stacks-morphisms-lemma-points-presentation}
applied to the groupoid $(U', R', s', t', c')$.
In this way we conclude that $f$ induces a bijection on points after any
...
f : [U/R] \longrightarrow M = \Spec(C)
$$
is a uniform categorical moduli space (and has properties listed in
Lemma \ref{lemma-moduli-space-finite-properties}).
\end{lemma}

\begin{proof}
Assume $C \to C'$ is a flat ring map. Let $(U', R', s', t', c')$ be
the base change of $(U, R, s, t, c)$ by $M' = \Spec(C') \to M$. Then $C'$
is the ring of $R'$-invariant functions on $U'$, see
Groupoids, Lemma \ref{groupoids-lemma-invariants-base-change}.
By Lemma \ref{lemma-check-uniform-categorical-quotient-on-affines}
it suffices to show that $f' : [U'/R'] \to M'$ is a categorical moduli space.
Replacing $(U, R, s, t, c)$ by $(U', R', s', t', c')$ we reduce
to proving $f$ is a categorical moduli space.
