...
$F : \textit{Coh}(\mathcal{O}_\mathcal{X}) \to \text{Mod}^{fg}(A)$
...
\end{enumerate}
such that $F \circ g_* : \text{Mod}(R) \to \text{Mod}(A)$
sends $M$ to $M \otimes_R B$ seen as $A$-module.
\end{lemma}

\begin{proof}
Denote $F : \QCoh(\mathcal{O}_\mathcal{X}) \to \text{Mod}(A)$ the
extension of $F$ produced by Lemma \ref{lemma-extend}.
The functor $g_*$ sends $\QCoh(\mathcal{O}_U)$ into
$\QCoh(\mathcal{O}_\mathcal{X})$, see FIXME.
Observe that $g_*\mathcal{O}_U$ is a commutative algebra object in
$\text{QCoh}(\mathcal{O}_\mathcal{X})$.
Hence $B = F(g_*\mathcal{O}_U)$ is a commutative
