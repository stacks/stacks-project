...
the functor $T \mapsto \Ob(\mathit{Pic}_{X/B, T})/\cong$.
\end{lemma}

\begin{proof}
Since the fibre category $\mathit{Pic}_{X/B, T}$ of the Picard stack
$\mathit{Pic}_{X/B}$ over $T$ is the category of invertible sheaves on
...
In Situation \ref{situation-pic} assume that $X \to B$ is flat, proper,
and of finite presentation, and that
$\mathcal{O}_T \to f_{T, *}\mathcal{O}_{X_T}$
is an isomorphism for all schemes $T$ over $B$.
Then the diagonal
$\text{Pic}_{X/B} \to \text{Pic}_{X/B} \times_B \text{Pic}_{X/B}$
is representable by immersions.
\end{lemma}
...
Simlarly, after base change (Remark \ref{remark-spaces-base-change})
...
Deformation Theory, Section \ref{defos-section-deformations-spaces}
