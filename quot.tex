...
\begin{remark}
\label{remark-not-flat}
In Lemma \ref{lemma-coherent-defo-thy} the assumption that $f : X \to B$ is
flat is not necessary. The proof of Lemma \ref{lemma-coherent-defo-thy}
uses the construction of an obstruction theory as in
...
which is how the assumption that $f$ is flat comes about.
To work around this one can use the (somewhat sketchy) material in
Contangent, Section \ref{cotangent-section-modules}
to build an obstruction theory in general. Instead of giving the details
we explain the underlying idea. Let $A$ be a finite type algebra over
the base $S$. Denote $X \otimes^\mathbf{L} A$ a ``derived base change''
...
$$
Thus morally speaking we obtain obstruction groups of exactly the
same form as in the proof of Lemma \ref{lemma-coherent-defo-thy}
with the only change being that one replaces the first occurrence of
$\mathcal{F}$ by the complex $Li^*(i_*\mathcal{F})$.
The complex $L$ constructed in
Cotangent, Proposition \ref{cotangent-proposition-conclusion}
is an avatar of $Li^*(i_*\mathcal{F})$.

\medskip\noindent
A ``direct'' construction of $L = Li^*(i_*\mathcal{F})$ and direct
proof of its relationship to the deformation theory of $\mathcal{F}$
minimizing the use of derived algebraic geometry and/or simplicial
methods should be possible. In fact, it suffices to construct
$\tau_{\geq -2}L$. We can even identify the cohomology sheaves
$$
H^i(L) =
\left\{
\begin{matrix}
...
\right.
$$
but we don't have a simple (global) construction of $L$.
...
Remark \ref{remark-not-flat}.}. Then $\textit{Coh}_{X/B}$ is
...
we have that $\mathcal{X}$ satsifies openness of versality, hence (3) holds.
