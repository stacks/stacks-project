...
(Topologies on Spaces, \ref{spaces-topologies-lemma-fppf})
...
\section{The $\mathit{Isom}$-functor}
\label{section-isom}
...
$$
c : H' \times_B H \longrightarrow I,\quad
(u', u) \longmapsto u \circ u'
$$
of algebraic spaces over $B$. Since $I \to B$ is separated, the
section $B \to I$ corresponding to $1 = \text{id}_\mathcal{F}$
is a closed immersion of finite presentation. Hence
$$
M = (H' \times_B H) \times_{c, I, 1} B
$$
is an algebraic space affine of finite presentation over $B$ as well.
Composition the other way around gives a morphism
$$
c' : M \longrightarrow I'
$$
and $\mathit{Isom}(\mathcal{F}, \mathcal{G})$ is clearly isomorphic
to the fibre product
$$
M \times_{c', I', 1} B
$$
As above this fibre product is an algebraic space affine and of
finite presentation over $B$ as well. Some details omitted.
\end{proof}
...
$$
is a functor. By definition the fibre categories are groupoids.
In fact, this functor turns $\textit{Coh}_{X/B}$ into a stack
...
\begin{proof}
We first show that $p$ is fibred in groupoids by checking conditions
...
effective. The statement on $\mathit{Isom}$ is exactly the content of
Lemma \ref{lemma-hom-sheaf}. Let us prove the statement on descent data.
...
Hence we may assume $X$ is affine as well.

\medskip\noindent
At this point we have the following algebra problem. Suppose given
a cartesian diagram of rings
$$
\xymatrix{
C & C' \ar[l] \\
B \ar[u] & B' \ar[l] \ar[u]
...
$$
with $C' \to C$ surjective with locally nilpotent kernel
and a finitely presented ring map $B' \to A$ as well as a finite type
$A$-module $M$. We have to show that there is a bijective correspondence
between
\begin{enumerate}
\item surjections of $A$-modules $M \to Q$ with $Q$ of finite presentation
over $A$ and flat over $B'$, and
\item pairs consisting of a surjection of $A \otimes_{B'} B$-modules
$M \otimes_{B'} B \to Q_1$ with $Q_1$ of finite presentation over
$A \otimes_{B'} B$ and flat over $B$ and a surjection
$M \otimes_{B'} C' \to Q_2$ of $A \otimes_{B'} C'$-modules with $Q_2$
of finite presentation over $A \otimes_{B'} C'$ and flat over $C'$
such that $Q_1 \otimes_B C = Q_2 \otimes_{C'} C$ as quotients of
$M' \otimes_{B'} C$.
\end{enumerate}
Given a quotient $M \to Q$ as in (1) we set $Q_1 = Q \otimes_{B'} B$ and
$Q_2 = Q \otimes_{B'} C'$. This is a pair as in (2); we omit the details.
Conversely, given a pair $(Q_1, Q_2)$ as in (2) we denote
$M \otimes_{B'} C \to Q_{12}$ the common quotient
$Q_1 \otimes_B C = Q_2 \otimes_{C'} C$ and we set
$$
Q = Q_1 \times_{Q_{12}} Q_2
$$
which comes equipped with a map
$$
M \longrightarrow
(M \otimes_{B'} B) \times_{(M \otimes_{B'} C)} (M \otimes_{B'} C')
\longrightarrow Q_1 \times_{Q_{12}} Q_2 = Q
$$
This map is surjective (the first arrow is surjective by
More on Morphisms, Lemma
\ref{more-morphisms-lemma-module-over-fibre-product-rings-bis}
and the second by a simple argument with fibre products using that
$C' \to C$ is surjective).
By More on Morphisms, Lemma
\ref{more-morphisms-lemma-module-over-fibre-product-rings}
we see that $Q_1 = Q \otimes_{B'} B$ and $Q_2 = Q \otimes_{B'} C'$.
By More on Morphisms, Lemma
\ref{more-morphisms-lemma-flat-module-over-fibre-product-rings}
we see that $Q$ is flat over $B'$ and that this construction is the
inverse of the construction from (1) to (2) above.
To finish the proof it suffices to show that
if $(Q_1, Q_2)$ are as in (2), then the module $Q$ as defined by the
displayed equation is of finite presentation over $A$.

\medskip\noindent
To prove $Q$ is of finite presentation over $A$ we may replace $A$ by a
polynomial ring over $B'$ (write
$A = B'[x_1, \ldots, x_n]/(f_1, \ldots, f_m)$ and apply
Algebra, Lemma \ref{algebra-lemma-finite-finitely-presented-extension}
to the map $B'[x_1, \ldots, x_n] \to A$). We may (and do) replace
$M$ by $A^{\oplus r}$ for some integer $r$ as $M$ is a quotient
of $A^{\oplus r}$ for some $r$. Let $K_1$ be the kernel of
$M \otimes_{B'} B \to Q_1$. Similarly for $K_2$ and $K_{12}$.
Note that $K_1$ is a finite $A \otimes_{B'} B$-module and
$K_2$ is a finite type $A \otimes_{B'} C'$-module
(Algebra, Lemma \ref{algebra-lemma-finite-presentation-module-independent}).
By flatness of $Q$ over $B'$ we see that
$K_{12} = K_1 \otimes_{B} C = K_2 \otimes_{C'} C$
(use Algebra, Lemma \ref{algebra-lemma-flat-tor-zero}).
Set $K = K_1 \times_{K_{12}} K_2$. Since $M$ is flat over $B'$ we see that
$M = (M \otimes_{B'} B) \times_{(M \otimes_{B'} C)} (M \otimes_{B'} C')$
by More on Morphisms, Lemma
\ref{more-morphisms-lemma-flat-module-over-fibre-product-rings}.
An easy argument with fibre products now shows that the sequence
$$
0 \to K \to M \to Q \to 0
$$
is a short exact sequence of $A$-modules. Since by
More on Morphisms, Lemma
\ref{more-morphisms-lemma-finite-module-over-fibre-product-rings}
the module $K$ is a finite type $A$-module, we win.
\end{proof}
