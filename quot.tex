...
discussion with the stack $\textit{Coh}_{X/B}$
...
the stack $\textit{Coh}_{X/B}$ is representable. We prove
$\textit{Coh}_{X/B}$ is an algebraic stack in
...
$\text{Quot}_{\mathcal{F}/X/B}$, $\text{Hilb}_{X/B}$, and
$\text{Pic}_{X/B}$ are algebraic spaces and that
$\textit{Pic}_{X/B}$ is an algebraic stack. See
...
$\textit{Curves}$ parametrizing families of curves is algebraic.

\medskip\noindent
In Section \ref{section-moduli-complexes}
we study moduli of complexes on a proper morphism
and we obtain an algebraic stack $\textit{Complexes}_{X/B}$.
...
We have
$$
\Mor_{\Sh((\Sch/S)_{fppf})}(T, \text{Hilb}_{X/B}) =
\left\{
\begin{matrix}
