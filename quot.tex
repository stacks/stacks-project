...
\medskip\noindent
Before we give the details we explain the underlying idea and how we
arrived at it, since we think it will clarify what follows. Let $A$
be a finite type algebra over the locally Noetherian base $S$.
Denote $X \otimes^\mathbf{L} A$ a ``derived base change''
...
space over $B$. Denote $f_U : X \times_B U \to U$ the second projection.
Consider the distinguished triangle
$$
Lf_U^*L_{U/B} \to L_{X \times_B U/B} \to E \to Lq^*L_{U/B}[1]
$$
...
E(\mathcal{F})[1]
$$
in $D(\mathcal{O}_{X \times_B U})$. It follows from the description of
the cohomology of $E$ in Cotangent, Lemma \ref{cotangent-lemma-tensor-product}
that $E(\mathcal{F})$
has the correct cohomology sheaves in degrees $0$, $-1$, and $-2$
(we won't use this statement in the following).
By construction the Atiyah class lifts to a map
$$
e_\mathcal{F} :
E(\mathcal{F})
\longrightarrow
Lf_U^*L_{U/B} \otimes_{\mathcal{O}_{X \times_B U}}^\mathbf{L} \mathcal{F}[1]
$$
fitting into a morphism of distinguished triangles
$$
\xymatrix{
\mathcal{F} \otimes^\mathbf{L} Lf_U^*L_{U/B}[1] \ar[r] &
...
$$
(\text{id}_\mathcal{F} \otimes Lf_U^*e_{U'}) \cup e_\mathcal{F} :
E(\mathcal{F})
\longrightarrow
\mathcal{F} \otimes_{\mathcal{O}_{X \times_B U}} f_U^*\mathcal{I}[2]
$$
...
E(\mathcal{F}),
\mathcal{F} \otimes_{\mathcal{O}_{X \times_B U}} f_U^*\mathcal{I})
$$
\end{remark}

\begin{lemma}
\label{lemma-ob-is-obstruction}
In the situation of Remark \ref{lemma-ob-is-obstruction} assume that
$\mathcal{F}$ is flat over $U$. Then the vanishing of the class
$\xi_{U'}$ is a nessecary and sufficient condition for the existence of a
