...
\mathcal{H}^n =
\bigoplus\nolimits_{n = - p + q}
\SheafHom(\mathcal{E}^p, \mathcal{F}^q)
$$
...
Let $(X, \mathcal{O}_X)$ be a ringed space.
Let $K$ be a perfect object of $D(\mathcal{O}_X)$.
Then $E = R\SheafHom(K, \mathcal{O}_X)$ is a perfect object too
and we have functorial isomorphisms
$$
H^0(X, K \otimes_{\mathcal{O}_X}^\mathbf{L} L) =
\Hom_{D(\mathcal{O}_X)}(E, L)
$$
for $L$ in $D(\mathcal{O}_X)$.
\end{lemma}

\begin{proof}
Formation of internal hom commutes with restriction to opens\
(Lemma \ref{lemma-restriction-RHom-to-U}),
hence we may check that $E$ is perfect when $K$ is represented
by a strictly perfect complex. Then it follows from
Lemma \ref{lemma-Rhom-strictly-perfect}
that $E$ is represented by a strictly perfect complex too.
Moreover, there is a canonical map
$$
E \otimes^\mathbf{L}_{\mathcal{O}_X} K \longrightarrow \mathcal{O}_X
$$
coming from $E \to R\SheafHom(K, \mathcal{O}_X)$ by the equality
(\ref{equation-internal-hom}) proved in
Lemma \ref{lemma-internal-hom}.
Flipping the sides we obtain
a canonical map $K \to R\SheafHom(E, \mathcal{O}_X)$.
Computing locally once again we see this is an isomorphism.
Finally, the last equality of the lemma follows form
(\ref{equation-internal-hom}) applied to
$\mathcal{O}_X$, $E$, and $L$.
\end{proof}
