...
$\text{Ext}^i(K|_{f^{-1}V}, L|_{f^{-1}V}) = 0$ for $i < 0$.
Then $\text{Ext}^i(K|_{f^{-1}V}, L|_{f^{-1}V}) = 0$ for $i < 0$
...
\item for any $U \in \mathcal{B}$ we have $\text{Ext}^i(K_U, K_U) = 0$
for $i < 0$.
\end{enumerate}
If a solution $(K, \rho_U)$ exists, then it is unique up to unique isomorphism
and moreover $\text{Ext}^i(K, K) = 0$ for $i < 0$.
\end{lemma}
...
$$
(\{K_{U'}\}_{U' \in \mathcal{B}'}, \{\rho_{V'}^{U'}\}_{U', V' \in \mathcal{B}'})
$$
...
of elements of $B$ contained in either $U \cap U_1$ or contained in
$U \cap U_2$. Then $(K_U, \rho^U_{U'})$ is a solution for the system
$(\{K_{U'}\}_{U' \in \mathcal{B}'},
\{\rho_{V'}^{U'}\}_{U', V' \in \mathcal{B}'})$ on the ringed space $U$.
