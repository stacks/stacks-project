...
\Gamma_{Z \cap U}(U, \mathcal{F}|_U)
$$
Observe that $\mathcal{H}_Z(U)$ is a module over $\mathcal{O}_X(U)$,
i.e., $\mathcal{H}_Z(\mathcal{F})$ is an $\mathcal{O}_X$-module.
Applying Modules, Lemma \ref{modules-lemma-i-star-equivalence} to
the morphism of ringed spaces $(Z, \mathcal{O}_X|_Z) \to (X, \mathcal{O}_X)$ we
may view $\mathcal{H}_Z(\mathcal{F})$ as an $\mathcal{O}_X|_Z$-module on $Z$.
Thus we obtain a functor
$$
...
\textit{Mod}(\mathcal{O}_X|_Z)$ is right adjoint to
...
Lemma \ref{lemma-adjoint-section-with-support}
and
Derived Categories, Lemma \ref{derived-lemma-derived-adjoint-functors}.
...
(Lemma \ref{lemma-adjoint-section-with-support}) this follows from
Derived Categories, Lemma \ref{derived-lemma-adjoint-preserve-K-injectives}.
\end{proof}
