...
Denote $h : (Z', \mathcal{O}_{X'}|_{Z'}) \to (Z, \mathcal{O}_X|Z)$ be the
induced morphism of ringed spaces. For any $K$ in $D(\mathcal{O}_X)$ there
is a canonical map
$$
Lh^*R\mathcal{H}_Z(K) \longrightarrow R\mathcal{H}_{Z'}(Lf^*K)
$$
in $D(\mathcal{O}_{X'}|_{Z'})$. Denote $i : Z \to X$ and $i' : Z' \to X'$
the inclusion maps. By the adjointness of
Lemma \ref{lemma-cohomology-with-support-sheaf-on-support} to construct our
map it suffices to construct a map
$$
i'_* Lh^* R\mathcal{H}_Z(K) \longrightarrow Lf^*K
$$
The map of functors $Lf^* \circ i_* \to i'_* \circ Lh^*$ of
...
and similarly for $Z'$). Hence it suffices to construct a map
$$
Lf^* i_* R\mathcal{H}_Z(K) \longrightarrow Lf^*K
$$
for which we can use $Lf^*$ applied to the adjunction mapping
$i_*R\mathcal{H}_Z(K) \to K$.
\end{remark}
