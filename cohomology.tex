...
We want to turn this into a functor and generalize this a bit.
...
$$
for the triangulated categories introduced
in
...
\medskip\noindent
In the rest of this section we give some special cases of this construction.
...
$U \subset X$ we have a left exact functor
$
\Gamma(U, -) :
\textit{Mod}(\mathcal{O}_X)
\longrightarrow
...
\begin{proof}
We can check this by computing stalks. Thus this lemma follows
from
...
\begin{proof}
Let $\mathcal{P} \subset \text{Ob}(\textit{Mod}(\mathcal{O}_X))$ be the
class of flat $\mathcal{O}_X$-modules. By
Modules, Lemma \ref{modules-lemma-module-quotient-flat}
and
...
The induced map $\text{colim}_i\ \mathcal{K}_i^\bullet \to \mathcal{G}^\bullet$
