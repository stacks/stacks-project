...
$K^{+}(\textit{Mod}(\mathcal{O}_X))$, or $D^{+}(\textit{Mod}(\mathcal{O}_X))$
depending on the situation. In addition, in many cases we actually think of
$RF$ as a functor into $D^{+}(\mathcal{B})$, see
...
$$
which gives rise to $R\Gamma(X, -)$ and $H^i(X, -) = R^i\Gamma(X, -)$.
Slightly more generally, for any open $U \subset X$ we have
$$
\Gamma(U, -) :
\textit{Mod}(\mathcal{O}_X)
\longrightarrow
...
$$
which gives rise to $R\Gamma(U, -)$ and $H^i(U, -) = R^i\Gamma(U, -)$.
...
$$
which gives rise to $Rf_*$ and $R^if_*$. These are called the
{\it derived pushforward} and the {\it higher direct images} respectively.
According to the remarks made above we often think of
$Rf_*$ as a functor
$$
Rf_* :
D^{+}(\textit{Mod}(\mathcal{O}_X))
\longrightarrow
D^{+}(\textit{Mod}(\mathcal{O}_Y))
$$
and whenever applying the functor $Rf_*$ to a sheaf $\mathcal{F}$ or a complex
of sheaves it is understood that the localization functor $j$ has
been applied first, i.e., that $\mathcal{F}$ has been replaced by
an injective resultion of $\mathcal{F}$.
...
It should be true that such an $f$-map gives rise to
...
R\Gamma(X, \mathcal{F}))
$$
in $D^{+}(\text{Mod}(\Gamma(Y, \mathcal{O}_Y))$. Such a map does indeed
exist. It is the map obtained by taking global sections over $Y$ of
the map $\psi$ in the following lemma.
...
be a morphism of complexes. For any injective resolutions
$\alpha : \mathcal{F}^\bullet \to \mathcal{I}^\bullet$,
resp.\ $\beta : \mathcal{G}^\bullet \to \mathcal{J}^\bullet$,
there exists a canonical morphism
$\psi : \mathcal{J}^\bullet \to f_*\mathcal{I}^\bullet$
such that
$$
\xymatrix{
\mathcal{G}^\bullet \ar[r]_\varphi \ar[d]_\beta &
f_*\mathcal{F}^\bullet \ar[d]^{f_*\alpha} \\
\mathcal{J}^\bullet \ar[r]^\psi &
f_*\mathcal{I}^\bullet
}
$$
commutes. Moreover the construction is functorial in the triple
$(\mathcal{G}^\bullet, \mathcal{F}^\bullet, \varphi)$.
\end{lemma}

\begin{proof}
Consider the category $\mathcal{A}$
of triples $(\mathcal{G}, \mathcal{F}, \varphi)$,
where $\mathcal{F}$ is a sheaf of $\mathcal{O}_X$-modules,
where $\mathcal{G}$ is a sheaf of $\mathcal{O}_Y$-modules, and where
$\varphi$ is an $f$-map from $\mathcal{G}$ to $\mathcal{F}$.
A morphism
$(\mathcal{G}, \mathcal{F}, \varphi) \to
(\mathcal{G}', \mathcal{F}', \varphi')$
is a pair $(a, b)$ of morphisms $a : \mathcal{G} \to \mathcal{G}'$
and $b : \mathcal{F} \to \mathcal{F}'$ such that
$\varphi' \circ a = b \circ \varphi$. This is an abelian category
using, as is easily seen by using kernel and cokernel componentwise.

\medskip\noindent
Note that we may view the datum
$(\mathcal{G}^\bullet, \mathcal{F}^\bullet, \varphi)$
as a bounded below complex of $\mathcal{A}$.
Now we claim that (1) the category $\mathcal{A}$ has enough
injectives, and (2) for any injective object
$(\mathcal{I}, \mathcal{J}, \psi)$ of $\mathcal{A}$
the sheaves $\mathcal{I}$ and $\mathcal{J}$ are injective
sheaves (on $X$ and $Y$). It is clear that this claim implies
the lemma, by using the existence of injective resolutions,
see Homology, Lemma \ref{}.

\medskip\noindent
To prove (1) we claim that $\mathcal{A}$ the category of sheaves
of modules on a ringed site $(\mathcal{C}, \mathcal{O})$.
Namely, let the objects of $\mathcal{C}$ consist of the disjoint
union of the set of opens of $X$ and the set of opens of $Y$.
Given objects $U, V$ of $\mathcal{C}$ there is at most 1 morphism
from $U$ to $V$: (a) if $U, V \subset X$ then $U \to V$ if and only if
$U \subset V$, (b) if $U, V \subset Y$ then $U \to V$ if and only if
$U \subset V$, (c) if $U \subset X$, $V \subset Y$ then $U \to V$
if and only if $f(U) \subset V$, and (d) if $U\subset Y$, $V \subset X$
there is no morphism from $U$ to $V$. It is clear that this forms
a category with products and fibre products. We turn this into a
site by letting an open covering be an open covering either in $X$
or in $Y$ (and choosing a suitable set of coverings as in
Sites, Example \ref{} and Sets, Section \ref{}). It is clear from
the definition of an $f$-map that a sheaf of sets on this site is exactly
a triple $(\mathcal{G}, \mathcal{F}, \varphi)$,
where $\mathcal{F}$ is a sheaf of sets on $X$,
where $\mathcal{G}$ is a sheaf of sets on $Y$, and where
$\varphi$ is an $f$-map from $\mathcal{G}$ to $\mathcal{F}$.
Then the triple $(\mathcal{O}_Y, \mathcal{O}_X, f^\sharp)$ is
a sheaf of rings $\mathcal{O}$ on $\mathcal{C}$. Finally it is
clear that $\mathcal{A}$ is the category of $\mathcal{O}$-modules
on $\mathcal{C}$.

\medskip\noindent
Hence we see that $\mathcal{A}$ has enough injectives, by
Injectives, Lemma \ref{}. We omit the proof of (2).
\end{proof}
...
\section{Two basic spectral sequences}
\label{section-basic-spectral-sequences}
...
as a sheaf of abelian groups. However, each term
...
$h(s)_{i_0 \ldots i_{p + 1}} = (i_0 = i_{\text{fix}}) s_{i_1 \ldots i_p}$ for this.
Then we compute
\begin{eqnarray*}
...
\section{Cech cohomology for sheaves of modules}
\label{section-cech-cohomology-sheaves}
