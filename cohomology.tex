...
According to the remarks made above we think of $Rf_*$ as an exact functor
$$
Rf_* :
D^{+}(X)
\longrightarrow
...
\medskip\noindent
{\bf Abuse of notation:} Whenever applying the functor $Rf_*$, or another
derived functor to a sheaf
$\mathcal{F}$ or a complex of sheaves it is understood that the localization
functor $j_X$ has been applied first, i.e., that $\mathcal{F}$ has been
replaced by an injective resolution of $\mathcal{F}$. More generally
we will say ``let $\alpha : \mathcal{F}^\bullet \to \mathcal{G}^\bullet$ be
a morphism of $D^{+}(X)$'' to be synonomous to
...
R\Gamma(X, \mathcal{F})
$$
in $D^{+}(\text{Mod}(\Gamma(Y, \mathcal{O}_Y))$. In particular it gives
rise to maps on cohomology
$$
\varphi : H^i(Y, \mathcal{G}) \longrightarrow H^i(X, \mathcal{F}).
$$
The construction of these maps is somewhat confusing in the most
general setting, because in general it is not easy to explicitly write
a map of complexes realizing this map (i.e., some quasi-isomorphisms
have to be inverted).
...
$$
is a morphism in $K^{+}(\textit{Mod}(\mathcal{O}_Y))$ which turns
into the first displayed morphism of the lemma upon applying the
