...
\section{Flasque or flabby sheaves}
...
Let $(X, \mathcal{O}_X)$ be a topological space.
Let $\mathcal{F}$ be a sheaf of $\mathcal{O}_X$-modules.
...
\item there exists a basis of quasi-compact open subsets, and
\item the intersection of any two quasi-compact opens is quasi-compact.
\item $H^p(X, \mathcal{F}) = 0$ for $p \geq n$ for any abelian sheaf
$\mathcal{F}$ which is a quotient of $j_!\underline{\mathbf{Z}}_U$
for some open $j : U \to X$.
\end{enumerate}
...
$$
\Hom_{D(X)}(Lf^*\mathcal{G}^\bullet, \mathcal{F}^\bullet) =
\Hom_{D(X)}(f^*\mathcal{K}^\bullet, \mathcal{I}^\bullet) =
\Hom_{K(\textit{Mod}(\mathcal{O}_X))}(f^*\mathcal{K}^\bullet,
\mathcal{I}^\bullet)
$$
...
\begin{align*}
\Hom_{K(\textit{Mod}(\mathcal{O}_Y))}((\mathcal{K}')^\bullet,
f_*\mathcal{I}^\bullet)
& =
\Hom_{K(\textit{Mod}(\mathcal{O}_Y))}(f^*(\mathcal{K}')^\bullet,
\mathcal{I}^\bullet) \\
& =
\Hom_{K(\textit{Mod}(\mathcal{O}_Y))}(f^*\mathcal{K}^\bullet,
\mathcal{I}^\bullet)
...
\item $H^p(U, \text{Coker}(\mathcal{F}^{q - 1} \to \mathcal{F}^q)) = 0$, and
...
$j_!\mathcal{O}_U \otimes_{\mathcal{O}_X}^\mathbf{L} K = j_!(K|_U)$.
To see this use that $j_!\mathcal{O}_U$ is flat, see
Modules, Lemma \ref{modules-lemma-j-shriek-flat}. Some details omitted.
\end{proof}
...
\noindent
n this section we study compact objects in the derived category of modules
