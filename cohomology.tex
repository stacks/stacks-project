...
$D(\mathcal{O}_X)$. The sheafification of $U \mapsto H^q(U, K)$
...
associated to the filtred complex
$$
...
and the $\Hom$'s and $\text{Ext}$'s are taken in the relevant
derived categories.
\end{lemma}
...
Assume there exists a set $\mathcal{B}$ of opens of $X$ such that
\begin{enumerate}
...
R\SheafHom(L, M) \otimes_{\mathcal{O}_X}^\mathbf{L} L \ar[d] \\
R\SheafHom(R\SheafHom(L, L), M) \ar[d] \\
M
}
$$
gotten by applying Lemma \ref{lemma-internal-hom-evaluate} twice
and using the maps $\mathcal{O}_X \to R\SheafHom(K, K)$ and
$\mathcal{O}_X \to R\SheafHom(L, L)$.
...
$$
\mathcal{E}\!{\it xt}^n(K, L) = H^n(R\SheafHom(K, L))
$$
by taking the $n$th cohomology sheaf of the object $R\SheafHom(K, L)$
of $D(\mathcal{O}_X)$. We will sometimes write
$\mathcal{E}\!{\it xt}^n_{\mathcal{O}_X}(K, L)$ for this object.
By Lemma \ref{lemma-section-RHom-over-U}
we see that this $\mathcal{E}\!{\it xt}^n$-sheaf
is the sheafification of the rule
$$
U \longmapsto \text{Ext}^n_{D(\mathcal{O}_U)}(K|_U, L|_U)
$$
By Example \ref{example-spectral-sequence} there is always a spectral
sequence
$$
E_2^{p, q} = H^p(X, \mathcal{E}\!{\it xt}^q(K, L))
$$
converging to $\text{Ext}^{p + q}_{D(\mathcal{O}_X)}(K, L)$
...
$R\Hom_X(K, L)$ or $R\Hom_{\mathcal{O}_X}(K, L)$ for this object.
By Lemma \ref{lemma-section-RHom-over-U}
we have
$$
H^0(R\Hom(K, L)) = \Hom_{D(\mathcal{O}_X)}(K, L)
$$
and
$$
H^p(R\Hom(K, L)) = \text{Ext}_{D(\mathcal{O}_X)}^p(K, L)
$$
