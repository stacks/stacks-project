...
thesis \cite{Joe}. Given any field $\kappa$ and
...
for $i = 1, \ldots, l$.
A {\it symbol} with be an expression $\{e_1, \ldots, e_l\}$ where
...
\medskip\noindent
Type (2) relations. Suppose that $(e_1, \ldots, e_l)$
...
\begin{definition}
\label{definition-determinant-finite-length}
Let $R$ be a local ring with maximal ideal $\mathfrak m$ and
residue field $\kappa$. For any finite length $R$-module $M$
we set
$$
\det\nolimits_\kappa(M)
=
\bigotimes\nolimits_n
\det\nolimits_\kappa( \mathfrak m^n M/ \mathfrak m^{n + 1}M)
$$
This makes sense since by Algebra, Lemma \ref{algebra-lemma-length-infinite}
we have $\mathfrak m^nM = 0$ for $n \gg 0$ and by
Algebra, Lemma \ref{algebra-lemma-dimension-is-length} we have that all
the graded pieces $\mathfrak m^n M/ \mathfrak m^{n + 1}M$ are
finite dimensional.
\end{definition}

\noindent
Note that if $\varphi : M \to N$ is an isomorphism, then we get an
isomorphism $\det\nolimits_\kappa(\varphi) : \det\nolimits_\kappa(M) \to \det\nolimits_\kappa(N)$.
Hence we see that $\det\nolimits_\kappa$ is a functor from the category of
finite length $R$-modules with isomorphisms, to the category of
$1$-dimensional $\kappa$-vector spaces with isomorphisms. This is
typical for a ``determinant functor'', as is the following additivity
property.

Let $(R, \mathfrak m, \kappa)$ be a local ring.
Let $M$ be an $R$-module of length $l = \text{length}_R(M) < \infty$.


\noindent
Note that this implies
$\mathfrak m e_i \in \langle e_1, \ldots, e_{i - 1}\rangle$
for $i = 1, \ldots, l$. Given such a quasi-basis we can define
a function $\alpha : M \to \{0, \ldots, l\}$ by the rule
$$
\alpha(x) = a \Leftrightarrow
x \in \langle e_1, \ldots, e_a\rangle,\text{ and }
x \not \in \langle e_1, \ldots, e_{a - 1}\rangle.
$$
Given an element $x \in M$ a {\it reduced representation} of $x$
with respect to the quasi-basis is a sequence
$r_1, \ldots, r_{\alpha(x)} \in R$ such that
such that $x = \sum_{i = 1, \ldots, \alpha(x)} r_i e_i$.
Note that this implies $r_{\alpha(x)} \in R$ is a unit.
In general a reduced expression is {\bf not} unique (contrary to
the case of vector spaces).

\begin{lemma}
\label{lemma-determinant-well-defined}
Let $(R, \mathfrak m, \kappa)$ be a local ring.
Let $M$ be an $R$-module of length $l = \text{length}_R(M) < \infty$.
Let $(e_1, \ldots, e_l)$, $(f_1, \ldots, f_l)$ be quasi-bases of $M$.
Let
$$
f_i = \sum\nolimits_{j = 1, \ldots, \alpha(f_i)} r_{ij}e_j,
\quad i = 1, \ldots, l
$$
be any reduced expressions. Then the image of the
determinant of the $l \times l$-matrix $(r_{ij})$ in $\kappa$
is invertible and is independent of the choices made in the reduced
expressions displayed above.
\end{lemma}

\begin{proof}
Before we prove the lemma we will show that there are certain
operations on the quasi-basis $(e_1, \ldots, e_l)$ that we may
perform without changing the truth value of the lemma.
For example, suppose that we have some $a \in \{1, \ldots, l\}$
and an element $e'_a \in M$ with $\alpha(e'_a) = a$.
In other words
$$
e'_a = r_1e_1 + \ldots + r_ae_a
$$
and $r_a \in R^*$. In this case
$(e_1, \ldots, e_{a - 1}, e'_a, e_{a + 1}, \ldots, e_l)$
is also a quasi-basis for $M$. Moreover, suppose we have
a quasi-basis $(f_1, \ldots, f_l)$ as in the lemma, and suppose
we have reduced expressions with coefficients $r_{ij}$ as in the lemma.
Then for $i$ such that $\alpha(f_i) \geq a$ we have the equality
\begin{eqnarray*}
f_i & = & 
(r_{i1} - r_1r_{ia}/r_a)e_1 + \ldots
+ (r_{ia - 1} - r_{a - 1}r_{ia - 1}/r_a)e_{a - 1} \\
& & + (r_{ia}/r_a)e'_a +
\sum\nolimits_{j = a + 1, \ldots, \alpha(f_i)} r_{ij}e_j
\end{eqnarray*}
which is a reduced expression also.
If $\alpha(f_i) < a$, then we keep the same reduced expression
$$
f_i = \sum\nolimits_{j = 1, \ldots, \alpha(f_i)} r_{ij}e_j,
$$
Let $(r'_{ij})$ be the matrix of coefficients
coming from the formulas displayed in this paragraph.
Then we see that
$$
\det(r_{ij}) = r_a\det(r'_{ij}).
$$
Hence if the lemma holds for the quasi-basis
$(e_1, \ldots, e_{a - 1}, e'_a, e_{a + 1}, \ldots, e_l)$
then it holds for the quasi-basis $(e_1, \ldots, e_l)$.
(The converse is true as well as can be seen by running the
argument backwards for example.)

\medskip\noindent
Here is another permissible operation. Namely, suppose that
$(e_1, \ldots, e_l)$ is a quasi-basis and $l \leq a > 1$
is an index such that $\mathfrak m e_a = 0$. Then we claim that
$(e_1, \ldots, e_{a - 2}, e_a, e_{a - 1}, e_{a + 1}, \ldots, e_l)$
is a quasi-basis as well, and if the lemma holds for this
quasi-basis then it holds for the original one.
Namely, suppose we are given a quasi-basis $(f_1, \ldots, f_l)$
as in the lemma, and suppose we have reduced expressions with
coefficients $r_{ij}$ as in the lemma. Then the same expression
with the order of $e_a$ and $e_{a - 1}$ is also a reduced
expression unless $\alpha(f_i) = a$ and $r_{i a - 1} \in \mathfrak m$.
But if so, then we can write
$$
r_{i a - 1}e_{a - 1} = c_{i1}e_1 + \ldots + c_{i a - 2}e_{a - 2}
$$
for some $c_{ij} \in R$
by definition of a quasi-basis. Hence, still in the case
$\alpha(f_i) = a$ and $r_{i a - 1} \in \mathfrak m$, we can write
$$
f_i =
(r_{i1} + c_{i1}) e_1 + 
\ldots + 
(r_{i a - 2} + c_{i a - 2}) e_{a - 2} + r_{ia} e_a
$$



\medskip\noindent
We will prove this by induction on the length $l$ of the module
$M$. If $l = 1$, then any reduced expression $f_1 = r_{11} e_1$
will necessarily have $r_{11} \in R^*$. Moreover all the elements
$r_{11}$ will reduce to the same element in $\kappa^*$ since
actually in this case $e_1$ is a basis for the $1$-dimensional
vector space $M$.

\medskip\noindent
Suppose that $l > 1$. Set $a = \alpha(f_1)$. 
...
$R$-module $M$, denote $\text{soc}(M)$ the {\it socle} of $M$.
This is the submodule of elements killed by $\mathfrak m$, so
$$
\text{soc}(M) = \{x \in M \mid \mathfrak m x = 0\}.
$$
...
\begin{proof}
There should be a beautiful way of proving this; please email
\href{mailto:stacks.project@gmail.com}{stacks.project@gmail.com}
if you know such a proof.


Consider a short exact sequence $0 \to K \to L \to M \to 0$
as in the statement of the lemma. Choose a filtration
$$
0 = W_0 \subset W_1 \subset W_2 \subset \ldots \subset W_l = L
$$
of $M$ such that $W_i/W_{i - 1} \cong \kappa$ as an $R$-module
and such that $K = W_k$ for some $0 \leq k \leq l$. Such a filtration
exists because both $K$ and $M$ have finite length.
Note that $k = \text{length}_R(K)$ and $l = \text{length}_R(L)$.
Now we claim that there are canonical isomorphisms
$$
K = \bigotimes\nolimits_{i = 1}^k W_i/W_{i - 1}, \quad
L = \bigotimes\nolimits_{i = 1}^l W_i/W_{i - 1}, \quad
M = \bigotimes\nolimits_{i = k + 1}^l W_i/W_{i - 1}.
$$
Having these isomorphisms will give us a map $\gamma_{K \to L \to M}$,
which might a priori still depend on the choice of the filtration.
...
\left\{
\begin{matrix}
\text{1-dimensional }\kappa\text{-vector spaces} \\
\text{with isomorphisms}
\end{matrix}
\right\}
$$
endowed with the maps $\gamma_{K \to L \to M}$ satsifying (1), (2) and
(3) of Lemma \ref{lemma-det-exact-sequences} is unique up to unique isomorphism.
\end{lemma}
...
\section{Period complexes}
\label{section-periodic-complexes}

\noindent
Of course there is a very generic notion of periodic complexes.
We will add these here as needed. For the moment we only need
the following case.

\begin{definition}
\label{definition-cyclic-complex}
Let $R$ be a ring. A {\it $(2, 1)$-periodic complex} over $R$ is given
by a triple $(M, \varphi, \psi)$ consisting of an $R$-module $M$ and
$R$-module maps maps $\varphi : M \to M$ such that
\begin{equation}
...
\end{equation}
is a complex. In this setting we define the {\it cohomology groups}
...
$$
We say the periodic complex is {\it exact} if the cohomology groups are zero.
\end{definition}

\noindent
It is clear that the collection of $(2, 1)$-periodic complexes forms a
category. In fact it is an abelian category, with the obvious notions
of kernel and cokernel.
