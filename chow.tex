...
$Y_\infty = q^{-1}D_\infty$ as closed subscheme of $X$
via the morphism $p$ then we have
$$
\text{div}(f) = [Y_0]_{n - 1} - [Y_\infty]_{n - 1}
$$
\end{enumerate}
\end{lemma}
...
the closure of the one point set $\{\xi'\} = \{(1, f)(\xi)\}$
where $\xi \in X$ is the generic point. Having said this let us
...
\medskip\noindent
Note that $q(\xi') = f(\xi)$ is not contained in $D_0$ or $D_\infty$
