...
$$
where $x_i$, $y_j$ are the chern roots of $\mathcal{E}$, $\mathcal{F}$.
We claim that
$$
...
$$
Namely, if $\mathcal{E}$ has a filtration whose successive quotients are
invertible modules $\mathcal{L}_i$ and $\mathcal{F}$ has a filtration whose
successive quotients are invertible sheaves $\mathcal{N}_j$, then
$\mathcal{E} \otimes \mathcal{F}$ has a filtration whose successive
quotients are the invertible sheaves $\mathcal{L}_i \otimes \mathcal{N}_j$.
Applying the above we find expressions for the chern classes
of the tensor product in terms of the chern classes of $\mathcal{E}$
and $\mathcal{F}$. Here are some examples:
$$
...
P(\mathcal{E})
$$
where $P = 2r^2c_2 - (r^2 - 1)c_1^2$.
Namely, since $\SheafHom_{\mathcal{O}_X}(\mathcal{E}, \mathcal{E}) =
\mathcal{E} \otimes \mathcal{E}^\vee$ this follows easily from the
calculations above by decomposing $X$ into parts where the rank
...
such that, for any $n \geq 1$ we have
$$
...
all symmetric functions over the integers.
Since the chern classes of a finite locally free $\mathcal{O}_X$-module
$\mathcal{E}$ are the elementary symmetric polynomials in the chern roots
$x_i$, we see that
$$
...
\end{align*}
and so on. For convenience we set
$P_0 = r$ in $\mathbf{Z}[r, c_1, c_2, c_3, \ldots]$ so that
$P_0(\mathcal{E}) = r(\mathcal{E})$ as a bivariant class
(see above).
\end{example}
...
$\mathcal{O}_X$-module of rank $r$ to be the formal expression
$$
...
\ldots \\
& =
\sum\nolimits_{p = 0, 1, 2, \ldots} \frac{1}{p!}P_p(\mathcal{E})
\end{align*}
...
Let $\mathcal{E}$ be a finite locally free $\mathcal{O}_X$-module.
If the rank of $\mathcal{E}$ is not constant then we can
still define the Chern character $ch(\mathcal{E})$ of $\mathcal{E}$
by the formula $\sum (1/p!)P_p(\mathcal{E})$.
This is clear from the discussion in Section \ref{section-splitting-principle}.
\end{remark}
...
$$
in $A^*(X) \otimes \mathbf{Q}$. More precisely, if
...
$$
The formula makes sense because we've seen that the total
chern class is multiplicative in short exact sequence
...
(-1)^p ch(\mathcal{E}^p)
$$
in $A^*(X) \otimes \mathbf{Q}$. This makes sense because
we've seen that the chern character is additive in short exact sequence
(Lemma \ref{lemma-chern-character-additive}). Similarly, for
...
formally from this case. Suppose we have a second finite complex
...
$\mathcal{O}_X$-modules, then we say
{\it total chern class $c(E)$ is defined},
{\it the chern character $ch(E)$ is defined}, or
{\it the bivariant class $P_p(E)$ is defined}. If this is the case,
then we define $c(E) = c(\mathcal{E}^\bullet) \in A^*(X)$,
...
$E_2 \in D(\mathcal{O}_{X_2})$ be a perfect object whose
restriction to $X_1 \cap X_2$ is zero. If $P_p(E_2)$ is defined, then
there is a canonical bivariant class
$$
c \in A^p(X_2 \to X)
$$
characterized by the property
$c \cap i_{2, *} \alpha_2 = P_p(E_2) \cap \alpha_2$
for $\alpha_2 \in A_k(X_2)$ and similarly after any base change by $X' \to X$
locally of finite type.
\end{lemma}

\begin{proof}
We are going to use the material of Section \ref{section-pre-derived}
without further mention. Consider a morphism of schemes $X' \to X$
...
$c \cap \alpha' = P_p(E_2) \cap \alpha'_2 \in A_{k - p}(X')$. This
...
perfect $E \in D(\mathcal{O}_X)$ with $E|_{X_1}$ equal to zero.
If $P_p(E)$ is defined, then
$i_{2, *}(c \cap \alpha) = P_p(E) \cap \alpha$
for all $\alpha \in A_k(X)$ and similarly after any base change by
...
and let $U = X \setminus Z$ be the complementary open subscheme. Let
$$
...
We assume
\begin{enumerate}
\item[(A0)] $P_p(Q)$ is defined (Section \ref{section-pre-derived}),
\item[(A1)] $b$ is an isomorphism over $\mathbf{A}^1_X$,
\item[(A2)] there exists a closed subscheme $T \subset W_\infty$
containing all points of $W_\infty$ lying over $U$ such that
$Q|_T$ is zero.
\end{enumerate}
We denote $E \subset W_\infty$ the inverse image of $Z$. Then
$W_\infty = T \cup E$ and $b$ induces a proper morphism $E \to Z$.
Then there exists a canonical bivariant class
$$
c \in A^p(Z \to X)
$$
such that $(Z \to X)_*(c \cap \alpha) = P_p(Q|_{X \times \{0\}}) \cap \alpha$
for $\alpha \in A_k(X)$ and similarly after any base change by $X' \to X$
locally of finite type.
\end{lemma}

\begin{proof}
Denote $c' \in A^p(E \to W_\infty)$ the bivariant class constructed
in Lemma \ref{lemma-silly} using $Q|_E$.
Denote $c'' \in A^0(W_\infty \to X)$ the bivariant class constructed
in Lemma \ref{lemma-gysin-at-infty}.
Then the operation
$$
c \cap - : \alpha \longmapsto (E \to Z)_*(c' \cap c'' \cap \alpha)
$$
and similarly after any base change is a bivariant class in $A^p(Z \to X)$
by Lemma \ref{lemma-push-proper-bivariant}. Since $E \to Z \to X$ is
equal to $E \to W_\infty \to W \to X$ we see that
\begin{align*}
(Z \to X)_*(c \cap \alpha)
& =
(W \to X)_*i_{\infty, *}(E \to W_\infty)_*(c' \cap c'' \cap \alpha) \\
& =
(W \to X)_*(i_{\infty, *}(P_p(Q) \cap c'' \cap \alpha) \\
& =
(W \to X)_*(P_p(Q) \cap i_{\infty, *}(c'' \cap \alpha)) \\
& =
(W \to X)_*(P_p(Q) \cap i_{0, *}(\alpha)) \\
& =
(W \to X)_*i_{0, *}(P_p(Q|_{X \times \{0\}}) \cap \alpha) \\
& =
P_p(Q|_{X \times \{0\}}) \cap \alpha
\end{align*}
The second equality holds by Lemma \ref{lemma-silly-silly}.
The third equality because $P_p(Q)$ is a bivariant class.
The fourth equality by Lemma \ref{lemma-gysin-at-infty}.
The fifth equality because $P_p(Q)$ is a bivariant class.
The final equality because $(W_0 \to W) \circ (W \to X)$
is the identity on $X$ if we identify $W_0$ with $X$ as we did
done above.
\end{proof}
