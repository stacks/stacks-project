...
such that $a, b$ are nonzero divisors on $M$, then
$\text{length}_A(M/abM) < \infty$ and
$(M/abM, a, b)$ is a $(2, 1)$-periodic exact complex.
\item if $a, b$ are nonzero divisors and $\dim(A) = 1$
...
Let $a, b \in A$.
Let $M$ be a finite $A$-module of dimension $1$
such that $a, b$ are nonzero-divisors on $M$.
...
with $\dim(M) = 1$. Assume $a, b, c$ are nonzero divisors on $M$.
Then
$$
...
Let $A$ be a Noetherian local ring.
Let $M$ be a finite $A$-module of dimension $1$.
Let $b \in A$ be a nonzero divisor on $M$, and let $u \in A^*$.
Then
$$
...
such that $a, b$ are nonzero divisors on
all three $A$-modules.
Then
$$
...
Let $a, b \in A$. Assume
\begin{enumerate}
\item $a$, $b$ are nonzero divisors on both $M$ and $M'$, and
...
length $1$. Since $a, b \in \mathfrak m_A$, they act trvially on
...
Let $a, b \in A$ nonzero divisors on $M$.
...
The morphism $X \to \text{Spec}(R)$ is projective by
Morphisms, Lemma \ref{morphisms-lemma-blowing-up-projective}.
By Lemma \ref{lemma-finite-in-codim-1} and the fact that
$X$ is quasi-compact we see that the fibre of $X \to \text{Spec}(R)$
...
Hence $X = U$ because $X \to \text{Spec}(R)$ is closed.
In other words $X$ is affine, say $X = \text{Spec}(R')$.
By Morphisms, Lemma \ref{morphisms-lemma-locally-finite-type-characterize}
we see that $R \to R'$ is of finite type. Since $X \to \text{Spec}(R)$ is
...
which each of $a$, $b$, $b - a$ are nonzero divisors.
Then
$$
...
0
$$
since at any height one prime of $A$ where $f, g \in A^*_{\mathfrak q}$
are both units we have $d_{A_{\mathfrak q}}(f, g) = 1$ by
...
are additive (Lemma \ref{lemma-length-additive})
