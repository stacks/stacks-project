...
resp.\ $c \cap i_{2, *} \alpha_2 = c_p(E_2) \cap \alpha_2$
for $\alpha_2 \in A_k(X_2)$
and similarly after any base change $X' \to X$ locally of finite type.
\end{lemma}
...
$c \cap \alpha' = P_p(E_2) \cap \alpha'_2 \in A_{k - p}(X'_2)$. This
...
In Lemma \ref{lemma-silly} the bivariant class $c \in A^p(X_2 \to X)$
...
$c \cap \alpha = P_p(E_2) \cap \alpha_2 \in A_{k - p}(X_2)$,
resp.\  $c \cap \alpha = c_p(E_2) \cap \alpha_2 \in A_{k - p}(X_2)$,
...
resp.\ $i_{2, *}(c \cap \alpha) = c_p(E) \cap \alpha$,
...
resp.\ $(Z \to X)_*(c \cap \alpha) = c_p(Q|_{X \times \{0\}}) \cap \alpha$
for $\alpha \in A_k(X)$ and similarly after any base change by $X' \to X$
locally of finite type.
\end{lemma}

\begin{proof}
We will prove this in case $Q|_T$ is zero; the other case is proved in
exactly the same way.

\medskip\noindent
Denote $E \subset W_\infty$ the inverse image of $Z$. Then
$W_\infty = T \cup E$ and $b$ induces a proper morphism $E \to Z$.
Denote $c_1 \in A^0(W_\infty \to X)$ the bivariant class constructed
in Lemma \ref{lemma-gysin-at-infty}.
Denote $c_2 \in A^p(E \to W_\infty)$ the bivariant class constructed
in Lemma \ref{lemma-silly} using $Q|_E$.
Then the operation
$$
c \cap - : \alpha \longmapsto (E \to Z)_*(c_2 \cap c_1 \cap \alpha)
$$
...
(Z \to X)_*(c \cap \alpha)
& =
(W \to X)_*i_{\infty, *}(E \to W_\infty)_*(c_2 \cap c_1 \cap \alpha) \\
& =
(W \to X)_*(i_{\infty, *}(P_p(Q) \cap c_1 \cap \alpha) \\
& =
(W \to X)_*(P_p(Q) \cap i_{\infty, *}(c_1 \cap \alpha)) \\
& =
...
In Lemma \ref{lemma-localized-chern-pre} the bivariant class $c$
is independent of the choice of the closed subscheme $T$.
Moreover, given a proper morphism $g : W' \to W$ which is an
isomorphism over $\mathbf{A}^1_X$, the bivariant operation
$c'$ constructed using $W'$ and $Q' = g^*Q$ equals $c$.
\end{lemma}
...
$c' \in A^p(Z \to X)$ corresponding to $b' = b \circ g : W' \to \mathbf{P}^1_X$
and $Q'$ is defined. Recall that
...
\item the restriction $E|_{X \setminus Z}$ is zero, resp.\ is isomorphic to a
...
\end{equation}
as bivariant classes on $X$
(where the left hand side refers to the construction in
...
by construction, we finally conclude that bivariant class constructed in
Lemma \ref{lemma-localized-chern-pre}
using $W, b, \mathcal{Q}^\bullet, T'$ gives us classes
