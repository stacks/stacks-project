...
be the zero section. By Lemma \ref{lemma-linebundle-formulae} we have
$$
...
- o_*(c \cap \alpha) \\
& =
p^*(c_1(\mathcal{L}) \cap c \cap \alpha)
\end{align*}
The last equality holds by the first displayed equality
with $\alpha$ replaced by $c \cap \alpha$.
Since $p^*$ is injective by a lemma cited above we get that
$c_1(\mathcal{L}$ is in the center of $A^*(X)$.
\end{proof}
...
Then $c_j(\mathcal{E}) \in A^j(X)$ commutes with every
...
$$
0 \to \mathcal{C}^\vee \to \mathcal{N} \to \mathcal{E} \to 0
$$
be a short exact sequence of finite locally free $\mathcal{O}_X$-modules.
Consider the closed embedding
$$
C = \underline{\text{Spec}}_X(\text{Sym}(\mathcal{C}))
\longrightarrow
N = \underline{\text{Spec}}_X(\text{Sym}(\mathcal{N}^\vee))
$$
Assume $\dim_\delta(X) = n$ and $\mathcal{C}$ constant rank $r$.
Then we have
$$
[C]_{n + r} = p^*(c_{top}(\mathcal{E}) \cap [X]_n)
$$
where $p : N \to X$ is the structure morphism.
\end{lemma}
...
has constant rank $r'$. Observe that $p^*\mathcal{E}$ has a canonical section
$$
...
is exactly $C \subset N$. Hence we see that
$$
[C]_{n + r} = c_{r'}(p^*\mathcal{E}) \cap [N]_{n + r + r'}
$$
by Lemma \ref{lemma-top-chern-class}.
Since $[N]_{n + r + r'} = p^*[X]_n$ we conclude by the fact that
chern classes are bivariant classes.
\end{proof}
...
\noindent
In this section we will use the polynomials
$P_p \in \mathbf{Z}[r, c_1, c_2, c_3, \ldots]$
introduced in Example \ref{example-power-sum}.
...
\section{Relations between some bivariant classes}
\label{section-relations-pre}

\noindent
In this section we prove some technical but essentially trivial
lemmas concerning the bivariant classes construction in
Section \ref{section-preparation-localized-chern}.
In fact, a bunch of these lemmas can be somewhat sharpened, but
as they'll be trumped by later results on localized chern
classes we do not worry overmuch.
...
\end{align*}
For the first we use the usual composition of bivariant classes, see
Section \ref{section-bivariant}. For the second we use restriction
$A^i(X) \to A^i(Z)$ (Remark \ref{remark-restriction-bivariant}) and the usual
composition $A^i(Z) \times A^j(Z \to X) \to A^{i + j}(Z \to X)$.
For the third, we send $(c, c')$ to $c \circ (Z \to X)_* \circ c'$
where the bivariant class $(Z \to X)_* \circ c'$ is the one from
Lemma \ref{lemma-push-proper-bivariant}. An alternative is to
use the restriction map $res : A^i(Z \to X) \to A^i(Z)$ (see
Remark \ref{remark-restriction-bivariant}) and
send $(c, c')$ to $res(c) \circ c'$. This gives the same result
because $res(c) = c \circ (Z \to X)_*$ on cycle classes.
We omit the pleasant verification that these multiplications
are associative in a suitable sense.
\end{remark}
...
\item the restrictions $E|_{X_1}$ and $F|_{X_1}$
...
As a curiosity, we remark that this lemma is true even if one only
assumes condition (2) for the restrictions of $E$ and $F$ to $X_1 \cap X_2$.
\end{proof}
...
\begin{proof}
We may prove the equality of bivariant classes one cycle class at a time
(see Lemma \ref{lemma-bivariant-zero}) and the construction of
these classes is compatible with base change. Hence we may assume
we have some $\alpha \in A_k(X)$ and we are trying to show that
$C \cap (Z \to X)_*(c'_p(Q) \cap \alpha) =
c_p(Q|_{W_\infty}) \cap C \cap \alpha = C \cap c_p(Q|_{X \times \{0\}}$.
As in the construction of $C \cap \alpha$, let
$\beta \in A_{k + 1}(W)$ be an element whose restriction
to $b^{-1}(\mathbf{A}^1_X)$ is the flat pullback of $\alpha$.
Then
$$
\text{flat pullback of }
c_p(Q|_{X \times \{0\}}) \cap \alpha
=
\text{restriction of }
c_p(Q) \cap \beta
\text{ to }b^{-1}(\mathbf{A}^1_X)
$$
in $A_{k + 1 - p}(\mathbf{A}^1_X)$ because they agree
after applying the gysin homomorphism
$i_0^* : A_{k + 1 - p}(\mathbf{A}^1_X) \to A_{k - p}(X)$
which is bijective
...
C \cap (Z \to X)_*(c'_p(Q) \cap \alpha)
& =
C \cap (c_p(Q|_{X \times \{0\}}) \cap \alpha) \\
& =
...
\end{align*}
The first equality is the final statement of
Lemma \ref{lemma-localized-chern-pre}.
The second equality is the definition of $C$ combined with the displayed
equation above. The third equality holds because $i_\infty^*$ is a
bivariant class. The fourth equality is the definition of $C$.
\end{proof}
...
For $r > 1$ note that the multiplication in
...
b : W \longrightarrow \mathbf{P}^1_X
$$
be a proper morphism of schemes. Let $Q_1, Q_2 \in D(\mathcal{O}_W)$ be
perfect objects. Denote $W_\infty \subset W$ the inverse image of the divisor
$D_\infty \subset \mathbf{P}^1_X$ with complement $\mathbf{A}^1_X$.
We assume
\begin{enumerate}
\item[(A0)] chern classes of $Q_1$ and $Q_2$ are defined
(Section \ref{section-pre-derived}),
\item[(A1)] $b$ is an isomorphism over $\mathbf{A}^1_X$,
\item[(A2)] there exists a closed subscheme $T \subset W_\infty$
containing all points of $W_\infty$ lying over $X \setminus Z$ such that
$Q_1|_T$ and $Q_2|_T$ are isomorphic to finite locally free
$\mathcal{O}_T$-modules of rank $< p_1$ and $< p_2$ placed in degree $0$.
\end{enumerate}
With notation as in Remark \ref{remark-ring-loc-classes} set
$$
c^{(p_1)}(Q_1) = 1 + c_1(Q_1|_{X \times \{0\}}) + \ldots +
c_{p_1 - 1}(Q_1|_{X \times \{0\}}) +
c'_{p_1}(Q_1) + c'_{p_1 + 1}(Q_1) + \ldots
$$
in $A^{(p)}(Z \to X)$ with $c'_p(Q_1)$ for $p \geq p_1$ as in
Lemma \ref{lemma-localized-chern-pre}. Similarly for $c^{(p_2)}(Q_2)$ and
$c^{(p_1 + p_2)}(Q_1 \oplus Q_2)$.
Then $c^{(p_1 + p_2)}(Q_1 \oplus Q_2) = c^{(p_1)}(Q_1)c^{(p_2)}(Q_2)$
in $A^{(p_1 + p_2)}(Z \to X)$.
\end{lemma}

\begin{proof}
Recall that the image of $c'_p(Q_1)$ in $A^p(X)$ is equal to
$c_p(Q_1|_{X \times \{0\}})$ for $p \geq p_1$ and similarly for
$Q_2$ and $Q_1 \oplus Q_2$, see Lemma \ref{lemma-localized-chern-pre}.
Hence the equality in degrees $< p_1 + p_2$ follows from the
additivity of Lemma \ref{lemma-additivity-on-perfect}.

\medskip\noindent
Let's take $p \geq p_1 + p_2$.
...
$$
c^{(p_1 + p_2)}(Q_1|_{W_\infty} \oplus Q_2|_{W_\infty}) =
c^{(p_1)}(Q_1|_{W_\infty})c^{(p_2)}(Q_2|_{W_\infty})
$$
in $A^{(p_1 + p_2)}(E \to W_\infty)$ by Lemma \ref{lemma-silly-sum-c}.
Since by construction
$$
c'_p(Q_1 \oplus Q_2) = (E \to Z)_* \circ
c'_p(Q_1|_{W_\infty} \oplus Q_2|_{W_\infty}) \circ C
$$
we conclude that suffices to show for all $i + j = p$ we have
$$
(E \to Z)_* \circ
c^{(p_1)}_i(Q_1|_{W_\infty})c^{(p_2)}_j(Q_2|_{W_\infty})
\circ C
=
c^{(p_1)}_i(Q_1)c^{(p_2)}_j(Q_2)
$$
in $A^{i + j}(Z \to X)$ where the multiplication is the one from
Remark \ref{remark-ring-loc-classes} on both sides. There are
three cases, depending on whether $i \geq p_1$, $j \geq p_2$,
or both.

\medskip\noindent
Assume $i \geq p_1$ and $j \geq p_2$. In this case the products are
defined by inserting $(E \to W_\infty)_*$, resp.\ $(Z \to X)_*$ in between
the two factors and taking compositions as bivariant classes.
In other words, we have to show
$$
(E \to Z)_* \circ c'_i(Q_1|_E) \circ
(E \to W_\infty)_* \circ c'_j(Q_2|_E) \circ C =
c'_i(Q_1) \circ (Z \to X)_* \circ c'_j(Q_2)
$$
By Lemma \ref{lemma-silly} the left hand side is equal to
$$
(E \to Z)_* \circ c'_i(Q_1|_E) \circ c_j(Q_2|_{W_\infty}) \circ C
$$
Since $c'_i(Q_1) = (E \to Z)_* \circ c'_i(Q_1|_E) \circ C$
the right hand side is equal to
$$
(E \to Z)_* \circ c'_i(Q_1|_E) \circ C \circ (Z \to X)_* \circ c'_j(Q_2)
$$
which is immediately seen to be equal to the above
by Lemma \ref{lemma-homomorphism}.

\medskip\noindent
Assume $i \geq p_1$ and $j < p_2$. Unwinding the products
in this case we have to show
$$
(E \to Z)_* \circ c'_i(Q_1|_E) \circ c_j(Q_2|_{W_\infty}) \circ C =
c'_i(Q_1) \circ c_j(Q_2|_{X \times \{0\}})
$$
Again using that $c'_i(Q_1) = (E \to Z)_* \circ c'_i(Q_1|_E) \circ C$
we see that it suffices to show $c_j(Q_2|_{W_\infty}) \circ C =
C \circ c_j(Q_2|_{X \times \{0\}})$ which is part of
Lemma \ref{lemma-homomorphism}.

\medskip\noindent
Assume $i < p_1$ and $j \geq p_2$. Unwinding the products
in this case we have to show
$$
(E \to Z)_* \circ c_i(Q_1|_E) \circ c'_j(Q_2|_E) \circ C =
c_i(Q_1|_{Z \times \{0\}}) \circ c'_j(Q_2)
$$
However, since $c'_j(Q_2|_E)$ and $c'_j(Q_2)$ are
...
$$
(E \to Z)_* \circ c'_j(Q_2|_E) \circ c_i(Q_1|_{W_\infty}) \circ C =
c'_j(Q_2) \circ c_i(Q_1|_{X \times \{0\}})
$$
which we did in the preceding paragraph.
\end{proof}
...
\item[(A2)] there exists a closed subscheme $T \subset W_\infty$
containing all points of $W_\infty$ lying over $X \setminus Z$ such that
$Q_1|_T$ and $Q_2|_T$ are zero.
\end{enumerate}
In this case the classes
$P'_p(Q_1), P'_p(Q_2), P'_p(Q_1 \oplus Q_2) \in A^p(Z \to X)$
constructed in Lemma \ref{lemma-localized-chern-pre}
satisfy $P'_p(Q_1 \oplus Q_2) = P'_p(Q_1) + P'_p(Q_2)$.
\end{lemma}
...
$g_i^{-1}(T'_i)$ to construct $c_p(Z \to X, E_i)$. The same lemma
tells us that we may replace $T'_i$ with
$T = g_1^{-1}(T'_1) \cap g_2^{-1}(T'_2) \cap g_2^{-1}(T'_2)$
(and hence we may assume it is the same for $i = 1, 2, 3$).
By More on Flatness, Lemma
\ref{flat-lemma-complex-and-divisor-eta-pull}
...
\medskip\noindent
Let us define our bivariant class. Let
$$
b : W \longrightarrow \mathbf{P}^1_X
$$
...
$$
as quasi-coherent graded $\mathcal{O}_Z$-algebras, see
Divisors, Lemma \ref{divisors-lemma-quasi-regular-immersion}.
Hence $C_ZX \to Z$ is the vector bundle associated to the finite locally
free module $\mathcal{C}_{Z/X}$ of rank $r$. Denote
$i : C_ZX \to N$ the closed immersion corresponding to the
surjection $\sigma : \mathcal{N}^\vee \to \mathcal{C}_{Z/X}$.
To finish the proof we have to show
$$
i_*([C_ZX]_n) = p^*c_{top}(\mathcal{E}) \cap [Z]_{n - r}
$$
This is a special case of Lemma \ref{lemma-easy-virtual-class}
and the proof is complete.
\end{proof}
