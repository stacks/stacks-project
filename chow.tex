...
\section{Cartier divisors and coherent sheaves}
...
$$
[\mathcal{Q}_1]_k - [\mathcal{Q}_2]_k
=
...
\sum m_i(X_i \to X)_*\text{div}_{\mathcal{L}|_{X_i}}(s_i)
$$
holds in $Z_k(X)$. To be continued...
\end{proof}
...
\medskip\noindent
We will freely use the description of the sheaf of meromorphic functions
$\mathcal{K}_Z$ from Divisors, Lemma \ref{divisors-lemma-locally-Noetherian-K}.
By Divisors, Lemma \ref{divisors-lemma-regular-meromorphic-section-exists}
there exists a regular meromorphic
section $s \in \Gamma(Z, \mathcal{K}_Z(\mathcal{N}))$.
Let us denote $\mathcal{J} \subset \mathcal{O}_Z$ the sheaf
of denominators of $s$, i.e., it is defined by the rule
$$
\mathcal{J}(V)
=
\{f \in \mathcal{O}_Z(V) \mid fs \in \mathcal{N}(V)\}.
$$
The formula makes sense since
$\mathcal{N}(V) \subset \mathcal{K}_Z(\mathcal{N})(V)$.
We will use the maps
$$
1 : \mathcal{J} \longrightarrow \mathcal{O}_Z,
\quad
s : \mathcal{J} \longrightarrow \mathcal{N}.
$$
Both are injective, the second exactly because $s$ is a
regular meromorphic section. Namely, we may tensor these
maps with $\mathcal{G}$ to obtain exact sequences
$$
0
\to
\mathcal{G} \otimes \mathcal{J}
\to
\mathcal{G}
\to
\mathcal{Q}_1
...
0.
$$
We will see below why there are zeros on the right
in these exact sequences. Assuming this for the moment we
see (using the fact that
...
\medskip\noindent
Finally, let $Z_j$ be the set of irreducible components of
$\delta$-dimension $k + 1$ of $Z$. We think of $Z_j$ as an
integral closed subscheme. Write
$$
[\mathcal{F}]_{k + 1}
=
\sum m_j[Z_j].
$$
Denote $s_j = s|_{Z_j}$ the restriction of $s$ to a section
of $\mathcal{K}_{Z_j}(\mathcal{N})$. Then we claim that
$$
[i_*\mathcal{Q}_2] - [i_*\mathcal{Q}_1]
$$
