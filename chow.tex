...
$i = 1, 2$ be bivariant classes. For any $\alpha \in \CH_k(X)$
we can ask whether
$$
c_1 \cap c_2 \cap \alpha = c_2 \cap c_1 \cap \alpha
$$
in $\CH_{k - p_1 - p_2}(Z_1 \cap Z_2)$. If this is true and if it holds
...
\end{enumerate}
The conditions are satisfied if $X$ is either normal or Cohen-Macaulay.
\end{lemma}
...
\item If $f : X \to Y$ is a proper morphism of codimension $r$
of schemes locally of finite type over $S$ and $X$ and $Y$
satisfy the equivalent conditions of
Lemma \ref{lemma-locally-equidimensional}
then proper pushforward is a map $f_* : \CH^p(X) \to \CH^{p + r}(Y)$.
\item If $f : X \to Y$ is a morphism of codimension $r$
of schemes locally of finite type over $S$ and $X$ and $Y$
satisfy the equivalent conditions of
Lemma \ref{lemma-locally-equidimensional}
...
then $c_1(\mathcal{L}) \cap - : \CH^p(X) \to \CH^{p + 1}(X)$.
\item If $X$ is a scheme locally of finite type over $S$
satisfying the equivalent conditions of
Lemma \ref{lemma-locally-equidimensional} and
$\mathcal{E}$ is a finite locally free $\mathcal{O}_X$-module,
then $c_i(\mathcal{E}) \cap - : \CH^p(X) \to \CH^{p + i}(X)$.
\end{enumerate}
...
c(Z \to X, \mathcal{N}) \circ c \cap [X]$ in $\CH_*(Z \times_X Y)$.
...
Similarly, denote $W' \to \mathbf{P}^1_X$ the blowing up of $\infty(Z)$.
Since $\infty(Z) \subset \infty(Z)$ we get an opposite inclusion
...
fibre. After shrinking $U$ we may assume $U_\infty = C_Z X$. Another
local calculation (omitted) show that the morphism $U_\infty \to W'_\infty$
induces the canonical morphism $C_Z X \to C_Y X \subset W'_\infty$
of normal cones over $Z$. Denote $W'' \subset W$ the strict transform of
$\mathbf{P}^1_Y \subset \mathbf{P}^1_X$ in $W$.
Then we see that $(W'' \cap U)_\infty = C_ZY$.
...
$i^* \in A^1(\mathbf{P}^1_Y \to W')$. On the one hand we have
$$
i_\infty^* \circ i^* [U]_{n + 1} =
i_\infty^* [(W'' \cap U)_\infty]_{n + 1} =
[C_ZY]_n
$$
because $i_\infty^*$ kills all classes supported over $\infty$
and it is clear that $i^*[U]$ and $[W'']$ agree as cycles
over $\mathbf{A}^1$ and $C_ZY$ is the fibre of $W'' \cap U$ over $\infty$.
On the other hand, we have
$$
i_\infty^* \circ i^* [U]_{n + 1} = i^* [U_\infty] =
o^*[C_YX]_n
$$
because $i_\infty^*$ and $i^*$ commute (Lemma \ref{lemma-gysin-commutes-gysin})
...
local complete intersection morphisms, see
...
and $i$ is a regular immersion\footnote{
See Divisors, Definition \ref{divisors-definition-regular-immersion}.
...
In particular, the conormal sheaf $\mathcal{C}_{X/P}$ is finite locally free
(see Divisors, Lemma \ref{divisors-lemma-quasi-regular-immersion}). Hence the
normal sheaf
$$
\mathcal{N}_{X/P} = \SheafHom_{\mathcal{O}_X}(\mathcal{C}_{X/P}, \mathcal{O}_X)
$$
is finite locally free as well and we have a surjection
$\mathcal{N}_{X/P}^\vee \to \mathcal{C}_{X/P}$ (because an isomorphism
is also a surjection).
The construction in Section \ref{section-gysin-higher-codimension}
gives us a canonical bivariant class
$$
i^! = c(X \to P, \mathcal{N}_{X/P}) \in A^*(X \to P)
$$
Combining this with the bivariant class $g^* \in A^*(P \to Y)$ of
Lemma \ref{lemma-flat-pullback-bivariant} we obtain
$$
...
$$
where $p$ is a smooth morphism. Then $(g')^* = p^* \circ g^*$ and
hence it suffices to show that $i^! = (i')^! \circ p^*$ in
$A^*(X \to P)$. Consider the commutative diagram
$$
\xymatrix{
...
and $i' = j \circ s$. There is a short exact sequence
$$
0 \to
s^*(\mathcal{C}_{X \times_P P'/P'}) \to
\mathcal{C}_{X/P'} \to
\mathcal{C}_{X/X \times_P P'} \to 0
$$
by Divisors, Lemma \ref{divisors-lemma-transitivity-conormal-quasi-regular}.
Thus by Lemma \ref{lemma-gysin-composition} we have $(i')^! = s^! \circ j^!$.
...
Since bivariant classes commute with flat pullback by definition
we find $j^! \circ p^* = \overline{p}^* \circ i^!$.
Thus it suffices to show that $s^* \circ \overline{p}^* = \text{id}$.
This is true because if $Z \subset X$ is an integral closed subscheme,
then $s(X) \cap \overline{p}^{-1}(Z)$ is a closed subscheme
regularly embedded in $\overline{p}^{-1}(Z)$ of the right codimension, see
