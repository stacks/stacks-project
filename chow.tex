...
Hence, according to Remark \ref{remark-more-elementary} the determinant
...
Let $a, b \in A$.
\begin{enumerate}
\item if $a, b$ are nonzero divisors,
then $\text{length}_A(A/(ab)) < \infty$ and
$(A/(ab), a, b)$ is a $(2, 1)$-periodic exact complex.
\item if $M$ is a finite $A$-module such that
$a, b$ are nonzero divisors on $M$, then
$\text{length}_A(M/abM) < \infty$ and
$(M/abM, a, b)$ is a $(2, 1)$-periodic exact complex.
\end{enumerate}
In particular $\det_\kappa(A/(ab), a, b) \in \kappa^*$,
resp.\ $\det_\kappa(M/abM, a, b) \in \kappa^*$ are defined.
\end{lemma}
...
Let $A$ be a Noetherian local ring of dimension $1$.
Let $a, b \in A$ be nonzero divisors.
We define the {\it tame symbol} of the pair $(a, b)$
to be the element
$$
d_A(a, b)
=
\det\nolimits_\kappa(A/(ab), a, b)  \in \kappa^*
$$
which is defined by Lemma \ref{lemma-symbol-defined} above. By
Lemma \ref{lemma-multiplicativity-symbol} below this is multiplicative
in both variables and hence extends to a bilinear map
$$
d_A( - , - ) :
Q(A)^* \times Q(A)^*
\longrightarrow
\kappa^*
$$
where $Q(A)$ is the total ring of fractions of $A$.
\end{definition}

\begin{lemma}
\label{lemma-multiplicativity-symbol}
Let $A$ be a Noetherian local ring of dimension $1$.
Let $a, b, c \in A$ be nonzero divisors.
Then
$$
d_A(a, bc) = d_A(a, b) d_A(a, c)
$$
and $d_A(a, b)d_A(b, a) = 1$.
\end{lemma}
...
Let $A$ be a Noetherian local ring of dimension $1$.
Let $b \in A$ be a nonzero divisor, and let $u \in A^*$.
Then
$$
...
\begin{proof}
Note that in this case $A/(ub) = A/(b)$ on which multiplication
by $b$ is zero. Hence $d_A(u, b) = \det_\kappa(u|_{A/(b)})$
...
\begin{remark}
\label{remark-symbol-is-steinberg}
Let $A$ be a Noetherian local ring of dimension $1$.
Let $x \in Q(A)^*$ such that also $1 - x \in Q(A)^*$.
Then we claim that
$$
d_A(x, 1 -x) = 1
$$
Write $x = a/b$ with $a, b \in A$ nonzero-divisors.
The hypothesis implies, since $1 - x = (b - a)/b$,
that also $b - a$ and $a - b$ are nonzero divisors.
Hence we compute
$$
d_A(x, 1 - x)
=
...
$$
This can be shown with the following sequence of steps:
\begin{enumerate}
\item[Step 1.] The relation holds when $A$ is a discrete
valuation ring. This follows by a case by case analysis
and Lemma \ref{lemma-symbol-is-usual-tame-symbol}.
\item[Step 2.] It suffices to show the relation when both
...
\item[Step 3.] Thus we may assume $A$ is Cohen-Macaulay.
