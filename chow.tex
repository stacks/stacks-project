...
$\alpha' \in K_0(X) \otimes \mathbf{Q}$ with
...
$\beta' \in K_0(X) \otimes \mathbf{Q}$ lifting $\beta$, then we get
$$
\alpha \cdot \beta = ch(\alpha') \cap \beta =
ch(\alpha') \cap ch(\beta') \cap [X]
$$
and we know that chern classes of vectorbundles commute.
...
the isomorphism $K_0(X) \otimes \mathbf{Q} \to \CH^*(X) \otimes \mathbf{Q}$
