...
\item $E_1 \to E_2$ can be represented be a map of bounded complexes
...
\begin{lemma}[Construction if $E$ is given by a suitable complex]
...
\begin{enumerate}
\item there exists a locally bounded complex of finite locally free
$\mathcal{O}_X$-modules representing $E$, and
...
\end{enumerate}
In this situation there exists a canonical bivariant class
$$
...
We will freely use the properties of this construction listed in
More on Flatness, Lemma \ref{flat-lemma-graph-construction}.
For example, by construction, the object $Q$ can be represented by
a locally bounded complex of finite locally free $\mathcal{O}_W$-modules
and hence the chern classes of $Q$ are defined\footnote{With our current
definitions it isn't clear that if the chern classes of $E$ are defined,
then also the chern classes of $Q$ are defined. But for example if
$X$ is quasi-compact, then $W$ is quasi-compact and the chern classes
of any perfect object of $D(\mathcal{O}_W)$ are defined.}.
...
Recalling that $Q|_{X \times \{0\}} \cong E$ by construction, we conclude that
the bivariant class constructed in Lemma \ref{lemma-localized-chern-pre}
...
Notation and assumptions as in Lemma \ref{lemma-independent-loc-chern}.
...
\begin{lemma}[Construction in general]
\label{lemma-loc-chern-general}
Let $(S, \delta)$ be as in Situation \ref{situation-setup}. Let $X$ be a scheme
locally of finite type over $S$. Let $i : Z \to X$ be a closed immersion.
Let $E \in D(\mathcal{O}_X)$ be a perfect object. Assume
\begin{enumerate}
\item the chern classes of $E$ are defined
(Definition \ref{definition-defined-on-perfect}), and
\item the restriction $E|_{X \setminus Z}$ is zero, resp.\ isomorphic to a
finite locally free $\mathcal{O}_{X \setminus Z}$-module of rank $< p$
sitting in cohomological degree $0$.
\end{enumerate}
...
if $X' \to X$ is locally of finite type such that
$E' = (X' \to X)^*E$ can be represented by a locally bounded
complex of finite locally free $\mathcal{O}_{X'}$-modules, then the
restriction (Remark \ref{remark-restriction-bivariant}) of
$P_p(Z \to X, E)$, resp.\ $c_p(Z \to X, E)$ by $X' \to X$ is equal to the class
$P_p(Z' \to X', E')$, resp.\ $c_p(Z' \to X', E')$ constructed
in Lemma \ref{lemma-independent-loc-chern}.
\end{lemma}

\begin{proof}
By assumption (1) there exists an envelope $X' \to X$ such that
$E'$ can be represented by a locally bounded complex of finite
locally free $\mathcal{O}_{X'}$-modules. Thus we can apply
Lemma \ref{lemma-envelope-bivariant} to construct our classes.
In order for this to work, we have to show that for the classes
$P_p(Z' \to X', E')$, resp.\ $c_p(Z' \to X', E')$ we have
$$
res_1(P_p(Z' \to X', E')) =  res_2(P_p(Z' \to X', E'))
\quad\text{in}\quad
A^p(Z' \times_Z Z' \longrightarrow X' \times_X X')
$$
and similarly for $c_p(Z' \to X', E')$. However, this equality
is clear from the compatibility of the construction of these classes
with base change stated and proved in Lemma \ref{lemma-base-change-loc-chern}
and the fact that $\text{pr}_1^*E' \cong \text{pr}_2^*E'$ in
$D(\mathcal{O}_{X' \times_X X'})$.
The uniqueness of Lemma \ref{lemma-envelope-bivariant}
implies that (*) characterizes the classes
$P_p(Z \to X, E)$, resp.\ $c_p(Z \to X, E)$.
This construction via our envelope $X' \to X$
is furthermore compatible with base change by any morphism $X'' \to X$
which is locally of finite type
(as we already know this is true for the classes constructed
in Lemma \ref{lemma-independent-loc-chern}). This proves the statement
(*) with $X'' \to X$. Finally, the uniqueness
of Lemma \ref{lemma-envelope-bivariant} and the fact that the classes
in Lemma \ref{lemma-base-change-loc-chern} satisfy
(\ref{equation-defining-property-localized-classes}) implies the
same relation for our classes. Some details omitted.
\end{proof}

\begin{lemma}
\label{lemma-base-change-loc-chern-general}
Notation and assumptions as in Lemma \ref{lemma-loc-chern-general}.
Let $f : X' \to X$ be a morphism of schemes which is locally of finite type.
Denote $E' = f^*E$ and $Z' = f^{-1}(Z)$. Then the bivariant class
$$
P_p(Z' \to X', E') \in A^p(Z' \to X'),
...
c_p(Z' \to X', E') \in A^p(Z' \to X')
$$
constructed as in Lemma \ref{lemma-loc-chern-general} using $X', Z', E'$
is the restriction (Remark \ref{remark-restriction-bivariant}) of the
bivariant class $P_p(Z \to X, E) \in A^p(Z \to X)$,
resp.\ $c_p(Z \to X, E) \in A^p(Z \to X)$.
\end{lemma}

\begin{proof}
This we have seen in the proof of Lemma \ref{lemma-loc-chern-general}.
\end{proof}

\begin{lemma}
\label{lemma-loc-chern-after-pushforward}
In the situation of Lemma \ref{lemma-loc-chern-general} we have
$$
...
By Lemma \ref{lemma-base-change-loc-chern-general} the restriction of
...
P_p(Z \to X, E) \in A^p(Z \to X)
$$
by the construction in Lemma \ref{lemma-loc-chern-general} and we define the
...
in Lemma \ref{lemma-loc-chern-general}.
\end{enumerate}
\end{definition}
...
Lemma \ref{lemma-base-change-loc-chern-general}
...
\begin{proof}
By the characterization of our classes in
Lemma \ref{lemma-loc-chern-general}
this reduces to the case where $E$ can be represented by
a locally bounded complex of finite locally free modules on $X$.
Then the construction of $P_p(Z' \to X, E)$,
