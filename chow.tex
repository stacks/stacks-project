...
Assume that
\begin{enumerate}
\item $I_\alpha = K_{\beta\gamma}$ and $I_{\beta\gamma} = K_\alpha$,
i.e., $(M, \alpha, \beta\gamma)$ is an
exact $(2, 1)$-periodic complex, and
\item $I_{\alpha\beta} = K_\gamma$.
\end{enumerate}
Then
\end{lemma}

\begin{proof}
The assumptions imply that $K_\gamma \subset I_\alpha$ and that
the sequence
$$
0 \to I_{\alpha\beta} \to I_\alpha \xrightarrow{\gamma} I_{\gamma\alpha} \to 0
$$
is exact. Say $r = \text{length}_R(I_{\beta\gamma})$,
$s = \text{length}_R(I_{\gamma\alpha})$ and
$t = \text{length}_R(I_{\alpha\beta})$. Choose
\begin{enumerate}
...
\end{enumerate}
Then we see that $\delta = \det_\kappa(M, \alpha, \beta\gamma)$ is the
element of $\kappa^*$ such that
$$
[z_1, \ldots, z_t,
