...
Let $(S, \delta)$ be as in Sitation \ref{situation-setup}.
Let $X$ be a scheme locally of finite type over $S$.
Assume that $X$ is quasi-compact. The
map $\mathcal{F} \mapsto [\mathcal{F}]_k$
...
This implies we have the map as shown in the lemma.

\medskip\noindent
For any integral closed subcscheme $Z \subset X$ of $\delta$-dimension
$k$ we have $[\mathcal{O}_Z]_k = [Z]$ (Lemma \ref{lemma-cycle-closed-coherent}).
Since $X$ is quasi-compact a $k$-cycle is a finite $\mathbf{Z}$-linear
combination of integral closed subschemes of $\delta$-dimension $k$.
...
It seems likely that the result of Lemma \ref{lemma-cycles-k-group}
is not true if $X$ is not quasi-compact. For example, suppose $X$ is
...
Let $(S, \delta)$ be as in Sitation \ref{situation-setup}.
Let $X$ be a scheme locally of finite type over $S$.
Assume $X$ is quasi-compact. Via the identification
$Z_k(X) = K_0(\text{Coh}_{\leq k}(X)/\text{Coh}_{\leq k - 1}(X))$
of Lemma \ref{lemma-cycles-k-group} the kernel of
$$
K_0(\text{Coh}_{\leq k}(X)/\text{Coh}_{\leq k - 1}(X))
\longrightarrow
K_0(\text{Coh}_{\leq k + 1}(X)/\text{Coh}_{\leq k - 1}(X))
$$
is exactly the group of cycles rationally equivalent to zero.
In other words $A_k(X)$ is identified with the image of
$$
Z_k(X)
\longrightarrow
K_0(\text{Coh}_{\leq k + 1}(X)/\text{Coh}_{\leq k - 1}(X)),
\quad
\sum n_i[Z_i] \longmapsto \sum n_i[\mathcal{O}_{Z_i}]
$$
\end{lemma}

\begin{proof}
The last statement follows from the first statement.

\medskip\noindent
Let $\alpha \in Z_k(X)$ be rationally equivalent to zero.
We want to show that $\alpha$ maps to zero in
$K_0(\text{Coh}_{\leq k + 1}(X)/\text{Coh}_{\leq k - 1}(X))$.
...
of $\delta$-dimension $k + 1$. Since $X$ is quasi-compact the sum
is actually finite. Note that the ideal sheaves
...
$$
of coherent $\mathcal{O}_{W_i}$-modules. Thus we have
\begin{eqnarray*}
\alpha & = & \sum [(W_i)_0] - [(W_i)_\infty] \\
& = & \sum [p_*\mathcal{O}_{(W_i)_0}] - [p_*\mathcal{O}_{(W_i)_\infty}] \\
& = & \sum [p_*\mathcal{O}_{W_i}] - [p_*\mathcal{I}_i]
- [p_*\mathcal{O}_{W_i}] - [p_*\mathcal{J}_i] \\
& = & 0
\end{eqnarray*}
by what was said above. These computations take place inside the abelian
group $K_0(\text{Coh}_{\leq k + 1}(X)/\text{Coh}_{\leq k - 1}(X))$
since the $\delta$-dimension of the supports of
all the coherent sheaves involved is never greater than $k + 1$.

\medskip\noindent
Finally, we have to show that if some element $\alpha \in Z_k(X)$
maps to zero, then $\alpha \sim_{rat} 0$. This is a consequence of
Homology, Lemma \ref{homology-lemma-serre-subcategory-K-groups}
and Lemma \ref{lemma-maps-between-coherent-sheaves} above.
\end{proof}
...
is injective or not (or even if it is supposed to be injective).
...
\item $b$ is an isomorphism over $X \setminus (D_1 \cap D_2)$,
\item $b^{-1}(D_1) = D'_1 + E$, $b^{-1}D_2 = D_2' + E$,
\item $D'_1 \cap D'_2 = \emptyset$,
\item for every integral closed subscheme $Z'$
with $\dim_\delta(Z') = n - 1$ we have
\begin{enumerate}
\item if $\epsilon_{Z'}(D'_1, E) > 0$, then setting
$Z = b(Z')$ we have
$\dim_\delta(Z) = n - 1$ and
$$
\epsilon_{Z'}(D'_1, E) < \epsilon_{Z'}(D_1, D_2),
$$
\item if $\epsilon_{Z'}(D'_2, E) > 0$, then setting
$Z = b(Z')$ we have
$\dim_\delta(Z) = n - 1$ and
$$
\epsilon_{Z'}(D'_2, E) < \epsilon_{Z}(D_1, D_2).
$$
\end{enumerate}
\end{enumerate}
\end{lemma}
...
\begin{proof}
Let $\mathcal{I} = \mathcal{I}_{D_1} + \mathcal{I}_{D_2}$.
Denote $Z = D_1 \cap D_2 \subset X$ (scheme theoretic intersection)
to corresponding closed subscheme.
Let $b : X' \to X$ be the blow up of $X$ in $\mathcal{I}$.
...
Similarly $D_2 = D_2' + E$. This takes care of assertions (1) -- (4).

\medskip\noindent
To prove (5) and (6) we may work locally on $X$.
Thus we may assume that $X = \text{Spec}(A)$ with
$A$ a Noetherian domain, and $D_1 = \text{Spec}(A/a)$,
$D_2 = \text{Spec}(A/b)$. Set $I = (a, b)$.
Since $A$ is a domain and $a, b \not = 0$ we can
...
By the dimesion formula we see that $\dim(A_{\mathfrak p}) = 1$
as well. The final assertion to be shown is that
$$
\text{length}_{A_{\mathfrak p}}(A_{\mathfrak p}/(a))
\;
\text{length}_{A_{\mathfrak p}}(A_{\mathfrak p}/(b))
>
\text{length}_{B_{\mathfrak q}}(B_{\mathfrak q}/(a))
\;
\text{length}_{B_{\mathfrak q}}(B_{\mathfrak q}/(s))
$$
where $B = A[s]/(as - b)$. Since $A_{\mathfrak p}$
and $B_{\mathfrak q}$ have the same residue fields...
To be continued.
...
\noindent
The following lemma can be found in \cite[Section 2.4]{F}.
We have adapted the formulation so it also works in the non-finite type setting.
It may happen $b$ may be the composition of infinitely many blow
