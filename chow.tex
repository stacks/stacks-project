...
Assume that
\begin{enumerate}
\item $I_{\beta\gamma} = K_\alpha$,
$I_{\gamma\alpha} = K_{\beta}$,
$I_{\alpha\beta} = K_\gamma$, and
\item\footnote{If the morphisms $\alpha, \beta, \gamma$
commute then hypothesis (2) follows from hypothesis (1).}
$K_\alpha \subset I_\gamma$,
$K_\beta \subset I_\alpha$, and
$K_\gamma \subset I_\beta$.
\end{enumerate}
Then
\begin{enumerate}
\item The triples
$(M, \alpha, \beta\gamma)$,
$(M, \beta, \gamma\alpha)$, and
$(M, \gamma, \alpha\beta)$ are exact $(2, 1)$-periodic complexes.
\item The maps
$\beta' : I_\alpha \to I_{\alpha\beta}$, $\alpha x \mapsto \alpha\beta x$,
$\gamma' : I_\beta \to I_{\beta\gamma}$, $\beta x \mapsto \beta\gamma x$, and
$\alpha' : I_\gamma \to I_{\gamma\alpha}$, $\gamma x \mapsto \gamma\alpha x$
are well defined.
\item The triples
$(I_\alpha, \gamma, \beta')$,
$(I_\beta, \alpha, \gamma')$, and
$(I_\gamma, \beta, \alpha')$ are exact $(2, 1)$-periodic complexes.
\item We have
$$
\det\nolimits_\kappa(M, \alpha, \beta\gamma)
=
\det\nolimits_\kappa(I_\gamma, \alpha', \beta)
\det\nolimits_\kappa(I_\beta, \alpha, \gamma')
$$
and similarly for $\det\nolimits_\kappa(M, \beta, \gamma\alpha)$,
and $\det\nolimits_\kappa(M, \gamma, \alpha\beta)$.
\end{enumerate}
\end{lemma}

\begin{proof}
The assumptions imply that
$\alpha\beta\gamma = \gamma\alpha\beta = \beta\gamma\alpha = 0$.
Thus we see that
$K_\alpha \subset I_\beta$,
$K_\beta \subset I_\gamma$, and
$K_\gamma \subset I_\alpha$.
This in turn implies combined with the assumptions that the
sequences
$$
\begin{matrix}
0 \to I_{\alpha\beta} \to I_\alpha \xrightarrow{\gamma} I_{\gamma\alpha} \to 0
\\
0 \to I_{\beta\gamma} \to I_\beta \xrightarrow{\alpha} I_{\alpha\beta} \to 0
\\
0 \to I_{\gamma\alpha} \to I_\gamma \xrightarrow{\beta} I_{\beta\gamma} \to 0
\end{matrix}
$$
are exact. Say
$r = \text{length}_R(I_{\beta\gamma})$,
$s = \text{length}_R(I_{\gamma\alpha})$, and
$t = \text{length}_R(I_{\alpha\beta})$.
Then we conclude that
$\text{length}_R(I_\alpha) = s + t$,
$\text{length}_R(I_\beta) = r + t$, and
$\text{length}_R(I_\gamma) = r + s$. Then since we have the exact
sequence $0 \to K_\alpha \to M \to I_\alpha \to 0$ we conclude
that $\text{length}_R(M) = r + s + t$. Hence the inclusions
$I_\alpha \subset K_{\beta\gamma}$,
$I_\beta \subset K_{\gamma\alpha}$, and
$I_\gamma \subset K_{\alpha\beta}$ have to be equalities for
reasons of length. This proves part (1).

\medskip\noindent
To see $\beta'$ is well defined
we have to show that if $\alpha x = 0$, then
$\alpha \beta x = 0$. By assumption (2) we see $\alpha x = 0$ implies
$x = \gamma y$, and hence $\alpha \beta x = \alpha \beta \gamma y = 0$
as desired. Similarly for $\alpha'$ and $\gamma'$. This proves part (2)
of the lemma.

\medskip\noindent
Note that the kernel of $\beta'$ is
$\alpha(K_{\alpha\beta}) = \alpha(I_\gamma) = I_{\alpha\gamma}$.
Similarly for the other cases.
Hence we also have short exact sequences
$$
\begin{matrix}
0 \to I_{\gamma\alpha} \to I_\alpha \xrightarrow{\beta'} I_{\alpha\beta} \to 0
\\
0 \to I_{\alpha\beta} \to I_\beta \xrightarrow{\gamma'} I_{\beta\gamma} \to 0
\\
0 \to I_{\beta\gamma} \to I_\gamma \xrightarrow{\alpha'} I_{\gamma\alpha} \to 0
\end{matrix}
$$
Thus part (3) of the lemma follows.

\medskip\noindent
Choose
\begin{enumerate}
\item an admissible sequence $x_1, \ldots, x_r \in I_{\beta\gamma}$
generating $I_{\beta\gamma}$
\item an admissible sequence $y_1, \ldots, y_s \in I_{\gamma\alpha}$
generating $I_{\gamma\alpha}$,
\item an admissible sequence $z_1, \ldots, z_t \in I_{\alpha\beta}$
generating $I_{\alpha\beta}$,
\item elements $\tilde x_i \in M$ such that $\beta\gamma(\tilde x_i) = x_i$,
\item elements $\tilde y_i \in M$ such that $\gamma\alpha(\tilde y_i) = y_i$,
\item elements $\tilde z_i \in M$ such that $\alpha\beta(\tilde z_i) = z_i$.
\end{enumerate}
According to Remark \ref{remark-more-elementary} the determinant
$\delta = \det_\kappa(M, \alpha, \beta\gamma)$ is the
unique element of $\kappa^*$ such that
\begin{eqnarray*}
& & [z_1, \ldots, z_t,
\alpha(\tilde y_1), \ldots, \alpha(\tilde y_s),
\tilde x_1, \ldots, \tilde x_r]
\\
& = &
(-1)^{r(s + t)} \delta
[x_1, \ldots, x_r,
\beta(\tilde z_1), \ldots, \beta(\tilde z_t),
\tilde y_1, \ldots, \tilde y_s]
\end{eqnarray*}
On the other hand, we see that
$\delta_1 = \det_\kappa(I_\gamma, \alpha', \beta)$
is characterized by
$$
[y_1, \ldots, y_s, \gamma(\tilde x_1), \ldots, \gamma(\tilde x_r)]
=
(-1)^{sr} \delta_1
[x_1, \ldots, x_r, \alpha(\tilde y_1), \ldots, \alpha(\tilde y_s)].
$$ 
This implies (via the identification $M/I_{\alpha\beta} = I_\gamma$) that
\begin{eqnarray*}
& &
[z_1, \ldots, z_t,
\alpha(\tilde y_1), \ldots, \alpha(\tilde y_s),
...
(-1)^{sr} \delta_1
[z_1, \ldots, z_t,
\beta(\tilde x_1), \ldots, \beta(\tilde x_r),
\tilde y_1, \ldots, \tilde y_s)]
\end{eqnarray*}
in $\det_\kappa(M)$. Next we use that
$\delta_2 = \det_\kappa(I_\beta, \alpha, \gamma')$ is characterized by
$$
[z_1, \ldots, z_t,
\beta(\tilde x_1), \ldots, \beta(\tilde x_r)]
=
...
\tilde y_1, \ldots, \tilde y_s)]
\end{eqnarray*}
Combining the formulas above we see that $\delta = \delta_1 \delta_2$
as desired.
\end{proof}
...
\begin{proof}
Write $\alpha = \sum m_j[Z_j]$ for some lolcally finite
...
\section{Gysin homomorphsms}
\label{section-gysin}

\noindent
We want to reformulate the definition of the Gysin
homomorphism in terms of coherent sheaves.
Let $X$ be a scheme, and let $i : D \to X$ be
an effective Cartier divisor. For any quasi-coherent sheaf
$\mathcal{F}$ on $X$ we define $L_0i*\mathcal{F} = i^*\mathcal{F}$
and
$$
L_1i^*\mathcal{F}
=
\text{Ker}(
\mathcal{I}_D \otimes_{\mathcal{O}_X} \mathcal{F} \to \mathcal{F}
)
$$
which we think of as a quasi-coherent sheaf on $D$ via
Morphisms, Lemma \ref{morphisms-lemma-i-star-equivalence}.
Thus there is an exact sequence
$$
0 \to
i_*(L_1i^*\mathcal{F}) \to
\mathcal{I}_D \otimes_{\mathcal{O}_X} \mathcal{F} \to
\mathcal{F} \to
i_*(L_0i^*\mathcal{F}) \to 0
$$
of quasi-coherent sheaves on $X$.

\begin{lemma}
\label{lemma-gysin-pullback}
Let $(S, \delta)$ be as in Sitation \ref{situation-setup}.
Let $X$ be locally of finite type over $S$.
Let $i : D \to X$ be an effective Cartier divisor.
Let $\mathcal{F}$ be a coherent sheaf on $X$ with
$\dim_\delta(\mathcal{F}) \leq k + 1$.
Then the element $[L_0i^*\mathcal{F}] - [L_1i^*\mathcal{F}]$
of $K_0(\text{Coh}_{\leq k + 1}(D)/\text{Coh}_{\leq k - 1}(D))$
lies in the subgroup $B_k(D)$ and maps to $i^*[\mathcal{F}]_{k + 1}$
via the map $B_k(D) \to A_k(D)$.
\end{lemma}
...
Let $X$ be locally of finite type over $S$.
Let $i : D \to X$ be an effective Cartier divisor.
The Gysin homomorphism $\alpha \mapsto i^*\alpha$ factors
through rational equivalence to give an operation
$$
i^* : A_{k + 1}(X) \to A_k(D)
$$
\end{lemma}
...
\begin{proof}
Let $\alpha \in Z_{k + 1}(X)$, and assume $\alpha \sim_{rat} 0$.
We have to show that $i^*\alpha \in A_k(D)$ is zero.
...
\begin{proof}
The second formula follows from the first because we have
seen that $c_1(\mathcal{O}_X(D)) \cap p^*\alpha
= D \cdot p^*\alpha = $
