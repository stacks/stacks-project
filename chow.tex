...
We will denote
$$
c_1(\mathcal{L}) \cap - : A_{k + s}(X) \to A_k(X)
$$
...
Remark \ref{remark-pullback-pairs} and Lemmas \ref{lemma-closed-in-X-gysin},
\ref{lemma-gysin-flat-pullback}, and \ref{lemma-gysin-commutes-gysin}.
...
(and hence satisfy all properties required of them in \cite{F}).
See also \cite[Theorem 17.1]{F}.
...
in the statement of the lemma. Thus, if $\beta \in A_{k + 1}(W)$
...
for any $\alpha \in A_k(X)$. Choose $\beta \in A_{k + 1}(W)$
...
is a class in $A_{k + 1 - p}(W')$ whose restriction to
...
The map
$\times : A_n(X) \otimes_{\mathbf{Z}} A_m(Y) \to A_{n + m - 1}(X \times_S Y)$
is well defined.
\end{lemma}
...
This produces maps
$$
c' \cap - : A_m(Y) \longrightarrow A_{m + n - 1}(X \times_S Y)
$$
...
factors through rational equivalence in the second variable, i.e.,
gives a well defined map
$Z_n(X) \otimes_{\mathbf{Z}} A_m(Y) \to A_{n + m - 1}(X \times_S Y)$.
By symmetry the same is true for the other variable and we conclude.
\end{proof}
...
\begin{proof}
Consider the element $[S]_1 \in A_1(S)$. We get a map
$A^p(X \to S) \to A_{1 - p}(X)$ by sending $c$ to $c \cap [S]_1$.

\medskip\noindent
Conversely, suppose we have $\alpha \in A_{1 - p}(X)$.
Then we can define $c_\alpha \in A^p(X \to S)$ as
follows: given $X' \to S$ and $\alpha' \in A_n(X')$
we let
$$
c_\alpha \cap \alpha' = \alpha \times \alpha'
$$
in $A_{n - p}(X \times_S X')$. To show that this is a bivariant
