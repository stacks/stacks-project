...
that $a, b : V \to V'$ are distinct morphisms $V \to V'$ over $Y$.
...
which is an isomorphism if and only if it is surjective. Since $X \to Y$ is
a universal homeomorphism we see that this is the case if and only if
$E_X = V_X$, i.e., if and only if $a_X = b_X$.
