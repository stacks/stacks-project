...
to get a pushout $V' = V \times_U U'$. Denote $V' \to Y'$ the morphism
...
show that the adjunction mapping $(V, U', \varphi) \to F(G(V, U', \varphi))$
is an isomorphism. To do this we may work \'etale locally over $Y'$.
Thus we may base change by an \'etale surjective morphism $Y'_1 \to Y'$
where $Y'_1$ is a scheme. In this case the statement is
More on Morphisms, Lemma
...
\begin{lemma}
\label{lemma-equivalence-categories-spaces-over-pushout-flat}
In the situation of
...
(the front and back squares are pushouts).
Now all the statements of the lemma follow from the corresponding statement
of More on Morphisms, Lemma
\ref{more-morphisms-lemma-equivalence-categories-schemes-over-pushout-flat}.

\medskip\noindent
For example, suppose that $W' \to Y'$ is flat. Choose a scheme
$W'_1$ and a surjective \'etale morphism $W'_1 \to Y_1' \times_{Y'} W'$.
Observe that $W'_1 \to W'$ is surjective \'etale as a composition of
surjective \'etale morphisms. We know that $G(F(W_1')) \to W_1'$
is an isomorphism by More on Morphisms, Lemma
\ref{more-morphisms-lemma-equivalence-categories-schemes-over-pushout-flat}
applied to $W'_1$ over $Y'_1$ and the front of the diagram. (Note that
the functor $G$ there agrees with our $G$ applied to schemes because pushouts
of the type we are considering agree for schemes and algebraic spaces by
Lemma \ref{lemma-pushout-along-thickening-schemes}.)
Then the construction of $G(F(W'))$ (as a pushout, i.e.,
as constructed in Lemma \ref{lemma-pushout-along-thickening}) shows that
$G(F(W'_1)) \to G(F(W))$ is surjective \'etale. Whereupon we conclude
that $G(F(W)) \to W$ is \'etale, see for example
Properties of Space, Lemma \ref{spaces-properties-lemma-etale-local}.
But $G(F(W)) \to W$ is an isomorphism on underlying reduced
algebraic spaces (by construction), hence it is an isomorphism.
We omit the details of the proofs of (1), (2), and (3).
\end{proof}
