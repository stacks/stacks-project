...
\lim D_{perf}(T_n) = D_{perf}(T)
$$
where the last step uses a Grothendieck existence result for
the derived category of the proper algebraic space $T$ over $R$
(compare Flatness on Spaces, Remark \ref{spaces-flat-remark-correct-generality}).
The paper cited shows that
this arrow determines a morphism $T \to X$ as desired. To apply the
second reference we use the same argument with coherent modules:
$$
\textit{Coh}(\mathcal{O}_X) \to \lim \textit{Coh}(\mathcal{O}_{X_n})
\xrightarrow{\lim g_n^*}
\lim \textit{Coh}(\mathcal{O}_{T_n}) = \textit{Coh}(\mathcal{O}_T)
$$
