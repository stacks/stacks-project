...
is the sheaf on $X_{etale}$ associated (see
...
This means that there is a natural differential
$$
\text{d}^a : \mathcal{O}_{X_{etale}} \longrightarrow (\Omega_{X/Y})^a
$$
since we have just seen that the value of $(\Omega_{X/Y})^a$ is
canonically identified with $\Gamma(U, \Omega_{U/Y})$.
To be continued...
\end{proof}
...
The universal derivation will be denoted
...
\begin{proof}
Assume (1). By
Morphism of Spaces, Lemma \ref{}
we see that $f$ is locally of finite presentation. Choose a diagram
$$
\xymatrix{
...
$$
with $U$, $V$ schemes and surjective etale vertical arrows. Then
$\psi$ is smooth by our definition of smooth morphisms of algebraic spaces, see
Morphism of Spaces, Definition \ref{}.
