...
$$
\text{Ext}^i_{\mathcal{O}_Y}(L, \mathcal{F}) \longrightarrow
...
\begin{proof}
The case of ``tor amplitude in $[a, b]$'' follows immediately from
Derived Categories of Spaces, Lemma
\ref{spaces-perfect-lemma-tor-independence-and-tor-amplitude}
(Observe that $Y_i$ and $X$ are tor independent over $Y$
as $Y_i \to Y$ is flat.)
Using some elementary topology one prove the case
``locally finite tor dimension''.
\end{proof}
