...
\begin{proof}
It suffices to prove this when $X'$ is a first order thickening of $X$.
Let us denote $\mathcal{O}$ the structure sheaf of $X$ on
$X_{spaces, \acute{e}tale}$ and $\mathcal{O}'$ the structure
sheaf of $X'$ on $X_{spaces, \acute{e}tale}$. By
Morphism of Spaces, Lemma \ref{spaces-morphisms-lemma-i-star-equivalence}
the kernel $\mathcal{I} = \text{Ker}(\mathcal{O}' \to \mathcal{O})$
is a quasi-coherent $\mathcal{O}$-module.

\medskip\noindent
Denote $\mathcal{O}_X = \mathcal{O}|_{X_{Zar}}$ the structure
sheaf of $X$ (as a scheme). Denote $\mathcal{A} = \mathcal{O}'|_{X_{Zar}}$.
This is a sheaf of rings on the scheme $X$ equipped with a
map $\mathcal{A} \to \mathcal{O}_X$ which is surjective by
Lemma \ref{lemma-first-order-thickening-surjective}.
The kernel is the quasi-coherent
