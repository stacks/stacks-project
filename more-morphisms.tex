...
$$
The diagram is a fibre square, the morphisms $a : X \to W$ and
$b : Y \to W$ are closed immersions, and there is a short exact sequence
$$
...
\ref{topology-section-colimits}). As a set this is just the pushout
of the underlying sets. The topology is the quotient topology (details
omitted). On $Y \amalg_Z X$ we have the maps of sheaves of rings
$$
...
in the category of sheaves of rings. To prove that we obtain a scheme
we have to show that every point has an affine open neighbourhood.
This is clear for points not in the image of $c$.

\medskip\noindent
Let $z \in Z$. Choose an affine open $U \subset X$ such that $z \in i^{-1}U$.
Choose an affine open $V \subset Y$ such that $z \in j^{-1}V \subset i^{-1}U$.
Choose an $f \in \Gamma(U, \mathcal{O}_U)$ such that
$z \in i^{-1}D(f) \subset j^{-1}V$. Since $j(Z) \cap V$ is closed in
$V$, there exists a $g \in \Gamma(V, \mathcal{O}_V)$ whose pullback
to the scheme $j^{-1}V$ is equal to the restriction of $f$ to
$j^{-1}V \subset i^{-1}U$. Then we see that after replacing $U$
by the standard open $D(f)$ and $V$ by the standard open $D(g)$
we have $z \in i^{-1}U = j^{-1}V$. Since the construction of the first
paragraph is clearly compatible with restriction to compatible open
subschemes, to prove that it produces a scheme we may assume $X$, $Y$, and
$Z$ are affine.
...
to principal affine opens.

\medskip\noindent
The scheme theoretic properties of the fibre product come from the fact
that $B \times_C A \to A$ and $B \times_C A \to B$ are surjective ring maps
and that $C \cong B \otimes_{B \times_C A} A$.
We omit the verification of the universal property of the pushout.
\end{proof}
...
\item If $f$ is flat and $x$ and $f'$ is Cohen-Macaulay at $x'$, then $f$
...
Hence it suffices to show the set is stable under generalization. By
...
we obtain the factorization and (5) and (2) hold by construction. By
...
neighbourhood of $s$ such that $n_{X/S}|_U \geq n$.
...
Lemma \ref{lemma-etale-makes-integral-split}
...
\section{Characterizing pseudo-coherent complexes}
...
$$
We claim that $X \times_A V$ and $W'$
are tor-independent over $X' \times_{A'} V'$.
If so, then the derived pullback of
...
\medskip\noindent
To prove the claim it is enough to show that $W'$ and $U \times_A V$ are
tor-independent over $U' \times_{A'} V'$ as every point of $W'$
maps to a point of $U' \times_A V'$. Then it is enough to show that
$W'$ and $U \times_A \mathbf{P}^n_A$ are
tor-independent over $U' \times_{A'} \mathbf{P}^n_{A'}$.
To see this we may work affine locally on $U'$ and $\mathbf{P}^n$.
Since $\mathbf{P}^n$ is covered by affine spaces,
the statement is translated into the following algebra fact:
if $B' \to B$ is a map of rings
and $\varphi : B'[x_1, \ldots, x_n] \to B'$ is a $B'$-algebra map, then
$B[x_1, \ldots, x_n]$ and $B'$ are tor-independent over
$B'[x_1, \ldots, x_n]$. To see this algebra fact holds, say
$\varphi(x_i) = b'_i$. The Koszul complex $K'_\bullet$ on
$x_i - b'_i$, $i = 1, \ldots, n$ gives a free
$B'[x_1, \ldots, x_n]$-resolution of $B'$.
Since $B \otimes_{B'} K'_\bullet$ is the corresponding
Koszul complex over $B[x_1, \ldots, x_n]$
and hence has no nonzero cohomology except in degree $0$
we see that the claim is true.

\medskip\noindent
Combining the two previous paragraphs
reduces us to the situation described in the next
paragraph.

\medskip\noindent
Assume $A$ is of finite type over $\mathbf{Z}$.
Choose an affine open neighbourhood $U \subset X$.
...
in this case. This finishes the proof.
\end{proof}
