...
\Spec(B') \ar[l] \ar[d] \\
\Spec(A[x_1, \ldots, x_n]) & \ar[l] \Spec(A')
}
$$
...
$$
where $(V, v) \to (Y, y)$ is an \'etale neighbourhood,
$U \to V$ is finite, $v$ is the unique point of $V$ mapping to $u$,
...
We may choose an open immersion $X \to T$ where $T \to Y$ is finite, see
Lemma \ref{lemma-quasi-finite-separated-pass-through-finite}.
...
free over $Y'$ containing $Y' \times_Y T$,
...
consists of $r$ points with $x_j \in T_i \Leftrightarrow i \in I_j$.
...
$Y_{I_1, \ldots, I_r}$.) In this case we obtain a decomposition
$X = X_1 \amalg \ldots \amalg X_r$ into open and closed parts
with $X_j \to Y$ an open immersion.
Since $w$ is a weigthing, we see that $w|_{X_j}$
is locally constant and we conclude.
\end{proof}
...
\noindent
In this section we give a bound for the affine
stratification number of a scheme which has a certain kind of
cover by an affine scheme.

\begin{lemma}
\label{lemma-affineness-of-large-open-noetherian}
Let $f : X \to Y$ be a quasi-finite morphism of affine Noetherian schemes.
...
\begin{proof}
Let $X' \to Y'$ be the base change of $f$ by a morphism $g : Y' \to Y$.
The formation of $\int_f w$ and hence the open $Y_d$ commute with base change.
If $g$ is finite and surjective, then
...
\medskip\noindent
We may choose an open immersion $X \to \overline{X}$ with $\overline{X}$
finite over $Y$, see Lemma
...
to the finite morphism $\overline{X} \to Y$. This lemma tells us that
there is a finite surjective (and flat) morphism $Y' \to Y$ such that
$Y' \times_Y \overline{X} \to Y'$ has a special form.
By the discussion in the first paragraph, we may replace $Y$ by $Y'$.
Thus we may assume there is a closed immersion
$\overline{X} \to T$ and closed subschemes
...
Then we may base change once more by $Y' \to Y$. Thus we may in
addition assume $Y$ is integral.
...
\medskip\noindent
For $y \in Y$ let $f^{-1}(\{y\}) = \{x_1, \ldots, x_m\}$ in $X$.
Since the residue field extensions are all trivial, we have
$(\int_f w)(y) = \sum w(x_j)$. If $y \in Y_d$, then $\sum w(x_j) = d$.
\end{proof}

\begin{proposition}
\label{proposition-asn-weighting}
Let $f : X \to Y$ be a surjective quasi-finite morphism of
Noetherian schemes. Let $w : X \to \mathbf{Z}_{> 0}$ be a
positive weighting of $f$. Assume $X$ affine and $Y$ separated.
Then the affine stratification number of $Y$ is at most
the number of distinct values of $\int_f w$.
\end{proposition}
