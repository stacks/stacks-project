...
in this case, see Morphisms, Lemma \ref{morphisms-lemma-etale-at-point}.
Note also that if $f : (V, v) \to (U, u)$ is a morphism of etale
...
\item $x \in X_s$ is isolated\footnote{This means that $f$ is
...
\item[(\romannumeral3)] the point $v$ maps to $x$ in $X$ and
$\kappa(x) = \kappa(v)$.
\end{enumerate}
...
\xymatrix{
v \ar[d] & V \ar[d] \ar@{^{(}->}[r] &
X_U = U \times_S X \ar[d] \ar[r] & X \ar[d] & x \ar[d] \\
u & U \ar@{=}[r] & U \ar[r] & S & s
}
$$
\end{lemma}
...
exists a unique point $x' \in X_u \subset X_U$ corresponding to
$x \in X_s$. Also $x'$ is isolated in $X_u$. Hence by
Lemma \ref{lemma-etale-makes-quasi-finite-finite-at-point} there
exists an etale neighbourhood $(U', u') \to (U, u)$
and an open $V_n \subset X_{U'}$ which works for $x_n$.
By the final assertion of
Lemma \ref{lemma-etale-makes-quasi-finite-finite-at-point}
the subschemes $V'_i = U'\times_U V_i$ for $i = 1, \ldots, n - 1$ still
