...
a map $\varphi : N \to M$ which induces an isomorphism
$N \otimes_B A = M$. Set $B' = B \times_A A'$ and $N' = N \times_M M'$
giving pullback diagrams
$$
\vcenter{
\xymatrix{
A & A' \ar[l] \\
...
\xymatrix{
M & M' \ar[l] \\
N \ar[u]_\varphi & N' \ar[l] \ar[u]
}
}
$$
Then $N' \otimes_{B'} B = N$ and $N' \otimes_{B'} A' = M'$.
\end{lemma}

\begin{proof}
We extend the diagrams above to
$$
\vcenter{
\xymatrix{
A & A' \ar[l] & I \ar[l] \\
...
\begin{lemma}
\label{lemma-module-over-fibre-product-rings-bis}
In the situation of Lemma \ref{lemma-module-over-fibre-product-rings}
for a $B'$-module $L'$ the map
$$
L' \longrightarrow 
...
\begin{proof}
As in the proof of Lemma \ref{lemma-module-over-fibre-product-rings}
...
\begin{lemma}
\label{lemma-finite-module-over-fibre-product-rings}
Let $A, A', B, B', I, M', M, N, \varphi, N'$ be as in
Lemma \ref{lemma-module-over-fibre-product-rings}.
Assume $I \subset \text{rad}(A')$, $N$ finite over $B$, and $M'$
finite over $A'$. Then $N'$ is finite over $B'$.
\end{lemma}

\begin{proof}
Choose generators $x_1, \ldots, x_r$ of $N$ over $B$.
Pick $y_1, \ldots, y_r \in M'$ mapping to
$\varphi(x_1), \ldots, \varphi(x_r)$ in $M$.
Note that $M$ is generated by $\varphi(x_1), \ldots, \varphi(x_r)$ over $A$,
hence we see that $M'$ is generated by $y_1, \ldots, y_r$ over $A'$ by
Algebra, Lemma \ref{algebra-lemma-NAK}.
Set $z_i = (x_i, y_i) \in N' = N \times_M M'$.
We claim that $z_1, \ldots, z_r$ generate $N'$ over $B'$.
Namely, given $\xi \in N'$ we can first choose $b'_1, \ldots, b_r' \in B'$
such that $\xi$ and $\sum b'_i z_i$ map to the same element of $N$.
The difference $\xi - \sum b'_i z_i$ is of the form $(0, \theta)$
for some $\theta$ in the image of $M' \otimes_{A'} I \to M'$.
Say $\theta$ is the image of $\sum y_i \otimes t_i$ with $t_i \in I$.
Since $J = \text{Ker}(B' \to B)$ maps isomorphically to $I$ (see proof
Lemma \ref{lemma-module-over-fibre-product-rings}) we can lift $t_i$
to $s_i \in J$.
Then it is clear that $\xi = \sum (b'_i + s_i)z_i$ as desired.
\end{proof}
