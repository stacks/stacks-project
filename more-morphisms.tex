...
infinitesimally move a map.

\medskip\noindent
Let $i : T \to T'$ be a closed immersion of schemes defined by an ideal of
...
To finish the proof of (2) we have to check that
\begin{enumerate}
\item[(i)] $(a'')^\sharp$ is a homomorphism of sheaves of rings,
\item[(ii)] it is a homomorphism of sheaves of $p^{-1}\mathcal{O}_S$-algebras
where $p : T \to S$ is the structure morphism, and
\item[(iii)] $(a'')^\sharp \bmod \mathcal{I}$ is equal to $(a|_U)^\sharp$.
\end{enumerate}
...
\begin{equation}
\label{equation-define-action-affine}
a'' = a' + \theta \circ \text{d}_{A/R} : A \longrightarrow B
\end{equation}
is a (i) ring map, (ii) $R$-linear, and
(iii) agrees with $a$ as a map into
...
of the lemma above is compatible with etale localization.
...
and assume that
\begin{enumerate}
\item $i_1 : T_1 \to T_1'$ is a closed immersion given by an ideal
$\mathcal{I}_1$ of square zero,
\item $i_2 : T_2 \to T_2'$ is a closed immersion given by an ideal
$\mathcal{I}_2$ of square zero, and
\item $X_2 \to X_1$ and $Y_2 \to Y_1$ are etale.
\end{enumerate}
Write $a_i = a'_i \circ i_k$ for $k = 1, 2$.
For any $\mathcal{O}_{T_1}$-linear map
$\theta_1 : a_1^*\Omega_{X_1/S_1} \to \mathcal{I}_1$ let
...
$$
commutes where the actions $\theta_2 \cdot a_2'$ and
$\theta_1 \cdot a_1'$ are as in Lemma \ref{lemma-action-by-derivations}.
\end{lemma}
...
h'(D_1(x)) = D_2(f(x))
$$
as elements of $I_2$. Now from the proof of
Lemma \ref{lemma-action-by-derivations}, see
Equations (\ref{equation-define-action}) and
(\ref{equation-define-action-affine})
...
\label{section-exactness-pushforward}

\noindent
In this section we study some very basic properties of pushforward
on sheaves for some types of morphisms.

\begin{lemma}
\label{lemma-integral-etale-cover-from-below}
Let $f : X \to Y$ be an integral morphism of schemes.
Let $U \to X$ be etale. Then there exist etale morphisms
$V_i \to Y$ and morphisms $V_i \times_Y X \to U$ such that
$\{V_i \times_Y X \to U\}$ is an etale covering of $U$.
FIXME: THIS IS WRONG.
\end{lemma}

\begin{proof}
Supppose that $V \to X$ is etale.
Then $V \times_Y X \to X$ is etale, and hence any morphism
from $V \times_Y X \to U$ is etale as well, see
Morphisms, Lemmas \ref{morphisms-lemma-base-change-etale} and
\ref{morphisms-lemma-etale-permanence}.
We will use this remark without further mention.
Pick $u \in U$. It is enough to construct an etale morphism
$V \to Y$ and a morphism $V \times_Y X \to U$ such that $v$ is
in the image. This question is local on $U$. Hence we may assume
that $U$ is affine and maps into an affine open of $Y$.
Thus we may assume $Y$ is affine which implies that $X$ is affine too
(by definition of an integral morphism).
Say $Y = \text{Spec}(A)$, $X = \text{Spec}(B)$ and $U = \text{Spec}(C)$.
The ring map $A \to B$ is integral and the ring map $B \to C$ is etale.
We can find a finite $A$-subalgebra $B' \subset B$ and an etale ring
map $B' \to C'$ such that $C \cong B \otimes_B' C'$, see
Algebra, Lemma \ref{algebra-lemma-etale}.
It follows that it suffices to prove the lemma for
$\text{Spec}(C') \to \text{Spec}(B') \to \text{Spec}(A)$.
Hence we may assume that $A \to B$ is a finite ring map.
In other words we may assume $f$ is a finite morphism of affine schemes
and $U$ is affine too.

\medskip\noindent
Let $y \in Y$ be the image of $u$. Apply
Lemma \ref{lemma-etale-splits-off-quasi-finite-part-technical-variant}
to $U \to Y$ and the point $u$ (but we pick only one of the finite parts).
This produces an etale neighbourhood
$(V, v) \to (Y, y)$ such that
$$
V \times_Y U = W \amalg W'
$$
with $W \to V$ finite with exactly one point $w$ mapping to $v$
such that $\kappa(v) \to \kappa(w)$ is purely inseparable.
Set $X' = V \times_Y X$. Here is a picture
$$
\xymatrix{
W \amalg W' \ar@{=}[r] \ar[d] &
V \times_Y U \ar[r] \ar[d] &
U \ar[d] \\
X' \ar@{=}[r] &
...
$$
In this situation $W \to X'$ is finite etale (use
Morphisms, Lemmas \ref{morphisms-lemma-finite-permanence} and
\ref{morphisms-lemma-base-change-etale}),
and $\kappa(x') \subset \kappa(w)$ is purely inseparable where $x'$ is
the image of $w$ in $X'$. Moreover $w$ is the only
point mapping to $x'$. It follows that $W \to X'$ is an isomorphism in
an open neighbourhood $(X')^\circ$ of $x'$ in $X'$, see
Etale Morphisms, Lemma \ref{etale-lemma-finite-etale-one-point}.
Since $X' \to V$ is finite, the image of $X' \setminus (X')^\circ$ in $V$
is a closed subset not containing $v$ (because $w$ is the only point
mapping to $v$). Thus we may remove this image from $V$ and after doing
so we get $W \cong X'$. For this
choice of $V$ we see that $X' \to X$ factors through $U$
as desired.
\end{proof}

\begin{lemma}
\label{lemma-prepare-integral-etale-almost-cocontinuous}
Let $f : X \to Y$ be an integral morphism of schemes.
Let $\{U_i \to X\}$ be an etale covering.
There exists an etale covering $\{V_j \to Y\}$ such that
for each $j$, either $V_j \times_Y X = \emptyset$, or the
morphism $V_j \times_Y X \to X$ factors through $U_i$ for some $i$.
\end{lemma}

\begin{proof}
For each $i$ we can choose etale morphisms
$g_{ij} : V_{ij} \to Y$ and morphisms $V_{ij} \times_Y X \to U_i$ over $X$,
such that $\{V_{ij} \times_Y X \to U_i\}$ are etale coverings, see
Lemma \ref{lemma-integral-etale-cover-from-below}.
This in particular implies that
$\bigcup_{ij} g_{ij}(V_{ij})$ contains the closed subset $f(X)$.
Hence the family of etale maps $g_{ij}$ together with the open immersion
$Y \setminus f(X) \to Y$ forms the desired etale covering of $Y$.
\end{proof}

\begin{lemma}
\label{lemma-integral-etale-almost-cocontinuous}
Let $f : X \to Y$ be an integral morphism of schemes.
Then the functor $V \mapsto X \times_Y V$ defines an almost
cocontinuous functor (see
Sites, Definition \ref{sites-definition-almost-cocontinuous})
$Y_{etale} \to X_{etale}$ between small etale sites
and an almost cocontinuous functor
$(\textit{Sch}/Y)_{etale} \to (\textit{Sch}/X)_{etale}$ between
big etale sites.
\end{lemma}

\begin{proof}
We have to show the following: given a morphism $V \to Y$
and any etale covering $\{U_i \to X \times_Y V\}$, there exists an
etale covering $\{V_j \to V\}$ such that for each $j$, either
$X \times_Y V_j$ is empty, or
$X \times_Y V_j \to Z \times_Y V$ factors through one of
the $U_i$. This follows on applying
Lemma \ref{lemma-prepare-integral-etale-almost-cocontinuous}
above to the integral morphism $X \times_Y V \to V$.
\end{proof}

\begin{lemma}
\label{lemma-integral-etale-pushforward-exact}
Let $f : X \to Y$ be an integral morphism of schemes.
(For example a finite morphism or a closed immersion.)
Let $f_{small} : \textit{Sh}(X_{etale}) \to \textit{Sh}(Y_{etale})$
be the corresponding morphism of small etale topoi. Then
\begin{enumerate}
\item $f_{small, *}$ is faithful and reflects injections, surjections, and
isomorphisms,
\item $f_{small}^{-1}f_{small, *}\mathcal{F} \to \mathcal{F}$ is surjective
for every sheaf $\mathcal{F}$ on $X_{etale}$,
\item $f_{small, *}$ on sheaves of sets commutes with coequalizers and
pushouts,
...
If 
$f_{big} :
\textit{Sh}((\textit{Sch}/X)_{etale})
\to
\textit{Sh}((\textit{Sch}/Y)_{etale})$
is the associated morphism of big etale topoi, then
\begin{enumerate}
\item[(5)] $f_{big, *}$ on sheaves of sets commutes with coequalizers
and pushouts, and
\item[(6)] $f_{big, *} :
\textit{Ab}((\textit{Sch}/X)_{etale})
\to
\textit{Ab}((\textit{Sch}/Y)_{etale})$
is exact.
\end{enumerate}
\end{lemma}

\begin{proof}
First, consider the small etale sites. By
Lemma \ref{lemma-integral-etale-cover-from-below}
we see that
Sites, Lemma \ref{sites-lemma-cover-from-below}
applies, and we get (1) and (2). Parts (3) and (4)
follows from
...
because
Lemma \ref{lemma-integral-etale-almost-cocontinuous}
garantees that we may apply these abstract results.

\medskip\noindent
In the case of the big sites the continuous functor
$V/Y \mapsto X \times_Y V/X$ which induces $f_{big}$ is
almost cocontinuous by
Lemma \ref{lemma-integral-etale-almost-cocontinuous}.
Hence we get properties (5) and (6) from
Sites, Lemma \ref{sites-lemma-morphism-of-sites-almost-cocontinuous}
and
Modules on Sites,
Lemma \ref{sites-modules-lemma-morphism-ringed-sites-almost-cocontinuous}.
...
Let $f : X \to Y$ be a closed immersion of schemes.
Let $\{U_i \to X\}$ be a syntomic (resp.\ smooth) covering.
There exists a syntomic (resp.\ smooth) covering $\{V_j \to Y\}$
such that for each $j$, either $V_j \times_Y X = \emptyset$, or the
morphism $V_j \times_Y X \to X$ factors through $U_i$ for some $i$.
\end{lemma}

\begin{proof}
For each $i$ we can choose syntomic (resp.\ smooth) morphisms
$g_{ij} : V_{ij} \to Y$ and morphisms $V_{ij} \times_Y X \to U_i$ over $X$,
such that $\{V_{ij} \times_Y X \to U_i\}$ are Zariski coverings, see
Lemma \ref{lemma-closed-immersion-cover-from-below}.
This in particular implies that
$\bigcup_{ij} g_{ij}(V_{ij})$ contains the closed subset $f(X)$.
Hence the family of synomtic (resp.\ smooth) maps $g_{ij}$ together
with the open immersion $Y \setminus f(X) \to Y$ forms the desired
syntomic (resp.\ smooth) covering of $Y$.
\end{proof}
...
\begin{proof}
We have seen this for $\tau = etale$ in
Lemma \ref{lemma-integral-etale-almost-cocontinuous}.
We have to show the following: given a morphism $V \to Y$
and any syntomic (resp.\ smooth)
covering $\{U_i \to X \times_Y V\}$, there exists a
smooth (resp.\ smooth) covering $\{V_j \to V\}$ such that for each $j$, either
$X \times_Y V_j$ is empty, or
$X \times_Y V_j \to Z \times_Y V$ factors through one of
the $U_i$. This follows on applying
...
Let $f : X \to Y$ be a closed immersion of schemes.
Let $\tau \in \{syntomic, smooth\}$ (the etale case is handled
in Lemma \ref{lemma-integral-etale-pushforward-exact}).
\begin{enumerate}
...
\noindent
In this section we prove some results on the adjunction mapping
$f^{-1}f_*\mathcal{F} \to \mathcal{F}$ in a bunch of different settings.

\begin{lemma}
\label{lemma-monomorphism-push-pull}
...
In the situation of
Lemma \ref{lemma-monomorphism-push-pull}
it is also the case that the canonical map
$\mathcal{F} \to f_{big}^{-1}f_{big!}\mathcal{F}$
is an isomorphism for sheaves of sets (by the same proof as
the proof of the lemma) and also for sheaves of abelian groups.
Note that the functor $f_{big!}$ for sheaves of abelian groups
is defined in
...
Let $f : X \to Y$ be a morphism of schemes.
Assume (A) above. Then the canonical map
$f_{small}^{-1}f_{small, *}\mathcal{F} \to \mathcal{F}$
is surjective for any sheaf $\mathcal{F}$ on $X_{etale}$.
\end{lemma}

\begin{proof}
Let $s \in \mathcal{F}(U)$ for some object $U$ of $X_{etale}$.
We have to show that $s$ is locally in the image.
By assumption we can find a covering $W_i \to U$ where each $W_i$ is
an open and closed subscheme of $X \times_Y V_i$ for some object
$V_i$ of $Y_{etale}$. The sheaf condition guarantees that the
restriction maps
$$
\mathcal{F}(X \times_Y V_i)
\longrightarrow
\mathcal{F}(W_i)
$$
are surjective. Hence this reduces us to the case that
$U = X \times_Y V_i$. In this case the map
$$
(f_{small}^{-1}f_{small, *}\mathcal{F})(X \times_Y V_i)
\longrightarrow
\mathcal{F}(X \times_Y V_i)
$$
is equal to the map
$$
(f_{small, *}f_{small}^{-1}f_{small, *}\mathcal{F})(V_i)
\longrightarrow
(f_{small, *}\mathcal{F})(V_i)
$$
which is surjective as the canonical map
$f_{small, *}\mathcal{F} \to f_{small, *}f_{small}^{-1}f_{small, *}\mathcal{F}$
is a one-sided inverse of the map
$f_{small, *}f_{small}^{-1}f_{small, *}\mathcal{F} \to
f_{small, *}\mathcal{F}$.
\end{proof}
...
\item $f_{small, *}$ reflects surjections.
\end{enumerate}
\end{lemma}
...
The other two properties follow formally, see
Modules on Sites, Lemma \ref{sites-modules-lemma-exactness-properties}.
\end{proof}
...
Then the functor $f_{small, *}$ transforms surjections into surjections.
\end{lemma}
...
Let $f : X \to Y$ be a morphism of schemes.
Denote $f_{small} : \textit{Sh}(X_{etale}) \to \textit{Sh}(Y_{etale})$
the associated morphism of small etale topoi. Assume
$f$ is integral (for example finite). Then
\begin{enumerate}
\item the map
$f_{small}^{-1}f_{small, *}\mathcal{F} \to \mathcal{F}$
is surjective for any sheaf $\mathcal{F}$ on $X_{etale}$,
\item $f_{small, *}$ is faithful,
\item $f_{small, *}$ reflects surjections,
\item $f_{small, *}$ transforms surjections into surjections, and
\item $f_{small, *} : \textit{Ab}(X_{etale}) \to \textit{Ab}(Y_{etale})$
is exact.
\end{enumerate}
\end{lemma}
...
Let $f : X \to Y$ be a morphism of schemes. Assume (C) holds.
Then the functor $f_{small, *}$ is faithful and reflects isomorphisms,
injections and surjections. Moreover, for any sheaf $\mathcal{F}$ on
$X_{etale}$ the canonical map
$f_{small}^{-1}f_{small, *}\mathcal{F} \to \mathcal{F}$ is surjective.
\end{lemma}
...
example shows that it does not imply that $f_*$ is exact.
...
morphism $U \to X$ is a local isomorphism. But $j_*$ is not exact on
$\textit{Ab}(X_{etale})$ (details omitted).
\end{remark}

\begin{lemma}
\label{lemma-property-C-universally-closed-implies}
Let $f : X \to Y$ be a morphism of schemes. Assume (C) holds
and that $f$ is universally closed. Then the functor
...
\begin{lemma}
\label{lemma-closed-immersion-push-pull-etale}
Let $f : X \to Y$ be a closed immersion of schemes.
Denote $f_{small} : \textit{Sh}(X_{etale}) \to \textit{Sh}(Y_{etale})$.
the associated morphism of small etale topoi.
Then the canonical map
$f_{small}^{-1}f_{small, *}\mathcal{F} \to \mathcal{F}$
is an isomorphism for any sheaf $\mathcal{F}$ on $X_{etale}$.
\end{lemma}

\begin{proof}
A closed immersion is separated, see
Schemes, Lemma \ref{schemes-lemma-immersions-monomorphisms},
and locally quasi-finite, see
Morphisms, Lemma \ref{morphisms-lemma-immersion-locally-quasi-finite}.
Hence $f_{small}^{-1}f_{small, *}\mathcal{F} \to \mathcal{F}$
is surjective by Lemma \ref{lemma-locally-quasi-finite-push-pull-etale}.

\medskip\noindent
By definition the sheaf $f_{small}^{-1}f_{small, *}\mathcal{F}$ is the
sheaf associated to the presheaf
$$
U \longmapsto \text{colim}_{a : U \to X \times_Y V} \mathcal{F}(X \times_Y V)
$$
where $V \to Y$ is etale and $a : U \to X \times_Y V$ is a morphism over $X$.
The map to $\mathcal{F}(U)$ maps $t \in \mathcal{F}(X \times_Y V)$ to
$a^*t \in \mathcal{F}(U)$. By
Sites, Lemma \ref{sites-lemma-directed}
the category of pairs $(V, a)$ is filtered. Suppose we have
a pair $s_1, s_2 \in \mathcal{F}(X \times_Y V)$ such that
$a^*s_1 = a^*s_2$. If we show that there exists a a pair $(V', a')$
mapping to $(V, a)$ such that the restrictions
$s_1|_{X \times_Y V'}$ and $s_2|_{X \times_Y V'}$ are equal, then we
are done. Note that $a(U) \subset X \times_Y V$ is an open subscheme, and that
$X \times_Y V \subset V$ is closed. Hence there
exists an open $V' \subset V$ such that $a(U) = X \times_Y V'$.
Then $\{a : U \to X \times_Y V'\}$ is an etale covering which implies
that $s_1|_{X \times_Y V'} = s_2|_{X \times_Y V'}$ as desired.
\end{proof}

\begin{remark}
\label{remark-finite-universally-injective}
Lemma \ref{lemma-closed-immersion-push-pull-etale}
also holds if $f : X \to Y$ is finite and universally injective.
In fact the proof works in this case also. But the correct generality
is probably the case where $f$ is integral and universally injective.
\end{remark}
