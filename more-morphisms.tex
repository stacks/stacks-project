...
of $Z_s$. If $\mathcal{F}$ is flat over $S$ at $x$, then
...
every fibre. If we ever need these results, we will carefull state and
prove them here.
\end{remark}
...
\section{The resolution property after blowing up}
\label{section-cd-by-blowing-up}

\noindent
We prove a result from \cite{Gross-thesis} that will be useful when defining
chern classes of perfect objects of the derived category.

\begin{lemma}
\label{lemma-get-resolution-property}
...
is ample. Then $X$ has the resolution property.
\end{lemma}

\begin{proof}
Let $\mathcal{F}$ be a finite type quasi-coherent module on $X$.
Fix an index $a \in \{1, \ldots, m\}$.
Set $U_a = X \setminus D_a$. We can find $r, n \geq 0$ and a surjection
$$
\bigoplus\nolimits_{i = 1, \ldots, r} \mathcal{L}_a^{\otimes -n}|_{U_a}
\longrightarrow
\mathcal{F}|_{U_a}
$$
This is true because $\mathcal{L}_a|_{U_a}$ is ample, see
Properties, Properties \ref{properties-proposition-characterize-ample}.
Denote $s_1, \ldots, s_r$ the components of the displayed map
viewed as sections
$$
s_i \in \Gamma(U_a,
\mathcal{F} \otimes_{\mathcal{O}_X} \mathcal{L}_a^{\otimes n})
$$
Since $U_a$ is the locus where the canonical section
$1 \in \mathcal{O}_X(D_a)$ doesn't vanish, we see from
Properties, Lemma \ref{properties-lemma-invert-s-sections}
there exists an $N \geq 0$ such that $s_i$ extends to a section
$$
s'_i \in  \Gamma(U_a,
\mathcal{F}
\otimes_{\mathcal{O}_X} \mathcal{L}_a^{\otimes n}
\otimes_{\mathcal{O}_X} \mathcal{O}_X(N D_a))
$$
In conclusion, we obtain a map
$$
\bigoplus\nolimits_{i = 1, \ldots, r}
\mathcal{L}_a^{\otimes n}
\otimes_{\mathcal{O}_X} \mathcal{O}_X(-N D_a))
\longrightarrow
\mathcal{F}
$$
which is surjective over $U_a$. Doing this for all $a$
and using that $X = U_1 \cup \ldots \cup U_m$
we find that $\mathcal{F}$ is the quotient of a
...
such that the scheme $Y$ has the resolution property.
\end{proposition}
...
(Morphisms, Lemma \ref{morphisms-lemma-base-change-proper}),
$f$ is completely decomposed
(Lemma \ref{lemma-base-change-cd}), and
$Y$ has the resolution property as a scheme affine over $Y_0$, see
Derived Categories of Schemes, Lemma
\ref{perfect-lemma-resolution-property-goes-up-affine}.
This reduces us to the case discussed in the next paragraph.
...
Properties, Lemma \ref{properties-lemma-point-and-maximal-points-affine}
and the fact that $X$ is quasi-compact we can find an affine
open covering $X = U_1 \cup \ldots \cup U_m$ such that
each $U_i$ contains $\eta_1, \ldots, \eta_n$. In particular
the open subset $U = U_1 \cap \ldots \cap U_m$ is dense in $X$.
Let $\mathcal{I}_i \subset \mathcal{O}_X$ be a coherent ideal
sheaf such that $U_i = X \setminus Z_i$ where $Z_i = V(\mathcal{I}_i)$.
Let
$$
...
Then $f$ is a projective morphism and
...
Moreover, $f$ maps the complement $X' \setminus D_i$ maps to the affine
scheme $U_i$. Hence the restriction of $\mathcal{O}_X(1)$
to $X' \setminus D_i$ is ample. It follows from
Lemma \ref{lemma-get-resolution-property}
that $X'$ has the resolution property.

\medskip\noindent
Since the center of the blowing up $f : X' \to X$ does not meet
$U = U_1 \cap \ldots \cap U_m$ we see that every point of
$U$ lifts to a point of $X'$ with the same residue field.
...
morphism $W \to Z$ such that $W$ has the resolution property.
Then it follows that $Y = X' \amalg W \to X$ is the desired
morphism.
\end{proof}
