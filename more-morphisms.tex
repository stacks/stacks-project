...
\begin{lemma}
\label{lemma-locally-quasi-finite-push-pull-etale}
...
\begin{proof}
Let $U \to Y$ be etale, and let $s \in \mathcal{F}(U)$ be a section.
We have to show there exists an etale covering $\{U_i \to U\}$ such
that $s|_{U_i}$ is in the image of the map
$(f_{small}^{-1}f_{small, *}\mathcal{F})(U_i) \to \mathcal{F}(U_i)$
for each $i$.
...
$a^*t \in \mathcal{F}(U)$.
Hence in order to prove the lemma it suffices to prove the following:
Given $U \to X$ etale we can find etale morphisms $V_i \to Y$,
etale morphisms $g_i : U_i \to U$ and morphisms $a_i : U_i \to X \times_Y V_i$
over $X$ such that
\begin{enumerate}
\item $U = \bigcup g_i(U_i)$,
\item $a_i$ identifies $U_i$ with an open and closed
subscheme of $X \times_Y V_i$.
\end{enumerate}
Namely, if this is true we can clearly find a section
$t_i \in \mathcal{F}(X \times_Y V_i)$ restricting to $s|_{U_i}$ via $a_i$
because the sheaf condition garantees that the map
$\mathcal{F}(X \times_Y V_i) \to \mathcal{F}(U_i)$ is surjective.

\medskip\noindent
This geometric statement reduces directly to the case where $U$ and $Y$
are affine schemes. Pick $u \in U$ and denote $x \in X$ and $y \in Y$ its
images. Since $X \to Y$ is locally quasi-finite, and $U \to X$ is
...
Now $g : W' \to U$ and $a : W' \to X \times_Y V$ is a pair as above.
By construction $u \in U$ is in the image of $g$. By varying $u \in U$
we can find our system $(g_i, U_i, a_i)$ as above and we win.
\end{proof}
