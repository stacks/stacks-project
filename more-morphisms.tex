...
We freely use the notation introduced in Section \ref{section-thickenings}.
...
of modules. Let $X \subset X'$ be a first order thickening of schemes.
We freely use the notation introduced in Section \ref{section-thickenings}.
Let $\mathcal{F}'$ be a quasi-coherent $\mathcal{O}_{X'}$-module
...
$$
of quasi-coherent $\mathcal{O}_{X'}$-modules where
...
the corresponding quasi-coherent $\mathcal{O}_X$-module
(Morphisms, Lemma \ref{morphisms-lemma-i-star-equivalence}), in other
words
$$
...
\begin{lemma}
\label{lemma-inf-aut}
Let $X \subset X'$ be a first order thickening of schemes. Let $\mathcal{F}'$, $\mathcal{G}'$ be quasi-coherent
$\mathcal{O}_{X'}$-modules. Set $\mathcal{F} = \mathcal{F}'|_X$ and
$\mathcal{G} = \mathcal{G}'|_X$. Let $\varphi : \mathcal{F} \to \mathcal{G}$
be an $\mathcal{O}_X$-linear map. The set of lifts of $\varphi$ to
an $\mathcal{O}_{X'}$-linear map $\varphi' : \mathcal{F}' \to \mathcal{G}'$
is, if nonempty, a principal homogenous space under
$$
...
order thickenings. Let $\mathcal{F}'$ be a quasi-coherent
$\mathcal{O}_{X'}$-module and set $\mathcal{F} = \mathcal{F}'|_X$.
Assume that $\mathcal{F}$ is flat over $S$ and $X = S \times_{S'} X'$.
...
\mathcal{C}_{X/X'}\mathcal{F}'$ is an isomorphism as well.
\end{lemma}
...
for some square zero ideals. Since $X = B \times_{B'} X'$ we
have $J = IB'$. Let $\mathcal{F}'$ correspond to the $B'$-module
$M'$ and $\mathcal{F}$ to the $B$-module $M$. Since
$\mathcal{F} = \mathcal{F}'|_X$ we see that the kernel of the
surjection $M' \to M$ is $JM' = IM'$. Thus we have a short exact
sequence
$$
0 \to IM' \to M' \to M \to 0
$$
The canonical map of the lemma is the map
$$
...
\medskip\noindent
To see the final statement we have to show that
$\gamma : IB' \otimes_B M \to IM'$
is injective if $\beta : I \otimes_A M \to IM'$ is injective. This is clear
as there is a surjective map $\alpha : I \otimes_A M \to IB' \otimes_B M$
such that $\gamma \circ \alpha = \beta$.
\end{proof}

\begin{lemma}
\label{lemma-inf-aut-rel}
Let $(f, f') : (X \subset X') \to (S \subset S')$ be a morphism of first
order thickenings. Let $\mathcal{F}'$, $\mathcal{G}'$ be quasi-coherent
...
\begin{proof}
Combine Lemmas \ref{lemma-inf-aut} and \ref{lemma-deform-module}.
\end{proof}

\begin{lemma}
\label{lemma-obs-lift-map}
...
\begin{proof}
It is clear from the proof of Lemma \ref{lemma-inf-aut} that the
...
This follows from Lemma \ref{lemma-obs-lift-map}
...
Let $f : X \to S$ be a morphism of schemes of finite presention.
...
neigbourhood $f'(x) \in V \subset S'$ such that $(f')^{-1}V \to V$ is an
