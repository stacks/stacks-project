...
$$
S_1 = S_1 \subset S_2 \ldots \ldots S_t = \emptyset
$$
...
\begin{lemma}
\label{lemma-smooth-part-relative-dim}
Let $f : X \to S$ be a morphism of schemes. Let $r \geq 0$. Assume
\begin{enumerate}
\item $S$ is quasi-compact and quasi-separated,
\item $X$ is quasi-separated,
\item $f$ is locally of finite type and of relative dimension $\leq r$,
\item there exists a closed subscheme $X' \subset X$ such that
$X' \to S$ has relative dimension $< r$ and such that $X \setminus X'$
is of finite presentation over $S$.
\end{enumerate}
Then there exist
\begin{enumerate}
\item an integer $t \geq 0$,
\item morphisms $T_i \to S$ for $i = 1, \ldots, t$,
\item integers $t_i \geq 0$ for $i = 1, \ldots t$,
\item closed subschemes $Z_{i, j} \subset T_i \times_S X$
for $j = 1, \ldots, t_i$ and $i = 1, \ldots, t$
\end{enumerate}
such that
\begin{enumerate}
\item $T_i \to S$ is of finite presentation,
\item $\coprod T_i \to S$ is surjective,
\item $T_i \times_S X \setminus \bigcup Z_{i, j}$
has relative dimension $< r$ over $T_i$,
\item for $j \not = j'$ the scheme $Z_{i, j} \cap Z_{i, j'}$ has
relative dimension $< r$ over $T_i$,
\item for each $j$ there exists a closed subscheme $Z'_{i, j} \subset Z_{i, j}$
such that $Z'_{i, j} \to T_i$ has relative dimension $< r$, such that
$Z_{i, j} \setminus Z'_{i, j}$ is dense in every fibre of $Z_{i, j} \to T_i$,
and such that the morphism
$$
Z_{i, j} \setminus Z'_{i, j}  \longrightarrow T_i
$$
is of finite presentation, is smooth of relative dimension $r$, and
has geometrically irreducible fibres.
\end{enumerate}
\end{lemma}

\begin{proof}
Let $X' \subset X$ be as in assumption (3). Set $U = X \setminus X'$.
In this paragraph we will show that it suffices to prove the lemma for
the morphism $f|_U : U \to S$. Namely, suppose that we have
$$
t, T_i \to S, t_i, Z_{i, j} \subset T_i \times_S U, Z'_{i, j} \subset Z_{i, j}
$$
as in the statement of the lemma for the morphism $U \to S$.
Denote $\overline{Z}_{i, j} \subset T_i \times_S X$ the scheme theoretic
image of the morphism $Z_{i, j} \setminus Z'_{i, j} \to T_i \times_S X$.
Then we claim that
$$
t, T_i \to S, t_i, \overline{Z}_{i, j} \subset T_i \times_S X
$$
are as in the statement of the lemma for the morphism $X \to S$.
To see this, we argue using the following steps:
\begin{enumerate}
\item $Z_{i, j} \setminus Z'_{i, j} \to T_i \times_S X$
is a quasi-compact immersion,
\item $Z_{i, j} \setminus Z'_{i, j}$ is a dense open in
$\overline{Z}_{i, j}$, say with complement $\overline{Z}'_{i, j}$,
\item every point of $\overline{Z}_{i, j}$ is the specialization
of a point of $Z_{i, j} \setminus Z'_{i, j}$,
\item every irreducible component of every fibre of
$\overline{Z}_{i, j} \to T_i$ has dimension $\geq r$,
\item the fibres of $\overline{Z}_{i, j} \to T_i$ are irreducible
(namely, the intersection of $\overline{Z}_{i, j}$ with $Z_{i, j'}$
cannot contain a generic point of a fibre),
\item $Z_{i, j} \setminus Z'_{i, j} =
\overline{Z}_{i, j} \setminus \overline{Z}'_{i, j}$ is dense in every fibre
of $\overline{Z}_{i, j} \to T_i$.
\end{enumerate}
The other statements are easier to obtain; we omit the details.

\medskip\noindent
By the result of the previous paragraphy, we see that we may
assume $f$ is of finite presentation. Then we can reduce to the
Noetherian case. Then we can reduce to $S$ is spec of a field.
Then it is easy. FIXME.
\end{proof}
