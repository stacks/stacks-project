...
$$
The difference $D = a'' - a'$ is a map from $A$ to $I$.
All we have to show is that it is an $R$-derivation, since then it will
...
open coverings to prove (3). A more global argument follows.
...
Let $S$ be a scheme. Let $i : T \to T'$ be a closed immersion of
schemes over $S$ defined by an ideal $\mathcal{I}$ of square zero.
Let $X$ be a scheme over $S$. Let
$a'_1, a'_2 : T' \to X$ be two morphisms of schemes over $S$ which
agree over $T$, say $a = a'_1|_T = a'_2|_T$.
This gives rise to a commutative diagram
$$
\xymatrix{
T \ar[r] \ar[d]_a & T' \ar[d]^{(a'_1, a'_2)} \\
X \ar[r]^-{\Delta_{X/S}} & X \times_S X
}
$$
Since the horizontal arrows are immersions with conormal sheaves
$\mathcal{I}$ and $\Omega_{X/S}$, by
Morphisms, Lemma \ref{morphisms-lemma-conormal-functorial},
we obtain a map $\theta : a^*\Omega_{X/S} \to \mathcal{I}$.
Then $a'_2 = \theta \cdot a'_1$
where $\cdot$ is the action defined in
...
Being etale is the same as being smooth and G-unramified
