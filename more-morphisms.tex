...
may assume that $Y = \text{Spec}(A)$ where $A$ is a domain.
...
\section{A technical application}
...
over $\overline{\kappa(t)}$ hence Noetherian. By choive of $S' \to S$.
$V_t \times_{\kappa(t)} \overline{\kappa(t)}$ is connected. Hence
$V_t \times_{\kappa(t)} \overline{\kappa(t)}$ is irreducible by
...
$s' = h(y')$ we have
\begin{enumerate}
\item $h : Y' \to S'$ is smooth of relative dimension $n$ with
...
\item $e$ is etale and $\kappa(s) = \kappa(s')$.
\end{enumerate}
\end{lemma}
...
a purely transcendental extension of $\kappa(s)$. Let $T \subset Y_s$
be the irreducible component containing $y$. Then $T$ is geometrically
connected over $\kappa(s)$ by
Varieties, Lemma \ref{varieties-lemma-geometrically-connected-criterion}.
Since it is also smooth over $\kappa(s)$ it is geometrically normal
and connected, hence geometricallu irreducible. Finally, note that
the smooth morphism $h$ is normal by
Lemma \ref{lemma-smooth-normal}.
Thus we may apply
Lemma \ref{lemma-normal-morphism-irreducible}
to $h : Y \to S$ and $T \subset Y_s$ to get the following extension of
the diagram
$$
\xymatrix{
X \ar[dd] & X' \ar[l]^g \ar[d]^\pi & & x' \ar@{|->}[d] \\
& Y \ar[d]^h & Y' \ar[d] \ar[l] & y \ar@{|->}[d] \\
S \ar@{=}[r] & S & S' \ar[l]_e & s & s' \ar@{|->}[l]
}
$$
...
the identification $Y_s = Y_{S', s'}$. Let $y' \in Y'$ be the point
corresponding to $y \in T$. Finally, replace $X'$ by $X' \times_Y Y'$
and $x'$ by $(x', y') \in X' \times_Y Y'$. This proves the lemma.
\end{proof}
