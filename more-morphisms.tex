...
Descent, Lemma \ref{fpqc-descent-lemma-syntomic-smooth-etale-permanence}
...
Descent, Lemma \ref{fpqc-descent-lemma-radicial-etale-open-immersion}
...
\noindent
In this section we discuss descent of etale morphisms in various topologies.
It may even be the case that
``etale morphisms satisfy descent for fpqc-coverings'',
but this seems to be quite hard if true. For the moment
we just show that
``separated etale morphisms satisfy descent for fppf-coverings''.
This is in the marvellous (for many reasons) paper by Raynaud and Gruson
hidden in the proof of \cite[Lemma 5.7.1]{GruRay}.
Here is the formal statement.

\begin{lemma}
\label{lemma-separated-etale-morphisms-fppf-descend}
Let $S$ be a scheme.
...
$V_i \to X_i$ is separated and etale,
then the descent datum is effective.
\end{lemma}

\begin{proof}
Being separated/etale are properties of morphisms of schemes
...
Hence Descent, Lemma \ref{fpqc-descent-lemma-descending-types-morphisms}
...
and assume that $\pi : V \to X$ is separated and etale.
...
and etale. This implies that it is quasi-finite and separated
by Morphisms, Lemmas
\ref{morphisms-lemma-flat-unramified-etale} and
\ref{morphisms-lemma-unramified-quasi-finite}.
Hence it is quasi-affine by
Lemma \ref{lemma-quasi-finite-separated-quasi-affine}.
Thus by
Descent, Lemma \ref{fpqc-descent-lemma-quasi-affine}
