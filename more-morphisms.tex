...
\medskip\noindent
To prove the lemma we have to show $j$ an open immersion see
...
\medskip\noindent
There are many ways to conclude from this observation, and we suggest
the reader finds his/her own argument at this point. We give below one
possible reasoning.

\medskip\noindent
First, the observations above show that the
...
because $Y' \to Y$ is the base change of the etale morphism $S' \to S$.
Since also $X' \to X$ is etale for the same reason we see that
$g'(V) \subset X$ is open (see \ref{}) and $V \to g'(V)$ is etale.
We conclude that $X \to Y$ is etale at $x$, see.



Note that $x$ corresponds
to a unique point $x' \in V$ lying over $s'$. Consider the diamgram
of local rings
$$
\xymatrix{
\mathcal{O}_{V, x'} &
\mathcal{O}_{Y', j'(x')} \ar[l] \\
\mathcal{O}_{X, x} \ar[u] &
\mathcal{O}_{Y, j(x)}. \ar[u] \ar[l]
}
$$
All residue fields are the same. The vertical arrows are (essentially)
etale as they come by base change from the etale ring map
$\mathcal{O}_{S, s} \to \mathcal{O}_{S', s'}$. The top horizontal
...
By Coherent, Lemma \ref{coherent-lemma-flat-base-change-cohomology}
for any flat morphism of schemes
$g : U \to S$ we have $g^*\mathcal{A} = f'_*\mathcal{O}_{X_U}$.
