...
Thus we may assume that $Y$ is integral. By
...
$$
Hence some irreducible component of $Z_\eta$ through $\xi'$ has
...
of the form $L = (l_1, \ldots, l_{n - 1})$ and $0 \leq l_i \leq d_i$.
For any $e_1, e_2 \geq 0$ in $x_n$ with
$e_1 + e_2 = d$ we consider the $A$-algebra
$$
B_{e_1, e_2} =
...
replacing $A'$ by  $A'_f$ for a suitable $f \in A'$, by applying
Algebra, Lemma \ref{algebra-lemma-generic-flatness}.
...
$K$-extensions. Hence there exists an isomorphism $B_h \cong B'_{h'}$
for suitable $h \in B$ and $h' \in B'$. In other words $X$ and
$X' = \text{Spec}(B')$ have a common affine open $U$. Diagram:
$$
\xymatrix{
X = \text{Spec}(B) \ar[rd] &
U \ar[l] \ar[r] \ar[d] &
...
connected over $y'$ if and only if $X_y$ is geometrically connected
over $y$. This follows from
Varieties, Section \ref{varieties-section-geometrically-connected}.
\end{proof}
