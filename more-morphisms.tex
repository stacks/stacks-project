...
\label{item-affine}
\item $f$ is locally of finite type if and only if $f'$ is locally of finite
type,
\label{item-finite-type}
\item $f$ is quasi-finite if and only if $f'$ is quasi-finite,
\label{item-quasi-finite}
\item $f$ is locally of finite presentation if and only if $f'$ is locally of
finite presentation,
\label{item-finite-presentation}
\item $f$ is locally of finite type of relative dimension $d$ if and only if
...
\label{item-finite}
\item $f$ is finite locally free (of rank $d$) if and only if $f'$
...
Let $W' = \text{Spec}(R')$ and denote $I \subset R'$ be the ideal
defining the closed subscheme $W' \cap S$. Say $U' = \text{Spec}(B')$
and $V' = \text{Spec}(A')$. Then we get a commutative diagram
$$
\xymatrix{
0 \ar[r] &
IB' \ar[r] &
...
\medskip\noindent
We may write $S = \text{Spec}(A)$ and we may write $A = \colim_i A_i$
...
Write $Y = \text{Spec}(A)$ and $A = \colim A_i$ as a directed
...
$S = \text{Spec}(A)$. Write $A = \colim_i A_i$ as a
...
Let $V \subset \text{Spec}(B)$ be an open subscheme
...
$Y = \text{Spec}(R)$, $X = \text{Spec}(A)$, $X' = \text{Spec}(A')$
and $W = \text{Spec}(B)$, $A = R[x_1, \ldots, x_n]/I$ and
...
is perfect. An example is $S = \text{Spec}(k)$, $X = \text{Spec}(k)$,
$Y = \text{Spec}(k[x]/(x^2)$ and $X \to Y$ the unique $S$-morphism.
\end{remark}
...
$$
Set $P'' = \mathbf{A}^n_{P}$. Then $U \to P' \to P''$ is a
...
This means that $U = \text{Spec}(R[x_1, \ldots, x_n]/(f_1, \ldots, f_c))
\to V = \text{Spec}(R)$ where $R[x_1, \ldots, x_n]/(f_1, \ldots, f_c)$ is a
...
Divisors, Definition \ref{divisors-definition-relative-H1-regular-immersion}.
Since $s' = \text{Spec}(\kappa(s')) \to \text{Spec}(\kappa(s)) = s$
