...
\begin{proof}
We just define $S'$ as the normalization of $S$ in $X$, so (5) holds
automatically. By
Morphisms, Lemma \ref{morphisms-lemma-normalization-in-universally-closed}
we see that (4) holds. By
...
$A = \Gamma(X, \mathcal{O}_X)$ a finite $R$-algebra.
...
\medskip\noindent
Let us show that $f'$ is surjective. As $f'$ is proper (see above)
...
\medskip\noindent
Finally, we come to the last and most interesting assertion, namely that
...
we see that $X_s$ is geometrically connected if and only if
$X_s \times_{\text{Spec}(\kappa(s)} \text{Spec}(k)$ is connected for every
