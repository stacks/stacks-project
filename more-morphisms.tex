...
\begin{lemma}
\label{lemma-smooth-geometrically-irreducible}
...
\begin{enumerate}
\item $h$ is flat,
\item $h$ is locally of finite presentation,
\item $T$ is quasi-compact,
\item $T$ is geometrically integral over $\kappa(s)$.
\end{enumerate}
Then we can find an etale neighbourhood $(S', s') \to (S, s)$
and an open $V \subset Y_{S'}$ such that
\begin{enumerate}
\item[(a)] $\kappa(s) = \kappa(s')$,
\item[(b)] all fibres of $V \to S'$ are geometrically ???,
\item[(c)] $V_{s'} = T \times_s s'$.
\end{enumerate}
\end{lemma}
...
After replacing $Y$ by $V$ we may assume that $Y_s = T$.
In particular $Y$ is quasi-compact and $Y_s$ is geometrically integral.
Our goal is then to show that after shrinking $Y$, without shrinking $Y_s$,
and after replacing $S$ by an elementary etale neighbourhood of $s$
we get to a situation where all fibres of $Y \to S$ are ???.

\medskip\noindent
Write $Y = Y_1 \cup \ldots \cup Y_n$ as a finite union of affine opens.
If $(Y_i)_s = \emptyset$ then we may throw it out by shrinking $Y$.
Thus we may assume that each $Y_{i, s}$ is nonempty which implies
