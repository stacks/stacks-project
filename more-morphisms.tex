...
\begin{lemma}
\label{lemma-closed-immersion-C}
...
me in the image of $h$ and we win.
\end{proof}
...
is surjective for any sheaf $\mathcal{F}$ on $X_{etale}$,
\item $f_{small, *}$ is faithful,
\item $f_{small, *}$ reflects injections, surjections, and isomorphisms,
\item $f_{small, *}$ transforms surjections into surjections,
\item $f_{small, *} : \textit{Ab}(X_{etale}) \to \textit{Ab}(Y_{etale})$
is exact,
\item the functor $Y_{etale} \to X_{etale}$, $V \mapsto X \times_Y V$ is
almost cocontinuous, and
\item $f_{small, *}$ commutes with coequalizers and pushouts (and
more generally with any finite, nonempty, connected colimits)
\end{enumerate}
\end{lemma}
