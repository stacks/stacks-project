...
\begin{lemma}
\label{lemma-stein-factorization-Noetherian}
...
\end{enumerate}
By the Noetherian case (Lemma \ref{lemma-stein-factorization-Noetherian})
...
\begin{lemma}
\label{lemma-stein-factorization-general}
...
$$
\xymatrix{
X \ar[rd] & X' \ar[d] \ar[l]^\pi \ar[r] & \mathbf{P}^n_R \ar[dl] \\
& S = \text{Spec}(R) &
}
$$
...
$$
\xymatrix{
X \ar[d] & X' \ar[l]^\pi \ar[d] \ar[r] & \mathbf{P}^n_{A'} \ar[dl] \\
...
\quad\text{and}\quad
\Gamma(X_B, \mathcal{O}_{X'_{B \otimes_A A'}}) = B \otimes_A A'.
$$
Hence we may replace $A$ by $B$ and we reduce to proving that
$X_p$ is connected. By
Lemma \ref{lemma-stein-projective}
we see that $X' \to \text{Spec}(A')$ has geometrically connected fibres.

\medskip\noindent
Let $\mathfrak p_1', \ldots, \mathfrak p_n'$ be the primes of $A'$ lying
over $\mathfrak p$. There are only finitely many of these since they correspond
$1 - 1$ to connected components of $X'_p$ (which is proper over $\kappa(p)$
hence Noetherian, hence has finitely many components). Let $A'' \subset A'$
be a finitely generated $A$-subalgebra such that the primes
$A'' \cap \mathfrak p_i'$ are pairwise distinct. Note that
that $\mathfrak p_1' \cap A'', \ldots, \mathfrak p_n' \cap A''$ are the only
prime lying over $\mathfrak p$ as $\text{Spec}(A') \to \text{Spec}(A'')$
is surjective, see
...
lying over $\mathfrak q$. Replacing $A$ by $B$ as above, we see that we
may decompose $\text{Spec}(A') = V_1 \coprod \ldots \coprod V_n$
each with a single point $p_i' \in V_i$ lying over $p$.
Let $X_i' \subset X'$ be the inverse image of $V_i$.
Note that $X'_p = \coprod X'_{i, p}$ and each $X'_{i, p}$ is connected,
since it is the fibre of $X' \to \text{Spec}(A')$ over $p'_i$.
...
$g \not \in \mathfrak p$ we see that $X_I \cap X_J = \emptyset$.
...
$$
contains a nontrivial idempotent which is not in $A$! This contradicts
the assumption that $A = \Gamma(X, \mathcal{O}_X)$ and we win.
\end{proof}
