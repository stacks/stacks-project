...
\medskip\noindent
The translation of (5) into algebra: If $A \to B$ is unramified
...
\noindent
In this section we collect some results on pushouts in the
category of schemes. See Categories, Section \ref{section-pushouts} for a
general discussion of pushouts in any category.
...
\begin{proof}
As $X \to Y$ is flat we see that generic points of irreducible components
of $X$ map to generic points of irreducible components of $Y$, see
Morphisms, Lemma \ref{morphisms-lemma-generalizations-lift-flat}.
On the other hand, the fibres of $X \to Y$ are locally Noetherian
because they are locally of finite type over a field. Thus every
quasi-compact open of $X$ has finitely many irreducible components.
...
Let $f : X \to S$ be a morphism of schemes which is flat and locally
of finite presentation. Let $n \geq 0$ and let $U \subset S$ be
a dense open such that $\dim(X_u) \leq n$ for all $u \in U$.
Then $\dim(X_s) \leq n$ for all $s \in S$.
\end{lemma}

\begin{proof}
Let $W \subset X$ be the open subscheme constructed and studied in
Lemmas \ref{lemma-flat-finite-presentation-CM-open} and
\ref{lemma-flat-finite-presentation-pieces-given-relative-dimension}.
...
for $d > n$. Since $f$ is flat and locally of finite presentation it is open
...
$b, b_i \in B$. In other words, $b$ is a common denominator for the
...
\noindent
We can use the results Section \ref{section-etale-localization} to prove
the scheme theoretic version of Zariski's main theorem.
...
\begin{enumerate}
\item $f$ is separated, locally of finite presentation\footnote{You can
replace ``finite presentation'' with ``finite type'' by
...
$V \cap T \not = \emptyset$.
Let $r$ be the largest integer such that $f(T) \subset Z_r$
...
integral morphism $Y \to X$ where $Y$ is a scheme, such that for
...
\medskip\noindent
By Limits, Lemma \ref{limits-lemma-proper-limit-of-proper-finite-presentation}
we can write $X = \lim X_i$ with $X_i \to S$ proper and
of finite presentation. Then $X_s = \lim X_{i, s}$. Assume
$X_s = U \amalg V$ with $U$ and $V$ open. We have to show $U$ or $V$ is empty.
We can find an $i$ such that $X_{i, s} = U_i \amalg V_i$ for
opens $U_i, V_i$ whose inverse image in $X_s$ 
are $U, V$ (Limits, Lemma \ref{limits-lemma-descend-opens}). Set
$R_i = \Gamma(X_i, \mathcal{O}_{X_i})$ and $S_i = \Spec(R_i)$.
Observe that we have ring maps $R \to R_i \to R$ whose composition
is $\text{id}_R$. Namely, we have
the first because $X_i$ is a scheme over $S$ and the second because
we have $X \to X_i$ and $R = \Gamma(X, \mathcal{O}_X)$.
Then 
$$
\xymatrix{
...
$$
is commutative. Let $s_i \in S_i$ be the image of $s$. Observe
that $\kappa(s_i) = \kappa(s)$. We obtain a
factorization $X_s \to X_{i, s_i} \to X_{i, s}$.
Thus $U, V$ are the inverse images of the
inverse images of $U_i, V_i$ in $X_{i, s_i}$.
Hence it suffices to prove the result for the morphism $X_i \to S$, i.e.,
we may assume $X \to S$ is proper and of finite presentation.

\medskip\noindent
Assume $X \to S$ is proper of finite presentation, $S = \Spec(R)$
is affine, $R = \Gamma(X, \mathcal{O}_X)$ and $s \in S$ is a point.
We have to show $X_s$ is connected. Write $R = \colim R_i$
as a direct colimit with $R_i$ of finite type over $\mathbf{Z}$.
Set $S_i = \Spec(R_i)$. Then there exists $0 \in I$ and $X_0 \to S_0$
such that $X  = X_0 \times_{S_0} S$, see
Limits, Lemma \ref{limits-lemma-descend-finite-presentation}.
Then $X_i = X_0 \times_{S_0} S_i \to S_i$ is proper for $i$
large enough, see
Limits, Lemma \ref{limits-lemma-eventually-proper}.
Let $s_i \in S_i$ be the image of $s$. Then $s = \lim s_i$ (equality of
schemes) and $X_s = \lim X_{i, s_i}$ because limits commute with fibre
products. Assume $X_s = U \amalg V$ with $U$ and $V$ open. We have to
show $U$ or $V$ is empty. We can find an $i$ such that
$X_{i, s_i} = U_i \amalg V_i$ for opens $U_i, V_i$ whose inverse
image in $X_s$  are $U, V$ (Limits, Lemma \ref{limits-lemma-descend-opens}).
Set $R'_i = \Gamma(X_i, \mathcal{O}_{X_i})$ and $S'_i = \Spec(R'_i)$.
Here is a diagram
$$
\xymatrix{
X \ar[d] \ar[rr] & & X_i \ar[ld] \ar[d]  \\
S \ar[r] & S'_i \ar[r] & S_i
}
$$
By Theorem \ref{theorem-stein-factorization-Noetherian} the fibres
of $X_i \to S'_i$ are connected. Let $s'_i \in S'_i$ be the image of $s$.
Consider the factorization $X_s \to X_{i, s'_i} \to X_{i, s_i}$.
Since the fibre $X_{i, s'_i}$ is connected, either the inverse image
of $U_i$ or the inverse image of $V_i$ in $X_{i, s'_i}$
is empty, hence either $U$ or $V$ is empty. This finishes the proof.
\end{proof}
...
pseudo-coherent on $\mathbf{A}^{n + m}_S$. This is straightforward to
prove directly, but it also follows from
...
for some $m$, then $E$ is bounded above. We first prove the condition
of relative pseudo-coherence localizes well.
...
to conclude that the pushfoward $E'$ of $E$ to $\mathbf{A}^{n + 1}_S$
is $m$-pseudo-coherent.
...
$R\Gamma(E, U)$ in $D(R[x_1, \ldots, x_n])$ by
Derived Categories of Schemes, Lemma \ref{perfect-lemma-pseudo-coherent-affine}
...
By assumption $E$ is bounded above, say $H^i(E) = 0$ for $i > a$.
Represent $E$ by a complex $\mathcal{E}^\bullet$ of $\mathcal{O}_Y$-modules
with $\mathcal{E}^i = 0$ for $i > a$. Then the derived tensor product above
...
$p^*\mathcal{E}^\bullet
\otimes_{\mathcal{O}_{\mathbf{A}_Y^n}} \mathcal{F}^i[-i]$.
Note that for $i \ll 0$ the complexes
...
$p^*\mathcal{E}^\bullet \otimes_{\mathcal{O}_{\mathbf{A}_Y^n}}
\mathcal{F}^i[-i]$
is pseudo-coherent relative to $S$ for all $i$. Note that $\mathcal{F}^i = 0$
for $i > 0$. Since also $\mathcal{F}^i$ is finite free this
