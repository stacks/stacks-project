...
\medskip\noindent
Assume (3). Suppose $T$ is a scheme which
(a) is locally of finite type over a field $k$, and
(b) has finitely many points. Then every point of $T$ is
isolated (namely, $\dim(T) > 0$ would imply $T$ has
infinitely many points). Pick $s \in S$. By the remark just made we
...
by the observation in the proof of Lemma \ref{lemma-characterize-finite}.
By Morphisms, Lemma \ref{morphisms-lemma-quasi-finite-points-open} the
set of points where $f$ is quasi-finite is an open $U \subset X$.
...
$f^{-1}(V) \to V$ is quasi-finite and proper, hence finite by
Lemma \ref{morphisms-lemma-quasi-finite-points-open}.
\end{proof}
