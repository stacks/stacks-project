...
\noindent
We will use the notion of an almost cocontinuous functor
between sites introduced in
Sites, Section \ref{sites-section-almost-cocontinuous}.
...
cocontinuous functor $Y_{etale} \to X_{etale}$ between small etale sites
...
given a scheme $U \to X$ over $X$ and any etale covering
$\{V_j \to Y \times_X U\}$, there exists an etale covering
$\{U_i \to U\}$ such that for each $i$, either $Y \times_X U_i$ is
empty, or $Y \times_X U_i \to Y \times_X U$ factors through one of
the $V_j$. It is clear that the question is local on $U$. Hence we
may assume that $U$ is an affine scheme. Set $V = Y \times_X U$.
Then $V$ is affine too, and $V \to U$ is the morphism of affine schemes
associated to an integral ring map $A \to B$.

\medskip\noindent
Note that the covering $\{V_j \to V\}$ can be refined by a standard
etale covering, see
Topologies, Lemma \ref{topologies-lemma-etale-affine}.
Hence we may assume $\{V_j \to V\}_{j = 1, \ldots, m}$ is a standard
etale covering. This means that $V_j = \text{Spec}(B_j)$ and
$B \to B_j$ is an etale ring map. We can find a finite $A$-subalgebra
$B' \subset B$ and etale ring maps $B' \to B'_j$ such that
$B_j \cong B \otimes_{B'} B'_j$ for each $j$. See
Algebra, Lemma \ref{algebra-lemma-etale}.
Hence we may assume that $A \to B$ is a finite ring map.

\medskip\noindent
Pick $u \in U$ which is in the image of $V \to U$. This means there
exists a $j$ and $v_j \in V_j$ whose image is $u$. Apply
Lemma \ref{lemma-etale-splits-off-quasi-finite-part-technical-variant}
to $V_j \to U$ and the point $v_j$ (but we pick only one of the finite parts).
This produces an etale neighbourhood
$(U', u') \to (U, u)$ such that
$$
U' \times_U V_j = W \amalg W'
$$
with $W' \to U$ finite with exactly one point $w'$ mapping to $u'$
such that $\kappa(u') \to \kappa(w')$ is purely inseparable.
This implies that $W' \to U' \times_U V$ is finite etale, and that
$\kappa(v') \subset \kappa(w')$ is purely inseparable where $v'$ is
the image of $w'$ in $V' = U' \times_U V$. Moreover $w'$ is the only
point mapping to $v'$. It follows that
$\kappa(v') = \kappa(w')$ and that $W' \to V'$ is an isomorphism in
an open neighbourhood of $w'$ in $W'$ (because $W' \to V'$ is finite
locally free and the degree at $v'$ is $1$ by what we said above).
Since $W' \to U'$ is finite, after shrinking $U'$ we may arrange it
so that $W' \cong V'$. For this
choice of $U'$ we see that $U' \times_U V \to V$ factors through $V_j$!

\medskip\noindent
The preceding paragraph shows that for any $u \in U$ in the image of $V \to U$
we can find an etale neighbourhood $(U', u') \to (U, u)$ such that
$U' \times_U V \to V$ factors through $V_j$ for some $j$. On the other hand,
the etale morphism $U \setminus Z \to U$ covers all the other points (here
$Z \subset U$ is the image of $V \to U$ which is a closed subset).
...
an almost cocontinuous functor
...
The question is local on the site $(\textit{Sch}/Y)_{syntomic}$
hence we reduce to proving the following statement:
Let $A \to A/I$ be a surjection of rings.
Let $A/I \to \overline{B}_j$, $j = 1, \ldots, m$ be syntomic
ring maps such that $\coprod \text{Spec}(\overline{B}_j) \to \text{Spec}(A/I)$
is surjective. Then we can find a standard syntomic covering
$\{\text{Spec}(A_i) \to \text{Spec}(A)\}$ such that for each $i$
either $A_i/IA_i$ is the zero ring, or the ring map
$A/I \to A_i/IA_i$ factors through $A/I \to \overline{B}_j$ for some $j$.

\medskip\noindent
By Algebra, Lemma \ref{algebra-lemma-lift-syntomic}
we may assume (after refining the covering) that
$\overline{B}_i = A_i/IA_i$ for certain syntomic ring maps
$A \to A_i$. Combined with the ring maps $A \to A_f$, $f \in I$
we obtain a syntomic covering of $\text{Spec}(A)$ of the desired
type. Some details omitted. The proof of the smooth case is omitted.
(Hint: Use Algebra, Lemma \ref{algebra-lemma-lift-smooth} instead
of Algebra, Lemma \ref{algebra-lemma-lift-syntomic}.)
\end{proof}
...
fibre over $w$ with $\kappa(w) = \kappa(w')$. Hence $W' \to W$ is an isomorphism
in a neighbourhood of $w$ (because $W' \to W$ is finite
locally free and the degree at $w$ is $1$ by what we said above).
Since $W \to V$ is finite we may after shrinking
$V$ (similarly to the above) assume that $W' \to W$ is an isomorphism.
...
In fact the proof works in this case also.
\end{remark}
