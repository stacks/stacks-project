...
\section{Etale dominates smooth}
...
be a nonempty open. Then there exists a point $w \in W$ whose
residue field is a finite separable extension of $k$.
\end{lemma}
...
Let $h : Y \to S$ be a smooth morphism of schemes.
Let $s \in S$ be a point, let $y \in $ with $h(y) = s$.
Let $T \subset Y_s$ be an open subscheme which is geometrically
irreducible over $\kappa(s)$ and contains $y$.
Then there exist an etale neighbourhood $(S', s') \to (S, s)$
and an open $V \subset Y_{S'}$ such that
\begin{enumerate}
\item $\kappa(s) = \kappa(s')$,
\item $S'$ is affine,
\item $V$ is affine,
\item all fibres of $V \to S'$ are geometrically irreducible,
\item $V_{s'} \subset T \times_s s'$, and
\item there exist a point $v \in V$ mapping to $s' \in S'$ and
to $y \in Y$.
\end{enumerate}
\end{lemma}
...
Schemes, Lemma \ref{schemes-lemma-fibre-topological}.
Hence we can find an open $V \subset Y$ such that $V_s = T$.
After replacing $Y$ by $V$ we may assume that $Y_s = T$, i.e.,
that $Y_s$ is geometrically irreducible.
