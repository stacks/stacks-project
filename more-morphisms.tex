...
The following are equivalent
\begin{enumerate}
\item $\mathcal{F}$ is a flat $\mathcal{O}_S$-module,
\item $Lf^*\mathcal{F}$ is isomorphic to a flat $\mathcal{O}_X$-module
placed in degree $0$.
\end{enumerate}
\end{lemma}

\begin{proof}
We omit the proof that (1) implies (2). Assume (2). To prove that $\mathcal{F}$
is flat is a local question, hence we may assume that $S$ is affine.
...
$Lf_i^* Lg_i^*\mathcal{F}$ is isomorphic to a flat $\mathcal{O}_{Y_i}$-module
placed in degree $0$. Since $f_i$ is flat and surjective, we see that
$Lg_i^*\mathcal{F}$ is isomorphic to a flat $\mathcal{O}_{T_i}$-module
placed in degree $0$.

\medskip\noindent
We will prove this implies the result by induction on $t$.
We will use the following notation: $S = \Spec(A)$ and $\mathcal{F}$
is the quasi-coherent module associated to the $A$-module $M$.

\medskip\noindent
Base case: $t = 1$. Write $S_0 = \Spec(A/I)$ where
$I$ is a finite generated ideal. Then $I$ is nilpotent as $S_0 = S$
set theoretically. The assumption is that $\text{Tor}_i^A(M, A/I) = 0$
for $i > 0$ and $M/IM$ is a flat $A/I$-module. By
Algebra, Lemma \ref{algebra-lemma-what-does-it-mean}
we see that $M$ is a flat $A$-module.

\medskip\noindent
Induction step. Assume $t > 1$. Write $S_{t - 1} = \Spec(A/I)$
where $I = (f_1, \ldots, f_r)$ is a finitely generated ideal.
Observe that $M_{f_r}$ is flat over $A_{f_r}$ by induction hypothesis
(because the stratification on the principal open $D(f_r)$ has length
at most $t - 1$). On the other hand, we know that $M \otimes_A^\mathbf{L} A/I$
is equal to $M/IM$ and that this is a flat $A/I$-module.
We conclude that $M$ is a flat $A$-module by
Algebra, Lemma \ref{algebra-lemma-flatness-scallop-pre}.
\end{proof}
