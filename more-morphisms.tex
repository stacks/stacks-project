...
Replace $X$ by the $\overline{\{x\}}$. Apply
Lemma \ref{lemma-quasi-finite-quasi-section-meeting-nearby-open}
to $\mathcal{O}_{S, s'} \to \mathcal{O}_{X, x'}$ and any nonempty
open $V$ as in that lemma. This gives us an integral closed subscheme
$x' \in Z \subset X$ such that $Z \to S$ is quasi-finite at $x'$.
Let $\xi \in Z$ be the generic point. Then $\xi$ is a closed point
of its fibre, and $\xi \leadsto x'$. As $f(\xi) \not = s'$ we
see $\dim(\mathcal{O}_{S, f(\xi)}) < \dim(S)$. Apply the induction
hypothesis to $s \leadsto \xi$ to win.
\end{remark}
