...
Hence the vanising of $\Omega_{B/A}$ follows from Nakayama's
...
and a quasi-finite flat morphism $U \to \mathbf{A}^d_S$ over $S$,
\item any quasi-finite morphism $g : U \to \mathbf{A}^d_S$ over $S$
defined on an open neighbourhood $U \subset X$ of $x$ is flat at $x$.
\end{enumerate}
\end{lemma}

\begin{proof}
The question is local on $X$ and $S$ hence we may assume $X$ and $S$
are affine. Say $S = \Spec(R)$ and $X = \Spec(A)$. Then $R \to A$
is a flat ring map of finite presentation. Let $\mathfrak p \subset A$
be the prime ideal corresponding to $x$. Then there exists an $f \in A$,
$f \not \in \mathfrak p$ and a quasi-finite map
$R[x_1, \ldots, x_d]  \to A_f$ by
Algebra, Lemma \ref{algebra-lemma-quasi-finite-over-polynomial-algebra}.
Thus it suffices to show: Givem $\varphi : R[x_1, \ldots, x_d] \to A$
quasi-finite at $\mathfrak p$ the corresponding morphism
$\Spec(\varphi)$ is flat at $\mathfrak p$ if and only if
$\Spec(A) \to \Spec(R)$ is Cohen-Macaulay at $\mathfrak p$.
(Here we have used openness of flatness for
$R[x_1, \ldots, x_n] \to A$ to go between (2) and (3), see
Theorem \ref{theorem-openness-flatness}.)
By Theorem \ref{theorem-criterion-flatness-fibre}
...
decompositon. Since $R^{sh}$ is a filtered colimit of \'etale
...
or an open of a quasi-projective scheme.
...
such that $Z$ maps into $\bigcup V)i$ and $W$ maps into $\bigcup U_i$.
