...
as well as $X \to Y$ is flat and of finite presentation, see
Morphisms, Proposition \ref{morphisms-proposition-generic-flatness}.
Note that every point $x$ in $X_y$ is a specialization of
a point $x' \in X_\eta$ by
Morphisms, Lemma \ref{morphisms-lemma-generalizations-lift-flat}
(here we use that $X \to Y$ is flat).
As the $X_i$ are closed in $X$ and cover the generic fibre this
implies that $X_y = \bigcup X_{i, y}$ for $y \in Y$.
