...
\noindent
In this section we make some set theoretical remarks on points and
geometric points. We suggest the reader skip this section on a
first reading.
...
closed\footnote{In the chapter on etale cohomology we used separably
closed, and here we really do mean algebraically closed.} field over $S$.
\end{definition}
...
\medskip\noindent
What is somewhat important in the definition above is that $k$ is 
a field over $S$ (as per our general conventions in
Section \ref{section-conventions}).
Another way to think about this is that we are given a geometric
point $\overline{s} : \text{Spec}(k) \to S$ and a lift of $\overline{s}$
to a morphism $\overline{x}$ into $X$. Here is a diagram
$$
\xymatrix{
...
$X(k) \to |X|$ maps into the subset $|X_s| \subset |X|$ (it is a subset
since the morphism $X_s \to X$ is a monomorphism).
...
the collection of geometric points $\overline{x}$ of $X$ cover all of $|X|$.
\item For any geometric point
$\overline{s} : \text{Spec}(k) \to S$ where
$k$ has sufficiently large cardinality the map
$$
...
is surjective.
\item Let $X \to Y$ be a morphism of algebraic spaces over $S$.
The map $X \to Y$ is surjective if and only if for all algebraically
closed fields $k$ over $S$ of sufficiently large cardinality the map
$X(k) \to Y(k)$ is surjective.
\end{enumerate}
\end{lemma}

\begin{proof}
Parts (1) and (2) are implied by (3) on taking $Y = S$ and $X \to Y = S$
the structure morphism. Part (4) follows from (3) and (1) applied to $Y$
and some set theoretical considerations. Namely, we need to show that
we may choose the algebraically closed fields $k$ in (3) and (4) so that
$\text{Spec}(k)$ is (isomorphic to) an object of $\textit{Sch}_{fppf}$
(as per our conventions, see
Section \ref{section-conventions}). Thus, apart from set theoretical
remarks it suffices to prove (3).

\medskip\noindent
After these preliminary remarks we prove (3).
Let $X \to Y$ be a morphism of algebraic spaces over $S$.
Let $k$ be an algebraically closed field over $S$ of large cardinality
(how large we will see later; it will be in terms of the schemes
$U$ and $V$ we choose below).
Let $\overline{y} \in Y(k)$ and $x \in |X|$ which map to the
same element $y$ of $|Y|$.
...
we see that if the cardinality of $k$ is larger than the cardinality
of $\kappa(u)$, then any maximal ideal
...
This concludes the proof modulo a set theoretical remark.

\medskip\noindent
The set theoretical remark is the following. It follows from the result
of the preceding paragraph that it is good enough to require that the
cardinality of $k$ is larger than the size $\lambda = \text{size}(U)$
of the scheme $U$ as defined in
Sets, Section \ref{sets-section-categories-schemes}.
Namely, the cardinality of $\kappa(u)$ is clearly bounded by
$\lambda$ for all $u \in U$. (Because if $R$ is a ring then
the cardinality of any residue field $\kappa(\mathfrak p)$ of $R$
is bounded by the cardinality of $R$.) And finally, by our construction
of the category $\textit{Sch}_{fppf}$ in
...
we see that any scheme whose size is $\leq \lambda^+$
...
Here $\lambda^+ > \lambda$ is the next bigger cardinal.
Now for any $s \in S$ we can (details omitted)
construct an algebraically closed field extension
$\kappa(s) \subset k$ whose cardinality is $\lambda^+$ provided that
$\lambda^+$ is bigger than or equal to the cardinality of $\kappa(s)$.
If not just choose $k = \overline{\kappa(s)}$.
In both cases $\text{Spec}(k) \to S$ is isomorphic to
a morphism in $\textit{Sch}_{fppf}$: in the first case by the discussion
above, and in the second case by
Sets, Lemma \ref{sets-lemma-what-is-in-it}.
Hence there are enough algebraically closed fields over $S$
of the required large cardinality.
\end{proof}
