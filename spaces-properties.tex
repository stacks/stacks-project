...
\item We say $X$ is {\it weakly locally separated}\footnote{This is probably
nonstandard notation.} if $\Delta$ is an
immersion.
\item We say $X$ is {\it locally separated} if $\Delta$ is a
...
\end{enumerate}
If $S$ is a scheme contained in $\textit{Sch}_{fppf}$, then we say
an algebraic space $X$ over $S$ is {\it separated},
{\it weakly locally separated}, {\it locally separated},
{\it quasi-separated}, or {\it Zariski locally quasi-separated}
if $X$ viewed as an algebraic space over $\text{Spec}(\mathbf{Z})$
...
\begin{lemma}
\label{lemma-points-monomorphism-general}
Let $S$ be a scheme. Let $X$ be an algebraic space over $S$.
Consider the map
$$
...
\noindent
We will see in Lemma \ref{lemma-points-monomorphism} that this map
...
and
\ref{spaces-lemma-composition-representable-transformations-property}).
Moreover, as $U_i \times_X U_i = U_i \times_{X_i} U_i$, and hence
each the projections $U_i \times_X U_i \to U_i$ are quasi-compact.

\medskip\noindent
If $X$ is quasi-compact then there exists a surjective etale morphism
...
we get factorizations $U_i \to X_i$ which are
...
also holds when $X$ is weakly locally separated (and if so maybe we should
rename ``weakly locally separated'' to ``locally separated'').
...
\begin{proof}
We have seen in Lemma \ref{lemma-points-monomorphism-general}
that the map is an injection in general.

\medskip\noindent
Assume that $X$ is reasonable. Let $\varphi_i : U_i \to X$ be as in
Definition \ref{definition-reasonable}. Note that this means
$|X| = \bigcup \varphi_i(|U_i|)$. Let $X_i \subset X$ be the open
subspace of $X$ corresponding to $\varphi_i(|U_i|)$, see
Lemma \ref{lemma-open-subspaces}. To prove the surjectivity
...
where there exists a scheme $U$ and a surjective etale morphism
$U \to X$ such that both projections $U \times_X U \to U$ are quasi-compact.

\medskip\noindent
Assume there exists a scheme $U$ and a surjective etale morphism
$U \to X$ such that both projections $s, t : R = U \times_X U \to U$
are quasi-compact. Pick $x \in |X|$. We have to show that
$x$ is in the image of the map displayed in the lemma.
We have seen that $|U| \to |X|$ is surjective,
see Lemma \ref{lemma-points-presentation}.
Let $u \in U$ be a point mapping to $x$, and write $k =\kappa(u)$
...
$$
Since $s$ and $t$ are etale and quasi-compact we see that
$R \times_{s, U} \text{Spec}(k)$ and
$\text{Spec}(k) \times_{U,t} R$ are finite disjoint unions of spectra
...
$R'$ is also a finite disjoint union of spectra of fields finite separable
over $k$ (in two ways),
...
Finally, Lemma \ref{lemma-point-like-spaces} applies
($R'$ is clearly quasi-compact), and we see that $X' = \text{Spec}(k')$.
...
\noindent
The following lemma shows that specialization of points behaves in a reasonable
manner on reasonable algebraic spaces.
...
specialization of $x'$.
Assume $X$ is reasonable. Then
\begin{enumerate}
\item there exists a scheme $U$, an etale morphism $\varphi : U \to X$
and points $u', u \in U$, $u' \leadsto u$ with $\varphi(u) = x$ and
$\varphi(u') = x'$,
\item for every surjective etale morphism $U \to X$ from a scheme to $X$
there exist points $u', u \in U$, $u' \leadsto u$ with $\varphi(u) = x$ and
$\varphi(u') = x'$,
\item for every etale morphism $\varphi : U \to X$ from a scheme $U$ and any
...
\begin{proof}
We first prove (1). Since $X$ is reasonable there exist
a set of schemes $U_i$ and etale morphisms $\varphi_i : U_i \to X$ such that
both projections $U_i \times_X U_i \to U_i$ are quasi-compact, and
the morphism $\coprod U_i \to X$ is surjective.
By Lemma \ref{lemma-characterize-surjective}
we see that $x \in \varphi(|U_i|)$ for some $i$.
Since $|U_i| \to |X|$ is open (Lemma \ref{lemma-topology-points})
we see that also $x' \in \varphi(|U_i|)$.
Thus we may replace $X$ by the
open subspace $X' \subset X$ whose underlying topological space
of points is the image of $|U_i| \to |X|$, see
Lemma \ref{lemma-open-subspaces}.
In other words, we may assume that there exists a scheme $U$ and
a surjective etale morphism $U \to X$ such that
$s, t : R = U \times_X U \to U$ are quasi-compact.
Choose a point $u' \in U$ mapping to $x'$.
Let $\{r'_1, \ldots, r_n'\} = t^{-1}(\{x'\})$
be the finite collection of points lying over
$x'$ in $R$; there are finitely many because $t$ is quasi-compact
and etale, hence quasi-finite (see
Etale Morphisms of Schemes, Section \ref{etale-section-etale-morphisms}).
Then by elementary properties of groupoids and
...
\{u'_1, \ldots, u'_m\}
$$
is the set of points of $U$ mapping to $x'$. In particular, also the
fibre of $|U| \to |X|$ over $x'$ is finite. Consider the closed set
$$
T = \overline{\{u'_1\}} \cup \ldots \cup \overline{\{u'_m\}} \subset |U|.
$$
By the first displayed formula above this is a $|R|$-invariant closed set
inside $|U|$. Hence we see that $T$ is the inverse image of a
subset $T'$ of $|X|$, see Lemma \ref{lemma-points-presentation}. Since
$|U| \to |X|$ is surjective and open we see that
$T'$ is closed and hence $T' = \overline{\{x\}}$.
Thus $T$ contains some point mapping to $x$ (as $U \to X$ is surjective).
I.e., see that for some $i$ there exists a specialization
$u'_i \leadsto u$ which maps to the given specialization $x' \leadsto x$.

\medskip\noindent
Let us prove (3). Namely, let $\varphi : U \to X$ be
an etale morphism such that $x$ is the image of the point $u \in U$.
On the other hand, let $\psi : V \to X$ be an etale
morphism from a scheme to $X$ such that there exists a specialization
$v' \leadsto v$ of points in $V$ with $\psi(v') = x'$ and $\psi(v) = x$.
Such a $V, v, v'$ exists because we (1) holds, as we proved in the first
paragraph of the proof.
Now consider the fibre product $W = U \times_X V$. Since
$|W| \to |U| \times_{|X|} |V|$ is surjective
(Lemma \ref{lemma-points-cartesian})
and since $u$ and $v$ map to $x$,
there exists a point $w \in W$ mapping to $u \in U$ and $v \in V$. Since
$W \to V$ is etale, hence flat generalizations lift along $W \to V$
(Morphisms, Lemma \ref{morphisms-lemma-generalizations-lift-flat})
and we can find $w' \leadsto w$ with $w'$ mapping to $v'$.
Set $u' \in U$ equal to the image of $w'$ under the
projection $W \to U$. This proves (2).
...
\begin{proof}
Let $u, u' \in |U|$ map to the same point $x$ of $|X|$, and $u' \leadsto u$.
Since $X$ is reasonable there exist a set of schemes $U_i$ and etale
morphisms $\varphi_i : U_i \to X$ such that both projections
$U_i \times_X U_i \to U_i$ are quasi-compact, and the morphism
$\coprod U_i \to X$ is surjective. Pick $i$ such that $x$ is in the
image of $U_i \to X$. Consider $U_i \times_X U \to U$. By
Lemma \ref{lemma-points-cartesian} and our choice of $i$ we
see that $u$ is the image of some point $w \in U_i \times_X U$.
Since $U_i \times_X U \to U$ is etale hence flat we see that
generalizations lift
(Morphisms, Lemma \ref{morphisms-lemma-generalizations-lift-flat})
and we find a $w' \in U_i \times_X U$ with $w' \leadsto w$ and
$w'$ mapping to $u'$. Denote $u_i', u_i \in U_i$ the image
of $w', w$. Since there are no specializations among points in
a fibre of an etale morphism of schemes
(because fibres of etale morphisms are etale schemes over fields
which are disjoint unions of spectra of fields, see
Etale morphism of Schemes, Section \ref{etale-section-etale-morphisms})
we see that $u_i' \not = u_i$, unless $w' = w$ and hence $u' = u$.
Thus we have reduced ourselves to proving the result
for $u_i', u_i \in U_i$. In other words, we may assume that
the two projections $s, t : R = U \times_X U \to U$ are quasi-compact.

\medskip\noindent
In this case we can finish the proof as follows.
Pick a point $r \in R$ with $t(r) = u$ and $s(r) = u'$.
This is possible by
Lemma \ref{lemma-points-presentation}.
Because generalizations lift along the etale morphism $t$
(see above) we can find a specialization $r' \leadsto r$ with
...
$$
Above we have seen that under the etale morphism $s$ there are no
specializations in fibres. Hence if $u^{(n + 1)} = u^{(n)}$ for some
$n$, then also $r^{(n)} = r^{(n - 1)}$ and hence also (by taking $t$)
$u^{(n)} = u^{(n - 1)}$. This then forces the whole tower to collapse,
in particular $u = u'$. Thus we see that if $u \not = u'$, then all
the specializations are strict and $\{u, u', u'', \ldots\}$
is an infinite set of points in $U$ which map to the same point in $|X|$.
However, as we saw in the proof of
Lemma \ref{lemma-reasonable-specialization},
the fact that the projections $U \times_X U \to U$ are quasi-compact implies
that there are at most finitely many points of $U$ mapping to the same
point of $|X|$. Namely, the set of points mapping to the same point
as $u$ is the set $t(s^{-1}(\{u\}))$ which is finite.
\end{proof}

\begin{lemma}
\label{lemma-reasonable-sober}
Let $S$ be a scheme.
Let $X$ be a reasonable algebraic space over $S$.
Then $|X|$ is a sober topological space (see
Topology, Definition \ref{topology-definition-generic-point}).
\end{lemma}

\begin{proof}
First, let us show that $|X|$ is Kolmogorov.
...
By Lemma \ref{lemma-reasonable-specialization} we can find a specialization
$u_1 \leadsto u_2$ in $U$ mapping to $x_1 \leadsto x_2$.
By Lemma \ref{lemma-reasonable-specialization} part (3) we can find
...
\medskip\noindent
To finish the proof we have to show that every irreducible closed
subset $T \subset |X|$ has a generic point. The fact that $X$ is
reasonable means that there exists an etale morphism $U \to X$
such that the two projections $s, t : R = U \times_X U \to U$ are
quasi-compact and such that the image of $|U|$ in $|X|$ meets $T$.
(Just take one of the $U_i$ from Definition \ref{definition-reasonable}.)
Let $X' \subset X$ be the open subspace of $X$ corresponding to the
open image of $U \to X$. As $T$ is irreducible we see that
$T \cap |X'|$ is irreducible and $T = \overline{T \cap |X'|}$.
Hence we may replace $X$ by $X'$, and $T$ by $T \cap |X'|$
and assume that there exists a surjective etale morphism $U \to X$
such that the two projections $s, t : R = U \times_X U \to U$ are
quasi-compact. Let $Z \subset U$ be the inverse image of $T$.
Let $Z = \bigcup_{i \in I} Z_i$ be the decomposition of $Z$ into irreducible
components. Let $\eta_i \in Z_i$ be the generic point.
Since generalizations lift along $t$ we see that
each point of the finite set $t(s^{-1}(\{\eta_i\}))$ is
a generic point of an irreducible component of $Z$.
Hence, if we define an equivalence relation on $I$ by
$i \sim j \Leftrightarrow \eta_j \in t(s^{-1}(\{\eta_i\}))$,
then we see that the equivalence classes are finite.
Suppose that there is more than one equivalence class. Then
we can decompose $I = I_1 \coprod I_2$ where both $I_1$ and $I_2$
are nonempty and unions of equivalence classes. It is easy to see
that
$$
Z = \bigcup\nolimits_{i \in I_1} Z_i \cup \bigcup\nolimits_{i \in I_2} Z_i
$$
decomposes $Z$ into closed $|R|$-stable subsets, which contradicts the
assumption that $T$ is irreducible. Hence there is only one equivalence
class (which is finite) and in this case it is easy to see that
$\eta_i$ maps to a point $\eta \in T$ which is a generic point.
\end{proof}
