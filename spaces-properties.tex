...
Before we do so let us make some preliminary remarks on etale morphisms
from schemes to algebraic spaces.
...
\noindent
This lemma implies that in the definition below any morphism between
objects of the etale site of $X$ is etale, and that any scheme
etale over an object of $X_{etale}$ is also an object of $X_{etale}$.
...
the fundamental objects of algebraic geometry. Moreover, we introduce
this site here because we want to use it in order to define the category
of quasi-coherent sheaves on $X$, and at the moment we only have results
on the category of quasi-coherent sheaves over schemes. Another possibility is
to consider all affine schemes $U$ which are etale over $X$. This we will
consider later (insert future reference here), and will be denoted
$(\textit{Aff}/X)_{etale}$.
