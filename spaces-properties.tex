...
Let $S$ be a scheme. Let $\mathcal{P}(U, u)$ be a property of
pairs $(U, u)$, where $U$ is a scheme over $S$, and $u \in U$ is a point.
Let us say that $\mathcal{P}$ is {\it etale local} if for any
etale morphism of schemes $U' \to U$ and a point $u' \in U'$ mapping to
$u \in U$ we have
$$
...
\noindent
We can use this concept to define properties of algebraic
...
Let $\mathcal{P}$ be a property of pointed schemes which is
etale local as in
...
\noindent
We can use the following lemma to define the dimension of an algebraic
...
from a scheme to $X$ and a point $u \in U$ with $a(u) = x$, see
Lemmas \ref{lemma-local-source-target-at-point} and
\ref{lemma-dimension-at-point-local}.
\end{definition}
...
Section \ref{section-stalks-structure-sheaf}.
...
Let $S$ be a scheme.
Let $X$ be an algebraic space over $S$.
Let $X_{affine, etale}$ denote the full subcategory of $X_{etale}$
whose objects are those $U/X \in \text{Ob}(X_{etale})$ with
$U$ affine. A covering of $X_{affine, etale}$ will be a standard
etale covering, see
...
$X_{affine, etale} \to X_{etale}$ is a special cocontinuous functor
as in
Sites, Definition \ref{sites-definition-special-cocontinuous-functor}.
\end{proof}
...
{\it small etale topos} of $X$ is the category $\textit{Sh}(X_{etale})$
...
we have $\textit{Sh}(X_{etale}) = \textit{Sh}(X_{spaces, etale})$,
...
we see that $\textit{Sh}(X_{etale}) = \textit{Sh}(X_{affine, etale})$.
...
\end{equation}
to indicate this. Note that the functor $j : X_{etale} \to Y_{etale}$
...
$\textit{Sh}(Y'_{etale})$\footnote{Also
...
$X_{space, etale}$ and consider all etale neighbourhoods. And there won't
be any difference because of the last assertion in the following lemma.
...
$\overline{x} \times_{\overline{u}, U, \varphi_i} U_i$ is not empty
for some $i$, see
Lemma \ref{lemma-points-cartesian}.
Then look at the cartesian diagram
$$
\xymatrix{
\overline{x} \times_{\overline{u}, U, \varphi_i} U_i
\ar[d]^{\text{pr}_1} \ar[r]_-{\text{pr}_2} & U_i
\ar[d]^{\varphi_i} \\
\text{Spec}(k) = \overline{x} \ar@/^1pc/[u]^\sigma
\ar[r]^-{\overline{u}} & U
}
$$
...
Therefore, $\overline{x} \times_{\overline{u}, U, \varphi_i} U_i$
is a disjoint union of finite separable extensions of $k$, see
Remark \ref{remark-recall}. Here $\overline{x} = \text{Spec}(k)$.
But $k$ is algebraically closed, so all these extensions are trivial,
and there exists a section $\sigma$ of $\text{pr}_1$. The composition
...
of $\overline{x}$ in $X$ with $U \in \text{Ob}(X_{etale})$.
\end{definition}
...
Lemma \ref{lemma-cofinal-etale}
says the opposite of the category of all etale neighbourhoods is filtered, and
the full subcategory of those which are in $X_{etale}$ is a cofinal
...
$X_{spaces, etale}$ corresponding to $\mathcal{F}$ on $X_{etale}$,
then
\begin{equation}
...
And, given a covering $\{U_i \to U\}_{i \in I}$ in $X_{etale}$ we see that
...
$\textit{Sh}(X_{etale}) \to \textit{Sets}$ given by the stalk functor
...
\begin{enumerate}
\item The functor
$f_{small}^{-1} : \textit{Ab}(Y_{etale}) \to \textit{Ab}(X_{etale})$ is exact.
\item The functor
$f_{small}^{-1} : \textit{Sh}(Y_{etale}) \to \textit{Sh}(X_{etale})$ is exact,
...
\medskip\noindent
To prove (4) we argue as follows. Note that by
...
& = \text{colim}_{(U, \overline{u})}\ f_{spaces, etale}^{-1}\mathcal{G}'(U) \\
...
Since the stalk of $\mathcal{G}'$ (resp.\ $f_{spaces, etale}^{-1}\mathcal{G}'$)
...
the category of sheaves on $\text{Spec}(k)_{etale}$ is
...
$$
\textit{Sh}(X_{etale}) \longrightarrow \textit{Sets},\quad
...
Let $S$ be a scheme.
Let $X$ be an algebraic space over $S$.
Let $p : \textit{Sh}(pt) \to \textit{Sh}(X_{etale})$
...
Let $U \in \text{Ob}(X_{etale})$ be an object whose structure morphism
...
we obtain a point $q : \textit{Sh}(pt) \to \textit{Sh}(U_{etale})$
...
for $\mathcal{G} \in \textit{Sh}(U_{etale})$. Set
...
empty for all $\varphi : U \to X$ in $\text{Ob}(X_{spaces, etale})$.
...
Let $X$ be an algebraic space over $S$.
Let $\mathcal{F}$ be an abelian sheaf on $X_{etale}$.
Let $U \in \text{Ob}(X_{etale})$ and $\sigma \in \mathcal{F}(U)$.
\begin{enumerate}
...
Etale Cohomology, Lemma \ref{etale-cohomology-lemma-describe-etale-local-ring}.
\end{proof}
...
If $A$ and $B$ are Noetherian we have seen this in
Etale Morphisms of Schemes, Lemma \ref{etale-lemma-etale-dimension}.
But it also holds in general.
...
Lemmas \ref{lemma-local-source-target-at-point} and
\ref{lemma-dimension-local-ring-local}.
...
$\dim(R) = \dim(R^{sh})$. To see this note that $R \to R^{sh}$
is a flat local ring homomorphism (see
Algebra, Lemma \ref{algebra-section-henselian})
and hence $\dim(R^{sh}) \geq \dim(R)$ by going down, see
Algebra, Section \ref{algebra-section-going-up}.
For the converse, we write $R^{sh} = \text{colim} R_i$ as
a directed colimit of local rings $R_i$ each of which is a
localization of an etale $R$-algebra. We can do this by
the construction of the strict henselization in
Algebra, Section \ref{algebra-section-henselian}.
Now if
$\mathfrak q_0 \subset \mathfrak q_1 \subset \ldots \subset \mathfrak q_n$
...
(\textit{Sh}(U_{spaces, etale}), \mathcal{O}_U)
\longrightarrow
(\textit{Sh}(V_{spaces, etale}), \mathcal{O}_V)
$$
...
\begin{proof}
The morphism $(f_{spaces, etale, c}, f_c^\sharp)$ is defined as a composition
$f' \circ j$
...
$$
(g_{small}, g^\sharp) :
(\textit{Sh}(U_{etale}), \mathcal{O}_U)
\longrightarrow
(\textit{Sh}(V_{etale}), \mathcal{O}_V)
$$
...
$$
of algebraic spaces over $Y$. In terms of sheaves on $Y_{etale}$ the morphism
$i$ induces an injection
...
between sheaves on $U_{spaces, etale}$ and sheaves on $U_{etale}$
and similarly for $V$. The second and fourth $2$-isomorphisms
are those of
Lemma \ref{lemma-relocalize-morphism}
...
as arrows $(\mathcal{H} \to h_V)$ in $\textit{Sh}(Y_{spaces, etale})$
...
$(g, g^\#)$ is isomorphic to $(f_{small}, f^\sharp)$.
...
\medskip\noindent
Let $U \in \text{Ob}(X_{etale})$, let $V \in \text{Ob}(Y_{etale})$
...
$$
\xymatrix{
(\textit{Sh}(X_{etale}/U), \mathcal{O}_U)
\ar[rr]_-{(j, j^\sharp)} \ar[d]_{(g_s, g_s^\sharp)} & &
(\textit{Sh}(X_{etale}), \mathcal{O}_X) \ar[d]^{(g, g^\sharp)} \\
(\textit{Sh}(V_{etale}), \mathcal{O}_V) \ar[rr] & &
(\textit{Sh}(Y_{etale}), \mathcal{O}_Y).
}
$$
By
...
the following functoriality property: Suppose given
morphisms $a : U' \to U$ in $X_{etale}$ and $b : V' \to V$ in $Y_{etale}$
