...
$\textit{Sch}_{fppf} = (\textit{Sch}/\Spec(\mathbf{Z}))_{fppf}$.
...
\textit{Sh}(X_{\acute{e}tale})
\longrightarrow
\textit{Sh}(\Spec(k)_{\acute{e}tale}) = \textit{Sets},
\quad
...
Let $S$ be a scheme. Let $X$ be an algebraic space over $S$.
The category $\text{QCoh}(\mathcal{O}_X)$ of quasi-coherent sheaves on $X$ has
...
$\text{QCoh}(\mathcal{O}_X)$ if and only if its restriction to $U$ is in
$\text{QCoh}(\mathcal{O}_U)$. Let $\mathcal{F}_i$ be a family of
