...
immediately from the assumption in (2).
...
The finiteness of the fibre of $|R| \to |X|$ is also clear since it
...
\label{section-three-conditions}
...
\begin{enumerate}
\item For every $x \in |X|$, the equivalent conditions of
...
|X|
$$
is bijective, i.e., the equivalent conditions of
...
\item The space $X$ is quasi-separated.
\item The space $X$ is representable, i.e., $X$ is a scheme.
\item The space $X$ is a quasi-separated scheme.
\end{enumerate}
We have
$$
\xymatrix{
...
\begin{proof}
The implications (3) $\Rightarrow$ (2) and (3) $\Rightarrow$ (1) are immediate.
The implications in the diamond on the left we have seen in
Section \ref{section-reasonable}.

\medskip\noindent
Assume (4). Let $U$ be an affine scheme, and let $U \to X$ be an etale
...
hold. This proves that (4) $\Rightarrow$ (3).

\medskip\noindent
Let us show that (5) implies (4).
Assume (5). By Lemma \ref{lemma-characterize-reasonable} there exists
...
Let $S$ be a scheme and let $X$ be an algebraic space over $S$.
Condition (4) of Lemma \ref{lemma-bounded-fibres} says that for any
...
(this has to do with descent of the property; insert future reference here).
...
\begin{remark}
\label{remark-properties-almost-reasonable}
This is a continuation of Remark \ref{remark-almost-reasonable}.
Here are some observations:
\begin{enumerate}
\item Any reasonable space is almost reasonable, see
Lemma \ref{lemma-bounded-fibres}. There exists an almost reasonable
space which is not reasonable (details omitted).
\item If $X$ is an algebraic space, and $X = \bigcup X_i$ is a
Zariski open covering such that each $X_i$ is almost reasonable,
then $X$ is almost reasonable.
\item If $X \to Y$ is a representable morphism of algebraic spaces, and
$Y$ is almost reasonable, then so is $X$. This follows readily from
Lemmas \ref{lemma-base-change-universally-bounded} and
\ref{lemma-U-universally-bounded}.
\end{enumerate}
If you would like to see the details please email
\href{mailto:stacks.project@gmail.com}{stacks.project@gmail.com}.
\end{remark}

\begin{remark}
\label{remark-fun-property-almost-reasonable}
This is a continuation of Remark \ref{remark-properties-almost-reasonable}.
...
same point in $|X|$. As we chose $U$ affine this contradicts
...
specialization of $x'$.
Assume the pair $(X, x')$ satsifies the equivalent conditions
...
Hence we may and do assume that $U$ is affine. As $x$ is in the
image of the open map $|U| \to |X|$, so is $x'$. Thus we may
replace $X$ by the Zariski open subspace corresponding to
the image of $|U| \to |X|$, in other words we may assume that
...
$s(r) = u_1$ and $t(r) = u_1$ (using Lemma \ref{lemma-points-cartesian}).
