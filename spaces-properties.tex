...
finite and etale in this case. Claim: there exists a dense set
of points which are contained in an $R$-invariant affine open of $U$.
...
projections $U \times_X U \to U$ are quasi-compact; for example if
$\varphi$ is quasi-compact. Then the fibres of $|U| \to |X|$ are finite.
\end{lemma}

\begin{proof}
Denote $R = U \times_X U$, and $s, t : R \to U$ the projections.
Let $u \in U$ be a point. The fibre of $|U| \to |X|$ which contains
$u$ is equal to $s(t^{-1}(\{u\}))$ by Lemma \ref{lemma-points-cartesian}.
Since $t : R \to U$ is etale and quasi-compact, it has finite fibres, see
Remark \ref{remark-recall}. Hence we win.
\end{proof}
...
\begin{enumerate}
\item For every $x \in |X|$, there exists a scheme $U$ and an
etale morphism $\varphi : U \to X$ such that $\varphi^{-1}(x)$
is finite and nonempty.
\item For every $x \in |X|$, there exists a scheme $U$ and an
etale morphism $\varphi : U \to X$ such that
\begin{enumerate}
\item $x$ is in the image of $|U| \to |X|$, and
\item there exists an integer $n$ such that for every morphism
$\text{Spec}(k) \to X$ the fibre product $\text{Spec}(k) \times_X U$
has at most $n$ points.
\end{enumerate}
\item For every affine scheme $U$ and etale morphism $\varphi : U \to X$ 
there exists an integer $n$ such that for every morphism
$\text{Spec}(k) \to X$ the fibre product $\text{Spec}(k) \times_X U$
has at most $n$ points.
\item The space $X$ is reasonable.
\end{enumerate}
We have (4) $\Rightarrow$ (3) $\Leftrightarrow$ (2) $\Rightarrow$ (1).
\end{lemma}
...
also holds when $X$ is weakly locally separated. To prove this one would
...
\medskip\noindent
Assume that $X$ is reasonable. Let $X = \bigcup X_i$ be a Zariski
open covering, and $\varphi_i : U_i \to X_i$ be as in
Lemma \ref{lemma-characterize-reasonable} (2). To prove the surjectivity
...
where there exists a scheme $U$ and a quasi-compact surjective etale morphism
$U \to X$.

\medskip\noindent
Assume there exists a scheme $U$ and a quasi-compact surjective etale morphism
$U \to X$. This means that both projections $s, t : R = U \times_X U \to U$
...
of finite separable extensions of $k$, see Remark \ref{remark-recall}.
...
\begin{lemma}
\label{lemma-point-like-spaces-reasonable}
Let $S$ be a scheme. Let $k$ be a field.
Let $X$ be an algebraic space over $S$ and assume that there exists
a surjective etale morphism $\text{Spec}(k) \to X$.
...
By Lemma \ref{lemma-points-monomorphism} we see that $\text{Spec}(k) \to X$
factors as $\text{Spec}(k) \to \text{Spec}(k') \to X$ where
...
\begin{proof}
We first prove (1). Since $X$ is reasonable there exists a
Zariski open covering $X = \bigcup X_i$ and for each $i$ a
scheme $U_i$ and a surjective quasi-compact etale morphism
$\varphi_i : U_i \to X_i$, see
Lemma \ref{lemma-characterize-reasonable}.
Pick $i$ such that $x \in |X_i|$. We will show that (1) holds
with $U = U_i$. As $x' \leadsto x$, we see
that also $x' \in |X_i|$. Thus we may replace $X$ by the open subspace $X_i$.

\medskip\noindent
In other words, we may assume that there exists a scheme $U$ and
a quasi-compact surjective etale morphism $U \to X$, and we will
prove (1) holds for this $U$. By
Lemma \ref{lemma-finite-fibres-presentation} the fibre of $|U| \to |X|$
over $x'$ is a finite collection of points $u'_1, \ldots, u'_n \in U$.
By the same lemma there are finitely many points
$r'_1, \ldots, r'_m \in R$ which form the complete inverse image
of the set $\{u'_i\}$ via either $s$ or $t$ (it is the same thing).
...
$$
Trivially we have $s(T') \subset T$, and because $R$ is an equivalence
relation we also have $t(T') = s(T')$ (since the set $\{r_j'\}$
is invariant under the inverse of $R$ by construction). Let $u \in T$
be any point. Then $u'_i \leadsto u$ for some $i$. Choose $r \in R$
with $s(r) = u$. Since generalizations lift along $s : R \to U$, see
Remark \ref{remark-recall}, we can find $r' \leadsto r$ with
$s(r') = u_i'$. Then $r' = r'_j$ for some $j$ and we conclude that
$u \in s(T')$. Hence $T = s(T') = t(T')$ is an $|R|$-invariant closed
set in $|U|$. This means $T$ is the inverse image of a
...
Such a $V, v, v'$ exists because (1) holds, as we proved in the first
paragraph of the proof. Now consider the fibre product $W = U \times_X V$.
Since $|W| \to |U| \times_{|X|} |V|$ is surjective
(Lemma \ref{lemma-points-cartesian})
and since $u$ and $v$ map to $x$,
there exists a point $w \in W$ mapping to $u \in U$ and $v \in V$. Since
$W \to V$ is etale, generalizations lift along $W \to V$,
see Remark \ref{remark-recall}. This means we can find
$w' \leadsto w$ with $w'$ mapping to $v'$. Set $u' \in U$ equal to the
image of $w'$ under the projection $W \to U$. This proves (2).
...
Since $U_i \times_X U \to U$ is etale we see that
generalizations lift (Remark \ref{remark-recall})
...
a fibre of an etale morphism of schemes (see Remark \ref{remark-recall})
...
same point in $|X|$. This contradicts
Lemma \ref{lemma-finite-fibres-presentation}, as desired.
\end{proof}
