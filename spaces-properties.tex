...
sense, see (insert future reference here).
...
\begin{proof}
Let $U \in \text{Ob}(X_{etale})$ and let
$V \in \text{Ob}(Y_{etale})$.
Let $s \in g^{-1}h_V(U)$ be a section. We may think of
...
$$
\xymatrix{
(\textit{Sh}(X_{etale})/g^{-1}h_V, \mathcal{O}_{g^{-1}h_V})
\ar[rr]_-{(j, j^\sharp)} \ar[d]_{(g', (g')^\sharp)} & &
(\textit{Sh}(X_{etale}), \mathcal{O}_X) \ar[d]^{(g, g^\sharp)} \\
(\textit{Sh}(V_{etale}), \mathcal{O}_V) \ar[rr] & &
(\textit{Sh}(Y_{etale}), \mathcal{O}_Y)
}
$$
Using the map $h_U \to g^{-1}h_V$ we furthermore obtain a morphism
of locally ringed topoi
$$
(\textit{Sh}(U_{etale}), \mathcal{O}_U)
=
(\textit{Sh}(X_{etale})/h_U, \mathcal{O}_{h_U})
\longrightarrow
(\textit{Sh}(X_{etale})/g^{-1}h_V, \mathcal{O}_{g^{-1}h_V})
$$
whose composition with $(j, j^\sharp)$ is the morphism
$(\textit{Sh}(U_{etale}), \mathcal{O}_U) \to
(\textit{Sh}(X_{etale}), \mathcal{O}_X)$
coming from the morphism of algebraic spaces $U \to X$.
See
Modules on Sites,
...
for the morphism and the fact that the composition works out.
Finally, by
Modules on Sites,
Lemma \ref{sites-modules-lemma-localize-morphism-locally-ringed-topoi}
all of the ringed topoi are locally ringed, and all of the
morphisms above are morphisms of locally ringed topoi.
The upshot of this is that we obtain a
commutative diagram
$$
\xymatrix{
(\textit{Sh}(U_{etale}), \mathcal{O}_U)
\ar[r] \ar[d]_{(h(s), h(s)^\sharp)} &
(\textit{Sh}(X_{etale}), \mathcal{O}_X) \ar[d]^{(g, g^\sharp)} \\
(\textit{Sh}(V_{etale}), \mathcal{O}_V) \ar[r] &
(\textit{Sh}(Y_{etale}), \mathcal{O}_Y)
}
$$
of morphisms of locally ringed topoi where the horizontal arrows come
from the given morphisms $U \to X$ and $V \to Y$.
