...
\begin{enumerate}
\item there exists a scheme $U$ and an etale morphism $\varphi : U \to X$
such that $\varphi^{-1}(x)$ is finite and nonempty, and
...
towards $X$ such that $\varphi^{-1}(\{x\}) \subset U$ is a finite nonempty set.
Let $\psi : W \to X$ be an etale morphism from an affine scheme towards $X$.
...
$$
Since $q$ is etale it is open (see Remark \ref{remark-recall}).
Hence there exists a quasi-compact open $W' \subset W \times_X U$
which surjects onto $W$.
The morphism $p$ is etale, hence locally quasi-finite (see remark on
etale morphisms above). Thus we may apply
Morphisms, Lemma
\ref{morphisms-lemma-locally-quasi-finite-qc-source-universally-bounded}
to see the fibres of $p|_{W'} : W' \to U$ are finite.
Hence by
Lemma \ref{lemma-points-cartesian}
and the assumption on $\varphi$ we conclude that the fibre 
of $\psi$ over $x$ is finite. In other words (2) holds.
\end{proof}
...
such that $x$ is in the image of $|U| \to |X|$, and
...
over $x$ are finite,
\item there exists a monomorphism $\text{Spec}(k) \to X$ with $k$ a field
in the equivalence class of $x$, and there exists a scheme $U$ and etale
morphism $U \to X$ such that the fibre product $\text{Spec}(k) \times_X U$ is
a finite nonempty scheme over $k$, and
\item there exists a monomorphism $\text{Spec}(k) \to X$ with $k$ a field
...
\begin{proof}
The equivalence of (1) and (2) follows on combining
Lemmas \ref{lemma-U-finite-above-x} and \ref{lemma-R-finite-above-x}.

\medskip\noindent
Assume (2). By Lemma \ref{lemma-R-finite-above-x} we obtain a monomorphism
$\text{Spec}(k) \to X$ in the equivalence class of $x$.
The finiteness of the fibre products $\text{Spec}(k) \times_X U$
for affine schemes $U$ and etale morphisms $U \to X$ follows
immediately from the assumption in (2). In other words, (2) implies (4).

\medskip\noindent
It is clear that (4) implies (3).

\medskip\noindent
Assume (3), and let us prove (2). The finiteness of the fibre of
$|U| \to |X|$ over $x$ is clear since this fibre is equal to
$|\text{Spec}(k) \times_X U|$ by Lemma \ref{lemma-points-cartesian}.
The finiteness of the fibre of $|R| \to |X|$ above $x$ is also clear since it
is equal to the union of the fibres of $|R| \to |U| \times_{|X|} |U|$
over the finite collection of points $(u, u')$ (with $u, u' \in U$
mapping to $x$) and we may apply Lemma \ref{lemma-R-finite-above-x} part (2).
\end{proof}
...
\begin{proof}
The implications $(\gamma) \Rightarrow (\beta)$ and
$(\gamma) \Rightarrow (\alpha)$ are immediate.
...
Zariski open covering such that each $X_i$ has $\mathcal{P}$.
The condition $(\alpha)$, resp.\ $(\gamma)$ for $X_i$
can be formulated as the condition that for every $x \in |X_i|$
there exists a scheme $U$, and etale morphisms
$\varphi : U \to X_i$ such that $\varphi : |U| \to |X_i|$
has finite fibre over $x$, resp.\ both 
$\varphi : |U| \to |X_i|$ and $|U \times_{X_i} U| \to |X_i|$
have finite fibres over $x$. The composition $U \to X_i \to X$ is
a morphism which has the same property for $x$ thought of as a point
of $X$. This proves the result for $(\alpha)$ and $(\gamma)$.
...
indeed the case that the rule given defines a functor $u$ as indicated.
It is clear that $u$ is cocontinuous and continuous, simply because an
covering $\{U_i \to U\}$ of $u(\varphi : U \to X)$ in $Y_{etale}$ is the
same thing as a covering of $(\varphi : U \to X)$ in $X_{etale}$. It
remains to show that $u$ induces the same morphism of topoi as $f_{small}$.
To see this we consider the diagram
$$
\xymatrix{
X_{etale} \ar[r] \ar[d]^u &
X_{spaces, etale} \ar@/_/[d]_{u_{spaces}} \\
Y_{etale} \ar[r] &
Y_{spaces, etale} \ar@/_/[u]_{v : V \mapsto X \times_Y V}
}
$$
of categories. Here the functor $u_{spaces}$ is the obvious extension of $u$
to the category $X_{spaces, etale}$. Thus the inner square is commutative.
In fact $u_{spaces}$ can be identified with the
localization functor $j : Y_{spaces, etale}/X \to Y_{spaces, etale}$.
Hence, by
Sites, Lemma \ref{sites-lemma-localization-given-products}
the cocontinuous functor $u_{spaces}$ and the functor $v$ of the diagram
...
$X \to Y$ of algebraic spaces is simply the restriction of $\mathcal{G}$
to the category $X_{etale}$. Note that the functor $X_{etale} \to Y_{etale}$
...
$f^*\mathcal{F} = f_{small}^{-1}\mathcal{F}$ for any sheaf of
$\mathcal{O}_Y$-modules $\mathcal{F}$. In particular,
...
$$
f^*\mathcal{F} =
f_{small}^{-1}\mathcal{F} \otimes_{f_{small}^{-1}\mathcal{O}_Y}
\mathcal{O}_X
$$
by definition we conclude that $f^*\mathcal{F} = f_{small}^{-1}\mathcal{F}$.
