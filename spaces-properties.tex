...
bijection between open subspaces (see
...
Hence $X' = W'/R'$ is an algebraic space
(see Spaces, Theorem \ref{spaces-theorem-presentation}) and
$X' \to X$ is a subspace by Spaces, Lemma \ref{spaces-lemma-finding-opens}.
By construction $|X'| = W$. Thus (1) is proved.

\medskip\noindent
To prove (2), note that if $\{X_i \subset X\}_{i \in I}$
is a Zariski covering, then $\{X_i \times_X U \to U\}_{i \in I}$ is
a Zariski open covering, and hence $\bigcup |X_i| = |X|$.
Conversely, if $\bigcup |X_i| = |X|$, then it follows from the construction
that $\bicup |X_i \times_X U| = |U|$ and hence $\{X_i \subset X\}_{i \in I}$
