...
$$
f^{-1}(U) \longrightarrow \text{Spec}_S(\Gamma(f^{-1}(U), \mathcal{O}_X))
$$
...
$$
with the following property: For every affine open $U \subset S$
...
\begin{proof}
We sketch two possible arguments. Let $X \to S$ be the scheme
representing the functor $F$. It is identified
...
\medskip\noindent
Another argument is to use Lemma \ref{lemma-glue-relative-proj-twists}.
Write $Y = \underline{\text{Proj}}_S(\mathcal{A})$.
This lemma shows that there exists a morphism of graded algebras
$$
\psi : \pi^*\mathcal{A}
\longrightarrow
\bigoplus_{n \geq 0} \mathcal{O}_Y(n)
$$
over $Y$. Also, it is clear that for every $y \in Y$
...
\to \mathcal{O}_Y(n + m)|_V$ are isomorphisms (just because this
holds on the Proj of a graded ring). Hence we see that
...
\right)_0
\end{equation}
In words: $\widetilde{\mathcal{M}}_T$ is the degree $0$ part
...
to the graded $B$-algebra $A$. Then $(\widetilde{\mathcal{M}}_T)|_V$
...
as above, the pullback of $\widetilde{\mathcal{M}}$ to
$T$ via a morphism $T \to \underline{\text{Proj}}_S(\mathcal{A})$
