...
\ref{lemma-relative-glueing},
\ref{lemma-spec-inclusion}
and
...
\noindent
As an application we define affine $n$-space over a base scheme
...
Let $S$ be a scheme and $n \geq 0$. {\it Affine $n$-space} over $S$ is the
scheme
$\mathbf{A}^n_S =
\underline{\text{Spec}}_S(\mathcal{O}_S[T_1, \ldots, T_n])$ over $S$.
...
general. On global sections we get a map of graded rings
\begin{equation}
...
\item the map $S_{nd} \to \Gamma(X, \mathcal{O}_X(nd))$
from (\ref{equation-global-sections})
...
corresponds to the element $f \cdot f_i^{nd/d_i}$. Thus it is clear that
...
Let $S$ be a graded ring. Set $X = \text{Proj}(S)$. Let $d \geq 1$ be an
integer. The following open subsets of $X$ are equal:
\begin{enumerate}
\item The largest open subset $W \subset X$ such that
each $\mathcal{O}_X(dn)|_W$ is invertible and all the multiplication
maps 
...
$f, g \in S_{+}$ homogeneous and $\deg(f) = \deg(g) + d$.
\end{enumerate}
Moreover, all the maps
$\widetilde M(nd)|_W = \widetilde M|_W \otimes_{\mathcal{O}_W}
\mathcal{O}_X(nd)|_W \to \widetilde{M(n)}|_W$
(see \ref{equation-multiply-more-generally}) are isomorphisms.
\end{lemma}

\begin{proof}
If $x \in D(fg)$ with $\deg(f) = \deg(g) + d$ then
on $D_{+}(fg)$ the sheaves $\mathcal{O}_X(dn)$
are generated by the element $(f/g)^n$. This easily implies (1).
...
By the definition of the twists of the structure sheaf we conclude
there exists an element $s$ of $(S_h)_d$ such that $s^n$
is a basis of $(S_h)_n$ as a module over $S_{(h)}$. We may write
...
\medskip\noindent
The existence of the generating section $s = f/g$ over $D_{+}(fg)$ whose
...
Let $\varphi : A \to B$ be a graded ring map.
Set
$$
U(\varphi)
=
\bigcup\nolimits_{f \in A_{+}\ \text{homogeneous}} D_{+}(\varphi(f))
\subset Y.
$$
...
X
$$
with the property that for every $d \geq 0$ the diagram
$$
\xymatrix{
A_d \ar[d] \ar[rr]_{\varphi} & &
B_d \ar[d] \\
\Gamma(X, \mathcal{O}_X(d)) \ar[r] &
\Gamma(U(\varphi), \mathcal{O}_Y(d)) &
\Gamma(Y, \mathcal{O}_Y(d)) \ar[l]
}
$$
is commutative. Moreover, for any $f \in A_{+}$ homogeneous the
restriction of $\text{Proj}(\varphi)$ to $D_{+}(\varphi(f))$ 
maps into $D_{+}(f)$ and corresponds to the ring map
$A_{(f)} \to B_{(\varphi(f))}$ induced by $\varphi$.
\end{lemma}
...
Then we have
$$
U(\psi \circ \varphi) = \text{Proj}(\varphi)^{-1}(U(\psi))
$$
and
$$
\text{Proj}(\psi \circ \varphi)
=
\text{Proj}(\varphi) \circ \text{Proj}(\psi)|_{U(\psi \circ \varphi)}
$$
\end{lemma}
...
With hypotheses as in Lemma \ref{lemma-morphism-proj} above.
If $A \to B$ is surjective then $U(\varphi) = Y$ and
$\text{Proj}(\varphi) : Y \to X$ is a closed immersion.
\end{lemma}
...
With hypotheses as in Lemma \ref{lemma-morphism-proj} above.
If $B \cong A_g$ as $A$-algebras for some $g \in A_0$
then $U(\varphi) = Y$ and $\text{Proj}(\varphi) : Y \to X$ is an open
immersion. Moreover the diagram
$$
\xymatrix{
Y = \text{Proj}(B) \ar[r] \ar[d] & 
X = \text{Proj}(A) \ar[d] \\
\text{Spec}(B_0) \ar[r] &
\text{Spec}(A_0)
}
...
Let $S$ be a graded ring.
Let $X = \text{Proj}(S)$.
The open subscheme $U_d \subset X$ represents the
...
\bigcup\nolimits_{i = 0, \ldots, n} D_{+}(X_i)
$$
where each $D_{+}(X_i)$ is isomorphic to $\mathbf{A}^n_{\mathbf{Z}}$
affine $n$-space over $\mathbf{Z}$. By base change we similarly have
that projective $n$-space over any base $S$ is covered by $n + 1$
opens isomorphic to affine $n$-space over $S$.
\end{lemma}

\begin{proof}
This is true because
$\mathbf{Z}[X_0, \ldots, X_n]_{+} = (X_0, \ldots, X_n)$ and
since
$$
\text{Spec}
\left(
\mathbf{Z}
\left[\frac{X_0}{X_i}, \ldots, \frac{X_n}{X_i}
\right]
\right)
\cong
...
\noindent
Over the scheme $U_d \subset X = \text{Proj}(S)$ (see above) we have the triple
...
F(T)
=
\text{colim}_{V \subset T\text{ quasi-compact open}}\ F(V).
$$
...
\text{Mor}(T, X)
=
\text{colim}_{V \subset T\text{ quasi-compact open}}\ \text{Mor}(V, X).
$$
...
Let $\varphi : S \to \Gamma_*(T,\mathcal{L})$ be a homomorphism
of graded rings. Set
$$
U(\varphi) = \bigcup\nolimits_{f \in S_{+}\text{ homogeneous}} T_{\varphi(f)}
$$
The morphism $\varphi$ induces a canonical morphism of schemes over $S$
$$
r_{\mathcal{L}, \varphi} :
U(\varphi) \longrightarrow X
$$
together with a map of $\mathbf{Z}$-graded $\mathcal{O}_T$-algebras
$$
\theta :
r_{\mathcal{L}, \varphi}^*\left(
\bigoplus\nolimits_{d \in \mathbf{Z}} \mathcal{O}_X(d)
\right)
\longrightarrow
\bigoplus\nolimits_{d \in \mathbf{Z}} \mathcal{L}^{\otimes d}
$$
characterized by the following properties:
\begin{enumerate}
\item For $f \in S_{+}$ homogeneous we have
$r_{\mathcal{L}, \varphi}^{-1}(D_{+}(f)) = T_{\varphi(f)}$.
\item For every $d \geq 0$ the diagram
$$
\xymatrix{
S_d \ar[d]_{(\ref{equation-global-sections})} \ar[r]_{\varphi} &
\Gamma(T, \mathcal{L}^{\otimes d}) \ar[d]^{restrict} \\
\Gamma(X, \mathcal{O}_X(d)) \ar[r]^{\theta} &
\Gamma(U(\varphi), \mathcal{L}^{\otimes d})
}
$$
is commutative.
\end{enumerate}
Moreover, for any $d \geq 1$ and any open subscheme $W \subset T$
such that the sections in $\varphi(S_d)$ generate $\mathcal{L}^{\otimes d}|_W$
the morphism $r_{\mathcal{L}, \varphi}|_W$ agrees with the morphism
$W \to \text{Proj}(S)$ of Lemma \ref{lemma-converse-construction}.
associated to
$\varphi|_{S^{(d)}} : S^{(d)} \to \Gamma_*(W, \mathcal{L}^{\otimes d})$.
\end{lemma}

\begin{proof}
Suppose that we have two pairs $(r : U(\varphi) \to X, \theta)$
and $(r' : U(\varphi) \to X, \theta')$ satisfying (1) and (2).
Property (1) implies that $r = r'$ as maps of underlying topological
spaces, since the opens $D_{+}(f)$ form a basis for the topology
on $X$, and since $X$ is a sober topological space (see
Algebra Section \ref{algebra-section-proj}
and Schemes, Lemma \ref{schemes-lemma-scheme-sober}).
Let $f \in S_{+}$ be homogeneous. Note that
$\Gamma(D_{+}(f), \bigoplus_{n \in \mathbf{Z}} \mathcal{O}_X(n)) = S_f$
...
S_f
\longrightarrow
\Gamma(T_{\varphi(f)}, \bigoplus \mathcal{L}^{\otimes n}).
$$
We know that multiplication by $f$ (resp.\ $\varphi(f)$)
is an isomorphism on the left (resp.\ right) hand side.
We also know that $\theta(x/1) = \theta'(x/1) = \varphi(x)|_{T_{\varphi(f)}}$
by (2) for all $x \in S$. Hence we deduce easily that $\theta = \theta'$
...
construct the pari $(r, \theta)$ locally on $T$. Hence we may assume
that $T = \text{Spec}(R)$ is affine, that $\mathcal{L} = \mathcal{O}_T$
and that for some $f \in S_{+}$ homogeneous we have
$\varphi(f)$ generates $\mathcal{O}_T = \mathcal{O}_T^{\otimes \deg(f)}$.
In other words, $\varphi(f) = u \in R^*$ is a unit. In this case the map
$\varphi$ is a graded ring map
$$
S \longrightarrow R[x] = \Gamma_*(T, \mathcal{O}_T)
$$
which maps $f$ to $ux^{\deg(f)}$. Clearly this extends (uniquely) to
a $\mathbf{Z}$-graded ring map $\theta : S_f \to R[x, x^{-1}]$ by
mapping $1/f$ to $u^{-1}x^{-d}$. This map in degree zero gives
the ring map $S_{(f)} \to R$ which gives the morphism
$r : T = \text{Spec}(R) \to \text{Spec}(S_{(f)}) = D_{+}(f) \subset X$.
...
commutativity when restricted to $W$ and $S^{(d)}$.
Whence the result.
\end{proof}
...
The image of the morphism $r_{\mathcal{L}, \varphi}$ need not be
...
Consider the composition $\varphi$ of the maps
$$
...
$r_{\mathcal{L}, \varphi} : T \to X = \text{Proj}(S)$
...
$\text{Proj}(A) \to \text{Proj}(A')$, and the diagram
$$
\xymatrix{
...
\begin{proof}
Let $R = \mathcal{O}_S(U)$ and $R' = \mathcal{O}_S(U')$.
If $R = R'_g$ for some $g \in R'$, then this is
Lemma \ref{lemma-localization-map-proj}. In the general case,
note that the map
$R \otimes_{R'} A' \to A$ is an isomorphism as
$\mathcal{A}$ is quasi-coherent
(see Schemes, Lemma \ref{schemes-lemma-widetilde-pullback} for example).
It follows that $A'_{+}$ generates the ideal $A_{+}$.
Hence the open subset $U(\varphi)$ of Lemma \ref{lemma-morphism-proj}
is all of $\text{Proj}(A')$. Since the bottom arrow
is an open immersion, to show that the diagram is
cartesian it suffices to show that the top arrow
is an isomorphism over each standard open $D(g)$ of $U'$
contained in $U$. This follows as $A'_g = A_g$ in this
case, using Lemma \ref{lemma-localization-map-proj} twice.
\end{proof}
...
Let $A = \mathcal{A}(U)$, $A' = \mathcal{A}(U')$ and $A'' = \mathcal{A}(U'')$.
The composition of the morphisms
$\text{Proj}(A) \to \text{Proj}(A')$, and
$\text{Proj}(A') \to \text{Proj}(A'')$ of
Lemma \ref{lemma-proj-inclusion} gives the
morphism $\text{Proj}(A) \to \text{Proj}(A'')$
of Lemma \ref{lemma-proj-inclusion}.
\end{lemma}
...
\begin{enumerate}
\item for every affine open $U \subset S$ there exists an isomorphism
$i_U : \pi^{-1}(U) \to \text{Proj}(\mathcal{A}(U))$, and
\item for $U \subset U' \subset S$ affine open the composition
$$
\xymatrix{
\text{Proj}(\mathcal{A}(U)) \ar[r]^{i_U^{-1}} &
\pi^{-1}(U) \ar[rr]^{inclusion} & &
\pi^{-1}(U') \ar[r]^{i_{U'}} &
\text{Proj}(\mathcal{A}(U'))
}
$$
...
In Situation \ref{situation-relative-proj}.
Let $\pi : \underline{\text{Proj}}_S(\mathcal{A}) \to S$
be the relative Proj of $\mathcal{A}$ over $S$, as in
Lemma \ref{lemma-glue-relative-proj}.
...
\mathcal{O}_{\underline{\text{Proj}}_S(\mathcal{A})}(n)$, and
a morphism of graded
$\mathcal{O}_{\underline{\text{Proj}}_S(\mathcal{A})}$-algebras
$$
\psi :
\pi^*\mathcal{A} \longrightarrow 
\bigoplus\nolimits_{n \geq 0}
\mathcal{O}_{\underline{\text{Proj}}_S(\mathcal{A})}(n)
$$
uniquely determined by the following property:
For every affine open $U \subset S$ and every $n \in \mathbf{Z}$
there is an isomorphism
$$
i_U^\sharp(n) :
i_U^*\mathcal{O}_{\text{Proj}(\mathcal{A}(U))}(n)
\longrightarrow
\mathcal{O}_{\underline{\text{Proj}}_S(\mathcal{A})}(n)|_{\pi^{-1}(U)}
$$
compatible with the multiplication maps (\ref{equation-multiply})
and such that
$$
\xymatrix{
\mathcal{A}_n(U)
...
\mathcal{O}_{\underline{\text{Proj}}_S(\mathcal{A})}(n)) \\
&
\Gamma(\text{Proj}(\mathcal{A}(U)),
\mathcal{O}_{\text{Proj}(\mathcal{A}(U))}(n))
\ar[ru]_-{i_U^\sharp(n)}
&
}
$$
...
In Situation \ref{situation-relative-proj}.
Let $T$ be a scheme.
Let $(d, f, \mathcal{L}, \psi)$ and $(d', f', \mathcal{L}', \psi')$
...
We sketch two possible arguments.
...
\medskip\noindent
Since all sheaves are quasi-coherent we can spell out the construction
...
is functorial, exact, etc.
\item Compatibility with tensor products as in
...
\item The formation of $\widetilde{M}$ commutes with base change.
\end{enumerate}
\end{lemma}
...
Denote $\pi : \underline{\text{Proj}}_S(\mathcal{A}) \to S$ the relative
Proj.
...
Set
$$
U(\varphi) = \bigcup\nolimits_{(U, V, a)} U_{\varphi(a)}
$$
where $(U, V, a)$ satisfies: $V \subset S$ affine open,
$U = f^{-1}(V)$, $a \in \mathcal{A}(V)_{+}$ is homogeneous.
Then $\varphi(a) \in \Gamma(U, \mathcal{L}^{\otimes \deg(a)})$
and $U_{\varphi(a)}$ is the corresponding open. The morphism
$\varphi$ induces a canonical morphism of schemes over $S$
$$
r_{\mathcal{L}, \varphi} :
U(\varphi) \longrightarrow \underline{\text{Proj}}_S(\mathcal{A})
$$
together with a map of graded $\mathcal{O}_X$-algebras
$$
\theta :
r_{\mathcal{L}, \varphi}^*\left(
\bigoplus\nolimits_{d \geq 0}
...
\right)
\longrightarrow
\bigoplus\nolimits_{d \geq 0} \mathcal{L}^{\otimes d}
$$
characterized by the following properties: (1)
For every open $V \subset S$ and every $d \geq 0$ the diagram
$$
\xymatrix{
\mathcal{A}_d(V) \ar[d]_{\psi} \ar[r]_{\varphi} &
\Gamma(f^{-1}(V), \mathcal{L}^{\otimes d}) \ar[d]^{restrict} \\
\Gamma(\pi^{-1}(V),
\mathcal{O}_{\underline{\text{Proj}}_S(\mathcal{A})}(d)) \ar[r]^{\theta} &
\Gamma(f^{-1}(V) \cap U(\varphi), \mathcal{L}^{\otimes d})
}
$$
is commutative. (2) For any $d \geq 1$ and any open subscheme $W \subset X$
such that $\varphi|_W : f^*\mathcal{A}_d|_W \to \mathcal{L}^{\otimes d}|_W$
is surjective the restriction of the morphism $r_{\mathcal{L}, \varphi}$
...
in Lemma \ref{lemma-relative-proj}.
\end{lemma}
