...
We can use the morphisms $i_U$ to transport
the sheaves $\mathcal{F}_U$ to sheaves $\mathcal{F}'_U$
on $f^{-1}(U)$ for $U \in \mathcal{B}$. Using the maps
introduced in the last displayed formula of the lemma we can
define isomorphisms
...
we have a triple of elements of $\mathcal{B}$.
...
$U_1 \cap U_2$ by elements of $\mathcal{B}$ and using the $\theta'$.
Hence we can use glueing of sheaves (see
Sheaves, Section \ref{sheaves-section-glueing-sheaves})
to produce $\mathcal{F}$ on $X$. By construction the last equation of
the lemma holds.
\end{proof}
...
$\mathcal{O}_{\mathbf{P}^n_R}(1)$ via (\ref{equation-global-sections}).
...
be as in Lemma \ref{lemma-glue-relative-proj}.
...
sheaves $\mathcal{O}_{\underline{\text{Proj}}_S(\mathcal{A})}(n)$
together with the isomorphisms $i_{U, n}^\sharp$ for all
$U \subset S$ affine open and all $n \in \mathbf{Z}$.
We omit the construction of the map $\psi$. (Functoriality
of glueing.)
We omit the verification that the diagrams commute and we
omit the verification that this uniquely characterizes the
