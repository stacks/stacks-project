...
(see Lemma \ref{lemma-structural-morphism-proj})
is an isomorphism.
\end{example}
...
by the opens $Y_s$, where $s \in \Gamma(Y, \mathcal{O}_Y(nd))$,
$n \geq 1$, and
\item a basis for the topology of $Y$ is given
...
$$
is commutative.
\item For any $f \in A_{+}$ homogeneous the
restriction of $r_\psi$ to $D_{+}(\psi(f))$
...
and the open subset $U(\psi)$. We omit the verification that
condition (1) uniquely determines the map $\theta$.
...
If $A \to B$ is surjective then $U(\psi) = Y$ and
$r_\psi : Y \to X$ is a closed immersion.
\end{lemma}
...
With hypotheses as in Lemma \ref{lemma-morphism-proj} above.
Assume $B \cong A_g$ as $A$-algebras for some $g \in A_0$.
Then $U(\psi) = Y$ and $r_\psi : Y \to X$ is an open
...
$\alpha : \varphi^*\mathcal{O}_X(d) \to \mathcal{L}^{\otimes d}$
...
Hence the open subset $U(\psi)$ of Lemma \ref{lemma-morphism-proj}
...
\begin{proof}
Omitted. Hint: Use the same idea as in the construction of
$\underline{\text{Proj}}_S(\mathcal{A})$. For transitivity
(or ``cocycle condition'') use the result on the $\theta$'s
in Lemma \ref{lemma-morphism-proj-transitive}.
It makes sense to add a lemma to Section \ref{section-relative-glueing}
on glueing relative sheaves over glueing relative schemes.
\end{proof}
...
N_{R, C, A, M, \gamma} =
(M^{(d)} \otimes_{A^{(d)}}
\bigoplus\nolimits_{n \in \mathbf{Z}} L^{\otimes n})_0
$$
