...
\noindent
The following lemma is relevant in case we have a construction that
is easier to understand over the affine opens of a base scheme $S$.
...
is a pair $f_{univ} : \underline{\text{Spec}}_S(\mathcal{A}) \to S$
and a morphism of $\mathcal{O}_S$-algebras
$$
\mathcal{A}
\longrightarrow
...
$$
can : X \longrightarrow \underline{\text{Spec}}(f_*\mathcal{O}_X)
$$
of schemes over $S$.
...
from Lemma \ref{lemma-proj-sheaves} (\ref{item-map})
...
If $A \to B$ is surjective then $V(\varphi) = Y$ and
...
then $V(\varphi) = Y$ and $\text{Proj}(\varphi) : Y \to X$ is an open
...
\begin{enumerate}
\item Let $n = \gcd(d, d')$. Write $n = ad = a'd'$. There exists
an isomorphism
...
restricted to $V_d \cap V_{d'}$. (Note that we allow
$V_d = \emptyset$.)
In addition we
...
where $U$ ranges over the affine opens of $S$. Glueing (without an a priori
defined functor) is bothersome since one has to check various diagrams commute
by ``bookkeeping''. See the proof of Lemma \ref{lemma-glue-relative-proj}
below. We first show that the homogeneous spectra
of the values of $\mathcal{A}$ over affines form a suitable
collection of schemes.

\begin{lemma}
\label{lemma-affine-inclusion}
...
Hence the open subset $V(\varphi)$ of Lemma \ref{lemma-morphism-proj}
...
\noindent
In particular the morphism of the lemma is an open
immersion.
...
$$
is the open immersion of Lemma \ref{lemma-affine-inclusion} above.
\end{enumerate}
\end{lemma}

\begin{proof}
Here is a standard glueing argument which first deals with the
case where $S$ is separated, and then does the general case.
Let $S = \bigcup V_i$ be an affine open covering.
Set $X_i = \text{Proj}(\mathcal{A}(V_i))$ and
$\pi_i : X_i \to V_i$ the structural morphism.
Set $U_{ij} = \pi_i^{-1}(V_i \cap V_j) \subset X_i$.
We are going to construct $X = \underline{\text{Proj}}_S(\mathcal{A})$
by glueing the schemes $X_i$ along the open subschemes
$U_{ij}$ as in Schemes, Section \ref{schemes-section-glueing-schemes}.

\medskip\noindent
First, let us assume that $S$ is separated.
In this case all the pairwise intersections
$V_i \cap V_j$ are affine as well. Hence by
Lemma \ref{lemma-affine-inclusion} we see that
$$
U_{ij} = \pi_i^{-1}(V_i \cap V_j) =
\text{Proj}(\mathcal{A}(V_i \cap V_j)) =
\pi_j^{-1}(V_i \cap V_j) = U_{ji}.
$$
This gives us the glueing morphisms
$\varphi_{ij} : U_{ij} \to U_{ji}$. At this point we have to
show that
(1) $\varphi_{ij}^{-1}(U_{ji} \cap U_{jk}) =  U_{ij} \cap U_{ik}$,
and (2) the cocycle condition. Part (1) follows from
the identity of opens $V_i \cap V_j \cap V_k = V_j \cap V_i \cap V_k$
in $S$ after pulling back by $\pi_i$.
We claim that the cocycle condition follows from the
assertion that for any $V \subset V' \subset V''$ affine open
in $S$ the composition
$$
\text{Proj}(\mathcal{A}(V))
\longrightarrow
\text{Proj}(\mathcal{A}(V'))
\longrightarrow
\text{Proj}(\mathcal{A}(V''))
$$
of the morphisms of Lemma \ref{lemma-affine-inclusion} associated
to the unclusions $V \subset V'$ and $V' \subset V''$ is the morphism
of Lemma \ref{lemma-affine-inclusion} associated to the inclusion
$V \subset V''$. Details omitted.

\medskip\noindent
In the general case we use that the schemes $V_i$,
$V_i \cap V_j$, $V_i \cap V_j \cap V_k$ are all separated.
Hence since the construction is known in each of those cases
we get the glueing morphisms $\varphi_{ij}$ and the
cocycle condition from the lemma in the separated case.
Details omitted.
\end{proof}
...
$\text{Proj}$ by writing a functor which the relatve
...
triple $(d, f \circ h, h^*\mathcal{L}, h^*\psi)$.
...
We will not formally define the functor which is represented by
the relative homogeneous spectrum, instead we will glue the schemes
representing the functors $F_d$ using the notion of equivelence to
glue them.
...
to Lemma \ref{lemma-proj-sheaves} part (\ref{item-map}).
...
\longrightarrow
S' \times_S \underline{\text{Proj}}_{S}(\mathcal{A})
$$
\end{lemma}
...
$$
\xymatrix{
\mathcal{A}_d(V) \ar[d]_{canonical} \ar[r]_{\varphi} &
...
see Constructions, Lemma \ref{lemma-relative-proj-base-change}.
