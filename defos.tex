...
(S_1, \mathcal{O}_S) \ar[r]^{t_1} &
(S'_1, \mathcal{O}_{S'_1})
}
}
\end{equation}
with horizontal arrows first order thickenings with
$\mathcal{G}_j = \text{Ker}(i_j^\sharp)$ and a $g$-map
of modules $\nu : \mathcal{G}_1 \to \mathcal{G}_2$
...
(\ref{equation-huge-1}) compatibly with $\nu$
the commutative diagram of the lemma is in one-to-one bijection with
...
$$
\xi : \mathcal{I}/\mathcal{I}^2 \longrightarrow \mathcal{N}_2
$$
