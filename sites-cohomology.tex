...
\begin{lemma}
\label{lemma-restriction-along-monmorphism-surjective}
...
$$
\Hom_{D(\mathcal{O})}(Lf^*\mathcal{G}^\bullet, \mathcal{F}^\bullet) =
\Hom_{D(\mathcal{O})}(f^*\mathcal{K}^\bullet, \mathcal{I}^\bullet) =
\Hom_{K(\textit{Mod}(\mathcal{O}))}(f^*\mathcal{K}^\bullet,
\mathcal{I}^\bullet)
$$
...
\begin{align*}
\Hom_{K(\textit{Mod}(\mathcal{O}'))}((\mathcal{K}')^\bullet,
f_*\mathcal{I}^\bullet)
& =
\Hom_{K(\textit{Mod}(\mathcal{O}'))}(f^*(\mathcal{K}')^\bullet,
\mathcal{I}^\bullet) \\
...
(Lemma \ref{lemma-restriction-along-monmorphism-surjective}).
...
$$
L\pi_!(K) =
L\pi_!(K \otimes_\mathcal{O}^\mathbf{L} \underline{B})
$$
in $D(\textit{Ab})$.
\end{lemma}
...
In the situation of Lemma \ref{lemma-O-homology-B-homology-general}
we obtain a functor of triangulated categories
$$
...
and we can use the functor $L\pi_! : D(\underline{B}) \to D(B)$.
In other words, the $B$-module structure on $L\pi_!(K)$ comes from
the (canonical, functorial) identification of $L\pi_!(K)$ with
...
\text{Coker}(\prod \mathcal{F}_n \to \prod \mathcal{F}_n)
$$
...
$\text{Im}(\mathcal{F}_1 \to \mathcal{F}_2)$.
...
\underline{\text{Ker}(\Lambda^{\oplus r} \to M)} \to 0
$$
in degree $b - 1$. By
Modules on Sites, Lemma
