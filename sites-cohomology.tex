...
$$
H^p(V_i, H^{m - p}(E)) = 0 \text{ for }
\{V_i \to V\} \in \text{Cov}_V \text{ and } p > p(V, m)
$$
\end{enumerate}
...
precisely for $X \in \Ob(\textit{LC})$, we will also denote the
induced morphism
$$
\epsilon : \textit{LC}_{qc}/X \longrightarrow \textit{LC}_{Zar}/X
$$
(of Sites, Lemma \ref{sites-lemma-localize-morphism})
by $\epsilon$. Let $X_{Zar}$ be the site whose objects are opens of $X$, see
Sites, Example \ref{sites-example-site-topological}.
There is a morphism of sites
$$
\pi : \textit{LC}_{Zar}/X \longrightarrow X_{Zar}
$$
...
$$
a : \Sh(\textit{LC}_{qc}/X) \longrightarrow \Sh(X)
$$
which will often be denoted $a$ as indicated.
...
\noindent
Here is a somewhat unexpected result.

\begin{lemma}
\label{lemma-unexpected}
The functor
$\epsilon_* : \textit{Ab}(\textit{LC}_{qc}) \to \textit{Ab}(\textit{LC}_{Zar})$
is exact. Similarly, for $X \in \Ob(\textit{LC})$ the functor
$\epsilon_* : \textit{Ab}(\textit{LC}_{qc}/X) \to
\textit{Ab}(\textit{LC}_{Zar}/X)$ is exact.
\end{lemma}

\begin{proof}
The first statement is equivalent to the assertion that
$R^1\epsilon_*\mathcal{F}$
is zero for any abelian sheaf $\mathcal{F}$ on $\textit{LC}_{qc}$.
Recall that $R^1\epsilon_*\mathcal{F}$ is the sheaf on $\textit{LC}_{Zar}$
associated to the presheaf $U \mapsto H^1_{qc}(U, \mathcal{F})$, see
Lemma \ref{lemma-higher-direct-images}.
We use the subscript ${}_{qc}$ to indicate we are taking cohomology
on the site $\textit{LC}_{qc}$ as opposed to $\textit{LC}_{Zar}$.
Let $\xi \in H^1_{qc}(U, \mathcal{F})$
for some $U \in \textit{LC}$. Let $u \in U$.
We have to find an open neighbourhood $u \in V \subset U$
such that $\xi$ maps to zero in $H^1_{qc}(V, \mathcal{F})$.
...
$Z = \bigcup f_{i_j}(E_j)$ is a neighbourhood of $x$.
Observe that $Y = \coprod_{j = 1, \ldots, m} E_j \to Z$
is proper as a continuous map from a quasi-compact space
to a Hausdorff one (Topology, Lemma \ref{topology-lemma-closed-map}).
Choose $u \in V \subset Z$ open.
Then $Y \times_Z V \to V$ is surjective and proper.
The reader shows succesively that the restriction of $\xi$
to $U_i$, $E_j$, $Y$, $Y \times_Z V$ is zero.
Hence we may and do assume there is a surjective proper
map $f : U' \to U$ such that $\xi$ restricts to zero on $U'$.

\medskip\noindent
Since $\mathcal{U} = \{U' \to U\}$ is a covering in $\textit{LC}_{qc}$
(Lemma \ref{lemma-proper-surjective-is-qc-covering})
we have the {\v C}ech to cohomology spectral sequence
of Lemma \ref{lemma-cech-spectral-sequence}.
Since $\xi$ maps to zero in $H^1_{qc}(U', \mathcal{F})$
we find that $\xi$ comes from a (unique) element in
$$
\check{H}^1(\mathcal{U}, \mathcal{F}) =
H^1(
\mathcal{F}(U') \to 
\mathcal{F}(U' \times_U U') \to
\mathcal{F}(U' \times_U U' \times_U U') \to \ldots)
$$
For $u \in V \subset U$ open set $\mathcal{V} = \{f^{-1}(V) \to V\}$
and consider
$$
\check{H}^1(\mathcal{V}, \mathcal{F}) =
H^1(
\mathcal{F}(f^{-1}V) \to 
\mathcal{F}(f^{-1}V \times_U f^{-1}V) \to \ldots)
$$
To show that $\xi$ maps to zero in this for some $V$, it
suffices to show that the colimit over all open neighbourhoods
is exact. By proper base change for pushforward
(see Cohomology, Lemma \ref{cohomology-lemma-proper-base-change})
the colimit of these complexes is
$$
\mathcal{F}(T) \to \mathcal{F}(T \times T) \to
\mathcal{F}(T \times T \times T) \to \ldots
$$
where $T = f^{-1}(\{u\})$ is the (nonempty) fibre of $f$ over $u$.
To show: this is exact in degree $1$. (We encourage
the reader to prove this themselves and skip the rest of this
paragraph.) The morphism of simplicial topological spaces
$(T^n) \to \{u\}$ is a homotopy equivalence, see
Simplicial, Lemma
\ref{simplicial-lemma-fibre-products-simplicial-object-w-section}.
Since $\mathcal{F}$ is functor, we conclude that the cosimplicial
abelian group with values $\mathcal{F}(T^n)$ is homotopy equivalent
to the constant cosimplicial abelian group with value $\mathcal{F}(\{u\})$.
In particular the cohomology in degree $1$ is zero as desired.

\medskip\noindent
The second statement is prove using exactly the same arguments as above.
\end{proof}
...
by a bounded below complex $\mathcal{F}^\bullet$.
By the case of a sheaf we see that
$\mathcal{F}^n = \pi_*\pi^{-1} \mathcal{F}^n = a_* a^{-1} \mathcal{F}^n$
and that $R^q\pi_*\pi^{-1} \mathcal{F}^n$ and $R^qa_*a^{-1}\mathcal{F}^n$
are zero for $q > 0$. By Leray's acyclicity lemma
(Derived Categories, Lemma \ref{derived-lemma-leray-acyclicity})
applied to $\pi^{-1}\mathcal{F}^\bullet$,
resp.\ $a^{-1}\mathcal{F}^\bullet$
and $\pi_*$, resp.\ $a_*$ we conclude.

\medskip\noindent
From now on assume $K = \mathcal{F}$.

\medskip\noindent
To see the statement for $\pi$ we compute for $U \subset X$ open
$$
R\Gamma(U, R\pi_*\pi^{-1}\mathcal{F}) =
R\Gamma(U, \pi^{-1}\mathcal{F}) =
R\Gamma(U, \mathcal{F})
$$
The first equality by Leray, see Lemma \ref{lemma-Leray-unbounded}.
The second equality by
Lemma \ref{lemma-cohomology-bigger-site} applied
to the functor $X_{Zar} \to \textit{LC}_{Zar}$.
Thus $\mathcal{F} \to R\pi_*\pi^{-1}\mathcal{F}$ is an isomorphism.

\medskip\noindent
By Lemma \ref{lemma-describe-pullback-pi} we have
$a_*a^{-1}\mathcal{F} = \mathcal{F}$. Thus it suffices to show that
$R^qa_*a^{-1}\mathcal{F} = 0$ for $q > 0$.
For this we can use $a = \epsilon \circ \pi$ and
the Leray spectral sequence Lemma \ref{lemma-relative-Leray}.
By Lemma \ref{lemma-unexpected}
we have $R^i\epsilon_*(a^{-1}\mathcal{F}) = 0$ for $i > 0$.
By Lemma \ref{lemma-describe-pullback-pi} we have
$\epsilon_*a^{-1}\mathcal{F} = \pi^{-1}\mathcal{F}$
and hence by the previous paragraph
$R^j\pi_*(\epsilon_*a^{-1}\mathcal{F}) = R^j\pi_*(\pi^{-1}\mathcal{F}) = 0$
for $j > 0$. This concludes the proof.
\end{proof}
...
\begin{proof}
This follows from
Lemma \ref{lemma-cohomological-descent-LC}
by
Remark \ref{sites-cohomology-remark-before-Leray}.
\end{proof}
