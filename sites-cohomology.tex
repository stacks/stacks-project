...
Sites, Lemma \ref{sites-lemma-colimit}. In particular, all coverings
in $\mathcal{C}$
and $\mathcal{C}_i$ have finite index sets. Moreover, assume given
\begin{enumerate}
\item an abelian sheaf $\mathcal{F}_i$ on $\mathcal{C}_i$ for all $i \in I$,
\item for $i' \geq i$ a map
$\varphi_{i'i} : f_{i'i}^{-1}\mathcal{F}_i \to \mathcal{F}_{i'}$
of abelian sheaves on $\mathcal{C}_{i'}$
\end{enumerate}
...
$$
\colim_{i \geq 0} H^p(X_i, \mathcal{F}_i) = H^p(X, \mathcal{F})
$$
for all $p \geq 0$.
\end{lemma}
...
We prove the result for $p > 0$ by induction on $p$.

\medskip\noindent
For each $i$ we pick an injection $f_{i0}^{-1}\mathcal{F}_0 \to \mathcal{I}_i$
...
\mathcal{F}_i
\longrightarrow
\prod\nolimits_{j \geq i} f_{ji, *}\mathcal{I}_j = \mathcal{G}_i
$$
where the component maps are the maps adjoint to the maps
$f_{ji}^{-1}\mathcal{F}_i \to \mathcal{F}_j \to \mathcal{I}_j$.
There are maps
$$
\psi_{i'i} : f_{i'i}^{-1}\mathcal{G}_i \to \mathcal{G}_{i'}
$$
whose compenents are the canonical maps
$f_{i'i}^{-1}f_{ji, *}\mathcal{I}_j \to f_{ji', *}\mathcal{I}_j$
for $j \geq i'$. Thus we find an injection
$\{\gamma_i\} : \{\mathcal{F}_i, \varphi_{i'i})
\to (\mathcal{G}_i, \psi_{i'i})$ of systems of abelian sheaves.
Note that $\mathcal{G}_i$ is an injective sheaf of abelian groups
on $\mathcal{C}_i$, see
Lemma \ref{lemma-pushforward-injective-flat}
and
Homology, Lemma \ref{homology-lemma-product-injectives}.
...
$\mathcal{U}_i$ of $\mathcal{C}_i$. We have
$$
\check{\mathcal{C}}^\bullet(\mathcal{U}, \mathcal{G}) =
\colim_{i' \geq i}
\check{\mathcal{C}}^\bullet(\mathcal{U}_{i'}, \mathcal{G}_{i'})
$$
(with obvious notation) by the case $p = 0$.
The right hand side is acyclic in positive degrees
