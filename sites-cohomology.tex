...
$$
of objects of $\mathcal{C}$. In this situation, given an abelian
sheaf $\mathcal{F}$ on $\mathcal{C}$ we get what looks like the
beginning of a distinguished triangle
$$
R\Gamma(X, \mathcal{F}) \to
R\Gamma(Z, \mathcal{F}) \oplus
R\Gamma(Y, \mathcal{F}) \to
R\Gamma(E, \mathcal{F})
$$
...
$Rf_{\tau', *}K' \in D^+_{\mathcal{A}'_X}(\mathcal{C}_{\tau'}/X)$ and
...
$$
is an isomorphism. Using the spectral sequence above and the
corresponding spectral sequence
$$
E_2^{p, q} = R^pf_{\tau, *}H^q(\epsilon_X^{-1}K')
\Rightarrow R^{p + q}f_{\tau, *}\epsilon_X^{-1}K'
$$
we reduce this to the case where $K'$
has a single nonzero cohomology sheaf $\mathcal{F}'$ in
$\mathcal{A}'_X$. Details omitted.
...
It is clear that $\epsilon_X^{-1}$ sends
$D^+_{\mathcal{A}'_X}(\mathcal{C}_{\tau'}/X)$ into
$D^+_{\mathcal{A}_X}(\mathcal{C}_\tau/X)$.
Conversely, $R\epsilon_{X, *}$ sends
$D^+_{\mathcal{A}_X}(\mathcal{C}_\tau/X)$ into
$D^+_{\mathcal{A}'_X}(\mathcal{C}_{\tau'}/X)$
since $(V_n)$ holds for all $n$ by Lemma \ref{lemma-V-C-all-n-general};
details omitted.
We have $\epsilon_X^{-1} \circ R\epsilon_{X, *} = \text{id}$
as functors on the entire derived category
by the general discussion in Section \ref{section-compare}.
We have $R\epsilon_{X, *} \circ \epsilon_X^{-1} = \text{id}$
on $D^+_{\mathcal{A}'_X}(\mathcal{C}_{\tau'}/X)$ by
Lemma \ref{lemma-V-C-all-n-general}. This finishes the proof.
\end{proof}
...
\begin{align*}
(V_n) &
\text{ For }X \in \Ob(\textit{LC}_{qc})
\text{ and abelian sheaf }\mathcal{F}
\text{ on }X
\text{ we have }\epsilon_{X, *}a_X^{-1}\mathcal{F} = \pi_X^{-1}\mathcal{F}\\
&
\text{ and }R^i\epsilon_{X, *}(a_X^{-1}\mathcal{F}) = 0
\text{ for }1 \leq i \leq n.\\
(C_n) &
\text{ For a proper morphism }f : X \to Y
\text{ in }\textit{LC}_{qc}
\text{ and abelian sheaf }\mathcal{F}
\text{ on }X
\text{ we}\\
&
\text{ have }a_Y^{-1}(R^if_*\mathcal{F}) = R^if_{qc, *}(a_X^{-1}\mathcal{F})
\text{ for }i = 0, \ldots, n.
\end{align*}
