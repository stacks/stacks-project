...
\begin{lemma}
\label{lemma-colim-coherent-topos}
\begin{slogan}
Cohomology commutes with filtered colimits on a coherent topos.
\end{slogan}
Let $\mathcal{C}$ be a site. Let $S \subset \Ob(\Sh(\mathcal{C}))$
be a subset. Assume\footnote{These conditions say that
$\Sh(\mathcal{C})$ is a coherent topos.}
\begin{enumerate}
\item $* \in S$ where $*$ denotes the final object of $\Sh(\mathcal{C})$,
\item elements of $S$ are quasi-compact
(Sites, Definition \ref{sites-definition-quasi-compact-topos}),
\item $\mathcal{K} \times_{\mathcal{K}'} \mathcal{K}''$ is quasi-compact
whenever given maps $\mathcal{K} \to \mathcal{K}'$ and
$\mathcal{K}'' \to \mathcal{K}'$ with
$\mathcal{K}, \mathcal{K}', \mathcal{K}'' \in S$,
\item for every $\mathcal{G}$ in $\Sh(\mathcal{C})$ there exists
a surjection $\coprod_{i \in I} \mathcal{K}_i \to \mathcal{G}$ with
$\mathcal{K}_i \in S$.
\end{enumerate}
Then the map
$$
