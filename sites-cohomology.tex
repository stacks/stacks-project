...
\noindent
We briefly discuss $R\lim$ on abelian groups. Consider the category
$\mathbf{N}$ whose objects are natural numbers and whose morphisms are unique
...
\end{equation}
as defined in Section \ref{section-unbounded}.
...
of modules, which is {\bf not} the same thing as an inverse system of objects
in the derived categories. In the following remark we discuss how to
...
\end{enumerate}
Moreover, if this holds, then we have $H^p(U, \mathcal{F}) = 0$
...
Sites, Equation (\ref{sites-equation-map-representable-into-sheaf}).
...
\medskip\noindent
If (2) holds, then $H^0(U, -)$ is an exact functor on the category
of abelian sheaves on $\mathcal{C}$. Thus its higher derived
functors vanish by
Derived Categories, Lemma \ref{derived-lemma-right-derived-exact-functor}.
\end{proof}
...
is {\it weakly contractible} if the equivalent conditions of
Lemma \ref{lemma-w-contractible} hold.
\end{definition}

\noindent
The small \'etale site of $\mathbf{A}^1_\mathbf{C}$ does not have
any weakly contractible objects. On the other hand, the small
pro-\'etale site of any scheme has plenty. It is convenient to list
some consequences of this here.

\begin{proposition}
\label{proposition-enough-weakly-contractibles}
Let $\mathcal{C}$ be a site such that every object $U$ of $\mathcal{C}$
has a covering $\{U_i \to U\}$ such that each $U_i$ is weakly contractible.
Let $\mathcal{O}$ be a sheaf of rings on $\mathcal{C}$.
Then
\begin{enumerate}
