...
corresponding to the continuous functor $u : \mathcal{D} \to \mathcal{C}$.
Let $(\mathcal{F}_i, \varphi_{ii'})$ be directed system of
...
$$
\mathcal{G}' = \colim \mathcal{G}_i \longrightarrow \mathcal{G}
$$
induces an isomorphism on sheafifications. Let $V \in \Ob(\mathcal{D})$.
Let us prove that $\mathcal{G}')^\#(V) \to \mathcal{G}^\#(V)$ is
surjective. To do this we will use Sites, Lemma
\ref{sites-lemma-sections-sheafification}.
For any $s \in \mathcal{G}^\#(V)$ there exists
a covering $\{V_i \to V\}$ and sections $s_i \in \mathcal{G}(V_i)$ such that
\begin{enumerate}
\item $s|_{V_i}$ is the image of $s_i$, and
\item for every $i, j$ there exists a covering
$\{V_{ijk} \to V_i \times_V V_j\}$ of $\mathcal{D}$ such that
the pullback of $s_i$ and $s_j$ to each $V_{ijk}$ agree.
\end{enumerate}
By assumption, for each $i$ we may choose a covering $\{V_{i, a} \to V_i\}$
with $V_{i, a} \in \mathcal{B}$. Then $\{V_{i, a} \to V\}$ is
a covering refining the given covering $\{V_i \to V\}$.
Denoting $s_{i, a}$ the image of $s_i$ in $\mathcal{G}(V_{i, a})$
we see that the pullback of $s_{i, a}$ and $s_{j, b}$ to the members
of the covering
$$
\{(V_{i, a} \times_V V_{j, b}) \times_{V_i \times_V V_j} V_{ijk}
\to V_{i, a} \times_V V_{j, b}\}
$$
agree. Hence we may assume that $V_i \in \mathcal{B}$. Repeating
the argument, we may also assume $V_{ijk} \in \mathcal{B}$ for
all $i, j, k$ (details omitted).
Then since $\mathcal{G}'(V_i) \to \mathcal{G}(V_i)$
is bijective by our definition of $\mathcal{B}$ in the statement
of the lemma, we get unique $s'_i \in \mathcal{G}'(V_i)$
mapping to $s_i$. The pullback of $s'_i$ and $s'_j$
to each $V_{ijk}$ agree in $\mathcal{G}'(V_{ijk})$ because
$V_{ijk} \in \mathcal{B}$ and because we have the agreement
for $s_i$ and $s_j$.
Then the cited lemma tells us there exists a unique section
$s' \in (\mathcal{G}')^\#$ such that $s'|_{V_i}$ is the image of $s'_i$.
It follows that $s'$ maps to $s$.
We omit the proof that $(\mathcal{G}')^\#(V) \to \mathcal{G}(V)$
is injective because it is similar.
\end{proof}
