...
nonzero morphism $E \to K$ of $\mathit{QC}(\mathcal{O})$ such that $|E| \leq \kappa$,
...
such that $\mathcal{H}^\bullet$ represents an object of $\mathit{QC}(\mathcal{O})$
...
Hence $\mathcal{H}^\bullet$ represents an object of $\mathit{QC}(\mathcal{O})$ as
desired.
...
\item any contravariant cohomological functor $H : \mathit{QC}(\mathcal{O}) \to \textit{Ab}$
