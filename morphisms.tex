...
\begin{lemma}
\label{lemma-set-finite-type-over}
Let $S$ be a scheme. There exists a set $F = \{f : X_f \to S\}$ of
morphisms with target $S$ such that for every finite type morphism
$g : X \to S$ there exists an $f \in F$ and an isomorphism
$X_f \to X$ over $S$.
\end{lemma}

\begin{proof}
Omitted, but this basically follows from the fact that given a
ring $R$ there is a set of isomorphism classes of finite type
$R$-algebras.
\end{proof}
