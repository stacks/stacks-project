...
\item there exists an open $V \subset Y$, $\eta \in |V|$
such that $f^{-1}(V) \to V$ is finite.
\end{enumerate}
\end{lemma}

\begin{proof}
Since $Y$ is decent we can represent $y$ by a quasi-compact
monomorphism $y : \Spec(k) \to Y$ where $k$ is a field.
Then $f^{-1}(\{y\}) = |X_y|$ because $X_y \to X$ is a monomorphism.
Thus $X_y$ is a quasi-separated algebraic space of finite type
over a field $k$ with finitely many points. Hence $X_y \to \Spec(k)$
is quasi-finite, hence $X \to Y$ is quasi-finite at all points
of $X$ over $Y$.

\medskip\noindent
Choose an affine scheme $V$ and an \'etale morphism $V \to Y$ mapping
a point $v \in V$ to $y$. Then $v$ is a generic point of an irreducible
component of $V$ by Decent Spaces, Lemma
\ref{decent-spaces-lemma-decent-generic-points}.

\medskip\noindent
Choose an affine scheme $U$ and a surjective \'etale morphism
$U \to V \times_Y X$. Then $U \to V$ is of finite type
and the fibre over $v$ is finite. By
...
Since $x$ is a codimension $1$ point, $X' \to X$ is finite over a
neighbourhood of $x$ (argue using the dimension formula, compare with
Chow Homology, Lemma \ref{chow-lemma-finite-in-codimension-one}).
