...
Let $k$ be an infinite field. Let $X$ be an algebraic space over $k$.
If there exists a purely transcendental field extension $k \subset K$
such that $X_K$ is a scheme, then $X$ is a scheme.
\end{lemma}
...
$$
for some nonzero $f \in k[x_1, \ldots, x_n]$. Since $k$ is infinite
there exist $a_1, \ldots, a_n \in k$ with $f(a_1, \ldots, a_n) \not = 0$.
...
let $G$ be a finite group acting freely on $X$. Set $Y = X/G$.
For $y \in |Y|$ the following are equivalent
\begin{enumerate}
\item $y$ is in the schematic locus of $Y$, and
\item there exists a $G$-invariant affine open $U \subset X$
...
$X \to Y$ is even finite \'etale. Thus if $y \in Y$ is in the
schematic locus, then it has an affine open neighbourhood $V \subset Y$
and the inverse image $U$ of $V$ in $X$ is a $G$-invariant open subscheme
finite over $V$, hence affine.

\medskip\noindent
Conversely, assume that $U \subset X$ as in (2) is given.
Then the quotient $V = U/G$ is a scheme, for example by
Properties of Spaces, Lemma \ref{spaces-properties-lemma-quotient-scheme}.
Then $V \to Y$ is an open immersion (as the base change to $X$ is so)
and we conclude that $y$ is in the schematic locus of $Y$.
\end{proof}
...
\begin{enumerate}
\item $X$ is separated and locally of finite type over $k$,
...
is a scheme for some finite extension $k'/k$. After enlaring
...
$\Spec(k) \times_X X_{k'}$ is contained in $U'$.
Since $X_{k'}$ is a separated scheme (as $X$ is separated) the intersection
of a finite number of affine opens is affine open
(Schemes, Lemma \ref{schemes-lemma-characterize-separated}).
Thus we see that
$$
U = \bigcap\nolimits_{\sigma \in G} \sigma(U')
$$
is a $G$-invariant affine open subscheme of $X_{k'}$
containing the closed subscheme $\Spec(l) \times_X X_{k'}$, i.e.,
the inverse image of our point $x$. By
Lemmas \ref{lemma-base-change-by-Galois} and
\ref{lemma-when-quotient-scheme-at-point} we conclude.
\end{proof}
