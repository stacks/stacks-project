...
Let $U \subset S$ be open. Assume
\begin{enumerate}
\item $U$ is dense in $S$ and restrocompact in $S$,
\item the base changes of $\alpha$ and $\beta$ to $U$ are equal,
...
compatible with specializations. If $S$ Noetherian, then there exists a
surjective proper morphism $g : T \to S$, an integer $t$, integers
...
have an expression as in the lemma. We claim the lemma holds.
...
and to $0$ over $T''$. It easily follows that the claim is true.
