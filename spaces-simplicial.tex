...
\begin{lemma}
\label{lemma-functor-cohomological-descent-abelian}
Let $a : \Sh(\mathcal{C}) \to \Sh(\mathcal{D})$ be a morphism of topoi.
Consider the full subcategory $D' \subset D(\mathcal{D})$
consisting of objects $K$ such that
$$
K \longrightarrow Ra_*a^{-1}K
$$
is an isomorphism. Then $D'$ is a saturated triangulated strictly full
subcategory of $D(\mathcal{D})$ and the functor
$a^{-1} : D' \to D(\mathcal{C})$ is fully faithful.
\end{lemma}
...
is omitted; it follows formally from the fact that $Ra_*$ and
$a^{-1}$ are exact functors of triangulated categories.
The final part follows from the
fact that $a^{-1}$ and $Ra_*$ are adjoint by
...
\begin{lemma}
\label{lemma-image-cohomological-descent-abelian}
Let $a : \Sh(\mathcal{C}) \to \Sh(\mathcal{D})$ be a morphism of topoi.
Let $K$ be an object of $D^+(\mathcal{C})$
such that $a^{-1}Ra_*H^q(K) \to H^q(K)$ is an isomorphism
for all $q$. Then $K = a^{-1}L$ for some $L \in D^+(\mathcal{D})$.
\end{lemma}

\begin{proof}
We claim that $L = Ra_*K$ works. In other words, we claim that
the canonical map $a^{-1}L \to K$ is an isomorphism.
Namely, consider the spectral sequence
$$
{}''E_2^{p, q} = R^pa_*H^q(K)
$$
converging to $H^{p + q}(L) = R^{p + q}a_*K$, see
Derived Categories, Lemma \ref{derived-lemma-two-ss-complex-functor}.
If we pull back this spectral sequence to $\mathcal{C}_{total}$
we get
$$
a^{-1}({}''E_2^{p, q}) = a^{-1}(R^pa_*H^q(K)) =
\left\{
\begin{matrix}
0 & \text{if} & p > 0 \\
H^q(K) & \text{if} & p = 0
...
\right.
$$
by our assumption on $K$. The convergence gives $H^q(a^{-1}L) = H^q(K)$
as desired.
\end{proof}

\begin{lemma}
\label{lemma-unbounded-image-cohomological-descent-abelian}
Let $a : \Sh(\mathcal{C}) \to \Sh(\mathcal{D})$ be a morphism of topoi.
Let $K$ be an object of $D(\mathcal{C})$. Let $d \geq 0$ be an integer.
Assume
\begin{enumerate}
\item $K \to R\lim \tau_{\geq -n} K$ is an isomorphism,
\item $R^pa_*H^q(K) = 0$ for $p > d$ and $q < 0$,
\item $a^{-1}a_*H^q(K) \to H^q(K)$ is an isomorphism for $q \in \mathbf{Z}$, and
\item another condition is needed here.
\end{enumerate}
Then $K = a^{-1}L$ for some $L \in D^+(\mathcal{D})$.
\end{lemma}
