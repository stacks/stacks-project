...
\item $g_n^{-1}$ has a left adjoint $g^{Sh}_{n!}$ which commutes
with finite connected limits, and
\item $g_n^{-1} : \textit{Ab}(\mathcal{C}_{total}) \to \textit{Ab}(\mathcal{C}_n)$
has a left adjoint
$g_{n!} : \textit{Ab}(\mathcal{C}_n) \to \textit{Ab}(\mathcal{C}_{total})$
which is exact.
\end{enumerate}
\end{lemma}
...
\medskip\noindent
Next, we come to the exactness properties of $g^{Sh}_{n!}$ and $g_{n!}$.
...
then we claim $\mathcal{H} = g^{Sh}_{n!}\mathcal{G}$ is the sheaf on
...
$b : \mathcal{G} \to \mathcal{F}_n$ we can define
...
then $g_{n!}\mathcal{G}$ is the abelian sheaf on $\mathcal{C}_{total}$
...
morphism of ringed topoi $g_n : (\Sh(\mathcal{C}_n), \mathcal{O}_n) \to
...
then $g_{n!}\mathcal{G}$ is the $\mathcal{O}$-module
...
\begin{proof}
We will use the description of $g_{n!}$ given in the proof of
Lemma \ref{lemma-restriction-to-components-site}.
As maps of the complex we take $\sum (-1)^i d^n_i$ where
