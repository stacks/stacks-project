...
\noindent
Let $\mathcal{C}$ be as in Situation \ref{situation-simplicial-site}.
In statement of the following lemma we will let
$g_n : \mathcal{C}_n \to \mathcal{C}_{Zar}$ be the functor of
Lemma \ref{lemma-restriction-to-components-site}. We will denote
$g_{n, p!} : \textit{PAb}(\mathcal{C}_n) \to \textit{PAb}(\mathcal{C}_{Zar})$
the functor from
...
adjoint to the map $\mathbf{Z} \to \bigoplus_{[n - 1] \to [n]} \mathbf{Z}$
