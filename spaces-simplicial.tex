...
given in the proof of Lemma \ref{lemma-restriction-to-components-site}
...
Using the description of $g_{n!}$
given in the proof of Lemma \ref{lemma-exactness-g-shriek-modules}
we see that the restriction to $\mathcal{C}_m$ of this map
is
$$
...
\noindent
In this section we are going to prove two special cases of
\cite[Proposition 3.2.9]{BBD} using a different method.
...
\begin{lemma}
\label{lemma-basic-postnikov}
In Situation \ref{situation-simplicial-site}. Set
$$
X_n = g_{n!}\mathbf{Z}
\quad\text{and}\quad
Y_n = (X_n \to \ldots \to X_0)[-n]
$$
as objects of $D(\mathcal{C}_{total})$ where the maps are
as in Lemma \ref{lemma-simplicial-resolution-Z-site}.
With the evident canonical maps $X_n \to Y_n$ and
...
\item $g_m^{-1}(Y_n) \cong \mathbf{Z}[-n] \oplus \mathbf{Z}^{\oplus r_{n, m}}$
for some integers $r_{n, m} \geq 0$,
\item $\mathbf{Z} = \text{hocolim} Y_n[n]$ in $D(\mathcal{C}_{total})$.
\end{enumerate}
\end{lemma}
...
(Derived Categories, Definition \ref{derived-definition-postnikov-system})
for $\ldots \to X_2 \to X_1 \to X_0$,
\item $g_m^{-1}(Y_n) \cong K_m[-n] \oplus K_m^{\oplus r_{n, m}}$
for some integers $r_{n, m} \geq 0$,
...
\begin{proof}
This is a consequence of Lemma \ref{lemma-basic-postnikov}
and the fact that an exact functor of triangulated categories
sends Postnikov systems to Postnikov systems and that
$\otimes^\mathbf{L}$ commutes with homotopy colimits.
\end{proof}
...
for some $n$. From the distinguished triangle
$$
...
$$
is zero. To see this, arguing as above, it suffices to show that
$$
\Hom(K_n[n + 1], K'_n)  = 0
$$
for all $n$ which follows from condition (3).
\end{proof}
...
\end{enumerate}
Then any map $\{K_n \to K'_n\}$ of simplicial systems 
comes from a map $K \to K'$ in $D(\mathcal{C}_{total})$.
\end{lemma}
