...
$X_{Zar}$\footnote{This notation suck, but at least is
consistent with the notation in
...
\begin{enumerate}
\item $\Sh(X_{Zar})$, and
\item category of objects $\mathcal{F} = (\mathcal{F}_n, \mathcal{F}(-))$
where $\mathcal{F}_n$ is a sheaf on $X_n$ and for every
$\varphi : [n] \to [m]$ the object
$\mathcal{F}(\varphi) : \mathcal{F}_n \to \mathcal{F}_m$
is an $X(\varphi)$-map such that
...
\begin{proof}
This is immediate from the definitions and the
definition of ``$f$-maps'' in
Sheaves, Definition \ref{sheaves-definition-f-map}.
\end{proof}

\begin{lemma}
\label{lemma-simplicial-space-site-functorial}
Let $f : X \to Y$ be a morphism of simplicial spaces.
Then the functor $u : Y_{Zar} \to X_{Zar}$
which associates to the open $V \subset Y_n$ the open $f_n^{-1}(V) \subset X_n$
defines a morphism of sites
$f_{Zar} : X_{Zar} \to Y_{Zar}$.
\end{lemma}
