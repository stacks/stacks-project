...
morphisms $(g_i, g'_i) : U_i \to V_{\alpha(i)} \times_S V_{\alpha'(i)}$.
\end{proof}
...
Let $f : X \to Y$ be a morphism of schemes. Each of the morphisms of sites
$f_{sites}$ listed in
Topologies, Section \ref{topologies-section-change-topologies}
becomes a morphism of ringed sites. Namely, we have seen in that section
that each of these morphisms of sites
$f_{sites} : (\textit{Sch}/X)_\tau \to (\textit{Sch}/Y)_{\tau'}$, or
$f_{sites} : (\textit{Sch}/X)_\tau \to Y_{\tau'}$ is given by the continuous
functor $Y'/Y \mapsto X \times_Y Y'/Y$. Hence, given $Y'/Y$ we let
$$
f_{sites}^\sharp :
\mathcal{O}(Y'/Y)
\longrightarrow
f_{sites, *}\mathcal{O}(Y'/Y) =
\mathcal{O}(X \times_Y Y'/X)
$$
be the usual map
$\text{pr}_{Y'}^\sharp : \mathcal{O}(Y') \to \mathcal{O}(X \times_Y Y')$.
Applications:
\begin{enumerate}
\item In Remark \ref{remark-Zarsiki-site-space} we have seen that
$\textit{Mod}(\mathcal{O}_X) = \textit{Mod}(X_{Zar}, \mathcal{O})$.
Hence we can reinterpret the construction $\mathcal{F} \mapsto \mathcal{F}^a$
as the pullback under the morphism of ringed sites
$\text{id}_{X, sites} : ((\textit{Sch}/X)_\tau, \mathcal{O})
\longrightarrow (X_{Zar}, \mathcal{O})$
or
$
\text{id}_{X, sites} : (X_{etale}, \mathcal{O}) \to (X_{Zar}, \mathcal{O}).
$
\item For any of the morphisms $f_{sites}$ of ringed sites
...
Let $\mathcal{C} = (\textit{Sch}/S)_\tau$, or $\mathcal{C} = S_{etale}$,
or $\mathcal{C} = S_{Zar}$.
...
We may also thinkg of this as a $\tau$-covering
...
By
Cohomology on sites,
Lemma \ref{sites-cohomology-lemma-cech-spectral-sequence-application}
...
above). Hence by Homology, Lemma \ref{homology-lemma-morphisms-lift}
...
\text{QCoh}(S)
\longrightarrow 
\text{QCoh}((\textit{Sch}/S)_\tau, \mathcal{O})
$$
...
$$
\xymatrix{
\text{QCoh}(T) \ar[rr]_{f_*} \ar[d]_{\mathcal{F} \mapsto \mathcal{F}^a} & &
\text{QCoh}(S) \ar[d]^{\mathcal{G} \mapsto \mathcal{G}^a} \\
\text{QCoh}(T_\tau) \ar[rr]^{f_{small, *}} & &
\text{QCoh}(S_\tau)
}
$$
...
compatibility over the big sites
would mean that formation of $f_*\mathcal{F}$ commutes
with any base change, which does not hold in general.

\medskip\noindent
Let us prove (3) in case $\tau = etale$. First of all, the statement make
sense as $f$, and any base change of $f$, transforms quasi-coherent sheaves
...
y composing representatives (this is well defined).
\end{enumerate}
\end{definition}

\noindent
Before we continue we need one more definition. We will see later
that this is just one of many such definitions.

\begin{definition}
\label{definition-etale-morphism-germs}
We say that a morphism of germs $f : (X, x) \to (S, s)$
is {\it etale} if there exists a representative $f : U \to S$ of $f$
which is an etale morphism of schemes.
\end{definition}

\noindent
With this terminology we can make the following definition.
...
\section{Etale local properties of germs}
\label{section-properties-germs-etale-local}

\begin{definition}
\label{definition-local-at-point}
Let $\mathcal{P}$ be a property of germs of schemes.
We say that $\mathcal{P}$ is {\it etale local} if for any
etale morphism of germs $(U', u') \to (U, u)$
...
\section{Properties of morphisms which are local on the target}
...
\item $f$ is universally injective and etale.
\end{enumerate}
\end{lemma}
...
Conversely, assume $f$ is universally injective and etale.
...
\medskip\noindent
Assume that $f$ is universally injective, surjective and etale.
...
Since $X \to Y$ is universally injective, we see that $\Delta$ is bijective
on points, see
Morphisms, Lemma \ref{morphisms-lemma-universally-injective}.
...
\begin{proof}
We are going to use Lemma \ref{lemma-properties-morphisms-local-source}.
By
Morphisms, Lemma \ref{morphisms-lemma-locally-quasi-finite-characterize}
the property of being locally quasi-finite is local for Zariski on source
and target. By
Morphisms, Lemmas \ref{morphisms-lemma-composition-quasi-finite}
and
\ref{morphisms-lemma-etale-locally-quasi-finite} we see the precomposition
...
\section{Properties of morphisms local on source-and-target}
\label{section-properties-etale-local-source-target}

\noindent
It turns out that a property of morphisms $\mathcal{P}$ which is both etale
local on the source and etale local on the target is not what
corresponds to the intuitive notion of a property of morphisms which is
...
\item (locality) a morphism $f : X \to Y$ has $\mathcal{P}$ if and only if
for every $x \in X$ there exists an etale neighbourhood
$a : (U, u) \to (X, x)$, an etale morphism $b : V \to Y$, and a morphism
$h : U \to V$ such that $f \circ a = b \circ h$ and such that $h$ has
$\mathcal{P}$.
\end{enumerate}
\end{definition}

\noindent
It turns out the definition excludes the behavior seen in
...
$(X_i, x_i) \to (X, x)$ is an etale neighbourhood, and
...
Then $(X_i, x_i) \to (X, x)$ is an etale neighbourhood, $Y_i \to Y$ is
...
etale neighbourhood, $Y \to Z$ is an etale morphism, and $h = f$ is as
in part (3) of
Definition \ref{definition-local-source-target}.
...
\item[(b)] for every $x \in X$ there exists an etale neighbourhood
$a : (U, u) \to (X, x)$ of $x$, an etale morphism $b : V \to Y$, and
a morphism $h : U \to V$ such that $f \circ a = b \circ h$ such that
$h$ has $\mathcal{P}$,
\item[(c)]
...
of schemes such that for every $x \in X$ there exists an etale
neighbourhood $a_x : (U_x, u_x) \to (X, x)$, an etale morphism
$b_x : V_x \to Y$, and a morphism $h_x : U_x \to V_x$ such that
$f \circ a_x = b_x \circ h_x$ and $h_x$ has $\mathcal{P}$. The proof of
the lemma is complete once we show that $f$ has $\mathcal{P}$.
