...
\emph{descent},
such as constructing objects over a given space by first making over a somewhat 
larger space
which projects down to the given space, or verifying a property of a space or a 
morphism by
pulling back along a covering map. The utility of such techniques is of course 
...
the class of morphisms which are \emph{quasicompact} and \emph{faithfully flat} 
...
descent morphism for modules, Grothendieck showed that it is sufficient for $f$ 
to be faithfully flat.
However, this excludes many natural examples: for instance, 
any split ring homomorphism is an effective descent morphism.
One natural example of this even arises in the proof of faithfully flat 
descent: for $f: R \to S$
any ring homomorphism, $1_S \otimes f: S \to S \otimes_R S$ is split by the 
multiplication map whether or not
it is flat.

\medskip\noindent
One may then ask whether one there is a natural ring-theoretic condition 
implying effective descent which includes both the case of
a faithfully flat morphism and that of a split ring homomorphism.
It may surprise the reader (at least it surprised this author) to learn that a 
complete answer to
this question has been known since around 1970! Namely, it is not hard to check 
that a necessary condition
for $f: R \to S$ to be an effective descent morphism for modules is that $f$ 
must be \emph{universally injective}
in the category of $R$-modules, that is, for any $R$-module $M$, the map $1_M 
\otimes f: M \to M \otimes_R S$
must be injective. This then turns out to be a sufficient condition as well. 
For example, if $f$ is split
in the category of $R$-modules (but not necessarily in the category of rings), 
then $f$ is an effective descent morphism.
 
\medskip\noindent
The history of this result is 
a bit involved: it was originally asserted by Olivier \cite{olivier}, who 
called universally injective morphisms
\emph{pure}, but without a clear indication of proof.
One can extract the result from the work of Joyal and Tierney 
\cite{joyal-tierney}, but to the best of our knowledge,
the first free-standing proof to appear in the literature is that of 
Mesablishvili \cite{mesablishvili1}.
The first purpose of this note is to expose Mesablishvili's proof; this 
requires little modification of 
his original presentation aside from correcting typos, with the one exception 
that we make explicit the relationship
between the customary definition of a descent datum in algebraic geometry and 
the one used in \cite{mesablishvili1}.
The proof turns to be entirely category-theoretic,
and consequently can be put in the language of monads (and thus applied in 
other contexts); see \cite{janelidze-tholen}.

\medskip\noindent
The second purpose of this note is to collect some information about which 
...
\begin{definition}
\label{definition-equalizer}
For two morphisms $g_1, g_2: B \to C$, an \emph{equalizer} of $g_1$ and $g_2$ 
is a morphism
$f: A \to B$ which satisfies $g_1 \circ f = g_2 \circ f$ and is universal for 
this property.
...
typically not stable under
applying a covariant functor. Just as for monomorphisms and epimorphisms, one 
can get around this in some
cases by exhibiting splittings.

\begin{definition}
\label{definition-split-equalizer}
A \emph{split equalizer} is a diagram
as in (\ref{equation-equalizer}) for which there exist auxiliary morphisms $h: 
B \to A$, $i: C \to B$ such that
\begin{equation}
...
to be an equalizer: the map $e$ factors uniquely through $f$ by writing $e = f 
\circ (h \circ e)$. Consequently, applying a covariant functor
to a split equalizer gives a split equalizer; applying a contravariant functor 
gives a \emph{split coequalizer},
...
\begin{definition}
\label{definition-rings}
Let $\textit{Rings}$ denote the category of commutative unital rings.
For $R \in \textit{Rings}$, let $\text{Mod}_R$ denote the category of 
$R$-modules.
\end{definition}

\begin{remark}
\label{remark-reflects}
For $R, S \in \textit{Rings}$, any functor $F: \text{Mod}_R \to \text{Mod}_S$ 
which is exact and takes nonzero objects
to nonzero objects reflects injections and surjections, because exactness 
implies that
$F$ preserves kernels and cokernels. For example, if $f: R \to S$ is a 
...
A morphism $f: M \to N$ in $\text{Mod}_R$ is \emph{universally injective} if 
for all $P \in \text{Mod}_R$,
the morphism $f \otimes 1_P: M \otimes_R P \to N \otimes_R P$ is injective. A 
morphism $f: R \to S$
in $\textit{Rings}$ is \emph{universally injective} if it is universally 
injective as a morphism in $\text{Mod}_R$.
\end{definition}
...
any split injection in
$\textit{Rings}$ is universally injective.
\end{example}

\begin{example}
\label{example-cover-universally-injective}
For $R \in \textit{Rings}$ and $f_1,\dots,f_n \in R$ generating the unit ideal, 
the morphism
$R \to R_{f_1} \oplus \cdots \oplus R_{f_n}$ is universally injective.
Although this is immediate from
Lemma \ref{lemma-faithfully-flat-universally-injective},
it is instructive
to check it directly: we immediately reduce to the case where $R$ is local, in 
which case
...
Any faithfully flat morphism $f: R \to S$ in $\textit{Rings}$ is universally 
injective.
\end{lemma}
...
$P \in \text{Mod}_R$, it is sufficient to check this after applying
$S \otimes_R \bullet: \text{Mod}_R \to \text{Mod}_S$ (see 
Remark~\ref{remark-reflects}). The resulting morphism
$1_S \otimes f \otimes {1_P}: S \otimes_R  R \otimes_R P \to S \otimes_R S 
\otimes_R P$
is split by the multiplication map $\mu: S \otimes_R S \otimes_R P \to S 
\otimes_R R \otimes_R P$
...
in terms of a splitting, using the usual construction of an injective 
cogenerator in $\text{Mod}_R$.

\begin{definition}
\label{definition-C}
Let $\textit{Ab}$ denote the category of abelian groups. 
For $R \in \textit{Rings}$, define the contravariant 
functor $C: \text{Mod}_R \to \text{Mod}_R$ by setting $C(M) := 
\Hom_{\textit{Ab}}(M, \mathbf{Q}/\mathbf{Z})$, with the $R$-action
on $C(M)$ given by $rf(s) = f(rs)$.
\end{definition}

\begin{lemma}
\label{lemma-C-is-faithful}
For $R \in \textit{Rings}$, the functor $C: \text{Mod}_R \to \text{Mod}_R$ is 
exact and reflects injections and surjections
(to surjections and injections).
\end{lemma}

\begin{proof}
It suffices to check that for $R = \mathbf{Z}$, $C$ is exact and takes nonzero 
objects to nonzero objects.
Since $C$ is evidently left exact, it is enough to check that for $A_1 \to A_2$ 
an injection of abelian groups
with $A_2 \neq 0$, $C(A_2)$ is nonzero and surjects onto $C(A_1)$. By Zorn's 
lemma, it is enough to check
this when $A_2 = A_1 + b \mathbf{Z}$ for some $b \in A_2 \setminus A_1$.
Choose any $f \in C(A_1)$.
Let $I$ be the ideal $\{n \in \mathbf{Z}: nb \in A_1\}$, which by hypothesis is 
nontrivial.
If $I = 0$, we may extend $f$ to $A_2$ by mapping $b$ to any nonzero element of 
$\mathbf{Q}/\mathbf{Z}$.
Otherwise, let $n$ be the positive generator of $I$; since 
$\mathbf{Q}/\mathbf{Z}$ is divisible, we may choose
some nonzero $s \in \mathbf{Q}/\mathbf{Z}$ such that $ns = f(nb)$ and then set 
$f(b) = s$.
\end{proof}
...
product, in the form of the natural 
...
A morphism $f: M \to N$ in $\text{Mod}_R$ is universally injective if and only 
if $C(f): C(N) \to C(M)$ is a split
surjection.
\end{lemma}
...
so by Lemma~\ref{lemma-C-is-faithful},
...
\noindent
Throughout this subsection, fix a morphism $f: R \to S$ in $\textit{Rings}$.
One can use the language of cosimplicial algebras to frame some of the 
following definitions, but we
prefer not to do so here.

\begin{definition}
\label{definition-maps}
...
index indicates where to insert
factors of 1. 
\end{definition}

\begin{definition}
\label{definition-DD}
For $M \in \text{Mod}_S$, a \emph{descent datum} on $M$ relative to $f$ is 
an isomorphism $\theta: M \otimes_{S,\delta^1_0} S_2 \cong M 
\otimes_{S,\delta^1_1} S_2$ of $S_2$-modules
...
\noindent
For example, for $M_0 \in \text{Mod}_R$, an isomorphism $M \cong M_0 \otimes_R 
S$ gives rise to a descent datum
by identifying $M \otimes_{S,\delta^1_0} S_2$ and $M \otimes_{S,\delta^1_1} 
S_2$ naturally with $M_0 \otimes_R S_2$.
This construction defines a functor $f^*: \text{Mod}_R \to DD_{S/R}$ called 
\emph{base extension along $f$}.
We say that $f$ is a \emph{descent morphism for modules} if $f^*$ is fully 
faithful.
We say that $f$ is an
\emph{effective descent morphism for modules} if $f^*$ is an equivalence of 
categories.
...
\begin{proof}
In Lemma~\ref{lemma-equalizer-M},
take $(M, \theta) = f^*(S)$.
\end{proof}
...
Define the functor $f_*: DD_{S/R} \to \text{Mod}_R$ by taking $f_*(M, \theta)$ 
to be the $R$-submodule of $M$
...
\end{equation}
is an equalizer. 
Using Lemma~\ref{lemma-equalizer-M}
and the fact that the restriction functor $\text{Mod}_S \to \text{Mod}_R$ is 
right adjoint to the base extension
...
\ar@<1ex>^{C(\theta \circ (1_M \otimes \delta_0^1))}[r] \ar@<-1ex>_{C(1_M 
\otimes \delta_1^1)}[r] \ar[d] &
C(M) \ar[r]\ar[d] & C(f_*(M,\theta)) \ar@{-->}[d]\\
C(M \otimes_{S,\delta_{12}^1} S_3)
\ar@<1ex>^{C((\theta \otimes \delta_2^2) \circ (1_M \otimes \delta^2_0)}[r] 
\ar@<-1ex>_{C(1_{M \otimes S_2} \otimes \delta^2_1)}[r] \ar[d]
& C(M \otimes_{S, \delta_1^1} S_2 )
 \ar^{C(\theta \circ (1_M \otimes \delta_0^1))}[r]\ar^{C(1_M \otimes 
\delta_1^1)}[d] & C(M)  \ar[d] \ar@{=}[dl]
 \\
 C(M \otimes_{S, \delta_1^1} S_2)
\ar@<1ex>^{C(\theta \circ (1_M \otimes \delta_0^1))}[r] \ar@<-1ex>_{C(1_M 
\otimes \delta_1^1)}[r] &
...
\ar@<-1ex>_{C(1_{M \otimes S_2} \otimes \delta^2_1)}[r]
& C(M \otimes_{S, \delta_1^1} S_2 )
 \ar^{C(\theta \circ (1_M \otimes \delta_0^1))}[r]& C(f_*(M,\theta) \otimes_R 
S).
}
\]
By Lemma~\ref{lemma-C-is-faithful}, (\ref{equation-equalizer-f2}) must also be 
an equalizer.
\end{proof}
...
not universally injective,
we can find $M \in \text{Mod}_R$ such that the map $1_M \otimes f: M \to M 
\otimes_R S$ has nontrivial kernel $N$.
...
\noindent
Throughout this subsection, fix a universally injective morphism $f: R \to S$
in $\textit{Rings}$, an object $M \in \text{Mod}_R$,
and a morphism $R \to A$ in $\textit{Rings}$. We now investigate the question 
of which properties of $M$ or $A$
can be checked after base extension along $f$. We start with some results from 
\cite{mesablishvili2}.
...
\]
is exact. By (\ref{equation-adjunction}), this last sequence can be rewritten as
\[
...
If $M \otimes_R S$ has one of the following properties as an object in 
$\text{Mod}_S$:
\begin{enumerate}
\item[(a)]
finitely generated;
...
finite projective;
\end{enumerate}
then so does $M$ as an object in $\text{Mod}_R$ (and of course conversely).
\end{theorem}

\begin{proof}
To prove (a), choose a finite set $\{n_i\}$ of generators of $M \otimes_R S$ in 
$\text{Mod}_S$.
Write each $n_i$ as $\sum_j m_{ij} \otimes s_{ij}$ with $m_{ij} \in M$ and 
$s_{ij} \in S$.
Let $F$ be the finite free $R$-module generated by the $m_{ij}$ and let $F \to 
M$ be the canonical homomorphism.
Then $F \otimes_R S\to M \otimes_R S$ is surjective, so $\text{Coker}(F \to M) 
\otimes_R S$ is zero
and hence $\text{Coker}(F \to M)$ is zero. This proves (a), from which (b) is 
immediate.

\medskip\noindent
To prove (c), let $0 \to M' \to M'' \to M \to 0$ be a short exact sequence in 
$\text{Mod}_R$. Since $\bullet \otimes_R S$
is a right exact functor, $M'' \otimes_R S \to M \otimes_R S$ is surjective, so 
by Lemma~\ref{lemma-C-is-faithful},
$C(M \otimes_R S) \to C(M'' \otimes_R S)$ is injective.  By 
Lemma~\ref{lemma-flat-to-injective},
...
Since $C(M \otimes_R S) \to C(M)$ is also a split injection by 
...
\]
is exact, so $M$ is flat in $\text{Mod}_R$. This proves (c).
...
\begin{proof}
The proofs of (a) and (b) are the same as for
Theorem \ref{theorem-descend-module-properties}.
To prove (c), recall that $A$ is formally unramified over $R$ if and only if the
module of relative differentials $\Omega_{A/R}$ vanishes
\cite[Proposition~17.2.1]{EGA4}.
Since $\Omega_{(A \otimes_R S)/S} = \Omega_{A/R} \otimes_R S$,
the vanishing descends by Theorem~\ref{theorem-descent}. To prove (d), recall 
that by definition $A$ is unramified
over $R$ if $A$ is formally unramified and of finite presentation over $R$.
To prove (e), recall that by
\cite[Th\'eor\`eme~17.6.1]{EGA4}, $A$ is \'etale over $R$ if and only if
