...
\usepackage{amsmath, amssymb, fullpage, url}
\usepackage[all]{xy}
...
\DeclareMathOperator{\Desc}{\mathbf{Desc}}
\DeclareMathOperator{\Hom}{Hom}
\DeclareMathOperator{\Mod}{\mathbf{Mod}}
\DeclareMathOperator{\Ring}{\mathbf{Ring}}

\title{Descent for universally injective morphisms}
...
Numerous constructions in algebraic geometry are made using techniques of \emph{descent},
such as constructing objects over a given space by first making over a somewhat larger space
which projects down to the given space, or verifying a property of a space or a morphism by
pulling back along a covering map. The utility of such techniques is of course dependent on identification
of a wide class of \emph{effective descent morphisms}.
Early in the Grothendieckian development of modern algebraic geometry,
the class of morphisms which are \emph{quasicompact} and \emph{faithfully flat} was shown to be effective
for descending objects, morphisms, and many properties thereof. 

As usual, this statement comes down to a property of rings and modules.
For a homomorphism $f: R \to S$ to be an effective
descent morphism for modules, Grothendieck showed that it is sufficient for $f$ to be faithfully flat.
However, this excludes many natural examples: for instance, 
any split ring homomorphism is an effective descent morphism.
One natural example of this even arises in the proof of faithfully flat descent: for $f: R \to S$
any ring homomorphism, $1_S \otimes f: S \to S \otimes_R S$ is split by the multiplication map whether or not
it is flat.

One may then ask whether one there is a natural ring-theoretic condition implying effective descent which includes both the case of
a faithfully flat morphism and that of a split ring homomorphism.
It may surprise the reader (at least it surprised this author) to learn that a complete answer to
this question has been known since around 1970! Namely, it is not hard to check that a necessary condition
for $f: R \to S$ to be an effective descent morphism for modules is that $f$ must be \emph{universally injective}
in the category of $R$-modules, that is, for any $R$-module $M$, the map $1_M \otimes f: M \to M \otimes_R S$
must be injective. This then turns out to be a sufficient condition as well. For example, if $f$ is split
in the category of $R$-modules (but not necessarily in the category of rings), then $f$ is an effective descent morphism.
 
The history of this result is 
a bit involved: it was originally asserted by Olivier \cite{olivier}, who called universally injective morphisms
\emph{pure}, but without a clear indication of proof.
One can extract the result from the work of Joyal and Tierney \cite{joyal-tierney}, but to the best of our knowledge,
the first free-standing proof to appear in the literature is that of Mesablishvili \cite{mesablishvili1}.
The first purpose of this note is to expose Mesablishvili's proof; this requires little modification of 
his original presentation aside from correcting typos, with the one exception that we make explicit the relationship
between the customary definition of a descent datum in algebraic geometry and the one used in \cite{mesablishvili1}.
The proof turns to be entirely category-theoretic,
and consequently can be put in the language of monads (and thus applied in other contexts); see \cite{janelidze-tholen}.

The second purpose of this note is to collect some information about which properties of modules, algebras, and morphisms can be descended along universally injective ring homomorphisms. The cases of finite modules
and flat modules were treated by Mesablishvili \cite{mesablishvili2}.

\section{Category-theoretic preliminaries}

We start by recalling a few basic notions from category theory which will simplify the exposition.
In this section, fix an ambient category.
\begin{defn}
For two morphisms $g_1, g_2: B \to C$, an \emph{equalizer} of $g_1$ and $g_2$ is a morphism
$f: A \to B$ which satisfies $g_1 \circ f = g_2 \circ f$ and is universal for this property.
...
\end{equation}
is an equalizer. Reversing arrows gives the definition of a \emph{coequalizer}.
\end{defn}

Since it involves a universal property, the property of being an equalizer is typically not stable under
applying a covariant functor. Just as for monomorphisms and epimorphisms, one can get around this in some
cases by exhibiting splittings.
\begin{defn}
A \emph{split equalizer} is a diagram
as in \eqref{eq:equalizer} for which there exist auxiliary morphisms $h: B \to A$, $i: C \to B$ such that
\begin{equation} \label{eq:split equalizer conditions}
h \circ f = 1_A, \qquad f \circ h = i \circ g_1, \qquad i \circ g_2 = 1_B.
\end{equation}
The point is that the equalities among arrows force \eqref{eq:equalizer} to be an equalizer: the map $e$ factors uniquely through $f$ by writing $e = f \circ (h \circ e)$. Consequently, applying a covariant functor
to a split equalizer gives a split equalizer; applying a contravariant functor gives a \emph{split coequalizer},
whose definition is apparent.
\end{defn}

\section{Universally injective morphisms}

\begin{defn}
Let $\Ring$ denote the category of commutative unital rings.
For $R \in \Ring$, let $\Mod_R$ denote the category of $R$-modules.
\end{defn}

\begin{remark} \label{R:reflects}
For $R, S \in \Ring$, any functor $F: \Mod_R \to \Mod_S$ which is exact and takes nonzero objects
to nonzero objects reflects injections and surjections, because exactness implies that
$F$ preserves kernels and cokernels. For example, if $f: R \to S$ is a faithfully flat ring homomorphism,
then $\bullet \otimes_R S: \Mod_R \to \Mod_S$ has these properties.
\end{remark}

\begin{defn}
A morphism $f: M \to N$ in $\Mod_R$ is \emph{universally injective} if for all $P \in \Mod_R$,
the morphism $f \otimes 1_P: M \otimes_R P \to N \otimes_R P$ is injective. A morphism $f: R \to S$
in $\Ring$ is \emph{universally injective} if it is universally injective as a morphism in $\Mod_R$.
\end{defn}

\begin{example}
Any split injection in $\Mod_R$ is universally injective. In particular, any split injection in
$\Ring$ is universally injective.
\end{example}

\begin{example}
For $R \in \Ring$ and $f_1,\dots,f_n \in R$ generating the unit ideal, the morphism
$R \to R_{f_1} \oplus \cdots \oplus R_{f_n}$ is universally injective.
Although this is immediate from Lemma~\ref{L:faithfully flat universally injective}, it is instructive
to check it directly: we immediately reduce to the case where $R$ is local, in which case
some $f_i$ must be a unit and so the map $R \to R_{f_i}$ is an isomorphism.
\end{example}

\begin{lemma} \label{L:faithfully flat universally injective}
Any faithfully flat morphism $f: R \to S$ in $\Ring$ is universally injective.
\end{lemma}
\begin{proof}
(taken from \cite[Tag~05CK]{stacks-project})
To check that $f \otimes {1_P}$ is injective for some
$P \in \Mod_R$, it is sufficient to check this after applying
$S \otimes_R \bullet: \Mod_R \to \Mod_S$ (see Remark~\ref{R:reflects}). The resulting morphism
$1_S \otimes f \otimes {1_P}: S \otimes_R  R \otimes_R P \to S \otimes_R S \otimes_R P$
is split by the multiplication map $\mu: S \otimes_R S \otimes_R P \to S \otimes_R R \otimes_R P$
taking $s_1 \otimes s_2 \otimes p$ to $s_1s_2 \otimes 1 \otimes p$.
\end{proof}

The key observation from \cite{mesablishvili1} is that  universal injectivity can be usefully reformulated
in terms of a splitting, using the usual construction of an injective cogenerator in $\Mod_R$.

\begin{defn}
Let $\Ab$ denote the category of abelian groups. 
For $R \in \Ring$, define the contravariant 
functor $C: \Mod_R \to \Mod_R$ by setting $C(M) := \Hom_{\Ab}(M, \QQ/\ZZ)$, with the $R$-action
on $C(M)$ given by $rf(s) = f(rs)$.
\end{defn}

\begin{lemma} \label{L:C is faithful}
For $R \in \Ring$, the functor $C: \Mod_R \to \Mod_R$ is exact and reflects injections and surjections
(to surjections and injections).
\end{lemma}
\begin{proof}
It suffices to check that for $R = \ZZ$, $C$ is exact and takes nonzero objects to nonzero objects.
Since $C$ is evidently left exact, it is enough to check that for $A_1 \to A_2$ an injection of abelian groups
with $A_2 \neq 0$, $C(A_2)$ is nonzero and surjects onto $C(A_1)$. By Zorn's lemma, it is enough to check
this when $A_2 = A_1 + b \ZZ$ for some $b \in A_2 \setminus A_1$.
Choose any $f \in C(A_1)$.
Let $I$ be the ideal $\{n \in \ZZ: nb \in A_1\}$, which by hypothesis is nontrivial.
If $I = 0$, we may extend $f$ to $A_2$ by mapping $b$ to any nonzero element of $\QQ/\ZZ$.
Otherwise, let $n$ be the positive generator of $I$; since $\QQ/\ZZ$ is divisible, we may choose
some nonzero $s \in \QQ/\ZZ$ such that $ns = f(nb)$ and then set $f(b) = s$.
\end{proof}

\begin{remark} \label{R:adjunction}
We will use frequently the standard adjunction between $\Hom$ and tensor product, in the form of the natural 
isomorphism of contravariant functors
\begin{equation} \label{eq:adjunction}
C(\bullet_1 \otimes_R \bullet_2) \cong \Hom_R(\bullet_1, C(\bullet_2)): \Mod_R \times \Mod_R \to \Mod_R
\end{equation}
taking $f: M_1 \otimes_R M_2 \to \QQ/\ZZ$ to the map $m_1 \mapsto (m_2 \mapsto f(m_1 \otimes m_2))$.
...
\]
is a split coequalizer diagram in $\Mod_R$, then so is
\[
\xymatrix@C=9pc{
C(M \otimes_R Q) \ar@<1ex>[r] \ar@<-1ex>[r] & C(N \otimes_R Q) \ar[r] & C(P \otimes_R Q)
}
\]
for any $Q \in \Mod_R$.
\end{remark}

\begin{lemma} \label{L:split surjection}
A morphism $f: M \to N$ in $\Mod_R$ is universally injective if and only if $C(f): C(N) \to C(M)$ is a split
surjection.
\end{lemma}
\begin{proof}
By \eqref{eq:adjunction}, for any $P \in \Mod_R$ we have a commutative diagram
\[
\xymatrix@C=9pc{
...
so both rows in the above diagram are surjective for $P = C(M)$. We may thus lift
$1_{C(M)} \in \Hom_R(C(M), C(M))$ to some $g \in \Hom_R(C(N), C(M))$ splitting $C(f)$.
Conversely, if $C(f)$ is a split surjection, then 
both rows in the above diagram are surjective,
so by Lemma~\ref{L:C is faithful},
$1_{P} \otimes f$ is injective.
\end{proof}

\begin{remark} \label{R:functorial splitting}
Let $f: M \to N$ be a universally injective morphism in $\Mod_R$. By choosing a splitting
$g$ of $C(f)$, we may construct a functorial splitting of $C(1_P \otimes f)$ for each $P \in \Mod_R$.
Namely, by \eqref{eq:adjunction} this amounts to splitting $\Hom_R(P, C(f))$  functorially in $P$,
and this is achieved by the map $g \circ \bullet$.
\end{remark}

\section{Descent for modules and their morphisms}

Throughout this section, fix a morphism $f: R \to S$ in $\Ring$.
One can use the language of cosimplicial algebras to frame some of the following definitions, but we
prefer not to do so here.
...
\end{align*}
In other words, the upper index indicates the source ring, while the lower index indicates where to insert
factors of 1. 
\end{defn}

\begin{defn}
For $M \in \Mod_S$, a \emph{descent datum} on $M$ relative to $f$ is 
an isomorphism $\theta: M \otimes_{S,\delta^1_0} S_2 \cong M \otimes_{S,\delta^1_1} S_2$ of $S_2$-modules
satisfying the \emph{cocycle condition}
\begin{equation} \label{eq:cocycle condition}
(\theta \otimes \delta_2^2) \circ (\theta \otimes \delta_2^0) = (\theta \otimes \delta_2^1):
M \otimes_{S, \delta^1_{01}} S_3 \to M \otimes_{S,\delta^1_{12}} S_3.
\end{equation}
Let $\Desc_{S/R}$ be the category of $S$-modules equipped with descent data relative to $f$.

For example, for $M_0 \in \Mod_R$, an isomorphism $M \cong M_0 \otimes_R S$ gives rise to a descent datum
by identifying $M \otimes_{S,\delta^1_0} S_2$ and $M \otimes_{S,\delta^1_1} S_2$ naturally with $M_0 \otimes_R S_2$.
This construction defines a functor $f^*: \Mod_R \to \Desc_{S/R}$ called \emph{base extension along $f$}.
We say that $f$ is a \emph{descent morphism for modules} if $f^*$ is fully faithful.
We say that $f$ is an
\emph{effective descent morphism for modules} if $f^*$ is an equivalence of categories.
\end{defn}

Our goal is to show that for $f$ universally injective, we can use $\theta$ to locate $M_0$ within $M$.
This process makes crucial use of some equalizer diagrams.
\begin{lemma} \label{L:equalizer M}
For $(M,\theta) \in \Desc_{S/R}$, the diagram
\begin{equation} \label{eq:equalizer M}
\xymatrix@C=8pc{M \ar^{\theta \circ (1_M \otimes \delta_0^1)}[r] & M \otimes_{S, \delta_1^1} S_2 \ar@<1ex>^{(\theta \otimes \delta_2^2) \circ (1_M \otimes \delta^2_0)}[r] \ar@<-1ex>_{1_{M \otimes S_2} \otimes \delta^2_1}[r] & 
...
Of the compatibilities required in \eqref{eq:split equalizer conditions}, 
the first follows from tensoring the cocycle condition \eqref{eq:cocycle condition} with $\sigma_1^1$
and the others are immediate.
\end{proof}
\begin{cor} \label{C:equalizer M}
For $(M, \theta) \in \Desc_{S/R}$, the diagram
\begin{equation} \label{eq:coequalizer CM}
\xymatrix@C=8pc{
C(M \otimes_{S,\delta_{12}^1} S_3)
\ar@<1ex>^{C((\theta \otimes \delta_2^2) \circ (1_M \otimes \delta^2_0)}[r] \ar@<-1ex>_{C(1_{M \otimes S_2} \otimes \delta^2_1)}[r]
& C(M \otimes_{S, \delta_1^1} S_2 )
 \ar^{C(\theta \circ (1_M \otimes \delta_0^1))}[r]& C(M).
}
\end{equation}
(obtained by applying $C$ to \eqref{eq:equalizer M}) is a split coequalizer.
\end{cor}

\begin{cor} \label{C:equalizer S}
The diagram
\begin{equation} \label{eq:equalizer S}
\xymatrix@C=8pc{S_1 \ar^{\delta^1_1}[r] & S_2 \ar@<1ex>^{\delta^2_2}[r] \ar@<-1ex>_{\delta^2_1}[r] & 
S_3
}
\end{equation}
is a split equalizer.
\end{cor}
\begin{proof}
In Lemma~\ref{L:equalizer M},
take $(M, \theta) = f^*(S)$.
\end{proof}

This suggests a definition of a potential quasi-inverse functor  for $f^*$.
\begin{defn}
Define the functor $f_*: \Desc_{S/R} \to \Mod_R$ by taking $f_*(M, \theta)$ to be the $R$-submodule of $M$
for which the diagram
\begin{equation} \label{eq:equalizer f}
\xymatrix@C=8pc{f_*(M,\theta) \ar[r] & M \ar@<1ex>^{\theta \circ (1_M \otimes \delta_0^1)}[r] \ar@<-1ex>_{1_M \otimes \delta_1^1}[r] & 
M \otimes_{S, \delta_1^1} S_2 
}
\end{equation}
is an equalizer. 
Using Lemma~\ref{L:equalizer M}
and the fact that the restriction functor $\Mod_S \to \Mod_R$ is right adjoint to the base extension
functor $\bullet \otimes_R S: \Mod_R \to \Mod_S$,
we deduce that $f_*$ is right adjoint to $f^*$.
\end{defn}

We are ready for the key lemma. In the faithfully flat case this is a triviality (see Remark~\ref{R:descent lemma}),
but in the general case some argument is needed.
\begin{lemma} \label{L:descent lemma}
If $f$ is universally injective, then the diagram
\begin{equation} \label{eq:equalizer f2}
\xymatrix@C=8pc{
f_*(M,\theta) \otimes_R S
\ar^{\theta \circ (1_M \otimes \delta_0^1)}[r] & M \otimes_{S, \delta_1^1} S_2 \ar@<1ex>^{(\theta \otimes \delta_2^2) \circ (1_M \otimes \delta^2_0)}[r] \ar@<-1ex>_{1_{M \otimes S_2} \otimes \delta^2_1}[r] & 
M \otimes_{S,\delta_{12}^1} S_3
}
\end{equation}
obtained by tensoring \eqref{eq:equalizer f} over $R$ with $S$ is an equalizer.
\end{lemma}
\begin{proof}
By Lemma~\ref{L:split surjection} and Remark~\ref{R:functorial splitting},
the map $C(1_N \otimes f): C(N \otimes_R S) \to C(N)$ can be split functorially in $N$. This gives the upper vertical arrows in the commutative diagram
\[
\xymatrix@C=8pc{C(M \otimes_{S, \delta_1^1} S_2)
\ar@<1ex>^{C(\theta \circ (1_M \otimes \delta_0^1))}[r] \ar@<-1ex>_{C(1_M \otimes \delta_1^1)}[r] \ar[d] &
C(M) \ar[r]\ar[d] & C(f_*(M,\theta)) \ar@{-->}[d]\\
C(M \otimes_{S,\delta_{12}^1} S_3)
\ar@<1ex>^{C((\theta \otimes \delta_2^2) \circ (1_M \otimes \delta^2_0)}[r] \ar@<-1ex>_{C(1_{M \otimes S_2} \otimes \delta^2_1)}[r] \ar[d]
& C(M \otimes_{S, \delta_1^1} S_2 )
 \ar^{C(\theta \circ (1_M \otimes \delta_0^1))}[r]\ar^{C(1_M \otimes \delta_1^1)}[d] & C(M)  \ar[d] \ar@{=}[dl]
 \\
 C(M \otimes_{S, \delta_1^1} S_2)
\ar@<1ex>^{C(\theta \circ (1_M \otimes \delta_0^1))}[r] \ar@<-1ex>_{C(1_M \otimes \delta_1^1)}[r] &
C(M) \ar[r] & C(f_*(M,\theta))
}
\]
in which the compositions along the columns are identity morphisms.
The second row is the coequalizer diagram
\eqref{eq:coequalizer CM}; this produces the dashed arrow.
From the top right square, we obtain auxiliary morphisms $C(f_*(M,\theta)) \to C(M)$ 
and $C(M) \to C(M\otimes_{S,\delta_1^1} S_2)$ which imply that the first row is a split coequalizer diagram.
By Remark~\ref{R:adjunction}, we may tensor with $S$ inside $C$ to obtain the split coequalizer diagram
\[
\xymatrix@C=8pc{
C(M \otimes_{S,\delta_2^2 \circ \delta_1^1} S_3)
\ar@<1ex>^{C((\theta \otimes \delta_2^2) \circ (1_M \otimes \delta^2_0)}[r] \ar@<-1ex>_{C(1_{M \otimes S_2} \otimes \delta^2_1)}[r]
& C(M \otimes_{S, \delta_1^1} S_2 )
 \ar^{C(\theta \circ (1_M \otimes \delta_0^1))}[r]& C(f_*(M,\theta) \otimes_R S).
}
\]
By Lemma~\ref{L:C is faithful}, \eqref{eq:equalizer f2} must also be an equalizer.
\end{proof}

\begin{remark} \label{R:descent lemma}
If $f$ is a split injection in $\Mod_R$, one can simplify the argument by splitting $f$ directly,
without using $C$. Things are even simpler if $f$ is faithfully flat; in this case,
the conclusion of Lemma~\ref{L:descent lemma} 
...
we can find $M \in \Mod_R$ such that the map $1_M \otimes f: M \to M \otimes_R S$ has nontrivial kernel $N$.
The natural projection $M \to M/N$ is not an isomorphism, but its image in $\Desc_{S/R}$ is an isomorphism.
Hence $f^*$ is not fully faithful.

We finally check that (c) implies (b). By Lemma~\ref{L:descent lemma}, for $(M, \theta) \in \Desc_{S/R}$,
the natural map $f^* f_*(M,\theta) \to M$ is an isomorphism of $S$-modules. On the other hand, for $M_0 \in \Mod_R$,
we may tensor \eqref{eq:equalizer S} with $M_0$ over $R$ to obtain an equalizer sequence,
so $M_0 \to f_* f^* M$ is an isomorphism. Consequently, $f_*$ and $f^*$ are quasi-inverse functors, proving the claim.
\end{proof}

\section{Descent for properties of modules}

Throughout this section, fix a universally injective morphism $f: R \to S$ in $\Ring$,
an object $M \in \Mod_R$,
and a morphism $R \to A$ in $\Ring$. We now investigate the question of which properties of $M$ or $A$
can be checked after base extension along $f$. We start with some results from \cite{mesablishvili2}.

\begin{lemma} \label{L:flat to injective}
If $M \in \Mod_R$ is flat, then $C(M)$ is an injective $R$-module.
\end{lemma}
\begin{proof}
Let $0 \to N \to P \to Q \to 0$ be an exact sequence in $\Mod_R$. Since $M$ is flat,
\[
0 \to N \otimes_R M \to P \otimes_R M \to Q \otimes_R M \to 0
\]
is exact.
By Lemma~\ref{L:C is faithful}, 
\[
0 \to C(Q \otimes_R M) \to C(P \otimes_R M) \to C(N \otimes_R M) \to 0
\]
is exact. By \eqref{eq:adjunction}, this last sequence can be rewritten as
\[
0 \to \Hom_R(Q, C(M)) \to \Hom_R(P, C(M)) \to \Hom_R(N, C(M)) \to 0.
\]
Hence $C(M)$ is an injective object of $\Mod_R$.
\end{proof}

\begin{theorem} \label{T:descend module properties}
If $M \otimes_R S$ has one of the following properties as an object in $\Mod_S$:
\begin{enumerate}
\item[(a)]
finitely generated;
\item[(b)]
...
finite projective;
\end{enumerate}
then so does $M$ as an object in $\Mod_R$ (and of course conversely).
\end{theorem}
\begin{proof}
To prove (a), choose a finite set $\{n_i\}$ of generators of $M \otimes_R S$ in $\Mod_S$.
Write each $n_i$ as $\sum_j m_{ij} \otimes s_{ij}$ with $m_{ij} \in M$ and $s_{ij} \in S$.
Let $F$ be the finite free $R$-module generated by the $m_{ij}$ and let $F \to M$ be the canonical homomorphism.
Then $F \otimes_R S\to M \otimes_R S$ is surjective, so $\coker(F \to M) \otimes_R S$ is zero
and hence $\coker(F \to M)$ is zero. This proves (a), from which (b) is immediate.

To prove (c), let $0 \to M' \to M'' \to M \to 0$ be a short exact sequence in $\Mod_R$. Since $\bullet \otimes_R S$
is a right exact functor, $M'' \otimes_R S \to M \otimes_R S$ is surjective, so by Lemma~\ref{L:C is faithful},
$C(M \otimes_R S) \to C(M'' \otimes_R S)$ is injective.  By Lemma~\ref{L:flat to injective},
$C(M \otimes_R S)$ is an injective object of $\Mod_S$, so the injection $C(M \otimes_R S) \to C(M'' \otimes_R S)$
is split in $\Mod_S$ and hence also in $\Mod_R$.
Since $C(M \otimes_R S) \to C(M)$ is also a split injection by Lemma~\ref{L:split surjection}, it follows that 
$C(M) \to C(M'')$ is a split injection in $\Mod_R$. That is, the sequence
\[
0 \to C(M) \to C(M'') \to C(M') \to 0
\]
is split exact. 
For $N \in \Mod_R$, by \eqref{eq:adjunction} we see that 
\[
0 \to C(M \otimes_R N) \to C(M'' \otimes_R N) \to C(M' \otimes_R N) \to 0
\]
is split exact. By Lemma~\ref{L:C is faithful}, 
\[
0 \to M' \otimes_R N \to M'' \otimes_R N \to M \otimes_R N \to 0
\]
is exact, so $M$ is flat in $\Mod_R$. This proves (c).
To deduce (d) from (c), note that if $N \in \Mod_R$ and $M \otimes_R N$ is zero,
then $M \otimes_R S \otimes_S (N \otimes_R S) \cong (M \otimes_R N) \otimes_R S$ is zero,
so $N \otimes_R S$ is zero and hence $N$ is zero.

To deduce (e) at this point, it suffices to recall that $M$ is finitely generated and projective if and only if it is finitely presented and flat.
\end{proof}

There is an obvious variant for $R$-algebras.
\begin{theorem} \label{T:descend algebra properties}
...
\end{theorem}
\begin{proof}
The proofs of (a) and (b) are the same as for Theorem~\ref{T:descend module properties}.
To prove (c), recall that $A$ is formally unramified over $R$ if and only if the
module of relative differentials $\Omega_{A/R}$ vanishes \cite[Proposition~17.2.1]{ega4-4}.
Since $\Omega_{(A \otimes_R S)/S} = \Omega_{A/R} \otimes_R S$,
the vanishing descends by Theorem~\ref{T:descent}. To prove (d), recall that by definition $A$ is unramified
over $R$ if $A$ is formally unramified and of finite presentation over $R$.
To prove (e), recall that by \cite[Th\'eor\`eme~17.6.1]{ega4-4}, $A$ is \'etale over $R$ if and only if
$A$ is flat and unramified over $R$.
\end{proof}

\begin{remark}
It would make things easier to have a faithfully
flat ring homomorphism $g: R \to T$ for which $T \to S \otimes_R T$ has some extra structure.
For instance, if one could ensure that $T \to S \otimes_R T$ is split in $\Ring$,
then it would follow that every property of a module or algebra which is stable under base extension
and which descends along faithfully flat morphisms also descends along universally injective morphisms.
An obvious guess would be to find $g$ for which $T$ is not only faithfully flat but also injective in $\Mod_R$,
but even for $R = \ZZ$ no such homomorphism can exist.
\end{remark}
...
A.J. de Jong et al, Stacks project, \url{http://www.stacksproject.org}.

\bibitem{ega4-4}
A. Grothendieck, \'El\'ements de g\'eom\'etrie alg\'ebrique
(r\'edig\'es avec la collaboration de Jean Dieudonn\'e): IV.
\'Etude locale des sch\'emas et des morphismes de sch\'emas,
Quatri\`eme partie, \textit{Publ. Math. IH\'ES} \textbf{32} (1967),
5--361.

\bibitem{janelidze-tholen}
G. Janelidze and W. Tholen, Facets of descent III: Monadic descent for rings and algebras,
\textit{Appl. Categorical Structures} \textbf{12} (2004), 461--477.

\bibitem{joyal-tierney}
A. Joyal and M. Tierney, \textit{An Extension of the Galois Theory of Grothendieck}, \textit{Mem. Amer.
Math. Soc.} \textbf{309}, Providence, RI, 1984.

\bibitem{mesablishvili1}
B. Mesablishvili, Pure morphisms of commutative rings are effective descent morphisms for modules --- a new proof,
\textit{Theory Appl. Categories} \textbf{7} (2000), 38--42.

\bibitem{mesablishvili2}
B. Mesablishvili, On some properties of pure morphisms of commutative rings, 
\textit{Theory Appl. Categories} \textbf{10} (2002), 180--186.

\bibitem{olivier}
J.-P. Olivier, Descente par morphismes purs, \textit{C. R. Acad. Sci. Paris Ser. A-B} \textbf{271} (1970), A821--A823.
...
$$
on $(\Sch/S)_\tau$, and
\item $(\text{id}_{small, \acute{e}tale, Zar})^*\mathcal{F}$ is the
...
\begin{enumerate}
\item[(3)] $(\text{id}_{\tau, \acute{e}tale})^*\mathcal{G}$ is the
...
$$
The cocylce condition guarantees that $h_{S_i} \times F$ is
