...
\item $\mathcal{F} \mapsto \mathcal{F}^a$ is right exact
as a functor
$\textit{QCoh}(\mathcal{O}_S) \to
\textit{Mod}((\textit{Sch}/S)_\tau, \mathcal{O})$,
\item $\mathcal{F} \mapsto \mathcal{F}^a$ is exact as a functor
$\textit{QCoh}(\mathcal{O}_S) \to \textit{Mod}(S_\tau, \mathcal{O})$,
\item $\mathcal{F} \mapsto \mathcal{F}^a$ is {\bf not} exact
as a functor
$\textit{QCoh}(\mathcal{O}_S) \to
\textit{Mod}((\textit{Sch}/S)_\tau, \mathcal{O})$
in general,
\item compatible with tensor product,
...
\textit{Hom}_{\mathcal{O}}(\mathcal{F}^a, \mathcal{G}^a)$.
\end{enumerate}
\end{lemma}
...
Thus (2) holds. Note that (1) is a special case of (2).
This also shows that the functor is right exact (i.e., commutes with
finite colimits), hence (3). The functor $\textit{QCoh}(\mathcal{O}_S) \to
...
This proves (4). To see (5), suppose that $S = \text{Spec}(\mathbf{Z})$.
...
D^+(\textit{QCoh}(\mathcal{O}_S))
$$
where the first arrow is the localization of
$K^+(\textit{QCoh}(\mathcal{O}_S))$ at the collection of
universal quasi-isomorphisms (details omitted).
A map of complexes of $A$-modules
$K^\bullet \to L^\bullet$ is said to be a
...
\medskip\noindent
In this paragraph we briefly discuss the relationship between quasi-coherent
modules and all modules on an affine scheme $S$. (This should be elaborated
on, generalized, and moved somewhere else.) By the discussion in
Schemes, Section \ref{schemes-section-quasi-coherent}
the embedding
...
Derived, Section \ref{derived-section-triangulated-sub}.
...
\medskip\noindent
In this paragraph we briefly discuss the relationship between adequate
...
$D_{\textit{Adeq}}(\mathcal{O}) \subset D(\mathcal{O})$ the
subcategory of complexes whose cohomology sheaves are quasi-coherent, see
Derived, Section \ref{derived-section-triangulated-sub}.
...
\end{equation}
is an equivalence. Namely, the inclusion functor
$\textit{Adeq}(\mathcal{O}_S) \subset
\textit{Mod}((\textit{Sch}/S)_{fppf}, \mathcal{O})$
has a right adjoint
$Q : \textit{Mod}((\textit{Sch}/S)_{fppf}, \mathcal{O}) \to
\textit{Adeq}(\mathcal{O}_S)$. This is true because the category of
adequate sheaves on $S$ has all colimits and a set of generators (namely,
the collection of linearly adequate functors give a set of generators
by
More on Algebra, Lemma
\ref{more-algebra-lemma-adequate-surjection-from-linear}).
Then $Q$ is left exact. An inverse to
(\ref{equation-compare-bounded-adequate}) should be the right derived
functor $RQ$. To be continued...
