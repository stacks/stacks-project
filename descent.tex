...
Let $\mathcal{F}$ be a quasi-coherent sheaf on $T$. Then there are
...
the sheaves $R^if_*$ are quasi-coherent. 
The equality $R^if_{small, *}(\mathcal{F}^a) = (R^if_*\mathcal{F})^a$
means that for any \'etale morphism $g : U \to S$ we have
$g^*R^if_*\mathcal{F} = R^if'_*(g')^*\mathcal{F})$
where $g' : U \times_S T \to T$ and $f' : U \times_S T \to U$
are the projections. This is true by
Coherent, Lemma \ref{coherent-lemma-flat-base-change-cohomology}.
\end{proof}
...
the derived category $D(\textit{QCoh}(\mathcal{O}_S))$. Namely, it turns
...
Remark \ref{remark-change-topologies-ringed}.
This morphism is flat, i.e., the map
$\epsilon^{-1}\mathcal{O}_S \to \mathcal{O}_{small}$ is flat.
To see this use that the family of points of $S_{\acute{e}tale}$
given by geometric points $\overline{s} \to S$ is conservative
(see
\'Etale Cohomology, Theorem \ref{etale-cohomology-theorem-exactness-stalks})
and that the maps on local rings
$\mathcal{O}_{S, s} \to \mathcal{O}_{small, \overline{s}}$ are flat, see
\'Etale Cohomology, Lemma
\ref{etale-cohomology-lemma-describe-etale-local-ring}.
Hence $\epsilon^*$ is an exact functor and induces
