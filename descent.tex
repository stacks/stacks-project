...
in the fpqc topology. By contrast, we show that the properties
``projective'' and ``quasi-projective'' are not local on the
base even in the Zariski topology.
...
\begin{proof}
Following Hironaka \cite[Example B.3.4.1]{HartshorneAG},
...
Let $X$ be projective 3-space over the complex numbers ${\bf C}$.
Let $C$ and $D$ be a line and a smooth conic in a plane inside $X$ such
that $C$ and $D$ intersect transversely at two points $P$ and $Q$,
and nowhere else. On $X-Q$, first blow up the curve $C$, and then blow
up the strict transform of the curve $D$ (Divisors, Definition 
\ref{divisors-definition-strict-transform}). On $X-P$, first blow up
the curve $D$, and then blow up the strict transform of the curve
$C$. Over $X-P-Q$, the two varieties we have constructed are canonically
isomorphic, and so we can glue them over $X-P-Q$. The result
is a smooth proper 3-fold $Y$ over ${\bf C}$. The morphism
$f:Y\to X$ is proper and Zariski-locally projective (since
it is a blow-up over $X-P$ and over $X-Q$), by Divisors,
Lemma \ref{divisors-lemma-blowing-up-projective}. We will show that
$Y$ is not projective over ${\bf C}$. That will imply that
$f$ is not projective.

To do this, let $L$ be the inverse image in $Y$ of a complex point
of $C-P-Q$, and $M$ the inverse image of a complex point of $D-P-Q$.
Then $L$ and $M$ are isomorphic to the projective line
${\bf P}^1_{{\bf C}}$. The inverse
image of $P$ consists of two lines $L_0$ and $M_0$, and we have
rational equivalences of cycles $L\sim L_0+M_0$ and $M\sim M_0$
(using that $C$ and $D$ are isomorphic to ${\bf P}^1$).
...
on $Y$. If $Y$ were projective over ${\bf C}$, it would have
an ample line bundle $H$, which would have degree $>0$ on all curves
in $Y$. In particular $H$ would have positive degree on $L_0+M_0'$,
contradicting that the degree of a line bundle is well-defined
on 1-cycles modulo rational equivalence on a proper scheme
over a field (Divisors,
Lemma \ref{divisors-lemma-proper-pushforward-rational equivalence}
and Lemma \ref{divisors-lemma-factors}).
So $Y$ is not projective over ${\bf C}$.
\end{proof}

In different terminology, Hironaka's 3-fold $Y$ is a small
resolution of the blow-up $X'$ of $X$ along the reduced subscheme
$C\cup D$; here $X'$ has two node singularities. If we define $Z$ by blowing
up $X$ along $C$ and then along the proper transform
of $D$, then $Z$ is a smooth projective 3-fold, and the non-projective
3-fold $Y$ differs from $Z$ by a ``flop'' over $X-P$.
...
Spaces, Example \ref{algebraic-spaces-example-non-representable-descent}.
