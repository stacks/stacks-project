...
The associated morphism of big sites
...
Namely, given $Y'/Y$ we let
$$
f_{big}^\sharp :
\mathcal{O}(Y'/Y)
\longrightarrow
f_{big, *}\mathcal{O}(Y'/Y) =
\mathcal{O}(X \times_Y Y'/X)
$$
be the usual map
$\text{pr}_{Y'}^\sharp : \mathcal{O}(Y') \to \mathcal{O}(X \times_Y Y')$.
This works more generally for each of the morphisms of sites
$f_{sites}$ listed in
Topologies, Section \ref{topologies-section-change-topologies}.
In Remark \ref{remark-Zarsiki-site-space}
we have seen that
$\textit{Mod}(\mathcal{O}_S) = \textit{Mod}(S_{Zar}, \mathcal{O})$,
hence we can reinterpret the construction $\mathcal{F} \mapsto \mathcal{F}^a$
as the pullback under the morphism of ringed sites
$$
((\textit{Sch}/S)_\tau, \mathcal{O})
\longrightarrow 
(S_{Zar}, \mathcal{O}), \quad\text{or}\quad
(S_{etale}, \mathcal{O}) \to (S_{Zar}, \mathcal{O}).
$$
Finally, we see that for any of the morphisms of
ringed sites $f_{sites}$ corresponding to $f$ listed in 
Topologies, Section \ref{topologies-section-change-topologies}
we have
$$
(f^*\mathcal{F})^a = f_{sites}^*\mathcal{F}^a
$$
because pullbacks are compatible with compositions of morphisms of sites, see
Modules on Sites,
