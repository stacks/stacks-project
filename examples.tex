...
\title{An example of a non-algebraic Hom-stack}

\maketitle



Let $A_0/\C$ be a non-zero abelian variety. Let $S := \Spec(\C\llbracket t \rrbracket)$ be the displayed trait with $S_n := \Spec(\C[t]/(t^n)) \subset S$. Let $f:X \to S$ be a map satisfying  the following:
\begin{enumerate}
	\item $f$ is projective and flat, and its fibres are geometrically connected curves of arithmetic genus $\geq 1$.
	\item The fibre $X_0 := X \times_S S_0$ is a nodal curve with smooth irreducible components.
	\item $X$ is a regular scheme.
\end{enumerate}
Let $A = A_0 \times S$ be the constant abelian scheme over $S$ associated to $A$. Then we will show:

\begin{proposition}
	\label{prop:NonAlgHomStack}
	The stack $\calF := \underline{\Hom}_S(X,B(A))$ is not algebraic.
\end{proposition}

Recall that $B(A) \simeq B(A_0) \times S$. This allows us to write 
\[ \calF(T) := \underline{\Hom}_S(X,B(A))(T) := \Hom_T(X \times_S T, B(A) \times_S T) \simeq \Hom_S(X \times_S T, B(A)) \simeq \Hom(X \times_S T, B(A_0))\] 
for any $S$-scheme $T$, i.e., the groupoid $\calF(T)$ is the groupoid of $A_0$-torsors on $X \times_S T$.  

\begin{remark}
	Proposition \ref{prop:NonAlgHomStack} seems to contradict \cite[Theorem 1.1]{AokiHomStacks}. The problem encountered below is the non-effectivity of formal objects for $\underline{\Hom}_S(X,B(A))$. The same problem is mentioned in the Erratum \cite{AokiHomStacksErr} to \cite{AokiHomStacks}; unfortunately, the Erratum goes on the assert that $\underline{\Hom}_S(X,Y)$ is algebraic if $Y$ is separated, which also contradicts Proposition \ref{prop:NonAlgHomStack} as $Y = B(A)$ is certainly separated.
\end{remark}

To prove Proposition \ref{prop:NonAlgHomStack}, by \cite[Tag 07x8]{StacksProject}, it is enough to show the following:

\begin{lemma}
The canonical map $\calF(S) \to \lim \calF(S_n)$ is not essentially surjective.
\end{lemma}
\begin{proof}
	Unwinding definitions, it is enough to check that $H^1(X,A_0) \to \lim H^1(X_n, A_0)$ is not surjective.  As $X$ is regular and projective, by Raynaud \cite[Proposition XIII.2.6]{RaynaudAmpleLB}, each $A_0$-torsor over $X$ is projective and hence torsion. In particular, the group $H^1(X,A_0)$ is torsion. It is thus enough to show: (a) the group $H^1(X_0,A_0)$ is non-torsion, and (b) the maps $H^1(X_{n+1},A_0) \to H^1(X_n,A_0)$ are surjective for all $n$. For (a), one constructs a non-torsion $A_0$-torsor on the nodal curve $X_0$ by glueing trivial $A_0$-torsors on each component of $X_0$ using non-torsion points on $A_0$ as the isomorphisms over the nodes; details left to the reader. For (b), deformation theory shows that the obstruction to deforming an $A_0$-torsor on $Y_n \to X_n$ to an $A_0$-torsor $Y_{n+1} \to X_{n+1}$ lies in $H^2(X_0,\omega)$ for a suitable vector bundle $\omega$  on $X_0$; the latter vanishes as $X_0$ is a curve, proving the claim.
\end{proof}
