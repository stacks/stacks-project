...
\begin{enumerate}
\item There exists an affine scheme $S$ and an affine open $U \subset S$
such that $S \setminus U$ does not contain a quasi-compact dense open.
...
Cohomology of Schemes, Lemma \ref{coherent-lemma-affine-compare-bounded})
...
$$
This is an object of $D_{\textit{QCoh}}(\mathcal{X})$. Note that in the
derived category we have
$$
E = \prod\nolimits_{m \geq 0} \mathcal{O}_\mathcal{X}[m]
$$
because this is true on affine objects over $\mathcal{X}$ by
Injectives, Remark \ref{injectives-remark-direct-sum-product-derived}
(details omitted). Since cohomology commutes
