...
The example below was taken from an upublished note of
Bart de Smit and Hendrik Lenstra. It also is discussed in
\cite[Exercise III.2.12]{Bourbaki-CA} over a finite field.
It is further discussed in \cite[Example 1.8]{Yekutieli}
...
poins $x_1 \in U_1$, $x_2 \in U_2$ mapping to the closed point $t \in T$
...
$X = [\mathbf{A}^1_k/\mathbf{Z}]$ where $k$ is a field of caracteristic zero
...
\medskip\noindent
Suppose that $Y = \text{Spec}(A)$ is an affine scheme and that
...
of Quillen's fundamental spectral sequence \cite[Corollary 8.2.6]{Reinhard}
and the nonvanishing of
$\text{Tor}_2^A(B, B) = \text{Ker}(J \otimes_A J \to J)$ we conclude that
$H_2(L_{B/A})$ is nonzero.

\begin{lemma}
\label{lemma-formally-etale-nontrivial-contangent-complex}
...
induces an addtion $m : G \times G \to G$ which turns $G$ into a group
...
\noindent
Let $S = \text{Spec}(\mathbf{F}_p)$ and let $\mu_p$ denote the
...
$\mu_p$-torsor $\text{Spec}(\mathbf{F}_p(t)[x]/(x^p - t))$ over
$T = \text{Spec}(\mathbf{F}_p(T))$ is fppf locally trivial, but
...
$\mathcal{S}$ is a stack in groupoids over $(\Sch/S)_{\acute{e}tale}$.
...
$\mathcal{S} \to \text{Spec}(\mathbf{F}_p)$ satisfies the
...
hence $H^1_{\acute{e}tale}(T, \mu_p) = H^1_{\acute{e}tale}(T', \mu_p)$.
Moreover, $\mathcal{S}$ is a limit preserving stack. Hence
if $U \to \mathcal{S}$ is smooth, then $U \to \text{Spec}(\mathbf{F}_p)$
is limit preserving and satisfies  the
infinitesimal lifting criterion for formal smoothness.
This implies that $U$ is smooth over $\mathbf{F}_p$.
In particular $U$ is reduced, hence $H^1_{\acute{e}tale}(U, \mu_p) = 0$.
Thus $U \to \mathcal{S}$ factors as
$U \to \text{Spec}(\mathbf{F}_p) \to \mathcal{S}$
and the first arrow is smooth. By descent of smoothness, we see
that $U \to \mathcal{S}$ being smooth would imply
$\text{Spec}(\mathbf{F}_p) \to \mathcal{S}$ is smooth. However, this
is not the case as
$\text{Spec}(\mathbf{F}_p) \times_\mathcal{S} \text{Spec}(\mathbf{F}_p)$
is $\mu_p$ which is not smooth over $\text{Spec}(\mathbf{F}_p)$.
...
There exists a functor $F : \textit{Sch}^{opp} \to \textit{Sets}$
...
\noindent
In the following we fix a big \'etale site $\Sch_{\acute{e}tale}$
as constructed in
...
\medskip\noindent
Consider the functor $F : \Sch_{\acute{e}tale} \to \textit{Ab}$
...
$\Sch_{\acute{e}tale}$ with the following properties
\begin{enumerate}
...
$$
F(X) =
\frac{\{a : |X| \to A \mid a \text{ is a constructible function}\}}%
{\text{locally constant functions }|X| \to A}
$$
...
$F(f)(b) = b \circ f$. Thus $F$ is a presheaf on $\Sch_{\acute{e}tale}$.
...
\ref{stacks-cohomology-proposition-derived-direct-image-quasi-coherent}.
...
and the description of products in $D(\mathcal{O}_\mathcal{X})$
...
of locally quasi-coherent sheaves with the flat base change propery on
...
\noindent
We continue our discussion. Since a quasi-cohernet module is locally
...
\medskip\noindent
Let $Y$ be projective 3-space over the complex numbers ${\bf C}$.
...
is a smooth proper 3-fold $V_Y$ over ${\bf C}$. The morphism
...
$V_Y$ is not projective over ${\bf C}$. That will imply that
$f$ is not projective.
...
isomorphic to ${\bf P}^1$ over $(C\cup D)-\{P,Q\}$.
The inverse
...
on $E$ and hence on $V_Y$. If $V_Y$ were projective over ${\bf C}$,
it would have
...
So $V_Y$ is not projective over ${\bf C}$.
\end{proof}
...
\section{Descent data for schemes need not be effective,
even for a projective morphism}
...
\medskip\noindent
Consider the action of the group $G={\bf Z}/2=\{ 1,g\}$
on projective 3-space
${\bf P}^3$ over the complex numbers by
$$g[x,y,z,w]=[y,x,w,z].$$
The action is free outside the two disjoint lines
$L_1=\{ [x,x,z,z]\}$ and $L_2=\{ [x,-x,z,-z]\}$ in
${\bf P}^3$. Let $Y={\bf P}^3-(L_1\cup L_2)$. There is a
smooth quasi-projective scheme $S=Y/G$ over ${\bf C}$ such that
...
in ${\bf P}^3$ under the morphism
\begin{align*}
{\bf P}^3 & \to \text{Proj } {\bf C}[x,y,z,w]^G\\
   & = \text{Proj } {\bf C}[u_0,u_1,v_0,v_1,v_2]/(v_0v_1=v_2^2),
\end{align*}
...
xy=w^2, z=0\}$. These are smooth conic curves in ${\bf P}^3$, contained
...
except that $Y$ here denotes an open subset of ${\bf P}^3$
rather than all of ${\bf P}^3$.)
...
of dimension 3 over ${\bf C}$,
for example by Algebraic Spaces, Lemma \ref{algebraic-spaces-lemma-quotient}.
