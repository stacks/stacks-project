...
Models, Lemma \ref{models-lemma-properties-form}
...
$$
\mathcal{O}_{Y_0}(ny) \not \in \Im(\text{Pic}(Y) \to \text{Pic}(Y_0))
\text{ for all }n > 0 \leqno{(3)}
$$
...
$$
Y_0(k) \to \text{Pic}(Y_0),\quad y \mapsto \mathcal{O}_{Y_0}(y)
$$
is injective. Now if $k$ is an uncountable algebraically closed field,
then using the countability of $\text{Pic}(Y)$ and the remark
just made, we can find a $y \in Y_0(k)$ satisfying (3) and hence (2).
...
and since $\text{Ext}^i_{\mathcal{O}_X}(\Omega_{X/k}, \mathcal{O}_X) =
...
Then $\mathcal{O}_Y(D)$ is a nontrivial element of $\text{Pic}(Y)$
...
Finally, to get a contradiction, we show that $\text{Pic}(Y) = 0$.
Namely, the map $\text{Pic}(Y) \to \text{Pic}(X)$ is injective,
...
If $X$ is general enough, then $\text{Pic}(X) = \mathbf{Z}$
...
the map $c_1 : \text{Pic}(X) \to H^1(X, \Omega_{X/k})$
