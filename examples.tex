...
connected curves of arithmetic genus $\geq 1$,
\item The fibre $X_0 = X \times_S S_0$ is a nodal curve with smooth
irreducible components whose dual graph has a loop,
...
\medskip\noindent
Recall that $[S/A]$ is on the one hand the quotient stack and on
the other hand equal to the stack classifying $A$-torsors, see
...
is the groupoid of $A_0$-torsors on $X \times_S T$.
...
Assume $X \to B$ is a proper morphism and $Y \to B$ is separated
and locally of finite type.
Let $b : \Spec(k) \to B$ be a morphism where $k$ is a field.
If the image of $X_b \to Y_b$ is finite, then there is an open
subspace $B' \subset B$ containing $b$ such that $X_{B'} \to Y_{B'}$
factors through a closed subspace $Z \subset Y_{B'}$ finite over $B'$.
\end{lemma}

\begin{proof}
The scheme theoretic image $Z = f(X) \subset Y$ is a proper algebraic
space over $S$ (Morphisms of Spaces, Lemma
\ref{spaces-morphisms-lemma-image-proper-is-proper}).
By asssumption $Z_b$ is finite. Hence $Z \to B$ is finite in
an open neighbourhood of $b$ for example by
...
\begin{proof}
We may assume $k$ is algebraically closed (details omitted).
In the proof products are over $k$.
The automorphism group of $(\mathbf{P}^1_k)^n$ contains
$G = \text{GL}_{2, k}^n$. If $C \subset (\mathbf{P}^1_k)^n$
is mapped to a point, then we can apply
Lemma \ref{lemma-flat-proper-family-cannot-collapse-fibre}
to the morphism
$$
G \times C \to G \times Z,\quad (g, c) \mapsto (g, f(g \cdot c))
$$
over $G$. Hence $g(C)$ is mapped to a point for $g \in G$ in a Zariski open
$U \subset G$. Suppose $x = (x_1, \ldots, x_n)$, $y = (y_1, \ldots, y_n)$
...
(exercise). Hence if $x, y \in C(k)$, we conclude that $f$ maps the whole
fibre of $\pi_I$ containing $x, y$ to a single point.
However, the $U(k)$-orbit of $C$ meets a Zariski open set of
fibres of $\pi_I$. By continuity $f$ collapses all fibres
of $\pi_I$. Considering the Stein factorization the reader
finds that $f$ factors through $\pi_I$.
\end{proof}

\begin{lemma}
\label{lemma-no-nonconstant-morphism-from-P1-to-group-scheme}
...
$$
\mathbf{P}^1 \times \ldots \times \mathbf{P}^1 \longrightarrow G,
...
this morphism is finite onto its image.
Hence $\dim(G) \geq n$ for all $n$, which is impossible.
\end{proof}

\noindent
The following is a special case of a result of Raynaud, see
\cite[Proposition XIII.2.6]{RaynaudAmpleLB}.
...
Let $P \to W$ be a $G$-torsor which is trivial in the generic point.
...
Lemma \ref{lemma-no-nonconstant-morphism-from-P1-to-group-scheme}
...
\medskip\noindent
Ad (a). One constructs a non-torsion $A_0$-torsor on the nodal
...
which occurs in a loop (i.e., such that removing the corresponding
edge from the dual graph decreases the first betti number).
...
$A_0 \to A_0$ by a nontorsion point (there is such a nontorsion
point as $k$ is algebraically closed and not the algebraic closure
of a finite field). One can show that the glueing is an algebraic
space (in fact one can show it is a scheme) and that it is an
nontorsion $A_0$-torsor over $X_0$.
