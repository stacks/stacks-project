...
set-theoretic questions related to the diagram we just displayed.
\end{proof}
...
relation (for example a pre-equivalence relation, or coming from a
groupoid in algebraic spaces over $B$, or coming from the action of
a group algebraic space over $B$ on $U$).
A morphism $\phi : U \to X$ is an orbit space for $R$ if and only if
\begin{enumerate}
