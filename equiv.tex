...
Let $S$ be a scheme. Let $X \to S$ and $Y \to S$ be proper flat morphisms
of finite presentation. An object $K \in D_{perf}(\mathcal{O}_{X \times_S Y})$
...
to $Y$ over $S$. Then then the corresponding Fourier-Mukai functors
$\Phi_K : D_\QCoh(\mathcal{O}_X) \to D_\QCoh(\mathcal{O}_Y)$
...
\begin{proof}
Let $K' \in D_{perf}(\mathcal{O}_{X \times_S Y})$ be the object assumed to
