...
as in Proposition \ref{proposition-fully-faithful}.
...
extension $k'/k$ and hence $D_{perf}(X)$ is equivalent to the category
...
siblings $F, F' : D_{perf}(X) \to D_{perf}(Y)$ are isomorphic.
Namely, we are given an isomorphism $F(\mathcal{O}_X) \cong F'(\mathcal{O}_X)$
compatible the action of the $k$-algebra
$k' = \text{End}_{D_{perf}(X)}(\mathcal{O}_X)$ which extends canonically
to an isomorphism on any graded $k'$-vector space.
