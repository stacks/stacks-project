...
The proof is similar to the proof of Lemma \ref{lemma-preserves-Coh}
and we only sketch the arguments.

\medskip\noindent
Step 1. By Lemma \ref{lemma-noah-pre} the functor $F$ is fully faithful.

\medskip\noindent
Step 2. For a closed point $x \in X$ we denote $\mathcal{O}_x$
the skyscraper sheaf with value $\kappa(x)$ at $x$.
An object $K$ of $D_{perf}(\mathcal{O}_X)$ is isomorphic to
a locally free $\mathcal{O}_X$-module of rank $r$ placed in degree $0$
if and only if
$$
\dim \Ext^i_X(K, \mathcal{O}_x) = 0 \text{ unless }i = 0\text{ and }
\dim \Ext^i_X(K, \mathcal{O}_x) = r
$$
Since $F(\mathcal{O}_x) \cong \mathcal{O}_x$ and $F$ is
fully faithful we find
$$
\dim \Ext^i_X(K, \mathcal{O}_x) = \dim \Ext^i(F(K), \mathcal{O}_x)
$$
for all $i \in \mathbf{Z}$. Thus, if $\mathcal{E}$ is a locally free
$\mathcal{O}_X$-module of rank $r$, then $F(\mathcal{E})$ is
a locally free $\mathcal{O}_X$-module of rank $r$ placed in degree $0$.
In particular, we have $F(\mathcal{O}_X) = \mathcal{L}[0]$ for some
invertible $\mathcal{O}_X$-module $\mathcal{L}$.

\medskip\noindent
Step 3. Let $U \subset X$ be a dense affine open with complement
$D \subset X$. Then $D = \sum_{i = 1, \ldots, r} D_i$
is a sum of prime divisors each of
which is an effective Cartier divisor, see Divisors, Lemma
\ref{divisors-lemma-complement-open-affine-effective-cartier-divisor-bis}
and its proof. By Step 2 we have invertible modules $\mathcal{L}_n$
such that $F(\mathcal{O}_X(-nD)) = \mathcal{L}_n[0]$.
The functor $F$ transforms the maps
$$
\mathcal{O}_X(-(n + 1)D) \to \mathcal{O}_X(-nD) \to \ldots \to
\mathcal{O}_X(-D) \to \mathcal{O}_X
$$
into maps $\mathcal{L}_{n + 1} \to \mathcal{L}_n$ such that the induced
map
$$
\Hom_X(\mathcal{L}_{n + 1}, \mathcal{O}_x) \to
\Hom_X(\mathcal{L}_n, \mathcal{O}_x)
$$
is nonzero if $x \in U$ and zero if $x \in D$. We conclude that we may write
$$
\mathcal{L}_n = \mathcal{L}(\sum e_i(n) D_i)
$$
with $e_i(n + 1) < e_i(n) < \ldots < e_i(0) = 0$. It follows that
for $K \in D_{perf}(\mathcal{O}_X)$ we have
\begin{align*}
\Gamma(U, K)
& \cong
\colim_n \Hom(\mathcal{O}_X(-nD), K) \\
& \cong
\colim_n \Hom(\mathcal{L}_n, F(K)) \\
& \cong
\colim_n \Hom(\mathcal{O}_X(-nD), F(K) \otimes \mathcal{L}^{\otimes -1}) \\
& \cong
\Gamma(U, F(K) \otimes \mathcal{L}^{\otimes -1})
\end{align*}
The first and last isomorphisms follow from Cohomology of Schemes, Lemma
\ref{coherent-lemma-homs-over-open} (details omitted). The second isomorphism
holds because $F$ is fully faithful. The third isomorphism holds
by comparing the system of divisors $-nD$ for varying $n$ with the
system of divisors $\sum e_i(n) D_i$ for varying $n$.

\medskip\noindent
Step 4. The functor $F$ sends $\textit{Coh}(\mathcal{O}_X)$ to
$\textit{Coh}(\mathcal{O}_X)$. Namely, this is clear from the formula
in Step 3.
...
a coherent $\mathcal{O}_{X \times X}$-module $\mathcal{K}$
flat over $X$ via $\text{pr}_1$ and finite over $X$ via $\text{pr}_2$.
Since $F(\mathcal{O}_X) = \mathcal{L}[0]$ we see that
$$
\mathcal{L} \cong F(\mathcal{O}_X) \cong F'(\mathcal{O}_X) \cong
\text{pr}_{2, *}\mathcal{K}
$$
...
\medskip\noindent
Step 6. Let $x \in X$ be a closed point.
For $n \geq 1$ denote $\mathcal{O}_{x, n}$ the skyscaper
sheaf at $x$ with value $\mathcal{O}_{X, x}/\mathfrak m_x^n$.
Then $\text{End}_X(\mathcal{O}_{x, n}) = \mathcal{O}_{X, x}/\mathfrak m_x^n$.
Since $F$ and $F'$ are siblings and since $F$ is fully faithful
we see that
$$
\text{End}_X(\mathcal{O}_{x, n}) \to
\text{End}_X(F(\mathcal{O}_{x, n})) \cong
\text{End}_X(f^*\mathcal{O}_{x, n} \otimes \mathcal{L}) \cong
\text{End}_X(f^*\mathcal{O}_{x, n})
$$
are bijections. We conclude that
...
\medskip\noindent
Since $G$ is fully faithful, we see that $H = F_r \circ G$ is a sibling of a
functor
$H' : M \mapsto (f')^*M \otimes^\mathbf{L}_{\mathcal{O}_X} \mathcal{L}'$
...
with roles of $F$ and $G$ switched. It follows that
$H$ is an auto-equivalence by Lemma \ref{lemma-sibling-faithful}
and that $H$ induces an auto-equivalence of
...
$$
F(H(M)) = F(F_r(G(M)) \longrightarrow G(M) 
$$
are isomorphisms functorial in $M$! Thus
$$
F \cong G \circ H^{-1}
$$
is a sibling of the functor $G \circ (H')^{-1}$. This finishes the
proof because $G \circ (H')^{-1}$ is a Fourier-Mukai functor
as compositions of Fourier-Mukai functors are Fourier-Mukai
