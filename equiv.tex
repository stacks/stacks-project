...
whence has coherent cohomology sheaves. Hence in each case
...
\medskip\noindent
Cases (2) follows immediately from case (1).

\medskip\noindent
Case (3) also follows from case (1). Namely, choose affine opens
$U = \Spec(A)$ and $V = \Spec(B)$ of $X$ and $Y$ mapping into the
...
of finite dimension we see that each $M^i$ has finite tor dimension
as an $A$-module. Thus $M^\bullet$ has finite tor dimension as
a complex of $A$-modules. This implies $K$ has bounded tor dimension
as a complex of $\text{pr}_1^{-1}\mathcal{O}_X$-modules as desired.
...
of $A$-modules and we conclude as before. FIXME.
\end{proof}
...
of $D_{perf}(X \times Y)$ such that
\begin{enumerate}
\item $H^q(K_i)$ is nonzero only for $q \leq 0$,
\item there exists an integer $m$ such that for every coherent
...
K_n)
$$
has vanshing cohomology sheaves outside $[-m, m] \cup [-m - n, m - n]$
and for $n > 2m$ the transition maps induce isomorphisms on cohomology
sheaves in $[-m, m]$.
\end{enumerate}
Then $K_n$ has vanshing cohomology sheaves outside
$[-m, m] \cup [-m - n, m - n]$ and for $n > 2m$ the
transition maps induce isomorphisms on cohomology sheaves in $[-m, m]$.
Moreover, for $n$ large enough we find $K_n = K \oplus C_n$ in
$D_{perf}(X \times Y)$ where $K$ has cohomology only indegrees $[-m, m]$
and $C_n$ only in degrees $[-m - n, m - n]$ and the transition maps
define isomorphisms between various copies of $K$.
\end{lemma}

\begin{proof}
Let $Z$ be the support of an $\mathcal{F}$ as in (2). For an object
$M$ of $D_{\QCoh, Z \times Y}(\mathcal{O}_{X \times Y})$ we have
$H^i(R\text{pr}_{2, *}(M)) = R\text{pr}_{2, *}H^i(M)$ (details omitted).
...
$x \in X$. Choose a bounded complex $M_n^\bullet$ of finite free
$\mathcal{O}_{X \times Y, z}$-modules representing
$K_n \otimes \mathcal{O}_{X \times Y, z}$. Then we know that
$$
M^\bullet \otimes_{\mathcal{O}_{X, x}}^\mathbf{L}
\mathcal{O}_{X, x}/\mathfrak m_x^t =
(\ldots \to M_n^i/\mathfrak m_x^t M_n^i \to \ldots)
$$
has nonzero cohomology only in the intervals
$[-m, m] \cup [-m - n, m - n]$ and that the transition
maps define isomorphisms between on cohomology in the range $[-m, m]$.
We conclude by More on Algebra, Lemma \ref{}
for example that $M_n^\bullet$ only has nonzero cohomology
in degrees $[-m, m] \cup [-m - n, m - n]$
and the transition maps induce isomorphisms on cohomology
in the range $[-m, m]$.
Since the set of closed points of $X \times Y$ is dense
this finishes the proof. FIXME.
\end{proof}
...
$\mathcal{A}$-modules. After replacing $K$ by a translation we may
assume $A^i = 0$ for $i > 0$ and $i < n$. The objects
$$
M_i = (A^{-i} \to \ldots \to A^0)[-i]
$$
form a Postnikov system in $D^b(\mathcal{A})$ for the complex
$A^{-n} \to \ldots \to A^0$ in $D^b(\mathcal{A})$.
...
$5$-lemma Homology, Lemma \ref{}
and Derived Cateogories, Lemma \ref{}; details omitted.
Similarly, if the
maps
$$
...
$$
is bijective for all $X, Y \in \Ob(\mathcal{A})$ and all $q \geq 0$.
...
Since $F$ is a sibling, the sequence $0 \to Y \to E \to X \to 0$
...
Derived Categories, Section \ref{}, we find that for any
...
in $\mathcal{D}$. Hence $E \cong F(E) \cong F(Y)[q - 1] \oplus F(X)
\cong Y[q - 1] \oplus X$. In other words, $E$ is isomorphic to the
direct sum of its cohomology objects. This implies that the
initial distinguished triangle is split, i.e., $\xi = 0$.
See Derived Categories, Lemma \ref{}.
\end{proof}
...
$$
are {\it siblings}, or we say $F'$ is a {\it sibling} of $F$
if $F$ and $F'$ are siblings in the sense of
Definition \ref{definition-siblings}. The equality holds
by Derived Categories of Schemes, Lemma \ref{perfect-proposition-DCoh}.
If $X$ is regular then $D_{perf}(X) = D^b_{\textit{Coh}}(X)$ by
Derived Categories of Schemes, Lemma \ref{}
and we use the same terminology for $k$-linear exact functors
$F, F' : D_{perf}(X) \to D_{perf}(Y)$.
\end{definition}

\noindent
Please see discussion in Remark \ref{remark-difficult}.
...
condition of Lemma \ref{} and hence are siblings.
\end{proof}
...
$\mathcal{F}'$ coherent $\mathcal{O}_X$-module corresponding to
the Matlis dual module. Then there is an isomorphism
$$
\Hom_X(\mathcal{F}, M) =
H^0(X, M \otimes_{\mathcal{O}_X}^\mathbf{L} \mathcal{F}'[-d])
$$
functorial in $M$ in $D_{perf}(X)$.
\end{lemma}

\begin{proof}
Denote $\omega^\bullet$ the dualizing complex of $X$ over $k$, see
Duality for Schemes, Section \ref{}.
Denote $D_X(K) = R\SheafHom_{\mathcal{O}_X}(K, \omega^\bullet)$.
Then $D_X$ is a anti-equivalence of $D_{perf}(X)$ inducing
an anti-equivalence on the category of coherent modules with
support of dimension $0$. Then we see that
$$
\Hom(\mathcal{F}, M) =
\Hom_X(D_X(M), D_X(\mathcal{F})) =
\Hom_X(D_X(M), \mathcal{F}')
$$
functorially in $M$. On the other hand, we can choose an open
neighbourhood $U$ of $x$ such that $\omega^\bullet|_U \cong \mathcal{O}_X[d]$
since $X$ is Gorenstein. Thus we have
$$
\Hom_X(D_X(M), \mathcal{F}') = H^0(X,
M \otimes_{\mathcal{O}_X}^\mathbf{L} \mathcal{F}'[-d])
$$
...
the skyscraper sheaf at $x$ with value $\kappa(x)$. For $K$ in
$D_{perf}(\mathcal{O}_X)$ if
$$
\Ext^i_X(\mathcal{O}_x, K) = 0
$$
then there exists an open neighbourhood $U$ of $x$ such that
$H^{i - d}(K)|_U = 0$ where $d = \dim(\mathcal{O}_{X, x})$.
In particular if $\Hom_X(\mathcal{O}_x, K[i]) = 0$ for all
...
\begin{proof}
By Lemma \ref{} we conclude that for every
coherent $\mathcal{O}_X$-module $\mathcal{F}$ whose support has
dimension $0$ there are isomorphisms
$$
...
\medskip\noindent
Let $x \in X$ be a closed point and denote $\mathcal{O}_x$ the
skyscraper sheaf with value $\kappa(x)$ at $x$. We find
$$
\dim_{\kappa(x)} 
...
\right.
$$
is equal to a free module of rank $r$ placed in degree $0$.
...
By More on Algebra, Lemma \ref{}
there exists an $n \gg 0$ such that
$$
...
\mathcal{O}_{X, x}/\mathfrak m_x^n)
$$
is nonzero. Next, we take $m \gg n$ and we consider the short
exact sequence
$$
...
the right vertical arrow is nonzero for all $m \gg n$.
On the other hand, we know this arrow is isomorphic to the arrow
$$
...
\mathcal{O}_{X, x}/\mathfrak m_x^n)
$$
Since we know the system of modules on the left (for varying $m$ and
fixed $n$) has vanishing limit (for example by More on Algebra, Lemma \ref{})
...
we have $F$ is a sibling to the Fourier-Mukai functor $F'$ given by
a coherent $\mathcal{O}_{X \times X}$-module $\mathcal{K}$
(finite over $X$ via $\text{pr}_2$ and flat over $X$ via $\text{pr}_1$).
Our arguments above show that $\mathcal{K}$ is set theoretically
...
Clearly this implies that $\mathcal{K} = \Delta_*\mathcal{L}$.
It follows that $F'(M) = M \otimes \mathcal{L}$
and the proof is complete.
\end{proof}
...
\begin{proof}
Recall that $G$ and $F$ have both adjoints, see Lemma \ref{}. In particular
the essential image $\mathcal{D}' \subset D_{perf}(Y)$ of $F$
satisfies the  equivalent conditions of
Derived Categories, Lemma \ref{derived-lemma-right-adjoint}.
We claim that $G$ factors through $\mathcal{D}'$.
To do this, consider an object $N$ of $D_{perf}(Y)$ such that
$\Hom_Y(K', N) = 0$ for all $K'$ in $\mathcal{D}'$. Then
Derived Categories, Lemma \ref{derived-lemma-right-adjoint}
tells us it suffices to show that $\Hom_Y(G(M), N)$ is always
zero (for all $M$ and $N$ as above). Now we have
$$
\Hom_Y(G(M), N) = \Hom_X(M, G_r(N))
$$
where $G_r$ is the right adjoint to $G$. Since
$G(\mathcal{F}) \cong F(\mathcal{F})$ for $\mathcal{F}$ as in (1)
we see that $\Hom_X(\mathcal{F}, G_r(N)) = 0$. Of course, the
same vanishing holds for $\Hom_X(\mathcal{F}, G_r(N)[i])$
for any $i \in \mathbf{Z}$. Thus $G_r(N) = 0$ by Lemma \ref{}
and the claim holds.
...
$S_X : D_{perf}(X) \to D_{perf}(X)$ a Serre functor, see Lemma \ref{}.
We have
\begin{align*}
...
\end{align*}
functorially in $M$. The second isomorphism comes from the result
proved above about $G_r \circ F$.
We conclude Lemma \ref{} applies to
the functor $S_X \circ G_l \circ G \circ S_X^{-1}$.
In particular, we see that $G_l \circ G$ is an equivalence
as a sibling of an equivalence (Lemma \ref{}). By
Categories, Lemma \ref{} we see that $G$ is fully faithful.

\medskip\noindent
Since $G$ is fully faithful, we see that $F_r \circ G$
is a sibling of a functor of to form $M \mapsto M \otimes \mathcal{L}'$
by the same argument as above with roles of $F$ and $G$ switched.
...
$M \mapsto M \otimes (\mathcal{L}')^{-1}$. Thus $F$
is a sibling of the Fourier-Mukai functor
$M \mapsto G(M  \otimes (\mathcal{L}')^{-1})$.
\end{proof}
...
are Fourier-Mukai functors, following \cite{Orlov-K3} and \cite{Ballard}.
...
As in Derived Categories, Example \ref{derived-example-key-postnikov}
...
form an infinite Postnikov system in $D_{perf}(X \times X)$
for the complex with terms $\mathcal{E}_i \boxtimes \mathcal{G}_i$. We have
$$
H^q(M_n[n]) = \left\{
\begin{matrix}
\mathcal{O}_\Delta & \text{if} & q = 0 \\
\Ker(\mathcal{E}_n \boxtimes \mathcal{G}_n \to
\mathcal{E}_{n - 1} \boxtimes \mathcal{G}_{n - 1}) & \text{if} & q = -n \\
0 & \text{if} & q \not = 0, -n
\end{matrix}
\right.
$$
...
$$
M_n[n] = \mathcal{O}_\Delta \oplus
\Ker(\mathcal{E}_n \boxtimes \mathcal{G}_n \to
\mathcal{E}_{n - 1} \boxtimes \mathcal{G}_{n - 1})[n]
$$
in $D_{perf}(X \times X)$. To see this use the vanishing of $\Ext$
in Derived Categories of Schemes, Lemma
\ref{perfect-lemma-ext-from-perfect-into-bounded-QCoh} and
Derived Categories, Lemma \ref{derived-lemma-split}.
...
fully faithful and the first by Derived Categories of Schemes, Lemma \ref{}.
We find these $\Ext^q$ are zero for $q < 0$.
Hence by
Derived Categories, Lemma \ref{derived-lemma-existence-postnikov-system}
we can build an infinite Postnikov system $K_0, K_1, K_2, \ldots$
in $D_{perf}(X \times Y)$ for the complex
...
$$
D_{perf}(X \times X) \longrightarrow D_{perf}(X),\quad
N \longmapsto R\text{pr}_{2, *}(\text{pr}_1^*\mathcal{F}
\otimes^\mathbf{L}_{\mathcal{O}_{X \times X}} N)
$$
to the original resolution of the diagonal chosen above, then we obtain
that the objects
$R\text{pr}_{2, *}(\text{pr}_1^*\mathcal{F}
\otimes^\mathbf{L}_{\mathcal{O}_{X \times X}} M_n)$
form a Postnikov system for the complex in $D_{perf}(X)$ with terms
$\Gamma(X, \mathcal{F} \otimes \mathcal{E}_i) \otimes_k \mathcal{G}_i$.
Applying the exact functor $F$ we conclude that the objects
$$
F(R\text{pr}_{2, *}(\text{pr}_1^*\mathcal{F}
\otimes^\mathbf{L}_{\mathcal{O}_{X \times X}} M_n))
$$
form a second infinite Postnikov system for the complex with terms
$$
F(\Gamma(X, \mathcal{F} \otimes \mathcal{E}_i) \otimes_k \mathcal{G}_i)
$$
By uniqueness of Postnikov systems
(Derived Categories, Lemma \ref{derived-lemma-existence-postnikov-system})
we find a compatible system of isomorphisms
$$
F(R\text{pr}_{2, *}(\text{pr}_1^*\mathcal{F}
...
$$
in $D_{perf}(Y)$. For $n$ sufficiently large we obtain a direct sum
decomposition
$$
F(R\text{pr}_{2, *}(\text{pr}_1^*\mathcal{F}
\otimes^\mathbf{L}_{\mathcal{O}_{X \times X}} M_n[n]))
=
F(\mathcal{F}) \oplus
F(\Ker(\mathcal{E}_n \boxtimes \mathcal{G}_n \to
\mathcal{E}_{n - 1} \boxtimes \mathcal{G}_{n - 1})[n])
$$
...
such that the second summand in this decomposition has nonzero cohomology
sheaves only in the interval $[-m - n, m - n]$.
We conclude the system $R\text{pr}_{2, *}(\text{pr}_1^*\mathcal{F}
\otimes^\mathbf{L}_{\mathcal{O}_{X \times Y}} K_n[n])$ of
$D_{perf}(Y)$ is essentially constant with value $F(\mathcal{F})$
and moreover $F(\mathcal{F})$ only has nonzero cohomology
sheaves only in the interval $[-m, m]$.

\medskip\noindent
Applying Lemma \ref{lemma-bounded-fibres} we conclude we can write
$$
K_n[n] = K \oplus C_n
$$
for $n \gg 0$. Denote $G$ the Fourier-Mukai functor corresponding to $K$.
We conclude that
$$
F(\mathcal{F}) \cong G(\mathcal{F})
$$
for every coherent $\mathcal{F}$ on $X$ whose support has dimension $0$.
...
Let $k$ be a field. Let $X$ and $Y$ be smooth proper schemes over $k$.
If there exists a $k$-linear equivalence
$F : D_{perf}(\mathcal{O}_X) \to D_{perf}(\mathcal{O}_Y)$
then there exists a Fourier-Mukai functor
$F' : D_{perf}(\mathcal{O}_X) \to D_{perf}(\mathcal{O}_Y)$
...
is an equivalence by Lemma \ref{lemma-close-equivalence}.
\end{proof}
