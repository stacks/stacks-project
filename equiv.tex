...
and $g$ mapping to zero in $G(X')$. Choosing a distinguished
triangle $X'' \to X \to X'$ we find that $X'' \in \Ob(\mathcal{D}_c)$
and $\xi'$ comes from $g'' \in G'(X'')$.
Finally, by Axiom TR3 the maps $X \to Y_2$ and $X' \to Y_3$ fit into
...
It remains to show that $K$ is in $D^b_{\textit{Coh}}(X)$.
...
\section{The regular proper case}
...
Let $\sigma : E \otimes E \to E \otimes E$ be the commutativity
constraint on the symmetric monoidal category $D(\mathcal{O}_X)$
(Cohomology of Sheaves, Lemma
...
constructed in Cohomology of Sheaves, Example
\ref{cohomology-example-dual-derived}.
...
Then the assertion of Theorem \ref{theorem-bondal-van-den-bergh}
is true.
\end{lemma}

\begin{proof}
As in the proof of Theorem \ref{theorem-bondal-van-den-bergh}
we reduce to showing the following: given an object
$K$ in $D_\QCoh(\mathcal{O}_X)$ such that
$\sum_{i \in \mathbf{Z}} \dim_k \Ext^i_X(E, K) < \infty$
for all $E$ in $D(\mathcal{O}_X)$ perfect, then $K$ is in
$D^b_{\textit{Coh}}(X)$.

\medskip\noindent
By Chow's lemma we can choose a birational proper morphism
$f : X' \to X$ with $X'$ projective, see
...
by our assumption on $K$. By the proof of
Theorem \ref{theorem-bondal-van-den-bergh}
