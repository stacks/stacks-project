...
the condition ``families of morphisms with fixed target which are
jointly universally submersive'' for non-Noetherian schemes.
...
A covering of $(\textit{Aff}/S)_h$ is any finite covering
$\{U_i \to U\}$ of $(\Sch/S)_h$ with $U_i$ and $U$ affine.
\end{enumerate}
\end{definition}

\noindent
The coverings in $(\textit{Aff}/S)_h$ are those finite families
$\{U_i \to U\}$ of morphisms of finite presentation between affine objects
of $(\Sch/S)_{ph}$ satisfying either of the following equivalent
conditions: (1) $\{U_i \to U\}$ can be refined by a standard ph covering
or (2) $\{U_i \to U\}$ is a V covering.
...
it suffices to show that the collection of finite h coverings
$\{U_i \to U\}$ with $U$, $U_i$ affine satisfies properties
(1), (2) and (3) of
Sites, Definition \ref{sites-definition-site}.
This is clear since for example, given a finite h covering
$\{T_i \to T\}_{i\in I}$ with $T_i, T$ affine, and for each
$i$ a finite h covering $\{T_{ij} \to T_i\}_{j\in J_i}$ with $T_{ij}$
affine , then $\{T_{ij} \to T\}_{i \in I, j\in J_i}$ is a h covering
...
$T/S$, $T$ affine, can be refined by a standard ph covering
$\{T_i \to T\}$ with $T_i \to T$ of finite presentation
(Lemma \ref{lemma-approximate-h-cover}; you can avoid using this
lemma of course). Hence (1) holds. We see $u$ is continuous simply
because a finite h covering of an affine by affines is a h covering.
