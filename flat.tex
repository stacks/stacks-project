...
\end{enumerate}
Then for any prime $\mathfrak q \subset S$ lying over $\mathfrak p \subset R$
which is an associated prime of
...
Let $\mathcal{F}$ be a finite type quasi-coherent $\mathcal{O}_X$-module.
\begin{enumerate}
\item Let $s \in S$. We sat $\mathcal{F}$ is {\it pure along $X_s$}
if there is no impurity $(g : T \to S, t' \leadsto t, \xi)$
with $(T, t) \to (S, s)$ an elementary \'etale neighbourhood.
...
\noindent
One observation that should be made here is that we intentionally restrict
ourselves here to morphisms which are of finite type and not just morphisms
which are locally of finite type. In this setting
