...
\section{Blow up squares and h topology}
...
\medskip\noindent
Second proof. The blow up of $X'$ in $X$ is the empty scheme.
...
Hence we get a blow up square of the form
$$
\xymatrix{
...
\item given a blow up square (\ref{equation-blow-up-square})
...
is the empty scheme (compare with the proof of Lemma \ref{lemma-thickening-h}).
...
is bijective for all $i$. Using the Zariski sheaf property we see that
$\mathcal{F}(T') \to \mathcal{F}(T)$ is injective. Repeating the
argument we find that it is bijective. Minor details omitted.

\medskip\noindent
Interlude B: consider a blow up square (\ref{equation-blow-up-square})
in the category $(\Sch/S)_h$. Then we claim  the diagram
$$
\xymatrix{
\mathcal{F}(E) & \mathcal{F}(X') \ar[l] \\
\mathcal{F}(Z) \ar[u] & \mathcal{F}(X) \ar[u] \ar[l]
...
By the interlude above we have $\mathcal{F}(Y_0) = \mathcal{F}(Y)$,
...
By Theorem \ref{theorem-flatten-module} there is a $V$-admissible
blowup $Y' \to Y$ such that the strict transform of $f' : X' \to Y'$
of $f$ is flat and of finite presentation over $Y'$. Also
$f'$ is proper and surjective as it inherits properness from $f$
and surjectivity as we may assume $V \subset Y'$ is dense
(see Remark \ref{remark-successive-blowups}). Denote $Z \subset Y$
the center of the blowing up and $E \subset Y'$ the exceptional
divisor. Recall that $X'$ is the blowing up of $X$ in $f^{-1}(Z)$,
see Divisors, Lemma \ref{divisors-lemma-strict-transform}.
In particular, the exceptional fibre of $X' \to X$ is $(f')^{-1}(E)$.
Thus we know that
$$
\vcenter{
\xymatrix{
...
\vcenter{
\xymatrix{
\mathcal{F}((f')^{-1}(E)) & \mathcal{F}(X') \ar[l] \\
\mathcal{F}(f^{-1}(Z)) \ar[u] & \mathcal{F}(X) \ar[u] \ar[l]
}
}
$$
...
Z \cap Y_t = \emptyset
$$
as above for the restriction $f^{-1}(Z) = Z \times_Y X \to Z$ of $f$ to $Z$.
Thus by induction hypothesis we find that
$\mathcal{F}(Z) \to \mathcal{F}(f^{-1}(Z))$
is an injective map of sets whose image is the equalizer
of the two maps $\mathcal{F}(f^{-1}(Z)) \to
\mathcal{F}(f^{-1}(Z) \times_Z f^{-1}(Z))$.

\medskip\noindent
Let $s \in \mathcal{F}(X)$ be in the equalizer of the two maps
$\mathcal{F}(X) \to \mathcal{F}(X \times_Y X)$.
By the above we see that the restriction $s|_{f^{-1}(Z)}$
...
corresponding to the arrows in the commutative diagram
$$
\xymatrix{
E \ar[ddd] \ar[rd] & & & (f')^{-1}(E) \ar[lll] \ar[ddd] \ar[ld] \\
& Y' \ar[d] & X' \ar[l] \ar[d] \\
& Y & X \ar[l] \\
Z \ar[ru] & & & f^{-1}(Z) \ar[lll] \ar[lu]
}
$$
the reader finds that $t$ and $t'$ restrict to the same element of
$\mathcal{F}(E)$ because they restrict to the same element of
$\mathcal{F}((f')^{-1}(E))$ and we have (2). Thus by the first of the two
