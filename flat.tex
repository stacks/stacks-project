...
Then any map $\varphi : \mathcal{F} \to \mathcal{F}'$ of
quasi-coherent $\mathcal{O}_X$-modules such that the map on stalks
$\varphi_x$ is injective for all $x \in X_s$ is injective.
\end{lemma}
...
is a generalization of a point $x$ of $X_s$. Hence the injectivity
of $\varphi_x$ implies the injectivity of $\varphi_{x'}$, whence
$\mathcal{K}_{x'} = 0$.
\end{proof}
...
\section{Flattening: functors and sheaves}
...
\forall \mathfrak q \in V(\mathfrak m_BB') \subset \text{Spec}(B') :
M'_{\mathfrak q}\text{ is flat over }A'.
\end{equation}
Note the similarity with
More on Algebra, Equation (\ref{more-algebra-equation-flat-at-primes-over}).
In particular, if $A' \to A''$ is a morphism of $\mathcal{C}$, then
if (\ref{equation-flat-at-primes-over}) holds for $A'$, then it holds
for $A''$, see
More on Algebra,
Lemma \ref{more-algebra-lemma-base-change-flat-at-primes-over}.
...
\begin{matrix}
\{*\} & \text{if }(\ref{equation-flat-at-primes-over})\text{ holds},\\
\emptyset & \text{else.} &
\end{matrix}
\right.
\end{equation}
...
\item The functor $H_p$ is limit preserving.
\end{enumerate}
\end{lemma}
...
\item If $f$ is of finite presentation and $\mathcal{F}$ is
of finite presentation, then the functor $F_n$ is limit preserving.
\end{enumerate}
\end{lemma}
...
\item If $f$ is of finite presentation and $\mathcal{F}$ is
of finite presentation, then the functor $F_{flat}$ is limit preserving.
\end{enumerate}
\end{lemma}
