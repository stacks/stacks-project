...
$Z_s = \text{Supp}(\mathcal{F}_s)$. In this case
$\mathcal{F}$ is an $\mathcal{O}_X$-module of finite presentation
if and only if
$\mathcal{G}$ is an $\mathcal{O}_Z$-module of finite presentation.
\end{lemma}
...
$M$ is finitely presented as a $B/(f_1, \ldots, f_m)$-module, see
...
\item $\alpha_{\xi} \otimes \kappa(\xi)$ is an isomorphism
where $\xi \in Y'_{s'}$ is the generic point, and
...
equivalent also
\begin{enumerate}
\item $\mathcal{F}_x$ is an $\mathcal{O}_{X, x}$-module of finite presentation,
\item $\mathcal{G}_{z'}$ is an $\mathcal{O}_{Z', z'}$-module of finite
presentation, and
\item $(\pi_*\mathcal{G})_{y'}$ is an $\mathcal{O}_{Y', y'}$-module of
finite presentation.
\end{enumerate}
\end{lemma}
