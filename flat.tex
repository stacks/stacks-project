...
Let $\Sigma \subset S$ be the subset of elements which are not
...
$\mathfrak q \subset S$ whose image in $\overline{S}$ are contained in
an element of $\text{Ass}_{\overline{S}}(\overline{M})$.
\end{lemma}
...
we have $\Sigma = B \setminus (\bigcup \mathfrak q_i)$.
Thus the maximal ideals of $\Sigma^{-1}B$ correspond one-to-one with the
...
universally injective $A$-module map $s : M \to M$, and the
map $M \to \Sigma^{-1}M$ is $A$-universally injective.
\end{lemma}
...
$M/sM$ is flat over $A$. This implies that $s : M \to M$
