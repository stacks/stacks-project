...
\label{section-applications-openness-flatness}

\noindent
Some applications to criteria for being \'etale.
...
image $y \in Y$. Assume
\begin{enumerate}
\item $Y$ is integral and unibranch at $y$,
\item $f$ is locally of finite type,
...
Then $x'$ as in (3) corresponds to a prime ideal
$\mathfrak q' \subset \mathfrak q$ such that $A \cap \mathfrak q' = (0)$.
It follows that $A_\mathfrak p \to B_\mathfrak q$ is injective.
By More on Algebra, Lemma
\ref{more-algebra-lemma-unramified-extension-unibranch-domain-is-etale}
the ring map $A_\mathfrak p \to B_\mathfrak q$
is isomorphic to the localization of an \'etale ring map $A \to C$
(small detail omitted).
In other words, we have an isomorphism $C_\mathfrak r \cong B_\mathfrak q$
of $A$-algebras
for some prime ideal $\mathfrak r \subset C$.

\medskip\noindent
If $f$ is locally of finite presentation (for example if $Y$ is
locally Noetherian), then we conclude from
Algebra, Lemma \ref{algebra-lemma-isomorphic-local-rings}
that $x$ has an affine neighbourhood in $X$ which is \'etale over $Y$.
In general, we can reduce to this case: by the above
we see that $f$ is flat at $x$ and hence by
Lemma \ref{lemma-finite-type-flat-X}
we see that $f$ is locally of finite presentation in
an open neighbourhood of $x$. This finishes the proof.
\end{proof}
...
\begin{enumerate}
\item $Y$ is integral and unibranch,
\item $f$ is locally of finite type,
...
On the other hand since $Y$ is unibranch and $U$ is \'etale over $Y$,
the scheme $U$ is unibranch. In particular, through every point of
$U$ there passes at most one irreducible component.
