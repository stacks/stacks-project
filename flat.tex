...
\to M_i \otimes_{B_i} \kappa(\mathfrak pB_i)$ is an isomorphism, and
\item $\text{Coker}(\alpha_n) = 0$.
\end{enumerate}
\end{definition}
...
$R$. In this case all the ring maps above are of finite presentation.
If, in addition, $M$ is of finite presentation over $S$, then it
follows from
Algebra, Lemma \ref{algebra-lemma-finite-finitely-presented-extension}
that each $M_i$ is of finite presentation both as an $A_i$-module
and as a $B_i$-module.
\end{remark}
