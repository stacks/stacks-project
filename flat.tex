...
Let $S$ be a Noetherian scheme. Let $f : X \to Y$ be a proper morphism
of schemes of finite type over $S$. Let $V \subset Y$ and $U = f^{-1}(V)$
be open and let $Z \subset V$ be a closed subset such that $f|_U$
is an isomorphism over an open neighbourhood of $Z$ in $V$.
Then there exists a $V$-admissible blowing up $Y' \to Y$
...
$f|_U$ is an isomorphism. Then $T$ is closed in $V$ and $T \cap Z = \emptyset$.
By Divisors, Lemma \ref{} we can after doing a $V$-admissible blowing
up assume that $T$ and $Z$ have disjoint closures in $Y$.
Set $Y_0 = Y \setminus \overline{T}$, $V_0 = V \setminus T$,
$U_0 = U \times_V V_0$, and $X_0 = X \times_Y Y_0$.
Since $U_0 \to V_0$ is an isomorphism, we can find a $V_0$-admissible
blowing up $Y'_0 \to Y_0$ such that the strict transform $X'_0$
of $X_0$ maps isomorphically to $Y'_0$, see
Lemma \ref{lemma-zariski-after-blowup}.
By Divisors, Lemma \ref{} there exists a $V$-admissible
blow up $Y' \to Y$ whose restriction to $Y_0$ is $Y'_0 \to Y_0$.
If $f' : X' \to Y'$ denotes the strict transform of $f$, then we
see what we want is true because $f'$ restricts to an isomorphism
over $Y'_0$.
\end{proof}
...
\begin{proof}
Throughout the proof all schemes will be separated of finite
type over $S$. This in particular implies these algebraic spaces and
...
X_1 \times_S X_2$ is separated and apply Morphisms of Spaces, Lemma
\ref{spaces-morphisms-lemma-universally-closed-permanence}).
Thus $X'_{12} \to  X_1' \times_S X_2'$ is a closed immersion.
It follows that if we define $X$ by glueing $X_1'$ and $X_2'$
along the common open subspace $X_{12}'$, then $X \to S$ is of finite type
...
Let $S$ be a Noetherian scheme. Let $U$ be a separated scheme of finite
type over $S$. Let $U = U_1 \cup U_2$ be opens such that
...
is an open $V_i \subset X_i$ and a proper surjective morphism
$\psi_i : V_i \to U$ extending $\text{id} : U_i \to U_i$, see Lemma \ref{}.
...
Observe that $\psi_i$ maps $Z_{i, i}$ isomorphically onto $Z_i$.
Denote $Z_{i, j} \subset V_i$ the inverse image of $Z_j$ under $\psi_i$.
Observe that $\psi_i : Z_{i, j} \to Z_j$ is a proper surjective map.
...
$Z_{i, i}$ and $Z_{i, j}$ in $X_i$. After replacing $X_i$ by a blow up of
$X_i$ in a closed subscheme contained in
$\overline{Z}_{i, i} \cap \overline{Z}_{i, j}$
we may assume that $\overline{Z}_{i, i}$ and $\overline{Z}_{i, j}$
are disjoint, see Divisors, Lemma \ref{}.
We assume this holds for both $X_1$ and $X_2$.
Observe that this property is preserved by
by $U_i$-admissible blowups of $X_i$.
...
We remark that if we replace $X_1$ by a $U_1$-admissible blowing up,
then $X_{12}$
...
p_1 : X_{12} \to X_2
$$
are surjective proper. We remark that if we replace $X_1$ by a
$U_1$-admissible blowing up, then $X_{12}$ is replaced by
the strict transform with respect to this blowing up.

\medskip\noindent
Denote $V_{12} \subset X_{12}$ the maximal open subscheme which admits
a morphism $\psi: V_{12} \to U$ (which is proper, see Lemma \ref{}).
Then $V_{12} = p_1^{-1}(V_1) = p_2^{-1}(V_2)$. Consider the
closed subscheme
$$
...
over an open neighbourhood of $Z_{1, 2}$ because $V_2 \to U$
over an open neighbourhood of $Z_2$. By
Lemma \ref{}
...
$$
Intersection taken in $X_1 \times_S X_2$. Namely, the inverse
image $p_1^{-1}(\overline{Z}_{1, 2})$ in $X_{12}$ maps isomorphically
to $\overline{Z}_{1, 2}$ and hence must map into
$\overline{Z}_{2, 2}$ by $p_2$ because this is true on the dense
open $Z_{1, 2}$. Howeover, we have
$\overline{Z}_{2, 2} \cap \overline{Z}_{2, 1} = \emptyset$
which concludes the argument.

\medskip\noindent
Consider the schemes
$$
W_i = U \coprod\nolimits_{(U \setminus Z_j)} (X_i \setminus \overline{Z_{ij}})
$$
Claim I: $W_i$ is separated. We omit the proof of this claim.

\medskip\noindent
Claim II: For any valuation ring $A$ over $S$ with fraction field $K$
and any morphism $h : \Spec(K) \to (U_1 \cap U_2)$ over $S$, there
is an $i$ and an extension of $h$ to a morphism $h'_i : \Spec(A) \to W_i$.

\medskip\noindent
Proof of the claim. For both $i = 1, 2$ there is a morphism
$h_i : \Spec(A) \to X_i$ extending $h$. Thus we only are in trouble
if $h_i(0) \in \overline{Z}_{i, j}$ for $i = 1, 2$. This is
impossible by the empty intersection of $X_{12}$ and the
sets given above.
...
$$
as in Lemma \ref{lemma-find-common-blowups}. By the above for every
solid diagram
$$
\xymatrix{
...
$$
where $\Im(\gamma) \subset U_1 \cap U_2$ the dotted arrow exists.
This implies that $W \to S$ is universally closed as well as
finite type and separated, hence we win.
\end{proof}
