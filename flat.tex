...
$W$ of $\xi$ where $\alpha$ is an isomorphism. Shrink $S$ to an open
...
Properties, Lemmas \ref{properties-lemma-extend-finite-presentation} and
\ref{properties-lemma-directed-colimit-diagram-finite-presentation}
(details omitted). By the result above and
...
$\mathcal{K}$ is quasi-coherent, $\mathcal{K}_U = 0$, and
$\mathcal{K}$ is flat over $S$. Since $U \subset S$ is scheme
theoretically dense in $S$ this implies that $\mathcal{K} = 0$.
