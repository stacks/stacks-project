...
Note that $g_i^*\mathcal{F}_i$ is universally pure over $S_i$, see
Lemma \ref{lemma-affine-locally-projective-pure}.
...
\medskip\noindent
At this point we can use the criterion for purity given in
Lemma \ref{lemma-criterion}.
...
$\mathcal{F}$ is pure along $X_s$ the lemma cited above implies that
$$
...
\quad\text{and}\quad
E_i = \{t \in S_i' \mid
\text{Ass}_{X_t}(\mathcal{F}_{i, t}) \subset W_i' \}
$$
are locally constructible subsets of $S'$ and $S'_i$. By
More on Morphisms,
Lemma \ref{more-morphisms-lemma-base-change-assassin-in-U}
we see that $E$ is the inverse image of
$E_i$ under the morphisms $S' \to S'_i$. By
Morphisms, Lemma \ref{morphisms-lemma-constructible-containing-open}
the inclusion proved above implies that $E$ contains an open neighbourhood
of $s'$. We may assume (after increasing $i$ once more) this open
neighbourhood is the inverse image of an open neighbourhood $V'_i$
of $s'_i$ in $S'_i$, see
Limits, Lemma \ref{limits-lemma-descend-opens}.
It follows from
Limits, Lemma \ref{limits-lemma-limit-contained-in-constructible}
that (after increasing $i$) we may assume $V'_i \subset E_i$. 
Then, applying the lemma cited above once more, we conclude that
$\mathcal{F}_i$ is pure along $(X_i)_{s_i}$.
\end{proof}
...
Lemma \ref{lemma-bourbaki-finite-type-general-base}
that $\text{WeakAss}(\mathcal{F}) \subset \text{Ass}_{X/S}(\mathcal{F})$.
