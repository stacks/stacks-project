...
separated scheme of finite type over $S$. Let $V \subset U$ be a
...
the strict transform with respect to this blowing up. Hence by
Lemma \ref{lemma-zariski-after-blowup} we may assume $Z \to Y$
is an open immersion. If $W \subset Y$ denotes the image, then we
see that the induced morphism $W \to U$ is proper because the
projection $Y \times_S U \to U$ is proper and $W \cong Z$
...
p_2 : X_{12} \to X_2
$$
are proper as $X_1$ and $X_2$ are proper over $S$.
We remark that if we replace $X_1$ by a
...
$U_i \to U \times_S (X_i \setminus \overline{Z_{ij}})$
...
Thus the image of the immersion is closed, when the immersion
...
is separated, we see that the arrow is unique as well.
...
$X \to S$ be a separated, quasi-compact morphism.
