...
Let $A$ be a set.
Assume $R$ is Noetherian, $R$ is complete with respect to $I$, and
$M$ is a finite $R$-module. Then
$$
\left(\bigoplus\nolimits_{\alpha \in A} M\right)^\wedge
\longrightarrow
\prod\nolimits_{\alpha \in A} M
$$
...
where $x_n = (x_{n, \alpha}) \in
\bigoplus\nolimits_{\alpha \in A} M/I^nM$
such that $x_{n, \alpha} = x_{n + 1, \alpha} \bmod I^nM$. 
As $M = M^\wedge$, see
Lemma \ref{lemma-completion-tensor}
we see that for any $\alpha$ there exists a $y_\alpha \in M$ such that
$x_{n, \alpha} = y_\alpha \bmod I^nM$. Note that for each $n$ there are only
finitely many $\alpha$ such that the elements $x_{n, \alpha}$ are nonzero.
Conversely, given $(y_\alpha) \in \prod_\alpha M$ sych that for each
$n$ there are only finitely many $\alpha$ such that $y_{\alpha} \bmod I^nM$
...
``convergent sums'' $\sum_\alpha y_\alpha$ with $y_\alpha \in M$
such that for each $n$ there are only finitely many $y_\alpha$
which are nonzero modulo $I^nM$. The displayed map maps this element
...
$\sum_j q_j \otimes y_{j, \alpha} = 0$ in $Q \otimes_R M$ for each $\alpha$.
Thus we can find an element
$(z_{1, \alpha}, \ldots, z_{k, \alpha}) \in \bigoplus_{l = 1, \ldots, k} M$
which maps to
$(y_{1, \alpha}, \ldots, y_{m, \alpha}) \in \bigoplus_{j = 1, \ldots, m} M$.
Moreover, if $y_{j, \alpha} \in I^{N_\alpha}M$ for $j = 1, \ldots, m$, then
we may assume that $z_{l, \alpha} \in I^{N_\alpha - c}M$ for
$l = 1, \ldots, k$.
...
Assume $R$ is Noetherian and complete with respect to $I$.
If $M$ is a finite flat $R$-module, then the completion
$\left(\bigoplus\nolimits_{\alpha \in A} M\right)^\wedge$
is a flat and Mittag-Leffler $R$-module.
In particular, the completion
...
\to \prod_{\alpha \in A} M$ is universally injective.
Thus, by
Lemmas \ref{lemma-ui-flat-domain} and
\ref{lemma-pure-submodule-ML}
it suffices to show that $\prod_{\alpha \in A} M$ is flat and Mittag-Leffler.
By
Proposition \ref{proposition-characterize-coherent}
(and
Lemma \ref{lemma-Noetherian-coherent})
we see that $\prod_{\alpha \in A} M$ is flat. By
Lemma \ref{lemma-finite-projective}
(and
Lemma \ref{lemma-Noetherian-finite-type-is-finite-presentation})
we see that $M$ is projective, i.e., we can write
$K \oplus M = R^{\oplus n}$ for some $R$-module $K$ and some integer $n$.
Then $\prod_{\alpha \in A} M$ is a direct summand of
$\prod_{\alpha \in A} R^{\oplus n}$ which is isomorphic to
$\prod_{(\alpha, i), \alpha \in A, i \in \{1, \ldots, n\}} R$.
Thus we conclude because a product of copies of $R$ is Mittag-Leffler
(Lemma \ref{lemma-product-over-Noetherian-ring})
and because a direct summand of a Mittag-Leffler module
is Mittag-Leffler
(Lemma \ref{lemma-pure-submodule-ML}).
\end{proof}
...
$R$-module, then the sequence of $I$-adic completions
$$
...
Hence it suffices to show that we can find spittings
...
$P_{n + 1} = \{x \in P \mid x \bmod I^nP \in \text{Im}(s_n)\}$.
...
$S^{\oplus m} \to M$. By
Lemma \ref{lemma-split-completed-sequence}
...
\item for finite $R$-module $Q$, any
