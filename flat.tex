...
T \times_{W, h} S
$$
is a open subscheme of $S$ containing the image of $S_n \to S$ for all $n$.
...
\medskip\noindent
Our goal is to see if we can find a pseudo-coherent $K \in D(\mathcal{O}_X)$
...
Derivd Categories of Schemes, Lemma \ref{perfect-lemma-affine-compare-bounded}.
...
$H^{a_n}(K_n) \otimes_{\mathcal{O}_{X_n}} \mathcal{O}_{X_{n - 1}}$
is zero, then $H^{a_n}(K_n)$ is zero too.
\end{proof}
...
subset of $X_n$ proper over $A_n$. If we ever need this, we will
