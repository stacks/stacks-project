...
\begin{proof}
Consider the scheme theoretic closure $Z \subset Y \times_S U$
...
\medskip\noindent
Consider the scheme theoretic closure $X_{12} \subset X_1 \times_S X_2$
...
by taking the scheme theoretic closure in $\mathbf{P}^{n_i}_S$.
...
$0 \to \mathcal{K} \to b^*\mathcal{F} \to \mathcal{F}' \to 0$
with $\mathcal{F}'$ finite locally free of rank $r$ and $\mathcal{K}$
finitely presented with $\mathcal{K}|_U = 0$.
\end{enumerate}
\end{lemma}

\begin{proof}
During this proof we will use some general properties of
Fitting ideals given in More on Algebra, Section
\ref{more-algebra-section-fitting-ideals}
and Divisors, Section \ref{divisors-section-fitting-ideals}.
Consider an affine open $V = \Spec(R)$. Let $M$ be the $R$-module
corresponding to $\mathcal{F}|_V$ and choose a presentation
$$
R^{\oplus m} \xrightarrow{A} R^{\oplus n} \to M \to 0
$$
Let $I_k \subset R$ be the ideal generated by the minors of $A$
of size $n - k$. This is the $k$th Fitting ideal of $M$.
For any prime $\mathfrak p \subset R$ corresponding to
a point of $V \cap U$ we see that $(I_{r - 1})_\mathfrak p = 0$
and $(I_r)_\mathfrak p = R_\mathfrak p$, see
More on Algebra, Lemma
\ref{more-algebra-lemma-fitting-ideal-finite-locally-free}.
In particular this implies that $V(I_r) \cap U = \emptyset$
and therefore $b : X' \to X$ is $U$-admissible, see
Divisors, Definition \ref{divisors-definition-admissible-blowup}
(we also use that the fitting ideal of $\mathcal{F}$ is finitely generated
as follows since there are only a finite number of minors of a given size).
Since $U$ is scheme theoretically dense, the map
$R = \Gamma(V, \mathcal{O}_X) \to \Gamma(U \cap V, \mathcal{O}_X)$
is injective, and looking at stalks we find that $I_{r - 1} = 0$.

\medskip\noindent
Let $f \in I_r$ and consider the affine blowup algebra
$R' = R[\frac{I_r}{f}]$. We obtain the presentation
$$
(R')^{\oplus m} \xrightarrow{A'} (R')^{\oplus n} \to M \otimes_R R' \to 0
$$
Let $I'_k$ be the $k$th Fitting ideal of $M \otimes_R R'$, i.e.,
the ideal generated by the $n - k$ size minors of $A'$.
Since $I'_k = I_k R'$ we see that $I'_{r - 1} = 0$
and $I'_r = fR'$. Consider the $R'$-modules
$$
K = (M \otimes_R R')[f]
\quad\text{and}\quad
M' = (M \otimes_R R')/K
$$
By More on Algebra, Lemma \ref{more-algebra-lemma-principal-fitting-ideal}
we see that $M'$ is a finite $R'$-module which can locally
be generated by $\leq r$ elements. Thus the $r$th Fitting ideal
of $M'$ is equal to $R'$. On the other hand, since $f$
is a nonzerodivisor on $R'$ we see
$$
\text{Fit}_{r - 1}(M') \subset
\text{Fit}_{r - 1}((M')_f) =
\text{Fit}_{r - 1}(M \otimes_R R'_f) = (I'_{r - 1})_f = 0
$$
The first equality because $M'_f \cong M \otimes_R R'_f$.
Thus $M'$ is finite locally free by
More on Algebra, Lemma
\ref{more-algebra-lemma-fitting-ideal-finite-locally-free}.
Then $M'$ is a projective $R'$ module and we can split the
short exact sequence
$$
0 \to K \to M \otimes_R R' \to M' \to 0
$$
Since $M \otimes_R R'$ is finitely presented (see displayed
presentation above), we conclude that $K$ is too (as a direct summand).
This finishes the proof as the blowup has an affine open covering
by spectra of affine blowup algebras, see
...
of zero $\rho_i$ in the matrix of $d_i$.
...
the ideals $\text{Fit}_{i, k_i}(E)$. Moreover, we see
from Lemma \ref{lemma-blowup-fitting-ideal} and looking over $V$
that $b_i$ is a $U$-admissible blowing up.
Thus $b$ is a $U$-admissible blowing up, see
Divisors, Lemma \ref{divisors-lemma-blowing-up-two-ideals}
and see
Divisors, Lemma \ref{divisors-lemma-dominate-admissible-blowups}
and its proof.
...
Choose an affine open $V$ and a complex of finite
free modules representing $E|_V$ as above.
Set
$$
\mathcal{F}_{i + 1} = \Coker(d_i)
$$
Since $\text{Fit}_{i, k_i}(E)|_V$
is the $(n_{i + 1} - \rho_i)$th Fitting ideal of $\mathcal{F}_{i + 1}$
we conclude from Lemma \ref{lemma-blowup-fitting-ideal}
that there exists a short exact sequence
$$
0 \to \mathcal{K}_{i + 1} \to \mathcal{F}_{i + 1} \to
\mathcal{F}'_{i + 1} \to 0
$$
where $\mathcal{F}'_{i + 1}$ is finite locally free of rank $\rho_i$
and $\mathcal{K}_{i + 1}$ restricts to zero on $V \cap U$.
Since $U \cap V$ is scheme theoretically dense in $V$
we conclude that $d_{i + 1}$ factors as
$$
\mathcal{O}_V^{\oplus n_{i + 1}} \to
\mathcal{F}'_{i + 1} \to
\mathcal{O}_V^{\oplus n_{i + 2}}
$$
where the first arrow is surjective and in particular locally split.

\medskip\noindent
Observe that we have $\sum (-1)^in_i = \sum (-1)^i r_i$
by looking at the definition of $r_i$.
Consider the dual perfect object $E^\vee$ of $D(\mathcal{O}_X)$, see
Cohomology, Lemma \ref{cohomology-lemma-dual-perfect-complex}.
On $V$ this corresponds to the dual complex
$$
\mathcal{O}_V^{\oplus n_b} \xrightarrow{d_{b - 1}^\vee}
\ldots \xrightarrow{d_a^\vee} \mathcal{O}_V^{\oplus n_a}
$$
to the one given above with $\mathcal{O}_V^{\oplus n_i}$
placed in degree $-i$. The analogue of $k_i$ for $E^\vee$ is
$k^\vee_i = r_{-i - 1} - r_{-i - 2} + \ldots + (-1)^{-a + i + 1}r_a$.
We claim that the ideal $\text{Fit}_{i, k^\vee_i}(E^\vee)$
is the same as the ideal $\text{Fit}_{-i, k_{-i}}(E)$.
This follows from the equality at the beginning
of this paragraph and the fact that the minors
of a matrix and its transpose agree (up to renumbering).
It follows that $\text{Fit}_{i, k^\vee_i}(E^\vee)$
is an invertible ideal. Applying the results proved above
to $E^\vee$ we find factorizations
$$
\mathcal{O}_V^{\oplus n_{i + 1}} \to
\mathcal{G}'_{i + 1} \to
\mathcal{O}_V^{\oplus n_i}
$$
of $d_i^\vee$ where $\mathcal{G}'_{i + 1}$ is finite locally free
and the first arrow is surjective (and hence locally split).
Moreover, $\mathcal{G}'_{i + 1}|_U$ is the cokernel of $d_{i + 1}^\vee|_U$.
Dualizing back we find factorizations
$$
\mathcal{O}_V^{\oplus n_i} \to
(\mathcal{G}'_{i + 1})^\vee \to
\mathcal{O}_V^{\oplus n_{i + 1}}
$$
where the second arrow is the embedding of a locally direct summand.
But here we actually find that
$$
0 \to (\mathcal{G}'_{i + 1})^\vee \to
\mathcal{O}_V^{\oplus n_{i + 1}} \to
\mathcal{O}_V^{\oplus n_{i + 2}}
$$
is an exact sequence: this is true over $U \cap V$ and
the cokernel of the first arrow is locally free hence
has no sections supported outside of $V \cap U$
(by scheme theoretic density again).
...
\begin{proof}
Combine Lemmas \ref{lemma-blowup-fitting-ideal} and
\ref{lemma-blowup-pd1-derived}.
\end{proof}
...
F_i/F_{i + 1}
$$
are finite locally free of rank $i$ on $D_n$.
\end{lemma}
...
finite locally free $\mathcal{O}_X$-modules. Denote
$d^i : \mathcal{E}^i \to \mathcal{E}^{i + 1}$ the differentials.
Denote $\mathcal{C}^\bullet$ the cone (Derived Categories,
Definition \ref{derived-definition-cone}) on the identity of
$\mathcal{E}^\bullet$. Thus
$$
\mathcal{C}^i = \mathcal{E}^i \oplus \mathcal{E}^{i + 1}
\quad\text{and}\quad
d^i_\mathcal{C} =
\left(
\begin{matrix}
d^i & \text{id}_{\mathcal{E}^{i + 1}} \\
0 & -d^{i + 1}
\end{matrix}
\right)
$$
Consider the scheme
$X \times \mathbf{P}^1_\mathbf{Z} = \mathbf{P}^1_X$
with projection morphisms $p : \mathbf{P}^1_X \to X$
and $q : \mathbf{P}^1_X \to \mathbf{P}^1_\mathbf{Z}$.
Using homogeneous coordinates $T_0, T_1$ on $\mathbf{P}^1$
we consider the maps
$$
\gamma_i =
\left(
\begin{matrix}
T_0 \text{id}_{\mathcal{E}^i} \\
-T_1 d^i
\end{matrix}
\right) :
p^*\mathcal{E}^i \otimes q^*\mathcal{O}(-1) \longrightarrow p^*\mathcal{C}^i
$$
A calculation show that there are commutative diagrams
$$
\xymatrix{
p^*\mathcal{E}^i \otimes q^*\mathcal{O}(-2) \ar[r]_{T_0} \ar[d]_{p^*d^i} &
p^*\mathcal{E}^i \otimes q^*\mathcal{O}(-1) \ar[r]_-{\gamma_i} &
p^*\mathcal{C}^i \ar[d]^{p^*d^i_\mathcal{C}} \\
p^*\mathcal{E}^{i + 1} \otimes q^*\mathcal{O}(-2) \ar[r]^{T_0 - T_1} &
p^*\mathcal{E}^{i + 1} \otimes q^*\mathcal{O}(-1) \ar[r]^-{\gamma_{i + 1}} &
p^*\mathcal{C}^{i + 1}
}
$$
In other words, we have $T_0 p^*d^i_\mathcal{C} \circ \gamma_i =
(T_0 - T_1)\gamma_{i + 1} \circ p^*d^i$.

\medskip\noindent
Let us view $X \times \mathbf{A}^1_\mathbf{Z} = \mathbf{A}^1_X$ as the
affine open of $\mathbf{P}^1_X$ corresponding to $T_0 \not = 0$, i.e.,
with affine coordinate $t = T_1/T_0$. Observe that
$\mathbf{A}^1_X \subset \mathbf{P}^1_X$ is a quasi-compact open
immersion such that $\mathbf{A}^1_X$ is a dense and
scheme theoretically dense open of $\mathbf{P}^1_X$.
Denote $\infty : \Spec(\mathbf{Z}) \to \mathbf{P}^1_\mathbf{Z}$
the point at infinity. We'll also denote $\infty : X \to \mathbf{P}^1_X$
the base change and similarly for schemes over $X$.

\medskip\noindent
Observe that over $\mathbf{A}^1_X$ the maps $\gamma_i$ have locally free
cokernel isomorphic to the pullback of $\mathcal{E}^{i + 1}$.

\begin{lemma}
\label{lemma-extends-over-U}
If $U \subset X$ is an open subscheme such that $\mathcal{E}^\bullet|_U$
is exact, then $\Coker(\gamma_i)|_{\mathbf{A}^1_U}$
extends uniquely to a locally free quotient
$p^*\mathcal{C}^i|_{\mathbf{P}^1_U} \to \mathcal{Q}^{i + 1}_U$
whose restriction to $U \times \{\infty\}$ is
$\Coker(d^i)|_U \oplus \Coker(d^{i + 1})|_U$.
\end{lemma}

\begin{proof}
Local calculation omitted. Hint: first observe that if $\mathcal{E}^\bullet$
is exact, then the ranks of the boundary maps are locally constant.
Hence we can find local bases where the boundary maps have an easy shape.
For example, suppose that
$\mathcal{E}^i = \mathcal{O}^{\oplus 2}$,
$\mathcal{E}^{i + 1} = \mathcal{O}^{\oplus 2}$, and
$d^i$ is given by the matrix
$$
\left(
\begin{matrix}
1 & 0 \\
...
\right)
$$
Then over the standard affine open subscheme $T_1 \not = 0$
with coordinate $s = T_0/T_1$ we see that $\gamma_i$
has the matrix
$$
\left(
\begin{matrix}
...
\quad
\begin{matrix}
\text{which has the same}\\
\text{image as the matrix}
\quad
\end{matrix}
\left(
\begin{matrix}
...
\right)
$$
over the open $s \not = 0$. Setting $s = 0$ in the second matrix
we see that its image is $\Ker(d^i) \oplus \Im(d^i)$ inside
$\mathcal{E}^i \oplus \mathcal{E}^{i + 1}$ as desired.
\end{proof}

\noindent
Let $U \subset X$ be as in the lemma. There exists a minimal
$\mathbf{P}^1_U \cup \mathbf{A}^1_X$-admissible blowing up
$$
b : W \longrightarrow \mathbf{P}^1_X
$$
such that there exist locally free quotients
$$
b^*p^*\mathcal{C}^i \longrightarrow \mathcal{Q}^{i + 1}
$$
agreeing with the quotients $b^*\Coker(\gamma_i)$ on the open
subscheme where these quotients are locally free; this locus
contains the open $\mathbf{P}^1_U \cup \mathbf{A}^1_X$ by the discussion above.
Such a blowing up exists; affine locally it is given by blowing up
$\mathbf{P}^1_X$ in the product of the ideals $\mathcal{I}_i$
given by the minors of size $\text{rk}(\mathcal{E}^i)$
of a local matrix for the map $\gamma_i$, see
Lemma \ref{lemma-blowup-fitting-ideal}.
Because $T_0 p^*d^i_\mathcal{C} \circ \gamma_i =
(T_0 - T_1)\gamma_{i + 1} \circ p^*d^i$ (see above)
we see that there is a unique
structure of a complex on $\mathcal{Q}^\bullet$ such
that $b^*p^*\mathcal{C}^\bullet \to \mathcal{Q}^\bullet[1]$
is a map of complexes. Computing locally we find
\begin{enumerate}
\item over $\mathbf{A}^1_X$ the complex $\mathcal{Q}^\bullet$
is equal to $p^*\mathcal{E}^\bullet|_{\mathbf{A}^1_X}$
with differentials $(1 - t)d^i$,
\item the restriction of $\mathcal{Q}^\bullet$ to $X \times \{0\}$
is equal to $\mathcal{E}^\bullet$.
\end{enumerate}
This allows us to get the following result.

\begin{lemma}
\label{lemma-graph-construction}
Let $X$ be a scheme. Let $\mathcal{E}^\bullet$ be a bounded complex
of finite locally free $\mathcal{O}_X$-modules. Let $U \subset X$
be an open subscheme such that $\mathcal{E}^\bullet|_U$ is acyclic.
There exists a proper morphism
$$
b : W \longrightarrow \mathbf{P}^1_X 
$$
and a bounded complex of finite locally free $\mathcal{O}_W$-modules
$\mathcal{Q}^\bullet$ with the following properties:
\begin{enumerate}
\item $b$ is an isomorphism over $\mathbf{P}^1_U \cup \mathbf{A}^1_X$,
\item the restriction of $\mathcal{Q}^\bullet$ to $X \times \{0\}$
is equal to $\mathcal{E}^\bullet$,
\item $\mathcal{Q}^\bullet$ restricts to an acyclic complex on
the closure of $U \times \{\infty\}$ in $W$.
\end{enumerate}
\end{lemma}

\begin{proof}
The only thing which isn't immediate from the above is part (3).
If $b$ is chosen to be an isomorphism over $\mathbf{P}^1_U$ then
$\mathcal{Q}^\bullet|_{U \times \{\infty\}}$ is equal to the
complex
$$
\ldots \to
\Coker(d^{i - 1})|_U \oplus \Coker(d^i)|_U
\to
\Coker(d^i)|_U \oplus \Coker(d^{i + 1})|_U
\to \ldots
$$
where the differential sends the summand $\Coker(d^i)$
via $-\text{id}$ to the summand $\Coker(d^i)$, see
Lemma \ref{lemma-extends-over-U}.
However, this doesn't quite yet prove (3).
Let $Z \subset W$ be the closure of $U \times \{\infty\}$.
The quotient map 
$$
p^*\mathcal{C}^i|_{U \times \{\infty\}} \to
\mathcal{Q}^{i + 1}|_{U \times \{\infty\}}
$$
is a direct sum of $\mathcal{E}^i|_U \to \Coker(d^i)|_U$ and
$\mathcal{E}^{i + 1}|_U \to \Coker(d^{i + 1})|_U$, see
Lemma \ref{lemma-extends-over-U}.
Being a direct sum of quotients is a closed condition
(details omitted). Hence over $Z$ we will also have quotients
$b^*p^*\mathcal{E}^i|_Z \to \mathcal{R}^i$ and
direct sum decompositions
$\mathcal{Q}^i|_Z = \mathcal{R}^{i - 1} \oplus \mathcal{R}^i$
such that the differential sends the summand $\mathcal{R}^i$
of $\mathcal{Q}^i|_Z$ to the summand $\mathcal{R}^i$ of
$\mathcal{Q}^{i + 1}$ via $-\text{id}$. This finishes the proof.
\end{proof}
