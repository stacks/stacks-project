...
\end{equation}
is a {\it blow up square}. Although the morphism $b : X' \to X$
...
using morphisms of finite presentation, we need a slight variant
of the above. Namely, we will say a commutative diagram
\begin{equation}
...
\end{equation}
is an {\it almost blow up square} if the following conditions
are satisfied
\begin{enumerate}
...
Example \ref{example-two-generators}.
Then $\Gamma(X, \mathcal{O}_X) \to \Gamma(X', \mathcal{O}_{X'})$
...
\ref{more-morphisms-lemma-check-h1-fibre-zero} and
\ref{more-morphisms-lemma-h1-fibre-zero-check-h0-kappa}
we conclude the surjectivity of the map for any
(affine) base change.
...
In this example we work with schemes in one
of the sites $(\Sch/\mathbf{F}_p)_h$ constructed in
Definition \ref{definition-big-small-h}.
We claim that the Zariski sheafification $\mathcal{F}$ of the functor
$$
X \longrightarrow \colim_F \Gamma(X, \mathcal{O}_X)
$$
is a h sheaf. First we claim that for
quasi-compact and quasi-separated schemes $X$
we have $\mathcal{F}(X) = \colim_F \Gamma(X, \mathcal{O}_X)$, i.e.,
sheafification is not necessary; this follows immediately from
...
Now, let's check conditions (1), (2), (3), and (4) of
...
$\mathcal{O}_X$ is an fppf sheaf and if
...
$$
is a h sheaf. To prove our claim, let's check conditions
...
$\mathcal{O}$ is an fppf sheaf and if $A$ is the equalizer of two maps
$B \to C$ of $\mathbf{F}_p$-algebras, then $\lim_F A$ is the equalizer
of the two maps $\lim_F B \to \lim_F C$.
