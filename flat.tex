...
$S = \Spec(R)$ and $D = \Spec(R/(f))$ for some nonzero divisor
...
Let $J \subset C$ be an ideal. There exists an ideal $I \subset A$
such that $IC_h = JJ'$ for some ideal $J' \subset C_h$. If $J$ is finitely
generated so are $I$ and $J'$.
\end{lemma}

\begin{proof}
We will use basic properties of fitting ideals, see
More on Algebra, Lemma \ref{more-algebra-lemma-fitting-ideal-basics}.
Set $I = \text{Fit}_0(C/J)$ where we think of $C/J$ as a finite
$A$-module. Then $IC$ is the fitting ideal of $C/J \otimes_A C$.
...
Let $A \to B$ be an \'etale ring map. Let $a \in A$ be a nonzero divisor.
...
According to Lemma \ref{lemma-trick-fitting-ideal} and its proof we take
...
for some nonzero divisor $a \in A$.
...
Since $S$ is quasi-compact and the \'etale morphisms $W_i \to S$ are open,
we can replace $S_i$ by a finite collection $W_1, \ldots, W_n$
of these opens and assume that for each $i$ there exists a
quasi-coherent sheaf of ideals $\mathcal{I}_i \subset \mathcal{O}_S$ such that
$\mathcal{I}_i \cdot \mathcal{O}_{S_i} = \mathcal{J}_i \mathcal{J}'_i$
for some finite type quasi-coherent ideal
$\mathcal{J}'_i \subset \mathcal{O}_{S_i}$.
By the discussion in the first paragraph of the proof, we may
replace $S$ by the blowup $S'$ in the product
$\mathcal{I}_1 \ldots \mathcal{I}_n$ (this is $U$-admissible by construction).
But by our choice of ideal this means that the blowups
$S_i^* \to S_i$ are isomorphisms! Namely, the base change of the
blowup in $\mathcal{I}_1 \ldots \mathcal{I}_n$ to $S_i$ is the
blowup in
$$
\mathcal{J}_i \cdot
\mathcal{J}'_i \mathcal{I}_1 \ldots \hat{\mathcal{I}_i} \ldots \mathcal{I}_n
$$
which factors through the given blowup $S_i^* \to S_i$ (in the notation
of the diagram above this means that $S_i^{\prime *} = S_i'$).
Hence we are now in the situation that $j_i^*\mathcal{F}$ is flat over
$S_i$. Hence $j_i^*\mathcal{F}$ is flat over $S$, see
Lemma \ref{lemma-etale-flat-up-down}.
...
\medskip\noindent
To finish the proof of the theorem we still have to show that we can
obtain a flat strict transform which is of finite presentation as well.
(The reader only interested in the finitely presented or even Noetherian
case may skip this argument.)
To do this, choose a surjection $\mathcal{F}_1 \to \mathcal{F}$ with
$\mathcal{F}_1$ an $\mathcal{O}_X$-module of finite presentation such
that $\mathcal{F}_{1, U} \to \mathcal{F}_U$ is an isomorphism, see
Properties, Lemmas \ref{properties-lemma-application-directed-colimit}.
By the result proved in the first part of the proof and
Divisors, Lemma \ref{divisors-lemma-dominate-admissible-blowups}
we can find a $U$-admissible blowup $S' \to S$ such that
both strict transforms $\mathcal{F}'_1$ and $\mathcal{F}'$ are flat
over $S'$. The induced map $\mathcal{F}'_1 \to \mathcal{F}'$ is
a surjection of $\mathcal{O}_{X \times_S S'}$-modules flat
over $S'$ which is an isomorphism over $U$. Hence the kernel
$\mathcal{K}$ is quasi-coherent and $\mathcal{K}_U = 0$.
Thus the sections of $\mathcal{K}$ are supported the inverse
image of $X \setminus U$. Doing a final blowup $b : S'' \to S'$
with exceptional divisor $E$ having support $S'' \setminus U$
(see third paragraph of Remark \ref{remark-successive-blowups})
we see that the strict transforms $\mathcal{F}_1''$ and $\mathcal{F}''$
of $\mathcal{F}_1'$ and $\mathcal{F}'$ become equal. Namely, the kernel of
$\text{pr}^*\mathcal{F}_1' \to \mathcal{F}_1''$
in any case contains $\text{pr}^*\mathcal{K}$ as it is supported on $E$.
