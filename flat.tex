...
such that $\mathcal{F}'_T$ is flat in dimensions $\geq n$.
...
In Situation \ref{situation-flat-dimension-n}.
Assume $X$ of finite presentation over $S$ and $\mathcal{F}$
an $\mathcal{O}_X$-module of finite presentation and
that there exists a complete d\'evisage
of $\mathcal{F}/X/S$ over some point $s \in S$.
Then after possibly replacing $S$ by an open neighbourhood
of $s$ the functor $F_n$ is representable by a closed subscheme
...
Recall that $d_1 > d_2 > \ldots > d_n \geq 0$.
TO BE CONTINUED...
\end{proof}
...
and such that $X_s \subset \bigcup h_j(Y_j)$.
