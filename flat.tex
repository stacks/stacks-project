...
$(S', s') \to (S, s)$ and open
$V \subset X \times_S \text{Spec}(\mathcal{O}_{S', s'})$ as in
Proposition \ref{proposition-finite-type-flat-at-point}.
...
and an open $V_x \subset X \times_S \text{Spec}(\mathcal{O}_{S_x, s_x})$
...
X \times_S \text{Spec}(\mathcal{O}_{S', s'})
\longrightarrow
X \times_S \text{Spec}(\mathcal{O}_{S_{x_i}, s_{x_i}})
$$
...
\xymatrix{
(X, x) \ar[d] & (X', x') \ar[l]^g \ar[d] \\
(S, s) & (\text{Spec}(\mathcal{O}_{S', s'}), s') \ar[l]
}
$$
such that $X' \to X \times_S \text{Spec}(\mathcal{O}_{S', s'})$
...
$\text{Spec}(\mathcal{O}_{S, s})$. Hence by
Proposition \ref{proposition-finite-type-flat-at-point}
...
$S$ are affine, say $X = \text{Spec}(B)$ and $S = \text{Spec}(A)$.
...
\xymatrix{
X \ar[d] & X' \ar[l]^g \ar[d] \\
S & \text{Spec}(\mathcal{O}_{S', s'}) \ar[l]
}
$$
such that $X' \to X \times_S \text{Spec}(\mathcal{O}_{S', s'})$
...
\xymatrix{
(X, x) \ar[d] & (Y_x, y_x) \ar[l]^{g_x} \ar[d] \\
(S, s) & (\text{Spec}(\mathcal{O}_{S_x, s_x}), s_x) \ar[l]
}
$$
such that $Y_x \to X \times_S \text{Spec}(\mathcal{O}_{S_x, s_x})$
...
Set
$$
X' = \coprod Y_{x_i} \times_{\text{Spec}(\mathcal{O}_{S_{x_i}, s_{x_i}})}
\text{Spec}(\mathcal{O}_{S', s'})
$$
...
More Algebra,
Definition \ref{more-algebra-definition-relatively-finitely-presented}).
If we ever need this result in the stacks project we will convert
this remark into a lemma with a proof.
\end{remark}
...
and its proof. Hence $S^h = \text{lim}_i\ T_i$ and we win by
...
$\text{Spec}(\Sigma^{-1}B) \to \text{Spec}(B)$. Thus
Lemma \ref{lemma-homothety-spectrum}
...
$S$ by $\text{Spec}(\mathcal{O}_{S, s})$ and assume that $S$ is local.
...
Let $S^h = \text{Spec}(\mathcal{O}_{S, s}^h)$ be the henselization
...
\'etale neighbourhood.
We may also replace $S$ by $\text{Spec}(\mathcal{O}_{S, s})$
...
Note that $X' \to \text{Spec}(\mathcal{O}_{S', s'})$ is of finite type
...
we may replace $S$ by $\text{Spec}(\mathcal{O}_{S', s'})$. Hence
...
f^{-1}(\text{Spec}(\mathcal{O}_{S', s'})) \cap
...
$\text{Spec}(\mathcal{O}_{S', s'})$ maps into $E_i$. As
$\mathcal{O}_{S', s'} =
\text{colim}_{i' \geq i}\ \mathcal{O}_{S'_{i'}, s'_{i'}}$
we see that $\text{Spec}(\mathcal{O}_{S'_{i'}, s'_{i'}})$
...
Lemma \ref{lemma-criterion}
we see that $\text{Spec}(\mathcal{O}_{S', s'}) \subset E$.
By
...
on $\text{Spec}(B)$ is pure relative to $\text{Spec}(A)$.
\end{proposition}
...
$\text{Ass}_{X/S}(\mathcal{F})$ lying over $\text{Spec}(\mathcal{O}_{S, s})$.
Set
$$
...
$\text{Spec}(\mathcal{O}_{S, s}) \subset E$. By
Morphisms, Lemma \ref{morphisms-lemma-constructible-containing-open}
...
If $\mathcal{F}$ is flat at $x$ over $S$ there exists an affine
elementary \'etale neighbourhood $(S', s') \to (S, s)$ and
an affine open $U' \subset X \times_S \text{Spec}(\mathcal{O}_{S', s'})$
...
$\text{Spec}(\mathcal{O}_{S', s'})$ and $X$ by an open of
$X \times_S \text{Spec}(\mathcal{O}_{S', s'})$ we may assume that
...
\begin{proof}
We can find a morphism of finite type $X \to S = \text{Spec}(A)$
...
\begin{equation}
\label{equation-iso}
F_{iso} : (\textit{Sch}/S)^{opp} \longrightarrow \textit{Sets}, \quad
...
Suppose that $R = \text{colim}_{i \in I}\ R_i$ is a directed colimit
...
$F_{fl}(\text{colim}_i\ A_i) = \text{colim}_i\ F_{fl}(A_i)$.
\end{enumerate}
\end{lemma}
...
=
\text{Spec}( \kappa(\mathfrak m_B) \otimes_A \kappa(\mathfrak m_{A'}) )
$$
...
H_p : (\textit{Sch}/S)^{opp} \longrightarrow \textit{Sets}, \quad
T \longrightarrow \left\{
\begin{matrix}
...
F_n : (\textit{Sch}/S)^{opp} \longrightarrow \textit{Sets}, \quad
T \longrightarrow \left\{
\begin{matrix}
...
$$
\xymatrix{
X_{\text{Spec}(\mathcal{O}_{T, t})} \ar[d] &
X_{\text{Spec}(\mathcal{O}_{T_i, t_i})} \ar[d] \ar[l] \\
\text{Spec}(\mathcal{O}_{T, t}) &
\text{Spec}(\mathcal{O}_{T_i, t_i}) \ar[l]
}
$$
...
Thus part (2) is proved and the proof of
...
\begin{equation}
\label{equation-flat}
F_{flat} : (\textit{Sch}/S)^{opp} \longrightarrow \textit{Sets}, \quad
T \longrightarrow \left\{
\begin{matrix}
...
$$
F_r : (\textit{Sch}/S)^{opp} \to \textit{Sets},\quad
T \mapsto
\left\{
\begin{matrix}
...
$$
in the category of Zariski sheaves on $\textit{Sch}/S$. Hence it follows
...
\item $\text{Spec}(\mathcal{O}_{T, t}) \to S$ factors through $Z$.
\end{enumerate}
...
$\text{Spec}(\mathcal{O}_{S, s}/\mathfrak m_s^n)$.
...
Then $N$ is a filtered colimit $N = \text{colim}_i F_i$
...
injective by
Lemma \ref{lemma-universally-injective-local}
wherefore $u = \text{colim}\ u_i$ is $A$-universally injective as desired.
