...
\section{Ample sheaves and cohomology}
...
Let $\mathcal{L}$ be an invertible $\mathcal{O}_X$-module.
Let $s \in \Gamma(X, \mathcal{L})$ be a section.
Let $\varphi : \mathcal{F}' \to \mathcal{F}$ be a map of quasi-coherent
$\mathcal{O}_X$-modules. Assume that
\begin{enumerate}
\item $X$ is quasi-compact and quasi-separated,
\item $\mathcal{F}$ is of finitely presented, and
\item $\varphi|_{X_s} : \mathcal{F}'|_{X_s} \to \mathcal{F}|_{X_s}$
is an isomorphism.
\end{enumerate}
Then there exists an $n \geq 0$ and a morphism
$\alpha : \mathcal{F} \to
\mathcal{F}' \otimes_{\mathcal{O}_X} \mathcal{L}^{\otimes n}$
such that multiplication by $s^n$ on $\mathcal{F}$ is equal
to the composition of $\alpha$ and
$\varphi \otimes \text{id}_{\mathcal{L}^{\otimes n}}$.
...
Let $A$ be a ring. Let $f \in A$. Let $\varphi : M' \to M$ be
a map of $A$-modules.
Assume $M$ is a finitely presented $A$-module, and assume that
$\varphi_f : (M')_f \to M_f$ is an isomorphism.
Then there exists an $n \geq 0$ and an $A$-module map
$\alpha : M \to M'$ such that
$\varphi \circ \alpha = f^n\text{id}_M$.
...
By
Properties, Lemma \ref{properties-lemma-quasi-coherent-colimit-finite-type}
we can write $\mathcal{F}$ as a directed limit of its quasi-coherent
$\mathcal{O}_X$-submodules of finite type.
Hence every $\xi \in H^p(X, \mathcal{F})$ is the image of
$\xi' \in H^p(X, \mathcal{F}')$ for some quasi-coherent
$\mathcal{O}_X$-submodule of finite type. Thus we may replace $\mathcal{F}$
by $\mathcal{F}'$ and assume $\mathcal{F}$ is of finite type.
...
$$
as the directed colimit of its $\mathcal{O}_X$-submodules of finite type (
Properties, Lemma \ref{properties-lemma-quasi-coherent-colimit-finite-type}
again). After shrinking $\Lambda$ we may assume that
$\mathcal{F} \subset \mathcal{F}_\lambda$ for all $\lambda \in \Lambda$.
...
group is zero. By
Cohomology, Lemma \ref{cohomology-lemma-quasi-separated-cohomology-colimit}
again it follows that $\xi$ maps to zero in $H^p(X, \mathcal{F}_\lambda)$ for
some $\lambda$. By Lemma \ref{}
there exists an $n$ such that
multiplication by $s^n$ on $\mathcal{F}_\lambda$
factors through $\mathcal{F}$. By considering the commutative
diagram
$$
\xymatrix{
\mathcal{F} \ar[r]_{s^n} \ar[d] &
\mathcal{F} \ar[d] \\
\mathcal{F}_\lambda \ar[r]_{s^n} &
\mathcal{F}_\lambda \ar[ul]
}
$$
...
multiplication by $s^n$ on $\mathcal{F}_\lambda$
factors through $\mathcal{F}$
we see that $s^n\xi = 0$.
\end{proof}
