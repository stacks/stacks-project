...
\end{reference}
\begin{slogan}
Serre's criterion for affinenes.
\end{slogan}
...
\section{Coherent sheaves on Proj}
...
Let $A$ be a Noetherian graded ring. Let $M$ be a finite
graded $A$-module. Set $X = \text{Proj}(A)$ and let $\widetilde{M}$ be
...
\item there exists an integer $d$ such that $N_n \to N'_n$
is an isomorphism for $d | n$.
\end{enumerate}
\end{lemma}
...
in terms of vanshing of cohomology after twisting.
...
\begin{proof}
We will use the criterion of Lemma \ref{lemma-vanshing-gives-ample}
without further mention. Assume $\mathcal{L}$ is ample. Let $\mathcal{G}$
be a coherent $\mathcal{O}_Y$-module. By the projection formula we have
$$
f_*(\mathcal{G} \otimes_{\mathcal{O}_Y} (f^*\mathcal{L})^{\otimes n})) =
f_*\mathcal{G} \otimes_{\mathcal{O}_X} \mathcal{L}^{\otimes n}
$$
Since $f_*\mathcal{G}$ is coherent
(Lemma \ref{lemma-finite-pushforward-coherent}) we see that
the higher cohomology groups of this sheaf are zero for $n \gg 0$.
Since these cohomology groups are the same as the cohomology groups of
$\mathcal{G} \otimes_{\mathcal{O}_Y} (f^*\mathcal{L})^{\otimes n})$
on $Y$ (Lemma \ref{lemma-relative-affine-cohomology}), we conclude
that $f^*\mathcal{L}$ is ample.
...
\begin{proof}
This follows from Lemma \ref{lemma-surjective-finite-morphism-ample},
