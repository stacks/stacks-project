...
\end{enumerate}
Then the complex $\check{\mathcal{C}}^\bullet$ is a complex of
quasi-coherent sheaves on $S$ which comes equipped with a canonical
isomorphism
$$
...
and quasi-compact (but it really does not matter). Denote
$X' = X_{S'} = S' \times_S X$ the base change of $X$ and denote
$f' : X' \to S'$ the base change of $f$. Also write $g' : X' \to X$
the projection. Let $\mathcal{U}' : X' = \bigcup U_{i, S'} = \bigcup U_i'$
be the induced covering of $X'$. In this case we claim
that
$$
...
a quasi-isomorphic complex in general!
\end{remark}
...
Lemma \ref{} and the result is clear.
...
sequence (see Cohomology, Lemma \ref{}), part of which is an exact sequence
$$
...
$H^p(V, \mathcal{F}') \oplus H^p(W, \mathcal{F}')$, pick a lift
$\tilde \xi' \in H^{p - 1}(V \cap W, \mathcal{F}')$ of this image,
and after that we embed $\mathcal{F}'$ in a quasi-coherent sheaf
