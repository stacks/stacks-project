...
\begin{proof}
We may assume that $Y = \Spec(A)$ with $A$ Noetherian.
Let $y$ correspond to the prime ideal $\mathfrak p \subset A$.
Let $B = \bigoplus \mathfrak p^d$ as a graded $A$-algebra.
Set $\mathcal{B} = f^*\widetilde{B}$ and $\mathcal{F} = \mathcal{B}$
viewed as a quasi-coherent graded $\mathcal{B}$-module of finite type.
Let $d_0$ be as in
Lemma \ref{lemma-graded-finiteness} part (2).
Then we see that
$$
H^1(X, \mathfrak{p}^n \mathcal{L}^{\otimes d}) = 0
$$
for all $d \geq d_0$ and all $n \geq 0$. Observe that for $g \in A$,
$g \not \in \mathfrak p$, we can replace $A$ by $A_g$,
the prime $\mathfrak p$ by $\mathfrak p A_g$,
and $X$ by the base change to $\Spec(A_g)$,
without affecting the vanishing proved above
(Lemma \ref{lemma-flat-base-change-cohomology}).

\medskip\noindent
Pick $d \geq d_0$ so that we can find $r \geq 0$ and sections
...
Since $A/\mathfrak p \to \kappa(\mathfrak p) = \kappa(y)$
is a localization and hence flat, we see that
$H^0(X_y, \mathcal{L}_y^{\otimes d}) =
H^0(X_{A/\mathfrak p}, \mathcal{L}^{\otimes d}|_{X_{A/\mathfrak p}})
\otimes_{A/\mathfrak p} \kappa(\mathfrak p)$
again by cohomology and base change
(Lemma \ref{lemma-flat-base-change-cohomology}).
Hence there exists a $g \in A$, $g \not \in \mathfrak p$
such that $s_{y, 0}, \ldots, s_{y, r}$ in the image of
$$
H^0(X_{(A/\mathfrak p)_g}, \mathcal{L}^{\otimes d}|_{X_{(A/\mathfrak p)_g}})
\longrightarrow
H^0(X_y, \mathcal{L}_y^{\otimes d})
$$
After replacing $A$ by $A_g$, etc, as explained above, we
may and do assume that $s_{y, i}$ comes from some
$\overline{s}_i \in
H^0(X_{A/\mathfrak p}, \mathcal{L}^{\otimes d}|_{X_{A/\mathfrak p}})$.
Next, we consider the long exact cohomology sequence
associated to the short exact sequence
$$
0 \to \mathfrak p\mathcal{L}^{\otimes d} \to
\mathcal{L}^{\otimes d} \to
\mathcal{L}^{\otimes d}|_{X_{A/\mathfrak p}} \to 0
$$
of $\mathcal{O}_X$-modules.
Observe that $X_{A/\mathfrak p}$ is the closed subscheme of $X$
cut out by $\mathfrak p \mathcal{O}_X$ hence this makes sense
(here we also use the equivalence of Lemma \ref{lemma-i-star-equivalence}
and below we will use the agreement of cohomology under pushforward by a
closed immersion from Lemma \ref{lemma-relative-affine-cohomology}).
The long exact cohomology sequence is
$$
0 \to
H^0(X, \mathfrak{p} \mathcal{L}^{\otimes d}) \to
H^0(X, \mathcal{L}^{\otimes d}) \to
H^0(X_{A/\mathfrak p}, \mathcal{L}^{\otimes d}|_{X_{A/\mathfrak p}}) \to
H^1(X, \mathfrak{p} \mathcal{L}^{\otimes d})
$$
Since the $H^1$ is zero by the above, we can choose
$s_0, \ldots, s_r \in H^0(X, \mathcal{L}^{\otimes d})$
mapping to $\overline{s}_0, \ldots, \overline{s}_r$.
