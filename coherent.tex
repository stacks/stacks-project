...
$$
\item For any $i$ the cohomology group $H^i(X, \mathcal{F})$ is a finite
$A_0$-module.
\item If $i > 0$, then $H^i(X, \mathcal{F}(d)) = 0$ for all $d$ large enough.
...
this defines a closed immersion $r : X \to \mathbf{P}^n_{A_0}$
such that $r^*\mathcal{O}_{\mathbf{P}^n_{A_0}}(1) = \mathcal{O}_X(1)$.
In particular, $X$ is Noetherian as a closed subscheme of the Noetherian
scheme $\mathbf{P}^n_{A_0}$.
We claim that the results of the lemma for $\mathcal{F}$
follow from the corresponding
results of Lemma \ref{lemma-coherent-projective} for the coherent sheaf
$r_*\mathcal{F}$ (Lemma \ref{lemma-i-star-equivalence}) on
$\mathbf{P}^n_{A_0}$. For example, by this lemma there
exists a surjection
$$
\bigoplus\nolimits_{j = 1, \ldots, r} \mathcal{O}_{\mathbf{P}^n_{A_0}}(d_j)
\longrightarrow r_*\mathcal{F}.
$$
Pulling back and using that $r^*r_*\mathcal{F} = \mathcal{F}$
(see lemma cited above) we obtain a corresponding surjection over $X$.
The statements on cohomology follow from the fact that
$H^i(X, \mathcal{F}(d)) = H^i(\mathbf{P}^n_{A_0}, r_*\mathcal{F}(d))$
for example by Lemma \ref{lemma-relative-affine-cohomology}.
Some details omitted.
\end{proof}

\begin{lemma}
\label{lemma-recover-tail-graded-module}
Let $A$ be a graded ring. Let $M$ be a graded $A$-module. Assume
\begin{enumerate}
\item $A_0$ is Noetherian,
\item $A$ is generated by finitely many elements of $A_1$ over $A_0$, and
\item $M$ is a finitely generated $A$-module.
\end{enumerate}
Set $X = \text{Proj}(A)$ and let $\widetilde{M}$ be the
quasi-coherent $\mathcal{O}_X$-module on $X$ associated to $M$.
The maps
$$
...
$K_{(f_i)}$ would not be zero, i.e., $\widetilde{K}$ would not be zero.
