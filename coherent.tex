...
Thus we see from
Lemma \ref{lemma-vanishing-nr-affines-quasi-separated}
that $H^p(X', \mathcal{F}') = 0$. Since
$H^0(S', R^pf'_*\mathcal{F}') = H^p(X', \mathcal{F}')$ by
Lemma \ref{lemma-quasi-coherence-higher-direct-images-application}
and since
$R^pf'_*\mathcal{F}'$ is quasi-coherent (by the first part of the proof)
...
$D^{+}(\textit{QCoh}(\mathcal{O}_S))$ (insert future reference here).
