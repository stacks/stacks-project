...
\text{with}
\quad
L^{-i, q} = R^q\pi_*\pi^*(\wedge^i\mathcal{E})(-i))
$$
converging to zero!
Note that locally on $S$ the sheaf $\mathcal{E}$ is trivial,
i.e., isomorphic to $\mathcal{O}_S^{\oplus n + 1}$ and we can
locally on $S$ use the result of Lemmas
...
It follows immediately that $L^{-i, q} = 0$ unless $i = q = 0$
or $i = n + 1$ and $q = n$. Hence there can only be one nonzero
differential in the spectral sequence namely the map
$d_{n + 1}$ inducing a map
$$
d_{n + 1}^{0, 0} :
\pi_*\mathcal{O}_{\mathbf{P}(\mathcal{E})}
\longrightarrow
R^n\pi_*\pi^*\wedge^{n + 1}\mathcal{E}(- n - 1)
$$
which has to be an isomorphism. To be continued.
\end{proof}
