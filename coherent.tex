...
the weaker variant (1) which is in force in the current situation.)
...
Let $f : X \to Y$ be a morphism of schemes.
Assume $f$ is finite, surjective and $Y$ locally Noetherian.
Let $Z \subset Y$ be an integral closed subscheme with
generic point $\xi$. Then
there exists a coherent sheaf $\mathcal{F}$ on $X$
such that the support of $f_*\mathcal{F}$ is equal to $Z$
and $(f_*\mathcal{F})_\xi$ is annihilated by $\mathfrak m_\xi$.
\end{lemma}

\begin{proof}
Note that $X$ is locally Noetherian by
Morphisms, Lemma \ref{morphisms-lemma-finite-type-noetherian}.
Because $f$ is surjective the fibre $X_\xi$ is not empty.
Pick $\xi' \in X$ mapping to $\xi$. Let $Z' = \overline{\{\xi'\}}$.
We may think of $Z' \subset X$ as a reduced closed subscheme,
see Schemes, Lemma \ref{schemes-lemma-reduced-closed-subscheme}.
Hence the sheaf $\mathcal{F} = (Z' \to X)_*\mathcal{O}_{Z'}$
is a coherent sheaf on $X$ (see
