...
into an absolute statement by mapping into $\text{Spec}(\mathbf{Z})$.
...
cohomology. This is true, if $S$ is quasi-compact and quasi-separated.
...
First of all the result is very easy if $f$ and $S$ are separated.
...
is a complex of quasi-coherent sheaves on $S$ which comes equipped with a
isomorphism
$$
...
\begin{remark}
\label{remark-base-change-complex}
Let $f : X \to S$ and $\mathcal{F}$ satisfy the hypotheses of
Lemma \ref{lemma-separated-case-relative-cech}. Choose a finite
affine open covering $\mathcal{U} : X = \bigcup U_i$ of $X$.
Suppose further that
$g : S' \to S$ is any morphism of schemes, say with $S'$ separated
and quasi-compact. Denote
$X' = X_{S'} = S' \times_S X$ the base change of $X$ and denote
$f' : X' \to S'$ the base change of $f$. Also write $g' : X' \to X$
the projection. Set $U_i' = (g')^{-1}(U_i) = U_{i, S'}$.
...
\check{\mathcal{C}}^\bullet(\mathcal{U}', f', \mathcal{F}')
$$
Moreover, exactly as in the proof of the lemma one sees that there
is an isomorphism
$$
\check{\mathcal{C}}^\bullet(\mathcal{U}, f, \mathcal{F})
\longrightarrow
Rf'_*\mathcal{F}'
$$
in $D^{+}(S')$. Hence we see that the complex
$\check{\mathcal{C}}^\bullet(\mathcal{U}, f, \mathcal{F})$
{\it universally} computes the higher direct images of $f : X \to S$.
Warning: This is something about this particular complex and
it is not preserved after replacing
...
\begin{remark}
\label{remark-hypercoverings}
Let $f : X \to S$ and $\mathcal{F}$ satisfy the hypotheses of
Lemma \ref{lemma-separated-case-relative-cech}, except we replace
the hypothesis that $X$ is separated by the hypothesis that $X$ is
quasi-separated. It turns out that we can still do a construction
of a Cech complex in this situation. Namely, we can find a
Zariski hypercovering $U_\bullet \to X$ of $X$ all of whose terms
are affine. This will give a complex with similar properties
in this case.
\end{remark}

\noindent
We return to an alternative proof of the fact that $Rf_*\mathcal{F}$
can be represented by a complex of quasi-coherent sheaves. Namely,
we will show that injective modules give quasi-coherent sheaves on
affines whose cohomologies are zero on quasi-compact opens. After that
we will use these to resolve $\mathcal{F}$.

\begin{lemma}
\label{lemma-kill-element-on-quasi-compact-open-affine}
Let $X$ be an affine scheme.
Let $U \subset X$ be a quasi-compact open subscheme.
Let $\mathcal{F}$ be a quasi-coherent sheaf on $X$.
Let $p \geq 1$ and $\xi \in H^p(U, \mathcal{F})$.
Then there exists a quasi-coherent sheaf $\mathcal{F}'$
and an injective $\mathcal{O}_X$-module map
$\varphi : \mathcal{F} \to \mathcal{F}'$ such
that $\varphi(\xi) = 0$.
\end{lemma}

\begin{proof}
Let $t$ be the minimal number of affine opens needed the cover $U$.
We will prove the lemma by induction on $U$.
If $t = 1$, then $H^p(U, \mathcal{F}) = 0$ for $p \geq 1$ by
Lemma \ref{lemma-quasi-coherent-affine-cohomology-zero}
and the result is clear.
Suppose $t > 1$, and write $U = V \cup W$ where $W$ is open affine and
$V$ is a union of $t - 1$ open affines. Then $V \cap W$ is a union
of $t - 1$ open affines too, and hence the induction hypothesis applies
to all of $V$, $W$ and $V \cap W$. We will use the Mayer-Vietoris
sequence (see Cohomology, Lemma \ref{cohomology-lemma-mayer-vietoris}),
part of which is an exact sequence
$$
H^{p - 1}(V, \mathcal{F}) \oplus H^{p - 1}(W, \mathcal{F}) \to
H^{p - 1}(V \cap W, \mathcal{F}) \to
H^p(U, \mathcal{F}) \to
H^p(V, \mathcal{F}) \oplus H^p(W, \mathcal{F}).
$$
Now if $p \geq 2$ then it is clear what we should do. Namely we first
embed $\mathcal{F}$ in a quasi-coherent sheaf $\mathcal{F}'$ such
that the image $\xi'$ of $\xi$ vanishes in
$H^p(V, \mathcal{F}') \oplus H^p(W, \mathcal{F}')$ (using
induction), then pick a lift
$\tilde \xi' \in H^{p - 1}(V \cap W, \mathcal{F}')$ of this image,
and finally we embed $\mathcal{F}'$ in a quasi-coherent sheaf
$\mathcal{F}''$ such that $\tilde \xi'$ maps to zero in
$H^p(V, \mathcal{F}') \oplus H^p(W, \mathcal{F}'')$ (using induction).
This does not work if $p = 1$ since in that case $p - 1 = 0$ and
the induction hypothesis does not apply. However, in this case
we simply need to show that after embedding $\mathcal{F}'$ into
a quasi-coherent sheaf $\mathcal{F}''$ the element $\tilde\xi'$
comes from an element of
$H^0(V, \mathcal{F}'') \oplus H^0(W, \mathcal{F}'')$.
This is true by
Properties, Lemma \ref{properties-lemma-inject-section-lifts}.
\end{proof}

\noindent
This lemma has the following amusing consequence.

\begin{lemma}
\label{lemma-injectives-compute-cohomology-quasi-compact-opens}
Let $X = \text{Spec}(A)$ be an affine scheme.
Let $I$ be an injective $A$-module.
Let $U \subset X$ be a quasi-compact open.
Then $H^p(U, \widetilde{I}) = 0$ for all $p > 0$.
\end{lemma}

\begin{proof}
Let $\xi \in H^p(U, \widetilde{I})$ with $p > 0$. By
Lemma \ref{lemma-kill-element-on-quasi-compact-open-affine}
there exists an injective map of quasi-coherent sheaves
$\varphi : \widetilde{I} \to \mathcal{F}$ such that $\xi$ maps to zero
in $H^p(U, \mathcal{F})$. Write $\mathcal{F} = \widetilde{M}$.
Then $\varphi$ corresponds to an injection of $A$-modules $I \to M$.
Since $I$ is injective we get $M = I \oplus M'$, in other words
the map $\varphi$ is a split injection. Hence $\xi = 0$.
\end{proof}
