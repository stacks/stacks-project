...
Note that $R^pi_*\mathcal{F} = 0$ either by
Lemma \ref{lemma-relative-affine-vanishing} or simply because
$i_*$ is exact (see Modules, Lemma \ref{modules-lemma-i-star-exact}).
...
We can apply Chow's lemma (Limits, Lemma \ref{limits-lemma-chow-Noetherian})
to the morphism $Z \to S$. Thus we get a diagram
$$
\xymatrix{
Z \ar[rd] & Z' \ar[d] \ar[l]^\pi \ar[r]_i & \mathbf{P}^n_S \ar[dl] \\
& S &
}
$$
