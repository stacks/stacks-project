...
$S'$ are affine. Say $S = \text{Spec}(A)$ and $S' = \text{Spec}(B)$.
...
\noindent
We would like to prove a little more in situation
(\ref{equation-base-change-diagram}).
Namely, if $f$ is quasi-compact and quasi-separated
we would like to represent $Rf_*\mathcal{F}$ by a complex of
quasi-coherent sheaves on $S$. This can be done in some cases,
for example if $S$ is quasi-compact and (semi-)separated, by relating it
to the question of whether $D_{QCoh}^{+}(S)$ is equivalent to
$D^{+}(\textit{QCoh}(\mathcal{O}_S))$, see
Section \ref{section-derived-quasi-coherent}.

\medskip\noindent
In this section we will use a different approach which produces a
complex having a good base change property. First of all the result
is very easy if $f$ and $S$ are separated. Since this is the case which
...
\begin{lemma}[Variant of Lemma \ref{lemma-property-irreducible}]
...
\begin{lemma}[Variant of Lemma \ref{lemma-property}]
...
\noindent
In this section we state the simplest case of a very general phenomenon
that will be discussed elsewhere (insert future reference here). Please
see Remark \ref{remark-explain-perfect-direct-image} for a tranlation
of the statement into algebra.
...
Noetherian adic formal scheme'' which is used in \cite{EGA}.
...
\begin{proof}
Let $\text{Mod}_{fg}(A, I)$ be the category of systems $(M_n)$ of finite
$A$-modules satisfying: (1) $M_n$ is annihilated by $I^n$ and (2)
...
it suffices to show $\text{Mod}(A, I)$ is equivalent to the category of
finite $A^\wedge$-modules. To see this it suffices to prove that given
an object $(M_n)$ of $\text{Mod}_{fg}(A, I)$ the module
$$
M = \lim M_n
$$
...
Let $X$ be a Noetherian scheme and let $\mathcal{I} \subset \mathcal{O}_X$
be a quasi-coherent sheaf of ideals. The category
...
$(\text{Coker}(\alpha_n))$ (details omitted). On the other hand, to describe
the kernel let
$$
...
\medskip\noindent
To finish the proof of the lemma we have to show that $\text{Coim} = \text{Im}$
in $\textit{Coh}(X, \mathcal{I})$. However, we have seen above that taking
kernels and cokernels commutes, over affines, with the description
of $\textit{Coh}(X, \mathcal{I})$ with a category of modules. Since
$\text{Im} = \text{Coim}$ holds in the category of modules we win.
\end{proof}
...
\begin{proof}
Omitted. Hint: Look on affine opens and use
Lemma \ref{lemma-inverse-systems-affine}.
\end{proof}
...
\begin{equation}
\label{equation-formal-existence}
...
\begin{lemma}
\label{lemma-exact}
The functor (\ref{equation-formal-existence}) is exact.
\end{lemma}
...
Lemma \ref{lemma-inverse-systems-affine}.
The functor is the functor $\text{Mod}_{fg}(A) \to \text{Mod}_{fg}(A^\wedge)$
...
\text{Mor}_{\textit{Coh}(X, \mathcal{I})}
...
(details omitted). The lemma follows.
\end{proof}
...
Then the functor (\ref{equation-formal-existence}) is fully faithful.
\end{lemma}
...
(\ref{equation-formal-existence}) follows from the theorem on formal
...
Set $B = \bigoplus I^n/I^{n + 1}$.
Consider the fibre product diagram
$$
\xymatrix{
X' = \Spec(B) \times_{\Spec(A)} X \ar[r]_-\pi \ar[d]_{f'} & X \ar[d]^f \\
\Spec(B) \ar[r] & \Spec(A)
}
$$
Note that $X'$ is the relative spectrum of the
quasi-coherent $\mathcal{O}_X$-algebra
$\mathcal{B} = \bigoplus \mathcal{I}^n/\mathcal{I}^{n + 1}$ by
Constructions, Lemma \ref{constructions-lemma-spec-properties}.
Since $\mathcal{G}$ is a finite type $\mathcal{B}$-module by
Lemma \ref{lemma-finite-over-rees-algebra}
we see that $\mathcal{G} = \pi_*\mathcal{G}'$ for some coherent
$\mathcal{O}_{X'}$-module $\mathcal{G}'$. (Observe that
$X'$ is a Noetherian scheme as $B$ is a finite type $A$-algebra
and $f'$ is projective hence of finite type.) Since $\pi$ is affine we have
$$
H^1(X', \mathcal{G}' \otimes \pi^*\mathcal{L}^{\otimes d})
=
H^1(X, \mathcal{G} \otimes \mathcal{L}^{\otimes d})
= \bigoplus H^1(X, \text{Ker}(\mathcal{F}_{n + 1} \to \mathcal{F}_n)
\otimes \mathcal{L}^{\otimes d} )
$$
see Lemma \ref{lemma-relative-affine-vanishing} and
Cohomology, Lemma \ref{cohomology-lemma-apply-Leray}.
Hence it suffices to prove that the $H^1$ on the left vanishes
for $d$ sufficiently large. As $\pi^*\mathcal{L}$ is $f'$-relatively
ample, this follows from
...
$\mathcal{I} = I\mathcal{O}_X$.
Then the functor (\ref{equation-formal-existence}) is an equivalence.
\end{lemma}

\begin{proof}
We have already seen that (\ref{equation-formal-existence}) is
...
in the image of (\ref{equation-formal-existence}). 
...
$$
which sends $(\mathcal{G}_n)$ to $(f^*\mathcal{G}_n)$.
\end{lemma}
...
$\textit{Coh}(X, \mathcal{I})$, and
$\alpha : \mathcal{G}^\wedge \to (\mathcal{F}_n)$
a map whose kernel and cokernel are annihilated by a power of $\mathcal{I}$.
Then $(\mathcal{F}_n)$ is in the essential image of
(\ref{equation-formal-existence}).
\end{lemma}

\begin{proof}
We claim there exists a unique (up to unique isomorphism) triple
$(\mathcal{F}, a, \beta)$ where $a : \mathcal{G} \to \mathcal{F}$
is an $\mathcal{O}_X$-module map,
$\beta : \mathcal{F}^\wedge \to (\mathcal{F}_n)$ is an isomorphism, and
...
Thus we see that $(M', N_{f_i}, \varphi_{f_i}^{-1}, can_{ij})$ is an
...
corresponds to an $A$-module map $N \to M$ (by the same proposition).
The composition of $N \to M$ with the
isomorphism $M \otimes_A A^\wedge \to M'$ is $\varphi$.
...
Let $i_e : X_e \to X$ be the closed subscheme cut out by $\mathcal{K}^e$.
Let $\mathcal{I}_e = i_e^{-1}\mathcal{I}\mathcal{O}_{X_e}$.
...
$\alpha : \mathcal{H}^\wedge \to (\mathcal{F}_n)$ whose
...
by the ideal sheaf of $Z$. Let $\mathcal{G}_e$ be the coherent sheaf on
$X_e$ corresponding to the object $(i_e^*\mathcal{F}_n)$ of
$\textit{Coh}(X_e, \mathcal{I}_e)$. The maps
$i_{e + 1}^*\mathcal{F}_n \to i_e^*\mathcal{F}_n$ induce canonical maps
$\mathcal{G}_{e + 1} \to \mathcal{G}_e$. A local computation shows that
these map induce isomorphisms
$\mathcal{G}_{e + 1}/\mathcal{K}^e\mathcal{G}_{e + 1} \to \mathcal{G}_e$.
Hence $(\mathcal{G}_e)$ is an object of $\textit{Coh}(X, \mathcal{K})$.

\medskip\noindent
Consider the map $\alpha$. It induces compatible systems of maps
$i_e^*\mathcal{H}/\mathcal{I}^n\mathcal{H} \to i_e^*\mathcal{F}_n$.
Whence a compatible system of maps $i_e^*\mathcal{H} \to \mathcal{G}_e$
(by the assumed equivalence of categories again).
Let $t$ be the integer such that $\mathcal{K}^t$ annihilates
the kernel and cokernel of the maps
$\mathcal{H}/\mathcal{I}^n\mathcal{H} \to \mathcal{F}_n$.
Then $\mathcal{K}^t$ also annihilates the kernel and cokernel of the
maps $i_e^*\mathcal{H}/\mathcal{I}^n\mathcal{H} \to i_e^*\mathcal{F}_n$
and hence of the maps $i_e^*\mathcal{H} \to \mathcal{G}_e$.
Thus we may apply Lemma \ref{lemma-existence-easy}.
\end{proof}

\begin{lemma}
\label{lemma-inverse-systems-push-pull}
Let $Y$ be a Noetherian scheme. Let $\mathcal{K} \subset \mathcal{O}_Y$
be a quasi-coherent sheaf of ideals.
Let $f : X \to Y$ be the blowing up of $Y$ in $\mathcal{K}$.
Let $\mathcal{J} \subset \mathcal{O}_Y$ be a quasi-coherent sheaf
of ideals and set $\mathcal{I} = f^{-1}\mathcal{J} \mathcal{O}_X$.
Let $(\mathcal{G}_n)$ be an object of $\textit{Coh}(Y, \mathcal{J})$,
$\mathcal{F}$ a coherent sheaf on $X$, and
$\beta : \mathcal{F}^\wedge \to (f^*\mathcal{G}_n)$ an isomorphism in
$\textit{Coh}(X, \mathcal{I})$. Then there exists an integer $t \geq 0$
and a map
$$
\alpha : (\mathcal{K}^t f_*\mathcal{F})^\wedge \to (\mathcal{G}_n)
$$
...
\begin{proof}
The map $\alpha$ corresponds to a compatible system of maps
$\alpha_n : \mathcal{K}^t f_*\mathcal{F} \to \mathcal{F}_n$.
We will show there exists a unique compatible system such that the
diagrams
$$
\xymatrix{
\mathcal{G}_n \ar[r] & f_*f^*\mathcal{G}_n \\
\mathcal{K}^t f_*\mathcal{F} \ar[r] \ar[u]^{\alpha_n} &
f_*\mathcal{F} \ar[u]_{f_*\beta_n}
}
$$
are commutative (this is not quite right).
By uniqueness we may work locally on $Y$.
Hence we may assume that $Y = \Spec(B)$ and that $\mathcal{J}$
corresponds to the ideal $J \subset B$. In this situation we see
that $(\mathcal{G}_n)$ corresponds to a finite $B^\wedge$-module $N$, see
Lemma \ref{lemma-inverse-systems-affine}.
Consider the fibre product diagram
$$
\xymatrix{
X' = \Spec(B^\wedge) \times_{\Spec(B)} X \ar[r] \ar[d]_{f'} & X \ar[d] \\
\Spec(B^\wedge) \ar[r] & \Spec(B)
}
$$
To be continued...
\end{proof}
