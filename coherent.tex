...
\begin{proof}
In this case all higher cohomology is zero (of any abelian sheaf),
because taking global section is exact. Namely,
$\Gamma(X, \mathcal{F}) = \prod_{x \in X} \mathcal{F}_x$ in this case.
Hence Lemma \ref{lemma-quasi-compact-h1-zero-covering} applies.
\end{proof}
...
\item In fact, we can find $n = n(X, S, f)$ such that for every
...
defined in exactly the same manner as in the previous lemma.
...
Lemma \ref{lemma-separated-case-relative-cech} one sees that there
is an isomorphism
$$
\check{\mathcal{C}}^\bullet(\mathcal{U}', f', \mathcal{F}')
\longrightarrow
...
In fact, there exists a closed subscheme $i : Z \to X$
whose underlying closed subset agrees with $\text{Supp}(\mathcal{F})$
such that $\mathcal{F} \cong i_*\mathcal{G}$
for some coherent $\mathcal{O}_Z$-module $\mathcal{G}$.
\end{lemma}

\begin{proof}
The closedness of the support
holds more generally (Lemma \ref{lemma-coherent-Noetherian})
for any $\mathcal{O}_X$-module of finite type on any
ringed space, see
Modules, Lemma \ref{modules-lemma-support-finite-type-closed}.
To see the second statement, consider the sheaf of ideals
$$
\mathcal{I}
=
...
$$
By a local calculation on affines, or
since $\mathcal{F}$ is of finite presentation
(Lemma \ref{lemma-coherent-Noetherian})
and Modules, Lemmas \ref{modules-lemma-internal-hom-locally-kernel-direct-sum}
and \ref{modules-lemma-coherent-abelian}
we see that $\mathcal{I}$ is a coherent sheaf of ideals, in particular
quasi-coherent. Denote $i : Z \to X$ the associated closed subscheme
(Morphisms, Lemma \ref{morphisms-lemma-closed-immersion-bijection-ideals}).
By Morphisms, Lemma \ref{morphisms-lemma-i-star-equivalence} and
the definition of $\mathcal{I}$ we see that $\mathcal{F} = i_*\mathcal{G}$
for some quasi-coherent sheaf $\mathcal{G}$ on $Z$. It follows
that $\mathcal{G}$ is a finite type $\mathcal{O}_Z$-module
from the fact that $\mathcal{F}$ is a finite type $\mathcal{O}_X$-module
(proof omitted). Since $Z$ is locally Noetherian
(Morphisms, Lemmas \ref{morphisms-lemma-immersion-locally-finite-type}
and \ref{morphisms-lemma-finite-type-noetherian}) it
follows that $\mathcal{G}$ is a coherent $\mathcal{O}_Z$-module.
Finally, by Algebra, Lemma \ref{algebra-lemma-support-closed}
we see that $Z = \text{Supp}(\mathcal{F})$ as desired.
\end{proof}
...
Lemma \ref{lemma-coherent-support-closed}. The others follow from
...
Cohomology, Lemma \ref{cohomology-lemma-localize-higher-direct-images}).
Hence we may assume $S = \text{Spec}(R)$ is the spectrum of
...
Let $S = \text{Spec}(A)$ with $A$ a Noetherian ring.
Let $f : X \to S$ be a proper morphism.
Let $\mathcal{F}$ be a coherent $\mathcal{O}_X$-module.
...
\medskip\noindent
If $Y = \text{Spec}(A)$ and $\mathcal{I} = \widetilde{I}$ we denote
...
Let $f : X \to \text{Spec}(A)$ be a proper morphism.
Let $\mathcal{F}$ be a coherent sheaf on $X$.
...
$$
\xymatrix{
X' = \text{Spec}(S) \times_{\text{Spec}(A)} X
\ar[r] \ar[d]_{f'} &
X \ar[d]^f \\
\text{Spec}(S) \ar[r] &
\text{Spec}(A)
...
is contained in $I^{m - c} H^p(X, I^n\mathcal{F})$
...
\longrightarrow
\text{lim}_n\ H^p(X, \mathcal{F}/I^n\mathcal{F})
$$
...
$$
Pick an element $x = (x_n) \in \text{lim}_n\ M_n$.
...
defines an element of $\text{lim}_n\ M/I^nM$.
We omit the verification that $y$ maps to $x$ under the
map
$$
M^\wedge = \text{lim}_n\ M/I^nM \longrightarrow \text{lim}_n\ M_n
$$
...
\xymatrix{
X_n =
\text{Spec}(\mathcal{O}_{Y, y}/\mathfrak m_y^n) \times_Y X
\ar[r]_-{i_n} \ar[d]_{f_n} &
X \ar[d]^f \\
\text{Spec}(\mathcal{O}_{Y, y}/\mathfrak m_y^n) \ar[r]^-{c_n} & Y
}
$$
...
\cong
\text{lim}_n\ H^p(X_n, \mathcal{F}_n)
$$
...
\medskip\noindent
Hence we may assume that $Y = \text{Spec}(A)$ is the spectrum of
...
Lemma \ref{lemma-scheme-finite-discrete-affine}.
...
\begin{proof}
Assume $f$ is proper with finite fibres. We want to show $f$ is
finite. In fact it suffices to prove $f$ is affine,
to be continued...
