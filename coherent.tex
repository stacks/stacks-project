...
third.
\item For every integral closed subscheme $Z \subset X$ there exists
...
This follows from the two-out-of-three property of $\mathcal{P}$.

\medskip\noindent
Let $Z \subset X$ be a closed subset such that the result holds for
every coherent sheaf whose support is strictly contained in $Z$.
Let $\mathcal{F}$ be a coherent sheaf with support $Z$.
Choose a filtration of $\mathcal{F}$ as in Lemma \ref{lemma-coherent-filter}.
Then property $\mathcal{P}$ holds for all of the subquotients
$\mathcal{F}_i/\mathcal{F}_{i - 1} = (Z_i \to X)_*(\mathcal{I}_i)$
who are supported on integral closed subschemes $Z_i$ strictly contained
in $Z$. Hence we see that we only get into trouble if for some $i$
we have $Z_i = Z$. In particular this means that $Z$ is an irreducible
closed subset. Let $\mathcal{G}$ be a coherent sheaf as in condition (2)
of the lemma adapated to our $Z$. By
Lemma \ref{lemma-prepare-filter-irreducible} we can find a sheaf
of ideals $\mathcal{I}$ on $Z$ and a short exact sequence
$$
0 \to (Z \to X)_*\mathcal{I} \to \mathcal{G} \to \mathcal{Q} \to 0
$$
where the support of $\mathcal{Q}$ is strictly contained in $Z$.
After replacing $\mathcal{I}$ by
$$
\mathcal{I} \cap \bigcap\nolimits_{i,\ Z_i = Z} \mathcal{I}_i
$$
we find that $\mathcal{I} \subset \mathcal{I}_i$ and we get
short exact sequences
$$
0 \to (Z \to X)_*\mathcal{I} \to (Z_i \to X)_*\mathcal{I}_i \to
\mathcal{Q}_i \to 0
$$
for those $i$ with $Z_i = Z$
where the support of $\mathcal{Q}_i$ is strictly contained in $Z$.
Putting everything together we see $\mathcal{Q}$ and $\mathcal{Q}_i$
have property $\mathcal{P}$ and hence by two-out-of-three so does
$(Z \to X)_*\mathcal{I}$ hence so does each $(Z_i \to X)_*\mathcal{I}_i$
and hence so does $\mathcal{F}$.

\medskip\noindent
Since $X$ is Noetherian we can use the mechanism of Noetherian
induction to see that the result of the preceding paragraph implies
the lemma (compare with the proof of Lemma \ref{lemma-coherent-filter}).
\end{proof}

\begin{lemma}
\label{lemma-property-bigger-rank}
Let $X$ be a Noetherian scheme.
Let $\mathcal{P}$ be a property of coherent sheaves on $X$ such that
\begin{enumerate}
\item For any short exact sequence of coherent sheaves if two
...
\begin{proof}
Let $Z \subset X$ be irreducible closed.
Consider a coherent sheaf $\mathcal{G}$ as in (3) adapted to $Z$.
By Lemma \ref{lemma-prepare-filter-irreducible}
there exists a sheaf of ideals $\mathcal{I}$ on $Z$ and
a short exact sequence
$$
0 \to
\left((Z \to X)_*\mathcal{I}\right)^{\oplus r} \to
\mathcal{G} \to
\mathcal{Q} \to 0
$$
where the support of $\mathcal{Q}$ is stricly contained in $Z$.
Hence if property $\mathcal{P}$ is known to hold for all sheaves
whose support is properly contained in $Z$, then by conditions
(1), (2) and (3) we deduce property $\mathcal{P}$ for
$(Z \to X)_*\mathcal{I}$. Slotting this into the proof of
Lemma \ref{lemma-property-rank-1} at the appropriate point gives the lemma.
