...
are affine and hence
$R^q(j_{i_0 \ldots i_p})_*\mathcal{F}_{i_0 \ldots i_p}$
...
$g : S' \to S$ is any morphism of schemes. Denote
...
that $S$ is quasi-compact and separated. In particular $X$
...
$\mathcal{F}_\bullet$ the restriction of $\mathcal{F}$ to $U_\bullet$.
The pushforward $(f_\bullet)_*\mathcal{F}_\bullet$ will be a
cosimplicial quasi-coherent sheaf on $S$ whose associated cochain
complex computes $Rf_*\mathcal{F}$. In other words, this is completely
analogous to the result in Lemma \ref{lemma-separated-case-relative-cech}.
Moreover, this complex satisfies
a base-change property similar to the one observed in
Remark \ref{remark-base-change-complex}.
\end{remark}
