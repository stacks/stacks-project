...
\begin{proof}
Choose an affine open neighbourhood $\text{Spec}(A) = U \subset X$
of $\xi$ with $Z' \cap U = \emptyset$.
Let $\mathfrak p \subset A$ be the prime ideal corresponding
to $\xi$. Since $\mathcal{F}$ is quasi-coherent there is an
$A$-module $M$ such that $\widetilde{M} = \mathcal{F}|_U$.
Since $X$ is Noetherian we see that $A$ is Noetherian and since
$\mathcal{F}$ is coherent we see that $M$ is a finite $A$-module.
By assumption $\mathfrak p M_{\mathfrak p} = 0$. Hence there exists
a $f \in A$, $f \not \in \mathfrak p$ such that $\mathfrak p M_f = 0$.
After replacing $A$ by $A_f$ we may assume that $\mathfrak p M = 0$.
In particular $M \otimes \kappa(\mathfrak p) = M_{\mathfrak p}$.
Let $r = \dim_{\kappa(\mathfrak p)} M_{\mathfrak p}$. This is a finite
integer as $M$ is a finite $A$-module. We may choose
$m_1, \ldots, m_r \in M$ whose images in $M_{\mathfrak p}$ form
a $\kappa(\mathfrak p)$-basis. This gives an $A$-module map
$$
(A/\mathfrak p)^{\oplus r} \longrightarrow M
$$
which becomes an isomorphism after localizing at $\mathfrak p$.
Hence we may invert another element in $A$ and assume that in fact
$(A/\mathfrak p)^{\oplus r} \to M$ is an isomorphism.

\medskip\noindent
Translating back to coherent sheaves, we see that we have found
an affine open $U \subset X$ containing $\xi$ and an isomorphism
$$
i'_*\left(\mathcal{O}_{Z \cap U}^{\oplus r}\right)
\longrightarrow
\mathcal{F}|_U
$$
where $i' : Z \cap U \to U$ is the inclusion morphism.
Consider the coherent sheaf
$i_*(\mathcal{O}_Z^{\oplus r}) \oplus \mathcal{F}$.
...
lemma does hold (with $m = 0$). Thus we may choose an affine open
$\text{Spec}(A) = U \subset X$ with $Z \cap U \not = \emptyset$.
Since $X$ is Noetherian we see that $A$ is a Noetherian ring, and
since $\mathcal{F}$ is coherent we see that $\mathcal{F}|_U = \widetilde{M}$
for some finite $R$-module $M$.
By Algebra, Lemma \ref{algebra-lemma-filter-Noetherian-module}
there exists a filtration by $A$-module
$0 = M_0 \subset M_1 \subset \ldots \subset M_r = M$ such that
each $A$-module $M_j/M_{j - 1}$ is of the form $A/\mathfrak p_j$
for some prime $\mathfrak p_j \subset A$. Let
$$
0 = \mathcal{G}_0 \subset \mathcal{G}_1 \subset \ldots
\subset \mathcal{G}_r = \mathcal{F}|_U
$$
be the corresponding filtration by quasi-coherent subsheaves
of $\widetilde{M} = \mathcal{F}|_U$, i.e., 
$\mathcal{G}_j = \widetilde{M_j}$.
Let $\xi_j \in U \subset X$ be the point corresponding to the prime
$\mathfrak p_j \subset A$. Let $Z_j = \overline{\{\xi_j\}}$ be the
corresponding integral closed subscheme.
By Properties, Lemma \ref{properties-lemma-extend-trivial}
we can find a filtration
by quasi-coherent subsheaves
$$
0 = \mathcal{F}_0 \subset \mathcal{F}_1 \subset \ldots
\subset \mathcal{F}_r = \mathcal{F}
$$
such that $\mathcal{F}_j|_U = \mathcal{G}_j$.
Finally, set $Z' = Z \setminus U$ which is a proper
closed subset of $Z$.

\medskip\noindent
Consider the subquotients $\mathcal{Q}_j = \mathcal{F}_j/\mathcal{F}_{j - 1}$.
The support of $\mathcal{Q}_j$ is contained in $Z' \cup Z_j$ by
construction. Note that $Z_j \not \subset Z'$.
By Lemma \ref{lemma-prepare-filter} we can find exact sequences
$$
0 \to \mathcal{Q}'_j \to \mathcal{Q}_j \to \mathcal{Q}''_j \to 0
$$
such that $\mathcal{Q}''_j$ is supported on $Z_j$ and
$\mathcal{Q}'_j$ is supported on $Z'$. Of course this means there
is a corresponding filtration of $\mathcal{F}$ whose successive
quotients are
$$
\mathcal{Q}'_1, \mathcal{Q}''_1, \ldots, \mathcal{Q}'_r, \mathcal{Q}''_r.
$$
By minimality of $Z$ the sheaves $\mathcal{Q}'_j$ each have a filtration
as in the lemma. Moreover, the sheaves $\mathcal{Q}''_j$ have such
a filtration too unless $Z_j = Z$.
...
of any quasi-coherent sheaf vanish. It turns out that this also
...
\begin{lemma}
\label{remark-quasi-separated-h1-zero-covering}
