...
has a finitely generated ideal of definition. Since we have already introduced
the terminology ``taut'' for continuous ring maps, we will not introduce the
concept of an ``adic map between adic rings''.
...
\begin{remark}[Tautness for adic-star]
\label{remark-taut-adic-star}
For a morphism $\varphi : A \to B$ of $\textit{WAdm}^{adic*}$
the condition ``$\varphi$ is taut'' is equivalent to
\begin{itemize}
\item[$(*)$] the topology on $B$ is $I$-adic for some ideal
of definition $I$ of $A$.
\end{itemize}
This follows from Lemma \ref{lemma-taut-is-adic}.
\end{remark}
