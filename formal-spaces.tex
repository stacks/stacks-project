...
complete linearly topologized ring. The kernel $I$ of the surjection
$A^\wedge \to A/JA$ is the closure of $JA$. If $J' \subset J$
is another weak ideal of definition, then $J/J' \subset B/J'$
is locally nilpotent, hence $\Im(I \to A/J'A) = JA/J'A \subset A/J'A$
is locally nilpotent (Algebra, Lemma \ref{algebra-lemma-locally-nilpotent}).
Thus, if $f \in I$, then
some power of $f$ maps to zero in $A/J'A$. Thus $f$ is topologically
nilpotent in $A^\wedge$ and $I$ is a weak ideal of definition of
$A^\wedge$, i.e., $A^\wedge$ is a weakly admissible topological ring.
Thus
$$
...
Hence a flat ring map $A \to B$ is faithfully flat if and only if
$A/J \to B/JB$ is faithfully flat. In this way we see that (2) -- (4)
are equivalent. It is clear that (1) implies (4). Conversely, (3)
implies (1) because every morphism $T \to Y$ with $T$ quasi-compact
factors through $\Spec(B/J)$ for some ideal of definition $J$ of $B$
and hence $X \times_Y T = \Spec(A/JA) \times_{\Spec(B/J)} T$
which is surjective over $T$ as a base change of the morphism
$\Spec(A/JA) \to \Spec(B/J)$. Some details omitted.
\end{proof}
...
representable and \'etale, the morphism $U \to V$ corresponds
...
is an affine formal algebraic space and $Y_i \to Y$ corresponds
to a morphism of $\textit{WAdm}^{count}$ with property $P$.
\end{enumerate}
\end{lemma}
...
\begin{example}
\label{example-representable-local-property}
Let $P$ be the property of morphisms of $\textit{WAdm}^{count}$
defined by the rule
$$
P(B \to A) =
\begin{matrix}
\text{for every weak ideal of definition }J \subset B
\text{ the closure of the} \\
\text{ ideal }JA \text{ is a weak ideal of definition of }A
\text{ and these ideals} \\
\text{ form a fundamental system of weak ideals of definition of }A.
\end{matrix}
$$
Then we claim this is a local property and under the assumptions
of Lemma \ref{lemma-property-defines-property-morphisms}
the equivalent conditions (1), (2), and (3) are
alse equivalent to the condition
\begin{enumerate}
\item[(4)] $f$ is representable by algebraic spaces.
\end{enumerate}
This follows from Lemmas \ref{lemma-representable-affine}
and \ref{lemma-representable-by-algebraic-spaces-local}; some details omitted.
\end{example}
