...
There are multiple possible notions of morphisms of finite type
between formal algebraic spaces. We could define a finite type morphism
of formal algebraic spaces to be a morphism $f : X \to Y$
which is representable by algebraic spaces and which satisfies
the equivalent conditions of Lemma \ref{lemma-characterize-finite-type}.
On the other hand, suppose that $A$ is a (weakly) admissible topological
ring. Let $I_\lambda$ be a fundamental system of (weak)
ideals of definition, so that $A = \lim A/I_\lambda$.
Then a natural condition for an affine formal algebraic space $X$
to be of finite type over $\text{Spf}(A)$ would be to ask that
$X = \text{Spf}(B)$ where $B$ is a weakly admissible topological ring
which is a quotient of
$$
A\{x_1, \ldots, x_n\} = \lim (A/I_\lambda)[x_1, \ldots, x_r]
$$
by a closed ideal. However, the first definition given above instead
translates into the condition that $X = \colim \Spec(B_\lambda)$ where
\begin{enumerate}
\item $A/I_\lambda \to B_\lambda$ is of finite type for all $\lambda$,
\item for $\lambda \leq \mu$ the ring map $B_\mu \to B_\lambda$
is an $A/I_\mu$-algebra map which induces an isomorphism
$B_\lambda = B_\mu/I_\lambda B_\mu$.
\end{enumerate}
...
all $\lambda$ (hint: use
Algebra, Lemma \ref{algebra-lemma-surjective-mod-locally-nilpotent}). However,
it is not clear (and we think not true in general) that we can recover
$B_\lambda$ as the quotient of $B = \lim B_\lambda$ by $I_\lambda$
nor is it clear that $B$ is a quotient of $A\{x_1, \ldots, x_r\}$.
In particular, it is not clear that $X$ is a classical affine
...
$$
is an isomorphism (see More on Morphisms of Schemes, Lemma
