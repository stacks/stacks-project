...
(this also uses that a thickening is a closed immersion which
is surjective and universally injective and hence satisfies
the conditions of the theorem). Moreover, these diagrams are
...
$R = Y \times_X Y \to Y$ and $R_\lambda$ is the restriction of $R$ to
$Z_\lambda$). Hence $X_\lambda \to X$ is representable and a closed
immersion, see Spaces, Lemma
...
X = \colim X_\lambda
$$
This is true because $Y \times_X X_\lambda = Z_\lambda \supset Y_\lambda$
and hence $Y = \colim Z_\lambda$. Every morphism $T \to X$ where $T$
is a scheme over $S$ lifts \'etale locally to a morphism into $Y$
which lifts \'etale locally into a morphism into some $Y_\lambda$.
...
into the language of formal schemes. Let $\mathfrak X$ be a formal scheme.
An {\it ideal of definition} for $\mathfrak X$ is an ideal sheaf
$\mathcal{I} \subset \mathcal{O}_\mathfrak X$ such that
$(\mathfrak X, \mathcal{O}_\mathfrak X/\mathcal{I})$ is a
scheme. A {\it fundamental system of ideals of definiton}
...
admissible topological ring $A$. Clearly, ideals
of definition correspond to representable morphisms
$X \to h_\mathfrak X$ which are thickenings.
In case $\mathfrak X$ is quasi-compact, then a
fundamental system of ideals of definition
corresponds to a system $X_\lambda \to h_\mathfrak X$ of
such morphisms such that $h_\mathfrak X = \colim X_\lambda$
where the transition maps come from inclusions between
the different members $X_\lambda$. Having said this we
see that Lemma \ref{lemma-structure-quasi-compact-quasi-separated}
...
\end{list}
We now give an alternative argument for this, which at the
core uses the same trick as used in the proof of
Lemma \ref{lemma-structure-quasi-compact-quasi-separated}.
We first use the induction principle (reformulated for
...
with a closed immersion $Z \to \mathfrak X$.
Note that $U \subset Z_U \subset Z$ and that
$V \subset Z_V \subset Z$. Thus the collection of all $Z$
constructed in this manner forms a family of closed subschemes
such that $\mathfrak X = \colim Z$ and the proof is complete.
\end{remark}
