...
\ref{spaces-more-morphisms-theorem-topological-invariance} and Lemma
...
\noindent
Let $X$ be a formal algebraic space.
In this section we try to convince the reader that looking for
a structure sheaf, more precisely a sheaf of rings
$\mathcal{O}_X$ on the \'etale site $X_\etale$
with good properties, is a somewhat pointless endevour
without imposing further conditions on $X$.
...
\medskip\noindent
A possible and reasonable definition of the structure sheaf
on a McQuillan affine formal algebraic space can be given
as follows. Let $A$ be a weakly admissible
topological ring and set $X = \text{Spf}(A)$.
We have seen in Lemma \ref{lemma-affine-etale-mcquillan}
that the category $(X_{affine, \etale})^{opp}$ is equivalent to
the category of completions $C = B^\wedge$ of \'etale $A$-algebras $B$.
Thus it would make sense to define the structure sheaf
$\mathcal{O}_X$ in this case by the rule
$$
X_{affine, \etale} \mapsto \textit{Rings},\quad
U = \text{Spf}(C) \longmapsto \mathcal{O}_X(U) = C
$$
This is indeed a sheaf. Let us check the sheaf condition
in a special case. Namely, suppose $U = \text{Spf}(C)$
where $C = B^\wedge$ and $A \to B$
is an \'etale ring map. Then $U \to X$ is surjective if
and only if $A \to B$ is faithfully flat, see
Lemma \ref{lemma-etale-surjective}.
In this case the complex
$$
0 \to A \to B \to B \otimes_A B \to B \otimes_A B \otimes_A B \to \ldots
$$
is universally exact, see Descent, Lemma \ref{descent-lemma-ff-exact}.
Hence for every weak ideal of definition $J \subset A$ the complex
$$
0 \to A/J \to B/JB \to
(B \otimes_A B)/J(B \otimes_A B) \to \ldots
$$
is exact. Because taking limits is left exact
we conclude that
$$
0 \to A^\wedge \to B^\wedge \to (B \otimes_A B)^\wedge
$$
is exact. Translating back to the definition of $\mathcal{O}_X$
given above we find that
$$
0 \to \mathcal{O}_X(X) \to \mathcal{O}_X(U) \to \mathcal{O}_X(U \times_X U)
$$
is exact (small detail omitted).
In other words, we find that $\mathcal{O}_X$ satisfies the sheaf
condition for the covering $\{U \to X\}$ of $X_{affine, \etale}$.
A similar argument works for other coverings
in the site $X_{affine, \etale}$.

\medskip\noindent
Let $X$ be a McQuillan affine formal algebraic space with structure
sheaf $\mathcal{O}_X$ as defined above.
Suppose we want to continue and study the higher cohomology
groups
$$
H^p(X_\etale, \mathcal{O}_X) = H^p(X_{affine, \etale}, \mathcal{O}_X)
$$
A key feature in the development of schemes is that these cohomology
groups are zero for $p > 0$. Now in our case this is not so clear.
Going back to our covering $\{U \to X\}$ associated to the faithfully
flat \'etale ring map $A \to B$ above, we'd need to show that
$$
H^p(0 \to A^\wedge \to B^\wedge \to (B \otimes_A B)^\wedge
...
every weak ideal of definition of $A$. In other words, the
topology on $M_i$ as a submodule of $B^{\otimes_A i + 1}$
or as a quotient module of $B^{\otimes_A i}$ is the same.
Therefore, if there exists a countable fundamental system
of weak ideals of definition in $A$, then the sequences
$$
0 \to A^\wedge \to B^\wedge \to M_1^\wedge \to 0,\quad
0 \to M_i^\wedge \to (B^{\otimes_A i + 1})^\wedge \to M_{i + 1}^\wedge \to 0
$$
remain exact by Lemma \ref{lemma-ses}. Having said this, the usual
arguments (using Cohomology on Sites, Lemma
\ref{sites-cohomology-lemma-cech-vanish-collection}) will show
that
$$
H^p(X_\etale, \mathcal{O}_X) = 0,\quad p > 0
$$
(There is a version where given an $A$-module $M$ one
computes cohomology of the $\mathcal{O}_X$-module associated to $M$.
If we ever need this, we will formulate a precise lemma and
give a detailed proof here.) In other words, if $X$ is a countably indexed
affine formal algebraic space, then the structure sheaf
has vanishing higher cohomology groups.
On the other hand, for non-countably indexed ones, all bets are off.
