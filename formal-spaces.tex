...
With $A, I, \mathfrak a, \bar I$ as in Remark \ref{remark-take-bar}.
Let $B$ be in (\ref{equation-C-prime}) for $(A, I)$.
Let $\bar B$ be the base change of $B$.
Suppose that multiplication by $f \in B$ on $\NL^\wedge_{B/A}$
is zero in $D(B)$. Then multiplication by
...
\begin{proof}
Since $B^\wedge$ is Noetherian, we see that $J$ is a finitely generated
...
applied to $B \to B^\wedge$ and the ideal $IB$, we conclude that
$C \times C'$ is isomorphic to $D \otimes_B B^\wedge$ for some finite
$B$-algebra $D$. (The reader can also prove this using
...
is $I$-adically complete hence henselian
(More on Algebra, Lemma \ref{more-algebra-lemma-complete-henselian})
the map $B \to C \times C'$ factors through $B'$.
Thus we may replace $B$ by $B'$ and $D$ by $D'$. Then
...
\begin{reference}
Special case of \cite[III Theorem 7]{Elkik}.
\end{reference}
Let $A$ be a Noetherian ring and $I = (a)$ a principal ideal.
