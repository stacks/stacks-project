...
\end{enumerate}
In particular the map $A[0] \to I^\bullet$ is a quasi-isomorphism, in 
other words the complex
$$
...
such that
\begin{enumerate}
\item $\tilde \alpha$ is a termwise split injection,
...
\noindent
There are other definitions of the derived category which are
more general (namely $\mathcal{A}$ can be an arbitrary abelian
category). Namely, the idea is that one takes the category
...
\medskip\noindent
To discuss the meaning of this category we remark that since
homotopic maps induce the same maps on cohomology there is a
...
At this point we have constructed the derived category
without mentioning distinghuished triangles. One reason for
introducing distinghuished triangles is to be able to speak
about exact functors between derived categories. Namely,
allthough the derived category $D^{+}(\mathcal{A})$ is
an additive category, it is not an abelian category itself (in most cases).
Hence it not possible to express what it means for a complex
in $D^{+}(\mathcal{A})$ to be exact. The solution to this
conundrum is to add additional structure to the category $D^{+}(\mathcal{A})$,
namely a set of distinghuished triangles as mentioned above.
This only partially solves the problem however, since it only
allows one to speak about the analogue of a short exact
sequence, and not more general complexes.
\end{remark}
...
{\it separated} if $F^iA = 0$ for $i \gg 0$ and
{\it exhaustive} if $F^iA = A$ for $i \ll 0$.
We will say the filtration is said to be {\it finite} if and only if
it is both separated and exhaustive.

\begin{lemma}
\label{lemma-filtered}
Let $\mathcal{A}$ be an abelian category.
The category of filtered objects $\text{Fil}(\mathcal{A})$
is an additive category but in general not an abelian
category.
\end{lemma}

\begin{proof}
Omitted. See Example \ref{example-not-abelian} for the last statement.
\end{proof}
