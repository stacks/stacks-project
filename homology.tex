...
\begin{lemma}
	\label{lemma-morphism-kernel-localization}
	Let $\mathcal{C}$ be a preadditive category.
	Let $S$ be a multiplicative system.
	Let $f:X\to Y$ be a morphism in $\mathcal{C}$.
	The following are equivalent:
	\begin{enumerate}
		\item $Q(f)=0$ in $S^{-1}\mathcal{C}$,
		\item there exists $s:Y\to Z$ in $S$ such that
		$s\circ f=0$ in $\mathcal{C}$, and
		\item there exists $t:W\to X$ in $S$ such that
		$f\circ t=0$ in $\mathcal{C}$.
	\end{enumerate}
\end{lemma}

\begin{proof}
	This follows from Categories, Lemmas
	\ref{lemma-equality-morphisms-left-localization} and
