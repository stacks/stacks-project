...
\begin{enumerate}
\item a chain complex $A_\bullet$ of abelian groups endowed with
$\mathbf{Z}$-bilinear maps $A_n \times A_m \to A_{n + m}$,
$(a, b) \mapsto ab$ such that
$$
d_{n + m}(ab) = d_n(a)b + (-1)^n ad_m(b)
$$
and such that $\bigoplus A_n$ becomes an associative and unital algebra, or
\item a cochain complex $A^\bullet$ of abelian groups endowed with
$\mathbf{Z}$-bilinear maps $A^n \times A^m \to A^{n + m}$, $(a, b) \mapsto ab$
such that
$$
d^{n + m}(ab) = d^n(a)b + (-1)^n ad^m(b)
$$
and such that $\bigoplus A^n$ becomes an associative and unital algebra.
\end{enumerate}
\end{definition}

\noindent
We often just write $A = \bigoplus A_n$ or $A = \bigoplus A^n$ and
think of this as an associative unital algebra endowed with a
...
A differential graded algebra is {\it commutative} if
