...
four morphisms induce an isomorphsm $x = y \oplus z$ as in
...
is that the canonical map $\text{Coim}(f) \to \text{Im}(f)$
...
$\text{Im}(x \to y) = \Ker(y \to z)$. We say it is {\it exact}
...
$j:\text{Im}(f)\to\Ker(g)$ be the canonical injection.
...
Let $p=y\times_z\Ker(\gamma)$ and $q=\text{Coker}(\alpha)\amalg_uv$. 
...
$\delta:\Ker(\gamma)\to\text{Coker}(\alpha)$ with
...
\text{Coim}(\psi)
& =
\text{Coker}(\Ker(\psi) \to A_1)
& \text{by definition} \\
& =
b\text{Coker}(a(\Ker(\psi) \to A_1))
& \text{by formula above} \\
& =
b\text{Coker}(\Ker(a\psi) \to aA_1))
...
& =
b\Ker(aA_2 \to \text{Coker}(a\psi))
& \text{by definition} \\
& =
\Ker(baA_2 \to b\text{Coker}(a\psi))
& b\text{ preserves kernels} \\
& =
\Ker(A_2 \to b\text{Coker}(a\psi))
& ba = \text{id}_\mathcal{A} \\
& =
\Ker(A_2 \to \text{Coker}(\psi))
& \text{by formula above} \\
& =
\text{Im}(\psi)
& \text{by definition}
...
\Ker(f), \text{Coker}(f) \in \Ob(\mathcal{C}) \}.
$$
...
$$
\begin{matrix}
0 \to \Ker(f) \to \Ker(gf) \to \Ker(g) \to 0 \\
...
$\Ker(f) \to X$ and $Y \to \text{Coker}(f)$ are transformed
...
$$
H^0(M, \varphi, \psi) = \Ker(\psi)/\text{Im}(\varphi)
, \quad\text{and}\quad
H^1(M, \varphi, \psi) = \Ker(\varphi)/\text{Im}(\psi).
$$
...
- [\Ker(\psi)] - [\text{Im}(\psi)] \\
& =
[\Ker(\varphi)/\text{Im}(\psi)] -
[\Ker(\psi)/\text{Im}(\varphi)] \\
...
$$
P \cong \Ker(\varphi)/\text{Im}(\psi).
$$
In exactly the same way we see that
$$
Q \cong \Ker(\psi)/\text{Im}(\varphi).
$$
...
the following formula
$$
H_i(A_\bullet) = \Ker(d_i)/\text{Im}(d_{i + 1}).
$$
...
the following formula
$$
H^i(A^\bullet) = \Ker(d^i)/\text{Im}(d^{i - 1}).
$$
...
$\text{gr}(\Ker(\beta)/\text{Im}(\alpha)) =
\Ker(\text{gr}(\beta))/\text{Im}(\text{gr}(\alpha)))$.
\end{lemma}
...
that $d_r \circ d_r = 0$ and $E_{r + 1} = \Ker(d_r)/\text{Im}(d_r)$
for $r \geq 1$.
...
$E_{r + 1} = \Ker(d_r)/\text{Im}(d_r)$
and
$E'_{r + 1} = \Ker(d'_r)/\text{Im}(d'_r)$.
\end{enumerate}
\end{definition}
...
of its predecessor. So $\Ker(\alpha) = \text{Im}(f)$,
$\Ker(f) = \text{Im}(g)$, and $\Ker(g) = \text{Im}(\alpha)$.
...
Then we have
\begin{enumerate}
\item $\Ker(d) = f^{-1}(\Ker(g)) = f^{-1}(\text{Im}(\alpha))$,
\item $\text{Im}(d) = g(\text{Im}(f)) = g(\Ker(\alpha))$,
...
the complex with terms $\Ker(d_1^{p, q})/\text{Im}(d_1^{p - 1, q})$
...
the complex with terms $\Ker(d_2^{p, q})/\text{Im}(d_2^{p, q - 1})$
...
Let $M^\bullet$ be a complex of abelian groups. Let
$$
0 \to M^\bullet \to A_0^\bullet \to A_1^\bullet \to A_2^\bullet
$$
be an exact complex of complexes of abelian groups
such that for all $p \in \mathbf{Z}$ the complexes
$$
0 \to
\text{Coker}(d_{M^\bullet}^p) \to
\text{Coker}(d_{A_0^\bullet}^p) \to
\text{Coker}(d_{A_1^\bullet}^p) \to
\text{Coker}(d_{A_2^\bullet}^p) \to \ldots
$$
...
Note that $x_0$ maps to zero in $\text{Coker}(A_1^{-1} \to A_1^0)$.
...
\medskip\noindent
Assume $x_0 \in \text{Im}(A_0^{-1} \to A_0^0)$. We claim that in this
...
\begin{proof}
Let $Z_i = \Ker(C_i \to D_i)$ and $I_i = \text{Im}(A_i \to B_i)$.
...
of abelian groups. Let $H_i = \Ker(M_i \to N_i)/\text{Im}(L_i \to M_i)$
