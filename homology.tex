...
\begin{enumerate}
\item $f$ is strict,
\item the morphism $\text{Coim}(f) \to \text{Im}(f)$
of Lemma \ref{lemma-coim-im-map} is an isomorphism.
\end{enumerate}
...
$\text{gr}(A) \to \text{gr}(\text{Coim}(f))= \text{gr}(\text{Im}(f))$
is surjective and the map
$\text{gr}(\text{Coim}(f))= \text{gr}(\text{Im}(f)) \to \text{gr}(B)$
is injective.
\end{lemma}
