...
Let $A_\bullet$ be a chain complex
with boundary maps $d_{A, n} : A_n \to A_{n - 1}$.
For any $k \in \mathbf{A}$ we define
$$
H_{i + k}(A_\bullet) \longrightarrow H_i(A[k]_\bullet)
$$
as the isomorphism coming from the identification
$A_{i + k} = A[k]_i$.
\end{definition}
...
For any $k \in \mathbf{A}$ we define
$$
H^{i + k}(A_\bullet) \longrightarrow H^i(A[k]_\bullet)
$$
as the isomorphism coming from the identification
$A^{i + k} = A[k]^i$.
\end{definition}
