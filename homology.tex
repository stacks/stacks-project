...
if it is preadditive and has finite products.
\end{definition}

\begin{remark}
Note that in definition \ref{definition-additive-category}
 $\mathcal{A}$ has direct sums. Also, since $\mathcal{A}$ has finite products it has the empty product, which is a terminal object, hence it also has a zero object.
 \end{remark}
...
Also, all fibre products and pushouts exist in $\mathcal{A}$.

\end{lemma}

\begin{proof}
To show that fibre products and finite limits exist it suffices to show
...
the kernel of $a - b$. The argument for pushouts and finite colimits 
is similar but dual.
\end{proof}
