...
injection. By \ref{example-fibre-product-pushouts}, the diagram is 
...
$i$ is a monomorphism this implies $u\circ e=\text{id}_{\text{Ker}(f)}$. Furthermore, we have $j\circ e\circ u=g\circ i\circ u=g\circ t=j$. 
Since $j$ is a monomorphism this implies $e\circ u=\text{id}_{\text{Ker}(k)}$. This proves (1). Now, (2) follows by duality.
\end{proof}
...
and by \ref{lemma-characterize-cartesian}, the sequence 
$0\to w\overset{u}\to x\oplus y\overset{v}\to z\to 0$ is exact. Thus, the 
diagram is cocartesian by \ref{lemma-characterize-cartesian}. Finally, 
$f$ is an epimorphism by \ref{lemma-cartesian-kernel} and 
\ref{lemma-characterize-injective}. This proves (1), and (2) 
follows by duality.
\end{proof}
...
\begin{proof}
Immediately from \ref{lemma-characterize-injective} and 
\ref{lemma-cartesian-cocartesian}.
\end{proof}
...
epimorphism by \ref{lemma-cartesian-cocartesian}. This implies (2).
...
sequence $\text{Coker}(\alpha) \to \text{Coker}(\beta) \to \text{Coker}(\gamma)$ 
is exact.
\end{enumerate}
\end{lemma}
...
\ref{lemma-check-exactness-fibre-product} there are an object $t$, an 
...
\ref{lemma-check-exactness-fibre-product} implies (1), and then 
...
\ref{lemma-cartesian-cocartesian}, uniqueness of $\delta$ is clear.
...
Keeping in mind \ref{lemma-cartesian-cocartesian} we get a commutative 
...
&\text{Ker}(\pi)\ar[r]^j
&p\ar[r]^{\pi}\ar[d]_{\pi'}
&\text{Ker}(\gamma)\ar[d]_i\ar[r]
&0\\
&x\ar[r]^f\ar[d]_{\alpha}
&y\ar[r]^g\ar[d]_{\beta}
&z\ar[d]_{\gamma}\ar[r]
&0\\
0\ar[r]
&u\ar[r]^k\ar[d]_p
&v\ar[r]^l\ar[d]_{\iota'}
&w
&\\
0\ar[r]
&\text{Coker}(\alpha)\ar[r]^{\iota}
&q
&&
}
$$
...
diagram above is cartesian, \ref{lemma-cartesian-kernel} yields an 
...
above is exact, there exists $\delta:\text{Ker}(\gamma)\to\text{Coker}(\alpha)$ with $\delta\circ\pi=p\circ a$. It follows 
...
\ref{lemma-check-exactness-fibre-product} to the exact sequence 
...
applying \ref{lemma-check-exactness-fibre-product} to the exact sequence 
...
\ref{lemma-check-exactness-fibre-product} implies that 
$\text{Ker}(\beta)\overset{g'}\to\text{Ker}(\gamma)\overset{\delta}\to
\text{Coker}(\alpha)$ 
is exact. Then, the claim follows by \ref{lemma-exact-kernel-sequence}
and duality.
\end{proof}
