...
\begin{example}
\label{example-differential-object-self-map}
Let $\mathcal{A}$ be an abelian category.
Let $(A, d)$ be a {\it differential object} of $\mathcal{A}$, i.e., an
object endowed with a selfmap $d$ such that $d \circ d = 0$.
...
$H(A, d) = \text{Ker}(d)/\text{Im}(d)$ its {\it homology}. By the
Snake Lemma \ref{lemma-snake} we get an exact couple
$$
\xymatrix{
...
=
\frac{d^{-1}(\alpha^r A)}
{(\alpha^{r - 1})^{-1}(dA) + \alpha A \cap d^{-1}(\alpha^r A)}.
$$
...
$(E_r, d_r)_{r \geq 0}$ described in
Example \ref{example-differential-object-self-map} above.
\end{definition}
...
Example \ref{example-differential-object-self-map} we get a spectral sequence.
...
$$
(This notation seems quite natural, but seems to be different from the
notation in most places in the literature. Moreover, the literature does
not seem to have a consistent choice of notation either.)
With these choices we see that $B_r \subset E_0$, resp.\ $Z_r \subset E_0$
(as defined in Example \ref{example-differential-object-self-map}) is
equal to
...
{\it converges} if $\text{gr} H(K) = E_{\infty}$.
\end{definition}
...
\begin{definition}
\label{definition-biregular}
Let $\mathcal{A}$ be an abelian category.
Let $(K^\bullet, F)$ be a filtered complex of $\mathcal{A}$.
We say the filtration on $K^\bullet$ is {\it biregular} if
for every $n \in \mathbf{Z}$ the filtration $F$ on $K^n$ is
finite (see Definition \ref{definition-filtered}).
\end{definition}

\noindent
This definition helps to single out a trivial case where the
...
Let $\mathcal{A}$ be an abelian category.
Let $(K^\bullet, F)$ be a filtered complex of $\mathcal{A}$.
The associated spectral sequence converges if
$K^\bullet$ is biregular.
\end{lemma}

\begin{proof}
We will use Lemma \ref{lemma-filtered-complex-ss-converges}.
Fix $p, n \in \mathbf{Z}$. Look at the left hand side of
...
are zero for $i < 0$. Also $R^0F = F$ and the sequence
$(R^iF, \delta)$ forms a universal $\delta$-functor
