...
references an additonial pair of subobjects
...
\begin{proof}
This is clear from the formula given for the diffirential
...
$d_r^{p - r, q + r - 1} : E^{p - r, q + r - 1} \to E_r^{p, q}$
...
$H^*(K^\bullet)$ and we have $\bigcap_p H^n(K^\bullet) = 0$ and
$\bigcup F^p H^n(K^\bullet) = H^n(K^\bullet)$ for all $n$,
