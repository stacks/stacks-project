...
\noindent
Consider morphisms of pairs $f : (X, x) \to (S, s)$.
This means $X$, $S$ are schemes, $x$, $s$ are points,
$f : X \to S$ is a morphism and $f(x) = s$. Two such morphisms
$f$, $f'$ are deemed to give the same morphism of pairs
if and only if $f$ and $f'$ agree in a neighbourhood of $x$.
Let $\tau \in \{Zariski, fpqc, fppf, etale, smooth, syntomic\}$.
We temporarily
introduce the following concept: We say that two morphisms
of pairs $f : (X, x) \to (S, s)$ and $f' : (X', x') \to (S', s')$
are {\it isomorphic locally on the base in the $\tau$-topology},
if there exists a pointed scheme $(S'', s'')$ and morphisms of pairs
...
(S'' \times_{g', S', f'} X', \tilde  x')
$$
over $(S'', s'')$ for some choice of points $\tilde x$ and $\tilde x'$ lying
over $(s'', x)$ and $(s'', x')$.
\end{enumerate}
Finally, we simply say that
...
\item If the morphisms $s$, $t$ are flat then the morphisms of pairs
...
syntomic) then the morphisms of pairs $s : (R, r) \to (U, u)$
