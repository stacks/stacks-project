...
Let $G$ be a group scheme locally of finite type over an algebraically
closed field $k$. Let $g_1, \ldots, g_n \in G(k)$ be $k$-rational points.
...
\begin{proof}
The proof proceeds along the same lines as the proof of the easier
Lemma \ref{lemma-points-in-affine} which suffices for all practical
purposes. Let $G^0 \subset G$ be the closed subgroup scheme found in
...
and similarly for $G \times_k T$, see Lemma \ref{lemma-compact-set-in-affine}.
