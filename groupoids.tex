...
Descent, Definition \ref{descent-definition-fibre-products-simplicial-scheme}.
...
Descent, Definition \ref{descent-definition-cartesian-morphism}.
...
of simplical schemes with low degree morphisms given by
$$
\xymatrix{
...
Lemma \ref{lemma-diagram-pull} (with the roles of $s$ and $t$ reversed)
...
$$
\textit{QCoh}(U) \longrightarrow \textit{QCoh}(U, R, s, t, c)
$$
which is a right adjoint to the forgetful functor
$(\mathcal{F}, \alpha) \mapsto \mathcal{F}$.
\end{lemma}

\begin{proof}
The pushfoward of a quasi-coherent module along a quasi-compact and
...
\medskip\noindent
Instead of verifying the cocycle condition for $\alpha$ directly, we
consider the following diagram of categories
$$
\xymatrix{
\textit{QCoh}(U) \ar[r]
\ar@{=}[d]_{\text{Lemma \ref{lemma-cartesian-modules-with-section}}} &
\textit{QCoh}(U, R, s, t, c)
\ar@{=}[d]^{\text{Lemma \ref{lemma-quasi-coherent-groupoid-simplicial}}} \\
\textit{CartQCoh}((R/U)_\bullet) \ar[r]^{t_{\bullet, *}} &
\textit{CartQCoh}(U_\bullet)
}
$$
with notation as in the references given and with
$t_\bullet : (R/U)_\bullet \to U_\bullet$ as in
Lemma \ref{lemma-quasi-coherent-groupoid-R-cartesian}.
To finish the proof observe that $t_\bullet^*$ corresponds to the functor
$(\mathcal{F}, \alpha) \mapsto \mathcal{F}$ (details omitted), hence the
functor corresponding to $t_{\bullet, *}$ is its right adjoint by
Lemma \ref{lemma-adjoint-functors-cartesian-modules}.
Finally, we omit the verification that the functor is given
by the construction in the first paragraph of the proof.
\end{proof}
...
of $\textit{QCoh}(\mathcal{O}_X)$, see
...
$\mathcal{F} \to t_*s^*\mathcal{F}$ in $\textit{QCoh}(U, R, s, t, c)$.
...
$\textit{QCoh}(U, R, s, t, c)$ also as restriction to $U$ is an
...
$\textit{QCoh}(U, R, s, t, c)$. This submodule contains $\sigma$
...
$$
Write $U_i = \text{Spec}(A_i)$, $U_{ijk} = \text{Spec}(A_{ijk})$,
$W_{ijk} = \text{Spec}(B_{ijk})$.
...
$\tau \in  \{Zariski, \acute{e}tale, fppf, smooth, syntomic\}$
the diagram
$$
\xymatrix{
