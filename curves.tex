...
If $I \subset \mathfrak m$ is an ideal such that $A/I$ is reduced,
then either $I = (0)$, $I = \mathfrak m$, or $I = (f)$ for some
nonzero $f \in \mathfrak m$.
\end{lemma}
...
If $f$ is a unit, then since $I$ is radical
(equivalent to reducedness of $A/I$) we see that $I = \mathfrak m$.
If $f \in \mathfrak m$, then we see that $f^n$ maps to
zero in $A/I$. Hence $f \in I$ by reducedness and we conclude
$I = (f)$.
\end{proof}
...
complete local Noetherian $k$-algebra. Assume
\begin{enumerate}
\item $A$ is reduced,
\item $\dim_\kappa(\mathfrak m/\mathfrak m^2) = 2$,
\item $\kappa/k$ is separable, and
\item there exists a nondegenerate
$q \in \text{Sym}^2_\kappa(\mathfrak m/\mathfrak m^2)$
which maps to zero in $\mathfrak m^2/\mathfrak m^3$.
\end{enumerate}
Then $A \cong \kappa[[x, y]]/(ax^2 + bxy + cy^2)$ as a $k$-algebra where
$ax^2 + bxy + cy^2$ is a representation of $q$ in coordinates.
\end{lemma}

\begin{proof}
By Algebra, Proposition
\ref{algebra-proposition-characterize-separable-field-extensions}
the extension $\kappa/k$ is formally smooth. Hence we can
find a $k$-algebra map $\kappa \to A$ such that $\kappa \to A \to \kappa$
is the identity, see
More on Algebra, Lemmas \ref{more-algebra-lemma-lift-continuous} and
...
Since $A$ is reduced, by Lemma \ref{lemma-reduced-quotient-regular-ring-dim-2}
...
and this would contradict assumption (2). Hence $Q$ generates
...
Let $k$ be a field. Let $(A, \mathfrak m, \kappa)$ be a
complete local Noetherian $k$-algebra. The following are equivalent
\begin{enumerate}
...
\begin{proof}
Assume (1) holds. Then $A = \kappa[[x, y]]/(ax^2 + bxy + cy^2)$ with
$ax^2 + bxy + cy^2$ nondegenerate. Then there exists a degree $2$
separable extension $\kappa'/\kappa$ such that $ax^2 + bxy + cy^2$
splits over $\kappa'$. Hence $A' = \kappa'[[x, y]]/(ax^2 + bxy + cy^2)$
is isomorphic to $\kappa'[[s, t]]/(st)$ after a change of coordinates.
Since $A \subset A'$ is finite \'etale of degree $2$, it follows
that $A$ satisfies (2); small detail omitted.

\medskip\noindent
Conversely, assume (2). Let $\kappa^{sep}$ be a separable algebraic
closure of $\kappa$. Let $A^{sh}$ be the corresponding strict henselization.
By Lemma \ref{lemma-multicross-algebra} we see that $(A^{sh})^\wedge$
is as in (\ref{equation-multicross}). Observe that we have the following
equality
$$
\left(\bigoplus\nolimits_{n \geq 0}
\mathfrak m^n/\mathfrak m^{n + 1} \right)
\otimes_\kappa \kappa^{sep} =
\bigoplus\nolimits_{n \geq 0} (\mathfrak m^{sh})^n/(\mathfrak m^{sh})^{n + 1}
$$
of graded $\kappa^{sep}$-algebras. See for example
More on Algebra, Lemma \ref{more-algebra-lemma-dumb-properties-henselization}.
Hence we see that $\dim_\kappa(\mathfrak m/\mathfrak m^2) = 2$ because
the corresponding thing is true for $(A^{sh})^\wedge$.
Similarly, the kernel of the canonical map
$\text{Sym}^2_\kappa(\mathfrak m/\mathfrak m^2) \to \mathfrak m^2/\mathfrak m^3$
is $1$-dimensional because the
corresponding thing is true for $(A^{sh})^\wedge$.
Finally, if we pick $q$ in this kernel, then $q$ becomes split
over $\kappa^{sep}$. This indeed implies that $q$ is nondegenerate
...
Let $k$ be a field. Let $A = k[[x_1, \ldots, x_n]]$. Let
$I = (f_1, \ldots, f_m) \subset A$ be an ideal. The following
are equivalent
\begin{enumerate}
\item $k \to A/I$ satisfies the assumptions of
Lemma \ref{lemma-nodal-algebraic},
\item $A/I$ is reduced of dimension $1$ and the
...
Then we can explicitly compute to show that (2) is true.
...
It follows that $n - 1 - t \leq 1$ because there is an element
of $\mathfrak m_A \setminus \mathfrak m_A^2 + I$ (otherwise
$I = \mathfrak m_A$ by Nakayama). Thus $t \geq n - 2$.
If $t = n$, then $\dim(A/I) \leq 0$ which is a contradiction.
If $t = n - 1$, then there is an invertible $(n - 1) \times (n - 1)$-minor
which is a contradiction. Hence $t = n - 2$.
Then $A/I$ is a quotient of $k[[x_{n - 1}, x_n]]$ and
Lemma \ref{lemma-reduced-quotient-regular-ring-dim-2}
implies that $I$ is generated by $x_1, \ldots, x_{n - 2}, f$
for some $f = f(x_{n - 1}, x_n)$. In this case the
condition on the minors exactly says that the quadratic
...
\begin{enumerate}
\item $x$ is a node,
\item $k \to \mathcal{O}_{X, x}^\wedge$ is as in
Lemma \ref{lemma-nodal-algebraic},
...
coordinates, the equivalence with (5) is 
...
matrix $(\partial f_j/\partial x_i)$. Hence (2) and (4)
are equivalent by Lemma \ref{lemma-fitting-ideal} applied
to the completion of $k[x_1, \ldots, x_n] \to R$
at the maximal ideal $(x_1, \ldots, x_n)$. Observe that
the completion of $\mathcal{O}_{X, x}$, i.e., the completion
of $R$ at $(x_1, \ldots, x_n)$, is reduced
(Algebra, Proposition \ref{algebra-proposition-ubiquity-nagata} and
More on Algebra, Lemma \ref{more-algebra-lemma-completion-reduced}).

\medskip\noindent
Now assume $k$ is an arbitrary field.
In each case the local ring $\mathcal{O}_{X, x}$ is reduced
(observe that $\mathcal{O}_{X, x}$ is reduced if and
only if its completion is reduced by the reference given above).
In cases (2), (4), (5) the residue field $\kappa(x)$ is
separable over $k$. Let us show this holds true as well
in cases (1) and (3). Namely, let $Z \subset X$ be the closed subscheme
of $X$ defined by the first Fitting ideal of $\Omega_{X/k}$.
The formation of $Z$ commutes with
field extension because the formation of $\Omega$ does
(Morphisms, Lemma \ref{morphisms-lemma-base-change-differentials})
and because this is true for Fitting ideals
...
is an isolated point of $Z_{\overline{k}}$ and
contains $\overline{x}$ with multiplicity one (as we have the
...
\medskip\noindent
Now we know that in all cases the local ring $\mathcal{O}_{X, x}$
is reduced and that $\kappa(x)/k$ is separable. Moreover, the
argument given above shows that if (1) or (3) holds, then
the first Fitting ideal of $\Omega_{X/k}$ generates $\mathfrak m_x$.
Conversely, if (4) holds, then $Z$ is geometrically reduced at $x$
(because $\kappa(x)/k$ separable and $Z$ is $x$ in a neighbourhood)
and hence $Z_{\overline{k}}$ is reduced at any point $\overline{x}$
of $X_{\overline{k}}$ lying over $x$,
hence (4) holds for all of these points, i.e., (1) and (3) hold.
In this way we see that (1), (3), (4) are equivalent.

\medskip\noindent
Recall that the number of geometric branches of $\mathcal{O}_{X, x}$
is the same as the number of geometric branches of its completion
and similarly for the $\delta$-invariant. See
More on Algebra, Lemma
\ref{more-algebra-lemma-one-dimensional-number-of-branches}
and Varieties, Lemma \ref{varieties-lemma-delta-same-after-completion}.
Hence the equivalence
of (2) and (5) by Lemma \ref{lemma-2-branches-delta-1}.

\medskip\noindent
Finally, we prove the equivalence of (2) $=$ (5) with
(1) $=$ (3) $=$ (4). First we note that the geometric number
...
hold for $x \in X$; namely, we have seen in the proof of
Lemma \ref{lemma-2-branches-delta-1}
that the points of the normalization of $X$ over $x$
are defined over a separable quadratic extension of $\kappa(x)$
(small detail omitted: completion commutes with normalization
in our current setting). Thus Varieties, Lemma
\ref{varieties-lemma-delta-invariant-and-change-of-fields-better}
applies and
shows that we have equality in the above displayed inequality.
