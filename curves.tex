...
zero or more generally if all the ramification is tame\footnote{This
exactly means that the residue field extensions $\kappa(x)/\kappa(y)$
are separable and $e_x$ is prime to the characteristic of $k$.}, then
$$
2g_x - 2 = (2g_Y - 2)\deg(f) +
\sum\nolimits_{x \in X} (e_x - 1)[\kappa(x) : k]
$$
where $e_x$ is the ramification index of $\mathcal{O}_{X, x}$ over
$\mathcal{O}_{Y, f(x)}$. For a different approach we refer the reader
to the proof of Lemma \ref{lemma-rhe}.
...
in the discussion above. Moreover, in the discussion we
proved $d_x = e_x - 1$ if the ramification is tame.
However, the inequalities weren't proved in general
(only in the case where $\kappa(x)$ is separable over $k$).
In the rest of the proof we give a uniform treatement
of (2) and (3) using material on differents of
a quasi-finite Gorenstein morphisms.
...
Dualizing Complexes, Lemma
\ref{dualizing-lemma-discriminant-quasi-finite-morphism-smooth}
we used above. Thus $\omega_{X/Y}$ is invertible by
...
Thus it suffices to show: the different $\mathfrak{D}_{C/k}$
of $C = \mathcal{O}_{X, x}/(s^{e_x})$ over $\kappa(y)$
(in the sense of
...
divide $e_x$. On the other hand, from the definition we see that
the different of $C/k$ is nonzero if and only if
$\tau_{C/k} \in \omega_{C/k}$ is nonzero (use that $\omega_{C/k}$
is an invertible $C$-module as the base change of $\omega_{B/A}$).
By Dualizing Complexes, Lemma \ref{dualizing-lemma-tau-nonzero}
this happens if and only if $\kappa(x)/\kappa(y)$
is separable and $e_x$ is prime to the characteristic as desired.
\end{proof}
