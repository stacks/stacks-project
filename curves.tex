...
$$
|D| = \mathbf{P}(H^0(X, \mathcal{O}_X(D))^\wedge)
$$
where $(-)^\wedge$ indicates $k$-linear dual and $\mathbf{P}$ is as
...
linear equations. If $L$ has dimension $r$, then $L = \mathbf{P}(V^\wedge)$
...
has a $k$-rational points $x_1, \ldots, x_n$ such that
...
(Lemma \ref{lemma-flat}) and finite (Lemma \ref{lemma-finite})
...
$\Delta : X \to X_{k' \otimes_k k' \otimes_k k'}$
...
$\kappa'$ as follows
\begin{enumerate}
\item let $q = ax^2 + bxy + cy^2$ be a nondegerate quadric
