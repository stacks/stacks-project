...
Lemma \ref{lemma-duality-dim-1}
we have $\omega_X \cong \Omega_{X/k}$ hence we conclude from the
equality above that the geometric genus of $X$ agrees with
the (arithmetic) genus in this case.

\begin{lemma}
\label{lemma-geometric-genus}
Let $k$ be a field. Let $X$ be a geometrically irreducible curve over $k$.
\begin{enumerate}
\item If $k$ is perfect, then the nonsingular projective model $Y$ of $X$ is
a smooth curve whose genus is defined.
\item In general there exists a finite extension $k'/k$
such that the nonsingular projective model $Y'$ of $(X_{k'})_{red}$
is a smooth curve whose genus is defined.
\item The genus of $Y'$ is independent of the choice of $k'$.
\end{enumerate}
\end{lemma}
...
an extension field of $k$ as in Lemma \ref{lemma-geometric-genus}.
\end{definition}
