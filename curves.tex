...
\noindent
Nonsingular, i.e., normal, curves are Gorenstein, see
Dualizing, Lemma \ref{dualizing-lemma-regular-gorenstein}.
...
Let $X$ be a proper curve over a field $k$ with $H^0(X, \mathcal{O}_X) = k$.
Assume $X$ is not Gorenstein and has genus $0$. Then there exists a diagram
$$
\xymatrix{
x' \ar[d] \ar[r] & X' \ar[d] \ar[r] & \Spec(k') \ar[d] \\
x \ar[r] & X \ar[r] & \Spec(k)
}
$$
where $k'/k$ is a nontrivial finite extension,
$X'$ is a nonsingular proper curve over $k'$
with $H^0(X', \mathcal{O}_{X'}) = k'$ and genus $0$,
$x'$ is a $k'$-rational point of $X'$,
$x$ is a $k$-rational point of $X$,
the map $X' \to X$ induces an isomorphism
$X' \setminus \{x'\} \to X \setminus \{x\}$,
and there is an exact sequence
$$
0 \to \mathcal{O}_X \to \mathcal{O}_{X'} \to k'/k \to 0
$$
...
Since $X$ is not Gorenstein, a fortiori $\nu$ is not an isomorphism.
...
Thus a section of $\mathcal{I}$ over $U \subset X$ is an
$f \in \mathcal{O}_X(U)$ such that
$f \nu_*\mathcal{O}_{X'}(U) \subset \mathcal{O}_X(U)$. In
...
is a module map. We conclude that $\mathcal{I} = \nu_*\mathcal{I}'$
...
$$
Pushing down to $X$ using $\nu$, using the estimate above,
...
the dimension is divisible by $n$. Note that $\mathcal{A}$
and $\mathcal{A}'$ cannot be zero. We conclude that
$\dim_k H^0(X', \mathcal{A}') = n$ which means that $\mathcal{I}'$
is the ideal sheaf of a $k'$-rational point $x'$.
In turn we conclude that $\dim_k H^0(X, \mathcal{A}) = 1$
which means that $\mathcal{I}$ is the ideal sheaf of a
