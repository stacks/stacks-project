...
and hence $\deg(\mathcal{L}) = 2$. In particular, $\mathcal{L}$
is ample by Varieties, Lemma \ref{varieties-lemma-ample-curve}.
We will show that $\mathcal{L}$ is very ample and the corresponding
map to projective space embeds $X$ as a plane curve.

\medskip\noindent
By Riemann-Roch (Lemma \ref{lemma-rr})
we have
$$
\dim_k H^0(X, \mathcal{L}) - \dim_k H^1(X, \mathcal{L}) = 2 + 1 = 3
$$
Thus $\dim_k H^0(X, \mathcal{L}) \geq 3$. Let $s$ be a nonzero section
of $\mathcal{L}$. Then the zero scheme of $s$ is an effective Cartier
...
The short exact sequence also shows that $H^1(X, \mathcal{L}) = 0$ or
you can argue using duality (Remark \ref{remark-rework-duality-locally-free})
and Varieties, Lemma \ref{varieties-lemma-check-invertible-sheaf-trivial}
to see this. We conclude all-in-all that $\mathcal{L}$
is a degree $2$ invertible $\mathcal{O}_X$-module
with $\dim_k H^0(X, \mathcal{L}) = 3$ and which is globally generated.

\medskip\noindent
Choose a $k$-basis $s_0, s_1, s_2$ of $H^0(X, \mathcal{L})$
and consider the morphism
$$
\varphi : X \longrightarrow \mathbf{P}^2_k
$$
of Constructions, Lemma \ref{constructions-lemma-projective-space}.
Let $Y \subset \mathbf{P}^2_k$ be the (scheme theoretic) image.
Since $X$ is reduced and irreducible, also $Y$ is reduced and irreducible.
Thus $Y = V(F)$ and $I(Y) = (F)$ for some
$F \in k[T_0, T_1, T_2]$ (Lemma \ref{lemma-equation-plane-curve}).
The induced morphism $f : X \to Y$ is a
dominant morphism of varieties and hence induces injective maps
$$
H^0(Y, \mathcal{O}_Y(n)) \longrightarrow H^0(X, \mathcal{L}^{\otimes n})
$$
(small detail omitted).
Thus $F$ is a generator of the kernel of the map of graded $k$-algebras
$$
k[T_0, T_1, T_2]
\longrightarrow
\bigoplus\nolimits_{n \geq 0} H^0(X, \mathcal{L}^{\otimes n})
$$
Because $s_0, s_1, s_2$ are linearly independent we see that
$F$ cannot be linear, i.e., $d = \deg(F) \geq 2$.
On the other hand, for $n \geq 2$ we have
$H^1(X, \mathcal{L}^{\otimes n}) = 0$ (proved in exactly the same
manner as the vanishing for $\mathcal{L}$ above) and we get
$$
\dim H^0(X, \mathcal{L}^{\otimes n}) = 2n + 1
$$
by Riemann-Roch. Taking $n = 2$ we get $5 < 6 = \dim k[T_0, T_1, T_2]_2$ and
we conclude that $d = 2$ is forced. Then for $n > 2$ we get
\begin{align*}
\dim_k (k[T_0, T_1, T_2]/(F))_n
& =
\dim_k k[T_0, T_1, T_2]_n - \dim_k k[T_0, T_1, T_2]_{n - 2} \\
& =
{n + 2 \choose 2} - {n \choose 2} \\
& = 2n + 1
\end{align*}
This implies that $\bigoplus H^0(X, \mathcal{L}^{\otimes n})$
is isomorphic to $k[T_0, T_1, T_2]/(F)$ as a graded $k$-algebra.
As $X$ is equal to $\text{Proj}$ of this algebra
(Morphisms, Lemma \ref{morphisms-lemma-proper-ample-is-proj})
we conclude (and of course it also follows that the map
$X \to Y$ is an isomorphism but we don't need this).
\end{proof}
