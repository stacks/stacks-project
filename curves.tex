...
Duality, Lemma \ref{duality-lemma-twisted-inverse-image} to
...
combinging Lemmas \ref{lemma-contracting-rational-tails} and
...
\medskip\noindent
Uniqueness: Let $c : X \to Y$ be a morphism morphism satisfying conditions
...
Example \ref{lemma-contracting-rational-tails}; a contradiction.
...
Example \ref{lemma-contracting-rational-tails}).
...
It follows that $H^1(X, \omega_X^{\otimes 3}) = 0$
by Lemma \ref{lemma-vanishing-twist}. (Yes, we also
have vanishing for $\otimes 2$ but we don't need this.)
By Lemma \ref{lemma-contract-gorenstein-canonical} applied with $m = 3$
we find that $c^*\omega_Y \cong \omega_X$.
We also have that $\omega_X = (X \to X_{n'})^*\omega_{X_m}$ by
