...
spaces and frequently references Knutson's book \cite{knutson}. There is an
...
\cite{lmb} in the functorirality of the lisse-\'etale site.  The cotangent
...
for proper pushoforwards of coherent and constructible sheaves.
\end{quote}
\smallskip
...
\item
Kresch, Vistoli: \emph{On coverings of Deligne-Mumford stacks and
...
Then there exists an algebraic stack $\mathcal{X} \thickslash H$ and a
morphism $\rho: \mathcal{X} \rightarrow \mathcal{X} \thickslash H$ which is
an fppf gerbe such that for every $\xi \in \mathcal{X}(T)$, the morphism
$\text{Aut}_{\mathcal{X}(T)} (\xi)
\rightarrow \text{Aut}_{\mathcal{X}\thickslash H (T)} (\xi) $
...
\item Alexeev and Knutson:  Complete moduli spaces of branchvarieties
