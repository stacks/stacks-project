...
$\text{Aut}(F) \subset \prod_{X \in \Ob(\mathcal{C})} \text{Aut}(X)$
...
Then there exists an isomorphism $t : F' \circ H \to F$.
The choice of such an isomorphism determines a continuous hommomorphism
...
$$
The map $h$ is well defined up to an inner automorphism of $G$.
...
$\textit{F\'Et}_X \to \textit{F\'Et}_X$ is exact.
\end{lemma}
...
\medskip\noindent
Let $Y \to Z$ be a morphism in $\textit{F\'Et}_{\overline{x}}$
which induces an bijection FIXME.

\medskip\noindent
If $Y \to Y'$ is a monomorphism in $\textit{F\'Et}_{\overline{x}}$
then we see that $Y \to Y \times_{Y'} Y$ is an isomorphism, and
...
$Y$ is finite over $X$ the morphism $Y \to Y'$ is moreover
finite, hence closed, hence it is the inclusion of an open
and closed subscheme of $Y'$. Thus to prove axiom
(\ref{item-connected-components}) FIXME.
\end{proof}
