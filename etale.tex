...
of affine schemes.
\item For any scheme $S$ we define a $\tau$-site
...
choice for the $\tau$-site of a scheme, or even the small
...
Let $T$ be a scheme. An {\it Zariski covering of $T$} is a family
...
open of $T$, see Schemes, Definition \ref{schemes-definition-standard-covering}.
...
A {\it Zariski site} is given by a site $\textit{Sch}_{Zar}$ as in
Sites, Definition \ref{sites-definition-site} with the following properties:
\begin{enumerate}
\item The underlying category $\textit{Sch}_{Zar}$ is a
full subcategory of the category of all schemes.
\item Fibre products in $\textit{Sch}_{Zar}$ exist and are
the same as fibre product in the category of schemes.
\item For every object $T$ of $\textit{Sch}_{Zar}$
and every finite type morphism of schemes $Y \to T$
the scheme $Y$ is isomorphic to an object of $\textit{Sch}_{Zar}$.
\item For every object $T$ of $\textit{Sch}_{Zar}$
and every immersion $Y \to T$
the scheme $Y$ is isomorphic to an object of $\textit{Sch}_{Zar}$.
\item Every covering of the site $\textit{Sch}_{Zar}$ is a Zariski covering.
\item For every $T \in \text{Ob}(\textit{Sch}_{Zar})$
and every Zariski covering $\{T_i \to T\}_{i\in I}$
there exists a covering $\{U_j \to T\}_{j \in J}$ of the site
$\textit{Sch}_{Zar}$ which refines $\{T_i \to T\}_{i\in I}$.
\end{enumerate}
\end{definition}

\begin{lemma}
\label{lemma-big-fppf-exists}
Let $\mathcal{C}_0$ be a set of schemes.
Let $\text{Cov}_0$ be a set of fppf-coverings of schemes.
There exists a big fppf site containing $\mathcal{C}_0$
whose set of coverings contains $\text{Cov}$.
\end{lemma}

\begin{proof}
Replace $\mathcal{C}_0$ by the union of $\mathcal{C}_0$ and
all schemes occuring in the coverings of $\text{Cov}_0$, if
necessary. Apply Sets, Lemma \ref{sets-lemma-create-category-schemes}
to find an ordinal $\alpha$ such that $\textit{Sch}_\alpha$
satisfies the conclusions of that lemma. After having found
$\alpha$ set $\mathcal{C} = \textit{Sch}_\alpha$ and
$\text{Cov}(\mathcal{C})$ the class of all fppf coverings
of objects of $\mathcal{C}$ by objects of $\mathcal{C}$.
Note that $\text{Cov}_0 \subset \text{Cov}(\mathcal{C})$.
Hence we may find a set of coverings $\text{Cov}(\mathcal{C}) \cap V_\beta$
as in Sets, Lemma \ref{sets-lemma-coverings-site} wich turns
$(\mathcal{C}, \text{Cov}(\mathcal{C}) \cap V_\beta)$ into a
site and such that any covering in $\text{Cov}(\mathcal{C})$ is
combinatorially equivalent to a covering in
$\text{Cov}(\mathcal{C}) \cap V_\beta$. For every
fppf covering $\{T_i \to T\}_{i \in I}$ all the morphisms
$T_i \to T$ are of finite presentation, hence finite type
(see Morphisms, Lemma \ref{morphisms-lemma-finite-presentation-finite-type}).
Thus if $T \in \text{Ob}(\textit{Sch}_\alpha)$ then all $T_i$
are isomorphic to objects of $\textit{Sch}_\alpha$ and so clearly
$\{T_i \to T\}_{i \in I}$ is tautologically equivalent to
a fppf covering in $\textit{Sch}_\alpha$. Hence also
tautologically equivalent to a covering
in $\text{Cov}(\mathcal{C}) \cap V_\beta$. Thus the lemma is
proved.

\medskip\noindent
Alternatively: Add the existence of universes to your list of
axioms of ZFC. Choose a universe containing $\mathcal{C}_0$. Let
$\textit{Sch}_{fppf}$ be the category of schemes in the universe
with coverings also elements of the universe.
\end{proof}

\begin{lemma}
\label{lemma-compare-fppf-sites}
Let $\textit{Sch}_{fppf}$ and $\textit{Sch}'_{fppf}$
...
A {\it big fppf site} is given by a site
$\textit{Sch}_{fppf}$ as in Sites, Definition \ref{sites-definition-site}
with the following properties:
\begin{enumerate}
\item The underlying category $\textit{Sch}_{fppf}$ is a
full subcategory of the category of all schemes.
\item Fibre products in $\textit{Sch}_{fppf}$ exist and are
the same as fibre product in the category of schemes.
\item Every element of $\text{Cov}$ is an fppf covering.
\item For every $T \in \text{Ob}(\textit{Sch}_{fppf})$
...
see Sites, Remark \ref{sites-remark-enlarge-coverings}.
\item For every scheme $T$ in $\textit{Sch}_{fppf}$
and every finite type morphism of schemes $Y \to T$
the scheme $Y$ is isomorphic to an object of $\textit{Sch}_{fppf}$.
\end{enumerate}
\end{definition}
...
(over $V$) $j:U \to \mathbf{A}^n_V$. If $j$ is defined by the sheaf of ideals 
$\mathcal{J}$,
commutative algebra gives an exact sequence 
$$
j^*(\mathcal{J}/\mathcal{J}^2) \to j^*\Omega^1_{\mathbf{A}^n_V/V} \to 
\Omega^1_{U/V} \to 0
$$
