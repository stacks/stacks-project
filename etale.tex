...
This is nonstandard notation and should only be used
for local ring maps essentially of finite type. In reality the
definition given here corresponds to maps which are
``formally unramified for the $\mathfrak m$-adic topologies''
(insert future reference here). However, it is convenient for a quick
discussion as in this chapter.}
if
\begin{enumerate}
...
{\it etale homomorphism of local rings}\footnote{This is nonstandard
notation and should only be used for local ring maps which are
essentially of finite type. In reality the definition given here
corresponds to maps which are ``formally etale for the
$\mathfrak m$-adic topologies'' (insert future reference here).
In fact, we will run into trouble with this definition later,
see remarks preceding Proposition \ref{proposition-etale-reduced}.
However, it is convenient for a quick discussion as in this chapter.}
if it is an unramified homomorphism of local rings and flat.
\end{definition}

\noindent
This notion is different from the definition of a ``global'' etale ring map
...
dimension $0$. Then it is shown (in
...
\item The morphism is said to be {\it etale} if it is etale at all its points.
\end{enumerate}
\end{definition}
...
\noindent
The result on ``reducedness'' does not hold with our
(nonstandard) definition of ``etale homomorphisms of local rings''.
...
But the ring map $A \to A^\wedge$ is an etale homomorphism of local rings
by our definitions. Here is a correct version.

\begin{proposition}
\label{proposition-etale-reduced}
Let $A$, $B$ be Noetherian local rings.
Let $f : A \to B$ be an etale homomorphism of local rings,
and assume that $B$ is essentially of finite type over $A$.
...
Let $f : A \to B$ be an etale homomorphism of local rings,
and assume that $B$ is essentially of finite type over $A$.
Then $A$ is normal if and only if $B$ is so.
\end{proposition}

\begin{proof}
See Algebra, Lemma \ref{algebra-lemma-descent-normal} for descending normality.
Conversely, assume $A$ is reduced. By assumption $B$ is a localization
of a finite type $A$-algebra $B'$ at some prime $\mathfrak q$.
After replacing $B'$ by a localization we may assume that $B'$
is etale over $A$, see Lemma \ref{lemma-characterize-etale-Noetherian}.
Then we see that Algebra, Lemma \ref{algebra-lemma-normal-goes-up} applies to
$A \to B'$ and $B'$ is reduced. Hence $B$ is reduced.
\end{proof}
...
associated morphism on analytic spaces (the $\mathbf{C}$-valued points given
