...
\begin{proof}
By Morphisms, Lemma \ref{morphisms-lemma-smooth-at-point} part (5)
we can find opens $U$ and $V$ as in the theorem such that
$U = \text{Spec}(A)$, and $x$ corresponding to $\mathfrak q \subset A$
such that
$$
...
\right)
$$
mapping to an element of $A$ not in $\mathfrak q$. Then by our discussion
following Definition \ref{definition-etale-ring} the ring map
$$
R[x_{c + 1}, \ldots, x_n] \to A
$$
is etale. Whence the theorem with $n$ replaced by $n - c$.
\end{proof}
...
morphisms as explained in, say, \cite[Expose I.5]{SGA1}.
...
Let $Y$ be a scheme.
Let $f : X \to Y$ be a morphismn.
Then $f$ is an open immersion if and only if it etale and
radicial (see Morphisms, Definition \ref{morphisms-definition-radicial}).
\end{theorem}

\begin{proof}
This is Descent,
Lemma \ref{descent-lemma-radicial-etale-open-immersion}.
\end{proof}
...
and therefore radicial. Since $X' \to S$ is etale, it
...
\item in terms of flat and unramified maps,
