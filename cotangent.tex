...
To see that it induces an isomorphism $H_2(L_{B/A}) \to H_2(L)$ we
use Quillen's spectral sequence. Namely, first observe that
$H_2(L_{B/A}) = H_2(L_{B/P})$ by the distinguished triangle
(\ref{equation-triangle}) for
the ring maps $A \to P \to B$ and the vanishing of cohomology groups
of $L_{P/A}$ (Lemma \ref{lemma-cotangent-complex-polynomial-algebra}).
Quillen's spectral sequence tells us that there is an exact sequence
$$
\wedge^2_B(I/I^2) \to \text{Tor}_2^P(B, B) \to H^{-2}(L_{B/P}) \to 0
$$
see Remark \ref{remark-elucidate-ss}.
...
Thus we see that $H_2(L_{B/A}) = H_2(L_{B/P}) \cong H_2(L)$.
We omit the verification that our diagram above produces this isomorphism.
\end{proof}
