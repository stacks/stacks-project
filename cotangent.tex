...
\item $A \to B$ is \'etale,
\item $A \to B$ is a filtered colimit of rings as above, and
\item add more here.
\end{enumerate}
\end{lemma}
...
\begin{proof}
Recall that the maps $A \to A^h$, $A \to A^{sh}$,
$B \to B^h$, and $B \to B^{sh}$ are filtered colimits of
\'etale ring maps, see discussion following
Algebra, Definition \ref{algebra-definition-henselization}.
Hence $L_{A^h/A}$, $L_{A^{sh}/A}$, $L_{B^h/B}$, and
...
\noindent
In this section we give a description of the cotangent complex of a
scheme $X$ over a ring $\Lambda$ similar to
Lemma \ref{lemma-compute-cotangent-complex}.
...
$$
We will also use the category $\mathcal{C}_{X/\Lambda}$ to construct
a naive obstruction theory for coherent sheaves.
...
Thus the result is clear from Remark \ref{remark-map-sections-over-U}.
\end{proof}
