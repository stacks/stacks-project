...
$$
E_1^{p, q} =
L_p\pi_!(\wedge^{q + 1}\mathcal{F} \otimes \text{Sym}^{k - q - 1}\mathcal{F})
\Rightarrow
L_{p + q}\pi_!(\text{Sym}^k(\mathcal{F}))
$$
...
of $\pi_!(\wedge^{q + 1}(\mathcal{F})) = 0$ will prove what we want.
This is true because $\wedge^{q + 1}(\mathcal{F})$ is a quotient
of $\mathcal{F}^{\otimes q + 1}$ and $\pi_!(\mathcal{F}^{\otimes q + 1})$
is a quotient of $\pi_!(\mathcal{F})^{\otimes q + 1} = 0$.
\end{proof}
...
$H^{-1}(L_{B/A}) = L_1\pi_!(\Omega) = I/I^2$, see
Lemma \ref{lemma-surjection}.
Hence $L_k\pi_!(\text{Sym}^k(\Omega)) = \wedge^k_B(I/I^2)$ by
