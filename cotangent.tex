...
Choose generators $f_t$, $t \in T$ for $\text{Ker}(P \to B)$.
...
$F' = \bigoplus_{t' \in T'} P'$. Let $Rel = \text{Ker}(F \to P)$
and $Rel' = \text{Ker}(F' \to P')$ where the maps are given
...
If $A$ and $B$ are Tor indepent $R$-algebras, then the object $E$
...
\begin{proof}
The construction of $E$ tells us it suffices to
show that $L_{B/R} \otimes_B^\mathbf{L} (A \otimes_R B) \to
L_{A \otimes_R B/A}$ induces an isomorphism on $H_0$ and $H_{-1}$
and that the cokernel of the map induced on $H_{-2}$ is isomorphic
to $\text{Tor}_1^R(A, B)$. By Lemma \ref{lemma-compare-higher}
...
L\pi_!(P^1_{\mathcal{O}/\underline{A}}(M)) \to M
...
to get a distinguished triangle in $D(B)$ and not just in
$D(\textit{Ab})$.
