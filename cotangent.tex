...
$\pi_!\mathcal{F} = 0$.
Then $L_l\pi_!(\text{Sym}_{\underline{B}}^k(\mathcal{F})) = 0$ for $l < k$.
\end{lemma}

\begin{proof}
We drop the subscript ${}_{\underline{B}}$ from the tensor products and
symmetric powers. Note that $\text{Sym}^k(\mathcal{F})$ is the
$S_k$-coinvariants of the module $\mathcal{F}^{\otimes k}$. Choose a
resolution
$$
\ldots \to F_2 \to F_1 \to F_0 \to \mathbf{Z} \to 0
$$
over the group ring $\mathbf{Z}[S_k]$ with $F_t$ finite free.
Think of $\mathcal{F}^{\otimes k}$ as a $\mathbf{Z}[S_k]$-module.
Then we see that $\text{Sym}^k(\mathcal{F})$ has a resolution
$$
\ldots \to
F_2 \otimes_{\mathbf{Z}[S_k]} \mathcal{F}^{\otimes k} \to
F_1 \otimes_{\mathbf{Z}[S_k]} \mathcal{F}^{\otimes k} \to
F_0 \otimes_{\mathbf{Z}[S_k]} \mathcal{F}^{\otimes k} \to
\text{Sym}^k(\mathcal{F}) \to 0
$$
This gives a spectral sequence (details omitted) expressing
$L_l\pi_!(\mathcal{F})$ modules of $\text{Sym}^k(\mathcal{F})$
in terms of the $L_l\pi_!$ of the $F_t \otimes \mathcal{F}^{\otimes k}$.
Since these are isomorphic to direct sums of copies of
$\mathcal{F}^{\otimes k}$ it suffices to prove
the vanishing for $\mathcal{F}^{\otimes k}$.
By Lemma \ref{lemma-eilenberg-zilber} and flatness
we see that $L\pi_!(\mathcal{F}^{\otimes k})$ is quasi-isomorphic to
$$
L\pi_!(\mathcal{F}) \otimes_B^\mathbf{L}
L\pi_!(\mathcal{F}) \otimes_B^\mathbf{L}
\ldots \otimes_B^\mathbf{L} L\pi_!(\mathcal{F})
$$
Since $L\pi_!(\mathcal{F})$ is quasi-isomorphic to a complex sitting in
...
$$
E_1^{p, q} = L_{- p - q}\pi_!(\text{Sym}^p_{\underline{B}}(\Omega))
...
one can show that
$L_k\pi_!(\text{Sym}^k(\mathcal{F})) = \wedge^k_B(L_1\pi_!(\mathcal{F}))$
(details omitted).
We can use this to describe the terms of the spectral sequence of
Theorem \ref{theorem-quillen-spectral-sequence}. Namely,
let $I = \text{Ker}(A \to B)$. Then
$H^{-1}(L_{B/A}) = L_1\pi_!(\Omega) = I/I^2$.
Hence $L_k\pi_!(\text{Sym}^k(\Omega)) = \wedge^k_B(I/I^2)$.
The $E_1$-page looks like
$$
\begin{matrix}
B \\
0 \\
...
0 & H^{-4}(L_{B/A}) & L_3\pi_!(\text{Sym}^2(\Omega)) \\
0 & H^{-5}(L_{B/A}) & L_4\pi_!(\text{Sym}^2(\Omega)) & \wedge^3(I/I^2)
\end{matrix}
$$
