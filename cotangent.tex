...
\begin{proof}
Although this lemma is more or less trivial, it is a bit messy, so we
think it makes sense to explain what happens for low $n$ before giving
the proof in general.
If $n = 0$, then (3) means that $P_1 = P_0 \times_B P_0$. Since the
ring map $P_0 \to B$ is surjective, this is of finite type over $A$ by
...
$A$-algebra with terms $P_n = P_0 \times_A \ldots \times_A P_0$. By
...
\medskip\noindent
Suppose for some $n \geq 0$ we have constructed $P_\bullet$ such that
\begin{enumerate}
\item $P_\bullet \to B$ is a trivial Kan fibration of simplicial sets,
\item $P_k$ is a finitely generated polynomial algebra for
$0 \leq k \leq n$, and
\item $P_\bullet = \text{cosk}_n \text{sk}_n P_\bullet$
\end{enumerate}
...
object $P'_\bullet$ of $\mathcal{A}$ using $\text{cosk}_{n + 1}$. Then
conditions (2) and (3) above for $P'_\bullet$ are satisfied with $n$
...
we conclude that $P'_\bullet \to B$ is a trivial Kan fibration.

\medskip\noindent
To finish the proof we take the inverse limit $P_\bullet = \lim P^n_\bullet$
of a sequence of simplicial algebras
$$
\ldots \to P^2_\bullet \to P^1_\bullet \to P^0_\bullet
$$
constructed as above using induction.
The map $P_\bullet \to B$ is a trivial Kan fibration by
