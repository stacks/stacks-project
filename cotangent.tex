...
$$
L_p\pi_!(\text{Tor}_q^\mathcal{O}(\mathcal{J}, \underline{B}))
\Rightarrow 
L_{p + q}\pi_!(\mathcal{J} \otimes_\mathcal{O}^\mathbf{L} \underline{B}) = 0
$$
(dual of
Derived Categories, Lemma \ref{derived-lemma-two-ss-complex-functor})
implies that
$\pi_!(\mathcal{J}/\mathcal{J}^2) = L_1\pi_!(\mathcal{J}/\mathcal{J}^2) = 0$.
Thus the complex
$\mathcal{J}/\mathcal{J}^2 \to \Omega$
has $\tau_{\geq -1}L\pi_!(\mathcal{J}/\mathcal{J}^2 \to \Omega)$
equal to $\tau_{\geq -1}L_{B/A}$.
Thus, for any object $U = (P \to B)$ of $\mathcal{C}_{B/A}$ we obtain a map
\begin{equation}
...
the associated long exact sequence of cohomology, and using the
vanishing of $L_1\pi_!(\mathcal{J}/\mathcal{J}^2)$ and
$\pi_!(\mathcal{J}/\mathcal{J}^2)$ see in
Remark \ref{remark-make-map}
we obtain what we want using
Lemma \ref{lemma-pi-lower-shriek-constant-sheaf}.
...
tensor product to the cotangent complex. In order to do this we
need a little bit of theory on simplicial modules.
The following result can be found in \cite{Eilenberg-Zilber}.

\begin{theorem}[Eilenberg-Zilber]
\label{theorem-tensor-simplicial-modules}
Let $B$ be a ring. Let $M_\bullet$, $M'_\bullet$ be simplicial $B$-modules.
The complex $s(M_\bullet \otimes_B M'_\bullet)$ is quasi-isomorphic to
the total complex associated to the double complex
$s(M_\bullet) \otimes_B s(M'_\bullet)$.
\end{theorem}

\begin{proof}
Omitted. Hint: There is a more general statement for bisimplicial
abelian groups, which you can apply to the bisimplicial abelian
group with terms $M_n \otimes_B M'_m$.
\end{proof}

\begin{lemma}
\label{lemma-vanishing-symmetric-powers}
Let $B$ be a ring. Let $M_\bullet$ be simplicial $B$-module
such that
\begin{enumerate}
\item $M_n$ is a flat $B$-module for all $n$, and
\item $H_0(s(M_\bullet)) = 0$.
\end{enumerate}
Then $H_l(s(\text{Sym}_B^k(M_\bullet))) = 0$ for $l < k$.
\end{lemma}

\begin{proof}
Note that $\text{Sym}^k_B(M)$ is the $S_k$-coinvariants of the
module $M^{\otimes k}$. Choose a resolution
$$
\ldots \to F_2 \to F_1 \to F_0 \to \mathbf{Z} \to 0
$$
over the group ring $\mathbf{Z}[S_k]$ with $F_t$ finite free.
Think of $M_\bullet^{\otimes k}$ as a $\mathbf{Z}[S_k]$-module.
Then we see that $\text{Sym}^k_B(M_\bullet)$ has a resolution
$$
\ldots \to
F_2 \otimes_{\mathbf{Z}[S_k]} M_\bullet^{\otimes k} \to
F_1 \otimes_{\mathbf{Z}[S_k]} M_\bullet^{\otimes k} \to
F_0 \otimes_{\mathbf{Z}[S_k]} M_\bullet^{\otimes k} \to
\text{Sym}^k_B(M_\bullet) \to 0
$$
This gives a spectral sequence expressing the homology of
$\text{Sym}^k_B(M_\bullet)$ in terms of the homology of the
$F_t \otimes M_\bullet^{\otimes k}$ which are isomorphic to
direct sums of copies of $M_\bullet^{\otimes k}$ (details omitted).
Thus it suffices to prove the vanishing for $M_\bullet^{\otimes k}$.
By the Eilenberg-Zilber theorem
(Theorem \ref{theorem-tensor-simplicial-modules})
and the fact that $M_\bullet$ is flat
we see that $s(M_\bullet^{\otimes k})$ is quasi-isomorphic to
$$
s(M_\bullet) \otimes_B^\mathbf{L} s(M_\bullet)\otimes_B^\mathbf{L}
\ldots \otimes_B^\mathbf{L} s(M_\bullet)
$$
Since $s(M_\bullet)$ is quasi-isomorphic to a complex sitting in
degrees $\leq -1$ (by the assumed vanishing) we get what we want
from our construction of derived tensor products.
\end{proof}
...
$$
E_2^{p, q} = L_{p + q}\pi_!(\text{Sym}^q_{\underline{B}}(\Omega))
\Rightarrow \text{Tor}^A_{p + q}(B, B)
$$
Moreover, $L_i\pi_!(\text{Sym}^k_{\underline{B}}(\Omega)) = 0$
for $i < k$.
\end{theorem}
...
By Lemmas \ref{lemma-vanishing-symmetric-powers} and
\ref{lemma-identify-pi-shriek}
we conclude that
$$
L_i\pi_!(\mathcal{K}^k/\mathcal{K}^{k + 1}) =
L_i\pi_!(\text{Sym}^k_{\underline{B}}(\Omega)) = 0
$$
...
$$
E_1^{p, q} = L_{- p - q}\pi_!(\mathcal{K}^p/\mathcal{K}^{p + 1})
$$
and differentials $E_r^{p, q} \to E_r^{p + r, q - r + 1}$.
After a renumbering we
obtain the spectral sequence of the statement of the theorem. To show
convergence we will show that for every $k$ there exists a $c$ such that
$L_i\pi_!(\mathcal{K}^n) = 0$ for $i < k$ and $n > c$\footnote{A posteriori
the ``correct'' vanishing $L_i\pi_!(\mathcal{K}^n) = 0$ for $i < n$ can be
concluded.}.

\medskip\noindent
Given $k \geq 0$ set $c = k^2$. We claim that
$$
L_i\pi_!(\mathcal{K}^{n + c}) \to L_i\pi_!(\mathcal{K}^n)
$$
...
have $L_i\pi_! = 0$ for $i < n$ (see above). Hence the claim implies
$L_i\pi_!\mathcal{K}^{n + c} = 0$ for $i < k$ and all $n \geq k$
...
hence induces zero on $L_i\pi_!$ for $i < k$, see
