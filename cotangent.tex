...
\begin{proof}
As $L\pi_!$ agrees for modules and abelian sheaves
(Lemma \ref{lemma-properties-lower-shriek-fibred-category})
it suffices to prove this when $\mathcal{F}$ is an
abelian sheaf. Given any object $U = (Q, \beta)$ of
$\mathcal{C}$ the rule
$$
(P, \alpha) \longmapsto
\text{free }\mathbf{Z}\text{-module on }
\Mor_\mathcal{C}((Q, \beta), (P, \alpha))
$$
defines an abelian sheaf $\mathcal{G}$. Note that
$\mathcal{G} = j_{U!}\mathbf{Z}_U$ in the notation of
Modules on Sites, Definition
\ref{sites-modules-definition-localize-ringed-site}. Hence we have
$$
\Hom_{\textit{Ab}(\mathcal{C})}(\mathcal{G}, \mathcal{F}) =
\mathcal{F}(Q, \beta)
$$
for any abelian sheaf $\mathcal{F}$ on $\mathcal{C}$.
Since the topology on $\mathcal{C}$ is chaotic, the section functors
are exact. Hence $\mathcal{G}$ is projective. Every abelian sheaf
is a quotient of a direct sum of $j_{U!}\mathbf{Z}_U$
...
$j_{U!}\mathbf{Z}_U$ with $U = (Q, \beta)$ we reduce to
$\mathcal{G} = j_{U!}\mathbf{Z}_U$. The complex above is then
the complex $s(\mathbf{Z}[S_\bullet])$ associated to the free
$\mathbf{Z}$-module $\mathbf{Z}[S_\bullet]$ on the simplicial set
$$
S_\bullet = \Mor_\mathcal{C}((Q, \beta), (P_\bullet, \epsilon))
$$
We have to show $s(\mathbf{Z}[S_\bullet])$ is acyclic in nonzero degree
and has cohomology module equal to $\mathbf{Z}$ in degree $0$.
Namely, we have $\pi_!j_{U!}\mathbf{Z}_U = \mathbf{Z}$
...
$$
\xymatrix{
F_{b, *} \ar[r] \ar[d] & P_\bullet \ar[d]_\epsilon \\
{*} \ar[r]^b & B
}
$$
...
We conclude that $\mathbf{Z}[S_\bullet] \to \mathbf{Z}$ is
a homotopy equivalence of simplicial abelian groups
(Simplicial, Remark \ref{simplicial-remark-homotopy-better}).
...
assumption of that lemma is satisfied. The second assumption we
can see as follows. By
...
\begin{proof}
This follows from Lemma \ref{lemma-compute-cotangent-complex} and
Lemma \ref{lemma-identify-pi-shriek} which tells us
...
\begin{proof}
Let $P_\bullet \to B$ be a resolution, see Remark \ref{remark-resolution}.
...
$\epsilon' : P_\bullet \otimes_A A' \to B'$ is a quasi-isomorphism, hence
a trivial Kan fibration of simplical sets by
Simplicial, Lemma \ref{simplicial-lemma-qis-simplicial-abelian-groups}.
By Lemma \ref{lemma-identify-pi-shriek}
we see that
$$
\mathcal{F}'(P_\bullet \otimes_A A', \epsilon')
$$
computes $L\pi'_!\mathcal{F}'$ and
$$
(g^{-1}\mathcal{F}')(P_\bullet, \epsilon)
$$
computes $L\pi_!(g^{-1}\mathcal{F}')$. On the other hand, the value of
$g^{-1}\mathcal{F}'$ on $(P_n, \epsilon_n)$ is
$\mathcal{F}'(P_n \otimes_A A', \epsilon_n')$ as the functor $u$
is defined by the base change by $A \to A'$.
Thus the agreement is clear.
\end{proof}
...
\begin{proof}
Observe that $L\pi^{B'}_! \circ Lh^* = Lf^* \circ L\pi^B_!$ where
$L\pi_!^B : D(\mathcal{C}, \underline{B}) \to D(B)$ and
$L\pi_!^{B'} : D(\mathcal{C}, \underline{B'}) \to D(B')$
are the functors constructed in
Lemma \ref{lemma-properties-lower-shriek-fibred-category}.
Namely, both are left adjoint to the obvious functor
$\pi^{-1} : D(B') \to D(\mathcal{C}, \underline{B})$.
In particular
$$
L_{B/A} \otimes_B^\mathbf{L} B' = Lf^*L_{B/A} =
Lf^* L\pi^B_! (\Omega_{\mathcal{O}/A} \otimes_\mathcal{O} \underline{B}) =
L\pi^{B'}_! Lh^*(\Omega_{\mathcal{O}/A} \otimes_\mathcal{O} \underline{B})
$$
the second equality by Lemma \ref{lemma-compute-cotangent-complex}.

\medskip\noindent
Note that $\Omega_{\mathcal{O}/A}$ is a flat $\mathcal{O}$-module,
hence $\Omega_{\mathcal{O}/A} \otimes_\mathcal{O} \underline{B}$ is a flat
$\underline{B}$-module. Thus
...
Thus the result follows from Lemma \ref{lemma-flat-base-change}
