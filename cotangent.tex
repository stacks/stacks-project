...
$L_{X/Y}|_{U_{\acute{e}tale}} = L_{U/V}$ in $D(\mathcal{O}_U)$.
\end{lemma}
...
$g_{small}^{-1}\mathcal{O}_{V_{\acute{e}tale}} =
p_{small}^{-1}f_{small}^{-1}\mathcal{O}_{Y_{\acute{e}tale}}$
because $q_{small}^{-1}\mathcal{O}_{Y_{\acute{e}tale}} =
\mathcal{O}_{V_{\acute{e}tale}}$
(Properties of Spaces, Lemma
\ref{spaces-properties-lemma-etale-exact-pullback}).
Tracing through the definitions we conclude that
$L_{X/Y}|_{U_{\acute{e}tale}} = L_{U/V}$.
\end{proof}
...
$$
where $\underline{\Lambda}$ is the constant sheaf with value
$\Lambda$ on $X_{\acute{e}tale}$.
\end{lemma}

\begin{proof}
Let $p : X \to \Spec(\Lambda)$ be the structure morphism.
Let $q : \Spec(\Lambda)_{\acute{e}tale} \to (*, \Lambda)$
...
$$
(L_q)_{\overline{t}} =
L_{\mathcal{O}_{\Spec(\Lambda)_{\acute{e}tale}, \overline{t}}/\Lambda}
$$
(Lemma \ref{lemma-stalk-cotangent-complex})
is zero. Since $\mathcal{O}_{\Spec(\Lambda)_{\acute{e}tale}, \overline{t}}$
...
Lemma \ref{lemma-compute-cotangent-complex}.
Namely, we construct a category $\mathcal{C}_{X/\Lambda}$
fibred over $X_{\acute{e}tale}$ and endow it with a sheaf of (polynomial)
$\Lambda$-algebras $\mathcal{O}$ such that
$$
L_{X/\Lambda} =
L\pi_!(\Omega_{\mathcal{O}/\underline{\Lambda}} \otimes_\mathcal{O}
...
In other words, $\mathbf{A}$ is an (infinite dimensional) affine space over
$\Spec(\Lambda)$. Morphisms are given by commutative diagrams.
Recall that $X_{\acute{e}tale}$ denotes the small \'etale site of $X$
whose objects are schemes \'etale over $X$.
There is a forgetful functor
$$
u : \mathcal{C}_{X/\Lambda} \to X_{\acute{e}tale},
\quad
...
In the situation above the category
$\mathcal{C}_{X/\Lambda}$ is fibred over $X_{\acute{e}tale}$.
\end{lemma}

\begin{proof}
Given an object $U \to \mathbf{A}$ of $\mathcal{C}_{X/\Lambda}$ and a morphism
$U' \to U$ of $X_{\acute{e}tale}$ consider the object $U' \to \mathbf{A}$ of
$\mathcal{C}_{X/\Lambda}$ where $U' \to \mathbf{A}$ is the composition of
$U \to \mathbf{A}$
and $U' \to U$. The morphism $(U' \to \mathbf{A}) \to (U \to \mathbf{A})$ of
$\mathcal{C}_{X/\Lambda}$ is strongly cartesian over $X_{\acute{e}tale}$.
\end{proof}

\noindent
We endow $\mathcal{C}_{X/\Lambda}$ with the topology inherited from
$X_{\acute{e}tale}$ (see Stacks, Section \ref{stacks-section-topology}).
The functor $u$ defines a morphism of topoi
$\pi : \Sh(\mathcal{C}_{X/\Lambda}) \to \Sh(X_{\acute{e}tale})$.
The site $\mathcal{C}_{X/\Lambda}$ comes with several sheaves of rings.
\begin{enumerate}
\item The sheaf $\mathcal{O}$ given by the rule
$(U \to \mathbf{A}) \mapsto \Gamma(\mathbf{A}, \mathcal{O}_\mathbf{A})$.
...
In the situation above, for every object $U \to X$ of $X_{\acute{e}tale}$
...
\otimes_\mathcal{O} \underline{\mathcal{O}}_X)$
computes the cotangent complex of the map of rings
$\underline{\Lambda} \to \mathcal{O}_X$ on $X_{\acute{e}tale}$.
...
\noindent
In this section we use the category $\mathcal{C}_{X/\Lambda}$ introduced
in the previous section to compute obstructions and infinitesimal deformations
for flat quasi-coherent modules over algebraic spaces.
...
\item a quasi-coherent $\mathcal{O}_{X_A}$-module $\mathcal{F}$ flat over $A$.
\end{enumerate}
...
the problem of lifting $\mathcal{F}$ to a quasi-coherent module
...
of pairs $(\mathcal{F}', \alpha)$ where $\mathcal{F}'$ is a quasi-coherent
module on $X_{A'}$ flat over $A'$ and
...
on $X_{\acute{e}tale}$ flat over $A$. We introduce the category
...
\begin{proof}
Omitted. Hints: $p'_*\mathcal{O}_{X_{A'}} = \mathcal{O}_X \otimes_\Lambda A'$
...
$\textit{QCoh}(X_A) \to
\textit{QCoh}(X_{\acute{e}tale}, \mathcal{O}_X \otimes_\Lambda A)$.
Finally, we can drop the assumption on quasi-coherence as an
extension of quasi-coherent modules is quasi-coherent.
\end{proof}
...
\medskip\noindent
Let $\mathcal{G}$ be a sheaf of $\mathcal{O}_X \otimes_\Lambda A$-modules
on $X_{\acute{e}tale}$ flat over $A$.
...
direction. We omit the verification that these functors are each
others opposites.
\end{proof}
...
$$
of $D(\mathcal{O}_X \otimes_\Lambda A)$.
For any $A'$ as above there exists a canonical
$$
\xi_{A'} \in
\text{Ext}^2_{\mathcal{O}_X \otimes A}(L, \mathcal{G} \otimes_A I)
$$
whose vanishing is a neccessary and sufficient condition for the existence
of a lift $\mathcal{G}'$ of $\mathcal{G}$. The construction of $\xi_{A'}$
is functorial in the thickening $A'$.
\end{lemma}

\begin{proof}
Omitted. Hint: using Lemma \ref{lemma-second-equivalence}
we can transfer the problem to the ringed site $\mathcal{C}_{X/\Lambda}$.
Since $\mathcal{O} \otimes_\Lambda A'$ is flat over $A'$ we can apply
Deformation Theory, Lemma \ref{defos-lemma-flat-ringed-topoi}.
To finish use adjointness properties of the functors in sight
to transfer the Ext groups from $\mathcal{C}_{X/\Lambda}$ back
to $X_{\acute{e}tale}$.
\end{proof}
...
With $\Lambda$, $X$, $A$, $\mathcal{F}$ as above.
There exists a canonical object $L = L(\Lambda, X, A, \mathcal{F})$ of
$D_{\textit{QCoh}}(X_A)$ such that given a surjection
$A' \to A$ of $\Lambda$-algebras with square zero kernel $I$ we have
\begin{enumerate}
...
$X \to \Spec(\Lambda)$ is locally of finite type, $\Lambda$ is Noetherian,
and $\mathcal{F}$ is of finite presentation, then $L$ is
pseudo-coherent.
\end{lemma}
