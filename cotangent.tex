...
\begin{remark}
\label{remark-compare-higher}
One can explicitly compute $\tau_{\geq -2}L_{B/A}$ in the following
manner; see \cite{Lichtenbaum-Schlessinger}.
Let $A \to B$ be a ring map. Choose a polynomial algebra $P$ over $A$
...
Then the complex of $B$-modules
$$
Rel/TrivRel \longrightarrow
F \otimes_P B \longrightarrow \Omega_{P/A} \otimes_P B
$$
is an avatar of $\tau_{\geq -2}L_{B/A}$ in $D(B)$. To find a map from
$L_{B/A}$ into this complex one can argue as in
Remark \ref{remark-explicit-comparison-map}. If we ever need this we will
formulate a precise lemma and give the proof here.
\end{remark}
...
$((X_A)_{\acute{e}tale}, \mathcal{O}_{X_A}) \to
(X_{\acute{e}tale}, \mathcal{O}_X \otimes A)$
