...
$D(\mathcal{X}_{fppf}, \mathcal{O}_\mathcal{X})$ whose cohomology
sheaves are parasitic. Then
...
Let $\mathcal{X}$ be an algebraic stack. Let $K$ be an object of
$D(\mathcal{X}_{fppf}, \mathcal{O}_\mathcal{X})$ such that
$R\Gamma(x, K) = 0$ for all objects $x$ of $\mathcal{X}$ lying
over an affine scheme $U$ such that $U \to \mathcal{X}$ is flat.
Then $H^i(\mathcal{X}, K) = 0$ for all $i$.
\end{lemma}
