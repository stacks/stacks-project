...
\medskip\noindent
Consider the total quotient ring $Q = S^{-1}R$ of $R$. We know that
...
By assumption PT holds for the ring map $S^{-1}R \to S^{-1}\Lambda$.
Hence we can find a factorization $S^{-1}A \to B' \to \Lambda$
with $B'$ smooth over $S^{-1}R$.

\medskip\noindent
We can find a factorization $A \to B \to \Lambda$ with $B$ of finite
presentation over $R$ such that $B' \cong S^{-1}B$ (details omitted).
After a replacement a la Elkik (details omitted) we can assume that
some $\pi \in S$ is strictly standard in $B$ over $R$. Note that $\pi$
is not a zero divisor in $R$ by construction. Thus, after replacing
$A$ by $B$, we see that we may assume that some nonzero divisor $\pi$
maps to a strictly standard element in $A$ over $R$.

\medskip\noindent
We know that $R/\pi^8R \to \Lambda/\pi^8\Lambda$ satisfies PT.
...
we can find a smooth ring map $R \to D$ and a factorization
