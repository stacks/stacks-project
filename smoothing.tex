...
\title{Smoothing Ring Maps; Under Construction}
...
$\text{Ker}(a : S^{-1}M \to S^{-1}M) = \text{Ker}(a^2 : S^{-1}M \to S^{-1}M)$
then there exists an $s \in S$ such that for any $n > 0$ we have
$\text{Ker}(s^na : M \to M) = \text{Ker}((s^na)^2 : M \to M)$.
\end{lemma}
...
Let $I \subset \mathfrak q$ be a prime.
Assume that $\mathfrak q$ is minimal over $I$ and that $\Lambda_\mathfrak q$
is a regular local ring of dimension $d$.
Then there exists an $n > 0$ and
$\lambda_1, \ldots, \lambda_d \in \Lambda$ such that
\begin{enumerate}
\item $\lambda_1^n, \ldots, \lambda_d^n \in I$,
\item $(\lambda_1, \ldots, \lambda_d)\Lambda_\mathfrak q =
\mathfrak q\Lambda_\mathfrak q$, and
\item for $i = 1, \ldots, d$ and any $e \geq 8$ we have
$$
\text{Ann}_{\Lambda/(\lambda_1^e, \ldots, \lambda_{i - 1}^e)\Lambda}
(\lambda_i) =
\text{Ann}_{\Lambda/(\lambda_1^e, \ldots, \lambda_{i - 1}^e)\Lambda}
(\lambda_i^2).
$$
...
\begin{proof}
Sketch (details to be added soon). Set $d = \dim \Lambda_\mathfrak q$.
Choose $n > 0$ and $\lambda_1, \ldots, \lambda_d \in \Lambda$ as in
Lemma \ref{lemma-find-sequence}. Choose generators
$a_1, \ldots, a_r$ of $H_{A/R}$. Choose $\lambda_{ij} \in \Lambda$ such that
$\lambda_i^n = \sum \lambda_{ij}a_j$. Set
$$
B = A[x_1, \ldots, x_d, z_{ij}]/(x_i^n - \sum z_{ij}a_j)
$$
There is a map $B \to \Lambda$ given by $x_i \mapsto \lambda_i$
...
$\Omega_{C_{x_i}/R}$ free. Hence we see that $x_i^e$
is strictly standard in $C/R$ for all $e$ sufficiently large by
Lemma \ref{lemma-compare-standard}.
...
and the sequence of elements $x_1, \ldots, x_d$ of $R$.
...
\Lambda/(\lambda_1^e, \ldots, \lambda_d^e) \supset
\mathfrak q/(\lambda_1^e, \ldots, \lambda_d^e)
$$
for some large $e$ divisible by $8$. By
Lemma \ref{lemma-delocalize-height-zero}
it suffices to resolve this after localizing at $\mathfrak q$.
But since $x_1, \ldots, x_d$ maps to the regular sequence
in $\Lambda_\mathfrak q$ we see that
$$
R/(x_1^e, \ldots, x_d^e) \to 
\Lambda_\mathfrak q/(\lambda_1^e, \ldots, \lambda_d^e)
$$
is a flat ring map of Artinian local rings with separable residue
field extension by assumption. Thus this map is a filtered colimit
...
D \to \Lambda_\mathfrak q/\mathfrak q^n
$$
where the first arrow is essentially smooth and the second is flat,
\item $E$ is contained in $D$ modulo $\mathfrak q^n$.
\end{enumerate}
\end{lemma}
...
$\lambda \in \Lambda_\mathfrak q/\mathfrak q^n\Lambda_\mathfrak q$
there exists a power $q$ of $p$ and a factorization
$$
k[y_1, \ldots, y_m]_\mathfrak p/\mathfrak p^n \to
D \to D' \to \Lambda_\mathfrak q/\mathfrak q^n
$$
...
$\mathfrak q^N \subset \mathfrak h_A\Lambda_\mathfrak q$.
\item
...
Set $B = A[x_1, \ldots, x_d, z_{ij}]/(x_i^N - \sum z_{ij}a_j)$.
\item
\label{item-elkik}
...
Choose $e > 0$ divisible by $8$ such that each $x_i^e$
is strictly standard in $C$ over $k[x_1, \ldots, x_d]$,
\item
\label{item-NP}
Choose an integer $m$ and a ring map
$\varphi : k[y_1, \ldots, y_m] \to \Lambda$ as in
Lemma \ref{lemma-solution-modulo} with $n = N + dc$ and $E$
a set consisting of the images of a set of generators for $A$ over $R$.
\item
\label{item-choose-pii}
Set $\mathfrak p = \varphi^{-1}(\mathfrak q)$. Choose
$\pi_1, \ldots, \pi_d \in \mathfrak p$ which map to a
basis of $\mathfrak p \otimes \kappa(\mathfrak p)$.
\item
...
$$
where $R = k[y_1, \ldots, y_m, t_1, \ldots, t_m]$ and $\gamma_i = \pi_i t_i$,
\item
\label{item-residual-solution}
Choose a factorization
$k[y_1, \ldots, y_m]_\mathfrak p/\mathfrak p^n \to D \to
\Lambda_\mathfrak q/\mathfrak q^n$
such that $D$ is essentially smooth over
$k[y_1, \ldots, y_m]_\mathfrak p/\mathfrak p^n$,
the map $D \to \Lambda_\mathfrak q/\mathfrak q^n$
is injective and there exist $\delta_1, \ldots, \delta_d \in \Lambda$,
$\delta_i \not \in \mathfrak q$ which map to elements of $D$
in the quotient by $\mathfrak q^n$,
there is a map $B \to \Lambda$ with
$x_i \mapsto \pi'_i = \pi_i \delta_i$
and for $i = 1, \ldots, d$ we have
$$
\text{Ann}_{\Lambda/({\pi'}_1^e, \ldots, {\pi'}_{i - 1}^e)\Lambda}({\pi'}_i)
=
\text{Ann}_{\Lambda/({\pi'}_1^e, \ldots, {\pi'}_{i - 1}^e)\Lambda}({\pi'}_i^2).
$$
\item Map $R \to \Lambda$ by $\varphi$ on $k[y_1, \ldots, y_m]$
...
\item We apply Lemma \ref{lemma-lift-solution} to
$$
R
\to
...
\Lambda
$$
the prime ideal $\mathfrak q$ and the elements $\gamma_1, \ldots, \gamma_d$.
We conclude it suffices to resolve the situation after dividing by the ideal
$I = (\gamma_1^e, \ldots, \gamma_d^e)$.
\item We denote
$\mathfrak r \subset R = k[y_1, \ldots, y_m, t_1, \ldots, t_d]$
the inverse image of $\mathfrak q$.
\item It suffices to resolve
$$
(R/I)_\mathfrak r \to
C \otimes_{k[x_1, \ldots, x_d]} (R/I)_\mathfrak r \to
\Lambda_\mathfrak q/I\Lambda_\mathfrak q
$$
\item Set $J = (\pi_1^e, \ldots, \pi_d^e)$ in $k[y_1, \ldots, y_m]$.
\item It suffices to resolve
$$
(R/JR)_\mathfrak p \to
...
\Lambda_\mathfrak q/\mathfrak q^n\Lambda_\mathfrak q
$$
\item It suffices to find a factorization
$$
B \otimes_{k[x_1, \ldots, x_d]} (R/\mathfrak p^nR)_\mathfrak p
\to D \to \Lambda_\mathfrak q/\mathfrak q^n\Lambda_\mathfrak q
$$
\item Such a factorization exists by our choice of $D$ above.
\end{enumerate}
We now give the justification for each of the steps.

\medskip\noindent
Items (\ref{item-power}), (\ref{item-generators}),
(\ref{item-dimension}), (\ref{item-standardizer}),
(\ref{item-elkik}) are clear.

\medskip\noindent
Ad (\ref{item-strictly-standard}). Note that $A_{a_i}$ is smooth
over $k$. Hence $B_{a_j}$, which is isomorphic to a polynomial
algebra over $A_{a_j}[x_1, \ldots, x_d]$, is smooth over $k[x_1, \ldots, x_d]$.
Thus $B_{x_i}$ is smooth over $A$. By
Lemma \ref{lemma-improve-presentation}
