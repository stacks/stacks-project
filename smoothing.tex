...
\end{equation}
of $C$-modules. Let $h \in R[x_1, \ldots, x_n]$ with image $a \in A$.
We will use as presentations for the localized rings
$$
...
$$
Since $\text{d}z_i = \text{d}x_i$ modulo
...
and $0 \leq c \leq \min(n, m)$ such that
$$
\pi = a' \det(\partial f_j/\partial x_i)_{i, j = 1, \ldots, c}
$$
...
$$
\pi = \rho(a') \det(r_{ji})_{i, j = 1, \ldots, c}
\bmod \pi^4R
$$
