...
\bmod \pi^4R
$$
Thus, using the adjoint matrix, we see there exists a $c \times c$ matrix
$s_{ij}$ with coefficients in $R$ such that
$\sum_{i = 1, \ldots, c} r_{ji} s_{ik} = u \pi  \delta_{jk}$
(Kronecker delta) for $j, k \in \{1, \ldots, c\}$ where $u \in R$ is an
element congruent to $1$ modulo $\pi$. We introduce auxiliary variables
...
$A \to B \to \Lambda$ such that some $s \in S$ maps to a strictly standard
element in $B$ over $R$.
\end{lemma}

\begin{proof}
We first apply Lemma \ref{lemma-smooth-standard-smooth} to $S^{-1}R \to B'$.
This allows us to reduce to the case where $B'$ is standard smooth over
$S^{-1}R$. Write $B' = S^{-1}R[x_1, \ldots, x_n]/(f_1, \ldots, f_c)$ where
$\det(\partial f_j/\partial y_i)_{j = 1, \ldots, c, i = 1, \ldots, c}$
is invertible in $B'$, in other words there exists a relation
$$
1 =
a_0 \det(\partial f_j/\partial y_i)_{j = 1, \ldots, c, i = 1, \ldots, c}
+
\sum\nolimits_{j = 1, \ldots, c} a_jf_j
$$
for some $a_j \in S^{-1}R[x_1, \ldots, x_n]$. Since each element of $S$ is
invertible in $B'$ we may (by clearing denominators) assume that
$f_j, a_j \in R[x_1, \ldots, x_n]$ and that
$$
s_0 = a_0 \det(\partial f_j/\partial y_i)_{j = 1, \ldots, c, i = 1, \ldots, c}
+
\sum\nolimits_{j = 1, \ldots, c} a_jf_j
$$
for some $s_0 \in S$. Write $A = R[y_1, \ldots, y_m]/(g_1, \ldots, g_t)$.
Let $s_j \in S$ and $h_j \in R[x_1, \ldots, x_n]$ be such that the image
of $y_j$ in $B'$ is the class of $h_j/s_j$. Using the Euclidean algorithm
we see that for some suitably large $N$ we have
$$
(s_1 \ldots s_m)^Ng_j = G_j(x_1, \ldots, x_n)
\bmod (s_1y_1 - h_1, \ldots, s_my_m - h_m)
$$
in $R[x_i, y_j]$. Since $R[x_1, \ldots, x_n] \to A \to B'$ maps
$g_j$ to zero we conclude that $s'_j G_j \in (f_1, \ldots f_c)$ for
some $s'_j \in S$. Consider the ring
$$
B = R[x_1, \ldots, x_n, y_1, \ldots, y_m]/
(f_j, g_i, s_jy_j - h_j)
$$
We claim that $s = s_0(s_1 \ldots s_m)^N s'_1 \ldots s'_m$
is strictly standard in $B$ over $R$ which finishes the proof.
Namely we will use the equations $f_1, \ldots, f_c, s_1y_1 - h_1, \ldots,
s_my_m - h_m$ and the coordinates $x_1, \ldots, x_c, y_1, \ldots, x_m$.
First note that the determinant of the matrix
$$
\left(
\begin{matrix}
\partial f_j/\partial x_i & \partial (s_jy_j - h_j) / \partial x_i \\
0 & \partial (s_jy_j - h_j) / \partial y_k
\end{matrix}
\right)
=
\left(
...
\right)
$$
times $a_0$ is equal to $s_0s_1 \ldots s_m$ hence divides $s$.
Finally, the condition $s g_j \in (f_1, \ldots, f_c, s_1y_1 - h_1, \ldots,
s_my_m - h_m)$ follows from our choice of $s$.
\end{proof}
...
some $\pi \in S$ is strictly standard in $B$ over $R$.
After replacing $A$ by $B$ we may assume that $\pi$ is
strictly standard in $A$. We know that
