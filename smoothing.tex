...
\noindent
A reference for this section is \cite[Section 39]{MatCA}
which in turn uses an argument of \cite{Faltings-einfacher}.
We haven't (yet) defined formal smoothness of rings relative
to a topology thus the characterization in terms of formal smoothness
is missing.
...
\begin{proof}
Combine
More on Algebra, Lemma \ref{more-algebra-lemma-transitive-colimit-lci-at-end}
with
Algebra, Lemma \ref{algebra-lemma-colimit-syntomic}.
\end{proof}
...
and that $H_1(L_{K/\mathbf{F}_p}) = 0$
as the extension $K/\mathbf{F}_p$ is separable, see
Algebra, Proposition
\ref{algebra-proposition-characterize-separable-field-extensions}.
...
\medskip\noindent
Proof of (2) $\Rightarrow$ (4) following Faltings.
Let $a_1, \ldots, a_n \in k$ be elements
...
$\text{Ker}(\beta)$ and we have to show that are linearly indepdent, i.e.,
