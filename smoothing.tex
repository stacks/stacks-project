...
\noindent
The following proposition was proved for henselian pairs by
Elkik in \cite{Elkik}. In the form stated below it can be found in
\cite{Arabia}, where they also prove that ring maps between smooth
algebras can be lifted.

\begin{proposition}
\label{proposition-lift-smooth}
Let $R$ be a ring and let $I \subset R$ be an ideal.
Let $R/I \to \overline{A}$ be a smooth ring map. Then
there exists a smooth ring map $R \to A$ such that
$A/IA$ is isomorphic to $\overline{A}$.
\end{proposition}

\begin{proof}
Choose a presentation $\overline{A} = (R/I)[x_1, \ldots, x_n]/\overline{J}$.
Set $\overline{C} = \text{Sym}^*_{\overline{A}}(\overline{J}/\overline{J}^2)$.
Note that $\overline{J}/\overline{J}^2$ is a finite projective
$\overline{A}$-module (follows from the definition of smoothness).
By Lemma \ref{lemma-improve-presentation} and its proof
the ring map $\overline{A} \to \overline{C}$ is smooth and we can
find a presentation $\overline{C} = R/I[y_1, \ldots, y_m]/\overline{K}$
with $\overline{K}/\overline{K}^2$ free over $\overline{C}$.
By Algebra, Lemma \ref{algebra-lemma-huber} we can even assume that
$\overline{C} = R/I[y_1, \ldots, y_m]/(\overline{f}_1, \ldots, \overline{f}_c)$
where $\overline{f}_1, \ldots, \overline{f}_c$ maps to a basis of
$\overline{K}/\overline{K}^2$ over $\overline{C}$. Choose
$f_1, \ldots, f_c \in R[y_1, \ldots, y_c]$ lifting
$\overline{f}_1, \ldots, \overline{f}_c$ and set
$$
C = R[y_1, \ldots, y_m]/(f_1, \ldots, f_c)
$$
By construction $C/IC = \overline{C}$. Consider the naive cotangent complex
$$
(f_1, \ldots, f_c)/(f_1, \ldots, f_c)^2
\longrightarrow
\bigoplus\nolimits_{i = 1, \ldots, m} C \text{d}y_i
$$
associated to the presentation of $C$. For every prime $\mathfrak q \supset IC$
of $C$ the images $\text{d}f_j$ are linearly independent in
$\bigoplus \kappa(\mathfrak q)\text{d}y_i$ because $\overline{C}$
is smooth over $R/I$. Hence we conclude that
$((f_1, \ldots, f_c)/(f_1, \ldots, f_c)^2)_{\mathfrak q}$ is free
of rank $c$ and maps to a direct summand of
$\bigoplus C_{\mathfrak q}\text{d}y_j$. Hence $R \to C$ is smooth at
$\mathfrak q$, see
Algebra, Lemma \ref{algebra-lemma-smooth-at-point}.
Thus we can find a $g \in C$ mapping to an invertible element
of $C/IC$ such that $R \to C_g$ is smooth, see
More on Algebra, Lemma \ref{more-algebra-lemma-localize-upstairs}.
We conclude that there exists a finite projective $\overline{A}$-module
$\overline{P}$ such that
$\overline{C} = \text{Sym}^*_{\overline{A}}(\overline{P})$
is isomorphic to $C/IC$ for some smooth $R$-algebra $C$.

\medskip\noindent
Choose an integer $n$ and a direct sum decomposition
$\overline{A}^{\oplus n} = \overline{P} \oplus \overline{Q}$.
...
$\overline{Q} \otimes_{\overline{A}} C/IC$.
...
\begin{align*}
D/ID & =
\text{Sym}_{C/IC}^*(\overline{Q} \otimes_{\overline{A}} C/IC) \\
& =
\text{Sym}_{\overline{A}}^*(\overline{Q}) \otimes_{\overline{A}} C/IC \\
& =
\text{Sym}_{\overline{A}}^*(\overline{Q}) \otimes_{\overline{A}}
\text{Sym}_{\overline{A}}^*(\overline{P}) \\
& =
\text{Sym}_{\overline{A}}^*(\overline{Q} \oplus \overline{P}) \\
& =
\text{Sym}_{\overline{A}}^*(\overline{A}^{\oplus n}) \\
& =
\overline{A}[x_1, \ldots, x_n]
\end{align*}
Choose $f_1, \ldots, f_n \in D$ which map to $x_1, \ldots, x_n$
in $D/ID = \overline{A}[x_1, \ldots, x_n]$. Set $A = D/(f_1, \ldots, f_n)$.
Note that $\overline{A} = A/IA$.
By an argument similar to the argument in the first paragraph of the
proof we see that $R \to A$ is smooth at all primes of $IA$.
Hence, after replacing $A$ by $A_f$ for a suitable $f \in A$
(see More on Algebra, Lemma \ref{more-algebra-lemma-localize-upstairs})
we win.
\end{proof}
...
\section{Application to the structure of formally smooth ring maps}
\label{section-application}

\noindent
For now we put here a list of things to prove in order:
\begin{enumerate}
\item A Noetherian regular local ring $(A, \mathfrak m)$ is formally smooth
over $\mathbf{Q}$, $\mathbf{F}_p$, or $\mathbf{Z}$ depending on whether
$A$ has characteristic zero, $p$, or has mixed characteristic.
Hint: Easy with material already available.
\item Let $A' \to A \to B$ be local ring homomorphism of Noetherian complete
local rings with $B/\mathfrak m_AB$ regular and $A' \to A$ surjective.
...
$$
with $A' \to B'$ flat, $B' \to B$ surjective and with
$B'/\mathfrak m_{A'}B' = B/\mathfrak m_AB$. Hint: this can be done
whenever $B/\mathfrak m_A B$ is a complete intersection ring.
\item A local ring homomorphism $A \to B$ of Noetherian local rings
such that
\begin{enumerate}
\item $A \to B$ is flat, and
\item $B/\mathfrak m_AB$ is geometrically regular over $A/\mathfrak m_A$
\end{enumerate}
is formally smooth in the $\mathfrak m_B$-adic topology.
Hint: Reduce to the complete case. Reduce to the case where $A$ is
regular by a previous result. In this case $B$ is regular hence formally
smooth over $\mathbf{Z}$. Finish the argument using the field case.
\item If $A \to B$ is a colimit of smooth algebras, then
$H_1(L_{B/A}) = 0$ and $\Omega_{B/A}$ is a flat $B$-module.
In other words $NL_{B/A}$ is quasi-isomorphic to a flat module
placed in degree $0$.
\item If $A$ is a regular local ring, then $NL_{A/\mathbf{Z}}$
(mixed characteristic case, or characteristic $0$ case)
or $NL_{A/\mathbf{F}_p}$ (characteristic $p$ case) is quasi-isomorphic
to a flat module placed in degree $0$.
\item If $A \to B$ is a flat morphism of Noetherian complete local rings
with $B/\mathfrak m_AB$ geometrically regular over $k = A/\mathfrak m_A$
then the map $\Omega_A \otimes_A B \to \Omega_B$ is injective.
Hint: If $K = B/\mathfrak m_A B$ is a field, then $K/k$ is separable.
In this case we reduce to the case $k[[x_i]] \to K[[x_i]]$ or
similarly for Cohen rings. By flatness of $\Omega$'s it suffices to
prove that $f.f.(k[[x_i]]) \to f.f.(K[[x_i]])$ is separable which is
discussed in \cite{Chevalley-power-series}. The mixed characteristic
case or characteristic zero case is immediate.
The general case, i.e., $\dim(B/\mathfrak m_AB) > 0$ follows from
the previous case by localizing.
\item Show that if $A$ is a Noetherian local ring over a field $k$
and $A$ is not regular, then $L_{A/k}$ is not quasi-isomorphic to
a flat module placed in degree $0$. Hint: Look at
$A/\mathfrak m^n$.
\item A local ring homomorphism $A \to B$ of local rings
such that
\begin{enumerate}
\item $A \to B$ is flat,
\item $A$ and $B$ are complete Noetherian local rings, and
\item $B/\mathfrak m_AB$ is geometrically regular over $A/\mathfrak m_A$
\end{enumerate}
is regular. Hint: Reduce to the case $A$ formally smooth. Show that
$L_{B/A}$ is quasi-isomorphic to a flat module placed in degree $0$.
Conclude.
\item With the previous result in hand go back to thinking about
G-rings, etc.
\end{enumerate}
