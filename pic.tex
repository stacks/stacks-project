...
$H^i(K)|_{U'} = 0$ for $i \not = 0$. Thus $K|_{U'} = H^0(K)|_{U'}[0]$.
It follows from
More on Algebra, Lemma \ref{more-algebra-lemma-isolate-a-cohomology-group}
(applied with $i = 0$) that $H^0(K)|_{U''}$ is finite projective
on some affine neighbourhood $U'' \subset U'$ of $s$.
Shrinking further to some affine open neighbourhood
$U''' \subset U''$ we may assume that $H^0(K)|_{U'''}$ is
free of rank $1$ (Algebra, Lemma \ref{algebra-lemma-finite-projective}).
...
\begin{proof}
Set $S = \Spec(k)$. The meaning of the phrase ``has a $k$-rational point''
is of course exactly that the structure morphism $f : X \to S$ has a section.
Since $S$ is the spectrum of a field, any morphsm $T \to S$ is flat.
