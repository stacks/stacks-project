...
for all $T \in \Ob((\Sch/S)_{fppf})$. Then
$$
0 \to \text{Pic}(T) \to \text{Pic}(X_T) \to \text{Pic}_{X/S}(T) \to 0
$$
...
for the fppf topology because it is isomorphic to $\text{Pic}_{X/k}$.
...
$$
\text{Pic}(X) = \text{Pic}(X_{k'})^{\text{Gal}(k'/k)}
$$
Moreover the action of $\text{Gal}(k'/k)$ on $\text{Pic}(X_{k'})$
is continuous.
\end{lemma}
...
$X_{k'} = X \times_{\Spec(k)} \Spec(k')$, and since $\text{Pic}(-)$
is a contravariant functor, it acts (from the left) on $\text{Pic}(X_{k'})$.
...
and we obtain
$$
\text{Pic}(X_{k'}) = \colim \text{Pic}(X_{k_\lambda})
$$
...
It follows that every element of $\text{Pic}(X_{k'})$ is fixed by
...
Let $\mathcal{L}$ be an element of $\text{Pic}(X_{k'})$ fixed by $G$.
...
and that $\sigma$ acts on $\text{Pic}(X_{k'})$ by pulling back by $f_\sigma$.
...
Then the map
$$
\text{Pic}(X)[n] \longrightarrow \text{Pic}(X_{k'})[n]
$$
is bijective for any purely inseparable extension $k'/k$.
\end{lemma}

\begin{proof}
First we observe that the map $\text{Pic}(X) \to \text{Pic}(X_{k'})$
...
which has order dividing $n$ in $\text{Pic}(X_{k'})$.
...
is a torsion element of $\text{Pic}(X)$ of the same order as $\mathcal{L}$.
