...
$\text{Hilb}^d_{X/S}$ and the transformation
$\text{Hilb}^d_{X/S} \to \text{Hilb}^d_{Y/S}$
...
\noindent
A few remarks on divisors on smooth curves.

\begin{lemma}
\label{lemma-divisors-on-curves}
Let $X \to S$ be a smooth morphism of schemes of relative dimension $1$.
Let $D \subset X$ be a closed subscheme. The following are equivalent
\begin{enumerate}
\item $D$ is a relative effective Cartier divisor on $X/S$, and
\item $D \to S$ is locally quasi-finite, flat, and
locally of finite presentation.
\end{enumerate}
If $S$ is Noetherian, then (1) and (2) are also equivalent to
\begin{enumerate}
\item[(3)] $D \to S$ is locally quasi-finite and flat.
\end{enumerate}
If $X \to S$ is proper (and $S$ arbitrary), then (1) and (2)
are also equivalent to
\begin{enumerate}
\item[(4)] $D \to S$ is finite locally free.
\end{enumerate}
\end{lemma}

\begin{proof}
Equivalence of (1) and (2). This follows from
Divisors, Lemma \ref{divisors-lemma-fibre-Cartier}
if we can show the equivalence of (1) and (2) when
$S$ is the spectrum of a field $k$. Since $X$ is smooth of dimension $1$
over $k$, we see that $X$ is a disjoint union of smooth curves
over $k$. Hence the local rings at closed points are discrete
valuation rings. It follows that every sheaf of ideals
