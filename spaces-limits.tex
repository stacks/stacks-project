...
Thus we know that
$\textit{QCoh}(\mathcal{O}_U) = \colim \textit{QCoh}(\mathcal{O}_{U_i})$,
$\textit{QCoh}(\mathcal{O}_R) = \colim \textit{QCoh}(\mathcal{O}_{R_i})$,
and
$\textit{QCoh}(\mathcal{O}_{R \times_{s, U, t} R}) =
\colim \textit{QCoh}(\mathcal{O}_{R_i \times_{s_i, U_i, t_i} R_i})$ by
Limits, Lemma \ref{limits-lemma-descend-modules-finite-presentation}.
We have $\textit{QCoh}(\mathcal{O}_X) = \textit{QCoh}(U, R, s, t, c)$ and
$\textit{QCoh}(\mathcal{O}_{X_i}) = \textit{QCoh}(U_i, R_i, s_i, t_i, c_i)$,
