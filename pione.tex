...
the complement of the ramification locus might be affine.
In this section we prove this to be the case when the schemes
are equicharacteristic and excellent.
...
$$
The second equality by the torsion property we just proved and
...
Hence we only care about $x$ whose local ring has dimension $\geq 2$.
But if $\dim(\mathcal{O}_{X, x}) \geq 2$ then purity of
...
Whence $e(V) \leq d -  2$. Thus $V$ is affine by Lemma \ref{lemma-conclude}.
\end{proof}
