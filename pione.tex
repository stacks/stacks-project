...
and $\Ker(h)$ is the smallest closed normal subgroup containing $\Im(h')$, and
...
finite coproduct of final objects.
\end{enumerate}
\end{lemma}
...
this implies $\Ker(h) \subset N$ as desired.
...
on $X'$ factors through $G$ as desired.
\end{proof}
...
vanishing linear term (Fields, Lemma \ref{fields-definition-separable}).
...
the morphism $\Spec(R^I) \to \Spec(R)$ is smooth at $R^I \cap \mathfrak q$.
...
\medskip\noindent
Let $(A, \mathfrak m, \kappa)$ be a discrete valuation ring with
...
$U^{sh}$, $U^h$, $U$ are the punctured spectra of $A^{sh}$, $A^h$,
and $A$. The label $1$ means that the map is trivial; this follows from
Lemma \ref{lemma-local-exact-sequence} among other things.
...
On the other hand,
More on Algebra, Lemma \ref{more-algebra-lemma-inertial-invariants-unramified}
applied to all finite Galois subextensions of $K^{sep}$
...
extensions of $A^h$ which correspond $1$-to-$1$
with finite separable extension of $\kappa$, see
...
as this is true for finite Galois extensions.
...
$\text{Gal}(K^{sep}/K) \to \text{Gal}(M/K)$
follows from the result of Lemma \ref{lemma-local-exact-sequence} for
$\pi_1(U^{sh}) \to \pi_1(U) \to \pi_1(X) \to 1$.
