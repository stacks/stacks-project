...
finite \'etale morphism $f : X \to \mathbf{A}^n_k$ such that
...
\medskip\noindent
Setting $X = \Spec(C)$ we conclude. Namely, the morphism
...
Let $p$ be a prime number. Let $i : Y \to X$ be a closed immersion
of connected affine schemes over $\mathbf{F}_p$. For any geometric
point $\overline{y}$ of $Y$ the map
$$
\pi_1(Y, \overline{y}) \to \pi_1(X, \overline{y})
$$
is injective.
...
\begin{proof}
Let $V \to Y$ be a finite \'etale morphism. It suffices to construct
a finite \'etale morphism $U \to X$ such that $V$ is isomorphic to
an open and closed subscheme of $Y \times_X U$, see
Lemma \ref{lemma-functoriality-galois-injective}.
Write $Y = \Spec(A)$ and $X = \Spec(R)$ so the closed immersion
$Y \to X$ is given by a surjection $R \to A$. By Lemma \ref{lemma-limit}
we can find a finitely generated $\mathbf{F}_p$-subalgebra
$A'$ such that $V \to Y$ comes from a finite \'etale $A'$-algebra $B'$.
In other words, we may assume $V = Y \times_{\Spec(A')} \Spec(B')$ with
$A' \to B'$ finite \'etale.

\medskip\noindent
Choose a surjection $\mathbf{F}_p[x_1, \ldots, x_n] \to A'$.
By the universal property of polynomial rings we can find a
commutative diagram
$$
\xymatrix{
\mathbf{F}_p[x_1, \ldots, x_n] \ar[r] \ar[d]^\chi &
A' \ar[r] \ar[d] &
B'  \ar[d] \\
R \ar[r] &
A \ar[r] &
...
$$
of $\mathbf{F}_p$-algebras. By Lemma \ref{lemma-dominate-affine-space} we can
find a finite \'etale morphism $U' \to \mathbf{A}^n_{\mathbf{F}_p}$
such that $\Spec(B')$ is isomorphic to an open and closed
subscheme of $U' \times_{\mathbf{A}^n_{\mathbf{F}_p}} \Spec(A')$.
Taking $U = U' \times_{\mathbf{A}^n_{\mathbf{F}_p}} X$
we conclude.
\end{proof}
