...
\item $H : \mathcal{C} \to \mathcal{C}'$ is fully faithful, and
\item for any object $X$ of $\mathcal{C}$ the map
$\Mor_\mathcal{C}(*, X) \to \Mor_{\mathcal{C}'}(*', H(X))$
is bijective where $*$ and $*'$ are the final objects of
$\mathcal{C}$ and $\mathcal{C}'$.
\end{enumerate}
\end{lemma}

\begin{proof}
The equivalence of (1) and (2) means that the functor
...
The equivalence of (1) and (3) says that $h$ is surjective if and only
if given any finite $G'$-set $X'$ the $G'$-fixed elements in $X'$
are the same as the $G$-fixed elements. This follows by the same reasoning.
\end{proof}
...
Lemma \ref{lemma-etale-makes-integral-split} and we find an
...
\section{Galois action on the geometric fundamental group}
