...
Let $A$ be a normal local domain of dimension $2$ with dualizing
module $\omega_A$. Let $f : X \to \Spec(A)$ be a resolution of
singularities. Let $\omega_X$ be as in Resolution of Surfaces,
...
Thus the trace map is a map $f_*\omega_X \to A$.
...
This composition is an isomorphism as it is an isomorphism
over the punctured spectrum of $A$ (by the assumption
in the lemma and the fact that $f$ is an isomorphism
over the punctured spectrum) and $A$ and $\omega_A$
are $A$-modules of depth $2$.
Hence $f_*\omega_X \to \omega_A$ is surjective.
We conclude that $Rf_*\omega_X = \omega_A$
which by duality implies $Rf_*\mathcal{O}_X = \mathcal{O}_{\Spec(A)}$.
Thus $H^1(X, \mathcal{O}_X) = 0$
which implies that $A$ defines a rational singularity.
If $f$ is minimal, then $E = 0$ because the map
$f^*\omega_A \to \omega_X$ is surjective by
a repeated application of Resolution of Singularities, Lemma
...
\begin{enumerate}
\item $A$ is an excellent regular local ring $A$,
...
Let $W \to \Spec(\mathcal{O}_{X, x})$ be a minimal
resolution of singularities, see Resolution of Surfaces, Theorem \ref{}.
After shrinking $X$, we can find a proper morphism $b : X' \to X$ such that
$X' \times_X \Spec(\mathcal{O}_{X, x}) =  W$. After shrinking $X$
further, we may assume $X'$ is regular (because $X'$ is a regular scheme
and hence the regular locus is open). Let $U = X \setminus \overline{\{x\}}$.
...
\medskip\noindent
Set $\omega_{X/Y} = H^0(f^!\mathcal{O}_Y)$. This is a coherent
$\mathcal{O}_X$-module which is $(S_2)$ and we have
$f^!\mathcal{O}_Y = \omega_{X/Y}[0]$. Moreover, after possibly
shrinking $X$ we may assume $X$ is Cohen-Macaulay and then $\omega_{X/Y}$
is a dualizing module for the scheme $X$.
By Duality for Schemes, Lemma \ref{} there exists a map
$$
\mathcal{O}_X \longrightarrow \omega_{X/Y}
$$
which is an isomorphism over $U$.
It follows that this map is an isomorphism.
In particular we see that $X$ is Gorenstein.

\medskip\noindent
Set $\omega_{X'/Y} = H^0((f')^!\mathcal{O}_Y)$. Exactly as above
we find that $(f')^!\mathcal{O}_Y = \omega_{X'/Y}[0]$
is a dualizing complex for $X'$. Since $X'$ is regular
the morphism $X' \to Y$ is a local complete intersection morphism.
By Duality for Schemes, Lemma \ref{} there exists a map
$$
...
we find $E|_W = 0$.
This means that $f' : X' \to Y$ is \'etale by
Duality for Schemes, Lemma \ref{}.
...
Then $X_0 \to Y_0$ is \'etale away from the fibre over
the closed point, hence $X_0$ is normal at those points.
Since $d > 2$ we have $\dim(\mathcal{O}_{X_0, x}) = d - 1$
is $\geq 2$.
The normalization $X_0' \to X_0$ is finite.
Let $x' \in X_0'$ be a point mapping to $x$.
By induction hypothesis the morphism
$X'_0 \to Y$ is \'etale at $x'$. From the inclusions
