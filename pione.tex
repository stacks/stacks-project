...
and we relate it to our discussion of the fundamental
groups of schemes.
...
\begin{proof}
(A normal local ring is a domain by definition.)
Let $(A, \mathfrak m, \kappa)$ be normal local with separably
closed fraction field $K$. Let us check that $A$ is henselian.
...
\medskip\noindent
If $A$ were not strictly henselian, then $\kappa$ would
not be separably algebraically closed. Then we would be able
to find a monic polynomial $f$ such that $\overline{f}$ is
an irreducible separable polynomial of degree $> 1$. (Hint: The map
$A[T] \to \kappa[T]$ defines a surjection on sets of monic
polynomials.) Arguing as above we find that $f$ has degree $1$.
...
\end{equation}
The label $1$ means that the mao is trivial; this follows from
