...
Then $I = \text{Ker}(\sigma)$ is the part of degree $> 0$ and we see that
...
the local ring $C_{\mathfrak q}$ has to be an isomorphism. By
Algebra, Lemma \ref{algebra-lemma-smooth-at-point}
we see that $A \to C$ is smooth at the prime $\overline{\mathfrak q}$
of $C$ corresponding to $\mathfrak q$ and since
$\Omega_{C/A, \mathfrak q} = 0$ it is \'etale at $\overline{\mathfrak q}$.
Thus $A \to C$ is \'etale at all primes of $C$ containing $JC$.
This implies we can find an $f \in C$, $f \not \in JC$ such that
$A \to C_f$ is \'etale. Replace $C$ by $C_f$. It follows that
$C/IC = C/JC \times R$ for some $A/I$ algebra $R$. Pick $f \in C$
mapping to $1$ in $C/JC$ and mapping to $0$ in $R$. Set
$A' = C_f$ to conclude the proof. Some details omitted.
\end{proof}
