...
\begin{proof}
Say $\deg(\overline{g}) = n$ and $\deg(\overline{h}) = m$ so
...
Let $R$ be a domain. Let $M$ be an $R$-module.
The $M$ is torsion free if and only if $M_\mathfrak m$ is a
...
We have seen the equivalence of (3) and (4) in
...
implies (4). Conversely, assume (4). If $M$ has torsion,
...
\item $\text{depth}(M_\mathfrak p) \geq 1$.
\end{enumerate}
...
This is impossible in both cases (1) and (2). Hence
...
$\text{depth}(N_\mathfrak p) \geq 1$.
\end{enumerate}
Then $\varphi$ is an isomorphism.
\end{lemma}

\begin{proof}
By Lemma \ref{lemma-inj-depth-1} we see that $\varphi$ is injective.
Let $N' \subset N$ be an finitely generated $R$-module containing
the image of $M$. Then $\text{depth}(N_\mathfrak p) \geq 1$ implies
$\text{depth}(N'_\mathfrak p) \geq 1$. Hence the assumptions of
the lemma hold for $\varpi : M \to N'$. In order to prove that
$\varphi$ is an isomorphism, it suffices to prove the same thing
for every such $N' \subset N$. Thus we may assume $N$ is a finite $R$-module.
...
\begin{proof}
This is a special case of Lemma \ref{lemma-isom-depth-2}.
\end{proof}
...
We apply Lemma \ref{lemma-isom-depth-2} to the map $j : M \to N$.
This is allowed because in case (2)(a) the map
$M_\mathfrak p \to N_\mathfrak p$ is an isomorphism and
in case (2)(b) we can apply Lemma \ref{lemma-hom-into-depth}
to see that $N_\mathfrak p$ has depth at least $1$.
\end{proof}
...
\ref{algebra-lemma-hathat-finitely-generated} we have
...
(and Algebra, Lemmas \ref{algebra-lemma-hathat} and
\ref{algebra-lemma-hathat-finitely-generated}),
...
Lemma \ref{lemma-henselization}.
Then $A^h$ is a Noetherian G-ring.
\end{lemma}
...
Let $\mathfrak q^h$ be a prime of the middle ring lying
...
\noindent
Let $R$ be a ring. An {\it invertible} $R$-module is a finite locally
free module of rank $1$. The set of isomorphism classes of these
...
L^{-1} = \Hom_R(L, R),
$$
namely, the evaluation map $L \otimes_R L^{-1} \to R$ is an isomorphism.
