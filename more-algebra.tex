...
objects in the derived categories. In the following remark we discuss how to
construct an $R\lim$ of such a system
well defined up to noncanonical isomorphism.

\begin{remark}
\label{remark-how-unique}
Suppose that we have an inverse system of rings $(A_n)$ as above.
Now suppose that we have an inverse system of objects $K_n^\bullet$
of $D(A_n)$. More precisely, suppose that we are given
\begin{enumerate}
...
\end{enumerate}
To be completely clear, by our definitions each $K_n^\bullet$ is a
complex of $A_n$-modules, but the maps $\varphi_n$ are in the derived
...
and isomorphisms $\psi_n : M_n^\bullet \to K_n^\bullet$ in $D(A_n)$ such that
the diagrams
$$
\xymatrix{
...
$$
commute in $D(A_{n + 1})$. Namely, set $M_1^\bullet = K_1^\bullet$. Suppose
we have constructed
...
(Injectives, Lemma \ref{injectives-lemma-derived-products}).
By the discussion in Remark \ref{remark-how-unique} we can also lift
...
Derived Categories, Section \ref{derived-section-derived-limit}.
We will use these facts in this section without further mention.
...
\begin{proof}
This is true because
$\Hom_B(N^\bullet, \Hom_A(B, I^\bullet)) = \Hom_A(N^\bullet, I^\bullet)$
...
for $i = 1, \ldots, r$. By Lemma \ref{lemma-when-surjective-to-completion}
...
Thus $\mathcal{C}$ is a weak Serre subcategory by
