...
Smoothing Ring Maps, Section \ref{}.
...
$\Spec(B) \to \Spec(A)$ is closed, we can find an $a \in A$
which maps to an invertible element of $A/I$ whose image in
$B$ lies in $(b_1, b_2)$. After replacing $A$ by the localization
$A_a$ we get that $(b_1, b_2) = B$. Then $\Spec(B) = D(b_1) \amalg D(b_2)$;
