...
On the other hand, a pseudo-coherent module is finite. Hence (3) holds.
Suppose that $K^\bullet$ is an $m$-pseudo-coherent complex.
...
Thus it is clear that each $H^i(K^\bullet)$, $i \geq m$ is finite.
Conversely, assume $K^\bullet$ is an object of $D^{-}(R)$ with
$H^i(K^\bullet)$, $i \geq m$ finite. Let $n$ be the largest integer
such that $H^n(K^\bullet)$ is nonzero. If $n < m$, then $K^\bullet$
is $m$-pseudo-coherent by
Lemma \ref{lemma-recognize-pseudo-coherent}.
If $n \geq m$, then we have the distinguished triangle
$$
(\tau_{\leq m - 1}K^\bullet, K^\bullet, H^m(K^\bullet)[-m])
$$
Since we have seen that $H^m(K^\bullet)[-m]$ is pseudo-coherent, we
can use
Lemma \ref{lemma-cone-pseudo-coherent}
to see that it suffices to prove that $\tau_{\leq m - 1}K^\bullet$
is $m$-pseudo-coherent. By induction on $n$ we win, i.e., (1) holds.
...
of finite tor dimension. Then $K^\bullet$ has finite tor dimension.
\end{lemma}
...
\begin{proof}
This is true because
$K^\bullet \otimes_A^{\mathbf{L}} M = K^\bullet \otimes_B^{\mathbf{L}} M$
...
\end{enumerate}
An $R$-module $M$ is perfect if and only if it has a finite
...
Lemma \ref{lemma-finite-push-pseudo-coherent})
that $K^\bullet \otimes_A A_g$ is $m$-pseuod-coherent as a complex of
