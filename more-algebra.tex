...
to be, well, compact. Below we will show that if $\mathcal{D}$
is the derived category of a ring, then the compact objects are
exactly the perfect objects.

\begin{lemma}
\label{lemma-compact-objects-subcategory}
Let $\mathcal{D}$ be a (pre-)triangulated category with direct sums.
Then the compact objects of $\mathcal{D}$ form the objects of a saturated,
strictly full, triangulated subcategory $\mathcal{D}_c$ of $\mathcal{D}$.
\end{lemma}

\begin{proof}
Let $(X, Y, z, f, g, h)$ be a distinghuised triangle of $\mathcal{D}$
...
is compact, then $X$, $Y$ are compact objects too.
This proves the lemma.
\end{proof}
