...
\section{Proof in the two-dimensional case}

\begin{lem}\label{polypoly}
Let $(R,\m)$ be a Noetherian local ring of dimension one,
and let $x\in\m$ be an element not contained in any minimal prime ideal of $R$.
Then the function $n\mapsto\length_R(R/x^n R)$ is a polynomial given by
\[ P(n)= n P(1).
\]
\end{lem}
\begin{proof}
Since $\dim R=1$, then $\dim(R/x^n R)=0$ and so $\length_R(R/x^n R)$ is finite for each $n$.
To show the lemma we will induct on $n$;
since $x^0R=R$, we have that $P(0)=\length_R(R/x^0R)=\length_R 0=0$.
Now let $n\geq 1$ and suppose the statement holds for $n-1$. The following sequence is exact
\[ \xymatrix{
0 \ar[r] & R/x^{n-1}R \ar[r]^-x & R/x^nR \ar[r] & R/xR \ar[r] & 0}
\]
where $x$ denotes the multiplication by $x$ map.
Since length is additive, we have that $P(n)=P(n-1)+P(1)=(n-1)P(1)+P(1)$
and so we have shown that $P(n)=nP(1)$ for all $n\geq 0$.
\end{proof}

\begin{lem}\label{minprimespoly}
In the situation of Lemma~\ref{polypoly},
let $t$ be the number of minimal prime ideals
of $R$.
Then $t\leq P(1).$
\end{lem}

\begin{proof}[Proof 1]
Let $\p_1,\ldots,\p_t$ be the minimal prime ideals of $R$ and $\sqrt{0}=\bigcap_{i=1}^t \p_i$.
It suffices to show the lemma when $R$ is reduced, since
$R/\sqrt{0}$ has $t$ minimal prime ideals given by the images of the $\p_i$, and for any $n\geq 0$
\[\length_R (R/x^n R)\geq \length_R\left((R/\sqrt{0})/x^n(R/\sqrt{0})\right)=\length_{R/\sqrt{0}}\left((R/\sqrt{0})/x^n(R/\sqrt{0})\right).\]

Consider the following exact sequence
\[ \xymatrix{
0 \ar[r] & R \ar[r]^-\phi & \prod_{i=1}^t R/\p_i \ar[r] &Q \ar[r] & 0}
\]
where $\phi$ is the projection onto each factor, $Q=\text{Coker}(\phi)$, and
$\phi$ is injective because $\bigcap_{i=1}^t \p_i=(0)$. Denote $\prod_{i=1}^t R/\p_i$ by $M$.
The nonmaximal prime ideals of $R$ are given by the $\p_i$, and localizing at $\p_i$ we see that
$R_{\p_i}\rightarrow (\prod_{i=1}^t R/\p_i)_{\p_i}=\left(R/\p_i\right)_{\p_i}$ is surjective and so $Q_{\p_i}=0$.
Then $\Supp(Q)=\{\m\}$, and $Q$ is a finite $R$-module since $M$ is, so $Q$ has finite length over $R$.

Now consider the diagram
\[ \xymatrix{
0 \ar[r] & R \ar[r] \ar[d]^-{x^n} & M \ar[r] \ar[d]^-{x^n} & Q \ar[r] \ar[d]^-{x^n} & 0 \\
0 \ar[r] & R \ar[r] & M \ar[r] & Q \ar[r] & 0}
\]
where the vertical maps are multiplication by $x^n$. This is injective on $R$ and on $M$ since $x$ is a nonzerodivisor. Let $Q'$ be the kernel of $x^n:Q\longrightarrow Q$.
By the Snake Lemma, the following sequence is exact:
\[ \xymatrix{
0 \ar[r] & Q' \ar[r] & R/x^nR \ar[r] & M/x^nM \ar[r] &Q/x^nQ \ar[r] & 0}.
\]

Since $Q'\subset Q$, we have that $\length_R Q'\leq \length_R Q<\infty$ and so $\length_R(Q/x^n Q)-\length_R Q'$ becomes a constant $l$ for $n>>0$. By Lemma~\ref{polypoly}, $\length_R(R/x^nR)=n P(1)$.
Then $nP(1)=\length_R(M/x^n M)-l$ for $n>>0$.

Now $M/x^nM$ has finite length over $R$ for any $n\geq 0$, and
from the exact sequence $0\rightarrow M/x^{n-1}M\rightarrow M/x^n M\rightarrow M/x\rightarrow 0$ we have that
$\length_R(M/x^nM)=\length_R(M/xM)=\length_R(M/x^{n-1}M)$. Then
\begin{equation*}
\begin{split}
\length_R(M/xM)&=\length_R (M/x^n M)-\length_R(M/x^{n-1}M) \\
&= nP(1)+l
-\left((n-1)P(1)+l\right) \\
&= P(1).
\end{split}
\end{equation*}
Now $\length_R(M/xM)\geq t$ since
\begin{center}
$0\subsetneq (R/\p_1\times 0)/\left((R/\p_1\times 0)\cap xM\right)\subsetneq\cdots\subsetneq \left(\prod_{i=1}^{t-1} R/\p_i\times 0\right)/\left(\left(\prod_{i=1}^{t-1} R/\p_i\times 0\right)\cap xM\right)\subsetneq M/xM$
\end{center}
%\[ 0\subsetneq (R/\p_1\times 0)/\left((R/\p_1\times 0)\cap xM\right)\subsetneq\cdots\subsetneq \left(\prod_{i=1}^{t-1} R/\p_i\times 0\right)/\left(\left(\prod_{i=1}^{t-1} R/\p_i\times 0\right)\cap xM\right)\subsetneq M/xM\]
is a filtration of $M/xM$, and so $P(1)\geq t$.
\end{proof}

\begin{proof}[Proof 2] Induct on $t$; if $t=1$ then $\length_R(R/xR)\geq 1$ since $R/xR\neq 0$. Now let $t>1$ and suppose the lemma holds for $t-1$. Let $\p_1,\ldots,\p_t$ be the minimal prime ideals of $R$. Since
\[ \xymatrix{
0 \ar[r] & \bigcap_{i=1}^{t-1}\p_i \ar[r] & R \ar[r] & R/\bigcap_{i=1}^{t-1}\p_i \ar[r] & 0}.
\]
is exact, then denoting $R/\bigcap_{i=1}^{t-1}\p_i=\overline{R}$, for each $n\geq 0$ the following sequence is exact:
\[ \xymatrix{
0 \ar[r] & \left(\bigcap_{i=1}^{t-1}\p_i\right)/\left(x^nR\cap(\bigcap_{i=1}^{t-1}\p_i)\right) \ar[r] & R/x^nR \ar[r] & \overline{R}/x^n\overline{R} \ar[r] & 0}.
\]

We first show that the length of $\left(\bigcap_{i=1}^{t-1}\p_i\right)/\left(x^nR\cap(\bigcap_{i=1}^{t-1}\p_i)\right)$ over $R$ is at least 1.
Elements of $x^nR\cap(\bigcap_{i=1}^{t-1}\p_i)$ are of the form $x^ny$, and since $x$ is not contained in any $\p_i$, then $y\in\bigcap_{i=1}^{t-1}\p_i$. Therefore \hbox{$x^nR\cap(\bigcap_{i=1}^{t-1}\p_i)=x^n\bigcap_{i=1}^{t-1}\p_i$}.
If $\bigcap_{i=1}^{t-1}\p_i/x^n\bigcap_{i=1}^{t-1}\p_i=0$, then $\bigcap_{i=1}^{t-1}\p_i=x^n\bigcap_{i=1}^{t-1}\p_i\subseteq\m\bigcap_{i=1}^{t-1}\p_i$ and so $\bigcap_{i=1}^{t-1}\p_i=0$ by Nakayama's lemma. But then $\bigcap_{i=1}^{t-1}\p_i\subset\p_t$ implies $\p_t$ contains some other $\p_i$, which is a contradiction because $\height\p_t=0$. Therefore $\bigcap_{i=1}^{t-1}\p_i/x^n\bigcap_{i=1}^{t-1}\p_i$
is nonzero and so has length at least 1.

By the inductive hypothesis, $\length_{\overline{R}}(\overline{R}/x\overline{R})=\length_{R}(\overline{R}/x\overline{R})\geq t-1$, since $\overline{R}$ has $t-1$ minimal prime ideals, given by the images of $\p_1,\ldots,\p_{t-1}$. Therefore
\begin{center} $\length_R(R/xR)=\length_R(\bigcap_{i=1}^{t-1}\p_i/x^n\bigcap_{i=1}^{t-1}\p_i)+\length_R(\overline{R}/x\overline{R})\geq t.$
\end{center}
\end{proof}

\begin{lem}\label{sopexists}
Let $(R,\m)$ be a Noetherian local ring of dimension $d>1$, let $f\in\m$ be an element not contained in any minimal prime ideal of $R$, and let $k\in\N$. Then there exist elements $g_1,\ldots,g_{d-1}\in\m^k$ such that $f,g_1,\ldots,g_{d-1}$ is a system of parameters.
\end{lem}

\begin{proof}
The existence of a system of parameters $f,g_1,\ldots,g_{d-1}$ is a standard result in dimension theory; see for instance $\S13$ of \cite{matscrt}. Then the ideal generated by $f,g_1^{k},\ldots,g_{d-1}^k$ is also $\m$-primary.
\end{proof}

\begin{lem}\label{syspar}
Let $(R,\m)$ be a Noetherian local ring of dimension two,
and let $f\in\m$ be an element not contained in any minimal prime ideal of $R$.
Then there exist an element $g\in\m$ and $N\in\N$ such that
\begin{enumerate}
\item[$(\emph{a})$]
$f,g$ form a system of parameters for $R$.
\item[$(\emph{b})$]
If $h\in\m^N$,
then $f+h,g$ is a system of parameters
and $\length_R R/(f,g)=\length_R R/(f+h,g)$.
\end{enumerate}
\end{lem}

\begin{proof}
Since $\bigcap_{j\geq 0}\m^j=(0)$ by Krull's intersection theorem,
there exists $k\in\N$ such that $f\in\m^k-\m^{k+1}$.
\begin{comment}
No power of $\m$ is contained in $(f)$, since otherwise $(f)$
be $\m$-primary and $f$ would be a system of parameters,
contradicting the fact that $\dim R=2$.
Furthermore, for any prime ideal $\p$ minimal over $f$,
no power of $\m$ is contained in $\p$.
Then the image of $\m^{k+1}$ in $R/fR$ is nonzero,
and by prime avoidance there exists an element $\overline{g}$
contained in $\overline{\m^{k+1}}$ but not in any minimal prime ideal of $R/fR$.
Let $g\in R$ be an element whose image in $R/fR$ is $\overline{g}$,
so that $g\in\m^{k+1}$.
Then no minimal prime ideals over $f$ contain $g$, so the prime ideals containing $(f,g)$
have height at least 2. Thus $\m$ is the only prime ideal that contains $(f,g)$ and
so $f,g$ is a system of parameters.
\end{comment}
By Lemma~\ref{sopexists} there is an element $g\in\m^{k+1}$ such that $f,g$ is a system of parameters.

Since $(f,g)$ is $\m$-primary, there exists $n\in\N$ such that $\m^n\subset(f,g)$.
Let $N=\max\{k+1,n\}$, and let $h\in\m^N$. Then $h=af+bg$ for some $a,b\in R$.
Note that the set of elements of the form $\{uf+g'\mid u\in R^\times, g'\in(g)\}$ has empty intersection with $\m^N$
since if not, then there exists $c\in\m^N$ with $uf+g'=c$, and so $uf=c-g'\in\m^{k+1}$, which contradicts $f\not\in\m^{k+1}$.

Now we will show that $R/(f,g)=R/(f+h,g).$
The images of $f$ and $f+h=(a+1)f+bg$ in $R/gR$ are $\overline{f}$ and $\overline{(a+1)f}$,
respectively, and since $a\in\m$, $a+1$ is a unit in $R$.
Therefore we have that the images of $f$ and $f+h$ in $R/gR$ are equivalent,
and so $R/(f,g)=\left(R/gR\right)/\overline{(f)}=\left(R/gR\right)/\overline{(f+h)}=R/(f+h,g)$.
Then $\Ass\left(R/(f+h,g)\right)=\Ass\left(R/(f,g)\right)=\{\m\}$ and so $f+h,g$ is a system of parameters.
\end{proof}

\begin{prop}\label{propdim2} Let $R$ be a Noetherian local normal domain of dimension $2$. Then there exists a nonzero element $f\in\m$ such that $R/fR$ is reduced.
\end{prop}

\begin{proof}
Let $f\in\m$ be a nonzero element. We will modify $f$ slightly to obtain an element that generates a radical ideal. The localization of $R$ at each height one prime ideal $\p$ is a DVR, so we denote by $\val_\p f$ the valuation of $f/1$ in $R_{\p}$. Let $\p_1,\ldots,\p_s,\q_1,\ldots,\q_t$ be the distinct height one prime ideals containing $f$, with $\val_{\p_i}f=1$ and $\val_{\q_j}f=m_j>1$ for each $i$ and each $j$. Then we define $\divis(f)=\sum_{i=1}^s \p_i+\sum_{j=1}^t m_j\q_j$ as a linear combination of height one primes with integer coefficients.

Choose $g$ and $N$ using Lemma~\ref{syspar}, and let $h\in\m^N$. Since $R$ is normal, $\Ass(R/(f+h))$ is the set of height one prime ideals $\p$ with $\val_\p(f+h)>0$, and by Lemma~\ref{minprimespoly} $|\Ass(R/(f+h))|\leq\length_{R/(f+h)}R/(f+h,g)=\length_R R/(f+h,g)=\length_R R/(f,g)$, which is finite because $f,g$ is a system of parameters. Therefore we see that the number of height one prime ideals that contain $f+h$ is bounded independent of $h\in\m^N$.

If $h\in\m^N$ is an element with $\val_{\p_i}h>1$ and $\val_{\q_j}h=1$ for each $i,j$, then $\val_{\p_i}(f+h)=\val_{\q_j}(f+h)=1$ for every $i$ and $j$, and so $\divis(f+h)=\sum_{i=1}^s \p_i+\sum_{j=1}^t \q_j+\sum_{k=1}^v e_k\mathfrak r_k$ for some height one prime ideals $\mathfrak r_k$. Replacing $f$ with $f+h$, $g$ and $N$ still fulfill the conditions of Lemma~\ref{syspar} and so we can repeat this to obtain an element $f+h'$ with $\val_{\p_i}(f+h')=\val_{\q_j}(f+h')=\val_{\mathfrak r_k}(f+h')=1$.
Since the number of height one prime ideals containing $f+h'$ is bounded, then by repeating this process finitely many times we obtain an element $f'$ with $\val_\p f'=1$ for every height one prime ideal $\p$ containing $f'$.
Since the associated prime ideals of $f'$ all have height one, then $R/f'R$ is reduced.

Now we will pick $h$ that satisfies the above criteria. By prime avoidance, for each $1\leq i\leq s$ we can find an element $a_i\in\p_i^N$ that is not contained in any $\q_j$, and for each $1\leq j\leq t$ an element $b_j\in\q_j$ not contained in $\q_j^{(2)}$ or in any $\q_k$ where $k\neq j$. Here $\q_j^{(2)}=\q_j^2 R_{\q_j}\cap R$ denotes the second symbolic power. Define $h=\prod_{i=1}^s a_i \prod_{j=1}^t b_j$. Then $h\in\m^N$ is in $\p_i^{(2)}$ and in $\q_j$ for each $i$ and $j$, and moreover $h$ is not contained in any $\q_j^{(2)}$. This is because each $a_i$ and $b_k,k\neq j$ is a unit in $R_{\q_j}$, and so $h=b_j\left(\prod_{i=1}^s a_i \prod_{k\neq j} b_k\right)\in\q_j^2R_{\q_j}$ implies $b_j\in\q_j^2R_{\q_j}$, which is a contradiction.
\end{proof}

\section{Proof in higher dimensions}

We first introduce some notation.
If $(R,\m)$ is a Noetherian local ring, $I$ is an ideal of definition, and $M$ is a finite $R$-module, then we denote
\[ \chi_{I,M}(n)=\length_R(M/I^{n+1}M).
\]

\begin{lem}\label{lengthchi} Let $(R,\m)$ be a Noetherian local ring, let $I$ be an $\m$-primary ideal of $R$, and let $0\rightarrow M'\rightarrow M\rightarrow M''\rightarrow 0$ be a short exact sequence of finite $R$-modules. If $\length_R M'$ is infinite, then the numerical polynomial
$\chi_{I,M}-\chi_{I,M''}-\chi_{I,M'}$
has degree less than the degree of $\chi_{I,M'}.$
\end{lem}

\begin{proof} \href{http://stacks.math.columbia.edu/tag/0123}{Tag 00KC} of \cite{stacksproject}.
\end{proof}

\begin{lem}\label{multiplicity} Let $(R,\m)$ be a Noetherian local ring of dimension $d$, let $g_1,\ldots,g_d$ be a system of parameters, and let $I=(g_1,\ldots,g_d)$. If $\frac{e_I}{d!}$ is the leading coefficient of the numerical polynomial $\length_R(R/I^{n+1})$, then $e_I\leq\length_R(R/I).$
\end{lem}

\begin{proof} Theorem 14.10 of \cite{matscrt}.
\end{proof}

\begin{lem}\label{minprimespolyhigher} Let $(R,\m)$ be a Noetherian local ring of dimension $d$, let $t$ be the number of minimal prime ideals of $R$ of dimension $d$, and let $(g_1,\ldots,g_d)$ be a system of parameters. Then $t\leq\length_R(R/(g_1,\ldots,g_n)).$
\end{lem}

\begin{proof} Let $\p_1,\ldots,\p_s,$ be the minimal prime ideals of $R$ where the first $t$ have dimension $d$, and denote $I=(g_1,\ldots,g_n)$. As in the proof of Lemma~\ref{minprimespoly} we can assume $R$ is reduced.

Consider the exact sequence
\[ \xymatrix{
0 \ar[r] & R \ar[r] & \prod_{i=1}^s R/\p_i \ar[r] &Q \ar[r] & 0},
\]
and denote $M=\prod_{i=1}^t R/\p_i$.
Localizing at any $\p_i$, the map $R_{\p_i}\rightarrow M_{\p_i}=(R/\p_i)_{\p_i}$ is surjective and so $Q_{\p_i}=0$. Therefore no height 0 prime of $R$ is in the support of $Q$, and so the degree of the numerical polynomial $\chi_{I,Q}$ equals $\dim(\Supp(Q))<d$.

$R$ has infinite length, and so by Lemma~\ref{lengthchi} we have that $\chi_{I,M}-\chi_{I,Q}-\chi_{I,R}$ is a numerical polynomial of degree less than $d$, and so we have that $\length_R(M/I^{n+1}M)$ and $\length_R(R/I^{n+1})$ are both degree $d$ and have the same leading coefficient $e_I$. Since $\chi_{I,M}=\sum_{i=1}^t \chi_{I,R/\p_i}$ and each $R/\p_i$ has dimension $d$, then $t\leq e_I\leq\length_R(R/I)$ where the last inequality is by Lemma~\ref{multiplicity}.
\end{proof}

\begin{lem}\label{sysparhigher}
Let $(R,\m)$ be a Noetherian local ring of dimension $d$, and let $f\in\m$ be an element not contained in any minimal prime ideal of $R$. Then there exist elements $g_1,\ldots,g_{d-1}\in\m$ and $N\in\N$ such that
\begin{enumerate}
\item $f,g_1,\ldots,g_{d-1}$ form a system of parameters for $R$.
\item If $h\in\m^N$, then $f+h,g_1,\ldots,g_{d-1}$ is a system of parameters and\\
$\length_R R/(f,g_1,\ldots,g_{d-1})=\length_R R/(f+h,g_1,\ldots,g_{d-1})$.
\end{enumerate}
\end{lem}

\begin{proof}
Let $k\in\N$ be such that $f\in\m^k-\m^{k+1}$, and pick $g_1,\ldots,g_{d-1}\in\m^{k+1}$ using Lemma~\ref{sopexists}. Then the rest of the proof is the same as in Lemma~\ref{syspar}.
\end{proof}

\begin{prop}\label{propdimd} Let $R$ be a catenary Noetherian local normal domain. Then there exists a nonzero element $f\in\m$ such that $R/fR$ is reduced.
\end{prop}

\begin{proof} The proof is the same as that of Proposition~\ref{propdim2}, using Lemma~\ref{minprimespolyhigher} instead of Lemma~\ref{minprimespoly} and Lemma~\ref{sysparhigher} instead of Lemma~\ref{syspar}.
We can use Lemma~\ref{minprimespolyhigher} because $R$ is a catenary domain, so every height one prime ideal of $R$ has dimension $d-1$, and hence $R/(f+h)$ is equidimensional.
Then we are done because $R/(f+h)$ has no embedded associated primes.
\end{proof}
