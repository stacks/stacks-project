...
\medskip\noindent
We say the topology on $M$ is a {\it linear} if there exists a fundamental
...
\noindent
The following lemma will be improved on in Smoothing Ring Maps,
Section \ref{smoothing-section-todo}.
...
Smoothing Ring Maps, Section \ref{smoothing-section-todo}
...
smooth over $k$ by Algebra, Lemma \ref{}
...
If $B \to C$ is a local complete intersection then
...
\begin{proof}
Please take a look at the proof of
Algebra, Lemma \ref{algebra-lemma-exact-sequence-NL}
for notation and terminology.
Choose a presentation $B[y_1, \ldots, y_m] \to C$ with kernel $J$.
Choose a presentation $A[x_s] \to B$ with kernel $I$.
Consider the induced presentation
$A[x_s, y_j] \to C$ with kernel $K$.
...
in terms of this terminology we refer to
