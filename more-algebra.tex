...
\medskip\noindent
In this section we say that $A \to B$ or $A \subset B$ is an
...
is finite, then the residue extension is finite and we have $ef \leq [L : K]$.
\end{lemma}
...
Let $A \to B$ be an extension of discrete valuation rings.
Let $K = f.f.(A)$ and let $K \subset K_1$ be a finite extension of
fields. Let $A_1 \subset K_1$ be the integral closure of $A$ in $K_1$.
On the other hand, denote $L = f.f.(B)$ and let
$L_1 = (K_1 \otimes_K L)_{red}$. Then $L_1$ is a nonempty finite product of
finite field extensions of $L$. Let $B_1$ be the integral closure of $B$
in $L_1$. We obtain compatible commutative diagrams
$$
\vcenter{
\xymatrix{
L \ar[r] & L_1 \\
...
$$
By the Krull-Akizuki theorem the rings $A_1$ and $B_1$ are Noetherian.
Since they are finite over $A$ and $B$ they have dimension $1$.
As an integral closure we conclude that $A_1$ is a Dedekind domain
and that $B_1$ is a finite product of Dedekind domains.
\end{remark}

\begin{lemma}
\label{lemma-purely-inseparable-case}
Let $A \subset B_0 \subset B$ be extensions of discrete valuation rings
with fractions fields $K \subset L_0 \subset L$. Let $\pi \in A$ be a
uniformizer. Assume
\begin{enumerate}
\item $B_0$ is a Nagata ring,
\item $A \subset B_0$ is weakly unramified,
\item $L$ is a degree $p$ purely inseparable extension of $L_0$.
\end{enumerate}
Then either $L$ comes from adjoining a $p$th root of $\pi$ to $L_0$
or with $K_1 = K[x]/(x^p - \pi)$ we have that $A_1 \to B_1$ in
Remark \ref{remark-construction}
is a weakly unramified extension of discrete valuation rings.
\end{lemma}

\begin{proof}
Let $e$ be the ramification index. If $e = 1$, then we are done. If not,
then $e = p$ by Lemmas \ref{lemma-inequality} and
\ref{lemma-ramification-index-a-power-of-p}.
This in turn implies that the residue fields of $B_0$ and $B$ agree.
Write $\pi_B^p = \pi u$ for some unit $u$ of $B$.
Since $\pi_B^p \in L_0$, we see that $u \in B_0^*$.

\medskip\noindent
Suppose there exists an integer $m \geq 0$ such that
$$
u = \sum\nolimits_{i < m} b_i^p \pi^i + b \pi^m
$$
with $b_i \in B_0$ and with $b \in B_0$ whose image in $\kappa_{B_0}$
is not a $p$th power. Then we see that
$$
(\pi_B/x - \sum\nolimits_{i < m} b_i x^i)^p = x^{pm}b
$$
where $x$ is as in the statement of the lemma.
Hence we see that $B \subset B_1$ is the extension gotten by
taking the $p$th root out of $b$ which is weakly unramified
as the image of $b$ in the residue field of $B$ is not a $p$th power.

\medskip\noindent
If there does not exist an integer $m$ as in the preceding paragraph,
then $u$ is a $p$th power in $B_0[x]/\pi^m$ for all $m$.
Since $B_0$ is Nagata, this means that $u$ is a $p$th power.
Whence the first case of the statement of the lemma holds.
\end{proof}
