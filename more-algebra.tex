...
Algebra, Lemma \ref{algebra-lemma-relation-ext-ext}.
\end{proof}
...
all $R$-modules $M$. Say $\text{Ext}^i_R(\kappa, K) = 0$ for $i > b$.
We will show by induction on $\dim(M)$ that $\text{Ext}^i(M, K) = 0$
...
\medskip\noindent
The base case. If $\dim(M) = 0$ then we can use induction on the
...
\medskip\noindent
Assume $\dim(M) > 0$. Consider the exact sequence $0 \to C \to M \to M' \to 0$
...
(to find $f$ use $\dim(M) > 0$, use $M$ has no embedded associated
