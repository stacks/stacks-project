...
\noindent
Let $R \to A$ be a ring map. We can also use K-flat resolutions to define
a derived base change functor
$$
...
$M^\bullet \otimes_R^{\mathbf{L}} A = K^\bullet \otimes_R A$.
You can use
...
functor of triangulated categories $D(R) \to D(A)$.
\end{lemma}
...
$F(M^\bullet) = M^\bullet \otimes_R A$. We let $S$ be the set of
...
Finally,
Derived Categories, Lemma \ref{derived-lemma-find-existence-computes}
also guarantees that $LF(K^\bullet) = F(K^\bullet) = K^\bullet \otimes_R A$
when $K^\bullet$ is K-flat, i.e., $LF$ is
...
$K_\bullet(R, f_1, \ldots, f_n)$, or $K_\bullet(R, f_\bullet)$.
\end{definition}
...
Let $(x_{ij})$ be an invertible $c \times c$-matrix with
...
$$
A/m^2 \cong K[x_1, \ldots, x_n]/(x_ix_j)
$$
and
$$
...
$$
with surjective horizontal arrows and $S$ and $R$ power series rings over
either fields or Cohen rings. Moreover, we may assume $R \to S$ is flat.
\end{lemma}
...
residue characteristic is zero and a power series over a field else.
...
Now we have to lift the composition $\Lambda[[x_1, \ldots, x_n]] \to A \to B$
to a map into $R$. This is possible because
...
\medskip\noindent
Finally, we prove that we may assume $R \to S$ is flat. Namely, if $\varphi$
is not flat, we replace it by the map
...
by the map $S' \to B$ which maps $y_i$ to zero. A regular system
of parameters of $R$ maps to a regular sequence in $S'$ hence
$\varphi'$ is flat by Algebra, Lemma \ref{algebra-lemma-flat-over-regular}.
(Note that if the residue characteristic is $p$, then $p, x_1, \ldots, x_n$
is a regular system of parameters on $R$ and $p$ is a nonzero
divisor on $S$ by our choice of $S$.)
\end{proof}
...
regular local rings over fields.
...
$\mathfrak m$ and residue field $\kappa$. If the
the image of $f$ in $\kappa$ is not a $p$th power, then
$R' = R[z]/(z^p - f)$ has maximal ideal $\mathfrak mR'$ hence
is regular. On the other hand, if $f = g^p \bmod \mathfrak m$,
then we can replace $f$ by $f - g^p$ and assume $f \in \mathfrak m$:
namely $D(f) = d(f - g^p)$ by the Leibniz rule and $R[z]/(z^p - f) \cong
R[z]/(z^p - f + g^p)$ by a simple coordinate change.
Since $D(f) \not \in \mathfrak m$ we see that $f \not \in \mathfrak m^2$.
Hence if we choose a regular system of parameters
$(f, f_2, \ldots, f_d)$ for $R$, then $(z, f_2, \ldots, f_d)$
is a regular system of parameters for $R'$.
\end{proof}

\begin{lemma}
\label{lemma-find-D}
Let $p$ be a prime number. Let $B$ be a Noetherian complete local domain
with $p = 0$ in $B$. Let $f \in B$ be an element which is not a $p$th
power in the fraction field of $B$. Then there exists a derivation
$D : B \to B$ such that $D(f)$ is not zero.
\end{lemma}

\begin{proof}
By Algebra, Lemma
\ref{algebra-lemma-complete-local-Noetherian-domain-finite-over-regular}
we can write $B$ as a finite extension of
$A = k[[x_1, \ldots, x_n]]$ for some field $k$ and integer $n$.
Note that $\text{d}f$ is not zero in $\Omega_{f.f.(B)/\mathbf{F}_p}$, see
Algebra, Lemma \ref{algebra-lemma-derivative-zero-pth-power}.
By Lemma \ref{lemma-power-series-ring-subfields} there exists a subfield
$k' \subset k$ of finite index such that setting
$A' = k'[[x_1^p, \ldots, x_n^p]]$ the element
...
\begin{proof}
For $n \geq 2$ we have $D(I^n) \subset I^{n - 1}$ by the Leibniz rule.
Hence $D$ induces maps $D_n : R/I^n \to R/I^{n - 1}$.
Taking the limit we obtain $D^\wedge$. We omit the verification that
$D^\wedge$ is a derivation.
To extend $D$ to $S^{-1}R$ just set $D(r/s) = D(r)/s - rD(s)/s^2$.
\end{proof}
...
\medskip\noindent
We are left with the case $A = k[[x_1, \ldots, x_d]]$ where $k$ is a field
of characteristic $p > 0$. Let $\mathfrak p$ be a prime of $A$.
...
is regular. Let $K \subset M \subset L$ be a subfield such that
