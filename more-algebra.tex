...
Galois, see
Algebra, Lemma \ref{algebra-lemma-normal-case}. Pick $\theta \in \kappa_2$
...
\begin{lemma}
\label{lemma-make-degree-p-galois-extension}
Let $A$ be a discrete valuation ring with uniformizer $\pi$. If the residue
characteristic of $A$ is $p > 0$, then for every $n > 1$ there exists a
degree $p$ separable totally ramified extension $K \subset L$
such that the integral closure $B$ of $A$ in $L$ has ramification index
$p$ and a uniformizer $\pi_B$ such that
$\pi_B^p = \pi + \pi^n b$ and $\pi_B^p = \pi + (\pi')^{np}b'$
for some $b, b' \in B$.
\end{lemma}

\begin{proof}
If the characteristic of $K$ is zero, then we can take the
extension given by $\pi_B^p = \pi$, see
Lemma \ref{lemma-pull-root-uniformizer}.
If the characteristic of $K$ is $p > 0$, then we can take the
extension of $K$ given by $z^p - \pi^n z = \pi^{1 - p}$.
Namely, then we see that $y^p - \pi^{n + p - 1} y = \pi$
...
\item $C = B[\pi^{1/p}]$, or
\item there exists a degree $p$ separable extension $K \subset K_1$
such that $L_1 = L \otimes_K K_1$ and $M_1 = M \otimes_K K_1$ are fields
and the maps of integral closures $A_1 \to B_1 \to C_1$
...
Lemma \ref{lemma-make-degree-p-galois-extension}
...
Let $A$ be a discrete valuation ring with uniformizer $\pi$.
Assume the faction field $K$ has characteristic $p > 0$
...
\begin{lemma}
\label{lemma-totally-ramified-Artin-Schreier}
Let $A$ be a discrete valuation ring with uniformizer $\pi$
whose fraction field $K$ has characteristic $p > 0$.
Let $\xi \in K$ be an element not in $A$. If the valuation of
$\xi$ is not divisible by $p$, then the extension $L = K[z]/(z^p - z - \xi)$
is a degree $p$ totally ramified Galois extension of $K$.
\end{lemma}

\begin{proof}
Let $\pi$ be a uniformizer of $A$.
Say $\xi = \pi^{-n}a$ where $p$ does not divide $n$.
Let $B \subset L$ be the integral closure of $A$ in $L$.
If $C = B_\mathfrak m$ for some maximal ideal $\mathfrak m$,
then it is clear that $p \text{ord}_C(z) = -n \text{ord}_C(\pi)$.
In particular $A \subset C$ has ramification index $> 1$.
It follows that $B = C$, etc.
\end{proof}

\begin{lemma}
\label{lemma-cohen}
Let $A$ be a discrete valuation ring annihilated by a prime $p$.
For every $n > 0$ there exists a ring map
$\overline{\sigma} : \kappa_A \to A/\mathfrak m_A^n$
which is a section to the residue map $A/\mathfrak m_A^n \to \kappa_A$.
\end{lemma}

\begin{proof}
Follows immediately from the Cohen structure theorem.
\end{proof}
...
such that with $M_1 = (M \otimes_K K_1)_{red}$ and $C_1$ the integral
...
$\xi = z^p - z \in L$. Choose $n \geq 1$ such that $\pi^n\xi \in B$.
Choose a ring map
$\overline{\sigma} : \kappa_B \to B/(\pi^n)$ as in Lemma \ref{lemma-cohen}
and lift to a map of sets $\sigma : \kappa_B \to B$. Then we can write
$$
\xi = \sum\nolimits_{i = n, \ldots, 1} \sigma(\lambda_i) \pi^{-i} + b
$$
for some $\lambda_i \in \kappa_B$ and $b \in B$. Let
$$
I = \{i \in \{1, \ldots, n\} \mid \lambda_i \in \kappa_A\}
$$
and
$$
J = \{j \in \{1, \ldots, n\} \mid \lambda_i \not \in \kappa_A\}
$$
We will argue by induction on the size of the finite set $J$.

\medskip\noindent
If $J = \emptyset$, then $\xi = \pi^{-n} a + b$ for some $a \in A$
and $b \in B$ (details omitted). Consider the degree $p$ Galois
extension $K_1$ of $K$ given by the equation $w^p - w = \pi^{-n}a$.
After passing to $L_1 = L \otimes_K K_1$ we see that $M_1$ is obtained
from $L_1$ by adjoining a root of $y^p - y = b$ and hence unramified.
Now, if $K_1 \supset K$ was unramified, then we conclude that
$B \subset C$ was already weakly unramified. Thus we see that
$K_1 \subset K$ is totally ramified. From now on we assume
$J \not = \emptyset$.
...
Choose $r > \max(r_j + 1, n)$.
Choose a separable totally ramified extension $K \subset K_1$ such that the
integral closure $A_1 \subset K_1$ of $A$ has a uniformizer $\pi'$ such that
$(\pi')^{p^r} = \pi + \pi^{n + 1}a$ for some $a \in A_1$ (see Lemma \ref{}).
Then we see that we can write
$$
\xi = \sum_{i \in I} \sigma(\lambda_i) (\pi')^{-i p^r} +
\sum_{j \in J} \sigma(\mu_j)^{p^{r_j}} (\pi')^{- j p^r} + b_1
$$
for some $b_1 \in B_1$. Note that $\sigma(\lambda_i)$ for $i \in I$
is a $p^r$-power modulo $\pi^n$. Hence we can rewrite this as
$$
\xi = \sum_{i \in I} x_i^{p^r} (\pi')^{-i p^r} +
\sum_{j \in J} \sigma(\mu_j)^{p^{r_j}} (\pi')^{- j p^r} + b_1
$$
Changing $z$ appropriately by elements of $B_1$ we obtain
$$
\sum_{i \in I} x_i (\pi')^{-i} +
\sum_{j \in J} \sigma(\mu_j) (\pi')^{- j p^{r - r_j}} + b_1
$$
Since there is a unique $j$ such that $j p^{r - r_j}$ is minimal
this defines a weakly unramified extension of $B_1$ as desired.
\end{proof}
