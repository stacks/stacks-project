...
\begin{enumerate}
\item If $B \otimes_A B \to B$ is flat and $C \otimes_B C \to C$
is flat, then $C \otimes_A C \to C$ is flat.
...
$$
of the mutliplication maps, the fact that
$$
...
\begin{lemma}
\label{lemma-go-down}
Let $A \to B \to C$ be ring maps. Assume that $B \to C$ is
faithfully flat and $C \otimes_A C \to C$ is flat.
...
\begin{proof}
Let $N$ be a $B$-module. Then $N$ is flat as a $K$-module,
whence $N$ is flat as a $B$-module. It follows that $B$ is
an absolutely flat ring. Hence $B$ is reduced and every local
...
Thus $\kappa(\mathfrak p) \subset \kappa(\mathfrak q)$ is
separable algebraic over $K$. Hence every generic point of $\Spec(A)$
...
\begin{proof}
Every ideal is the union of its finitely generated subideals.
Hence (2) implies (1) by Algebra, Lemma \ref{}.
It is clear that (3) implies (1) and (2).
Conversely, suppose that $N \subset M$ with $M$ a flat $A$-module.
We can write $M = \colim M_i$ with each $M_i$ finite free, see
Algebra, Theorem \ref{}.
Setting $N_i \subset M_i$ the inverse image of $N$ we see that
it suffices to do the case $M = R^{\oplus n}$. In this case
the module $N$ has a finite filtration whose subquotients
are ideals. Hence we win by
Algebra, Lemma \ref{}.
\end{proof}

\begin{lemma}
\label{lemma-product-weak-dimension-at-most-1}
Let $A_i$ be a family of valuation rings. Then $\prod A_i$ 
has weak dimension at most $1$.
\end{lemma}

\begin{proof}
Let $I \subset \prod A_i$ be a finitely generated ideal.
Let $I_i \subset A_i$ be the image of $I$. Since $A_i$ is a
valuation ideal, the ideal $I_i$ is generated by a single element.
Say $I_i = (f_i)$. Then $I$ is generated by the element $f = (f_i)$.
Note that $f = g e$ for some nonzero divisor $g$ and idempotent $e$.
Thus $I \cong (e)$ is flat.
\end{proof}
...
Then $A'$ is flat (by assumption on $A$ and the fact that $B$ is flat)
and finite over $A$. To show that $A' = A$ we may localize and assume
$A$ is local. In this case $A'$ is free over $A$. If the rank of $A'$
over $A$ is $>1$, then $A' \otimes_A A' \to A'$ has a nonzero kernel.
This contradicts the fact that $B \otimes_A B \to B$ is injective
since $A' \otimes_A A' \to B \otimes_A B$ is injective.
\end{proof}
...
\begin{proof}
Valuative criterion of properness combined with the fact that
$x$ is integral over $A$ if and only if $\Spec(A[x]) \to \Spec(A)$
is universally closed. Elementary proof?
\end{proof}
...
Lemma \ref{lemma-find-valuation-rings}.
...
The ring map $V \to L$ is flat as every finitely generated ideal
$I$ of $V$ is principal and $I \otimes_V L \to L$ is injective
by direct computation.
The ring map $V \to L$ is an epimorphism, as it is a product
of epimorphisms.
\end{proof}
...
and consider the base change to $B$. As $A \to B$ is flat, this
gives the cartesian diagram
$$
\xymatrix{
...
$$
Note that $V \to B \otimes_A V$ is weakly \'etale, hence $B \otimes_A V$
...
(Algebra, Lemmas \ref{}, \ref{}, and \ref{}).
...
is a colimit of \'etale extensions of $\kappa(\mathfrak p)$
...
then $B \otimes_A L$ has an idempotent for some algebraic field
extension $L \supset \kappa(\mathfrak p)$.

\medskip\noindent
Let $\kappa(\mathfrak p) \subset L$ be an algebraic field extension.
Let $A' \subset L$ be the integral closure of $A/\mathfrak p$ in $L$.
By Lemma \ref{} we see that $A'$ is a strictly henselian local ring
whose residue field is a purely inseparable extension of the residue
field of $A$. Thus $B \otimes_A A'$ is a local ring by
Lemma \ref{}.
On the other hand, $B \otimes_A A' \subset B \otimes_A L$
is integrally closed by Lemma \ref{}.
It follows that the ring $B \otimes_A L$ does not have nontrivial
