...
when considering the Tor functors. Each of these spectral sequences
is treated as an example. We will often use the following
notation. If $R$ is a ring and $M$ is an $R$-module, then
$- \otimes_R M : \text{Mod}_R \to \text{Mod}_R$ is a right exact
functor. The category $\text{Mod}_R$ has enough projectives.
Hence the functor $- \otimes_R M$ has left derived functors
$\text{Tor}_i^R(-, M)$ and a total right derived functor
$$
