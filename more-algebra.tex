...
$$
D^-(\text{Mod}_R) \longrightarrow D^{-}(\text{Mod}_R)
$$
which is denoted $-\otimes^{\mathbf{L}} M$.

\medskip\noindent
But we can do a little better. Suppose $K^\bullet \in D^{-}(R)$.
...
for $i \leq m + 1$. Choose $\beta : E^\bullet \to L^\bullet$ with $E^\bullet$
...
P^\bullet \ar[u] \ar[r] & E^\bullet \ar[u]
}
$$
...
Assume that $B$ is pseudo-coherent as an $A$-module.
Let $K^\bullet$ be an $m$-pseudo-coherent complex of $B$-modules.
Then $K^\bullet$ is $m$-pseudo-coherent as a complex of $A$-modules. 
\end{lemma}

\begin{proof}
Choose a bounded complex of finite free $B$-modules $E^\bullet$
and a map $\alpha : E^\bullet \to K^\bullet$ which is an isomorphism
on cohomology in degrees $> m$ and a surjection in degree $m$.
...
pseudo-coherent as a complex of $A$-modules.
...
Let $m \in \mathbf{Z}$.
Consider the ring map $R[x] \to R$ which maps $x$ to zero.
Then $K^\bullet$ is $m$-pseudo-coherent as a complex of $R$-modules
if and only if $K^\bullet$ is $m$-pseudo-coherent as a complex
of $R[x]$-modules.
\end{lemma}

\begin{proof}
Let $n$ be the largest integer for which $H^n(K^\bullet)$ is nonzero.
We will prove the lemma by induction on $n - m$. If $n < m$, then there
is nothing to prove, see
Lemma \ref{lemma-recognize-pseudo-coherent}.
(In fact the cases $n = m, m + 1$ are also handled by that lemma and
Lemma \ref{lemma-relatively-finitely-presented}.)
