...
(Derived Categories, Lemmas \ref{derived-lemma-product-derived-Ab4-star} and
\ref{derived-lemma-enough-K-injectives-Ab4-star}).
...
Derived Categories, Section \ref{derived-lemma-two-ss-complex-functor}
...
$$
with notation as above. A generalization of this construction is discussed in
...
\begin{proof}
Choose a K-injective complex $I^\bullet$ representing $K$. Then
$\Hom_A(A_g, I^\bullet)$ represents $R\Hom(A_g, K)$. Note that
$\Hom_A(A_g, I^\bullet)$ is a K-injective complex of $A_g$-modules
by Lemma \ref{lemma-hom-K-injective}.
Hence $\Hom_A(A_g, I^\bullet)$ is a K-injective complex of $A$-modules
by Lemma \ref{lemma-K-injective-flat}.
Thus $R\Hom(A_f, R\Hom(A_g, K))$ is represented by the complex
$\Hom_A(A_f, \Hom_A(A_g, I^\bullet))$. For $A$-any module $M$
the canonical map
$$
\Hom_A(A_{fg}, M) \longrightarrow
\Hom_A(A_f, \Hom_A(A_g, M))
$$
which sends $\varphi$ to the map
$(a/f^n \mapsto (b/g^m \mapsto \varphi(ab/f^ng^m)))$
is an isomorphism. Thus we see that
$$
\Hom_A(A_{fg}, I^\bullet) \longrightarrow
\Hom_A(A_f, \Hom_A(A_g, I^\bullet))
$$
is an isomorphism. The statement of the lemma follows
as the left hand side represents
$R\Hom(A_{fg}, K)$.
\end{proof}
...
Let $A$ be a Noetherian ring and $I \subset A$ an ideal. If
$K$ is a pseudo-coherent object of $D(A)$, then the cohomology
modules $H^i(K^\wedge)$ of the derived completion are the $I$-adic
completions of the cohomology modules $H^i(K)$.
\end{lemma}

\begin{proof}
By definition $K$ is represented by a bounded above complex $K^\bullet$
of finite free $A$-modules. Then
$K \otimes_A^\mathbf{L} A/I^n = K^\bullet/I^nK^\bullet$.
Since $K^\wedge = R\lim K \otimes_A^\mathbf{L} A/I^n$
