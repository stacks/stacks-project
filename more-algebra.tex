...
$S'$ is J-0 by Lemma \ref{lemma-J-0-go-up} and our choice of $R'$.
Then we apply Lemma \ref{lemma-J-0-go-down} to see that $S$
is J-0 as desired.
\end{proof}
...
\begin{lemma}
\label{lemma-find-D}
Let $A$ be a domain of characteristic $p$ with fraction field $K$.
Let $f \in A$ be an element which is not a $p$th power in $K$.
If $A$ is of finite type over a Noetherian complete local ring,
then there exists a derivation $D : A \to A$ such that $D(f)$ is
not zero.
\end{lemma}
