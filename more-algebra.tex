...
In this situation we have moreover:
\begin{enumerate}
\item If $A \to A'$ is surjective, then the map $C \to B$ is surjective, and
\item If $A \to A'$ is surjective and $A' \to B$ is flat, then
$C \otimes_A A' \cong B$.
\end{enumerate}
Namely, if $A \to A'$ is surjective, then $\mathfrak m_A$ surjects onto
$\mathfrak m_{A'}$ and hence Nakayama's lemma (see
Algebra, Lemma \ref{algebra-lemma-NAK} part (3)) shows that
$C/\mathfrak m_A^nC \to B/\mathfrak m_A^nB$ is surjective for all $n$.
This proves (1). Part (2) follows from part (1) and
...
$R_g \cong R'_g$ for some nonzero divisor $g \in R$, then $g^ND$ extends
