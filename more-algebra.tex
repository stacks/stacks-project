...
Remark \ref{remark-Rlim-cohomology}.
...
Moreover, the cokernel of this map is $R^1\lim_n H^p(K_n^\bullet)$
...
$$
\xymatrix{
M_{n + 1}^\bullet \ar[d] \ar[r]_{\psi_{n + 1}} &
K_{n + 1}^\bullet \ar[d]^{\varphi_n} \\
M_n^\bullet \ar[r]^{\psi_n} & K_n^\bullet
}
$$
commute in $D(A_{n + 1})$.
\end{lemma}
...
The isomorphism class of this functor will depend only on the choice
of the lift $E$ we will choose below. Hence if the system $(E_n)$
comes with such a lift there is a well defined functor.

\medskip\noindent
Construction of the functor. First we we lift $(E_n)$ to an object $E$
of $D(\mathbf{N}, A)$, see Lemma \ref{lemma-lift-to-system-complexes}.
By Lemma \ref{lemma-get-ML-system} we may
represent $E$ by an inverse system of complexes $(E_n^\bullet)$
such that all the transition maps $E_{n + 1}^\bullet \to E_n^\bullet$
are termwise surjections. Next, take any $K \in D(A)$ and
represent $K$ by a K-flat complex $K^\bullet$ of
$A$-modules. Then the transition maps of the system
$$
\ldots \to
\text{Tot}(K^\bullet \otimes_A E_3^\bullet) \to
\text{Tot}(K^\bullet \otimes_A E_2^\bullet) \to
\text{Tot}(K^\bullet \otimes_A E_1^\bullet)
$$
are surjections of complexes, and we see that we can simply take
the limit to get a derived limit of the system $(K \otimes_A^\mathbf{L} E_n)$.
Clearly, the assignment
$$
K^\bullet \longmapsto \lim \text{Tot}(K^\bullet \otimes_A E_n^\bullet)
$$
is functorial in the K-flat complex $K^\bullet$. To show that this
gives a well defined functor on $D(A)$ it suffices to show that
a quasi-isomorphism of K-flat complexes $K_1^\bullet \to K_2^\bullet$
induces a quasi-isomorphism of these limit complexes. However, this
follows from the fact that the induced map of systems
$(K_1^\bullet \otimes_A E_n^\bullet) \to (K_2^\bullet \otimes_A E_n^\bullet)$
is a quasi-isomorphism of complexes of $\text{Mod}(\mathbf{N}, A)$
and the fact that $R\lim : D(\mathbf{N}, A) \to D(A)$ is well defined.
\end{remark}
...
and the natural maps between these quotients.
...
by a matrix (in fact diagonal) whose entries are in the ideal
...
\begin{lemma}
\label{lemma-noetherian-naive-completion-is-completion}
