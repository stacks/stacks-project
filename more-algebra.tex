...
$\kappa$. Let $L/K$ be a Galois extension with Galois group $G$.
...
\item if the characteristic of $\kappa_A$ is $p > 0$, then
...
\theta : I \longrightarrow \mu_e(\kappa_C)
\end{equation}
which is characterized by the property that
$\sigma(\pi_C) = \theta(\sigma) \pi_C \bmod \pi_C^2$.
...
If we can prove (c), then part (d) will follow as $I_t$
will be isomorphic to $\mu_e(\kappa_C)$ as the arguments above show
that $|I_t| \leq |\mu_e(\kappa_C)|$.
...
induces an isomorphism $I_t \to \mu_e(\kappa(\mathfrak m))$.
\end{definition}
