...
We can write $y = x^e f$ for some $f \in R$, $f \not \in (x)$.
Then $f \in \mathfrak p$ as $x \not \in \mathfrak p$
and $\mathfrak p$ is prime. To finish the proof it suffices
to show that $f$ is a prime element since then $(f) = \mathfrak p$.
This follows easily from the fact that the image of $f$
in $R_x$ is prime and the fact that $x$ is prime.
\end{proof}
