...
which is flat over $A'$ (see Lemma \ref{lemma-flat-module-powers}).
We have moreover
\begin{enumerate}
...
Note that $D'(f) = D(f)$ is not zero in $B \otimes_A M$ as $D(f)$
is not zero in $A' \subset M$ and $A \to B$ is faithfully flat.
Hence $B \otimes_A L$ is regular by
