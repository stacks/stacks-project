...
\begin{proof}
Assume (2). Let $f \in I$. We have short exact sequences
$$
0 \to
...
$$
by Lemma \ref{lemma-map-into-Rlim}. We conclude the middle term
is zero since $f$ acts both as an isomorphism and nilpotently on
the outer terms (recall that $f_i^n$ acts by an endomorphism of
$K_n^\bullet$ which is homotopic to zero). Thus $K$ is
derived complete with respect to $I$.

\medskip\noindent
Assume that $K$ is derived complete with respect to $I$. Consider
the filtrations
$$
K_n^\bullet \supset
...
complete with respect to $I$ by Lemma \ref{lemma-naive-derived-completion}.
By a formal argument (omitted) we see that it suffices
to show $K \to K'$ is an isomorphism if $K$ is derived complete
with respect to $I$. Say $(f_1, \ldots, f_r) = I$ and let
$K_n^\bullet$ be the Koszul complex with respect to $f_1^n, \ldots, f_r^n$.
By Lemma \ref{lemma-characterize-derived-complete-Koszul} we have
$K = R\lim (K \otimes_A^\mathbf{L} K_n^\bullet)$.
By Lemma \ref{lemma-sequence-Koszul-complexes} the pro-objects
$\{K_n^\bullet\}$ and
$\{A/(f_1^n, \ldots, f_r^n)\}$ of $D(A)$ are isomorphic.
Thus $K \to K'$ is an isomorphism, by
