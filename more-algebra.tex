...
\begin{proof}
This is true because for any $A$-module $M$ we have
$M \otimes_A B = \text{colim}\ M \otimes_A B_i$.
\end{proof}
...
$$
There are three ring maps $p_1, p_2, m : B \to C$ defined by the rules
$$
p_1(x_i) = x_i,\quad
p_1(x_i) = y_i,\quad
m(x_i) = x_i + y_i.
...
$$
We may write uniquely $\tau = p_1(\tau_1) + p_2(\tau_2)$ with
...
$y_i \mapsto 0$. Then $q_1(\tau) = q_1(p_1(\tau_1) + p_2(\tau_2)) = \tau_1$.
...
$\underline{M}(B)^{(0)} \cap \underline{M}(B)^{(1)}$.
...
$\theta \in G(B)^{(1)}$ implies that $\psi(ul) = u\psi(l)$
in $G(C)$ for $l \in L(C)$ and $u \in C^*$ a unit. This implies that
$\psi$ is a linear transformation of functors, see
