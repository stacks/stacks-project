...
valuative criterion. We have already seen this
...
Assume that $S$ is Noetherian, that $f$ and $h$ are of finite type, and
...
\begin{proof}
We first prove that $f$ is separated. To do this we will verify
part (4) of Lemma \ref{lemma-Noetherian-dvr-valuative-separation} holds.
Thus we consider a diagram
$$
\xymatrix{
...
$$
where $A$ is a discrete valuation ring and $K$ is the residue
field of a generic point $\eta$ of an irreducible component of $X$.
Since $h$ is dominant, there exists a point $u \in U$ mapping
to $\eta$, see Morphisms, Lemma \ref{morphisms-lemma-quasi-compact-dominant}.
Set $K' = \kappa(u)$ so that $K'/K$ is a finitely generated field extension.
By
Algebra, Lemma \ref{algebra-lemma-exists-dvr}
or
Properties, Lemma \ref{properties-lemma-locally-Noetherian-specialization-dvr}
there exists a discrete valuation ring $A' \subset K'$ dominating $A$.
Thus we get a diagram
$$
\xymatrix{
\Spec(K') \ar[r]_u \ar[d] & U \ar[r]_h & X \ar[d]^f \\
\Spec(A') \ar[r] & \Spec(A) \ar[r] & S
}
$$
Now by the assumption of the lemma, there is exactly one morphism
$\Spec(A') \to X$ fitting into this diagram. This means that we cannot
have two morphisms $\Spec(A) \to X$ fitting into the first diagram
of the proof, as the morphism $\Spec(A') \to \Spec(A)$ is an
epimorphism of schemes.

\medskip\noindent
By the result of the previous paragraph we know that $f$ is separated.
...
and then take the closure unside $\mathbf{P}^n_X$, or you can directly
...
\noindent
Using the above we can describe the category of modifications of
