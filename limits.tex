...
Recall that $S_i = \underline{\Spec}_{S_0}(\mathcal{A})$,
...
assume the corresponding opens $D(g_{j, i}) \subset \Spec(R_i)$
...
identified with an open subscheme of $\Spec(A_{i, l})$.
Let $S_{i, l}$ be the scheme obtained by glueing
$V_i$ and $\Spec(A_{i, l})$ along the open $W_i$, see
...
and $W_{i'} \to W_i$ and
the morphism $\Spec(A_{i', l'}) \to \Spec(A_{i, l})$ induced
