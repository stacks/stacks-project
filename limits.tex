...
and
$A \otimes_{A_i} B_i = A[x_1, \ldots, x_n]/(f_{1, i'}, \ldots, f_{m, i'})$.
Since $\otimes$ commutes with directed colimits we see that
...
\medskip\noindent
It is similarly completely mechanical to prove assertion (3) in this
affine setting. To finish the proof we claim that the affine case implies
...
equal: $\varphi_{i, S} = \psi_{i, S}$. Pick an affine open covering
$Y_i = \bigcup V_{j, i}$ such that each $V_{j, i}$ maps into
an affine open of $S$. Since $Y_i$ and $X_i$ are of finite presentation
over $S_i$, and since $S_i$ is quasi-compact and quasi-separated, also
$X_i$ and $Y_i$ are quasi-compact and quasi-separated. Hence the immersions
$V_{j, i} \to Y_i$ are quasi-compact, and the inverse images
$U_{j, i} = \varphi_i^{-1}(V_{j, i})$ and $U_{j, i}' = \psi_i^{-1}(V_{j, i})$
are quasi-compact opens of $X_i$.
