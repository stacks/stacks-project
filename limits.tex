...
be prime ideals with $\mathfrak p_i \not \subset \mathfrak p_j$
...
$$
\Spec(S) \longrightarrow \mathbf{A}^1_R = \Spec(R[x])
$$
given by the $R$-algebra map sending $x$ to $a$ have dimension $\leq d$.
Then there exists an $n_0$ such that for $n \geq n_0$ the fibres of
$$
\Spec(S) \longrightarrow \mathbf{A}^1_R = \Spec(R[z])
$$
given by the $R$-algebra map $R[z] \to S$ sending $z$ to $a^n + b$
...
$$
\Spec(S_\lambda) \longrightarrow \mathbf{A}^1_R = \Spec(R[x])
$$
have dimension $\leq d$, see Lemma \ref{lemma-limit-dimension}.
Fix such a $\lambda$. If we can find $n_0$ which works for $R \to S_\lambda$,
$a_\lambda$, $b_\lambda$, then $n_0$ works for $R \to S$ (as the fibres
of $\Spec(S_\lambda)$ over $\mathbf{A}^1_R$ contain the fibres of
$\Spec(S)$ over $\mathbf{A}^1_R$).
...
$R \to S$ such that each $R_\lambda$ and $S_\lambda$ is of finite
type over $\mathbf{Z}$. Choose $\lambda_0 \in \Lambda$ and elements
$a_{\lambda_0}, b_{\lambda_0} \in S_{\lambda_0}$ mapping to $a, b \in S$.
For $\lambda \geq \lambda_0$ denote $a_\lambda, b_\lambda \in S_\lambda$
...
\Spec(R_\lambda[x])
$$
have dimension $\leq d$, see Lemma \ref{lemma-descend-dimension-d}.
Fix such a $\lambda$. If we can find $n_0$ which works for
$R_\lambda \to S_\lambda$, $a_\lambda$, $b_\lambda$, then $n_0$
works for $R \to S$. Namely, the fibres of $\Spec(S) \to \Spec(R[x])$
have the same dimension as the fibres of
$\Spec(S_\lambda) \to \Spec(R_\lambda[x])$ by
Morphisms, Lemma \ref{morphisms-lemma-dimension-fibre-after-base-change}.
...
Thus we may assume $\dim(R) = n$ and the result holds for all
situations with smaller dimension of $R$.
...
$$
\pi : \Spec(S) \to \mathbf{A}^1_R = \Spec(R[x])
$$
corresponding to the $R$-algebra map $R[x] \to S$ sending $x$
to $a$. The assumption tells us that $\pi$ has fibres of dimension $\leq d$.
Hence the fibres of $\Spec(S) \to \Spec(R)$ have dimension $\leq d + 1$.
...
$$
\gamma_n : \Spec(S) \longrightarrow \mathbf{A}^1_R = \Spec(R[z])
$$
given by the $R$-algebra map $R[z] \to S$ sending $z$ to $a^n + b$
is the composition of $\phi$ with the morphism
$$
\tau_n :  \mathbf{A}^2_R = \Spec(R[x, y]) \to \mathbf{A}^1_R = \Spec(R[z])
$$
sending $z$ to $x^n + y$. We will show that in both case (1) and
case (2) there exists an integer $n_0$ and a nonempty open
$V \subset \Spec(R)$ such that for $n \geq n_0$ the fibres of
$\pi_n$ at points over $V$ have dimension $\leq d$.

\medskip\noindent
Case (1). In this case there is a dense open subscheme
$U \subset \Spec(S)$ such that $\phi|_U : U \to \mathbf{A}^2_R$ is flat.
Then the composition
$$
U \xrightarrow{\phi|_U} \mathbf{A}^2_R
\xrightarrow{\pi_n} \mathbf{A}^1_R = \Spec(R[z])
$$
is flat as well. Hence the fibres of this morphism have at least
codimension $1$ in the fibres of $U \to \Spec(R)$. In other words,
they have dimension $\leq d$. On the other hand, since $U$ is dense
in $\Spec(S)$, we can find a nonempty open
$V \subset \Spec(R)$ such that $U_s \subset X_s$ is dense for
$s \in V$. Since $\dim(X_s \setminus U_s) \leq d$ we conclude that
our claim is true.

\medskip\noindent
Case (2). In this case we can find a nonzero $g = \sum c_{ij} x^i y^j$
in $R[x, y]$ such that $\Im(\phi) \subset V(g)$. In fact, we may assume
$g$ is irreducible over $\text{Frac}(R)$. Finally, since $\pi$
is dominant, we see that $g$ is not contained in $R[x]$.
Let $s$ be the degree of $g$ as a polynomial in $y$ and
...
For such $n$ the polynomial $g(x, -x^n)$ 
...
Let $n'_0$ be the integer we have by induction on $d$
...
\medskip\noindent
Assume $R \to S$ is of finite presentation. If $d = 0$, then
...
\medskip\noindent
Having said this, we will use induction on $\dim(R)$ as well.

\medskip\noindent
Let $\mathfrak q_1, \ldots, \mathfrak q_r \subset S$ be the
minimal primes, corresponding to components of $\Spec(S)$
whose generic fibres over $R$ have dimension $d$.
Let $\mathfrak p_i \subset R$ be the corresponding primes of $R$.
As $d > 0$ by Lemma \ref{} we can find an element $a \in S$ such that
the image of $a$ in $\kappa(\mathfrak q_i)$ is transcendental over
$\kappa(\mathfrak p_i)$.
Consider the morphism
$$
X = \Spec(S) \longrightarrow \Spec(R[x])
$$
corresponding to mapping $x$ to $a$. Let $U \subset X$ be the open
subset where the fibres have dimension $\leq d - 1$.
By construction $U$ contains all the generic points of
the generic fibres of $\Spec(S) \to \Spec(R)$.
However, it then immdediately follows that there exists
a dense open $V \subset \Spec(R)$ such that
$X_V \to \mathbf{A}^1_V$ has fibres of dimension $\leq d - 1$.

\medskip\noindent
We may assume that $V = D(f)$ for some $f \in R$.
...
By Lemma \ref{} there exists an $n > 0$ such
