...
Let $S$ be a scheme. Let $s \in S$ be a closed\footnote{In fact, it
suffices if $s$ is a finite type point of $S$, see
Morphisms, Definition \ref{morphisms-definition-finite-type-point}.}
point such that
the punctured spectrum $V = \Spec(\mathcal{O}_{S, s}) \setminus \{s\}$
is quasi-compact. For $U = S \setminus \{s\}$ the base change functor
$$
\left\{
\begin{matrix}
...
\begin{proof}
Let $W \subset S$ be any open subscheme. By glueing of relative schemes, see
...
g^{-1}(W \cap U) \to W \cap U\text{ is an isomorphism}
\end{matrix}
\right\}
$$
...
of $s$, see
Lemma \ref{lemma-descend-finite-presentation}.
Choose $f_1, \ldots, f_r \in \mathcal{O}_{S, s}$
such that $\{s\} = V(f_1, \ldots, f_r)$, see
Algebra, Lemma \ref{algebra-lemma-qc-open}.
Pick an affine open neighbourhood $W_0 \subset X$
of $s$ such that $f_1, \ldots, f_r$ come from
$f_{0, 1}, \ldots, f_{0, r} \in \mathcal{O}_S(W_0)$.
Then $V(f_{0, 1}, \ldots, f_{0, r}) = \{s\} \amalg T$
with $T$ closed (here we use that $s$ is a closed point).
After shrinking $W_0$ we may assume $V(f_{0, 1}, \ldots, f_{0, r}) = \{s\}$.
Thus for every affine open $s \in W \subset W_0$ we see that $U \cap W$
is quasi-compact (by the same algebra lemma).
