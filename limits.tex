...
implies (1).  
First we reduced to the case when $S$ is affine.  Assume that (3) implies (1)
when the base is affine.  Now let $f: X \to S$ be a separated morphism of 
finite type.  Being proper is local on the base, so if 
...
set $Z \subseteq \mathbf A^n \times X_\alpha$.  Let $\bar Z$ denote the
closure of $Z$ in $\mathbf{A}^n \times X$.  Then for topological
reasons,
$$1 \times f(\bar Z) \cap \mathbf{A}^n \times S_\alpha 
= 1 \times f(Z).$$
Hence $1 \times f(Z)$ is closed, and we have reduced to the affine
case.

In the case $S$ affine we can apply Chow's Lemma
\ref{lemma-chow-finite-type}
...
above.  Fix closed $Z \subseteq X$.  Since $X' \to X$ is surjective,
...
$X \times \mathbf{A}^n \to S \times \mathbf{A}^n$ is closed.  A
product of proper maps is proper, so 
$$X' \times_S \mathbf{P}^n_S \to X \times_S \mathbf{P}^n_S = 
X \times \mathbf{P}^n$$
...
morphism.  Therefore, the map $X' \to X' \times_S \P^n_S$ is proper,
since it is a section to a separated map.  
Composing all these proper morphisms, we see
...
remains to show (4) implies (1).
We begin by reducing to $S$ affine.  Since being separated is a local
property, as in the proof of Lemma
\ref{lemma-limited-base-change}, if we can show that whenever 
$X \to S$ has (4) that the restriction $X_\alpha \to S_\alpha$ has (4)
where $S_\alpha \subseteq S$ is an (affine) open subset and $X_\alpha :=
...
\to V$ has (4) for opens $U \subseteq X$ and $V \subseteq S$ with
$f(U) \subseteq V$.

We next wish to reduce to the case $X \to S$ is finite type.  Assume
that we know (4) implies (1) when $X$ is finite type. Since
$S$ is Noetherian, $X$ is locally of finite type, and hence is locally
Noetherian as well.  Thus, $X \to S$ is quasi-separated, and therefore
we may apply the (standard) valuative criterion to check whether $X$
is separated.  Let $X = \bigcup_\alpha X_\alpha$ be an affine open
...
$\text {Spec } A$.  Therefore, the two dotted arrows factor through
$X_\alpha \cup X_\beta \to X$, which is a scheme of finite type over
$S$.  Since $X_\alpha \cup X_\beta$ is an open subset of $X$, by our
previous remark, $X_\alpha \cup X_\beta$ satisfy (4), so by
assumption, is separated.  This is a contradiction.  Therefore, we
have reduced to $X \to S$ is finite type.  

Since $X \to S$ is finite type, and $S$ is an affine Noetherian
scheme, $X$ is also Noetherian.  Therefore, $X \to X \times_S X$ will
...
not in the image.  $X$ is Noetherian and has finitely many irreducible
components.  Therefore, $y$ is in the closure of the image of one of
the irreducible components $X_0 \subseteq X$.  Give $X_0$ the reduced
induced structure.  The composition $X_0 \to X \to X \times_S X$
factors through the closed subscheme
 $X_0 \times_S X_0 \subseteq X \times_S X$ with the reduced induced
structure.  The closure of the image
 $\Delta: X_0 \to X_0 \times_S X_0$,
 which we denote $\bar X_0$ contains $y$.
Since $X_0 \to X_0 \times_S X_0$ is locally closed, the image of $X_0$
will be open in $\bar X_0$.  Therefore, each projection
$X_0 \times_S X_0 \to X_0$ provides an inverse map to
 $X_0 \to X_0 \times_S X_0$, showing that $X_0$ and $\bar X_0$ are
birational.  Since $\bar{X}_0$ is an integral subscheme of a
Noetherian scheme, it will be Noetherian.  Thus, the local ring
$\mathcal O_{{\bar X_0, y}}$ is a local Noetherian ring with fraction
field $K$ equal to the function field of $X_0$.  By the Krull-Akizuki
theorem from Algebra (see Hartshorne Ex. II.4.11(a)), there exists a
discrete valuation ring $R$ dominating $\mathcal O_{{\bar X_0, y}}$
...
the closed point of $A$ to $y \in X_0 \times_S X_0$.  There cannot
even exist 
...
(4).  Therefore, we have shown (1) implies (4).
\end{proof}
...
(1) implies (2) implies (3) implies (4).  We will now show (4) implies
  ...
  $X' \to \mathbf{P}^n_S$ is a closed immersion to show that $X \to S$
  is proper.  For the sake of contradiction, assume that $X' \to
  ...
  irreducible component $X_0'$ of $X'$, but not in the image.  Since
  $X' \to \mathbf{P}^n_S$ is a finite type 
  map of Noetherian schemes, and as in the previous lemma, we get (by
  Krull-Akizuki) a discrete valuation ring $A$ of the function field
  $K$ of $X'$ that sits in the solid commutative diagram:

  \begin{equation}
    \xymatrix{
      \text{Spec } K \ar[r] \ar[d] & X' \ar [r] \ar[d] &
      \mathbf{P}^n_S \ar[d] \\
      \text{Spec } A \ar@{-->}[r] \ar[urr] & X \ar[r] & S\\
    }
  \end{equation}
  with the image of $K$ equal to the generic point of $X_0'$ and the
  closed point of $A$ mapping to $y \in \mathbf{P}^n_S$.  By
  construction, there does not exist a set theoretic lift to $X'$.
  Since $X \to S$ satisfies (4), there exists a dotted arrow to $X$
  over $S$.  Since $X' \to X$ is birational and surjective, the dotted
  arrow lifts to a morphism to $X'$ over $S$.  By further composing
  with the immersion $X' \to \mathbf{P}^n_S$, we obtain another
  morphism (not depicted in the diagram) from
  $\text{Spec } A \to \mathbf{P}^n_S$.  Since $\mathbf{P}^n_S$
  is proper over $S$, is satisfies (2), and so these two morphisms
