...
Let $(S_i, f_{ii'})$ be an inverse system of schemes over $I$.
Assume all the morphisms $f_{ii'} : S_i \to S_{i'}$ are affine.
Let $S = \text{lim}_i\ S_i$. Suppose we are given an $i$ and a
...
we see that $T \times_{S_i} S = \text{lim}_{i' \geq i}\ T \times_{S_i} S_{i'}$.
...
all the schemes $S_i$ are quasi-compact. Let $S = \text{lim}_i\ S_i$
...
$F(T) = \text{colim}_i\ F(T_i)$. Let $X$ be a scheme over $S$, and
...
Then $U_{k, j} = \text{lim}_{i' \geq i}\ U_{k, j, i'}$
and $V_j = \text{lim}_{i' \geq i}\ V_j$
(see Lemma \ref{lemma-directed-inverse-system-has-limit}).
...
Suppose that $R = \text{colim}_{i \in I}\ R_i$. For some $i \in I$
...
By
Properties, Lemma \ref{properties-lemma-quasi-coherent-colimit-finite-type}
we can write $\mathcal{A} = \text{colim}_i \mathcal{F}_i$ as a filtered
...
immersions.
\end{enumerate}
Let $f : X = \text{lim}_i\ X_i \to S = \text{lim}_i\ S_i$ be the limit.
Let $d \geq 0$.
