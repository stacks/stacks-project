...
In fact $S$ is affine and $S = \text{Spec}(\colim_i R_i)$
with $R_i = \Gamma(S_i, \mathcal{O})$.
\end{lemma}

\begin{proof}
Just define $S = \text{Spec}(\colim_i R_i)$.
...
Write $S = \text{Spec}(R)$. We have $R = \colim_i R_i$
...
$$
\text{Mor}_S(\lim_i T_i, X) =
\colim_i \text{Mor}_S(T_i, X)
$$
\item For any directed partially ordered set $I$, and any
inverse system $(T_i, f_{ii'})$ of $S$-schemes over $I$
...
\colim_i \text{Hom}_{\mathcal{O}_S(V)}(\mathcal{O}_X(U), A_i)
$$
...
Set $T := \text{Spec}(A) = \lim_i T_i$.
The formula above becomes in terms of morphism sets of schemes
$$
\text{Mor}_V(\lim_i T_i, U) =
\colim_i \text{Mor}_V(T_i, U).
$$
...
$$
\text{Mor}_S(\lim_i T_i, U) =
\colim_i \text{Mor}_S(T_i, U)
$$
...
$F : (\textit{Sch}/S)^{opp} \to \textit{Sets}$ is
...
let $h_X : (\textit{Sch}/S)^{opp} \to \textit{Sets}$ be its
...
statement is the following: Suppose that $A = \colim_i A_i$
...
$$
\colim_{i' \geq i} (A_{i'} \otimes_{A_i} B_i)
\longrightarrow
\colim_{i' \geq i} (A_{i'} \otimes_{A_i} C_i)
$$
...
$$
X_i = \underline{\text{Spec}}_S(\mathcal{A}_i)
\longrightarrow
S
$$
...
$$
\xymatrix{
\text{Spec}(\kappa(y)) \ar[r] \ar[d] &
\mathbf{A}^n \times X \ar[d] \ar[r] & X \ar[d] \\
\text{Spec}(A) \ar[r] & \mathbf{A}^n \times S \ar[r] & S
}
$$
Now property (4) implies that there exists a morphism
$\text{Spec}(A) \to X$ which fits into this diagram.
Since we already have the morphism $\text{Spec}(A) \to \mathbf{A}^n$
from the left lower horizontal arrow we also get a morphism
$\text{Spec}(A) \to \mathbf{A}^n \times X$ fitting into the
...
Z_s
\longrightarrow
(Z_{i'})_{s_{i'}} \times_{\text{Spec}(\kappa(s_{i'}))} \text{Spec}(\kappa(s))
\longrightarrow
(Z_i)_{s_i} \times_{\text{Spec}(\kappa(s_i))} \text{Spec}(\kappa(s))
\longrightarrow
X_s
$$
