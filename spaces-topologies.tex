...
$(\textit{Spaces}/S)_\tau$ and the big site $(\textit{Spaces}/X)_{fppf}$
of an algebraic space $X$ over $S$
has as underlying category the category $\textit{Spaces}/S$
respectively a suitable subcategory of $\textit{Spaces}/S$.
...
We will show later (insert future reference here)
...
\noindent
We briefly discuss the notion of an fppf covering of algebraic spaces.
...
Topologies, Definition \ref{definition-fppf-covering}. In particular,
if $X$ and all the $X_i$ are schemes, then we recover the usual notion
of an fppf covering of schemes.
