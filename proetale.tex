...
$U \mapsto \mathcal{O}(U)_f$. Let $K \in D(\mathcal{O})$. We denote
...
\item $R\SheafHom_\mathcal{O}(E, K) = 0$ for all $E$ in $D(\mathcal{O}_f)$,
\item $T(K, f) = 0$.
\end{enumerate}
\end{lemma}

\begin{proof}
It is clear that (2) implies (1). Assume (1). Let $\mathcal{I}^\bullet$ be a
K-injective complex of $\mathcal{O}$-modules with injective terms
representing $K$, see Injectives, Theorem
\ref{injectives-theorem-K-injective-embedding-grothendieck}.
Condition (1) signifies that
$\SheafHom_\mathcal{O}(\mathcal{O}_f, \mathcal{I}^\bullet)$
is acyclic. Let $\mathcal{F}^\bullet$ be a complex of $\mathcal{O}_f$-modules
representing $E$. Then
$$
R\SheafHom_\mathcal{O}(E, K) =
...
K-injective complex of $\mathcal{O}_f$-modules by
Cohomology on Sites, Lemma \ref{sites-cohomology-lemma-hom-K-injective}.
Since $\SheafHom_\mathcal{O}(\mathcal{O}_f, \mathcal{I}^\bullet)$
is acyclic it is zero in $D(\mathcal{O}_f)$ hence this last $R\SheafHom$
is zero. Thus we get (2).
...
The rule which associated to $U$ the set $\mathcal{I}(U)$
...
Let $(\mathcal{C}, \mathcal{O})$ be a ringed on a site.
...
Let $(\mathcal{C}, \mathcal{O})$ be a ringed on a site.
Let $f_1, \ldots, f_r$ be global sections of $\mathcal{O}$.
Let $\mathcal{I} \subset \mathcal{O}$ be the ideal sheaf
generated by $f_1, \ldots, f_r$.
...
$$
is bijective whenever $E$ is in $D_{comp}(\mathcal{O})$.
\end{lemma}

\begin{proof}
The proof is identical to the proof of
More on Algebra, Lemma \ref{more-algebra-lemma-derived-completion}.
We first formally reduce to the case $r = 1$.
Next suppose $r = 1$ and set $f = f_1$.
We will use the equivalences of Lemma \ref{lemma-hom-from-Af}
without further mention. Define
$K^\wedge$ as the cone of the canonical map
$R\SheafHom_\mathcal{O}(\mathcal{O}_f, K) \to K$.
We claim that
$$
R\SheafHom_\mathcal{O}(\mathcal{O}_f, R\SheafHom_\mathcal{O}(\mathcal{O}_f, K))
=
R\SheafHom_\mathcal{O}(\mathcal{O}_f, K)
$$
This is true because, if $\mathcal{I}^\bullet$ is a K-injective complex
of $\mathcal{O}$-modules representing $K$, then
$R\SheafHom_\mathcal{O}(\mathcal{O}_f, K)$ is represented by
$\SheafHom_\mathcal{O}(\mathcal{O}_f, \mathcal{I}^\bullet)$ which is
a K-injective complex of $\mathcal{O}$-modules by
Cohomology on Sites, Lemmas \ref{sites-cohomology-lemma-hom-K-injective} and
\ref{sites-cohomology-lemma-K-injective-flat}.
Thus the left hand side of the displayed equation is computed by
$\SheafHom_\mathcal{O}(\mathcal{O}_f,
\SheafHom_\mathcal{O}(\mathcal{O}_f, \mathcal{I}^\bullet))$
which is equal to
$\SheafHom_\mathcal{O}(\mathcal{O}_f, \mathcal{I}^\bullet)$ by an
application of
Modules on Sites, Lemma \ref{sites-modules-lemma-epimorphism-modules}.
The formula above and the distinguished triangle
$$
R\SheafHom_\mathcal{O}(\mathcal{O}_f, K) \to K \to K^\wedge \to
R\SheafHom_\mathcal{O}(\mathcal{O}_f, K)[1]
$$
imply that $K^\wedge$ is derived $f$-complete. Finally, if $E$ is derived
$f$-complete, then
$$
R\SheafHom_\mathcal{O}(R\SheafHom_\mathcal{O}(\mathcal{O}_f, K), E) = 0
$$
because $R\SheafHom_\mathcal{O}(\mathcal{O}_f, K)$ is represented by
a complex of $\mathcal{O}_f$-modules. Thus the triangle above shows
that $R\SheafHom_\mathcal{O}(K, E) = R\SheafHom_\mathcal{O}(K^\wedge, E)$
as desired.
\end{proof}
...
\item for every ring map $A \to B$ and finite generated ideal $I \subset A$
a map of complexes $K(A, I) \to K(B, IB)$,
\end{enumerate}
such that
\begin{enumerate}
\item[(a)] for $A \to B$ and $I \subset A$ finitely generated the diagrams
$$
\xymatrix{
...
\item[(c)] the maps $K(A, I) \otimes_A^\mathbf{L} B \to K(B, IB)$
are quasi-isomorphisms, and
\item[(d)] if $I = (f_1, \ldots, f_r)$ then there is a commutative
diagram
$$
\xymatrix{
...
\begin{proof}
Suppose we can construct $K(A_0, I_0)$ for all rings $A_0$ of finite type
over $\mathbf{Z}$. Then we take
$$
K(A, I) = \colim_{(A_0, I_0) \subset (A, I)\text{ large enough}} K(A_0, I_0)
$$
where the colimit is over finite type $\mathbf{Z}$-subalgebras
$A_0 \subset A$ and ideal $I_0 \subset A_0$ with $I = I_0A$.
The partial ordering is given by $(A_0, I_0) \leq (A_0', I_0')$
if and only if $A_0 \subset A_0'$ and $I_0' = I_0A_0'$.
This is a directed partially ordered set. Finally,
given $\varphi : A \to B$ and $I \subset A$ for every
large enough pair $(A_0, I_0)$ we get a pair $(\varphi(A_0), \varphi(I_0))$
large enough for $(B, IB)$. In this way we get a map on colimits.
Properties (a), (b), (c), and (d)
follow readily from this and the corresponding
properties for finite type $\mathbf{Z}$-algebras.

\medskip\noindent
Let $\mathcal{C}_0$ be the category of pairs $(A, I)$ with $A$ a finite
type $\mathbf{Z}$-algebra and $I \subset A$ an ideal. A morphism
$(B, J) \to (A, I)$ is a ring map $\varphi : A \to B$ such that
$J = \varphi(I)B$ (note reversal of arrows).
Endow $\mathcal{C}_0$ with the chaotic topology. We endow $\mathcal{C}_0$
with the sheaf of rings $\mathcal{O} : (A, I) \mapsto A$. The ideals
$I$ fit together to give a sheaf of ideals $\mathcal{I} \subset \mathcal{O}$.
...
Since the topology in $\mathcal{C}_0$ is chaotic, the values
$\mathcal{J}^\bullet(U)$ is a resolution of $A$ by injective
$A$-modules. Hence the value $\mathcal{F}^\bullet(U)$ is an
object of $D(A)$ representing $R\Gamma_Z(A)$ where $Z = V(I)$.
...
The we just set $K(A, I) = \mathcal{F}^\bullet(U)$ where $U = (A, I)$.
...
is a special cocontinuous functor. Hence it suffices to work with
$\mathcal{C}$', in other words we may assume that for every
...
\medskip\noindent
Let $U$ be an object and $I \subset \mathcal{O}(U)$ as above.
...
also generates $\mathcal{I}|_U$, then
there exists a covering $\{U_j \to U\}$ such that
$I\mathcal{O}(U_j) = I'\mathcal{O}(U_j)$. Hence
$K_{U, I}^\bullet$ and $K_{U, I'}^\bullet$ are the {\it same}
for any object lying over one of the $U_j$. The statement
on sheafications follows. Denote $K_U^\bullet$ the common value.

\medskip\noindent
It immediately follows that $K_U^\bullet|_{U'} = K_{U'}^\bullet$
whenever we are given a morphism $U' \to U$ and hence a localization
morphism $\mathcal{C}/U' \to \mathcal{C}/U$. Thus the complexes
...
Then $E^\wedge = R\SheafHom(K, E)$ will do the job.
Namely, by the local description of $K$ we see that
locally $K$ is a colimit of the Koszul complexes $K_n$ on
$f_1^n, \ldots, f_r^n$ where $f_1, \ldots, f_r$ are generators
of $\mathcal{I}$. Hence
$$
E^\wedge = R\SheafHom(\colim K_n, E) = R\lim R\SheafHom(K_n, E).
$$
For $f$ a local section of $\mathcal{I}$ we see that
$$
R\SheafHom_\mathcal{O}(\mathcal{O}_f, E^\wedge) =
R\lim R\SheafHom_\mathcal{O}(\mathcal{O}_f, R\SheafHom(K_n, E))
$$
which is zero. Thus $E^\wedge$ is derived complete.
There is a more elementary way to see this too, namely,
compute $R\SheafHom_\mathcal{O}(\mathcal{O}_{f_i}, E^\wedge)$
locally to see that it is zero by argueing as in the proof
of Lemma \ref{lemma-derived-completion}.
Finally, if
$E$ was derived complete, then we see immediately that
$E \to E^\wedge$ is an isomorphism.
\end{proof}
