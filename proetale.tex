...
$$
where $K_n = K_n(\mathcal{O}, f_1^n, \ldots, f_r^n)$
...
Thus, since derived completion commutes with localization we may assume
...
derived version of the theorem on formal functions (Cohohomology of Schemes, 
Theorem \ref{coherent-theorem-formal-functions}). For example, suppose
$f : X \to S$ is a morphism of schemes. Assume that
\begin{enumerate}
\item $\mathcal{I} \subset \mathcal{O}_S$ is generated by global sections
$f_1, \ldots, f_r$, and
\item $K = \mathcal{F}[0]$ where $\mathcal{F}$ is an $\mathcal{O}_X$-module.
\end{enumerate}
Then the lemma says that
$$
R\lim\left(
Rf_*(\mathcal{F}) \otimes_\mathcal{O} K(\mathcal{O}, f_1^n, \ldots, f_r^n)
\right)
=
R\lim Rf_*\left(
\mathcal{F} \otimes_{\mathcal{O}'}  K(\mathcal{O}', g_1^n, \ldots, g_r^n)
\right)
$$
FIXME!
\end{remark}
