...
\begin{lemma}
\label{lemma-completed-pullback-system}
Let $\Lambda$ be a Noetherian ring. Let $I \subset \Lambda$ be an ideal.
Let $f : \Sh(\mathcal{D}) \to \Sh(\mathcal{C})$ be a morphism of topoi.
If $(K_n)$ is an inverse system of objects in $D(\mathcal{C}, \Lambda/I^n)$
then
$$
Lf_{comp}^*R\lim K_n = R\lim Lf^*K_n
$$
\end{lemma}

\begin{proof}
Observe that $R\lim Lf^*K_n$ is a derived complete object by the
discussion following Definition \ref{definition-derived-complete}.
For a derived complete object $E$ of $D(\mathcal{D}, \Lambda)$
we have
\begin{align*}
\Hom(E, R\lim Lf^*K_n) & =
\lim \Hom(E, Lf^*K_n) \\
...
& = \Hom(Rf_*E, R\lim K_n) \\
& = \Hom(E, Lf^* R\lim K_n) \\
& = \Hom(E, Lf_{comp}^* R\lim K_n)
\end{align*}
because $E$ is derived complete.
Thus the result follows by the Yoneda lemma.
\end{proof}
...
\ref{lemma-completed-pullback-system} we can unwind the left
hand side
$$
Lg_{comp}^* Rf_* K = Lg_{comp}^* (R\lim Rf_*K_n) = R\lim Lg^* Rf_* K_n
$$
Using the same lemmas we can unwind the right hand side
\begin{align*}
Rf'_* L(g')^*_{comp} K
& =
Rf'_* L(g')^*_{comp} R\lim K_n \\
& =
Rf'_* R\lim L(g')^* K_n \\
& =
R\lim Rf'_* L(g')^* K_n
...
\epsilon^{-1} Rf'_* L(g')^* \epsilon^{-1} L_n
$$
