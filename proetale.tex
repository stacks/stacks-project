...
fact (just proved) that is commutes with direct sums.
\end{proof}

\begin{remark}
\label{remark-extend-to-all}
Let $X$ be a scheme. Because $X_\proetale$ has enough weakly contractible
for all $K$ in $D(X_\proetale)$ we have $K = R\lim \tau_{\geq -n}K$ by
Cohomology on Sites, Proposition
\ref{sites-cohomology-proposition-enough-weakly-contractibles}.
...
\begin{lemma}
\label{lemma-quasi-compact-quasi-separated-projection}
Let $f : X \to Y$ be a morphism of schemes. Let $\Lambda$ be a ring.
\begin{enumerate}
\item Let $E \in D(X_\proetale, \Lambda)$ and $K \in D(Y_\proetale, \Lambda)$
be a perfect complex. Then $Rf_*(E) \otimes_\Lambda^\mathbf{L} K =
Rf_*(E \otimes_\Lambda^\mathbf{L} Lf^*K)$.
\item Let $M$ be a $\Lambda$-module. Assume $f$ quasi-compact and
quasi-separated and $E \in D(X_\proetale, \Lambda)$ finite tor dimension.
Then $Rf_*(E) \otimes_\Lambda^\mathbf{L} \underline{M} =
Rf_*(E \otimes_\Lambda^\mathbf{L} Lf^*\underline{M})$.
\end{enumerate}
\end{lemma}

\begin{proof}
Part (1) is a completely general fact which follows by localization
from the projection formula. FIXME.
\end{proof}
