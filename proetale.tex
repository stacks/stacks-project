...
Cohomology of Sites, Proposition
...
Cohomology of Sites, Lemma \ref{sites-cohomology-lemma-hom-K-injective}.
...
Cohomology of Sites, Lemmas \ref{sites-cohomology-lemma-hom-K-injective} and
...
\medskip\noindent
Let $\Lambda$ be a ring and let $\mathcal{C}$ be a site. We will
denote $\underline{\Lambda}$ the constant sheaf with value $\Lambda$
on $\mathcal{C}$.

\begin{lemma}
\label{lemma-flat-sections}
Let $\mathcal{C}$ be a site. Let $\Lambda$ be a coherent ring.
For $U \in \Ob(\mathcal{C})$ the sections $\underline{\Lambda}(U)$
is a flat $\Lambda$-module.
\end{lemma}

\begin{proof}
Recall that $\underline{\Lambda}$ is constructed by applying the
$+$ construction to the constant presheaf with value $\Lambda$
twice, see Sites, Theorem \ref{sites-theorem-plus}.
The first time we perform this construction we obtain the presheaf
which sends $U \in \Ob(\mathcal{C})$ to either $\Lambda$ or $0$.
Moreover, we get $0$ if and only if $U$ is sheaf theoretically empty.
see Sites, Definition \ref{sites-definition-empty} and
Lemma \ref{sites-lemma-characterize-empty}.
The second time we perform the $+$ construction we obtain
\begin{equation}
\label{equation-compute-sheafification}
\underline{\Lambda}(U) =
\colim_{\mathcal{U}} \text{Map}(\pi_0(\mathcal{U}), \Lambda)\
\end{equation}
where the colimit is over all coverings
$\mathcal{U} = \{U_i \to U\}_{i \in I}$ with all $U_i$ not
sheaf theoretically empty of $U$ and
$\pi_0(\mathcal{U}) = I/\sim$ where $\sim$ is the equivalence relation
generated by pairs $(i, i')$ such that $U_i \times_U U_{i'}$ is
not sheaf theoretically empty. As $\Lambda$
is a coherent ring we see that $\text{Map}(\pi_0(\mathcal{U}), \Lambda)$
is a flat $\Lambda$-module by
Algebra, Proposition \ref{algebra-proposition-characterize-coherent}.
Since a colimit of flat modules is flat we conclude.
\end{proof}
...
\begin{proof}
It suffices to show that $\underline{\Lambda}^\wedge(U)$ is flat
as a $\Lambda$-module for each $U \in \Ob(\mathcal{C})$, see
Modules on Sites, Lemmas
\ref{sites-modules-lemma-flatness-presheaves} and
...
By the description of sections over $U$ of
(\ref{equation-compute-sheafification}) we see that
the transition maps are surjective (this also follows
by taking a set theoretic section to $\Lambda/I^{n + 1} \to \Lambda/I^n$
and using that sheafification is done on the level of presheaves
of sets). We conclude by
