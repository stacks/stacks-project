...
\item the space $Y = \Spec(B_Z^\sim)$ is w-local,
\item the induced map $p : Y \to X$ is w-local, and
\item $Z = p^{-1}(X_0)$ is the set of closed points of $Y$.
\end{enumerate}
\end{lemma}
...
to $A' \to C'$ and $I \subset A'$ to get 
$C = (C')_Z^\sim$ where $Z = V(IC') \subset \Spec(C')$.
Note that since $A' \to C'$ is faithfully flat, the space $Z$
surjects onto the set of closed points of $A'$ and in particular
onto $\Spec(A)$. Moreover, as $Z \subset \Spec(C)$
...
$D = (D')_T^\sim$ where $T = V(JD') \subset \Spec(D')$.
...
\begin{proof}
For a prime $\mathfrak p \subset A$ the ring map
$\kappa(\mathfrak p) \to B \otimes_A \kappa(\mathfrak p)$ is
ind-\'etale by
More on Algebra, Lemma \ref{more-algebra-lemma-base-change-weakly-etale}.
In particular the induced maps on residue fields are (separable)
algebraic. Thus we may apply
...
We may replace $B$ by $C = B_{V(IB)}^\sim$, see
Lemma \ref{lemma-w-local-algebraic-residue-field-extensions}.
...
$A \to B$ such that $B$ is w-local and such that every local ring at a
maximal ideal of $B$ is strictly henselian.
Choose an extremally disconnected space $T$ and a surjective
continuous map $T \to \pi_0(\Spec(B))$, see
...
to find an ind-Zariski ring map $B \to D$ such that
$\pi_0(\Spec(D)) \to \pi_0(\Spec(B))$ realizes $T \to \pi_0(\Spec(B))$
and such that
$$
\xymatrix{
\Spec(D) \ar[r] \ar[d] & \pi_0(\Spec(D)) \ar[d] \\
\Spec(B) \ar[r] & \pi_0(\Spec(B))
}
$$
is cartesian in the category of topological spaces. Note that $\Spec(D)$
is w-local, that $\Spec(D) \to \Spec(B)$ is w-local, and that the
set of closed points of $\Spec(D)$ is the inverse image of the
set of closed points of $\Spec(B)$, see Lemma \ref{lemma-silly}.
...
local rings at the corresponding maximal ideals of $B$).
It follows from
...
is at most $2^{2^{2^\kappa}}$. Namely, the ring map $A \to D$ is
constructed as a composition
$$
A \to A_w = A' \to C' \to (C')_Z^\sim = B \to D.
$$
...
It is then also clear that $|B| \leq \kappa$ because $B$ is
a localization of $C'$.
Thus $B$ has at most $2^\kappa$ maximal ideals.
...
Hence $D$ has at most the size indicated.
\end{remark}
