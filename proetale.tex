...
Let $A/I \to \overline{C}$ be an ind-\'etale ring map.
Then there exists an ind-\'etale ring map
$A \to C$ such that $\overline{C} \cong C/IC$ as $A/I$-algebras.
\end{lemma}
...
$C = \colim_\mathcal{C} B$ is a solution to the problem.

\medskip\noindent
Suppose that $A \to B \to \overline{C}$ and $A \to B' \to \overline{C}$
are two objects of $\mathcal{C}$. Then $B \otimes_A B' \to \overline{C}$
...
$B' \otimes_{f, B, g} B' \to \overline{C}$. Thus the category $\mathcal{C}$
is directed.
...
$C/IC$ at some multiplicative subset $\overline{S} \subset C/IC$
(Algebra, Lemma \ref{algebra-lemma-pure}).
Replacing $C$ by $S^{-1}C$ where $S \subset C$ is the inverse image
of $\overline{S}$ gives an ind-\'etale extension as in the lemma.
\end{proof}
...
\begin{proof}
Assume (2)(a), (2)(b), and (2)(c). Let $A \to B$ be faithfully flat
...
\medskip\noindent
Assume (1). We have (2)(c) by
Lemma \ref{lemma-have-sections-strictly-henselian}.
Let $A \to A_w$ be the ring map constructed in
Lemma \ref{lemma-make-w-local}.
By (1) there is a section $A_w \to A$. Thus $\Spec(A)$ is homeomorphic
to a closed subset of $\Spec(A_w)$. By
Lemma \ref{lemma-closed-subspace-w-local} we see (2)(a) holds.
...
it follows that $\mathcal{I}$ has sections \'etale locall on $X$.
...
Let $D^b_{flc}(X_\etale, \Lambda)$, resp.\ $D_{flc}(X_\proetale, \Lambda)$
be the full subcategory of
$D^b(X_\etale, \Lambda)$, resp.\ $D^b(X_\proetale, \Lambda)$
consisting of those complexes whose cohomology sheaves are locally
constant sheaves of $\Lambda$-modules of finite type. Then
$$
\epsilon^{-1} :
D_{flc}^b(X_\etale, \Lambda)
\longrightarrow
D_{flc}^b(X_\proetale, \Lambda)
$$
...
\begin{proof}
The functor
$\epsilon^{-1} : D^+(X_\etale, \Lambda) \to D^+(X_\proetale, \Lambda)$
is fully faithful by Lemma \ref{lemma-relative-comparison}.
Hence it suffices to show that the functor of the lemma is essentially
surjective. Let $K$ be an object of $D_{flc}(X_\proetale, \Lambda)$.
Let $a \leq b$ be such that $H^i(K) = 0$ for $i \not \in [a, b]$.
We will prove $K$ is in the essential image of $\epsilon^{-1}$
by induction on $b - a$. If $b = a$, then this follows from
Lemma \ref{lemma-compare-locally-constant}.
In general we can choose a distinguished triangle
$$
M \to K \to L \to M[1]
$$
in $D_{flc}(X_\proetale, \Lambda)$ such that the induction hypothesis applies
to $M$ and $L$. For example we can take $a \leq c \leq b$ and take
$M = \tau_{\leq c}K$ and $L = \tau_{\geq c + 1}K$, see
Derived Categories, Remark
\ref{derived-remark-truncation-distinguished-triangle}.
Then $L$ and $M[1]$ as well as the map $L \to M[1]$ come from
$X_\etale$, whence $K$ does as well.
\end{proof}
...
\noindent
It is not clear that a closed immersion of schemes determines
an exact pushforward on abelian pro-\'etale sheaves.
On the other hand, the following is always true.

\begin{lemma}
\label{lemma-closed-immersion}
Let $i : Z \to X$ be a closed immersion morphism of schemes. Then
$i_{\proetale, *}$ is fully faithful and
$i_\proetale^{-1}i_{\proetale, *} \cong \text{id}_{\Sh(Z_\proetale)}$.
\end{lemma}

\begin{proof}
The two assertions are equivalent by
Sites, Lemma \ref{sites-lemma-exactness-properties}.
The question is (Zariski) local on $X$, hence we may assume that
$X$ is affine. In this case we see that the
essential image of the base change functor
$$
u : X_\proetale \to Z_\proetale
$$
contains all affine schemes which are pro-\'etale over $Z$, see
Lemma \ref{lemma-lift-ind-etale}. It follows that every object of
$Z_\proetale$ has a covering by objects in the essential image of $u$.
The rest of the argument is almost completely formal.

\medskip\noindent
It is clear that for every sheaf of sets $\mathcal{G}$ on $Z_\proetale$
the map
$$
i_\proetale^{-1}i_{\proetale, *}\mathcal{G} \longrightarrow \mathcal{G}
$$
is surjective by working out the definition of pullback (details omitted).
Suppose that $V$ is an object of $Z_\proetale$ and $s, s'$ are sections
of $i_\proetale^{-1}i_{\proetale, *}\mathcal{G}$ over $V$ which map
to the same section of $\mathcal{G}$ over $V$. We will show $s = s'$.
After replacing $V$ by the members of a covering, $s$ and $s'$ come from
sections $t$, $t'$ of $i_{\proetale, *}\mathcal{G}$ over $U$, $U'$ where
$U$, $U'$ are objects of $X_\proetale$ endowed with morphisms $V \to U_Z$
and $V \to U'_Z$ over $Z$. We may assume $U = U'$ by working with
$U \times_X U'$ and the diagonal map $V \to (U \times_X U')_Z$.
Now we $t, t' \in \mathcal{G}(U_Z)$ map to the same element of
$\mathcal{G}(V)$. After replacing $U$ and $V$ by members of coverings
we may assuem $U$ and $V$ are affine. We may also assume that the
morphism $V \to U_Z$ is pro-\'etale by
Proposition \ref{proposition-weakly-etale}.
Then we can find a pro-\'etale morphism $U' \to U$ such that
$U'_Z = V$ as schemes over $U_Z$, see Lemma \ref{lemma-lift-ind-etale}.
Thus we see that $t|_{U'} = t'|_{U'}$ which implies that $s = s'$.
\end{proof}
...
equivalent. Every object of $X_\proetale$, resp.\ $Z_\proetale$
has a covering by objects which are spectra of algebras ind-\'etale
over $A$, resp.\ $B$ by Proposition \ref{proposition-weakly-etale}.
Moreover, since $X \to Y$ is a universal homeomorphism, a family
of morphisms $\{U_i \to U\}$ of $X_\proetale$ is a covering if
and only if $\{U_{i, Z} \to U_Z\}$ is a covering.
The result follows formally from these facts.
\end{proof}
...
\item every point of $U$ specializes to a point of $U_Z = Z \times_X V$.
\end{enumerate}
Then $i^{-1}\mathcal{F}(U_Z) = \mathcal{F}(U)$ for all abelian sheaves on
$X_\proetale$.
\end{lemma}
...
Our goal is to show that $(i^{-1}\mathcal{F})(Z) = \mathcal{F}(X)$.
Write $X = \Spec(A)$ and $Z = \Spec(A/I)$. By
Lemma \ref{lemma-affine-alternative} we may work with
the morphism of sites $u : X_{app} \to Z_{app}$, $V \mapsto V_Z$.

\medskip\noindent
For $A/I \to \overline{C}$ an ind-\'etale ring map, let
$\mathcal{C}$ be the category of factorizations $A \to B \to \overline{C}$
with $A \to B$ \'etale. In the proof of Lemma \ref{lemma-lift-ind-etale}
we have seent that $\mathcal{C}$ is a directed category and that
$C = \colim_\mathcal{C} B$ is an ind-\'etale algebra with
$C/IC = \overline{C}$. We claim that for an ind-\'etale algebra
$D$ over $A$ we have
$$
\Mor_A(D, C) = \Mor_{A/I}(D/ID, \overline{C})
$$
Namely, let $A \to D \to \overline{C}$ be a factorization where $D$
is an ind-\'etale $A$-algebra. Write $D = \colim_I B_i$ as a filtered
colimit with $B_i$ \'etale over $A$. By choice of $\mathcal{C}$
we obtain a transformation $I \to \mathcal{C}$ and hence a map
$D \to C$ compatible with maps to $\overline{C}$. Whence the conclusion.

\medskip\noindent
It follows that the functor $u$ has a left adjoint
$v : \Spec(\overline{C}) \mapsto \Spec(C)$.
If $\{W_i \to Z\}$ is a covering of $Z_{app}$ then
$\{v(W_i) \to Z\}$ is a covering of $X$ (as every point of $X$
specializes to a point of $Z$).
Moreover, for $W$ in $Z_{app}$ it is clear that $v(W)$
is an affine scheme such that every point specializes to a point
lying over $Z$ (details omitted).
It follows that $v$ is a continuous functor.
Similarly, we see that if $\{U_i \to v(W)\}$ is a covering in
$X_{app}$, then $\{U_{i, Z} \to W\}$ is a covering in $Z_{app}$
such that $\{v(U_{i, Z} \to v(W)\}$ refines $\{U_i \to v(W)\}$.
Thus we see that $v$ is concontinuous as well.

\medskip\noindent
Now we can apply Sites, Lemma \ref{sites-lemma-have-functor-other-way-morphism}
(with roles of $u$ and $v$ reversed)
to see that $(i^{-1}\mathcal{F})(W) = \mathcal{F}(v(W))$ for any
$W$ in $Z_{app}$. (Note that sheafification isn't necessary because
$v$ is continuous.)

\medskip\noindent
Finally, since $X$ is w-contractible and every point of $X$ specializes
to $Z$ we have $v(Z) = X$ (details omitted). The lemma follows.
\end{proof}
...
It suffices to show that $X_\proetale$ has enough objects on which
...
Then $V_Z$ is cut out by an ideal $I \subset A$.
...
Let $X$ be a scheme. Let $\Lambda$ be a Noetherian ring.
Let $D^b_c(X_\etale, \Lambda)$, resp.\ $D^b_c(X_\proetale, \Lambda)$
be the full subcategory of
$D^b(X_\etale, \Lambda)$, resp.\ $D^b(X_\proetale, \Lambda)$
...
$$
\epsilon^{-1} :
D_c^b(X_\etale, \Lambda)
\longrightarrow
D_c^b(X_\proetale, \Lambda)
$$
is an equivalence of categories.
\end{lemma}
...
is fully faithful by Lemma \ref{lemma-relative-comparison}.
Hence it suffices to show that the functor of the lemma is essentially
surjective. Let $K$ be an object of $D_c(X_\proetale, \Lambda)$.
Let $a \leq b$ be such that $H^i(K) = 0$ for $i \not \in [a, b]$.
We will prove $K$ is in the essential image of $\epsilon^{-1}$
by induction on $b - a$. If $b = a$, then this follows from
...
M \to K \to L \to M[1]
$$
in $D_c(X_\proetale, \Lambda)$ such that the induction hypothesis applies
to $M$ and $L$. For example we can take $a \leq c \leq b$ and take
$M = \tau_{\leq c}K$ and $L = \tau_{\geq c + 1}K$, see
Derived Categories, Remark
\ref{derived-remark-truncation-distinguished-triangle}.
