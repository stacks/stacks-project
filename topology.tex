...
\begin{proof}
This follows from Lemma \ref{lemma-Hausdorff} as $s(Y) = \{x \in X \mid x = s(f(x))\}$.
\end{proof}
...
\noindent
The following terminology is taken from \cite{Hochster} and
...
In that paper it is shown that in the category of quasi-compact
...
Let $f : X \to Y$ be a continuous map of Hausforff quasi-compact
topological spaces. If $f$ is surjective and $Y$ extremally disconnected,
then $f$ has a continuous section.
\end{proposition}

\begin{proof}
By Lemma \ref{lemma-find-compact-subset} there exists a quasi-compact subset
$E \subset X$ such that $f(E) = Y$ but $f(E') \not = Y$ for all
proper closed subsets $E' \subset E$. By
Lemma \ref{lemma-isomorphism} we find that $f|_E : E \to Y$
...
property of the Stone-{\v C}ech compactification. Let $\varphi : \beta(X) \to X^*$
be this factorization. Then $X \to \varphi^{-1}(X)$ is a section to
$\varphi^{-1}(X) \to X$ hence has closed image (Lemma \ref{lemma-section-closed}).
