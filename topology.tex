...
\begin{lemma}
\label{lemma-extremally-disconnected-refine-open-covering}
Let $X$ be an extremally disconnected, Hausdorff, quasi-compact space.
Every open covering of $X$ has a refinement by a finite covering
$X = \coprod U_i$ with $U_i$ open and closed.
\end{lemma}

\begin{proof}
Let $X = \bigcup_{i \in I} U_i$ be an open covering. Let $x \in X$.
Choose an $i$ with $x \in U_i$. We can find disjoint opens
$x \in U_x$ and $X \setminus U_i \subset V_x$, see
Lemma \ref{lemma-quasi-compact-in-Hausdorff}. Then the closure
$E_x = \overline{U_x}$ is open and closed in $X$ and contained
in $U_i$. Then $X = \bigcup E_x$ is an open covering refining the
given open covering. Since $X$ is quasi-compact we can find a finite list
$x_1, \ldots, x_n$ such that $X = \bigcup_{i = 1, \ldots, n} E_{x_i}$.
By Proposition \ref{proposition-projective-in-category-hausdorff-qc}
there is a continuous section $\sigma$ to the map
$$
\coprod E_{x_i} \longrightarrow X
$$
(The proposition applies as each $E_x$ is quasi-compact and Hausdorff.)
Hence $X = \coprod \sigma^{-1}(E_{x_i})$ is the disjoint
open covering of $X$ refining the given open covering.
\end{proof}
