...
$U_1 \setminus U_2$ (Lemma \ref{lemma-Noetherian}).
Then $Z \in U'_1$ but $Z \not \in U'_2$. Hence $c$ induces
a bijection between the subsetsm of $X'$ of the form $U'$ and the
opens of $X$.
...
Let $I$ be a directed partially ordered set. Let $X_i$ be an inverse
system of spectral spaces over $I$ whose transition maps are spectral maps.
Then the inverse limit $X = \lim X_i$ is a spectral space and the projection
maps $X \to X_i$ are spectral.
\end{lemma}

\begin{proof}
The limit $X = \lim X_i$ exists by Lemma \ref{lemma-limits}.
As a set $X = \lim X_i$. Denote $p_i : X \to X_i$ the projection.
Because $I$ is directed we can apply Lemma \ref{lemma-describe-limits}.
A basis for the topology is given by the opens
$p_i^{-1}(U_i)$ for $U_i \subset X_i$ open. Since a basis for
the topology of $X_i$ is given by the quasi-compact open, it suffices
to consder quasi-compact opens $U_i$. We claim that in this case
$p_i^{-1}(U_i)$ is a quasi-compact open of $X$.

\medskip\noindent
Denote $X'_i$ the space $X_i$ endowed with the constructible topology.
Let $X' = \lim X'_i$ be the limit space. Denote $p_i' : X' \to X'_i$
the projection. Let $E_i$ be a constructible subset of $X_i$ for some $i$.
As the transition map $f_{i'i} : X_{i'} \to X_i$ is spectral, the
set $E_{i'} = f_{i'i}^{-1}(E_i)$ is a constructible subset of $X_{i'}$.
Therefore $(p'_i)^{-1}(E_i) = \lim_{i' \geq i} E_{i'}$ is a closed
and quasi-compact subset of $X'$ by
Lemmas \ref{lemma-spectral-sub},
\ref{lemma-constructible-hausdorff-quasi-compact}, and
\ref{lemma-inverse-limit-quasi-compact}.
As sets $X'_i = X_i$ and $X' = X$ and as maps of sets
$p'_i = p_i$. The maps $X'_i \to X_i$ and $X' \to X$ are continuous.
Hence the image of $(p'_i)^{-1}(E_i) \to X$ is quasi-compact. Applying this
with $E_i = U_i \subset X_i$ a quasi-compact open the claim follows.

\medskip\noindent
Given $U_i \subset X_i$ and $U_j \subset X_j$ quasi-compact open, then
$p_i^{-1}(U_i) \cap p_j^{-1}(U_j) = p_k^{-1}(U_k)$ for some $k \geq i, j$
and quasi-compact open $U_k \subset X_k$ (namely, the intersection of the
pullbacks of $U_i$ and $U_j$ in $X_k$). Finally, if $Z \subset X$ is
irreducible and closed, then $p_i(Z) \subset X_i$ is irreducible
and therefore has a unique generic point $\xi_i$ (because $X_i$
is a finite sober topological space). Then $\xi = \lim \xi_i$ is a
generic point of $Z$ (it is a point of $Z$ as $Z$ is closed).
This finishes the proof.
