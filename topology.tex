...
\begin{proof}
    We suppose that $ X $ is Hausdorff. Let $(x,y) \notin \Delta(X)$ i.e. $x \neq y$. There are $ U $ and $ V $ disjoint open sets of $X$ such that $ x \in U $ and $ y \in V $, $U\cap V \neq \emptyset$ implies that $ U\times V \subset (X\times X) \setminus \Delta(X) $. This shows that $ (X\times X) \setminus \Delta(X) $ is an open set of $ X \times X$ which is equivalent to say that the diagonal $\Delta(X) \subset X \times X$ is closed in $X\times X$. The converse is similar : The complement $ (X\times X) \setminus \Delta(X) $ consist precisely of $(x,y) \in X\times X$ with $ x\neq y $. Thus, if $ \Delta(X) \subset X\times X $ is closed, then, by the definition of the product topology, for every such $ (x,y) $, there are opens $ U, V \subset X$ with $(x,y) \in U\times V$ and $(U \times V)\cap\Delta(X) = \emptyset $. In other words, with $x \in U$ and $ y \in V $ such that $ U\cap V = \emptyset $.
\end{proof}
...
\begin{proof}
    If $f$ is separated, by Definition \ref{definition-separated}, $\Delta$ is a closed map. The fact that $X$ is closed in $X$ gives us that $\Delta(X)$ is closed in $ X\!\times_Y\! X $. It shows that $(1)$ implies $(2)$.
    
    Now if $\Delta(X) \subset X\!\times_Y\! X$ is a closed subset, it means that if we call $ \mathcal U$ the set $(X \!\times\! X \setminus \Delta(X)) \cap (X\!\times_Y\! X) $, $\mathcal U$ is open in $X\!\times_Y\! X$. Thus we have an open set $W$ such that $ W\cap (X\!\times_Y\! X) \subset \mathcal U$. However, by definition of the product topology, if $(x,x') \in W\cap (X\!\times_Y\! X) $, we have $U$ and $V$ open sets of $X$ such that $x \in U$, $x' \in V$ and $ U\times V \subset W $. If we had $ U\cap V \neq \emptyset$, we would have $z \in U\cap V $. However, $(z,z) \in X\!\times_Y\! X$, so $ (z,z) \in (U\times V)\cap(X\!\times_Y\! X) \subset \mathcal U$, which is absurd. Therefore $ U\cap V =\emptyset $, and we have two disjoint open neighborhoods for $x$ and $x'$. It proves that $(2)$ implies $(3)$.
    
    Finally, we suppose that given distinct points $x,x'\in X$ mapping to the same point of $Y$, there exist disjoint open neighbourhoods of $x$ and $x'$. Let $ F $ be a closed set of $X$. We can notice that $ (X\!\times\!X \setminus \Delta(F))\cap(X\!\times_Y\!X) = [(X\!\times\!X \setminus \Delta(X))\cap(X\!\times_Y\!X)] \cup \Delta(X\setminus F)$. As $X\setminus F$ is open, $\Delta(X\setminus F)$ is too. Thus we just have to show that $ (X\!\times\! X\setminus\Delta(X))\cap (X\!\times_Y\! X) $ is open in $X\! \times_Y\! X$. Let $(x,x') \in (X\!\times\!X \setminus \Delta(X))\cap(X\!\times_Y\!X)$. Because $x\neq x'$ and $f(x) = f(x')$, the hypothesis gives $U$ and $V$ disjoint open neighborhoods for $x$ and $x'$. Therefore $U\times V \subset \Delta(X)$, and even more so $ (U\times V)\cap (X\!\times_Y\!X) \subset (X\times X\setminus\Delta(X))\cap (X\!\times_Y\!X) $, which shows that $ (X\times X\setminus\Delta(X))\cap (X\!\times_Y\!X) $ is open in $(X\!\times_Y\!X)$ as required. We have shown that $ (3) $ implies $ (1) $.
\end{proof}
...
\begin{proof}
	Let us consider $\sigma(\mathcal B)$ the reunions of finite intersections of elements from $\mathcal B$. Since by its definition, $\sigma(\mathcal{B})$ is closed under finite intersection and reunions, $\emptyset = \bigcup_\emptyset B \in \sigma(\mathcal B)$ and we supposed that $ X = \bigcup_{B \in \mathcal{B}} B $, it is a topology.
 
    It verifies the properties listed in definition \href{https://stacks.math.columbia.edu/tag/004P}{5.5.1} : 
	Firstly, $ \mathcal B \subset \sigma(\mathcal B)$. Secondly, if $ U \in \sigma(\mathcal B) $ and if $ x \in U $, we have a set $ I $ such that $ U = \bigcup_{i \in I}\bigcap_{B \in \mathcal P_i \subset \mathcal B} B $. Thus we have an $ i_0 \in I $ verifying $ x \in \bigcap_{B \in \mathcal P_{i_0} \subset \mathcal B} B $ with $ \#\mathcal P_{i_0} < \infty $. By induction, it is clear that : if $ \{B_1,...,B_n\} \subset \mathcal B $ and $ x \in \bigcap_{i = 1}^n B_i $, then there is a $ B_{n+1} $ such that $ x \in B_{n+1} \subset \bigcap_{i = 1}^n B_i $. It implies that we have a $ B_{i_0} \subset \mathcal B $ verifying $ x \in B_{i_0} \subset \bigcap_{B \in \mathcal P_{i_0}} B \subset U $.
 
	For the uniqueness of this topology, let $ \mathcal T $ be a topology on $ X $ such that $ \mathcal B $ is a base for it. If $ U \in \mathcal T $, we supposed that for every $ x \in U $ there is a $ B_x \in \mathcal B $ such that $ x \in B_x \subset U $. Therefore $ U \subset \bigcup_{x \in U} B_x \subset U $, which means $ U = \bigcup_{x \in U} B_x \in \sigma(\mathcal B) $. It shows that $ \mathcal T \subset \sigma(\mathcal B) $. Since $ \mathcal T$ is a topology containing $ \mathcal B $, it implies that $ \sigma(\mathcal B) \subset \mathcal T$. Therefore by double inclusion : $\sigma(\mathcal B) = \mathcal T$.
\end{proof}
...
\begin{proof}
	If $ x \in U = \bigcup_{i\in I} U_i $, there is an $ i_x \in I $ such that $ x \in U_{i_x} $. Thus we have a $ B_{i_x} $ verifying $ x \in B_{i_x} \subset U_{i_x} $. Now defining $$ J := \{i_x \, | \: x \in U\} $$ and for $ j \in J$, $$ V_j := B_j \in \mathcal B $$
    gives the desired covering of $ U $ by elements of $\{V_j\}_j$.
\end{proof}
...
\begin{proof}
    If we consider that $ \bigcap_\emptyset B = X $, then we can apply Lemma \ref{lemma-make-base} to the collection of subsets $ \mathcal A $ containing the finite intersections of elements from $ \mathcal B $.
\end{proof}
...
\begin{proof}
   The set $ \mathcal{T} = \{f^{-1}(U) \, | \: U \subset X \text{ open}\}$ is a topology on $Y$. Firstly, $\emptyset = f^{-1}(\emptyset)$ and $f^{-1}(X) = Y$. So $\mathcal{T}$ contain $\emptyset$ and $Y$. 
    Now let $\{V_i\}_{i\in I}$ be a collection of open subsets where $V_i \in \mathcal{T}$ and write $V_i = f^{-1}(U_i)$ where $U_i$ is an open subset of X, then 
    $$\bigcup_{i\in I}V_i = \bigcup_{i\in I}f^{-1}(U_i) = f^{-1}\left(\bigcup_{i\in I}U_i\right).$$
    So $\bigcup_{i\in I}V_i \in \mathcal{T}$ as $\bigcup_{i\in I}U_i$ is open. Now let $V_1, V_2 \in \mathcal{T}$. We have $ U_1, U_2$ such that $V_1 = f^{-1}(U_1)$ and $V_2 = f^{-1}(U_2)$. Then 
    $$V_1 \cap V_2 = f^{-1}(U_1) \cap f^{-1}(U_2) = f^{-1}(U_1 \cap U_2).$$
    So $V_1 \cap V_2 \in \mathcal{T}$ because $U_1 \cap U_2$ is open. 

    Finally a topology on $Y$ such that $f$ is continuous contain $\mathcal{T}$ according to the definition of a continuous function, so $\mathcal{T}$ is the weakest topology on $Y$ such that $f$ is continuous. It proves that (1) and (2) are equivalent. 

    And (2) and (3) equivalent follows from $f^{-1}(X\setminus U) = Y \setminus f^{-1}(U)$ for all $U \subset X$.
\end{proof}
...
\begin{proof}
    The set $\mathcal{T} =\{V\subset Y \,|\: f^{-1}(V) \text{ is open}\}$ is a topology on $Y$. Firstly $\emptyset = f^{-1}(\emptyset)$ and $f^{-1}(Y) = X$. So $\mathcal{T}$ contain $\emptyset$ and $Y$. 
    Now let $(U_i)_{i\in I}$ be a collection of open subset where $U_i \in \mathcal{T}$ then 
    $$\bigcup_{i\in I}f^{-1}(U_i) = f^{-1}\left(\bigcup_{i\in I}U_i\right).$$
    Thus $\bigcup_{i\in I}U_i \in \mathcal{T}$ as $\bigcup_{i\in I}f^{-1}(U_i)$ is open. 
    Furthermore if $U_1, U_2 \in \mathcal{T}$ then 
    $$f^{-1}(U_1) \cap f^{-1}(U_2) = f^{-1}(U_1 \cap U_2).$$
    So $U_1 \cap U_2 \in \mathcal{T}$ because $f^{-1}(U_1) \cap f^{-1}(U_2)$ is open. 
    ...
    Finally, (2) and (3) equivalence follows from $f^{-1}(X\setminus U) = Y \setminus f^{-1}(U)$ for all $U \subset X$.
\end{proof}
...
\begin{proof}
    Firstly, we tackle the case $n=2$. Let $A \subset X$ and $B \subset X$ be two noetherian topological spaces. Let $\{F_n\}_{n \in \mathbf{N}}$ a decreasing sequence of closed subsets of $A\cup B$ with the induced topology. Then we have $\{G_n\}_{n \in \mathbf{N}}$ a decreasing sequence of closed subsets of $X$ verifying $F_n = G_n\cap (A\cup B)=(G_n \cap A)\cup(G_n \cap B)$ for all $n \in \mathbf{N}$. \\
    As $A$ is noetherian and $\{G_n \cap A\}_{n \in \mathbf{N}}$ a decreasing sequence of closed subsets of $A$, there exists $N_A \in \mathbf{N}$ such that for all $n \geq N_A,G_n \cap A = G_{N_A} \cap A$ \\
    Same for $B$ and $ \{G_n \cap B\}_{n \in \mathbf{N}} $ with the existence of a $N_B \in \mathbf{N}$ such that $G_n \cap B=G_{N_B} \cap B$ if $n \geq N_B$.\\
    Let $N = \text{max}\{N_A,N_B\}$. For all $n \geq N$, we have $ F_n = (G_n \cap A)\cup(G_n \cap B)=(G_N \cap A)\cup(G_N \cap B) = F_N$. So it proves $A \cup B$ is noetherian.
    
    Thanks to a finite induction, the lemma remains true for $n \geq 2$.
\end{proof}
...
\begin{proof}
    Let suppose that $X$ is catenary. According to the definition \href{https://stacks.math.columbia.edu/tag/02I1}{5.11.4}, for every pair of irreducible closed subsets $Y \subset Y'$, $\text{codim}(Y,Y')<\infty$. Let $Y \subset Y' \subset Y''$ be a triple of irreducible closed subsets of $X$. Let $$Y = Y_0 \subset Y_1 \subset ... \subset Y_{e_1} = Y'$$ be a maximal chain of irreducible closed subsets between $Y$ and $Y'$ where $e_1 = \text{codim}(Y,Y')$. Let also $$ Y'=Y_{e_1}\subset Y_{e_1+1}\subset ... \subset Y_{e_1+e_2}=Y" $$ be a maximal chain of irreducible closed subsets between $Y'$ and $Y''$ where $e_2 = \text{codim}(Y',Y")$.\\
    As the two chains are maximal, the concatenation $Y=Y_0\subset Y_1 \subset ... \subset Y_{e_1}=Y'=Y_{e_1}\subset Y_{e_1+1}\subset ... \subset Y_{e_1+e_2}=Y''$ is maximal too (between $Y$ and $Y''$) and its length equals to $e_1+e_2$. As $X$ is catenary, each maximal chain has the same length equals to the codimension. Thus the point (2)
    $$ \text{codim}(Y,Y") = e_1 + e_2 = \text{codim}(Y,Y') + \text{codim}(Y',Y") $$
    is verified.
    
    For the reciprocal, we show by induction that : if $Y = Y_1 \subset ... \subset Y_n = Y'$, then $ \text{codim}(Y,Y') = \text{codim}(Y_1,Y_2) + ... + \text{codim}(Y_{n-1},Y_n)$. Therefore, it forces maximal chains to have the same length.
\end{proof}
...
\begin{proof}
    We suppose that $\bigcap_{\alpha \in A}Z_{\alpha}=\emptyset$. So we have $\bigcup_{\alpha \in A}(X\setminus Z_{\alpha})=X$ by complementation. As the subsets $Z_\alpha$ are closed, $\bigcup_{\alpha \in A}(X\setminus Z_{\alpha})$ is an open covering of the quasi-compact space $X$. Thus there exists a finite subset $J \subset A$ such that $X = \bigcup_{\alpha \in J}(X\setminus Z_{\alpha})$. The complementary is then empty, which means that $\bigcap_{\alpha \in J}Z_{\alpha} = \emptyset$. It proves there exists a finite subcollection of $\{Z_{\alpha}\}_{\alpha \in J}$ verifying $\bigcap_{\alpha \in J}Z_{\alpha}=\emptyset$, which concludes by contraposition.
\end{proof}
...
\begin{proof}
    Suppose that $T$ is stable under specialization, then for all $y\in T$ we have $\overline{\{y\}} \subset T$. Thus $T = \bigcup_{y\in T} \overline{\{y\}}$ which is an union of closed subsets of $X$. Reciprocally, suppose that $T = \bigcup_{i\in I}F_i$ where $F_i$ are closed subsets of $X$. If $y\in T$ then there exists $i\in I$ such that $y\in F_i$. As $F_i$ is closed, we have $\overline{\{y\}} \subset F_i \subset T$, which proves that $T$ is stable under specialization.
\end{proof}
...
\begin{proof}
    We write $\mathring{\overline{T}}=X \cap \mathring{\overline{T}} = (\bigcup	U_i) \cap \mathring{\overline{T}} = \bigcup (\mathring{\overline{T}} \cap U_i)$. As closure commutes with intersection when subsets are open, we have $$\overline{T \cap U_i}=\overline{T}\cap \overline{U_i}$$ for all $i$. The interior also commutes with intersection, thus $$\overset{\circ}{\widearc{\overline{T}\cap \overline{U_i}}} = \mathring{\overline{T}}\cap \mathring{\overline{U_i}}=\mathring{\overline{T}} \cap U_i.$$ Therefore, $\emptyset = \mathring{\overline{T \cap U_i}}=\mathring{\overline{T}} \cap U_i$ (as the $T \cap U_i$ subsets are nowhere dense in $X$). It implies $\mathring{\overline{T}}=\bigcup (\mathring{\overline{T}} \cap U_i)=\bigcup \emptyset = \emptyset$ which means that $T$ is nowhere dense in $X$.
\end{proof}
