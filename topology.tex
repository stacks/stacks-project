...
We claim that $V \cap U$ is retrocompact in $U$. Namely,
let $W \subset U$ be open and quasi-compact. Then $W$
is open and quasi-compact in $X$. Hence $V \cap W = V \cap U \cap W$
is quasi-compact as $V$ is retrocompact in $X$. The claim follows.
If $V, V'$ are retrocompact open in $X$, then we have 
$V' \cap V^c \cap U = (V' \cap U) \cap (U \setminus (U\cap V))$.
In other words, the inverse image of the intersection $V' \cap V^c$
has a similar shape in $U$.
The result follows directly from this and the definition
