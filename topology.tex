...
\noindent
Note that $X$ is Jacobson if and only if
...
Then for $x\in U\cap Z$ we have that $\overline{\{x\}}\cap U$ is a nonempty subset of $Z\cap U$,
...
in $X$ such that $Z\cap U$ is nonempty and does not contain points closed in $X$.
As $X$ has a basis of quasi-compact open sets we may replace $U$ by an open quasi-compact neighborhood
of a point in $Z\cap U$ and so we may assume that $U$ is quasi-compact open.
\medskip\noindent
So by Lemma \ref{lemma-quasi-compact-closed-point}, there exists a point $x\in Z\cap U$ closed in $Z\cap U$,
...
\begin{itemize}
\item[$(*)$] Every nonempty locally closed subset of $T$ has a point closed in $X$.
\end{itemize}
...
then there exists a $T_i$ such that $A\cap T_i$ is nonempty, it is locally closed in $T_i$ and so in $X$.
...
\begin{proof}
We just prove that the correspondence $E \mapsto E \cap X_0$ is inyective.
Indeed if $E\neq E'$ then without loss of generality $E\setminus E'$ is nonempty, and it is a finite union of locally closed sets (details omitted).
Then as $X$ is Jacobson then $(E\setminus E')\cap X_0=E\cap X_0\setminus E'\cap X_0$ is not empty.
\end{proof}
...
we just have to see that if $U$ is open in $X$ then $U\cap X_0$ is retrocompact in $X_0$ if and only if $U$ is retrocompact in $X$.
This follows if we prove that for $U$ open in $X$ then $U\cap X_0$ is quasi-compact compact if and only if $U$ is quasi-compact.
From  Lemma \ref{lemma-jacobson-inherited} it follows that we may replace $X$ by $U$ and assume that $U=X$.
Finally notice that any collection of opens $\mathcal{U}$ of $X$ cover $X$ if and only if they cover $X_0$, using the Jacobson property
of $X$ in the closed $X\setminus \bigcup\mathcal{U}$ to find a point in $X_0$ if it were nonempty. 
\end{proof}
