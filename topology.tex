...
\noindent
Let $f : X \to Y$ be a continuous map of topological spaces.
In this case we obtain a factorization $X \to f(Y) \to Y$
of maps of sets. We can endow $f(Y)$ with the
quotient topology coming from the surjection $Y \to f(Y)$
or with the induced topology coming from the injection $f(Y) \to X$.
The map
$$
(f(Y), \text{quotient topology})
\longrightarrow
(f(Y), \text{induced topology})
$$
is continuous.
...
by Lemma \ref{lemma-limits}. Because $I$ is directed we can describe
$X$ as follows. As a set $X = \lim X_i$. Denote $p_i : X \to X_i$ the
projection. A basis for the topology is given by the opens
...
where $W = \{0, 1\}$ as in the proof of
...
\begin{proof}
Since we can write each $T_i$ as a finite union of constructible
locally closed subsets, we may assume that $T_i = U_i \cap V_i^c$
with $U_i, V_i \subset X$ quasi-compact open. Let $S$ be the
