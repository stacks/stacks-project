...
Assume that
(a) $f$ is open,
(b) all fibres of $f$ are nonempty and connected, and
(c) the connected components of $X$ are open.
Then $f$ induces a bijection between the sets of connected
components of $X$ and $Y$.
\end{lemma}

\begin{proof}
Let $T$ be a connected component of $X$.
As $f$ is open and $T$ is open we see that
$f(T)$ is an open connected subset of $Y$.
As connected components are always closed
(Lemma \ref{lemma-connected-components})
we see that $X \setminus T$ is open, and hence $f(X \setminus T)$
is open, hence (as $f$ is surjective) $f(T)$ is closed. Hence
$f(T)$ is a connected, open and closed subset of $Y$.
In particular $f(T)$ is a connected component of $Y$. The lemma follows.
\end{proof}
...
Let $X$ be a nonempty quasi-compact topological space.
...
$Z \in \mathcal{T}$. Clearly $Z = \{x\}$ for some $x$ and
