...
$$
F : \textit{Cat} \to \textit{Sets}, \quad
(\text{Ob}, \text{Arrows}, s, t, \circ)
\mapsto
\text{Ob} \coprod \text{Arrows}.
$$
Then $F$ is faithful, $\textit{Cat}$ has direct limits and
$F$ commutes with them, $\textit{Cat}$ has directed colimits and
$F$ commutes with them, and $F$ reflects isomorphisms.
...
the result commutes with $F$. Applying this to
...
easy to see that $\mathcal{S}^\#$ is a stack and
the functor $\mathcal{S} \to \mathcal{S}^\#$
