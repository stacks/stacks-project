...
is representable, we see from
Categories, Lemma \ref{categories-lemma-fibred-category-representable-goes-up}
that $x_i \to x$ is representable and $x_i \times_x y \to y$ is
...
\noindent
Note that if $\mathcal{S}$ is fibred in groupoids, then
$$
...
Then $F$ is a cocontinuous functor between the structure of sites
inherited from $\mathcal{C}$ hence induces a morphism of topoi
$\textit{Sh}(\mathcal{X}) \to \textit{Sh}(\mathcal{Y})$.
\end{lemma}
...
cartesian morphisms. Hence $F$ transforms coverings of $\mathcal{X}$
into coverings of $\mathcal{Y}$. This immediately implies that $F$ is
cocontinuous, see
Sites, Definition \ref{sites-definition-cocontinuous}.
The final assertion follows from this and
Sites, Lemma \ref{sites-lemma-cocontinuous-morphism-topoi}.
\end{proof}

\begin{lemma}
\label{lemma-stack-over-fibred-in-groupoids}
...
of $\mathcal{Y}$. Let $(x_i, \varphi_{ij})$ be a descent datum with
respect to this covering. Then $(x_i, \varphi_{ij})$ is also a descent
datum with respect to the covering $\{q(y_i) \to q(x)\}$ of
$\mathcal{C}$. As $\mathcal{X}$ is a stack in groupoids we obtain an
object $x$ over $q(y)$ and isomorphisms $x|_{q(y_i)} \to x_i$
over $q(y_i)$ compatible with the $\varphi_{ij}$.
...
\medskip\noindent
Now we want to do something a tad tricky. Namely, let
...
$\mathcal{C}$. Hence it seems clear what we should do: Just require that
$\mathcal{X} \to \mathcal{Y}$ {\it is} a gerbe in the sense above. Except
the notion so obtained is not independent under replacing $\mathcal{X}$
