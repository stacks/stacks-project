...
with $\beta$ strongly cartesian. Consider any second morphism
...
indeed $(a, \beta)$ is strongly cartesian. Finally, we have to show
...
To finish we have to show that we have descend for morphisms and
descend for objects. Let $\mathcal{U} = \{U_i \to U\}$ be a covering
...
As we have the identifications of fibre categories
$(u^p\mathcal{S})_{U} = \mathcal{S}_{u(U)}$, 
$(u^p\mathcal{S})_{U_i} = \mathcal{S}_{u(U_i)}$, and so
on we see that descend data relative to $\mathcal{U}$ are identical
...
assumptions for our particular construction). We do this in several steps.
...
\item $u$ commutes with nonempty finite limits.
\end{enumerate}
Consider the set $S \subset \text{Arrows}(u_{pp}\mathcal{S})$ of morphisms
of the form
$$
(a, \text{id}_V, \alpha) :
(U', \phi' : V \to u(U'), x')
...
(U, \phi : V \to u(U), x)
$$
with $\alpha$ strongly cartesian. Then $S$ is a right multiplicative system.
\end{lemma}
...
(U, \phi : V \to u(U), x)
$$
with $\gamma$ strongly cartesian from $S$. In this situation set
...
$$
is an element of $S$ which form a solution of the existence problem posed by
Categories, Definition \ref{categories-definition-multiplicative-system}
part (3).
...
(U', \phi : V \to u(U'), x')
$$
is an element of $S$ which equalizes the morphisms
...
(U_1, \phi_1 : V_1 \to u(U_1), x_1)
$$
is an element of $S$ and equalizes $(a, b, \alpha)$ and $(a', b', \alpha')$
as desired.
\end{proof}
