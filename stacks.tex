...
$f : \textit{Sh}(\mathcal{X}) \to \textit{Sh}(\mathcal{Y})$ characterized
by $f^{-1}(\mathcal{G})(x) = \mathcal{G}(F(x))$.
\end{lemma}
...
cartesian. By
Categories, Lemma \ref{categories-lemma-fibred-category-representable-goes-up}
again this means that $F(x_i \times_x x') = F(x_i) \times_{F(x)} F(x')$.
...
an isomorphism $F(x_i) \to y_i$ in $\mathcal{Y}_{U_i}$. Thus
$\{x_i \to x\}_{i \in I}$ is a covering of $\mathcal{X}$ such that
$\{F(x_i) \to F(x)\}_{i \in I}$ is isomorphic to $\{y_i \to F(x)\}_{i \in I}$.
Hence $F$ is cocontinuous, see
Sites, Definition \ref{sites-definition-cocontinuous}.
The final assertion follows from this,
Sites, Lemma \ref{sites-lemma-cocontinuous-morphism-topoi}, and
