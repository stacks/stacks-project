...
the functor $F$ maps strongly cartesian morphisms to srtongly cartesian
...
Categories, Section \ref{categories-section-fibred-in-sets}
for notation.
...
Then $\mathcal{S}$ is a stack in setoids if and only if the unique (up to
unique isomorphism) equivalent category $\mathcal{S}'$ fibred in sets (see
Categories, Lemma \ref{categories-lemma-setlike-fibres})
is a stack in sets (and hence corresponds to a sheaf of sets).
In other words, if and only if the presheaf
$$
