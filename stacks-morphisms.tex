...
of finite presentation over $W$. Since
$\text{pr}_0(U')$ is an open subspace of $U$ containing $p(z)$ we
may assume $U = \text{pr}_0(U')$. Now we see that
...
we see that $\text{pr}_0(U') \cap V(f_1, \ldots, f_r) \to \mathcal{X}$
...
$$
where $U$ is a scheme, $k$ is a field, and $U \to \mathcal{X}$ is locally
of finite type. Let $z \in |F|$ be such that
$\dim(\mathcal{O}_{F, \overline{z}}) = 0$.
Then, after replacing $U$ by a Zariski open neighbourhood of 
$p(z)$, the morphism
$$
...
$$
Hence $G$ is locally quasi-finite over $u$ as $\mathcal{X}$ is quasi-DM.
By
More on Groupoids in Spaces, Lemma
\ref{spaces-more-groupoids-lemma-groupoid-on-field-dimension-equal-stabilizer}
we have $\dim(R) = 0$. Hence $F \to U$ is a morphism of algebraic spaces
which is locally of finite type whose fibre over $u$ has dimension $0$.

\medskip\noindent
Let $e : u \to R$ be the identity and denote $z : u \to F$ the composition
of $e$ with the projection $R \to F$. Consider the map of local rings
$$
\varphi : \mathcal{O}_{U, u} \longrightarrow \mathcal{O}_{F, \overline{z}}
$$
We have seen above that the fibre $F_u$ of $F$ over $u$ has dimension $0$.
Thus we see that
$$
\mathfrak m_{\overline{z}} =
\sqrt{\varphi(\mathfrak m_u)\mathcal{O}_{F, \overline{z}}}
$$
On the other hand, as the morphism $U \to \mathcal{X}$ is smooth
we see that $F \to u$ is a smooth morphism of algebraic spaces.
Hence $\mathcal{O}_{F, \overline{z}}$ is a regular local ring, a fortiori
Cohen-Macaulay. Let $d = \dim(\mathcal{O}_{F, \overline{z}})$. By
...
$Z \times_\mathcal{X} u$ is a closed subspace of $F = U \times_\mathcal{X} u$,
that $z$ is a point of this closed subspace, and that
$$
\dim(\mathcal{O}_{Z \times_\mathcal{X} u, \overline{z}}) = 0.
$$
