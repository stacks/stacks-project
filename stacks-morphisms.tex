...
\begin{example}
\label{example-smooth-quotient-stack}
Let $B$ be a smooth algebraic space over a scheme $S$, and let
$G$ be a group algebraic space over $S$ which is flat
and locally of finite presentation over $S$. Let $G$ act on $B$ over $S$;
then the quotient stack $[B/G]$ is smooth over $S$
even if $G$ is not smooth over $S$!
(The quotient $[B/G]$ is an algebraic stack by Criteria, Theorem
\ref{criteria-theorem-flat-groupoid-gives-algebraic-stack}.)
The smoothness of $[B/G]$ over $S$ follows from the fact that smoothness
descends under fppf coverings:
Choose a surjective smooth morphism $U \to [B/G]$ where $U$ is a scheme.
Smoothness of $[B/G]$ over $S$ is equivalent to smoothness of $U$ over $S$.
Observe that $U \times_{[B/G]} B$ is smooth over $B$ and hence smooth
over $S$ (because compositions of smooth morphisms are smooth).
On the other hand, $U \times_{[B/G]} B \to U$ is locally of
finite presentation, flat, and surjective (because it is
the base change of $B \to [B/G]$ which has those properties
for example by Criteria, Lemma
\ref{criteria-lemma-flat-quotient-flat-presentation}).
...
$K = \mathbf{C}((t))$. Namely, the top vertical arrow is given
