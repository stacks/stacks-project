...
\begin{proof}
The inertia stack $\mathcal{I}_{\mathcal{X}'}$ is defined as the
...
\ref{stacks-properties-section-properties-morphisms}
when the morphism is representable by algebraic spaces
because such a morphism is automitically DM by
Lemma \ref{lemma-trivial-implications}.
...
an automorphism of $(a, \alpha, \beta)$ is a $2$-morphsm
...
\xymatrix{
\mathcal{I}_{\mathcal{X}/\mathcal{Y}} \ar[d] &\
\Spec(K) \ar[d]^j \ar[l]_{(a \circ j, \text{id})} \\
\mathcal{X} & \Spec(A) \ar[l]_a \ar[lu]_{(a, \theta)}
}
$$
...
is equivalent to the existence of a $2$-commutative diagram
$$
\xymatrix{
\Spec(K) \ar[r]_-x \ar[d]_j & \mathcal{X} \ar[d]^f \\
...
be two $2$-morphisms. Then $\gamma \circ (\gamma')^{-1}$
...
However, we do want to point out that we do not know how to prove
this lemma for general algebraic stacks $\mathcal{Y}$ if instead
of the category of dotted arrows we just look at the set of $2$-commutive
squares (as in Lemma \ref{lemma-cat-dotted-arrows-independent})
up to isomorphism. To prove the lemma we are allowed to replace
...
Then we can go from a triple $(a, \alpha, \beta)$ to a triple
$(a', \alpha', \beta')$ where $a' = (y', a, \beta)$ and
...
algebraic stakcs and the morphism is proper. Whence it
...
Let $f : \mathcal{X} \to \mathcal{Y}$ be a morphism of algebraic stacks.
If $\Delta_f$ is quasi-separated and the category of dotted arrows
is always a setoid, then $\Delta_f$ is separated.
\end{lemma}
...
the uniquness part of the valuative criterion implies
...
we conlude is universally closed by the valuative criterion
...
we can find a valutation ring $A$ with fraction field $K$
...
\begin{proof}
A proper morphism is the same thing as a separated, quasi-compact,
locally of finite type, and universally closed morphism.
Thus this lemma follows from Lemmas \ref{lemma-uniqueness-and-diagonal} and
