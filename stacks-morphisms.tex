...
where $p$ is a prime number. Let $X = [U/\mathbf{G}_m]$.
This is an algebraic stack. There is a stratification of $X$ by strata
\begin{enumerate}
\item $X_0$ is where $x_0$ is not zero,
\item $X_1$ is where $x_0$ is zero but $x_1$ is not zero,
\item $X_2$ is where $x_0, x_1$ are zero, but $x_2$ is not zero,
\item and so on, and
\item $X_{\infty}$ is where all the $x_i$ are zero.
\end{enumerate}
Each stratum is a gerbe over a scheme with group $\mu_{p^i}$ for $X_i$
and $\mathbf{G}_m$ for $X_{\infty}$. The strata are reduced locally
closed substacks. There is no coarser stratification with the same properties.
\end{example}
