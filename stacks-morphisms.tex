...
Let $f : \mathcal{X} \to \mathcal{Y}$ be a morphism of algebraic stacks.
\begin{enumerate}
\item We say $f$ is {\it distilled}\footnote{Temporary nomenclature.
We will use it until we come up with a better word.} if $\Delta_f$ is
...
\begin{enumerate}
\item We say $\mathcal{X}$ is
{\it distilled over $S$}\footnotemark[\value{footnote}]
if the structure morphism $\mathcal{X} \to S$ is distilled.
...
\item We say $\mathcal{X}$ is {\it distilled}\footnotemark[\value{footnote}]
if $\mathcal{X}$, viewed as an algebraic stack over $\text{Spec}(\mathbf{Z})$,
is distilled over $\text{Spec}(\mathbf{Z})$.
...
$i$ is separated,
\item if $\mathcal{Z} \to \mathcal{T}$ is distilled,
...
\begin{enumerate}
\item $i$ is representable by algebraic spaces and locally of finite type,
\item if $\mathcal{Y} \to \mathcal{T}$ is distilled, then $i$ is locally
quasi-finite,
...
\item if $f$ is distilled, then $i$ is locally quasi-finite,
...
\begin{enumerate}
\item If $\mathcal{Y}$ is distilled over $S$ and $f$ is distilled,
then $\mathcal{X}$ is distilled over $S$.
...
\item If $\mathcal{Y}$ is distilled and $f$ is distilled,
then $\mathcal{X}$ is distilled.
...
$g : \mathcal{Y} \to \mathcal{Z}$ be morphisms of algebraic stacks.
\begin{enumerate}
\item If $g \circ f$ is distilled then so is $f$.
...
Proof of (1). Assume $g \circ f$ is distilled. Consider the factorization
$$
\mathcal{X} \to
\mathcal{X} \times_\mathcal{Y} \mathcal{X} \to
\mathcal{X} \times_\mathcal{Z} \mathcal{X}
...
that $g \circ f$ is distilled. Hence (1) follows on applying
Morphisms of Spaces,
...
$\mathcal{X}$ is distilled $\Leftrightarrow$
$\mathcal{X}$ is distilled over $S$.
...
over the base scheme $S$.
\begin{enumerate}
\item[(4)] If $\mathcal{X}$ is distilled over $S$, then $f$ is distilled.
...
$\Delta_{\mathcal{Y}/S}$ is separated, then $f$ is quasi-separated.
\end{enumerate}
\end{lemma}
...
are quasi-compact.
\item $f$ is distilled if and only if $\Delta_{f, 1}$ and $\Delta_{f, 2}$
are locally quasi-finite.
\end{enumerate}
\end{lemma}

\begin{proof}
Assume that $\Delta_{f, 2}$ and $\Delta_{f, 1}$ are universally closed.
Then $\Delta_{f, 1}$ is separated and universally closed by
...
Lemma \ref{lemma-first-diagonal-separated-second-diagonal-closed}.
Part (3) follows from the fact that $\Delta_{f, 2}$ is always locally
quasi-finite by
Lemma \ref{lemma-properties-diagonal-representable}
