...
$\textit{Isom}_{\mathcal{X}}(x, x)$ over $X$. We omit the verification.
...
a surjective \'etale morphism $\text{Spec}(k) \to \mathcal{Z}$.
\end{lemma}
...
morphism $\text{Spec}(k) \to \mathcal{Z}$. Set $U = \text{Spec}(k)$ and
...
In fact $\overline{U} = \text{Spec}(k_0)$ where $k_0 \subset k$ is the
...
\text{Spec}(k_0) = \overline{U} = U/P \to [U/R] = \mathcal{Z}
\end{equation}
...
$F \to \text{Spec}(k)$ is unramified at $z$. Then, after replacing $U$ by
...
commutes with arbitrary base change by a morphism which is representable
by algebraic spaces. Hence it suffices to show that the morphism
$F \to \text{Spec}(k)$ is \'etale at $z$. Since it is flat and locally
of finite presentation as a base change of $U \to \mathcal{X}$ and since
$F \to \text{Spec}(k)$ is unramified at $z$ by assumption, this follows
from
Morphisms of Spaces,
...
$z : \text{Spec}(k) \to \mathcal{Z}_x$.
In particular the composition $x : \text{Spec}(k) \to \mathcal{X}$
...
\xymatrix{
U \ar[d] & F  \ar[l] \ar[d] \\
\mathcal{X} & \text{Spec}(k) \ar[l]_-x
}
$$
in other words $F = U \times_{\mathcal{X}, x} \text{Spec}(k)$. By
...
$$
\text{Spec}(k) \times_{z, \mathcal{Z}_x, z} \text{Spec}(k)
=
\text{Spec}(k) \times_{x, \mathcal{X}, x} \text{Spec}(k)
$$
with \'etale projection maps to $\text{Spec}(k)$ by construction of $z$.
Since
$$
F \times_U F =
(\text{Spec}(k) \times_\mathcal{X} \text{Spec}(k))
\times_{\text{Spec}(k)} F
$$
...
$$
Then $V \to \text{Spec}(k)$ is a smooth morphism of schemes and $V \to U$ is an
...
By construction
$$
V(f_1, \ldots, f_d) \times_\mathcal{X} \text{Spec}(k)
\longleftarrow
...
$$
a finite separable extension of $k$. It follows that $T \to \text{Spec}(k)$
is unramified at $v$, see
Morphisms, Lemma \ref{morphisms-lemma-unramified-at-point}.
By definition of an unramified morphism of algebraic spaces this means that
$V(f_1, \ldots, f_d) \times_\mathcal{X} \text{Spec}(k) \to \text{Spec}(k)$
...
finite type, and for every morphism $\text{Spec}(k) \to \mathcal{Y}$
where $k$ is a field the space $|\mathcal{X}_k|$ is discrete.
\end{definition}

\noindent
Here $\mathcal{X}_k = \text{Spec}(k) \times_\mathcal{Y} \mathcal{X}$
is the ``fibre'' of $f$ over $k$. This definition is motivated by the
observation that a morphism of schemes is locally quasi-finite if and
only if it is locally of finite type with discrete fibres.

\medskip\noindent
The condition that $f$ be quasi-DM is natural. For example, let $k$ be
a field and consider the morphism
$\pi : [\text{Spec}(k)/\mathbf{G}_m] \to \text{Spec}(k)$
...
Also, note that the section $\text{Spec}(k) \to [\text{Spec}(k)/\mathbf{G}_m]$
does not have discrete fibres, hence is not quasi-finite, and we'd
...
$f$ is locally quasi-finite (insert future reference here).

\medskip\noindent
Let us briefly discuss the condition on the fibres. Assume $f$ is quasi-DM and
locally of finite type. Suppose that $Y$ is a scheme
and $Y \to \mathcal{Y}$ is a morphism. Then
$\mathcal{X}_Y = Y \times_\mathcal{Y} \mathcal{X}$ is a
quasi-DM algebraic stack. Hence we can find a scheme $X$ and a
surjective, flat, locally quasi-finite, locally finitely presented
morphism $X \to \mathcal{X}_Y$, see
Theorem \ref{theorem-quasi-DM}.
We will show later (insert future reference here) that the condition on
fibres implies the morphism of schemes $X \to Y$ is locally quasi-finite.
...
stacks. If the base change $f' : \mathcal{Z} \times_\mathcal{Y} \mathcal{X}
\to \mathcal{Z}$ of $f$ is flat, then $f$ is flat.
\end{lemma}
...
Since the base change of $f'$ by $W \to \mathcal{Z}$ is
a flat morphism
...
Let $f : \mathcal{X} \to \mathcal{Y}$ be a morphism of algebraic stacks.
Let $\mathcal{Y}' \to \mathcal{Y}$ be a surjective, flat, locally finitely
presented morphism of algebraic stacks. If the base change
$f' : \mathcal{Y}' \times_\mathcal{Y} \mathcal{X} \to \mathcal{Y}'$ of
$f$ is locally of finite presentation, then $f$ is locally of finite
presentation.
\end{lemma}
...
x_{can} : [U/G] \longrightarrow \mathcal{X}
$$
of stacks in groupoids over $(\textit{Sch}/S)_{fppf}$. To see that it is
...
Then $\mathcal{I}_Z \times_\mathcal{Z} \text{Spec}(k) \to \text{Spec}(k)$
...
locally of finite presentation by
Lemma \ref{lemma-gerbe-fppf}.
Hence $\text{Spec}(k) \to Z$ is surjective, flat, and locally of finite
...
$x, x'$ are objects of $\mathcal{X}$ over $\text{Spec}(k)$ whose images
...
Let $k$ be a field.
Take $U = \text{Spec}(k[x_0, x_1, x_2, \ldots])$
...
$\mathcal{I}_\mathcal{X} \to \mathcal{X}$ is quasi-compact.
Then every $x \in |X|$ is a point of a locally closed substack
$\mathcal{Z} \subset \mathcal{X}$ which is a gerbe.
\end{lemma}

\begin{proof}
By transfinite induction we construct for every ordinal $\alpha$ a
closed subset $T_\alpha \subset |\mathcal{X}|$. For $\alpha = 0$
we set $T_0 = |\mathcal{X}|$. Given $T_\alpha$ let $\mathcal{X}_\alpha$
be the reduced closed substack of $\mathcal{X}$ with
$|\mathcal{X}_\alpha| = T_\alpha$, see
Properties of Stacks, Definition
\ref{stacks-properties-definition-reduced-induced-stack}.
Note that
$$
\mathcal{I}_{\mathcal{X}_\alpha} =
\mathcal{I}_\mathcal{X} \times_\mathcal{X} \mathcal{X}_\alpha
$$
by
Lemma \ref{lemma-monomorphism-cartesian-square-inertia}.
Hence $\mathcal{I}_{\mathcal{X}_\alpha} \to \mathcal{X}_\alpha$ is
quasi-compact as a base change, see
Lemma \ref{lemma-base-change-quasi-compact}.
Thus by
Proposition \ref{proposition-open-stratum}
there exists a maximal (see proof proposition) dense open substack
$\mathcal{U}_\alpha \subset \mathcal{X}_\alpha$ such that
$\mathcal{U}_\alpha$ is a gerbe. Set
$$
T_{\alpha + 1} = T_\alpha \setminus |\mathcal{U}_\alpha|.
$$
...
large enough. This will prove the lemma because, if so, then
for any $x \in |\mathcal{X}|$ there exists a smallest ordinal
$\beta$ with $x \not \in T_\beta$ (because the ordinals are
well-ordered). In this case $\beta$ has to be a successor
ordinal by the definition of $T_\beta$ for limit ordinals. Hence
$\beta = \alpha + 1$ and $x \in |\mathcal{U}_\alpha|$ and
we win.

\medskip\noindent
Finally, to prove the claim, assume that $T_\alpha \not = \emptyset$
...
there exists a locally closed substack $\mathcal{X}' \subset \mathcal{X}$
which is a gerbe and such that $x \in |\mathcal{X}'|$.
Hence a residual gerbe $\mathcal{Z}_x \subset \mathcal{X}'$
of $\mathcal{X}'$ at $x$ exists by
