...
\end{enumerate}
Given any quasi-compact open $V \subset S = \text{lim}_i\ S_i$
there exists an $i \in I$ and a quasi-compact open $V_i \subset S_i$
such that $f_i^{-1}(V_i) = V$.
Moreover, given a second such pair $(i', V_{i'} \subset S_{i'})$
...
$(i, V_i)$ and $(i', V_{i'})$ be two solutions to the problem
posed in the lemma. It suffices to show that
$f_{i''i}^{-1}(V_i \cap U_{j, i''})$ and
...
in $R$. It is easy to see that this implies that
the ideals $\sum g_jR_{i''}$ and $\sum h_jR_{i''}$ have the same radical
in $R_{i''}$ for some $i'' \geq i, i'$.

\medskip\noindent
We prove existence.
By the uniqueness and
our decompositions $S_i = U_{1, i} \cup \ldots \cup S_{m, i}$
imply that it is enough to prove existence in the affine case.
...
Choose $i_0 \in I$. Note that $I$ is nonempty as the limit is directed.
For convenience we write $S_0 = S_{i_0}$ and $i_0 = 0$.
In the proof of Lemma \ref{} we constructed
...
$D(a_{s,j}) \setminus U_j$ of $D(a_{s, j})$.
These form an inverse system over $\{j \in I\mid j \geq i\}$
whose limit is the reduced closed subscheme
$D(a_s) \setminus U_j = \emptyset$. Hence by Lemma \ref{} above
we conclude that $D(a_{s, j}) \subset U_j$ for some 
$j \geq i$. Thus for such an index $j$ we see that
$D(a_{s, j}) \subset S_j$ is an affine open
(see Properties, Lemma \ref{}) containing the image $f_j(s)$.
...
As before we see that $\text{lim}_{i' \geq i} S_{i'} \setminus W_{i'}$
is empty and hence that for a suitable $i' \geq i$ we
...
sheaf on $S_i$ and hence that $S_i$ is quasi-affine
(insert future reference here). Hence we win.
\end{proof}
...
$S = \text{lim}_i\ \overline{S_i}$. Write $Z_i = \overline{S_i} \setminus S_i$
as reduced closed subschemes of $\overline{S_i}$.
Then $\text{lim}_i\ Z_i = \emptyset$. Hence by Lemma \ref{} above
we conclude that $Z_i = \emptyset$ for all large enough $i$ which
proves the lemma.
\end{proof}
...
the $f_i$ and the $y_{ij}$. Details omitted.
\end{proof}
...
that $W_i$ is quasi-affine for all $i$, see Lemma \ref{}.
...
$s(g_j) \in R$ come from $g_{j, i} \in R_i$.
Moreover, by Lemma \ref{} we can assume the corresponding
opens $D(g_{j, i}) \subset W_i$ are affine and cover $W_i$.
...
we can write $B_i$ as a filtered limit of subalgebras $A_{i, l} \subset B_i$
...
$V_i$ and $\text{Spec}(A_{i, l})$ along $W_i$.
It is easy to show that $S$ is the directed limit
of the schemes $S_{i, l}$.
\end{proof}
