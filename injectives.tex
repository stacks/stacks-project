...
and we have consistently tried to mimic Grothendieck's approach
(see proof of
...
$\varphi^n = \varphi$, $N^{n + 1} = \Im(U \to M^n \to M^{n + 1})$
...
$\Im(N^{m - 1} \to N^m) = \Ker(N^m \to N^{m + 1})$ for
all $m \geq k + 1$. Then we can choose
$$
N^{k - 1} \longrightarrow \Ker(N^k \to N^{k + 1}) \times_{M^k} M^{k - 1}
$$
surjecting onto $\Ker(N^k \to N^{k + 1})$ as in
...
\ref{derived-lemma-product-derived-Ab4-star}
