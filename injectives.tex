...
\section{The Gabriel-Popesco theorem}
\label{section-gabriel-popesco}
...
\begin{lemma}
\label{lemma-gabriel-popesco-left-adjoint}
...
products to products. Hence $i$ commutes with limits. To check the set
...
in the proof of Lemma \ref{lemma-gabriel-popesco-left-adjoint}.
...
\begin{lemma}
\label{lemma-gabriel-popesco}
\begin{reference}
\cite[Corollary 4.1]{serpe}
\end{reference}
Let $\mathcal{A}$ be a Grothendieck abelian category. Let
$R$, $F$, $G$ be as in the Gabriel-Popesco theorem
(Theorem \ref{theorem-gabriel-popesco}). Then we obtain
derived functors
$$
...
Theorems \ref{theorem-gabriel-popesco} and
...
\begin{proof}
Let $R, F, G, RG$ be as in Lemma \ref{lemma-gabriel-popesco}
