...
\colim_{\beta \in \alpha} \text{Mor}_{\mathcal{C}}(A, B_\beta)
\longrightarrow
\text{Mor}_{\mathcal{C}}(A, \colim_{\beta \in \alpha} B_\beta).
\end{equation}
...
$\colim \text{Mor}(A, B_n) \to \text{Mor}(A, \colim B_n)$
is not surjective.
\end{example}
...
So it follows that $\text{Mor}(A, \colim B_n)$ is a one-element set
for every set $A$.
However, $\colim \text{Mor}(A , B_n)$ is {\bf not} a one-element set. 
...
$\colim \text{Mor}(A, B_n)$, but they induce the same map
$A \to \colim B_n$.
\end{example} 
...
Let $f: A \to \colim B_\beta$ be a map.
...
\colim_{\beta < \alpha} \text{Hom}_R(\mathfrak a, \mathbf{M}_{\beta}(N)).
$$
...
We have
$$
\text{Hom}_{\mathcal{O}_{X, x}}(\mathcal{F}_x, I_x)
=
\text{Hom}_{\mathcal{O}_X}(\mathcal{F}, i_{x, *}I_x).
$$
see Sheaves, Lemma \ref{sheaves-lemma-stalk-skyscraper-adjoint}. Hence it is
