...
\section{Elements annihilated by an ideal}
\label{section-annihilated}
...
$$
which sends $M$ to the submodule of $I$-torsion. This functor is left exact
and has a derived extension $R\Hom(B, -) : D(A) \to D(B)$. Note that for
every $K \in D(A)$ there is a canonical map $i_*R\Hom(B, K) \to K$ where
$i_* : D(B) \to D(A)$ is the obvious functor.

\begin{lemma}
\label{lemma-exact-support-adjoint}
...
With notation as above, assume $A$ is Noetherian. Then $R\Hom(B, -)$ maps
...
This follows for example from Lemma \ref{lemma-exact-support-ext}
by computing the ext modules $\text{Ext}^i_A(B, K)$ by
resolving $B$ by finite free $A$-modules. Details omitted.
\end{proof}
...
morphism (for example if $X$ is locally Noetherian).
Then
\begin{enumerate}
...
In this situation above. Assume the scheme $X$ is locally Noetherian.
...
The corresponding algebra result is
...
equality holds by Lemma \ref{lemma-exact-support-adjoint}
