...
\begin{lemma}
\label{lemma-compose}
Let $R$ be a ring. Given complexes $K, L, M$ of $R$-modules there
...
$$
\alpha = (\alpha^{p, q}) \in
\prod\nolimits_{p + q = n} \Hom_A(K^{-q}, \Hom^p(L^\bullet, M^\bullet))
$$
Each $\alpha^{p, q}$ is an element
$$
\alpha^{p, q} = (\alpha^{r, s, q}) \in
\prod\nolimits_{r + s + q = n} \Hom_A(K^{-q}, \Hom_A(L^{-s}, M^r))
$$
If we make the identifications
\begin{equation}
\label{equation-identification}
\Hom_A(K^{-q}, \Hom_A(L^{-s}, M^r)) = \Hom_A(K^{-q} \otimes_A L^{-s}, M^r)
\end{equation}
...
Let $R$ be a ring. Let $K, L, M$ be objects of $D(R)$. With the construction
as above there is a canonical isomorphism (\ref{equation-internal-hom})
functorial in $K, L, M$.
\end{lemma}
...
$$
by Lemma \ref{lemma-compose}. Hence the statement follows by taking
$H^0$ and using that $\text{Tot}(K^\bullet \otimes_R L^\bullet)$
represents the derived tensor product and that we may compute
homomorphisms in the derived category by homotopy classes
of maps into $I^\bullet$.
\end{proof}
...
Let $A$ be a Noetherian ring. A {\it dualizing complex} is an
object $\omega_A^\bullet$ of $D(A)$ such that
\begin{enumerate}
...
$$
K \longrightarrow R\Hom_A(K, \omega_A^\bullet)
$$
is an anti-equivalence $D_{\textit{Coh}}(A) \to D_{\textit{Coh}}(A)$.
\end{lemma}

\begin{proof}
Choose a finite complex $I^\bullet$ of injective $A$-modules representing
$\omega_A^\bullet$. 
