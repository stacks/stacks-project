...
Cohomology on Schemes, Section \ref{coherent-section-theorem-formal-functions}.
Moreover, we will discuss a generalization to this theorem in
Pro-\'Etale Cohomolgy, Section \ref{proetale-section-formal-functions}.
...
Let $X$ be a scheme. Let $(K_n)$ be an inverse system
of $D_\QCoh(\mathcal{O}_X)$ such that the maps $H^q(K_{n + 1}) \to H^q(K_n)$
...
Namely, let $\mathcal{B}$ be the set of affine opens of $X_{Zar}$.
The required vanishing follows from
Cohomology of Schemes, Lemma
\ref{coherent-lemma-quasi-coherent-affine-cohomology-zero}
and the vanishing of $R^1\lim$ because the transition maps
...
The third by Cohomology on Sites, Lemma
...
Let $(A, \mathfrak m)$ is a Noetherian local ring and $f \in \mathfrak m$.
...
$$
is a faithful functor between the category of schemes
finite \'etale over $U$ and the category of schemes finite \'etale
over $U_0$.
\end{lemma}

\begin{proof}
Let $a, b : V \to W$ be two morphisms of schemes finite \'etale over $U$
whose restriction to $U_0$ are the same. The assumption
...
$A$ is $f$-adically complete. Then
$$
V \longmapsto V_0 = V \times_U U_0
$$
is a fully faithful functor between the category of schemes
finite \'etale over $U$ and the category of schemes finite \'etale
over $U_0$.
\end{lemma}

\begin{proof}
The functor is faithful by Lemma \ref{lemma-faithful} (to see the
assumption of that lemma holds, apply
...
In Situation \ref{situation-local-lefschetz}. Let $V$ be finite \'etale
...
Assume $Y$ as in (1) exists. Then we obtain an
$\mathcal{O}_X$-module map
$$
(Y \to X)_*\mathcal{O}_Y \longrightarrow j_*\pi_*\mathcal{O}_V
$$
We claim
Divisors, Lemma \ref{divisors-lemma-check-isomorphism-via-depth-and-ass}
applies to this map. If so, then $B = \Gamma(Y, \mathcal{O}_Y)$
and we see that (2) holds. Let $x \in X$. If $x \in U$, then the map is an
...
In Situation \ref{situation-local-lefschetz}. Let $V$ be finite \'etale
over $U$. Let $A^\wedge$ be the completion of $A$, let $X' = \Spec(A^\wedge)$
and let $U'$ and $V'$ be the base changes of $U$ and $V$ to $X'$.
The following are equivalent
\begin{enumerate}
...
of a solution $Y \to X$. Let $Y' \to X'$ be as in (2). The implication
in the opposite direction is a straightforward application of
More on Algebra, Proposition \ref{more-algebra-proposition-equivalence}.
...
to obtain the algebra structure)
we find an $A$-algebra $B$ such that
$\text{Can}(B)$ is isomorphic to
$(B', B_i, \alpha_i, \alpha_{ij})$.
Setting $Y = \Spec(B)$ we see that $Y \to X$ is a morphism
which comes equipped with compatible isomorphisms
$V \cong Y \times_X U$ and $Y' = Y \times_X X'$.
Finally, by descent we see that $Y \to X$ is finite \'etale
...
\begin{proof}
We can reduce to the complete case using
Lemma \ref{lemma-purity-and-completion}
but in this case an easier option is available.
Namely, by Lemma \ref{lemma-reformulate-purity} we have to show that
$B = \Gamma(V, \mathcal{O}_V)$ is a finite \'etale algebra over $A$.
Let $A^\wedge$ be the completion of $A$ with respect to $f$.
Recall that $A \to A^\wedge$ is faithfully flat
(Algebra, Lemma \ref{algebra-lemma-completion-faithfully-flat}).
Then $B \otimes_A A^\wedge$ is equal to the global sections
of the base change of $V$ to $\Spec(A^\wedge)$, see
Cohomology of Schemes, Lemma \ref{coherent-lemma-flat-base-change-cohomology}.
Hence we may assume $A$ is $f$-adically complete by descent
(Algebra, Lemmas \ref{algebra-lemma-descend-properties-modules} and
\ref{algebra-lemma-etale}).
...
In Situation \ref{situation-local-lefschetz}. Assume the punctured spectrum
of the strict henselization of $A$ is connected. Then the functor
$$
Y \longmapsto Y \times_X U
$$
is a fully faithful functor between the category of schemes
finite \'etale over $X$ and the category of schemes finite \'etale
over $U$.
\end{lemma}
...
\begin{proof}
Let $U$ be the puctured spectrum of $A$.
...
\item $A[1/f]$ satisfies $(S_2)$,
\item the formal fibres of $A$ are $(S_2)$,
\item $H^1_\mathfrak m(A/fA)$ is a finite $A$-module,
\item $\dim(A/\mathfrak p) \geq 3$ for every minimal prime
$\mathfrak p \subset A$,
\item $V_0 = V \times_U U_0$ is equal to $Y_0 \times_{X_0} U_0$
for some $Y_0 \to X_0$ finite \'etale.
\end{enumerate}
...
\begin{proof}
Let $A^\wedge$ be the completion of $A$.
Assumptions (3) and (4) serve to guarantee that $A^\wedge[1/f]$
is $(S_2)$, see Algebra, Lemma \ref{algebra-lemma-Sk-goes-up}.
Conditions (2) and (6) imply (6) for $A^\wedge$ via
Proposition \ref{proposition-ratliff} (note that minimal
primes of $A^\wedge$ lie over minimal primes of $A$ because
$A \to A^\wedge$ is flat). Property (5) is preserved by
passing from $A$ to $A^\wedge$ as follows from
Lemma \ref{lemma-torsion-change-rings}. Thus we reduce to the complete
case using Lemma \ref{lemma-purity-and-completion}.

\medskip\noindent
Assume $A$ is complete. In this case there exists a finite \'etale
morphism $Y \to X$ lifting $Y_0 \to X_0$ by
More on Algebra, Lemma \ref{more-algebra-lemma-finite-etale-equivalence}
(also uses
More on Algebra, Lemma \ref{more-algebra-lemma-complete-henselian}).
By Lemmas \ref{lemma-fully-faithful-general} and \ref{lemma-faithful-general}
there exists an isomorphism $V \times_U U' \to Y \times_X U'$ for some
$U' \subset U$ open containing $U_0$ lifting the given isomorphism over $U_0$.
To finish the proof we will show that this isomorphism extends to an
isomorphism $V \to Y \times_X U$.

\medskip\noindent
Namely, let $\mathfrak p \subset A$ correspond to a point of $U$ but not
a point of $U'$. Then $V(\mathfrak p) \cap V(f) = \{\mathfrak m\}$.
Hence $\dim(A/\mathfrak p) = 1$ by
Algebra, Lemma \ref{algebra-lemma-one-equation}.
Hence $\mathfrak p$ corresponds to a closed point of $U$.
In particular $U \setminus U'$ is finite.
If we can extend our isomorphism to a morphism over
$U \cup \{\mathfrak p\}$, then we win by induction on
the cardinality of $U \setminus U'$.
To extend over $U \cup \{\mathfrak p\}$
it suffices to extend the morphism over $\Spec(A_\mathfrak p)$.
This is true because the category of schemes of finite presentation
over $A_\mathfrak p$ is the limit of the category of schemes of
finite presentation over $A_g$, $g \in A \setminus \mathfrak p$ by
Limits, Lemma \ref{limits-lemma-descend-finite-presentation}.
Note that both $V \times_U \Spec(A_\mathfrak p)$
and $Y \times_U \Spec(A_\mathfrak p)$ are finite \'etale
over $A_\mathfrak p$. Hence we will obtain our extension if
Lemma \ref{lemma-same-etale-extensions}
applies to the local ring $A_\mathfrak p$.
Below we will show that $\text{depth}(A_\mathfrak p) \geq 2$.
This will show that $A_\mathfrak p^{sh}$ has depth $\geq 2$
(More on Algebra, Lemma \ref{more-algebra-lemma-henselization-depth})
and hence the assumption of the lemma above by
Lemma \ref{lemma-depth-2-connected-punctured-spectrum}.

\medskip\noindent
Recall that $\dim(A/\mathfrak p) = 1$. Hence condition (6) and the
fact that $A$ is catenary give $\dim(A_\mathfrak p) \geq 2$.
Since $A_\mathfrak p$ is a localization of $A[1/f]$ we find the
desired depth condition holds by (3).
\end{proof}
