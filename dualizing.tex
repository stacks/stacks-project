...
Then the assumption and hence the conclusion of
...
Let $A \to B$ be a ring map. Let $X$ be a quasi-compact and
quasi-separated scheme over $A$.
Let $h : X' = X \times_{\Spec(A)} \Spec(B) \to X$ be the projection.
We have
$$
...
\begin{enumerate}
\item $M \in D(\mathcal{O}_X)$ is perfect, $K \in D_\QCoh(X)$, and $X$
and $\Spec(B)$ are Tor independent over $\Spec(A)$, or
\item $M \in D(\mathcal{O}_X)$ is pseudo-coherent,
$K \in D_\QCoh^+(X)$, and $A \to B$ is flat.
\end{enumerate}
\end{lemma}
...
R\SheafHom(M, K) \otimes_{\mathcal{O}_X}^\mathbf{L} h_*\mathcal{O}_{X'}
$$
in both cases (details omitted; hint: use
Derived Categories of Schemes, Lemma
\ref{perfect-lemma-quasi-coherence-internal-hom}).
Hence, by replacing
