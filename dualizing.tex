...
$\mathfrak{D}_f \subset \mathcal{O}_X$.
\end{definition}
...
If the Dedekind different of $A \to B$ is defined, then
$\mathfrak{D}_{Y/X}|_U = \widetilde{\mathfrak{D}_{A/B}}$.
\end{lemma}
...
\begin{enumerate}
\item $f$ is flat over an open $j : V \to Y$,
\item the canonical map $\omega_{Y/X} \to j_*\omega_{V/X}$ is injective, and
\item the trace element $\tau_{V/X}$ comes from a section
$\tau \in \Gamma(Y, \omega_{Y/X})$.
\end{enumerate}
Then we can define the different $\mathfrak{D}_f$ as the annihilator of
...
\begin{proof}
Omitted. Hint: carefully unwind all the definitions and use
Lemmas \ref{lemma-dedekind-complementary-module} and
\ref{lemma-dedekind-different-ideal}.
\end{proof}
