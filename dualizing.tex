...
Let $X$ be a Nagata scheme. Let $j : U \to X$ be the inclusion of an
open subscheme with complement $Z$. Let $\mathcal{F}$ be a coherent
$\mathcal{O}_U$-module. If
$$
\text{codim}(\overline{\{x\}} \cap Z, \overline{\{x\}}) \geq 2
$$
for every $x \in \text{Ass}(\mathcal{F})$, then $j_*\mathcal{F}$
is coherent.
\end{lemma}
...
\medskip\noindent
Let $Y = \overline{\{x\}} \cap U$ with its reduced induced closed subscheme
structure. Choose a nonzero map
$\varphi_x : \mathcal{F}_x \to \kappa(x) = \mathcal{O}_{Y, x}$.
By Coherent, Lemma \ref{coherent-lemma-map-stalks-local-map}
there is an open $x \in V \subset U$ and a map
$\varphi_V : \mathcal{F}|_V \to \mathcal{O}_Y|_V$ whose stalk
...
$f^n\mathcal{F} \to \mathcal{O}_Y$ for some $n$.
Precomposing with multiplication by $f^n$ we obtain a map
$\mathcal{F} \to \mathcal{O}_Y$ whose stalk at $x$ is nonzero.
...
By induction hypothesis $j_*\mathcal{F}'$ is coherent.
If we can show that $j_*\mathcal{O}_Y$ is coherent,
then we consider
$$
0 \to j_*\mathcal{F}' \to j_*\mathcal{F} \to j_*\mathcal{O}_Y
$$
By Schemes, Lemma \ref{schemes-lemma-push-forward-quasi-coherent}
the sheaf $j_*\mathcal{F}$ is quasi-coherent.
Hence the image of $j_*\mathcal{F}$ in $j_*\mathcal{O}_Y$
...
\medskip\noindent
It remains to prove that $j_*\mathcal{O}_Y$ is coherent.
Let $Y \to Y'$ be the normalization of $X$ in $Y$. Since $X$ is Nagata,
the morphism $\pi : Y' \to X$ is finite, see
Morphisms, Lemma \ref{morphisms-lemma-nagata-normalization-finite}.
Hence $\pi_*\mathcal{O}_{Y'}$ is coherent, see
Cohomology of Schemes, Lemma \ref{coherent-lemma-finite-pushforward-coherent}.
Finally, the map
$$
\pi_*\mathcal{O}_{Y'} \longrightarrow j_*\mathcal{O}_Y
$$
is an isomorphism by Lemma \ref{}.
\end{proof}
