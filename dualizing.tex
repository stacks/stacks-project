...
If $M \to I$ is an embedding of $M$ into an injective $R$-module,
then there is an isomorphism $I \cong E \oplus I'$ compatible with
the embeddings of $M$.
The injective hull $E$ of $M$ is unique up to isomorphism.
\end{lemma}

\begin{proof}
Assume $M \to E$ and $M \to I$ as the statement of the lemma.
Choose a map $\alpha : E \to I$ extending the map $M \to I$.
Choose a map $\beta: I \to E$ extending the map $M \to E$.
The composition $\beta \circ \alpha : E \to E$ induces the indentity
on $M$, hence has trivial kernel as $M \subset E$ is an essential
extension. Thus the image $E'$ of $\beta \circ \alpha$ is an injective
submodule of $E$ containing $M$. It follows that $E = E' \oplus E''$
with $M \subset E'$. Since $E$ is an essential extenion we conclude
$E'' = 0$. Thus $\alpha : E \to I$ has a left inverse as desired.

\medskip\noindent
The uniqueness statement follows immediately from this. Or it
can be proven directly by similar arguments.
\end{proof}
...
Note that for an $R_\mathfrak p$-module $N$ we have
$\Hom_{R_\mathfrak p}(N, E) = \Hom_R(N, E)$ hence it is clear
that $E$ is injective as an $R_\mathfrak p$-module.
\end{proof}

\begin{lemma}
\label{lemma-injective-hull-indecomposable}
Let $R$ be a ring. Let $\mathfrak p \subset R$ be a prime.
The injective hull of $R/\mathfrak p$ is equal to the injective
hull of $\kappa(\mathfrak p)$ and is indecomposable.
\end{lemma}

\begin{proof}
Say $R/\mathfrak p \subset E$ is an injective hull. As
$R/\mathfrak p \subset \kappa(\mathfrak p)$ we can extend the embedding
to a map $\kappa(\mathfrak p) \to E$. Hence $E$ is also the injective
hull of $\kappa(\mathfrak p)$. Observe that for
$f \in R$, $f \not \in \mathfrak p$ the map $f : E \to E$ is an isomorphism
(as $f : \kappa(\mathfrak p) \to \kappa(\mathfrak p)$ is an isomorphism
and using the uniqueness of injective hulls). Hence, if $E' \subset E$ is a
nonzero submodule, then $\kappa(\mathfrak p) \cap E' = \kappa(\mathfrak p)$.
In particular, if $E'$ is injective, then $E'$ is the injective
hull of $\kappa(\mathfrak p)$ and $E' = E$.
\end{proof}
