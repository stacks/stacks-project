...
Section \ref{section-trivial}. In particular, when $B = A[x]$
is the polynomial ring we see that
$$
R\Hom(B, A) = \left(\prod\nolimits_{n \geq 0} A\right)[0]
$$
Thus the right adjoint to $Rf_*$ does not preserve finiteness conditions
even when $Y$ is the spectrum of a field and $f$ is of finite type.
\end{example}
...
$$
Observe that if $g$ is flat (or more generally
$g$ and $f$ are Tor independent), then the base change
map is an isomorphism
(Derived Categories of Schemes, Lemma \ref{perfect-lemma-compare-base-change})
and thus the last displayed transformation is an isomorphism as well.
