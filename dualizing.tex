...
\begin{proof}
This follows immediately from Lemma \ref{lemma-sitting-in-degrees}.
...
$\text{Ext}^i_B(B/\mathfrak m_A B, B)$ is $0$ for $i \gg 0$.
...
$\text{Ext}^i_A(\kappa_A, A)$ is nonzero for exactly one $i$,
namely for $i = \dim(A)$, and
$\text{Ext}^{\dim(A)}_A(\kappa_A, A) \cong \kappa_A$
(see Lemma \ref{lemma-normalized-finite}, \ref{lemma-apply-CM}, and
\ref{lemma-gorenstein-CM}). Thus we see that
$\text{Ext}^i_B(B/\mathfrak m_A B, B)$ is zero except for one $i$,
namely $i = \dim(A)$ and
$\text{Ext}^{\dim(A)}_B(B/\mathfrak m_A B, B) \cong B/\mathfrak m_A B$.
...
\section{Gorenstein and syntomic morphisms}
\label{section-gorenstein-lci}

\noindent
This section discusses the fact that a syntomic morphism is Gorenstein 
and it works out what these morphisms look like in the quasi-finite case.
...
Lemma \ref{lemma-flat-quasi-finite-shriek}. Hence
the equivalence follows from
Lemma \ref{lemma-affine-flat-Noetherian-gorenstein}.

\medskip\noindent
Before we give the second proof, we elaborate on condition (4).
As $A \to B$ is flat in all 4 cases, the module
...
Thus we see that condition (4), in the presence of flatness,
is equivalent to condition (4) for the maps
$\kappa(\mathfrak p) \to B \otimes_A \kappa(\mathfrak p)$.

\medskip\noindent
Second proof. By what was said in the previous paragraph,
...
Remarks \ref{remark-CM-morphism-compare-dualizing} and
\ref{remark-collect-results-qf-gorenstein}
...
\begin{proof}
This follows by combining Lemmas \ref{lemma-lci-gorenstein} and
\ref{lemma-gorenstein-quasi-finite}.
\end{proof}
