...
\begin{proof}
This is true because $\Hom_S(N, \Hom_R(S, E)) = \Hom_R(N, E)$.
\end{proof}
...
\noindent
To state the next lemma, denote $\text{Mod}_{fg}(R)$ the category of finite
...
$\text{Mod}_{fg}(R) \to \text{Mod}_{fg}(R)$ and
$D \circ D \cong \text{id}$.
\end{lemma}

\begin{proof}
We have seen that $D \circ D = \text{id}$ on $\text{Mod}_{fg}(R)$
...
we conlude that the kernel of $M \to E^{\oplus n}$ is zero.
\end{proof}
...
Let $(R, \mathfrak m, \kappa)$ be a Noetherian local ring.
\begin{enumerate}
\item An $R$-module satisfies the ascending chain condition
if and only if it is a finite $R$-module.
\item An $R$-module satisfies the descending chain condition
if and only if it is $\mathfrak m$-power torsion and has a
finite dimensional $\mathfrak m$-torsion submodule.
\end{enumerate}
\end{lemma}

\begin{proof}
We omit the proof of (1). Let $M$ be an $R$-module with the descending
chain condition. Let $x \in M$. Then $\mathfrak m^n x$ is a descending
chain of submodules, hence stabilizes. Thus
$\mathfrak m^nx = \mathfrak m^{n + 1}x$ for some $n$. By Nakayama's lemma
(Algebra, Lemma \ref{algebra-lemma-NAK})
this implies $\mathfrak m^n x = 0$, i.e., $x$ is $\mathfrak m$-power torsion.
Since $M[\mathfrak m]$ is a vector space over $\kappa$ it has to be
finite dimensional in order to have the descending chain condition.

\medskip\noindent
Conversely, assume that $M$ is $\mathfrak m$-power torsion and has a
finite dimensional $\mathfrak m$-torsion submodule $M[\mathfrak m]$.
Let $E$ be an injective hull of $\kappa$. By
Lemma \ref{lemma-torsion-submodule-sum-injective-hulls}
we see that $M$ is a submodule of $E^{\oplus n}$ for some $n$.
Hence it suffices to prove the descending chain condition for $E$.
If $E \subset M_1 \subset M_2 \ldots$ is a sequence of submodules,
then
$$
...
nonzero element annihilated by $\mathfrak m$, whence has a nonzero
map into $E$. This finishes the proof.
\end{proof}
...
Of course we have $E = \Hom_R(R, E) = D(R)$.
In Lemma \ref{lemma-describe-categories}
and its proof we have seen that modules with acc
are quotients of $R^{\oplus n}$ and modules with dcc are submodules
of $E^{\oplus n}$. Note that $D$ is exact and conservative, i.e.,
for $M$ in either left of right hand side we have $D(M) = 0$
if and only if $M = 0$. The result then follows formally.
