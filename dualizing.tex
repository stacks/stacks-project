...
\noindent
In this section we discuss a baby case of the finiteness theorem.
Here is a lemma of Faltings which shows that it suffices to prove
...
$$
The final zero because $H^1(X, \mathcal{F}') = 0$ as $X$ is affine
...
Thus, reading the arguments in the first paragraph of the proof backwards,
we have the desired finiteness of
$H^1_{Z^\wedge}(M^\wedge)$ by Lemma \ref{lemma-finiteness-pushforward}
provided assumption (3) of that lemma are satisfied. Let
...
$R\Hom(N, \omega_A^\bullet) = \Hom_A(N, E)$
...
(for general morphisms $f$) with the notation in \cite{RD}.
...
\section{Trace maps for twisted inverse image}
\label{section-trace}
...
Lemma \ref{lemma-twisted-inverse-image}. By Categories, Section
\ref{categories-section-adjoint} we obtain a transformation of functors
$$
...
$$
for $L \in D_\QCoh(\mathcal{O}_X)$ which characterizes twisted inverse
image is given by
$$
...
\section{Twisted inverse image and pullback}
...
twisted inverse image for $f, g, h$. For any $K \in D_\QCoh(\mathcal{O}_Z)$
the diagram
$$
\xymatrix{
...
Let $a$ be the twisted inverse image. Then $a$ commutes with direct sums.
\end{lemma}
...
\begin{lemma}
\label{lemma-flat-comparison-shriek}
Let $S$ be a Noetherian scheme. Let $f : X \to Y$ be a perfect
(e.g., flat) morphism of compactifyable schemes over $S$. Then the map
$$
\mu_{f,  K} :
Lf^*K \otimes_{\mathcal{O}_X}^\mathbf{L} f^!\mathcal{O}_Y
\longrightarrow
...
More on Morphisms, Lemma
\ref{more-morphisms-lemma-flat-finite-presentation-perfect}.)
If $f$ is an open immersion, then this is true because we can take
$\overline{X} = Y$ in the construction of $f^!$ and the map. If $f$
is a perfect proper morphism, then $f^! = a$ and the result is true by
Lemma \ref{lemma-compare-with-pullback-flat-proper-noetherian}.
If $f = g \circ h$ and the result is true for $g$ and $h$, then
the result is true for $f$ by the commutative diagram
of Lemma \ref{lemma-map-pullback-to-shriek-well-defined}.

\medskip\noindent
Let $j : U \to X$ be an open immersion. Then $\mu_j$ being an isomorphism
and $j^! = j^*$ we see that $\mu_{f \circ j}$ is isomorphic to the
restriction of $\mu_f$ to $U$. Hence it suffices to prove the
result in case $X$ is affine. Choose an immersion $X \to \mathbf{A}^n_Y$
...
Then $U \subset \mathbf{P}^n_Y$ is open and the result is true for
$U \to Y$ because it is true for $\mathbf{P}^n_Y \to Y$ (flat and proper)
and for the open immersion $U \to \mathbf{P}^n_Y$. Finally, the
morphism $X \to U$ is a perfect closed immersion by
More on Morphisms, Lemma \ref{more-morphisms-lemma-perfect-permanence}.
Hence the result is true for $X \to U$ and the proof is complete.
\end{proof}

\begin{remark}
\label{remark-relative-dualizing-complex}
Let $f : X \to Y$ be a flat proper morphism of Noetherian schemes
In this situation, $\omega_{X/Y}^\bullet = f^!\mathcal{O}_Y$
is sometimes called the {\it relative dualizing complex}
(for reasons that will become clear later). By
Lemma \ref{lemma-flat-comparison-shriek} there is a functorial isomorphism
$f^!K = Lf^*K \otimes_{\mathcal{O}_X}^\mathbf{L} \omega_{X/Y}^\bullet$
...
$$
Rf_*f^!K = Rf_*(Lf^*K \otimes_{\mathcal{O}_X}^\mathbf{L} \omega_{X/Y}^\bullet)
= K \otimes_{\mathcal{O}_Y}^\mathbf{L} Rf_*\omega_{X/Y}^\bullet
\to K
$$
for every $K \in D_\QCoh^+(\mathcal{O}_Y)$. Moreover, if $g : Y' \to Y$ is a
morphism of Noetherian schemes and $X' = Y' \times_Y X$, then by
Lemma \ref{lemma-more-base-change} we have
...
$$
for the projection is the base change of $\text{Tr}_f$.
\end{remark}
...
if it has finite injective dimension as a module over itself. An example
of a Gorenstein ring is a regular ring.
...
\begin{remark}
\label{remark-gorenstein-formal-fibres}
Let $(A, \mathfrak m, \kappa)$ be a Noetherian local ring.
If $A$ has a dualizing complex, then the formal fibres of $A$
are Gorenstein. To prove this, let $\mathfrak p$ be a prime of $A$.
The formal fibre of $A$ at $\mathfrak p$ is isomorphic to the formal fibre
of $A/\mathfrak p$ at $(0)$ and $A/\mathfrak p$ has a dualizing complex
...
$$
functorially in $\mathcal{F}$. This holds essentially by definition of the
twisted inverse image for the morphism $X \to \Spec(k)$ and our choice of
$\omega_X$; the real miracle of duality comes from the fact that a twisted
inverse image functor exists and that it transforms dualizing complexes
