...
\noindent
The formulation in the following lemma has the advantage that the first
two conditions are inherited by schemes of finite type over $X$. Given
these, the third condition, which is a condition on the coherent sheaf
$\mathcal{F}$ is necessary by Remark \ref{remark-no-finiteness-pushforward}.
...
\begin{proof}
By Proposition \ref{proposition-kollar} it suffices to check that
$A = \mathcal{O}_{\overline{\{x\}}, z}$ satisfies the condition
on associated primes of its completion of the propostion.
...
By condition (3) we have $\dim(A) \geq 2$.
...
Hence $\dim(A^\wedge/\mathfrak p') = \dim(A) \geq 2$ as desired.
\end{proof}
...
Let $(A, \mathfrak m)$ be a complete local ring. Let $f \in \mathfrak m$
be a nonzerodivisor such that for every minimal prime $\mathfrak p$ of $A$
with $f \not \in \mathfrak p$ we have $\dim(A/\mathfrak p) \geq 2$.
...
\begin{proof}
Let $a, b : V \to V'$ be two morphisms of schemes finite \'etale
over $U$ whose restriction to $U_0$ are the same.
The assumption means $V(\mathfrak p)$ meets $U_0$ for
every minimal prime of $A$. This implies that every irreducible
component of $V$ meets $V_0$ (small detail omitted).
Hence $a = b$ by \'Etale Morphisms, Proposition
...
Let $(A, \mathfrak m)$ be a complete local ring. Let $f \in \mathfrak m$
be a nonzerodivisor such that $H^1_\mathfrak m(A/fA) = 0$.
Let $U$, resp.\ $U_0$ be the punctured spectrum of $A$, resp.\ $A/fA$.
Then
$$
V \longmapsto V_0 = V \times_U U_0
...
\begin{proof}
The functor is faithful by Lemma \ref{lemma-faithful}
(the assumption is satisfied by
Lemma \ref{lemma-kollar-finiteness-H1-local}).
Since the category of finite \'etale coverings has an internal
hom (\'Etale Morphisms, Lemma \ref{etale-lemma-internal-hom-finite-etale})
...
\medskip\noindent
The vanishing of $H^1_\mathfrak m(A/fA)$ tells us that
$\Gamma(U_0, \mathcal{O}_{U_0}) = A/fA$ by
Lemma \ref{lemma-local-cohomology-is-local-cohomology} and
Cohomology, Section \ref{cohomology-section-cohomology-support}
As $f$ is a nonzerodivisor we obtain short exact sequences
$$
0 \to A/fA \xrightarrow{f^n} A/f^{n + 1}A \to A/f^n A \to 0
$$
Induction on $n$ shows that $H^1_\mathfrak m(A/f^{n + 1}A) = 0$
for all $n$. Hence the same reasoning shows that
$\Gamma(U_n, \mathcal{O}_{U_n}) = A/f^{n + 1}A$. It follows that
$$
\lim \Gamma(U_n, \mathcal{O}_{U_n}) = \lim A/f^{n + 1}A = A
$$
the final equality because $A$ is complete.
