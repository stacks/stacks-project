...
Let $f : X \to Y$ be a smooth proper morphism of relative dimension $d$.
Let $a$ be the right adjoint of
$Rf_* : D_\QCoh(\mathcal{O}_X) \to D_\QCoh(\mathcal{O}_Y)$ as in
Lemma \ref{lemma-twisted-inverse-image}. Then there is an isomorphism
$$
\wedge^d \Omega_{X/S}[d] \longrightarrow a(\mathcal{O}_Y)
$$
in $D(\mathcal{O}_X)$.
\end{lemma}
...
The first equality by
Lemma \ref{lemma-compare-with-pullback-flat-proper-noetherian}.
The second equality because $p \circ \Delta = \text{id}_X$.
The third equality by Lemma \ref{lemma-regular-immersion}.
...
to some extend. There is an approach to duality theory using
