...
and $Y = V(J)$. Then
$$
R\Gamma_Z(M_A) = R\Gamma_Y(M)
$$
functorially in $M \in D(B)$. Here $M_A \in D(A)$ is the restriction.
\end{lemma}

\begin{proof}
If $I = (f_1, \ldots, f_r)$ then $J$ is generated by the images
$g_1, \ldots, g_r \in B$ of $f_1, \ldots, f_r$. By
Lemma \ref{lemma-local-cohomology-adjoint}
the functor $R\Gamma_Z$ is computed by tensoring over $A$
by $A \to \prod\nolimits_{i_0} A_{f_{i_0}} \to
\prod\nolimits_{i_0 < i_1} A_{f_{i_0}f_{i_1}}
\to \ldots \to A_{f_1\ldots f_r}$
and $R\Gamma_Y$ us computed by tensoring over $B$ by
$B \to \prod\nolimits_{i_0} B_{g_{i_0}} \to
\prod\nolimits_{i_0 < i_1} B_{g_{i_0}g_{i_1}}
\to \ldots \to B_{g_1\ldots g_r}$. Thus the lemma follows
...
\noindent
In the rest of this section we discuss a baby case of the finiteness theorem.
We suggest the reader skip this material
and start reading Section \ref{section-finiteness-II}.
...
Let $A$ be a uninversally catenary Noetherian local ring.
...
\begin{proof}
By Lemma \ref{lemma-check-finiteness-local-cohomology-by-annihilator}
the lemma is translated into a statement on annihilators
of the local cohomology modules. An important role will be played by
the finite $A$-modules
$$
...
following fact: an ideal $J \subset A_\mathfrak p$ annihilates
...
\medskip\noindent
Proof of finiteness for $i < s$.
We will show by descending induction that there exists an
ideal $J$ with $V(J) \subset Z$ such that the associated primes of
$J H^i_Z(M)$ are in $Z_n$. For $n = -1$ this implies the desired
result. The base case is $n = \dim(Z)$.

\medskip\noindent
Thus we assume given $J$ with the property for $n$.
Let $\mathfrak q \in Z_n$. Then we consider the spectral
sequence
$$
H_\mathfrak q^j(H^i_Z(M)_\mathfrak q) \Rightarrow
H^{i + j}_\mathfrak q(M_\mathfrak q)
$$
...
$0 \leq i < s$ and all $\mathfrak q \in Z_n \setminus Z_{n - 1}$.
Then $JJ'$ will work for $n - 1$. FIXME.
...
$$
for $0 \leq i < s$ and all $\mathfrak q \in Z_n \setminus Z_{n - 1}$.
...
to prove that
$$
H^{i - \dim((A/\mathfrak p)_\mathfrak q)}_\mathfrak p(M_\mathfrak p) = 0
$$
for $0 \leq i < s(M)$. This vanishes exactly by
our definition of $s(M)$ and Lemma \ref{lemma-depth}.

\medskip\noindent
To prove the final assertion that $H^s_Z(M)$ is not finite we work
backwards through the arguments above. Namely, from the spectral
sequences displayed above we see that it suffices to prove
one of the groups $H^s_\mathfrak q(M_\mathfrak q)$ is not
annihilated by an ideal $J' \subset A$ with $V(J) \subset Z$
for some $\mathfrak q \in Z$. Next, the arguments show it
is enough to find a $\mathfrak p$ such that
$H^{s + d(\mathfrak p) - n}_\mathfrak p(M_\mathfrak p)$
is not zero for some $\mathfrak p \in X \setminus Z$.
The existence of a such a $\mathfrak p$ then follows
from our definition of $s$ and the nonvanishing in
Lemma \ref{lemma-depth}.
\end{proof}
...
$H^i_Z(M)$ are finite for $i \leq n$. Then the proof
