...
as $B$-modules (the left hand side starts out as a $B$-module
and the right hand side has a $B$-module structure via the action
of $B$ on $B$).
\end{lemma}
...
a bounded complex $F_\bullet$ of $P$-modules (possible as
...
isomorphism $H^0(f^!\mathcal{O}_X) = \omega_{Y/X}$.
...
\section{Differents}
\label{section-differents}

\noindent
There are many different differents available in the literature.
We list some of them here; for more information we suggest
the reader consult \cite{Kunz}.

\medskip\noindent
Let $A \to B$ be a ring map. Let $I = \Ker(B \otimes_A B \to B)$.
The {\it Noether different} is the image of the annihilator of $I$
under the map $B \otimes_A B \to B$.

\medskip\noindent
Let $A \to B$ be a finite type ring map. The {\it K\"ahler different} is the
zeroth fitting ideal of $\Omega_{B/A}$ as a $B$-module.

\medskip\noindent
Let $A \to B$ be a ring map such that $A$ is Noetherian, $A \to B$ is finite,
...
$$
\mathcal{D}_{B/A} = \{x \in L \mid x\mathcal{L}_{B/A} \subset B\}.
$$
...
\item $\text{Trace}_{L/K}(B) \subset A$, and
\item the Dedekind different is an ideal of $B$.
\end{enumerate}
Then we have (1) $\Rightarrow$ (3), (2) $\Rightarrow$ (3), and
(3) $\Rightarrow$ (4).
\end{lemma}

\begin{proof}
Condition (3) signifies that $1 \in \mathcal{L}_{B/A}$ which
clearly implies (4). If $A \to B$ is flat, then we see that
$\text{Trace}_{B/A} : B \to A$ is defined and that
$\text{Trace}_{L/K}$ is the base change. Hence (3) holds.
...
\item the Dedekind different is equal to
$\{b \in B \mid b\omega_{B/A} \subset B\tau_{B/A}\}$.
\end{enumerate}
\end{lemma}
...
Dedekind different in the affine finite flat case by
Lemma \ref{lemma-flat-dedekind-complementary-module-trace}.

\begin{definition}
\label{definition-different}
Let $f : X \to Y$ be a flat quasi-finite morphism of Noetherian schemes.
Let $\omega_{X/Y}$ be the relative dualizing module and let
$\tau_{X/Y} \in \Gamma(X, \omega_{X/Y})$ be the trace element
(Remarks \ref{remark-relative-dualizing-for-quasi-finite} and
\ref{remark-relative-dualizing-for-flat-quasi-finite}).
The annihilator of
$$
\Coker(\mathcal{O}_X \xrightarrow{\tau_{X/Y}} \omega_{X/Y})
$$
is the {\it different} of $X/Y$. It is a coherent ideal
$\mathcal{D}_{X/Y} \subset \mathcal{O}_X$.
\end{definition}
...
\label{lemma-flat-dedekind-complementary-module-trace}
Let $f : Y \to X$ be a flat quasi-finite morphism of Noetherian schemes.
Let $U = \Spec(A) \subset X$, $V = \Spec(B) \subset Y$
be affine open subschemes with $f(V) \subset U$.
If the Dedekind different of $A \to B$ is defined, then
$\mathcal{D}_{Y/X}|_U = \widetilde{\mathcal{D}_{A/B}}$.
\end{lemma}

\begin{proof}
Clear  from the discussion preceding Definition \ref{definition-different}
and especially Lemma \ref{lemma-flat-dedekind-complementary-module-trace}.
\end{proof}

\begin{remark}
\label{remark-different-generalizations}
We can generalize Definition \ref{definition-different}.
Suppose that $f : X \to Y$ is a quasi-finite morphism of Noetherian schemes
with the following properties
\begin{enumerate}
\item $f$ is flat over an open $j : U \to X$,
\item the canonical map $\omega_{X/Y} \to j_*\omega_{U/Y}$ is injective, and
\item the trace element $\tau_{U/Y}$ comes from a section
$\tau \in \Gamma(X, \omega_{X/Y})$.
\end{enumerate}
Then we can define the different $\mathcal{D}_{X/Y}$
as the annihilator of $\omega_{X/Y}/\mathcal{O}_X\tau$.
This generalization incorporates the classical Dedekind different
in the case of a finite extension of normal domains. Howeover, for
non-flat maps between non-normal rings, this generalization no longer
measures ramification of the morphism, see Example \ref{example-no-different}.
\end{remark}
...
$1 \in \mathcal{L}_{B/A}$ if and only if $n = m$. Moreover, an
easy computation shows that if $n = m$, then $\mathcal{L}_{B/A} = B$
and the Dedekind different is $B$ as well. In other words, we find that
the different of Remark \ref{remark-different-generalizations}
