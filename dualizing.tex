...
\end{equation}
in this situation. In this section we will show (among other things)
\begin{enumerate}
\item $\omega_{B/A}$ is independent of the choice of the factorization,
\item formation of $\omega_{B/A}$ commutes with flat base change,
\item if $A \to B$ is flat, there is a canonical trace element
$\tau \in \omega_{B/A}$,
\item use this to define a different ideal $\mathcal{D}_{B/A} \subset B$.
\end{enumerate}
...
(Algebra, Lemma \ref{algebra-lemma-finite-projective}).
Hence given $b \in B$ the {\it trace} $\text{Tr}_{B/A}(b)$
of the $A$-linear map $B \to B$ given by
multiplication by $b$ on $B$ is defined.
...
$\text{Tr}_{B'/A'} = (\text{Tr}_{B'/A'}, \text{Tr}_{B'/A'})$
...
\xrightarrow{ev} A
$$
where the first arrow sends $b \in B$ by the $A$-linear operator
...
to the map $b' \mapsto \lambda(b')b$, is not an isomorphism. However,
the exact sequences above show that it is an isomorphism when
$B$ is flat over $A$ and this produces the usual trace $\text{Tr}_{B/A}$
in that case. In the general case, we consider the diagram
$$
\xymatrix{
