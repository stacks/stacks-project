...
Then the base change maps of Remark \ref{remark-base-change-map}
...
and a compactifyable scheme $X$ over $S$ we define
$$
\omega_X^\bullet = p^!\omega_S^\bullet
$$
where $p : X \to S$ is the structure morphism.
By Lemma \ref{lemma-shriek-dualizing} $\omega_X^\bullet$
is a dualizing complex for $X$. If $f : X \to Y$ is a morphism of
compactifyable schemes over $S$, then we have
\begin{enumerate}
\item $\omega_X^\bullet = f^!\omega_Y^\bullet$,
\item there is a canonical isomorphism
$$
f^!M = R\SheafHom_{\mathcal{O}_X}(
Lf^*R\SheafHom_{\mathcal{O}_Y}(M, \omega_Y^\bullet), \omega_X^\bullet)
$$
for $M \in D_{\textit{Coh}}^+(\mathcal{O}_Y)$, and
\item if in addition $f$ is proper there is a canonical map
$$
Rf_*R\SheafHom_{\mathcal{O}_X}(K, f^!M)
\to
R\SheafHom_{\mathcal{O}_Y}(Rf_*K, M)
$$
which is an isomorphism for all $K \in D_\QCoh(\mathcal{O}_X)$
and $M \in D_\QCoh^+(\mathcal{O}_Y)$.
\end{enumerate}
See Lemmas \ref{lemma-upper-shriek-composition},
\ref{lemma-shriek-via-duality}, and
\ref{lemma-proper-noetherian-relative}. We will now use
a bootstrap procedure to get a theory which works for
all finite type schemes over $S$.
...
\begin{enumerate}
\item $\omega_i^\bullet = \omega_{U_i}^\bullet = p_i^!\omega_S^\bullet$
as above,
...
{\it good dualizing complex with respect to $\omega_S^\bullet$
...
we see that good dualizing complexes are indeed dualizing complexes.
...
by compactifyable schemes. If there exists a good dualizing complex with
respect to $\omega_S^\bullet$ and $\mathcal{U}$, then it is unique
...
If there exists a good dualizing complex
with respect to $\omega_S^\bullet$ and $\mathcal{U}$, then
there exists a good dualizing complex with respect to
...
To go from a good dualizing complex $(K, \alpha_i)$ for $\mathcal{V}$ to a
good dualizing complex for $\mathcal{U}$ is achieved by forgetting
about $\alpha_i$ for $i = n + 1$. Conversely, let $(K, \alpha_i)$ be a
good dualizing complex for $\mathcal{U}$.
...
is a good dualizing complex for the covering
...
is another good dualizing complex for the covering
$U_{n + 1} = \bigcup U_i \cap U_{n + 1}$.
...
a good dualizing complex with respect to $\omega_S^\bullet$ and
$\mathcal{U}$.
\end{lemma}
...
be a good dualizing complex for
...
over $S$, there is an object $K \in D(\mathcal{O}_X)$, given up to unique
ismorphism, which comes equipped with isomorphisms
$\alpha_U : K|_U \to \omega_U^\bullet$ for every open subscheme $U \subset X$
which has a compactification over $S$, such that if
$\mathcal{U} : X = \bigcup U_i$ is a finite open covering
by opens which are compactifyable over $S$, then
$(K, \alpha_{U_i})$ is a good dualizing complex with respect to
...
{\it good dualizing complex for $X$ with respect to $\omega_S^\bullet$}
and is denoted $\omega_X^\bullet$.
\end{definition}

\noindent
It is clear from the construction that this does not conflict with our earlier
choice of $\omega_X^\bullet = p^!\omega_S^\bullet$ where $p : X \to S$ is the
structure morphism in case $X$ has a compactification over $S$. More generally
...
Let $\omega_X^\bullet$ and $\omega_Y^\bullet$ be good dualizing complexes
with respect to $\omega_S^\bullet$. Then $\omega_X^\bullet$
is a good dualizing complex with respect to $\omega_Y^\bullet$.
\end{lemma}
...
Let $(L, \beta_j)$ be a good dualizing complex with respect to
$\omega_S^\bullet$ and $\mathcal{V}$.
Let $(K, \gamma_{ji})$ be a good dualizing complex with respect to
$\omega_S^\bullet$ and $\mathcal{U}$.
...
\begin{proof}
Immediate from the construction of good dualizing complexes
given just above
...
Let $a$ be the twisted inverse image for $f$.
Then there is a canonical isomorphism
...
{\bf Bootstrap.}
Let $(S, \omega_S^\bullet)$ be as in Situation \ref{situation-dualizing}.
For a scheme $X$ of finite type over $S$ denote $\omega_X^\bullet$ a
good dualizing complex for $X$ with respect to $\omega_S^\bullet$.
Define
$$
...
Define
$$
f^! = D_X \circ Lf^* \circ D_Y :
D_{\textit{Coh}}^+(\mathcal{O}_Y)
\to
D_{\textit{Coh}}^+(\mathcal{O}_X)
$$
This agrees with the previously defined upper shriek functor
if $X$ and $Y$ are compactifyable over $S$ by
Lemma \ref{lemma-shriek-via-duality}.
Moreover, if $f : X \to Y$ and $g : Y \to Z$ are composable
morphisms between schemes of finite type over $S$, then
there is a canonical isomorphism
\begin{align*}
(g \circ f)^! & = D_X \circ L(g \circ f)^* \circ D_Z \\
...
It is immediately verified that the functors $f^!$ and the arrows
$(g \circ f)^! \to f^! \circ g^!$ turn $X \mapsto D_{\textit{Coh}}^+(X)$
into a pseudo functor from the category of schemes of finite type
over $S$ into the $2$-category of categories. Finally, if
$f$ is proper there is a canonical map
$$
Rf_*R\SheafHom_{\mathcal{O}_X}(K, f^!M)
\to
R\SheafHom_{\mathcal{O}_Y}(Rf_*K, M)
$$
which is an isomorphism for all $K \in D_\QCoh(\mathcal{O}_X)$
and $M \in D_{\textit{Coh}}^+(\mathcal{O}_Y)$. Namely, in this case
$\omega_X^\bullet = a(\omega_Y^\bullet)$ by
Lemma \ref{lemma-proper-map-good-dualizing-complex}
where $a$ is the twisted inverse image for $f$,
hence $f^!$ is the restriction of $a$ to $D_{\textit{Coh}}^+(\mathcal{O}_Y)$
by Lemma \ref{lemma-shriek-via-duality} applied to $f : X \to Y$
over the base scheme $Y$ (the issue is that we do not know $Y$ is
compactifyable over $S$ hence we cannot immediately apply
Lemma \ref{lemma-shriek-via-duality} to $f$ over $S$), and we see the
result is true by Lemma \ref{lemma-proper-noetherian-relative}.
