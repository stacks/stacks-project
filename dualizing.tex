...
$$
by the proof of Lemma \ref{lemma-local-cohomology-adjoint}.
...
$$
(insert future reference here) is an isomorphism for all $L$
because this is true on affines by Lemma \ref{lemma-dualizing}.
...
schemes. Then the functor $Rf_* : D_\QCoh(X) \to D_\QCoh(Y)$ has a
right adjoint.
\end{lemma}
...
is corresponds to the functor $K \longmapsto R\Hom(B, K)$, see
...
more generally $g$ and $f$ are Tor independent. Then
...
Then the assumptions and hence the conclusions of
Lemma \ref{lemma-when-sheafy} hold for all opens $V$ of $Y$.
\end{lemma}
...
$$
is nonzero. However, the restriction to $U$ is the map
...
& =
\Hom_{D(\mathcal{O}_X)}(
L \otimes^\mathbf{L}_{\mathcal{O}_X} Lf^*K, a(\mathcal{O}_Y)) \\
& =
...
perfect object (Derived Categories, Theorem
\ref{perfect-theorem-bondal-van-den-Bergh}) we can argue as in
...
However, the result holds for perfect objects in general, see
Lemma \ref{lemma-compare-with-pullback-perfect}.
\end{proof}

\begin{lemma}
\label{lemma-generatory-P1}
...
Lemma \ref{perfect-lemma-orthogonal-koszul-complex} this implies that
...
generate $D_\QCoh(X)$, see Lemma \ref{lemma-generatory-P1}.
...
\section{Definition of upper shriek functors}
...
The construction above is well defined. Moreover, if
$f : X \to Y$, $g : Y \to Z$
...
\medskip\noindent
Indepedence of the choice of the compactification is a special
