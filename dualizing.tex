...
and $\mathcal{O}_X(D)$ in degree $1$. The result follows.
\end{proof}
...
\longrightarrow K
$$
where the arrow comes from the canonical map
$R\SheafHom(i_*\mathcal{O}_D, \mathcal{O}_X) \to \mathcal{O}_X$
induced by $\mathcal{O}_X \to i_*\mathcal{O}_D$.
The equality holds as can be seen by representing $K$ by a K-flat
complex $\mathcal{K}^\bullet$ and choosing an injective resolution
$\mathcal{O}_X \to \mathcal{I}^\bullet$ and using that
$$
i_*(i^*\mathcal{K}^n
\otimes_{\mathcal{O}_D}
\SheafHom(\mathcal{O}_D, \mathcal{I}^m)) =
\mathcal{K}^n
\otimes_{\mathcal{O}_X}
\SheafHom(i_*\mathcal{O}_D, \mathcal{I}^m)
$$
...
\begin{proof}
To prove the map is an isomorphism, it suffices to show that it
is an isomorphism after applying $i_*$. By the discussion following
(\ref{equation-map-effective-Cartier}) the left hand side becomes
$$
K \otimes_{\mathcal{O}_X}^\mathbf{L} i_*\mathcal{N}[-1] =
K \otimes_{\mathcal{O}_X}^\mathbf{L} (\mathcal{O}_X \to \mathcal{O}_X(D))
$$
(see also the proof of Lemma \ref{lemma-compute-for-effective-Cartier})
...
\end{align*}
the final equality because the complex is perfect.
...
We refer to \cite{F} for a fully general version of this; here we will
restrict to the case where $X$ is Gorenstein.
