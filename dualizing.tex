...
\begin{remark}
\label{remark-relative-dualizing-complex-algebraic}
Let $R \to A$ be a finite type homomorphism of Noetherian rings.
Choose a presentation $A = P/I$ with $P = R[x_1, \ldots, x_n]$.
In this situation we often use the notation
$$
\omega_{A/R}^\bullet = R\Hom(A, P)[n]
$$
in $D(A)$ where $R\Hom(A, -) : D(P) \to D(A)$ is the functor of
Section \ref{section-trivial}. By Remark \ref{remark-local-calculation-shriek}
this object of $D(A)$ is well defined up to isomorphism
as it is an avatar of $f^!\mathcal{O}_{\Spec(R)}$ where
$f : \Spec(A) \to \Spec(R)$ is the morphism of affine schemes
corresponding to the ring map $R \to A$.
The reader can also directly show that $\omega_{A/R}^\bullet$
is independent up to isomorphism in $D(A)$ of the choice of
presentation (fun exercise).
\end{remark}
