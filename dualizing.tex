...
$\Ext^i_A(K, \omega_A^\bullet) \to \Ext^i_A(\tau_{\leq n}K, \omega_A^\bullet)$
is an isomorphism for some $n \gg - i$. By
Derived Categories of Schemes, Lemma \ref{perfect-lemma-coherent-internal-hom}
applied to $\tau_{\leq n}K$ and $\omega_A^\bullet$
we see conclude that $\Ext^i_A(K, \omega_A^\bullet)$ is
a finite $A$-module for all $i$. Hence $R\Hom_A(K, \omega_A^\bullet)$
