...
If $Y$ is quasi-compact and quasi-separated, then the diagram
$$
\xymatrix{
...
If $f : X \to Y$ is a finite morphism of Noetherian schemes, then the diagram
$$
\xymatrix{
...
dualizing module, see Remark \ref{remark-dualizing}.
...
a factorization $A \to B_1 \to B$ with $B_1 \to B$ injective,
$A \to B_1$ finite, $\Spec(B) \to \Spec(B_1)$ an open
immersion, such that $B' \to B$ and $B'' \to B$ factor through $B_1$.
\end{lemma}

\begin{proof}
Let $B_1 \subset B$ be the $A$-subalgebra generated by the images
of $B' \to B$ and $B'' \to B$. As $B'$ and $B''$ are each generated
by finitely many elements integral over $A$, we see that $B_1$ is
generated by finitely many elements integral over $A$ and we conclude
that $B_1$ is finite over $A$
(Algebra, Lemma \ref{algebra-lemma-characterize-finite-in-terms-of-integral}).
Consider the maps
$$
B = B' \otimes_{B'} B \to B_1 \otimes_{B'} B \to B \otimes_{B'} B = B
$$
...
$$
\xymatrix{
\Spec(B_1) \ar[d] & \Spec(B) \ar[d] \ar[l] \\
\Spec(B') & \Spec(B) \ar[l]
}
$$
...
\begin{proof}
Let $B', B'', B_1$ be as in Lemma \ref{lemma-dominate-factorizations}.
We obtain a canonical map
$$
\omega_1 = \Hom_A(B_1, A) \otimes_{B_1} B \longrightarrow
...
$B'_g \to B_g$ is an isomorphism and hence $B'_g \to (B_1)_g \to B_g$
are isomorphisms. It suffices to show that $(\omega_1)_g \to \omega'_g$
is an isomorphism. The kernel and cokernel of the ring map $B' \to B_1$
...
\begin{lemma}
\label{lemma-dualizing-base-change}
Consider a cocartesian diagram
$$
\xymatrix{
B \ar[r] & B' \ar@{=}[r] & B \otimes_A A' \\
A \ar[u] \ar[r] & A' \ar[u]
}
$$
of Noetherian rings. If $A \to B$ is quasi-finite and $A \to A'$ is flat, then
there is a canonical isomorphism $\omega_{B/A} \otimes_B B' = \omega_{B'/A'}$.
\end{lemma}

\begin{proof}
By construction of $\omega_{B/A}$ we may assume that $A \to B$ is finite.
Then $\omega_{B/A} = \Hom_A(B, A)$ and $\omega_{B'/A'} = \Hom_{A'}(B', A')$.
Since $B' = B \otimes_A A'$ the result follows from More on Algebra, Remark
\ref{more-algebra-remark-pseudo-coherence-and-base-change-ext}.
\end{proof}
...
Let $A \to B$ be a quasi-finite homorphism of Noetherian rings.
Then $f : Y = \Spec(B) \to \Spec(A) = X$ is a compactifyable morphism
of Noetherian schemes. Thus $f^!$ is defined, see
Section \ref{section-upper-shriek}. Choosing a factorization
$A \to B' \to B$ as in the definition of $\omega_{B/A}$ we
see that $f^!$ is the composition of $j^!$ and $(f')^!$
where $j : Y \to Y'$ and $f' : Y' \to X$ are the corresponding
morphisms of affine schemes. Combining
Lemmas \ref{lemma-shriek-open-immersion} and
\ref{lemma-finite-twisted}
(and the definition of the upper shriek functor)
we find that $f^!\mathcal{O}_X$ corresponds to
$R\Hom_A(B', A) \otimes_{B'} B$. We conclude that
$$
\widetilde{\omega_{B/A}} = H^0(f^!\mathcal{O}_X)
$$
In other words, the module studied in this section by elementary
means is a kind of relative dualizing module.
\end{remark}
...
\section{Traces for quasi-finite ring maps}
...
Section \ref{section-trace}. Namely, it is a generalization of the trace
...
(Algebra, Lemma \ref{algebra-lemma-finite-projective}).
Hence given $b \in B$ we can consider the {\it trace} $\text{Tr}_{B/A}(b)$
of the $A$-linear map $B \to B$ given by
multiplication by $b$ on $B$.
Since $\omega_{B/A} = \Hom_A(B, A)$ in this case we see
that $\text{Tr}_{B/A} \in \omega_{B/A}$.
...
for any flat base change as in Lemma \ref{lemma-dualizing-base-change}
and product decomposition $B' = C \times D$ with $A' \to C$ finite
the image of $\tau$ in $\omega_{C/A'}$ is $\text{Tr}_{C/A'}$.
Here we use Lemma \ref{lemma-dualizing-product} to get
$\omega_{B'/A'} \to \omega_{C/A'}$.
\end{definition}
...
Then $\text{Tr}_{B/A} \in \omega_{B/A}$ is a trace element.
\end{lemma}

\begin{proof}
Suppose we have a flat map $A \to A'$ with $A'$ Noetherian and
a product decomposition $B \otimes_A A' = C \times D$ with $A' \to C$
finite. Of course in this case $A' \to D$ is also finite.
Set $B' = B \otimes_A A'$.
Since the construction of traces commutes with base change
we see that $\text{Tr}_{B/A}$ maps to $\text{Tr}_{B'/A'}$.
Thus the proof is finished by noticing that
$\text{Tr}_{B'/A'} = (\text{Tr}_{C/A'}, \text{Tr}_{D/A'})$
under the isomorphism $\omega_{B'/A'} = \omega_{C/A'} \times \omega_{D/A'}$
...
\begin{enumerate}
\item If $A \to A'$ is a flat map with $A'$ Noetherian, then with
$B' = A' \otimes_A B$ the image of $\tau$ in $\omega_{B'/A'}$ is a
trace element.
...
suppose that we have a flat map $R \to R'$ with $R'$ Noetherian and a product
decomposition $B \otimes_R R' = C \times D$ with $R' \to C$ finite.
Then with $A' = R'_f$ we see that $B \otimes_A A' = C \times D$.
Since $R' \to C$ is finite, a fortiori $A' \to C$ is finite.
...
of (2). Suppose we have a flat map $A \to A'$ with $A'$ Noetherian and
a product decomposition $B_g \otimes_A A' = C \times D$ with $A' \to C$
finite. Set $B' = B \otimes_A A'$. Then
$\Spec(C) \to \Spec(B')$ is an open immersion as $B_g \otimes_A A' = B'_g$
and the image is closed because $B' \to C$ is finite (as $A' \to C$ is finite).
Thus we see that $B' = C \times D'$ and $D = D'_g$. Then we can use
...
fact that
$\text{Tr}_{C/A'} = (\text{Tr}_{C_1/A'}, \text{Tr}_{C_2/A'})$
...
\begin{proof}
Suppose we have a flat map $A \to A'$ with $A'$ Noetherian and a product
decomposition $B \otimes_A A' = C \times D$ with $A' \to C$ finite.
We have to show that the image of $\tau$ in $\omega_{C/A'}$ is
$\text{Tr}_{C/A'}$. Observe that $g_1, \ldots, g_m$
generate the unit ideal in $B' = B \otimes_A A'$ and that
$\tau$ maps to a trace element in $\omega_{B'_{g_i}/A'}$
by Lemma \ref{lemma-trace-base-change}. Hence we may replace
$A$ by $A'$ and $B$ by $B'$ to get to the situation as described
in the next paragraph.

\medskip\noindent
Here we assume that $B = C \times D$ with $A \to C$ is finite.
Let $\tau_C$ be the image of $\tau$ in $\omega_{C/A}$.
We have to prove that $\tau_C = \text{Tr}_{C/A}$ in $\omega_{C/A}$.
...
and we conclude that $\tau_C = \text{Tr}_{B/A}$ as desired.
\end{proof}
...
In this way we reduce to the case where $B = B'_g$ for some $g \in B'$.
This case is treated in the next paragraph.

\medskip\noindent
Assume we have $A \to B$ finite and $g \in B$ with $B_g$ flat over $A$.
It is our task to construct a trace element in
$\omega_{B_g/A} = \Hom(B, A) \otimes_B B_g$.
...
$\text{Tr}_{B/A}$. In the general case, we consider
the diagram
$$
\xymatrix{
& \Hom_A(B, A) \otimes_A B \ar[r] \ar[d] &
\Hom_A(B, A) \otimes_A B_g \ar[d] \\
B \ar[r]_-\mu \ar@{..>}[rru] \ar@{..>}[ru]^\psi &
\Hom_A(B, B) \ar[r] &
\Ker(F_0^\vee \otimes_A B_g \to F_1^\vee \otimes_A B_g)
}
$$
By flatness of $A \to B_g$ we see that the right vertical arrow is an
isomorphism. Hence we obtain the unadorned dotted arrow.
Since $B_g = \colim \frac{1}{g^n}B$, since
...
\Hom_A(B, A) \otimes_B B_g = \omega_{B_g/A}
$$
...
$B = C \times D$ and $g = (f, h)$ where $A \to C$ flat, $D_h$ is flat, and
$f$ is a unit in $C$.
To show: $\tau$ maps to $\text{Tr}_{C/A}$ in $\omega_{C/A}$.
In this case we can choose a $n_D$ and
...
Then if we take $n = n_D$ and $\psi = (f^{n_D} \psi_C, \psi_D)$ the
desired compatibility is clear because $\text{Tr}_{C/A} = ev \circ \psi_C$
as remarked above.

\medskip\noindent
To prove the desired property in general, suppose that
$A \to A'$ is a flat ring map with $A'$ Noetherian and that
$B_g \otimes_A A' = C \times D$ with $A' \to C$ finite.
Set $B' = B \otimes_A A'$. Then $\Spec(C) \to \Spec(B')$
is an open immersion as $B_g \otimes_A A' = B'_g$ and
then image is closed as $B' \to C$ is finite (since $A' \to C$
is finite). Thus $B' = C \times D'$ and $D'_g = D$.
We conclude that $A' \to B' = C \times D'$ and $g$
is as in the previous paragraph.
Since formation of the displayed diagram above
commutes with flat base change, the formation of $\tau$ commutes
with the flat base change $A \to A'$ (details omitted).
Thus the desired compatibility follows from the result
