...
$M \in D_{\textit{Coh}}^+(\mathcal{O}_Y)$, and
\item if in addition $f$ is proper there is a canonical isomorphism
$$
...
\noindent
We will now use a bootstrap procedure to get a theory which works for
all finite type schemes over $S$ given a pair $(S, \omega_S^\bullet)$
as in Situation \ref{situation-dualizing}.
...
(which exists and is unique up to unique isomorphism; it comes with
a canonical identification with the complex
$\omega_X^\bullet$ from Section \ref{section-duality} if $X$
is compactifyable).
...
$M \in D_{\textit{Coh}}^+(\mathcal{O}_Y)$; most importantly
$$
Rf_*R\SheafHom_{\mathcal{O}_X}(K, \omega_X^\bullet) =
R\SheafHom_{\mathcal{O}_Y}(Rf_*K, \omega_Y^\bullet)
$$
...
To get around this we use that $\omega_X^\bullet = a(\omega_Y^\bullet)$ by
...
\medskip\noindent
The final assertions follow from the construction of good
