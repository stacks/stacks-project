...
of the derived category of of $A$ are equivalent, see
...
$R\Gamma_Z(M)^\wedge$ of local cohomology. Then there are short exact
sequences
$$
0 \to R^1\lim H^{i - 1}_Z(M/I^nM) \to H^i(R\Gamma_Z(M)^\wedge) \to
\lim H^i_Z(M/I^nM) \to 0
$$
In particular we have $R\Gamma_Z(M)^\wedge$ has vanishing cohomology
in negative degrees.
\end{lemma}
...
$$
of usual completions. Thus the result is true, see for example
More on Algebra, Lemma
...
$$
H^0(R\Gamma_Z(M)^\wedge) = \colim H^0_{J'}(M)
$$
where the (filtered) colimit is over $J' \subset J$ such that
$V(J') \cap V(I) \subset V(J)$.
\end{lemma}
...
The support of $s$ intersected with $D(g_j)$ is disjoint from
$D(g_j) \cap V(I)$ by the arguments above.
Thus the support $T$ of $s$ is a closed subset of
$\Spec(A)$ with $T \cap V(I) \subset V(J)$. Hence $T = V(J')$ for some
ideal $J' \subset J$ with $V(J') \cap V(I) \subset V(J)$ (small
detail omitted).
Conversely, any $s$ whose support $T$ satisfies $T \cap V(I) \subset V(J)$
maps to zero in $M_{g_j}^\wedge$ for all $j$.
