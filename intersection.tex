...
Constructions, Section \ref{constructions-definition-projective-bundle}.
...
Since $G$ acts doubly transitively on $\mathbf{P}(V)$ we see that
$$
T = \{(x, y, g) \mid x \in X, y \in Y, g \in G, gx = y\}
$$
is a fibre bundle over $X \times Y$ with fibre equal to the stabilizer
of a pair of points. Hence $T$ is a variety. The morphism
$T \to G$ is surjective, because any translate of $X$ intersects $Y$
(insert future reference here). Since the dimensions of fibres of a
dominant morphism of varieties do not jump in codimension $1$
...
irreducible polynomial (insert future reference here).
...
Thus we can argue as above to conclude.
FIXME. Argument is too rough, although it clearly works.
\end{proof}
...
$\mathbf{P} = \mathbf{P}(E^\wedge)$. Choose a general line
$\ell \subset \mathbf{P}$ passing through $\text{id}_V$.
Set $C' \subset \mathbf{P}(V) \times \ell$ equal to the
closed subscheme having fibre $g(X)$ over $[g] \in \ell$.
This is well defined and satisfies all the requirements by
Lemma \ref{lemma-make-family}. FIXME: Add more arguments.
\end{proof}
