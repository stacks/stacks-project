...
is the generic point. Let $I, J \subset A$ cut out the trace of $V$ and $W$
in $\Spec(A)$. Set $\overline{I} = I + \mathfrak m^2/\mathfrak m^2$.
Then $\dim_\kappa \overline{I} \leq \dim(X) - \dim(V)$ with equality
if and only if $A/I$ is regular (this follows from
the definition of regular rings and dimension theory).
Similarly for $\overline{J}$. If the multiplicity is $1$, then
...
proper) and we find that $I$ and $J$ are generated by
regular sequences $f_1, \ldots, f_r$ and $g_1, \ldots, g_s$.
Thus the tor groups of $A/I$ and $A/J$ are computed by
the Koszul comples on $f_1, \ldots, g_s$ which has only
one cohomology group, namely, the in degree $0$ where one
obtains $\kappa$. This finishes the proof.
...
Let $X$ be a nonsingular projective variety. Let $\alpha$ be a $(k + 1)$-cycle
on $X \times \mathbf{P}^1$ which intersects $X \times a$ and $X \times b$
properly. Then
$$
pr_{X,*}( \alpha \cdot X \times a - \alpha \cdot X \times b).
$$
is rationally equivalent to zero. Conversely, any $k$-cycle which is
...
\noindent
In the case of transversal intersections of closed subschemes
the intersection number is $1$.
...
\begin{proof}
Observe that $\dim(T) = r + s - \dim(X)$ by the assumption that the
intersections are transversal. Let $(A, \mathfrak m, \kappa) =
(\mathcal{O}_{X, \xi}, \mathfrak m_\xi, \kappa(\xi))$ where $\xi \in Z$
is the generic point. Let $I_0, I, J_0, J \subset A$ cut out the trace of
$Y_{i_0}$, $Y$, $Z_{j_0}$, $Z$
in $\Spec(A)$. Set $\overline{I} = I + \mathfrak m^2/\mathfrak m^2$.
Then $I \subset I_0 \subset \mathfrak m$ and
$J \subset J_0 \subset \mathfrak m$ and $I + J = \mathfrak m$.
By Lemma \ref{lemma-transversal} we see that
$I_0 = (f_1, \ldots, f_{n - r})$ and $J_0 = (g_1, \ldots, g_{n - s})$.
where $f_1, \ldots, g_{n - s}$ is a regular system of parameters
for the regular local ring $A$. Since $I + J = \mathfrak m$, the map
$$
I \oplus J \to
\mathfrak m/\mathfrak m^2 = \bigoplus \kappa f_a \oplus \bigoplus \kappa g_b
$$
is surjective. We conclude that we can find $f_a' \in I$ and $g'_b \in J$
whose residue classes in $\mathfrak m/\mathfrak m^2$ are equal to the
residue classes of $f_a$ and $g_b$. Then $f'_1, \ldots, g'_{n - s}$
is a regular system of parameters of $A$. This implies that $I = I_0$
and $J = J_0$ which in turn gives (2), (3), and (4).
Then the coefficient is $1$ by the result of Lemma \ref{lemma-transversal}.
...
Constructions, Lemma \ref{constructions-lemma-proj-functor-strict}.
...
\begin{proof}
In fact this will be true for $U = \mathbf{P}(V) \setminus X$.
For a closed point $p$ of $U$ we obtain a morphism
$r_p|_X : X \to \mathbf{P}(W_p)$. This morphism is proper because
$X$ is a proper scheme. On the other hand, the fibres of $r_p$ are affine
(details omitted). Hence the fibres of $r_p|X$ are proper and affine,
whence finite. Finally, a proper morphism with finite fibres is finite.
FIXME: Add references.
\end{proof}
...
$$
T = \{(y, p) \mid y \in X, y \not = x, p \in \overline{xy}\}
$$
and the morphism $T \to \mathbf{P}(V)$ given by $(y, p) \mapsto p$.
The dimension of $T$ is $\dim(X) + 1 < \dim(\mathbf{P}(V))$.
Hence $T \to \mathbf{P}(V)$ is not surjective. On the other hand,
consider the finite type scheme
$$
T' = \{p \mid \overline{xp}\text{ tangent to }X\text{ at }x\}
$$
Then the dimension of $T'$ is $\dim(X) < \dim(\mathbf{P}(V))$.
Thus the morphism $T' \to \mathbf{P}(V)$ is not surjective either.
Let $U \subset \mathbf{P}(V) \setminus X$ be open and disjoint
from these images. Then for $p \in U$ closed the projection
$r_p|_X : X \to \mathbf{P}(W_p)$ is finite, injective on the
tangent space at $x$ and $r_p^{-1}(\{r_p(x)\}) = \{x\}$.
It follows that there is an open neighbourhood $R$ of $r_p(x)$
...
Since $\dim(T) = \dim(Y) + \dim(Z) + 1$ we see that the general
fibre of $T \to \mathbf{P}(V)$ has dimension at most
$\dim(Y) + \dim(Z) + 1 - n$. Let
$U \subset \mathbf{P}(V) \setminus (Y \cup Z)$ be the open over
...
$$
is the image of $F \to Y$, $(y, z, p) \mapsto y$.
FIXME: Add references.
\end{proof}
...
$C \cap X$ contains $Z$ with multiplicity $1$ (details omitted).
...
\begin{lemma}
\label{lemma-move}
Let $C \subset \mathbf{P}^N$ be a closed subvariety of dimension $c$.
Let $Z_i \subset \mathbf{P}^N$ be a finite list of subvarieties.
There exists a closed subvariety $C' \subset \mathbf{P}^N \times \mathbf{P}^1$
such that $C'_0 = C$ and such that $C'_\infty$ properly intersects
each of the given $Z_i$.
\end{lemma}
...
$[W_a]_r = pr_{X,*}(W \cdot X \times a)$ and similar for $[W_b]_r$.
...
may be applied. This tells us that
$$
...
pr_{X,*}((V \times {\bf P}^1 \cdot W) \cdot X\times b)
$$
which is true more or less by definition of rational equivalence,
more precisely by Lemma \ref{lemma-rational-equivalence-and-intersection}.

\medskip\noindent
General case. Pick a closed immersion $X \subset \mathbf{P}^N$.
It suffices to prove the result when $\beta = [Z]$ for some
...
intersects the support of $\alpha$ properly, it may not be the
case that $Z$ intersects the ``intermediate'' varieties $W_{i, a_i}^t$
...
C \cdot X = [Z] + \sum m_j [Z_j]
$$
and such that the maximum of the integers
$$
\dim(Z_j \cap W_{i, a_i}^t),\quad \dim(Z_j \cap W_{i, b_i}^t)
$$
is strictly smaller than $d$. Thus we see that the result is true
for each $Z_j$ by induction. Next, pick
$C' \subset \mathbf{P}^N \times \mathbf{P}^1$
as in Lemma \ref{lemma-move} with respect to all the subvarieties
$X$, $Z$, $Z_j$, $W_{i, a_i}^t$, $W_{i, b_i}^t$ above.
Write $C' \cdot X \times \mathbf{P}^1 = \sum n_k [E_k]$ for
some subvarieties $E_a \subset X \times \mathbf{P}^1$ of
dimension $s + 1$. Arguing as above we see that
$$
[Z] + \sum m_j [Z_j] =
C \cdot X =
C'_0 \cdot X =
(C' \cdot X \times \mathbf{P}^1) \cdot X \times 0 =
\sum n_k[E_{k, 0}]_r
$$
...
\gamma =
C'_\infty \cdot X =
(C' \cdot X \times \mathbf{P}^1) \cdot X \times \infty =
\sum n_k[E_{k, \infty}]_r
$$
properly intersects everything in sight. Thus by the base case
and linearity, we see that
$$
\gamma \cdot \alpha \sim_{rat} 0
$$
Also, by our choice of $C'$ we see that $E_{k, 0}$ and $E_{k, \infty}$
intersect $\alpha$ properly. Hence by the special case applied
to $E_k \subset X \times \mathbf{P}^1$ and $\alpha$ we see that
$$
