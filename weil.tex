...
extenesion $k''/k'$. We obtain
$$
H^0(X) \otimes_{H^0(\Spec(k'))} H^0(\Spec(k'')) =
H^0(X \times_{\Spec(k')} \Spec(k''))
$$
by axiom (B)(a). Thus in order to show that
...
\noindent
By Lemma \ref{lemma-negative-cohomology}
this means also that there are no nonzero
negative cohomology groups. However, we immediately
run into similar questions of the same kind if we
consider different degrees of cohomology.
...
of dimension $d$ over $k'$ with $k'/k$ a finite separable extension.
Suppose that
$$
