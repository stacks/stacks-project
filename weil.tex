...
The class $[\Delta_P]$ of the diagonal of $P$ in $A^*(P \times P)$
can be written
as
$$
[\Delta_P] =
...
(p \times p)^*[\Delta_X]
$$
for some $\alpha_i \in A^{r - 1 - i}(P)$ with $\alpha_{r - 1} = 1$.
\end{lemma}
...
$$
ch : K_0(X) \otimes \mathbf{Q} \longrightarrow A_*(X) \otimes \mathbf{Q}
$$
see Chow Homology, Lemma \ref{}
we can define cohomology classes of cycles
$$
\gamma : A_*(X) \longrightarrow H^*(X)
$$
by the formula
$$
...
$$
ch_i(\Psi_2(\xi)) = 2^i ch_i(\xi) \in A_{\dim(X) - i}(X) \otimes \mathbf{Q}
$$
and
$$
...
Then there exists a cycle $\alpha' \in A^{r - 1}(X')$ such that
...
$$
Let $\alpha' \in A^{r - 1}(X')$ be as in
Lemma \ref{lemma-divide-pullback-good-blowing-up}.
Then $\pi_*j^*\alpha' = [Z]$ in $A_*(Z)$ implies that
