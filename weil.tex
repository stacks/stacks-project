...
and CHow Homology, Lemma \ref{chow-lemma-flat-pullback-rational-equivalence}.
...
\noindent
We do the minimal amount of work needed in the next section.
...
\medskip\noindent
Let $X$ be a quasi-compact and quasi-separated scheme and
...
Let $X$ be a quasi-compact and quasi-separated scheme and let
...
As above this proves the $\psi^2$ is multiplicative.
\end{proof}
...
\medskip\noindent
The third axiom is the compatibility of the first chern class will pullbacks.
\begin{enumerate}
\item[(A3)] $c_1^H$ is compatible with pullbacks
\end{enumerate}
This means precisely that given a morphism $f : X \to Y$ of smooth
projective schemes over $k$ and an invertible $\mathcal{O}_Y$-module
$\mathcal{N}$ we have $f^*c_1^H(\mathcal{L}) = c_1^H(f^*\mathcal{L})$.

\medskip\noindent
The fourth axiom is the projective space bundle formula.
Let $X$ be a smooth projective scheme over $k$.
Let $\mathcal{E}$ be a locally free $\mathcal{O}_X$-module of rank $r$.
...
Set $c = c_1^H(\mathcal{O}_P(1)) \in H^2(P)(1)$.
\begin{enumerate}
\item[(A4)] In the situation above the map
$$
\bigoplus\nolimits_{i = 0, \ldots, r - 1} H^*(X)(-i)
...
\end{enumerate}
With axioms (A1) -- (A4) we can already define chern classes
...
$$
ch^H(\mathcal{L}) = 1 + c_1^H(\mathcal{L}) + (1/2)c_1^H(\mathcal{L})^2 +
(1/6)c_1^H(\mathcal{L})^3 + \ldots
$$
...
$$
compatible with pullbacks such that
$ch^H(\mathcal{E}) = \gamma(ch(\mathcal{E}))$
for $\mathcal{E}$ in $K_0(X)$.
\end{lemma}
...
$$
see Chow Homology, Lemma \ref{chow-lemma-K-tensor-Q}
we can define $\gamma$ by the formula
$$
...
\medskip\noindent
The sixth, seventh, and eigth axioms are as follows
\begin{enumerate}
\item[(A6)] Let $X$ be a smooth projective scheme over $k$ equidimensional
...
$\lambda_Y$ and $\lambda_X$ are as in axiom (A5) for $X$ and $Y$.
\end{enumerate}
...
is an isomorphism.
\end{enumerate}
We will show that data (D0), (D1), and (D2') satisfying axioms
(A1) -- (A8) give rise to a Weil cohomology theory.
...
$$
Hence because $b^*a$ restricts to zero on $E$ we get what we want from (A6).
\end{proof}

\begin{lemma}
\label{lemma-poincare-duality}
Assume given data (D0), (D1), and (D2') satisfying axioms (A1) -- (A7).
Then axiom (A) holds with $\int_X = \lambda$ as in axiom (A5).
\end{lemma}
...
axiom (A5). On the other hand, define a map
$$
\epsilon :
...
$\lambda : H^{2d}(X)(d) \to F$ of axiom (A5).
...
$$
the map $\int_{\Spec(k)} = \lambda$ of axiom (A5) must send $1$ to $1$.
\end{proof}
...
the class of $\Delta$ and $\Delta'$ as in axiom (A5).
...
$$
We have $j_*(1) = \gamma([E])$ by (A6)(b). Thus this is equal to
$$
