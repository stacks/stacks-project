...
Let $M$ be the union of the indices occuring in $M_b$, $b = 1, \ldots, t$.
We write
$$
\Psi_k =
...
there is one $l$ with $a_{kl} = w_k$. This is dissallowed as this would
...
$\kappa$ of $k$ of degree $6$. This is a contradication with the
...
(Defintion \ref{definition-top-genus})
...
and $H^0(X_K, \mathcal{O}_{X_K}) = K$. By Riemann-Hurewitz
...
Results in the literature imply that one can improve the bound
...
of $C$ and its Jacobian are the same thing. For an abelian variety
we have semistable reduction if the action of Galois
on the $\ell$-torsion is trivial for any $\ell \geq 3$
not equal to the residue characteristic.
Thus we can for example choose $\ell = 5$
in the formula (\ref{equation-bound}) for $B_g$
(but the proof would take a lot more work).
\end{remark}
