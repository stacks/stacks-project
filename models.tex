...
Arguing as exactly as before this implies $g_k = 0$.
If the set $\{l : l \not = k,\ l \not \in M\}$ is empty,
then $\{1, \ldots, n\} = M \cup \{k\}$ and $g_{top} = 0$
because $e = n - 1$ in this case
and the result holds by
Lemma \ref{lemma-non-irreducible-minimal-type-genus-at-least-one}.
...
$$
is $\geq \frac{1}{2}$ because $m_k \geq 2m_{i_b}$ by the previous
paragraph and is $\geq 1$ if $w_k < w_{i_b}$.
Hence if $t \geq 2$ or $t = 1$ and $w_k < w_{i_1}$, then $\Psi_k' \geq 0$
(here we use the existence of an $l$ as shown above).
Assume $t = 1$ and $w_k = w_{i_1}$. If there at least two nonzero terms
...
by Algebraic Curves, Proposition \ref{curves-proposition-projective-line}.
