...
of $(-2)$-indices (as defined in the previous section) and
...
and $a_{ii} < 0$ from (\ref{equation-diagonal-negative})
...
\noindent
Fix an integer $g$ and let $n, m_i, a_{ij}, w_i, g_i$ be a
numerical type of genus $g$.
We assume the type is minimal. We also assume $g > 1$ as we've
classified minimal types of genus $0$ and $1$ in
Section \ref{section-classification-genus-one}.
If $n = 1$, then there are finitely many possibilities, see
Equation (\ref{equation-irreducible}) and the discussion surrounding it.
...
has at most $2g$ elements,
\item for $j \in J$ we have $m_jw_j \leq 2g$, $m_j|a_{jj}| \leq 6g$, and
$g_j \leq g$, and
\item for $j, j' \in J$ we have $m_ja_{jj'} \leq 6g$.
\end{enumerate}
\end{lemma}

\begin{proof}
For $j \in J$ the contribution $m_j(w_i(g_j - 1) - \frac{1}{2} a_{jj})$
to the genus $g$ is $> 0$ and in fact $w_j(g_j - 1) - \frac{1}{2}a_{jj}$
is $\geq \frac{1}{2}w_i$ by our divisibility conditions (see
discussion preceding Definition \ref{definition-type-minus-two}).
This gives (1) and (2). For (3) we can use
Lemma \ref{lemma-bound-neighbours} and (2).
\end{proof}

\begin{lemma}
\label{lemma-bound-wm}
Fix $g \geq 2$. For every minimal numerical type $n, m_i, a_{ij}, w_i, g_i$
of genus $g$ we have $m_iw_i \leq 384g$.
\end{lemma}
...
nonempty as $g \geq 2$.
We have seen that $m_jw_j$ and $m_j|a_{jj}|$ are bounded
by $6g$ for $j \in J$.
...
that $m_{i_1}w_{i_1} \leq 6g$ and $w_{i_1}a_{ji_1} \leq 6g$.
...
$m_{i_2}w_{i_2} \leq 12g$ and $w_{i_2}a_{i_1i_2} \leq 12g$.
Then $m_{i_2}|a_{i_2i_2}| \leq 24g$ and so on.
Eventually we conclude that
$m_{i_k}w_{i_k} \leq 384g$ for $k = 1, \ldots, 7$.
...
$w_im_i \leq 384g$ unless $I$ is as described in
Lemma \ref{lemma-long} or Lemma \ref{lemma-Dn}.
...
that we have
$$
2m_2 \geq m_1 + m_2, \ldots, 2m_{t - 1} \geq m_{t - 2} + m_t
$$
Equality holds in $2m_i \geq m_{i - 1} + m_{i + 1}$
if and only if $i$ does not ``meet'' any indices besides
$i - $ and $i + 1$. And if $i$ does meet
another index, then this index is in $J$ (by maximality of $I$).
In particular, the map
$\{2, \ldots, t - 1\} \to \mathbf{Z}$, $i \mapsto m_i$ is concave.

\medskip\noindent
Let $m = \max(m_i, i \in \{2, \ldots, t- 1\})$.
Let $s$, resp.\ $s'$ in $\{2, \ldots, t - 1\}$ be the
smallest, resp.\ biggest index with $m_s = m = m_{s'}$.
By concavity we see that $m_i = m$ for $s \leq i \leq s'$.
If $s > 2$, then we do not have equality in
$2m_s \geq m_{s - 1} + m_{s + 1}$
and we see that $s$ meets an index from $J$.
In this case $mw \leq 384g$ by the result of the second paragraph
of the proof.
Similarly, if $s' < t - 1$, then $s'$ meets an index from $J$
and we get $mw \leq 384g$ as well.
Finally, if $s = 2$ and $s' = t - 1$, then we conclude
...
$i = 1$ or with $i = t$. In this case there is a chain of length
two connecting either $i = 2$ or $i = t - 1$ to an index of $J$
and we obtain the desired inequality as well.
\end{proof}
