...
In general, the morphism $X \to S$ is at-worst-nodal
of relative dimension $1$, see
...
\medskip\noindent
Thus we see that $X' \to S$ is still at-worst-nodal of relative dimension $1$
...
Thus by induction we see that there is a sequence of
...
Perhaps a few words are in order as to how to translate the statement
given there into the statement given above.

\medskip\noindent
First, if $x \in X$ is a point where the morphism $X \to \Spec(R)$ is smooth,
...
First we shrinking $V$ to an affine neighbourhood of the image of $x'$.
Say $V = \Spec(R')$. Then $R \to R'$ is \'etale. Since $R$ is a
discrete valuation ring, we see that $R'$ is a finite
product of quasi-local Dedekind domains. Hence (for example using
prime avoidance) a principal localization $R'_f$ of $R'$ is a discrete
valuation ring whose maximal ideal corresponds to the image of $x'$.
...
\begin{proof}
Let $x \in X$ be a singular point of the special fibre. Choose a diagram
$$
\xymatrix{
X \ar[d] &
U \ar[r] \ar[d] \ar[l] &
...
$X/R$ is singular. Thus $A = R'[u, v]/(uv - \pi^n)$
for some $n \geq 0$. Since $x$ is a singular point,
we see that $n \geq 2$, see discussion in
...
Let $R$ be a discrete valuation ring with fraction field $K$
and residue field $k$. Let $C$ be a smooth projective curve
over $K$ with $H^0(C, \mathcal{O}_C) = K$.
Let $X \to X'$ be the contraction of an
exceptional curve $E \subset X$ of the first kind
in a proper regular model $X$. Then, if $X$
at-worst-nodal of relative dimension $1$ over $R$,
so is $X'$.
\end{lemma}
...
$\pi \in \mathfrak m_{x'} \subset \mathcal{O}_{X', x'}$ but
$\pi \not \in \mathfrak m_{x'}^2$.
Since $\mathcal{O}_{X', x'}$ is regular of
dimension $2$, this implies that $\mathcal{O}_{X'_k, x'}$
...
over $K$ with $H^0(C, \mathcal{O}_C) = K$. The following are equivalent
\begin{enumerate}
\item there exists a proper model $X$ of $C$ which is
at-worst-nodal of relative dimension $1$ over $R$,
\item a minimal model $X$ of $C$ is at-worst-nodal
...
that minimal models are often unique (Lemma \ref{lemma-minimal-model-unique}).
Keeping this in mind the implication (2) $\Rightarrow$ (1) is clear.

\medskip\noindent
Let $X$ be a proper model which is
at-worst-nodal of relative dimension $1$ over $R$
as in (1). Applying Lemma \ref{lemma-blowup-at-worst-nodal}
we see that we may assume $X$ is regular as well. Let
$$
X = X_m \to X_{m - 1} \to \ldots \to X_1 \to X_0
$$
be as in Lemma \ref{lemma-pre-exists-minimal-model}.
We need to show that $X_0$ is
at-worst-nodal of relative dimension $1$ over $R$.
This follows from Lemma \ref{lemma-blowdown-at-worst-nodal}
and induction.
\end{proof}
