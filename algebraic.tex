...
\begin{proof}
The reader is strongly advised to ponder this question on his/her own
before reading this incomprehensible proof. We have seen in
...
By the $2$-Yoneda lemma (see
Section \ref{section-2-yoneda})
we think of $u$, $v$ as a $1$-morphisms
$u, v : (\textit{Sch}/U)_{fppf} \to \mathcal{X}$
and we consider the $2$-fibre products
$$
(\textit{Sch}/U)_{fppf} \times_{j \circ u, \mathcal{Y}} \mathcal{X},
(\textit{Sch}/U)_{fppf} \times_{j \circ v, \mathcal{Y}} \mathcal{X}
$$
By assumption this is representable by an algebraic space $F_{j \circ u}$,
$F_{j \circ v}$ over $U$ and the morphisms
$F_{j \circ u} \to U$, $F_{j \circ v} \to U$ are monomorphisms.
But since $(1, u)$, $(1, v)$ give a $1$-morphism of
$(\textit{Sch}/U)_{fppf}$ into the displayed $2$-fibre products,
we see that $F_{j \circ u} = F_{j \circ v} = U$ (here we use
that if $V \to U$ is a monomorphism of algebraic spaces which has a
section, then $V = U$).
The $2$-morphism $t : j \circ u \to j \circ v$ gives an equivalence
$$
\tilde t :
(\textit{Sch}/U)_{fppf} \times_{j \circ u, \mathcal{Y}} \mathcal{X}
\longrightarrow
(\textit{Sch}/U)_{fppf} \times_{j \circ v, \mathcal{Y}} \mathcal{X}
$$
which maps $(V, a, b, \varphi) \mapsto (V, a, b, \varphi \circ t_a)$
where $t_a : j(u(a)) \to j(v(a))$ is the value of $t$ on a
(here we think of objects of the $2$-fibre products as in
Categories, Lemma \ref{categories-lemma-2-product-categories-over-C}).
Since both sides are represented by $U$ we see that the $1$-morphisms
$(1, u)$ (into the left hand side) and $(1, v)$ (into the right hand side)
correspond via this equivalence, i.e.,
$\tilde t (1, u) \cong (1, v)$ (via a unique isomorphism).
Projecting this isomorphism to $\mathcal{X}$ we see obtain the
desired isomorphism $u \cong v$.
\end{proof}
...
$U \to T$ is an open immersion. Thus we see that $U$ is identified with
an open subscheme of $T$. By the universal property of the $2$-fibre
product and the fact that $f_i^*(y) \cong j(x_i)$, we have that
$f_i : T_i \to T$ factors through the open subscheme $U \subset T$
for all $i$. Clearly this means that $U = T$. Thus $y$ comes from some
