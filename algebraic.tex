...
$\mathcal{Z}$. Then the $1$-morphism
$\mathcal{Z} \times_{g, \mathcal{Y}, f} \mathcal{X} \to \mathcal{Z}$
comes from a unique morphism $F \to G$ by
Categories, Lemma \ref{categories-lemma-2-category-fibred-setoids}.
This morphism is representable by algebraic spaces (hint:
combine
...
\mathcal{R}
$$
\item if $U$, $R$ are algebraic spaces representing
...
However, this does not affect the proof of this lemma at all, since this
only comes up when we check that we obtain a groupoid in algebraic spaces.
So the simple fact that the opposite of a groupoid is a groupoid too helps
alleviate the problem, and we won't mention it at all.

\medskip\noindent
We get the morphisms $t, s : R \to U$ and $c : R \times_{s, U, t} R \to R$
from (\ref{equation-morphisms-spaces}). Recall that the sheaf $U$ is
isomorphic to the sheaf $T \mapsto \text{Ob}(\mathcal{U}_T)/\!\cong$, and
similarly for $R$. Hence to show that we obtain a groupoid in algebraic
...
We have given the $1$-morphism $f : \mathcal{U} \to \mathcal{X}$,
and a $2$-arrow $\tau : f \circ \text{pr}_1 \to x \circ \text{pr}_0$
is given to us by definition of $\mathcal{R}$ as the $2$-fibre product.
Namely, on an object $(a, b, \alpha)$ of $\mathcal{S}_R$ over $T$ it is
the map $\alpha^{-1} : b \to a$. It remains to show that
$$
...
Except, to apply the lemma we need to prove the conditions hold
for the structure $(\mathcal{S}_U, \mathcal{S}_R, s, t, c)$.
The fact that $U$, $R$ represent $\mathcal{U}$, $\mathcal{R}$
means that we are given equivalences $j : \mathcal{U} \to \mathcal{S}_U$
and $j' : \mathcal{R} \to \mathcal{S}_R$. Choose quasi-inverses
$j^{-1} : \mathcal{S}_U \to \mathcal{U}$ and
$(j')^{-1} : \mathcal{S}_R \to \mathcal{R}$.
Note that
$\mathcal{S}_{R \times_{s, U, t} R} = \mathcal{S}_R \times_{\mathcal{S}_U} \mathcal{S}_R$, hence $j, j'$ induce an equivalence
$$
j' \times j' :
\mathcal{R} \times_{\text{pr}_1, \mathcal{U}, \text{pr}_0} \mathcal{R}
\longrightarrow
...
$$
and we chose a quasi-inverse $(j' \times j')^{-1}$.
Then the morphisms
$s, t : R \to U$ and $c : R \times_{s, U, t} R \to R$ are the unique
morphisms of algebraic spaces such that the induced $1$-morphisms
$s, t : \mathcal{S}_R \to \mathcal{S}_U$ and
$c : \mathcal{S}_{R \times_{s, U, t} R} \to \mathcal{S}_R$
are equal to
\begin{align*}
s & = j \circ \text{pr}_1 \circ (j')^{-1}, \\
t & = j \circ \text{pr}_1 \circ (j')^{-1}, \\
c & = j' \circ \text{pr}_1 \circ (j' \times j')^{-1}
\end{align*}


satisfy $j \circ t \cong \text{pr}_0 \circ j'$,
$j \circ s \cong \text{pr}_1 \circ j'$
and $j' \circ c \cong \text{pr}_{02} \circ j' \times j'$.
Set $f' = f \circ j$. Set $\tau' = \tau \star \text{id}_{j'}$.
By the above we see that $\tau' : f' \circ s \to f' \circ t$.

and we obtain our morphism of stacks in groupoids
$F : [U/R] \to \mathcal{X}$.
\end{proof}
