...
\medskip\noindent
To prove the second condition it suffices to show that
the $\mathit{Isom}$-sheaves are algebraic spaces, see
Lemma \ref{lemma-representable-diagonal}.
This follows from
...
Lemma
\ref{lemma-map-presheaves-representable-by-spaces-transformation-property}.
Hence if we can show that the canonical map
$\mathcal{S}_U \to [U/R]$ is surjective and smooth, then
we win by
Lemma \ref{lemma-composition-representable-transformations-property}.
...
Well, then we just perform the base change
$$
\mathcal{X}'_S
=
(\textit{Sch}/S)_{fppf} \times_{(\textit{Sch}/S')_{fppf}} \mathcal{X}'
$$
which is an algebraic stack according to
Lemma \ref{lemma-2-fibre-product}. Moreover, it comes with a natural
functor $p : \mathcal{X}'_S \to (\textit{Sch}/S)_{fppf}$ and hence by
Lemma \ref{lemma-category-of-spaces-over-smaller-base-scheme}
...
Let $S \to S'$ be a morphism of this site.
Let $\mathcal{X}'$ be an algebraic stack over $S'$.
The {\it base change of $\mathcal{X}'$} is the
