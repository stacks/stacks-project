...
$$
of stacks in groupoids over $(\textit{Sch}/S)_{fppf}$. For each integer
$d \geq 1$ consider a category $\mathcal{H}_d(\mathcal{X}/\mathcal{Y})$
defined as follows:
\begin{enumerate}
\item An object $(U, Z, y, x, \alpha)$ where $U, Z$ are objects of
in $(\textit{Sch}/S)_{fppf}$ and $Z$ is a finite locally free of degree
$d$ over $U$, where
$y \in \text{Ob}(\mathcal{Y}_U)$, $x \in \text{Ob}(\mathcal{X}_Z)$
and $\alpha : y|_Z \to F(x)$ is an isomorphism, in other words the data
fits into the $2$-commutative diagram
$$
\vcenter{
\xymatrix{
Z \ar[r]_x \ar[d] & \mathcal{X} \ar[d]^F \\
U \ar[r]^y & \mathcal{Y}
}
}
\quad\text{or}\quad
