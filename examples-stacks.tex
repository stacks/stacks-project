...
\begin{enumerate}
\item An object of $G\textit{-Principal}$ is a triple $(U, a, X)$ where
\begin{enumerate}
\item $U$ is an object of $(\textit{Sch}/S)_{fppf}$,
\item $a : U \to B$ is a morphism over $S$, and
...
\begin{enumerate}
\item An object of $G\textit{-Torsors}$ is a triple $(U, a, X)$ where
\begin{enumerate}
\item $U$ is an object of $(\textit{Sch}/S)_{fppf}$,
\item $a : U \to B$ is a morphism, and
\item $X$ is an fppf $G_U$-torsor over $U$ where $G_U = U \times_{a, B} G$.
\end{enumerate}
See
Groupoids in Spaces,
Definition \ref{spaces-groupoids-definition-principal-homogeneous-space}.
...
over $U$ is the disjoint union over $a : U \to B$
of the categories of fppf $U \times_{a, B} G$-torsors,
hence is a groupoid.
...
U \ar[rrrd]_a \ar[r]^f & U' \ar[rrd]^{a'} & & X \ar[d] \\
& & & B
}
$$
\end{enumerate}
...
\quad
(U, a, P, \varphi) \longmapsto U
$$
is a functor. Note that the fibre category of $[[X/G]]$
over $U$ is the disjoint union over $a \in \text{Mor}_S(U, B)$
of $U \times_{a, B} G$-torsors $P$ endowed with a $G$-equivariant
...
$\xi_i = (U_i, a_i, P_i, \varphi_i)$ be objects of $[[X/G]]$ over $U_i$,
and let $\varphi_{ij} : \text{pr}_0^*\xi_i \to \text{pr}_1^*\xi_j$
be a descent datum. This in particular implies that we get a descent
datum on the triples $(U_i, a_i, P_i)$ for the stack in groupoids
$G\textit{-Torsors}$. Hence we may assume that
$a_i = a|_{U_i}$ for some morphism $a : U \to B$, and
that $P_i = U_i \times_U P$ for some fppf $G_U = U \times_{a, B} G$-torsor
...
$c((g, x), (g', x')) = (m(g, g'), x')$. By definition (see
...
$$
[X/_{\!p}G] \longrightarrow [[X/G]],
\quad
(U, x) \longrightarrow (U, 
$$
