...
a scheme $X$ is just the category $\textit{QCoh}(\mathcal{O}_X)$
...
\Mor_{\textit{QCoh}(\mathcal{O}_Y)}(f^*\mathcal{F}, \mathcal{G})
\end{equation}
...
\Mor_{\textit{QCoh}(\mathcal{O}_Z)}(g^*f^*\mathcal{F}, \mathcal{H}) \\
& =
\coprod\nolimits_{g \in \Mor_S(Z, Y)}
\Mor_{\textit{QCoh}(\mathcal{O}_Z)}((f \circ g)^*\mathcal{F}, \mathcal{H}) \\
& =
...
$$
\textit{QCoh}(\mathcal{O}_T) \longrightarrow DD(\mathcal{U})
$$
