...
\noindent
Note that if, in the above lemma, $X$ is an affine scheme,
...
Let $\psi : \Gamma(Y, \mathcal{O}_Y) \to \Gamma(X, \mathcal{O}_X)$
...
$
\Gamma(Y, \mathcal{O}_Y) \to
\Gamma(X, \mathcal{O}_X) \to
\mathcal{O}_{X, x}
$.
We claim that the map $\Psi : X \to Y$ is continuous.
Since the standard opens $D(g)$, $g \in \Gamma(Y, \mathcal{O}_Y)$
are a basis for the toppology of $Y$, it suffices to prove
that $\Psi^{-1}(D(g))$ is open. By construction of $\Psi$
the inverse image $\Psi^{-1}(D(g))$ is exactly the set
$D(\psi(g)) \subset X$ which is open by Lemma \ref{lemma-f-open}.
Clearly, because $\psi(g)$ is invertible on $D(\psi(g))$
we see that there is a canonical map
$$
\Gamma(D(g), \mathcal{O}_Y) = \Gamma(Y, \mathcal{O}_Y)_g
\longrightarrow
\Gamma(\Psi^{-1}(D(g)), \mathcal{O}_X) = \Gamma(D(\psi(g)), \mathcal{O}_X) 
$$
