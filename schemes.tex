...
It has a basis of open sets $D(f)$, $f \in R$ which we call standard opens,
see Algebra, Definition \ref{algebra-definition-Zariski-topology}.
In addition, this basis has the pleasing property that $D(fg) =
D(f) \cap D(g)$, $f,g\in R$. In
Sheaves, Section \ref{sheaves-section-bases} we defined
...
\ref{sheaves-lemma-extend-off-basis-structures}. Moreover, we showed
in
...
standard open. Because every standard open is quasi-compact
(see Algebra, Lemma \ref{algebra-lemma-qc-open})
every open covering of it can be refined by a finite open
covering by standard opens.
...
is a covering $D(f) = \bigcup_{i=1}^n D(f_i)$,
where $f_1,\ldots, f_n \in R$.
\end{enumerate}
\end{definition}

\noindent
Note that $\text{Spec}(R) = \bigcup_{i=1}^n D(f_i)$ if and only
if $f_1,\ldots,f_n$ generate the unit ideal of $R$. Also,
recall that $D(f_i) \subset D(f)$ only if $f \in \sqrt{(f_i)}$,
and in this case $D(f_i) = D(ff_i)$ and also $R_{f_i} = R_{ff_i}$.
Given this property for all $i$ we have
$$
D(f) = \bigcup\nolimits_{i = 1}^n D(f_i)
\Leftrightarrow
(f_1,\ldots,f_n)\text{ generate the unit ideal of }R_f
$$

\begin{lemma}
\label{lemma-M-tilde-sheaf}
