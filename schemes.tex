...
\begin{lemma}
	\label{lemma-compose-after-closed-immersion}
	Let $f:Z\to X$, $f':Z'\to X$ be closed immersions
	of locally ringed spaces.
	If $g:Z'\to Z$ is a morphism over $X$,
	then it is a closed immersion.
\end{lemma}

\begin{proof}
	Let $\mathcal{I}$ and $\mathcal{I}'$ be the ideal sheaves
	associated to $f$ and to $f'$, respectively.
	It is clear that $g$ is a homeomorphism onto
	the closed subset $f^{-1}(Z')$ of $Z$. Since
	$\mathcal{O}_X\to f'_*\mathcal{O}_{Z'}$ equals the composite
	$\mathcal{O}_X
	\to f_*\mathcal{O}_Z
	\to f_*g_*\mathcal{O}_{Z'}$,
	we deduce $\mathcal{I}\subset\mathcal{I}'$.
	On stalks at $x\in X$, the last composite equals
	$\mathcal{O}_{X,x}
	\to\mathcal{O}_{X,x}/\mathcal{I}_x
	\to\mathcal{O}_{X,x}/\mathcal{I}'_x$.
	Thus, $\mathcal{O}_Z\to g_*\mathcal{O}_{Z'}$ is surjective,
	for example by
	Sheaves, Lemma \ref{lemma-stalks-closed-pushforward}.
	It is left to see that the kernel of
	$\mathcal{O}_Z\to g_*\mathcal{O}_{Z'}$
	is locally generated by sections. The map
	$f_*\mathcal{O}_Z\to f_*g_*\mathcal{O}_{Z'}$ identifies with
	$\mathcal{O}_Z/\mathcal{I}
	\to\mathcal{O}_Z/\mathcal{I}'$,
	which has kernel $\mathcal{I}'/\mathcal{I}$.
	This is an $\mathcal{O}_X$-module locally generated by sections,
	for $\mathcal{I}'$ is locally generated by sections.
	Hence, since both $\mathcal{O}_X/\mathcal{I}$ and
	$\mathcal{O}_X/\mathcal{I}'$ are annihilated by
	$\mathcal{I}$, by Modules, Lemma \ref{lemma-i-star-exact},
	the pullback of $f_*\mathcal{O}_Z\to f_*g_*\mathcal{O}_{Z'}$
	along $f$ identifies with $\mathcal{O}_Z\to g_*\mathcal{O}_{Z'}$
	and its kernel equals $f^*(\mathcal{I}'/\mathcal{I})$,
	a module locally generated by sections
	by Modules, Lemma \ref{lemma-pullback-locally-generated}.
\end{proof}

\begin{lemma}
	\label{lemma-closed-immersion-ideals-locally-ringed-spaces}
	Let $i : Z \to X$ and $i' : Z' \to X$
	be closed immersions of locally ringed spaces
	and consider the ideal sheaves
	...
	\begin{enumerate}
		...
		If this happens, then $g$ is a closed immersion.
		\item We have $Z \cong Z'$ over $X$ if and only if
		$\mathcal{I} = \mathcal{I}'$.
	\end{enumerate}
	In other words, there is a contravariant isomorphism between
	the poset of ideal sheaves over $X$ locally
	generated by sections (ordered by inclusion)
	and that of closed immersions of locally ringed spaces
	into $X$, modulo isomorphism over $X$.
	The order in the closed subspaces of $X$ is
	given by the existence of a map $Z \rightarrow Z'$ over $X$
	(by monicity of closed immersions, there is at most one such map).
\end{lemma}

\begin{proof}
	In the correspondence, the map to the
	right sends an ideal sheaf $\mathcal{I} \subset \mathcal{O}_X$
	locally generated by sections to the
	closed subspace of $X$ associated to this sheaf of ideals
	(Definition \ref{definition-closed-subspace}); whereas
	the map to the left sends a closed immersion $i: Z \rightarrow X$ to
	$\operatorname{Ker}\left(\mathcal{O}_X
	\rightarrow i_* \mathcal{O}_Z\right)$.
	The composite ``first to the right, then to the left''
	is the identity by Example \ref{example-closed-subspace},
	whereas the composite ``first to the left, then to the right''
	is the identity by \ref{lemma-closed-immersion}. We see
	the correspondence inverts the ordering:
	On the one hand, if we have ideal sheaves
	$\mathcal{I}' \subset \mathcal{I} \subset \mathcal{O}_X$,
	then we get a map $Z \rightarrow Z'$ over $X$
	by \ref{lemma-characterize-closed-subspace}, where $Z$ and $Z'$
	are the closed subspaces associated to $\mathcal{I}$ and
	$\mathcal{I}'$, respectively. On the other hand, if we have
	closed subspaces $i: Z \rightarrow X$ and $i': Z' \rightarrow X$
	and a map $a: Z \rightarrow Z'$ over $X$, then $\mathcal{I}$ equals
	the kernel of $\mathcal{O}_X \rightarrow i_* \mathcal{O}_Z$, i.e., of
	$\mathcal{O}_X
	\rightarrow i_*' \mathcal{O}_{Z'}
	\rightarrow i_*' a_* \mathcal{O}_X$.
	But this contains $\mathcal{I}'$.
	Lastly, note that by Lemma \ref{lemma-compose-after-closed-immersion},
	a map $Z\to Z'$ over $X$ is always a closed immersion.
\end{proof}

\begin{lemma}
\label{lemma-restrict-map-to-closed}
\begin{slogan}
	Closed immersions of locally ringed spaces
...
Thus we may glue (see Sheaves, Lemma \ref{lemma-sheaf-of-subsheaves})
