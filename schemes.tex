...
$\varphi_i : U_i \to X_i$ of locally ringed spaces such that
\begin{enumerate}
\item $\varphi_i^{-1}(U_{ij}) = U_i \cap U_j$, and
\item $\varphi_{ij} =
\varphi_j|_{U_i \cap U_j} \circ \varphi_i^{-1}|_{U_{ij}}$.
\end{enumerate}
...
f_i : X_i \to Y,\ f_j \circ \varphi_{ij} = f_j|_{U_{ji}}\} \\
f & \mapsto & (f_i = f|_{U_i} \circ \varphi_i^{-1})_{i \in I}
\text{Mor}(Y, X) & = & \{
(Y = \bigcup\nolimits_{i \in I} V_i,\ (\psi_i : V_i \to X_i)_{i \in I})
\mid
\psi_i^{-1}(U_{ij}) = V_i \cap V_j,\ 
\psi_j|_{V_i \cap V_j} = \varphi_{ij} \circ \psi_i|_{V_i \cap V_j}
\}
\end{eqnarray*}
