...
$(Z, U)$ consisting of a closed subsscheme $Z$ of an open subscheme $U$
...
$$
\xymatrix{
Z = Y \times_{(Y \times_S Y)} (X\times_S X) \ar[r] \ar[d] &
X \times_S X \ar[d]^{a \times b} \\
Y \ar[r]^-{\Delta_{Y/S}} & Y \times_S Y
}
$$
Thus the lemma follows from Lemma \ref{lemma-diagonal-immersion} and
...
Note that (2) implies that for every point $x \in X$ the map
$f^\sharp_x : \mathcal{O}_{Y, f(x)} \to \mathcal{O}_{X, x}$ is surjective.
Let $a, b : Z \to X$ be two morphisms of schemes such that
$f \circ a  = f \circ b$. Then (1) implies $a = b$ as underlying
maps of topological spaces, and the surjectivity of the maps
$f^\sharp_x$ forces $a^\sharp_z = b^\sharp_z$, $\forall z \in Z$
which implies that $a^\sharp = b^\sharp$.
Hence $f : X \to Y$ is a monomorphism. Thus the diagonal morphism
$X \to X \times_Y X$ is an isomorphism. In particular, $f$
is separated. Hence Lemma \ref{lemma-push-forward-quasi-coherent}
...
In the proof above we used the notion of a monomorphism
in the category of schemes. For more on this see
Morphisms, Section \ref{morphisms-section-monomorphisms}.
\end{remark}
