...
Note that $f$ is a nonzerodivisor on $S_{(F)}$ since
...
case that $IS_{(F)}$ is generated by a single nonzerodivisor as desired.
\end{proof}
...
$$
and $C_0 = B_0/J_0$. The image $\overline{h}_0$ of $h_0$ is a nonzerodivisor
in $C_0$ (see
More on Algebra, Lemma \ref{more-algebra-lemma-divide-by-torsion}).
As $h$ is a nonzerodivisor in $B$ we see that
