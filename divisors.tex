...
$\mathcal{O}_X$-module.
\begin{enumerate}
\item We say a local section $s \in \mathcal{F}(U)$ is {\it torsion}
if its image in every stalk $\mathcal{F}_x$, $x \in U$ is torsion.
...
for coherent modules on locally Noetherian schemes.
...
Let $X$ be a Noetherian scheme. Let $D \subset X$ be a closed subscheme
...
Then $f$ is a nonzero divisor on a neighbourhood of $x$ by
