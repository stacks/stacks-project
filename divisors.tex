...
\medskip\noindent
Set $f_0 : X_0 \to S_0$ equal to $\text{Spec}$ of the
ring map $A_0 \to C_0$. Set $D_0 = \text{Spec}(C_0/\overline{h}_0C_0)$.
...
\begin{proof}
Choose affine open neighbourhoods $\text{Spec}(A)$ of $s$ and
$\text{Spec}(B)$ of $x$ such that
$\varphi(\text{Spec}(B)) \subset \text{Spec}(A)$.
...
The assumption in (1) means that, after shrinking $\text{Spec}(B)$, we may
...
$D = V(f) \subset \text{Spec}(B_g)$ is a relative effective Cartier
divisor.
...
$D_0 = V(f_0) \subset \text{Spec}((B_0)_{g_0})$ is a relative effective
...
and
More on  Algebra, Lemma \ref{more-algebra-lemma-H1-regular-quasi-regular}
\end{proof}
...
Let $i : Z \to X$ be an immersion of schemes.
Choose an open subscheme such that $i$ identifies $Z$ with a closed
...
Let $i : Z \to X$ be an immersion of schemes ringed space.
We have the following implications:
...
by the $f_i$ and the $g_j$. Now suppose that $x \in I \cap J^2$. Because
$x \in J^2$ can write
$$
x =
\sum a_{i_1i_2} g_{i_1}g_{i_2} +
\sum a_{ij} g_if_j +
\sum\nolimits_{j_1 \leq j_2} a_{j_1j_2} f_{j_1}f_{j_2}
$$
and we see that $y = \sum\nolimits_{j_1 \leq j_2} a_{j_1j_2} f_{j_1}f_{j_2}$
is in $I$. Consider the following commutative diagram
$$
\xymatrix{
0 \ar[r] &
I \ar[r] &
...
I^{\oplus r} \ar[r] \ar[u] &
A^{\oplus r} \ar[r] \ar[u] &
(A/I)^{\oplus r} \ar[r] \ar[u] &
0 \\
0 \ar[r] &
\wedge^2(I^{\oplus r}) \ar[r] \ar[u] &
\wedge^2(A^{\oplus r}) \ar[r] \ar[u] &
\wedge^2((A/I)^{\oplus r}) \ar[r] \ar[u] &
0
}
$$
with exact rows and each column the Koszul complex on $f_1, \ldots, f_r$.
By assumption the complex on the right has trivial cohomology in the
spot $(A/I)^{\oplus r}$. Hence by the long exact cohomology sequence we see
that
$$
\text{Coker}(I^{\oplus r} \to I)
\longrightarrow
\text{Coker}(A^{\oplus r} \to A)
$$
is injective. This implies that $y$ is contained in the image of
$I^r \to I$, i.e., $y \in IJ$, i.e., $x \in IJ$ and we win.
\end{proof}

\noindent
The following lemma does not hold for quasi-regular immersions, see
Algebra, Remark \ref{algebra-remark-join-quasi-regular-sequences}.

\begin{lemma}
\label{lemma-composition-regular-immersion}
Let $i : Z \to Y$ and $j : Y \to X$ be morphisms of schemes.
\begin{enumerate}
...
\section{Regular immersions and morphisms}
...
Moreover, a relative $H_1$-regular immersion is a
relative quasi-regular immersion, see
Lemma \ref{lemma-regular-quasi-regular-immersion}.
...
to the case that $X = \text{Spec}(B)$ and $S = \text{Spec}(A)$ are affine
...
$H_1(K_\bullet(B_0, f_{0, 1}, \ldots, f_{0, r}))$ may not be zero.
On the other hand, as $B_0$ is Noetherian, it is a finitely
generated $B_0$-module. Let $\xi_1, \ldots, \xi_n$ be generators.
Let $A_0 \subset A_1 \subset A$ be a larger finite type
...
$$
\text{colim}_{A_0 \subset A_1 \subset A}\ 
H_1(K_\bullet(B_1, f_{1, 1}, \ldots, f_{1, r}))
= 0
$$
by
...
$X_1 \to S_1$ equal to $\text{Spec}$ of the
ring map $A_1 \to B_1$ and
$Z_1 = \text{Spec}(B_1/(f_{1, 1}, \ldots, f_{1, r}))$.
...
where $c = \dim_x(X_s) - \dim_x(T_s)$, then we can choose $U$ and $Z$
such that $T \cap U = Z$.
\end{enumerate}
...
Lemma \ref{lemma-fibre-Cartier}
it follows that $Z_1 = V(f_1) \subset \text{Spec}(B)$
...
$Z \to \text{Spec}(B)$ is a regular immersion and $Z$ is
flat over $S$, in particular $Z \to \text{Spec}(B)$ is
a relative quasi-regular immersion over $\text{Spec}(A)$.
This proves (1).
