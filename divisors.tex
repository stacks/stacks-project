...
\begin{proof}
In this proof, for a sheaf of modules $\mathcal{F}$, use $\mathcal{F}^*$ to denote
its dual. So $\mathcal{O}_S(D) = \mathcal{I}_D^*$.
Let $\phi: \mathcal{I}_{D_1} \otimes \mathcal{I}_{D_2} \to
\mathcal{I}_D$ be a morphism of $\mathcal{O}_S$ modules that maps 
$x \otimes y$ to $xy$ on each open subset of $S$. Let $\psi$ be the
natural inclusion $\mathcal{I}_{D_1}^* \otimes \mathcal{I}_{D_2}^* \to
(\mathcal{I}_{D_1} \otimes \mathcal{I}_{D_2})^*$. We will prove that
$\phi, \psi$ are isomorphisms, and $\Phi = (\phi^*)^{-1}\psi$ is the unique
isonorphism that maps $1_{D_1} \otimes 1_{D_2}$ to $1_D$. 

For any $x \in S$, pick an affine open neighborhood $U = \Spec(A)$ and
$f,g \in A$ such that $D_1, D_2$ are cut out by $f,g$ on $U$. Suppose
$x$ corresponds to prime ideal $\mathfrak{p}$ of $A$. Then
$(\mathcal{I}_{D_1})_x, (\mathcal{I}_{D_2})_x, (\mathcal{I}_{D})_x$
are free $A_{\mathfrak{p}}$-mod of rank 1 generated by $f,g,fg$,
respectively. $\phi(f \otimes g) = fg, \psi(f^* \otimes g^*) =
(f\otimes g)^*$, so $\phi_x, \psi_x$ are isomorphisms for each $x$. 
So $\phi$ (and thus $\phi^*$) and $\psi$ are
isomorphisms. 

To see that $\Phi(1_{D_1} \otimes 1_{D_2}) = 1_D$, again check on each
stalk. (In the above notation, this is to check that $\Phi_x(fg \cdot f^*\otimes g^*) = fg
\cdot (fg)^*$.) To prove uniqueness, observe
that $fg$ is a nonzerodivisor and $f^* \otimes g^*$ is the generator
of the stalk, so the map on each stalk is uniquely determined. 
Thus the map $\mathcal{O}_{D_1} \otimes_{\mathcal{O}_S}
\mathcal{O}_{D_2} \to \mathcal{O}_D$ is uniquely determined by the
condition $1_{D_1} \otimes 1_{D_2} \mapsto 1_D$.


%% $(1_{D_1} \otimes 1_{D_2})_x = f \cdot f^* \otimes g \cdot g^*$.
%% $(1_D)_x = fg \cdot (fg)^*$. (View $f^*, g^*, (fg)^*$ as their image
%% after localization at $\mathfrak{p}$.) So $1_{D_1} \otimes 1_{D_2}$ maps to $1_D$
%% at each stalk. Hence $1_{D_1} \otimes 1_{D_2} \to 1_D$.
%% 
%% To prove uniqueness, observe that $fg$ is a nonzerodivisor in
%% $A_\mathfrak{p}$, and $f^* \otimes g^*$ is the generator of the stalk
%% of $(\mathcal{I}_{D_1})^* \otimes (\mathcal{I}_{D_2})^*$ at $x$. So
%% the image of $fg \cdot f^* \otimes g^*$ uniquely determines the map
%% $(\mathcal{I}_{D_1})^* \otimes (\mathcal{I}_{D_2})^* \to
%% (\mathcal{I}_D)_x$. So the map $\mathcal{O}_{D_1} \otimes
%% \mathcal{O}_{D_2} \to \mathcal{O}_D$ is uniquely determined by the
%% condition $1_{D_1} \otimes 1_{D_2} \to 1_D$.
