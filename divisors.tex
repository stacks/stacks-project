...
Let $S$ be a scheme. Let $\mathcal{F}$ be a finite quasi-coherent
...
$\text{Fit}_k(\mathcal{F}) = R$ for $k \geq r$.
\end{enumerate}
\end{lemma}
...
closed subscheme cut out by functions which are affine locally
...
that this commutes with base change. The notion of a
strictly standard element was useful in the chapter on Popescu's theorem.
But the closed subscheme defined by these elements is (as far as we know)
not used in the literature\footnote{If $f$ is a
...
of $\Omega_{X/S}$ is used to get a good closed subscheme. Moreover, in general
the closed subschemes of $X$ defined by the fitting ideals of $\Omega_{X/S}$
define a stratification of $X$ in terms of the rank of $\Omega_{X/S}$
which can be helpful; it is related to embedding dimensions of
...
\item $\text{depth}(\mathcal{O}_{X, x}) \geq 2$ and
...
for all $x \in X$. If $\dim(\mathcal{O}_{X, x}) = 0$, then
...
$D \subset S$ whose ideal sheaf $\mathcal{I}_D \subset \mathcal{O}_X$
...
Then the assigment $(D, D') \mapsto \mathcal{O}_X(D - D')$ defines
a group homomorphism
$$
\text{EffCart}(S)^{gp} \longrightarrow \text{Pic}(S)
$$
where the left hand side is the group completion of
$\text{EffCart}(S)$. In other words, when we write $\mathcal{O}_X(D - D')$
...
componet of $D$ signifies $\mathfrak p$ is minimal over $(f)$.
...
The puctured spectrum $U = \Spec(A) \setminus \{\mathfrak m\}$
...
On the other hand $\Spec(B/fB)$ is the puctured spectrum of $\Spec(A/fA)$.
...
abelian groups
$$
\text{Norm}_\pi : \text{Pic}(X) \to \text{Pic}(Y)
$$
...
multipliciation by $f$ defines a $\mathcal{O}_V$-linear endomorphism
...
then the map (\ref{equation-c1}) $\text{Pic}(X) \to \text{Cl}(X)$
is injective.
\end{lemma}
...
meromorphic section of $\mathcal{L}$. The assuption means that
...
In order to prove this we may work locall on $X$.
...
(\ref{equation-c1}) $\text{Pic}(X) \to \text{Cl}(X)$.
The following are equivalent
\begin{enumerate}
\item the local rings of $X$ are UFDs, and
\item $X$ is normal and $\text{Pic}(X) \to \text{Cl}(X)$
is surjective.
\end{enumerate}
In this case $\text{Pic}(X) \to \text{Cl}(X)$ is an isomorphism.
\end{lemma}
...
sheaf $\mathcal{L}$, a regular meromorphic sectio $s$, and $f \in R(X)^*$
...
be the mininal primes over $rr'$. Each $R_{\mathfrak p_i}$
is a discrete valuation ring
...
By faithfull flatness of $R \to A$ we find that
...
$\mathcal{L}'$ on the basis of quasi-compact opens of $U'$ which is sufficent
...
$\text{Pic}(\mathbf{A}^n_R) \to \text{Pic}(U)$ is surjective
by Lemma \ref{lemma-extend-invertible-module}.
The vanishing of $\text{Pic}(\mathbf{A}^n_R)$ is equivalent to
...
$$
\mathbf{Z} \longrightarrow \text{Pic}(\mathbf{P}^n_R),\quad
...
local rings are UFDs and we see that $\text{Pic}(X) = \text{Cl}(X)$
...
However, once (1) and (2) are esthablished, we see that
...
$a$ in $A' = A[\frac{I}{a}]$ one direct is clear. Conversely, assume
...
dominated by a blow up (More on Flatness, Lemma
...
there exists a proper morphism $p : X' \to X$ and an
...
Since $X$ is Noetherian, so is $U$
(Properties, Lemma \ref{properties-lemma-locally-closed-in-Noetherian})
and hence $(f, j)$ is quasi-compact
(Properties, Lemma
\ref{properties-lemma-morphism-Noetherian-schemes-quasi-compact}).
The projection $g : Y \times_S X \to X$ is proper
(Morphisms, Lemma \ref{morphisms-lemma-base-change-proper}).
Since $(f, j)$ defines a section of the proper hence separated
morphism $g^{-1}(U) \to U$, it defines a closed immersion $U \to g^{-1}(U)$
(Schemes, Lemma \ref{schemes-lemma-section-immersion}).
We conclude that $(f, j)$ induces an isomorphism of $U$ with
the open subscheme $X' \cap g^{-1}(U)$ of $X'$ (Morphisms, Lemma
\ref{morphisms-lemma-quasi-compact-scheme-theoretic-image}).
The composition $p : X' \to X$ of $X' \to Y \times_S X$
and $g$ is proper
(Morphisms, Lemmas \ref{morphisms-lemma-closed-immersion-proper} and
\ref{morphisms-lemma-composition-proper})
and $p^{-1}(U) \to U$ is an isomorphism by the above.
