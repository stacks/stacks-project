...
there are canonical injective maps
$$
...
Proof of reduction step omitted. Let $I = \{x \in A \mid x(a/b) \in A\}$
...
map that gives rise to the map $1$ if $\mathcal{F} = \widetilde{M}$.
Consider on the other hand the map
$\sigma : IM \to M_b, x \mapsto ax/b$.
Since $b$ is not a zero divisor we have $M \subset M_b$.
...
It is injective because $a$ is a nonzero divisor also.
