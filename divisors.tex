...
$$
C_ZX = \underline{\text{Spec}}_Z(\mathcal{C}_{Z/X, *})
$$
see
Constructions,
...
$$
N_ZX = \underline{\text{Spec}}_Z(\text{Sym}(\mathcal{C}_{Z/X}))
$$
see
Constructions,
...
A slightly weaker condition is to require that,
with $J = (f_1, \ldots, f_r)$, the map
$$
R/J[T_1, \ldots, T_r]
\longrightarrow
\bigoplus\nolimits_{n \geq 0}
...
{\it quasi-regular sequence}\footnote{There is also
a notion of a Koszul-regular sequence, see
Algebra, Remark \ref{algebra-remark-koszul-regular}.}, see
...
Let $\mathcal{J}$ be a sheaf of ideals.
If $\mathcal{J}$ is regular then it is quasi-regular.
\end{lemma}

\begin{proof}
The lemma immediately reduces to proving that a regular sequence
on a ringed space is a quasi-regular sequence.
Let $f_1, \ldots, f_r \in \Gamma(X, \mathcal{O}_X)$ generate $\mathcal{J}$.
Let $x$ be a point of the support of $\mathcal{O}_X/(f_1, \ldots, f_r)$.
If $f_1, \ldots, f_r$ is a regular sequence then the maps
$$
f_i :
...
\mathcal{O}_{X, x}/(f_1, \ldots, f_{i - 1})
$$
are injective. In other words, the image of the sequence
$f_1, \ldots, f_r$ in the ring $\mathcal{O}_{X, x}$ is a regular sequence.
Hence by
Algebra, Lemma \ref{algebra-lemma-regular-quasi-regular}
the image of the sequence
$f_1, \ldots, f_r$ in the ring $\mathcal{O}_{X, x}$ is a quasi-regular
sequence. Thus the map (\ref{equation-map-quasi-regular}) induces an
isomorphism on stalks at $x$. Since this works for every point
in the support of $\mathcal{O}_X/(f_1, \ldots, f_r)$ we conclude
that $f_1, \ldots, f_r$ is a quasi-regular sequence.
\end{proof}

\begin{lemma}
\label{lemma-regular-ideal-sheaf-quasi-coherent}
Any regular or quasi-regular sheaf of ideals on a scheme
is a finite type quasi-coherent sheaf of ideals.
\end{lemma}
...
Then $\mathcal{J}$ is regular (resp.\ quasi-regular) if and only if
for every $x \in \text{Supp}(\mathcal{O}_X/\mathcal{J})$ there exists
an affine open neighbourhood $x \in U \subset X$, $U = \text{Spec}(A)$
such that $\mathcal{J}|_U = \widetilde{I}$ and such that $I$
is generated by a regular (resp.\ quasi-regular) sequence
$f_1, \ldots, f_r \in A$.
\end{lemma}

\begin{proof}
By assumption we can find an open neighbourhood $U$ of $x$ over which
$\mathcal{J}$ is generated by a regular (resp.\ quasi-regular)
sequence $f_1, \ldots, f_r \in \mathcal{O}_X(U)$. After shrinking
$U$ we may assume that $U$ is affine, say $U = \text{Spec}(A)$.
...
(resp.\ (\ref{equation-map-quasi-regular}) is an isomorphism)
implies the correponding property of the map
$A/(f_1, \ldots, f_{i - 1}) \to A/(f_1, \ldots, f_{i - 1})$
(resp.\ the map $A/I[T_1, \ldots, T_r] \to \bigoplus I^n/I^{n + 1}$).
Thus $f_1, \ldots, f_r \in A$ is a regular (resp.\ quasi-regular)
...
$\mathcal{O}_{X, x}$,
\item there exists an affine neighbourhood $U = \text{Spec}(A)$ of $x$ such
...
that (3) $\Leftrightarrow$ (5) and (4) $\Leftrightarrow$ (6).
It is clear that (3) $\Rightarrow$ (1) and (4) $\Rightarrow$ (2).
We have (1) $\Rightarrow$ (3) by
Algebra, Lemma \ref{algebra-lemma-regular-sequence-in-neighbourhood}.
We have (2) $\Rightarrow$ (4) by
Algebra, Lemma \ref{algebra-lemma-quasi-regular-sequence-in-neighbourhood}.
The implication (1) $\Rightarrow$ (2) is
Algebra, Lemma \ref{algebra-lemma-regular-quasi-regular}.
Finally, the only interesting implication is (2) $\Rightarrow$ (1) which
is equivalent to
Algebra, Lemma \ref{algebra-lemma-quasi-regular-regular}.
\end{proof}

\begin{definition}
\label{definition-regular-closed-immersion}
Let $Z \to X$ be a closed immersion of schemes.
\begin{enumerate}
\item We say that $Z \to X$ is a {\it regular closed immersion} if the
quasi-coherent ideal sheaf $\mathcal{I} \subset \mathcal{O}_X$
corresponding to $Z$ is a regular ideal sheaf.
\item We say that $Z \to X$ is a {\it quasi-regular closed immersion}
if the quasi-coherent ideal sheaf $\mathcal{I} \subset \mathcal{O}_X$
corresponding to $Z$ is a quasi-regular ideal sheaf.
\end{enumerate}
\end{definition}
...
that $\mathcal{I}$ is a (quasi-)regular ideal if and only if
$\mathcal{I}'$ is a (quasi-)regular ideal. Thus whether or not the
factorization $Z \to U$ is a (quasi-)regular immersion is independent
of the choice of $U$.

\begin{definition}
\label{definition-regular-immersion}
Let $f : Z \to X$ be an immersion of schemes.
\begin{enumerate}
\item We say $f$ is a {\it regular immersion} if there exists a
factorization $f = j \circ i$ where $j$ is an open immersion and
$i$ is a regular closed immersion.
\item We say $f$ is a {\it quasi-regular immersion} if there exists a
factorization $f = j \circ i$ where $j$ is an open immersion and
$i$ is a quasi-regular closed immersion.
\end{enumerate}
\end{definition}
...
Assume that $J/I \subset A/I$ is generated by a quasi-regular sequence.
...
that $I \cap J^2 = IJ$. To prove this choose $f_1, \ldots, f_r \in J$
whose images in $A/I$ form a quasi-regular sequence which generates $J/I$.
Also, choose $g_j \in I$ which generate $I$. Then $J$ is generated
by the $f_i$ and the $g_j$. Now suppose that
$x \in I \cap J^2$. Then we can write
$$
x =
\sum a_{i_1i_2} g_{i_1}g_{i_2} +
\sum a_{ij} g_if_j +
\sum_{j_1 \leq j_2} a_{j_1j_2} f_{j_1}f_{j_2}
$$
Because $x \in I$ we see that the image of $x$ in $A/I$ is zero.
On the other hand, this is the term $\sum a_{j_1j_2} f_{j_1}f_{j_2}$.
Since $J^2/(I + J^3)$ is free over $A/J$ with basis $f_{j_1}f_{j_2}$,
$j_1 \leq j_2$ as $f_1, \ldots, f_r$ is quasi-regular
we see that $a_{j_1j_2} \in J$.
