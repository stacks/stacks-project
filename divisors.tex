...
Let $X$ be a locally Noetherian scheme having no embedded components.
Then $\mathcal{K}_X$ is quasi-coherent and for every quasi-coherent
$\mathcal{O}_X$-module, the sheaf $\mathcal{K}_X(\mathcal{F})$
of meromorphic section of $\mathcal{F}$ is quasi-coherent.
\end{lemma}

\begin{proof}
Let $X^0 \subset X$ be the set of generic points of irreducible components
of $X$. For each $\eta \in X^0$ the spectrum of the local ring
$\mathcal{O}_{X, \eta}$ is a singleton. Consider the morphism
$$
j :
Y = \coprod\nolimits_{\eta \in X^0} \Spec(\mathcal{O}_{X, \eta})
\longrightarrow
X
$$
whose components are the morphisms discussed in Schemes, Section
\ref{schemes-section-points}.
By Lemma \ref{lemma-pullback-meromorphic-sections-defined}
pullbacks of meromorphic functions along $j$ are defined.
Clearly $\mathcal{O}_Y = \mathcal{K}_Y$.
Thus we see that we obtain a map
$$
\mathcal{K}_X \longrightarrow j_*\mathcal{O}_Y
$$
Since $j$ is quasi-compact and quasi-separated, we see that it suffices
to show that this map is an isomorphism (by
Schemes, Lemma \ref{schemes-lemma-push-forward-quasi-coherent}).
We may check this on affine opens $U = \Spec(A) \subset X$.
The left hand side evaluates to $Q(A)$ by
Lemma \ref{lemma-locally-Noetherian-K}.
The right hand side evaluates to
$\prod_{\mathfrak p \subset A\text{ minimal}} A_\mathfrak p$.
Since $A$ has no embedded associated primes
we see that
$(0) = \bigcup_{\mathfrak p \subset A\text{ minimal}} \mathfrak p$.
...
Let $\mathcal{L}$ be an invertible $\mathcal{O}_X$-module.
Then $\mathcal{L}$ has a regular meromorphic function.
\end{lemma}

\begin{proof}
The isomorphism $\mathcal{K}_X = j_*\mathcal{O}_Y$
given in the proof of Lemma \ref{lemma-quasi-coherent-K}
shows that $\Gamma(X, \mathcal{K}_X(\mathcal{L}))$
is equal to $\prod_{\eta \in X^0} \mathcal{L}_\eta$.
Thus if we pick $s = \prod s_\eta$, with
$s_\eta$ a generator of the stalk $\mathcal{L}_\eta$,
then we see that $s$ is a regular meromorphic section.
Some details omitted.
\end{proof}
...
\medskip\noindent
Suppose $X$ is a scheme. Let $X^0 \subset X$ be the set of
generic points of irreducible components of $X$. For each $\eta \in X^0$
...
=
\prod\nolimits_{\eta \in X^0} j_{\eta, *}(\mathcal{L}_\eta).
$$
...
\medskip\noindent
We claim that in case (2) the map $\varphi$ is an isomorphism
for any invertible sheaf $\mathcal{L}$.
Before proving this let us show that case (2) follows from this.
...
clear that the global section $s = \prod_{\eta \in X^0} s_\eta$
...
The set of minimal primes of $A$ is finite by assumption, say
$\mathfrak q_1, \ldots, \mathfrak q_t$,
and the set of zerodivisors of $A$ is equal to
$\mathfrak q_1 \cup \ldots \cup \mathfrak q_t$ as $A$ is reduced
by assumption. Hence the result follows from
Algebra, Lemma \ref{algebra-lemma-total-ring-fractions-no-embedded-points}.
\end{proof}
