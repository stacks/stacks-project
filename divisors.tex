...
quasi-coherent $\mathcal{O}_X$-modules.
If $\mathcal{G}$ is reflexive and $\mathcal{F}$ is coherent,
...
Then for the conormal sheaf we have $\mathcal{C}_{D/S} = \mathcal{I}_D|D =
\mathcal{O}_S(D)^{\otimes -1}|_D$.
\end{lemma}
...
be a quasi-coherent sheaf of ideals. Assume at least one of the following
conditions holds
\begin{enumerate}
\item There exist reduced and irreducible effective Cartier divisors
$D_i \subset X$, $i = 1, \ldots, n$ and a closed subset $Z \subset X$
of codimension $\geq 2$, such that $\mathcal{O}_X/\mathcal{I}$
is supported on $Z \cup \bigcup D_i$.
\item The local ring $\mathcal{O}_{X, x}$ is a UFD for every point
$x$ of the support of $\mathcal{O}_X/\mathcal{I}$.
\item The scheme $X$ is regular.
\end{enumerate}
Then there exists an invertible ideal sheaf
$\mathcal{I} \subset \mathcal{J} \subset \mathcal{O}_X$
such that the support of $\mathcal{J}/\mathcal{I}$
has codimension $\geq 2$. Moreover, in case (1) we have
$\mathcal{J}^{\otimes -1} = \mathcal{O}_X(\sum a_i D_i)$
for some $a_i \geq 0$.
\end{lemma}

\begin{proof}
Case (1). Let $\xi_i \in D_i$ be the generic point and let
...
$\mathcal{I}_{x_i} \subset \mathcal{O}_i$.
We claim that the ideal sheaf $\mathcal{J}$
of the effective Cartier divisor $D = \sum a_i D_i$ works.

\medskip\noindent
Namely, suppose that $x \in D$. Let $A = \mathcal{O}_{X, x}$.
Let $f_i \in A$ be a local equation for $D_i$;
we only consider those $i$ such that $x \in D_i$. Then $f_i$ is
a nonzerodivisor and $A/(f_i)$ is a domain and
$\mathcal{O}_i = A_{(f_i)}$. Let
$I = \mathcal{I}_x \subset A$. We chose $a_i$ such that
$I A_{(f_i)} = f_i^{a_i}A_{(f_i)}$. It follows that
$I \subset (\prod f_i^{a_i})$ because $(\prod f_i^{a_i})$
is the kernel of $A \to \prod A_{(f_i)}/f_i^{a_i}A_{(f_i)}$.
This proves that $\mathcal{I} \subset \mathcal{J}$.
Moreover, we also see that $\mathcal{I}_{x_i} = \mathcal{J}_{x_i}$
which proves that $x_i$ is not in the support of $\mathcal{J}/\mathcal{I}$.
Hence the support of $\mathcal{I}/\mathcal{J}$ has codimension
at least $2$. This finishes the proof in case (1).

\medskip\noindent
Observe that (3) is a special case of (2) because a regular local
ring is a UFD (More on Algebra, Lemma
\ref{more-algebra-lemma-regular-local-UFD}).
In case (2) let $D_i$ be the irreducible components of the support
of $\mathcal{O}_X/\mathcal{I}$ which have codimension $1$.
By Lemma \ref{lemma-weil-divisor-is-cartier-UFD} each $D_i$
is an effective Cartier divisor. In this way we reduce to case (1).
\end{proof}
...
Since $A$ is reduced this implies that $f$ is a nonzero divisor.
...
$\mathcal{O}_X$-module $\mathcal{L}$. Namely, we will see below that there
exists an open covering $Y = \bigcup V_j$ such that
...
\medskip\noindent
Pick $y \in Y$. We have to show there exists an affine open neighbourhood
...
This is possible by the claim in the proof of
Lemma \ref{lemma-norm-invertible}.
...
Assume that $X$ has an ample invertible $\mathcal{O}_X$-module.
If either
\begin{enumerate}
