...
$z : \Spec(k) \to \mathcal{X}$ where $k$ is a field such that
$x = f(z)$.
\end{enumerate}
...
that $\mathcal{Z}_x$ is a gerbe. It turns out that $\mathcal{Z}_x$
...
Let $\mathcal{X}$ be a locally Noetherian algebraic stack over
a scheme $S$.
Let $x \in |\mathcal{X}|$ be a point of $\mathcal{X}$.
Let $[U/R]\to \mathcal{X}$ be a presentation, and let $u\in U$
be a point that maps to $x$. Let $e\colon U \rightarrow R$ be the ``identity''
map and let $s\colon R\to U$ be the ``source''
map, which is a smooth morphism of algebraic spaces. Let $R_u$
be the fiber of $s\colon R\to U$ over $u$.
...
This is independent of the choice of presentation and the point
$u$ over $x$,
by Lemma
...
or more generally when $\mathcal{X}$ is an algebraic space
(Definition
\ref{spaces-properties-definition-dimension-at-point}).
...
if $\mathcal{X}$ is a scheme (Lemma \ref{properties-lemma-dimension})
or an algebraic space (Definition
...
then $\dim(\mathcal{X})$ is an integer. For an arbitrary algebraic
stack $\mathcal{X}$,
