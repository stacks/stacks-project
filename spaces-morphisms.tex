...
where $\xi \in |X|$ runs over the points specializating to $x$ at
...
Any $x' \in |X'|$ with $\dim(\mathcal{O}_{X', \overline{x}'}) = 0$
(Properties of Spaces, Definition
\ref{spaces-properties-definition-dimension-local-ring}), then
$x'$ is the image of some $y \in |Y|$ with
$\dim(\mathcal{O}_{Y, \overline{y}}) = 0$.
\end{lemma}
...
the sets of points whose local rings have dimension $0$ (Properties of Spaces,
...
and that for a geometric point $\overline{z}$ of $Z$ we have:
$\dim(\mathcal{O}_{Z, \overline{z}}) = 0 \Rightarrow
\dim(\mathcal{O}_{X, f(\overline{z})}) = 0$. Then
