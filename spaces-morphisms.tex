...
Properties of Spaces,
Lemma \ref{spaces-properties-lemma-descent-universally-bounded}.
...
\medskip\noindent
Let $S$ be a scheme.
Let $f : X \to Y$ be a representable, quasi-compact, and quasi-separated
morphism of algebraic spaces over $S$. Let $V$ be a scheme
...
morphism of schemes, by the remark
of the preceding paragraph we may compute $f'_*(\mathcal{F}|_U)$
by thinking of $\mathcal{F}|_U$ as a quasi-coherent sheaf on the scheme
$U$, and $f'$ as a morphism of schemes. We will frequently use this
without further mention.
...
By
Etale Cohomology,
Lemma \ref{etale-cohomology-proposition-closed-immersion-pushforward}
the map
...
$$
\mathcal{F} \to i_*(i^{-1}\mathcal{F}
\otimes_{i^{-1}\mathcal{A}} \mathcal{B}).
$$
This map is $\mathcal{A}$-linear where $\mathcal{A}$ acts on the right hand
side via $\varphi$. Note that the sheaf on the right hand side of the arrow
...
$i^{QCoh}_* : \text{QCoh}(\mathcal{O}_Z)
\to \text{QCoh}(\mathcal{O}_X)$.
...
$\text{Spec}(K) \to Y$ where $K$ is a field the reduction
$(\text{Spec}(K) \times_Y X)_{red}$ is either empty or
...
$\text{Spec}(K) \to Y$ there is at most one lift of this morphism
...
morphism of algebraic spaces over $S$.
\begin{enumerate}
\item If $f$ is decent then $f$ is radical.
...
the morphism $(\text{Spec}(K) \times_Y X)_{red} \to \text{Spec}(K)$
...
over field whose structure morphism $X \to \text{Spec}(K)$ is universally
injective and has $\mathcal{P}$ is representable by the spectrum of a field.
Namely, then $X \to \text{Spec}(K)$ will be a morphism of schemes and
...
Lemmas \ref{lemma-base-change-relative-conditions},
\ref{lemma-composition-relative-conditions}, and
\ref{lemma-properties-trivial-implications} (to see that an immersion is
decent) we see that the discussion in the first pararagraph applies.
Let $X$ be a nonempty decent reduced algebraic space
universally injective over a field $K$. In particular we see that $|X|$
is a singleton. By
Properties of Spaces, Lemma \ref{spaces-properties-lemma-when-field}
we conclude that $X \cong \text{Spec}(L)$ for some extension
...
\ref{lemma-composition-separated}, and
\ref{lemma-immersions-monomorphisms}.
...
U \times_X U
\longrightarrow
U \times_{\text{Spec}(K)} U
$$
As $X \to \text{Spec}(K)$ is universally injective $j$ is surjective,
and as $X \to \text{Spec}(K)$ is locally separated $j$ is an immersion.
...
$\text{Spec}(K) \to Y$ where $K$ is a field
\begin{enumerate}
\item the space $|\text{Spec}(K) \times_Y X|$ is a singleton,
\item there exists a monomorphism
$\text{Spec}(L) \to \text{Spec}(K) \times_Y X$, and
\item $K \subset L$ is purely inseparable.
\end{enumerate}
(For example if $X \to Y$ is a surjective weakly radicial morphism
then $X(k) \to Y(k)$ is surjective for every algebraically closed field
$k$.) If needed later we will may call such a morphism {\it weakly radicial}.
Note that the base change
$X_{\overline{\mathbf{Q}}} \to \text{Spec}(\overline{\mathbf{Q}})$
of the morphism in Example \ref{example-universally-injective-not-radicial}
...
\noindent
Given a property of morphisms of schemes which is etale local on the
source-and-target we may use it to define a corresponding
...
which is etale local on the source-and-target, see
...
\section{Local on source-and-target at a point}
\label{section-local-source-target-at-point}

\noindent
In this section we explain how to obtain a property of morphisms
of algebraic spaces at a point from a property of pointed morphisms
of schemes.

\begin{definition}
\label{definition-local-at-point}
Let $S$ be a scheme. Let $\mathcal{Q}(h, u)$ be a property of
pairs $(h : U \to V, u)$, where $h : U \to V$ is a morphism of schemes
over $S$, and $u \in U$ is a point. Let us say that $\mathcal{Q}$ is
{\it etale local on the source-and-target} if for any commutative
diagram
$$
\xymatrix{
U' \ar[r]_{h'} \ar[d] & V' \ar[d] \\
U \ar[r]^h & V
}
$$
with etale vertical arrows and a point $u' \in U'$ mapping to $u \in U$
we have
$$
\mathcal{Q}(h, u) \Leftrightarrow \mathcal{Q}(h', u')
$$
\end{definition}

\noindent
We can use this concept to define properties of morphisms of algebraic
spaces at a point. Namely, by imposing either of the two equivalent
...
Let $\mathcal{Q}$ be a property of pointed morphisms which is
etale local on the source-and-target as in
Definition \ref{definition-local-at-point}.
Consider commutative diagrams
$$
\xymatrix{
U \ar[d]_a \ar[r]_h & V \ar[d]^b \\
...
$$
where $U$ and $V$ are schemes and $a, b$ are etale.
The following are equivalent
\begin{enumerate}
\item for any diagram as above and $u \in U$ with $a(u) = x$ we have
$\mathcal{Q}(h, u)$, and
\item for some diagram as above and $u \in U$ with $a(u) = x$ we have
$\mathcal{Q}(h, u)$.
\end{enumerate}
If $X$ and $Y$ are representable, then this is also
equivalent to $\mathcal{Q}(f, x)$.
\end{lemma}
...
Let $S$ be a scheme. Let $\mathcal{Q}$ be a property of pointed morphisms
of schemes which is etale local on the source-and-target, see
Definition \ref{definition-local-at-point}.
...
\noindent
The following lemma should not be blindly used to go from a property of
morphisms to a property of morphisms at a point. For example if
$\mathcal{P}$ is the property of being flat, then the property
$Q(h, u)$ in the following lemma means ``$h$ is flat in a neighbourhood
of $u$'' which is not the same as ``$h$ is flat at $u$''.
...
Consider the property $\mathcal{Q}(h, u)$ of pairs $(h : U \to V, u)$,
where $h : U \to V$ is a morphism of schemes over $S$, and $u \in U$
is a point defined by the rule
$$
\mathcal{Q}(h, u)
\Leftrightarrow
\text{there exists an open neighbourhood }W\text{ of }u\text{ in }U
\text{ such that }h|_W\text{ has }\mathcal{P}
$$
Then $\mathcal{Q}(h, u)$ is etale local on the source-and-target as in
Definition \ref{definition-local-at-point}. Moreover, given a morphism
of algebraic spaces $f : X \to Y$ over $S$ and $x \in |X|$
the following are equivalent
\begin{enumerate}
\item for any diagram as in
Lemma \ref{lemma-local-source-target-at-point}
and $u \in U$ with $a(u) = x$ we have $\mathcal{Q}(h, u)$,
\item for some diagram as in
Lemma \ref{lemma-local-source-target-at-point}
and $u \in U$ with $a(u) = x$ we have $\mathcal{Q}(h, u)$, and
\item there is a Zariski open neighbourhood $X' \subset X$ of $x$
such that $X' \to Y$ has $\mathcal{P}$
(Definition \ref{definition-P}).
\end{enumerate}
And if $X$ and $Y$ are representable, then this is also
equivalent to $\mathcal{Q}(f, x)$.
\end{lemma}

\begin{proof}
We first prove that $\mathcal{Q}(h, u)$ is etale local on the
source-and-target. Namely, let
$$
\xymatrix{
U' \ar[r]_{h'} \ar[d]_a & V' \ar[d] \\
U \ar[r]^h & V
}
$$
and $u' \in U'$, $u \in U$ be as in
Definition \ref{definition-local-at-point}.
If $\mathcal{P}(h, u)$ holds, then $\mathcal{P}$ holds for
$h|_W$ for some neighbourhood $W$ of $u$. By
Lemma \ref{lemma-local-source-target}
this means that $\mathcal{P}$ holds for $h'|_{a^{-1}(W)}$.
Conversely, if $\mathcal{P}(h', u')$ holds, then $\mathcal{P}$ holds for
$h'|_{W'}$ for some neighbourhood $W'$ of $u'$. By
Lemma \ref{lemma-local-source-target}
this means that $h|_{a(W')}$ has $\mathcal{P}$.

\medskip\noindent
The equivalence of (1) and (2) was observed in
Lemma \ref{lemma-local-source-target-at-point}.
We see that (2) implies (3) by taking $X'$ to be the open subspace
associated to the image of $|U| \to |X|$. The implication
(3) $\Rightarrow$ (1) is clear from the definitions.
\end{proof}
...
consider a geometric point $\overline{s} : \text{Spec}(k) \to S$ of $S$
...
\noindent
In this section we briefly discuss what it means for a morphism
of algebraic spaces to be flat at a point.


\noindent
Using the lemma above we may define what it means for a morphism
to be flat at a point.
...
We say $f$ is {\it flat at $x$} if the equivalent conditions of
Lemma \ref{lemma-local-source-target-at-point}
hold for the property $\mathcal{P}(\psi, u)$
described in Lemma \ref{lemma-flat-at-point}.
\end{definition}

\noindent
We could also have defined flatness in terms of the map on etale local
rings (see
Lemma \ref{lemma-flat-at-point-etale-local-rings}).
We chose the construction above because it works in greater generality
(i.e., for more types of properties) than the approach using properties
of the induced map on etale local rings.

\begin{lemma}
\label{lemma-flat-flat-at-all-points}
Let $S$ be a scheme.
Let $f : X \to Y$ be a morphism of algebraic spaces over $S$.
The following are equivalent
\begin{enumerate}
\item $f$ is flat, and
\item $f$ is flat at all $x \in |X|$.
\end{enumerate}
\end{lemma}
...
\begin{proof}
Omitted. Hint: Argue as in the proof of
...
Lemma \ref{lemma-dimension-local-ring-fibre}.
\item We say the
{\it transcendence degree of $x/f(x)$} is $d$
if the equivalent conditions of
Lemma \ref{lemma-local-source-target-at-point}
hold for the property
$\mathcal{P}_d$ described in
Lemma \ref{lemma-transcendence-degree-at-point}.
\item We say the
...
hold for the property
$\mathcal{P}_d$ described in
Lemma \ref{lemma-dimension-at-point}.
\end{enumerate}
\end{definition}
...
applied to $\psi : U \to V$ and $u \in U$
as in
Lemma \ref{lemma-local-source-target-at-point}.
\end{proof}
...
\end{matrix}
$$
Moreover, equality holds if for some morphism $\text{Spec}(k) \to Z$
...
the set $U_n$ of points where $\psi$ has relative dimension
...
$\psi$ is locally quasi-finite if and only if $f$ is locally quasi-finite,
and that $f$ has relative dimension $0$ at all $x \in |X|$ if and
only if $\psi$ has relative dimension $0$ at all $u \in U$.
Hence the result follows from the result for $\psi$ which is
...
Lemma \ref{lemma-local-source-target}
with $\psi$ smooth and surjective vertical arrows. By
Morphisms, Lemma \ref{morphisms-lemma-smooth-locally-finite-presentation}
$\psi$ is locally of finite presentation. Hence $X \to Y$ is
...
\noindent
For a morphism of schemes the property of being unramified
(resp.\ G-unramified) is stable under base change
...
Let $S$ be a scheme.
\begin{enumerate}
\item A morphism of algebraic spaces $f : X \to Y$ is {\it unramified} if
the equivalent conditions of Lemma \ref{lemma-local-source-target} hold with
$\mathcal{P} = \text{unramified}$.
\item A morphism of algebraic spaces $f : X \to Y$ is {\it G-unramified} if
the equivalent conditions of Lemma \ref{lemma-local-source-target} hold with
$\mathcal{P} = \text{G-unramified}$.
\end{enumerate}
\end{definition}
...
Lemma \ref{lemma-local-source-target}.
Then all we are saying is that the morphism $\psi$ is
...
$$
over $S$. If $X \to B$ is universally closed and $Y \to B$ is
