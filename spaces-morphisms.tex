...
\end{enumerate}
In particular, if $X$ and $Y$ are representable, then this is also
equivalent to $f$ having property $\mathcal{P}$.
\end{lemma}
...
\noindent
For a morphism of schemes the property of being flat is etale local on
both the source and the target. See
Descent, Lemmas \ref{descent-lemma-descending-property-flat} and
\ref{descent-lemma-flat-fpqc-local-source}.
Hence we may define the notion of a flat morphism of algebraic spaces as
follows.
...
Since $\alpha : \text{Spec}(K') \to \text{Spec}(K)$ is a quasi-compact
flat surjection this implies there is
at most one lift of $y : \text{Spec}(K) \to Y$ to a morphism
into $X$ (see FIXME). Thus we see that (1) holds.
