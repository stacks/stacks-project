...
morphisms. For any $\mathcal{F} \in \textit{Mod}(\mathcal{O}_X)$
...
$f_* : \textit{QCoh}(X) \to \textit{QCoh}(Y)$ agrees with the
...
$X(K) = \text{Mor}_S(\Spec(K), X)$. We first translate the
...
$\Spec(k'_u)$ are (isomorphic to) objects of $\textit{Sch}_{fppf}$
