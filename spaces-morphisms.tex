...
Algebraic Spaces, Lemma \ref{spaces-lemma-composition-immersions}).
...
of $X$ and $Y$ as algebraic spaces over $\text{Spec}(\mathbf{Z})$, see
...
Let $S$ be a scheme. Let $p : X \to \text{Spec}(k)$ be a morphism of
algebraic spaces over $S$ where $k$ is a field. Then
$p : X \to \text{Spec}(k)$ is universally open.
\end{lemma}

\begin{proof}
Choose a scheme $U$ and a surjective \'etale morphism $U \to X$.
The composition $U \to \text{Spec}(k)$ is universally open (as a morphism
of schemes) by
Morphisms, Lemma \ref{morphisms-lemma-scheme-over-field-universally-open}.
Let $Z \to \text{Spec}(k)$ be a morphism of schemes. Then
$U \times_{\text{Spec}(k)} Z \to X \times_{\text{Spec}(k)} Z$ is surjective,
see
Lemma \ref{lemma-base-change-surjective}.
Hence the first of the maps
$$
|U \times_{\text{Spec}(k)} Z| \to |X \times_{\text{Spec}(k)} Z| \to |Z|
$$
...
\noindent
The following lemma guarantees in particular that a morphism
$X \to \text{Spec}(A)$ is quasi-compact as soon as
...
hence we may assume that $Y = \text{Spec}(B)$ for some ring $B$.
...
Let $T = \text{Spec}(B[a_i ; i \in I])$. Let $T_i = D(a_i) \subset T$.
...
$V(a_i; i \in I \setminus J) = \text{Spec}(B[a_j; j\in J])$
and the set of points of this scheme lying over $v$ is bijective
with $\text{Spec}(\kappa(v)[a_j; j \in J])$ and $g$ restricts to
...
Let $S$ be a scheme. Let $k$ be a field and let $Z \to \text{Spec}(k)$
be a monomorphism of algebraic spaces over $S$. Then either
$Z = \emptyset$ or $Z = \text{Spec}(k)$.
\end{lemma}
...
\ref{spaces-properties-proposition-very-reasonable-open-dense-scheme}.
By
Schemes, Lemma \ref{schemes-lemma-mono-towards-spec-field}
we see that either $Z' = \emptyset$ or $Z' \cong \text{Spec}(k)$.
In the first case we conclude that $Z = \emptyset$ and in the
second case we conclude that $Z' = Z = \text{Spec}(k)$
as $Z \to \text{Spec}(k)$ is a monomorphism which is an
isomorphism over $Z'$.
\end{proof}
...
a morphism $\text{Spec}(k) \to X$ which is locally of finite type.
...
\begin{enumerate}
\item There exists a morphism $\text{Spec}(k) \to X$
...
$\text{Spec}(\kappa(u)) \to U$ is of finite type, and $U \to X$ is
...
\medskip\noindent
Conversely, assume $\text{Spec}(k) \to X$ is locally of finite type and
represents $x$. Let $U \to X$ be a surjective \'etale morphism where $U$
is a scheme. By assumption $U \times_X \text{Spec}(k) \to U$ is locally
of finite type. Pick a finite type point $v$ of
$U \times_X \text{Spec}(k)$ (there exists at least one, see
...
$x : \text{Spec}(k) \to X$. Then $f \circ x$ is locally of finite type by
Lemma \ref{lemma-composition-finite-type}.
...
$\text{Spec}(k) \to Y$ which is locally of finite type.
As $f$ is surjective the algebraic space
$X_k = \text{Spec}(k) \times_Y X$ is nonempty, therefore
...
$z : \text{Spec}(k) \to Z$ where $k$ is a field,
\item $f$ is locally of finite type,
...
with $\varphi(u) = x$, see
Lemma \ref{lemma-identify-finite-type-points}.
Set $u = \text{Spec}(\kappa(u))$ as usual. The projection morphisms
...
Properties of Spaces, Remark \ref{spaces-properties-remark-recall})
...
Algebraic Spaces, Theorem \ref{spaces-theorem-presentation}
...
$\overline{x} : \text{Spec}(k) \to X$ over $x$ and
...
$x$ over $Z$.
\item Let $x : \text{Spec}(K) \to X$ be a representative of $x$. If
\begin{enumerate}
...
\end{matrix}
$$
Note that if $Y = \text{Spec}(k)$ is the spectrum of a field, then
...
that $z \in X' \times_Y \text{Spec}(k)$. Hence the lemma is clear.
\end{proof}
...
\'Etale Morphisms of Schemes, Theorem \ref{etale-theorem-etale-radiciel-open}.
(See also
Spaces, Lemma
\ref{spaces-lemma-representable-transformations-property-implication}
...
Here is the what it means for a morphism to be \'etale at a point.
...
space $\mathbf{A}^\infty = \text{Spec}(k[x_1, x_2, \ldots])$ modulo
