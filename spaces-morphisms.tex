...
$U \times_X \text{Spec}(k) \to U$ (there exists at least one, see
...
``locally finite type points''.} if the equivalent conditions of
...
$Z$ has at least one closed point (by
...
$|Z|$ is a singleton, and a morphism $f : Z \to X$ which is a monomorphism
and locally of finite type such that $\{x\} = |f|(|Z|)$.
\end{enumerate}
\end{lemma}
...
$|Z|$ is a singleton. Thus (2) holds.

\medskip\noindent
Conversely, if (2) holds then $x$ is a finite type point of $X$ by
Lemma \ref{lemma-finite-type-points-morphism}
(and
