...
shows that $\mathbf{A}^1_k/\mathbf{Z} \to \text{Spec}(k)$ is universally
...
the space $|X_k|$ is discrete. Here $X_k = \text{Spec}(k) \times_Y X$.
\end{enumerate}
\end{lemma}

\begin{proof}
Assume $f$ is locally quasi-finite. Let $\text{Spec}(k) \to Y$ be as
in (2). Choose a surjective \'etale morphism $U \to X$ where $U$ is a scheme.
Then $U_k = \text{Spec}(k) \times_Y U \to X_k$ is an
...
we see that $X_k \to \text{Spec}(k)$ is locally quasi-finite. By
definition this means that $U_k \to \text{Spec}(k)$ is locally
...
Morphisms, Proposition \ref{proposition-generic-flatness-reduced}
...
$\mathbf{A}^1_{\mathbf{Q}}/\mathbf{Z} \to \text{Spec}(\mathbf{Q})$
