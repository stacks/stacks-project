...
Descent, Lemma \ref{descent-lemma-flat-base-change-scheme-theoretic-image}.
...
Let $S$ be a scheme. Let $f : X \to Y$ be a morphism of algebraic spaces
over $S$. If $X$ is reduced, then the scheme theoretic image of $f$ is
the reduced induced algebraic space structure on $\overline{|f|(|X|)}$.
\end{lemma}

\begin{proof}
This is true because the reduced induced algebraic space structure on
$\overline{|f|(|X|)}$ is clearly the smallest closed subspace
...
only if $i$ is. By
Properties, Section \ref{spaces-properties-section-properties-modules}
