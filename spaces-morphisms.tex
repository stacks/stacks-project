...
spaces over $S$. For any sheaf $\mathcal{F}$ (of sets or abelian groups)
on $Z_{etale}$ the canonical map
$i_{small}^{-1}i_{small, *}\mathcal{F} \to \mathcal{F}$ is an isomorphism.
\end{lemma}
...
we have
$$
(i_{small}^{-1}i_{small, *}\mathcal{F})|_V =
(i')_{small}^{-1}i'_{small, *}(\mathcal{F}|_V)
$$
More on Morphisms,
Lemma \ref{etale-cohomology-proposition-closed-immersion-pushforward}
states that the map
$(i')_{small}^{-1}i'_{small, *}(\mathcal{F}|_V) \to \mathcal{F}|_V$
is an isomorphism. Since $V \to Z$ is surjective and etale this implies
that $i_{small}^{-1}i_{small, *}\mathcal{F} \to \mathcal{F}$ is an
isomorphism.
\end{proof}
...
Let $U$ be a scheme and let $U \to X$ be a surjective etale morphism.
Set $V = Z \times_X U$. Then $V$ is a scheme and $i' : V \to U$ is
a closed immersion of schemes. Given a map of sheaves over $X$ (resp.\ $Z$)
we may check that it is an isomorphism or zero after restricting to
$U$ (resp.\ $V$).
...
i_*i^*\mathcal{F}
=
i_*(\mathcal{O}_Z \otimes_{i^{-1}\mathcal{O}_X} i^{-1}\mathcal{F})
$$
Note that
$$
\text{Mor}(i_*\mathcal{O}_Z \otimes_{\mathcal{O}_X} \mathcal{F},
i_*(\mathcal{O}_Z \otimes_{i^{-1}\mathcal{O}_X} i^{-1}\mathcal{F}))
=
\text{Mor}(\mathcal{O}_Z
\otimes_{i^{-1}\mathcal{O}_X} i^{-1}\mathcal{F},
\mathcal{O}_Z \otimes_{i^{-1}\mathcal{O}_X} i^{-1}\mathcal{F})
$$
because $i^{-1}i_*\mathcal{O}_Z = \mathcal{O}_Z$. Hence we obtain
a map
$i_*\mathcal{O}_Z \otimes_{\mathcal{O}_X} \mathcal{F}
\to i_*(\mathcal{O}_Z \otimes_{i^{-1}\mathcal{O}_X} i^{-1}\mathcal{F})$.


First we check that the
map $\mathcal{I}\mathcal{F} \to \mathcal{F} \to i_*i^*\mathcal{F}$
is zero. To see this note that any section of $\mathcal{I}\mathcal{F}$
is locally a finite sum $\sum h_as_a$ with $h_a$ local sections of
$\mathcal{I}$ and $s_a$ local sections of $\mathcal{F}$. Hence it suffices
to show that a local section of the form $hs$ maps to zero in
$i_*i^*\mathcal{F}$. Say $h \in \mathcal{I}(U)$ and $s \in \mathcal{F}(U)$
for some $U$ etale over $X$.
By definition we have $i_*i^*\mathcal{F}(U) = i^*\mathcal{F}(Z \times_X U)$.
By definition of $\mathcal{I} = \text{Ker}(\mathcal{O}_X \to i_*\mathcal{O}_Z)$
we see that $h$ restricts to zero in $\mathcal{O}_Z(Z \times_X U)$. Hence
$hs$ maps to zero because it maps to the restriction of $s$ times the
restriction of $h$. 
...
\medskip\noindent
We claim that for any quasi-coherent sheaf $\mathcal{F}$ on $Z$
the canonical map
$$
i^*i_*\mathcal{F} \longrightarrow \mathcal{F}
$$
is an isomorphism. This claim implies in particular that $i_*$ is
fully faithful. Choose a scheme $U$ and a surjective etale morphism $U \to X$.
Set $V = Z \times_X U$, so that $j : V \to U$ is a closed immersion of schemes.
By Equation (\ref{equation-representable-pushforward}) and the discussion
surrounding it, we see that the claim above is implied by the statement
that $j^*j_*\mathcal{F}' \to \mathcal{F}'$ is an isomorphism for every
quasi-coherent sheaf $\mathcal{F}'$ on $V$. Since $j$ is a closed immersion
of schemes, this can be found in the proof of
...
restricting to $U$ and using the result for the schemes case).
Next, suppose that $\mathcal{G}$ is any quasi-coherent
$\mathcal{O}_X$-module such that $\mathcal{I}\mathcal{G} = 0$.
It suffices to show that the canonical map
$$
\mathcal{G} \longrightarrow i_* i^*\mathcal{G}
$$
is an isomorphism. Restricting to the etale cover $U$ this reduces to
proving that $\mathcal{G}' \to j_* j^*\mathcal{G}'$ is an isomorphism for
every quasi-coherent sheaf on $U$ which is annihilated by $\mathcal{I}$.
Again this follows from the proof of the lemma in the schemes case, see
Morphisms, Lemma \ref{morphisms-lemma-i-star-equivalence}.
\end{proof}
