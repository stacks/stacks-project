...
Let $Z$ be the closed subspace
$(X \times_S T) - \bigcup_{i \in I} (X_i \times_S T_i)$
of $X \times_S T$.
It suffices to prove that the image $f_T(Z)$ of $Z$ under
$f_T : X \times_S T \to T$ is not closed.

\medskip\noindent
There exists a point $y \in Y$ such that there is no
neighborhood $V$ of $y$ in $Y$ such that $X_V$ is quasi-compact.
Otherwise we could cover $Y$ with finitely many such $V$ and
Lemma \ref{lemma-quasi-compact-local}
would imply $f$ quasi-compact. Fix such a $y \in Y$.

\medskip\noindent
First we check that $f_T(Z_y) \ne T_y$.  Let $t \in T$ be the point
lying over $y$ with $\kappa(t) = \kappa(y)$ such that $a_i = 1$
in $\kappa(t)$ for all $i$. Then $t \in T_i$ for all $i$, and the fiber
of $Z_y \to T_y$ above $t$ is isomorphic to $(X - \bigcup_{i \in I} X_i)_y$,
which is empty. Thus $t \in T_y - f_T(Z_y)$.
...
invertible on some neighborhood $V$ of $y$. Let $J$ be the finite set of
$j \in I$ such that $a_j$ appears in $g$. Since $X_V$ is not quasi-compact,
we may choose a point $x \in |X| - \bigcup_{j \in J} |X_j|$ lying above some
$v \in V$. Since $g$ has a coefficient that is invertible on $V$, we can
find a point $t' \in T$ lying above $v$ such that $t' \not \in V(g)$ and
$t' \in V(a_i)$ for all $i \notin J$. This is true because
$V(a_i; i \in I, i \not\in J) = \text{Spec}(B[a_j; j\in J])$
and the set of points of this scheme lying over $v$ is bijective
with $\text{Spec}(\kappa(v)[a_j; j \in J])$. In other words $t' \notin T_i$
for each $i \notin J$. By
Properties of Spaces, Lemma \ref{spaces-properties-lemma-points-cartesian}
...
For (1), (2), and (3) think of $X$ and $Y$ as
algebraic spaces over $\text{Spec}(\mathbf{Z})$ and apply
Lemma \ref{lemma-composition-separated}.
Details omitted.
\end{proof}
...
and
Spaces, Definition \ref{spaces-definition-separated}.
To prove (1), (2), and (3) think of $X$ and $Y$ as algebraic spaces over
...
If $\Delta_{X/Y}$ is universally closed then $\Delta_{X/Y}$ is
universally closed, a monomorphism (hence finite fibres), and locally of
finite type. Thus
More on Morphisms,
