...
\medskip\noindent
Another motivation for the material in this section is that it will garantee
later that an algebraic stack whose inertia stack is trivial is equivalent
to an algerbaic space.
...
separated over a scheme is a scheme. But first...\ a lemma.
...
$$
\xymatrix{
V' \ar[r] \ar[rd] & X_{T'} \ar[r]_\psi \ar[d] & X \ar[d] \\
& T' \ar[r] & T
}
$$
Assume
\begin{enumerate}
\item $T' \to T$ is an etale morphism of schemes,
\item $X$ is an algebraic space,
\item $X \to T$ is a quasi-separated morphism,
\item $V'$ is an open subspace of $X_{T'}$, and
\item $V' \to T'$ is quasi-affine.
\end{enumerate}
In this situation set
$$
U = \{x \in |X| \text{ such that } |\psi|^{-1}(\{x\}) \subset |V'| \}
$$
Then $U$ is a (possibly empty) open subspace of $X$ which is representable.
\end{lemma}

\begin{proof}
Note that $V'$ is a representable, i.e., a scheme.
As $T' \to T$ is etale its image is open. Hence we may
replace $T$ by the image so $\{T' \to T\}$ is an etale covering.
Set $T'' = T' \times_T T'$, $X' = X_{T'}$, and $X'' = X_{T''}$.
Denote the two projections $a, b : X'' \to X'$. Note that
$U'' = a^{-1}(V') \cap b^{-1}(V')$ is an open subspace of $X''$
whose image under the etale morphism $X'' \to X$ is $U$.
As $X \to T$ is quasi-separated, and
$a^{-1}(V')$, $b^{-1}(V')$ are quasi-compact over $T''$ we conclude that
$U'' \to T''$ is quasi-compact. Hence its image $U'$
(resp.\ $U$) in $X'$ (resp.\ $X$) is a subspace such that
$U' \to T'$ (resp.\ $U \to T$) is quasi-compact.
Note that $U'' = T'' \times_T U$ and $U' = T' \times_T U$.
Moreover, as $U'$ is an open subscheme of $V'$ which is quasi-affine
over $T'$ the morphism $U' \to T'$ is quasi-affine. Hence we may apply
Descent, Lemmas \ref{descent-lemma-quasi-affine} and
\ref{descent-lemma-descent-data-sheaves}
to conclude that $U$ is a quasi-affine scheme over $T$.
Some details omitted (especially references to earlier lemmas).
\end{proof}
...
$V'$ in $X_{T'}$ which is an affine scheme. After shrinking $T'$ we
may assume $p'$ is the only point of $T'$ mapping to $p$. In this case
we see that $x'$ is the only point of $|X_{T'}|$ mapping to $x$ (which is
defined to be the image of $u$ in $|X|$). We apply
Lemma \ref{lemma-neighbourhood-scheme}
to obtain an open subspace $W \subset X$ which is a scheme, and
which contains $x$. Since this works for every $x$ we see that $X$
is a scheme by
Properties of Spaces, Lemma \ref{spaces-properties-lemma-subscheme}.
This ends the proof.
\end{proof}
...
\noindent
Suppose that $a : F \to G$ is a map of presheaves on
$(\textit{Sch}/S)_{fppf}$. Suppose $a : F \to G$ is representable by
algebraic spaces, see
Spaces, Definition \ref{spaces-definition-morphism-representable-by-spaces}.
Then we say $a$ is {\it surjective} (resp.\ {\it etale}) if for every
scheme $T$ and map $\xi : h_T \to G$ the morphism of algebraic spaces
$h_T \times_{\xi, G} F \to T$ is surjective (resp.\ etale).
...
Spaces, Lemmas \ref{spaces-lemma-representable-diagonal}, and
\ref{spaces-lemma-representable-by-spaces-over-space}.
...
from a scheme towards $X$. Then it is straightforward to see that $U \to F$
is surjective and etale also. Hence the theorem boils down to proving that
$\Delta_F$ is representable. Let $U$ be a scheme, and let $U \to F$
be surjective and etale. Set $R = U \times_F U$, which is now an algebraic
space. But $R \to U \times_S U$ is a monomorphism, hence separated.
Moreover, since $U \to F$ is etale, we see that $R \to U$ is etale, in
particular locally of finite type. We conclude that also $R \to U \times_S U$
is locally of finite type, and combined with being a monomorphism
it is locally quasi-finite. Hence
Propoisition \ref{proposition-locally-quasi-finite-separated-over-scheme}
applies and $R$ is a scheme. Hence $F = U/R$ is an algebraic
space according to
Spaces, Theorem \ref{spaces-theorem-presentation}.
Lots of details omitted (to be filled in soon).
\end{proof}
