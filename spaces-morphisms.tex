...
\noindent
The following lemma shows this is the same thing as being
...
Let $S$ be a scheme.
Let $f : X \to Y$ be a morphism of algebraic spaces over $S$.
Then $f$ is radicial if and only if
$f$ is universally injective, i.e.,
for every morphism $Y' \to Y$ the induced map $|Y' \times_Y X| \to |Y'|$
is injective.
\end{lemma}

\begin{proof}
Assume $f$ is radicial. Let $g : Y' \to Y$ be a morphism of algebraic
...
of $Y' \times_Y X$. We conclude that $f$ is universally injective.

\medskip\noindent
Assume $f$ is universally injective.
Let $K$ be a field over $S$, and let $a, b : \text{Spec}(K) \to X$
...
we conclude that $a = b$. In other words, $f$ is radicial.
\end{proof}
...
\item for every affine scheme $Z$ and any morphism
$Z \to Y$ the morphism $Z \times_Y X \to Z$ is radicial,
\item there exists a scheme $Z$ and a surjective morphism
$Z \to Y$ such that $Z \times_Y X \to Z$ is radicial, and
\item there exists a Zariski covering $Y = \bigcup Y_i$ such that
each of the morphisms $f^{-1}(Y_i) \to Y_i$ is radicial.
\end{enumerate}
\end{lemma}

\begin{proof}
We will use that being radicial is preserved under base change
(Lemma \ref{lemma-base-change-radicial})
