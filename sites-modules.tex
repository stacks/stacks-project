...
\noindent
Extension by zero is in general not defined. It is only defined in case
the object $U$ is in some sense a suboject of the final object.

\begin{lemma}
\label{lemma-extension-by-zero}
Let $(\mathcal{C}, \mathcal{O})$ be a ringed site.
Let $U \in \text{Ob}(\mathcal{C})$.
Assume every object $X$ of $\mathcal{C}$ has at most
one morphism into $U$. In this case the
restriction functor
$j_U^* : \textit{Mod}(\mathcal{O}) \to \textit{Mod}(\mathcal{O}_U)$
has a left adjoint
$j^{Mod}_{U!} : \textit{Mod}(\mathcal{O}_U) \to \textit{Mod}(\mathcal{O})$.
So
$$
\text{Mor}_{\textit{Mod}(\mathcal{O}_U)}(\mathcal{G}, \mathcal{F}|_U)
=
\text{Mor}_{\textit{Mod}(\mathcal{O}_U)}(\mathcal{G}, j_U^*\mathcal{F})
=
\text{Mor}_{\textit{Mod}(\mathcal{O})}(j^{Mod}_{U!}\mathcal{G}, \mathcal{F})
$$
...
that there does exist an extension by the empty set
$j^{Sh}_{U!} : \textit{Sh}(\mathcal{C}/U) \to \textit{Sh}(\mathcal{C})$
on sheaves of sets which is left adjoint to $j_U^{-1}$.
By Sites, Lemma \ref{sites-lemma-essential-image-j-shriek}
this induces an equivalence of categories
between $\textit{Sh}(\mathcal{C}/U)$ and $\textit{Sh}(\mathcal{C})/h_U^\#$.
Moreover, by assumption $h_U^\#$ is a subsheaf of $*$
(the singleton sheaf, see Sites, Example \ref{sites-example-singleton-sheaf}).
Hence nonempty products in $\textit{Sh}(\mathcal{C}/U)$ are turned into
products in $\textit{Sh}(\mathcal{C})$ under $j^{Sh}_{U!}$.

\medskip\noindent
For a sheaf of abelian groups $\mathcal{G}$ on $\mathcal{C}/U$ set
$$
j^{Ab}_{U!}\mathcal{G} = j^{Sh}_{U!}\mathcal{G} \times_{h_U^\#} *.
$$
Then we have
$$
j^{Ab}_{U!}(\mathcal{G} \times \mathcal{G})
=
j^{Sh}_{U!}(\mathcal{G} \times \mathcal{G}) \times_{h_U^\#} *
=
j^{Sh}_{U!}\mathcal{G} \times j^{Sh}_{U!}\mathcal{G} \times_{h_U}^\# *
=
j^{Ab}_{U!}\mathcal{G} \times j^{Ab}_{U!}\mathcal{G}
$$
In particular the addition map on $\mathcal{G}$ gives an addition map
on $j^{Ab}_{U!}\mathcal{G}$. This turns $j^{Ab}_{U!}$ in to a functor
$\textit{Ab}(\mathcal{C}/U) \to \textit{Ab}(\mathcal{C})$ which is
left adjoint to $j_U^{-1}$. A similar construction works for
$j^{Mod}_{U!}$. Verifications omitted.
\end{proof}
...
$$
by Sites, Lemma \ref{sites-lemma-morphism-sites-localization}.
