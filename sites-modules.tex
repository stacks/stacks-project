...
(as a cokernel of a map of abelian sheaves is a coequalizer of
sheaves of sets).
\end{proof}

\noindent
Some parts of the following lemma are purely category theoretical.
We collect them all here for easy reference.

\begin{lemma}
\label{lemma-exactness-properties}
Let $f : \textit{Sh}(\mathcal{C}) \to \textit{Sh}(\mathcal{D})$ be
a morphism of topoi. Consider the following properties:
\begin{enumerate}
\item $f_*$ is faithful,
...
\item[(c)] (8) $\Rightarrow$ (1), (2), (3), (9), (10), (11).
\item[(d)] (3) $\Leftrightarrow$ (10),
\item[(e)] (4) $\Rightarrow$ (7),
\item[(f)] (5) $\Rightarrow$ (7),
\item[(g)] (6) $\Rightarrow$ (4), (7),
\item[(h)] (4) $\Leftrightarrow$ (12), and
\item[(i)] (9) $+$ (10) $\Rightarrow$ (11).
\end{enumerate}
\end{lemma}
...
$$
If (8) holds, then the vertical arrows are isomorphisms.
...
Thus we see that (8) implies (9), (10), (11). It is clear that (8) implies
...
This means that (10) holds.
Assume (10). Let $\mathcal{F}$ be a sheaf on $\mathcal{C}$.
...
\medskip\noindent
Proof of (e). This is clear from the definitions.

\medskip\noindent
Proof of (f). Assume (5). Let
$0 \to \mathcal{F}_1 \to \mathcal{F}_2 \to \mathcal{F}_3 \to 0$
be a short exact sequence of abelian sheaves on $\mathcal{C}$.
Then $\mathcal{F}_3$ is the coequalizer of two maps
...
\medskip\noindent
Proof of (g). A map of sheaves $\mathcal{F} \to \mathcal{F}'$
is surjective if and only if the map
$\mathcal{F}' \amalg_{\mathcal{F}} \mathcal{F}' \to \mathcal{F}'$
is injective. Hence we see that (6) implies (4).
By part (e) we see that (4) implies (7).

\medskip\noindent
Proof of (h). Assume (4). Let $\mathcal{F} \to f^{-1}\mathcal{G}$ be
...
$$
and we see the required result. Conversely, assume (12).
...
$$
Because the lower horizontal arrow is surjective and by (12) we can
...
\medskip\noindent
Proof of (i). This is immediate from the definitions.
\end{proof}
