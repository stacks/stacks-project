...
\begin{lemma}
\label{lemma-differentials-localization}
Let $\mathcal{C}$ be a site. Let $\varphi : \mathcal{O}_1 \to \mathcal{O}_2$
...
Let $U$ in $\mathcal{C}$ and $f \in \mathcal{O}(U)$. By
Sites, Lemma \ref{sites-lemma-localization-enough}
the site $\mathcal{C}/U$ has enough points and for any point
$q$ of $\mathcal{C}/U$ we have
$\mathcal{O}_{U, q} \cong \mathcal{O}_p$ where $p$ is the
corresponding point of $\mathcal{C}$. In particular all the stalks
$\mathcal{O}_{U, q}$ are local rings.
Consider the map of sheaves of sets on $\mathcal{C}/U$
$$
\mathcal{O}_U \amalg \mathcal{O}_U \longrightarrow \mathcal{O}_U
$$
which maps a local section
$a_i \in \mathcal{O}_U(V)$, $i = 1, 2$ in the $i$th
component to $a_1f|_V$, resp.\ $a_2(1 - f)|_V$.
Since $\mathcal{O}_{U, q}$ is local this map is surjective
on stalks. Hence it is surjective. Hence we can find a covering
$\{U_j \to U\}$ such that for each $j$ either $1 = a_jf|_{U_j}$
or $a_j(1 - f)|_{U_j} = 1$ for some
$a_j \in \mathcal{O}_U(U_j) = \mathcal{O}(U_j)$. This proves (1) holds.
\end{proof}
...
\begin{proof}
Namely, we claim that $(\mathcal{C}, \mathcal{O})$
is locally ringed if and only if the following property holds:
(*) For any sheaf of sets $\mathcal{F}$ and any morphism of sheaves of sets
$f : \mathcal{F} \to \mathcal{O}$ the map
$$
\mathcal{F} \times \mathcal{O}
\amalg
\mathcal{F} \times \mathcal{O}
\longrightarrow
\mathcal{F} \times \mathcal{O}
$$
defined by the rule $(s, a) \mapsto (s, af(s))$ on the first component and
the rule $(s, a) \mapsto (f, a(1 - f(s)))$ on the second component is
surjective.

\medskip\noindent
It is clear that this condition implies that the ringed site is locally
ringed by taking $\mathcal{F} = h_U^\#$ for varying objects $U$ of
$\mathcal{C}$. Conversely, assume $(\mathcal{C}, \mathcal{O})$ is locally
ringed. Then given $\mathcal{F}$, $f$ and a section $(s, b)$ of the right
hand side over $U$ we obtain $f(s) \in \mathcal{O}(U)$. Applying the
assumption we see that there exists a covering of $U$ such that on each
member either $f(s)$ or $1 - f(s)$ restricts to an invertible section
of $\mathcal{O}$. Hence $(s, b)$ is in the image of the map on the members
of this covering as desired.

\medskip\noindent
Now condition (*) does not make any reference to the underlying site
and hence it is intrinsic.
\end{proof}
