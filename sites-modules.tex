...
\xymatrix{
(\textit{Sh}(\mathcal{C})/\mathcal{F}, \mathcal{O}_{\mathcal{F}})
\ar[r]_{j_{\mathcal{F}}} \ar[d]_{f'} &
(\textit{Sh}(\mathcal{C}), \mathcal{O}) \ar[d]^f \\
(\textit{Sh}(\mathcal{D})/\mathcal{G}, \mathcal{O}'_{\mathcal{G}})
\ar[r]^{j_{\mathcal{G}}} &
(\textit{Sh}(\mathcal{D}), \mathcal{O}')
}
$$
and moreover we have $f'_*j_{\mathcal{F}}^{-1} = j_{\mathcal{G}}^{-1}f_*$
...
we have the diagram of underlying topoi and the
equality $f'_*j_{\mathcal{F}}^{-1} = j_{\mathcal{G}}^{-1}f_*$.
...
Part (2) is clear also since $(f')^\sharp$ is just the
