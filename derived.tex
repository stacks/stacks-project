...
\end{enumerate}
To prove the claim, using the assumption of the lemma we can choose
...
Next by descending induction on $i$ we pick $E^i \subset X^i$,
$E^i \in \Ob(\mathcal{B})$ such that
$d^i(E^i) = (D^{i + 1} + E^{i + 1}) \cap \Ker(d^{i + 1})$.
This is possible because $X^\bullet$ is bounded above.
Then setting $Y^i = E^i + D^i$ we get (1), (2), and (3).
...
For the converse, suppose that $X^\bullet$ is a bounded complex
of $\mathcal{A}$, say $X^i = 0$ for $i \not \in [a, b]$.
Define $B^i \subset Z^i \subset X^i$ by $B^i = \Im(d^{i - 1})$
and $Z^i = \Ker(d^i)$. The short exact sequences
$$
0 \to B^i \to Z^i \to H^i(X^\bullet) \to 0
\quad\text{and}\quad
0 \to Z^i \to X^i \to B^{i + 1} \to 0
$$
show that in $K_0(\mathcal{A})$ we have
$$
\sum (-1)^i[X^i] =
\sum (-1)^i[Z^i] + (-1)^i[B^{i + 1}] =
\sum (-1)^i[H^i(X^\bullet)]
$$
On the other hand, the existence of the distinguished triangles
of ``stupid truncations'' (see
Homology, Section \ref{homology-section-truncations})
$$
...
$K_0(D^b(\mathcal{A})) \to K_0(\mathcal{A}) \to
K_0(D^b(\mathcal{A}))$ is the identity too.
\end{proof}
