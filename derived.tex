...
Deligne's expos\'ee XVII in \cite{SGA4}.
Let $\mathcal{A}$, $\mathcal{B}$ be abelian categories.
Let $F : \mathcal{A} \to \mathcal{B}$ be an additive functor.
This induced an exact functor $K^{+}(\mathcal{A}) \to K^{+}(\mathcal{B})$
of triangulated categories. The right derived functor $RF$ of $F$ is,
if it exists, a functor $RF : D^{+}(\mathcal{A}) \to D^{+}(\mathcal{B})$ 
which is ``closely related to $F$''. Since the derived category
$D^{+}(\mathcal{A})$ is the localization of $K^{+}(\mathcal{A})$, see
Lemma \ref{lemma-bounded-derived},
this is a special case of the following situation.
...
\noindent
In this situation, we denote $Q : \mathcal{D} \to S^{-1}\mathcal{D}$ and
the localization functor. Let $X \in \text{Ob}(\mathcal{D})$. Recall from
...
a morphism of triangles
$(s_4, s_4', s_4'') : (X_2, Y_2, Z_3) \to (X_4, Y_4, Z_4)$
...
constant over $\mathcal{I}$. For any object $W$ in $\mathcal{D}'$ we can
...
$$
which shows that the middle arrow is an isomorphism. In this way we conclude
that $C$ is the colimit $\text{colim}_\mathcal{I} F(Z'')$. To see that the
ind-object is essentially constant we need to show that for any $Z' \to Z''$
in the system we can find $Z'' \to Z'''$ in the system such that
$F(Z'') \to C \to F(Z') \to F(Z''')$ equals $F(Z'') \to F(Z''')$.
To see this one argues that the same thing is true for $A$ and $B$.
Details omitted.
\end{proof}
