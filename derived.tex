...
maps such that the right square of the left diagram commmutes and the
...
\end{enumerate}
Then the compsition $K_1^\bullet \to K_n^\bullet$ factors through
...
\ref{lemma-bounded-below-injectives-K-injective}.
...
$(K_n, f_n)$ be an inverse system of objects of $\mathcal{D}$.
...
Let $\mathcal{A}$ be an abelian category with exact countable products.
Let $K_i^\bullet$, $i \in I$ be a collection of complexes indexed by
a countable index set. Then the termwise product $\prod K_i^\bullet$
is the product of $K_i^\bullet$ in $D(\mathcal{A})$.
\end{lemma}

\begin{proof}
Namely, let $L^\bullet$ be a complex. Suppose given maps
