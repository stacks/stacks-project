...
The {\it kernel of $F$} is the strictly full (pre-)triangulated subcategory
described in
Lemma \ref{lemma-triangle-functor-kernel}.
\item Let $H : \mathcal{D} \to \mathcal{A}$ be a homological functor.
The {\it kernel of $H$} is the strictly full (pre-)triangulated subcategory
described in
...
\right\}
$$
In other words it is the smallest triangulated subcategory of
$\mathcal{D}$ containing $\mathcal{B}$ and stable under taking
direct summands.
\end{lemma}

\begin{proof}
First note that the kernel is automatically a strictly full
triangulated subcategory stable under taking direct summands, see
Lemma \ref{lemma-triangle-functor-kernel}.
...
\noindent
Let $\mathcal{D}$ be a triangulated category.
At this point we have constructions between
\begin{enumerate}
...
$S \mapsto \mathcal{B}(S) =
\text{Ker}(Q : \mathcal{D} \to S^{-1}\mathcal{D})$ and
$\mathcal{B} \mapsto S(\mathcal{B})$
where $S$ is the multiplicative set of
(\ref{equation-multiplicative-system}).
...
i.e., it is the smallest strictly full triangulated subcategory of
$\mathcal{D}$ containing $\mathcal{B}$ and stable under taking direct
summa
\end{enumerate}


\begin{proposition}
\label{proposition-correspondence}
Let $\mathcal{D}$ be a triangulated category.
