...
such that $\Hom(E, K[i]) = 0$ for all $i \in \mathbf{Z}$. Then
$\Hom(X, K) = 0$ for all $X$ in $\langle E \rangle_1$. Arguing by induction
on $n$ and using Lemma \ref{lemma-representable-homological}
we see that $\Hom(X, K) = 0$ for all $X$
in $\langle E \rangle_n$. However, since $K \in \langle E \rangle_n$
for some $n$ we conclude that $\text{id}_K = 0$, i.e., $K = 0$.
\end{proof}
