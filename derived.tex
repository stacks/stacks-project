...
\section{Derived categories: canonical delta-functor}
\label{section-derived-canonical-delta-functor}
...
Let $\mathcal{A}$ be an abelian category. We denote
$\text{Fil}^f(\mathcal{A})$ the category of filtered objects
...
\begin{enumerate}
\item If $H^n(\text{gr}(K^\bullet)) = 0$ for all $n \ll 0$, then there exists
a filtered quasi-isomorphism $K^\bullet \to L^\bullet$ with $L^\bullet$
bounded below.
\item If $H^n(\text{gr}(K^\bullet)) = 0$ for all $n \gg 0$, then there exists
a filtered quasi-isomorphism $M^\bullet \to K^\bullet$ with $M^\bullet$
bounded above.
...
\begin{proof}
Suppose $n \in \mathbf{Z}$ is such that $H^n(\text{gr}(K^\bullet)) = 0$. By
Homology, Lemma \ref{homology-lemma-filtered-acyclic}
the sequence
$$
K^{n - 1} \xrightarrow{d^{n - 1}} K^n \xrightarrow{d^n} K^{n + 1}
$$
is an exact sequence of objects of $\mathcal{A}$ and the morphisms
$d^{n - 1}$ and $d^n$ are strict. Hence the two induced filtrations
on $\text{Im}(d^{n - 1}) = \text{Ker}(d^n)$ are equal and
$\text{gr}(K^{n - 1}) \to \text{gr}(\text{Ker}(d^n))$ is surjective, see
Homology, Lemma \ref{homology-lemma-characterize-strict}.
This means that the truncation
$$
\tau_{\leq n}K^\bullet =
(\ldots \to K^{n - 2} \to K^{n - 1} \to \text{Ker}(d^n) \to 0 \to \ldots)
$$
has the properties $H^i(\tau_{\leq n}K^\bullet) = H^i(K^\bullet)$
for $i \leq n$ and $H^i(\tau_{\leq n}K^\bullet) = 0$ for $i \geq n$.
Dually the map $\text{gr}(K^n/\text{Ker}(d^n)) \to \text{gr}(K^{n + 1})$
is injective, see
Homology, Lemma \ref{homology-lemma-characterize-strict}.
This means the quotient $K^\bullet/\tau_{\leq a}K^\bullet$ has the property
that $H^i(K^\bullet/\tau_{\leq n}K^\bullet) = 0$
for $i \leq n$ and $H^i(K^\bullet/\tau_{\leq n}K^\bullet) = H^i(K^\bullet)$
for $i \geq n$. (Warning: This only works because we assumed that
$H^n(\text{gr}(K^\bullet)) = 0$!)

\medskip\noindent
At this point we can use exactly the same arguments as in the proof of
Lemma \ref{lemma-complex-cohomology-bounded}
to obtain the desired morphisms of complexes of $\text{Fil}^f(\mathcal{A})$.
\end{proof}
...
\section{Derived functors: generalities}
...
of triangulated category on $\mathcal{D}$ and $\mathcal{D}'$.
\end{situation}
...
$$
RF(X) = \text{colim}_{s : X' \to X} F(X')
\quad\text{and}\quad
RF(Y) = \text{colim}_{s' : Y' \to Y} F(Y')
$$
...
$$
\xymatrix{
\text{lim}_{\mathcal{I}}\ \text{Mor}_{\mathcal{D}'}(F(X''), W) \ar[r] &
\text{Mor}_{\mathcal{D}'}(A, W) \\
\text{lim}_{\mathcal{I}}\ \text{Mor}_{\mathcal{D}'}(F(Y''), W) \ar[u] \ar[r] &
\text{Mor}_{\mathcal{D}'}(B, W) \ar[u] \\
\text{lim}_{\mathcal{I}}\ \text{Mor}_{\mathcal{D}'}(F(Z''), W) \ar[u] \ar[r] &
\text{Mor}_{\mathcal{D}'}(C, W) \ar[u] \\
\text{lim}_{\mathcal{I}}\ \text{Mor}_{\mathcal{D}'}(F(X''[1]), W) \ar[u]
\ar[r] &
\text{Mor}_{\mathcal{D}'}(A[1], W) \ar[u] \\
\text{lim}_{\mathcal{I}}\ \text{Mor}_{\mathcal{D}'}(F(Y''[1]), W) \ar[u]
\ar[r] &
...
$\text{colim}_\mathcal{I}\ F(Z'')$. To see that the ind-object
is essentially constant it now suffices to show that for any
object $W$ in $\mathcal{D}'$ the map
$$
\text{colim}_{\mathcal{I}}\ \text{Mor}_{\mathcal{D}'}(W, F(Z''))
\longrightarrow
...
$$
\xymatrix{
\text{colim}_{\mathcal{I}}\ \text{Mor}_{\mathcal{D}'}(W, F(X''))
\ar[r] &
\text{Mor}_{\mathcal{D}'}(W, A) \\
\text{colim}_{\mathcal{I}}\ \text{Mor}_{\mathcal{D}'}(W, F(Y''))
\ar[u] \ar[r] &
\text{Mor}_{\mathcal{D}'}(W, B) \ar[u] \\
\text{colim}_{\mathcal{I}}\ \text{Mor}_{\mathcal{D}'}(W, F(Z''))
\ar[u] \ar[r] &
\text{Mor}_{\mathcal{D}'}(W, C) \ar[u] \\
\text{colim}_{\mathcal{I}}\ \text{Mor}_{\mathcal{D}'}(W, F(X''[1]))
\ar[u] \ar[r] &
\text{Mor}_{\mathcal{D}'}(W, A[1]) \ar[u] \\
\text{colim}_{\mathcal{I}}\ \text{Mor}_{\mathcal{D}'}(W, F(Y''[1]))
\ar[u] \ar[r] &
...
\section{Derived functors: usual case}
...
\begin{definition}
\label{definition-derived-functor-classical}
In
Situation \ref{situation-classical}.
\begin{enumerate}
...
$\mathcal{I}$. Then for every bounded below complex $K^\bullet$
there exists a quasi-isomorphism $K^\bullet \to I^\bullet$
with $I^\bullet$ bounded below, each $I^n \in \mathcal{I}$, and
...
and consists of objects from $\mathcal{I}$, and a termwise
...
\begin{proof}
Let $b$ be an element of the right hand side.
...
map is surjective. Moreover, the result of
Lemma \ref{lemma-morphisms-equal-up-to-homotopy}
shows that $\beta$ is well determined up to homotopy.
\end{proof}
...
Definition \ref{definition-derived-functor-classical}.
...
\begin{proof}
As $I^{p, \bullet}$ is an injective resolution of $K^p$ we see that
$RF$ is defined at $K^p[0]$ with value $F(I^{p, \bullet})$.
Similarly, as $H^p_{I}(I^{\bullet, \bullet})$ is an injective resolution
of $H^p(K^\bullet)$ the derived functor $RF$ is defined at $H^p(K^\bullet)[0]$
with value $F(H^p_{I}(I^{\bullet, \bullet}))$. By
Homology, Lemma \ref{homology-lemma-double-complex-gives-resolution}
the total complex $sI^\bullet$ is an injective resolution of
$K^\bullet$. Hence $RF$ is defined at $K^\bullet$ with value
$F(sI^\bullet)$. Now the spectral sequences are the spectral
sequences associated to the double complex $F(I^{\bullet, \bullet})$, see
Homology, Lemmas \ref{homology-lemma-ss-double-complex} and
\ref{homology-lemma-first-quadrant-ss}.
Note that since $\text{Ker}(d_1^{p, q})$, $\text{Im}(d_1^{p, q})$, and
$H^p_I(I^{\bullet, \bullet})$ are injective we have
$F(\text{Ker}(d_1^{p, q})) = \text{Ker}(F(d_1^{p, q}))$, etc.
...
$$
which need not always be an isomorphism. We omit the construction of $t$.
...
The map $t$ is given by the identifications
$$
R(G \circ F)(A^\bullet) =
(G \circ F)(I^\bullet) =
G(F(I^\bullet))) =
RG(F(I^\bullet)) = RG(RF(A^\bullet))
$$
where the third ``$=$'' is
Lemma \ref{lemma-leray-acyclicity}.
We omit the verification that this is functorial in $A^\bullet$.
\end{proof}
...
By analogy with
Definition \ref{definition-derived-category}
