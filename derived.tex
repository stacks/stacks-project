...
$\Ker((b')^n) \supset \text{Im}(A_2^n \to B_2^n)$.
...
I^n/(\text{Im}(d^{n - 1}) + \alpha(\Ker(d_K^n))) \ar@{..>}[r] & M
}
$$
...
$K^{n + 1}$ and $I^n/(\text{Im}(d^{n - 1}) + \alpha(\Ker(d_K^n)))$
...
\Ker(d^n)/\text{Im}(d^{n - 1})$ is an isomorphism.
...
$\text{Im}(d^{n - 1}) = \Ker(d^n)$ and $\text{Im}(d^{-1})$
are elements of $\mathcal{I}$ using property (2) and the exact sequences
$$
0 \to \Ker(d^n) \to I^n \to \text{Im}(d^n) \to 0.
$$
...
then implies that $\text{Im}(d^0) = \Ker(d^1)$ is an elements of
$\mathcal{I}$ and from then on one continues as before to show that
$\text{Im}(d^{n - 1}) = \Ker(d^n)$ is an element of $\mathcal{I}$
...
$$
\begin{matrix}
0 \to \Ker(d_K^0) \to K^0 \to \text{Coim}(d_K^0) \to 0 \\
0 \to \Ker(d_K^1) \to K^1 \to \text{Coim}(d_K^1) \to 0 \\
0 \to \Ker(d_K^2) \to K^2 \to \text{Coim}(d_K^2) \to 0 \\
\ldots
\end{matrix}
$$
of the exact category $\text{Fil}^f(\mathcal{A})$
and the maps $u_i : \text{Coim}(d_K^i) \to \Ker(d_K^{i + 1})$.
