...
\begin{enumerate}
\item $\mathcal{A}[a, b]$ for $a \leq b$ be the full subcategory of
$\mathcal{T}$ consisting of all objects $A[-i]$ with $i \in [a, b]$
with $A \in \Ob(\mathcal{A})$ (please note the minus sign),
\item $smd(\mathcal{A})$ be the full subcategory of $\mathcal{T}$
consisting of all objects which are direct summands of objects
of $\mathcal{A}$,
\item $add(\mathcal{A})$ be the full subcategory of $\mathcal{T}$
consisting of all objects which are finite direct sums of objects
of $\mathcal{A}$,
...
$$
$$
\left(\bigcup \mathcal{A}_i\right) \star \left(\bigcup \mathcal{A}_i\right) =
\bigcup \mathcal{A}_i \star \mathcal{A}_i,
$$
$$
\mathcal{C}_n\left(\bigcup \mathcal{A}_i\right) =
\bigcup \mathcal{C}_n(\mathcal{A}_i)
$$
...
Let $K^\bullet$ be a complex of $\mathcal{A}$. Let $b \geq a$. Let
$\mathcal{E} \subset \Ob(\mathcal{A})$ be a subset.
Assume that $K^i = 0$ for $i \not \in [a, b]$ and that
each $K^i$ is a direct summand of a finite direct sum of elements of
$\mathcal{E}$. Then $K^\bullet$ is in
$\mathcal{C}_{b - a + 1}(\mathcal{E}[a, b])$.
\end{lemma}

\begin{proof}
We will use Lemma \ref{lemma-cone-n} without further mention.
If $K$ has only one nonzero cohomology, then this is clear.
If $K^i = 0$ for $i \not \in [a, b]$ then we can consider the
distinguished triangle
$$
\sigma_{\geq a + 1}K^\bullet \to K^\bullet \to K^a[-a]
$$
and we conclude by induction on $b - a$.
\end{proof}
...
$$
\langle E \rangle_1 =
smd\left(\bigcup\nolimits_{m \geq 1} add(E[-m, m])]\right)
$$
...
necessarily a triangulated subcategory.
...
\bigcup\nolimits_{m \geq 1} \mathcal{C}_n(E[-m, m])
$$
...
and induction. Since $\langle E \rangle_1 =
smd(\bigcup\nolimits_{m \geq 1} add(E[-m, m])])$
...
= \bigcup\nolimits_{n, m \geq 1} \mathcal{C}_n(E[-m, m])
$$
...
It is clear that $\bigcup \langle E \rangle_n$
is preserved under shifts, direct sums, direct summands.
