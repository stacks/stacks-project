...
For any object $W$ in $\mathcal{D}'$ we can
...
Let $X, Y$ be objects of $\mathcal{D}$. Then $RF$ is defined at $X \oplus Y$
if and only if it is defined at $X$ and $Y$. Moreover, in this case
...
shows that $RF$ is defined at $X \oplus Y$. Conversely, assume that
$RF$ is defined at $X \oplus Y$. Since $S$ is a saturated system
$S$ is the set of arrows which become invertible under the additive
localization functor $Q : \mathcal{D} \to S^{-1}\mathcal{D}$, see
Categories, Lemma \ref{categories-lemma-what-gets-inverted}.
...
(Categories, Remark
\ref{categories-remark-left-localization-morphisms-colimit}).
By assumption and some arguments about product categories which we omit,
the composite functor $(X \to X', Y \to Y') \mapsto F(X' \oplus Y')$ is
essentially constant with value $RF(X) \oplus RF(Y)$.
Thus, to prove $RF$ is defined at $X \oplus Y$ with the correct value,
it suffices to show that the displayed functor is cofinal, see
Categories, Lemma \ref{categories-lemma-cofinal-essentially-constant}.
...
$c \circ s'_1 = d \circ s'_2$. The proof for $LF$ is dual.
\end{proof}
