...
$\text{Coim}(d^{a - 1}) = \Im(d^{a - 1})$ in $\text{Fil}^f(\mathcal{A})$
...
i.e., the morphisms $f$ such that $\text{Coim}(f) = \Im(f)$.
...
$$
\text{Coker}(a) = \text{Coim}(d_I^0) = \Im(d_I^0) \longrightarrow J^1.
$$
...
$$
\text{Coker}(c) = \text{Coim}(d_J^0) = \Im(d_J^0) \longrightarrow I^2.
$$
