...
\begin{proof}
We will use that $H^0$ is a homological functor by
Lemma \ref{lemma-cohomology-homological}.
Note that $\text{Ac}(\mathcal{A})$ is a strictly full saturated triangulated
subcategory of $K(\mathcal{A})$ follows from
Lemma \ref{lemma-homological-functor-kernel}.
In the same manner $\text{Qis}(\mathcal{A})$ is a saturated multiplicative
system compatible with the triangulated structure is a consequence of
Lemma \ref{lemma-triangle-functor-localize}.
...
\right\}
$$
By the long exact cohomology sequence, see above or see
