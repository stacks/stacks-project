...
\begin{align*}
R(G \circ F)(Q_A(X))
& = \text{colim}_{s : X \to X' \in S}\ G(F(X')) \\
& \xrightarrow{t'} \text{colim}_{s : X \to X' \in S}\ RG(Q_B(F(X'))) \\
& = \text{colim}_{s : X \to X' \in S}\ RG(F'(X')) \\
& = RG(\text{colim}_{s : X \to X' \in S}\ F'(X')) \\
& = RG(RF'(X)).
\end{align*}
...
$$
see
Lemma \ref{lemma-compose-derived-functors}.
