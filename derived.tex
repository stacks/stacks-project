...
Moreover, we always make the identifications
$H^i(K^\bullet[n]) = H^{i + n}(K^\bullet)$, see
Homology, Definitions \ref{homology-definition-comology-shift}.
We will use this without further mention in the following.

\begin{lemma}
\label{lemma-cohomology-homological}
Let $\mathcal{A}$ be an abelian category. Let
$(K^\bullet, L^\bullet, M^\bullet, f, g, h)$ be a distinguished
triangle in $K(\mathcal{A})$. Then the sequence
$$
\ldots \to
H^i(K^\bullet) \xrightarrow{H^i(f)}
H^i(L^\bullet) \xrightarrow{H^i(g)}
H^i(M^\bullet) \xrightarrow{H^i(h)}
H^{i + 1}(K^\bullet) \xrightarrow{H^{i + 1}(f)} \ldots
$$
is exact.
\end{lemma}

\begin{proof}
Recall that a homotopy equivalence acts as an isomorphism on cohomology, see
Homology, Lemma \ref{homology-lemma-map-cohomology-homotopy-cochain}.
Hence we may assume that we have a termwise split short exact sequence
of complexes $0 \to A^\bullet \to B^\bullet \to C^\bullet \to 0$
and the distinguished triangle is the triangle
$(A^\bullet, B^\bullet, C^\bullet, \alpha, \beta, \delta)$ of
...
\begin{proof}
Since a homotopy equivalence induces an isomorphism on cohomology, see
Homology, Lemma \ref{homology-lemma-map-cohomology-homotopy-cochain},
we see that $\text{Ac}(\mathcal{A})$ is a strictly full subcategory.
It is clearly also an additive subcategory and saturated (see
Definition \ref{definition-saturated}) as the cohomology of a direct
sum is the direct sum of the cohomologies.
To finish the proof of the first assertion we use
Lemma \ref{lemma-triangulated-subcategory}.
It is clear that $\text{Ac}(\mathcal{A})$ is preserved under
the functors $[1], [-1]$. Finally, suppose that
$(X, Y, Z, f, g, h)$ is a distinguished triangle in $K(\mathcal{A})$
with $X, Y$ objects of $\text{Ac}(\mathcal{A})$.
By the long exact cohomology sequence of
Lemma \ref{lemma-cohomology-homological}
we see that $Z$ is an acyclic object too.

\medskip\noindent
Recall that the multiplicative system corresponding to
$\text{Ac}(\mathcal{A})$ is the set
$$
\left\{
\begin{matrix}
...
Lemma \ref{lemma-cohomology-homological}
we see that $f$ is an element of this set if and only if $f$ is
a quasi-isomorphism.
\end{proof}
