...
\begin{lemma}
	...
	For each $n\in\mathbb{Z}$, define $[n]^\text{opp}=\Theta\circ [-n]\circ \Theta^{-1}$
	(in other words, $[n]^\text{opp}$ is the opposite functor to $[-n]:\mathcal{D}\to\mathcal{D}$);
	for each triangle $(X,Y,Z,f,g,h)$ of $\mathcal{D}$, define
	$(X,Y,Z,f,g,h)^\text{opp}
	=(Z,Y,X,g^\text{opp},f^\text{opp},(-h[-1])^\text{opp})$
	(whence $(-h[-1])^\text{opp}=-h^\text{opp}[1]^\text{opp}$)
	and define $\mathcal{T}^\text{opp}
	=\{\Delta^\text{opp}\mid\Delta\in\mathcal{T}\}$.
	Then $(\mathcal{D},[\ ],\mathcal{T})$ is a (pre-)triangulated category
	if and only if $(\mathcal{D},[\ ],\mathcal{T})^\text{opp}
	:=(\mathcal{D}^\text{opp},[\ ]^\text{opp},\mathcal{T}^\text{opp})$ is.
\end{lemma}

\begin{proof}
	It suffices to show that $(\mathcal{D},[\ ],\mathcal{T})$ (pre-)triangulated
	implies $(\mathcal{D},[\ ],\mathcal{T})^\text{opp}$ (pre-)triangulated,
	for $((\mathcal{D},[\ ],\mathcal{T})^\text{opp})^\text{opp}
	=(\mathcal{D},[\ ],\mathcal{T})$.
	
	\medskip\noindent
	Suppose that $(\mathcal{D},[\ ],\mathcal{T})$ is a pre-triangulated category.
	Then $(\mathcal{D},[\ ],\mathcal{T})^\text{opp}$ satisfies TR2,
	since if we consider the opposite sequence of \eqref{equation-rotate} in
	$(\mathcal{D}^\text{opp},[\ ]^\text{opp})$,
	then any four consecutive terms form a distinguished triangle of $\mathcal{T}^\text{opp}$.
	Using again the opposite sequence of \eqref{equation-rotate},
	we deduce TR1 for $(\mathcal{D},[\ ],\mathcal{T})^\text{opp}$.
	Lastly, TR3 for $(\mathcal{D},[\ ],\mathcal{T})^\text{opp}$ follows from TR3 for
	$(\mathcal{D},[\ ],\mathcal{T})$ plus TR2 for
	$(\mathcal{D},[\ ],\mathcal{T})^\text{opp}$.
