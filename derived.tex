...
$$
Calculation omitted. The functor is faithful as its kernel is zero,
...
\ldots \ar[r] & I_{-2}^\bullet \ar[r] & I_{-1}^\bullet 
}
$$
in the category of complexes such that
\begin{enumerate}
\item the vertical arrows are quasi-isomorphisms,
\item $I_{-1}^\bullet$ is a bounded above complex with terms in
$\mathcal{I}$,
\item the arrows $I_{- n - 1}^\bullet \to I_{-n}^\bullet$
are termwise split surjections and each kernel
$\text{Ker}(I^i_{- n - 1} \to I^i_{-n})$ is an
