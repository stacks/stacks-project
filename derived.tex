...
the isomorphism $(Z/X)/(Y/X) \cong Z/Y$ and TR(b) and TR(c)
...
$$
If $\mathcal{D}'$ is Karoubian, then $\mathcal{E}$ is a saturated
...
exact sequences
$0 \to \Im(d^n) \to I^{n + 1} \to \Im(d^{n + 1}) \to 0$
for $n \geq n_0$. These sequences induce distinguished triangles
$(\Im(d^n), I^{n + 1}, \Im(d^{n + 1}))$ by
Lemma \ref{lemma-derived-canonical-delta-functor}.
This implies inductively that
each $\Im(d^n)$ is acyclic for $RF$ by
...
$(F(\Im(d^n)), F(I^{n + 1}), F(\Im(d^{n + 1})))$
of $D^{+}(\mathcal{B})$ imply that
$$
0 \to F(\Im(d^n)) \to F(I^{n + 1}) \to F(\Im(d^{n + 1})) \to 0
$$
...
satisfy the relations\footnote{The notation $H^p(R^qF(K^\bullet))$
really means the $p$th cohomology group of the complex with terms
$R^qF(K^n)$. Not the $p$th cohomology of the $q$th derived functor
of $F$ applied to $K^\bullet$.}
$$
{}'E_2^{p, q} = H^p(R^qF(K^\bullet))
\quad
\text{and}
\quad
...
t : R(G \circ F) \longrightarrow RG \circ RF.
$$
of functors from $D^{+}(\mathcal{A})$ to $D^{+}(\mathcal{C})$.
\end{lemma}
...
Let $\mathcal{A}$ be an abelian category. If $\mathcal{A}$ has exact
countable products, then $D(\mathcal{A})$ has countable products. In
fact given a collection of complexes $K_i^\bullet$ indexed by a
countable index set $I$ the termwise product $\prod K_i^\bullet$
...
\end{reference}
Let $\mathcal{D}$ be a triangulated with direct sums which is compactly
generated. Let $H : \mathcal{D} \to \textit{Ab}$ be a contravariant
...
\end{reference}
Let $\mathcal{D}$ be a triangulated with direct sums which is compactly
generated. Let $F : \mathcal{D} \to \mathcal{D}'$ be an
additive functor of triangulated categories which transforms direct sums
into direct sums. Then $F$ has a right adjoint. If $F$ is an exact
functor, then so is the adjoint.
\end{proposition}
...
$$
This is a cohomological functors as $F$ is exact and tranforms direct sums
...
Exactness if $F$ is exact, follows from Lemma \ref{lemma-adjoint-is-exact}.
\end{proof}
