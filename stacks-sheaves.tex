...
$$
H^p_{\acute{e}tale}(x, \mathcal{F}) =
H^p(U_{\acute{e}tale}, \mathcal{F}|_{U_{\acute{e}tale}}).
$$
\end{lemma}
...
an object of $\textit{Ab}(\mathcal{X}_\tau)$ or
$\textit{Mod}(\mathcal{X}_\tau, \mathcal{O}_\mathcal{X})$.
The the sheaf $R^if_*\mathcal{F}$ is the sheaf associated to the
presheaf
$$
y \longmapsto
...
\noindent
In this section we compare the constructions of this chapter with the
constructions for schemes and algebraic spaces.
...
\medskip\noindent
We first compare our definitions here with the case of schemes
...
Assume $\mathcal{X}$ is representable by a scheme $X$.
...
to $f$. For $\tau \in \{Zar,\linebreak[0] \acute{e}tale,\linebreak[0]
smooth,\linebreak[0] syntomic,\linebreak[0] fppf\}$
the morphism of ringed topoi
...
\noindent
Next we compare sheaves on algebraic stacks representable by
algebraic spaces. We only do this for the \'etale cohomology.

\begin{lemma}
\label{lemma-compare}
Let $S$ be a scheme. Let $\mathcal{X}$ be an algebraic stack over $S$.
Assume $\mathcal{X}$ is representably by an algebraic space $F$.
...
induced morphism of algebraic spaces, and
$f_{small} : F_{\acute{e}tale} \to G_{\acute{e}tale}$
...
Let $S$ be a scheme. Let $\mathcal{X}$ be an algebraic stack over $S$.
Let $\mathcal{F}$ be an $\mathcal{O}_\mathcal{X}$-module such that
\begin{enumerate}
...
\begin{enumerate}
\item If $\epsilon : \mathcal{X}_{fppf} \to \mathcal{X}_{\acute{e}tale}$
...
Combining
Adequate Modules,
Lemma \ref{adequate-lemma-quasi-coherent-and-flat-base-change}
and
...
sheaves on $\mathcal{X}_{\acute{e}tale}$ and $\mathcal{F}_{fppf}$
...
(resp.\ $Rf_{\acute{e}tale, *}\mathcal{F}$)
