...
is isomorphic to the big site of the scheme it lies over.
...
The functor $p$ induces an isomorphism of sites
$\mathcal{X}_\tau/x \to (\textit{Sch}/U)_\tau$.
\end{lemma}

\begin{proof}
Note that $(\textit{Sch}/U)_\tau$ is the localization of the site
$(\textit{Sch}/S)_{fppf}$ at the object $U$. It follows immediately from
...
Stacks, Definition \ref{stacks-definition-topology-inherited}
that coverings in $\mathcal{X}_\tau$ are in bijective correspondence
with coverings in $(\textit{Sch}/U)_\tau$.
\end{proof}
...
(without necessarily being sheaves). However, it seems best, for the moment
to resist the temptation to introduce a notation or terminology for each of
these variants.
