...
$x^{-1}\mathcal{F}$ is a sheaf on $(\textit{Sch}/U)_{fppf}$ (or on
...
\noindent
Let $f : \mathcal{X} \to \mathcal{Y}$ be a $1$-morphism of categories
fibred in groupoids over $(\textit{Sch}/S)_{fppf}$. Let $\mathcal{F}$
be a presheaf on $\mathcal{X}$. Let $y \in \text{Ob}(\mathcal{Y})$.
...
$$
\text{pr} :
(\textit{Sch}/V)_{fppf} \times_{y, \mathcal{Y}} \mathcal{X}
\longrightarrow
\mathcal{X}
$$
...
Let $f : \mathcal{X} \to \mathcal{Y}$ be a $1$-morphism of categories
fibred in groupoids over $(\textit{Sch}/S)_{fppf}$. The following are
equivalent
\begin{enumerate}
\item $f$ is representable, and
\item for every $y \in \text{Ob}(\mathcal{Y})$ the functor
$\mathcal{X}^{opp} \to \textit{Sets}$,
$x \mapsto \text{Mor}_\mathcal{Y}(f(x), y)$
is representable.
\end{enumerate}
\end{lemma}
...
for every $y \in \text{Ob}(\mathcal{Y})$
lying over $U$ the $2$-fibre product
$(\textit{Sch}/U)_{fppf} \times_{y, \mathcal{Y}} \mathcal{X}$
is representable, i.e., of the form $(\textit{Sch}/V_y)_{fppf}$ for some
...
representable. For every $y \in \text{Ob}(\mathcal{Y})$ we choose
an object $u(y) \in \text{Ob}(\mathcal{X})$ representing the functor
$x \mapsto \text{Mor}_\mathcal{Y}(f(x), y)$ of
...
the base change
$(\textit{Sch}/V')_{fppf} \times_{y', \mathcal{Y}} \mathcal{X} \to
(\textit{Sch}/V)_{fppf} \times_{y, \mathcal{Y}} \mathcal{X}$
of $V' \to V$.
...
categories fibred in groupoids over $(\textit{Sch}/S)_{fppf}$. Let
$\tau \in \{Zar, \acute{e}tale, smooth, syntomic, fppf\}$.
...
$\textit{Sh}(\mathcal{X}_\tau) \to \textit{Sh}(\mathcal{Y}_\tau)$
...
$x^{-1}\mathcal{F}$ is a (pre)sheaf of modules on $(\textit{Sch}/U)_{fppf}$.
...
Let $f : \mathcal{X} \to \mathcal{Y}$ be a $1$-morphism of categories
fibred in groupoids over $(\textit{Sch}/S)_{fppf}$. The pullback functor
$f^* = f^{-1} :
...
Let $f : \mathcal{X} \to \mathcal{Y}$ be a $1$-morphism of categories
fibred in groupoids over $(\textit{Sch}/S)_{fppf}$. If
$f$ induces an equivalence of stackifications, then the morphism
of topoi
$f : \textit{Sh}(\mathcal{X}_{fppf}) \to \textit{Sh}(\mathcal{Y}_{fppf})$
is an equivalence.
\end{lemma}
...
$y_i^*\mathcal{G}$ (as sheaves on $(\textit{Sch}/U_i)_{fppf}$ we
...
$(\textit{Sch}/T)_{fppf}$ (by the simple description of the
...
\textit{QCoh}((\textit{Sch}/X)_{fppf}, \mathcal{O}_X) \longrightarrow
\textit{Mod}((\textit{Sch}/X)_{fppf}, \mathcal{O}_X)
$$
isn't exact, see
Descent, Lemma \ref{descent-lemma-equivalence-quasi-coherent-limits}.
...
as $(\textit{Sch}/S)_{fppf}$ has fibre products.
...
is the presheaf on $(\textit{Sch}/U)_{fppf}$ which to
...
over a fibre product diagram in $(\textit{Sch}/S)_{fppf}$. Similarly for
...
Let $p : \mathcal{X} \to (\textit{Sch}/S)_{fppf}$ and
$q : \mathcal{Y} \to (\textit{Sch}/S)_{fppf}$ the structure morphisms.
...
$$
f_{p!}\mathcal{F}(y) =
\text{colim}_{(x, \phi) \in (\mathcal{I}_y^f)^{opp}} \mathcal{F}(x)
$$
...
$$
\text{Hom}_{\textit{Ab}(\mathcal{X}_\tau)}(\mathcal{F}, f^{-1}\mathcal{G})
=
\text{Hom}_{\textit{Ab}(\mathcal{Y}_\tau)}(f_!\mathcal{F}, \mathcal{G})
$$
...
$(\textit{Sch}/S)_{fppf}$. However, since this is a $(2, 1)$-category and
...
$(\textit{Sch}/S)_{fppf}$, then there is a morphism of {\v C}ech complexes
$$
\check{\mathcal{C}}^\bullet(\mathcal{U} \to \mathcal{X}, \mathcal{F})
\longrightarrow
...
fibred in groupoids over $(\textit{Sch}/S)_{fppf}$ as in
...
$$
is $2$-commutative diagram of categories fibred in groupoids over
$(\textit{Sch}/S)_{fppf}$, then there is a morphism
...
(\textit{Sch}/U)_{fppf} \times_{x, \mathcal{X}} \mathcal{U} \ar[r] \ar[d] &
\mathcal{U} \ar[d]^f \\
\mathcal{Y} \ar@{=}[r] &
(\textit{Sch}/U)_{fppf} \ar[r]^-x & \mathcal{X}
}
$$
...
$\mathcal{V}_n = 
(\textit{Sch}/U)_{fppf} \times_{x, \mathcal{X}} \mathcal{U}_n$.
Hence we see from
...
$\mathcal{X}/x \cong (\textit{Sch}/U)_{fppf}$. This implies for example that
Lemma \ref{lemma-homotopy}
implies
...
$(\textit{Sch}/S)_{fppf}$ and let $\mathcal{F}$ be an abelian presheaf
...
(\textit{Sch}/T)_{fppf} \times_{x, \mathcal{X}} \mathcal{U} \to 
(\textit{Sch}/T)_{fppf}, x^{-1}\mathcal{F})
$$
by
...
\check{\mathcal{C}}^\bullet(
(\textit{Sch}/T)_{fppf} \times_{y, \mathcal{Y}} \mathcal{V} \to 
(\textit{Sch}/T)_{fppf}, y^{-1}e^{-1}\mathcal{F})
$$
also by
...
$$
(\textit{Sch}/T)_{fppf} \times_{y, \mathcal{Y}} \mathcal{V} \to
(\textit{Sch}/T)_{fppf} \times_{x, \mathcal{X}} \mathcal{U}
$$
...
in groupoids over $(\textit{Sch}/S)_{fppf}$. Let
$\tau \in \{Zar, \acute{e}tale, smooth, syntomic, fppf\}$.
Let
$$
...
of abelian sheaves on $(\textit{Sch}/T)_\tau$. It suffices to show
...
u_i^{-1}f^{-1}\mathcal{H}$ of abelian sheaves on $(\textit{Sch}/T_i)_\tau$
is exact. Since
$u_i^{-1}f^{-1}\mathcal{F} = x^{-1}\mathcal{F}|_{(\textit{Sch}/T_i)_\tau}$
...
We may think of $y$ as a morphism $(\textit{Sch}/V)_{fppf} \to \mathcal{Y}$.
By definition the $2$-fibre product
$\mathcal{X} \times_\mathcal{Y} (\textit{Sch}/V)_{fppf}$
...
$(\textit{Sch}/U)_{fppf} \to \mathcal{X}$. Then $\{f(x) \to y\}$ is
...
$$
(\textit{Sch}/V_i)_{fppf} \to (\textit{Sch}/U)_{fppf} \to \mathcal{X}.
$$
Then $\{f(x_i) \to y\}$ is
...
\medskip\noindent
More generally, when $f : \mathcal{X} \to \mathcal{Y}$ is a $1$-morphism
of algebraic stacks over $S$, then we have a left exact pushforward functor
$f_* : \textit{Ab}(\mathcal{X}_\tau) \to \textit{Ab}(\mathcal{Y}_\tau)$
which has a total derived version
$Rf_*\mathcal{F}$ and higher direct images $R^if_*\mathcal{F}$.
Indicating which topology is used to compute these is important!
...
of algebraic stacks for the \'etale and the fppf topology. Recall that
$$
f_p : \mathcal{U}_p =
...
Assume that $f$ is representable by an algebraic spaces, surjective and
smooth. Then there is a spectral sequence
$$
...
Assume that $\mathcal{U}$ is representable by an algebraic space and that
$f$ is surjective, flat, and locally of finite presentation. Then there is
a spectral sequence
$$
...
\noindent
Let $f : \mathcal{X} \to \mathcal{Y}$ be a $1$-morphism of categories
fibred in groupoids over $(\textit{Sch}/S)_{fppf}$. Consider the
pushforward
$$
f_* :
\textit{Mod}(\mathcal{O}_\mathcal{X})
...
$$
f = j_{big} : \mathcal{X} = (\textit{Sch}/X)_{fppf} \to
(\textit{Sch}/Y)_{fppf} = \mathcal{Y}
$$
The pushforward
...
consider $T = \text{Spec}(k) \to \mathbf{A}^2_k = Y$ mapping to $0$.
...
$$
for any $y \in \text{Ob}(\mathcal{Y})$ such that the associated
