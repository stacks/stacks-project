...
\item The {\it associated affine syntomic site} $\mathcal{X}_{syntomic}$
is the structure of site on $\mathcal{X}_{affine}$ inherited from
...
\'Etale Cohomology, Lemma \ref{etale-cohomology-lemma-alternative}.
...
Having said this we omit the proof.
\end{proof}
...
\noindent
To show that the construction is interesting, let us state and prove
a characterization of $\mathit{QC}(\text{Spf}(A))$ for a Noetherian
adic ring $A$.
