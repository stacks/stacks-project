...
over $(\Sch/S)_{fppf}$. For any $\tau \in \{Zariski, \acute{e}tale, smooth,
...
over $(\Sch/S)_{fppf}$. Let $\tau \in \{Zariski, \acute{e}tale, smooth,
...
$$
H^p(x, \mathcal{F}) = H^p((\Sch/U)_\tau, x^{-1}\mathcal{F})
$$
and if $\tau = \acute{e}tale$, then we also have
$$
H^p(x, \mathcal{F}) = H^p(U_{\acute{e}tale}, \mathcal{F}|_{U_{\acute{e}tale}}).
$$
\end{lemma}
...
Let $S$ be a scheme. Let $\mathcal{X}$ be a category fibred in groupoids
over $(\Sch/S)_{fppf}$. Denote
$\epsilon : \mathcal{X}_{fppf} \to \mathcal{X}_{\acute{e}tale}$
the comparison morphism. Let $\mathcal{F}$ be an
$\mathcal{O}_\mathcal{X}$-module such that
\begin{enumerate}
\item $\mathcal{F}$ is locally quasi-coherent, and
\item for any morphism $\varphi : x \to y$ of $\mathcal{X}$ which lies
...
(\ref{equation-comparison-modules}) is an isomorphism.
\end{enumerate}
Then $R\epsilon_*\mathcal{F} = \epsilon_*\mathcal{F}$.
\end{lemma}

\begin{proof}
The sheaf $R^p\epsilon_*\mathcal{F}$ is the sheaf associated to the presheaf
$x \mapsto H^p_{fppf}(x, \mathcal{F})$. Hence it suffices to show that
$H^p_{fppf}(x, \mathcal{F}) = 0$ for $p > 0$
whenever $x$ lies over an affine scheme $U$. By
Lemma \ref{lemma-cohomology-restriction}
we have $H^p_{fppf}(x, \mathcal{F}) = H^p((\Sch/U)_{fppf}, x^{-1}\mathcal{F})$.
Combining
Adequate, Lemma \ref{adequate-lemma-quasi-coherent-and-flat-base-change}
and
...
\noindent
We formulate the following comparison statements as lemmas, even though
their proofs consist mainly in working out the meaning of the definitions.
We encourage the reader to think about why these statements are true
rather than reading the formal proofs.
...
Let $\mathcal{X}$ be an algebraic stack over $S$.
\begin{enumerate}
\item Assume $\mathcal{X}$ is representable by a scheme $X$.
For $\tau \in \{Zar,\linebreak[0] \acute{e}tale,\linebreak[0]
smooth,\linebreak[0] syntomic,\linebreak[0] fppf\}$
there is a canonical equivalence
$$
(\Sh(\mathcal{X}_\tau), \mathcal{O}_\mathcal{X}) =
(\Sh((\Sch/X)_\tau), \mathcal{O}_X)
$$
of ringed topoi.
\item Assume $\mathcal{X}$ is representably by an algebraic space $F$.
Then there exists a continuous and cocontinuous functor
$
F_{\acute{e}tale} \to \mathcal{X}_{\acute{e}tale}
$
which induces morphisms of ringed topoi
$$
\xymatrix{
(\Sh(\mathcal{X}_{\acute{e}tale}), \mathcal{O}_\mathcal{X}) \ar[r]^-{\pi_F} &
(\Sh(F_{\acute{e}tale}), \mathcal{O}_F) &
(\Sh(F_{\acute{e}tale}), \mathcal{O}_F) \ar[r]^-{i_F} &
(\Sh(\mathcal{X}_{\acute{e}tale}), \mathcal{O}_\mathcal{X})
}
$$
...
\begin{proof}
Part (1) follows by choosing an equivalence
$(\Sch/X)_\tau \to \mathcal{X}_\tau$ and using the functoriality of
the construction $\mathcal{X} \leadsto \mathcal{X}_\tau$.

\medskip\noindent
To see (2) choose an equivalence $j : \mathcal{S}_F \to \mathcal{X}$, see
Algebraic Stacks, Sections \ref{algebraic-section-split} and
\ref{algebraic-section-representable-by-algebraic-spaces}.
An object of $F_{\acute{e}tale}$ is a scheme $U$ together with an
...
$i_F : \Sh(F_{\acute{e}tale}) \to \Sh(\mathcal{X}_{\acute{e}tale})$
...
$$
is a coequalizer diagram in $\Sh(F_{\acute{e}tale})$. Using the
...
$$
is a coequalizer diagram in $\Sh(F_{\acute{e}tale})$. Using that
$j$ is an equivalence and that $F = U/R$ it follows that
the coequalizer in $\Sh(\mathcal{X}_{\acute{e}tale})$ of the
...
$u : F_{\acute{e}tale} \to \mathcal{X}_{\acute{e}tale}$ the
functor of the proof of Lemma \ref{lemma-compare} above.
Given a sheaf $\mathcal{F}$ on $\mathcal{X}_{\acute{e}tale}$ we have
$$
...
\mathcal{F}|_{F_{\acute{e}tale}} = i_F^{-1}\mathcal{F} = \pi_{F, *}\mathcal{F}
\end{equation}
in this situation.
...
space $F$.
\begin{enumerate}
\item $\mathcal{I}|_{F_{\acute{e}tale}}$ is injective in
$\textit{Ab}(F_{\acute{e}tale})$ for $\mathcal{I}$ injective in
$\textit{Ab}(\mathcal{X}_{\acute{e}tale})$, and
\item $\mathcal{I}|_{F_{\acute{e}tale}}$ is injective in
$\textit{Mod}(F_{\acute{e}tale}, \mathcal{O}_F)$
for $\mathcal{I}$ injective in
$\textit{Mod}(\mathcal{X}_{\acute{e}tale}, \mathcal{O})$.
\end{enumerate}
\end{lemma}

\begin{proof}
This follows formally from the fact that the restriction functor
$\pi_{F, *} = i_F^{-1}$ is an exact left adjoint of $i_{F, *}$, see
Homology, Lemma \ref{homology-lemma-adjoint-preserve-injectives}.
\end{proof}
...
of algebraic stacks over $S$.
\begin{enumerate}
\item Assume $\mathcal{X}$, $\mathcal{Y}$ are representable by schemes
$X$, $Y$. Then the morphisms of ringed topoi
$f : (\Sh(\mathcal{X}_\tau), \mathcal{O}_\mathcal{X}) \to
(\Sh(\mathcal{X}_\tau), \mathcal{O}_\mathcal{X})$
agree with the morphisms of ringed topoi
$f : (\Sh((\Sch/X)_\tau), \mathcal{O}_X) \to 
(\Sh((\Sch/Y)_\tau), \mathcal{O}_Y)$ via the identifications of
Lemma \ref{lemma-compare}.
\item Assume $\mathcal{X}$, $\mathcal{Y}$ are representable by
algebraic spaces $F$, $G$. Denote $f : F \to G$ the induced morphism
of algebraic spaces, and $f_{small} : F_{\acute{e}tale} \to G_{\acute{e}tale}$
the corresponding morphism of ringed topoi. Then
$$
\xymatrix{
(\Sh(\mathcal{X}_{\acute{e}tale}), \mathcal{O}_\mathcal{X})
\ar[d]_{\pi_F} \ar[rr]_f & &
(\Sh(\mathcal{Y}_{\acute{e}tale}), \mathcal{O}_\mathcal{Y}) \ar[d]^{\pi_G} \\
(\Sh(F_{\acute{e}tale}), \mathcal{O}_F) \ar[rr]^{f_{small}} & &
(\Sh(G_{\acute{e}tale}), \mathcal{O}_G)
}
$$
...
\begin{proof}
Part (1) follows on unwinding the definitions.
Part (2) is similar to
...
$u : F_{\acute{e}tale} \to \mathcal{X}$ and
$u' : G_{\acute{e}tale} \to \mathcal{Y}$ as in the proof of
...
(especially,
Properties of Spaces, Remark \ref{spaces-properties-remark-explain-equivalence}
and
...
(f_*\mathcal{F})|_{G_{\acute{e}tale}} =
f_{small, *}(\mathcal{F}|_{F_{\acute{e}tale}})
\end{equation}
for any sheaf $\mathcal{F}$ on $\mathcal{X}_{\acute{e}tale}$.
Moreover, if $\mathcal{F}$ is a sheaf of $\mathcal{O}$-modules, then
(\ref{equation-compare-big-small}) is an isomorphism of
$\mathcal{O}_G$-modules on $G_{\acute{e}tale}$.
This in particular implies that we can compute cohomology
on the level of the underlying morphism of algebraic spaces.
...
\begin{enumerate}
\item For any $\mathcal{F} \in \textit{Ab}(\mathcal{X}_{\acute{e}tale})$
we have
$$
(Rf_*\mathcal{F})|_{G_{\acute{e}tale}} =
Rf_{small, *}(\mathcal{F}|_{F_{\acute{e}tale}})
$$
