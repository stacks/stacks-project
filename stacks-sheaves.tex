...
notation would be
$c_\varphi : f_{small}^{-1}y^*\mathcal{F}|_{V_{\acute{e}tale}}) \to
x^*\mathcal{F}|_{U_{\acute{e}tale}}$. If $\psi : y \to z$ is a
...
this description simplifies a bit.
...
functor, see
Sites, Lemma \ref{sites-definition-special-cocontinuous-functor}
...
\section{Lower shriek}
\label{section-lower-shriek}

\noindent
The main goal is to prove that pullback $f^{-1}$ has a left adjoint $f_!$
on abelian sheaves. We first prove two preliminary lemmas about fibre
products and equalizers in categories fibred in groupoids and their
behaviour with respect to morphisms.
...
\section{The sheafified {\v C}ech complex}
...
$$
is called the {\it sheafified {\v C}ech complex} associated to the situation.
We will denote this complex $\mathcal{K}^\bullet(f, \mathcal{F})$.
The {\it extended sheafified {\v C}ech complex} is the complex
$$
\ldots \to 0 \to
\mathcal{F} \to
f_{0, *}f_0^{-1}\mathcal{F} \to
...
f_{2, *}f_2^{-1}\mathcal{F} \to \ldots
$$
with $\mathcal{F}$ in degree $-1$. The extended sheafified {\v C}ech complex
...
Generalities on sheafified {\v C}ech complexes.
\begin{enumerate}
\item If
$$
\xymatrix{
...
$\mathcal{K}^\bullet(f, \mathcal{F}) \to
\mathcal{K}^\bullet(g, e^{-1}\mathcal{F})$.
\item if $h$ and $e$ are equivalences, then the map of (1) is an isomorphism,
\item if $f, f' : \mathcal{U} \to \mathcal{X}$ are $2$-isomorphic, then
the associated sheafified {\v C}ech complexes are isomorphic,
\end{enumerate}
\end{lemma}
...
If there exists a $1$-morphism $s : \mathcal{X} \to \mathcal{U}$
such that $f \circ s$ is $2$-isomorphic to $\text{id}_\mathcal{X}$
then the extended sheafified {\v C}ech complex is homotopic to zero.
\end{lemma}
...
also if $\mathcal{F}$ is a presheaf, only we cannot use the reference
to
Sites, Section \ref{sites-section-pullback}
...
Let us ``compute'' the value of the sheafified {\v C}ech complex on an
...
$$
In words: The value of the sheafified {\v C}ech complex on an object $x$ of
...
results for the sheafified {\v C}ech complex.
\end{remark}
