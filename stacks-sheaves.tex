...
\begin{proof}
To make sense out of the statement and especially condition (2), recall that
we think of $\mathcal{X}_{affine}$ as a site by endowing the category
$\mathcal{X}_{affine}$ with the chaotic topology
(Definition \ref{definition-alternative}). A sheaf
of $\mathcal{O}$-modules $\mathcal{F}$ is the same thing as
a presheaf of $\mathcal{O}$-modules.
Having said this we omit the proof; hints: (a) conditions (1) and (2)
are equivalent exactly because the topology on
$\mathcal{X}_{affine}$ is the chaotic one, and (b) to prove the
equivalence of (2) -- (8) one argues in exactly
the same manner as in the proof of Descent, Lemma
\ref{descent-lemma-quasi-coherent-alternative}.
\end{proof}
...
\noindent
Let $p : \mathcal{X} \to (\Sch/S)_{fppf}$ be a category fibred in groupoids.
In this section we introduce a triangulated category $\mathit{QC}(\mathcal{X})$
whose objects may be viewed as objects in some derived category of modules
associated to $\mathcal{X}$ which are ``derived quasi-coherent'' in a sense.
As a motivation for the precise definition we point the reader to the
characterization, in Lemma \ref{lemma-quasi-coherent-alternative},
...
$\mathcal{X}_{affine}$, but in general are not quasi-coherent. The last
...
\begin{proof}
We have to show that for every morphism $x \to x'$ of $\mathcal{X}_{affine}$
the map
$$
H^p(M)(x') \otimes_{\mathcal{O}(x')} \mathcal{O}(x) \to H^b(M)(x)
$$
is an isomorphism and we are given that the map
$$
R\Gamma(x', M) \otimes_{\mathcal{O}(x')}^\mathbf{L} \mathcal{O}(x)
\to R\Gamma(x, M)
$$
...
\noindent
Another interesting observation is that if we apply the definition in case
our category fibred in groupoid $\mathcal{X}$ is representable by an
algebraic space $X$, then we recover $D_\QCoh(\mathcal{O}_X)$.
...
Restriction along the inclusion functor $X_{affine} \to \mathcal{X}_{affine}$
is compatible with structure sheaves and produces a functor
$\mathit{QC}(\mathcal{X}) \to \mathit{QC}(X_{affine}, \mathcal{O}_X)$, see
...
there is a canonical equivalence of $\mathit{QC}(X_{affine}, \mathcal{O}_X)$
with $D_\QCoh(\mathcal{O}_X)$. All in all we obtain an exact
functor
$$
G : \mathit{QC}(\mathcal{X}) \longrightarrow D_\QCoh(\mathcal{O}_X)
$$
Let us construct a quasi-inverse.

\medskip\noindent
Consider the morphism $\pi_F : (\mathcal{X}_\etale, \mathcal{O}_\mathcal{X})
\to (X_\etale, \mathcal{O}_X)$ constructed in Lemma \ref{lemma-compare}.
Given any object $K$ of $D(X_\etale, \mathcal{O}_X)$ we can consider
$L\pi^*K$ in $D(\mathcal{X}_\etale, \mathcal{O}_\mathcal{X})$. Then we
can represent $L\pi^*K$ by a K-injective complex $\mathcal{I}^\bullet$ and
consider the complex of $\mathcal{O}$-modules on $\mathcal{X}_{affine}$
given by the rule
$$
x \longmapsto \mathcal{I}^\bullet(x)
$$
This construction defines an exact functor
$F : D(X_\etale, \mathcal{O}_X) \to D(\mathcal{X}_{affine}, \mathcal{O})$.
It has the following property: given $K$ as above and given an object
$x$ lying over the affine scheme $U$, then $F(K)(x) = R\Gamma(U, Lf^*K)$
where $f : U \to X$ is the morphism corresponding to $x$. We omit the
proof of this statement.

\medskip\noindent
We claim that if $K$ is in $D_\QCoh(\mathcal{O}_X)$, then $F(K)$
is in $\mathit{QC}(\mathcal{X})$. Namely, suppose that $x \to x'$
is a morphism of $\mathcal{X}_{affine}$ lying over the morphism
$h : U \to U'$ of affine schemes. By the above we see that
$F(K)(x) = R\Gamma(U, Lf^*K)$ and $F(K)(x') = R\Gamma(U', L(f')^*K)$
where $f : U \to X$ and $f' : U' \to X$ are such that
$f = f' \circ h$. Hence we see that
$$
F(K)(x) = R\Gamma(U, Lf^*K) = R\Gamma(U, Lh^*L(f')^*K) =
R\Gamma(U', L(f')^*K) \otimes_{\mathcal{O}(U')}^\mathbf{L} \mathcal{O}(U)
$$
as desired. The final equality is
Derived Categories of Schemes, Lemma
\ref{perfect-lemma-quasi-coherence-pullback}.

\medskip\noindent
The claim tells us that $F$ determines an exact functor
$F' : D_\QCoh(\mathcal{O}_X) \to \mathit{QC}(\mathcal{X})$
in the other direction. It is rather straightforward to see
that $G \circ F'$ is the identity functor.
FIXME.
\end{proof}
...
and Formal Algebraic Spaces, Definition
