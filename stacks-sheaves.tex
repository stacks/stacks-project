...
Let $\mathcal{X}$ be an algebraic stack over $S$.
\begin{enumerate}
\item The category $\textit{QCoh}(\mathcal{O}_\mathcal{X})$ is a Grothendieck
abelian category. Consequently, $\textit{QCoh}(\mathcal{O}_\mathcal{X})$
...
\medskip\noindent
Part (1) means $\textit{QCoh}(\mathcal{O}_\mathcal{X})$ (a) has all colimits,
...
$$
\textit{QCoh}(\mathcal{O}_\mathcal{X})^{opp}
\longrightarrow
\textit{Sets},
\quad
\mathcal{G}
...
for $\mathcal{G}$ in $\textit{QCoh}(\mathcal{O}_\mathcal{X})$.
By the Yoneda lemma
