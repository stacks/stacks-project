...
$d = d(C, IC)$ and $q(C, IC)$ the integers found in
Local Cohomology, Section \ref{local-cohomology-section-uniform},
\item an integer $n \geq \max(q(C, IC) + (d + 1)c, 2(d + 1)c + 1)$, and
...
\begin{proof}
Observe that by our assumption on $\NL_{B/A}^\wedge$ and by
More on Algebra, Lemmas
\ref{more-algebra-lemma-base-change-property-ext-1-annihilated} and
\ref{more-algebra-lemma-two-term-base-change}
the module $\Ext^1_C(\NL_{B/A}^\wedge \otimes_B C, N)$ is annihilated by
$(IC)^c$ for all $C$-modules $N$. Consider the obstruction class
$$
o(\psi_n) \in \Ext^1_C(\NL_{B/A}^\wedge \otimes_B C, I^nC/I^{2n}C)
$$
of Remark \ref{remark-improve-homomorphism}. Since
$n \geq q(C, IC) + (d + 1)c$ we conclude by
...
\ref{more-algebra-lemma-two-term-base-change}
the module $\Ext^1_C(\NL_{B/A}^\wedge \otimes_B C, N)$ is annihilated by
$a^c$ for all $C$-modules $N$. Consider the obstruction class
$$
o(\psi_n) \in \Ext^1_C(\NL_{B/A}^\wedge \otimes_B C, a^nC/a^{2n}C)
$$
...
Hence we conclude that the class $o(\psi_n)$ maps to zero in
$$
\Ext^1_C(\NL_{B/A}^\wedge \otimes_B C, a^{n - c}C/a^{2n - c}C)
$$
and a fortiori in
$$
\Ext^1_C(\NL_{B/A}^\wedge \otimes_B C, a^{n - c}C/a^{2n - 2c}C)
$$
...
Let $S$ be a Noetherian graded ring. There exist integers
$d_0 = d(S)$ and $q_0 = q(S)$ such that for any pair $(A, I)$
consisting of an ideal $I$ in a Noetherian ring $A$ with
$\text{Gr}_I(A) \cong S$ we have $d(A, I) = d_0$ and
$q(A, I) \leq q_0$. Here $d(A, I)$ and $q(A, I)$ are the integers found in
Local Cohomology, Section \ref{local-cohomology-section-uniform}.
\end{lemma}

\begin{proof}
We may and do assume $S$ is not the zero ring (as that case is trivial).
We may and do assume $S$ is generated in degree $1$ over $S_0$
since if not then no pairs $(A, I)$ as in the lemma exist.
Set $Y = \text{Proj}(S)$.

\medskip\noindent
Recall that $d(A, I)$ is the maximum dimension of the fibres of
the morphism $p : X = \text{Proj}(\bigoplus_{n \geq 0} I^n) \to \Spec(A)$
from the blowing up of $\Spec(A)$ in $I$ to $\Spec(A)$.
Recall that
$$
X \times_{\Spec(A)} \Spec(A/I) =
\text{Proj}(\bigoplus\nolimits_{n \geq 0} I^n/I^{n + 1}) = Y
$$
Since $p$ is an isomorphism over $\Spec(A) \setminus V(I)$
we see that $d(A, I)$ is the maximum dimension $d_0$ of the
fibres of $Y \to \Spec(S_0) = \Spec(A/I)$.

\medskip\noindent
By Cohomology of Schemes, Lemmas \ref{coherent-lemma-coherent-on-proj} and
\ref{coherent-lemma-recover-tail-graded-module} we can choose
a $q_0 = q(S) \geq 0$ such that for all $q \geq q_0$ we have
$H^i(Y, \mathcal{O}_Y(q)) = 0$ for $1 \leq i \leq d$ and
$H^0(Y, \mathcal{O}_Y(q)) = S_q$. Let $(A, I)$ be a pair as
in the lemma. Using the isomorphisms above we see that $Y$
is the exceptional divisor of the blowing up. Thus
$\mathcal{O}_X(1) = \mathcal{O}_X(-Y)$. Hence we get
short exact sequences
$$
0 \to \mathcal{O}_X(q + 1) \to \mathcal{O}_X(q) \to \mathcal{O}_Y(q) \to 0
$$
Taking the associated cohomology sequence we find
$$
H^i(X, \mathcal{O}_X(q + 1)) \to
H^i(X, \mathcal{O}_X(q)) \to
H^i(Y, \mathcal{O}_Y(q))
$$
Hence we see that
$$
\ldots \to
H^i(X, \mathcal{O}_X(q_0 + 2)) \to
H^i(X, \mathcal{O}_X(q_0 + 1)) \to
H^i(X, \mathcal{O}_X(q_0))
$$
are all surjective for $i > 0$. Since we have the vanishing for
very large $q$ (by references above applied to $X$) we conclude
$H^i(X, \mathcal{O}_X(q)) = 0$ for $i > 0$ and $q \geq q_0$.
Finally, for $i = 0$ we obtain a commutative diagram
$$
\xymatrix{
0 \ar[r] &
I^{q + 1} \ar[d] \ar[r] &
...
0 \\
0 \ar[r] &
H^0(X, \mathcal{O}_X(q + 1)) \ar[r] &
H^0(X, \mathcal{O}_X(q)) \ar[r] &
H^0(Y, \mathcal{O}_Y(q)) \ar[r] &
0
}
$$
with exact rows for $q \geq q_0$.
Moreover, since $q \geq q_0$ the right vertical arrow is
an isomorphism by choice of $q_0$. Again if $q + 1$ is large
enough the left vertical arrow is an isomorphism as well.
Then inductively we find that we have isomorphisms for
lower $q$ as long as $q \geq q_0$. This finishes the proof.
\end{proof}
...
\begin{proof}
Let $n \geq 1$ be any integer. Let $g'_1, \ldots, g'_m$ and $K'$ be
as in the lemma. Since $g_i = g'_i \in I^nP$ we obtain a canonical
$A$-algebra homomorphism
$$
\psi_n : B \longrightarrow B'/I^nB'
$$
which induces an isomorphism $B/I^nB \to B'/I^nB'$. The idea is
to use Lemma \ref{lemma-get-morphism-general-better}
to lift a reduction of $\psi_n$ modulo
a lower power of $I$ to an isomorphism.

\medskip\noindent
By Definition \ref{definition-rig-smooth-homomorphism}
we can choose a $c \geq 0$ such that $I^c$ annihilates
$\Ext^1_B(\NL_{B/A}^\wedge, N)$ for all $B$-modules $N$.
By More on Algebra, Lemmas \ref{more-algebra-lemma-approximate-complex} and
...
By Lemma \ref{lemma-bound-q-and-d} there exist integers
$d_0 = d(\text{Gr}_I(B))$ and $q_0 = q(\text{Gr}_I(B))$ such
that $d(B', IB') = d_0$ and $q(B', IB') \leq q_0$.
Hence if $n > \max(c_1 + 1, q_0 + (d_0 + 1)c, 2(d_0 + 1)c + 1)$
then we can apply Lemma \ref{lemma-get-morphism-general-better}
...
We first prove the principal ideal case and then use the results
of Section \ref{} to finish the proof.
