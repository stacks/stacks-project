...
Dualizing Complexes, Lemma \ref{dualizing-lemma-quotient-by-idempotent}.
...
If $U \to X$ ranges over \'etale morphisms wtih $U$ affine, then
...
$$
is quasi-compact. Hence we conclude the equvalence holds by the equality
$$
|V_j \times_Y V_{j'}| =
...
we obtain a commutaive diagram
$$
\xymatrix{
...
it suffices to show that given any commtutative diagram
$$
\xymatrix{
\Spec(K) \ar[r] \ar[d] & Y \ar[d] \\
...
Formal Spaces, Lemma \ref{formal-spaces-lemma-map-into-algebraic-space}
we find a morphism $\Spec(R) \to Y$ lifing $g$ as desired.
\end{proof}
...
Algebraizing the composition $\text{Spf}(R') \to Y$ using
Formal Spaces, Lemma \ref{formal-spaces-lemma-map-into-algebraic-space}
we find a morphism $\Spec(R') \to Y$ lifing $g$. By the discussion
...
$\mathfrak m$-adic completion $A^\wedge$. Then base change defines
an equivalence of categories between the category (\ref{equation-modification})
for $A$ with the category (\ref{equation-modification})
for the completion $A^\wedge$.
\end{lemma}

\begin{proof}
Set $S = \Spec(A)$ as in (\ref{equation-modification}) and
$T = V(\mathfrak m)$. Similarly, Write $S' = \Spec(A^\wedge)$ and
$T' = V(\mathfrak m^\wedge)$. The morphism $S' \to S$ defines an
isomorphism $S'_{/T'} \to S_{/T}$ of formal completions.
Let $\mathcal{C}_{S, T}$, $\mathcal{C}_{S_{/T}}$, $\mathcal{C}_{S'_{/T'}}$,
and $\mathcal{C}_{S', T'}$ be the corresponding
...
\mathcal{C}_{S'_{/T'}} = \mathcal{C}_{S', T'}
$$
Note that $f : X \to S$ is an object of (\ref{equation-modification})
if and only if $f : X \to S$ is an object of $\mathcal{C}_{S, T}$
and $f$ is proper. Hence, to finish the proof we have to show that an object
$f : X \to S$ of $\mathcal{C}_{S, T}$ is proper over $S$
if and only if the base change $f' : X' \to S'$ is proper over $S'$.
This you can deduce from
Lemma \ref{lemma-output-proper}
(translating the properness into properties of the formal
completion which lives in $ \mathcal{C}_{S_{/T}} =
\mathcal{C}_{S'_{/T'}}$), or you can deduce it from
