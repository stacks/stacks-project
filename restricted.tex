...
$\text{Gr}_I(B) \cong S$ we have $d(A, I) = d_0$ and
$q(B, I) \leq q_0$. Here $d(A, I)$ and $q(A, I)$ are the integers found in
Local Cohomology, Section \ref{local-cohomology-section-uniform}.
\end{lemma}
...
\text{Proj}(\bigoplus_{n \geq 0} I^n/I^{n + 1}) = Y
$$
...
$H^i(X, \mathcal{O}_X(q))$ for $i > 0$ and $q \geq q_0$.
...
$$
with exact rows for $q \geq q_0$ (some details omitted).
...
$$
is exact and such that $\text{Gr}_I(B) \cong \text{Gr}_I(B')$.
...
of a finite $A$-algebra $C$ with respect to the ideal $IC$. Let
