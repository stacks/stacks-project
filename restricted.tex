...
to a morphism of $\textit{WAdm}^{count}$ which is adic and
...
to a morphism of $\textit{WAdm}^{count}$ which is adic and topologically
of finite type, and
...
integer $c \geq 0$ such that $I^c \subset I'$ and $(I')^c \subset I$.
In particular $V(I) = V(I')$. Hence $B$ is rig-smooth over $(A, I)$
if and only if $B$ is rig-smooth over $(A, I')$ by
Lemma \ref{lemma-base-change-rig-smooth-homomorphism}
(applied to $\text{id} : A \to A$ twice). An alternative
argument is to deduce it from
Definition \ref{definition-rig-smooth-homomorphism}
and the inclusions $I^c \subset I'$ and $(I')^c \subset I$.
However, to do so we need to show that the naive cotangent complex
$\NL_{B/A}^\wedge$ constructed using $(A, I)$ is the same as
the naive cotangent complex $\NL_{B/A}^\wedge$
constructed using $(A, I')$. This can be seen
from the construction of $\NL_{B/A}^\wedge$ in (\ref{equation-NL})
using that $A[x_1, \ldots, x_r]$ completed with respect to $I$
is the same as $A[x_1, \ldots, x_r]$ completed with respect to $I'$
(both are equal to $A\{x_1, \ldots, x_r\}$ by
Formal Algebraic Spaces, Remark
\ref{formal-spaces-remark-I-adic-completion-and-restricted-power-series}).
\end{proof}
