...
\noindent
In Formal Algebraic Spaces, Section \ref{section-finite-type}
...
\end{itemize}
See Formal Algebraic Spaces, Definition
...
\begin{proof}
Combine Formal Algebraic Spaces, Remark
\ref{formal-spaces-remark-composition-variant-Noetherian}
...
Observe that this means $B \to A$ is adic (Formal Algebraic Spaces, Lemma
...
morphism of schemes (by Formal Algebraic Spaces, Lemma
...
\noindent
A typical example is given in Example \ref{example-formal-modification}
and indeed we will later show that every formal modification is
``formal locally'' of this type (insert future reference here).
...
\begin{example}
\label{example-formal-modification}
Let $S$ be a scheme. Let $Y$ be a locally Noetherian algebraic space
over $S$, let $T \subset |Y|$ be a closed subset, and let $V \subset Y$
be the open subspace with $|V| = |Y| \setminus T$. Picture
$$
V \to Y \quad |Y| = |V| \amalg T
$$
Finally, let $f : X \to Y$ be a proper morphism of locally
Noetherian algebraic spaces such that $f^{-1}(V) \to V$ is
an isomorphism. Set $T' \subset |X|$ equal to the inverse image of $T$.
Picture
$$
\vcenter{
\xymatrix{
...
\xymatrix{
& |X| \ar[d] & T' \ar[l] \ar[d] \\
|V| \ar[r] \ar[ru] & |Y| & T \ar[l]
}
}
$$
In this situation we claim that the induced morphism
$$
f_{/T} : X_{/T'} \longrightarrow Y_{/T}
$$
of formal completions is a formal modification.
...
formal algebraic spaces. Since the diagram
$$
\xymatrix{
X_{/T'} \ar[r] \ar[d]^{f_{/T}} & X \ar[d]^f \\
Y_{/T} \ar[r] & Y
}
$$
is cartesian (as follows from the definitions) and since $f$ is
proper, it follows that $f_{/T}$ is proper. By
Lemma \ref{lemma-completion-proper-surjective-rig-surjective}
the morphism $f_{/T}$ is rig-surjective...
FIXME
\end{example}
