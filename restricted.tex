...
the category of algebraic spaces over $X$ which are quasi-compact,
quasi-separated, and such that the structure morphism $f : X \to Y$
...
$S = \Spec(k)$, $Y = \mathbf{A}^1_k$, and $T = \{0\}$. Then
...
Thus it is necessary to allow objects in the source
category to be nonseparated.

\begin{proof}
Write $X = \Spec(A)$. Let $I \subset A$ be an ideal such that
$T = V(I)$. Then $X_{/T} = \text{Spf}(A^\wedge)$ where $A^\wedge$
is the $I$-adic completion of $A$. To be continued soon.
\end{proof}
