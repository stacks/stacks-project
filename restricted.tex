...
More on Algebra, Lemma \ref{more-algebra-lemma-projective-amplitude}.
If $R$ is Noetherian and $H^i(K)$ is a finite $R$-module for
...
\ref{more-algebra-lemma-projective-amplitude-pseudo-coherent}.
\end{proof}

\begin{remark}
\label{remark-smoothness-ext-1-zero}
The following two statements follow from Lemma \ref{lemma-ext-1-zero}
and Algebra, Definition \ref{algebra-definition-smooth} and
...
Let $K \in D(A)$ with $H^i(K) = 0$ for $i \not \in \{-1, 0\}$.
The following are equivalent
\begin{enumerate}
\item there exists a $c \geq 0$ such that $I^c$ annihilates
$H^{-1}(K)$ and multiplication by $a \in I^c$ on $H^0(K)$
factors through a projective module,
\item there exists a $c \geq 0$ such that $I^c$ annihilates
$\Ext^1_R(K, M)$ for every $R$-module $M$.
\end{enumerate}
If $R$ is Noetherian and $H^i(K)$ is a finite $R$-module
for $i = -1, 0$, then these are also equivalent to
\begin{enumerate}
\item[(3)] $H^{-1}(K)$ is $I$-power torsion and $H^0(K)$
defines a finite locally free module on $\Spec(R) \setminus V(I)$, and
\item[(4)] there exists a $c \geq 0$ such that $I^c$ annihilates
$\Ext^1_R(K, M)$ for every finite $R$-module $M$.
\end{enumerate}
\end{lemma}
...
\section{Uniform vanishing of a system of Exts}
\label{section-uniform}

\noindent
Let $R$ be a ring and let $J \subset R$ be an ideal. In this section we say an
$R$-module $M$ is {\it $J$-projective} if the following equivalent conditions
hold
\begin{enumerate}
\item for every $a \in J$ the map $a : M \to M$ factors through a projective
$R$-module,
\item for every $a \in J$ the map $a : M \to M$ factors through a free
$R$-module, and
\item $\Ext^1_R(M, N)$ is annihilated by $J$ for every $R$-module $N$.
\end{enumerate}
Given a short exact sequence of $R$-modules
$$
0 \to K \to P \to M \to 0
$$
if $M$ is $J$-projective and $P$ is projective, then $K$ is $J$-projective.
Additionally, if $M$ is a finite $R$-module and $J$-projective, then
$M^\vee = \Hom_R(M, R)$ is $J$-projective.
...
$\text{Proj}$ of the Rees algebra $\bigoplus_{n \geq 0} I^n$. Given a
finite $A$-module $M$ we denote $\mathcal{G}_M$ the modules
associate to the graded module $\bigoplus_{n \geq 0} I^nM$ over
the Rees algebra. Then there is a short exact sequence
$$
0 \to \tau_M \to p^*\widetilde{M} \to \mathcal{G}_M \to 0
$$
where $\tau_M$ is the coherent submodule of sections of $p^*\widetilde{M}$
supported on the exceptional divisor. Given a short exact sequence
$$
0 \to K \to L \to M \to 0
$$
of finite $A$-modules, we obtain the following diagram
$$
\xymatrix{
&
p^*\widetilde{K} \ar[r] \ar[d] &
p^*\widetilde{L} \ar[r] \ar[d] &
p^*\widetilde{M} \ar[r] \ar[d] &
0 \\
0 \ar[r] &
\mathcal{G}_K \ar[r] &
\mathcal{G}_L \ar[r] &
...
$$
where the rows are exact, except for the middle spot at the bottom
where the cohomology sheaf is equal to $\Coker(\tau_L \to \tau_M)$
as follows from an application of the snake lemma. The injectivity
of $\mathcal{G}_K \to \mathcal{G}_L$ follows as it is true over
the complement of the exceptional divisor and the fact that
$\mathcal{G}_L$ has no nonzero section supported on the exceptional
divisor.

\begin{lemma}
\label{lemma-tau-for-almost-projective}
In the situation above if $M$ is $I^c$-projective, then
$\tau_M$ is annilated by $I^c$.
\end{lemma}

\begin{proof}
Follows as multiplication by $a$ on $I^c$ factors through a finite free
module and since $\tau_A = 0$.
\end{proof}

\begin{proposition}
\label{proposition-vanishing-coh-almost-projective}
In the situation above, let $d$ be the maximum of the dimensions
of the fibres of $p$\footnote{So $d \leq t - 1$ if $I = (a_1, \ldots, a_t)$
since then $X \subset \mathbf{P}^{t - 1}_A$.}. Then there exists an integer
$N(A, I)$ such that
\begin{enumerate}
\item for any $c \geq 0$,
\item for every $I^c$-projective finite $A$-module $M$,
\item for $i \in \{0, 1, \ldots, d\}$, and
\item for every $q \geq N(A, I) + (d - i)c$
\end{enumerate}
then for $i > 0$ the map
$H^i(X, \mathcal{G}_M(q)) \to H^i(X, \mathcal{G}_M(q - (d - i)c))$
is zero and for $i = 0$ the map
$$
H^0(X, \mathcal{G}_M(q)) \to H^0(X, \mathcal{G}_M(q - dc))
$$
has image contained in the image of
$I^{q - dc}M \to H^0(X, \mathcal{G}_M(q - dc))$.
\end{proposition}
...
Let $M$ be an $I^c$-projective finite $A$-module. Choose a short
exact sequence
$$
0 \to K \to A^{\oplus r} \to M \to 0
$$
and consider the corresponding complex
$$
0 \to \mathcal{G}_K \to \mathcal{O}_X^{\oplus r} \to \mathcal{G}_M \to 0
$$
which is non-exact only in the middle with cohomology annihilated
by $I^c$ by Lemma \ref{lemma-tau-for-almost-projective} and the discussion
above.
...
\medskip\noindent
We will prove the vanishing of the map
$H^i(X, \mathcal{G}_M(q)) \to H^i(X, \mathcal{G}_M(q - (d - i)c))$
by descending induction on $d \geq i > 0$.

\medskip\noindent
Base case. For $i = d$ the statement is that
$H^d(X, \mathcal{G}_M(q)) = 0$ for $q \geq N(A, I)$.
This is true because by the surjectivity of
$\mathcal{O}_X^{\oplus r} \to \mathcal{G}_M$
and the right exactness of $H^d(X, -)$ on coherent modules
we get the vanishing from our choice of $N(A, I)$.

\medskip\noindent
Induction step. Let $0 < i < d$ and assume the statement holds for $i + 1$.
Denote $\mathcal{H} = \Ker(\mathcal{O}_X^{\oplus r} \to \mathcal{G}_M)$.
We first consider the exact sequence
$$
H^i(X, \mathcal{O}_X^{\oplus r}(q - (d - i)c)) \to
H^i(X, \mathcal{G}_M(q - (d - i)c)) \to
H^{i + 1}(X, \mathcal{H}(q - (d - i)c))
$$
which shows that
$H^i(X, \mathcal{G}_M(q - (d - i)c)) \subset
H^{i + 1}(X, \mathcal{H}(q - (d - i)c))$
as $q - (d - i)c \geq N(A, I)$ gives the vanishing of the first module
by our choice of $N(A, I)$. Thus it suffices to show that
$H^{i + 1}(X, \mathcal{H}(q)) \to H^{i + 1}(X, \mathcal{H}(q - (d - i)c))$
is zero. To study this, we consider the maps of exact sequences
$$
\xymatrix{
H^{i + 1}(X, \mathcal{G}_K(q)) \ar[r] \ar[d] &
H^{i + 1}(X, \mathcal{H}(q)) \ar[r] \ar[d] \ar@{..>}[ld] &
H^{i + 1}(X, (\mathcal{H}/\mathcal{G}_K)(q)) \ar[d] \\
H^{i + 1}(X, \mathcal{G}_K(q - c)) \ar[r] \ar[d] &
H^{i + 1}(X, \mathcal{H}(q - c)) \ar[r] \ar[d] &
H^{i + 1}(X, (\mathcal{H}/\mathcal{G}_K)(q - c)) \\
H^{i + 1}(X, \mathcal{G}_K(q - (d - i)c)) \ar[r] &
H^{i + 1}(X, \mathcal{H}(q - (d - i)c))
}
$$
Since $\mathcal{H}/\mathcal{G}_K$ is annihilated by $I^c$
we see that the canonical map $\mathcal{H}(q) \to \mathcal{H}(q - c)$
factors through the submodule $\mathcal{G}_K(q - c)$! This gives the dotted
arrow in the diagram. Since $K$ is $I^c$-projective, by induction
the map $H^{i + 1}(X, \mathcal{G}_K(q - c)) \to
H^{i + 1}(X, \mathcal{G}_K(q - (d - i)c))$ is zero and we conclude
what we want.

\medskip\noindent
Let us prove the final statement. It suffices to show that
$$
\Im(H^0(X, \mathcal{G}_M(q)) \to H^0(X, \mathcal{G}_M(q - dc))
\subset
\Im(H^0(X, \mathcal{O}_X^{\oplus r}(q - dc)) \to H^0(X, \mathcal{G}_M(q - dc))
$$
because for $q \geq N(A, I) + dc$ we have
$H^0(X, \mathcal{O}_X^{\oplus r}(q - dc)) = (I^{q - dc})^{\oplus r}$
by our choice of $N(A, I)$. To show the inclusion above, it suffices
to show that for $\sigma \in H^0(X, \mathcal{G}_M(q))$ with boundary
$\xi \in H^1(X, \mathcal{H}(q))$ the image of $\xi$ in
$H^1(X, \mathcal{H}(q - dc))$ is zero. This follows by the
exact same argument as before.
\end{proof}

\begin{lemma}
\label{lemma-bound-tor-almost-projective}
Let $I$ be an ideal of a Noetherian ring $A$. Let $d$ and $N(A, I)$ be as in
Proposition \ref{proposition-vanishing-coh-almost-projective}. For every
$I^c$-projective finite $A$-module $M$ and any $q \geq N(A, I) + (d + 1)c$
the map $\text{Tor}_i^A(M, A/I^q) \to \text{Tor}_i^A(M, A/I^{q - (d + 1)c})$
is zero for all $i > 0$.
\end{lemma}
...
0 \to K \to A^{\oplus r} \to M \to 0
$$
Recall that $K$ is $I^c$-projective. The long exact sequence of $\text{Tor}$
gives an isomorphism
$$
\text{Tor}_1^A(M, A/I^q) = K \cap I^q(A^{\oplus r})/I^qK
$$
and isomorphisms
$\text{Tor}_{i + 1}^A(M, A/I^q) \cong \text{Tor}_i^A(K, A/I^q)$ for
$i \geq 1$. By induction on $i$ it suffices to show the result for $i = 1$.

\medskip\noindent
As in the proof of
Proposition \ref{proposition-vanishing-coh-almost-projective}
consider the sequence of coherent $\mathcal{O}_X$-modules
$$
0 \to \mathcal{G}_K \to \mathcal{O}_X^{\oplus r} \to \mathcal{G}_M \to 0
$$
and denote $\mathcal{H}$ the kernel of
$\mathcal{O}_X^{\oplus r} \to \mathcal{G}_M$.
An element $a \in K \cap I^q(A^{\oplus r})$ determines a
global section $\sigma_a$ of $\mathcal{G}_K$ whose image in
$\mathcal{O}_X^{\oplus r}$ vanishes to order $q$ along the
exceptional divisor $E$ (i.e., locally divisible by a $q$th
power of the nonzerodivisor cutting out $E$).
Since $\mathcal{H}/\mathcal{G}_K$ is annihilated by $I^c$ we see that
$\sigma_a$ vanishes to order $q - c$ along $E$.
This means that $\sigma_a$ is the image of a section
$\sigma_a'$ of $\mathcal{G}_K(q - c)$. By
Proposition \ref{proposition-vanishing-coh-almost-projective}
we find that $\sigma_a'$ comes from an element of
$I^{q - (d + 1)c}K$.
This in turn means that $a$ is contained in $I^{q - (d + 1)c}K$.
The lemma follows.
\end{proof}

\noindent
For our next result we need to know how the construction above interacts with
taking duals. Given a coherent $\mathcal{O}_X$-module $\mathcal{G}$
we set
$\mathcal{G}^\vee = \SheafHom_{\mathcal{O}_X}(\mathcal{G}, \mathcal{O}_X)$.
There are canonical maps
$$
p^*\widetilde{M^\vee} \to \mathcal{G}_{M^\vee} \to
(\mathcal{G}_M)^\vee \to (p^*\widetilde{M})^\vee
$$
functorial in the finite $A$-module $M$.

\begin{lemma}
\label{lemma-duals-for-almost-projective}
The map $\mathcal{G}_{M^\vee} \to (\mathcal{G}_M)^\vee$ is always injective
and if $M$ is $I^c$-projective, then its cokernel is annihilate by $I^c$.
\end{lemma}

\begin{proof}
Namely, injectivity holds as the three maps are isomorphisms
over the open $\Spec(A) \setminus V(I)$
and the sheaf $\mathcal{G}_{M^\vee}$ has no nonzero sections supported
on the exceptional divisor. To see the statement on the cokernel
use that $a : M \to M$ for $a \in I^c$
factors through a finite free module and that
the cokernel is zero for $M = A$.
\end{proof}

\begin{lemma}
\label{lemma-bound-hom-for-almost-projective}
In the situation above, let $d$ be the maximum of the dimensions
of the fibres of $p$. There exists an integer $N(A, I)$ such that
for any $c \geq 0$ and for every $I^c$-projective finite $A$-module $M$
and any $q \geq N(A, I) + (d + 1)c$
the module $\Hom_A(M, I^q)$ is contained in $I^{q - dc}\Hom_A(M, A)$.
\end{lemma}

\begin{proof}
Since $I^q \subset A$ we have $\Hom_A(M, I^q) \subset \Hom_A(M, A)$
and hence the statement makes sense.
We will show that $N(A, I)$ as in the statement and proof of
Proposition \ref{proposition-vanishing-coh-almost-projective} works.

\medskip\noindent
Consider an $A$-linear map $\varphi : M \to I^q$. Applying the
construction above we obtain a map $\mathcal{G}_M \to \mathcal{O}_X(q)$
which we may view as a global section
$$
\sigma_\varphi \in H^0(X, (\mathcal{G}_M)^\vee(q))
$$
By Lemma \ref{lemma-duals-for-almost-projective} the canonical map
$(\mathcal{G}_M)^\vee(q) \to (\mathcal{G}_M)^\vee(q - c)$
(which vanishes to order $c$ along the exceptional divisor)
has image contained in the submodule $\mathcal{G}_{M^\vee}(q - c)$.
Thus we see that the image of $\sigma_\varphi$ is in
$$
H^0(X, \mathcal{G}_{M^\vee}(q - c))
$$
By Proposition \ref{proposition-vanishing-coh-almost-projective}
and the fact that $M^\vee$ is $I^c$-projective we conclude that
the image of $\sigma_\varphi$ in $H^0(X, \mathcal{G}_{M^\vee}(q - (d + 1)c))$
is contained in the image of
$$
I^{q - dc}M^\vee \longrightarrow
H^0(X, \mathcal{G}_{M^\vee}(q - (d + 1)c))
$$
Recall that $M^\vee = \Hom_A(M, A)$. Pick $\alpha \in I^{q - dc}\Hom_A(M, A)$
mapping to the image of $\sigma_\varphi$. Then $\psi$ and
$\alpha$ restrict to same map between coherent modules on the open subscheme
$U = \Spec(A) \setminus V(I)$. Observe that $\Hom_A(M, I^{q - dc})$
is $I$-power torsion free as we have $I^{q - dc} = H^0(X, \mathcal{O}_X(q))$
by our choice of $N(A, I)$ and this module of global sections is $I$-power
torsion free. We conclude that $\psi$ and $\alpha$ give the same
element of $\Hom_A(M, I^{q - dc})$ as desired.
\end{proof}

\begin{lemma}
\label{lemma-bound-almost-projective}
Let $I$ be an ideal of a Noetherian ring $A$. Let $t$ be an upper bound
for the number of generators of $I$. There exists an integer
$N(A, I)$ such that for any $c \geq 0$ and for every $I^c$-projective
finite $A$-module $M$ and any $q \geq N(A, I) + tc$
the map $\Ext^i_A(M, I^q) \to \Ext^i_A(M, I^{q - tc})$ is zero
for all $i > 0$.
\end{lemma}

\begin{proof}
Let $d$ and $N(A, I)$ be as in 
Lemma \ref{lemma-bound-hom-for-almost-projective}.
Note that $d \leq t - 1$, see footnote in statement of
Proposition \ref{proposition-vanishing-coh-almost-projective}.
We will prove the lemma with $t$ replaced by $d + 1$ which
of course implies the lemma.
...
0 \to K \to A^{\oplus r} \to M \to 0
$$
Recall that $K$ is $I^c$-projective. The long exact sequence of $\Ext$
gives a surjection
$$
\Hom_A(K, I^q) \to \Ext^1_A(M, I^q)
$$
and isomorphisms $\Ext^i_A(K, I^q) \cong \Ext^{i + 1}_A(M, I^q)$ for
$i \geq 1$. By induction on $i$ it suffices to show the result for $i = 1$.
We have
\begin{align*}
\Hom_A(K, I^q)
& \subset I^{q - dc}\Hom_A(K, A) \\
& =
I^c \cdot I^{q - (d + 1)c}\Hom_A(K, A) \\
& \subset
I^c\Hom_A(K, I^{q - (d + 1)c})
\end{align*}
where the first (and only nontrivial) inclusion is given by
Lemma \ref{lemma-bound-hom-for-almost-projective}.
From the commutative diagram
$$
\xymatrix{
\Hom_A(K, I^q) \ar[r] \ar[d] &
\Ext^1_A(M, I^q) \ar[d] \\
\Hom_A(K, I^{q - (d + 1)c}) \ar[r] &
\Ext^1_A(M, I^{q - (d + 1)c})
}
$$
we conclude that the image of
$\Ext^1_A(M, I^q) \to \Ext^1_A(M, I^{q - (d + 1)c})$
is contained in $I^c \Ext^1_A(M, I^{q - (d + 1)c})$.
However, since $M$ is $I^c$-projective, we see that
$I^c$ annihilates $\Ext^1_A(M, I^{q - (d + 1)c})$ as desired.
\end{proof}

\begin{lemma}
\label{lemma-ext-1-annihilated-definite}
Let $A$ be a Noetherian ring and let $I \subset A$ be an ideal.
Let $c \geq 0$. Let $K \in D(A)$. The following are equivalent
\begin{enumerate}
\item $H^i(K) = 0$ for $i \not \in \{-1, 0\}$ and $H^i(K)$
is a finite $A$-module for all $i$ and multiplication by
$I^c$ is zero on $\Ext^1_A(K, N)$ for any $A$-module $N$ and
\item $K$ can be represented by a complex $K^{-1} \to K^0$
with $K^0$ finite free, $K^{-1}$ a finite $I^c$-projective
module, such that for any $a \in I^c$ the map $a : K^{-1} \to K^{-1}$
factors through $d_K^{-1} : K^{-1} \to K^0$.
\end{enumerate}
\end{lemma}
...
Let $I$ be an ideal of a Noetherian ring $A$. Let $t$ be an upper bound
for the number of generators of $I$. There exists an integer
$N(A, I)$ with the following properties:
\begin{enumerate}
\item for any $c \geq 0$,
\item for any $K \in D(A)$ satisfying the equivalent conditions
of Lemma \ref{lemma-ext-1-annihilated-definite}, and
\item for any $q \geq N(A, I) + tc$,
\end{enumerate}
the map
$\Ext^1_A(K, I^q/I^{2q}) \to \Ext^1_A(K, I^{q - tc}/I^{2q - 2tc})$
is zero.
\end{lemma}

\begin{proof}
Let $K^{-1} \to K^0$ be a complex representing $K$ as in
Lemma \ref{lemma-ext-1-annihilated-definite} part (2).
By Lemma \ref{lemma-map-out-of-almost-free} part (1)
for any $A$-module $N$ we have
$$
\Ext^1_A(K, N) = \Coker(\Hom_A(K^0, N) \to \Hom(K^{-1}, N))
$$
First, we use this with $N = I^q/I^{2q}$ to see that any class $\xi$
in $\Ext^1_A(K, I^q/I^{2q})$ comes from an element $\varphi$ in
$\Hom_A(K^{-1}, I^q/I^{2q})$. The short exact sequence
$0 \to I^{2q} \to I^q \to I^q/I^{2q} \to 0$ gives an exact
sequence
$$
\Hom_A(K^{-1}, I^q) \to
\Hom_A(K^{-1}, I^q/I^{2q}) \to
\Ext^1_A(K^{-1}, I^{2q})
$$
By Lemma \ref{lemma-bound-almost-projective} we see that the map
$\Ext^1_A(K^{-1}, I^{2q}) \to \Ext^1_A(K^{-1}, I^{2q - tc})$ is zero.
Hence there is a map $\psi : K^{-1} \to I^q$ such that
$\psi$ and $\varphi$ have the same image in $\Hom_A(K^{-1}, I^q/I^{2q - tc})$.
On the other hand, we can use
Lemma \ref{lemma-bound-hom-for-almost-projective}
to see that the image $\psi'$ of $\psi$ in
$\Hom_A(K^{-1}, I^{q - tc})$ is contained in $I^c\Hom_A(K^{-1}, I^{q - tc})$
(see argument in the proof of Lemma \ref{lemma-bound-almost-projective}).
So $\psi' \in I^c\Hom_A(K^{-1}, I^{q - tc})$.
However, since multiplication by elements of $I^c$
on $K^{-1}$ factors through $K^0$ we conclude there
is an element $\tilde \psi \in \Hom_A(K^0, I^{q - tc})$
such that
$\psi' = \tilde \psi \circ d_K^{-1}$\footnote{Namely, write
$\psi' = \sum a_i \psi_i$ with $a_i \in I^c$ and
$\psi_i : K^{-1} \to I^{q - tc}$, choose $h_i : K^0 \to K^{-1}$
such that $a_i \text{id}_{K^{-1}} = h_i \circ d_K^{-1}$ and
set $\tilde \psi = \sum \psi_i \circ h_i$.}.
Combining all of the above we conclude that
$\xi$ maps to zero in $\Ext^1_A(K, I^{q - tc}/I^{2q - tc})$
and a fortiori in $\Ext^1_A(K, I^{q - tc}/I^{2q - 2tc})$.
\end{proof}
...
There exists an integer $N(A \to C, I)$ depending only on
...
\item an integer $n \geq \max(N(A \to C, I) + tc, 2tc + 1)$, and
...
\begin{proof}
We choose $N(A \to C, I) = N(C, IC)$ as in
Lemma \ref{lemma-bound-two-term-complex}.
Then given $c, B, n, \psi_n$ as in the statement of the
lemma, consider the obstruction class
$$
o(\psi_n) \in \Ext^1_C(\NL_{B/A}^\wedge \otimes_B C, I^nC/I^{2n}C)
$$
of Remark \ref{remark-improve-homomorphism}.
By our choice of $N(A \to C, I)$ and the fact that
$n \geq N(A \to C, I) + tc$ this class maps to zero in
$$
\Ext^1_C(\NL_{B/A}^\wedge \otimes_B C, I^{n'}C/I^{2n'}C)
$$
where $n' = n - tc$. By the discussion in
