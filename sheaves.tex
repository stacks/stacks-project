...
$$
\rho_U^{f^{-1}(V)} \circ \varphi_V :
\mathcal{G}(V) \longrightarrow
\mathcal{F}(f^{-1}(V)) \longrightarrow
\mathcal{F}(U)
$$
...
It follows from Lemma \ref{lemma-condition-star-sections}
that $\rho_U^{f^{-1}(V)} \circ \varphi_V = \varphi_V^U$
whenever $U$ is as above.

\medskip\noindent
We also leave it to the reader to show that the construction
