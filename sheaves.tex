...
of neighbourhoods of $u$ which are contained in $U$ is cofinal
in the collection of all neighbourhoods of $u$ in $X$.
...
of abelian groups is $0$. We separate out the case
...
\begin{lemma}
\label{lemma-equivalence-categories-abelian}
Let $X$ be a topological space.
Let $j : U \to X$ be the inclusion of an open subset.
The functor
$$
...
\begin{lemma}
\label{lemma-equivalence-categories-structures}
Let $X$ be a topological space.
Let $j : U \to X$ be the inclusion of an open subset.
Let $(\mathcal{C}, F)$ be a type of algebraic structure
such that $\mathcal{C}$ has an initial object $e$.
...
\begin{lemma}
\label{lemma-equivalence-categories-modules}
Let $(X, \mathcal{O})$ be a ringed space.
Let $j : (U, \mathcal{O}|_U) \to (X, \mathcal{O})$
be an open subspace.
The functor
...
$i^{-1}$. It turns out that for a closed immersion there is a right
adjoint for $i_*$, which we will denote $i^!$.
...
\begin{proof}
If $x \not \in Z$, then there exist arbitrarily small neighbourhoods $U$
of $x$ which do not meet $Z$. Because $\mathcal{F}$ is a sheaf
