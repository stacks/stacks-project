...
f_*\mathcal{F}
$$
which turns $f_*\mathcal{F}$ into a presheaf of $f_*\mathcal{O}$-modules.
\end{lemma}
...
The pushforward $f_*\mathcal{O}$, as defined in
...
\noindent
In other to clarify the problem of formulating a suitable
adjointness property of $f_*$, $f^{-1}$ we introduce the
categories of (pre)sheaves of modules.

\begin{definition}
\label{definition-category-modules}
Let $X$ be a topological space.
Let $\mathcal{O}$ be a presheaf of rings on $X$.
\begin{enumerate}
\item The category of presheaves of $\mathcal{O}$-modules
is denoted $\textit{PMod}(\mathcal{O})$.
\item If $\mathcal{O}$ is a sheaf, then the category of
sheaves of $\mathcal{O}$-modules is denoted
$\textit{Mod}(\mathcal{O})$.
\end{enumerate}
\end{definition}

\noindent
Although it is not strictly necessary, let us agree to write
$\textit{Mod}(\mathcal{O})$ only when $\mathcal{O}$ is a sheaf
of rings. In other words, whenever we write the symbol
$\textit{Mod}(\mathcal{O})$ we implicitly assume $\mathcal{O}$
is a sheaf of rings.
...
\end{eqnarray*}
Clearly, there is no way for the pair of functors $(f_*, f^{-1})$
on sheaves of modules to be adjoint, because their target categories
do not match. This would only work if by some miracle the
sheaves of rings $\mathcal{O}_X, \mathcal{O}_Y$ satisfy the
relations $\mathcal{O}_X = f^{-1}\mathcal{O}_Y$ and 
$\mathcal{O}_Y = f_*\mathcal{O}_X$ which is almost never
true in practice.

\medskip\noindent
The solution to the conundrum is to realize that we are supposed to
think of the sheaf of rings $\mathcal{O}_X$ as the sheaf of functions
on $X$. 
