...
\noindent
First we explain that it suffices in (1) to
check the sheaf condition on a cofinal system of coverings. Suppose
...
which is contained in of the open sets
...
each consist of a single element.
\end{proof}
...
Let $X$ be a topological space.
Let $\mathcal{B}$ be a basis for the topology on $X$.
Let $\mathcal{F}_{\mathcal{B}}$ be a sheaf of sets on $\mathcal{B}$.
There exists a unique sheaf of sets $\mathcal{F}$
on $X$ such that $\mathcal{F}(U) = \mathcal{F}_{\mathcal{B}}(U)$
