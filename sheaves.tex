...
The notation and definitions above apply inparticular to:
...
of the underlying presheaf of sets. Note that the underlying
presheaf of sets is a sheaf of sets if and only if the
diagrams
$$
\xymatrix{
F(\mathcal{F}(U)) \ar[r]
...
But this is not good enough; we also need $F$ to
{\it reflect isomorphisms}.
...
the fact that $\mathcal{F}(U) = \mathcal{F}^\#(U)$
