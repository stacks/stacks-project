...
$$
where the first arrow sends $x$ to $\sum s_i \otimes s \otimes t_i$
and the second arrow sends $s \otimes s' \otimes t$ to $\psi(s' \otimes t)s$.
We conclude that $x \mapsto \sum \psi(s \otimes t_i)s_i$ is
...
inverible $\mathcal{O}_Y$-module is invertible.
\end{lemma}
...
$R$-modules, hence as soon as $\text{Pic}(R)$ is not the trivial group,
...
\medskip\noindent
Determinants. Assume that $\mathcal{O}_{X, x}$ is a local ring for
each $x \in X$ (this assumption is forced as we only defined the
Picard group of $X$ in this case). Given a  finite locally free
...
$$
\det : K_0^{naive}(X) \longrightarrow \text{Pic}(X),\quad
