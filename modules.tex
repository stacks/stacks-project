...
\begin{lemma}
\label{lemma-generated-by-local-sections-sections}
	Let $(X, \mathcal{O}_X)$ be a ringed space.
	Let $\mathcal{F}$ be a sheaf of $\mathcal{O}_X$-modules.
	Given a set $I$, and
	local sections $s_i\in\mathcal{F}(U_i)$, $i\in I$, denote
	$U=\bigcup_i U_i$, and suppose
	$\mathcal{G}$ is the subsheaf generated by the
	$s_i$. Let $V\subset X$ be open. For a section
	$t\in\mathcal{F}(V)$,
	we have $t\in\mathcal{G}(V)$ if and only if the following
	condition holds:
	\begin{itemize}
		\item[($*$)] Each $x\in V$ has an open neighborhood $V'\subset V$ such that,
		if $x\not\in U$, then $t|_{V'}=0$, and if $x\in U$,
		then there is a finite number of indices
		$i_1,\dots,i_n\in I$ such that $x\in W=V'\cap U_{i_1}\cap\cdots\cap U_{i_n}$
		and $t|_W$ is an $\mathcal{O}_X(W)$-linear
		combination of $s_{i_1}|_W,\dots,s_{i_n}|_W$.
	\end{itemize}
\end{lemma}
\begin{proof}
	\textit{Define} $\mathcal{G}$ to be the presheaf whose
	sections over $V$ are the sections of $\mathcal{F}(V)$ satisfying property ($*$).
	We shall show that this is the smallest subsheaf of
	$\mathcal{F}$ containing the sections $s_i$.
	Note that $\mathcal{G}$ is an $\mathcal{O}_X$-subpresheaf
	of $\mathcal{F}$ ($\mathcal{G}$ is closed under restrictions
	and under $\mathcal{O}_X$-linear combinations).
	
	To verify that $\mathcal{G}$ is a sheaf, note that a subpresheaf
	of a separated presheaf (as $\mathcal{F}$ is) is separated, so we
	must only check the gluing axiom. Let
	$V=\bigcup_{\alpha}V_\alpha$ be an open cover and suppose that
	$t_\alpha\in\mathcal{G}(V_\alpha)$ agree on intersections. We
	verify that the resulting glued section $t\in\mathcal{F}(V)$ is
	in $\mathcal{G}(V)$: let $x\in V$ and pick $\alpha$ such that
	$x\in V_\alpha$. If $x\not\in U$, there is an open neighborhood
	$x\in V_\alpha'\subset V_\alpha$ with $t|_{V_\alpha'}=0$. On the
	other hand, if $x\in U$, there is an open neighborhood
	$x\in V_\alpha'\subset V_\alpha$ and a finite amount of sections
	$s_{i_1},\dots,s_{i_n}$ such that
	$x\in W_\alpha=V_\alpha'\cap U_{i_1}\cap\cdots\cap U_{i_n}$
	and $t|_{W_\alpha}$ is a $\mathcal{O}_X(W_\alpha)$-linear
	combination of $s_{i_1}|_{W_\alpha},\dots,s_{i_n}|_{W_\alpha}$.
	Then $V'=V\cap V_\alpha'$ and
	$W=V'\cap U_{i_1}\cap\cdots\cap U_{i_n}=V\cap W_\alpha$ does the job.
	
	To see minimality of $\mathcal{G}$, first note that
	$s_i\in\mathcal{G}(U_i)$ for all $i\in I$.
	Now suppose that $\mathcal{H}\subset\mathcal{F}$ is a subsheaf
	such that $s_i\in\mathcal{H}(U_i)$ for all $i$. We show that this
	implies $\mathcal{G}\subset\mathcal{H}$. Let
	$t\in\mathcal{G}(V)$, pick $x\in V$,
	and let $V'\subset V$ be the open neighborhood that arises from ($*$).
	Then it follows that $t|_{V'}\in\mathcal{H}(V')$.
	Thus $t\in\mathcal{H}(V)$, for $\mathcal{H}$ is a
	subsheaf of $\mathcal{F}$.
\end{proof}
