...
\end{enumerate}
The resulting sheaf of $\mathcal{O}_X$-modules
$\mathcal{F}_M = \mathcal{F}_1 = \mathcal{F}_2 = \mathcal{F}_3$
is quasi-coherent. The construction gives a functor from
the category of $R$-modules to the category of quasi-coherent
sheaves on $X$. This functor commutes with arbitrary colimits.
Moreover, given any $\mathcal{O}_X$-module
$\mathcal{G}$ we have
$$
...
\begin{proof}
Omitted. But, this is mostly clear from the definitions and the
adjointness of $\pi^*$ and $\pi_*$.
\end{proof}
...
$U_{jk}$ such that $E \subset \bigcup_k U_{jk}$ and such
that $s_j|_{U_{jk}}$ is a finite sum $\sum_{i \in I_{jk}} f_{jki}$
with $I_{jk} \subset I$, and $f_{jki}$ in the summand
$\mathcal{O}_X$ corresponding to $i \in I$. Arguing as in
the proof of Lemma \ref{lemma-section-direct-sum-quasi-compact}
we see that actually $s_j|_U$ is a sum
$\sum_{i \in I_j} f_{ij}$ with $I_j \subset I$ finite
and $f_{ij} \in \mathcal{O}_X(U) = \Gamma(U, \mathcal{O}_U)$.
...
\Gamma(U, \mathcal{O}_U)
$$
with matrix given by the $(f_{ij})$. It is then clear that
...
likely nonstandard notation} is a morphism
$i : (Z, \mathcal{O}_Z) \to (X, \mathcal{O}_X)$
