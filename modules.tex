...
allthough there we will mostly just duplicate the discussion
from this chapter.
...
\item Finite direct sums are the same as the correponding
...
of exact sequences of $\mathcal{O}_X$-modules over a directed partially
...
We omit the proof of the functorial behaviour of the
exact sequence.
\end{proof}
...
Let $\mathcal{F} = \colim \mathcal{F}_i$
...
\mathcal{O}_X, \quad
e_j \longmapsto \sum\nolimits_{j \in \mathbf{N}} f_i 1_{L_j} e_{ij}
$$
...
then $\text{Ker}(\psi)$ is of finite type.
\item If $\theta : \mathcal{G} \to \mathcal{F}$ is surjective with
$\mathcal{G}$ of finite type, then $\text{Ker}(\theta)$ is of finite type.
\end{enumerate}
\end{lemma}
...
By part (1) we see $\text{Ker}(\theta \circ \eta)$ is of finite type.
Since $\text{ker}(\theta) = \eta(\text{Ker}(\theta \circ \eta))$ we win.
\end{proof}
...
A good way to garantee that $i_*\mathcal{O}_Z$ is a
...
locally on $(X, \mathcal{O}_X)$ cut out by some regular functions
...
Let $i : (Z, \mathcal{O}_X) \to (X, \mathcal{O}_X)$ be a morphism
...
$$
is an isomorphism. We check this on stacks. Say $z \in Z$ and $x = i(z)$.
We have
$$
(i^*i_*\mathcal{F})_z =
...
are isomorphic to $\Gamma(U, \mathcal{O}_U)^{\oplus n}$
...
\begin{proof}
Let $I$ be a partially ordered set and let $\{\mathcal{G}_i\}$ be
...
Let $(X, \mathcal{O}_X)$ be a ringed space.
Assume that all stalks $\mathcal{O}_{X, x}$ are local
rings\footnote{We should at least assume that they are nonzero.
...
(Lemma \ref{lemma-abelian}). By Lemma \ref{lemma-stalk-module-differentials}
this follows from
...
$\mathcal{I} = \text{Ker}(\pi)$ is its kernel. Assume $\mathcal{I}$ an ideal
...
$f^*\text{d}_{X/S}(a)$ mapsto $\text{d}_{X'/S'}(f^*a)$
...
it suffices to check on stalks (Lemma \ref{lemma-abelian}). Here
by Lemma \ref{lemma-stalk-module-differentials} we see the result holds by
...
The {\it naive cotangent complex} $L_f = L_{X/Y}$ of a morphism of ringed
spaces $f : (X, \mathcal{O}_X) \to (Y, \mathcal{O}_Y)$ is
$L_{\mathcal{O}_X/f^{-1}\mathcal{O}_Y}$.
\end{definition}
