...
\begin{proof}
This can be seen using the results above. However, we prefer to prove
it by considering the endomorphisms $E$ of the identity functor
$\text{id} : D(A) \to D(A)$ as an $R$-linear exact functor of
triangulated categories. Namely, an element $e \in E$ corresponds
to a rule which assigns to every $P$ in $D(A)$ a map
$$
e_P : P \longrightarrow P
$$
in $D(A)$ such that $e_P[1] = e_{P[1]}$ and such that for any morphism
$P \to P'$ the diagram
$$
\xymatrix{
P' \ar[r]_{e_P'} & P' \\
P \ar[u] \ar[r]^{e_P} & P \ar[u]
}
$$
is commutative. It is a pleasant exercise to show that $E$ is the
center of $A$. Hence the lemma follows.
\end{proof}
