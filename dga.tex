...
as a differential graded $A$-module over itself. Moreover, we have the
shifts $A[k]$ for all $k \in \mathbf{Z}$. Direct summands of direct sums
...
The map $A \to M$, $a \mapsto am$ will be a left inverse to the
...
$f = (f_{p, q})$ such that
$$
f_{p, q}(a x) = (-1)^{n\deg(a)}a f_{p - \deg(a), q + \deg(a)}(m)
$$
for $a \in A$ homogeneous and $m \in L^{-q - \deg(a)}$.
...
$$
Note that the first term of this expression is in
$\Hom_\mathcal{B}(L^{-q}, M^{p + 1})$ and the second term is in
$\Hom_\mathcal{B}(L^{-q - 1}, M^p)$ hence the infinite product
above makes sense. In other words, given $p + q = n + 1$ we have
$$
\text{d}(f)_{p, q} =
...
\begin{enumerate}
\item $\text{d}$ has square zero,
\item an element $f = (f_{p, -p})$ in $\Hom^0(L, M)$
has $\text{d}(f) = 0$ if and only if the morphisms $f_{-p, p} : L^p \to M^p$
fit together into a homomorphism of differential graded $A$-modules,
