...
$$
H^n(R\Hom(N, M)) = \text{Ext}^n_{D(B, \text{d})}(N, M)
$$
...
R\Hom(M, N)
\longrightarrow
R\Hom(M \otimes_E^\mathbf{L} K^\bullet, N \otimes_E^\mathbf{L} K^\bullet)
$$
in $D(R)$ which on cohomology modules gives the maps
$$
\text{Ext}^n_{D(E, \text{d})}(M, N) \to
\text{Ext}^n_{D(\mathcal{O})}
...
\longrightarrow
R\Hom(\mathcal{F}^\bullet, \mathcal{G}^\bullet).
$$
