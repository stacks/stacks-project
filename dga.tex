...
{\it $P$ is projective as a graded $A$-module} to mean
...
monomorphisms,
\item the quotiens $F_{i + 1}P/F_iP$ are projective as graded $A$-modules.
\end{enumerate}
...
monomorphisms,
\item the quotients $F_{i - 1}I/F_iI$ are injective as graded $A$-modules.
\end{enumerate}
In fact, condition (2) is a consequence of condition (3), see
Lemma \ref{lemma-target-graded-projective}.
...
\begin{proof}
We have to show that (\ref{equation-restriction}) transforms homotopic
maps into homotopic maps and distinguished triangles into distinguished
triangles. For the first, if $h : N_1 \to N_2$ is a right $B$-module map
homogeneous of degree $-1$, then the induced map
$c_h : \Hom_\mathcal{B}(N, N_1) \to \Hom_\mathcal{B}(N, N_2)$
is a right $A$-module map homogeneous of degree $-1$. Moreover
$c_{(\text{d} \circ h + h \circ d)} = \text{d} \circ c_h + c_h \circ d$
which proves the first statement. To see the second
...
In the situation above, if $N$ has property (P) as a differential
graded $B$-module, then the right derived functor of $F$ exists.
We denote it $R\Hom(N, -) : D(A, \text{d}) \to D(B, \text{d})$.
\end{lemma}

\begin{proof}
Let $N' \to N''$ be a quasi-isomorphism of differential graded
$B$-modules. Then the map
$\Hom_\mathcal{B}(N, N') \to \Hom_\mathcal{B}(N, N'')$
is a quasi-isomorphism because on $H^0$ it produces the map
$\Hom_{D(B, \text{d})}(N, N') \to \Hom_{D(B, \text{d})}(N, N')$
by Lemma \ref{lemma-hom-derived}. Thus the result is clear and
in fact, $R\Hom(N, -)$ is simply given by $\Hom_\mathcal{B}(N, -)$.
\end{proof}

\begin{remark}
\label{remark-injectives-missing}
We are missing the discussion of objects with property (I)
dual to property (P) and the existence of right resolutions
into objects with property (I). Accepting this for the moment
we see that a right derived functor of $\Hom_\mathcal{B}(N, -)$
always exists and is given on $N'$ by $\Hom_\mathcal{B}(N, I)$
where $N' \to I$ is an I-resolution. However, it is then clear
from what has been already proved that the functor $R\Hom(N, -)$
is the same as the functor $R\Hom(P, -)$ where $P \to N$ is
a P-resolution. Thus the lemma above suffices to at least define
all these derived functors.
\end{remark}
...
is a quotient of $\bigoplus_{p + q = n} M^p \otimes_{A^0} N^q$
(but not equal to that in general).
\item If $(A, \text{d})$ is a differential graded algebra, and
$M$ and $N$ are (left and right) differential graded $A$-modules, then
$M \otimes_A N$ is a differential graded $R$-module.
...
\begin{proof}
We have to show that $- \otimes_A N$ transforms homotopic maps
into homotopic maps and distinguished triangles into distinguished
triangles. For the first, if $h : M \to M'$ is a right $A$-module map
homogeneous of degree $-1$, then the induced map
$h \otimes 1 : M \otimes_A N \to M' \otimes_A N$ is a right $B$-module map
homogeneous of degree $-1$. Moreover
$(\text{d} \circ h + h \circ d) \otimes 1 =
\text{d} \circ (h \otimes 1) + (h \otimes 1) \circ d$
which proves the first statement. To see the second
...
$L \otimes_A N \to M \otimes_A N$. This finishes the proof.
\end{proof}
