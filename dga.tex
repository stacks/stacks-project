...
\noindent
If $(A, \text{d})$ is just a graded algebra, then we do obtain the
...
\medskip\noindent
Choose a basis $x_i, i \in I$ of graded element for $P$ as an $A$-module.
Let $d_i = \deg(x_i)$.
...
short exact sequence. Hence we obtain a boundary map
$\delta : P \to P_1 \cap P_2[1]$ in $K(A, \text{d})$.
...
By elementary properties of triangulated categories
we can find a map $s : P \to P_1 \oplus P_2$ such that
$\pi \circ s = \text{id}_P + dh + hd$ for some $h$.
Since $P_1 \oplus P_2 \to P$ is surjective and since $P$
is projective as a graded $A$-module we can choose a lift
$\tilde h : P \to P_1 \oplus P_2$ of $h$. Then we change
$s$ into $s + d\tilde h + \tilde h d$ to get
$\pi \circ s = \text{id}_P$. This means we obtain a direct
sum decomposition $P = s^{-1}(P_1) \oplus s^{-1}(P_2)$.
An easy calculation shows that the pieces correspond to
$E[n]$ and $E$ in the derived category and the lemma is proved.
\end{proof}
