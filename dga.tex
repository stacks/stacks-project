...
by admissible monomorphisms, whose graded pieces are projective
as graded $A$-modules. By the description of graded projective modules
in Section \ref{section-projectives-over-algebras} it suffices to prove that
$$
...
by admissible monomorphisms, whose graded pieces are injective
as graded $A$-modules. By the description of graded injective modules in
...
Let $R$ be a ring. A {\it functor of graded categories over $R$}
...
\noindent
Our use of the symbold $K(\mathcal{A})$ is nonstandard, but at least
...
$$
\text{d}(f_{p, q}) =
\text{d}_M \circ f_{p, q} + (-1)^{p + q + 1} f_{p, q} \circ \text{d}_L
$$
Note that the first term of this expression is in
...
$$
of Lemma \ref{lemma-construction}. In the statement of the following
lemma the assumption means that the right derived functor of
$\Hom_\mathcal{A}(K^\bullet, -) : K(\mathcal{A}) \to D(\textit{Ab})$
is everywhere defined on $D(\mathcal{A})$, see
Derived Categories, Definition \ref{derived-definition-everywhere-defined}.
The conclusion is that we can then lift this to the derived
category of $E$-modules.

\begin{lemma}
\label{lemma-existence-of-derived}
In the situation above, suppose that the right derived functor
$R\Hom_\mathcal{A}(K^\bullet, -)$ into $D(\textit{Ab})$ is everywhere
defined on $D(\mathcal{A})$. Then we obtain a canonical exact functor
$$
