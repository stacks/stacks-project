...
graded $A^{opp} \otimes_R B$-module. If $N$ has property (P)
as a differential graded $B$-module, then
$$
...
$$
Since $N$ has property (P) as a $B$-module we see that the
map
$$
\Hom_{\text{Mod}^{dg}_{(B, \text{d})}}(N, M) \to
\Hom_{\text{Mod}^{dg}_{(B, \text{d})}}(N, I)
$$
induced by $M \to I$ is a quasi-isomorphism by
