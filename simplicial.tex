...
\label{lemma-abelian}
Let $\mathcal{A}$ be an abelian category.
The categories $\text{Simp}(\mathcal{A})$ and
...
$\alpha_1(i) < \alpha_2(i)$. Let $\alpha_0$ be the largest
surjective map $[n] \to [k]$ such that $x_{\alpha_0} \not = 0$.
Let $\beta_0 : [k] \to [n]$ be the injective map such
that $\alpha_0 \circ \beta_0 = \text{id}_{[k]}$ such that
$\beta_0(i) = \min\{j \mid \alpha_0(j) = i\}$.
\end{proof}
...
\begin{lemma}
\label{lemma-map-associated-complexes-qis}
Let $\mathcal{A}$ be an abelian category.
Let $U$ be a simplicial object of $\mathcal{A}$.
The canonical morphism of chain complexes
$N(U) \to s(U)$ is a quasi-isomorphism.
In fact,
$$
S(U) = N(U) \oplus A(U)
$$
for some acyclic complex $A(U)$.
\end{lemma}

\begin{proof}
Define $A(U)_n$ to be the image of
$$
\bigoplus_{\varphi : [n] \to [m]\text{ surjective}, m < n} N(U_m)
...
Definition \ref{definition-split}. First we show that
...
$$
with sign $(-1)^i$, $i = 0, \ldots, n$. First we will prove
by ascending induction on $m$, for $0 \leq m < n - 1$ that
all the maps $U(\varphi \circ \delta^n_i)$
map $N(U_m)$ into $A(U)_{n - 1}$. Whenever the map
$\varphi \circ \delta^n_i : [n - 1] \to [m]$
