...
for $i \leq j$.
\end{enumerate}
Conversely, given a system of maps $h_{n, i}$ satsifying the
...
by the formula
$$
s(h)_n = \sum_{i = 0}^n h_{n + 1, i} \circ s^n_i.
$$
...
= \\
\sum_{j = 0}^{n + 1} \sum_{i = j + 1}^n
(-1)^{j + i} h_{n, i - 1} \circ d^n_j \circ s^n_i
+
\sum_{j = 0}^{n + 1} \sum_{i = 0}^j 
(-1)^{j + i} h_{n, i} \circ d^n_j \circ s^n_i \\
+ \\
\sum_{i = 0}^{n - 1} \sum_{j = 0}^n
(-1)^{i + j} h_{n, i} \circ s^{n - 1}_i \circ d^n_j
\end{matrix}
