...
The construction $A \mapsto \tilde A$ is functorial.
\end{lemma}
...
Let $\mathcal{A}$ be an abelian category.
Let $U$, $V$ be simplicial objects of $\mathcal{A}$.
Any pair maps $a, b : U \to V$ whose
corresponding maps of chain complexes $N(a), N(b) : N(U) \to N(V)$
are homotopic, are themselves homotopic in the sense of
Definition \ref{definition-homotopy}. Moreover, the corresponding
homotopy $h : U \times \Delta[1] \to V$ produces back the
given homotopy via the procedure of Section \ref{section-homotopy}.
\end{lemma}

\begin{proof}
By Lemma \ref{lemma-represent-homotopy} it suffices
to show there exists a morphism
$\psi : N(U \times \Delta[1]) \to \widetilde{N(U)}$
such that the compositions $\psi \circ N(e_i)$ are the
two maps $N(U) \to \widetilde{N(U)}$.
...
$$
If we compute $d_{s(W), n}(x_\alpha)$, then we have to omit
