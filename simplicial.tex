...
functor defining
$$
\lim\nolimits_{\mathcal{U}^{opp}} \mathcal{V}
$$
...
by the formula
$$
(\text{cosk}_mU)_n = \lim\nolimits_{(\Delta/[n])_{\leq m}^{opp}} U(n)
$$
...
$$
U_i = \lim\nolimits_{(\Delta/[i])_{\leq n}^{opp}} U(i)
$$
...
\text{colim}_n\ i_{n!} \text{sk}_n S(A_\bullet)$ by construction
of $S$.
