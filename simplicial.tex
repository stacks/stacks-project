...
$x = x' + x''$ with $d_i(x') = 0$
and $x'' \in \text{Im}(s_{i - 1})$
by taking $x' = (x - s_{i - 1}(d_i(x)))$ and
$x'' = s_{i - 1}(d_i(x))$. Moreover, the element
$z \in U_m$ such that $x'' = s_{i - 1}(z)$
is unique because $s_{i - 1}$ is injective.
...
$d_n(x_{n - 1}) = 0$. Continue like this to get
\begin{eqnarray*}
x & = & x_n + s_n(z_n), \\
...
x_1 & = & x_0 + s_0(z_0)
\end{eqnarray*}
where $d_{i + 1}(x_i) = 0$ for all $i = n, \ldots, 0$.
By our general remark above all of the $x_i$ are determined
uniquely by $x$.
We claim that
$x_i \in
\text{Ker}(d_{n + 1}) \cap
...
\medskip\noindent
We prove this by descending induction on $i$.
It is clear for $i = n$ by construction of $x_n$ and $z_n$.
Let us prove it for $0 \leq i < n$ assuming the result for $i + 1$.
First of all we have $d_{i + 1}(x_i) = 0$ by construction.
So pick a $j$ with $i + 1 < j \leq n + 1$. We have
$d_j(x_{i + 1}) = 0$ by induction. Hence
$$
0 = d_j(x_{i + 1})
= d_j(x_i) + d_j(s_i(z_i))
= d_j(x_i) + s_i(d_{j - 1}(z_i)).
$$
The last equality by the relations of Remark \ref{remark-relations}.
These relations also imply that
$d_{i + 1}(d_j(x_i)) = d_{j - 1}(d_{i + 1}(x_i)) = 0$
because $d_{i + 1}(x_i)= 0$ by construction.
Then the uniqueness in the general remark above shows the equality
$0 = x' + x'' = d_j(x_i) + s_i(d_{j - 1}(z_i))$
can only hold if both terms are zero. We conclude that
$d_j(x_i) = 0$ and by injectivity of $s_i$ we also
conclude that $d_{j - 1}(z_i) = 0$. This proves the claim.
...
of simplicial sets we see from Lemmas \ref{lemma-splitting-simplicial-sets}
and \ref{lemma-imshriek-sets}
...
\medskip\noindent
Finally, we prove (7). First we prove it in case $\mathcal{A}$ is
the category of abelian groups. Let $(x_\alpha)$
be an element of $\bigoplus_{\alpha : [n] \to [k]\text{ surjective}} A$
which maps to zero. Then for every injective $\beta : [k] \to [n]$
we have $\sum_{\alpha \circ \beta = \text{id}} x_\alpha = 0$.
Define a (total) ordering on the set of surjective $\alpha : [n] \to [k]$
by the rule $\alpha_1 < \alpha_2$ if and only if there
exists an $i$, $0 \leq i \leq n$, such that
$\alpha_1(j) = \alpha_2(j)$ for $i < j \leq n$ and
$\alpha_1(i) < \alpha_2(i)$.
To get a contradiction assume $(x_\alpha)$ is not zero.
Let $\alpha_0$ be the largest
surjective map $[n] \to [k]$ such that $x_{\alpha_0} \not = 0$.
Let $\beta_0 : [k] \to [n]$ be the injective map such
that $\beta_0(i) = \min \{j \mid \alpha_0(j) = i\}$.
Then $\alpha_0 \circ \beta_0 = \text{id}_{[k]}$, and if
$\alpha \circ \beta_0 = \text{id}_{[k]}$ then
$\alpha \leq \alpha_0$. Thus the sum
$\sum_{\alpha \circ \beta_0 = \text{id}} x_\alpha$
has only one term in it and we conclude $x_{\alpha_0} = 0$,
contradiction.

\medskip\noindent
The result (7) for a general abelian category
follows since the result for abelian groups shows that
all the induced maps
$$
\xymatrix{
\text{Hom}(X, \bigoplus_{\alpha : [n] \to [k]\text{ surjective}} A)
\ar@{=}[r] \ar[d] &
\bigoplus_{\alpha : [n] \to [k]\text{ surjective}} \text{Hom}(X, A)
\ar[d] \\
\text{Hom}(X, \bigoplus_{\beta : [k] \to [n]\text{ injective}} A)
\ar@{=}[r] &
\bigoplus_{\alpha : [k] \to [n]\text{ injective}} \text{Hom}(X, A)
}
$$
are injective.
\end{proof}
...
Let $\mathcal{A}$ be an abelian category.
Any simplicial object $V$ is
$$
\text{colim}_n  i_{n!}\text{sk}_n V
$$
\end{lemma}
...
The {\it associated chain complex $s(U)$ of $U$} is
the chain complex
$$
...
$N(U_m) = \bigcap_{i = 1}^m \text{Ker}(d^m_i)$.
We define $N(U)$ to be the chain complex
$$
\ldots \to N(U_2) \to N(U_1) \to N(U_0) \to 0 \to 0 \to \ldots
$$
with boundary map $d_n : N(U_n) \to N(U_{n - 1})$ given
by the restriction of $d^n_0$ to the direct summand
$N(U_n)$ of $U_n$. This is a complex because
$d^n_0 \circ d^{n + 1}_0 = d^n_0 \circ d^{n + 1}_1$
and $d^{n + 1}_1$ is zero on $N(U_{n + 1})$ by definition.
...
the maps $d^n_i$, $i > 0$ are zero on $N(U_n)$,
whereas the restriction of $d^n_0$ is the boundary
...
of the kernels of the maps $d^n_i$, $i = 1, \ldots, n$
on $K(A, k)_n$ is zero when $n > k$. This is clear from
the description of the maps $d^n_i$ in
Lemma \ref{lemma-eilenberg-maclane-object}
since if $\alpha \circ d^n_i$ is not surjective for
all $i = 1, \ldots, n$, then $\alpha$ is not surjective.
\end{proof}
...
is zero on $N(U_m)$ unless $j = 0$. If $j = 0$, then
$d^m_0(N(U_m)) \subset N(U_{m - 1})$ and hence
...
The map $d^n_0 \circ s^{n - 1}_j$, if $j > 0$, is equal to
$s^{n - 2}_{j - 1} \circ d^{n - 1}_0$. Since 
$d^{n - 1}_0$ maps $N(U_{n - 1})$ into $N(U_{n - 2})$,
we see that the image $d^n_0 ( s^{n - 1}_j (N(U_{n - 1}))$
is contained in $s^{n - 2}_{j - 1}(N(U_{n - 2}))$ which
is contained in $A(U_{n - 1})$ by definition. For all
other combinations of $(i, j)$ we have
either $d^n_0 \circ s^{n - 1}_j = s^{n - 2}_{j - 1} \circ d^{n - 1}_i$
(if $0 < i < j$), or
$d^n_0 \circ s^{n - 1}_j = s^{n - 2}_j \circ d^{n - 1}_{i - 1}$
(if $i > j + 1$) and in these cases the map is zero because
...
$$
K(V_m, m) = i_{m!} \text{sk}_mV \to V \to \text{cosk}_m \text{sk}_m V
$$
whose composition is injective. Hence we get a short exact
sequence
$$
0 \to K(V_m, m) \to V \to W \to 0
$$
and $W_i = 0$ for $i = 0, \ldots, m$. This short exact sequence
...
Let $\mathcal{A}$ be an abelian category.
The functor $N$ is faithful.
\end{lemma}

\begin{proof}
This is immediate from the canonical
splitting of Lemma \ref{lemma-splitting-abelian-category}.
\end{proof}

\begin{lemma}
\label{lemma-dold-kan}
Let $\mathcal{A}$ be an abelian category.
...
Let $A_\bullet$ be a chain complex with boundary maps
$d_n : A_n \to A_{n - 1}$. For each $n \geq 0$ denote
$$
I_n =
\Big\{
...
We define a simplicial object $U(A_\bullet)$ as follows:
\begin{enumerate}
\item $U_n(A_\bullet) = \bigoplus_{\alpha \in I_n} A_{k(\alpha)}$, which
we sometimes write as
$\bigoplus_{\alpha \in I_n} A_{k(\alpha)} \cdot \alpha$,
\item given $\varphi : [m] \to [n]$ we define
$U(A_\bullet)(\varphi)$ by its restriction to
the direct summand $A_{k(\alpha)}$ as follows
\begin{enumerate}
...
the identity map to the summand $A_{k(\alpha \circ \varphi)}$, and
\item if $\alpha \circ \varphi \in I_m$
and $k(\alpha \circ \varphi) = k(\alpha) - 1$
then we use the map $d_{k(\alpha)}$ to the summand 
...
U(A_\bullet)$ is an exact functor from chain complexes
to simplicial objects. If $A_i = 0$ for $i = 0, \ldots, n$
then $U(A_\bullet)_i = 0$ for $i = 0, \ldots, n$.
The objects $K(A, k)$, see Definition \ref{defintition-eilenberg-maclane},
are equal to $U(A[-k])$ where $A[-k]$ is the chain complex
with $A$ in degree $k$ and zero elsewhere.
Let $\tau_{\leq k}A_\bullet$ denote the complex
equal to $A_\bullet$ in degrees $\leq k$ and zero elsewhere.
There are a canonical maps
$A_\bullet \to \tau_{\leq k}A_\bullet \to \tau_{\leq k - 1}A_\bullet$.
The kernel of the first map has no terms in degrees $\leq k$,
whereas $\text{Ker}(\tau_{\leq k}A_\bullet \to \tau_{\leq k - 1}A_\bullet)
= A_k[-k]$. By exactness of $N$ and $U(-)$ we deduce that
$$
N(U(A_\bullet))_k
\to
N(U(\tau_{\leq k}A_\bullet))_k
\leftarrow
N(U(A_k[-k])) = A_k
$$
are functorial isomorphisms.
Note that the unique map $A_k \to U(A_\bullet)_k$
corresponding to the summand $\alpha = \text{id}_{[k]}$
is what gives rise to this identification.

\medskip\noindent
In order to show that $N(-) \circ U(-)$ is the identity
functor on $\text{Ch}_{\geq 0}(\mathcal{A})$,
the last thing we have to verify
is that we recover the map $d_{k + 1} : A_{k + 1} \to A_k$
from the map $N(U(A_\bullet))_{k + 1} \to N(U(A_\bullet))_k$.
Actually, it will turn out that this is not the case,
but that the map
$A_{k + 1} = N(U(A_\bullet))_{k + 1} \to N(U(A_\bullet))_k$
is equal to $(-1)^{k + 1}d_{k + 1}$. This is still enough
to conclude that $N$ is an equivalence (see below).
By truncating as above, it is clear that it is enough
to prove this formula in the case where $A_\bullet$
is a complex whose only nonzero terms are in degrees
$k + 1$ and $k$. We first do a special case.

\medskip\noindent
Let $A$ be an object of $\mathcal{A}$ and
let $k \geq 0$ be an integer. Consider
the simplicial object $E$ constructed in Lemma
\ref{lemma-extension}. By construction this is
just $U(A_\bullet)$ with $A_k = A_{k + 1} = A$,
$A_i = 0$ for $i \not = k, k + 1$ and
$d_{k + 1} = \text{id}_A$. The identifications
$N(E)_k = A$ and $N(E)_{k + 1} = A$ we found
above come from the short exact sequence
$$
0 \to K(A, k) \to E \to K(A, k + 1) \to 0
$$
together with the identitfications $N(K(A, k))_k = A$
and $N(K(A, k + 1))_{k + 1} = A$.
We will compute the induced differential
$A = N(E)_{k + 1} \to N(E)_k = A$ in the
case where $A$ is an abelian group.
The terms of degree $k + 1$ and $k$ of $E$ and
the maps $d^{k + 1}_i$ look like
$$
d^{k + 1}_i :
A \cdot \text{id}_{[k + 1]} \oplus
\bigoplus\nolimits_{j = 0}^k A \cdot \sigma^k_j
\longrightarrow
A
$$
and act as follows:
\begin{enumerate}
\item $d^{k + 1}_i$ kills $A \cdot \text{id}_{[k + 1]}$
unless $i = k + 1$ in which case you get the identity
map to $A$.
\item $d^{k + 1}_i$ kills $A \cdot \sigma^k_j$
unless $i = j, j + 1$ in which case you get the
identity map to $A$.
\end{enumerate}
So an element is in the kernel of $d^{k + 1}_i$ for
$i = 1, \ldots, k + 1$, i.e., in $N(E)_{k + 1}$, if and only if
it looks like 
$$
x \cdot \text{id}_{[k + 1]} +
\sum\nolimits_{j = 0}^k (-1)^{j + k + 1} x \cdot \sigma^k_j
$$
Furthermore, this element maps to the element
$x \in A = N(K(A, k+1))_{k +1}$
under the canonical map $N(E) \to N(K(A, k + 1))$.
And finally, we see that under the map $d^{k + 1}_0$
(which is the definition of the differential on $N(E)$)
we get $(-1)^{k + 1}x$. By the usual trickery with $\text{Hom}$'s
we see that the differential $N(E)_{k + 1} \to N(E)_k$
agrees with $(-1)^{k + 1}\text{id}_A$ in the case of
a general abelian category.

\medskip\noindent
The general case of a complex $A_\bullet$ concentrated in
degrees $k, k + 1$ follows from the above and functoriality
by considering the map of complexes
$$
\xymatrix{
\ldots \ar[r] & 0 \ar[r] & A_{k + 1} \ar[r] \ar[d] &
A_{k + 1} \ar[r] \ar[d] & 0 \ar[r] & \ldots \\
\ldots \ar[r] & 0 \ar[r] & A_{k + 1} \ar[r] & A_k \ar[r] & 0 \ar[r] & \ldots
}
$$
This finishes the proof of the relation between the differntial
on $N(U(A_\bullet))$ and the differential on $A_\bullet$.

\medskip\noindent
Let us modify the construction of $U(A_\bullet)$
by putting in a sign $(-1)^{k(\alpha)}$ in case (2)(d),
and let us call this $U^+(A_\bullet)$. Then it is clear
that $N \circ U^+$ is the identity functor on
$\text{Ch}_{\geq 0}(\mathcal{A})$. (Another way to
resove this issue is to use that clearly $N \circ U \circ N \circ U$
is the identity functor...)

\medskip\noindent
We know that $N$ is faithful, see Lemma \ref{lemma-N-faithful}.
If we can show that $U$ is essentially surjective, then
