...
by the formula
$$
s(h)_n = \sum\nolimits_{i = 0}^n (-1)^i h_{n + 1, i + 1} \circ s^n_i.
$$
...
(-1)^{j + i} d^{n + 1}_j \circ h_{n + 1, i + 1} \circ s^n_i
+ 
\sum_{i = 0}^{n - 1} \sum_{j = 0}^n
(-1)^{i + j} h_{n, i + 1} \circ s^{n - 1}_i \circ d^n_j \\
= \\
\sum_{j = 0}^{n + 1}
\left(
\sum_{i = j}^n
...
\left(
\sum_{i = j + 1}^n
(-1)^{j + i} h_{n, i} \circ s^{n - 1}_{i - 1} \circ d^n_j
+
\sum_{i = j}^j 
(-1)^{j + i} h_{n, i}
+
\sum_{i = j - 1}^{j - 1} (-1)^{j + i} h_{n, i + 1}
\right.\hfill\\
\hfill\left.
+
\sum_{i = 0}^{j - 2}
(-1)^{j + i} h_{n, i + 1} \circ s^{n - 1}_i \circ d^n_{j - 1}
\right) \\
+ \\
\sum_{i = 0}^{n - 1} \sum_{j = 0}^n
(-1)^{i + j} h_{n, i + 1} \circ s^{n - 1}_i \circ d^n_j \\
...
\}
$$
There exists a chain complex $\tilde A$
such that $\text{Mor}_{\text{Ch}(\mathcal{A})}(\tilde A, -)$
...
there are two maps $\tilde a, \tilde b : A \to \tilde A$
(first summand and second summand),
and a map $\tilde A \to A[-1]$ which give
...
\medskip\noindent
We conclude that any morphism $f : \tilde A \to B$
...
0
$$
comes with sections of the morphisms $\tilde A_n \to A[-1]_n$
...
there is a unique morphism $B \to \tilde A$
...
$s_n$ and the splittings of $\tilde A_n \to A[-1]_n$
as well.
\end{lemma}
...
Set $B_n \to \tilde A_n = A_n \oplus A_n \oplus A_{n - 1}$
...
(p_n, q_n, f_n \circ r_n)(a_n(f_n(x_3)) - b_n(f_n(x_3)) + s_n(d_n(x_3))) \\
& = &
...
connecting the morphisms
of chain complexes $s(e_0) : s(U) \to s(U \times \Delta[1])$
and $s(e_0) : s(U) \to s(U \times \Delta[1])$. By
Lemma \ref{lemma-map-into-tilde} above we get a corresponding morphism
$$
\Phi : \widetilde{s(U)} \to s(U \times \Delta[1])
$$
According to the construction, $\Phi_n$ restricted to the summand
$s(U)[-1]_n = s(U)_{n - 1}$ of $\widetilde{s(U)}_n$
is equal to $h_{n - 1}$. And
$$
h_{n - 1} = \sum\nolimits_{i = 0}^{n - 1}
(-1)^i s^n_i \cdot \alpha^n_{i + 1} :
U_{n - 1} \to \bigoplus U_n \cdot \alpha^n_i.
$$
...
It follows that $\delta(\sigma)$ factors as in
Lemma \ref{lemma-map-into-tilde}. By that lemma
we obtain a canonical morphism
$\Psi : s(U \times \Delta[1]) \to \widetilde{s(U)}$.
...
The only way $\alpha^n_j \circ \delta^n_i = \alpha^{n - 1}_0$
is if $j = n$ and $i = n$, or $j = n + 1$ and
$i$ is arbitrary. Similarly, the only way
$\alpha^n_j \circ \delta^n_i = \alpha^{n - 1}_n$
is if $j = 1$ and $i = 0$, or $j = 0$ and $i$ is arbitrary.
On the other hand, the map $d_{s(W), n}$
restricted to the summand $U_n \cdot \alpha^n_j$
with $1 \leq j \leq n$ is the sum of the maps
$(-1)^i d^n_i$ into the summands
$U_{n - 1} \cdot (\alpha^n_j \circ \delta^n_i)$
which occur in the decomposition of $W$.
Hence the map $
d_{s(U \times \delta[1]), n} \circ \sigma_n
-
\sigma_{n - 1} \circ d_{s(W), n}
$
maps the summand $U_n \cdot \alpha^n_j$ to
$U_{n - 1} \cdot \alpha^{n - 1}_0 \oplus U_{n - 1} \cdot \alpha^{n - 1}_n$
via
$(0, 0)$ if $j \not = 1$, and
via $(d^n_0 , -d^n_0)$ if $j = 1$.
