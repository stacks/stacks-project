...
\end{eqnarray*}
which proves the result. Here we have used that
$\text{sk}_n (X \times \Delta[n + 1]) = X \times \text{sk}_n \Delta[n + 1]$
and that
$i_{n!}(X \times \text{sk}_n \Delta[n + 1]) =
X \times i_{n !} \text{sk}_n \Delta[n + 1]$.
The first equality is obvious. The second follows because for
any simplicial set $U$ and and any simplicial object $W$
of $\mathcal{C}$ we have $\text{Mor}(U \times X, W) =
\text{Mor}(U, \text{Mor}(X, W))$.
\end{proof}
