...
Lemma \ref{lemma-finite-separated}).
The minimal set of hypothesis we will work with are described below.
This is minimal in the sense that we
only state the assumptions used in the proof; we know of no applications
but the case where the morphisms $s$, $t$ are also flat and of
finite presentation.
...
that the fibre of $s$ over $u$ is a finite set (in the case of
schemes, see
Morphisms, Lemma \ref{morphisms-lemma-finite-fibre}
in the general case insert future reference here).
\end{situation}
