...
scheme $U$ over $S$ and a surjective etale morphism $U \to (X/Y)_{fin}$.
\end{lemma}
...
(Here we implicitly used that the composition of representable, surjective, and
...
$$
we obtain a pair $(a, Z)$ with $Z = \bigcup_{(i, j) \in I} V_{i, j}$
...
$$
Claim: Each of these morphisms is etale, and the disjoint union of all of them
gives a surjective map onto $(X/Y)_{fin}$.

\medskip\noindent
The etaleness of the morphisms follows from
Lemma \ref{lemma-finite-criterion-etale}.
To show surjectivity we have to show the following: Given a scheme $T$ over
...
In other words, we may assume that $T = \text{Spec}(k)$ is
...
Choose $a : (U, u) \to (Y, y)$ as above. Picture
$$
\xymatrix{
W \coprod
\ \coprod\nolimits_{i = 1, \ldots, n}
...
$$
Now, since $k$ is algebraically closed and
...
$$
Now we know that the image of the left upper arrow ends up in
$\bigcup V_{i, j}$. Recall also that $Z$ is an open subscheme
...
$$
is an open subscheme. Now since by construction (see
More on Morphisms, Lemma
\ref{more-morphisms-lemma-etale-splits-off-quasi-finite-part-technical-variant})
each
$V_{i, j}$ has a unique point $v_{i, j}$ lying over $u$
with purely inseparable residue field extension
$\kappa(u) \subset \kappa(v_{i, j})$ we see that each
