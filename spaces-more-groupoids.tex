...
\item if $(X/Y)_{fin}$ is an algebraic spaces, then $(X/Y, \sigma)_{fin}$
...
\medskip\noindent
Denote $R_s$ (respectively ${}_tR$) the algebraic space $R$ seen as an
algebraic space over $U$ via $s$ (respectively $t$). Let
...
$g : U' \to U$. Moreover, by
Lemma \ref{lemma-finite-sheaf}
there exists a universal open and closed subspace
$Z \subset R \times_{s, U, g} U'$ which is finite over $U'$
and such that $(1_{U'}, e \circ g) : U' \to R \times_{s, U, g} U'$
factors through $Z$. Picture so far:
$$
\xymatrix{
Z \ar[r] \ar[rd] & R \times_{s, U, g} U' \ar[d] \ar[r] & U' \ar[d]_g \\
& R \ar[r]^s & U
}
$$

\medskip\noindent
Note that $i : R \to R$ is an automorphism which exchanges the roles
of $s$ and $t$. Thus $i$ extends to an isomorphism
$i : R \times_{s, U, g} U' \to U' \times_{g, U, t} R$,
$(u', r) \mapsto (i(r), u')$ over $U'$ which we
denote $i$ also. By the universal property there is an isomorphism
$\alpha : U' \to ({}_tR/U, e)_{fin}$ such that
$i(Z) \subset U' \times_{g, U, t} R$ is the pullback of the
universal open and closed subspace containing $e$ over
$({}_tR/U, e)_{fin} \times_U {}_tR$ via $\alpha$.
...
preferably by etale localization. Here is the definition (very similar to
...
We say $R$ is {\it split over $u$} if there exists an open
...
$s, t$ are separated, then $P$ is also closed in $R$ (compare with
Lemma \ref{lemma-finite-separated}).

\medskip\noindent
We will start working with a groupoid scheme as described below.
Our technique of proof leads us outside of the category of schemes,
which is why we have formulated a splitting for the case of groupoids
in algebraic spaces above.
We know of no applications but the case where the morphisms $s$, $t$
are also flat and of finite presentation.
...
that the fibre of $s : |R| \to |U|$ over $u$ is finite, see
