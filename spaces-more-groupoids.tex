...
Lemma \ref{lemma-group-scheme-over-field-separated}.
In this section we will show that under some conditions such
a separated group algebraic space is representable, i.e., a scheme.
In fact, we do not know an example of a separated group algebraic space
over a field which is not representable; if you do please email
\href{mailto:stacks.project@gmail.com}{stacks.project@gmail.com}.

\medskip\noindent
In fact, over a separated group algebraic space over an algebraically
closed field is always a scheme.
...
\begin{lemma}
\label{lemma-group-space-scheme-over-infinite-field}
Let $k$ be a field. Let $G$ be a group algebraic space over $k$
If $G$ is separated and $k$ is infinite, then $G$ is a scheme.
\end{lemma}
...
After enlaring $K'$ we may assume that there exists an affine open
...
Since $G_{K'}$ is a separated scheme (as $G$ is separated) the intersection
of a finite number of affine opens is affine open
(Schemes, Lemma \ref{schemes-lemma-characterize-separated}).
Thus we see that
$$
U = \bigcap\nolimits_{\sigma \in H} \sigma(U')
$$
is an $H$-invariant affine open subscheme of $G_{K'}$
containing the closed subscheme $\Spec(L) \times_{G_{K^{perf}}} G_{K'}$, i.e.,
the inverse image of our point $g$. By
