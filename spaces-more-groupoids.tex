...
$(G, m)$ is a group algebraic spaces over $\text{Spec}(k)$.
\end{situation}
...
with $U = \text{Spec}(k)$ for some field $k$.
\end{situation}
...
(\text{Spec}(k), G, p, p, m)
\end{equation}
where $p : G \to \text{Spec}(k)$ is the structure morphism of $G$, see
...
\item a point $r' : \text{Spec}(k') \to R'$ where
$(U', R', s', t', c')$ is the restriction of $(U, R, s, t, c)$
via $\text{Spec}(k') \to \text{Spec}(k)$
\end{enumerate}
such that
\begin{enumerate}
\item the point $r'$ maps to $r$ under the morphism $R' \to R$, and
\item the maps
$s' \circ r', t' \circ r' : \text{Spec}(k') \to \text{Spec}(k')$
are automorphisms.
\end{enumerate}
\end{lemma}

\begin{proof}
Let's represent $r$ by a morphism $r : \text{Spec}(K) \to R$ for some
...
In
Situation \ref{situation-groupoid-on-field}.
If $r : \text{Spec}(k) \to R$ is a morphism such that
$s \circ r, t \circ r$ are automorphisms of $\text{Spec}(k)$, then the map
$$
...
with
Descent of Spaces, Lemma \ref{spaces-descent-lemma-descend-unibranch}
we see that it suffices to prove this in case $k$ is algebraically closed
and $r$ comes from a morphism $r : \text{Spec}(k) \to R$ such that
$s \circ r$ and $t \circ r$
are automorphisms of $\text{Spec}(k)$. By
...
\begin{enumerate}
\item $\dim(R) = \dim_r(R)$,
\item the transcendence degree of $r$ over $\text{Spec}(k)$
via $s$ equals the transcendence degree of $r$ over $\text{Spec}(k)$
via $t$, and
...
degree of $r$ over $\text{Spec}(k)$ via $s$. Choose an \'etale morphism
...
that $r$ is represented by a morphism $r : \text{Spec}(k) \to R$ such
that both $s \circ r$ and $t \circ r$ are automorphisms of $\text{Spec}(k)$.
...
Let
$G = \text{Spec}(k)
\times_{\Delta, \text{Spec}(k) \times_B \text{Spec}(k), t \times s} R$
...
morphisms $a, b : T \to \text{Spec}(k)$ given by
