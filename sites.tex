...
we are given a morphism $U_i \to U$ of $\mathcal{C}$ with target $U$.
...
using Sets, Lemma \ref{sets-lemma-coverings-site}.
...
$\text{Cov}(\mathcal{T}_X)_{\kappa, \alpha}$,
see Sets, Lemma \ref{sets-lemma-coverings-site}.
...
has a set of objects. (More about this later; see Sets,
Lemma \ref{sets-lemma-sets-with-group-action} for a suitable
...
The site gotten by suitably restricting the collection of
coverings (as in Sets, Lemma \ref{sets-lemma-coverings-site})
will be denoted $\mathcal{T}_G$.
\end{example}
...
\begin{enumerate}
\item 
A {\it morphism of families of maps with the fixed target
of $\mathcal{C}$ from  $\mathcal{U}$ to $\mathcal{V}$},
or simply {\it a morphism from $\mathcal{U}$ to $\mathcal{V}$}
...
equivalence relation on all families of morphisms with the same target.
...
all families of morphisms with the same target.
\end{enumerate}
\end{lemma}
...
each define the structure of a site on $\mathcal{C}$. Suppose
...
$$
In particular, if also for every $\mathcal{U} \in \text{Cov}_2$
...
\begin{proof}
We advise the reader to skip the proof of this lemma on a first reading.
...
than a mathematical object. Allthough we may think about
...
for example; we do this in the case s of points of topoi
in Section \ref{section-points}. Also, it seems pretty clear
that there exists a set of $\Phi$ such that any $\Phi$ satisfying
(1), (2), (3) is equivalent to one the members of the set.
\end{remark}
...
the functor $v$ is also continuous but in
the second Example \ref{example-open-map-cocontinuous}
it is not. Hence the following lemma
applies to the first example, but not to the second.
...
unnecessary, and $g^{-1}$ has a left adjoint
$g_{!} = (u_p\ )^\#$. Also, $g^{-1}$ commutes with
...
\noindent
In the situation of the lemma above we see that we have a sequence
of adjoint functors
$$
g_{!},\ g^{-1},\ g_*.
$$
...
On the other hand, in the topological setting the functor
...
$g_!$ above commutes with equalizers.
\end{lemma}

\begin{proof}
We have $g_! = (u_p\ )^\#$.
Recall that the equalizer of a pair of maps between sheaves
is equal to the corresponding equalizer as presheaves.
And sheafification commutes with equalizers as well. Thus it
suffices to show that $u_p$ commutes with equalizers in this case.
To do this it suffices that colimits over the categories
$(\mathcal{I}_V^u)^{opp}$
of Section \ref{section-functoriality-PSh} commute
with equalizers. This follows from Lemma \ref{lemma-almost-directed} and
...
It may appear at first that this functor is cocontinuous as
...
results of the previous section we obtain three functors
$j_{U!}$, $j_{U}^{-1}$ and $j_{U*}$.

\begin{definition}
\label{definition-localization}
Let $\mathcal{C}$ be a site.
Let $U \in \text{Ob}(\mathcal{C})$.
The functor $j_{U*}$ is called the {\it direct image functor}.
For a sheaf $\mathcal{F}$ on $\mathcal{C}$ the sheaf
$j_U^{-1}\mathcal{F}$ is called the {\it restriction of $\mathcal{F}$
to $\mathcal{C}/U$}.
For a sheaf $\mathcal{G}$ on $\mathcal{C}/U$ the sheaf
the sheaf $j_{U!}\mathcal{G}$ is called the
{\it extension of $\mathcal{G}$ by the empty set}.
\end{definition}

\noindent
Note that $j_{U!}(*) = h_U^\#$, and hence for every sheaf
$\mathcal{F}$ over $\mathcal{C}/U$ there is a canonical map
of sheaves $j_{U!}\mathcal{F} \to h_U^\#$. This characterizes
...
\noindent
Suppose that $\mathcal{C}$ has fibre products.
In this case the functor $j_U$ clearly commutes with
fibre products, and Lemma \ref{lemma-preserve-equalizers} applies.
Hence under this
hypothesis the functor $j_{U!}$ commutes with
equalizers. In particular, if $\mathcal{F} \subset \mathcal{F}'$
in $\textit{Sh}(\mathcal{C}/U)$,
then $j_{U!}\mathcal{F} \subset j_{U!}\mathcal{F}'$.
...
We may compute
$(j_U^{-1}j_{U*}\mathcal{F})(X \to U) = j_{U*}\mathcal{F}(X)
= \text{colim} \mathcal{F}(Y \to U)$, where the colimit
is over pairs $(Y \to U, X \to Y)$
...
value on $\text{id}_X$, i.e., $\mathcal{F}(X \to U)$.
On the other hand,
$(j_U^{-1}j_{U!}\mathcal{F})(X \to U) =
j_{U!}\mathcal{F}(X) = (u_p\mathcal{F})^\#(X)$, and
$u_p\mathcal{F}(X) = \text{colim} \mathcal{F}(Y \to U)$
where the colimit is over pairs $(Y \to U, Y \to X)$
...
$$
is injective. Hence there exists a $k$ such that $s, s'$
...
of abelian groups. Since all colimits are representable in
the category of abelian groups, we certainly may define
a functor $u_p^{ab}$ on abelain presheaves by the same colimits
...
a quadruple $(\mathcal{F}, +, 0, i)$ as explained above.
If $(\mathcal{F}, +, 0, i)$ lives on $\mathcal{C}$, then we use
$(f^{-1}\mathcal{F}, f^{-1}+, f^{-1}0, f^{-1}i)$.
If $(\mathcal{G}, +, 0, i)$ lives on $\mathcal{D}$, then we use
$(f_*\mathcal{G}, f_*+, f_*0, f_*i)$. This works because
...
\noindent
We will discuss the category of sheaves of modules over
a sheaf of rings in a later chapter.
