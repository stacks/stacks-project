...
of Categories, Definition \ref{categories-definition-limit} (resp.\ 
Categories, Definition \ref{categories-definition-colimit}). Finally, if
...
\}
\\
\text{Mor}_{\mathcal{I}^u_V}((U, \phi), (U', \phi'))
&
=
&
\{
...
\end{enumerate}
Then the index categories $(\mathcal{I}^u_V)^{\text{opp}}$ of are filtered (see
...
directed categories.
Hence it suffices to show that $\mathcal{I}_V$ is nonempty and connected.
...
are topoi, then setting $f_* = \alpha$ and $f^{-1}$ equal to the quasi-inverse
...
equal to the the category of sheaves on a site). We will occasionally
...
$i : \textit{Sh}(pt) \to \textit{Sh}(\mathcal{S})$ given by
the functors
\begin{equation}
\label{equation-sheaves-pt-sets}
i^{-1}\mathcal{F} = \mathcal{F}(e),\quad
i_*E = (U \mapsto \text{Mor}_{\textit{Sets}}(U, E))
\end{equation}
...
$$
i_* :
\textit{Sets} = \textit{Sh}(pt)
\longrightarrow
\textit{Sh}(\mathcal{S}),\quad
...
$$
{}_pu(\mathcal{F})(V) := \lim\nolimits_{{}_V\mathcal{I}^{opp}} {}_V\mathcal{F}
$$
...
$\textit{Sh}(\mathcal{C})$. Moreover, the functor $f^{-1}$ is fully faithful.
\end{lemma}
...
Let $\mathcal{F}, \mathcal{G} \in \text{Ob}(\textit{Sh}(\mathcal{C}))$.
We have to show that the map
$$
\text{Mor}_{\textit{Sh}(\mathcal{C})}(\mathcal{F}, \mathcal{G})
\longrightarrow
\text{Mor}_{\textit{Sh}(\mathcal{D})}(f^{-1}\mathcal{F}, f^{-1}\mathcal{G})
$$
is bijective. But the right hand side is equal to
\begin{align*}
\text{Mor}_{\textit{Sh}(\mathcal{D})}(f^{-1}\mathcal{F}, f^{-1}\mathcal{G})
& =
\text{Mor}_{\textit{Sh}(\mathcal{C})}(f_*f^{-1}\mathcal{F}, \mathcal{G}) \\
& =
\text{Mor}_{\textit{Sh}(\mathcal{C})}(g^{-1}f^{-1}\mathcal{F}, \mathcal{G}) \\
& =
\text{Mor}_{\textit{Sh}(\mathcal{C})}(\mathcal{F}, \mathcal{G})
\end{align*}
...
\vcenter{
\xymatrix{
\textit{Sh}(\mathcal{C}/V) \ar[rd]_{j_V} \ar[rr]_j & &
\textit{Sh}(\mathcal{C}/U) \ar[ld]^{j_U} \\
& \textit{Sh}(\mathcal{C}) &
}
}
\end{equation}
...
$$
U \longmapsto
\text{Mor}_{\textit{Sh}(\mathcal{C}/U)}(
...
Let $\mathcal{C}$ be a site.
Let $\{U_i \to U\}_{i \in I}$ be a covering of $\mathcal{C}$.
The category $\textit{Sh}(\mathcal{C}/U)$ is equivalent
...
$\textit{Sh}(\mathcal{C}/U) = \textit{Sh}(\mathcal{C})/h_U^\#$ and
$\textit{Sh}(\mathcal{D}/V) = \textit{Sh}(\mathcal{D})/h_V^\#$ of
Lemma \ref{lemma-essential-image-j-shriek}
...
$$
\xymatrix{
\textit{Sh}(\mathcal{C}/U) \ar[r]_{j_U} \ar[d]_{f_c} &
\textit{Sh}(\mathcal{C}) \ar[d]^f \\
\textit{Sh}(\mathcal{D}/V) \ar[r]^{j_V} &
\textit{Sh}(\mathcal{D}).
}
...
$f' : \textit{Sh}(\mathcal{C}/u(V)) \to \textit{Sh}(\mathcal{D}/V)$
is as in
Lemma \ref{lemma-localize-morphism}
and
$j_{U/u(V)} : \textit{Sh}(\mathcal{C}/U) \to \textit{Sh}(\mathcal{C}/u(V))$
is as in
Lemma \ref{lemma-relocalize}.
Using the identifications
$\textit{Sh}(\mathcal{C}/U) = \textit{Sh}(\mathcal{C})/h_U^\#$ and
...
$$
\xymatrix{
\textit{Sh}(\mathcal{C}/U') \ar[r]_{j_{U'/U}} \ar[d]_{f_{c'}} &
\textit{Sh}(\mathcal{C}/U) \ar[d]^{f_c} \\
\textit{Sh}(\mathcal{D}/V') \ar[r]^{j_{V'/V}} &
\textit{Sh}(\mathcal{D}/V).
}
$$
commutes.
\end{lemma}
...
Hence
$$
g^{-1}g_*\mathcal{F}(U) = \lim\nolimits_{U', u(U') \to u(U)} \mathcal{F}(U')
$$
...
$$
g_*g^{-1}\mathcal{G}(V) = \lim_{U, \psi : u(U) \to V} \mathcal{G}(u(U))
$$
...
of $\mathcal{C}$. Then $j_{\mathcal{F}} : \textit{Sh}(\mathcal{C})/\mathcal{F}
\to \textit{Sh}(\mathcal{C})$ constructed in
Lemma \ref{lemma-localize-topos}
...
Let $\mathcal{G} \in \textit{Sh}(\mathcal{D})$,
$\mathcal{F} \in \textit{Sh}(\mathcal{C})$
and $s : \mathcal{F} \to f^{-1}\mathcal{G}$ a morphism of sheaves.
There exists a commutative diagram of topoi
$$
\xymatrix{
\textit{Sh}(\mathcal{C})/\mathcal{F} \ar[r]_{j_{\mathcal{F}}} \ar[d]_{f_s} &
\textit{Sh}(\mathcal{C}) \ar[d]^f \\
\textit{Sh}(\mathcal{D})/\mathcal{G} \ar[r]^{j_{\mathcal{G}}} &
\textit{Sh}(\mathcal{D}).
}
...
$f' :
\textit{Sh}(\mathcal{C})/f^{-1}\mathcal{G}
\to
\textit{Sh}(\mathcal{D})/\mathcal{F}$
is as in
Lemma \ref{lemma-localize-morphism-topoi}
and
...
\textit{Sh}(\mathcal{C})/\mathcal{F} \ar[d]^{f_s} \\
\textit{Sh}(\mathcal{D})/\mathcal{G}' \ar[r]^{j_{\mathcal{G}'/\mathcal{G}}} &
\textit{Sh}(\mathcal{D})/\mathcal{G}.
}
$$
commutes.
\end{lemma}
...
$i : \textit{Sh}(pt) \to \textit{Sh}(\mathcal{S})$,
...
$$
f_*i_*E(U) = i_*E(u(U)) = \text{Mor}_{\textit{Sets}}(u(U), E)
$$
...
Let $\mathcal{C}$ be a site. Let
$p : \textit{Sh}(pt) \to \textit{Sh}(\mathcal{C})$ be a point of
...
$p : \textit{Sh}(pt) \to \textit{Sh}(\mathcal{C})$ be a point of
the topos associated to $\mathcal{C}$.
The functor $p_* : \textit{Sets} \to \textit{Sh}(\mathcal{C})$
...
surjections, and it reflects isomorpisms.
\end{lemma}
...
$$
p_*E(U) = \text{Mor}_{\textit{Sets}}(u(U), E)
$$
...
$$
\lim\nolimits_i \text{colim}_{\{(U, x)\}^{opp}}\ \mathcal{F}_i(U)
=
\text{colim}_{\{(U, x)\}^{opp}}\ \lim\nolimits_i \mathcal{F}_i(U).
$$
...
\section{Points and and morphisms of topoi}
...
Then
$$
\mathcal{F}_q = \text{colim}_{(U, x)}\ \mathcal{F}(U)
$$
...
& =
\text{colim}_{(V, x)}\ \text{colim}_{U, \phi : V \to u(U)}\ \mathcal{F}(U) \\
& = \text{colim}_{(V, x, U, \phi : V \to u(U))} \mathcal{F}(U) \\
& = \text{colim}_{(U, x)} \mathcal{F}(U) \\
& = \mathcal{F}_q
\end{align*}
Explanation:
...
$$
\textit{Sh}(pt) \xrightarrow{i} \textit{Sh}(\mathcal{S})
\xrightarrow{h} \textit{Sh}(\mathcal{D})
$$
...
$$
g_*i_*E(U) = i_*E(v(u(U)) = \text{Mor}_{\textit{Sets}}(v(u(U)), E)
$$
...
is right adjoint to the the stalk functor
...
Let $f : \textit{Sh}(\mathcal{D}) \to \textit{Sh}(\mathcal{C})$
be a morphism of topoi. Let $p : \textit{Sh}(pt) \to \textit{Sh}(\mathcal{D})$
be a point. Then $q = f \circ p$ is a point of the topos
$\textit{Sh}(\mathcal{C})$ and we have
a canonical identification
$$
(f^{-1}\mathcal{F})_p = \mathcal{F}_q
$$
...
$q_j : \textit{Sh}(pt) \to \textit{Sh}(\mathcal{C})$ the composition
of $p_j$ with the localization morphism
$\textit{Sh}(\mathcal{C}/U_i) \to \textit{Sh}(\mathcal{C})$.
...
\text{Mor}_{\textit{PSh}(\mathcal{C})}(*, \mathcal{F})
$$
...
\noindent
Of course the same definition applies to sheaves as well. Now,
let $f : \textit{Sh}(\mathcal{D}) \to \textit{Sh}(\mathcal{C})$ be a
morphism of topoi. Then for any sheaf $\mathcal{G}$ on $\mathcal{D}$
there is a pullback map
$$
f^{-1} :
\Gamma(\mathcal{D}, \mathcal{F})
\longrightarrow
\Gamma(\mathcal{C}, f^{-1}\mathcal{F})
$$
...
$$
\xymatrix{
\textit{Sh}(\mathcal{D}) \ar[rd]_h \ar[rr]_f & &
\textit{Sh}(\mathcal{C}) \ar[ld]^g \\
& \textit{Sh}(\mathcal{B})
}
$$
Next, suppose that we have a sheaf $\mathcal{F}$ on $\mathcal{D}$.
...
$$
\xymatrix{
\textit{Sh}(\mathcal{D}) \ar[d]_h \ar[r]_f &
\textit{Sh}(\mathcal{C}) \ar[d]^g \\
\textit{Sh}(\mathcal{B}) \ar[r]^e & \textit{Sh}(\mathcal{A})
}
$$
