...
\noindent
Note that, being and adjoint pair, the functor
...
Note that $j_{U!}(*) = h_U^\#$ because
$$
\text{Mor}_{\textit{Sh}(\mathcal{C})}(j_{U!}(*), \mathcal{F})
=
\text{Mor}_{\textit{Sh}(\mathcal{C}/U)}(*, j_U^{-1}\mathcal{F})
=
\mathcal{F}(U)
=
\text{Mor}_{\textit{Sh}(\mathcal{C})}(h_U^\#, \mathcal{F}).
$$
Hence for every sheaf
...
\begin{proof}
This is a special case of Lemma \ref{lemma-back-and-forth} but we
give a direct proof here also.
...
$u(a) = u(b)$, then there exists a covering $\{f_i : U_i \to U\}$
...
by the value $s_{(U, \text{id})} \in \mathcal{F}(U)$. By property (3) given
any $(U', \psi)$ there exists a covering $\{U'_i \to U'\}$ such that the
compositions $u(U'_i) \to u(U') \to u(U)$ are of the form $u(\alpha_i)$
for some $\alpha_i : U'_i \to U$. Hence
$$
s_{(U', \psi)}|_{U'_i} = \alpha_i^*s_{U, \text{id}}.
$$
Since $\mathcal{F}$ is a sheaf it follows that indeed $s_{(U', \psi)}$
is determined by $s_{(U, \text{id})}$. By a similar argument, using
property (4), we see given any
$s \in \mathcal{F}(U)$ there exists a (unique) element
$(s_{(U', \psi)})$ of the limit with $s_{(U, \psi)} = s$
(some details omitted). This proves
that $g^{-1}g_*\mathcal{F} \to \mathcal{F}$ is an isomorphism.

\medskip\noindent
Let $\mathcal{G}$ be a sheaf on $\mathcal{D}$ and let $V$ be an
object of $\mathcal{D}$. Then we see that
$$
g_*g^{-1}\mathcal{G}(V) = \text{lim}_{U, \psi : u(U) \to V}\ \mathcal{G}(u(U))
$$
An element of this limit will be indicated $(s_{(U, \psi)})$.
The map $\mathcal{G}(V) \to g_*g^{-1}\mathcal{G}(V)$ is
given by $s \mapsto (\psi^*s)_{(U, \psi)}$. Since by axiom (5) there
exists a covering of $V$ by objects of the form $u(U)$ we see
easily that this map is bijective.
\end{proof}
...
\begin{enumerate}
\item $h_U^\sharp \in \text{Ob}(\mathcal{C}')$ for all
...
$U \mapsto h_U^\sharp$ satisfies the assumptions of
Lemma \ref{lemma-equivalence}, hence induces an
...
$g_*h_U^\sharp = h_{v(U)} = h_{h_U^\sharp}$.
\end{enumerate}
\end{lemma}
...
$U \mapsto h_U^\sharp$ commutes with fibre products,
...
are statements about morphisms $f : h_U^\sharp \to h_{U'}^\sharp$.
Such a morphism is the same thing as an element of $h_{U'}^\sharp(U)$.
...
= \text{Mor}_{\textit{Sh}(\mathcal{C})}(h_U^\sharp, \mathcal{F})
= \mathcal{F}(U)
$$
...
Then
\begin{align*}
g_*h_U^\sharp(\mathcal{F})
& =
\text{Mor}_{\textit{Sh}(\mathcal{C}')}(h_{\mathcal{F}}, g_*h_U^\sharp) \\
& =
\text{Mor}_{\textit{Sh}(\mathcal{C})}(g^{-1}h_{\mathcal{F}}, h_U^\sharp) \\
& =
\text{Mor}_{\textit{Sh}(\mathcal{C})}(\mathcal{F}, h_U^\sharp) \\
& =
\text{Mor}_{\mathcal{C}'}(\mathcal{F}, h_U^\sharp)
\end{align*}
...
such that
\begin{enumerate}
\item the functor $v$ satisfies the hypotheses of
...
$h_U^\sharp$ and all $f^{-1}h_V^\sharp$ for all
...
products as both $V \mapsto h_V^\sharp$ and $f^{-1}$ do. Moreover,
...
(u^sg_*h_U^\sharp)(V)
& =
g_*h_U^\sharp(f^{-1}h_V^\sharp) \\
& =
\text{Mor}_{\textit{Sh}(\mathcal{C})}(f^{-1}h_V^\sharp, h_U^\sharp) \\
& =
\text{Mor}_{\textit{Sh}(\mathcal{D})}(h_V^\sharp, f_*h_U^\sharp) \\
& = f_*h_U^\sharp(V)
\end{align*}
...
properties of sheafificatio and the Yoneda lemma.
We omit the verification that these identities are functorial in $U$
and $V$. Hence we see that
we have $u^s \circ g_* = f_*$ for sheaves of the form $h_U^\sharp$.
