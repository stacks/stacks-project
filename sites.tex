...
H^0(\mathcal{U}, \mathcal{F})
$$
See Categories, \autoref{categories-subsubsection-limits} for
a discussion of limits and colimits. We point out that later
we will see that $\mathcal{F}^{+}(U)$ is the zeroth Cech
cohomology of $\mathcal{F}$ over $U$. For the moment we recall
that the colimit $\mathcal{F}^{+}(U)$ may be described
...
\medskip\noindent
Proof of (\ref{plus-iso}). By (\ref{sep}) the map
$\mathcal{F} \to \mathcal{F}^+$ is injective,
and it follows easily from Lemma \ref{lemma-plus-surjective}
that it is surjective as well.
