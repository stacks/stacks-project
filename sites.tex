...
is also the colimit of sites. Namely, suppose that we have
\begin{enumerate}
\item a directed index set $I$,
\item for $i \in I$ a site $\mathcal{C}_i$,
\item for $i' \geq i$ a morphisms of sites
$f_{i'i} : \mathcal{C}_{i'} \to \mathcal{C}_i$
given by a continuous functor $u_{i'i} : \mathcal{C}_i \to \mathcal{C}_{i'}$
\end{enumerate}
such that $f_{i'' i} = f_{i' i} \circ f_{i'' i'}$.
For simplicity we assume that every covering in $\mathcal{C}_i$
has a finite index set. Then we can define
\begin{enumerate}
\item a category $\mathcal{C} = \colim \mathcal{C}_i$
using the functors $u_{i'i}$, and
\item a set of families of morphisms with fixed target
$\text{Cov}(\mathcal{C})$ defined as the union of the images
of $\text{Cov}(\mathcal{C}_i)$ by the coprojections
$u_i : \mathcal{C}_i \to \mathcal{C}$.
\end{enumerate}

\begin{lemma}
\label{lemma-colimit-sites}
In the situation above the category $\mathcal{C}$ endowed with
$\text{Cov}(\mathcal{C})$ is a site and the functors $u_i$ define
morphisms of sites $f_i : \mathcal{C} \to \mathcal{C}_i$.
\end{lemma}

\begin{proof}
Let $\{U^j_i \to U_i\}$ be a covering of $\mathcal{C}_i$ for some $i$.
Let's write $U^j \to U$, resp.\ $U^j_{i'} \to U_{i'}$ instead of
$u_i(U^j_i) \to u_i(U_i)$, resp.\ $u_{i'i}(U_i^j) \to u_{i'i}(U)$.
We will use similar notation for other coverings, objects, morphisms
that will occur in the proof.

\medskip\noindent
Let's prove that if $V \to U$ is a morphism of $\mathcal{C}$, then
$U^j \times_U V$ exists. Namely, $V \to U$ comes from $V_{i'} \to U_{i'}$
for some $i' \geq i$. Then $U^j_{i'} \times_{U_{i'}} V_{i'}$ exists.
Since each $u_{i'' i'}$ is continuous we see that
$$
u_{i'' i'}(U^j_{i'} \times_{U_{i'}} V_{i'}) =
U^j_{i''} \times_{U_{i''}} V_{i''}
$$
for all $i'' \geq i'$. It follows by a formal argument that
$U^j \times_U V$ exists and is the image of $U^j_{i'} \times_{U_{i'}} V_{i'}$
by the functor $u_{i'}$. It also follows that
$\{U^j \times_U V \to V\}$ is a covering in $\mathcal{C}$.
...
let $\{U^j \to U\}_{j \in J}$ be a covering of $\mathcal{C}$
and for each $j$ let $\{U^{jk} \to U^j\}$ be a covering of
$\mathcal{C}$. Then we can find an $i$ and a covering
$\{U^j_i \to U_i\}_{j \in J}$ of $\mathcal{C}_i$ whose image by $u_i$ is
$\{U^j \to U\}$. Since $J$ is {\bf finite} we may choose an $i' \geq i$
and coverings $\{U^{jk}_{i'} \to U^j_{i'}\}$ of
$\mathcal{C}_{i'}$ whose image by $u_{i'}$ gives $\{U^{jk} \to U^j\}$.
Then we conclude that $\{U^{jk} \to U\}$ is a covering of $\mathcal{C}$.

\medskip\noindent
We omit the proof of axiom (1) of Definition \ref{definition-site}.

\medskip\noindent
It follows from the arguments above that the functors $u_i$ are
continuous. To finish the proof we have to show that
$f_i^{-1} := u_{i, s}$
...
(Categories, Lemma \ref{categories-lemma-exact-adjoint}).
Before we do this we describe this functor more explicity
for a particular index $0 \in I$.

\medskip\noindent
Let $\mathcal{F}_0$ be a sheaf on $\mathcal{C}_0$ and for $i \geq 0$
we set $\mathcal{F}_i = f_{i0}^{-1}\mathcal{F}_0$. Let $i \geq 0$
be an index and let $U_i$ be an object of $\mathcal{C}_i$.
We claim that
\begin{equation}
\label{equation-compute-pullback-to-limit}
f_0^{-1}\mathcal{F}_0(U) = \colim_{i' \geq i} \mathcal{F}_{i'}(U_{i'})
\end{equation}
where $U$ and $U_{i'}$ are the images of $U_i$ in $\mathcal{C}$ and
$\mathcal{C}_{i'}$ as above.

\medskip\noindent
Proof of the claim. A straightforward argument with colimits (similar
to the above) shows that the sheaf $f_0^{-1}\mathcal{F}_0$ is the
sheaf associated to the presheaf $\mathcal{G}$ given by the right hand side of
(\ref{equation-compute-pullback-to-limit}). We omit the proof.
Thus to finish the proof we have to show that the right hand side
satisfies the sheaf condition. However, this is clear because
any covering $\{U^j \to U\}$ in $\mathcal{C}$
comes from a finite level, say $\mathcal{U}_{i'} = \{U^j_{i'} \to U_{i'}\}$
for some $i' \geq i$. Then we have
$$
H^0(\mathcal{U}, \mathcal{G}) =
\colim_{i'' \geq i'} H^0(\mathcal{U}_{i''}, \mathcal{F}_{i''}) =
...
\medskip\noindent
Assume the claim holds. Then $f_0^{-1}$ commutes with finite limits because
finite limits of sheaves are computed in the category of presheaves
(Lemma \ref{lemma-limit-sheaf})
and filtered colimits commute with finite limits.
\end{proof}

\noindent
We continue the discussion. Assume given
\begin{enumerate}
\item a sheaf $\mathcal{F}_i$ on $\mathcal{C}_i$ for all $i \in I$,
\item for $i' \geq i$ a map
$\varphi_{i'i} : f_{i'i}^{-1}\mathcal{F}_i \to \mathcal{F}_{i'}$
of sheaves on $\mathcal{C}_{i'}$
\end{enumerate}
...
Let $0 \in I$, $X_0 \in \text{Ob}(\mathcal{C}_0)$, set $X = u_0(X_0)$
and $X_i = u_{i0}(X_0)$ for $i \geq 0$. Then
$$
\colim_{i \geq 0} \mathcal{F}_i(X_i) = \mathcal{F}(X)
$$
\end{lemma}

\begin{proof}
We may and do replace $I$ by $\{i \in I, i \geq 0\}$. A formal argument
shows that
$$
\colim_{i \in I} \mathcal{F}_i(X_i) =
\colim_{j \in I} \colim_{i \geq j} f_{ij}^{-1}\mathcal{F}_j(X_i)
$$
By (\ref{equation-compute-pullback-to-limit})
we see that the inner colimit is equal to
$f_j^{-1}\mathcal{F}_j(X)$ hence we conclude by
Lemma \ref{lemma-colimit-sites} applied with $\mathcal{B} = \Ob(\mathcal{C})$
and $\text{Cov} = \text{Cov}(\mathcal{C})$.
\end{proof}
...
if and only if $\coprod h_{V_i} \to h_V$ is surjective,
...
is callend an {\it embedding} if $f_*$ is fully faithfull.
\end{definition}
