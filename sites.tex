...
\noindent
The material in this section will help prove that for a finite (or more
generally integral) morphism of schemes $f : X \to Y$ the push forward
on abelian etale sheaves is exact.
...
\item for every object $V$ of $\mathcal{D}$ there exist objects
$U_i$ of $\mathcal{C}$ and morphisms $u(U_i) \to V$ such that
$\{u(U_i) \to V\}$ is a covering of $\mathcal{D}$.
\end{enumerate}
In this case the functor
$u^s : \textit{Sh}(\mathcal{D}) \to \textit{Sh}(\mathcal{C})$
is faithful and reflects isomorphisms, injections and surjections.
Moreover, for any sheaf $\mathcal{F}$ on $\mathcal{D}$ the canonical
map $u_su^s\mathcal{F} \to \mathcal{F}$ is surjective.
\end{lemma}

\begin{proof}
Let $\alpha, \beta : \mathcal{F} \to \mathcal{G}$ be maps of sheaves
...
Hence it is clear that if $u^s\alpha = u^s\beta$, then $\alpha = \beta$.
Similarly, it is clear that if $u^s\alpha$ is injective, then
$\alpha$ is injective.

\medskip\noindent
Suppose that $u^s\alpha$ is surjective. Then for $V, U_i, u(U_i) \to V$
...
$s$ is locally in the image. In other words $\alpha$ is surjective.

\medskip\noindent
Because $u^s$ reflects surjections
it suffices to prove that $u^su_su^s\mathcal{F} \to u^s\mathcal{F}$
is surjective. Since $u^s\mathcal{F}(U) = \mathcal{F}(u(U))$
this means it suffices to prove that for every object $U$ of
$\mathcal{C}$ the map $u_s(u^s\mathcal{F})(u(U)) \to \mathcal{F}(u(U))$
is surjective. Recall that $u_s(u^s\mathcal{F})$ is the sheaf associated to
$u_p(u^s\mathcal{F})$. And
$$
u_p(u^s\mathcal{F})(u(U)) =
\text{colim}_{u(U) \to u(U')} \mathcal{F}(u(U')),
$$
see Section \ref{section-functoriality-PSh}.
The index category of this colimit contains the pair
$(U, \text{id} : u(U) \to u(U))$ and hence we see that the map
$u_p(u^s\mathcal{F})(u(U)) \to \mathcal{F}(u(U))$ is surjective
(as it has a section).
\end{proof}
