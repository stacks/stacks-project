...
$w^p\mathcal{F} : V \mapsto \mathcal{F}(w(V))$, and
\item $g_!$ is exact.
\end{enumerate}
...
The value of $u_p\mathcal{F}$ at $V$ is the colimit of $\mathcal{F}$
over the opposite of the category $\mathcal{I}_V^u$ whose objects
are pairs $(U, \varphi)$ with $\varphi : V \to u(U)$. See
Section \ref{section-functoriality-PSh}. Since $w$ is a left adjoint we
see that $\varphi$ corresponds to a morphism $w(V) \to U$. Thus the
pair $(w(V), \text{id})$ is an initial object of $\mathcal{I}_V^u$.
This proves (1)(a).
...
the coeqalizer of $a$, $b$ in $\Sh(\mathcal{C})$ lies in $E$.
