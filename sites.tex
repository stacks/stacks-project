...
we write {\it $\underline{U} = h_U$} for the representable
...
be functors. Assume that $u$ is cocontinuous, $u$ is continuous,
and that $v$ is a right adjoint to $u$.
Then (a) $v$ gives rise to a morphism of sites
$f$ from $\mathcal{C}$ to $\mathcal{C}$, and (b) the
morphisms of topoi $g$ and $f$ are identical.
\end{lemma}

\begin{proof}
Let $V$ be an object of $\mathcal{D}$. We have $u^ph_V = h_{v(V)}$ because
$u^ph_V(U) = \text{Mor}_{\mathcal{D}}(u(U), V) =
\text{Mor}_{\mathcal{C}}(U, v(V)$
by assumption.
