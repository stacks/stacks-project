...
\item Assume there exists a subset $\mathcal{B} \subset \Ob(\mathcal{C})$
...
Set $a = \colim_{i' \geq i} a_{i'}$ and similary for $b$.
For $i \geq i$ denote
$$
...
$E = \colim_{i' \geq i} E_{i'}$. This just says that $\coprod E_i \to E$
is a surjective map of sheaves. hence, if $E = *$, i.e., if $a = b$,
then because $*$ is quasi-compact, we see that $E_i = *$ for some $i$,
and we conclude $a_i = b_i$ for some $i$. This proves (2).
...
\medskip\noindent
Proof of (4). Assume (4)(a), (b), (c) are satisfied.
By (4)(b) and Lemma \ref{lemma-conclude-quasi-compact} the objects
$U \in \mathcal{B}$ are quasi-compact. Hence by (4)(a) and
Lemma \ref{lemma-image-of-quasi-compact} we see that
$\Sh(\mathcal{C})$ is quasi-compact. Thus $\Psi$ is injective by (2).
Let $s : * \to \colim \mathcal{F}_i$ be a global section of the colimit.
Choose $m \geq 0$ and $U_1, \ldots, U_m \in \mathcal{B}$
as in (4)(a). The restriction $s_j$ of $s$ to $U_j$, i.e., the
composition $h_{U_j}^\# \to * \to \colim \mathcal{F}_i$, is an
element of $(\colim \mathcal{F}_i)(U_j)$. On the members of a
covering of $U_j$ the section $s_j$ comes from a section of some
$\mathcal{F}_i$. By (4)(b) we may assume this covering
is finite $\{U_{jk} \to U_j\}_{k = 1, \ldots, o_j}$
with $U_{jk} \in \mathcal{B}$. Note that
$\coprod_k h_{U_{jk}}^\# \to h_{U_j}^\#$ is surjective by
Lemma \ref{lemma-covering-surjective-after-sheafification}.
Thus, after replacing $U_1, \ldots, U_m$ by
$U_{11}, \ldots, U_{m o_m}$ we may assume that
$s_j = s|_{U_j}$ is the image of some $s'_j \in \mathcal{F}_{i_j}(U_j)$.
By choosing an $i \in I$ with $i \geq i_j$ for all $j$ and
taking the image of $s'_j$ in $\mathcal{F}_i$ we may assume
$s'_j \in \mathcal{F}_i(U_j)$ for all $i$.
Observe that
$$
\xymatrix{
\coprod_{1 \leq j, j' \leq m} h_{U_j}^\# \times h_{U_{j'}}^\#
\ar@<1ex>[r] \ar@<-1ex>[r] &
\coprod_{1 \leq j \leq m} h_{U_j}^\# \ar[r] &
{*}
}
$$
is a coequalizer diagram. Hence in order to ``glue'' the sections
$s'_j$ we have to show that the two maps
$h_{U_j}^\# \times h_{U_{j'}}^\# \to \mathcal{F}_i$
are the same, after possibly increasing $i$.
We do know, by the existence of $s$ and our choices above,
that the two induced maps
$h_{U_j}^\# \times h_{U_{j'}}^\# \to \colim_{i' \geq i} \mathcal{F}_i$
are the same.
Finally, for each pair $1 \leq j, j', m$ using (4)(c) choose
$m_{jj'} \geq 0$, $W_{jj'1}, \ldots, W_{jj'm_{jj'}} \in \mathcal{B}$, and
morphisms $W_{jj'k} \to U_j$ and $W_{jj'k} \to U_{j'}$ such that
$\coprod h_{W_{jj'k}}^\# \to h_{U_j}^\# \times h_{U_{j'}}^\#$ is surjective.
Then we see that it suffices to equalize the
restrictions $s'_j|_{W_{jj'k}}$ and $s'_{j'}|_{W_{jj'k}}$.
Since these both map to $s|_{W_{jj'k}}$ and since $W_{jj'l}$
is quasi-compact, we see that we can achieve this by
Lemma \ref{lemma-directed-colimits-sections} part (2).
This finishes the proof.
\end{proof}

\begin{remark}
\label{remark-variants}
Let $\mathcal{C}$ be a site. Suppose that
\begin{enumerate}
...
\end{enumerate}
Then properties (4)(a), (b), (c) of
Lemma \ref{lemma-directed-colimits-global-sections} hold
with $\mathcal{B} \subset \Ob(\mathcal{C})$ the set of
