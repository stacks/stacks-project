...
$$
and we have $f'_*j_U^{-1} = j_V^{-1}f_*$.
\end{lemma}
...
and an adjoint $(f')^{-1} = (u')_s = ((u')_p\ )^\#$. The final
...
$$
(Note that $f^{-1}h_V^\# = (h_{u(V)})^\# = h_U^\#$, see
...
for some $U \in \text{Ob}(\mathcal{C})$.
Hence the material of
Section \ref{section-localization}
applies and the result is clear.
\end{proof}
...
Lemma \ref{lemma-pullback-representable-sheaf}.
At this point the result follows from
Lemma \ref{lemma-morphism-sites-localization}.
\end{proof}
