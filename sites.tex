...
$$
\Mor_{\Sh(\mathcal{C})}(\underline{U}, \mathcal{F})
=
\mathcal{F}(U)
$$
...
Topoi.
\begin{enumerate}
\item  A {\it topos} is the category $\Sh(\mathcal{C})$ of sheaves
...
of topoi $f : \Sh(\mathcal{D}) \to \Sh(\mathcal{C})$.
The composition
$$
\xymatrix{
\Sh(\mathcal{C}) \ar[r]^g &
...
$\Sh(X)$? To see this, let $\mathcal{T}_X$ be the site of
Example \ref{example-site-topological}.
By
Lemma \ref{lemma-point-site-topos}
...
let $f : \Sh(\mathcal{D}) \to \Sh(\mathcal{C})$ be a
