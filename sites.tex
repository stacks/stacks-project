...
It is the sheafification of the presheaf that associates
to each object $U$ of $\mathcal{C}$ a singleton set, with unique
restriction mappings. We often denote this sheaf by $*$.
The value of $*$ on an object $U$ is empty if $U$ has
an empty covering, i.e., the empty collection of morphisms
with target $U$ is a covering of $U$. The value of $*$ on $U$
is a singleton if $U$ does not have an empty covering.
\end{example}
...
\vcenter{
\xymatrix{
{}^u_V\mathcal{I} \ar[r]_j \ar[d]_{u'} & \mathcal{C} \ar[d]^u \\
\mathcal{D}/V \ar[r]^-{j_V} & \mathcal{D}
}
}
\quad\text{where}\quad
