...
We sometimes call this a {\it scyscraper sheaf}.
\end{definition}
...
$$
\textit{Sh}(\mathcal{T}_G) = G\textit{-Sets} \longrightarrow \textit{Sets}
$$
...
A {\it morphism $f : p \to p'$} is given by a transformation of functors
$$ f : p_* \to p'_* $$
(see Definition \ref{definition-pushforward-point}).
\end{definition}
...
\item Describe $\text{Mor}(p, p')$ in terms of 
$\text{Mor}(u', u)$, where $u, u' : \mathcal{C} \to \textit{Sets}$
are the corresponding functors.
\item More examples, e.g., specialization of points in topological
spaces.
\end{enumerate}
