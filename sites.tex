...
$$
{}_pu\mathcal{F}(V) =
\text{Mor}_{\textit{PSh}(\mathcal{D})}(h_V, {}_pu\mathcal{F}) =
\text{Mor}_{\textit{PSh}(\mathcal{C})}(u^ph_V, \mathcal{F}) =
\text{Mor}_{\textit{Sh}(\mathcal{C})}((u^ph_V)^\#, \mathcal{F})
$$
...
and cocontinuous functor of sites which induces a morphism of topi
...
$\varphi : * \to g^{-1}\mathcal{G}$ on $\mathcal{C}/\mathcal{F}$
