...
of $\mathcal{J}_V$ there exists a unique section
...
\medskip\noindent
So let us pick a $(U, \psi) \in \text{Ob}(\mathcal{J}_V)$.
...
Details left to the reader. FIXME.
\end{proof}
...
necessary for the definition to work\footnote{We will always assume that
there is at least one neighbourhood of $p$ to avoid the case
where $u(U)=\emptyset$ for all $U$.}. On the other hand,
...
$u_p$ had already been defined which commutes with $s$, then
we would have the following identities
$$
u_pF(sF(s\mathcal{F})) =
F(u_psF(s\mathcal{F})) =
F(su_pF(s\mathcal{F})) =
F(sF(u_ps\mathcal{F})).
$$
Let construct two morphisms
$F(sF(u_ps\mathcal{F})) \to F(u_ps\mathcal{F})$.
The first is the map $c_{F(u_ps\mathcal{F})}$.
For the second, since $u_p$ is a functor, we can
consider $u_psc_{\mathcal{F}} : u_psF(s\mathcal{F}) \to u_ps\mathcal{F}$.
