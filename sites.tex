...
\noindent
The following lemma provides another way to prove a continuous functor
...
\begin{proof}
Let $V$ be an object of $\mathcal{D}$. We have $u^ph_V = h_{v(V)}$
because $u^ph_V(U) = \Mor_\mathcal{D}(u(U), V) = \Mor_\mathcal{C}(U, v(V))$
by assumption. Hence for any presheaf $\mathcal{F}$ on $\mathcal{C}$ we have
\begin{eqnarray*}
v^p\mathcal{F}(V)
& = &
\mathcal{F}(v(V)) \\
