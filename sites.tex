...
which exists, see Remark \ref{remark-explain-left-exact}.
Moreover, $u$ commutes with limit (see Remark \ref{remark-explain-left-exact})
...
\noindent
Here is a definition of a topos which is suitable
for our purposes. In fact, it is more important to
know what a morphism of topoi is.
...
the pair of functors $(f^{-1} = u_s, f_* = u^s)$ is a morphism of topi.
\end{lemma}
...
by a morphism $\alpha : U \to U'$ such that $\psi = \psi' \circ \alpha$.
...
In fact an element $s$ of ${}_pu(\mathcal{F})(V)$ is nothing
but a compatible system of elements
$c(\psi)(s) = s_{(U,\psi)} \in \mathcal{F}(U)$
for $(U, \psi)$ ranging over ${}_V\mathcal{I}$.
...
Let $\mathcal{C}$ and $\mathcal{D}$ be sites.
A functor $u : \mathcal{C} \to \mathcal{D}$ is called
{\it cocontinuous} if for every $U \in \text{Ob}(\mathcal{C})$
and every covering
$\{V_j \to u(U)\}_{j \in J}$ there exists a
covering
...
To do this it suffices to show that for any object $(U, \psi)$
...
\medskip\noindent
So let us pick a $(U, \psi) \in \text{Ob}({}_V\mathcal{I})$.
...
as well. Hence the family of elements
$\sigma_i = c(\psi_i)(s_{\gamma(i)}) \in \mathcal{F}(U_i)$
form a system of elements such that $\sigma_i$ and $\sigma_{i'}$ agree on 
...
for $\mathcal{F}$. This is the desired element $s_{(U, \psi)}$.
Details left to the reader.

\medskip\noindent
A bit more clarification could be added here. FIXME.
\end{proof}
...
Moreover, the inclusion $i : \textit{Sh}(\mathcal{D})
\to \textit{PSh}(\mathcal{D})$ is left exact by
Lemma \ref{lemma-limit-sheaf}. Finally, the functor
$u^p$ is left exact because it is a rght adjoint
...
\noindent
It is clear from the above that a cocontinuous functor
gives a morphism of topoi. It is in the opposite direction
from the morphism of topoi that we get out of a 
left exact continous functor
(as in Proposition \ref{proposition-get-morphism}).
...
$\mathcal{F}$ with values in $\mathcal{A}$ is a sheaf if and
only if the associated presheaf of sets is a sheaf. Moreover,
we worked out the notion of stalks, psuhforward, pullback,
adjointness of pullback and pushforward. In addition extending
...
Let $\mathcal{C}$ be a site.
Let us denote $\textit{PSh}(\mathcal{C}, \mathcal{A})$,
resp.\ $\textit{Sh}(\mathcal{C}, \mathcal{A})$ the category
of presheaves, resp.\ sheaves with values in $\mathcal{A}$ on $\mathcal{C}$.
It is still true that a presheaf with values in $\mathcal{A}$ is
a sheaf if and only if its underlying presheaf of sets is a sheaf.
The proof is the same as the proof of Sheaves,
Lemma \ref{sheaves-lemma-sheaves-structure}.
...
$$
Moreover, the functor ${}^\#$ is adjoint to the inclusion
functor. The reason is that we have shown the colimits in sheafification
are directed.
...
We define the {\it pushforward} functor
$f_* = u^p$ for (pre)sheaves of algebraic structures
and in fact for (pre)sheaves with value in any category
...
\noindent
We briefly discuss two methods for dealing with pullback and
pushforward for a general morphism of sites.
...
Here
\begin{enumerate}
\item $\mathcal{F}$ is a sheaf of sets,
\item $+ : \mathcal{F} \times \mathcal{F} \to \mathcal{F}$ is
a morphism of sheaves of sets,
\item $0 : * \to \mathcal{F}$ is a morphism from the
singleton sheaf (see Example \ref{example-singleton-sheaf}
to $\mathcal{F}$, and
\item $i : \mathcal{F} \to \mathcal{F}$ is a morphism of sheaves
of sets.
\end{enumerate}
These data have to satisfy the following conditions
\begin{enumerate}
\item $+$ is associative and commutative,
\item $0$ is a unit for $+$, and
\item $+ \circ (1, i) = 0 \circ (\mathcal{F} \to *)$.
\end{enumerate}
...
\medskip\noindent
This method also works for (pre)sheaves of rings by thinking
...
algebraic structures that one encounters in practice. However,
we present another method, just because we can and because it
leads us to introduce the ``free sheaf of algebraic structures
on a sheaf of sets'', which is an important notion anyways.

\begin{remark}
\label{remark-no-pullback-presheaves}
Note that we do not define a pullback functor for presheaves
of abelian groups, rings, etc.
\end{remark}


\noindent
{\bf Second method.}
We add axioms that say there are free algebraic structures
on sets, in order to define pullbacks using them.

\begin{definition}
\label{definition-adequate-algebraic-structure}
We say that an algebraic structure $(\mathcal{A}, s)$
as above is {\it adequate} if
\begin{enumerate}
\item $u$ reflects epimorphisms,
\item $u$ has a left adjoint $F$, and
\item $\mathcal{C}$ has coequalizers.
\end{enumerate}
\end{definition}
...
$$
\text{Map}(X, sA) = \text{Mor}_\mathcal{A}(F(X), A)
$$
bifunctorially.
We say that $F(X)$ is the {\it free algebraic structure on the
set $X$}. This will allows us to define the {\it free presheaf
of algebraic structures on a presheaf of sets} and the
{\it free sheaf of algebraic structures on a sheaf of sets}.
See Lemmas \ref{lemma-free-presheaf-on} and \ref{lemma-free-sheaf-on} below.

\begin{lemma}
\label{lemma-sF-Fs}
Let $(\mathcal{A}, s)$ be an adequate type of algebraic structure.
Let $A \in \text{Ob}(\mathcal{A})$, and let $X$ be a set.
Consider the mappings $c_A : F(sA) \to A$ and $i_X : X \to sF(X)$ which
provide the adjointness of $F$ and $s$. 
\begin{enumerate}
\item The morphism $c_A$ is an epimorphism.
\item The map $i_X$ is injective.
\item The morphism $c_A$ is the coequalizer of the
morphisms $F(sc_A), c_{F(sA)} : F(sF(sA)) \to F(sA)$.
\end{enumerate}
In other words,
$$
\xymatrix{
F(sF(sA)) \ar@<1ex>[r]^{F(sc_A)} \ar@<-1ex>[r]_{c_{F(sA)}} &
F(sA) \ar[r]^{c_A} &
A
}
$$
...
\begin{proof}
We will use the properties of the pair $(\mathcal{A}, s)$ listed in
Sheaves, Lemma \ref{sheaves-lemma-properties-algebraic-structures}
without further mention.
To prove (1) note that the composition
$$
\xymatrix{
sA \ar[r]^-{i_{sA}} &
sF(s(A)) \ar[r]^-{sc_A} &
...
$$
is the identity. Similarly, to prove (2) use that
$$
\xymatrix{
F(X) \ar[r]^-{F(i_X)} &
F(sF(X)) \ar[r]^-{c_{F(X)}} &
...
\medskip\noindent
Next we prove (3).
Let us temporarily use the notation that $i_X(x) = [x]$.
First we claim that after applying $s$ the diagram in the lemma is
a coequalizer diagram. Let $e \in sF(sA)$. This maps to some
element $a \in sA$ under $sc_A$, i.e., we have $a = sc_A(e)$. 
By the above also $sc_A([a]) = a$. Then consider
$[e] \in sF(sF(sA))$. We have $sF(sc_A)([e]) = [a]$ and
$sc_{F(sA)}([e]) = e$. Thus we see that any element $e \in sF(sA)$
maps to the same element as $[sc_A(e)]$ in the coequalizer
of the diagram of sets. Clearly this implies the claim.

\medskip\noindent
Let $F(sA) \to B$ be the coequalizer in $\mathcal{A}$.
Let $B \to A$ be the canonical map. Since $sF(sA) \to sB$
is surjective by our conditions on $(\mathcal{A}, s)$ we
see that $sB \to sA$ is an isomorphism. Hence $B \to A$
is an isomorphism.
\end{proof}

\begin{lemma}
\label{lemma-free-presheaf-on}
Let $\mathcal{C}$ be a category.
Assume that $(\mathcal{A}, s)$ is an adequate type of algebraic
structure. Then the functor
$s : \textit{PSh}(\mathcal{C}, \mathcal{A}) 
\to \textit{PSh}(\mathcal{C})$
has a left adjoint which we will denote $F$ as well.
\end{lemma}

\begin{proof}
Let $\mathcal{F}$ be a presheaf of sets.
Denote $F(\mathcal{F})$ the presheaf with values in $\mathcal{A}$
whose value on $U$ is the object $F(\mathcal{F}(U))$ with
obvious restriction mappings. A morphism from $F(\mathcal{F})$
into a presheaf $\mathcal{G}$ with values in $\mathcal{A}$
is the same a morphism of $\mathcal{F}$ into $s(\mathcal{G})$
because of the adjointness of $s$ and $F$.
\end{proof}

\begin{lemma}
\label{lemma-coequalizer-presheaf}
Assumptions as in Lemma \ref{lemma-free-presheaf-on} above.
Notation $c_{\mathcal{F}} : F(s\mathcal{F}) \to \mathcal{F}$
as in Lemma \ref{lemma-sF-Fs} above.
For every presheaf $\mathcal{F}$ with values in $\mathcal{A}$ the
diagram
$$
\xymatrix{
F(sF(s\mathcal{F}))
\ar@<1ex>[r]^{F(sc_\mathcal{F})}
\ar@<-1ex>[r]_{c_{F(s\mathcal{F})}} &
F(s\mathcal{F})
\ar[r]^{c_\mathcal{F}} &
\mathcal{F}
}
...
\begin{proof}
This is true because it is true on the level of sections over $U$
for every object $U$ of $\mathcal{C}$.
\end{proof}

\begin{lemma}
\label{lemma-free-sheaf-on}
Let $\mathcal{C}$ be a category.
Assume that $(\mathcal{A}, s)$ is an adequate type of algebraic
structure. Then the functor
$s : \textit{Sh}(\mathcal{C}, \mathcal{A}) 
\to \textit{Sh}(\mathcal{C})$
has a left adjoint, namely $\mathcal{F} \mapsto F(\mathcal{F})^\#$.
\end{lemma}
...
\begin{lemma}
\label{lemma-coequalizer-sheaf}
Assumptions as in Lemma \ref{lemma-free-sheaf-on} above.
Notation $c_{\mathcal{F}} : F(s\mathcal{F})^\# \to \mathcal{F}$
as in Lemma \ref{lemma-sF-Fs} above.
For every sheaf $\mathcal{F}$ with values in $\mathcal{A}$ the
diagram
$$
\xymatrix{
F(sF(s\mathcal{F})^\#)^\#
...
$$
is a coequalizer diagram. (Since $s$ commutes with sheafification
the ambiguity in the notation is harmless.)
\end{lemma}

\begin{proof}
By Lemma \ref{lemma-coequalizer-presheaf} the top row
...
$$
is a coequalizer diagram in the category of presheaves with
values in $\mathcal{A}$. The lemma follows trivially.
\end{proof}

\noindent
The following lemma says that the pullback of the
free sheaf of algebraic structures on a sheaf of sets
is (or rather, should be)
the free sheaf of algebraic structures on the pullback
of the sheaf of sets.

\begin{lemma}
\label{lemma-pullback-free-presheaf}
Let $f : \mathcal{D} \to \mathcal{C}$ be a morphism of sites
given by a functor $u : \mathcal{C} \to \mathcal{D}$.
\begin{enumerate}
\item Let $\mathcal{F}$ be a presheaf of sets on $\mathcal{C}$.
Then
$$
\text{Mor}_{\textit{PSh}(\mathcal{D}, \mathcal{A})}
(F(u_p\mathcal{F}), \mathcal{G})
=
\text{Mor}_{\textit{PSh}(\mathcal{C}, \mathcal{A})}
(F(\mathcal{F}), u^p\mathcal{G})
$$
\item Let $\mathcal{F}$ be a sheaf of sets on $\mathcal{C}$.
Then
$$
\text{Mor}_{\textit{Sh}(\mathcal{D}, \mathcal{A})}
(F(f^{-1}\mathcal{F})^\#, \mathcal{G})
=
\text{Mor}_{\textit{Sh}(\mathcal{C}, \mathcal{A})}
(F(\mathcal{F}), f_*\mathcal{G})
$$
\end{enumerate}
\end{lemma}

\begin{proof}
This follows by using adjointness of $F$, $s$,
of sheafification and $\textit{Sh} \to \textit{PSh}$
and of $u_p$ and $u^p$.
\end{proof}

\noindent
At this point we have enough theory to define the pullback
functor. Namely, suppose that $f : \mathcal{D} \to \mathcal{C}$
is a morphism of sites corresponding to the functor
$u : \mathcal{C} \to \mathcal{D}$. Note that the functor
$u^p$ commutes with both the functors $F$ and $s$. Hence
\begin{eqnarray*}
\text{Mor}_{\textit{PSh}(\mathcal{D}, \mathcal{A})}
(F(u_psF(s\mathcal{F})), F(u_ps\mathcal{F}))
& = &
\text{Mor}_{\textit{PSh}(\mathcal{C}, \mathcal{A})}
(F(sF(s\mathcal{F})), u^pF(u_ps\mathcal{F})) \\
& = &
\text{Mor}_{\textit{PSh}(\mathcal{C}, \mathcal{A})}
(F(sF(s\mathcal{F})), F(u^pu_ps\mathcal{F})) \\
& \leftarrow &
\text{Mor}_{\textit{PSh}(\mathcal{C}, \mathcal{A})}
(F(sF(s\mathcal{F})), F(s\mathcal{F}))
\end{eqnarray*}
Thus we see that the morphisms $c_{F(s\mathcal{F})}$ and
$F(sc_{\mathcal{F}})$ give rise to a pair of morphisms
$$
\alpha, \beta :
F(u_psF(s\mathcal{F}))
\longrightarrow
F(u_ps\mathcal{F})
$$
Denote $u_p\mathcal{F} \in \textit{PSh}(\mathcal{D}, \mathcal{A})$
the coequalizer of this pair of morphisms. Clearly this
is functorial in $\mathcal{F}$.

\begin{lemma}
\label{lemma-adjointness}
With notation as above, the functor $u_p$ is a
left adjoint to $u^p$.
\end{lemma}

\begin{proof}
Let $\mathcal{G}$ be a presheaf on $\mathcal{D}$ with values in $\mathcal{A}$.
Let $\mathcal{F}$ be a presheaf on $\mathcal{C}$ with values in $\mathcal{A}$.
Suppose that $\mathcal{F} \to u^p\mathcal{G}$ is a morphism.
FIXME.
\end{proof}

\noindent
Etc, etc, etc,... More to come. FIXME.
