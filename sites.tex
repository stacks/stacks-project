...
\noindent
The following lemma can no doubt be strengthened.

\begin{lemma}
\label{lemma-morphism-sites-localization}
Let $f : \mathcal{C} \to \mathcal{D}$ be a morphism of sites
given by a functor $u : \mathcal{D} \to \mathcal{C}$ satisfying the
assumptions of Proposition \ref{proposition-get-morphism}.
Assume also that $\mathcal{C}$ has fibre products.
Let $V \in \text{Ob}(\mathcal{D})$. Set $U = u(V)$. There exists
a commutative diagram of morphisms of sites
$$
\xymatrix{
...
$$
where the left vertical arrow corresponds to the induced
functor $u' : \mathcal{D}/V \to \mathcal{C}/U$, $V'/V \mapsto u(V')/u(V)$.
\end{lemma}

\begin{proof}
It is clear that $u$ induces a functor $u'$ as indicated.
It is clear that this functor is continuous, and that
it also satisfies the assumptions of
Proposition \ref{proposition-get-morphism}.
Hence we get a morphism of sites $\mathcal{C}/U \to \mathcal{D}/V$.
The diagram commutes because we have
$f_* \circ j_{U*} = j_{V*} \circ f'_*$.
Namely, we may use Lemma \ref{lemma-localization-given-products}
upstairs and downstairs (because we have final objects and fibre product
so also products) and hence for a sheaf $\mathcal{F}$
on $\mathcal{C}/U$ and an object $Y$ of $\mathcal{D}$ we have
\begin{eqnarray*}
f_*(j_{U*}(\mathcal{F}))(Y) & = j_{U*}(\mathcal{F})(u(Y)) \\
& = \mathcal{F}(u(Y) \times U) \\
& = \mathcal{F}(u'(Y \times V)) \\
& = f'_*(\mathcal{F})(Y \times V) \\
& = j_{V*}(f'_*\mathcal{F}))(Y)
\end{eqnarray*}
as desired.
\end{proof}
...
$$
and the left vertical arrow is a special cocontinuous functor.
In particular we get a commutative diagram of topoi
$$
\xymatrix{
...
\begin{proof}
The commutativity of the diagram is clear. We have to check
...
\medskip\noindent
Property (1). Let $U'/U$ be an object of $\mathcal{C}/U$.
Let $\{V_j/u(U) \to u(U')/u(U)\}_{j \in J}$ be a covering of $u(U')/u(U)$
in $\mathcal{D}/u(U)$. Since $u$ is cocontinuous there exists a
...
$\{u(U_i')/u(U) \to u(U')/u(U)\}$ refines $\{V_j/u(U) \to u(U')/u(U)\}$
(use the same $\alpha$ and the same maps $\phi_i$). Hence (1) holds
