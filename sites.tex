...
Let $\mathcal{C}$ and $\mathcal{D}$ be sites.
Let $u : \mathcal{C} \to \mathcal{D}$ be a functor.
The functor $u$ is called {\it cocontinuous\footnote{The definition
of cocontinuous in \cite[Expos\'e III, Definition 2.1]{SGA4} translated
into the language of sites seems to only require (1). But it seems that
we really need (2) as well.}}
if for every $U \in \text{Ob}(\mathcal{C})$
and every covering $\{V_j \to u(U)\}_{j \in J}$
we have
\begin{enumerate}
\item there exists a covering
$\{U_i \to U\}_{i\in I} \in \text{Cov}(\mathcal{C})$
such that the family of maps $\{u(U_i) \to u(U)\}_{i \in I}$
refines the covering $\{V_j \to u(U)\}_{j \in J}$, and
\item if $\{U_i \to U\}_{i \in I}$ is a family of maps
such that the family of maps $\{u(U_i) \to u(U)\}_{i \in I}$
is refined by the covering $\{V_j \to u(U)\}_{j \in J}$, then
$\{U_i \to U\}$ can be refined by a covering of $U$ in $\mathcal{C}$.
\end{enumerate}
\end{definition}
...
Using condition (1) of Definition \ref{definition-cocontinuous}
...
\end{eqnarray*}
Thus it is a left adjoint and hence left exact.
