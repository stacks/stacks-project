...
are representable, and for any $X\in \text{Ob}(\mathcal{C})$
the sequence
$$
\xymatrix{
...
$$
Here the arrow maps the component $h_U^\#$ corresponding to
$s$ to $\mathcal{F}$ such that $\text{id}_U \in h_U(U)$ maps
to the section $s$. This is an epimorphism
...
we saw in Remark \ref{remark-pt-topos} any set of sets can be turned
...
Let $\mathcal{C}$ be a site.
Let $p = u : \mathcal{C} \to \textit{Sets}$ be a functor as above.
...
\begin{lemma}
\label{lemma-stalk-presheaf-onto-stalk-sheaf}
Let $\mathcal{C}$ be a site.
Let $p = u : \mathcal{C} \to \textit{Sets}$
be a functor satisfying property (2) of Definition
\ref{definition-point}.
Let $\mathcal{F}$ be a presheaf of sets.
The canonical map $\mathcal{F}_p \to (\mathcal{F}^\#)_p$
is surjective.
\end{lemma}

\begin{proof}
For a presheaf $\mathcal{F}$, denote $\theta : \mathcal{F}
\to \mathcal{F}^+$ the canonical map. It suffices to show
that $\theta_p : \mathcal{F}_p \to (\mathcal{F}^+)_p$
is surjective for every presheaf $\mathcal{F}$.
Let $\sigma \in (\mathcal{F}^+)_p$.
Let $(U, x, s)$ denote a triple giving rise to
$\sigma$. Let $\{f_i : U_i \to U\}$
be a covering such that $s|_{U_i} = \theta(s_i)$ for
some $s_i \in \mathcal{F}(U_i)$, see Lemma \ref{lemma-plus-surjective}.
Pick an $i_0$ such that there exists a $y \in u(U_{i_0})$
which maps to $x \in u(U)$. Then the triple
$(U_{i_0}, y, s_{i_0})$ corresponds to an element
of $\mathcal{F}_p$ which maps to the
element $\sigma$.
\end{proof}

\begin{lemma}
\label{lemma-neighbourhoods-drected}
Let $\mathcal{C}$ be a site. Assume that $\mathcal{C}$ has
all finite limits. Let $p = u : \mathcal{C} \to \textit{Sets}$
be a functor which commutes with all finite limits.
Then the opposite of the category
of neighbourhoods of $p$ is directed.
\end{lemma}

\begin{proof}
This is shown in exactly the
...
\begin{lemma}
\label{lemma-stalk-presheaf-sheaf}
Assume $\mathcal{C}$ and $p$ as in Definition \ref{definition-point}.
Let $\mathcal{F}$ be a presheaf.
The canonical map $\mathcal{F} \to \mathcal{F}^\#$ induces
an isomorphism $\mathcal{F}_p \to (\mathcal{F}^\#)_p$ on stalks.
\end{lemma}

\begin{proof}
For a presheaf $\mathcal{F}$, denote $\theta : \mathcal{F}
\to \mathcal{F}^+$ the canonical map.
It suffices to show that $\theta_p : \mathcal{F}_p \to (\mathcal{F}^+)_p$
is an isomorphism for every presheaf $\mathcal{F}$.
In the proof of
Lemma \ref{lemma-stalk-presheaf-onto-stalk-sheaf}
we showed that it is surjective.

\medskip\noindent
We show that $\mathcal{F}_p \to (\mathcal{F}^+)_p$ is injective.
Suppose that $\sigma, \sigma' \in \mathcal{F}_p$ map to the
same element of $(\mathcal{F}^+)_p$. We may represent $\sigma$
by a triple $(U, x, s)$ and $\sigma'$ by a triple $(U', x', s')$.
By Lemma \ref{lemma-neighbourhoods-drected} we may assume
that $U = U'$ and $x = x'$. Similarly, after replacing $(U,x)$
by a ``smaller'' neighbourhood, we may assume that
$\theta(s) = \theta(s')$ in $\mathcal{F}^+(U)$. 
This means that there exists a covering $\{U_i \to U\}$
such that $s|_{U_i} = s'|_{U_i}$ for all $i$. By assumption
there exists an index $i_0$ and an element $x_{i_0} \in u(U_{i_0})$
mapping to $x$. Then we see that $(U, x, s)$ and $(U, x, s')$
define the same element in $\mathcal{F}_p$, namely they are
both equal to the equivalence class of $(U_{i_0}, x_{i_0}, s|_{U_{i_0}})$.
\end{proof}

\begin{lemma}
\label{lemma-points-exact}
Assume $\mathcal{C}$ and $p$ as in
Definition \ref{definition-point}.
The stalk functor $\textit{Sh}(\mathcal{C}) \to \textit{Sets}$,
$\mathcal{F} \to \mathcal{F}_p$ is exact.
In addition it commutes with all colimits.
\end{lemma}

\begin{proof}
Let $\mathcal{I} \to \textit{Sh}(\mathcal{C})$,
$i \mapsto \mathcal{F}_i$ be a diagram of sheaves.
We have to show that the stalk of the colimit of this
system agrees with the colimit of the stalks.
For limits we have to show this if $\mathcal{I}$ is finite.

\medskip\noindent
Let $\mathcal{F}$ be the colimit of the system as a {\it presheaf}.
According to Lemma \ref{lemma-colimit-sheaves} the colimit in
the category of sheaves is just the sheafification of $\mathcal{F}$.
Recall also that $\mathcal{F}$ has a simple description, see
Section \ref{section-limits-colimits-PSh}.
By Lemma \ref{lemma-stalk-presheaf-sheaf} it suffices to prove
that $\mathcal{F}_p = \text{colim}_{\mathcal{I}} \mathcal{F}_{i, p}$.
OK, and the expression on the right equals
$$
\text{colim}_{i}\ 
\text{colim}_{\{(U,x)\}^{opp}}\ 
\mathcal{F}_i(U)
$$
whereas the expression on the left equals
$$
\text{colim}_{\{(U,x)\}^{opp}}\ 
\text{colim}_{i}\ 
\mathcal{F}_i(U).
$$
Hence we see the result from Categories,
Lemma \ref{categories-lemma-colimits-commute}.

\medskip\noindent
Next, let $\mathcal{F}$ be the limit of the system as a {\it presheaf}.
Also, assume the diagram category $\mathcal{I}$ is finite.
...
Arguing as above we have to show that
$$
\text{lim}_{i}\ 
\text{colim}_{\{(U,x)\}^{opp}}\ 
\mathcal{F}_i(U)
...
\mathcal{F}_i(U).
$$
To prove this holds it
suffices to show that the opposite of the
category of neighbourhoods is directed
see Categories, Lemma
\ref{categories-lemma-directed-commutes}.
And this is shown in Lemma \ref{lemma-neighbourhoods-drected} above.
\end{proof}
...
is a functor such that property (2) of
Definition \ref{definition-point}
...
final object.) A {\it point $p$} of such a site $\mathcal{C}$
is a functor $u : \mathcal{C} \to \textit{Sets}$ such that
\begin{enumerate}
...
points in the case that $\mathcal{C}$ has finite limits. This we 
will show below. It follows by interpreting $u$ in terms of the
stalk functor in the general case. The proof of the equivalence
below will show that even in the general case the functor $u$
commutes with all finite limits that exist in $\mathcal{C}$.

\begin{definition}
\label{definition-morphism-points}
Let $\mathcal{C}$ be a site.
Let $p, p'$ be points of $\mathcal{C}$.
A morphism $p \to p'$ is 
\end{definition}
...
\begin{lemma}
\label{lemma-notion-points-agree}
Let $\mathcal{C}$ be a site whose underlying category has
all finite limits. Let $p = u : \mathcal{C} \to \textit{Sets}$
be a functor. Then $p$ is a point in the sense of
Definition \ref{definition-point} if and only if 
$p$ is a point in the sense of Definition \ref{definition-point-general}.
\end{lemma}

\begin{proof}
It is clear from Lemma \ref{lemma-points-exact} that a point
in the sense of Definition \ref{definition-point}
is a point in the sense of Definition \ref{definition-point-general}.
Conversely, suppose that $p$ is a point in the sense
of Definition \ref{definition-point-general}. Let
$\mathcal{I} \to \mathcal{C}$, $i \mapsto U_i$
be a finite diagram in $\mathcal{C}$. Let
$\text{lim}_i U_i \in \text{ob}(\mathcal{C})$ be the limit. We see that
$$
\text{lim}_i\ u(U_i) =
\text{lim}_i\ (h_{U_i})_p =
( \text{lim}_i\ h_{U_i} )_p =
(h_{\text{lim}_i\ U_i})_p =
u(\text{lim}_i\ U_i).
$$
Here we have used Lemma \ref{lemma-points-recover} multiple times.
The third equality is immediate from the definition of the representable
presheaves and the definition of a limit.
\end{proof}
...
(0') -- (3') above the after suitably enlarging
$\text{Cov}(\mathcal{C})$ (compare Sets,
Subsection \ref{sets-subsection-coverings-site}) we get a site.
