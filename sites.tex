...
\subsubsection{Injective and surjective maps of presheaves}
\label{subsubsection-injective-surjective}
...
\subsection{Limits and colimits in $\textit{PSh}$}
\label{subsection-limits-colimits-PSh}
...
of Categories, \autoref{categories-definition-limit} (resp.\ 
Categories, \autoref{categories-definition-colimit}). Finally, if
...
\subsection{Functoriality of $\textit{PSh}$}
\label{subsection-functoriality-PSh}
...
$u^p\mathcal{G} = \mathcal{G} \circ u$. Note that by the previous subsection
...
Categories, Subsubsection \ref{categories-subsubsection-limit-sets}.
\end{proof}
...
H^0(\mathcal{U}, \mathcal{F})
$$
See Categories, \autoref{categories-subsection-limits} for
...
\autoref{categories-subsubsection-limit-sets}
...
\begin{proposition}
\label{proposition-limit-sheaves}
Let $\mathcal{F} : \mathcal{I} \to \textit{Sh}(\mathcal{C})$
be a diagram. Then both $\lim_{\mathcal{I}} \mathcal{F}$ and
$\text{colim}_{\mathcal{I}} \mathcal{F}$ exist.
\end{proposition}
...
\subsection{Representable sheaves}
\label{subsection-representable-sheaves}
...
\label{subsection-continuous-functors}
...
\label{subsection-morphism-sites}
...
Subsection \ref{subsection-functoriality-PSh}
are directed (see
...
Hence it suffices to show that $u_s$ is left exact. 
By Categories, Lemma \ref{categories-lemma-characterize-left-exact}
it suffices to prove that $u_s$ commutes with products and
equalizers. Let $a,b : \mathcal{F} \to \mathcal{G}$ be two
morphisms between sheaves on $\mathcal{C}$. Let $\mathcal{H}
\to \mathcal{F}$ be the equalizer. According to the proof of
Proposition \ref{proposition-limit-sheaves} $\mathcal{H}$
is also the equalizer in the category of presheaves.
Clearly the map $u_s\mathcal{H} \to u_s\mathcal{F}$ maps
into the equalizer of the maps $u_sa$ and $u_sb$. 
