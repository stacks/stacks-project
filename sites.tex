...
$$
\text{Mor}_{\textit{PSh}(\mathcal{D})}(u_ph_U, \mathcal{G})
=
\text{Mor}_{\textit{PSh}(\mathcal{C})}(h_U, u^p\mathcal{G})
=
u^p\mathcal{G}(U) = 
\mathcal{G}(u(U))
$$
...
The functor $\textit{PSh}(\mathcal{C}) \to \textit{Sh}(\mathcal{C})$,
...
directed partially ordered set $\text{Ob}(\mathcal{J}_U)$ where
...
and for any $X\in \text{Ob}(\mathcal{C})$ the sequence
$$
\xymatrix{
...
sheaf. Since it is perhap better to avoid this terminology when the
...
or simply {\it $U$} for the {\it representable sheaf}
associated to the object $U$ of $\mathcal{C}$.
\end{definition}
...
Section \ref{section-functoriality-PSh} are filtered (see
Categories, Definition \ref{categories-definition-directed}).
\end{lemma}
...
Because the categories $\mathcal{I}_Y^{\text{opp}}$ are filtered
...
There are many sites that give rise to the topos $\textit{Sh}(pt)$.
For example, suppose that $S$ is a set (of sets) which contains at
least one nonempty element. Let $\mathcal{S}$ be the category whose objects
are elements of $S$ and whose morphisms are arbitrary set maps. Assume that
$\mathcal{S}$ has fibre products. For example this
...
We claim that $\textit{Sh}(\mathcal{S})$ is equivalent to the category of
sets. We first prove this in case $S$ contains $e \in S$ which is a singleton.
Namely, suppose that $\mathcal{F}$ is a sheaf on $\mathcal{S}$.
For any $U \in \text{Ob}(\mathcal{S}) = S$ we can find a covering
$\{\varphi_u : e \to U\}_{u \in U}$, where $\varphi_u$
maps the unique element of $e$ to $u \in U$. The sheaf condition
implies in this case that $\mathcal{F}(U) = \prod_{u \in U} \mathcal{F}(e)$.
In other words
$\mathcal{F}(U) = \text{Mor}_{\textit{Sets}}(U, \mathcal{F}(e))$.
Moreover, this rule is compatible with restriction mappings. Hence
the functor
$$
\textit{Sets} \longrightarrow \textit{Sh}(\mathcal{S}),\quad
E \longmapsto (U \mapsto \text{Mor}_{\textit{Sets}}(U, E))
$$
is an equivalence of categories. If $\mathcal{S}$ does not contain a
singleton, choose any nonempty $\tilde e \in S$ and a map
$\varphi : \tilde e \to \tilde e$ whose image is a singleton.
...
$$
and show that the arguments above continue to work. Details omitted.
\end{remark}
...
$$
f : \textit{Sh}(\mathcal{T}_G) \longrightarrow \textit{Sh}(\mathcal{T}_H)
$$
...
$$
\text{Mor}_{G\textit{-Sets}}(T_\varphi, S) =
\text{Mor}_{H\textit{-Sets}}(T, f_*S)
$$
...
have $h = g \circ f$
where $f : \textit{Sh}(\mathcal{C}) \to \textit{Sh}(\mathcal{D})$,
resp.\ $g : \textit{Sh}(\mathcal{D}) \to \textit{Sh}(\mathcal{E})$,
resp.\ $h : \textit{Sh}(\mathcal{C}) \to \textit{Sh}(\mathcal{E})$ is the
...
\item[(c)] fibre products exist in $\mathcal{C}$ and
$u$ commutes with them.
\end{enumerate}
In this case the functor $g_!$ above commutes with fibre products and
equalizers (and more generally with any finite, nonempty connected
limits).
\end{lemma}
...
$e_{\mathcal{C}} \in \text{Ob}(\mathcal{C})$,
$e_{\mathcal{D}} \in \text{Ob}(\mathcal{D})$ such that
$u(e_{\mathcal{C}}) = e_{\mathcal{D}}$.
\end{enumerate}
Let $g_!, g^{-1}, g_*$ be as above.
Then, setting $f_* = g^{-1}$, $f^{-1} = g_!$ gives a canonical morphism
of topoi $f : \textit{Sh}(\mathcal{D}) \to \textit{Sh}(\mathcal{C})$.
...
$j_U : \textit{Sh}(\mathcal{C}/U) \to \textit{Sh}(\mathcal{C})$
...
$\mathcal{G}$ garantees that the sections $s_i$ glue to a unique
...
$$
This is clearly the same thing as $\text{Mor}_{\mathcal{C}}(V, X)$.
...
more generally finite, nonempty, connected limits). In particular, if
...
Localization and presheaves.
Let $\mathcal{C}$ be a category.
Let $U$ be an object of $\mathcal{C}$.
Strictly speaking the functors $j_U^{-1}$, $j_{U*}$ and $j_{U!}$
have not been defined for presheaves.
...
\label{section-localization-morphisms}

\noindent
The following lemma is important in order to understand relation
between localizing and morphisms of sites and topoi.
...
$V'/V \mapsto u(V')/U$ corresponds to a morphism of sites
$f' : \mathcal{C}/U \to \mathcal{D}/V$, we get
...
$$
and we have $f'_*j_U^{-1} = j_V^{-1}f_*$. Moreover, if we identify
$\textit{Sh}(\mathcal{C}/U)$, resp.\ $\textit{Sh}(\mathcal{C}/V)$ with
$\textit{Sh}(\mathcal{C})/h_U^\#$, resp.\ $\textit{Sh}(\mathcal{C})/h_V^\#$
via
Lemma \ref{lemma-essential-image-j-shriek}
then the functor $(f')^{-1}$ is described by the rule
$$
...
$f^{-1} : \textit{Sh}(\mathcal{D})/h_V^\# \to \textit{Sh}(\mathcal{C})/h_U^\#$.
...
Lemma \ref{lemma-morphism-sites-localization}
above.
...
cocontinuous functor. We stress that this is a really different
kind of diagram, and in particular, in general the equality
$f'_*j_U^{-1} = j_V^{-1}f_*$ seen in
Lemma \ref{lemma-morphism-sites-localization}
...
such that $\mathcal{C}'$ is a site with a subcanonical topology
having fibre products and a final object
(in other words, $\mathcal{C}'$ has all finite limits).
Moreover, given a set of sheaves $\{\mathcal{F}_i\}_{i \in I}$ we
may choose $\mathcal{C}'$ such that each $g_*\mathcal{F}_i$
is a representable sheaf.
\end{lemma}

\begin{proof}
Consider the full subcategory
$\mathcal{C}_1 \subset \text{Sh}(\mathcal{C})$ consisting of all
$h_U^\#$ for all $U \in \text{Ob}(\mathcal{C})$, the given sheaves
...
$\textit{Sh}(\mathcal{C}) \to \textit{Sh}(\mathcal{D}')$
we obtain the following statement:
Any morphism of topoi
$f : \textit{Sh}(\mathcal{C}) \to \textit{Sh}(\mathcal{D})$
fits into a commutative diagram
$$
\xymatrix{
\textit{Sh}(\mathcal{C}) \ar[d]_g \ar[r]_f &
\textit{Sh}(\mathcal{D}) \ar[d]^e \\
\textit{Sh}(\mathcal{C}') \ar[r]^{f'} &
\textit{Sh}(\mathcal{D}')
}
$$
...
the apparantly more general case of topoi. In reality this is not
...
Then the category $\textit{Sh}(\mathcal{C})/\mathcal{F}$
is a topos and there is a canonical morphism of topoi
$$
j_{\mathcal{F}} :
\textit{Sh}(\mathcal{C})/\mathcal{F}
\longrightarrow
\textit{Sh}(\mathcal{C})
$$
which is a localization as in
Section \ref{section-localization}
such that
\begin{enumerate}
...
for some $U \in \text{Ob}(\mathcal{C})$. By
Hence the material of
Section \ref{section-localization}
applies. In particular, there is an equivalence
$\textit{Sh}(\mathcal{C}/U) = \textit{Sh}(\mathcal{C})/h_U^\#$
such that the composition
$$
\textit{Sh}(\mathcal{C}/U)
\to
\textit{Sh}(\mathcal{C})/h_U^\#
\to \textit{Sh}(\mathcal{C})
$$
is equal to $j_{U!}$, see
Lemma \ref{lemma-essential-image-j-shriek}.
Denote
$a : \textit{Sh}(\mathcal{C})/h_U^\# \to \textit{Sh}(\mathcal{C}/U)$
the inverse functor. Define $j_{\mathcal{F}!} = j_{U!} \circ a$,
$j_{\mathcal{F}}^{-1} = j_U^{-1} \circ a$ and
...
\begin{remark}
\label{remark-pushforward-localization}
In the situation of
...
of $\textit{Hom}$-sheaves is desirable, hence we postpone this to
a later time.
\end{remark}

\begin{definition}
\label{definition-localization-topos}
Let $\mathcal{C}$ be a site.
Let $\mathcal{F}$ be a sheaf on $\mathcal{C}$.
\begin{enumerate}
\item The topos $\textit{Sh}(\mathcal{C})/\mathcal{F}$
is called the
{\it localization of the topos $\textit{Sh}(\mathcal{C})$ at $\mathcal{F}$}.
\item The morphism of topoi
$j_{\mathcal{F}} :
\textit{Sh}(\mathcal{C})/\mathcal{F}
\to
\textit{Sh}(\mathcal{C})$ of
Lemma \ref{lemma-localize-topos}
is called the {\it localization morphism}.
\end{enumerate}
\end{definition}
...
\noindent
Perhaps superfluously we are going to show that whenever the sheaf
$\mathcal{F}$ is equal to $h_U^\#$ for some object $U$ of the site,
then the localization of the topos is equal to the category of sheaves
on the localization of the site at $U$. Moreover, we are going to check
that any functorialities are compatible with this identification.

\begin{lemma}
\label{lemma-localize-topos-properties}
Let $\mathcal{C}$ be a site.
\begin{enumerate}
\item If $\mathcal{F} = h_U^\#$ for some object $U$ of $\mathcal{C}$, then
$j_{\mathcal{F}} : \textit{Sh}(\mathcal{C})/\mathcal{F}
\to \textit{Sh}(\mathcal{C})$
constructed in
Lemma \ref{lemma-localize-topos}
...
of
Lemma \ref{lemma-essential-image-j-shriek}.
\item If $s : \mathcal{G} \to \mathcal{F}$ is a morphism of sheaves
on $\mathcal{C}$ then there exists a natural commutative diagram of
morphisms of topoi
$$
\xymatrix{
\textit{Sh}(\mathcal{C})/\mathcal{G} \ar[rd]_{j_\mathcal{G}} \ar[rr]_j & &
\textit{Sh}(\mathcal{C})/\mathcal{F} \ar[ld]^{j_\mathcal{F}} \\
& \textit{Sh}(\mathcal{C}) &
}
$$
where $j = j_{\mathcal{G}/\mathcal{F}}$ is the localization of the
topos $\textit{Sh}(\mathcal{C})/\mathcal{F}$ at the object
$\mathcal{G}/\mathcal{F}$.
\item If $\mathcal{G} = h_V^\#$ and $\mathcal{F} = h_U^\#$ and
$s : \mathcal{G} \to \mathcal{F}$ comes from a morphism $V \to U$
of $\mathcal{C}$ then the diagram in (2) is identified with the
diagram in
...
\begin{proof}
Assertion (1) follows as we have seen in
Lemma \ref{lemma-essential-image-j-shriek}
that the composition
$\textit{Sh}(\mathcal{C}/U) \to \textit{Sh}(\mathcal{C})/h_U^\#
\to \textit{Sh}(\mathcal{C})$
is $j_{U!}$ and similarly the functor
$\textit{Sh}(\mathcal{C})/h_U^\# \to \textit{Sh}(\mathcal{C})$
is $j_{\mathcal{F}!}$ by
Lemma \ref{lemma-localize-topos}.
Hence $j_{\mathcal{F}!} = j_{U!}$ via the identification, and
so $j_{\mathcal{F}}^{-1} = j_U^{-1}$ (by adjointness) and so
$j_{\mathcal{F}, *} = j_{U, *}$ (by adjointness again).

\medskip\noindent
Let $\mathcal{G} \to \mathcal{F}$ be as in (2). We have equality of functors
$j_{\mathcal{G}!} = j_{\mathcal{F}!} \circ j_!$. This is clear
from the description of these functors (as forgetful functors).
By adjointness again we also obtain the equalities
$j_{\mathcal{G}}^{-1} = j^{-1} \circ j_{\mathcal{F}}^{-1}$, and
$j_{\mathcal{G}, *} = j_{\mathcal{F}, *} \circ j_*$. This proves (2).

\medskip\noindent
To see (3) we have to show that the morphism of topoi associated to
the cocontinuous functor $j_{U/V}$ (see
Lemma \ref{lemma-relocalize})
equals the morphism of topoi $j$ of (2)
via the identifications
$\textit{Sh}(\mathcal{C}/V) = \textit{Sh}(\mathcal{C})/h_V^\#$
and
$\textit{Sh}(\mathcal{C}/U) = \textit{Sh}(\mathcal{C})/h_U^\#$
of
Lemma \ref{lemma-essential-image-j-shriek}.
Suppose that $\varphi : \mathcal{H} \to h_U^\#$ is an object of
$\textit{Sh}(\mathcal{C})/h_U^\#$. By the proof of
Lemma \ref{lemma-essential-image-j-shriek}
...
$$
on $\mathcal{C}/U$. The pullback $j_{V/U}^{-1}\mathcal{H}_\varphi$ to
...
$$
where $f : V \to U$ is the given morphism of $\mathcal{C}$.
On the other hand, the pullback $j^{-1}(\mathcal{H}/h_U^\#)$ is the object
$$
\xymatrix{
...
$$
of $\textit{Sh}(\mathcal{C})/h_V^\#$
by the description of $j^{-1} = j_{\mathcal{G}/\mathcal{F}}^{-1}$ in
Lemma \ref{lemma-localize-topos}.
Applying the procedure of the proof of
Lemma \ref{lemma-essential-image-j-shriek}
to $(\mathcal{H}', \varphi')$ we see that $\mathcal{H}'_{\varphi'}$
is given by exactly the same rule as the one describing
$j_{V/U}^{-1}\mathcal{H}_\varphi$ above.
In other words, we have shown that
$j_{U/V}^{-1}$ equals the functor $j^{-1}$ as desired.
\end{proof}

\begin{lemma}
\label{lemma-morphism-topoi-localization}
...
$$
\xymatrix{
\textit{Sh}(\mathcal{C})/\mathcal{F} \ar[r]_{j_{\mathcal{F}}} \ar[d]_{f'} &
\textit{Sh}(\mathcal{C}) \ar[d]^f \\
\textit{Sh}(\mathcal{D})/\mathcal{G} \ar[r]^{j_{\mathcal{G}}} &
\textit{Sh}(\mathcal{D}),
}
$$
and we have $f'_*j_{\mathcal{F}}^{-1} = j_{\mathcal{G}}^{-1}f_*$.
Moreover, $f'$ is characterized by the property that
$$
(f')^{-1}(\mathcal{H} \xrightarrow{\varphi} \mathcal{G})
=
(f^{-1}\mathcal{H} \xrightarrow{f^{-1}\varphi} \mathcal{F}).
...
sites we may change sites at will. Hence we may use the result of
Remark \ref{remark-morphism-topoi-comes-from-morphism-sites}
and assume that $f$ is given by a continuous functor
...
Lemma \ref{lemma-localize-topos-properties}
we may identify $j_{\mathcal{F}}$ and $j_U$
and $j_{\mathcal{G}}$ and $j_V$. Hence the assertions of the lemma
follow from the corresponding assertions of
Lemma \ref{lemma-morphism-sites-localization}.
\end{proof}

\begin{lemma}
\label{lemma-morphism-topoi-localization-properties}
...
Lemma \ref{lemma-morphism-topoi-localization}
...
U
\longmapsto
u^pE(U) = \text{Mor}_{\textit{Sets}}(u(U), E) = \text{Map}(u(U), E).
\end{equation}
...
definition of $u^pE$. The second is a restatement of the adjointness
of $u^p$ and the stalk functor (but now restricted to sheaves). The third
assertion follows as, for any set $E$, we have
$$
\text{Map}(\mathcal{F}_p, E) =
\text{Mor}_{\textit{PSh}(\mathcal{C})}(\mathcal{F}, u^pE) =
\text{Mor}_{\textit{Sh}(\mathcal{C})}(\mathcal{F}^\#, u^sE) =
\text{Map}(\mathcal{F}^\#_p, E)
$$
...
$$
\text{Mor}_{\textit{Sh}(\mathcal{C})}(\mathcal{F}, p_*E)
=
\text{Map}(\mathcal{F}_p, E)
$$
...
Let $X$ be a topological space. Let $\mathcal{T}_X$ be the site of
...
$\textit{Sh}(X)$? To see this, let $\mathcal{T}_X$ be the site of
Example \ref{example-site-topological}.
By
Lemma \ref{lemma-point-site-topos}
a point of $\textit{Sh}(X)$ corresponds to a point of this site.
Let $p$ be a point of the site $\mathcal{T}_X$ given by the functor
$u : \mathcal{T}_X \to \textit{Sets}$. We are going to use the
...
the site $\mathcal{T}_X$ are in one-to-one correspondence with
irreducible closed subsets of $X$. In particular, if $X$ is
a sober topological space, then points of $\mathcal{T}_X$ and
...
\label{section-localization-points}
...
Before we do this we prove a helper lemma.
...
$u(U) \subset S_0$ for all $U \in \text{Ob}(\mathcal{C})$. Let $\mathcal{S}$
be the site constructed out of the powerset $S = \mathcal{P}(S_0)$ in
Remark \ref{remark-pt-topos}.
Then
\begin{enumerate}
\item there is an equivalence $\textit{Sh}(pt) \to \textit{Sh}(\mathcal{S})$,
\item $u : \mathcal{C} \to \mathcal{S}$ is a continuous functor which induces
a morphism of sites
$f : \textit{Sh}(\mathcal{S}) \to \textit{Sh}(\mathcal{C})$, and
\item the composition
$$
\textit{Sh}(pt) \to
\textit{Sh}(\mathcal{S}) \to
\textit{Sh}(\mathcal{C})
$$
is the morphism of topoi $(p_*, p^{-1})$, see
Lemma \ref{lemma-point-site-topos}.
\end{enumerate}
\end{lemma}
...
the sheaf $\mathcal{F}_E$ on $\mathcal{S}$ defined by the rule
$V \mapsto \text{Mor}_{\textit{Sets}}(V, E)$.
Part (2) is clear from the definition of a point of $\mathcal{C}$
(Definition \ref{definition-point})
and the definition of a morphism of sites
(Definition \ref{definition-morphism-sites}).
Finally, consider $f_*\mathcal{F}_E$. By construction we have
$$
f_*\mathcal{F}_E = \mathcal{F}_E(u(U)) = \text{Mor}_{\textit{Sets}}(u(U), E)
$$
...
$$
\xymatrix{
& \textit{Sh}(pt) \ar[d]^p \ar[ld]_q \\
\textit{Sh}(\mathcal{C}/U) \ar[r]^{j_U} &
\textit{Sh}(\mathcal{C})
}
$$
...
Lemma \ref{lemma-site-point-morphism}.
We may identify $\textit{Sh}(pt) = \textit{Sh}(\mathcal{S})$
as in that lemma, and we may write
$p = f : \textit{Sh}(\mathcal{S}) \to \textit{Sh}(\mathcal{C})$
...
$$
\xymatrix{
\textit{Sh}(\mathcal{S}/u(U)) \ar[r]_-{j_{u(U)}} \ar[d]_{p'} &
\textit{Sh}(\mathcal{S}) \ar[d]^p \\
\textit{Sh}(\mathcal{C}/U) \ar[r]^{j_U} &
\textit{Sh}(\mathcal{C}),
}
$$
...
Note that $j_U \circ j_x = \text{id}_{\mathcal{S}}$, and
$v_x \circ u' = v$.
These observations imply that we have the following commutative
diagram of topoi
$$
\xymatrix{
\textit{Sh}(\mathcal{S}) \ar[rd]^a \ar[rdd]_q
\ar `r[rrr] `d[dd]^p [rrdd] & & & \\
&\textit{Sh}(\mathcal{S}/u(U)) \ar[r]_-{j_{u(U)}} \ar[d]^{p'} &
\textit{Sh}(\mathcal{S}) \ar[d]^p & \\
&\textit{Sh}(\mathcal{C}/U) \ar[r]^{j_U} &
\textit{Sh}(\mathcal{C}) &
}
...
\begin{enumerate}
\item The morphism
$a : \textit{Sh}(\mathcal{S}) \to \textit{Sh}(\mathcal{S}/u(U))$
is the morphism of topoi assoicated to the cocontinuous functor
...
\item The compostion $p' \circ a$ gives a morphism of topoi.
...
\vcenter{
\xymatrix{
\text{Mor}_{\mathcal{C}/U}(W/U, V/U) \ar[r] \ar[d] &
\text{Mor}_{\mathcal{C}}(W, V) \ar[d] \\
\text{Mor}_{\mathcal{C}/U}(W/U, U/U) \ar[r] &
\text{Mor}_{\mathcal{C}}(W, U)
}
}
\quad
\vcenter{
...
(j_{U!}\mathcal{G})_p =
\coprod\nolimits_{x \in u(U)}
\text{colim}_{(\varphi : V \to U, y),\ u(\varphi)(y) = x}
\ \mathcal{G}(V/_\varphi U).
$$
...
Let $f, g : \textit{Sh}(\mathcal{C}) \to \textit{Sh}(\mathcal{D})$
...
\text{Mor}_{\textit{Sh}(\mathcal{C})}(\mathcal{G}, f_*\mathcal{F})
\ar[d]_{t \circ -} \ar@{=}[r] &
\text{Mor}_{\textit{Sh}(\mathcal{C})}(f^{-1}\mathcal{G}, \mathcal{F})
\ar[d]^{- \circ t}
\\
\text{Mor}_{\textit{Sh}(\mathcal{C})}(\mathcal{G}, g_*\mathcal{F})
\ar@{=}[r] &
\text{Mor}_{\textit{Sh}(\mathcal{C})}(g^{-1}\mathcal{G}, \mathcal{F})
}
$$
...
Let $*$ denote the final object of the category of sheaves, see
Example \ref{example-singleton-sheaf}.
Consider the sheaf $\mathcal{H} = \mathcal{G} \coprod_{\mathcal{F}} *$.
The map $\mathcal{F} \to \mathcal{G}$ is surjective if and only
if the map $* \to \mathcal{H}$ is an isomorphism. By construction
all the maps on stalks $*_{p_i} = \{*\} \to \mathcal{H}_{p_i}$
are bijective. If $\phi$ is not surjective,
then there exists a $U$ and a section $s \in \mathcal{H}(U)$
which is not equal to the section $*$. By assumption we see there
exists an index $i$ and $x\in u_i(U)$ such that $(U, x, s)$ and
$(U, x, *)$ define distinct points of $\mathcal{H}_{p_i}$.
...
\begin{lemma}
\label{lemma-localization-enough}
...
$j_{U!} : \textit{Sh}(\mathcal{C}/U) \to \textit{Sh}(\mathcal{C})/h_U^\#$, see
Lemma \ref{lemma-essential-image-j-shriek}.
...
all $\mathcal{F}$ in $\textit{Sh}(\mathcal{C})$,
...
\item[(e)] (6) $\Rightarrow$ (4),
\item[(f)] (4) $\Leftrightarrow$ (11), and
\item[(g)] (8) $+$ (9) $\Rightarrow$ (10).
\end{enumerate}
\end{lemma}
...
Thus we see that (7) implies (8), (9), (10). It is clear that (7) implies
(3) and hence (1). Finally, if $\beta : f_*\mathcal{F} \to f_*\mathcal{F}'$
is a map of sheaves, then
$\alpha = f^{-1}\beta : \mathcal{F} = f^{-1}f_*\mathcal{F} \to
f^{-1}f_*\mathcal{F}' = \mathcal{F}'$ is a map of sheaves on $\mathcal{C}$.
Chasing diagrams we see that the following diagram
$$
\xymatrix{
f_*\mathcal{F} \ar[r]^{f_*\alpha} & f_*\mathcal{F}' \\
f_*f^{-1}f_*\mathcal{F} \ar[r]^{f_*f^{-1}\beta} \ar[u] &
f_*f^{-1}f_*\mathcal{F}' \ar[u] \\
f_*\mathcal{F} \ar[u] \ar[r]^\beta & f_*\mathcal{F}' \ar[u]
}
$$
is commutative, in other words $f_*\alpha = \beta$.
...
\medskip\noindent
Proof of (e). If $\mathcal{F} \to \mathcal{F}'$ is surjective then the map
$\mathcal{F}' \amalg_{\mathcal{F}} \mathcal{F}' \to \mathcal{F}'$
is injective. Hence (6) implies that
$f_*\mathcal{F}' \amalg_{f_*\mathcal{F}} f_*\mathcal{F}' \to f_*\mathcal{F}'$
is injective also. And this in turn implies that
$f_*\mathcal{F} \to f_*\mathcal{F}'$ is surjective.
Hence we see that (6) implies (4).
...
\noindent
Here is a condition on a morphism of sites which garantees that
...
by the functor $u : \mathcal{C} \to \mathcal{D}$ be a functor.
...
$f_* : \textit{Sh}(\mathcal{D}) \to \textit{Sh}(\mathcal{C})$ reflects
...
\prod (u^s\mathcal{F})(u(U_i))$ by the sheaf condition
for some $U_i$ objects of $\mathcal{C}$ and similarly for $\mathcal{G}$.
...
of the category $\textit{Sh}(\mathcal{C})$ of sheaves on $\mathcal{C}$.
...
$\mathcal{F}(U) = \text{Mor}_{\textit{Sh}(\mathcal{C})}(h_U^\#, \mathcal{F})$.
...
Definition \ref{definition-site}
applied the the empty covering of $U$.
\end{proof}
...
$\mathcal{T}_X \to \mathcal{T}_Z$, $U \mapsto Z \cap U$ is
not cocontinuous. But it is almost continuous in the sense defined above.
We know that $i_*$ while not exact on sheaves of sets, is
exact on sheaves of abelian groups, see
Sheaves on Spaces, Remark \ref{sheaves-remark-i-star-not-exact}.
And this holds in general for continuous and almost continuous functors.

\begin{lemma}
\label{lemma-almost-cocontinuous-sheafification}
Let $\mathcal{C}$, $\mathcal{D}$ be sites.
Let $u : \mathcal{C} \to \mathcal{D}$ be a functor.
Assume that $u$ is continuous and almost continuous.
...
\begin{proof}
Let $U \in \text{Ob}(\mathcal{C})$. We have to show that
...
u^p(\mathcal{G}^+)(U) =
\mathcal{G}^+(u(U)) =
\text{colim}_{\mathcal{V}}\ \check H^0(\mathcal{V}, \mathcal{G})
$$
where the colimit is over the coverings $\mathcal{V}$ of $u(U)$ in
$\mathcal{D}$. On the other hand, we see that
$$
u^p(\mathcal{G})^+(U) =
\text{colim}_{\mathcal{U}}\ \check H^0(u(\mathcal{U}), \mathcal{G})
$$
...
Then $u^s = u^p : \textit{Sh}(\mathcal{D}) \to \textit{Sh}(\mathcal{C})$
commutes with pushouts and coequalizers (and more generally
finite, nonempty, connected colimits).
\end{lemma}

\begin{proof}
Let $\mathcal{I}$ be a finite, nonempty, connected index category.
Let $\mathcal{I} \to \textit{Sh}(\mathcal{D})$,
...
Denote $\text{colim}^{Psh}$ the colimit in the category
of presheaves. Since $\mathcal{I}$ is finite, nonempty and connected
we see that $\text{colim}^{Psh}_i\ \mathcal{G}_i$
is a presheaf satisfying the assumptions of
Lemma \ref{lemma-almost-cocontinuous-sheafification}
(because a finite nonempty connected colimit of singleton sets is a
singleton). Hence that lemma gives
\begin{align*}
u^s(\text{colim}_i\ \mathcal{G}_i) & =
u^s((\text{colim}^{Psh}_i\ \mathcal{G}_i)^\#) \\
& = (u^p(\text{colim}^{Psh}_i\ \mathcal{G}_i))^\# \\
& = (\text{colim}^{PSh}_i\ u^p(\mathcal{G}_i))^\# \\
& = \text{colim}_i\ u^s(\mathcal{G}_i)
\end{align*}
as desired.
\end{proof}
