...
$h : Y \to Y''$ such that $b = t^{-1}h$\ \ \ \ \footnote{Here is a
more down-to-earth way to see this:
% [DG] because I don't understand what it has to do with
%      Remark \ref{remark-left-localization-morphisms-colimit}
...
diagram commutes.
% However, we do know that $Q(f''g) = Q(hf)$ in
% $S^{-1}\mathcal{D}$ because the right square commutes,
% the outer square commutes in $S^{-1}\mathcal{D}$ by assumption, and
% because $Q(s), Q(t)$ are isomorphisms.
% [DG] I don't understand this :/ Instead here is what I do:
...
above and we win.
% [DG] I don't see how you get $d \in S$, nor why you want it.
\end{proof}
