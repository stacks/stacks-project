...
an unique $\varphi$ verifying $F(\varphi) = \phi'$, using that $F$ is fully
...
\begin{proof}
Let suppose that $f$ is a monomorphism. Let $W$ be an object of $\mathcal C$
...
X \ar[r]^{f} & Y }
$$
commutes and if $W\in\Ob(\mathcal C)$ and $\alpha,\beta : X\to Y$
such that $ f\circ\alpha = f\circ\beta $, we have a unique $\gamma$ verifying
$$
...
$\Mor_\mathcal{C}(W, X) \to \colim_{j \geq i} \Mor_\mathcal{C}(W, M_j)$
is bijective.
\end{enumerate}
...
\colim_{i \in \mathcal{I}^{opp}} \Mor_\mathcal{C}(M_i, W)$ is bijective.
\end{enumerate}
In cases (2), (3), and (4) the value of the essentially constant system is $X$.
\end{lemma}
