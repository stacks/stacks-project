...
\begin{lemma}
    \label{lemma-equalizer-mono}
    Let $\mathcal{C}$ be a category, and let $e : X \to Y$ be
    an equalizer of the morphisms  $f : Y \to Z$ and  $g : Y \to Z$ 
    of $\mathcal{C}$. Then
    $e$ is a monomorphism.
\end{lemma}

    \begin{proof}
        Given any morphisms $a,b : W \to X$ such that $e \circ a = e \circ b$,
        we have a commutative diagram
         $$
\xymatrix{ 
   X \ar[r]^e &  Y \ar@<1ex>[r]^f \ar@<-1ex>[r]_g & Z \\
   & W \ar[u]_{e \circ a = e \circ b}  \ar@{-->}[lu]
}
$$
where the dotted arrow exists by the universal property of equalizers. 
By the same property, the dotted arrow is unique, thus a = b.
     \end{proof}
