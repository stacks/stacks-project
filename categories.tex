...
The set
$$
\hat S = \{f \in \text{Arrows}(\mathcal{C}) \mid Q(f)
\text{ is an isomorphism}\}
$$
is the smallest saturated multiplicative system containing $S$.
In particular, if $S$ is saturated, then $\hat S = S$.
\end{lemma}

\begin{proof}
It is clear that $S \subset \hat S$ and that $\hat S$ satisfies MS1.
By the remarks following
Definition \ref{definition-saturated-multiplicative-system}
it follows that $\hat S$ satisfies MS4.
...
$fhw \in S$ for some $w$. We conclude that $f \in \hat S$ if and only
if there exist morphisms $g, h$ such that $gf, fh \in S$.
Provided we prove that $\hat S$ is a multiplicative system it is
clear that this implies that $\hat S$ is the smallest saturated
system containing $S$.

\medskip\noindent
To finish the proof of the lemma we have to show that
LMS2, RMS2, LMS3, RMS3 hold for $\hat S$.
