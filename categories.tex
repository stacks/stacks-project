...
\begin{lemma}
	\label{lemma-left-localization-iso-fun-cat}
	Let $\mathcal{C},\mathcal{D}$ be categories and let $S$ be a left multiplicative system in $\mathcal{C}$. Denote $\operatorname{Fun}_S(\mathcal{C},\mathcal{D})$ to the full subcategory of $\operatorname{Fun}(\mathcal{C},\mathcal{D})$ of functors $\mathcal{C}\to\mathcal{D}$ that send $S$ to isomorphisms in $\mathcal{D}$. The functor $\operatorname{Fun}(S^{-1}\mathcal{C},\mathcal{D})\to\operatorname{Fun}_S(\mathcal{C},\mathcal{D})$ induced by precomposition by $Q$ is an isomorphism of categories.
\end{lemma}

\begin{proof}
	This functor is a bijection on objects by Lemma \ref{lemma-properties-left-localization}, and it is faithful by Lemma \ref{lemma-precomposition-functor-is-faithful}. Only fullness is left to be proven. Let $F,G:S^{-1}\mathcal{C}\to\mathcal{D}$ be functors and let $\eta:FQ\to GQ$ be a natural transformation. This means that the diagram
	\[
	\xymatrix{
		FQ(X) \ar@{->}[r]^{\eta_X} \ar@{->}[d]_{FG(f)} & GQ(X) \ar@{->}[d]^{GQ(f)} \\
		FQ(Y) \ar@{->}[r]^{\eta_Y} & GQ(Y)
	}
	\]
	commutes for all morphisms $f:X\to Y$ in $\mathcal{C}$. Hence, it suffices to argue that the diagram
	\[
	\xymatrix{
		FQ(X) \ar@{->}[r]^{\eta_X} \ar@{->}[d]_{F(a)} & GQ(X) \ar@{->}[d]^{G(a)} \\
		FQ(Y) \ar@{->}[r]^{\eta_Y} & GQ(Y)
	}
	\]
	commutes for all morphisms $a:Q(X)\to Q(Y)$ in $S^{-1}\mathcal{C}$. But $a$ must be of the form $a=Q(s)^{-1}\circ Q(f)$, for some $(s:Y\to Y')\in S$ and some $f:X\to Y'$ in $\mathcal{C}$. On the other hand, the diagram
	\[
	\xymatrix{
		FQ(X) \ar@{->}[r]^{\eta_X} \ar@{->}[d]_{FG(f)} & GQ(X) \ar@{->}[d]^{GQ(f)} \\
		FQ(Y') \ar@{->}[r]^{\eta_{Y'}} & GQ(Y') \\
		FQ(Y) \ar@{->}[u]^{FQ(s)} \ar@{->}[r]^{\eta_Y} & GQ(Y) \ar@{->}[u]_{GQ(s)}
	}
	\]
	commutes.
\end{proof}
...
\begin{lemma}
	\label{lemma-right-localization-iso-fun-cat}
	Let $\mathcal{C},\mathcal{D}$ be categories and let $S$ be a right multiplicative system in $\mathcal{C}$. Denote $\operatorname{Fun}_S(\mathcal{C},\mathcal{D})$ to the full subcategory of $\operatorname{Fun}(\mathcal{C},\mathcal{D})$ of functors $\mathcal{C}\to\mathcal{D}$ that send $S$ to isomorphisms in $\mathcal{D}$. The functor $\operatorname{Fun}(S^{-1}\mathcal{C},\mathcal{D})\to\operatorname{Fun}_S(\mathcal{C},\mathcal{D})$ induced by precomposition by $Q$ is an isomorphism of categories.
\end{lemma}

\begin{proof}
	This is dual to \ref{lemma-left-localization-iso-fun-cat}.
\end{proof}
...
\begin{lemma}
	\label{lemma-iterated-category-localization}
	Let $\mathcal{C}$ be a category and let $S,T$ be left or right multiplicative systems of morphisms of $\mathcal{C}$ (i.e., both left, both right or one of each handedness), and suppose that $S\subset T$. Let $Q:\mathcal{C}\to S^{-1}\mathcal{C}$ be the localization functor. The set $Q(T)$ is a multiplicative system of the same handedness of $T$ in $S^{-1}\mathcal{C}$ and $Q(T)^{-1}(S^{-1}\mathcal{C})\cong T^{-1}\mathcal{C}$.
\end{lemma}

\begin{proof}
	If $T$ is a left multiplicative system,
	then the set $Q(T)$ satisfies LMS1 for
	$Q$ is bijective on objects and preserves identities.
	We see it satisfies LMS2.
	Given morphisms $Q(t):Q(X)\to Q(Z)$ and $s^{-1}g:Q(X)\to Q(Y)$,
	where $t\in T$ and $s:Y'\to Y$ is in $S$,
	there are dotted morphisms that make the following diagram commute:
	\[
	\xymatrix{
		Q(X) \ar@{->}[r]^{Q(g)} \ar@{->}[d]_{Q(t)} & Q(Y') \ar@{.>}[d]^{Q(t')} & Q(Y) \ar@{->}[l]_{Q(s)} \\
		Q(Z) \ar@{.>}[r]^{Q(f)} & Q(W) & 
	}
	\]
	where $t'\in T$. Hence, defining $t''=t'\circ s\in T$,
	we have that $Q(t'')\circ s^{-1}g=Q(t')\circ Q(g)=Q(f)\circ Q(t)$,
	which proves LMS2. Lastly, to see LMS3, suppose we have $a,b:Q(X)\to Q(Y)$
	and a morphism $t\in T$ with target $X$ such that
	$a\circ Q(t)=b\circ Q(t)$. By Lemma \ref{lemma-morphisms-left-localization}, we may assume $a=s^{-1}f$,
	$b=s^{-1}g$, for some $s:Y'\to Y$ in $S$. Hence, we have
	$Q(s)^{-1}\circ Q(f)\circ Q(t)
	=Q(s)^{-1}\circ Q(g)\circ Q(t)$, i.e.,
	$Q(f\circ t)= Q(g\circ t)$.
	By Lemma \ref{lemma-equality-morphisms-left-localization},
	this means that there is $s\in S$ with source $Y$ such that
	$s\circ f\circ t=s\circ f\circ t$.
	By LMS3 for $T$, we get a $t'\in T$ postcomposable with $s$ such that
	$t'\circ s\circ f=t'\circ s\circ g$.
	In particular, $t'\circ s\in T$,
	and after passing the last equality through $Q$, we are done.
	
	\medskip\noindent
	It is left to verify that $Q(T)^{-1}(S^{-1}\mathcal{C})$
	satisfies the universal property of $T^{-1}\mathcal{C}$.
	We are going to use Lemmas \ref{lemma-properties-left-localization} and \ref{lemma-properties-right-localization}.
	If $G:\mathcal{C}\to\mathcal{D}$ is a functor between categories
	such that $G(T)$ are all isomorphisms, in particular $G(S)$ are
	all isomorphisms and, hence, $G$ factors uniquely through $Q$ by
	a functor $H:S^{-1}\mathcal{C}\to\mathcal{D}$. Thus, $H$ sends
	$Q(T)$ to isomorphisms in $\mathcal{D}$. Therefore, $H$ factors
	uniquely through the localization functor
	$S^{-1}\mathcal{C}\to Q(T)^{-1}(S^{-1}\mathcal{C})$.
	
	In a diagram:
	\[
	\xymatrix{
		\mathcal{C}\ar[r]^G\ar[d]_Q&\mathcal{D}\\
		S^{-1}\mathcal{C}\ar@{.>}[ur]|H\ar[d]&\\
		Q(T)^{-1}(S^{-1}\mathcal{C})\ar@{.>}[uur]&
	}
	\]
\end{proof}
...
\begin{remark}
	Let $\mathcal{C}$ be a category.
	Let $S$ be a multiplicative system. Given objects $X,Y$ of $\mathcal{C}$,
	remember that in Remarks \ref{remark-left-localization-morphisms-colimit} and \ref{remark-right-localization-morphisms-colimit}
	it was proven that the categories $Y/S$, $S/X$ are respectively filtered and cofiltered.
	There is two sided version of equations \ref{equation-left-localization-morphisms-colimit} and \ref{equation-right-localization-morphisms-colimit} given by
	\begin{equation}
		\label{equation-localization-morphisms-iterated-colimit}
		\def\colim{\mathop{\mathrm{colim}}}
		\Mor_{S^{-1}\mathcal{C}}(X, Y)\cong
		\colim_{(t : Y \to Y') \in Y/S}
		\colim_{(s : X' \to X) \in (S/X)^{opp}}
		\Mor_\mathcal{C}(X', Y').
	\end{equation}
	Let's see how. Suppose $T$ is a multiplicative system in $\mathcal{C}$ containing $S$.
	In particular, $Q(T)$ is a multiplicative system in $S^{-1}\mathcal{C}$,
	Lemma \ref{lemma-iterated-category-localization}. Denoting
	$\hat{Q}:S^{-1}\mathcal{C}\to Q(T)^{-1}(S^{-1}\mathcal{C})$
	to the localization functor of $S^{-1}\mathcal{C}$
	with respect to $Q(T)$, we have
	\begin{equation*}
		\def\Mor{\mathop{\mathrm{Mor}}}
		\def\colim{\mathop{\mathrm{colim}}}
		\begin{aligned}
			\Mor_{Q(T)^{-1}(S^{-1}\mathcal{C})}(\hat{Q}(X),\hat{Q}(Y))
			&=\colim_{(Q(t) : Q(Y) \to Q(Y')) \in Q(Y)/Q(T)}
			\Mor_{S^{-1}\mathcal{C}}(Q(X),Q(Y)),
			&\text{by \eqref{equation-left-localization-morphisms-colimit},}\\
			&=\colim_{(Q(t) : Q(Y) \to Q(Y')) \in Q(Y)/Q(T)}
				\colim_{(s : X' \to X) \in (S/X)^{opp}}\Mor_{\mathcal{C}}(X',Y'),
				&\text{by \eqref{equation-right-localization-morphisms-colimit}.}
			\end{aligned}
	\end{equation*}
	Now, in the particular case $T=S$, the first term is isomorphic to
	$\Mor_{S^{-1}\mathcal{C}}(Q(X),Q(Y))$,
	by Lemma \ref{lemma-iterated-category-localization}.
	Hence, by Lemma \ref{lemma-cofinal}, it suffices to prove that
	the functor $Y/S\to Q(Y)/Q(S)$ induced by $Q$ is cofinal.
	This is an easy verification that one does using the invertibility of the morphisms $Q(S)$,
	so we leave it to the reader.
\end{remark}

Let $\mathcal{C}$ be a category and let $S$ be a multiplicative system in $\mathcal{C}$.
Equation \ref{equation-localization-morphisms-iterated-colimit}
expresses the two sided calculus of fractions.
Using Lemma \ref{lemma-colimits-commute} plus the description of a filtered colimit given before
Lemma \ref{lemma-directed-commutes},
we have that a morphism $Q(X)\to Q(Y)$ in
$S^{-1}\mathcal{C}$ is the same as a triple
$X\xleftarrow{s}X'\xrightarrow{f}Y'\xleftarrow{t}Y$ of morphisms in $\mathcal{C}$, with $s,t\in S$,
modulo the following equivalence relation:
two such triples $X\xleftarrow{s_i}X_i\xrightarrow{f}Y_i\xleftarrow{t_i}Y$,
for $i=1,2$, are equivalent if there is a third such triple
$X\xleftarrow{s_3}X_3\xrightarrow{f}Y_3\xleftarrow{t_3}Y$ and a
commutative diagram in $\mathcal{C}$:

\[
\xymatrix{
	& X_1 \ar@{->}[r]^{f_1} \ar@{->}[ld]_{s_1} & Y_1 \ar@{->}[d]_{v_1} &  \\
	X & X_3 \ar@{->}[u]_{u_1} \ar@{->}[r]^{f_3} \ar@{->}[l]|{s_2} \ar@{->}[d]^{u_2} & Y_3 & Y \ar@{->}[lu]_{t_1} \ar@{->}[l]|{t_2} \ar@{->}[ld]^{t_3} \\
	& X_2 \ar@{->}[r]^{f_2} \ar@{->}[lu]^{s_3} & Y_2 \ar@{->}[u]^{v_2} & 
}
\]

The equivalence class of
$X\xleftarrow{s}X'\xrightarrow{f}Y'\xleftarrow{t}Y$
is denoted as $t^{-1}fs^{-1}$. The isomorphism \eqref{equation-localization-morphisms-iterated-colimit} sends the left fraction equivalence class $s^{-1}f$
to $s^{-1}f\operatorname{id}_X^{-1}$. On the other hand,
the inverse map sends the double sided fraction $t^{-1}fs^{-1}$ to $(u\circ t)^{-1}g$,
where $u\in S$ and $g$ are morphisms of $\mathcal{C}$ that make the
following diagram commute:
\[
\xymatrix{
	X \ar@{->}[rd]_{g} & X' \ar@{->}[r]^{f} \ar@{->}[l]_{s} & Y' \ar@{->}[ld]^{u} & Y \ar@{->}[l]_{t} \\
	& X'' &  & 
}
\]
Such $u$ and $g$ exist thanks to LMS2.

The composition of $t^{-1}fs^{-1}$ with the two sided equivalence
class of $Y\xleftarrow{u}Y''\xrightarrow{g}Z'\xleftarrow{v}Z$
is defined in the following way:
Using RMS2, pick a commutative diagram
\[
\xymatrix{
	X & X' \ar@{->}[r]^{f} \ar@{->}[l]_{s} & Y' & Y \ar@{->}[l]_{t} & Y'' \ar@{->}[r]^{g} \ar@{->}[l]_{u} & Z' & Z \ar@{->}[l]_{v} \\
	&  &  & Y''' \ar@{->}[llu]^{w} \ar@{->}[ru]_{h} &  &  & 
}
\]
where $w\in S$, and define
$(v^{-1}gu^{-1})\circ (t^{-1}fs^{-1})
=v^{-1}(g\circ h)(w\circ s)^{-1}$.

This is equivalent as to use LMS2 to pick a commutative diagram
\[
\xymatrix{
	X & X' \ar@{->}[r]^{f} \ar@{->}[l]_{s} & Y' \ar@{->}[rd]_{k} & Y \ar@{->}[l]_{t} & Y'' \ar@{->}[r]^{g} \ar@{->}[l]_{u} & Z' \ar@{->}[lld]^{r} & Z \ar@{->}[l]_{v} \\
	&  &  & Z'' &  &  & 
}
\]
with $r\in S$, and define
$(v^{-1}gu^{-1})\circ (t^{-1}fs^{-1})
=(r\circ v)^{-1}(k\circ f)s^{-1}$. Details ommited.

Ultimately, the two sided calculus of fractions associated to some multiplicative system $S$
reduces to the right or left calculus of fractions associated to $S$.
...
\begin{lemma}
	\label{lemma-precomposition-functor-is-faithful}
	Let $F:\mathcal{A}\to\mathcal{B}$ be a functor between categories that is essentially surjective on objects, and let $\mathcal{C}$ be a category. The functor $\text{Fun}(\mathcal{B},\mathcal{C})\to\text{Fun}(\mathcal{A},\mathcal{C})$ induced by precomposition by $F$ is faithful.
\end{lemma}

\begin{proof}
	Suppose $t,s:G\to H$ are natural transformations in $\text{Fun}(\mathcal{B},\mathcal{C})$ such that $t_F=s_F$ and pick an object $y$ of $\mathcal{B}$. We have to show that $t_y=s_y$. There is an object $x$ of $\mathcal{A}$ and an isomorphism $f:F(x)\to y$ in $\mathcal{B}$. The diagram
	\[
	\xymatrix{
		GF(x) \ar@{->}[d]_{G(f)}^{\cong} \ar@{->}[r]^{t_{F(x)}=s_{F(x)}} & HF(x) \ar@{->}[d]^{H(f)}_{\cong} \\
		G(y) \ar@{->}[r]_{t_{y}\text{ or }s_{y}} & H(y)
	}
	\]
	commutes. Hence, $t_y=s_y$.
\end{proof}
