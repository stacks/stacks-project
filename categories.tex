...
\item the diagram (\ref{equation-pentagon}) commutes,
\item the arrows
$l_X \otimes \text{id}_Y, l_{X \otimes Y} :
\mathbf{1} \otimes X \otimes Y \to X \otimes Y$ agree,
\item the arrows
$\text{id}_X \otimes r_Y , r_{X \otimes Y} :
X \otimes Y \otimes \mathbf{1} \to X \otimes Y$ agree, and
\item the arrows $\text{id}_X \otimes l_Y, r_X \otimes \text{id}_Y :
X \otimes \mathbf{1} \otimes Y \to X \otimes Y$ agree.
\end{enumerate}
This means a monoidal category satisfies the assumptions of
\cite[Theorem 5.2]{associativity}.
\end{lemma}

\begin{proof}
We have seen (1) in the proof of Lemma \ref{lemma-monoidal-unit}.
Parts (2) and (5) are required in our definition of a monoidal category.
...
Part (3) follows immediately. Part (4) is proved in a similar manner.
\end{proof}
...
\medskip\noindent
Besides the convention on removing parentheses when writing iterates
of $\otimes$, from now on we identify $X \otimes \mathbf{1}$ and
