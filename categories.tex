...
\xymatrix{
H(x) \ar[rrrd] &
H(x_1) \ar[l] \ar[r] \ar[rrd] &
H(x_2) \ar[rd] &
\ldots \ar[l] \ar[r] &
H(x_{2n - 2}) \ar[ld] &
H(x_{2n - 1}) \ar[lld] \ar[l] \ar[r] & H(x') \ar[llld] \\
& & & y & & &
}
$$
where all the horizontal morphisms come from morphisms in $\mathcal{I}$,
and the vertical arrows in $\mathcal{J}$.
\end{enumerate}
\end{definition}
...
Then for every diagram $M : \mathcal{J} \to \mathcal{C}$
the limit $\lim_\mathcal{J} M$ exists if and only
...
morphisms in $\mathcal{J}$ go from an $y$ to a $z$ there are no
...
commute with nonempty products.
\end{lemma}
...
$\colim M_i \times \colim N_i \to \colim M_i$ as follows.
...
$\text{id}_\mathcal{C} \to v \circ u$ and
$u \circ v \to \text{id}_\mathcal{D}$. Moreover, if $\alpha : u(X) \to Y$
...
$$
(a, c) : (V, h^*x, y', h^*f \circ b') \longrightarrow (U, x, y, f)
$$
...
and let $b_1 = c \circ c_1$ for some morphism $c_1 : y_1 \to y'$.
Then
$$
(a_1', c_1) : (W, x_1, y_1, f_1) \longrightarrow (V, h^*x, y', h^*f \circ b')
$$
(where $a_1' : x_1 \to h^*x$ is the unique morphism lying over the
given morphism $p(a_1) = q(b_1) : W \to V$ such that $a_1 = a \circ a_1'$)
...
Since $\xi$ is surjective and since $f'$ maps to $x$ we see that
...
cardinality at most $\max(\aleph_0, |G|)$. The lemma tells us
...
colimits once it is know that limits exist. We illustrate it using
...
$f_i \in \Mor_\mathcal{C}(y, G(x_i))$ such that for any
