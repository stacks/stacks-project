...
\begin{proof}
    To construct $j : \mathcal{B} \to \mathcal{A}$, there are two steps. Firstly, we define the map $j : \Ob(\mathcal{B}) \to \Ob(\mathcal{A})$ that associates $j(X)$ to $X \in \mathcal{B}$. Secondly, if $X,Y \in \Ob(\mathcal{B})$ and $\phi : X \to Y$, we consider $\phi' := i_Y \circ\phi\circ i_X^{-1}$. There is an unique $\varphi$ verifying $F(\varphi) = \phi'$, using that $F$ is fully faithful. Now, if we define $j(\phi) = \varphi$, the diagram
        $$\xymatrix{
        X \ar[r]^{i_X} \ar[d]_{\phi}
        &
        ...
        $$
    commutes. Thus, as each component $i_X$ is an isomorphism, $\{i_X\}_X$ is an isomorphism of functor $\text{id}_\mathcal{B} \to F\circ j$. To conclude, we have to also prove that $j \circ F$ is isomorphic to $\text{id}_\mathcal{A}$. If $Y \in \Ob(\mathcal A)$, by full faithfulness of $F$ , there is a unique isomorphism $k_Y : Y \to j(F(Y))$ such that $F(k_Y) = i_{F(Y)}$. Let $Y,Z \in \Ob(\mathcal A)$ and $\phi : Y \to Z$. We know that  
    $$ F(k_Z \circ \phi) = F(k_Z)\circ F(\phi) = i_{F(Z)}\circ F(\phi) $$
    and
    $$ F(j(F(\phi))\circ k_Y) = F(j(F(\phi))\circ F(k_Y) = F(j(F(\phi))\circ i_{F(Y)} $$ by construction. However, we proved that $ \text{id}_\mathcal{B} \cong F\circ j $, which gives the relation $ i_{F(Z)}\circ F(\phi) = F(j(F(\phi))\circ i_{F(Y)} $. Therefore, 
    $$ F(k_Z \circ \phi) = F(j(F(\phi))\circ k_Y) $$
    and then
    $$ k_Z \circ \phi = j(F(\phi))\circ k_Y $$ by faithfulness of $F$. It proves that $k : \text{id}_\mathcal{A} \to j\circ F$ is an isomorphism of functor, which concludes the proof.
\end{proof}
...
\begin{proof}
    Let $t \in \Ob(\mathcal C)$ and $ \varphi \in \Mor_{\mathcal C}(t,z) $. As $g$ is representable, it gives two morphisms $p$ and $q$ such that 
        $$\xymatrix{
        y \times_z t \ar[r]^{q} \ar[d]_p
        &
        t \ar[d]^{\varphi}
        \\
        y \ar[r]^{g}
        ...
        $$
    commutes. Again, as $f$ is also representable, we have $p'$ and $q'$ such that
        $$\xymatrix{
        x \times_y (y\! \times_z\! t) \ar[r]^{q'} \ar[d]_{p'}
        &
        y \times_z t \ar[d]^{p}
        \\
        x \ar[r]^{f}
        ...
        $$
    commutes. Therefore it is the same for the diagram
        $$\xymatrix@C4.5pc{
        x \times_y (y\! \times_z\! t) \ar[r]^{q \circ q'} \ar[d]_{p'}
        &
        t \ar[d]^{\varphi}
        \\
        x \ar[r]^{f\circ g}
        ...
        $$
    with $ q \circ q' \in \Mor_\mathcal{C}(x \times_y (y\! \times_z\! t),t) $.
    Now let $w\in \Ob(\mathcal C)$ and $x \times_z t$ be $ x \times_y (y \times_z t) $. By definition of the fibre product, there are unique morphisms $\delta$ and $\gamma$ such that 
        $$
        \xymatrix{
    ...
    commute. Then, $\gamma$ makes the diagram
        ...
        &
        &
        x \times_z t \ar[d]_{p'} \ar[r]_{q\circ q'}
        &
        t \ar[d]^{\varphi}
        \\
        &
...
\begin{proof}
    Let suppose that $f$ is a monomorphism. Let $W$ be an object of $\mathcal C$ and $\alpha, \beta \in \Mor_\mathcal C(W,X)$ such that $f\circ \alpha = f\circ \beta$. Therefore $\alpha = \beta$ as $f$ is monic. In addition, we have the commutative diagram 
        $$\xymatrix{
        X \ar[r]^{\text{id}_X} \ar[d]_{\text{id}_X}
        &
        X \ar[d]^{f}
        \\
        X \ar[r]^{f}
    ...
    Now let suppose that $ X\times_Y X \cong X $. The diagram
        $$\xymatrix{
        X \ar[r]^{\text{id}_X} \ar[d]_{\text{id}_X}
        &
        X \ar[d]^{f}
        ...
        $$
    commutes and if $W\in\Ob(\mathcal C)$ and $\alpha,\beta : X\to Y$ such that $ f\circ\alpha = f\circ\beta $, we have a unique $\gamma$ verifying
