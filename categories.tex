...
\noindent
Coproducts of pairs, push outs, and coequalizers are examples of colimits.
...
$$
Note that on the right we have the limit over the opposite category.
\end{enumerate}
...
(Finitely many objects and finitely many morphisms.) A (co)limit is called
{\it nonempty} if the index category is nonempty. A (co)limit is called
{\it connected} if the index category is connected, see
...
We say the (inverse or not) system
$(M_i, f_{ii'})$ is a {\it directed system} if the partially
...
just like with ordinary fibre products. Here are two futher lemmas
...
\medskip\noindent
Let $p : \mathcal{S} \to \mathcal{C}$ be a category over $\mathcal{C}$.
Given an object $x \in \mathcal{C}$ with $p(x) = U$, and given a morphism
...
for every $z \in \text{Ob}(\mathcal{C})$ the map
$$
...
stronly cartesian morphism $\varphi : y \to x$ with $p(\varphi) = f$,
...
$$
only depends on the data $(x, U = p(x), f : V \to U)$. Hence we
...
\noindent
In particular, given a fibred category $p : \mathcal{S} \to \mathcal{C}$
over a category $\mathcal{C}$ we have the diagonal morphism
$$
\Delta = \Delta_{\mathcal{S}/\mathcal{C}} :
\mathcal{S} \longrightarrow \mathcal{S} \times_\mathcal{C} \mathcal{S}
$$
in the $(2, 1)$-category of fibred categories over $\mathcal{C}$.

\begin{lemma}
\label{lemma-intertia-fibred-category}
Let $\mathcal{C}$ be a category, and let $p : \mathcal{S} \to \mathcal{C}$
be a fibred category over $\mathcal{C}$. Consider the category
$I_\mathcal{S}$ over $\mathcal{C}$ whose
\begin{enumerate}
\item objects are pairs $(x,\alpha)$,
where $x$ is an object of $\mathcal{S}$ and $\alpha$ is an automorphism of $x$
with $p(\alpha) = \text{id}_{p(x)}$, and
\item morphisms $(x, \alpha) \to (y, \beta)$
are given by morphisms $\phi : x \to y$ in $\mathcal{S}$ 
...
$$
\item the functor $I_\mathcal{S} \to \mathcal{C}$ is the functor
$(x, \alpha) \mapsto p(x)$, $\phi \mapsto p(\phi)$.
\end{enumerate}
Then
\begin{enumerate}
...
\item $I_\mathcal{S}$ is a fibred category over $\mathcal{C}$.
\end{enumerate}
\end{lemma}
...
Consider the functor
$I_\mathcal{S} \to \mathcal{S}
\times_{\Delta, \mathcal{S} \times_\mathcal{C} \mathcal{S}, \Delta}
\mathcal{S}$
which to an object $(x, \alpha)$ assigns the object
$(x, x, (\alpha, \text{id}_x))$ of
$\mathcal{S}
\times_{\Delta, \mathcal{S} \times_\mathcal{C} \mathcal{S}, \Delta}
\mathcal{S}$, and to a morphism $\phi$ assigns the morphism $(\phi, \phi)$.
We claim that a quasi-inverse to that morphism is given by the
morphism of fibred categories
$\mathcal{S}
\times_{\Delta, \mathcal{S} \times_\mathcal{C} \mathcal{S}, \Delta}
\mathcal{S} \to I_\mathcal{S}$
which to an object $(x, y, (\iota, \kappa))$ assigns the object
$(x, \kappa^{-1} \circ \iota)$ of $I_\mathcal{S}$, and to a given morphism
$(\phi, \phi') : (x, y, (\iota, \kappa)) \to (z, w, (\lambda, \mu))$
assigns the morphism $\phi$.  Indeed, the endomorphism of $I_\mathcal{S}$
induced
by composing the two morphisms above is the identity on the nose, and
the endomorphism induced on
$\mathcal{S}
\times_{\Delta, \mathcal{S} \times_\mathcal{C} \mathcal{S}, \Delta}
\mathcal{S}$ is isomorphic to
the identity via the natural isomorphism
$(\iota^{-1} \circ \kappa, \kappa \circ \iota^{-1} \circ \kappa) :
(x, x, (\kappa^{-1} \circ \iota)) \buildrel{\sim} \over \longrightarrow
(x, y, (\iota, \kappa))$. Some details omitted.
\end{proof}

\begin{definition}
\label{definition-inertia-fibred-category}
Let $\mathcal{C}$ be a category, and let $p : \mathcal{S} \to \mathcal{C}$
be a fibred category over $\mathcal{C}$.
By the {\it inertia fibred category $I_\mathcal{S}$ of $\mathcal{S}$}
we mean the fibred category $I_\mathcal{S} \to \mathcal{C}$ constructed in
Lemma \ref{lemma-intertia-fibred-category}.
\end{definition}

\noindent
Note that there is a canonical $1$-morphism
\begin{equation}
\label{equation-inertia-to-initial}
\mathcal{I}_\mathcal{S} \longrightarrow \mathcal{S}
\end{equation}
of fibred categories over $\mathcal{C}$. In terms of the description of
Lemma \ref{lemma-intertia-fibred-category}
it simply maps the pair $(x, \alpha)$ to $x$ and the morphism
$\phi : (x, \alpha) \to (y, \beta)$ to $\phi : x \to y$.
...
From this we can construct a fibred category over $\mathcal{C}$
as follows. Define
$$
\text{Ob}(\mathcal{S}) =
...
what the functor $p : \mathcal{S} \to \mathcal{C}$ is.
...
fibred category constructed in Example \ref{example-functor-categories}.
A {\it split fibred category} is a fibred category isomorphic (!)
over $\mathcal{C}$ to one of these categories.
\end{definition}
...
This is almost exactly the same as Example \ref{example-functor-categories},
except that this time
we consider presheaves of groupoids instead of presheaves of categories,
and we construct a category fibred in groupoids instead of a fibred category.
Suppose that $F : \mathcal{C} \to \text{Groupoids}$ is a contravariant functor
to the category of groupoids, see Remark \ref{remark-functor-into-sets} and
Definition \ref{definition-functor-into-2-category}.
For $f : V \to U$ in $\mathcal{C}$ we will
suggestively write $F(f) = f^\ast$ for the functor from $F(U)$ to $F(V)$.
...
\noindent
If $F$ is as in the example above, then 
we will write $p_F : \mathcal{S}_F \to \mathcal{C}$
for the resulting functor. Moreover, in accordance with
Definition \ref{definition-split-fibred-category}
we say $\mathcal{S}_F$ is a {\it split} category fibred in groupoids.
...
\noindent
If $C$ is a set with an
equivalence relation $\sim$, then we can make a setoid
...
with sets. Discrete categories are setoids.
For any setoid $\mathcal{C}$ there is a canonical procedure to make
a discrete category equivalent to it, namely one replaces
$\text{Ob}(\mathcal{C})$ by the set of isomorphism
...
Let $\mathcal{C}$ be a category.
\begin{enumerate}
\item A {\it category fibred in setoids}
is a category fibred in groupoids all of whose fibre categories are
setoids.
\item A {\it category fibred in sets}, or a {\it category fibred
...
(resp.\ {\it $2$-category of categories fibred in sets over $\mathcal{C}$})
...
\begin{enumerate}
\item Its objects will be categories
$p : \mathcal{S} \to \mathcal{C}$ fibred in setoids (resp.\ sets).
\item Its $1$-morphisms $(\mathcal{S}, p) \to (\mathcal{S}', p')$
will be functors $G : \mathcal{S} \to \mathcal{S}'$ such that
$p' \circ G = p$ (since every morphism is strongly cartesian
$G$ automatically preserves them).
...
\medskip\noindent
Note that every $2$-morphism is automatically an isomorphism.
Hence these $2$-categories are actualy $(2, 1)$-categories.
...
$F \to \mathcal{S}_F$ discussed in Examples
\ref{example-functor-categories} and \ref{example-functor-groupoids}.
...
$F : \mathcal{C}^{\text{opp}} \to \textit{Sets}$
...
Consider the representable presheaf $F = h_X = \text{Mor}(-, X)$
...
under the correpondence of Lemma \ref{lemma-2-category-fibred-sets} we have
$$
...
in the $2$-category of groupoids over $\mathcal{C}$ with $\mathcal{S}'$
fibred in sets, then
\begin{enumerate}
\item $\mathcal{S}$ is fibred in setoids over $\mathcal{C}$, and
\item $\text{Ob}(\mathcal{S}_U) \to \text{Ob}(\mathcal{S}'_U)$
...
in groupoids which are equivalent to catgories fibred in sets.
...
\noindent
We spell out what this entails.
...
\begin{proof}
The first assertion follows immediately from Lemma \ref{lemma-setlike-fibres}.
...
see (\ref{equation-inertia-to-initial}), is an equivalence (of categories
over $\mathcal{C}$).
\end{enumerate}
\end{lemma}
...
sets if one starts out with such.
A similar result holds for the $2$-category of categories fibred in setoids.
\end{lemma}
...
\item on morphisms: the arrow $(g : V'/U \to U/V)$ maps to
the composition
$$
