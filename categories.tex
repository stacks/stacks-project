...
Consider the representable presheaf $F = h_X = \text{Mor}_{\mathcal{C}}(-, X)$
...
$$
We also point out that the category $\mathcal{C}/X$ is equivalent
to the category $\mathcal{S}_{h_X} = \mathcal{S}_F$ associated
to $F$ in
Example \ref{example-functor-groupoids}
if we think of $F$ as a functor from $\mathcal{C}$ to the category
...
rule $F_i(U) = \text{Ob}(\mathcal{S}_i)/\cong$, we have
$$
