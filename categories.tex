...
\begin{enumerate}
\item the $2$-morphisms $\alpha_x$ and $\alpha_{f, g}$ are all
isomorphisms,
...
$$
\end{enumerate}
Note that if $\mathcal{C}$ is actually a $(2,1)$-category,the
...
Moreover $F(x)$ is a lift of $U$ if $x$ is a lift of $U$.
...
$$
given by $\psi \longmapsto (\varphi \circ \psi, p(\varphi))$
is bijective.
\end{definition}
...
Assume $p : \mathcal{S} \to \mathcal{C}$ is a fibred category.
Choose strongly cartesian morphisms
$f^\ast x \to x$ lying over $f$ for all $f: V \to U = p(x)$.
For any morphism $f : V \to U$ of $\mathcal{C}$ the
functor $f^* : \mathcal{S}_U \to \mathcal{S}_V$ described
...
$g : U \to W$ there is a unique isomorphism
$$
\alpha_{f, g} :
(g \circ f)^\ast
\longrightarrow
f^\ast \circ g^\ast
$$
as functors $\mathcal{S}_W \to \mathcal{S}_V$
...
$(\alpha_{f, g})_y$ in the fibre category
$\mathcal{S}_W$. It is an isomorphism since both
the morphism $(f \circ g)^*y \to y$ and the morphism
and the composition $f^*g^*y \to g^*y \to Y$ are strongly
cartesian morphisms lifting $g \circ f$ (see discussion
...
We omit the verification that $\alpha_{f, g}$ and
...
\begin{proof}
Basically what one has to show here is that if given
$f : \mathcal{X} \to \mathcal{S}$ and
$g : \mathcal{Y} \to \mathcal{S}$ are morphisms of fibred
...
\noindent
Here is the obligatory lemma on $2$-fibre products.

\begin{lemma}
\label{lemma-2-product-fibred-categories}
Let $\mathcal{C}$ be a category.
The $(2, 1)$-category of categories fibred in groupoids
...
$\mathcal{C}$.  Then $G$ is faithful
(resp.\ fully faithful, resp.\ an equivalence)
if and only
if for each $U\in\text{Ob}(\mathcal{C})$ the induced functor
$G_U\colon \mathcal{S}_U\to \mathcal{S}'_U$ is faithful
(resp.\ fully faithful, resp.\  an equivalence).
\end{lemma}

\begin{proof}
Clearly if $G$ is faithful (resp.\ fully faithful, resp.\ an equivalence)
then so is $G_U$.
...
wuith an equivalence relation this corresponds to taking the quotient
...
A similar result holds for categories fibred in setoids.
\end{lemma}
...
\begin{proof}
Since $\mathcal{S}$ is fibred in groupoids we can choose
for every morphism $f : V \to U$ of $\mathcal{C}$ and every object $x$
of $\mathcal{S}$ a morphism $f^*x \to x$ lying over $f$.
...
Think of $\mathcal{C}/U$ as a category over $\mathcal{C}$.
The functor
$$
...
$$
be the set of base preserving natural transformations.
If $f : U\to V$ the induced functor $\mathcal{C}/U \to \mathcal{C}/V$
induces the
morphism $F(f) : F(V)\to F(U)$. Clearly $F$ is a functor, and
