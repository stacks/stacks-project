...
Let $\mathcal{I}$ be an index category satisfying the
hypotheses of Lemma \ref{lemma-split-into-directed} above.
Let $M, N : \mathcal{I} \to \textit{Sets}$ be two
diagrams. Suppose that $a, b : M \to N$ are morphisms
of diagrams. In this case
$$
\text{colim}_i\ 
\text{Equalizer}(\xymatrix{
M_i
\ar@<1ex>[r]^{a_i}
\ar@<-1ex>[r]_{b_i} &
N_i
...
$$
In words: equalizers commute with colimits over $\mathcal{I}$.
\end{lemma}

\begin{proof}
Follows from Lemmas \ref{lemma-split-into-directed} and
\ref{lemma-directed-commutes} and the fact that equalizers commute with
products in the category of sets (including empty products!).
\end{proof}
