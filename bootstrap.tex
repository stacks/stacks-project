...
WARNING: The proof of this lemma is currently wrong because it relies
on the erroneous
Groupoids, Lemma \ref{groupoids-lemma-slice}.
\end{lemma}
...
Groupoids, Lemma \ref{groupoids-lemma-make-CM}
...
\medskip\noindent
Notation: In the following given a morphism $g : U' \to U$ we denote
$R'$ the restriction of $R$ to $U'$. We denote $h$ the composition
$$
h = s \circ \text{pr}_1 : U' \times_{g, U, t} R \longrightarrow U.
$$
For a point $u \in U$ we denote $F_u = s^{-1}(u)$ (scheme theoretic fibre).
Similarly for $R'$ we denote $F'_{u'} = (s')^{-1}(u')$. Note that if
$g(u') = u$, then
$$
F'_{u'} = h^{-1}(u) \times_{\text{Spec}(\kappa(u))} \text{Spec}(\kappa(u')).
$$
We denote $(u, u) \in R$ the point $e(u)$; as $R \to U \times_S U$
is a monomorphism, this is the unique point lying over
$\Delta_{U/S}(u)$.

\medskip\noindent
Step 2. Assume $s, t$ are Cohen-Macaulay and locally of finite presentation.
Let $u \in U$ be a point. We claim that there exists a $g : U' \to U$
such that
\begin{enumerate}
\item $g$ is an immersion,
\item $u \in U'$,
\item $g$ is locally of finite presentation,
\item $h$ is Cohen-Macaulay and locally of finite presentation,
\item the morphisms $s', t' : R' \to U'$ are Cohen-Macaulay and locally
of finite presentation, and
\item $\dim_{(u, u)}(F'_u) = 0$.
\end{enumerate}
As $s$ is locally of finite presentation the schemes $F_u$ and $F'_u$ are
locally of finite type, hence we may argue by induction on
$\dim_{(u, u)}(F_u)$. If $\dim_{(u, u)}(F_u) = 0$ there is nothing to prove.
Assume $\dim_{(u, u)}(F_u) > 0$.
Since $j$ is an equivalence relation the stabilizer
group scheme $G$ of $(U, R, s, t, c)$ is the trivial group scheme.
Hence the assumptions of
Groupoids, Lemma \ref{groupoids-lemma-slice}
are satisfied, and we may find a morphism $g : U' \to U$ which has
properties (1), (2), (3), $\dim_{(u, u)}(F'_u) < \dim_{(u, u)}(F_u)$
and instead of (4) and (5) we have that the composition
$$
h = s \circ \text{pr}_1 : U' \times_{g, U, t} R \longrightarrow U
$$
is Cohen-Macaulay at the point $(u, (u, u))$. Now we apply
Groupoids, Lemma \ref{groupoids-lemma-make-CM-generalize}
...
Since $(u, u) \in U'' \times_{g, U, t} R$ we see that $u \in U''$.
Hence we may replace $U'$ by $U''$ and assume that in fact $h$ is
Cohen-Macaulay everywhere! Consider the resulting
groupoid scheme $(U', R', s', t', c')$ and the diagram
$$
\xymatrix{
...
The diagram shows $s'$ is a base change of $h$, and hence we
conclude that $s'$ is locally of finite presentation
and Cohen-Macaulay as desired.

\medskip\noindent
Step 3. For each $u \in U$ we can find a $g : U' \to U$ such that
\begin{enumerate}
\item $g$ is an immersion,
\item $u \in U'$,
\item $g$ is locally of finite presentation,
...
\end{enumerate}
Namely, take $g : U' \to U$ as in Step 2.
Since $h^{-1}(u) = F'_u$ we see that $h$ has relative dimension
$\leq 0$ at $(u, (u, u))$. Hence, by
Groupoids, Lemma \ref{groupoids-lemma-make-CM-generalize},
...
presentation and locally quasi-finite as a base change of $h$.

\medskip\noindent
Step 4. For each point $u \in U$ choose a $g_u : U'_u \to U$ as in
Step 3 and denote $R'_u$ the restriction of $R$ to $U'_u$.
Moreover, in accordance with the above
...
and that the cardinality of the index set is the cardinality of
...
$h : U' \times_{g, U, t} R \to U$ is equal to $\coprod h_{u}$ hence surjective.
Since each $h_u$ is flat and locally of finite presentation we conclude that
$h$ is flat and locally of finite presentation. This implies that
