...
this corresponds to a septuple $(\text{Ob}, \text{Arrows}, s, t, c)$.
...
$(\text{Ob}/P, P\backslash \text{Arrows}/P, \overline{s},
\overline{t}, \overline{c})$
as follows:
\begin{enumerate}
\item $\text{Ob}/P$ is the set of $P$-isomorphism classes,
\item $P\backslash \text{Arrows}/P$ is the set of arrows in
$\mathcal{C}$ up to pre-composing and post-composing by arrows of $P$,
...
of the groupoid
$(\text{Ob}/P, P\backslash \text{Arrows}/P, \overline{s},
\overline{t}, \overline{c})$ via the map of sets
$\text{Ob} \to \text{Ob}/P$. We omit the details.

\begin{lemma}
\label{lemma-divide-subgroupoid}
Let $S$ be a scheme.
Let $(U, R, s, t, c)$ be a groupoid scheme over $S$.
Let $P \subset R$ be an open subscheme.
Assume that
\begin{enumerate}
\item $(U, P, s|_P, t|_P, c|_{P \times_{s, U, t}P})$ is a groupoid scheme,
\item $j : R \to U$ is separated and locally quasi-finite,
\item $U$ is affine, $s|_P, t|_P : P \to U$ are finite locally free, and
$j|_P : P \to U \times_S U$ is a monomorphism.
\end{enumerate}
Then $U/P$ is representable by an affine scheme $M$ and
$P = U \times_M U$ and $R$ is the restriction of a groupoid scheme
$(M, \overline{R}, \overline{s}, \overline{t}, \overline{c})$ on
$M$ via the morphism $U \to M$.
\end{lemma}

\begin{proof}
By
Groupoids, Proposition \ref{groupoids-proposition-finite-flat-equivalence}
the affine scheme $M$ representing $U/P$ exists, and $P = U \times_M U$.
The identification $P = U \times_M U$ is such that $t|_P = \text{pr}_0$
and $s|_P = \text{pr}_1$.
To get $\overline{R}$ we are going to descend the scheme $R$ via the finite locally
free morphism $U \times_S U \to M \times_S M$. Namely, note that
$$
(U \times_S U) \times_{(M \times_S M)} (U \times_S U)
=
P \times_S P
$$
by the above. Thus a descent datum for $R$ relative to
$U \times_S U / M \times_S M$ consists of an isomorphism
$$
\varphi :
...
$$
where the composition is taken in the category $(U(T), R(T), s, t, c)$.
...
To check the cocycle condition (see
Descent, Definition \ref{descent-definition-descent-datum})
we have to verify that $\varphi_{02} = \varphi_{12} \circ \varphi_{01}$
as maps over
$$
(U \times_S U)
\times_{(M \times_S M)} (U \times_S U)
\times_{(M \times_S M)} (U \times_S U) =
...
((p_1, p_1'), (p_2, p_2'),
p_1^{-1} \circ r \circ p_1') \\
\varphi_{12} & (r, (p_1, p_1'), (p_2, p_2')) & \mapsto &
((p_1, p_1'), (p_2, p_2'),
p_2^{-1} \circ r \circ p_2')
\end{matrix}
$$
which implies what we want. This means that we may apply
More on Morphisms, Lemma
\ref{more-morphisms-lemma-separated-locally-quasi-finite-morphisms-fppf-descend}
to get a scheme $\overline{R} \to M \times_S M$ and an isomorphism
$$
R \to \overline{R} \times_{(M \times_S M)} (U \times_S U)
$$
which recovers the descent data in the usual way.

\medskip\noindent
Since $U \times_S U \to M \times_S M$ is finite locally free we
conclude that $R \to \overline{R}$ is finite locally free.
Hence $R \to \overline{R}$ is surjective as a map of sheaves.
This and our choice of $\varphi$ implies that the sheaf $\overline{R}$
represents the sheafification of the presheaf sheaf
$$
T \longmapsto R(T)/\sim_P
$$
where $r \sim_P r' \Leftrightarrow r' = p^{-1} \circ r \circ p'$
for some $p, p' \in P(T)$. Hence we can define the groupoid
structure on $(M = U/P, \overline{R} = P\backslash R/P)$ exactly as in
the discussion of the ``plain'' groupoid case discussed above the statement
of the lemma, and that $(U, R, s, t, c)$ is the pullback of
this groupoid structure via the morphism $U \to M$.
...
$U \to F$ which is which is representable,
surjective, flat and locally of finite presentation, or
...
is an algebraic space by assumption. Moreover, by assumption the
2 projections $s, t : R \to U$ are surjective, flat and locally of
...
Namely, let $(U, R, s, t, c)$ is a groupoid in algebraic spaces
over $S$ such that $s, t$ are flat and locally of finite presentation, and
$j = (t, s) : R \to U \times_S U$ is an equivalence relation.
...
Lemma \ref{})
this reduces us to the case where $U$ is a scheme. But if $U$ is a scheme,
then the morphism $j : R \to U \times_S U$ (being a monomorphism and
locally of finite type) is locally quasi-finite and separated (see
Morphisms of Spaces, Lemma \ref{})
whence $R$ is a scheme by
Morphisms of Spaces, Lemma \ref{}.
...
we may reduce to that case where
$s, t$ are flat, locally of finite presentation and locally quasi-finite.
Let $U = \bigcup U)$ be an affine open covering.
Let $(U_i, R_i = R|_{U_i}, s_i, t_i, c_i)$ be the restriction of $R$
to $U_i$. It is clear that $s_i, t_i$ are still flat, locally of finite
presentation, and locally quasi-finite (as $R_i$ is an open subscheme of $R$
...
Hence if we can show that $U_i/R_i$ is an algebraic space, then it will
follow that $U/R$ is an algebraic space, by
Spaces, Lemma \ref{spaces-lemma-glueing-algebraic-spaces}.
Thus we reduce to the case where $U$ is affine.

\medskip\noindent
Assume $(U, R, s, t, c)$ is a groupoid scheme over $S$,
with $U$ affine, such that $s, t$ are flat, locally of finite presentation,
and locally quasi-finite and $j = (t, s) : R \to U \times_S U$ is an
equivalence relation. Choose $u \in U$.
We apply
More on Groupoids in Spaces, Lemma \ref{}
to $u \in U, R, s, t, c$. We obtain an affine scheme $U'$, an etale
morphism $U' \to U$, a point $u' \in U'$ with $\kappa(u) = \kappa(u')$
such that the restriction $R' = R|_{U'}$ is quasi-split over $u'$.
Again note that $U/R = U'/R'$ by the references mentioned above.
If we can find an open neighbourhood $W \subset U'$ of $u'$ such that
$W/R'|_W$ is an algebraic space, then it follows by the argument of
the preceding paragraph that $U/R$ is an algebraic space.
Hence we may assume that $R$ is quasi-split over $u$ and the
goal is to find an open neighbourhood $W$ such that $W/R|_W$ is
an algebraic space.
...
$(U', R', s', t', c')$ by a finite locally free morphism
$U \to U'$ such that $P = U \times_{U'} U$.
By descent theory (insert references) this implies that
$s', t'$ are flat, locally
of finite presentation, and locally quasi-finite.
Since $R \to R'$ is finite locally free and since
$P$ is open in $R$ and maps to $e'(U') \subset R'$ we see
that $e'(U') \subset R'$ is open in $R'$! Hence we see that
$s', t' : R' \to U'$ are etale! End of story.
