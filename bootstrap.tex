...
\noindent
In fact, it suffices to have a smooth cover by a scheme as the lemma below
shows. One can further improve on this by ``bootstrapping the diagonal''
similar to what was done in Section \ref{section-bootstrap-diagonal}. If
we ever need this we will add this here.
...
\begin{proof}
The proof mirrors the proof of Lemma \ref{lemma-spaces-etale}.
Let $R = U \times_F U$. By (2) the presheaf $R$ is representable
by a scheme and by (3) the projections $R \to U$ are smooth. Denote
$(U, R, s, t, c)$ the groupoid associated to the equivalence relation
$j : R \to U \times_S U$ (see
...
$U$ by $R$ for the smooth topology (follows exactly as in the proof of
Algebraic Spaces, Lemma \ref{spaces-lemma-space-presentation}).
Thus we have morphisms (tranformations of functors)
$$
U \to F \to X.
...
$V = T \times_X U$ is a scheme surjective and smooth over $T$.
In other words, $\{V \to U\}$ is a smooth covering. This proves that
