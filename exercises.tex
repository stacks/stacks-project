...
Let $\mathcal{A}$ be an abelian category.
We denote $\text{Fil}^f(\mathcal{A})$ the full subcategory
...
then $\alpha$ is a quasi-isomorphism.
\end{exercise}
...
the structural morphism $f : X = \mathbf{A}^1_{k} \to \text{Spec}(k) = S$.
\begin{enumerate}
...
\varphi : R \to S
$$
and an integer $e \geq 1$ such that $\varphi(R_d) \subset S_{de}$
...
$S_e$, $S_{2e}$, etc in degree $1$, $2$, etc) and $\varphi$
...
$\text{Pic}(\mathbf{P}^3_k) = \mathbf{Z}$ to show that $\text{Pic}(X) = 0$.)
\end{exercise}
...
$$
such that for any affine opens $\text{Spec}(A) \subset X$,
$\text{Spec}(R) \subset S$
