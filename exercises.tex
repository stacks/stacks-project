...
$$
\Omega_{A/R} = \bigoplus\nolimits_{a\in A} A \cdot \text{d}a \Big/
\big\langle \text{d}(a_1a_2)-a_1\text{d}a_2-a_2\text{d}a_1, \text{d}r\big\rangle.
$$
...
Prove that the image of $\text{Spec}(B) \to \text{Spec}(A)$
contains a nonempty open subset of $\text{Spec}(A)$ using the following steps:
\begin{enumerate}
